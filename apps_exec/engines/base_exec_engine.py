"""
Base Executive Brief Engine — Foundation for all apps_exec engines.

Mirrors apps_rg BaseRGEngine pattern with exec-specific contracts.
"""
...
try:
    ...
    logging.getLogger(__name__).debug("Pydantic not available")
...
class BaseExecEngine(SemanticCacheMixin, EmbeddingMixin, ABC):
    ...
    def __init__(self, config: Any = None, **kwargs: Any) -> None:
        ...
        try:
            ...
            self.specs = load_exec_specs()
        except ImportError:
            self.specs = None
            self.logger.warning("[%s] exec specs not available", self.name)
        ...
        try:
            from apps_exec.config.knowledge_base import FROZEN_SNAPSHOT
...
_emit_records_telemetry_event("p4", 'apps_exec.engines.base_exec_engine', "module_loaded")
