...
- **Operator Prompt efficiency (consolidated):** Ask SINGLE vs MULTIPLE immediately after NEW/EXISTING; allow bulk-paste of Name, Title, About, and LinkedIn URL in one input with semantic parsing; permit **NEW + MULTIPLE** (K=2–4) only when operator explicitly confirms immediate post-application outreach — otherwise BLOCK. Minimal prompt lines are never requested from the user (system auto-populates internally).
...
- SINGLE → proceed.
- MULTIPLE → EXISTING allowed (K=2–5); NEW allowed only if explicitly confirmed as post-application outreach (K=2–4).
  If NEW + MULTIPLE selected, ask:
  (2A) "Confirm this is immediate post-application outreach (required for NEW + MULTIPLE)? YES/NO."
    - YES → allow MULTIPLE (K=2–4); proceed.
    - NO or invalid → immediate BLOCK with:
      {
        "status": "error",
        "failed_checks": ["NEW contacts support MULTIPLE mode ONLY for explicitly confirmed immediate post-application outreach."]
      }
...
- If NEW (from Prompt 1):
  ...
  - If MULTIPLE (NEW): assume Premium=NO; do not ask (3A); default to Short (NEW) for each contact.
...
- NEW + MULTIPLE without explicit **YES** to (2A) or K∉[2..4] for NEW batches.
...
NEW (post-application confirmed via 2A) — internal header and lines
  Header: `BATCH | Short (NEW) | K=<2..4>`
  Lines: K internally generated minimal lines for Short (NEW) (Premium assumed NO).
  Reject duplicates; BLOCK if K outside 2..4 or any line fails validation.
...
- If NEW:
  • If MULTIPLE: require (2A)=YES and K ∈ [2..4]; set route to Short (NEW) with Premium=NO.
...
- Invalid responses to (1) or (2); missing/invalid (2A) when NEW + MULTIPLE; NEW batch K∉[2..4]; EXISTING batch K∉[2..5].
...
===============================================================================
END OF LINKEDIN OUTREACH - CANONICAL v1.3
===============================================================================
