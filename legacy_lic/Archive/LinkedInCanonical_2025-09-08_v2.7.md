# LinkedInCanonical - 2025-09-08 v2.7 (ZLO from v2.6 with Header Hardening, Transition and Bridge Enforcement, and C-Level DRQ)
...
- Scoring integrity: calibrated rubrics by archetype. 10 out of 10 only if all “10/10 only if” criteria hold and no hard caps apply.
...
- Header must be three plain lines in this exact order: URL first, Message Type second, Subject third. These three lines are not fenced.
...
New v2.7 format, transition, bridge, and DRQ blockers:
...
Existing blockers (carry forward from v2.6):
...
## CHANGELOG (v2.7)
- Added hard enforcement of the three line plain header (URL, Message Type, Subject) with body fenced exactly once.
- Added Transition sentence requirement before Insight 1 for all non Short message types, with QA rows and blockers.
- Strengthened Bridge before bullets enforcement with explicit row and blocker.
- Added C Level Deep Research Query requirement with QA rows and blockers, including primary source requirement and sector thesis tie in mapping to Insights and CTA.
- Retained all v2.6 archetypes, routing bias removal, RAG override precedence, QA order, scoring transparency, IVM loop, and AI Filter v8 last gate.

---

## DIFF CHECKLIST (v2.6 to v2.7)
| Invariant carried from v2.6 | Status |
|---|---|
...
| **Header 3 line order and single fenced body enforcement** | ADDED |
| **Transition before Insight 1** | ADDED |
| **C Level DRQ requirement and thesis mapping** | ADDED |
