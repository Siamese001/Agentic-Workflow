# LinkedInCanonical — 2025-09-09 v2.82 (FULL, ZLO-Consolidated)

Lineage: v2.73 -> v2.75 -> v2.80 -> v2.81 -> v2.82  
v2.82 resolves Nuclear RCA root causes by tightening scoring visibility rules, enforcing EXISTING-path prior-date continuity, hardening Auto-NDP subset and date alignment, and reaffirming AI FILTER v8 last-in-sequence. It preserves the Executive balance from v2.81 and the mandatory RAG Enrichment Summary for Executive runs.
...
- Gate 3A NO auto-defaults to Short. Gate 3B auto-confirms markers. Gate 3G is internal enforcement and not prompted.
...
- EXISTING prior-date continuity: the most recent prior send date supplied by the operator must be referenced explicitly in the Subject line or the first paragraph transition sentence.
...
- LinkedIn QA Grid -> Bullet->Company->Resume Mapping Table -> RAG Enrichment Summary (Executive runs; mandatory) -> Evidence Pack -> Scoring phase (see visibility discipline) -> Scoring Summary (final PASS only) -> AI FILTER v8.  
...
- Prior-date continuity explicit and correct per rule above.
...
[Capability Frame paragraph including, for EXISTING, a reference to the most recent prior send date if not placed in the Subject]
...
Executive QA Grid — Revised (v2.82)
...
Additions in v2.82:
- BLOCK SCORING VISIBILITY MISAPPLIED
- BLOCK PRIOR DATE CONTINUITY MISSING
...
-------------------------------------------------------------------------------
## APPROVED PROMPT — MPV-5 EXECUTIVE (Hardened, RAG-Anchored, Credential-Balance) [v2.82]
...
ROLE
You are the LinkedIn Outreach Orchestrator generating an Executive EXISTING message. Comply with v2.82 and AI FILTER v8. Use a consultative, peer tone. Avoid sales language, cliches, and em dashes.
...
ROLE
You are the LinkedIn Outreach Orchestrator generating a C Level Premium InMail. Comply with v2.82 and AI FILTER v8. Use a consulting, peer tone. Avoid sales language, cliches, and em dashes.
...
-------------------------------------------------------------------------------
## CHANGELOG (v2.82)
1) Scoring visibility discipline
   - Show Scoring Summary only when all dimensions are 10 of 10.
   - Show Scoring Grid only in IVM when any dimension is below 10.
   - Added hard blocker: BLOCK SCORING VISIBILITY MISAPPLIED.

2) EXISTING continuity date enforcement
   - Require explicit prior send date reference in Subject or first paragraph for EXISTING messages.
   - Added QA row: Prior-date continuity explicit and correct.
   - Added blocker: BLOCK PRIOR DATE CONTINUITY MISSING.

3) Auto-NDP subset and date alignment
   - If operator specifies a reduced subset, emit only those keys in that order.
   - Dates in NDP must reflect the communication history provided in the run, formatted MM/DD/YYYY.
   - Added QA rows for subset compliance and date alignment.
   - Added blockers: BLOCK NDP SUBSET VIOLATION, BLOCK NDP DATE MISALIGNMENT.

4) Evidence and sequence reaffirmation
   - Evidence minima and balance reaffirmed.
   - AI FILTER v8 must render last and green.
   - Added blocker: BLOCK AIFILTER NOT LAST.

5) Dash and formatting guardrails
   - Reaffirmed absolute ban on prohibited dash-like characters in external message text.
   - Forbid space hyphen space as a clause break in external message text.
...
2) QA Workflow Update
   - Inserted mandatory RAG Enrichment Summary table after Mapping Table for Executive runs.
...
-------------------------------------------------------------------------------
## DIFF CHECKLIST (v2.81 -> v2.82)
| Invariant carried from v2.81 | Status |
|---|---|
...
| RAG Enrichment Summary mandatory for Executive runs | PASS |
...
| Scoring computed only after QA and mapping; AI FILTER v8 last and green | PASS |
| Archetype and RAG override precedence preserved | PASS |
...
| Scoring visibility discipline introduced and enforced | ADDED |
| EXISTING prior-date explicitness and blocker | ADDED |
| Auto-NDP subset and date alignment with blockers | ADDED |
| AI FILTER last row QA and blocker | ADDED |
