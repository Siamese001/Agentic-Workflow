...
def categorize_misnamed(
    file_path: Path, filename: str, classes: list[str], has_sovereign: bool
) -> dict:
    ...
    if not classes:
        ...
        result["recommendation"] = (
            f"Rename to {filename.replace('Agent', '')}.py or add proper Agent class"
        )
    ...
    if expected_class not in classes:
        ...
        if main_class:
            if any(
                suffix in main_class
                for suffix in ["Handler", "Manager", "Helper", "Util", "Service"]
            ):
                result["issue"] = (
                    f"Utility class '{main_class}' incorrectly named with 'Agent' suffix in filename"
                )
                ...
                result["recommendation"] = (
                    f"Either rename file to {main_class}.py or rename class to {expected_class}"
                )
...
def main():
    ...
    print(
        f"  Total issues: {len(findings['HIGH']) + len(findings['MEDIUM']) + len(findings['LOW'])}"
    )
    print("=" * 80 + "\n")
