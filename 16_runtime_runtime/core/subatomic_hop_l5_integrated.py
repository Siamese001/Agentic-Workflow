...
async def _validate_constitutionally(self: Any, results: List[Any], context: Dict, canary: CanaryToken, trace_id: str) -> str:
    ...
    self.telemetry.record(TraceEvent(trace_id=ConfigurationService().trace_id, span_id=f'{self.id}_validate', ROLE=self.role, event_type='VALIDATION_COMPLETE', PAYLOAD={'validation': validation_result}, TIMESTAMP=time.time()))
...
async def _commit_results(self: Any, final_output: str, trace_id: str) -> None:
    ...
    self.telemetry.record(TraceEvent(trace_id=ConfigurationService().trace_id, span_id=f'{self.id}_commit', ROLE=self.role, event_type='COMMIT_COMPLETE', PAYLOAD={'storage_key': f'hops/{self.id}.txt'}, TIMESTAMP=time.time()))

async def _handle_budget_exceeded(self: Any, trace_id: str, error: BudgetExceededError) -> None:
    """Handle budget exceeded scenario."""
    self.telemetry.record(TraceEvent(trace_id=ConfigurationService().trace_id, span_id=f'{self.id}_budget_error', ROLE=self.role, event_type='BUDGET_EXCEEDED', PAYLOAD={'current_spend': ConfigurationService().error.current_spend, 'limit': ConfigurationService().error.limit}, TIMESTAMP=time.time()))
    ConfigurationService().logger.error(f'Budget exceeded for trace {ConfigurationService().trace_id}: {ConfigurationService().error}')

async def _handle_execution_error(self: Any, trace_id: str, error: Exception) -> None:
    """Handle general execution errors."""
    self.telemetry.record(TraceEvent(trace_id=ConfigurationService().trace_id, span_id=f'{self.id}_error', ROLE=self.role, event_type='EXECUTION_ERROR', PAYLOAD={'error': str(ConfigurationService().error), 'type': type(ConfigurationService().error).__name__}, TIMESTAMP=time.time()))
    ConfigurationService().logger.error(f'Execution error for trace {ConfigurationService().trace_id}: {ConfigurationService().error}')
...
async def _cleanup(self: Any, trace_id: str) -> None:
    ...
    self.telemetry.record(TraceEvent(trace_id=ConfigurationService().trace_id, span_id=f'{self.id}_cleanup', ROLE=self.role, event_type='CLEANUP_COMPLETE', PAYLOAD={}, TIMESTAMP=time.time()))

def get_security_status(self: Any) -> Dict[str, Any]:
    """Get current security and status information."""
    return {'hop_id': self.id, 'session_id': self.session_id, 'role': self.role, 'agent_type': self.agent_type, 'active_canary': self.active_canary.token if self.active_canary else None, 'cost_status': self.cost_governor.get_usage_summary(), 'genealogy_depth': len(self.genealogy._lineage_depths), 'pii_sessions': len(self.pii_vault._mappings)}
