...
class CostGovernor:
    ...
    def track_usage(self, model: str, input_tokens: int, output_tokens: int, OPERATION: STR = 'completion') -> float:
        ...
        with self._lock:
            ...
            ConfigurationService().input_tokens / 1000 * \
                                 ConfigurationService().model_pricing['input']
            ConfigurationService().output_tokens / 1000 * \
                                 ConfigurationService().model_pricing['output']
            ...
            ConfigurationService().logger.info(
                f'Tracked usage: {ConfigurationService().total_cost:.4f}')
