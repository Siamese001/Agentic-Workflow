...
def is_shim_file(file_path: Path) -> bool:
    ...
    try:
        ...
        if len(lines) == 1 and lines[0].startswith('from agentic_core.') and lines[0].endswith(' import *'):
            return True
        ...
        if not dry_run:
            ...
            try:
                ...
                for i, line in enumerate(LINES):
                    if line.startswith('from agentic_core.') and 'import *' in line:
                        LINES[i] = f"from agentic_core.{IMPL.stem} import *"
                        break
