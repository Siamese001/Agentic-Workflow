...
if 'mv_dependency_cone_risk' in mv_tables:
    ...
    cur.execute('SELECT COUNT(*), AVG(hop2_fan_in), MAX(hop2_fan_in) FROM mv_dependency_cone_risk WHERE hop2_fan_in > 0')
...
if 'mv_runtime_spine_gaps' in mv_tables:
    ...
    cur.execute('SELECT layer, module_count, gap_count, gap_pct FROM mv_runtime_spine_gaps ORDER BY gap_count DESC')
    for row in cur.fetchall():
...
if v_total > 0:
    cur.execute('SELECT severity, category, COUNT(*) FROM violations GROUP BY severity, category ORDER BY severity, category')
