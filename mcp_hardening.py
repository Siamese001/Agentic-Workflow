...
def get_version_locked_design(file_id: str, version_id: str, logger: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if logger:
            logger.info(f"✅ Figma: Retrieved version-locked design v{version_id}")
...
def check_design_drift(file_id: str, canonical_version: str, logger: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        canonical_design = get_version_locked_design(file_id, canonical_version, logger)
        ...
        for key, expected_value in canonical_vars.items():
            if key not in current_vars or current_vars[key] != expected_value:
                drift_detected = True
                drift_report.append(f"Variable '{key}' drift: expected {expected_value}, got {current_vars.get(key, 'MISSING')}")
        ...
        if logger:
            if drift_detected:
                logger.warning(f"⚠️ Design drift detected: {len(drift_report)} violations")
            else:
...
def execute_cost_controlled_search(query: str, max_daily_queries: int = 500, logger: Optional[Any] = None) -> Optional[str]:
    ...
    try:
        ...
        if current_count > max_daily_queries:
            if logger:
                logger.error(f"❌ Brave Search aborted: Daily query limit ({max_daily_queries}) exceeded")
            return None  # Abort search, save cost
        
        if logger:
            logger.info(f"✅ Search budget check passed: {current_count}/{max_daily_queries}")
        
    except Exception as e:
        if logger:
            logger.warning(f"L4 Redis rate limiter failed ({e}). Proceeding without limit check.")
