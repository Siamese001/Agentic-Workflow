...
async def demonstrate_hardened_k25():
    ...
    print("=== Hardened K.2.5 Deep Research Example ===\n")
...
You must respond with valid JSON matching the DeepResearchOutput schema."""
    ...
    try:
        print("Executing K.2.5 Deep Research with titanium-grade reliability...\n")
        ...
        print(f"Strategic Insights: {len(research_output.strategic_layer.key_findings)} findings")
        print(f"Technical Analysis: {len(research_output.technical_layer.architecture_overview)} components")
        print(f"Leadership Profiles: {len(research_output.leadership_layer.executive_profiles)} profiles")
        print(f"Total Citations: {len(research_output.citation_map)} sources")
        
        # Show sample finding
        if research_output.strategic_layer.key_findings:
            print("\nSample Strategic Finding:")
            print(f"- {research_output.strategic_layer.key_findings[0].finding}")
            print(f"  Confidence: {research_output.strategic_layer.key_findings[0].confidence_score}")
...
async def demonstrate_stateful_continuation():
    ...
    print(f"Refined response length: {len(response2)} characters")
...
async def demonstrate_error_handling():
    ...
    try:
        print("Testing context overflow protection...")
...
async def main():
    ...
    print("🛡️  HardenedGeminiExecutor - Titanium Grade Demo\n")
    print("This demo showcases:")
    print("- Automatic retry on rate limits")
    print("- Pre-flight token validation")
    print("- Structured JSON logging")
    print("- Safety settings for professional content")
    print("- Stateful continuation support\n")
