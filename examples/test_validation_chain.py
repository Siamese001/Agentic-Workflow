...
class MockStateManager:
    ...
    async def commit_state(self, state: Any) -> None:
        ...
        print(f"  💾 Checkpointed: {state.current_step}")
...
async def demonstrate_validation_chain():
    ...
    for gate in gates:
        print(f"  - {gate.gate_name}: {gate.rubric[:50]}...")
    ...
    try:
        ...
        for gate_name, info in status["gate_histories"].items():
            ...
            print(f"    Oscillating: {info['is_oscillating']}")
...
async def demonstrate_checkpoint_recovery():
    ...
    print(f"Last checkpoint: {checkpoint_state.current_step}")
    print(f"Content: {checkpoint_state.data_payload['valid_content']}")
...
async def demonstrate_oscillation_detection():
    ...
    print(f"Oscillation threshold: {gate.oscillation_threshold}")
    ...
    try:
        ...
        print(f"\nGate History:")
        print(f"  Total attempts: {history['attempts']}")
        print(f"  Last failures: {history['last_failures']}")
...
async def demonstrate_standard_gates():
    ...
    print("Standard Validation Gates:")
    for gate in gates:
        print(f"\n  🚦 {gate.gate_name}")
        print(f"     Rubric: {gate.rubric}")
        print(f"     Fatal: {gate.fatal_on_fail}")
        print(f"     Max Repairs: {gate.max_repair_attempts}")
        print(f"     Oscillation Detection: {gate.detect_oscillation}")


async def main():
    """Run all demonstrations."""
    print("=" * 80)
    print("RESILIENT VALIDATION CHAIN DEMONSTRATION")
    print("Self-Healing Pipeline with Atomic Checkpointing")
    print("=" * 80)
    ...
    print("✓ Self-healing validation loops")
    print("✓ Atomic checkpointing after each gate")
    print("✓ Oscillation detection to prevent infinite loops")
    print("✓ Recovery from checkpoints after failures")
    print("✓ Configurable gate behaviors")
    print("✓ Progress persistence and resume")
    print("=" * 80)
