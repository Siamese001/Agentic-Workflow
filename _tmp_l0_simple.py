...
c.execute(
    "SELECT source_file, symbol FROM edges WHERE relation_type='proposal_commits_routing' AND source_file LIKE '%L0_routing%'"
)
for r in c.fetchall():
    print(" ", r)
...
c.execute(
    "SELECT source_file, symbol FROM edges WHERE relation_type='routes_path' AND source_file NOT LIKE '%test%' AND source_file NOT LIKE '%tests%'"
)
for r in c.fetchall():
    print(" ", r)
