...
class RepositoryCleaner:
    ...
    def clean_directory(self, directory: Path) -> Dict[str, int]:
        ...
        if chains:
            print(f"  Found {len(chains)} shim chains")
            
            for base, chain in chains.items():
                print(f"    - {base}: {len(chain)} files")
                if self.clean_shim_chain(chain):
                    dir_stats["chains_cleaned"] += 1
        
        else:
            print(f"  No shim chains found")
    ...
    def clean_all(self, exclude_dirs: Optional[Set[str]] = None):
        ...
        print("Cleaning runaway refactoring artifacts...")
        print("=" * 80)
    ...
    def print_summary(self, results: Dict[str, Dict[str, int]]):
        ...
        print(f"Shim chains found: {total_chains}")
        print(f"Shim chains cleaned: {total_cleaned}")
        print(f"Shim files deleted: {self.stats['shim_files_deleted']}")
        ...
        for dir_name, result in results.items():
            if result["chains_found"] > 0:
                print(f"  {dir_name}:")
                print(f"    Chains: {result['chains_found']} found, {result['chains_cleaned']} cleaned")
