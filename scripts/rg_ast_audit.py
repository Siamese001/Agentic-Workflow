...
def analyze_file(filepath: Path) -> FileAnalysis | None:
    ...
    try:
        ...
        return FileAnalysis(
            path=str(filepath), classification="error", classification_reason=f"Parse error: {e}"
        )
...
def classify_file(analysis: FileAnalysis) -> FileAnalysis:
    ...
    if analysis.is_enum or analysis.is_basemodel or analysis.is_typeddict:
        ...
        type_kind = (
            "Enum" if analysis.is_enum else ("BaseModel" if analysis.is_basemodel else "TypedDict")
        )
    ...
    if analysis.has_classes and analysis.has_stateful_methods:
        ...
        analysis.classification_reason = (
            f"Class with stateful methods: {', '.join(analysis.stateful_methods[:3])}"
        )
        return analysis
...
def main():
    ...
    manifest = {
        ...
        "types_to_rename": {
            t: Path(t).stem.replace("Agent", "") + "_types.py" for t in types_files
        },
    ...
    full_analysis = {
        "summary": {
            ...
            "legacy": len(legacy),
            "unknown": len(unknown),
            "sovereign_alignment_score": round(len(engines) / len(analyses) * 100, 1)
            if analyses
            else 0,
        },
        ...
        "files": [
            {
                ...
                "imported_by": [
                    str(Path(p).relative_to(apps_rg_dir.parent)) for p in a.imported_by
                ],
                "imports": a.imports,
            }
            for a in analyses
