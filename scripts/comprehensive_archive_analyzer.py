...
def analyze_file(file_path: Path, archive_folder: str) -> FileAnalysis | None:
    ...
    try:
        ...
        return FileAnalysis(
            path=str(file_path), archive_folder=archive_folder, has_syntax_error=True, loc=0
        )
    except Exception:
        return None

    analysis = FileAnalysis(
        path=str(file_path), archive_folder=archive_folder, loc=len(content.splitlines())
    )
