...
def test_layer_integrity_enforcement():
    ...
    try:
        ...
        unknown_layers = [agent.get("class_name", "Unknown") for agent in agents 
                        if agent.get("layer", "unknown") in ["unknown", "tests"]]
        
        assert not unknown_layers, f"Hardening violation: Agents found in unmapped layers: {unknown_layers}. 100% pass required."
        print(f"✅ Layer integrity enforcement passed. No agents in unknown/test layers.")
...
def test_sovereign_inheritance_audit():
    ...
    try:
        ...
        core_agents = [agent for agent in agents 
                      if agent.get("layer", "") in ["L0", "L1", "L2", "L4", "L5"]]
        ...
        assert not orphan_names, f"Found core agents bypassing Sovereign architecture: {orphan_names}. 100% pass required."
        print(f"✅ Sovereign inheritance audit passed. All {len(core_agents)} core agents inherit from SovereignBaseAgent.")
...
async def test_signal_saturation_on_orphans():
    ...
    try:
        ...
        test_agents = [
            ("L2_execution.ToolRegistry.ToolsmithAgent", "ToolsmithAgent"),
            ...
            try:
                ...
                if hasattr(agent, 'heal_repository'):
                    result = agent.heal_repository(
                        telemetry_id="HARDENING-V2",
                        auto_approve=True,
                        custom_flag="RECOVERY_TEST"
                    )
                    assert "skipped" in result or "status" in result or "violations_found" in result, f"{class_name} failed to process sovereign signals"
...
def test_manifest_drift_protection():
    ...
    try:
        ...
        assert not drift, f"Manifest drift detected! Registry and Manifest.json are out of sync: {drift}. 100% pass required."
...
def test_compliance_score_calculation():
    ...
    try:
        ...
        assert abs(calculated_score - expected_score) < 0.0001, "Compliance score calculation is incorrect"
