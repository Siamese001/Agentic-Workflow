...
class TestCoreFoundationPurity:
    ...
    def test_agentic_core_inheritance(self):
        ...
        assert "from agentic_core.L2_execution.mcp.mcp_hardened_mixin import MCPHardenedMixin" in content
    ...
    def test_no_old_imports_remain(self):
        ...
        for root, dirs, files in os.walk("."):
            ...
            dirs[:] = [d for d in dirs if not d.startswith('.') and d not in ['__pycache__', 'node_modules', 'venv', '.venv', 'archives']]
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        lines = content.split('\n')
                        actual_imports = [line for line in lines if line.strip().startswith('from apps_shared.base_agents') and not line.strip().startswith('#')]
                        if actual_imports:
        ...
        active_old_imports = [imp for imp in old_imports_found if 'archives' not in imp and 'deprecated' not in imp]
        
        assert len(active_old_imports) == 0, f"Found old imports remaining in active files: {active_old_imports}"
