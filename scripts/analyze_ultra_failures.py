#!/usr/bin/env python3
"""
Analyze ultra-aggressive validation failures to achieve 100% completion
"""

import json
from collections import Counter, defaultdict

def analyze_ultra_failures():
    """Load and analyze ultra-aggressive validation failures"""
    results_path = r"C:\Users\amita\Documents\Work\AI Job Search\AI\ML\DL\GenAI\LLM 101\LLM Pipelines\Resume Gen\Git\Agentic_Workflow-10_11\scripts\windsurf_validation_keys_ultra_results.json"
    
    with open(results_path, 'r') as f:
        data = json.load(f)
    
    print("=== ULTRA-AGGRESSIVE VALIDATION FAILURE ANALYSIS ===")
    ...
    sample_keys = list(data['failed_keys'].keys())[:15]
    for key in sample_keys:
        reason = data['failed_keys'][key]['reason']
        print(f"{key:70s} -> {reason}")

if __name__ == "__main__":
    analyze_ultra_failures()
