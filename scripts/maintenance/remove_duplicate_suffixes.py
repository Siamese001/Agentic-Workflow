"""
Remove Duplicate Files with _flat and _1 Suffixes

RCA: These duplicates were created during Phase 1-8 architectural sovereignty work.
The flattening process created duplicates instead of consolidating to single canonical files.

This script:
1. Identifies all *_flat.py and *_1.py files
2. Checks if canonical version (without suffix) exists
3. Removes duplicate if canonical exists
4. Reports files that need manual review
"""
from __future__ import annotations

import sys
from pathlib import Path

# Add project root to path
project_root = Path(__file__).parent.parent.parent
sys.path.insert(0, str(project_root))


def find_duplicate_files() -> tuple[list[Path], list[Path]]:
    """Find all files with _flat and _1 suffixes."""
    flat_files = list(project_root.rglob("*_flat.py"))
    one_files = list(project_root.rglob("*_1.py"))
    
    # Exclude archives
    flat_files = [f for f in flat_files if "archives" not in str(f)]
    one_files = [f for f in one_files if "archives" not in str(f)]
    
    return flat_files, one_files


def get_canonical_path(duplicate_path: Path) -> Path:
    """Get the canonical path by removing suffix."""
    name = duplicate_path.stem
    
    if name.endswith("_flat"):
        canonical_name = name[:-5]  # Remove "_flat"
    elif name.endswith("_1"):
        canonical_name = name[:-2]  # Remove "_1"
    else:
        return duplicate_path
    
    return duplicate_path.parent / f"{canonical_name}.py"


def analyze_duplicates(
    flat_files: list[Path], one_files: list[Path]
) -> dict[str, list[tuple[Path, Path, bool]]]:
    """
    Analyze duplicates and categorize them.
    
    Returns:
        Dict with categories:
        - safe_to_delete: Canonical exists, duplicate can be removed
        - needs_review: Canonical doesn't exist, need to rename
    """
    results = {
        "safe_to_delete": [],
        "needs_review": [],
    }
    
    all_duplicates = flat_files + one_files
    
    for dup_path in all_duplicates:
        canonical_path = get_canonical_path(dup_path)
        canonical_exists = canonical_path.exists()
        
        if canonical_exists:
            results["safe_to_delete"].append((dup_path, canonical_path, True))
        else:
            results["needs_review"].append((dup_path, canonical_path, False))
    
    return results


def remove_duplicates(safe_to_delete: list[tuple[Path, Path, bool]], dry_run: bool = True) -> int:
    """Remove duplicate files that have canonical versions."""
    removed_count = 0
    
    for dup_path, canonical_path, _ in safe_to_delete:
        rel_dup = dup_path.relative_to(project_root)
        rel_canonical = canonical_path.relative_to(project_root)
        
        if dry_run:
            print(f"  [DRY-RUN] Would delete: {rel_dup}")
            print(f"            Canonical exists: {rel_canonical}")
        else:
            try:
                dup_path.unlink()
                print(f"  ✓ Deleted: {rel_dup}")
                removed_count += 1
            except Exception as e:
                print(f"  ✗ Failed to delete {rel_dup}: {e}")
    
    return removed_count


def main(dry_run: bool = True) -> int:
    """
    Main execution.
    
    Returns:
        Exit code (0 for success)
    """
    print("\n" + "=" * 70)
    print("DUPLICATE SUFFIX REMOVAL TOOL")
    print("=" * 70)
    print(f"Mode: {'DRY-RUN (no changes)' if dry_run else 'EXECUTE (will delete files)'}")
    print("=" * 70)
    
    # Find duplicates
    print("\n[1] Scanning for duplicate files...")
    flat_files, one_files = find_duplicate_files()
    
    print(f"    Found {len(flat_files)} *_flat.py files")
    print(f"    Found {len(one_files)} *_1.py files")
    print(f"    Total: {len(flat_files) + len(one_files)} duplicates")
    
    # Analyze
    print("\n[2] Analyzing duplicates...")
    results = analyze_duplicates(flat_files, one_files)
    
    safe_count = len(results["safe_to_delete"])
    review_count = len(results["needs_review"])
    
    print(f"    Safe to delete: {safe_count} (canonical exists)")
    print(f"    Needs review: {review_count} (no canonical)")
    
    # Remove safe duplicates
    if safe_count > 0:
        print(f"\n[3] {'Previewing' if dry_run else 'Removing'} safe duplicates...")
        removed = remove_duplicates(results["safe_to_delete"], dry_run)
        
        if not dry_run:
            print(f"\n✓ Removed {removed} duplicate files")
    
    # Report files needing review
    if review_count > 0:
        print(f"\n[4] Files needing manual review ({review_count}):")
        for dup_path, canonical_path, _ in results["needs_review"]:
            rel_dup = dup_path.relative_to(project_root)
            rel_canonical = canonical_path.relative_to(project_root)
            print(f"    • {rel_dup}")
            print(f"      → Should rename to: {rel_canonical}")
    
    # Summary
    print("\n" + "=" * 70)
    if dry_run:
        print("DRY-RUN COMPLETE")
        print(f"  Would delete: {safe_count} files")
        print(f"  Manual review: {review_count} files")
        print("\nRun with --execute to perform actual deletion")
    else:
        print("CLEANUP COMPLETE")
        print(f"  Deleted: {removed} files")
        print(f"  Manual review: {review_count} files")
    print("=" * 70)
    
    return 0


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Remove duplicate files with _flat and _1 suffixes")
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Actually delete files (default is dry-run)",
    )
    
    args = parser.parse_args()
    
    sys.exit(main(dry_run=not args.execute))
