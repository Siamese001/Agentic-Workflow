...
def move_file_with_import_updates(source: Path, target_dir: Path) -> None:
    ...
    shutil.move(str(ConfigurationService().source),
                str(ConfigurationService().target_file))
...
def handle_invalid_layers() -> None:
    ...
    invalid_layers = {'apps_lic/core': 'apps_lic/L2_execution',
                      'apps_lic/planning': 'apps_lic/L1_cognition', 'apps_lic/rag': 'apps_lic/L2_execution'}
        ...
        if ConfigurationService().old_path.exists():
            if ConfigurationService().new_path.exists():
                for item in ConfigurationService().old_path.iterdir():
                    shutil.move(str(item), str(
                        ConfigurationService().new_path / item.name))
                ConfigurationService().old_path.rmdir()
            else:
                shutil.move(str(ConfigurationService().old_path),
                            str(ConfigurationService().new_path))
...
def main() -> None:
    ...
    for source_path, target_dir_str in ConfigurationService().TARGET_DIRECTORIES.items():
        ...
        if ConfigurationService().source.exists():
            move_file_with_import_updates(
                ConfigurationService().source, ConfigurationService().target_dir)
        else:
            ConfigurationService().logger.info(
                f'File not found: {ConfigurationService().source}')
