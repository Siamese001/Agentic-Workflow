...
class SSOTConsolidationAnalyzer:
    ...
    def analyze_file_sovereign_features(self, file_path: Path) -> dict:
        ...
        try:
            ...
            # Sovereign V2.5 Feature Detection
            features = {
                ...
                "has_tracing": "tracing_mixin" in content or "TraceRegistry" in content,
                "has_standard_heal": "@standard_heal" in content,
                "has_sovereign_base": "SovereignBaseAgent" in content,
                "class_count": 0,
                "agent_classes": [],
                "mixin_classes": [],
            ...
            # Extract classes and analyze their nature
            ...
            features["complexity_score"] = (
                features["class_count"] * 10
                + len(features["agent_classes"]) * 20
                + len(features["mixin_classes"]) * 15
                + (1 if features["has_immutable_staging_buffer"] else 0) * 25
                + (1 if features["has_healer_mixin"] else 0) * 20
                + (1 if features["has_mcp_hardened"] else 0) * 20
                + (1 if features["has_abc_inheritance"] else 0) * 10
    ...
    def compare_file_versions(self, filename: str) -> dict:
        ...
        utils_features = (
            self.analyze_file_sovereign_features(utils_file) if utils_file.exists() else None
        )
        base_features = (
            self.analyze_file_sovereign_features(base_file) if base_file.exists() else None
        )

        comparison = {
            "filename": filename,
            "utils_exists": utils_file.exists(),
        ...
        if not utils_features and not base_features:
            ...
            # Both exist - compare Sovereign V2.5 features
            utils_score = utils_features.get("complexity_score", 0)
            base_score = base_features.get("complexity_score", 0)

            # Check for critical features
            utils_has_critical = (
                utils_features.get("has_healer_mixin", False)
                or utils_features.get("has_mcp_hardened", False)
                or utils_features.get("has_immutable_staging_buffer", False)
            )

            base_has_critical = (
                base_features.get("has_healer_mixin", False)
                or base_features.get("has_mcp_hardened", False)
                or base_features.get("has_immutable_staging_buffer", False)
            )
            ...
            elif utils_score > base_score * 1.2:  # 20% threshold
                comparison["decision"] = "USE_UTILS"
                comparison["reason"] = (
                    f"Utils version significantly more advanced ({utils_score} vs {base_score})"
                )
            elif base_score > utils_score * 1.2:
                comparison["decision"] = "USE_BASE"
                comparison["reason"] = (
                    f"Base version significantly more advanced ({base_score} vs {utils_score})"
                )
            else:
