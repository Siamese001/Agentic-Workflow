...
class AgentDispositionAnalyzer:
    ...
    def _extract_functions(self, tree: ast.AST) -> List[Dict]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                functions.append({
                    'name': node.name,
                    'is_method': node.name.startswith('_') or node.name in ['__init__', '_process', 'run_phase'],
                    'args_count': len(node.args.args)
                })
        return functions
    
    def _classify_file(self, filename: str, content: str, classes: List[Dict], 
        ...
        # Check for core foundation patterns
        core_patterns = [
            'Mixin', 'Base', 'Interface', 'Protocol', 'Abstract', 
            'HealerMixin', 'MCPHardenedMixin', 'SubatomicTestingMixin',
            'canon_base_agent_interface', 'foundation'
        ]
        ...
        if agent_classes and len(classes) == 1:  # Single agent class
            # Check if it's a full agent with state
        ...
        # Check for stateless tools
        if not agent_classes and len(functions) > 0:
            # Pure functional scripts without agent state
            return "STATELESS_TOOLS", {
                "reason": "Functional utilities without agent state",
                "function_count": len(functions)
            }
        
        # Check for validators/tools
        ...
        # Default to legacy for unclear cases
    ...
    def analyze_directory(self) -> Dict:
        ...
        for file_path in python_files:
            ...
            self.analysis_details[file_path.name] = {
