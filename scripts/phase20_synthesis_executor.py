...
class CoreSynthesisExecutor:
    ...
    def _load_synthesis_plan(self) -> Dict:
        ...
        try:
            with open("core_refinery_analysis_results.json", "r") as f:
                results = json.load(f)
                ...
                try:
                    ...
                    if extracted_logic:
                        ...
                        if self._merge_logic_into_target(target_path, extracted_logic, file_info):
                            print(f"🔄 Synthesized: {file_info['file_path']} -> {file_info['synthesis_target']}")
    ...
    def _extract_synthesis_logic(self, content: str, file_info: Dict) -> Optional[str]:
        ...
        for line in lines:
            ...
            if line.strip().startswith('def ') and not line.strip().startswith('def __'):
                ...
                if line.strip() and current_indent <= method_indent and not line.strip().startswith(('@', '"""', "'''")):
                    # End of method
                    extracted_methods.append('\n'.join(current_method))
                    current_method = []
                    in_method = False
