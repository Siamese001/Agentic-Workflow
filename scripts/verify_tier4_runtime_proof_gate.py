"""Tier 4 runtime-proof gate verification entrypoint.

Orchestrates:
  1. tier_fixture_bootstrap.materialize() (deterministic JSON fixtures)
  2. python scripts/verify_tier4_enforcement_gate.py  (metadata gate)
  3. python -m pytest <Tier 4 targeted fixture files> -q
  4. agentic_core.runtime.prove_requirements.tier4_runtime_proof_gate.evaluate()

Writes:
  - artifacts/runtime/requirements_proof/tier4_runtime_proof_gate_result.json
  - artifacts/runtime/requirements_proof/tier4_runtime_proof_gate_report.md
...
Does NOT run any tests outside the targeted Tier 4 fixture files.
"""
...
TARGETED_TESTS = (
    "tests/runtime/test_tier4_cluster_fixtures.py",
)


def _run_metadata_gate() -> str:
    proc = subprocess.run(
        [sys.executable, "scripts/verify_tier4_enforcement_gate.py"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
        timeout=120,
...
def main() -> int:
    ...
    result = tier4_runtime_proof_gate.evaluate(
        metadata_gate_status=metadata_status,
        targeted_tests_status=tests_status,
        targeted_tests_run=tests_run,
    )
    result_path = tier4_runtime_proof_gate.write_result(result)
    report_path = tier4_runtime_proof_gate.write_report(result)

    print(f"Tier 4 runtime proof gate: {result['result']}")
