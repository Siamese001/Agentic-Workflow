...
def test_validation_state_persistence():
    ...
    try:
        ...
        assert "toxicity" in agent._state["last_validation_violations"], "Should record toxicity violation"
...
def test_jailbreak_detection():
    ...
    try:
        ...
        assert "jailbreak_attempt" in result.get("violations", []), "Should have jailbreak violation"
        assert "jailbreak_probe" in result.get("checks_performed", []), "Should record jailbreak check"
...
def test_policy_violation_detection():
    ...
    try:
        ...
        assert "policy_violation" in result.get("checks_performed", []), "Should record policy check"
