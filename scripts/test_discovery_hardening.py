...
def test_import_pollution_check():
    ...
    try:
        ...
        from L5_safety.validators.HierarchyAgent import HierarchyAgent
        ...
        print(f"✅ Import pollution check passed. No forbidden modules loaded.")
...
def test_manifest_sync_integrity():
    ...
    try:
        ...
        assert live_count == manifest_count, f"Manifest is out of sync with the codebase. Live: {live_count}, Manifest: {manifest_count}"
...
def test_ssot_inheritance_compliance():
    ...
    try:
        ...
        assert not non_compliant, f"SSOT inheritance violation: {len(non_compliant)} agents not inheriting from SovereignBaseAgent"
        print(f"✅ SSOT inheritance compliance passed. All {len(agents)} core agents inherit from SovereignBaseAgent")
