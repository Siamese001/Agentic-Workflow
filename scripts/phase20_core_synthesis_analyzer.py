...
class CoreSynthesisAnalyzer:
    ...
    def analyze_file(self, file_path: Path) -> CoreAnalysisResult:
        ...
        try:
            ...
            disposition, synthesis_target, weight, rationale = self._determine_disposition(
                file_path, tree, cfg_complexity, data_flow_nodes, circular_deps, 
                contract_compliance, sovereign_requirements
            )
    ...
    def _determine_disposition(self, file_path: Path, tree: ast.AST, cfg_complexity: int, 
                             data_flow_nodes: int, circular_deps: List[str], 
                             contract_compliance: bool, sovereign_requirements: List[str]) -> Tuple[Disposition, Optional[str], float, str]:
        """Determine file disposition and synthesis target."""
        filename = file_path.name
        
        # Check for circular dependencies - immediate archive
        if circular_deps:
            return Disposition.ARCHIVE, None, 0.0, f"Circular dependencies: {', '.join(circular_deps)}"
        ...
        if has_agent_classes and sovereign_requirements:
            ...
            if "Healing" in sovereign_requirements:
                return Disposition.SYNTHESIZE, "healer_mixin.HealerMixin", 0.8, "Healing logic - merge into HealerMixin"
            elif "Hardening" in sovereign_requirements:
                return Disposition.SYNTHESIZE, "subatomic_testing_mixin.SubatomicTestingMixin", 0.7, "Hardening logic - merge into SubatomicTestingMixin"
            elif "Autonomy" in sovereign_requirements:
                return Disposition.SYNTHESIZE, "SovereignBaseAgent", 0.9, "Autonomous logic - merge into SovereignBaseAgent"
        ...
        if cfg_complexity > 10 and sovereign_requirements:
            return Disposition.SYNTHESIZE, "SovereignBaseAgent", 0.6, f"Complex logic ({cfg_complexity} CFG nodes) with sovereign requirements"
        elif cfg_complexity > 5:
            return Disposition.KEEP, None, 0.5, f"Moderate complexity ({cfg_complexity} CFG nodes)"
        else:
            return Disposition.ARCHIVE, None, 0.0, f"Low complexity ({cfg_complexity} CFG nodes) - likely utility"
    ...
    def execute_analysis(self) -> List[CoreAnalysisResult]:
        ...
        for file_path in python_files:
            ...
            print(f"   🛡️ Sovereign Requirements: {', '.join(result.sovereign_requirements) if result.sovereign_requirements else 'None'}")
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"- **ARCHIVE Disposition:** {archive_count} files ({archive_count/total_files*100:.1f}%)")
        report.append(f"- **SYNTHESIZE Disposition:** {synthesize_count} files ({synthesize_count/total_files*100:.1f}%)")
        report.append("")
        ...
        for result in self.analysis_results:
            ...
            report.append(f"- Sovereign Requirements: {', '.join(result.sovereign_requirements) if result.sovereign_requirements else 'None'}")
            report.append("")
...
def main():
    ...
    print("=" * 80)
    print(f"📄 Report saved: CORE_REFINERY_ANALYSIS.md")
    print(f"📊 Results saved: core_refinery_analysis_results.json")
