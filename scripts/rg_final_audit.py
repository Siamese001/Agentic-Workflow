...
def main():
    ...
    for path in ROOT.rglob("*.py"):
        ...
        if not any(subdir in str(path).replace('\\', '/') for subdir in [
            "engines/hops", "engines/generation", "engines/refinement", 
            "engines/safety", "engines/orchestration", "engines/quality", 
            "shared/core"
        ]):
            continue
        total_files += 1
