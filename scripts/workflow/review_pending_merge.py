...
def count_real_lines(path: Path) -> int:
    ...
    try:
        ...
        for line in lines:
            ...
            if stripped_line.startswith('from __future__') or stripped_line.startswith('import '):
                continue
            real += 1
        return real
...
def _categorize_pending_file(f: Path, approved_by_name: Dict[str, List[Path]]) -> Dict[str, Any]:
    ...
    result: Dict[str, Any] = {'file': f, 'pending_real': pending_real,
                              'pending_has_code': pending_has_code, 'category': None}
...
def _categorize_files(pending_files: List[Path], approved_by_name: Dict[str, List[Path]]) -> Dict[str, List[Path]]:
    ...
    categories: Dict[str, List[Path]] = {'has_more_code': [], 'has_code_vs_stub': [],
...
def main() -> None:
    ...
    pending_files = [f for f in REVIEW_PENDING.rglob('*.py')
                     if '__pycache__' not in str(f)]
    categories = _categorize_files(pending_files, approved_by_name)

    # Assign categorized files to variables for clarity in logging
    pending_has_more_code = categories.get('has_more_code', [])
    pending_has_code_vs_stub = categories.get('has_code_vs_stub', [])
    pending_same_or_less = categories.get('same_or_less', [])
    pending_unique_with_code = categories.get('unique_with_code', [])
    pending_unique_stub = categories.get('unique_stub', [])

    LOGGER.info(
        f'\nFiles with more code than approved versions ({len(pending_has_more_code)}):')
    for f in pending_has_more_code[:20]:
        LOGGER.info(
            f'  - {f.relative_to(REVIEW_PENDING)}')
    LOGGER.info(
        f'\nStubs replacing real code ({len(pending_has_code_vs_stub)}):')
    for f in pending_has_code_vs_stub[:20]:
        LOGGER.info(
            f'  - {f.relative_to(REVIEW_PENDING)}')
    LOGGER.info(
    ...
    needs_review = len(pending_has_more_code) + len(pending_has_code_vs_stub) + len(pending_unique_with_code)
