...
class TestUnifiedSovereignHardening:
    ...
    def test_metadata_read_only_protection(self):
        ...
        assert not hasattr(AGENT_METADATA, 'update'), "Metadata interface must be strictly read-only"
        ...
        for file_path in critical_files:
            if file_path.exists():
                ...
                for term in forbidden_terms:
                    ...
                    if term in content and not term.startswith('#') and 'test_ultra_hardening' not in str(file_path):
                        # Check if it's actually code (not in comments)
                        lines = content.split('\n')
                        for i, line in enumerate(lines, 1):
                            if term in line and not line.strip().startswith('#'):
                                pytest.fail(f"CRITICAL: Found forbidden term '{term}' in {file_path}:{i}")
