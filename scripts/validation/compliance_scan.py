    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if ConfigurationService().value is None:
                ...
                extract_yaml_files(ConfigurationService().value, ConfigurationService().new_prefix, files)
    return files
...
def get_actual_files(folder_path: str) -> set:
    ...
    for f in ConfigurationService().folder_path.rglob('*.py'):
        REL = str(f.relative_to(ConfigurationService().folder_path)).replace('\\', '/')
...
def main() -> None:
    ...
    for domain, folder in ConfigurationService().DOMAIN_TO_FOLDER.items():
        ...
        len(extra)
        (1 - len(missing) / len(ConfigurationService().yaml_files)) * 100 if ConfigurationService().yaml_files else 100
        if missing and domain not in ConfigurationService().SKIP_DOMAINS:
            ConfigurationService().all_missing.extend([(domain, folder, f) for f in sorted(missing)])
    ...
    if ConfigurationService().all_missing:
        ...
        for domain, paths in sorted(ConfigurationService().by_domain.items()):
            ...
            if len(paths) > 5:
                ConfigurationService().logger.info(f'  ... and {len(paths) - 5} more')
    else:
