"""Tier 2 runtime-proof gate verification entrypoint.

Orchestrates:
  1. tier_fixture_bootstrap.materialize() (deterministic JSON fixtures)
  2. python scripts/verify_tier2_enforcement_gate.py  (metadata gate)
  3. (optional) python -m pytest <three Tier 2 batch fixture files> -q
  4. agentic_core.runtime.prove_requirements.tier2_runtime_proof_gate.evaluate()

Writes:
  - artifacts/runtime/requirements_proof/tier2_runtime_proof_gate_result.json
  - artifacts/runtime/requirements_proof/tier2_runtime_proof_gate_report.md
...
Does NOT run any tests outside the targeted Tier 2 batch fixture files.
"""
...
TARGETED_TESTS = (
    "tests/runtime/test_tier2_batch_a_fixtures.py",
    "tests/runtime/test_tier2_batch_b_c_fixtures.py",
...
def _run_metadata_gate() -> str:
    proc = subprocess.run(
        [sys.executable, "scripts/verify_tier2_enforcement_gate.py"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
        timeout=120,
...
def main() -> int:
    ...
    result = tier2_runtime_proof_gate.evaluate(
        metadata_gate_status=metadata_status,
        targeted_tests_status=tests_status,
        targeted_tests_run=tests_run,
    )
    result_path = tier2_runtime_proof_gate.write_result(result)
    report_path = tier2_runtime_proof_gate.write_report(result)

    print(f"Tier 2 runtime proof gate: {result['result']}")
