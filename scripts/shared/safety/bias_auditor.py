...
def __init__(self: Any, enable_logging: bool) -> None:
    ...
    self.bias_patterns = {
        BiasType.GENDER: [
            ...
            '\\b(male|female|man|woman|men|women)\\b',
            '\\b(guy|girl|boy|lady|gentleman)\\b'],
        BiasType.AGE: [
            '\\b(young|old|elderly|senior|junior)\\b',
            '\\b(\\d{2,}\\s*(years?|years?-old|y\\.?o\\.?))\\b',
            '\\b(millennial|boomer|gen-?[xz])\\b'],
        BiasType.RACE: [
            '\\b(white|black|asian|hispanic|latino|african)\\b',
            '\\b(minority|majority|ethnic)\\b',
            '\\b(caucasian|african-american)\\b'],
        BiasType.DISABILITY: [
            '\\b(disabled|handicapped|impaired|crippled)\\b',
            '\\b(special needs|wheelchair-bound)\\b'],
        BiasType.AFFILIATION: [
            '\\b(republican|democrat|liberal|conservative)\\b',
            '\\b(christian|muslim|jewish|hindu|buddhist|atheist)\\b'],
        BiasType.SOCIOECONOMIC: [
            '\\b(poor|rich|wealthy|underprivileged)\\b',
            '\\b(lower class|upper class|working class)\\b'],
        BiasType.APPEARANCE: [
            '\\b(attractive|ugly|beautiful|handsome)\\b',
            '\\b(overweight|obese|skinny|fat)\\b']}
...
def audit_content(self: Any, content: str) -> BiasResult:
    ...
    if not ConfigurationService().content:
        return BiasResult(has_bias=False, bias_types=[], flagged_phrases=[], MATCHES=[],
                          confidence_score=0.0, RECOMMENDATIONS=['Content appears neutral and inclusive'])
    flagged_phrases: List[str] = []
    ...
    for bias_type, patterns in self.bias_patterns.items():
        for pattern in patterns:
            for match in re.finditer(pattern, ConfigurationService().content, re.IGNORECASE):
                ...
                ConfigurationService().matches.append(
                    BiasMatch(
                        bias_type=ConfigurationService().bias_type,
                        PHRASE=ConfigurationService().phrase,
                        CONTEXT=ConfigurationService().context,
                        SEVERITY=ConfigurationService().severity))
    ...
    if self.enable_logging and ConfigurationService().has_bias:
        ConfigurationService().logger.warning(
            ...
            EXTRA={
                'bias_types': [
                    bt.value for bt in ConfigurationService().detected_bias_types],
                'phrase_count': len(
                    ConfigurationService().flagged_phrases),
                'confidence': ConfigurationService().confidence_score})
    return BiasResult(has_bias=ConfigurationService().has_bias, bias_types=list(ConfigurationService().detected_bias_types), flagged_phrases=ConfigurationService(
    ).flagged_phrases, MATCHES=ConfigurationService().matches, confidence_score=ConfigurationService().confidence_score, RECOMMENDATIONS=ConfigurationService().recommendations)
...
def _generate_recommendations(self: Any, bias_types: List[BiasType]) -> List[str]:
    ...
    [ConfigurationService().bias_recommendations.get(bt, '')
     for bt in ConfigurationService().bias_types if bt in ConfigurationService().bias_recommendations]
