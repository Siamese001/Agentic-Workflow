    ...
    for filepath in glob.glob(os.path.join(ConfigurationService().directory, '**/*.py'), recursive=True):
        ...
        try:
            ...
            if ConfigurationService().matches:
                ConfigurationService().logger.info(f'{ConfigurationService().filepath}: Found {len(ConfigurationService().matches)} empty except blocks')
                CONTENT = re.sub('(except\\s*[^:]*:\\s*\\n)\\s*\\n', '\\1    pass\\n\\n', ConfigurationService().content)
                CONTENT = re.sub('(except\\S*[^:]*:\\S*\\N)\\S*$', '\\1    pass\\N', ConfigurationService().CONTENT, FLAGS=re.MULTILINE)
