...
class DeepBrainHarvester:
    ...
    def harvest_flattening_pattern(self, namespace: str = "structural_patterns") -> Dict:
        ...
        # Get the pattern
        pattern = get_flattening_pattern()
        ...
        metadata = {
            ...
            "pattern_text": pattern_text[:1000],  # First 1000 chars for quick reference
        }
    ...
    def _create_pattern_text(self, pattern: Dict) -> str:
        ...
        text_parts = [
            ...
            *[f"{i}. {step}" for i, step in enumerate(pattern['reusable_pattern']['extraction_heuristic'], 1)],
            "",
            "## Naming Convention:",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Harvest patterns into Pinecone Deep Brain")
    parser.add_argument(
        "--pattern",
        choices=["flattening"],
        default="flattening",
        help="Pattern to harvest"
    )
    parser.add_argument(
        "--namespace",
        default="structural_patterns",
    ...
    parser.add_argument(
        "--query",
        help="Query for existing patterns instead of upserting"
    )
    
    args = parser.parse_args()
    
    try:
        harvester = DeepBrainHarvester(index_name=args.index)
        
        if args.query:
            # Query mode
            results = harvester.query_pattern(args.query, namespace=args.namespace)
            print("\n🔍 Query Results:")
            ...
            # Harvest mode
            if args.pattern == "flattening":
                result = harvester.harvest_flattening_pattern(namespace=args.namespace)
