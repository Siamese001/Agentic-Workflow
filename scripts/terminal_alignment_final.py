...
def fix_dataclass_field_syntax(file_path: Path) -> bool:
    ...
    content = re.sub(
        r'field\(default=([^,]+), description="([^"]+)"\)', r"field(default=\1)  # \2", content
    )
...
def fix_subatomic_mixin_imports(file_path: Path) -> bool:
    ...
    content = re.sub(
        r"from apps_lic\.shared\.core\.mixins import.*SubatomicTestingMixin.*\n", "", content
    )
