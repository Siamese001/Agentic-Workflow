...
class TestSimulationIntegrity:
    ...
    def test_e2e_flow_success(self):
        ...
        hop1_mock = MagicMock()
        hop1_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop1_analysis", {
            "Archetype": "C_LEVEL",
        ...
        # HOP2: Research
        hop2_mock = MagicMock()
        hop2_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop2_research", {
        ...
        hop3_mock = MagicMock()
        hop3_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop3_sender_grounding", {
            "sender_grounding": {
        ...
        # HOP4: Routing
        hop4_mock = MagicMock()
        hop4_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop4_routing", {
            "route": "INMAIL",
        ...
        hop5_mock = MagicMock()
        hop5_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop5_generation", {
            "selected_draft": {
                "text": draft_text,
                "checksum": checksum,
                "score": 9.0
        ...
        # HOP6: Validation
        hop6_mock = MagicMock()
        hop6_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop6_validation_report", {
            "passed": True,
            "validation_results": [
        ...
        # HOP7: Gate Decision
        hop7_mock = MagicMock()
        hop7_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop7_gate_decision", {
            "decision": "PASS",
            "action": "PROCEED",
        ...
        # HOP8: QA Report
        hop8_mock = MagicMock()
        hop8_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop8_qa_report", {
            "total_score": 85.0,
        ...
        # HOP9: Integration
        hop9_mock = MagicMock()
        hop9_mock.run_phase = MagicMock(side_effect=lambda buf, reg: buf.write_once("hop9_integration", {
    ...
    def test_checksum_lock_enforcement(self):
        ...
        def hop9_side_effect(buf, reg):
            ...
            buf.write_once("hop9_integration", {
                "status": "READY_FOR_DELIVERY",
