...
# For DAG invariants (use abstract identifiers; actual hashing is done by validator)
DAG_SPECS = {
    "resume_engine_dag": {
        "node_count": 12,
    },
...
def gen_runtime_binding_keys():
    keys = []
    # runtime observation for modules
    for executor in L2_EXECUTORS:
    ...
    keys.append(f"coverage.runtime.observed_memory_call::temporal_store")
    return keys
...
def gen_dag_invariant_keys():
    ...
    for dag_name, spec in DAG_SPECS.items():
        ...
        keys.append(f"l3.dag.node_count::{dag_name}::{node_count}")
...
def gen_prompt_governance_keys():
    ...
    for p in PROMPT_FILES:
        ...
        keys.append(f"prompt.governance.strict_version::v10_10")
