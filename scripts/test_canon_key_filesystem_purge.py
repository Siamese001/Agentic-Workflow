...
class TestCanonKeyFilesystemPurge:
    ...
    def test_structure_blueprint_clean(self):
        ...
        for pattern in deprecated_patterns:
            assert pattern not in content, \
                f"structure_blueprint.py still contains {pattern}"
    ...
    def test_void_compliance_refactored(self):
        ...
        for i, line in enumerate(lines):
            if any(pattern in line for pattern in ['CANON_KEY_EXCEPTIONS', 'ACTIVE_CANON_KEYS', 'CANON_KEY_TO_FOLDER_MAP']):
                if not line.strip().startswith('#'):
                    assert False, f"void_compliance.py line {i+1} has active Canon Key reference: {line.strip()}"
