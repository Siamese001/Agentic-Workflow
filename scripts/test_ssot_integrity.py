...
class TestSSOTFinalization:
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        lines = content.split('\n')
                        actual_imports = [line for line in lines if line.strip().startswith('from agentic_core.utils.core_extensions') and not line.strip().startswith('#')]
                        if actual_imports:
    ...
    def test_ssot_completeness(self):
        ...
        assert len(python_files) >= 95, f"SSOT should contain at least 95 files, found {len(python_files)}"
