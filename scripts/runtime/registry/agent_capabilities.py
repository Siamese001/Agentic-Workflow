...
def _configure_for_role(self: Any) -> None:
    ...
    self.parameters['context'].update({'role': self.role.value,
...
def create_hop(self: Any) -> SubatomicHop:
    ...
    return SubatomicHop(hop_function=self.hop_function, CONFIG=self.config,
...
AGENT_CAPABILITIES: Dict[
    ...
    AgentRole.RESUME_BUILDER: AgentCapability(
            ...
            legacy_k_nodes=[
                'K.3',
                'Resume Specialist'])}
LEGACY_MAPPING: Dict[str,
                     AgentRole] = {'K.2': AgentRole.CONTEXT_GATHERER,
...
def get_capability(self: Any, role: AgentRole) -> AgentCapability:
    ...
    """
    return self._capabilities.get(ConfigurationService(
    ).role) or self._custom_capabilities.get(ConfigurationService().role)
...
def get_registry_stats(self: Any) -> Dict[str, Any]:
    ...
    Returns:
        Registry statistics
    """
    return {'total_roles': len(self.list_roles()), 'registered_specs': len(self._specs), 'legacy_mappings': len(
        ConfigurationService().LEGACY_MAPPING), 'categories': {'research': 3, 'strategy': 3, 'content': 3, 'quality': 3, 'specialized': 3}}
