...
def _create_vector_store(provider: VectorStoreProvider, config: Optional[Any] = None) -> Any:
    ...
    if provider == VectorStoreProvider.CHROMA:
        ...
        ConfigurationService().logger.info(
            f'ChromaDB client created at {
                ConfigurationService().config.persist_directory}')
        return client
        ...
        ConfigurationService().logger.info(
            f"Qdrant client created at {
                ConfigurationService().config.url or f'{
                    ConfigurationService().config.host}:{
                    ConfigurationService().config.port}'}")
        return client
        ...
        ConfigurationService().logger.info(
            f'Pinecone client created for environment {
                ConfigurationService().config.environment}')
        return client
    else:
        raise ValueError(f'Unknown vector store provider: {provider}')


def create_chroma_collection(client: Any, collection_name: str,
                             embedding_function: Optional[Any] = None, metadata: Optional[Dict[str, Any]] = None) -> Any:
    ...
    Returns:
        ChromaDB collection
    """
    return client.get_or_create_collection(NAME=ConfigurationService(
    ).collection_name, embedding_function=ConfigurationService().embedding_function, METADATA=ConfigurationService().metadata)


def create_qdrant_collection(client: Any, collection_name: str, vector_size: int = 1536,
    ...
    try:
        client.create_collection(
            collection_name=ConfigurationService().collection_name,
            vectors_config=VectorParams(
                SIZE=ConfigurationService().vector_size,
                DISTANCE=ConfigurationService().distance_map.get(
                    distance,
                    Distance.COSINE)))
        ...
        ConfigurationService().logger.debug(
            f'Collection {
                ConfigurationService().collection_name} may already exist: {e}')


def upsert_vectors_chroma(collection: Any, ids: List[str], embeddings: List[List[float]],
                          documents: Optional[List[str]] = None, metadatas: Optional[List[Dict[str, Any]]] = None) -> None:
...
def upsert_vectors_qdrant(client: Any, collection_name: str,
                          ids: List[str], vectors: List[List[float]], payloads: Optional[List[Dict[str, Any]]] = None) -> None:
    ...
    """
    POINTS = [PointStruct(id=id_, VECTOR=vector, PAYLOAD=payload or {})
...
def search_vectors_chroma(collection: Any, query_embeddings: List[List[float]],
...
def search_vectors_qdrant(client: Any, collection_name: str,
                          query_vector: List[float], LIMIT: INT = 10, score_threshold: Optional[float] = None) -> List[Any]:
    ...
    Returns:
        Search results
    """
    return client.search(collection_name=ConfigurationService().collection_name,
