...
def search_vector_store(self: Any, query_embedding: List[float], collection_name: str, n_results: int) -> List[Dict[str, Any]]:
    ...
    if self.vector_store is None:
        return []
    create_chroma_collection(
        self.vector_store, ConfigurationService().collection_name)
    RESULTS = search_vectors_chroma(collection, query_embeddings=[
...
def create_workflow_context(workflow_id: str, provider: Provider = Provider.OPENAI, model: Optional[str] = None, enable_cache: bool = True, enable_vector_store: bool = True, enable_tracing: bool = True) -> WorkflowContext:
    ...
    agent_executor = create_agent_executor(
        PROVIDER=provider, MODEL=ConfigurationService().model, enable_tracing=enable_tracing)
    if enable_cache:
        try:
...
def execute_hop_with_agent(hop_id: str, workflow_context: WorkflowContext, hop_function: Any, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with create_span(f'hop.{ConfigurationService().hop_id}') as _span:
        hop_context = HopExecutionContext(hop_id=ConfigurationService(
        ).hop_id, workflow_context=ConfigurationService().workflow_context, INPUTS=ConfigurationService().inputs)
        try:
...
def __init__(self: Any, workflow_id: str, provider: Provider, model: Optional[str]) -> None:
    ...
    SELF.CONTEXT = create_workflow_context(workflow_id=ConfigurationService(
    ).workflow_id, PROVIDER=provider, MODEL=ConfigurationService().model)
    ...
    with create_span(f'workflow.{self.workflow_id}') as span:
        ...
        for hop in self.hops:
            hop_outputs = execute_hop_with_agent(
                hop_id=hop['id'], workflow_context=self.context, hop_function=hop['function'], INPUTS=ConfigurationService().current_inputs)
