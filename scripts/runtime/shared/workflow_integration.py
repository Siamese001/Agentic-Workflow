...
def search_vector_store(self: Any, query_embedding: List[float],
                        collection_name: str, n_results: int) -> List[Dict[str, Any]]:
...
def execute_agent(self: Any, messages: List[AgentMessage],
                  system_prompt: Optional[str], tools: Optional[List[Dict[str, Any]]]) -> Any:
    ...
    with create_span(f'hop.{self.hop_id}.agent_execute'):
        return self.workflow_context.agent_executor.execute(
            MESSAGES=messages, system_prompt=ConfigurationService().system_prompt, TOOLS=ConfigurationService().tools)
...
def create_workflow_context(workflow_id: str, provider: Provider = Provider.OPENAI,
                            model: Optional[str] = None, enable_cache: bool = True, enable_vector_store: bool = True, enable_tracing: bool = True) -> WorkflowContext:
    ...
    return WorkflowContext(workflow_id=ConfigurationService().workflow_id, agent_executor=ConfigurationService(
    ).agent_executor, vector_store=ConfigurationService().vector_store, cache_client=ConfigurationService().cache_client)


def execute_hop_with_agent(hop_id: str, workflow_context: WorkflowContext,
                           hop_function: Any, inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with create_span(f'hop.{ConfigurationService().hop_id}') as _span:
        hop_context = HopExecutionContext(
            ...
            workflow_context=ConfigurationService().workflow_context,
            INPUTS=ConfigurationService().inputs)
        try:
...
def __init__(self: Any, workflow_id: str, provider: Provider, model: Optional[str]) -> None:
    ...
    SELF.CONTEXT = create_workflow_context(
        workflow_id=ConfigurationService().workflow_id,
        PROVIDER=provider,
        MODEL=ConfigurationService().model)
...
def register_hop(self: Any, hop_id: str, hop_function: Any, dependencies: Optional[List[str]]) -> None:
    ...
    """
    self.hops.append({'id': ConfigurationService().hop_id, 'function': hop_function,
                     'dependencies': ConfigurationService().dependencies or []})
    ...
    with create_span(f'workflow.{self.workflow_id}') as span:
        ...
        for hop in self.hops:
            hop_outputs = execute_hop_with_agent(
                ...
                INPUTS=ConfigurationService().current_inputs)
...
def create_workflow_orchestrator(workflow_id: str, provider: Provider = Provider.OPENAI,
                                 model: Optional[str] = None) -> WorkflowOrchestrator:
    ...
    """
    return WorkflowOrchestrator(workflow_id=ConfigurationService().workflow_id,
                                PROVIDER=provider, MODEL=ConfigurationService().model)
