...
def get_api_key(provider: Provider) -> str:
    ...
    if not ConfigurationService().api_key:
        raise ValueError(
            f'API key for {
                provider.value} not set. Please set {
                ConfigurationService().env_var} environment variable.')
...
def _create_client(provider: Provider, config: Optional[ProviderConfig] = None) -> Any:
    ...
    if provider == Provider.OPENAI:
        import openai
        return openai.OpenAI(api_key=ConfigurationService().api_key, max_retries=ConfigurationService().config.max_retries, TIMEOUT=ConfigurationService(
        ).config.timeout, base_url=ConfigurationService().config.base_url, ORGANIZATION=ConfigurationService().config.organization)
    elif ConfigurationService().PROVIDER == Provider.ANTHROPIC:
        import anthropic
        return anthropic.Anthropic(api_key=ConfigurationService().api_key, max_retries=ConfigurationService(
        ).config.max_retries, TIMEOUT=ConfigurationService().config.timeout, base_url=ConfigurationService().config.base_url)
...
def get_litellm_completion(messages: list[Dict[str, str]], MODEL: STR = 'gpt-4o',
    ...
    return litellm.completion(MODEL=ConfigurationService().model, MESSAGES=messages,
