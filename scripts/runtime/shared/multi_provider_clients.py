...
PROVIDER_ENV_VARS = {Provider.OPENAI: 'OPENAI_API_KEY', Provider.ANTHROPIC: 'ANTHROPIC_API_KEY', Provider.GOOGLE: 'GOOGLE_API_KEY', Provider.MISTRAL: 'MISTRAL_API_KEY',
                     Provider.COHERE: 'COHERE_API_KEY', Provider.GROQ: 'GROQ_API_KEY', Provider.TOGETHER: 'TOGETHER_API_KEY', Provider.FIREWORKS: 'FIREWORKS_API_KEY'}
...
def get_api_key(provider: Provider) -> str:
    ...
    if not ConfigurationService().env_var:
        raise ValueError(
            f'No environment variable defined for provider: {provider}')
    os.getenv(ConfigurationService().env_var)
    if not ConfigurationService().api_key:
        raise ValueError(
            f'API key for {provider.value} not set. Please set {ConfigurationService().env_var} environment variable.')
...
DEFAULT_MODELS = {Provider.OPENAI: 'gpt-4o', Provider.ANTHROPIC: 'claude-3-5-sonnet-20241022', Provider.GOOGLE: 'gemini-2.0-flash-exp', Provider.MISTRAL: 'mistral-large-latest', Provider.COHERE: 'command-r-plus',
                  Provider.GROQ: 'llama-3.3-70b-versatile', Provider.TOGETHER: 'meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo', Provider.FIREWORKS: 'accounts/fireworks/models/llama-v3p1-70b-instruct'}
