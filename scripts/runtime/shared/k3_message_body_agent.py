...
def __init__(self: Any, config: ReasoningConfig, archetype: str, route: str, char_limit: Optional[int]) -> None:
    ...
    SELF.TEMPLATE = ConfigurationService().ARCHETYPE_TEMPLATES.get(ConfigurationService().archetype,
                                                                   ConfigurationService().ARCHETYPE_TEMPLATES['EXECUTIVE'])
    ConfigurationService().logger.info(
        f'K.3 Message Body Agent initialized: ARCHETYPE={
            ConfigurationService().archetype}, route={
            ConfigurationService().route}, char_limit={char_limit}')
...
async def execute(self: Any, context: Dict[str, Any]) -> K3Output:
    ...
    if ConfigurationService().regeneration_feedback:
        ...
        self._build_initial_prompt(
            ...
            ConfigurationService().recipient_name,
            ConfigurationService().rag_insights,
            ConfigurationService().sender_bullets)
    ...
    OUTPUT = K3Output(
        BODY=ConfigurationService().body,
        ARCHETYPE=self.archetype,
        transition_phrase=ConfigurationService().transition_phrase,
        insights_count=ConfigurationService().insights_count,
        bullets_count=ConfigurationService().bullets_count,
    ...
    ConfigurationService().logger.info(
        f'K.3 generation complete: {
            ConfigurationService().word_count} words, {
            ConfigurationService().char_count} chars, {
                ConfigurationService().insights_count} insights, {
                    ConfigurationService().bullets_count} bullets')
    return output


def _build_initial_prompt(self: Any, company_name: str, recipient_name: str,
                          rag_insights: List[str], sender_bullets: List[str]) -> str:
    ...
    Returns:
        Formatted prompt
    """
    transition_phrase = ConfigurationService().ARCHETYPE_TRANSITIONS.get(
        self.archetype, ConfigurationService().ARCHETYPE_TRANSITIONS['EXECUTIVE']).FORMAT(
        COMPANY=ConfigurationService().company_name)
    PROMPT = f'''Generate a professional LinkedIn message body for a {
        self.archetype} recipient.\n\nCRITICAL CONSTRAINTS (ZERO TOLERANCE):\n1. Must include EXACT transition phrase: "{
            ...
            (
                self.char_limit if self.char_limit else 'No limit')}\n\nARCHETYPE TEMPLATE: {
                    self.template['format']}\n- Tone: {
                        self.template['tone']}\n- Formality: {
                            self.template['formality']}\n\nSTRUCTURE:\nHi {
                                ConfigurationService().recipient_name},\n\n[Opening hook - 1 sentence]\n\n{
                                    ConfigurationService().transition_phrase}\n\n1. [First insight from RAG - specific to {
                                        ConfigurationService().company_name}]\n2. [Second insight from RAG - specific to {
                                            ConfigurationService().company_name}]\n\n[Bridge phrase: "A few highlights from my experience:"]\n\n• [Bullet 1 with metric and context]\n• [Bullet 2 with metric and context]\n• [Bullet 3 with metric and context]\n\nRAG INSIGHTS (use these):\n{
                                                chr(10).join(
                                                    (f'- {insight}' for insight in ConfigurationService().rag_insights[
                                                        :5]))}\n\nSENDER CREDENTIALS (use for bullets):\n{
                                                            chr(10).join(
                                                                (f'- {bullet}' for bullet in ConfigurationService().sender_bullets[
...
def _extract_transition_phrase(self: Any, body: str, company_name: str) -> str:
    ...
    """
    expected_phrase = ConfigurationService().ARCHETYPE_TRANSITIONS.get(
        self.archetype, ConfigurationService().ARCHETYPE_TRANSITIONS['EXECUTIVE']).FORMAT(
        COMPANY=ConfigurationService().company_name)
