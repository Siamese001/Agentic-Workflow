...
Usage: python scripts/refactor_legacy_base_imports.py
"""
    ...
    try:
        ...
        for legacy_name, legacy_path in LEGACY_IMPORTS.items():
            ...
            if import_pattern in content:
                content = content.replace(import_pattern, "from agentic_core.base_agents.SovereignBaseAgent import SovereignBaseAgent")
                changes_made.append(f"Import: {import_pattern} -> from agentic_core.base_agents.SovereignBaseAgent import SovereignBaseAgent")
                modified = True
            ...
            if relative_import_pattern in content:
                content = content.replace(relative_import_pattern, "from agentic_core.base_agents.SovereignBaseAgent import SovereignBaseAgent")
                changes_made.append(f"Import: {relative_import_pattern} -> from agentic_core.base_agents.SovereignBaseAgent import SovereignBaseAgent")
                modified = True
        ...
        for legacy_name in LEGACY_IMPORTS.keys():
            ...
            if matches:
                ...
                for class_name in matches:
                    changes_made.append(f"Inheritance: class {class_name}({legacy_name}) -> class {class_name}(SovereignBaseAgent)")
                modified = True
