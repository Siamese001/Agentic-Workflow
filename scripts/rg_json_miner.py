...
def mine_workflows():
    ...
    # Sort by version/time to find the latest "Truth"
    files.sort(key=os.path.getmtime, reverse=True)
    golden_master = files[0]  # Assume latest file is the target
    ...
    with open(OUTPUT_REPORT, "w", encoding="utf-8") as report:
        ...
        try:
            with open(golden_master, "r", encoding="utf-8") as f:
                data = json.load(f)
            ...
            # --- SECTION 2: THE WORKFLOW DAG (Nervous System) ---
            report.write("## 2. 🔄 Workflow Topology (DAG)\n\n")
            
            # Try multiple possible locations for workflow steps
            steps = None
            workflow_key = None
            ...
            if steps and isinstance(steps, list):
                ...
                for step in steps:
                    if isinstance(step, dict):
                        ...
                        agent = step.get("agent", step.get("tool", step.get("node", step.get("type", "Unknown"))))
            ...
            # --- SECTION 3: PROMPT ENCYCLOPEDIA (Brain) ---
            report.write("## 3. 🧠 Prompt Encyclopedia\n\n")
            report.write("*Exact text extraction of all detected prompt templates.*\n\n")
            
            prompts = extract_keys_recursive(data, ["prompt", "system_prompt", "user_prompt", "template", "instruction"])
            prompt_count = 0
            ...
            # --- SECTION 4: CONFIGURATION LEDGER (Hyperparameters) ---
            report.write("## 4. ⚙️ Configuration & Tuning\n\n")
            report.write("| Parameter Path | Value |\n")
            report.write("| :--- | :--- |\n")
            
            config_keys = ["temperature", "model", "tokens", "timeout", "retry", 
                          "max_", "min_", "threshold", "limit", "count", "size",
                          "weight", "score", "confidence", "budget"]
...
def extract_k_nodes(data, parent_key=""):
    ...
    if isinstance(data, dict):
        for k, v in data.items():
            ...
            # Check if key matches K-node pattern
