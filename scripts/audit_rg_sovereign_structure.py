...
class RGSovereignAuditor:
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for base in node.bases:
                    if isinstance(base, ast.Name):
                        ...
                        bases.append(f"{base.value.id}.{base.attr}" if hasattr(base.value, 'id') else str(base))
                ...
                has_execute_or_process = any(method in ['execute', '_process', 'run', 'run_phase'] for method in methods)
    ...
    def _classify_file_logic(self, filename: str, content: str, classes: List[Dict], 
                           imports: List[str], functions: List[Dict]) -> Tuple[str, Dict]:
        ...
        # Check for passive data (Models, Enums, Schemas)
        model_indicators = ['Model', 'Schema', 'Enum', 'Type', 'Data']
        if any(indicator in filename for indicator in model_indicators):
            # Verify it's actually passive data
            has_classes = len(classes) > 0
            all_passive = all(
                cls.get('is_model', False) or cls.get('is_enum', False) or 
                not cls.get('has_execute_or_process', False)
                for cls in classes
            )
        ...
        agent_base_indicators = ['HealerMixin', 'MCPHardenedMixin', 'BaseAgent', 'AgentBase', 'SovereignBaseAgent']
        ...
        # Check for agent-like methods beyond just execute/_process
        agent_method_indicators = ['execute', '_process', 'run', 'run_phase', 'heal_repository', 'orchestrate', 'dispatch']
        ...
        has_agent_naming = (
            filename.endswith('Agent.py') or 
            filename.endswith('Engine.py') or
            any(cls.get('is_agent', False) for cls in classes) or
            any('Engine' in cls.get('name', '') for cls in classes)
        )
        
        # Check for sovereign agent characteristics
        if (inherits_from_agent_base or has_agent_methods) and has_agent_naming:
            return "SOVEREIGN_AGENT", {
                ...
                "agent_classes": [cls['name'] for cls in classes if cls.get('is_agent', False) or 'Engine' in cls.get('name', '')],
                "inherits_from_base": inherits_from_agent_base,
                "has_agent_methods": has_agent_methods
            }
        
        # Check for stateless tools (more sophisticated detection)
        ...
        # Check if file has no stateful classes
        ...
        if classes:
            ...
            if has_execute_classes and (has_state_classes or inherits_from_agent_base):
                return "SOVEREIGN_AGENT", {"reason": "Contains classes with execute/process methods and state or agent inheritance"}
    ...
    def _analyze_subdirectory(self, subdir_path: Path, subdir_name: str) -> Dict:
        ...
        for file_path in python_files:
            ...
            if "error" not in analysis:
                ...
                # Map classification to correct key
    ...
    def _generate_unknown_ledger(self, results: Dict):
        ...
        for file_path, analysis in engines_files.items():
            if "error" not in analysis:
                ...
                # Check if any class inherits from a Base Agent
    ...
    def _suggest_fixed_name(self, file_path: str, analysis: Dict) -> str:
        ...
        # Remove "Agent" suffix and add appropriate suffix
    ...
    def _print_audit_summary(self, results: Dict):
        ...
        if results["unknown_ledger"]:
            ...
            for item in results["unknown_ledger"][:5]:  # Show first 5
        ...
        if results["nomenclature_fixes"]:
            ...
            for item in results["nomenclature_fixes"][:5]:  # Show first 5
        ...
        if results["migration_candidates"]:
            ...
            for item in results["migration_candidates"][:5]:  # Show first 5
