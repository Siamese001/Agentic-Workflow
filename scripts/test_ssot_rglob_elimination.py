...
def test_4_compliance_orchestrator_uses_ssot():
    ...
    agent_path = PROJECT_ROOT / "agentic_core" / "L5_safety" / "validators" / "ComplianceOrchestratorAgent.py"
    content = agent_path.read_text(encoding='utf-8')
...
def test_6_agent_discovery_audit_uses_ssot():
    ...
    script_path = PROJECT_ROOT / "agentic_core" / "L0_maintenance" / "scripts" / "agent_discovery_audit.py"
...
def test_7_filesystem_reconciler_uses_ssot():
    ...
    agent_path = PROJECT_ROOT / "agentic_core" / "L5_safety" / "validators" / "FilesystemSSOTReconcilerAgent.py"
    content = agent_path.read_text(encoding='utf-8')
...
def test_8_no_backup_scanning():
    ...
    key_files = [
        ...
        PROJECT_ROOT / "agentic_core" / "L5_safety" / "validators" / "ComplianceOrchestratorAgent.py",
            ...
            if 'self.project_root.rglob("*.py")' in line:
                ...
                if '.sovereign_healing_backup' not in context and 'Fallback' not in context:
                    if 'agentic_core' not in context:
                        print(f"   ⚠ {file_path.name} line {i+1}: project_root.rglob without exclusions")
