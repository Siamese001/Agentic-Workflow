...
class ActionNode:
    ...
    def _execute_single_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        ...
        params = step.get('params', {}) # Assuming the Cognitive Node formats params correctly
    ...
    def _safe_path(self, filename: str) -> Path:
        ...
        if not str(target).startswith(str(self.work_dir)):
            raise ValueError(f"SECURITY VIOLATION: Path '{filename}' attempts to escape workspace.")
        return target
    ...
    def _tool_run_command(self, command: str) -> str:
        ...
        if any(b in command for b in blacklist):
            raise ValueError("SECURITY VIOLATION: Command contains blacklisted patterns.")
