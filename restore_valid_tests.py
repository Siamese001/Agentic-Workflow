...
def check_module_exists(module: str) -> bool:
    ...
    # Try lowercase version (snake_case)
    parts = module.split('.')
    snake_parts = []
    for part in parts:
...
def analyze_deleted_files():
    ...
    for file_path, commit in deleted_files:
        ...
        # Check if any tested module still exists
        existing_modules = []
        missing_modules = []
        ...
        if existing_modules and not missing_modules:
            # All tested modules exist - should NOT have been deleted
