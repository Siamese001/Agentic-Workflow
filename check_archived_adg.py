...
def check_archived_adg():
    ...
    if archive_file.exists():
        with gzip.open(archive_file, 'rt') as f:
            data = json.load(f)
        print(f'📊 Archived ADG Analysis (March 16, 2026):')
        ...
        if current_file.exists():
            ...
            print(f'\n🔍 Comparison:')
            print(f'   Edge ratio: {len(current_data.get("edges", [])) / len(data.get("edges", [])) * 100:.1f}%')
            print(f'   Node ratio: {len(current_data.get("nodes", [])) / len(data.get("nodes", [])) * 100:.1f}%')
        else:
