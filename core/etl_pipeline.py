...
class BackfillPipeline:
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        self.stats["chunks_per_second"] = self.stats["chunks_created"] / duration if duration > 0 else 0
    ...
    async def _extract_from_git(self) -> List[CodeChunk]:
        ...
        for repo_config in self.sources["git_repos"]:
            ...
            try:
                ...
                self.logger.error(f"Failed to extract from Git repo {repo_path}: {e}")
                self.stats["errors"] += 1
    ...
    async def _extract_from_s3(self) -> List[CodeChunk]:
        ...
        for bucket_config in self.sources["s3_buckets"]:
            ...
            try:
                ...
                self.logger.error(f"Failed to extract from S3 bucket {bucket_name}: {e}")
                self.stats["errors"] += 1
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            for chunk in batch:
                try:
                    ...
                    self.logger.error(f"Failed to transform chunk {chunk.name}: {e}")
                    self.stats["errors"] += 1
            
            # Progress logging
            if i % (batch_size * 10) == 0:
                self.logger.info(f"Transformed {i + len(batch)}/{len(chunks)} chunks")
    ...
    async def _load_to_qdrant(self, entries: List[CanonEntry]):
        ...
        for i in range(0, len(entries), batch_size):
            ...
            try:
                ...
                # Progress logging
                if i % (batch_size * 2) == 0:
                    self.logger.info(f"Loaded {i + len(batch)}/{len(entries)} vectors to Qdrant")
...
class ContinuousIngester:
    ...
    async def ingest_success(self, entry: CanonEntry):
        ...
        try:
            ...
            entry.metadata["ingestion_timestamp"] = datetime.utcnow().isoformat()
            ...
            self.logger.error(f"Failed to ingest success pattern {entry.id}: {e}")
            self.stats["errors"] += 1
    ...
    async def ingest_failure(self, entry: CanonEntry, error_trace: str):
        ...
        try:
            ...
            self.logger.error(f"Failed to ingest failure pattern {entry.id}: {e}")
            self.stats["errors"] += 1
    ...
    async def cleanup_expired_failures(self):
        ...
        try:
            ...
            self.logger.info(f"Cleaned up {len(expired)} expired failure patterns")
