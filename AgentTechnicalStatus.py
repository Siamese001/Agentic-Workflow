...
class NuclearAuditor:
    ...
    def _analyze_agent_class(self, class_node: ast.ClassDef, file_path: Path, content: str) -> AgentTechnicalStatus:
        ...
        for node in class_node.body:
            if isinstance(node, ast.FunctionDef):
                ...
                for decorator in node.decorator_list:
                    if isinstance(decorator, ast.Name) and decorator.name in ["abstractmethod", "abc.abstractmethod"]:
                        has_abstract = True
    ...
    def generate_technical_status_table(self) -> str:
        ...
        for status in sorted_agents:
            ...
            table.append(f"| {status.class_name} | {status.layer} | {status.file_path} | "
