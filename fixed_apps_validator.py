#!/usr/bin/env python3
"""
Fixed validation of apps/ folder structure against L5 agentic architecture.
"""
...
def get_actual_apps_structure(apps_path: str) -> Dict:
    """Get the actual apps/ directory structure with fixed walking logic."""
    root = Path(apps_path)
                ...
                for ops_dir in phase_dir.iterdir():
                    ...
                    for general_dir in ops_dir.iterdir():
                        ...
                        for utility_dir in general_dir.iterdir():
                            ...
                            # Fixed: Check if helper-methods-ops exists
                            helper_dir = utility_dir / 'helper-methods-ops'
                            if helper_dir.exists() and helper_dir.is_dir():
                                structure[layer_name][phase_name][ops_name][general_name][utility_name]['helper-methods-ops'] = []
                                
                                # List files
                                for file_path in helper_dir.iterdir():
                                    if file_path.is_file():
                                        structure[layer_name][phase_name][ops_name][general_name][utility_name]['helper-methods-ops'].append(file_path.name)
                            else:
                                # Some paths might not have helper-methods-ops, check for files directly
                                files = []
                                for file_path in utility_dir.iterdir():
                                    if file_path.is_file():
                                        files.append(file_path.name)
                                if files:
                                    structure[layer_name][phase_name][ops_name][general_name][utility_name]['files'] = files
            else:
...
def main():
    ...
    print("🔍 FIXED APPS/ FOLDER STRUCTURE VALIDATION")
    print("=" * 60)
    ...
    sample_files = collect_all_files(actual_structure)[:5]
    for file_path in sample_files:
        print(f"   - {file_path}")
    
    # Validate layers
    print("\n🏗️  Validating layer structure...")
    all_missing = []
    all_extra = []
    all_valid = []
