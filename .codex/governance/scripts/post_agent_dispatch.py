...
Activation: env `POST_AGENT_DISPATCHER=1`.
...
any standalone script entry from `.codex/hooks.json`.
...
#
# Order matches `.codex/hooks.json` post_agent_response chain so the
...
LEGACY_SCRIPTS: list[str] = [
    ...
    # [notion-wave-enforcement-removal] Notion-coupled capture scripts RETIRED — the
    ...
    # [notion-wave-enforcement-removal] post_agent_plan_registration_capture +
    # post_agent_plan_supersession_retire REMOVED (Notion plan-registration enforcement retired).
    # [§32 2026-06-14] post_agent_fortknox_integrity_audit.py REMOVED — Fort Knox arm decommissioned.
]
...
def main() -> int:
    if os.environ.get("POST_AGENT_DISPATCHER") != "1":
