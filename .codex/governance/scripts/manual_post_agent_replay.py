...
-----
    ...
    python .codex/governance/scripts/manual_post_agent_replay.py < response.txt

    # From clipboard (Windows / pyperclip):
    python .codex/governance/scripts/manual_post_agent_replay.py --clipboard

    # From a file:
    python .codex/governance/scripts/manual_post_agent_replay.py --file response.txt

    # Dry-run (show what would run without invoking hooks):
    python .codex/governance/scripts/manual_post_agent_replay.py --file response.txt --dry-run

What it does
------------
Reads the response text, wraps it in the legacy editor payload shape
``{"tool_info": {"response": "..."}}``, and pipes that JSON through
every entry in ``hooks.post_agent_response`` from ``.codex/hooks.json``.
