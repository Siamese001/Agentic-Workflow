...
def _check_scripts_root(rel: str) -> Violation | None:
    ...
    return Violation(
        ...
        message=(
            ...
            f"scripts/proof/. See .codex/rules/ssot-folder-enforcement.md."
        ),
    )
...
def _check_repo_root_py(rel: str) -> Violation | None:
    ...
    return Violation(
        ...
        message=(
            ...
            f"Move to {suggested}. "
            f"See .codex/rules/ssot-folder-enforcement.md."
        ),
    )
        ...
        if rel.endswith(".py"):
            ...
            return Violation(
                ...
                message=(
                    ...
                    f"{suggested}. See .codex/rules/ssot-folder-enforcement.md."
                ),
            )
    return None
...
def _check_hook_script_misroute(rel: str) -> Violation | None:
    """Hook-prefix file landing outside .codex/governance/scripts/."""
    if not rel.endswith(".py"):
        return None
    if rel.startswith(".codex/governance/scripts/"):
        return None
    filename = PurePosixPath(rel).name
    if re.match(r"^(pre|post)_(read|write|run|user_prompt|mcp_tool_use|"
        ...
        return Violation(
            ...
            suggested=".codex/governance/scripts/",
            message=(
                f"Filename '{filename}' looks like a legacy editor hook script "
                f"(pre_*/post_* prefix) but is being written outside "
                f".codex/governance/scripts/. Hook scripts MUST live in "
                f".codex/governance/scripts/ to be discoverable by .codex/hooks.json. "
                f"See .codex/rules/ssot-folder-enforcement.md."
            ),
        )
    return None
