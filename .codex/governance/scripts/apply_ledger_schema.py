...
Reads canonical DDL from .codex/schemas/decision_ledger.schema.sql and applies it.
For existing tables, adds any missing columns via ALTER TABLE ADD COLUMN
(SQLite's only supported non-destructive evolution).

Usage:
    python .codex/governance/scripts/apply_ledger_schema.py           # apply
    python .codex/governance/scripts/apply_ledger_schema.py --check   # dry-run + diff report
    python .codex/governance/scripts/apply_ledger_schema.py --force   # recreate FTS index
...
SCHEMA_PATH = REPO_ROOT / ".codex" / "schemas" / "decision_ledger.schema.sql"
