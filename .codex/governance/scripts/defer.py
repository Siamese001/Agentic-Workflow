...
-----
    ...
    python .codex/governance/scripts/defer.py "DEFERRED_SCOPE: plan=foo-abc123 wave=W1 phase=W1.A layer=L0 fan_in=5 surface=None coverage_gap_pct=10.0 est_tokens=100 reason=test"

    # From stdin (one or more markers, one per line):
    echo "DEFERRED_SCOPE: ..." | python .codex/governance/scripts/defer.py --stdin

    # From a file:
    python .codex/governance/scripts/defer.py --file markers.txt
...
HOOK_SCRIPT = REPO_ROOT / ".codex" / "governance/scripts" / "post_agent_deferred_scope_capture.py"
