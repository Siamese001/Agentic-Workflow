#!/usr/bin/env python3
"""post_agent_mcp_preflight_audit.py — MCP destructive-call health preflight audit.

Companion to the preflight check inside ``pre_mcp_gate.py``. Runs on every
``post_agent_response`` payload. Two responsibilities:
...
``.codex/plans/mcp-destructive-gate-preflight-e9a14b.md`` W1 Phase P2.
"""
