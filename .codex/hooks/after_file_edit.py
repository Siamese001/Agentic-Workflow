...
if file_path.startswith(".codex/plans/_archive/") or "/plans/_archive/" in file_path:
...
def _audit_plan_wave_summary_top(norm_path: str) -> int | None:
    ...
    # .codex/plans/. A plan file is a *.md whose parent dir is named "plans"
    ...
    reason = (
        ...
        "See .codex/rules/plan-location.md."
    )
...
# [notion-wave-enforcement-removal] File-driven Notion plan registration
# (_enqueue_plan_if_eligible) and Notion re-sync (_sync_plan_to_notion_if_registered) REMOVED.
# The windsurf/cursor-era plan-registration / Notion-status pipeline never functioned and is
# retired. The disk-side plan-format validation above (_audit_plan_wave_summary_top) is KEPT —
# it is self-contained and never touches Notion.
