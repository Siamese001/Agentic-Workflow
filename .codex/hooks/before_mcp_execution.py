"""beforeMCPExecution — legacy-token guard + pre_mcp_gate + MCP hygiene (W1.2).

Notion plan-create auditor REMOVED (notion-wave-enforcement-removal): the windsurf/cursor-era
Notion plan/status/wave enforcement is retired.
"""
...
normalized_for_hygiene = normalize_mcp_payload(payload)
hygiene_rc = run_mcp_before_hygiene_stage(normalized_for_hygiene)
if hygiene_rc != 0:
...
write_receipt(
    ...
    payload,
    "allow",
    "MCP request accepted (pre_mcp_gate + mcp_before_hygiene)",
)
