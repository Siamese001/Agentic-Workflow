...
Scope: blocks creation of files matching ``plans/*.md`` or ``.codex/plans/*.md`` (any worktree)
...
plan-mode scratch files under the user-home ``~/.codex`` (e.g. ``~/.codex/plans/<name>.md``) are
...
_PLAN_PATH_RE = re.compile(r"(^|[\\/])(\.codex[\\/])?plans[\\/][^\\/]+\.md$", re.IGNORECASE)
...
def main() -> int:
    try:
        ...
        if "/_archive/" in lowered or "/templates/" in lowered:
            return 0
        # Native Claude Code plan-mode scratch lives under the user-home ``~/.codex``
        # (e.g. ``~/.codex/plans/<name>.md``), never under a repo checkout or worktree.
        ...
        print(
            ...
            "\n"
            "Classify the work first (.codex/rules/work-item-classification.md):\n"
