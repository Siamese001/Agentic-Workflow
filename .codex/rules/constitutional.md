...
27. **Config schema purity.** `.codex/hooks.json` (only `command`+`working_directory`+`show_output`) and `.mcp.json` (only `command`+`args`+`env`+`disabled`) MUST contain ONLY official-schema fields. Unknown keys silently disable a subsystem. Gate MCP fields with `check_mcp_config_schema.py`; validate the hook registry and targets with `verify_codex_primary.py`.
