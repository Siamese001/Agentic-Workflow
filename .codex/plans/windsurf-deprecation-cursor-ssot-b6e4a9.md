...
PLAN_CREATED: slug=windsurf-deprecation-cursor-ssot-b6e4a9 path=.codex/plans/windsurf-deprecation-cursor-ssot-b6e4a9.md status=Not Started
...
| W4 | W4.1-W4.5 | Skills, workflows, hooks, rules retirement | ~14k | legacy editor equivalents exist or can be created as redirect/deprecation stubs | DONE | No active agent procedure requires `.windsurf/skills`, `.windsurf/workflows`, `.windsurf/rules`, or `.codex/hooks.json` |
...
| W1.2 | Update `.codex/rules/**` and `.codex/skills/**` instructions that still point to `.windsurf` as active | DONE |
| W1.3 | Add a guard that blocks new `.windsurf/plans`, `.windsurf/skills`, `.windsurf/workflows`, `.windsurf/rules`, and `.codex/hooks.json` changes | DONE |
...
| W4.1 | Map `.codex/skills/*` to `.codex/skills/*` equivalents and identify gaps | DONE |
...
| W4.3 | Retire `.codex/rules/*` after legacy editor `.mdc` equivalents are verified | DONE |
| W4.4 | Remove `.codex/hooks.json` from active dispatch and migrate any unique hook coverage | DONE |
...
|---------|-----------------------------|-----------------------|
| Agent guidance | `AGENTS.md` + `.codex/rules/*.mdc` | No active instruction role |
| Plans | `.codex/plans/*.md` | Historical archive or deletion |
| Skills | `.codex/skills/*/SKILL.md` | Deleted after gap check or archived as provenance |
| Workflows | `.codex/skills/*` or documented tool procedure | Deleted after migration |
| Hooks | `.codex/hooks.json` + `.codex/governance/scripts/**` | Deleted after coverage parity |
...
| Scripts | `tools/**` or `.codex/governance/scripts/**` | Deleted or compatibility wrapper removed |
...
python ops_scripts/ci/check_plan_format_compliance.py --strict --paths .codex/plans/windsurf-deprecation-cursor-ssot-b6e4a9.md
```
