...
PLAN_CREATED: slug=windsurf-tree-deletion-ci-parity-b8e4f1 path=.codex/plans/windsurf-tree-deletion-ci-parity-b8e4f1.md status=Not Started
...
- **Complication** — Constitutional gates still require `.codex/hooks.json`, `.mcp.json`, `check_windsurf_config_schema.py`, dual-write `artifacts/windsurf/`, and 700+ tests under `tests/**/windsurf/`. Deleting the tree without gate migration breaks CI.
...
| `ops_scripts/ci/check_hook_consolidation.py` | `HOOKS_JSON_PATH` → `.codex/hooks.json` |
...
- `.codex/plans/` — diff vs `.codex/plans/` + `_archive/windsurf_legacy/`; delete windsurf-only dupes.
...
| Gate | Target |
|------|--------|
| `check_windsurf_config_schema.py` | Validate `.codex/hooks.json` + `.mcp.json` only; rename to `check_cursor_config_schema.py` |
...
- [ ] Notion Plans: no `Plan File Path` under `.codex/plans/`
