...
- **Situation.** The repo migrated legacy editor → legacy editor → Claude Code. Claude Code (`AGENTS.md`,
...
- **Complication.** "Deprecate all references" collides with blast radius. Three distinct
  ...
  3. **LIVE Claude Code wiring, historically named** — 26+ `post_cursor_agent_*.py` governance
     ...
     and ~20 `.codex/rules/` files whose prose says "Codex".
...
| P4.1 | Rewrite `.codex/rules/**` prose | 31 rule `.md` (actual; not ~20) | "Codex" → "Claude Code"; markers/identifiers preserved | ~9k | ✅ Done — 58 repl, 0 residual actor-prose |
...
- **P4.1** Rewrite "Codex" → "Claude Code"/"the agent" across active `.codex/rules/**`.
...
| 2 | No "Codex" / "legacy editor" prose in active `.codex/rules/**` or `AGENTS.md` (history docs allowlisted) | Verify: Grep with allowlist |
