...
# legacy editor Decommission — Full Relocation to `.codex/` SSOT

## Context (SCQA)

- **Situation.** legacy editor is no longer used. `AGENTS.md` already declares `.codex/` the
...
from `.cursor/rules|skills` to `.codex/rules|skills`. Delete `.mcp.json` +
