"""
Resume Generation Engine v9.40
...
class BulletProvenance(Enum):
    """Defines the provenance of a bullet point."""
    Verbatim = "Verbatim"
...
ReasoningConfig.K6_IBM_OVERVIEW_CONFIG = ReasoningConfig(
    cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=False
)
ReasoningConfig.K8_EY_BULLETS_CONFIG = ReasoningConfig(
    cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=False
)
ReasoningConfig.K8_EY_OVERVIEW_CONFIG = ReasoningConfig(
    cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=False
)
ReasoningConfig.K9_EARLY_CAREER_BULLETS_CONFIG = ReasoningConfig(
    cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=False
)
ReasoningConfig.K9_EARLY_CAREER_OVERVIEW_CONFIG = ReasoningConfig(
    cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=False
)
...
def reasoning_config_to_api_params(reasoning_config: ReasoningConfig) -> dict:
    ...
    temperature = _get_generation_temperature() # Refactored: intensity param was unused
...
class ClerkExtractor:
    ...
    def _build_experience_sections(self) -> List[Dict]:
        ...
        for exp in self.master_resume.get("professional_experience", []):
            bullets = []
            # Handle both 'bullet_pool' and 'highlights' keys for bullet points
            ...
            experience_sections.append({
                ...
                "bullets": bullets,
                # For backward compatibility, use the same source as bullets
                "highlights": bullet_source
            })
...
"""
                ...
                synthesis_config = genai.GenerationConfig(temperature=0.1, max_output_tokens=api_params["generation_config"].max_output_tokens)
            ...
            for failure in self.previous_failures:
                if failure.rule_id == "VG_WORD_COUNT_K1":
                    ...
                    feedback_instruction = (
                        ...
                        "You MUST generate a new summary that is strictly between 110 and 150 words.\n\n"
...
{archetype_instruction}
        ...
        **NON-NEGOTIABLE REQUIREMENTS (FAILURE WILL CAUSE REJECTION):**
        1.  **WORD COUNT:** The summary MUST be between 120 and 150 words.
        2.  **SENTENCE COUNT:** The summary MUST be 5 to 6 sentences long.
        
        Do not fail these requirements. Summaries with 4 sentences or 100 words will be rejected. Aim for 130-140 words.

        **TASK:**
1. Write a **5-6 sentence** executive summary that strictly adheres to **both** the **120-150 word count** and **5-6 sentence** requirements.
2. Start with a sentence that directly addresses the '{primary_theme}' theme.
3. Weave in at least 4-5 of the non-negotiable keywords: {', '.join(differentiators)}.
4. All claims MUST be supported by the candidate's achievements. Do NOT invent facts.
5. Return ONLY the summary paragraph, with no preamble or explanation.
"""
...
5. **DO NOT** use commas.
6. The headline must be between 8 and 11 words total.
...
"""
        ...
        if "[Placeholder" in headline or not (8 <= word_count <= 11):
    ...
    def _generate_tailored_bullets_for_experience(
        ...
        """
        Generic method to select and reorder bullets for a given experience.
        """
        ...
        # 2. Get key themes and master bullets from the scaffold
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'AI Leadership')
...
**Instructions:**
1. Select the TOP {expected_bullet_count} bullets that best match the job's themes and keywords.
2. Reorder them by relevance (most relevant first).
3. DO NOT modify the bullet text - use exact text from the master list.
4. Return ONLY the {expected_bullet_count} selected bullets, one per line, with no numbers or formatting.
...
"""
        ...
        # 4. Call the LLM API
        system_prompt = f"You are a resume optimization expert for {section_id}. You select and reorder bullets based on job fit. You never modify or invent bullet text - only select from the provided list."
        response_text = self._call_gemini_api(prompt, reasoning_config, section_id, system_prompt)

        # 5. Handle response and fallback
        if "[Placeholder" in response_text:
            logging.warning(f"{section_id} LLM selection failed. Returning first {expected_bullet_count} bullets.")
            fallback_bullets = master_bullets_structured[:expected_bullet_count] # Return as-is
            return fallback_bullets

        selected_bullets_text = [line.strip() for line in response_text.split('\n') if line.strip()]
        
        return self._validate_llm_bullet_selection(
            selected_bullets_text=selected_bullets_text,
            master_bullets_structured=master_bullets_structured,
    ...
    def _generate_k5_unify_bullets(self) -> List[Dict]:
        ...
        return self._generate_tailored_bullets_for_experience(
            company_name="Unify",
            section_index=0, # Corresponds to Unify in master_resume
            expected_bullet_count=7,
...
3. Output must be a single paragraph, approximately {word_count_range[0]}-{word_count_range[1]} words.
4. Return ONLY the rewritten overview text with no preamble or explanation.
"""
        system_prompt = "You are an expert resume editor. You rewrite professional overviews to align with job descriptions, never inventing new facts. All content must be traceable to the source material."
    ...
    def _generate_k6_ibm_bullets(self) -> List[Dict]:
        ...
        # Get the master list from the scaffold first
        master_bullets = self.enriched_scaffold['experience_sections'][1]['bullets']
        provenance_split = {BulletProvenance.Verbatim.value: 2, BulletProvenance.Customized.value: 2, BulletProvenance.Synthetic.value: 2}
        return self._generate_tailored_bullets_for_experience(
            company_name="IBM",
            section_index=1, # Corresponds to IBM in master_resume
            master_bullets_structured=master_bullets,
            prompt_context_label="Master Resume Bullets (IBM)",
            provenance_split=provenance_split,
            expected_bullet_count=6,
    ...
    def _generate_lightly_customized_bullets(
        ...
        source_bullets: List[Dict], # Now expects list of dicts
        reasoning_config: ReasoningConfig, # Added reasoning config
        section_id: str # Use canonical section ID
    ) -> List[Dict]:
        """
        Lightly rewrite a small, fixed list of bullets using an LLM.
        Used for older roles (e.g., EY, Early Career) where selection isn't the goal,
        but light tailoring is.
        """
        ...
        bullets_text = '\n'.join([f"- {b['bullet_text']}" for b in source_bullets]) # Get text from dict

        prompt = f"""You are an expert resume editor. Lightly rewrite the following resume bullets to align with a job's theme.
...
2. Gently emphasize concepts from the original text that align with the '{primary_theme}'.
3. DO NOT invent new facts, skills, metrics, or experience. All claims MUST be derived from the original bullets.
4. Maintain the same number of bullets as the original list.
5. Return ONLY the rewritten bullets, each on a new line, starting with '• '.
"""
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                print(f"Warning: No GEMINI_API_KEY, returning original {section_id} bullets.")
                return [{"text": b, "provenance": BulletProvenance.Verbatim.value} for b in source_bullets]
            
            genai.configure(api_key=api_key)
            model = genai.GenerativeModel('gemini-1.5-flash', system_instruction="You are an expert resume editor. You rewrite resume bullets to align with job themes, never inventing new facts. All content must be traceable to the source material.")
            
            reasoning_config = ReasoningConfig.K8_EY_BULLETS_CONFIG # Re-using EY's config
            api_params = reasoning_config_to_api_params(reasoning_config)
            enhanced_system = enhance_system_prompt_with_reasoning(model.system_instruction, reasoning_config, section_id)
            ...
            rewritten_bullets = [line.replace("• ", "").strip() for line in response.text.strip().split('\n') if line.strip()]

            if len(rewritten_bullets) != len(source_bullets):
                print(f"Warning: {section_id} LLM returned {len(rewritten_bullets)} bullets, expected {len(source_bullets)}. Returning original.")
                return [{"text": b, "provenance": BulletProvenance.Verbatim.value} for b in source_bullets]

            return [{"text": b, "provenance": BulletProvenance.Customized.value, "word_count": count_words_clean(b)} for b in rewritten_bullets]
            
        except Exception as e:
            print(f"Warning: {section_id} LLM bullet generation failed: {e}. Returning original bullets.")
            return [{"text": b, "provenance": BulletProvenance.Verbatim.value} for b in source_bullets]
    ...
    def _generate_k8_ey_bullets(self) -> List[Dict]:
        ...
        # Get highlights as dicts with Verbatim provenance initially
        source_bullet_dicts = [{
            "bullet_text": text, "provenance": BulletProvenance.Verbatim.value
            } for text in ey_exp.get('highlights', [])[:2]
        ]

        return self._generate_lightly_customized_bullets(source_bullet_dicts, ReasoningConfig.K8_EY_BULLETS_CONFIG, ResumeSection.K8_EY_BULLETS.value)

    def _generate_k8_ey_overview(self) -> str:
        """Generate K.8B EY overview, lightly tailored by LLM."""
        ey_exp = next((exp for exp in self.master_resume['professional_experience'] 
                       if 'Ernst & Young' in exp['company'] or 'EY' in exp['company']), None)
        source_overview = ey_exp['overview'] if ey_exp else ""
        ...
        prompt = f"""You are an expert resume writer. Lightly rewrite the following professional overview to align with a job's theme.
...
**Instructions:**
1. Rewrite the overview to be concise (40-60 words).
2. Gently emphasize concepts from the original text that align with the '{primary_theme}'.
...
"""
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                print("Warning: No GEMINI_API_KEY, returning original K.8B overview.")
                return source_overview
            ...
            api_params = reasoning_config_to_api_params(reasoning_config)
            enhanced_system = enhance_system_prompt_with_reasoning(model.system_instruction, reasoning_config, ResumeSection.K8_EY_OVERVIEW.value)
            ...
            print(f"Warning: K.8B LLM generation failed: {e}. Returning original overview.")
            return source_overview
    ...
    def _generate_k9_early_career_bullets(self) -> List[Dict]:
        ...
        return self._generate_lightly_customized_bullets(source_bullets, ResumeSection.K9_EARLY_CAREER_BULLETS.value)
        source_bullet_dicts = [{
             "bullet_text": text, "provenance": BulletProvenance.Verbatim.value
            } for text in early_exp.get('highlights', [])[:1]
        ]

        return self._generate_lightly_customized_bullets(source_bullet_dicts, ReasoningConfig.K9_EARLY_CAREER_BULLETS_CONFIG, ResumeSection.K9_EARLY_CAREER_BULLETS.value)

    def _generate_k9_early_career_overview(self) -> str:
        """Generate K.9B Early Career overview, lightly tailored by LLM."""
        early_exp = next((exp for exp in self.master_resume['professional_experience'] 
                       if 'Early Career' in exp['company']), None)
        source_overview = early_exp['overview'] if early_exp else ""
...
"""
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                print("Warning: No GEMINI_API_KEY, returning original K.9B overview.")
                return source_overview
            ...
            api_params = reasoning_config_to_api_params(reasoning_config)
            enhanced_system = enhance_system_prompt_with_reasoning(model.system_instruction, reasoning_config, ResumeSection.K9_EARLY_CAREER_OVERVIEW.value)
            ...
            print(f"Warning: K.9B LLM generation failed: {e}. Returning original overview.")
            return source_overview
    ...
    def _generate_k10_competencies(self) -> List[Dict]:
        ...
        # Convert master text list to the structured dict format expected by the helper
        ...
        if not all_competencies_structured:
            logging.warning("K.10: No master competencies found in MASTER_RESUME_JSON.")
            return []

**All Available Competencies:**
{comps_text}
        # 2. Define the target split
        provenance_split = {BulletProvenance.Verbatim.value: 2, BulletProvenance.Customized.value: 2, BulletProvenance.Synthetic.value: 2}
...
- Role Level: {role_level}
        ...
        # This will now use the advanced _call_gemini_api with full reasoning
        # and implement the V/T/S split logic.
        return self._generate_tailored_bullets_for_experience(
            master_bullets_structured=all_competencies_structured,
            prompt_context_label="All Available Competencies",
            provenance_split=provenance_split,
            expected_bullet_count=6,
...
- Prioritize competencies that are explicitly mentioned or strongly implied in the job description."""
        ...
        try:
            ...
            if not api_key:
                ...
                fallback_comps = all_competencies_structured[:6] # Return as-is
            ...
            for comp in selected_comps:
                # Use exact matching since the LLM was instructed not to modify text
                if comp in master_comp_texts:
                    original_comp_obj = next((c for c in all_competencies_structured if c['text'] == comp), None)
                    if original_comp_obj:
                        # Append the original object, preserving its Verbatim provenance
                        validated_comps.append(original_comp_obj)
            
            if len(validated_comps) == 6:
...
**Paragraph 1: The Hook - "I Understand Your Core Problem."** (80-100 words)
...
**Paragraph 2: The Proof - "Here's Proof I've Solved This Before."** (90-120 words)
...
**Paragraph 3: The Vision - "Here's How We Can Solve It Together."** (80-100 words)
...
def count_words_clean(text: str) -> int:
    """
    Helper to count words in a clean way, ignoring punctuation.
    This is more accurate than a simple .split() as it handles cases like
    "word." and standalone "--" which would otherwise be miscounted.
    """
...
class PreFlightValidator:
    ...
    RULES_CONFIG = [
        ...
        },
        {
            ...
            "error_message": "K.1: {word_count} words (target: 110-150)"
        },
        {
            ...
            "error_message": "K.1: {sentence_count} sentences (target: 5-6)"
        },
        # --- Distribution Rules ---
        {
        ...
        {
            "rule_id": "VG_K1_DIFFERENTIATOR_KEYWORDS", "severity": ValidationSeverity.CRITICAL, "category": "content",
            "validator": "_validate_k1_differentiators",
            "error_message": "K.1 Summary contains {found}/{total} top differentiator keywords."
        },
        {
            "rule_id": "JD_KEYWORD_ENFORCEMENT", "severity": ValidationSeverity.HIGH, "category": "content",
            "validator": "_validate_jd_keyword_enforcement",
            "error_message": "JD Keyword Enforcement: Found {found}/{total} keywords in resume (min 5 required)."
        },
        {
        ...
        {
            "rule_id": "VG_BULLET_WORD_COUNT_TOLERANCE", "severity": ValidationSeverity.CRITICAL, "category": "word_count",
            "validator": "_validate_bullet_word_count_tolerance",
            "error_message": "Individual bullet word count outside +/-{tolerance:.0%} tolerance: {violations}"
        },
        {
            "rule_id": "COVER_LETTER_JD_RELEVANCE", "severity": ValidationSeverity.HIGH, "category": "content",
            "validator": "_validate_cover_letter_relevance",
            "error_message": "Cover letter relevance to JD is too low. Similarity: {similarity:.2f} (target: >= 0.35)"
        },
    ]
    # Add Headline specific rules
    RULES_CONFIG.extend([
        {
    ...
    def _register_rules(self):
        ...
        # Register declarative rules
        for config in self.RULES_CONFIG:
            validator_func = getattr(self, config["validator"])
            
            # Create a closure for the error message to format it lazily
            def create_error_message_lambda(template):
                return lambda data: template.format(**data.get("error_details", {}))
        ...
        for section in required_sections:
            rule = ValidationRule(
                ...
                severity=ValidationSeverity.CRITICAL,
                validator=lambda d, s=section: d['staging_buffer'].get(s.value) is not None and len(str(d['staging_buffer'].get(s.value))) > 0,
    ...
    def validate(
        ...
        """
        Run all validation gates using the ValidationEngine.
        ...
        data = self._prepare_validation_data(staging_buffer, thematic_analysis, job_description, actual_counts)

        # Execute validation using the registered rules
    ...
    def _prepare_validation_data(
        ...
        thematic_analysis: ThematicAnalysis,
        job_description: str,
        actual_counts: Dict) -> Dict:
        """Prepares a dictionary of data needed by the validation rules."""
        """Prepares a dictionary of data needed by the validation rules."""        
        unify_words = count_words_clean(staging_buffer.get(ResumeSection.K5_UNIFY_OVERVIEW.value, '')) + count_words_in_list_clean(staging_buffer.get(ResumeSection.K5_UNIFY_BULLETS.value, []))
        ibm_words = count_words_clean(staging_buffer.get(ResumeSection.K6_IBM_OVERVIEW.value, '')) + count_words_in_list_clean(staging_buffer.get(ResumeSection.K6_IBM_BULLETS.value, []))
        total_words = actual_counts.get("TOTAL", 1)

        owner_info = self.master_resume.get('owner', {})
        contact_info = owner_info.get('contact', {})

        jd_keywords_tracked = thematic_analysis.competitive_intelligence.differentiator_keywords_raw if hasattr(thematic_analysis, 'competitive_intelligence') else []
        ...
        return {
            ...
            "differentiators": thematic_analysis.competitive_intelligence.get_top_differentiators(3) if hasattr(thematic_analysis, 'competitive_intelligence') else [],
    ...
    # --- Headline Specific Validators (Hardened per PyTest) ---
    def _validate_headline_format_no_titles(self, data: Dict) -> bool:
        ...
        data["error_details"] = {"headline": headline, "forbidden": [t for t in forbidden_titles if t in headline]}
        # Check word boundaries or start of string
        return not any(f" {title} " in f" {headline} " or headline.startswith(title + " ") for title in forbidden_titles)
    ...
    # --- Bullet Word Count Tolerance Validator (From V/T/S Patch) ---
    ...
    def _validate_bullet_word_count_tolerance(self, data: Dict) -> bool:
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            avg_wc = averages.get(section_key)
            if avg_wc is None or avg_wc <= 0:
                logging.debug(f"Skipping bullet tolerance for {section_key}: Invalid average {avg_wc}")
                continue # Skip if average wasn't calculated or is invalid
            ...
            if not isinstance(bullets, list):
                 ...
                 continue # Skip if data format is wrong

            for i, bullet in enumerate(bullets):
                 # Bullets should be dicts with 'word_count' after Artist generation
                if isinstance(bullet, dict):
                    actual_wc = bullet.get('word_count')
                    # Allow None word_count to pass (might be verbatim with no count stored yet)
                    if actual_wc is not None and not (min_wc <= actual_wc <= max_wc):
        ...
        if not all_bullets_valid:
            data["error_details"] = {"violations": violations[:5]} # Report first 5 violations
        return all_bullets_valid

    # --- Provenance Split Validator (From V/T/S Patch) ---
    PROVENANCE_SPLIT_TARGETS = {
        ResumeSection.K5_UNIFY_BULLETS: {BulletProvenance.Verbatim.value: 2, BulletProvenance.Customized.value: 3, BulletProvenance.Synthetic.value: 2},
        ResumeSection.K6_IBM_BULLETS: {BulletProvenance.Verbatim.value: 2, BulletProvenance.Customized.value: 2, BulletProvenance.Synthetic.value: 2},
        ResumeSection.K10_COMPETENCIES: {BulletProvenance.Verbatim.value: 2, BulletProvenance.Customized.value: 2, BulletProvenance.Synthetic.value: 2},
    }
    ...
    def _validate_total_word_count(self, data: Dict) -> bool:
        data["error_details"] = {"total_words": data['total_words'], "min": self.WORD_COUNT_MIN, "max": self.WORD_COUNT_MAX}
        return self.WORD_COUNT_MIN <= data['total_words'] <= self.WORD_COUNT_MAX

    def _validate_k1_word_count(self, data: Dict) -> bool:
        word_count = count_words_clean(data['staging_buffer'].get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, ''))
        data["error_details"] = {"word_count": word_count}
        return 120 <= word_count <= 150

    def _validate_k1_sentence_count(self, data: Dict) -> bool:
        """VG_SENTENCE_COUNT_K1: Validate K.1 Executive Summary sentence count (5-6 sentences)."""
        ...
        data["error_details"] = {"sentence_count": sentence_count}
        return 5 <= sentence_count <= 6
    ...
    def _validate_no_placeholders(self, data: Dict) -> bool:
        ...
        placeholders = re.findall(r'(\[placeholder for.*?\])', buffer_str, re.IGNORECASE)
        if placeholders:
            data["error_details"] = {"placeholders": placeholders[:3]}
            return False
        return True

    def _validate_k1_differentiators(self, data: Dict) -> bool:
        differentiators = data.get('differentiators', [])
        if not differentiators:
            return True # No differentiators to check
        ...
        data["error_details"] = {"found": found_count, "total": len(differentiators)}
        return found_count >= len(differentiators)

    def _validate_jd_keyword_enforcement(self, data: Dict) -> bool:
        found_count = len(data.get('jd_keywords_found', []))
        data["error_details"] = {"found": found_count, "total": len(data.get('jd_keywords_tracked', []))}
    ...
    def _validate_narrative_mining_presence(self, data: Dict) -> bool:
        ...
        return (
            ...
            len(narratives.get("common_problems", [])) > 0
        )

    def _validate_cover_letter_relevance(self, data: Dict) -> bool:
        similarity = data.get('cover_letter_jd_similarity', 0.0)
        data["error_details"] = {"similarity": similarity}
        return similarity >= 0.35

    def _validate_headline_word_count(self, data: Dict) -> bool:
        """VG_HEADLINE_WORD_COUNT: Validate K.0 Headline word count (8-11 words)."""
        headline = data['staging_buffer'].get(ResumeSection.K0_HEADLINE.value, '')
        ...
        return min_wc <= word_count <= max_wc
...
class RenderActionType(Enum):
    """Defines the types of rendering actions for resume sections."""
    SIMPLE_STRING = "simple_string"
    STATIC_HEADER = "static_header"
    EXPERIENCE_SECTION = "experience_section"
    # Corrected typo from previous patch
...
class FileRenderer:
    ...
    # Configuration for the resume rendering process.
    # This list defines the exact order and rendering logic for each section,
    # making the resume structure explicit and easy to modify.
    RESUME_RENDER_CONFIG = [
        {"section": ResumeSection.K0_NAME, "action": RenderActionType.SIMPLE_STRING, "suffix": ""},
        {"section": ResumeSection.K0_HEADLINE, "action": RenderActionType.SIMPLE_STRING, "suffix": ""},
        {"section": ResumeSection.K0_CONTACT, "action": RenderActionTipe.SIMPLE_STRING, "suffix": "\n"},
        {"section": ResumeSection.K0_CONTACT, "action": RenderActionType.SIMPLE_STRING, "suffix": "\n"},
        {"section": ResumeSection.K0_EXECUTIVE_SUMMARY_HEADER, "action": RenderActionType.STATIC_HEADER, "text": "\nEXECUTIVE SUMMARY"},
        {"section": ResumeSection.K1_EXECUTIVE_SUMMARY, "action": RenderActionType.SIMPLE_STRING, "suffix": "\n"},
        {"section": ResumeSection.K0_EXPERIENCE_HEADER, "action": RenderActionType.STATIC_HEADER, "text": "\nPROFESSIONAL EXPERIENCE"},
        {"section": ResumeSection.K5_UNIFY_BULLETS, "action": RenderActionType.EXPERIENCE_SECTION, "overview_key": ResumeSection.K5_UNIFY_OVERVIEW, "master_exp_index": 0},
        {"section": ResumeSection.K6_IBM_BULLETS, "action": RenderActionType.EXPERIENCE_SECTION, "overview_key": ResumeSection.K6_IBM_OVERVIEW, "master_exp_index": 1},
        {"section": ResumeSection.K7_TRADERSENSE_BULLETS, "action": RenderActionType.EXPERIENCE_SECTION, "overview_key": ResumeSection.K7_TRADERSENSE_OVERVIEW, "master_exp_index": 2},
        {"section": ResumeSection.K8_EY_BULLETS, "action": RenderActionType.EXPERIENCE_SECTION, "overview_key": ResumeSection.K8_EY_OVERVIEW, "master_exp_index": 3},
        {"section": ResumeSection.K9_EARLY_CAREER_BULLETS, "action": RenderActionType.EXPERIENCE_SECTION, "overview_key": ResumeSection.K9_EARLY_CAREER_OVERVIEW, "master_exp_index": 4},
        {"section": ResumeSection.K0_COMPETENCIES_HEADER, "action": RenderActionType.STATIC_HEADER, "text": "\nSTRATEGIC & TECHNICAL COMPETENCIES"},
        {"section": ResumeSection.K10_COMPETENCIES, "action": RenderActionType.COMPETENCIES_SECTION},
        {"section": ResumeSection.K0_EDUCATION_HEADER, "action": RenderActionType.STATIC_HEADER, "text": "\nEDUCATION"},
        {"section": ResumeSection.K11_EDUCATION, "action": RenderActionType.EDUCATION_SECTION},
        {"section": ResumeSection.K0_CERTIFICATIONS_HEADER, "action": RenderActionType.STATIC_HEADER, "text": "\nCERTIFICATIONS & CREDENTIALS"},
        {"section": ResumeSection.K12_CERTIFICATIONS, "action": RenderActionType.CERTIFICATIONS_SECTION},
    ]
    ...
    def _render_resume_artifact(self, staging_buffer: ImmutableStagingBuffer, company_name: str, job_title: str) -> Tuple[str, str]:
        ...
        path = f"Resume_{company_name}_{job_title}.md"
        return path, content
    ...
    def _render_skills_artifact(self, staging_buffer: ImmutableStagingBuffer, company_name: str, job_title: str, job_description: str) -> Tuple[str, str]:
        ...
        path = f"Skills_{company_name}_{job_title}.txt"
        return path, content
    ...
    def _render_cover_letter_artifact(self, staging_buffer: ImmutableStagingBuffer, company_name: str, job_title: str) -> Tuple[str, str]:
        ...
        path = f"CoverLetter_{company_name}_{job_title}.txt"
        return path, content
    ...
    def _render_qa_report_artifact(self, company_name: str, job_title: str) -> Tuple[str, str]:
        ...
        path = f"QA_Report_{company_name}_{job_title}.md"
        # Content is generated in HOP-8, so we return an empty string here.
        # The orchestrator will populate it.
        return path, ""

    def _render_app_tracker_artifact(self, company_name: str, job_title: str, file_paths: Dict[str, str]) -> Tuple[str, str]:
        ...
        try:
            ...
            validation_result = validator.validate_tracker_data([app_tracker_data])
            if "BLOCKED" in validation_result.get("result", ""):
                print(f"⚠️  App Tracker validation failed: {validation_result.get('failure_histogram_by_rule', 'Unknown error')}")
        except Exception as e:
            print(f"Warning: App tracker validation failed during execution: {e}")
            
        content = json.dumps(app_tracker_data, indent=2)
        path = f"AppTracker_{company_name}_{job_title}.json"
        return path, content
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        This refactored method is data-driven, making the layout easy to change.
        """
        output_lines = []
        ...
        for config in self.RESUME_RENDER_CONFIG:
            ...
            value = staging_buffer.get(config["section"].value)

            if value is None and action not in [RenderActionType.STATIC_HEADER]:
                continue # Skip empty sections unless they are static headers

            if action == RenderActionType.SIMPLE_STRING:
                output_lines.append(f"{value}{config.get('suffix', '')}")
            elif action == RenderActionType.STATIC_HEADER:
                output_lines.append(config["text"])
            elif action == RenderActionType.EXPERIENCE_SECTION:
                master_exp_index = config["master_exp_index"]
                if master_exp_index < len(master_experience):
                    master_exp_data = master_experience[master_exp_index]
                    overview_text = staging_buffer.get(config["overview_key"].value)
                    output_lines.append(self._render_experience_section(master_exp_data, overview_text, value))
            elif action == RenderActionType.COMPETENCIES_SECTION:
                output_lines.append(self._render_competencies_section(value))
            elif action == RenderActionType.EDUCATION_SECTION:
                output_lines.append(self._render_education_section(value))
            elif action == RenderActionType.CERTIFICATIONS_SECTION:
                output_lines.append(self._render_certifications_section(value))

        return "\n".join(output_lines)

    def _render_experience_section(self, master_exp: Dict, overview: str, bullets: List[Any]) -> str:
        ...
        for bullet in bullets:
            if isinstance(bullet, dict):
                bullet_lines.append(f"• {bullet.get('bullet_text', str(bullet))}")
            else: # Fallback for simple string lists
                bullet_lines.append(f"• {str(bullet)}")
        
        lines.append("\n\n".join(bullet_lines)) # Join bullets with \n\n
        
        return "\n".join(lines) + "\n\n" # Join header/overview/bullets with \n, add \n\n at end
    ...
    def _render_certifications_section(self, certifications_list: List[str]) -> str:
        ...
        lines.extend([f"• {cert}" for cert in certifications_list])
        return "\n\n".join(lines) + "\n\n"
    ...
    def _render_competencies_section(self, competencies_list: List[Dict]) -> str:
        ...
        return "\n\n".join([f"• {c.get('text', c.get('bullet_text', str(c)))}" for c in competencies_list]) + "\n\n"
    ...
    def _render_app_tracker(
        ...
        tracker['Versioned Resume'] = f"{candidate_name}_Resume_{company_name}_{job_title}"
...
class WorkflowOrchestrator:
    def __init__(self, master_resume: Dict):
        ...
        # v9.11: Store multiple overview similarity results
        # v9.11: Store exec summary similarity results
    ...
    def _execute_hop_3_artist_generation(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        for attempt in range(1, 6):
            ...
            validation_results, all_passed = validator.validate(temp_buffer, thematic_analysis, job_description)
            
            if all_passed or attempt == 5:
                # If it's the last attempt and it *still* failed, raise an error to halt the workflow.
                if not all_passed and attempt == 5:
                    print(f"  ✗ HOP-3 FAILED: Validation failed after {attempt} attempts. Halting.")
                    # We must manually add the final failed validation results to the orchestrator
                    # so the QA report can see them before we halt.
                    self.validation_results.extend(feedback_results or [])
    ...
    def execute_workflow(
        ...
        v8.40: Consolidated workflow execution into individual hop methods.
    ...
    def _invoke_deduplication_analysis(self, staging_buffer: ImmutableStagingBuffer) -> bool:
        ...
        v9.11: Expanded to calculate overview-bullet similarity for all 4 sections.
    ...
    def _format_ascii_bar_chart(self, label: str, value: float, target: float, temperature: Optional[float] = None, max_value: float = 1.0, width: int = 30) -> str:
        ...
        return f"{label:<25} [{bar:<{width}}] {value:.1%} πιστός {temp_str} (Tgt:{target:.0%}) {status}"
    ...
    def _build_qa_section_1_signal_quality(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis) -> List[str]:
        ...
        section_configs = {
            "Executive Summary": (ResumeSection.K1_EXECUTIVE_SUMMARY, 0.80, 0.25, ReasoningConfig.K1_EXECUTIVE_SUMMARY_CONFIG),
            "Unify Overview": (ResumeSection.K5_UNIFY_OVERVIEW, 0.60, 0.05, ReasoningConfig.K5_UNIFY_OVERVIEW_CONFIG),
            "Unify Bullets": (ResumeSection.K5_UNIFY_BULLETS, 0.70, 0.20, ReasoningConfig.K5_UNIFY_BULLETS_CONFIG),
            "IBM Overview": (ResumeSection.K6_IBM_OVERVIEW, 0.60, 0.05, ReasoningConfig.K6_IBM_OVERVIEW_CONFIG),
            "IBM Bullets": (ResumeSection.K6_IBM_BULLETS, 0.70, 0.20, ReasoningConfig.K6_IBM_BULLETS_CONFIG),
            "EY Overview": (ResumeSection.K8_EY_OVERVIEW, 0.50, 0.025, ReasoningConfig.K8_EY_OVERVIEW_CONFIG),
            "EY Bullets": (ResumeSection.K8_EY_BULLETS, 0.50, 0.025, ReasoningConfig.K8_EY_BULLETS_CONFIG),
            "Early Career Overview": (ResumeSection.K9_EARLY_CAREER_OVERVIEW, 0.50, 0.025, ReasoningConfig.K9_EARLY_CAREER_OVERVIEW_CONFIG),
            "Early Career Bullets": (ResumeSection.K9_EARLY_CAREER_BULLETS, 0.50, 0.025, ReasoningConfig.K9_EARLY_CAREER_BULLETS_CONFIG),
            "Competencies": (ResumeSection.K10_COMPETENCIES, 0.85, 0.15, ReasoningConfig.K10_COMPETENCIES_CONFIG),
        }
        ...
        for label, (section_enum, target_score, weight, reasoning_config) in section_configs.items():
            ...
            if content:
                ...
                if sc_count > 1:
                    ...
                    # synthesis step, which is hard-coded to 0.1
                    temp = 0.1
                else:
                ...
                # Call the bar chart formatter directly instead of appending to rows
                # v9.11: Pass the calculated temperature
                lines.append(self._format_ascii_bar_chart(
                    label=label, value=score, target=target_score, temperature=temp
                ))
            else:
                # Optionally show skipped sections differently or omit them
                lines.append(f"{label:<25} [SKIPPED]{' '*23} (Target: {target_score:.0%}) -")

        if total_weight > 0:
            average_signal = total_weighted_score / total_weight
            # Recalculate overall_target considering only non-skipped sections
            overall_target_numerator = sum(target * w for _, (section_enum, target, w, _) in section_configs.items() if staging_buffer.get(section_enum.value) is not None)
            actual_total_weight = sum(w for _, (section_enum, target, w, _) in section_configs.items() if staging_buffer.get(section_enum.value) is not None)
            ...
            # v9.11: Temperature doesn't apply to the overall score
            summary_bar = self._format_ascii_bar_chart(
                label="Total Weighted Score", value=average_signal, target=overall_target, temperature=None
            )
    ...
    def _build_qa_section_3_hop_summary(self) -> List[str]:
        ...
        for hop in self.hop_checkpoints:
            try:
                start = datetime.fromisoformat(hop.timestamp_start)
                end = datetime.fromisoformat(hop.timestamp_end)
                duration = (end - start).total_seconds()
            except (ValueError, TypeError):
                duration = -1.0  # Indicate an error in parsing

            rows.append([
                hop.hop_id, hop.hop_name, hop.status.value,
                f"{duration:.3f}", hop.output_hash or "N/A"
            ])
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines
    ...
    def _build_qa_section_4_distribution(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        for vr in dist_results:
            rows.append([vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message if not vr.passed else ""])
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines
    ...
    def _calculate_master_avg_bullet_length(self) -> Dict[str, float]:
        ...
        # For Unify and IBM experience sections
        experience_sections_to_avg = {
            "Unify": "Unify Consulting",
            "IBM": "IBM",
        }
        for key, company_name_part in experience_sections_to_avg.items():
            total_words = 0
            bullet_count = 0
            exp = next((e for e in self.master_resume.get("professional_experience", []) if company_name_part in e.get("company", "")), None)
            if exp:
                bullets = exp.get("bullet_pool", []) # Assuming bullet_pool holds the raw bullet strings
                total_words = sum(count_words_clean(b) for b in bullets)
                bullet_count = len(bullets)
            avg_lengths[key] = (total_words / bullet_count) if bullet_count > 0 else 25.0 # Default if no bullets
        
        # For Competencies section
        competencies_list = self.master_resume.get("strategic_and_technical_competencies", [])
        avg_lengths["Competencies"] = (sum(count_words_clean(b) for b in competencies_list) / len(competencies_list)) if competencies_list else 28.0 # Default for competencies
    ...
    def _build_qa_section_6_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        headers = ["Section", "Bullet #", "Provenance", "Word Count", "Target Range", "Status", "Text Snippet"]
        rows = []
        master_avg_lengths = self._calculate_master_avg_bullet_length() # Pre-calculate averages
        provenance_sections = { # Define sections to include here
        ...
        for name, section_enum in provenance_sections.items():
            ...
            if isinstance(bullets, list) and bullets and isinstance(bullets[0], dict):
                for i, bullet_data in enumerate(bullets):
                    word_count = bullet_data.get('word_count', 0)
                    avg_len = master_avg_lengths.get(name, 25.0) # Default if not found
                    tolerance = 0.20 # 20% tolerance as implied by older SECTION_CONSTRAINTS_V521
                    ...
                    rows.append([
                        name, str(i + 1), bullet_data.get('provenance', 'N/A'),
                        str(word_count),
                        target_range_str, # Added
                        status, # Added
    ...
    def _build_qa_section_8_overview_similarity(self) -> List[str]:
        """Builds Section 8 of the QA report: Overview vs. Bullet Similarity."""
        lines = ["", "8. OVERVIEW VS. BULLET SIMILARITY CHECK", ""]
    ...
    def _build_qa_section_5_word_count(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        # Map Rule IDs to display names and target ranges
        rule_map = {
            "VG_TOTAL_WORD_COUNT": ("Total Resume", f"{PreFlightValidator.WORD_COUNT_MIN}-{PreFlightValidator.WORD_COUNT_MAX}"),
            "VG_WORD_COUNT_K1": ("K.1 Exec Summary", "110-150") # Hardcoded as per prompt
        }
    ...
    def _build_qa_section_13_prod_readiness(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        if not prod_ready:
            ...
            if critical_failures:
                ...
                for f in critical_failures[:3]: lines.append(f"    - {f.rule_id}: {f.message}")
            if high_failures:
                lines.append("  HIGH FAILURES:")
                for f in high_failures[:3]: lines.append(f"    - {f.rule_id}: {f.message}")
        return lines
    ...
    def _build_qa_section_14_cover_letter(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        headers = ["Rule ID", "Status", "Message"]
        rows = [[vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message if not vr.passed else ""] for vr in cl_results] if cl_results else [["N/A", "INFO", "No cover letter validation results found."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines

    def _build_qa_section_15_jd_enforcement(self) -> List[str]:
        """Builds Section 15 of the QA report: JD Enforcement Validation."""
        lines = ["", "15. JD ENFORCEMENT VALIDATION", ""]
    ...
    # Each dictionary defines a section, the method to build it, and the arguments it requires.
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_9_exec_summary_overlap", "args": []},
        {"method": "_build_qa_section_10_bullet_deduplication", "args": []},
    ...
    def _generate_qa_report( # Renamed for clarity, already modular
        ...
        This orchestrator pattern makes the report structure explicit and easy to modify.
        NOTE: This method already implements the requested modular structure.
        """
        validation_results_out = []
        # v9.11: Removed ==== framing lines
        report_lines = [
        ...
        for section_config in self.QA_REPORT_SECTIONS:
            ...
            try:
                ...
                report_lines.extend(builder_method(*call_args))
