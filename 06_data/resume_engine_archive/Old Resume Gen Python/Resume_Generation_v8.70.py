...
class ArtistGenerator:
    ...
    def _call_gemini_api(self, prompt: str, reasoning_config: ReasoningConfig, section_id: str, system_prompt: str) -> str:
        ...
        try:
            ...
            if sc_count > 1:
                # --- TRUE SELF-CONSISTENCY (MULTI-CALL) ---
                logging.info(f"  Running Self-Consistency for {section_id} (x{sc_count} samples)...")
                ...
                for i in range(sc_count):
                    try:
                        ...
                        candidate_responses.append(response.text.strip())
                    except Exception as e:
                        logging.warning(f"    SC sample {i+1} failed: {e}")
...
class WorkflowOrchestrator:
    ...
    def _execute_hop_3_artist_generation_with_retry(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # Instantiate ArtistGenerator inside the loop with all required context
            artist = ArtistGenerator(
                self.master_resume, enriched_scaffold, job_description, thematic_analysis, feedback_results
            )
            artist_output_raw, current_hop_results = artist.generate(
                feedback_results=feedback_results, attempt=attempt
            )
            hop3_results.extend(current_hop_results)
