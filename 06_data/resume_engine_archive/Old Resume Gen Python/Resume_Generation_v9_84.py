...
import time
import requests
from typing import Dict, List, Tuple, Optional, Any, Set, Union, ClassVar
...
__version__ = "9.83" # Consolidated refactoring of configuration and data-driven components.
# v9.79: Final Hardening: Corrected Competencies formatting and fixed QA Table 16 rendering.
try:
...
class ContentConstraintsConfig:
    ...
    # Overall Resume
    TOTAL_WORD_COUNT_MIN: int = 899
    TOTAL_WORD_COUNT_MAX: int = 999
...
"""
    ...
    def _synthesize_hybrid_analysis(
        ...
        if partial_result.phase1_success:
            ...
            # <<< FIX START >>>
            # Replace ThematicTheme(...) with dictionary literals
            primary_theme = {
        ...
        # Synthesize
        return ThematicAnalysis(  # type: ignore
...
"""
        ...
        # This call assumes you have added _calculate_master_avg_bullet_length to the ArtistGenerator class
...
{bullets_text}  # Use formatted text
...
"""
            ...
            if "[Placeholder" in response_text:
                ...
                if len(rewritten_bullets) != len(source_bullets_text):
                    print(
                        f"Warning: {section_id} LLM returned {len(rewritten_bullets)} bullets, expected {len(source_bullets_text)}. Returning original.")
                    return [{"text": b, "provenance": BulletProvenance.Verbatim.value,
                             "word_count": count_words_clean(b)} for b in source_bullets_text]

                # Return structured dicts
                return [{"text": b, "provenance": provenance, "word_count": count_words_clean(b)} for b in
                        rewritten_bullets]

        except Exception as e:
            logging.warning(f"{section_id} LLM bullet rewrite failed: {e}. Returning original bullets.",
                            exc_info=True)
            return [{"text": b, "provenance": BulletProvenance.Verbatim.value,
                     "word_count": count_words_clean(b)} for b in source_bullets_text]
    ...
    def _generate_synthetic_bullets(
        ...
        differentiators = thematic_analysis.competitive_intelligence.get_top_differentiators(5) if hasattr(
            thematic_analysis, 'competitive_intelligence') else []
...
4. Maintain a professional tone. Aim for conciseness (around {self.constraints.EY_OVERVIEW_WORD_COUNT_MIN}-{self.constraints.EY_OVERVIEW_WORD_COUNT_MAX} words), but prioritize thematic relevance.
...
2. Gently emphasize concepts from the original text that align with the '{primary_theme}'.
3. DO NOT invent new facts, skills, metrics, or experience. All claims MUST be derived from the original overview.
4. Maintain a professional tone. Aim for conciseness (around {self.constraints.EARLY_CAREER_OVERVIEW_WORD_COUNT_MIN}-{self.constraints.EARLY_CAREER_OVERVIEW_WORD_COUNT_MAX} words), but prioritize thematic relevance.
...
# <<< FIX: MOVE HELPER FUNCTION DEFINITIONS HERE >>>
def count_words_clean(text: str) -> int:
    """
    Helper to count words using standard splitting.
    v9.81: Corrected bug where regex \b\w+\b overcounted hyphenated words
    (e.g., "time-to-production" was 3 words) and symbols (e.g., "$32M" was 2 words).
    This simple split() method aligns with user expectations and standard word processors.
    """
    if not text: return 0
    # Use simple, intuitive space splitting. This matches the user's 94-word count.
...
class PreFlightValidator:
    ...
    # Removed WORD_COUNT_MIN/MAX, now sourced from self.constraints

    RULES_CONFIG = [
        # --- Word Count Rules ---
        {
            "rule_id": "VG_TOTAL_WORD_COUNT", "severity": ValidationSeverity.CRITICAL, "category": "word_count",
            "validator": "_validate_total_word_count", # <<< FIX: Method Name >>>
        ...
        },
        {
            ...
            "validator": "_validate_k1_word_count", # <<< FIX: Method Name >>>
            "error_message": "K.1: {word_count} words (target: {min}-{max})" # <<< FIX: Use min/max
        },
        {
            "rule_id": "VG_SENTENCE_COUNT_K1", "severity": ValidationSeverity.CRITICAL, "category": "structure",
            "validator": "_validate_k1_sentence_count", # <<< FIX: Method Name >>>
            "error_message": "K.1: {sentence_count} sentences (target: {min}-{max})" # <<< FIX: Use min/max
        },
        # --- Distribution Rules ---
        {
        ...
        },
        {
            ...
            "error_message": "Cover letter relevance to JD is too low. Similarity: {similarity:.2f} (target: >= {min_sim})" # <<< FIX: Use min_sim
        },
        {
        ...
        },
        {
            ...
            "error_message": "Cover letter paragraph word counts are out of spec. P1: {p1_wc} ({p1_min}-{p1_max}), P2: {p2_wc} ({p2_min}-{p2_max}), P3: {p3_wc} ({p3_min}-{p3_max})" # <<< FIX: Use min/max
        },
    ...
    RULES_CONFIG.extend([
        {
            ...
            "validator": "_validate_headline_word_count", # <<< FIX: Method Name >>>
        ...
        # <<< FIX: ADD THIS RULE BACK >>>
        # --- Bullet Word Count Tolerance ---
        {
    ...
    def _register_rules(self):
        ...
        for config in self.RULES_CONFIG:
            # <<< FIX: Check if validator is callable (lambda) or string (method name) >>>
            validator_ref = config["validator"]
            if isinstance(validator_ref, str):
                # It's a method name, get the actual method
                validator_func = getattr(self, validator_ref)
            elif callable(validator_ref):
                 # It's already a callable (potentially a lambda, though we're removing those needing 'self')
            ...
            def create_error_message_lambda(template):
                ...
                if "tolerance" in expected_args:
                    # <<< FIX: Use self.BULLET_WORD_COUNT_TOLERANCE >>>
            ...
            rule = ValidationRule(
                ...
                validator=validator_func, # Use the resolved function/method
    ...
    def validate(
        ...
        data = self._prepare_validation_data(staging_buffer, thematic_analysis, job_description, actual_counts, section_averages) # Pass section_averages
    ...
    # <<< FIX: Method name typo and add section_averages parameter >>>
    ...
    def _validate_cover_letter_relevance(self, data: Dict) -> bool:
        ...
        data["error_details"] = {
            ...
            "min_sim": min_sim # Pass min_sim for the error message
        }
    ...
    def _validate_cover_letter_structure(self, data: Dict) -> bool:
        ...
        if len(paragraphs) < 5: # Not enough paragraphs to check
            data["error_details"] = {"p1_wc": 0, "p2_wc": 0, "p3_wc": 0, "p1_min":0, "p1_max":0, "p2_min":0, "p2_max":0, "p3_min":0, "p3_max":0} # Add defaults
            return False
        ...
        # Pass min/max values for error message formatting
        data["error_details"] = {
    ...
    def _validate_headline_word_count(self, data: Dict) -> bool:
        ...
        data["error_details"] = {
            "word_count": word_count,
            "min": self.constraints.HEADLINE_WORD_COUNT_MIN, # Use self.constraints
            "max": self.constraints.HEADLINE_WORD_COUNT_MAX, # Use self.constraints
            "headline": headline
        }
    ...
    # <<< FIX: ADD THIS METHOD (if it wasn't already added from previous instructions) >>>
    def _validate_bullet_word_count_tolerance(self, data: Dict) -> bool:
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK: # Iterate using the updated list
            ...
            # <<< FIX: Determine avg key based on section >>>
            ...
            # Add keys for EY and EarlyCareer if they have specific averages calculated
            ...
            else: avg_key = "Unify" # Use a default like Unify if specific key not found

            avg_wc = averages.get(avg_key) # Use the determined key
            ...
            for i, bullet in enumerate(bullets):
                ...
                if isinstance(bullet, dict):
                    ...
                    bullet_text = bullet.get('text', '') # Get text for logging
                    if actual_wc is None: # If word_count wasn't pre-calculated, calculate it now
                        actual_wc = count_words_clean(bullet_text)
    ...
    def _calculate_master_avg_bullet_length(self) -> Dict[str, float]:
        ...
        # <<< FIX: Add default averages for EY and Early Career if needed by _validate_bullet_word_count_tolerance >>>
        # These sections often have fewer bullets, so using a general default might be fine
        avg_lengths["EY"] = avg_lengths.get("EY", 25.0) # Example default
        avg_lengths["EarlyCareer"] = avg_lengths.get("EarlyCareer", 25.0) # Example default
...
class FileRenderer:
    ...
    RESUME_RENDER_CONFIG = [
        {"type": "simple", "source": ResumeSection.K0_NAME, "prefix": "## ", "suffix": "\n\n"},
        {"type": "simple", "source": ResumeSection.K0_HEADLINE, "suffix": "\n\n"},
        {"type": "simple", "source": ResumeSection.K0_CONTACT, "suffix": "\n\n"},
        {"type": "header", "text": "## EXECUTIVE SUMMARY"},
        {"type": "simple", "source": ResumeSection.K1_EXECUTIVE_SUMMARY, "suffix": "\n"},
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        for config in self.RESUME_RENDER_CONFIG:
            ...
            if render_type == "header":
                ...
                if content:
                    line = f'{config.get("prefix", "")}{content}{config.get("suffix", "")}'
    ...
    def _render_education_section(self, education_list: List[Dict]) -> str:
        """Helper to render the education section."""
        lines = []
    ...
    def _render_certifications_section(self, certifications_list: List[str]) -> str:
        """Helper to render the certifications section."""
...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_6_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        lines.append("```markdown")
        # Renamed "Bullet #" to "Item" to accommodate overviews
        ...
        for section_data in self.QA_PROVENANCE_SECTIONS:
            ...
            if section_data.get("overview_enum"):
                ...
                if overview_text:
                    word_count = count_words_clean(overview_text)
            ...
            if isinstance(bullets, list) and bullets:
                for i, bullet_item in enumerate(bullets):
                    if isinstance(bullet_item, dict):
                        ...
                        word_count = bullet_item.get('word_count', count_words_clean(bullet_text))
                        provenance = bullet_item.get('provenance', 'N/A')
                    else: # Fallback for simple string list (should not happen)
                        bullet_text = str(bullet_item)
                        word_count = count_words_clean(bullet_text)
                        provenance = "N/A" # Unknown provenance
    ...
    def _build_qa_section_9_pairwise_similarity(self) -> List[str]:
        ...
        # Data is in self.similarity_matrix_data['duplicates_found']
        duplicates = self.similarity_matrix_data.get('duplicates_found', [])
        
        if not duplicates:
            rows.append(["N/A", "N/A", f"<{0.90:.2f}", "✓ PASS"])
        else:
...
class ResumeSection(Enum):
    ...
    K10_COMPETENCIES = "K.10_COMPETENCIES"
    # Non-resume sections
    K2_SKILLS = "K.2_SKILLS" # For rendering
    K13_COVER_LETTER = "K.13_COVER_LETTER" # For rendering
