...
__version__ = "9.86" # Consolidated refactoring of configuration and data-driven components.
try:
...
class ContentConstraintsConfig:
    ...
    # Overall Resume
    TOTAL_WORD_COUNT_MIN: int = 929
    TOTAL_WORD_COUNT_MAX: int = 1029
...
"""
    ...
    def _synthesize_hybrid_analysis(
        ...
        # Initialize with local analysis data
        ...
        retrieval_sources = [] # Start fresh for sources

        # Overwrite with successful web RAG phase data
        if partial_result.phase1_success:
            phase1 = partial_result.phase1_result
            primary_theme = { # Use dict literal
                "name": phase1["thematic_analysis"]["primary_theme"].get("name", "N/A"),
                "confidence": phase1["thematic_analysis"]["primary_theme"].get("confidence", 0.0),
                "keywords": phase1["thematic_analysis"]["primary_theme"].get("keywords", []),
            ...
            secondary_themes = [
                ...
                for t in phase1["thematic_analysis"].get("secondary_themes", [])[:5]
            ]
            role_classification = phase1.get("role_classification", local_analysis.role_classification) # Fallback just in case
            logger.info("Using Phase 1 web data")
            retrieval_sources.append(RetrievalSource("PHASE1_THEMATIC", "Web_RAG", 1.0, "SUCCESS"))
        else:
            logger.info("Using local NLP for thematic data")
            retrieval_sources.append(RetrievalSource("PHASE1_THEMATIC", "Local_NLP", 0.5, "FALLBACK"))

        if partial_result.phase2_success:
            phase2 = partial_result.phase2_result
            patterns_data = phase2.get("authenticity_patterns", {})
            confidence_data = phase2.get("pattern_confidence", {})
            authenticity_patterns = {
                "status": "STRONG" if confidence_data.get("overall", 0.0) > 0.7 else "MODERATE",
                # Ensure we get the nested patterns correctly
                "patterns": patterns_data.get("executive_summary_patterns", []),
                "fallback_applied": False,
                "fallback_reason": None
            }
            logger.info("Using Phase 2 web data")
            retrieval_sources.append(RetrievalSource("PHASE2_AUTHENTICITY", "Web_RAG", 1.0, "SUCCESS"))
        else:
            logger.info("Using local NLP for authenticity patterns")
            retrieval_sources.append(RetrievalSource("PHASE2_AUTHENTICITY", "Local_NLP", 0.5, "FALLBACK"))

        if partial_result.phase3_success:
            phase3 = partial_result.phase3_result
            comp_analysis = phase3.get("competitive_analysis", {})
            search_summary = phase3.get("search_summary", {})
            differentiator_kws = comp_analysis.get("differentiator_keywords", [])
            competitive_intel = CompetitiveIntelligence(
                peer_jds_analyzed_count=search_summary.get("peer_jds_analyzed", 0),
                differentiator_keywords=[kw.get("keyword") for kw in differentiator_kws if kw.get("keyword")],
                differentiator_keywords_raw=[kw.get("keyword") for kw in differentiator_kws if kw.get("keyword")],
                differentiator_keywords_weighted=differentiator_kws
            )
            logger.info("Using Phase 3 web data")
            retrieval_sources.append(RetrievalSource("PHASE3_COMPETITIVE", "Web_RAG", 1.0, "SUCCESS"))
        else:
            logger.info("Using local NLP for competitive intel")
            retrieval_sources.append(RetrievalSource("PHASE3_COMPETITIVE", "Local_NLP", 0.5, "FALLBACK"))

        # Add Phase 4 source info if applicable (even if data isn't directly used here)
        if partial_result.phase4_success:
             retrieval_sources.append(RetrievalSource("PHASE4_NARRATIVE", "Web_RAG", 1.0, "SUCCESS"))
        else:
             retrieval_sources.append(RetrievalSource("PHASE4_NARRATIVE", "Web_RAG", 0.0, "FAILED"))


        # Synthesize using the potentially overwritten variables
        return ThematicAnalysis(
            primary_theme=primary_theme,
            secondary_themes=secondary_themes,
            role_classification=role_classification,
            positioning_directives=local_analysis.positioning_directives, # Keep local positioning for hybrid
            authenticity_patterns=authenticity_patterns,
            competitive_intelligence=competitive_intel,
            problem_solution_narratives=partial_result.phase4_result.get("problem_solution_narratives") if partial_result.phase4_success else None, # Use if available
            signal_quality_score=partial_result.success_rate,
            retrieval_method="HYBRID_PARTIAL_WEB",
            retrieval_sources=retrieval_sources,
            weighting_formula=local_analysis.weighting_formula # Keep local weighting for hybrid
        )
...
class PreFlightValidator:
    ...
    RULES_CONFIG = [
        ...
        },
        {
            ...
            "error_message": "Cover letter relevance to JD is too low. Similarity: {similarity:.2f} (target: >= {min_sim})" # Use min_sim
        },
        {
        ...
        # NOTE: Headline rules moved inside extend below
    ]
    ...
    def validate(
        ...
        # Count words for each section in the buffer
    ...
    def _validate_k1_differentiators(self, data: Dict) -> bool:
        ...
        # Requirement is >= 3 (not all of them necessarily)
    ...
    def _validate_bullet_word_count_tolerance(self, data: Dict) -> bool:
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            # Determine the correct key for average length lookup
            avg_key = "Default"
            ...
            else: avg_key = "Unify" # Fallback, though should cover all checked sections
            ...
            for i, bullet in enumerate(bullets):
                if isinstance(bullet, dict):
                    ...
                    if actual_wc is None:
                        actual_wc = count_words_clean(bullet_text) # Calculate if missing
                    ...
                    if not (min_wc <= actual_wc <= max_wc):
                     ...
                     # Handle case where bullet might be a string (e.g., K7)
                     if section_key not in [ResumeSection.K7_TRADERSENSE_BULLETS.value]: # Skip K7 check
...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_16_final_format(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        # List of all known formatting rules. This method will report on any
        ...
        # Create a set of found rules for easy lookup
        found_rules = {vr.rule_id for vr in format_results}

        headers = ["Rule ID", "Status", "Message"]
        rows = []

        # Iterate through the *target* list to ensure order and catch missing rules
