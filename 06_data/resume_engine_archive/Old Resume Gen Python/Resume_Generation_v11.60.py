...
"""
    ...
    def _analyze_with_resilient_web_search(
        ...
        """
        v11.50 Final Fix: Added JSON roundtrip conversion before synthesis & caching.
        """
        import logging
        logger = logging.getLogger(__name__)
        ...
        if partial_result.full_success:
            ...
            # --- START FINAL FIX: JSON Roundtrip ---
                ...
                try:
                    def default_serializer(o):
                        ...
                        if hasattr(o, '__dataclass_fields__'):
                            return asdict(o)
                        # Handle other non-serializable objects (like Field) by coercing to string
                        # This is a lossy but effective way to break object references
                        try:
                            json.dumps(o) # Check if serializable
                            return o # If it is, return it
                        except TypeError:
                            # If not serializable, convert to string
                            return f"__CONVERTED_STR__{str(o)}__"
                    
                    # Use the default serializer to handle complex types
                    ...
                    # Try a simpler, more aggressive conversion on failure
                    try:
                         simple_str = json.dumps(str(data)) # Just dump the string representation
        ...
        try:
            # We need to convert the *final* 'analysis' object (which is a dataclass)
            # using the same robust roundtrip method.
            if analysis:
