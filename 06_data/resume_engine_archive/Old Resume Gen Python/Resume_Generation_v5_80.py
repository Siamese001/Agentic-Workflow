...
class EnhancedJobDescriptionAnalyzer:
    ...
    def __init__(
        ...
        if self.enable_web_search and self.api_key:
            try:
                self.web_client = GeminiWebSearchClient(self.api_key, self.config)
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict):
        ...
        if not os.environ.get("GEMINI_API_KEY"):
            ...
            print("✓ GEMINI_API_KEY detected - Gemini API integration enabled")
    ...
    def execute_workflow(
        ...
        print("=" * 80)
        print("RESUME GENERATION ENGINE v5.80 - GEMINI API")
        print("=" * 80)
    ...
    def _generate_qa_report(
        ...
        def ascii_bar(actual, target, width=20):
            """Generates an ASCII bar with actual vs. target markers."""
            actual_pos = int(actual * width)
            target_pos = int(target * width)
            bar = ""
            for i in range(width):
                if i == target_pos and i == actual_pos: bar += "✅"
                elif i == target_pos: bar += "|"
                elif i < actual_pos: bar += "█"
                else: bar += "░"
            return f"[{bar}]"

        report_lines.append("=" * 80)
        report_lines.append("RESUME QA REPORT (v5.75 HARDENED - 9 SECTIONS)")
        report_lines.append(f"Generated: {datetime.now().isoformat()}")
        report_lines.append("=" * 80)

        # Section 1: Signal Quality
        report_lines.extend(["", "1. SIGNAL QUALITY (Per-Section Analysis)", ""])
        # ... (Implementation for Section 1 would go here, using ascii_bar)
        ...
        headers = ["Hop ID", "Hop Name", "Status", "Web Searches", "LLM Calls"]
        rows = []
        total_searches = 0
        total_llm_calls = 0
        ...
        rows.append(["TOTAL", "", "", str(total_searches), str(total_llm_calls)])
        
        table_lines = self._format_plain_text_table(headers, rows, ['L', 'L', 'L', 'R', 'R'])
        ...
        table_lines = self._format_plain_text_table(headers, rows)
        report_lines.extend(table_lines)
        ...
        report_lines.append(f"Production Readiness Status: {'PASS' if prod_ready else 'FAIL'}")
        if not prod_ready:
            report_lines.append(f"  - Critical Failures: {len(critical_failures)}")
            report_lines.append(f"  - High Failures: {len(high_failures)}")
            report_lines.append("  - Reason: Production readiness requires zero CRITICAL or HIGH severity failures.")
        else:
            report_lines.append("  - Reason: All CRITICAL and HIGH severity checks passed.")
