"""
Resume Generation Engine v5.35 - COMPREHENSIVE HYPHENATION & ENHANCED QA

v5.35 CHANGES FROM v5.34:
✓ REPLACED: Simple hyphenation rules with comprehensive Hyphenation_Rules.json (44+ terms)
✓ ENHANCED: TextSanitizer with 7-layer sanitization suite:
  - Unicode normalization (smart quotes, em-dashes, ellipsis)
  - Punctuation spacing correction
  - Markdown artifact removal (asterisks, underscores, backticks)
  - Corporate jargon simplification (utilize→use, leverage→use)
  - Filler word reduction (In order to→To, Due to the fact that→Because)
✓ ADDED: EnhancedQAValidator with bullet provenance tracking (M/D/N notation)
✓ ADDED: JDAlignmentScorer for skills/competencies ordering by JD relevance
✓ ADDED: QA tables for Unify bullets (provenance + word count adherence)
✓ ADDED: QA tables for IBM bullets (provenance + word count adherence)
✓ ADDED: Skills (1-12) ordered by JD alignment score
✓ ADDED: Competencies (6) ordered by JD alignment score
✓ ADDED: Temperature tracking per section with weighted average
✓ ENFORCED: All QA tables output in markdown table syntax

Critical Enhancements:
- Hyphenation rules applied throughout all 10 hops at HOP-4.5
- Comprehensive text sanitization before staging buffer lock
- Enhanced QA validation with bullet-level provenance tracking
- JD alignment scoring for skills and competencies ordering
- Full temperature transparency in QA reporting

All v5.34 functionality maintained:
...
✓ All 50 classes from v5.34
✓ Word count validation and dynamic constraints
"""
...
class EnhancedQAValidator:
    ...
    def __init__(self, master_resume: Dict):
        ...
        self._calculate_master_baselines()
    
    def _calculate_master_baselines(self):
        """Calculate average word counts from master resume bullets."""
        self.baselines = {}
        
        for exp in self.master_resume.get('experience', []):
            company = exp.get('company', '').split()[0]  # First word of company name
    ...
    def track_bullet_provenance(
        ...
        Args:
            ...
            master_company_key: Key to lookup master resume baseline
        """
        ...
        # Get master baseline
        master_key = master_company_key or company
        baseline = self.baselines.get(master_key, {'avg_word_count': 30})
        master_avg = baseline['avg_word_count']
        
        # Calculate delta percentage
        delta_pct = ((word_count - master_avg) / master_avg) * 100 if master_avg > 0 else 0
        ...
        provenance = BulletProvenanceData(
            ...
            master_avg_word_count=master_avg,
    ...
    def generate_provenance_table_markdown(self, company: str) -> str:
        ...
        lines = []
        lines.append(f"## {company} Bullets - Provenance & Word Count Analysis\n")
        lines.append("| Bullet # | Provenance | Word Count | Master Avg | Delta % | Adherence |")
        lines.append("|----------|------------|------------|------------|---------|-----------|")
        
        for i, prov in enumerate(bullets, 1):
            provenance_str = prov.format_provenance()
            adherence = "✓ PASS" if abs(prov.word_count_delta_pct) <= 20 else "✗ FAIL"
            
            lines.append(
                f"| {i} | {provenance_str} | {prov.word_count} | "
                f"{prov.master_avg_word_count:.1f} | {prov.word_count_delta_pct:+.1f}% | {adherence} |"
            )
        
        # Summary statistics
        avg_delta = sum(p.word_count_delta_pct for p in bullets) / len(bullets)
        passing = sum(1 for p in bullets if abs(p.word_count_delta_pct) <= 20)
        
        lines.append("")
        lines.append(f"**Summary:** {passing}/{len(bullets)} bullets within ±20% tolerance | "
