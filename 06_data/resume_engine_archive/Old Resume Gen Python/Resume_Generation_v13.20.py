...
# --- START SCHEMA CONSOLIDATION: Reference Global Schema ---
# (Ensure APP_TRACKER_SCHEMA_V4 is defined globally *before* this class)
# Example:
# APP_TRACKER_SCHEMA_V4 = { ... 54 fields ... }
# ---
...
class DataEnricher:
    ...
    def _check_for_forbidden_verbs(self, text: str) -> List[str]:
        """[REFACTORED] Check for forbidden verbs in the text using a list comprehension."""
        text_lower = text.lower()
        return [verb for verb in self.FORBIDDEN_VERBS if re.search(r'\b' + verb + r'\b', text_lower)]
     ...
     orchestrator=None
        ...
        for section in experience_sections:
            for bullet in section.get("bullets", []):
                ...
                # Enforce FORBIDDEN_VERBS rule using the merged method
                forbidden_found = self._check_for_forbidden_verbs(
...
# Removed calculate_master_avg_bullet_length import/dependency
...
class ArtistGenerator:
    ...
    # --- START NEW: Hardcoded Bullet Word Count Ranges ---
    BULLET_WORD_COUNT_RANGES = {
        ...
        # TraderSense is copied, not generated/validated here
    }
    # --- END NEW ---

    # --- START OVERWRITE: _call_gemini_api ---
    # Unchanged from previous version provided
...
"""
    ...
    # --- generate & _generate_artist_output (Unchanged) ---
    def generate(
        self,
    ...
    # --- LLM Generation Methods (Headline, Summary, Skills - Unchanged) ---
...
"""
    ...
    # --- Overview Generation (Unchanged, uses synthesis) ---
...
"""
    ...
    # --- Methods calling _generate_tailored_overview_for_experience (Unchanged) ---
...
"""
    ...
    # --- START OVERWRITE: _validate_and_potentially_rewrite_bullets ---
    def _validate_and_potentially_rewrite_bullets(
        self,
        selected_bullets_structured: List[Dict],
        # Removed master_avg_lengths and section_name_for_avg
        min_target: int, # Added min_target
        max_target: int, # Added max_target
        section_id_for_logging: str,
        temperature_override: Optional[float] = None
    ) -> List[Dict]:
        """
        [MODIFIED] Checks word count against provided min/max range, attempts rewrite if needed.
        Raises HopExecutionError on failure. Uses hardcoded min/max ranges passed in.
        """
        final_bullets = []
        ...
        for i, bullet_data in enumerate(selected_bullets_structured):
            ...
            # Check if word count is within the hardcoded target range
            if not (min_target <= word_count <= max_target):
                ...
                try:
                    # Call the rewrite helper function with the specific target range
                    rewritten_text = self._rewrite_bullet_for_word_count(
                        original_bullet_text=original_text,
                        target_word_count_range=(min_target, max_target), # Use hardcoded range
    ...
    # --- END OVERWRITE: _validate_and_potentially_rewrite_bullets ---
...
"""
    ...
    # --- START NEW: Generic Bullet Generation Helper ---
    def _generate_bullets_for_section(
        ...
        """
        [NEW] Generic helper to generate bullets for any experience section.
        Handles both full V/C/S generation and customized-only generation.
        """
        ...
        if list(provenance_targets.keys()) == ['Customized']:
            ...
            # The `section_index` is not strictly needed if we pass the company name.
            return self._generate_tailored_bullets_for_experience(
                company_name=company_name, section_index=-1, provenance_targets=provenance_targets,
                reasoning_config=reasoning_config, section_id=section_enum, temperature_override=temperature_override
            )
    # --- START OVERWRITE: _generate_tailored_bullets_for_experience ---
    def _generate_tailored_bullets_for_experience(
        ...
        Raises HopExecutionError on failures. Uses hardcoded word count ranges for validation.
        """
        ...
        if not isinstance(bullet_source, list):
             raise HopExecutionError(f"{section_id.name} master bullet source ('{experience_section.get('bullet_pool') or experience_section.get('highlights')}') is not a list for company '{company_name}'.")
        ...
        if not master_bullets_structured:
             logging.warning(f"No valid master bullets found for company '{company_name}' in section {section_id.name}.")
        ...
        if verbatim_count > 0:
            ...
            if len(master_bullets_structured) < verbatim_count:
                raise HopExecutionError(f"{section_id.name} Cannot select {verbatim_count} Verbatim bullets: Only {len(master_bullets_structured)} master bullets available for '{company_name}'.")
...
"""
            ...
            try:
                ...
                response_select = self._call_gemini_api(prompt_select, reasoning_config, f"{section_id.name}_SelectV", system_prompt_select, temperature_override=temperature_override)
                ...
                logging.error(f"{section_id.name} Verbatim selection/validation failed: {he}")
        ...
        if customized_count > 0:
            ...
            if len(available_for_custom) < customized_count:
                raise HopExecutionError(f"{section_id.name} Cannot customize {customized_count} bullets: Only {len(available_for_custom)} unique master bullets remaining after verbatim selection for '{company_name}'.")
            ...
            try:
                ...
                logging.error(f"{section_id.name} Customization failed: {he}")
                raise he # Halt workflow
        ...
        try:
            ...
            final_bullets = self._validate_and_potentially_rewrite_bullets(
                ...
                min_target=min_target, # Pass hardcoded min
                max_target=max_target, # Pass hardcoded max
                section_id_for_logging=section_id.name, # Use Enum name for logging
                temperature_override=temperature_override # Pass temperature for potential rewrites
            )
        except HopExecutionError as he:
            logging.error(f"{section_id.name} Word count validation/rewrite failed: {he}")
            raise he # Halt workflow
...
"""
        ...
        try:
            ...
            response_reorder = self._call_gemini_api(prompt_reorder, ReasoningConfig.DEFAULT, f"{section_id.name}_Reorder", system_prompt_reorder, temperature_override=temperature_override)
            ...
            if len(reordered_texts) != total_expected_count:
                raise HopExecutionError(f"{section_id.name} Reordering returned wrong count ({len(reordered_texts)}), expected {total_expected_count}. Output preview: '{response_reorder[:100]}...'")
            ...
            for reordered_text in reordered_texts:
                 ...
                 if cleaned_reordered in original_texts_map:
                     ...
                     if cleaned_reordered in used_original_texts:
                          raise HopExecutionError(f"{section_id.name} Reordering validation failed: LLM returned duplicate bullet text '{cleaned_reordered[:50]}...'.")
                     ...
                     raise HopExecutionError(f"{section_id.name} Reordering validation failed: LLM modified text or returned unknown bullet ('{cleaned_reordered[:50]}...').")
            ...
            if len(final_ordered_bullets_dicts) != total_expected_count:
                raise HopExecutionError(f"{section_id.name} Reordering validation failed: Final count mismatch ({len(final_ordered_bullets_dicts)}) after matching, expected {total_expected_count}.")
            ...
            logging.error(f"{section_id.name} Reordering failed: {he}")
            raise he # Halt workflow
        except Exception as e:
            logging.error(f"{section_id.name} Reordering unexpected error: {e}", exc_info=True)
            raise HopExecutionError(f"{section_id.name} Reordering failed: {e}") from e
    # --- END OVERWRITE: _generate_tailored_bullets_for_experience ---

    # --- Methods calling _generate_tailored_bullets_for_experience (Unchanged) ---
    ...
    def _generate_k10_competencies(self, temperature_override: Optional[float] = None) -> List[Dict]:
        ...
        try:
            ...
            generated_bullets = self._generate_bullets_for_section(
    ...
    # --- START OVERWRITE: _generate_k8_ey_bullets ---
    ...
    # --- END OVERWRITE: _generate_k8_ey_bullets ---

    # --- START OVERWRITE: _generate_k9_early_career_bullets ---
    ...
    # --- END OVERWRITE: _generate_k9_early_career_bullets ---
...
# --- END OVERWRITE: _count_sentences (Regex Fix) ---
...
class PreFlightValidator:
    ...
    RULES_CONFIG = [
        ...
        {
            "rule_id": "VG_TOTAL_WORD_COUNT", "severity": ValidationSeverity.CRITICAL, "category": "word_count", "validator": lambda ctx: ContentConstraintsConfig.TOTAL_WORD_COUNT_MIN <= ctx.total_words <= ContentConstraintsConfig.TOTAL_WORD_COUNT_MAX,
        ...
        },
        {
         ...
         {
            "rule_id": "VG_HEADLINE_WORD_COUNT", "severity": ValidationSeverity.CRITICAL,"category": "structure", "validator": lambda ctx: ContentConstraintsConfig.HEADLINE_WORD_COUNT_MIN <= count_words_ms_word_style(ctx.staging_buffer.get(ResumeSection.K0_HEADLINE.value, '')) <= ContentConstraintsConfig.HEADLINE_WORD_COUNT_MAX,
        ...
        # --- Distribution Rules ---
        {
            "rule_id": "WORD_DISTRIBUTION_UNIFY_IBM", "severity": ValidationSeverity.HIGH, "category": "distribution", "validator": lambda ctx: ContentConstraintsConfig.UNIFY_IBM_COMBINED_PERCENT_MIN <= ctx.unify_ibm_percent <= ContentConstraintsConfig.UNIFY_IBM_COMBINED_PERCENT_MAX,
            "error_message": "Unify+IBM: {percent:.1f}% of total (target: {min}-{max}%)"
        },
        {
            "rule_id": "UNIFY_IBM_RATIO", "severity": ValidationSeverity.HIGH, "category": "distribution", "validator": lambda ctx: ctx.ibm_words > 0 and ContentConstraintsConfig.UNIFY_IBM_RATIO_MIN <= ctx.unify_ibm_ratio <= ContentConstraintsConfig.UNIFY_IBM_RATIO_MAX,
        ...
        # --- START COVER LETTER SIGNATURE CONSOLIDATION ---
        ...
        # --- END COVER LETTER SIGNATURE CONSOLIDATION ---
        {
        ...
        # VG_COVER_LETTER_SIGNATURE_MULTILINE removed above
        {
        ...
        {
            "rule_id": "VG_HEADLINE_NO_COMMAS", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: ',' not in ctx.staging_buffer.get(ResumeSection.K0_HEADLINE.value, ''),
            "error_message": "K.0 Headline contains commas. Headline: '{headline}'"
        },
        {
            "rule_id": "VG_RESUME_HEADER_H2", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: True, # Checked during rendering
            "error_message": "Resume headers not consistently H2: {failed_headers}"
        },
        {
            "rule_id": "VG_EDU_CERTS_FORMAT", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: True, # Checked during rendering
            "error_message": "Education/Certification format error: {details}"
        },
        {
            "rule_id": "VG_EXPERIENCE_BULLET_STYLE", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: True, # Checked during rendering
            "error_message": "Experience bullets do not consistently use '* ': {details}"
        },
        {
            "rule_id": "VG_COMPETENCIES_FORMATTING", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: True, # Checked during rendering
            "error_message": "Competencies list not using '*' bullets: {details}"
        },
        {
            "rule_id": "VG_EXPERIENCE_RENDER_FORMAT", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: True, # Checked during rendering
        ...
        },
        {
            ...
            "validator": lambda d: ContentConstraintsConfig.MIN_JD_KEYWORDS <= len(d.get('jd_keywords_found', [])) <= SignalControlConfig.RESUME_MAX_JD_KEYWORDS,
        ...
        },
        {
            ...
            "validator": lambda d: ContentConstraintsConfig.COVER_LETTER_JD_RELEVANCE_THRESHOLD <= d.get('cover_letter_jd_similarity', 0.0) <= SignalControlConfig.CL_MAX_JD_SIMILARITY,
        ...
        },
        {
            ...
            "validator": lambda d: "track record of measurable AI transformation" not in d['staging_buffer'].get(ResumeSection.K13_COVER_LETTER.value, ''),
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        """
        # --- START COVER LETTER SIGNATURE CONSOLIDATION ---
        # Update map for signature rule changes
        rule_map = {
            ...
            # --- Cover Letter (K.13) ---
            "VG_COVER_LETTER_SIGNATURE_VALID": ResumeSection.K13_COVER_LETTER, # NEW Consolidated Rule
            # "COVER_LETTER_SIGNATURE": ResumeSection.K13_COVER_LETTER, # REMOVED
            # "VG_COVER_LETTER_SIGNATURE_MULTILINE": ResumeSection.K13_COVER_LETTER, # REMOVED
    ...
    def _register_rules(self):
        ...
        for config in self.RULES_CONFIG:
            ...
            def create_error_message_lambda(template, rule_id):
                ...
                return lambda ctx: template.format_map(defaultdict(lambda: 'N/A', **ctx._cache.get(rule_id, {}), **format_args))
    ...
    def validate(
        ...
        # ... (Existing validate logic remains the same) ...
    ...
    # --- START COVER LETTER SIGNATURE CONSOLIDATION ---
    # Removed _validate_cover_letter_signature
    # Removed _validate_cover_letter_signature_multiline

    def _validate_cover_letter_signature_valid(self, context: ValidationContext) -> bool:
        """[NEW] Checks both presence and multi-line format of the signature."""
        cover_letter = context.staging_buffer.get(ResumeSection.K13_COVER_LETTER.value, '')
        expected_sig = context.expected_signature
        # Checks: expected signature exists? It contains newlines? The letter ends with it?
        # Ensure expected_sig is a non-empty string before checking for newlines
        return bool(expected_sig and '\n' in expected_sig and cover_letter.strip().endswith(expected_sig))
    # --- END COVER LETTER SIGNATURE CONSOLIDATION ---

    # --- START CHANGE: Update Bullet Word Count Validator ---
    def _validate_bullet_word_count_range(self, context: ValidationContext) -> bool: # Renamed method, use context
        """
        ...
        # --- Use Hardcoded Ranges (Matches ArtistGenerator) ---
        HARDCODED_BULLET_RANGES = ArtistGenerator.BULLET_WORD_COUNT_RANGES

        staging_buffer = context.staging_buffer

        # Use the sections defined for checking word count
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            section_key = section_enum.value

            # Get the hardcoded min/max targets directly using the enum
            target_range = HARDCODED_BULLET_RANGES.get(section_enum)
            if target_range is None:
                 logging.warning(f"Hardcoded word count range not found for {section_enum.name} in validator. Skipping.")
                 continue # Skip if range definition is missing
            min_wc, max_wc = target_range

            bullets = staging_buffer.get(section_key, [])
            # Ensure bullets data is a list before iterating
            if not isinstance(bullets, list):
                 logging.warning(f"Data for {section_key} is not a list ({type(bullets)}), skipping bullet range check.")
                 continue
            ...
            for i, bullet in enumerate(bullets):
                ...
                if isinstance(bullet, dict):
                    ...
                    logging.warning(f"Unexpected item type in {section_key}[{i}]: {type(bullet)}. Skipping range check.")
                    continue
    ...
    # Keep all other validator methods (_validate_k1_sentence_count, etc.) unchanged from v13.10
    # ... (rest of PreFlightValidator methods) ...
...
class WorkflowOrchestrator:
    ...
    # --- START REFACTOR: Section 8 (Pairwise Similarity Summary) ---
    def _build_qa_section_8_pairwise_similarity(self) -> List[str]:
        """Builds the NEW QA Section 8: Pairwise Similarity Summary Only."""
        lines = ["", "8. PAIRWISE SIMILARITY SUMMARY (Deduplication)", ""] # Renumbered Header
        lines.append("```markdown")
        if not self.similarity_matrix_data:
            ...
            lines.append(f"  Duplicates Found (>= 0.90): {duplicates_count}")
            if duplicates_count == 0:
                lines.append("  Status: ✅ PASS (No duplicates found)")
            else:
                # Still include FAIL status here as the DUPLICATE_BULLETS rule (High) would have failed
                lines.append("  Status: ❌ FAIL (Potential duplicates found, see logs for details)")
        lines.append("```")
        return lines
    # --- END REFACTOR ---

    # --- START REFACTOR: Section 9 (Pipeline Health) ---
    def _build_qa_section_9_pipeline_health(self) -> List[str]:
        """Builds the NEW QA Section 9: Pipeline Health."""
    ...
    # --- START REFACTOR: Section 10 (Structural Validation) ---
    ...
    # --- START REFACTOR: Section 11 (Cover Letter QA) ---
