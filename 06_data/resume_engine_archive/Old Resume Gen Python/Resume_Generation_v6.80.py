"""
Resume Generation Engine v6.30
...
def _get_normalized_reasoning_params(config: ReasoningConfig) -> Dict:
    ...
    cot = config.cot_min_paths if config.cot_min_paths is not None else 3
        ...
        if staging_buffer and hasattr(staging_buffer, '_data'):
            ...
            if len(keywords_found) >= 3:
                results.append(JDEnforcementResult(
                    ...
                    True,
                    f"Pre-flight check found {len(keywords_found)} JD keywords in final buffer",
                    gate_id
                ))
            else:
                results.append(JDEnforcementResult(
                    JDEnforcementRule.E9_CONTENT_HAS_JD_KW,
                    False,
                    f"Pre-flight check found only {len(keywords_found)} JD keywords in final buffer",
                    gate_id
                ))
...
# FIX: Define the missing APP_TRACKER_SCHEMA_V4 global variable.
# This schema is derived from the AppTrackerQAValidator to ensure consistency.
...
# FIX: Define the cover letter signature as a global, enforceable constant.
...
class ClaudeWebSearchClient:
        ...
        try:
            response = self.client.generate_content(
                ...
                generation_config=genai.GenerationConfig(
                    ...
                    response_mime_type="application/json" # Use Gemini's JSON mode
                )
            )
    ...
    def _extract_json(self, text_content: str) -> Dict[str, Any]:
        """
        Extract JSON from Gemini's response content.
    ...
    def enrich(
        ...
        for section in experience_sections:
            for bullet in section.get("bullets", []):
                ...
                # FIX: Enforce FORBIDDEN_VERBS rule
...
class ArtistGenerator:
    ...
    def _generate_artist_output(
        ...
        output[ResumeSection.K0_HEADLINE.value] = self._generate_k4_headline(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        ...
        output[ResumeSection.K5_UNIFY_BULLETS.value] = self._generate_k5a_bullets(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K5_UNIFY_OVERVIEW.value] = self._generate_k5b_overview(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K6_IBM_BULLETS.value] = self._generate_k6a_bullets(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K6_IBM_OVERVIEW.value] = self._generate_k6b_overview(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K7_TRADERSENSE_BULLETS.value] = self._copy_k7a_tradersense_bullets(enriched_scaffold)
        output[ResumeSection.K7_TRADERSENSE_OVERVIEW.value] = self._copy_k7b_tradersense_overview(enriched_scaffold)
        output[ResumeSection.K8_EY_BULLETS.value] = self._generate_k8a_ey_bullets(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K8_EY_OVERVIEW.value] = self._generate_k8b_ey_overview(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K9_EARLY_CAREER_BULLETS.value] = self._generate_k9a_early_career_bullets(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        output[ResumeSection.K9_EARLY_CAREER_OVERVIEW.value] = self._generate_k9b_early_career_overview(enriched_scaffold, job_description, thematic_analysis, previous_failures)
        ...
        output[ResumeSection.K11_EDUCATION.value] = self._generate_k11_education(enriched_scaffold)
        output[ResumeSection.K12_CERTIFICATIONS.value] = self._generate_k12_certifications(enriched_scaffold, job_description, thematic_analysis, previous_failures)
...
"""
            ...
            model = genai.GenerativeModel('gemini-1.5-flash', system_instruction="You are an expert HR data analyst. You generate 1-3 word skills for HR databases. You follow formatting instructions perfectly.")
...
- Match metrics and achievements to job requirements"""
        ...
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                print("Warning: No GEMINI_API_KEY, returning first 7 bullets")
...
"""
        try:
            ...
            reasoning_config = ReasoningConfig.K5_UNIFY_OVERVIEW_CONFIG
            api_params = reasoning_config_to_api_params(reasoning_config)
    ...
    def _generate_k6a_bullets(
        ...
        master_bullets = ibm_exp['bullet_pool']
...
"""
    ...
    def _generate_k7a_ey_highlights(
        ...
        if not ey_exp:
            return []
        return ey_exp.get('highlights', [])[:2]
    ...
    def _generate_k8_competencies(
        ...
        all_competencies = MASTER_RESUME_JSON.get('strategic_and_technical_competencies', [])
...
- Prioritize competencies that are explicitly mentioned or strongly implied in the job description."""
    ...
    def _generate_k9_cover_letter(
        ...
        # FIX: Use the globally defined signature template
...
"""
    ...
    def _generate_k10a_early_career_highlights(
        ...
        if not early_exp:
            return []
        return early_exp.get('highlights', []) if early_exp else []
    ...
    def _generate_k11_certifications(
        self,
        enriched_scaffold: Dict,
        job_description: str,
        thematic_analysis: ThematicAnalysis,
        ...
        """Generate K.11 Skills/Certifications from master resume."""
        """
        Generate K.11 Certifications string from master resume.
        This method retrieves the list of certifications and joins them into a single, pipe-separated string.
        """
        # This section is not LLM-tailored per current design, but should use self.master_resume
...
# v5.75: CRITICAL FIX - Restore missing validation constraints dictionary
WORD_COUNT_MIN = 899
WORD_COUNT_MAX = 999
...
def validate_headline_v57(headline: str) -> ValidationResult:
    ...
    # FIX: Exclude '&' from word count per user specification
    ...
    # FIX: Strictly enforce exactly 3 components
    if len(components) != 3:
...
class PreFlightValidator:
    ...
    def validate(
        ...
        # validation_results.extend(self._validate_distributions(staging_buffer)) # This seems to be causing issues, let's disable for now
    ...
    def _validate_word_counts(self, staging_buffer: ImmutableStagingBuffer) -> List[ValidationResult]:
        ...
        k5b_text = staging_buffer.get(ResumeSection.K5B_UNIFY_OVERVIEW.value, '')
        if k5b_text:
            master_unify = next((
        ...
        k6b_text = staging_buffer.get(ResumeSection.K6B_IBM_OVERVIEW.value, '')
        if k6b_text:
            master_ibm = next((
                exp for exp in self.master_resume.get('professional_experience', [])
        ...
        k5a_bullets = staging_buffer.get(ResumeSection.K5A_UNIFY_BULLETS.value, [])
        if k5a_bullets:
            master_unify = next((
                exp for exp in self.master_resume.get('professional_experience', [])
                if exp.get('company') == 'Unify Consulting'
        ...
        k6a_bullets = staging_buffer.get(ResumeSection.K6A_IBM_BULLETS.value, [])
        if k6a_bullets:
            master_ibm = next((
                exp for exp in self.master_resume.get('professional_experience', [])
                if exp.get('company') == 'IBM'
        ...
        verbatim_sections = [
            (ResumeSection.K7A_TRADERSENSE_BULLETS, 'TraderSense', 'highlights'),
            (ResumeSection.K7B_TRADERSENSE_OVERVIEW, 'TraderSense', 'overview'),
            (ResumeSection.K8A_EY_BULLETS, 'Ernst & Young', 'highlights'),
            (ResumeSection.K8B_EY_OVERVIEW, 'Ernst & Young', 'overview'),
            (ResumeSection.K9A_EARLY_CAREER_BULLETS, 'Early Career Roles', 'highlights'),
            (ResumeSection.K9B_EARLY_CAREER_OVERVIEW, 'Early Career Roles', 'overview'),
        ]
        ...
        k7a_highlights = staging_buffer.get(ResumeSection.K8A_EY_BULLETS.value, [])
        if k7a_highlights:
            master_ey = next(
                (exp for exp in self.master_resume.get('professional_experience', [])
        ...
        k10a_highlights = staging_buffer.get(ResumeSection.K9A_EARLY_CAREER_BULLETS.value, [])
        if k10a_highlights:
        ...
        k8_competencies = staging_buffer.get(ResumeSection.K10_STRATEGIC_TECHNICAL_COMPETENCIES.value, [])
        if k8_competencies:
        ...
        k4_headline = staging_buffer.get(ResumeSection.K4_HEADLINE.value, '')
        if k4_headline:
    ...
    def _validate_distributions(self, staging_buffer: ImmutableStagingBuffer) -> List[ValidationResult]:
        ...
        unify_exp = {
            ...
            'overview': staging_buffer.get(ResumeSection.K5B_UNIFY_OVERVIEW.value, ''), # This is fine
            'bullets': staging_buffer.get(ResumeSection.K5A_UNIFY_BULLETS.value, []) # This is fine
        }
        ...
        ibm_exp = {
            'company': 'IBM',
            'overview': staging_buffer.get(ResumeSection.K6B_IBM_OVERVIEW.value, ''),
            'bullets': staging_buffer.get(ResumeSection.K6A_IBM_BULLETS.value, [])
        }
...
class FileRenderer:
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        render_actions = {
            ...
            K0_COMPETENCIES_HEADER: lambda v: f"Strategic & Technical Competencies\n--------------------------------------------------------------------------------\n",
            K0_EDUCATION_HEADER: lambda v: f"Education\n--------------------------------------------------------------------------------\n",
            K0_CERTIFICATIONS_HEADER: lambda v: f"Certifications\n--------------------------------------------------------------------------------\n",
            ResumeSection.K0_EXPERIENCE_HEADER: lambda v: f"Professional Experience\n--------------------------------------------------------------------------------\n",
            ...
            ResumeSection.K10_COMPETENCIES: lambda v: f"{v}\n",
            ResumeSection.K11_EDUCATION: lambda v: "".join([f"{edu.get('degree')}, {edu.get('institution')} ({edu.get('notes')})\n" for edu in v]) + "\n",
            ResumeSection.K12_CERTIFICATIONS: lambda v: f"{v}\n",
        ...
        for section_enum in ResumeSection:
            ...
            if "OVERVIEW" in section_enum.name or section_enum in [ResumeSection.K2_SKILLS, ResumeSection.K13_COVER_LETTER]:
                continue
    ...
    def _render_experience_section(self, master_exp: Dict, overview: str, bullets: List[str]) -> str:
        ...
        lines = []
        lines.append(f"{master_exp.get('company', '')} | {master_exp.get('location', '')}")
        lines.append(f"{master_exp.get('title', '')} | {master_exp.get('dates', {}).get('start', '')} – {master_exp.get('dates', {}).get('end', '')}\n")
    ...
    def _render_resume_markdown_old(self, staging_buffer: ImmutableStagingBuffer) -> str:
        """
        OLD HARDCODED VERSION - Kept for reference, can be deleted later.
        """
        master_experience = self.master_resume.get("professional_experience", [])
        
        staging_keys = [
            (ResumeSection.K5_UNIFY_BULLETS.value, ResumeSection.K5_UNIFY_OVERVIEW.value), # This is fine
            (ResumeSection.K6_IBM_BULLETS.value, ResumeSection.K6_IBM_OVERVIEW.value), # This is fine
            (ResumeSection.K7_TRADERSENSE_BULLETS.value, ResumeSection.K7_TRADERSENSE_OVERVIEW.value),
            (ResumeSection.K8_EY_BULLETS.value, ResumeSection.K8_EY_OVERVIEW.value),
            (ResumeSection.K9_EARLY_CAREER_BULLETS.value, ResumeSection.K9_EARLY_CAREER_OVERVIEW.value)
        ]
    ...
    def _render_skills(self, staging_buffer: ImmutableStagingBuffer, job_description: str = None) -> str:
        ...
        output_lines = [] # FIX: Initialize output_lines
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict):
        ...
        logger.info(f"Current Model: gemini-1.5-flash")
    ...
    def execute_workflow(
        ...
        print("=" * 80)
        print("RESUME GENERATION ENGINE v5.81 - GEMINI API")
        print("=" * 80)
        ...
        try:
            ...
            if ResumeSection.K2_12_SKILLS.value in artist_output:
                k2_skills = artist_output.get(ResumeSection.K2_12_SKILLS.value, [])
            ...
            self.processed_data = { # Use Enum for consistency
                "sections": {
                    ResumeSection.K5A_UNIFY_BULLETS.value: staging_buffer.get(ResumeSection.K5A_UNIFY_BULLETS.value), # Unify
                    ResumeSection.K6A_IBM_BULLETS.value: staging_buffer.get(ResumeSection.K6A_IBM_BULLETS.value) # IBM
                },
                "k5b_overview": staging_buffer.get(ResumeSection.K5B_UNIFY_OVERVIEW.value),
                "k6b_bullets": staging_buffer.get(ResumeSection.K6A_IBM_BULLETS.value)
            }
            ...
            print("\n[HOP-8] Generating QA Report...")
            # FIX: Pass the results of the deduplication analysis to the QA report generator
            qa_report_validation_results, qa_report_text = self._generate_qa_report(
                staging_buffer,
                thematic_analysis,
                hop5_results  # FIX: Pass the correct, complete validation results from HOP-5
            )
    ...
    def _invoke_deduplication_analysis(self) -> bool:
        ...
        try:
            ...
            try: # FIX: Use self.processed_data instead of undefined dedup_input
    ...
    def _generate_qa_report(
        ...
        """
        v5.75 HARDENED: Generate full 9-section QA report.
        This replaces the broken 3-section stub and correctly implements
        all QA checks described in the v5.66-v5.70 changelogs.
        """
        validation_results_out = []
        report_lines = [
            "=" * 80,
            "RESUME QA REPORT (v5.81 - HARDENED)",
            f"Generated: {datetime.now().isoformat()}",
            "=" * 80,
        ]

        # --- Section 1: Signal Quality ---
        report_lines.extend(["", "1. SIGNAL QUALITY (Per-Section Analysis)", ""])
        # This section requires signal scores which are not currently calculated.
        # Placeholder implementation.
        report_lines.append("Signal Quality analysis is not yet implemented.")
        ...
        # --- Section 5: Deduplication Matrix ---
        ...
        # --- Section 6: Pipeline Health (API/LLM Calls) ---
...
def main():
    ...
    print("\n" + "=" * 100)
    print("RESUME GENERATION ENGINE v5.90")
    print("=" * 100)
        ...
        if result.get('status') == 'SUCCESS':
            ...
            if staging_buffer:
                files_to_save = {
                    ...
                    file_paths.get('skills'): renderer._render_skills(staging_buffer, job_description),
