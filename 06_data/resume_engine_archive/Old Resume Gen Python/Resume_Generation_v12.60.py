...
"""
                ...
                synthesis_prompt += "\n**FINAL SYNTHESIZED ANSWER (ensure it strictly follows original prompt constraints like word count, format, etc.):**\n"
...
5.  Output ONLY the headline text.
...
5.  Output ONLY the list of skills, separated by newlines. Do not use bullets, numbers, or any other formatting.
...
**ABSOLUTELY CRITICAL:** The final overview MUST be strictly between {min_wc} and {max_wc} words total. Do NOT exceed {max_wc} words. Output ONLY the overview text, with no preamble or explanation.
...
ABSOLUTELY CRITICAL: The rewritten overview MUST be strictly between {min_wc} and {max_wc} words. Output ONLY the rewritten overview text.
...
ABSOLUTELY CRITICAL: The rewritten bullet MUST be strictly between {min_wc} and {max_wc} words (inclusive). Count words carefully using standard English rules (hyphenated words count as one).

Output ONLY the rewritten bullet text.
...
"""
    ...
    def _validate_and_potentially_rewrite_bullets(
        ...
        tolerance = self.BULLET_WORD_COUNT_TOLERANCE # e.g., 0.20
...
5.  Output ONLY the rewritten bullets, one per line, starting with "• ".
...
6.  Output ONLY the {count} new bullet points, one per line, starting with "* ".
...
2. Output ONLY the selected bullet points, exactly as they appear in the MASTER LIST, one per line. Do not add numbers, prefixes, or explanations.
...
3.  Output ONLY the reordered bullet points, exactly as provided in the input list, one per line. Do not add numbers, prefixes, explanations, or modify the text.
...
CRITICAL: Adhere strictly to the word count targets for each paragraph. Ensure the exact signature block is appended correctly with forced line breaks. Output ONLY the complete cover letter text.
