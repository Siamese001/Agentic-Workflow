...
# Inside class EnhancedJobDescriptionAnalyzer:
    ...
    def _dict_to_thematic_analysis(self, data: Dict) -> "'ThematicAnalysis'":
        ...
        # --- START FIX: Handle missing 'competitive_intelligence' key in cache ---
        ...
        retrieval_sources = []
        # Safely reconstruct RetrievalSource objects
        for src_data in data.get("retrieval_sources", []):
            if isinstance(src_data, dict):
                try:
                 ...
                 logging.warning(f"Skipping invalid retrieval source data in cache: {src_data}")


        # --- START FIX: Add .get() fallbacks for all other keys ---
        return ThematicAnalysis(
            ...
            competitive_intelligence=comp_intel, # Use the safely created object
            ...
            # Ensure weighting_formula is handled
...
"""
    ...
    def _generate_k1_executive_summary(self, temperature_override: Optional[float] = None) -> str:
        ...
        if self.previous_failures:
            ...
            messages = [f.message for f in self.previous_failures if f.rule_id in ["VG_WORD_COUNT_K1", "VG_SENTENCE_COUNT_K1"]]
            if messages:
                feedback_instruction = (
...
1.  **WORD COUNT:** The summary MUST be **strictly between {self.constraints.EXEC_SUMMARY_WORD_COUNT_MIN} and {self.constraints.EXEC_SUMMARY_WORD_COUNT_MAX} words**. Count carefully before outputting.
2.  **SENTENCE COUNT:** The summary MUST have **exactly {self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MIN} or {self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MAX} sentences**. Count carefully before outputting.
...
"""
        ...
        if "[Placeholder" in headline or not (self.constraints.HEADLINE_WORD_COUNT_MIN <= word_count <= self.constraints.HEADLINE_WORD_COUNT_MAX):
            ...
            # Refined Fallback
            fb_diff1 = differentiators[0] if differentiators else "Strategic Planning"
            fb_diff2 = differentiators[1] if len(differentiators) > 1 else "Execution"
            return f"{primary_theme} Leadership | {fb_diff1} | {fb_diff2}" # More relevant fallback
...
# Inside class ArtistGenerator:
    def _generate_tailored_bullets_for_experience(
        ...
        differentiators = []
        # Ensure competitive_intelligence exists and is not None before accessing
        comp_intel = getattr(self.thematic_analysis, 'competitive_intelligence', None)
        if comp_intel:
            differentiators = comp_intel.get_top_differentiators(5)
        ...
        if verbatim_count > 0:
            if not master_bullets_structured:
                ...
                # --- START FIX: Strengthen Verbatim selection prompt ---
...
"""
        ...
        if len(final_bullets) < total_expected_count:
            ...
            # Ensure structure consistency when padding
            padding = [{'bullet_text': b['bullet_text'], **b, 'provenance': BulletProvenance.Verbatim.value} for b in padding_candidates[:needed]] # Ensure provenance is Verbatim for padding
        ...
        section_name_for_avg = "Default" # Default key
        ...
        elif section_id.startswith("K.10"): section_name_for_avg = "Competencies" # Handle competencies

        # Log which average is being used
...
"""
        ...
        # Create map using both possible text keys for robustness
        generated_map = {}
        for b in final_bullets:
             text_key = b.get('text', b.get('bullet_text')) # Prioritize 'text'
             if text_key: # Ensure we have a non-empty key
        ...
        for text in reordered_texts:
            if text in generated_map and text not in used_from_generated:
                ...
                matched = False
                # Initialize dup_detector if needed (assuming DuplicateDetector is available)
                ...
                best_match_score = 0.95 # Threshold
                ...
                if matched and best_match_bullet:
                    ...
                    used_from_generated.add(original_text_of_match) # Add the original key
        ...
        # If reordering resulted in the wrong count, fallback to generated order
        if len(final_ordered_bullets) != total_expected_count:
             ...
             # Make sure the fallback list items have consistent structure ('text', 'provenance', 'word_count')
             consistent_fallback = []
             for b in final_bullets[:total_expected_count]:
                 ...
                 consistent_fallback.append({
                     ...
                     # Copy other potential keys if needed
                 })
             return consistent_fallback

        # Ensure final list items have consistent structure before returning
        final_output_list = []
...
6. **ABSOLUTELY CRITICAL:** The final output MUST be a single paragraph strictly between **{min_wc} and {max_wc} words**. Failure to meet this word count will cause rejection.
...
**Instructions:**
1. **ABSOLUTELY CRITICAL:** Rewrite the bullet so the final output is strictly between **{min_wc} and {max_wc} words**. Failure to meet this word count range will cause rejection.
...
"""
        ...
        if len(final_competencies) < total_expected_count:
            ...
            padding = [{'bullet_text': c['bullet_text'], **c, 'provenance': BulletProvenance.Verbatim.value} for c in padding_candidates[:needed]] # Ensure provenance
    ...
    def _generate_k13_cover_letter(self) -> str:
        ...
        FIX: Safely access differentiators list to prevent IndexError.
        """
        ...
        # Safely get differentiators
        differentiators = []
        # Ensure competitive_intelligence is not None before accessing attributes
        comp_intel = getattr(self.thematic_analysis, 'competitive_intelligence', None)
        if comp_intel:
            # Use get_top_differentiators which returns a list
            differentiators = comp_intel.get_top_differentiators(5)
            # Ensure the result is actually a list, default to empty list if not
            if not isinstance(differentiators, list):
                logging.warning(f"get_top_differentiators returned unexpected type: {type(differentiators)}. Defaulting to empty list.")
                differentiators = []
        else:
             logging.warning("Competitive intelligence object missing in thematic analysis for cover letter generation.")
             differentiators = []
        ...
        # Handle potential None for authenticity_patterns before accessing .get()
        auth_patterns_data = self.thematic_analysis.authenticity_patterns or {}
        ...
        # --- START FIX: Safely prepare differentiator keywords for the prompt ---
        # Provide fallback text if list doesn't have enough elements
...
**ABSOLUTELY CRITICAL:** Ensure each paragraph's word count STRICTLY adheres to the specified range.
...
"""
        ...
        try:
            ...
            # Add more robust checks for structure before returning
            if not cover_letter_text.strip().startswith(today):
                 logging.warning("Generated cover letter missing date. Attempting to fix.")
                 # Attempt to prepend date block if main body seems present
                 if "Dear Hiring Manager," in cover_letter_text:
                     cover_letter_text = f"{today}\n\nHiring Manager\n[Company Name]\n\n{cover_letter_text.strip()}"
                 else:
                      logging.warning("Could not fix missing date. Using fallback.")
                      return self._generate_fallback_cover_letter()

            if not cover_letter_text.strip().endswith(signature.split('\n')[-1]): # Check last line of signature
                 logging.warning("Generated cover letter missing or incomplete signature block. Attempting to fix.")
                 # Attempt to append signature block if main body seems present
                 sincerely_pos = cover_letter_text.rfind("Sincerely,")
                 if sincerely_pos != -1:
                      # Trim anything after Sincerely, and append the correct block
                      cover_letter_text = cover_letter_text[:sincerely_pos].strip() + f"\n\n{signature}"
                 else:
                      logging.warning("Could not find 'Sincerely,' to fix signature. Using fallback.")
                      return self._generate_fallback_cover_letter()
