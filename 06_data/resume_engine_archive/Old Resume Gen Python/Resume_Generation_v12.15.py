...
class ArtistGenerator:
    ...
    # --- START MODIFICATION: UNIVERSAL_SCHEDULE is now defined in Orchestrator ---
    # The ArtistGenerator no longer knows about schedules.
    # --- END MODIFICATION ---
    ...
    def _generate_artist_output(
        ...
        v12.50: Iterates config but only runs methods present in `sections_to_generate`.
        """
        output = {}
        ...
        for config in self.ARTIST_GENERATION_CONFIG:
            ...
            if section_enum not in sections_to_generate:
                ...
                # If it's an LLM method we're skipping, add None to output
                else:
            ...
            try:
                method = getattr(self, method_name)
                # Pass the specific temperature for this section
        ...
        # Return *only* the newly generated content (and copied content)
        # Filter out the None values for sections that were skipped
        final_output = {k: v for k, v in output.items() if v is not None}
        return final_output
    ...
    # --- LLM Generation Methods (All Unchanged) ---
    # These methods already accept `temperature_override` and will raise
    # HopExecutionError on failure, which is the required behavior.
    ...
    # --- Bullet Generation Helpers (All Unchanged) ---
    # These helpers already raise HopExecutionError on failure.
...
# ... (rest of reorder prompt) ..."""
    ...
    def _generate_k10_competencies(self, temperature_override: Optional[float] = None) -> List[Dict]:
        ...
        try:
            generated_bullets = self._generate_tailored_bullets_for_experience(
                company_name=fake_company_name, section_index=-1, 
    ...
    def _generate_k9_early_career_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        ...
        customized_bullets = self._generate_lightly_customized_bullets(
            source_bullets_text=source_bullets[:target_count],
            section_id=ResumeSection.K9_EARLY_CAREER_BULLETS.value, # Corrected Enum
            thematic_analysis=self.thematic_analysis,
            temperature_override=temperature_override
        )
        master_avg_lengths = calculate_master_avg_bullet_length(self.master_resume)
        return self._validate_and_potentially_rewrite_bullets(
             customized_bullets, master_avg_lengths, "EarlyCareer", ResumeSection.K9_EARLY_CAREER_BULLETS.value, temperature_override # Corrected Enum
        )
...
class PreFlightValidator:
    ...
    # Consolidated Rules Configuration (Unchanged)
    RULES_CONFIG = [
        # ... (All rule configs remain the same as in the provided file) ...
    ...
    def validate(
        ...
        # --- NEW: Map failures back to sections ---
        failed_sections = set()
        ...
        for vr in failures:
            ...
            if isinstance(section_or_flag, ResumeSection):
                ...
                # The validator methods must be updated to add this.
                if vr.details and "failed_sections" in vr.details:
                    # `failed_sections` in details should be List[ResumeSection]
    ...
    def _prepare_validation_data(
        ...
        """Prepares a dictionary of data needed by the validation rules. (Unchanged)"""
    ...
    # --- Validation Helper Methods (Refactored to add failure context) ---
    ...
    # --- REFACTORED: _validate_per_section_signal ---
    def _validate_per_section_signal(self, data: Dict) -> bool:
        ...
        if not thematic_analysis: 
             data["error_details"] = {
                 ...
                 "failed_sections": set(self.SECTION_SIGNAL_TARGETS_CONFIG.values()) # Fail all sections
             }
             return False
        ...
        for label, (section_enum, target_min_score, _, _, _) in self.SECTION_SIGNAL_TARGETS_CONFIG.items():
            ...
            if content:
                ...
                if score < target_min_score:
                    ...
                    failed_sections.add(section_enum) # --- NEW ---
            else:
                # If content is missing, it's a STRUCTURE rule's job to fail it.
                # Don't fail signal score if the section wasn't even generated.
                pass 

        if failures:
            data["error_details"] = {
                "failures": "; ".join(failures),
                "failed_sections": failed_sections # --- NEW ---
            }
            return False
    ...
    def _validate_cover_letter_structure(self, data: Dict) -> bool:
        ...
        if len(paragraphs) <= max(p1_idx, p2_idx, p3_idx):
            data["error_details"] = { "p1_wc": 0, "p2_wc": 0, "p3_wc": 0, "p1_min": self.constraints.COVER_LETTER_P1_WORD_COUNT_MIN, "p1_max": self.constraints.COVER_LETTER_P1_WORD_COUNT_MAX, "p2_min": self.constraints.COVER_LETTER_P2_WORD_COUNT_MIN, "p2_max": self.constraints.COVER_LETTER_P2_WORD_COUNT_MAX, "p3_min": self.constraints.COVER_LETTER_P3_WORD_COUNT_MIN, "p3_max": self.constraints.COVER_LETTER_P3_WORD_COUNT_MAX, "message": f"Insufficient paragraphs found ({len(paragraphs)})." }
            return False
        ...
        data["error_details"] = { "p1_wc": p1_wc, "p1_min": self.constraints.COVER_LETTER_P1_WORD_COUNT_MIN, "p1_max": self.constraints.COVER_LETTER_P1_WORD_COUNT_MAX, "p2_wc": p2_wc, "p2_min": self.constraints.COVER_LETTER_P2_WORD_COUNT_MIN, "p2_max": self.constraints.COVER_LETTER_P2_WORD_COUNT_MAX, "p3_wc": p3_wc, "p3_min": self.constraints.COVER_LETTER_P3_WORD_COUNT_MIN, "p3_max": self.constraints.COVER_LETTER_P3_WORD_COUNT_MAX, }
    ...
    def _validate_headline_word_count(self, data: Dict) -> bool:
        ...
        data["error_details"] = { "word_count": word_count, "min": self.constraints.HEADLINE_WORD_COUNT_MIN, "max": self.constraints.HEADLINE_WORD_COUNT_MAX, "headline": headline }
    ...
    # --- REFACTORED: _validate_bullet_word_count_tolerance ---
    def _validate_bullet_word_count_tolerance(self, data: Dict) -> bool:
        ...
        averages = data.get('section_averages', self._calculate_master_avg_bullet_length()) 
        staging_buffer = data['staging_buffer']
    ...
    # --- REFACTORED: _validate_provenance_split_check ---
            ...
            for section_enum in bullet_sections:
                ...
                if isinstance(bullets, list):
                    for bullet in bullets:
                        text_to_add = None
                        if isinstance(bullet, dict):
                            text_to_add = bullet.get('text', bullet.get('bullet_text')) 
                        elif isinstance(bullet, str): 
                            text_to_add = bullet
                        if text_to_add:
...
class WorkflowOrchestrator:
    ...
    # --- QA Report Section Builders (Unchanged) ---
    # ... (All _build_qa_section_... methods remain identical) ...
    ...
    def _build_qa_section_7_overview_similarity(self) -> List[str]:
        lines = ["", "7. OVERVIEW VS. BULLET SIMILARITY CHECK", ""] 
    ...
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_7_overview_similarity", "args": []}, 
    ...
    # --- Utility methods for QA report formatting (Unchanged) ---
