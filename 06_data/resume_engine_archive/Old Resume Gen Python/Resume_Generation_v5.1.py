"""
Resume Generation Engine v5.0 - ULTIMATE OPTIMIZATION (ALL v4.2 FEATURES)
===========================================================================
...
- Temperature bonus application (BALANCED: +0.02, CREATIVE: +0.05)
- Ratio penalty enforcement (Unify/IBM 1.20-1.50 soft band)
- Coherence penalty applied to final signals
- Role-aware headline generation
...
class BaselineResumeMetricsV5:
    ...
    # Unify/IBM ratio soft band (from v4.2 OPT 4)
    UNIFY_IBM_RATIO_CONFIG = {
        "ideal_midpoint": 1.36,  # 265/195 = 1.36 for v4.9.0 targets
        "acceptable_band": (1.20, 1.50),
        "fail_band": (1.10, 1.60),
        "penalty_per_deviation": -0.005  # Per 0.05 deviation
    }
    
    # Temperature mode configs (from v4.2 OPT 6)
    TEMPERATURE_CONFIGS = {
...
class NineHopPipelineV5:
    ...
    def hop5_calculate_signals(self, hop1_result: Dict, 
                              ...
                              temperature: TemperatureMode) -> Dict:
        """
        Calculate signals using v4.2 elasticity models with bonuses/penalties.
        NOW INCLUDES: Temperature bonus, ratio penalty, coherence penalty (v4.2 fixes).
        """
        section_signals = {}
        ...
        # Base signal calculation with elasticity
        ...
        # Calculate ratio penalty (v4.2 OPT 4)
        ...
        # Calculate coherence penalty (v4.2 OPT 5)
        ...
        # Calculate final signals with all bonuses/penalties (v4.2 fix)
        word_count_data = {
            'unify': unify_words,
            'ibm': ibm_words,
        ...
        # Calculate weighted average of FINAL signals (not base)
        if final_signals:
        ...
        return {
            "section_signals": section_signals,  # Base signals
            "final_signals": final_signals,       # After bonuses/penalties
            "weighted_average": weighted_avg,     # Average of final signals
            "ratio_penalty": ratio_penalty,
            "coherence_penalty": coherence_penalty,
            "coherence_cv": coherence_result.get('cv', 0.0),
            "calculation_method": "v4.2_elasticity_curves_with_bonuses"
        }
    ...
    # ========================================================================
    # v4.2 MISSING METHODS
    # ========================================================================
    
    def _calculate_ratio_penalty(self, unify_words: int, ibm_words: int) -> float:
        """
        Calculate penalty for Unify/IBM ratio outside soft band (v4.2 OPT 4).
        
        Ideal: 1.36 (265/195 for v4.9.0)
        Acceptable: 1.20–1.50 (−0.005 per 0.05 deviation)
        Fail: <1.10 or >1.60
        """
        if ibm_words == 0:
            return -0.10  # Major penalty for missing IBM section
    ...
    def _calculate_final_signals(
        ...
        """
        Calculate final signals with all bonuses/penalties applied (v4.2 fix).
        
        v4.2 FIX: Apply temperature bonus, ratio penalty, coherence penalty.
        """
        final_signals = {}
    ...
    def _generate_role_aware_headline(self, role_key: str) -> str:
        """
        Generate role-aware headline in XYZ format (v4.2 feature).
        Fixed: 60–90 characters, NO elasticity.
        Format: [Role/Positioning] | [Domain] | [Related Competency & Related Competency]
        """
        headlines = {
            "vp_presales": "VP Pre-Sales Solutions | Enterprise SaaS Revenue Growth | Technical Sales Leadership",
            "chief_ai": "Chief AI Officer | LLM Product Launches | Strategic AI Partnerships",
            "head_of_ai": "Head of AI | Enterprise AI Adoption | Product-Driven Growth"
        }
        
        headline = headlines.get(role_key, self.master_resume.HEADLINE)
        
        # Validate 60–90 char constraint
        if len(headline) < 60:
            headline = headline + " | Strategic Solutions"
    ...
    def hop789_format_outputs(self, hop_results: Dict, target_role: str) -> Dict:
        """
        Format exactly 4 outputs (not 5 like v4.2).
        NOW WITH: Role-aware headline generation (v4.2 feature).
        """
        hop4 = hop_results.get('hop4', {})
        hop3 = hop_results.get('hop3', {})
        hop5 = hop_results.get('hop5', {})
        ...
        # OUTPUT 1: Resume (with role-aware headline)
    ...
    def _format_resume(self, hop4: Dict, hop3: Dict, target_role: str = "default") -> str:
        """Format complete resume with role-aware headline (v4.2 feature)."""
        lines = []
        lines.append("=" * 80)
        lines.append(self.master_resume.NAME.center(80))
        lines.append("=" * 80)
        lines.append("")
        # Use role-aware headline instead of static
    ...
    def _format_signal_report(self, hop5: Dict, hop6: Dict) -> str:
        """Format signal report with elasticity curves and v4.2 bonuses/penalties."""
        lines = []
        lines.append("SIGNAL STRENGTH (with v4.2 Elasticity + Bonuses/Penalties)")
        lines.append("=" * 80)
        
        # Base signals
        lines.append("\nBASE SIGNALS (Elasticity Only):")
        lines.append("-" * 80)
        section_signals = hop5.get('section_signals', {})
        ...
        # Penalties/Bonuses
        lines.append("\nBONUSES & PENALTIES (v4.2):")
        lines.append("-" * 80)
        ratio_penalty = hop5.get('ratio_penalty', 0.0)
        coherence_penalty = hop5.get('coherence_penalty', 0.0)
        coherence_cv = hop5.get('coherence_cv', 0.0)
        lines.append(f"Unify/IBM Ratio Penalty:   {ratio_penalty:+.4f}")
        lines.append(f"Coherence Penalty (CV={coherence_cv:.1f}%): {coherence_penalty:+.4f}")
        
        # Final signals
        lines.append("\nFINAL SIGNALS (After Bonuses/Penalties):")
        lines.append("-" * 80)
        ...
        lines.append("")
        lines.append(f"WEIGHTED AVERAGE (Final): {hop5.get('weighted_average', 0):.3f}")
    ...
    def execute_pipeline(self, jd_text: str, target_role: str,
        ...
        hop789 = self.hop789_format_outputs(self.hop_results, target_role)
