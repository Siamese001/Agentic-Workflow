...
class ActualResumeContentGenerator:
    ...
    def generate_k8_competencies(self) -> list:
        ...
        all_competencies = self.master['competencies']
...
4. Return ONLY the 6 selected bullets, one per line, no numbers"""
    ...
    def _generate_k8_competencies(
        ...
        all_competencies = MASTER_RESUME_JSON.get('competencies', [])
...
class TextSanitizer:
    """
    v5.35: Comprehensive text sanitization engine.
    ...
    def sanitize(
        ...
        for key, value in data.items():
            if isinstance(value, str):
                data[key] = self._sanitize_text(value)
            elif isinstance(value, list):
                data[key] = [self._sanitize_text(item) if isinstance(item, str) else item for item in value]
    ...
    def _normalize_unicode(self, text: str) -> str:
        for rule in self.rules['rules']['sanitization_suite']['unicode_normalization']:
            if 'from' in rule and rule['from'] in text:
                text = text.replace(rule['from'], rule['to'])
    ...
    def _fix_punctuation_spacing(self, text: str) -> str:
        ...
        if len(text) != original_len: self.sanitization_counts['punctuation_fixes'] += 1
        return text
    ...
    def _remove_markdown_artifacts(self, text: str) -> str:
        ...
        text = text.replace('`', '')
        if len(text) != original_len: self.sanitization_counts['markdown_removed'] += 1
        return text

    def _simplify_jargon(self, text: str) -> str:
        for rule in self.rules['rules']['sanitization_suite']['corporate_jargon_simplification']:
            if re.search(r'\b' + rule['from'] + r'\b', text, re.IGNORECASE):
                text = re.sub(r'\b' + rule['from'] + r'\b', rule['to'], text, flags=re.IGNORECASE)
...
def validate_cover_letter_signature(cover_letter_text: str) -> ValidationResult:
    ...
    header = MASTER_RESUME_JSON.get('header', {})
...
class FileRenderer:
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        txt += f"{header.get('email', '')} | {header.get('phone', '')} | {header.get('linkedin', '')}\n\n"
...
def main():
    ...
    try:
        ...
        qa_report_text = result.get('qa_report', '')
        
        qa_checks = [
            ("1. SIGNAL QUALITY", "1. SIGNAL QUALITY" in qa_report_text),
            ("2. THEMATIC COMPLIANCE", "2. THEMATIC COMPLIANCE" in qa_report_text),
            ("3. CONTENT AUTHENTICITY", "3. CONTENT AUTHENTICITY" in qa_report_text),
            ("4. AI DETECTION DEFENSE", "4. AI DETECTION DEFENSE" in qa_report_text),
            ("5. DEDUPLICATION MATRIX", "5. DEDUPLICATION MATRIX" in qa_report_text),
            ("6. PIPELINE HEALTH", "6. PIPELINE HEALTH" in qa_report_text),
            ("7. WORD COUNT COMPLIANCE", "7. WORD COUNT COMPLIANCE" in qa_report_text),
            ("8. STRUCTURAL VALIDATION", "8. STRUCTURAL VALIDATION" in qa_report_text),
            ("9. PRODUCTION READINESS", "9. PRODUCTION READINESS" in qa_report_text)
        ]
