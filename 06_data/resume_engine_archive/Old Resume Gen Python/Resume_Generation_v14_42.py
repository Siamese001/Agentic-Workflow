...
class DuplicateDetector:
    ...
    def compute_overview_bullet_similarity(
            ...
            results = {
                "section": section_id,
                "overview_length": text_utils.count_words_ms_word_style(overview_text) if overview_text else 0,
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict):
        ...
        self.logger.info(f"✓ Logging initialized. Detailed logs will be written to: {log_filename}")
            ...
            if isinstance(items, list) and items:
                for i, item_data in enumerate(items):
                    ...
                    if isinstance(item_data, dict):
                        ...
                        word_count = item_data.get('word_count', text_utils.count_words_ms_word_style(item_text)) # Use helper
                        provenance = item_data.get('provenance', 'N/A')
                    elif isinstance(item_data, str):
                        item_text = item_data
                        word_count = text_utils.count_words_ms_word_style(item_text) # Use helper
...
Excellent written and verbal communication skills, including interacting with and presenting to senior leadership, externally and internally.
    ...
    # --- ENHANCED: Write Final Output Files to Disk ---
    if result.get('status') == 'SUCCESS' and 'file_paths' in result and 'file_contents' in result:
        print("\n--- Writing Output Files ---")
        output_dir = "resume_outputs"  # Directory to save files
        ...
        for key, file_path in file_paths.items():
            ...
            if content is not None:
                # Use os.path.basename to prevent issues if file_path is absolute
                ...
                print(f"  ⚠️ Content missing for key '{key}', skipping file: {file_path}")
        print("--- File Writing Complete ---")
    elif 'file_paths' in result:
        print("\n--- Skipping File Writing (Workflow did not succeed) ---")
        print(f"    Intended file paths: {result['file_paths']}")
