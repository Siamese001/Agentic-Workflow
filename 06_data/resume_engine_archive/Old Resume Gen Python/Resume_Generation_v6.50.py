...
class PreFlightValidator:
    ...
    def _validate_distributions(self, staging_buffer: ImmutableStagingBuffer) -> List[ValidationResult]:
        ...
        unify_exp = {
            'company': 'Unify Consulting',
            'overview': staging_buffer.get(ResumeSection.K5B_UNIFY_OVERVIEW.value, ''),
            'bullets': staging_buffer.get(ResumeSection.K5A_UNIFY_BULLETS.value, [])
        }
...
class FileRenderer:
    ...
    def _render_resume_json(
        ...
        return {
            "metadata": { # This part is fine
                "company": company_name,
                "job_title": job_title,
                "generated": datetime.now().isoformat(),
                "version": __version__
            },
            "header": self.master_resume.get("owner", {}), # This is fine
            "headline": staging_buffer.get(ResumeSection.K4_HEADLINE.value, ''),
            "executive_summary": staging_buffer.get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, ''),
            "experience": experience_list,
            "competencies": staging_buffer.get(ResumeSection.K10_STRATEGIC_TECHNICAL_COMPETENCIES.value, ''),
            "education": self.master_resume.get("education", []), # This is fine
            "certifications": staging_buffer.get(ResumeSection.K12_CERTIFICATIONS_AND_CREDENTIALS.value, '')
        }
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        txt += f"{staging_buffer.get(ResumeSection.K4_HEADLINE.value, '')}\n\n"
        txt += "Executive Summary\n"
        txt += "--------------------------------------------------------------------------------\n"
        txt += f"{staging_buffer.get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, '')}\n\n"
        txt += "Professional Experience\n"
        txt += "--------------------------------------------------------------------------------\n\n"
        
        # v5.36: Dynamically render experience sections from master resume
        staging_keys = [
            (ResumeSection.K5A_UNIFY_BULLETS.value, ResumeSection.K5B_UNIFY_OVERVIEW.value),
            (ResumeSection.K6A_IBM_BULLETS.value, ResumeSection.K6B_IBM_OVERVIEW.value),
        ...
        k8_content = staging_buffer.get(ResumeSection.K10_STRATEGIC_TECHNICAL_COMPETENCIES.value, '')
        if k8_content:
            txt += f"{k8_content}\n\n"
        
        txt += "\nEducation\n"
        txt += "--------------------------------------------------------------------------------\n"
        for edu in staging_buffer.get(ResumeSection.K11_EDUCATION.value, []):
            txt += f"{edu.get('degree')}, {edu.get('institution')} ({edu.get('notes')})\n\n"
        
        txt += "\nCertifications\n"
        txt += "--------------------------------------------------------------------------------\n"
        # K.12 is a pre-formatted string from the staging buffer.
        certifications_text = staging_buffer.get(ResumeSection.K12_CERTIFICATIONS_AND_CREDENTIALS.value, "")
        if certifications_text:
            txt += f"{certifications_text}\n\n"
