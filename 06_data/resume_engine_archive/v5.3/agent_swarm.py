# File: agent_swarm.py
# Version: Consolidated 5.3 (Patched 2/2)
# Fixes: Restored prompt templates + Improved artifact assembly
...
from core import (
    ...
    ValidationResult, ThematicAnalysis, MasterResumeIndex,
...
# ============================================================================
# AGENT SWARM ARCHITECTURE - V5.2 10 AGENTS PATCH
# ============================================================================
...
AGENT_COMPLEXITY = {
    ...
    "FactualConsistency_Validator": 25,
...
class OntologyMapperAgent(SwarmAgent):
    ...
    def normalize_skills(self, raw_skills: List[str]) -> List[str]:
        # V5.2 Enhanced: Uses atomic prompt builder and JSON mode
...
class FactualConsistency_Validator(SwarmAgent):
    """QA Class 2: Factual/Medium"""
    def __init__(self): 
        super().__init__("FactualConsistency_Validator")
...
class Library_Specialist:
    ...
    Implements the rehydrated MasterResumeIndex from v5.2 patches.
    """
    ...
    def rehydrate_master_index(self, master_resume: Dict[str, Any]) -> MasterResumeIndex:
        """
        V5.2 PATCH: Rehydrate MasterResumeIndex from master resume data.
...
class RAG_Synthesizer:
    """
    4-Phase RAG orchestrator implementing v3.8 signal quality tracking.
    """
    
    def __init__(self, complexity: int = 90):
        """Initialize RAG Synthesizer."""
        self.complexity = complexity
        self.phases = [RAGPhase.EXTRACTION, RAGPhase.ENRICHMENT, 
    ...
    def execute_rag_mission(self, mission: RAGMission, 
        ...
        try:
            ...
            if mission.phase == RAGPhase.EXTRACTION:
                results = self._execute_extraction(mission, context)
            elif mission.phase == RAGPhase.ENRICHMENT:
                results = self._execute_enrichment(mission, context)
            elif mission.phase == RAGPhase.SYNTHESIS:
                results = self._execute_synthesis(mission, context)
            elif mission.phase == RAGPhase.VALIDATION:
                results = self._execute_validation(mission, context)
            
            # Calculate signal score
            results['signal_score'] = self._calculate_signal_score(results, context)
            self.signal_scores[mission.phase.value] = results['signal_score']
            
            return results
            
        except PhaseTimeoutError as e:
            logger.error(f"RAG phase timeout: {e}")
            results['error'] = str(e)
            return results
    ...
    def _execute_extraction(self, mission: RAGMission, 
        ...
        if not response:
            return {'evidence': [], 'signal_score': 0.0}
    ...
    def _execute_enrichment(self, mission: RAGMission, 
        ...
        prompt = get_rag_phase_prompt(RAGPhase.ENRICHMENT, mission, context)
        response = self.gemini_service.generate(prompt, temperature=0.5)
        
        if not response:
            return {'evidence': base_evidence, 'signal_score': 0.0}
        
        # Enrich evidence
        enriched = base_evidence.copy()
        for item in enriched:
            item['enriched'] = True
            item['confidence'] = min(1.0, item.get('confidence', 0.5) + 0.2)
    ...
    def _execute_synthesis(self, mission: RAGMission, 
        ...
        if not response:
            return {'synthesis': '', 'signal_score': 0.0}
        
        return {
            'phase': 'SYNTHESIS',
            'synthesis': response.text,
            'evidence': context.get('enriched_evidence', [])
        }
    ...
    def _execute_validation(self, mission: RAGMission, 
        ...
        if not response:
            return {'validation': 'FAILED', 'signal_score': 0.0}
        
        # Parse validation result
        is_valid = 'PASS' in response.text.upper() or 'VALID' in response.text.upper()
    ...
    def _calculate_signal_score(self, results: Dict[str, Any], 
        ...
        if 'evidence' in results and results['evidence']:
            avg_confidence = sum(e.get('confidence', 0) for e in results['evidence']) / len(results['evidence'])
            factors.append(avg_confidence)
        
        # Synthesis quality
        if 'synthesis' in results and results['synthesis']:
            synthesis_length = len(results['synthesis'].split())
            if synthesis_length > 50:
                factors.append(min(1.0, synthesis_length / 200))
        
        # Validation result
        if 'validation' in results:
            factors.append(1.0 if results['validation'] == 'PASSED' else 0.5)
...
class Governor:
    ...
    Implements dynamic template mapping and HIL escalation from v5.2 patches.
    """
    ...
    def __init__(self, config: Optional[CrewConfiguration] = None):
        ...
        self.qa2 = FactualConsistency_Validator()
        ...
        # Initialize specialists (if _initialize_specialists exists)
        if hasattr(self, '_initialize_specialists'):
    ...
    def _initialize_specialists(self):
        ...
        self.rag_synthesizer = RAG_Synthesizer(complexity=90)
    ...
    def process_request(self, context: CrewContext) -> Dict[str, Any]:
        ...
        V5.2 10 Agents Patch: Integrated multi-tier validation flow.
        """
        ...
        try:
            ...
            # QA1 (Linguistic/Fast)
            veto = self.qa1.check(draft)
            if veto.level.value != 0: 
                return self._handle_veto(veto, context, results)
            
            veto = self.format_police.check_ats(draft)
            if veto.level.value != 0: 
                return self._handle_veto(veto, context, results)
            
            veto = self.bias_scrubber.scan(draft)
            if veto.level.value != 0: 
                return self._handle_veto(veto, context, results)
            
            # QA2 (Factual)
            veto = self.qa2.validate(draft, context.master_resume)
            if veto.level.value != 0: 
                return self._handle_veto(veto, context, results)
    ...
    def _generate_content(self, context: CrewContext) -> Dict[str, str]:
        ...
        for section in sections:
            # V5.2 PATCH: Dynamic template mapping
            ...
            # --- V5.3 FIX: USE RICH PROMPT TEMPLATES ---
    ...
    def _map_section_to_template(self, section: ResumeSection) -> str:
        """
        V5.2 PATCH: Dynamic template mapping based on section type.
    ...
    def _assemble_artifacts(self, context: CrewContext) -> Dict[str, Any]:
        ...
        # --- V5.3 FIX: IMPROVED RESUME ASSEMBLY ---
        ...
        artifacts['tracker'] = {
            ...
            'notes': 'Generated by v5.2 recovery system'
        }
...
class HIL_EscalationAgent:
    ...
    V5.2 PATCH: Uses json.dump for persistence instead of memory references.
    """
    ...
    def escalate(self, issue: str, context: Dict[str, Any], 
        ...
        V5.2 PATCH: Persist using json.dump instead of in-memory storage.
        """
        escalation = {
            'timestamp': datetime.now().isoformat(),
            'issue': issue,
        ...
        # V5.2 PATCH: Persist to file using json.dump
        try:
...
class CrewOrchestrator:
    ...
    def process_job_application(self, job_description: str, company_name: str,
                               job_title: str, master_resume: Dict[str, Any]) -> Dict[str, Any]:
        """
