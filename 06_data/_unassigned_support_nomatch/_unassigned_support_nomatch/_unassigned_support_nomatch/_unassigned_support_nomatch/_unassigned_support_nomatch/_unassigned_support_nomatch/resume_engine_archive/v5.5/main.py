...
# Version: Consolidated 5.5 - Agentic RAG
...
class WorkflowV52:
    """
    Main workflow class for v5.5 agentic RAG system.
    Integrates v3.8 deterministic logic with v5.5 agentic architecture.
    """
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.logger.info(f"Workflow v5.5 initialized with agentic RAG system (version: {__version__})")
    ...
    def run(
        ...
        """
        self.logger.info("=" * 80)
        self.logger.info(f"Starting Workflow v5.5 for {company_name} - {job_title}")
        self.logger.info("=" * 80)
    ...
    def _save_artifacts(
        ...
        if 'artifacts' in results:
            ...
            if 'output_markdown' in artifacts:
                markdown_file = output_path / f"{base_filename}.md"
                with open(markdown_file, 'w') as f:
                    f.write(artifacts['output_markdown'])
                self.logger.info(f"📄 Saved markdown to: {markdown_file}")
            
            # Save LaTeX output
            if 'output_latex' in artifacts:
                latex_file = output_path / f"{base_filename}.tex"
                with open(latex_file, 'w') as f:
                    f.write(artifacts['output_latex'])
                self.logger.info(f"📄 Saved LaTeX to: {latex_file}")
            
            # Legacy output field
            if 'output' in artifacts:
                output_file = output_path / f"{base_filename}_output.txt"
                with open(output_file, 'w') as f:
                    f.write(artifacts['output'])
...
def print_summary(results: Dict[str, Any]) -> None:
    ...
    if 'execution_metadata' in results:
        ...
        if 'execution_time_seconds' in metadata:
            print(f"   ⏱️  Execution Time: {metadata['execution_time_seconds']:.2f}s")
...
def main():
    """
    Main entry point for V5.5 Agentic RAG Workflow.
    """
    script_path = os.path.relpath(__file__) if "__file__" in globals() else "main.py"
    
    parser = argparse.ArgumentParser(
        description=f'V5.5 Agentic RAG System - Resume Workflow Engine (Version: {__version__})',
...
Consolidated Features:
  - **Agentic RAG Pipeline (v5.5):** 7-agent RAG flow with Blackboard model.
  - **Atomic QA (v5.5):** Factual validation split into 4 atomic agents (Metric, Tenure, Entity, Claim).
  - **Cost Control (v5.5):** `CostEstimator` and `CostTracker` agents.
  - **Production Output (v5.5):** `MarkdownToLatexAgent` for PDF generation.
  - **ChromaDB persistent memory** (Library_Specialist)
  - **25+ validation rules** (PreFlightValidator)
...
This is the consolidated 4-file version merging:
  ...
  - main.py: workflow orchestration + runner
    ...
    parser.add_argument(
        '--version',
        action='version',
        version=f'V5.5 Agentic RAG System - Version {__version__}'
    )
