"""
Resume Generation Engine v5.75 - SKILLS PIPELINE INTEGRATION HARDENING PATCH

v5.75 CHANGES - CRITICAL SKILLS PIPELINE FIX:
✅ PATCH: HR-VALIDATED SKILLS GENERATION INTEGRATION
   - PROBLEM: _generate_k2_skills() existed but was never called in workflow
   - ROOT CAUSE: Function was orphaned in v5.70, not wired into orchestrator
   - SOLUTION: Added HOP-3.5 to explicitly call _generate_k2_skills after ArtistGenerator
   - RESULT: 12 HR-database compatible skills now generated via LLM validation
   - FALLBACK: If HR skill generation fails, uses master competencies
   - IMPACT: Skills are now validated against job posting themes/keywords

✅ VERIFIED FIXES:
   - LLM call happens with job_description and thematic_analysis context
   - Skills are formatted as 1-3 word keywords suitable for HR recruiting systems
   - Skills output is stored in staging buffer as 'K.2_Skills'
   - File rendering receives validated skills instead of fallback competencies
   - Complete end-to-end test confirms all 5 outputs generated correctly

✅ FINAL 5 OUTPUTS:
   1. Resume Text with Bullets (resume_md) - Markdown format
   2. 12 Skills List (skills) - HR-validated, JD-aligned competencies
...
v5.71 CHANGES - REASONING TOGGLES HARDENED PATCH
...
class ActualResumeContentGenerator:
    ...
    def _validate_master_resume(self):
        ...
        required_fields = ['owner', 'professional_experience', 'education', 
    ...
    def generate_name(self) -> str:
        ...
        return self.master['owner']['name']
    
    def generate_contact(self) -> str:
        """Generate K.0 Contact from actual data."""
        h = self.master['owner']['contact']
        return f"{h['email']} | {h['phone']} | {h['linkedin']}"
    
    def generate_k1_executive_summary(self) -> str:
        """Generate K.1 Executive Summary from actual data."""
        # v5.90: executive_summary is no longer in master, it's generated.
        # This method is now a placeholder/fallback.
        return "Executive summary to be generated by LLM."
    
    def generate_k4_headline(self) -> str:
        """Generate K.4 Headline from actual data."""
        recent_title = self.master['professional_experience'][0]['title']
    ...
    def generate_k5a_bullets(self) -> list:
        ...
        unify_exp = next((exp for exp in self.master['professional_experience'] 
                         if 'Unify' in exp['company']), None)
        
        if not unify_exp:
            raise ValueError("Unify Consulting not found in MASTER_RESUME_JSON")
        
        actual_bullets = unify_exp['bullet_pool'][:7]
    ...
    def generate_k6a_bullets(self) -> list:
        ...
        ibm_exp = next((exp for exp in self.master['professional_experience'] 
                       if 'IBM' in exp['company']), None)
        if not ibm_exp:
            raise ValueError("IBM not found in MASTER_RESUME_JSON")
        return ibm_exp['bullet_pool'][:6]
    ...
    def generate_k7a_bullets(self) -> list:
        ...
        ey_exp = next((exp for exp in self.master['professional_experience'] 
    ...
    def generate_k8_competencies(self) -> list:
        ...
        all_competencies = self.master['strategic_and_technical_competencies']
    ...
    def generate_k10a_bullets(self) -> list:
        ...
        early_exp = next((exp for exp in self.master['professional_experience'] 
                         if 'Early Career' in exp['company']), None)
        if not early_exp:
            return []
        return early_exp.get('highlights', [])
    ...
    def generate_k11_skills(self) -> str:
        ...
        return " | ".join(self.master['certifications_and_credentials'])
...
def reasoning_config_to_api_params(reasoning_config: ReasoningConfig) -> dict:
    """
    Convert reasoning toggle configuration to Gemini API parameters.
...
class ArtistGenerator:
    ...
    def _generate_artist_output(
        ...
        return {
            ...
            'K.2_Skills': self._generate_k2_skills(job_description, thematic_analysis),
...
- If JD emphasizes "backend engineering" → emphasize technical architecture and scalability"""
        ...
        try:
            ...
            if not api_key:
                print("Warning: No GEMINI_API_KEY, returning master summary")
            ...
            tailored_summary = response.text.strip()
...
- Match metrics and achievements to job requirements"""
    ...
    def _generate_k6a_bullets(
        ...
        ibm_exp = next((exp for exp in MASTER_RESUME_JSON['professional_experience'] 
                       if 'IBM' in exp['company']), None)
        if not ibm_exp:
            raise ValueError("IBM not found in MASTER_RESUME_JSON")
...
class PreFlightValidator:
    ...
    def validate(
        ...
        validation_results.append(validate_cover_letter_signature(staging_buffer.get('K.9', '')))
...
class FileRenderer:
    ...
    def _render_resume_json(
        ...
        for i, exp in enumerate(master_experience[:5]):  # Limit to first 5 experiences
            if i < len(staging_keys):
                ...
                experience_list.append({
                    ...
                    "bullets": staging_buffer.get(bullets_key, exp.get("bullet_pool", []))
                })
        ...
        return {
            ...
            "header": self.master_resume.get("owner", {}),
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        txt = f"{owner_info.get('name', 'Amit Ayer')}\n\n"
        txt += f"{contact_info.get('email', '')} | {contact_info.get('phone', '')} | {contact_info.get('linkedin', '')}\n\n"
        ...
        for i, exp in enumerate(master_experience[:5]):  # Render first 5 experiences
            if i < len(staging_keys):
                ...
                txt += f"{exp.get('title', '')} | {exp.get('dates', {}).get('start', '')} – {exp.get('dates', {}).get('end', '')}\n\n"
        ...
        txt += "Strategic & Technical Competencies\n"
        txt += "--------------------------------------------------------------------------------\n"
        # K.8 content is a single string with bullets already formatted
        k8_content = staging_buffer.get('K.8', '')
        if k8_content:
            txt += f"{k8_content}\n\n"
        ...
        for edu in self.master_resume.get('education', []):
            # FIX: Remove bullet point per user request
            txt += f"{edu.get('degree')}, {edu.get('institution')} ({edu.get('notes')})\n\n"
        
        txt += "\nCertifications\n"
        txt += "--------------------------------------------------------------------------------\n"
        for cert in self.master_resume.get('certifications_and_credentials', []):
            # FIX: Remove bullet point per user request
            txt += f"{cert}\n\n"
...
class WorkflowOrchestrator:
    ...
    def execute_workflow(
        ...
        try:
            ...
            if 'K.2_Skills' in artist_output:
            ...
            sanitizer = TextSanitizer() # FIX: Instantiate the class
            ...
            dedup_input_data = {
                "sections": {
                    "K.5A": staging_buffer.get("K.5A"),
                    "K.6A": staging_buffer.get("K.6A"),
                    "K.7A": staging_buffer.get("K.7A"),
                    "K.10A": staging_buffer.get("K.10A"),
                },
                "k5b_overview": staging_buffer.get("K.5B"),
                "k6b_bullets": staging_buffer.get("K.6A") # Assuming K.6B vs K.6A bullets
            }
            dedup_success = self._invoke_deduplication_analysis(dedup_input_data)
            if dedup_success:
        ...
        try:
            ...
            try:
                self.similarity_matrix_data = self.dup_detector.compute_similarity_matrix(
                    ...
                    sections=dedup_input.get("sections", {})
                )
            except Exception as e:
...
def main():
    """
    Main execution function to run the resume generation workflow.
    Note: The comprehensive end-to-end test has been refactored into
    a dedicated test suite using pytest in `test_resume_generation.py`.
    This function now serves as a simple entry point for execution.
    """
    import sys
    import traceback
    ...
    try:
        ...
        # Define the job description for this run
        job_description = """
