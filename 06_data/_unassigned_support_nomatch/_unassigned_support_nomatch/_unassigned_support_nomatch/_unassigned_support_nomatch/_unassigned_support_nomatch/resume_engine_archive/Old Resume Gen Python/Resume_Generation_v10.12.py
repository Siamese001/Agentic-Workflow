...
class WorkflowOrchestrator:
    ...
    # (These _build_qa_section_ methods remain unchanged from v10.10, including the new Section 10)
    # --- Ensure all 16 methods exist here ---
    def _build_qa_section_1_signal_quality(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis) -> List[str]:
        ...
        for label, (section_enum, target_min_score, target_max_score, weight, reasoning_config) in section_configs.items():
            ...
            if content:
                ...
                if label == "Executive Summary" and sc_count <= 1: temp = 1.0 # K.1 override check
    ...
    def _build_qa_section_2_thematic_compliance(self, thematic_analysis: ThematicAnalysis) -> List[str]:
        ...
        lines.append("```markdown")
        headers = ["Theme", "Confidence/Relevance", "Keywords"]
        rows = []
        expected_rows = 0
        ...
        if thematic_analysis and thematic_analysis.secondary_themes:
            for i, st in enumerate(thematic_analysis.secondary_themes[:4]): # Limit to top 4 secondary
                rows.append([f"Secondary {i+1}", f"{st.get('relevance', 0):.1%}", ", ".join(st.get('keywords', []))])
            expected_rows += min(len(thematic_analysis.secondary_themes), 4)

        if len(rows) != expected_rows and expected_rows > 0: # Avoid adding error if no themes found at all
            lines.insert(2, f"ERROR   TRUNCATION_DETECTED   FAIL   Expected {expected_rows} theme rows, got {len(rows)}")
    ...
    def _build_qa_section_6_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        for section_data in self.QA_PROVENANCE_SECTIONS:
            ...
            avg_len = master_avg_lengths.get(name, 25.0)
    ...
    def _build_qa_section_7_authenticity(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation as provided before)
        lines = ["", "7. CONTENT AUTHENTICITY (Hallucination & Signal Check)", ""] # Updated title slightly
    ...
    # --- Section 10 is the new Signal Flow Map ---
    # (Method _build_qa_section_10_signal_flow_map already provided)

    # --- Re-numbered sections follow ---
    def _build_qa_section_11_pipeline_health(self) -> List[str]:
        # (Implementation as provided before, just renumbered)
        lines = ["", "11. PIPELINE HEALTH (Resource Consumption)", ""] # Corrected number
        lines.append("```markdown")
    ...
    def _build_qa_section_12_structural(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation as provided before, just renumbered)
        lines = ["", "12. STRUCTURAL VALIDATION", ""] # Corrected number
        struct_rule_ids = [
    ...
    def _build_qa_section_13_prod_readiness(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation as provided before, just renumbered)
        lines = ["", "13. PRODUCTION READINESS", ""] # Corrected number
        lines.append("```markdown")
    ...
    def _build_qa_section_14_cover_letter(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation as provided before, just renumbered)
        lines = ["", "14. COVER LETTER QA", ""] # Corrected number
    ...
    def _build_qa_section_15_jd_enforcement(self) -> List[str]:
        # (Implementation as provided before, just renumbered)
        lines = ["", "15. JD ENFORCEMENT VALIDATION", ""] # Corrected number
        lines.append("```markdown")
    ...
    def _build_qa_section_16_final_format(
        self,
        validation_results: List[ValidationResult],
        file_contents: Dict[str, str]
    ) -> List[str]:
        # (Implementation as provided before, just renumbered)
        lines = ["", "16. FINAL OUTPUT ARTIFACTS", ""] # Corrected number
        lines.append("```markdown")
        ...
        if qa_content:
            ...
            section_headers = ["1. SIGNAL QUALITY", "2. THEMATIC COMPLIANCE", "3. HOP-BY-HOP", "10. HOP-0 RAG SIGNAL FLOW MAP", "15. JD ENFORCEMENT", "16. FINAL OUTPUT ARTIFACTS"]
    ...
    QA_REPORT_SECTIONS = [
        ...
        # Ensure 'file_contents' is passed here
        {"method": "_build_qa_section_16_final_format", "args": ["validation_results", "file_contents"]},
    ]
    ...
    def _generate_qa_report(
        ...
        v10.0: Passes file_contents to section 16 builder and updates its own copy.
        """
        ...
        for section_config in self.QA_REPORT_SECTIONS:
            ...
            try:
                ...
                for name in arg_names:
                     # Ensure the *most current* version of file_contents is passed, esp. to section 16
                ...
                # Perform the pre-formatting check (excluding section 16 itself which contains the content)
                # v10.1: Improved check logic
                if method_name != "_build_qa_section_16_final_format":
                    # Check if the section *intended* to use pre-formatted tables but failed
                    # Look for markdown table syntax within a ```markdown block, excluding Section 1
                    section_text_block = "\n".join(section_lines)
        ...
        validation_results_out.append(ValidationResult(
            ...
            # v10.1: Changed severity to HIGH as requested
            severity=ValidationSeverity.HIGH,
        ...
        # Exclude the initial placeholder lines if sections were added
        # v10.1 Ensure correct joining
        if len(report_lines) > 2:
             # Join sections starting from the first actual section content
             qa_report_text = "\n".join(report_lines).strip() # Join all lines including headers
        else:
