...
class ValidationContext:
    ...
    def __getattr__(self, name):
        ...
        if calculation_method:
            ...
            # Fallback for accessing cached rule details directly
            if name.startswith("VG_"):
                 # Check if the details for this specific rule ID were cached by another calculation
                 if name in self._cache:
                      return self._cache[name]
    ...
    def _calculate_unify_ibm_ratio(self):
        ...
        if ibm_w == 0: return 0.0 # Avoid division by zero
    ...
    # --- START ADDED METHODS for K.5/K.6 Overview Checks ---
    def _calculate_k5_overview_details(self):
        """Calculates details for K.5 Unify Overview rules."""
        text = self.staging_buffer.get(ResumeSection.K5_UNIFY_OVERVIEW.value, '')
        ...
        # Cache details for specific rule IDs
        self._cache["VG_WORD_COUNT_K5_OVERVIEW"] = details
        self._cache["VG_SENTENCE_COUNT_K5_OVERVIEW"] = details
        return details

    def _calculate_k6_overview_details(self):
        """Calculates details for K.6 IBM Overview rules."""
        text = self.staging_buffer.get(ResumeSection.K6_IBM_OVERVIEW.value, '')
        word_count = count_words_ms_word_style(text)
        sentence_count = _count_sentences(text)
        details = {
            'word_count': word_count,
        ...
        # Cache details for specific rule IDs
        self._cache["VG_WORD_COUNT_K6_OVERVIEW"] = details
        self._cache["VG_SENTENCE_COUNT_K6_OVERVIEW"] = details
        return details
    # --- END ADDED METHODS ---
    ...
    def _calculate_headline_details(self):
        ...
        details = {
            ...
            'max': ContentConstraintsConfig.HEADLINE_WORD_COUNT_MAX,
             # Also add component word count min/max for use in error message if needed
        ...
        self._cache["VG_HEADLINE_NO_TITLES"] = details # Shares details
        self._cache["VG_HEADLINE_NO_COMMAS"] = details # Shares details
        self._cache["VG_HEADLINE_COMPONENT_WC"] = details # Add cache for component WC rule if separate
        return details
    ...
    def _calculate_cover_letter_jd_similarity(self):
        ...
        # Cache details for rule
        self._cache["VG_COVER_LETTER_RELEVANCE_RANGE"] = {
    ...
    def _calculate_cover_letter_structure_details(self):
        """Calculates details for VG_COVER_LETTER_STRUCTURE rule."""
        cl_text = self.staging_buffer.get(ResumeSection.K13_COVER_LETTER.value, '')
        ...
        try:
             ...
             p2_idx = p1_idx + 1
             p3_idx = p2_idx + 1
             if p1_idx < len(paras): p1_wc = count_words_ms_word_style(paras[p1_idx])
             if p2_idx < len(paras): p2_wc = count_words_ms_word_style(paras[p2_idx])
             if p3_idx < len(paras): p3_wc = count_words_ms_word_style(paras[p3_idx])
        except (StopIteration, IndexError):
             error_msg = "Could not find expected paragraphs"
...
class PreFlightValidator:
    ...
    # --- START ADDED: Banned Intro Phrases Pattern ---
    ...
    # --- START CHANGE: Consolidated Rules Configuration (Add new rules) ---
    RULES_CONFIG = [
        ...
        },
        {
            ...
            "error_message": "K.1 Exec Summary: {sentence_count} sentences (target: {min}-{max})"
        },
        {
            "rule_id": "VG_WORD_COUNT_K1", "severity": ValidationSeverity.CRITICAL, "category": "word_count",
            "validator": "_validate_k1_word_count",
            "error_message": "K.1 Exec Summary: {word_count} words (target: {min}-{max})"
        },
        {
        ...
        # --- START ADDED: K.5/K.6 Overview Word Count Rules ---
        {
            "rule_id": "VG_WORD_COUNT_K5_OVERVIEW", "severity": ValidationSeverity.HIGH, "category": "word_count",
            "validator": "_validate_k5_overview_word_count",
            "error_message": "K.5 Unify Overview: {word_count} words (target: {min_wc}-{max_wc})"
        },
        {
            "rule_id": "VG_WORD_COUNT_K6_OVERVIEW", "severity": ValidationSeverity.HIGH, "category": "word_count",
            "validator": "_validate_k6_overview_word_count",
            "error_message": "K.6 IBM Overview: {word_count} words (target: {min_wc}-{max_wc})"
        },
        # --- END ADDED ---
        # --- START ADDED: K.5/K.6 Overview Sentence Count Rules ---
        {
            "rule_id": "VG_SENTENCE_COUNT_K5_OVERVIEW", "severity": ValidationSeverity.HIGH, "category": "structure",
            "validator": "_validate_k5_overview_sentence_count",
            "error_message": "K.5 Unify Overview: {sentence_count} sentences (target: {min_sc}-{max_sc})"
        },
        {
            "rule_id": "VG_SENTENCE_COUNT_K6_OVERVIEW", "severity": ValidationSeverity.HIGH, "category": "structure",
            "validator": "_validate_k6_overview_sentence_count",
            "error_message": "K.6 IBM Overview: {sentence_count} sentences (target: {min_sc}-{max_sc})"
        },
        # --- END ADDED ---
        {
        ...
        # --- Distribution Rules ---
        {
            ...
            "error_message": "Unify+IBM: {unify_ibm_percent:.1f}% of total (target: {min}-{max}%)"
        },
        {
            ...
            "error_message": "Unify/IBM ratio: {unify_ibm_ratio:.2f} (target: {min}-{max})"
        },
        ...
        },
        {
            ...
            "validator": lambda ctx: ',' not in ctx.headline_details['headline'],
        ...
        },
        {
            ...
            "error_message": "Cover letter relevance to JD is {cover_letter_jd_similarity:.2f} (target: {min_sim:.2f}-{max_sim:.2f})."
        },
        {
        ...
        },
        {
            ...
            "error_message": "Cover letter paragraph word counts out of spec. P1: {p1_wc} ({p1_min}-{p1_max}), P2: {p2_wc} ({p2_min}-{p2_max}), P3: {p3_wc} ({p3_min}-{p3_max})"
        },
        {
    ...
    # --- START CHANGE: Update _initialize_rule_map (Add new rules) ---
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        rule_map = {
            ...
            "VG_WORD_COUNT_K5_OVERVIEW": ResumeSection.K5_UNIFY_OVERVIEW,      # <-- ADDED
            "VG_SENTENCE_COUNT_K5_OVERVIEW": ResumeSection.K5_UNIFY_OVERVIEW, # <-- ADDED
            # --- IBM (K.6) ---
            ...
            "VG_WORD_COUNT_K6_OVERVIEW": ResumeSection.K6_IBM_OVERVIEW,      # <-- ADDED
            "VG_SENTENCE_COUNT_K6_OVERVIEW": ResumeSection.K6_IBM_OVERVIEW, # <-- ADDED
            # --- EY (K.8) ---
            ...
            "VG_NO_INTRO_PHRASES": "COMPLEX_PER_SECTION" # <-- ADDED
        }

        # Add simple structure rules dynamically if not already present
    ...
    def _register_rules(self):
        ...
        for config in self.RULES_CONFIG:
            ...
            if isinstance(validator_ref, str):
                if hasattr(self, validator_ref):
                 ...
                 validator_func = validator_ref # Correctly assign callable directly
            else:
            ...
            rule = ValidationRule(
                ...
                validator=validator_func, # Pass the resolved function/lambda
        ...
        required_sections = [
            ...
            # Add header sections if strictly required
        ...
        for section in required_sections:
            rule_id = f"STRUCTURE_{section.name}_PRESENT"
            # Avoid re-registering if already in RULES_CONFIG
            if not any(r.rule_id == rule_id for r in self.engine.rules):
                rule = ValidationRule(
                    rule_id=rule_id,
                    severity=ValidationSeverity.CRITICAL,
                    # Lambda now uses the section variable from the loop correctly
                    validator=lambda ctx, s=section: (
                        ctx.staging_buffer.get(s.value) is not None and
                        ( (isinstance(ctx.staging_buffer.get(s.value), str) and ctx.staging_buffer.get(s.value).strip() != "" and ctx.staging_buffer.get(s.value) != "HEADER_PLACEHOLDER") or # Check for empty string and placeholder
                          (isinstance(ctx.staging_buffer.get(s.value), list) and ctx.staging_buffer.get(s.value)) or # Check for non-empty list
                          (isinstance(ctx.staging_buffer.get(s.value), dict) and ctx.staging_buffer.get(s.value)) # Check for non-empty dict
                        )
                    ),
    ...
    def validate(
        ...
        # Execute validation
        validation_results = self.engine.validate(context) # Pass context object
        ...
        for vr in failures:
            ...
            if isinstance(section_or_flag, ResumeSection):
                ...
                if "failed_sections" in rule_cache and isinstance(rule_cache["failed_sections"], set):
                    failed_sections_from_rule = rule_cache["failed_sections"]
                    # Ensure items are ResumeSection enums before adding
                    valid_failed_sections = {s for s in failed_sections_from_rule if isinstance(s, ResumeSection)}
                    failed_sections.update(valid_failed_sections)
                    if len(valid_failed_sections) != len(failed_sections_from_rule):
                         logging.warning(f"Complex rule {vr.rule_id} provided non-enum items in failed_sections.")
    ...
    # --- START ADDED: K.5/K.6 Overview Validation Methods ---
    def _validate_k5_overview_word_count(self, context: ValidationContext) -> bool:
        """Validates word count for K.5 Unify Overview."""
        details = context.k5_overview_details # Trigger calculation
        return details['min_wc'] <= details['word_count'] <= details['max_wc']

    def _validate_k5_overview_sentence_count(self, context: ValidationContext) -> bool:
        """Validates sentence count for K.5 Unify Overview."""
        details = context.k5_overview_details # Trigger calculation
        return details['min_sc'] <= details['sentence_count'] <= details['max_sc']

    def _validate_k6_overview_word_count(self, context: ValidationContext) -> bool:
        """Validates word count for K.6 IBM Overview."""
        details = context.k6_overview_details # Trigger calculation
        return details['min_wc'] <= details['word_count'] <= details['max_wc']

    def _validate_k6_overview_sentence_count(self, context: ValidationContext) -> bool:
        """Validates sentence count for K.6 IBM Overview."""
        details = context.k6_overview_details # Trigger calculation
        return details['min_sc'] <= details['sentence_count'] <= details['max_sc']
    # --- END ADDED ---
    ...
    def _validate_cover_letter_signature_valid(self, context: ValidationContext) -> bool:
        ...
        # Check if expected sig exists, has newlines, and CL ends with it (ignoring trailing whitespace)
        return bool(expected_sig and '\n' in expected_sig and cover_letter.rstrip().endswith(expected_sig))
    ...
    def _validate_bullet_word_count_range(self, context: ValidationContext) -> bool:
        ...
        against hardcoded ranges. Adds failed sections to cache for error reporting.
        """
        all_bullets_valid = True
        violations = []
        failed_sections = set() # Store ResumeSection enums
        HARDCODED_BULLET_RANGES = ArtistGenerator.BULLET_WORD_COUNT_RANGES # Use ranges defined in Artist
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            if target_range is None:
                logging.warning(f"Missing hardcoded word count range for {section_key} in ArtistGenerator.")
                continue # Skip check if range not defined
            ...
            for i, bullet in enumerate(bullets):
                ...
                else: continue # Skip invalid bullet types
                ...
                if not (min_wc <= actual_wc <= max_wc):
                    ...
                    failed_sections.add(section_enum) # Add the Enum object

        if not all_bullets_valid:
            # Cache details for error message lambda and retry logic
            context._cache["VG_BULLET_WORD_COUNT_RANGE"] = {
                "violations": ", ".join(violations[:3]) + ('...' if len(violations)>3 else ''), # Store example violations
                "failed_sections": failed_sections # Store the set of failed Enum objects
            }
    ...
    def _validate_cover_letter_full_structure(self, context: ValidationContext) -> bool:
        ...
        if not valid:
             # Cache details for error message lambda
    ...
    def _validate_headline_format_no_titles(self, context: ValidationContext) -> bool:
        """Checks K.0 Headline format (3 components, WC) and ensures no forbidden titles."""
        details = context.headline_details # Trigger calculation
        ...
        min_comp_wc = details.get('comp_min', ContentConstraintsConfig.HEADLINE_COMPONENT_WORDS_MIN)
        max_comp_wc = details.get('comp_max', ContentConstraintsConfig.HEADLINE_COMPONENT_WORDS_MAX)

        for comp in components:
            word_count = count_words_ms_word_style(comp)
            if not (min_comp_wc <= word_count <= max_comp_wc):
                component_wc_valid = False # Component word count fail
            for title in forbidden_titles:
                 # Check for whole word match to avoid partial matches like 'chiefly'
                 if re.search(r'\b' + re.escape(title) + r'\b', comp.lower()):
                     forbidden_found.append(title)

        if forbidden_found:
             # Update cache specifically for this rule's failure
        ...
        if not component_wc_valid:
             # Update cache for component word count failure if needed for a separate rule or message
             context._cache["VG_HEADLINE_COMPONENT_WC"] = { # Example cache key
                  "headline": headline, "min": min_comp_wc, "max": max_comp_wc
             }
             return False # Fail if component word count is wrong
        return True
    ...
    def _validate_no_placeholders(self, context: ValidationContext) -> bool:
        ...
        def check_item(item, current_key_enum=None):
            if isinstance(item, str):
                if "[Placeholder" in item:
                    snippet = item.split("[Placeholder", 1)[0][-30:] + "[Placeholder" + item.split("[Placeholder", 1)[1][:30]
                    found_placeholders.append(f"{current_key_enum.value if current_key_enum else 'Unknown'}: ...{snippet}...") # Log snippet with section key
                    if current_key_enum: failed_sections.add(current_key_enum)
            elif isinstance(item, dict):
                for k, v in item.items():
                    # Try to map key back to enum for context
                    key_enum = None
                    try: key_enum = ResumeSection(k)
                    except ValueError: pass
                    check_item(v, key_enum or current_key_enum) # Pass down context
            elif isinstance(item, list):
                for elem in item:
                    check_item(elem, current_key_enum) # Pass down context

        check_item(buffer_data)
        if found_placeholders:
            context._cache["CONTENT_NO_PLACEHOLDERS"] = {
                "placeholders": ", ".join(found_placeholders[:3]) + ('...' if len(found_placeholders)>3 else ''),
                "failed_sections": failed_sections # Store failed sections for retry logic
                }
            return False
        return True
    ...
    def _validate_forbidden_verbs(self, context: ValidationContext) -> bool:
        """
        VG_FORBIDDEN_VERBS: Checks all generated bullets/overviews against the forbidden verb list.
        """
        ...
        # Sections to check (bullets, competencies, overviews)
        sections_to_check_enums = list(self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK) + [
             ResumeSection.K5_UNIFY_OVERVIEW, ResumeSection.K6_IBM_OVERVIEW,
             ResumeSection.K8_EY_OVERVIEW, ResumeSection.K9_EARLY_CAREER_OVERVIEW,
             ResumeSection.K1_EXECUTIVE_SUMMARY # Check Exec Summary too
        ]

        for section_enum in sections_to_check_enums:
            section_key = section_enum.value
            content = staging_buffer.get(section_key)
            texts_to_check = []
            ...
            if isinstance(content, str):
                ...
                for i, item in enumerate(content):
                    ...
                    if isinstance(item, dict):
                        bullet_text = item.get('text', item.get('bullet_text',''))
                    elif isinstance(item, str):
                        bullet_text = item
                    if bullet_text: texts_to_check.append((bullet_text, i)) # Store index for bullets

            for text_item in texts_to_check:
                 item_text = ""
                 item_index = -1
                 if isinstance(text_item, tuple): # Bullet with index
                      item_text, item_index = text_item
                 elif isinstance(text_item, str): # Overview or Exec Summary
                      item_text = text_item
                 ...
                 found_verbs = [
                     ...
                     if re.search(r'\b' + re.escape(verb) + r'\b', item_text_lower)
                 ]

                 if found_verbs:
                     all_content_valid = False
                     location = f"{section_key}" + (f"[{item_index}]" if item_index != -1 else "")
                     violations.append(f"{location}: Found '{', '.join(found_verbs)}'")
                     failed_sections.add(section_enum) # Add the Enum object
    ...
    # --- START ADDED: Banned Intro Phrases Validation Method ---
    def _validate_no_intro_phrases(self, context: ValidationContext) -> bool:
        """
        VG_NO_INTRO_PHRASES: Checks relevant sections for banned introductory phrases.
        """
        all_content_valid = True
        violations = []
        failed_sections = set() # Store ResumeSection enums
        ...
        # Sections to check (bullets, competencies, overviews, cover letter body)
        sections_to_check_enums = list(self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK) + [
             ResumeSection.K5_UNIFY_OVERVIEW, ResumeSection.K6_IBM_OVERVIEW,
             ResumeSection.K8_EY_OVERVIEW, ResumeSection.K9_EARLY_CAREER_OVERVIEW,
             ResumeSection.K1_EXECUTIVE_SUMMARY, ResumeSection.K13_COVER_LETTER
        ]
        ...
        for section_enum in sections_to_check_enums:
            ...
            content = staging_buffer.get(section_key)
            texts_to_check = []
            is_cl = (section_enum == ResumeSection.K13_COVER_LETTER)

            if isinstance(content, str):
                # For Cover Letter, only check body paragraphs
                if is_cl:
                    body_match = re.search(r"Dear Hiring Manager,\s*(.*?)\s*Sincerely,", content, re.DOTALL)
                    if body_match:
                         body_text = body_match.group(1).strip()
                         # Check each paragraph within the body
                         texts_to_check.extend([(para.strip(), f"Para {i+1}") for i, para in enumerate(body_text.split('\n\n')) if para.strip()])
                else: # For overviews and exec summary
                    texts_to_check.append((content.strip(), None)) # Store as tuple (text, index/label)
            elif isinstance(content, list): # For bullets/competencies
                for i, item in enumerate(content):
                    bullet_text = ""
                    if isinstance(item, dict):
                        bullet_text = item.get('text', item.get('bullet_text',''))
                        ...
                        bullet_text = item
                    if bullet_text: texts_to_check.append((bullet_text.strip(), i)) # Store index

            for text_item, index_or_label in texts_to_check:
                 match = self.BANNED_INTRO_PHRASES_PATTERN.match(text_item)
                 if match:
                     all_content_valid = False
                     # Determine location string
                     location = f"{section_key}"
                     if isinstance(index_or_label, int): location += f"[{index_or_label}]"
                     elif isinstance(index_or_label, str): location += f" ({index_or_label})" # e.g., (Para 1)

                     violations.append(f"{location}: Starts with '{match.group(0).strip()}'")
                     failed_sections.add(section_enum) # Add the Enum object

        if not all_content_valid:
            context._cache["VG_NO_INTRO_PHRASES"] = {
                "violations": ", ".join(violations[:3]) + ('...' if len(violations)>3 else ''),
                "failed_sections": failed_sections
            }
        return all_content_valid
    ...
    def _validate_per_section_signal(self, context: ValidationContext) -> bool:
        ...
        for label, (section_enum, target_min, target_max, _, _) in self.SECTION_SIGNAL_TARGETS_CONFIG.items():
            ...
            if content:
                ...
                # Use slightly wider tolerance for validation if needed, or stick to strict target
                if not (target_min <= score <= target_max):
                    ...
                    failed_sections.add(section_enum) # Add Enum object
            # else: Allow missing sections for this check; structure check will fail later if required

        if not all_sections_pass:
            context._cache["VG_PER_SECTION_SIGNAL_SCORE"] = {
                "failures": ", ".join(failures[:3]) + ('...' if len(failures)>3 else ''),
                "failed_sections": failed_sections # Store the set of failed Enum objects
            }
    ...
    def _validate_k1_differentiator_range(self, context: ValidationContext) -> bool:
        ...
        # Always cache details for error message lambda
    ...
    def _validate_jd_keyword_range(self, context: ValidationContext) -> bool:
        ...
        thematic = context.thematic_analysis
        full_resume_text = ""
        # Aggregate text from relevant content sections
        for key_enum in ResumeSection:
             ...
             if key not in [ResumeSection.K0_NAME.value, ResumeSection.K0_CONTACT.value] and not key.endswith("_HEADER"):
                 ...
                 if isinstance(value, str): full_resume_text += value + " "
                      ...
                      # Extract text from potential dicts in lists (like bullets)
                      for item in value:
                           if isinstance(item, dict): full_resume_text += item.get('text', '') + " "
                           elif isinstance(item, str): full_resume_text += item + " "
        ...
        # Filter empty strings from keywords set
        all_jd_keywords = {kw for kw in all_jd_keywords if kw}

        found_keywords = {kw for kw in all_jd_keywords if kw.lower() in full_resume_lower}
        ...
        context._cache["VG_JD_KEYWORD_RANGE"] = { # Always cache details
            "found": found_count, "min": min_target, "max": max_target,
            "jd_keywords_found": list(found_keywords)
        }
        return valid
    ...
    def _validate_cover_letter_narrative(self, context: ValidationContext) -> bool:
        ...
        # Basic keyword checks (adjust keywords as needed)
        hook_present = any(kw in cl_text for kw in ["enthusiastic", "excited", "apply for", "interest in"])
        proof_present = any(kw in cl_text for kw in ["demonstrated", "achieved", "delivered", "resulted in", "experience", "record of", "proven track"])
        vision_present = any(kw in cl_text for kw in ["contribute", "goals", "opportunity", "eager to discuss", "next step", "value i bring"])
    ...
    def _validate_cover_letter_structure(self, context: ValidationContext) -> bool:
        """Validates word counts of CL paragraphs using cached details."""
        details = context.cover_letter_structure_details # Trigger calculation
        if details.get("error"): return False # Fail if paragraphs couldn't be found
        ...
        for section_enum, targets in self.PROVENANCE_SPLIT_TARGETS.items():
            ...
            for bullet in bullets:
                if isinstance(bullet, dict):
                    ...
                    counts[prov] += 1
                elif isinstance(bullet, str): # Handle sections like K7
                     # Assume Verbatim if it's just a string list (like K7)
                     prov = BulletProvenance.Verbatim.value
                     counts[prov] += 1

            for prov_type_enum in BulletProvenance: # Iterate through all possible provenances
                 prov_type = prov_type_enum.value
                 target_count = targets.get(prov_type, 0) # Default target is 0 if not specified
                 actual_count = counts.get(prov_type, 0)
                 if actual_count != target_count:
                     # Check if the discrepancy is just because a type wasn't expected (target 0, actual > 0)
                     # Or if a required type is missing/wrong count
                     all_sections_pass = False
    ...
    def _validate_authenticity_signal(self, context: ValidationContext) -> bool:
        ...
        buffer_data = context.staging_buffer.data
        thematic = context.thematic_analysis
        auth_patterns_data = getattr(thematic, 'authenticity_patterns', {})
        auth_patterns = auth_patterns_data.get('patterns', {}) if isinstance(auth_patterns_data, dict) else {}

        if not auth_patterns: return True # Pass if no patterns extracted

        verbs = auth_patterns.get('achievement_verb_patterns', [])
        phrasing = auth_patterns.get('competency_phrasing', [])
        # Create target signals: top verbs + first word of competency phrases
        target_signals = set(v.lower() for v in verbs[:10]) | set(p.lower().split(':')[0].split()[0] for p in phrasing[:5] if ':' in p and p.split()) # Top signals (first word only for phrases)
        target_signals = {s for s in target_signals if s} # Remove empty strings

        full_resume_text = ""
        # Aggregate text from relevant sections (bullets, competencies, exec summary)
        sections_to_scan = list(self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK) + [ResumeSection.K1_EXECUTIVE_SUMMARY]
        for section_enum in sections_to_scan:
        ...
        # Check for presence of target signal words (case-insensitive, whole word)
        found_signals = set()
        resume_lower = full_resume_text.lower()
        for signal in target_signals:
            if re.search(r'\b' + re.escape(signal) + r'\b', resume_lower):
                found_signals.add(signal)

        match_ratio = len(found_signals) / len(target_signals) if target_signals else 0.0

        valid = match_ratio >= 0.3 # Require at least 30% of target signals found
        # Always cache details for error message
        context._cache["VG_AUTHENTICITY_SIGNAL_CHECK"] = {
            "details": f"Found {len(found_signals)}/{len(target_signals)} ({match_ratio:.1%}) auth signals."
        }
        return valid
    ...
    def _validate_exec_summary_vs_sections(self, context: ValidationContext) -> bool:
        ...
        dd = DuplicateDetector() # Assuming DuplicateDetector class is available
        sections_content = {}
        # Gather content from experience sections (overviews and bullets) + competencies
        enums_to_compare = [
        ...
        for section_enum in enums_to_compare:
        ...
        # Perform similarity calculation (assuming method exists)
        try:
             similarity_results = dd.compute_executive_summary_similarity(exec_summary_text, sections_content)
        except Exception as e:
             logging.warning(f"Could not compute exec summary similarity for validation: {e}")
             context._cache["VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY"] = {"failures": "Similarity computation failed."}
             return False # Fail if calculation errors out

        threshold = 0.70 # Define the threshold
        all_pass = True
        failures = []
        failed_sections = set() # Store ResumeSection enums
        ...
        for result in similarity_results:
            ...
            if max_sim >= threshold:
                ...
                failures.append(f"{section_label}: Max Similarity {max_sim:.2f} >= {threshold:.2f}")
                try:
                     # Map label back to enum for retry logic
                     failed_sections.add(ResumeSection[section_label])
                except KeyError:
                     logging.warning(f"Could not map section label '{section_label}' back to ResumeSection enum.")

        if not all_pass:
             context._cache["VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY"] = {
                 "failures": ", ".join(failures[:3]) + ('...' if len(failures)>3 else ''),
                 "failed_sections": failed_sections # Store failed Enum objects
             }
        return all_pass
