...
__version__ = "14.12" # <-- Increment version for tracking changes
try:
...
class EnhancedJobDescriptionAnalyzer:
    ...
    def __init__(
        ...
        # --- Domain themes (UPDATED based on SaaS Roles) ---
        self.domain_themes = {
            'AI/ML': ['ai', 'artificial intelligence', 'machine learning', 'ml', 'deep learning',
                     'neural network', 'llm', 'generative ai', 'nlp', 'computer vision',
                     'data science', 'predictive', 'algorithms', 'caio', 'ml strategy',
                     'ai engineering', 'ml engineering', 'ai research', 'data modeling'],
            'Cloud': ['cloud', 'aws', 'azure', 'gcp', 'kubernetes', 'docker', 'infrastructure',
                     'devops', 'microservices', 'scalability', 'distributed', 'ci/cd',
                     'sre', 'site reliability', 'cloud infra', 'cloud reliability'],
            'Leadership': ['lead', 'leadership', 'manage', 'director', 'executive', 'vp', 'evp',
                          'chief', 'head', 'head of', 'team', 'strategy', 'vision', 'roadmap',
                          'cfo', 'cmo', 'coo', 'cpo', 'cro', 'cto', 'ciso', 'svp',
                          'program management', 'pmo', 'matrixed leader', 'executive sponsor'],
            'Product': ['product', 'product management', 'product roadmap', 'r&d', 'innovation',
                        'design', 'ux', 'user experience', 'features', 'agile', 'scrum',
                        'market fit', 'customer research', 'user insights', 'market feedback',
                        'product lines', 'product portfolio', 'product strategy', 'competitive analysis',
                        'feature planning', 'market intelligence', 'usability testing', 'product owner'],
            'Customer_Success': ['customer success', 'post-sales', 'adoption', 'retention', 'renewal',
                                 'customer health', 'value realization', 'technical success',
                                 'onboarding', 'expansions', 'recurring revenue', 'implementation',
                                 'professional services', 'ps', 'account management', 'client relationship',
                                 'services delivery', 'engagement manager'],
            'Sales_GTM': ['sales', 'go-to-market', 'gtm', 'revenue', 'quota', 'pre-sales',
                          'solution engineering', 'field cto', 'sales architecture', 'revops',
                          'revenue operations', 'commercial ops', 'sales strategy', 'pipeline',
                          'business development', 'bd', 'rfp', 'proposals', 'value engineering',
                          'roi analysis', 'account executive', 'ae', 'sales enablement', 'sales training',
                          'territory management', 'strategic accounts', 'enterprise accounts'],
            'Engineering_IC': ['engineer', 'software engineer', 'ic', 'developer', 'hands-on',
                               'software development', 'coding', 'qa', 'testing', 'automation engineer',
                               'solution architect'], # Added common IC terms
            'Partnerships': ['partnership', 'partnerships', 'alliances', 'strategic alliances',
                             'channel', 'channel sales', 'ecosystem', 'business development', 'bd',
                             'partner ecosystem', 'co-selling', 'joint solutions', 'oem', 'isv',
                             'partner engineering', 'partner solutions architect', 'partner integrations',
                             'partner onboarding', 'partner enablement', 'partner success', 'partner marketing',
                             'channel development', 'distributor liaison'],
            'Enterprise': ['enterprise', 'b2b', 'saas', 'platform', 'solution', 'architecture',
                          'integration', 'api', 'deployment', 'implementation'], # Relatively stable

            'Business': ['revenue', 'growth', 'p&l', 'roi', 'kpi', 'metrics',
                        'business', 'commercial', 'budget', 'operations', 'operational efficiency',
                        'cross-functional alignment'], # Added Ops terms here

            'Data': ['data', 'analytics', 'database', 'sql', 'warehouse', 'pipeline',
                    'etl', 'big data', 'reporting', 'visualization', 'bi', 'business intelligence',
                     'data strategy'], # Added BI

            # --- NEW THEMES ---
            'Marketing': ['marketing', 'brand', 'communications', 'pr', 'public relations',
                          'content marketing', 'thought leadership', 'digital marketing', 'seo', 'sem',
                          'ppc', 'paid acquisition', 'demand generation', 'campaigns', 'martech',
                          'marketing operations', 'marketing automation', 'analyst relations', 'ar',
                          'field marketing', 'abm', 'account based marketing', 'lead nurturing',
                          'email marketing', 'marketing analytics', 'partner marketing', 'co-marketing',
                          'social media', 'community management', 'vertical marketing', 'brand strategy',
                          'brand identity'],
            'Finance': ['finance', 'financial planning', 'fp&a', 'accounting', 'controller',
                        'revenue recognition', 'asc606', 'treasury', 'cash management', 'capital',
                        'investor relations', 'ir', 'capital markets', 'shareholder', 'finops',
                        'cloud cost', 'cost management', 'tax', 'audit', 'compliance', # Compliance overlaps Security
                        'm&a', 'mergers', 'acquisitions', 'corporate development', 'corp dev',
                        'billing', 'collections', 'ar', 'revenue assurance', 'cloud economics',
                        'cost efficiency', 'capital structuring', 'debt', 'equity', 'financing',
                        'finance systems', 'erp', 'growth investments', 'capital allocation',
                        'due diligence', 'ledger', 'financial close'],
            'IT/CIO': ['it', 'information technology', 'cio', 'enterprise applications',
                       'business systems', 'business technology', 'crm', 'erp', 'it security', # IT Security overlaps CISO
                       'infrastructure management', 'gtm systems', 'it operations'],
            'Security/CISO': ['security', 'information security', 'infosec', 'cybersecurity', 'ciso',
                              'product security', 'appsec', 'compliance', 'risk management', 'soc2',
                              'iso27001', 'grc', 'governance', 'cyber risk', 'security program',
                              'threat monitoring', 'soc', 'security operations']
            # --- END NEW THEMES ---
        }
        # --- End Domain Themes Update ---

        # Initialize web RAG components (Rest of __init__ is unchanged)
    ...
    def analyze(self, job_description: str) -> Tuple[ThematicAnalysis, int]: # Return TA and call count
        ...
        try:
            ...
            logger.error(f"FATAL: Pre-RAG analysis (HOP -0.5) failed: {e}. Halting workflow.")
            # Assuming HopExecutionError is defined/imported
            raise HopExecutionError(f"HOP -0.5 Pre-RAG Analysis failed: {e}") from e


        # If web search is explicitly disabled, raise error instead of fallback
        ...
        # If web search enabled but RAG components failed init, raise error
        ...
        # Directly call the Web RAG analysis.
        # Any exception here will now propagate up and halt the workflow.
        try:
            ...
            return analysis, self.total_api_calls_hop0
             ...
             # Catch the exception, log it as fatal, and re-raise wrapped error
             logger.error(f"FATAL: Web RAG analysis failed at HOP-0: {e}. Halting workflow.", exc_info=True) # Log traceback
...
"""
    ...
    def _classify_role(self, keywords: Dict[str, int], jd_text: str) -> Dict:
        ...
        # --- START REVISED ROLE CLASSIFICATION ---
        ...
        # Industry focus check (Unchanged)
        industry_focus = "Technology" # Default
        if 'fintech' in jd_lower or 'banking' in jd_lower: industry_focus = "FinTech"
        elif 'healthcare' in jd_lower or 'pharma' in jd_lower: industry_focus = "Healthcare"
        elif 'retail' in jd_lower or 'commerce' in jd_lower: industry_focus = "Retail"
...
class ArtistGenerator:
    ...
    def __init__(self, master_resume: Dict, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis, artist_specs: Dict, previous_failures: List[ValidationResult] = None):
        ...
        # This line now calls the method to parse the PRE-LOADED specs.
        self.SECTION_GENERATION_SPECS = self._parse_specs(self.artist_specs)
        # --- END: Load specs ---

    # --- Centralized Provenance Targets (UPDATED for K0-K11) ---
    ...
    # --- START: New method to load and parse specs (Corrected from v14.06) ---
    def _parse_specs(self, raw_specs: Dict) -> Dict['ResumeSection', Dict[str, Any]]: # Use quotes if ResumeSection not defined yet
        ...
        try:
            ...
            # --- FIX: Replace 'filename' with the actual filename string ---
    ...
    # --- Centralized Gemini API Call Method (from v14.06) ---
...
"""
    ...
    # --- Main Generation Method (from v14.06) ---
    def generate(
        self,
    ...
    # --- Internal Generation Orchestrator (from v14.06) ---
    ...
    # --- Copy Methods (Updated for K0-K11 scheme) ---
    ...
    # --- START NEW DATA-DRIVEN METHODS (from v14.06) ---
    ...
    def _get_overview_wc_range(self, section_enum: ResumeSection) -> Tuple[int, int]:
        ...
        # Add ranges for other potential overviews if needed
        # elif section_enum == ResumeSection.SOME_OTHER_OVERVIEW:
    ...
    # --- Context Builders (Updated for K0-K11) ---
    def _build_context_k0_headline(self, spec: Dict) -> Dict:
        ...
        if comp_intel:
            # Safely get keywords, ensuring it's a list
            diff_keywords = getattr(comp_intel, 'differentiator_keywords', [])
            if not isinstance(diff_keywords, list): diff_keywords = []
            differentiators = diff_keywords
    ...
    def _build_context_k1_summary(self, spec: Dict) -> Dict:
        ...
        min_exec_wc = self.constraints.EXEC_SUMMARY_WORD_COUNT_MIN
        max_exec_wc = self.constraints.EXEC_SUMMARY_WORD_COUNT_MAX
        ...
        return {
            ...
            "min_wc": min_exec_wc, # Use defined min
            "max_wc": max_exec_wc, # Use defined max
    ...
    def _build_context_narrative(self, spec: Dict) -> Dict:
        ...
        if comp_intel:
             diff_keywords = getattr(comp_intel, 'differentiator_keywords', [])
             if isinstance(diff_keywords, list): rag_keywords = diff_keywords[:5]
    ...
    def _build_context_k10_skills(self, spec: Dict) -> Dict:
        ...
        try:
            primary_theme_kw = []
            if self.thematic_analysis and self.thematic_analysis.primary_theme:
                 primary_theme_kw_raw = self.thematic_analysis.primary_theme.get('keywords', [])
                 if isinstance(primary_theme_kw_raw, list): primary_theme_kw = primary_theme_kw_raw

            diff_kw = []; comp_intel = getattr(self.thematic_analysis, 'competitive_intelligence', None)
            if comp_intel:
                 diff_keywords_raw = getattr(comp_intel, 'differentiator_keywords', [])
                 if isinstance(diff_keywords_raw, list): diff_kw = diff_keywords_raw
    ...
    def _build_context_k11_cover_letter(self, spec: Dict) -> Dict:
        ...
        if comp_intel:
             diff_keywords = getattr(comp_intel, 'differentiator_keywords', [])
             if isinstance(diff_keywords, list): differentiators = diff_keywords
        narratives_data = getattr(self.thematic_analysis, 'problem_solution_narratives', None); narratives = narratives_data if isinstance(narratives_data, dict) else {}
        problem = narratives.get('common_problems', ['solving key challenges'])[0] if narratives.get('common_problems') else 'solving key challenges'
        solution = narratives.get('solution_patterns', ['delivering impactful results'])[0] if narratives.get('solution_patterns') else 'delivering impactful results'
    ...
    # --- Post-Processors (Updated for K0-K11) ---
    ...
    # --- Generic Generator Method (from v14.06) ---
    ...
    # --- Helper Methods for Context Building (Updated for K0-K11) ---
...
"""
    ...
    # --- Bullet Generation Helpers (Updated for K0-K11) ---
...
def _count_sentences(text: str) -> int:
    ...
    # The split includes the terminators as separate elements, so we filter them out
    # and also any empty strings that result from the split.
    # A typical result of split is [sentence, terminator, sentence, terminator, ...].
    # We count the non-terminator, non-empty parts.
    count = len([s for s in sentences if s and s not in '.!?'])
...
class ValidationContext:
    ...
    Calculates metrics only when needed by a rule. (Includes cross-section similarity)
    """
    ...
    # --- Property to get/cache DuplicateDetector ---
    @property
    ...
    # --- START: Refactored Generic Calculation Logic ---
    def _calculate_metric_details(self, section_enum: ResumeSection, metrics_to_calc: List[Tuple[str, Callable]], constraints: Dict[str, Any]) -> Dict:
        ...
        for metric_name, calc_func in metrics_to_calc:
            ...
            try:
                details[metric_name] = calc_func(text) if isinstance(text, str) else 0
            except Exception as e:
                logging.warning(f"Error calculating metric '{metric_name}' for section {section_enum.name}: {e}")
    ...
    def __getattr__(self, name):
        ...
        if name in self._cache:
             logging.warning(f"Accessing cached value '{name}' directly via __getattr__ fallback.")
             return self._cache[name]
    ...
    def _calculate_cover_letter_jd_similarity(self):
        ...
        if cover_letter_text and self.job_description:
            try:
                ...
                similarity = self.dup_detector.calculate_cosine_similarity(cover_letter_text, self.job_description)
            except Exception as e:
    ...
    # --- START: New Method for Cross-Section Similarity Calculation ---
    def _calculate_cross_section_similarity_details(self) -> Dict:
        """Calculates pairwise similarity between key sections."""
        details = {"failures": [], "checked_pairs": 0}
        threshold = 0.65
        sections_to_compare = [
        ...
        for section_enum in sections_to_compare:
            ...
            #     logging.warning(f"Skipping similarity check for {section_enum.name}: Content missing or invalid type.")

        checked_pairs_set = set() # Avoid double checking (A vs B and B vs A)
        ...
        for i in range(len(sections_to_compare)):
            for j in range(i + 1, len(sections_to_compare)):
                ...
                if text1 and text2: # Only compare if both sections have content
                    try:
                        similarity = self.dup_detector.calculate_cosine_similarity(text1, text2)
                        details["checked_pairs"] += 1
                        if similarity >= threshold:
                            details["failures"].append(f"{enum1.name} vs {enum2.name}: {similarity:.2f}")
                    except Exception as e:
                        logging.warning(f"Error calculating similarity between {enum1.name} and {enum2.name}: {e}")
...
from datetime import datetime # Ensure datetime is imported for validation methods
from typing import Dict, List, Optional, Any, Tuple, Set, Union # Ensure types are imported
from functools import partial # For partial function application in rule registration
...
class PreFlightValidator:
    ...
    # Class constant for signal targets configuration (UPDATED for K0-K11 and Raw Scores)
    # These MIN/MAX values now refer to RAW SCORES
    SECTION_SIGNAL_TARGETS_CONFIG = {
        # Label: (Enum, Min_Target_Raw, Max_Target_Raw, Weight, ReasoningConfig)
        "Headline": (ResumeSection.K0_HEADLINE, 0.70, 1.00, 0.05, ReasoningConfig.K0_HEADLINE_CONFIG),
        "Executive Summary": (ResumeSection.K1_EXECUTIVE_SUMMARY, 0.80, 1.10, 0.25, ReasoningConfig.K1_EXECUTIVE_SUMMARY_CONFIG),
        "Unify Overview": (ResumeSection.K2_UNIFY_OVERVIEW, 0.70, 1.00, 0.05, ReasoningConfig.K2_UNIFY_OVERVIEW_CONFIG),
        "Unify Bullets": (ResumeSection.K2_UNIFY_BULLETS, 0.70, 1.00, 0.20, ReasoningConfig.K2_UNIFY_BULLETS_CONFIG),
        "IBM Overview": (ResumeSection.K3_IBM_OVERVIEW, 0.60, 0.90, 0.05, ReasoningConfig.K3_IBM_OVERVIEW_CONFIG),
        "IBM Bullets": (ResumeSection.K3_IBM_BULLETS, 0.65, 0.95, 0.20, ReasoningConfig.K3_IBM_BULLETS_CONFIG),
        "TraderSense Narrative": (ResumeSection.K4_TRADERSENSE_NARRATIVE, 0.40, 0.65, 0.025, ReasoningConfig.K4_TRADERSENSE_NARRATIVE_CONFIG),
        "EY Narrative": (ResumeSection.K5_EY_NARRATIVE, 0.50, 0.75, 0.025, ReasoningConfig.K5_EY_NARRATIVE_CONFIG),
        "Early Career Narrative": (ResumeSection.K6_EARLY_CAREER_NARRATIVE, 0.40, 0.65, 0.025, ReasoningConfig.K6_EARLY_CAREER_NARRATIVE_CONFIG),
        "Competencies": (ResumeSection.K9_COMPETENCIES, 0.85, 1.15, 0.10, ReasoningConfig.K9_COMPETENCIES_CONFIG),
        # Note: Skills (K10) and Cover Letter (K11) are handled separately or excluded from weighted average
    }
    ...
    PROVENANCE_SPLIT_TARGETS = ArtistGenerator.PROVENANCE_SPLIT_TARGETS # Use definition from ArtistGenerator
    ...
    # --- NEW: Centralized list of required sections for presence validation ---
    REQUIRED_SECTIONS = [
    ...
    # --- RULES_CONFIG including the new VG_RAG_MIN_QUALITY rule ---
    RULES_CONFIG = [
        # --- Add this rule ---
        {
            "rule_id": "VG_RAG_MIN_QUALITY", "severity": ValidationSeverity.CRITICAL, "category": "signal",
            "validator": lambda ctx: getattr(ctx.thematic_analysis, 'signal_quality_score', 0.0) >= 0.50, # Set your threshold (e.g., 0.50)
        ...
        { # Bullet WC Range (Applies to K.2, K.3, K.9)
            ...
            "error_message": lambda ctx: f"Bullet word counts outside hardcoded ranges: {ctx._cache.get('VG_BULLET_WORD_COUNT_RANGE', {}).get('violations', 'N/A')}"
        },
        ...
        { # K.0 Headline Titles
            ...
            "error_message": lambda ctx: f"K.0 Headline contains forbidden titles: {ctx._cache.get('VG_HEADLINE_NO_TITLES', {}).get('forbidden', 'N/A')}. Headline: '{ctx.headline_details.get('headline', '')}'"
        },
        { # K.0 Headline Commas
            "rule_id": "VG_HEADLINE_NO_COMMAS", "severity": ValidationSeverity.CRITICAL, "category": "structure",
            "validator": lambda ctx: ',' not in ctx.headline_details.get('headline', ''), # Direct lambda, safe access
            "error_message": lambda ctx: f"K.0 Headline contains commas. Headline: '{ctx.headline_details.get('headline', '')}'"
        },
        { # K.0 Headline Component WC (Handled within _validate_headline_format_no_titles)
            "rule_id": "VG_HEADLINE_COMPONENT_WC", "severity": ValidationSeverity.HIGH,"category": "structure",
            "validator": "_validate_headline_format_no_titles", # Re-use validator
            "error_message": lambda ctx: f"K.0 Headline component word count outside range ({ctx._cache.get('VG_HEADLINE_COMPONENT_WC', {}).get('min', '?')}-{ctx._cache.get('VG_HEADLINE_COMPONENT_WC', {}).get('max', '?')}). Headline: '{ctx._cache.get('VG_HEADLINE_COMPONENT_WC', {}).get('headline', '')}'"
        },
        ...
        { # No Placeholders
            ...
            "error_message": lambda ctx: f"Found placeholder text in content: {ctx._cache.get('CONTENT_NO_PLACEHOLDERS', {}).get('placeholders', 'N/A')}"
        },
        { # Forbidden Verbs
            ...
            "error_message": lambda ctx: f"Forbidden verbs found in generated content: {ctx._cache.get('VG_FORBIDDEN_VERBS', {}).get('violations', 'N/A')}"
        },
        { # Banned Intro Phrases
            ...
            "error_message": lambda ctx: f"Banned introductory phrases found: {ctx._cache.get('VG_NO_INTRO_PHRASES', {}).get('violations', 'N/A')}"
        },
        { # Per-Section Signal Score (Raw Score Version)
            "rule_id": "VG_PER_SECTION_SIGNAL_SCORE", "severity": ValidationSeverity.HIGH, "category": "content",
            "validator": "_validate_per_section_signal_raw", # Use new raw score validator
            "error_message": lambda ctx: f"One or more sections outside target raw signal score range: {ctx._cache.get('VG_PER_SECTION_SIGNAL_SCORE', {}).get('failures', 'N/A')}"
        },
        { # K.1 Differentiators
            "rule_id": "VG_K1_DIFFERENTIATOR_RANGE", "severity": ValidationSeverity.CRITICAL, "category": "content",
            "validator": lambda ctx: ctx.constraints.K1_MIN_DIFFERENTIATORS <= ctx._calculate_k1_differentiator_range(ctx)['found'] <= ctx.signal_constraints.K1_MAX_DIFFERENTIATORS, # Pass context explicitly
        ...
        { # JD Keyword Range (Global)
            ...
            "error_message": lambda ctx: f"Resume contains {ctx._cache.get('VG_JD_KEYWORD_RANGE', {}).get('found', '?')} unique JD keywords (target: {ctx._cache.get('VG_JD_KEYWORD_RANGE', {}).get('min', '?')}-{ctx._cache.get('VG_JD_KEYWORD_RANGE', {}).get('max', '?')})."
        },
        ...
        { # K.11 Cover Letter Narrative
            ...
            "validator": lambda ctx: ctx._calculate_cover_letter_narrative(ctx)['valid'], # Pass context explicitly
            "error_message": lambda ctx: f"K.11 Cover letter may be missing narrative integrity. Hook: {ctx._cache.get('COVER_LETTER_NARRATIVE_INTEGRITY', {}).get('hook', '?')}, Proof: {ctx._cache.get('COVER_LETTER_NARRATIVE_INTEGRITY', {}).get('proof', '?')}, Vision: {ctx._cache.get('COVER_LETTER_NARRATIVE_INTEGRITY', {}).get('vision', '?')}"
        },
        ...
        { # Provenance Split Check (Applies to K.2, K.3, K.9)
            ...
            "error_message": lambda ctx: f"Provenance split mismatch: {ctx._cache.get('VG_PROVENANCE_SPLIT_CHECK', {}).get('violations', 'N/A')}"
        },
        { # Authenticity Signal Check
            ...
            "error_message": lambda ctx: f"Authenticity signal (verbs/phrasing) from HOP-0 not detected in resume content: {ctx._cache.get('VG_AUTHENTICITY_SIGNAL_CHECK', {}).get('details', 'N/A')}"
        },
        { # Exec Summary vs Section Similarity
            ...
            "error_message": lambda ctx: f"Executive Summary similarity to sections exceeds threshold: {ctx._cache.get('VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY', {}).get('failures', 'N/A')}"
        },
    ]
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        rule_map = {
            ...
            "VG_RAG_MIN_QUALITY": "GLOBAL", # Added new global rule
            # Visual Rules
            ...
            "VG_HEADLINE_COMPONENT_WC": ResumeSection.K0_HEADLINE, # Added mapping
        ...
        # Dynamically add structure rules for all REQUIRED_SECTIONS
        ...
        # Add explicit mappings for headers if not covered by REQUIRED_SECTIONS loop
        header_enums = [
            ResumeSection.K0_EXECUTIVE_SUMMARY_HEADER, ResumeSection.K0_EXPERIENCE_HEADER,
            ResumeSection.K0_EDUCATION_HEADER, ResumeSection.K0_CERTIFICATIONS_HEADER,
            ResumeSection.K0_COMPETENCIES_HEADER
    ...
    def _register_rules(self):
        ...
        logger = logging.getLogger(__name__)
        all_rules_config = list(self.RULES_CONFIG) # Start with the static rules

        # Dynamically generate structure presence rules from the REQUIRED_SECTIONS list
        for section_enum in self.REQUIRED_SECTIONS:
            rule_id = f"STRUCTURE_{section_enum.name}_PRESENT"
            # Avoid adding duplicates if already defined in RULES_CONFIG
        ...
        for config in all_rules_config:
            ...
            if isinstance(validator_ref, str):
                ...
                if validator_func is None:
                    ...
                    # Optional: raise AttributeError(msg) # Or raise error to halt setup
            elif callable(validator_ref):
                 validator_func = validator_ref # Use the lambda or partial directly
            else:
            ...
            def create_error_message_lambda(template, rule_id_for_cache):
                # Ensure template is a string or callable
                ...
                def error_lambda(ctx: ValidationContext):
                    try:
                        ...
                        # If template is callable, call it with context/details
                        if callable(template):
                             return str(template(ctx)) # Call original lambda from config
                        # Otherwise, treat as string template and format
                        else:
                        ...
                        logger.error(f"Error formatting error message for rule {rule_id_for_cache}: {e}. Template type: '{type(template)}' Details: {details}")
    ...
    def _validate_section_presence(self, context: ValidationContext, section_enum: ResumeSection) -> bool:
        """Generic validator to check for the presence and non-emptiness of a section."""
        content = context.staging_buffer.get(section_enum.value)
        if content is None:
            return False
        if isinstance(content, str):
            # Check against known placeholder strings more explicitly
            return content.strip() not in ["", "HEADER_PLACEHOLDER"] and not content.strip().startswith("[Placeholder")
        if isinstance(content, (list, dict)):
            return bool(content) # Returns False for empty list/dict
        return True # For other non-empty types like numbers
    ...
    def _validate_cover_letter_full_structure(self, context: ValidationContext) -> bool:
        ...
        if not valid:
             # Cache details for error message formatting
    ...
    def _validate_cover_letter_structure(self, context: ValidationContext) -> bool:
        # This rule checks paragraph word counts using details calculated by ValidationContext
        details = context.cover_letter_structure_details # Uses K.11 internally now
        if details.get("error"): return False # Fail if structure parsing failed
    ...
    def _validate_bullet_word_count_range(self, context: ValidationContext) -> bool:
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            if target_range is None:
                 logging.warning(f"Word count range missing for {section_enum.name} in ArtistGenerator config. Skipping check.")
                 continue # Skip if no range defined for this section
            min_wc, max_wc = target_range
            bullets = context.staging_buffer.get(section_key, [])
            if not isinstance(bullets, list):
                logging.warning(f"Expected list for {section_key} bullets, got {type(bullets)}. Skipping check.")
                continue
            ...
            for i, bullet in enumerate(bullets):
                ...
                if isinstance(bullet, dict):
                    ...
                    logging.warning(f"Invalid bullet item type in {section_key}[{i}]: {type(bullet)}. Skipping.")
                    continue
    ...
    def _validate_headline_format_no_titles(self, context: ValidationContext) -> bool:
        """Validates K.0 Headline: 3 components, WC range, no titles. Caches details."""
        details = context.headline_details # Trigger calculation & caching
        headline = details.get('headline', '')
        if not headline or '|' not in headline:
             details['error'] = "Missing pipes"
             context._cache["VG_HEADLINE_NO_TITLES"] = details; context._cache["VG_HEADLINE_COMPONENT_WC"] = details
             return False # Fail if no pipes

        components = [c.strip() for c in headline.split('|')]
        if len(components) != 3:
             details['error'] = f"Expected 3 components, found {len(components)}"
             context._cache["VG_HEADLINE_NO_TITLES"] = details; context._cache["VG_HEADLINE_COMPONENT_WC"] = details
             return False # Fail if not exactly 3 components

        forbidden_titles = ['director', 'vp', 'manager', 'lead', 'head', 'chief', 'principal', 'senior', 'executive']
        forbidden_found = []
        component_wc_violations = []
        min_comp_wc = context.constraints.HEADLINE_COMPONENT_WORDS_MIN
        max_comp_wc = context.constraints.HEADLINE_COMPONENT_WORDS_MAX
        ...
        for i, comp in enumerate(components):
            ...
            for title in forbidden_titles:
                 if re.search(r'\b' + re.escape(title) + r'\b', comp.lower()):
                      forbidden_found.append(title)

        # Update cache for specific rules before returning
        details_titles = details.copy(); details_titles['forbidden'] = list(set(forbidden_found))
        context._cache["VG_HEADLINE_NO_TITLES"] = details_titles

        details_wc = details.copy(); details_wc['min'] = min_comp_wc; details_wc['max'] = max_comp_wc; details_wc['wc_violations'] = component_wc_violations
        context._cache["VG_HEADLINE_COMPONENT_WC"] = details_wc

        # Return False if either check failed
        return not forbidden_found and not component_wc_violations
    ...
    def _validate_no_placeholders(self, context: ValidationContext) -> bool:
        ...
        def check(item, key_enum=None):
            ...
            if isinstance(item, str) and ("[Placeholder" in item or "[Your Name]" in item or "[Company Name]" in item):
    ...
    def _validate_no_intro_phrases(self, context: ValidationContext) -> bool:
        ...
        for section_enum in sections_to_check:
            ...
            if isinstance(content, str):
                if is_cl:
                    # Extract body paragraphs more carefully
                    body_text = content # Start with full text
                    # Remove header/salutation
    ...
    def _validate_per_section_signal_raw(self, context: ValidationContext) -> bool:
        """Validates signal score using raw scores and raw target ranges."""
        valid = True; failures = []; failed = set()
        # Use updated config mapping K0-K11 enums with RAW targets
        for label, (section_enum, target_min_raw, target_max_raw, _, _) in self.SECTION_SIGNAL_TARGETS_CONFIG.items():
            content = context.staging_buffer.get(section_enum.value)
            raw_score = 0.0
            if content:
                # Assuming calculate_signal_score is modified to return RAW score
                # For now, simulate by calling original and maybe scaling/adjusting
                # Replace this with the actual call to the modified function
                try:
                     # raw_score = calculate_signal_score_raw(content, context.thematic_analysis) # Call the new raw score function
                     # --- Placeholder simulation ---
                     normalized_score = calculate_signal_score(content, context.thematic_analysis)
                     raw_score = normalized_score # In reality, raw could be > 1.0
                     if section_enum == ResumeSection.K1_EXECUTIVE_SUMMARY and raw_score > 0.9: raw_score = 1.15 # Simulate exceeding 1.0
                     # --- End simulation ---
                except Exception as e:
                     logging.warning(f"Error calculating raw signal score for {label}: {e}")
                     raw_score = 0.0

            # Check against strict RAW target range
            if not (target_min_raw <= raw_score <= target_max_raw):
                valid = False
                failures.append(f"{label}({section_enum.name}): Raw {raw_score:.2f} (Tgt: {target_min_raw:.2f}-{target_max_raw:.2f})")
    ...
    def _calculate_k1_differentiator_range(self, context: ValidationContext) -> Dict:
        # Calculates and caches details, returns the dict for lambda
        ...
        # Ensure differentiators are valid strings before checking
        ...
        context._cache["VG_K1_DIFFERENTIATOR_RANGE"] = details
        return details

    def _validate_jd_keyword_range(self, context: ValidationContext) -> bool:
        full_text = ""; buffer_data = context.staging_buffer.data
        # Aggregate text, excluding headers/contact and Cover Letter
        sections_to_include = [
            se for se in ResumeSection
            if se not in [ResumeSection.K0_NAME, ResumeSection.K0_CONTACT, ResumeSection.K11_COVER_LETTER]
            and not se.name.endswith("_HEADER")
        ]
    ...
    def _calculate_cover_letter_narrative(self, context: ValidationContext) -> Dict:
        ...
        # Simple keyword checks for narrative components
    ...
    def _validate_provenance_split(self, context: ValidationContext) -> bool:
        ...
        for section_enum, targets in self.PROVENANCE_SPLIT_TARGETS.items():
            ...
            if not isinstance(bullets, list):
                logging.warning(f"Expected list for {section_enum.value} provenance check, got {type(bullets)}. Skipping.")
                continue
            ...
            for prov_type_enum in BulletProvenance: # Assumes BulletProvenance enum exists
    ...
    def _validate_authenticity_signal(self, context: ValidationContext) -> bool:
        ...
        patterns_dict = {}
        if isinstance(auth_patterns_data, dict):
             patterns_dict = auth_patterns_data.get('patterns', {})
             if not isinstance(patterns_dict, dict): patterns_dict = {} # Ensure nested patterns is dict
        if not patterns_dict: return True # Pass if no patterns

        verbs = patterns_dict.get('achievement_verb_patterns', []); phrasing = patterns_dict.get('competency_phrasing', [])
        # Ensure verbs/phrasing are lists of strings
        valid_verbs = [v for v in verbs if isinstance(v, str)]
        valid_phrasing = [p for p in phrasing if isinstance(p, str)]

        target_signals = set(v.lower() for v in valid_verbs[:10]) | set(p.lower().split(':')[0].split()[0] for p in valid_phrasing[:5] if ':' in p and p.split())
        target_signals = {s for s in target_signals if s} # Remove empty strings
    ...
    def _validate_exec_summary_vs_sections(self, context: ValidationContext) -> bool:
        ...
        for sec_enum in enums_to_compare:
            ...
            if content:
                 # Extract text from bullets/items safely
                 if isinstance(content, list):
                      texts = [str(item.get('text', str(item))) if isinstance(item,dict) else str(item) for item in content]
                      sections_content[sec_enum.name] = [t for t in texts if t] # Filter empty strings
                 elif isinstance(content, str) and content.strip():
                      sections_content[sec_enum.name] = content
        ...
        for res in similarity_results:
            ...
            if max_sim >= threshold:
                ...
                try: failed.add(ResumeSection[label]) # Map label string back to enum
    ...
    def validate(
        ...
        """
        Run validation rules using the ValidationEngine.
        Optionally filters rules to only those relevant to sections_under_test.
        Returns: (all_results, all_passed, failed_sections_enums)
        """
        logger = logging.getLogger(__name__)
        ...
        failed_sections_enums = set() # Track enums of sections with failures
        ...
        if sections_under_test:
            ...
            for rule_id, section_map in self.RULE_TO_SECTION_MAP.items():
                if section_map == "GLOBAL" or section_map == "VISUAL" or section_map in sections_under_test:
                    ...
                    # For complex rules, check if *any* of the sections they *might* affect
                    # are under test. This is an approximation.
                    # Example: Bullet WC affects K2, K3, K9. If any are under test, run the rule.
                    if rule_id == "VG_BULLET_WORD_COUNT_RANGE" and any(s in sections_under_test for s in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK):
                         relevant_rule_ids.add(rule_id)
                    elif rule_id == "VG_PROVENANCE_SPLIT_CHECK" and any(s in sections_under_test for s in self.PROVENANCE_SPLIT_TARGETS):
                         relevant_rule_ids.add(rule_id)
                    # Add similar logic for other complex rules (VG_FORBIDDEN_VERBS, VG_NO_INTRO_PHRASES) checking against their affected section lists
                    # For simplicity now, include them if any section is under test
                    elif rule_id in ["VG_FORBIDDEN_VERBS", "VG_NO_INTRO_PHRASES"]:
                         relevant_rule_ids.add(rule_id)
        ...
        all_results = self.engine.validate(context, categories=None) # Run all relevant rules
        # Filter results again to ensure only rules intended for this run are included
        final_results_for_run = [r for r in all_results if r.rule_id in {rule.rule_id for rule in rules_to_run}]


        # Check for HIGH or CRITICAL failures
        ...
        # Identify sections associated with HIGH/CRITICAL failures
        if not all_passed:
            for vr in final_results_for_run:
                if not vr.passed and vr.severity.value >= ValidationSeverity.HIGH.value:
                    ...
                    if isinstance(section_map, ResumeSection):
                        ...
                        # Add sections identified by the complex validator's cached details
                        ...
                        if isinstance(failed_in_cache, set):
                             failed_sections_enums.update(failed_in_cache)
                    # Ignore GLOBAL/VISUAL failures for section mapping
...
# ============================================================================
# HOP-7: FILE RENDERING (OVERWRITE with Fence Enforcement)
# ============================================================================
...
class FileRenderer:
    ...
    # +++ START: Added Fence Stripping Helper +++
    def _strip_fences(self, content: str, artifact_name: str) -> str:
        ...
        if found_fence:
            ...
            # Optionally, add a ValidationResult here if you want this logged formally
            # validation_results.append(ValidationResult(
            #     rule_id=f"RENDER_{artifact_name.upper()}_FENCE_STRIPPED", passed=True, severity=ValidationSeverity.WARNING,
            #     message=f"Markdown fences stripped from {artifact_name}."
            # ))
        return stripped_content
    # +++ END: Added Fence Stripping Helper +++
...
# Inside class FileRenderer:
    ...
    def _render_app_tracker_artifact(self, file_paths: Dict[str, str], jd_url: str = "") -> Tuple[str, str, List[ValidationResult]]:
        ...
        # --- App Tracker should NOT have fences stripped ---
    ...
    # --- Resume Render Config (Unchanged from original code) ---
    ...
    def _initialize_render_dispatch(self):
        ...
        self._RENDER_DISPATCH = {
            ...
            # --- Need handlers for the new types used in config ---
            ...
            # Keep original list handler for generic lists if needed elsewhere
            "list": self._handle_render_list,
        }

    # --- Render Dispatch Handlers (Unchanged from original code) ---
    ...
    def _handle_render_list(self, config: Dict, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        if content and isinstance(content, list):
            ...
            if config["source"] == ResumeSection.K7_EDUCATION:
                # Education format: Degree, Institution (Notes)
    ...
    # --- Specific Render Methods (Unchanged from original code) ---
    ...
    def _render_list_section(self, content_list: List[Union[str, Dict]], item_prefix: str = "") -> str:
        ...
        for item in content_list:
            ...
            # Apply prefix only if needed (e.g., Certs, but not Edu or Competencies which have format baked in)
            if text_to_render:
                # Competencies format (from K9 post-processor) is "Skill: Description"
                # Check if text already starts with potential markdown like * or **
    ...
    # --- Render Skills and App Tracker (Unchanged fence logic) ---
...
class WorkflowOrchestrator:
    ...
    # --- START REFACTOR: Simplify _invoke_deduplication_analysis ---
    ...
    def _gather_content_for_dedup(self, staging_buffer: ImmutableStagingBuffer) -> Tuple[Dict, Dict, str, Dict]:
        """Gathers and organizes all content needed for the three deduplication analyses."""
        sections_for_matrix = {}
        overview_bullet_pairs = {}
        exec_summary_comparison_data = {}
        exec_summary_text = self._get_text_from_item(staging_buffer.get(ResumeSection.K1_EXECUTIVE_SUMMARY.value))
        ...
        for section_enum, label in section_map.items():
            ...
            # 1. Gather content for Executive Summary comparison
            if isinstance(content, list):
                exec_summary_comparison_data[label] = [self._get_text_from_item(item) for item in content if self._get_text_from_item(item)]
            elif isinstance(content, str):
                exec_summary_comparison_data[label] = content

            # 2. Gather content for the main pairwise similarity matrix
            if label != "Skills": # Exclude skills from the main matrix
                if label == "Cover Letter" and isinstance(content, str):
                    # Split cover letter into paragraphs for more granular comparison
                    body_paras = [p.strip() for p in content.split('\n\n') if p.strip() and "Sincerely," not in p and "Dear" not in p]
                    if body_paras: sections_for_matrix[label] = body_paras
                elif isinstance(content, list):
                    texts = [self._get_text_from_item(item) for item in content if self._get_text_from_item(item)]
                    if texts: sections_for_matrix[label] = texts
                elif isinstance(content, str):
                    sections_for_matrix[label] = [content]

            # 3. Gather pairs for Narrative vs. Master Highlight comparison
            master_index_map = {"TraderSense Narrative": 2, "EY Narrative": 3, "Early Career Narrative": 4}
            if label in master_index_map and isinstance(content, str):
                master_idx = master_index_map[label]
                if master_idx < len(master_experience):
                    master_highlights = [h for h in master_experience[master_idx].get('highlights', []) if isinstance(h, str) and h.strip()]
                    if content and master_highlights:
                        overview_bullet_pairs[label] = {"overview": content, "bullets": master_highlights}

        return sections_for_matrix, overview_bullet_pairs, exec_summary_text, exec_summary_comparison_data

    def _invoke_deduplication_analysis(self, staging_buffer: ImmutableStagingBuffer) -> bool:
        """
        Invokes all deduplication analyses and stores results on the instance.
        ...
        if not hasattr(self, 'dup_detector') or self.dup_detector is None:
            self.logger.warning("DuplicateDetector not found. Skipping deduplication analysis.")
            return False

        # Gather all necessary content in a structured way
        sections_for_matrix, overview_bullet_pairs, exec_summary_text, exec_summary_comparison_data = self._gather_content_for_dedup(staging_buffer)
        ...
        try:
            ...
            self.logger.error(f"Error computing pairwise similarity matrix: {e}", exc_info=False)
            self.similarity_matrix_data = None
...
from typing import Dict, List, Optional, Any, Tuple, Union
from datetime import datetime
import json # For formatting thematic analysis details
from collections import defaultdict # For formatting error messages
# ============================================================================
# QA REPORT GENERATOR CLASS (Refactored w/ Specific Methods for 6, 9, 10)
# ============================================================================
import textwrap # Needed for _wrap_cell_text
import re
import logging
...
import json # For formatting thematic analysis details
from collections import defaultdict # For formatting error messages
# Assume necessary classes/functions are defined elsewhere
# ============================================================================
# QA REPORT GENERATOR CLASS (Corrected v14.11 Structure - No Generic Builder)
# ============================================================================
import textwrap
import re
import logging
...
# Assume necessary classes/functions (ValidationResult, ValidationSeverity, etc.) are defined elsewhere
...
from collections import defaultdict # For formatting error messages
# Assume necessary classes/functions (ValidationResult, ValidationSeverity, etc.) are defined elsewhere
# Assume __version__ is defined globally
...
# ============================================================================
# QA REPORT GENERATOR CLASS (v14.11 Structure - Corrected Syntax/Scope)
# ============================================================================
import textwrap
import re
import logging
...
import json
from collections import defaultdict
# Assume necessary classes/functions (ValidationResult, ValidationSeverity, etc.)
# and global variables (__version__, APP_TRACKER_SCHEMA_DATA) are defined elsewhere.
# Assume utility functions (calculate_signal_score, count_words_ms_word_style, _count_sentences)
# are defined in the global scope or imported.
...
import json # For formatting thematic analysis details
from collections import defaultdict # For formatting error messages
# Assume necessary classes/functions (ValidationResult, ValidationSeverity, ResumeSection, etc.)
# and global variables (__version__, APP_TRACKER_SCHEMA_DATA) are defined elsewhere.
# Assume utility functions (calculate_signal_score, count_words_ms_word_style, _count_sentences)
# are defined in the global scope or imported.
# Assume WorkflowOrchestrator, ImmutableStagingBuffer, ThematicAnalysis,
# PreFlightValidator, ArtistGenerator, DuplicateDetector classes are defined/imported.

class QAReportGenerator:
    """Generates the QA report by building individual sections (v14.11 structure - No Generic Builder)."""

    # --- QA_REPORT_SECTIONS calling specific methods for 1-12 ---
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_10_cover_letter_qa", "args": ["validation_results"]}, # Specific method
        {"method": "_build_qa_section_11_jd_enforcement", "args": []},
        {"method": "_build_qa_section_12_final_format", "args": ["validation_results", "file_contents"]},
    ]
    # --- END QA_REPORT_SECTIONS ---

    def __init__(self, orchestrator: 'WorkflowOrchestrator'):
        """Initializes the QA Report Generator."""
        self.orchestrator = orchestrator # Provides access to checkpoints, dup_detector, etc.
        self.logger = logging.getLogger(__name__)
    ...
    def generate(
        ...
        for i, section_config in enumerate(self.QA_REPORT_SECTIONS, 1):
            ...
            try:
                ...
                # Pass args in the order specified by arg_names
                final_args_list = [call_args[name] for name in arg_names if name in call_args]
                section_lines = builder_method(*final_args_list) # Call specific method
        ...
        # Determine overall status based ONLY on QA generation/formatting checks
    ...
    def _build_qa_section_1_signal_quality(
        ...
        kw_status_icon = "❌ FAIL"
        kw_details = "N/A (Rule Result Missing)"
        if jd_rule:
        ...
        # Assuming PreFlightValidator class is accessible
        ...
        for label, (section_enum, target_min_raw, target_max_raw, weight, _) in raw_target_config.items():
            ...
            if content:
                try:
                    ...
                    if section_enum in [ResumeSection.K1_EXECUTIVE_SUMMARY, ResumeSection.K9_COMPETENCIES] and raw_score > 0.9:
                        raw_score = min(raw_score * 1.2, 1.3)
                except Exception as e:
            ...
            temp_used = final_temps.get(section_enum.name, 'N/A') # Use Enum name as key
            lines.append(self._format_ascii_bar_chart(label, raw_score, target_min_raw, target_max_raw, temp_used, score_display_mode="raw"))

            total_weighted_raw_score += raw_score * weight
            total_section_weight += weight
            total_weighted_min_target += target_min_raw * weight
            total_weighted_max_target += target_max_raw * weight
        ...
        lines.append("-" * 95)
        lines.append(self._format_ascii_bar_chart("Weighted Average", avg_raw_score, avg_min_target, avg_max_target, avg_temp_val, is_summary=True, score_display_mode="raw"))
        lines.append("-" * 95)

        skills_enum = ResumeSection.K10_SKILLS
        skills_content = staging_buffer.get(skills_enum.value)
        skills_raw_score = 0.0
        if skills_content:
            try:
        ...
        lines.append(self._format_ascii_bar_chart("Skills (Not in Avg)", skills_raw_score, None, None, 'N/A', score_display_mode="raw", status_override="INFO"))

        cl_enum = ResumeSection.K11_COVER_LETTER
        cl_content = staging_buffer.get(cl_enum.value)
        cl_raw_score = 0.0
        cl_target_min_raw, cl_target_max_raw = 0.80, 1.10 # Example targets
        if cl_content:
            try:
                normalized_score = calculate_signal_score(cl_content, thematic_analysis) # Needs to be accessible
                cl_raw_score = normalized_score
                if cl_raw_score > 0.9:
                    cl_raw_score = min(cl_raw_score * 1.1, 1.3)
            except Exception as e:
        ...
        lines.append(self._format_ascii_bar_chart("Cover Letter (Not in Avg)", cl_raw_score, cl_target_min_raw, cl_target_max_raw, 'N/A', score_display_mode="raw", status_override=cl_status))
        ...
        if signal_fail_rule and not signal_fail_rule.passed:
             ...
             try:
                 ...
                 # Check if message is callable (lambda) before calling
                 if callable(signal_fail_rule.message):
                     fail_msg = signal_fail_rule.message(safe_details)
                 else:
    ...
    def _build_qa_section_2_signal_flow_map(
        ...
        rows_config = [ {"section": ResumeSection.K0_HEADLINE, "label": "K.0 (Headline)", "signals": ["Primary", "Diff_Pct", "Archetype", None, None, "Auth"]}, {"section": ResumeSection.K1_EXECUTIVE_SUMMARY, "label": "K.1 (Exec Summary)", "signals": ["Primary", "Diff_Count", "Archetype", "Narrative", "Secondary", "Auth"]}, {"section": ResumeSection.K2_UNIFY_BULLETS, "label": "K.2 (Unify Bullets)", "signals": ["Primary", "Diff_Pct", None, None, "Secondary", "Auth"]}, {"section": ResumeSection.K2_UNIFY_OVERVIEW, "label": "K.2 (Unify Overview)", "signals": ["Primary", "Diff_Pct", None, None, "Secondary", "Auth"]}, {"section": ResumeSection.K3_IBM_BULLETS, "label": "K.3 (IBM Bullets)", "signals": ["Primary", "Diff_Pct", None, None, "Secondary", "Auth"]}, {"section": ResumeSection.K3_IBM_OVERVIEW, "label": "K.3 (IBM Overview)", "signals": ["Primary", "Diff_Pct", None, None, "Secondary", "Auth"]}, {"section": ResumeSection.K4_TRADERSENSE_NARRATIVE, "label": "K.4 (TraderSense Nar.)", "signals": ["Primary", "Diff_Pct", None, "Narrative", None, "Auth"]}, {"section": ResumeSection.K5_EY_NARRATIVE, "label": "K.5 (EY Narrative)", "signals": ["Primary", "Diff_Pct", None, "Narrative", None, "Auth"]}, {"section": ResumeSection.K6_EARLY_CAREER_NARRATIVE, "label": "K.6 (Early Car. Nar.)", "signals": ["Primary", "Diff_Pct", None, "Narrative", None, "Auth"]}, {"section": ResumeSection.K9_COMPETENCIES, "label": "K.9 (Competencies)", "signals": ["Primary", "Diff_Pct", None, None, "Secondary", "Auth"]}, {"section": ResumeSection.K10_SKILLS, "label": "K.10 (Skills)", "signals": ["Primary", "Diff_Pct", None, None, "Secondary", None]}, {"section": ResumeSection.K11_COVER_LETTER, "label": "K.11 (Cover Letter)", "signals": ["Primary", "Diff_Pct", None, "Narrative", None, "Auth"]}, ]
        table_rows = []
        comp_intel = getattr(thematic_analysis, 'competitive_intelligence', None)
        differentiators = getattr(comp_intel, 'differentiator_keywords', []) if comp_intel else []
        valid_diffs = [kw for kw in differentiators if kw and isinstance(kw, str)] # Pre-filter valid keywords
        ...
        for row_cfg in rows_config:
            ...
            row_data.append("✓" if signal_presence[0] == "Primary" else "N/A") # Primary
            if signal_presence[1] == "Diff_Pct" and content:
                score = calculate_signal_score(content, thematic_analysis) # Needs to be accessible
        ...
        col_widths = [25, 18, 20, 18, 18, 18, 25]
    ...
    # Section 3: Hop Summary (Modified for API Calls)
    def _build_qa_section_3_hop_summary(self) -> List[str]:
        lines = ["", "3. HOP-BY-HOP EXECUTION SUMMARY", ""]
        lines.append("```markdown")
        headers = ["Hop ID", "Hop Name", "Status", "Duration (s)", "Gemini API Calls", "Output Hash"]
        rows = []
        ...
        for hop in hop_checkpoints:
            ...
            api_calls = hop.metadata.get("gemini_api_calls", 0)
            rows.append([
        ...
        if len(rows) != expected_rows:
            rows.append(["ERROR", "TRUNCATION_DETECTED", "FAIL", "N/A", "N/A", f"Expected {expected_rows}, got {len(rows)}"])

        lines.extend(self._format_plain_text_table(headers, rows, alignments=['L', 'L', 'L', 'R', 'R', 'L']))
        lines.append("```")
        lines.append("Note: 'Gemini API Calls' reflects the total calls to the `generate_content` endpoint for each hop.")
        return lines

    # Section 4: Word Count Distribution
    def _build_qa_section_4_word_count_distribution(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        lines = ["", "4. WORD COUNT & DISTRIBUTION COMPLIANCE", ""]
        lines.append("This section combines overall resume word count checks, key section constraints (Headline, Exec Summary, Cover Letter paragraphs), and distribution metrics (Unify/IBM) into a single table. Bullet length validation is covered in Section 5.")
        lines.append("")
        lines.append("```markdown")
        headers = ["Section", "Rule ID", "Value", "Target Range", "Status", "Message / Details"]
        rows = []
        rules_to_include = { "VG_TOTAL_WORD_COUNT": "Overall Resume", "VG_HEADLINE_WORD_COUNT": "K.0 Headline", "VG_SENTENCE_COUNT_K1": "K.1 Exec Summary", "VG_WORD_COUNT_K1": "K.1 Exec Summary", "VG_WORD_COUNT_K2_OVERVIEW": "K.2 Unify Overview", "VG_SENTENCE_COUNT_K2_OVERVIEW": "K.2 Unify Overview", "VG_WORD_COUNT_K3_OVERVIEW": "K.3 IBM Overview", "VG_SENTENCE_COUNT_K3_OVERVIEW": "K.3 IBM Overview", "VG_NARRATIVE_WORD_COUNT_K4": "K.4 TraderSense Nar.", "VG_NARRATIVE_SENTENCE_COUNT_K4": "K.4 TraderSense Nar.", "VG_NARRATIVE_WORD_COUNT_K5": "K.5 EY Narrative", "VG_NARRATIVE_SENTENCE_COUNT_K5": "K.5 EY Narrative", "VG_NARRATIVE_WORD_COUNT_K6": "K.6 Early Career Nar.", "VG_NARRATIVE_SENTENCE_COUNT_K6": "K.6 Early Career Nar.", "WORD_DISTRIBUTION_UNIFY_IBM": "Experience Dist.", "UNIFY_IBM_RATIO": "Experience Dist.", "CL_P1_WORD_COUNT": "K.11 Cover Letter P1", "CL_P2_WORD_COUNT": "K.11 Cover Letter P2", "CL_P3_WORD_COUNT": "K.11 Cover Letter P3", }
        cl_structure_result = next((vr for vr in validation_results if vr.rule_id == "COVER_LETTER_STRUCTURE"), None)
        cl_paragraph_details = cl_structure_result.details if cl_structure_result and isinstance(cl_structure_result.details, dict) else {}
        ...
        for rule_id, section_label in rules_to_include.items():
            ...
            if rule_id.startswith("CL_P"):
                p_num_str = rule_id.split('_')[1]; p_key_wc, p_key_min, p_key_max = f"{p_num_str.lower()}_wc", f"{p_num_str.lower()}_min", f"{p_num_str.lower()}_max"
                if cl_structure_result:
                    wc = cl_paragraph_details.get(p_key_wc, 'N/A'); min_r = cl_paragraph_details.get(p_key_min, 'N/A'); max_r = cl_paragraph_details.get(p_key_max, 'N/A')
                    value_str = f"{wc} words" if wc != 'N/A' else "N/A"; target_str = f"{min_r}-{max_r}" if min_r != 'N/A' else "N/A"; status = "N/A"; msg = ""
                    if not cl_structure_result.passed and cl_paragraph_details.get('error'): status, msg = "FAIL", cl_paragraph_details.get('error') # Use cached error
                    elif wc != 'N/A' and min_r != 'N/A' and max_r != 'N/A' and isinstance(wc, int) and isinstance(min_r, int) and isinstance(max_r, int): status = "PASS" if min_r <= wc <= max_r else "FAIL"
                    else: status, msg = "WARN", "Paragraph details missing or invalid types"
                    if status == "FAIL" and not msg: msg = f"Count ({wc}) outside range ({min_r}-{max_r})"
                else: msg = "COVER_LETTER_STRUCTURE rule missing"
                rows.append([section_label, rule_id, value_str, target_str, status, msg])
            elif result:
                details = result.details or {}; value_str = "N/A"
                # Extract value based on expected keys in details
                ...
                elif 'unify_ibm_percent' in details: value_str = f"{details['unify_ibm_percent']:.1f}%"
                elif 'unify_ibm_ratio' in details: value_str = f"{details['unify_ibm_ratio']:.2f}"
                target_str = "N/A"
                ...
                status = "PASS" if result.passed else "FAIL"; msg = "[Error formatting msg]"
                if not result.passed:
                    try: msg_template = str(result.message); msg = msg_template.format_map(defaultdict(lambda: '[N/A]', **details)) if not callable(result.message) else result.message(defaultdict(lambda: '[N/A]', **details))
                    except Exception as e: self.logger.warning(f"Error formatting QA msg for {rule_id}: {e}")
                else: msg = ""
                rows.append([section_label, rule_id, value_str, target_str, status, msg])
            else:
                rows.append([section_label, rule_id, "N/A", "N/A", "WARN", "Validation result not found"])
        lines.extend(self._format_plain_text_table(headers, rows)); lines.append("```")
        return lines
    ...
    def _build_qa_section_5_provenance(
        ...
        lines = ["", "5. BULLET PROVENANCE & WORD COUNT", ""]; lines.append("```markdown")
        headers = ["Section", "Item", "Provenance", "Word Count", "Target Range", "Status", "Text Snippet"]; rows = []
        sections_to_process = [ {"label": "K.2 Unify", "bullets_enum": ResumeSection.K2_UNIFY_BULLETS}, {"label": "K.3 IBM", "bullets_enum": ResumeSection.K3_IBM_BULLETS}, {"label": "K.9 Competencies", "bullets_enum": ResumeSection.K9_COMPETENCIES}, ]
        ...
        for section_data in sections_to_process:
            label, bullets_enum = section_data["label"], section_data.get("bullets_enum")
            if not bullets_enum: continue
            target_range = artist_bullet_ranges.get(bullets_enum)
            if not target_range: self.logger.warning(f"Word count range missing for {bullets_enum.name}. Using default."); target_range = (20, 35)
            min_target, max_target = target_range; target_range_str = f"{min_target}-{max_target}"
            self.logger.debug(f"Processing Section '{label}', using Target={target_range_str}")
            ...
            if isinstance(bullets, list) and bullets:
                for i, bullet_item in enumerate(bullets):
                    ...
                    if isinstance(bullet_item, dict):
                        bullet_text = bullet_item.get('text', ''); word_count = bullet_item.get('word_count', count_words_ms_word_style(bullet_text)); provenance = bullet_item.get('provenance', 'N/A')
                    else: self.logger.warning(f"Unexpected item type in {bullets_enum.value}[{i}]: {type(bullet_item)}. Skipping."); continue
                    self.logger.debug(f"  Bullet {i+1}: WordCount={word_count}, Target={target_range_str}")
                    status, _ = self._check_word_count(word_count, min_target, max_target)
                    rows.append([ label, str(i + 1), provenance, str(word_count), target_range_str, status, bullet_text[:60] + ("..." if len(bullet_text) > 60 else "") ])
            elif not isinstance(bullets, list): self.logger.warning(f"Expected list for bullets in section '{label}' ({bullets_enum.value}), but got {type(bullets)}. Skipping.")
            elif not bullets: rows.append([label, "N/A", "N/A", "0", target_range_str, "WARN", "No bullets found for section."])
        lines.extend(self._format_plain_text_table(headers, rows)); lines.append("```")
        return lines

    # Section 6: Content Authenticity (Specific Method - CORRECTED)
    def _build_qa_section_6_content_authenticity(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        title = "6. CONTENT AUTHENTICITY (Hallucination & Signal Check)"
        description = "Checks for potential content hallucinations and alignment with authenticity signals from RAG."
        rule_id_patterns = ["HALLUCINATION", "VG_AUTHENTICITY_SIGNAL_CHECK"]
        lines = ["", title, "", description, ""]
        lines.append("```markdown")
        headers = ["Rule ID", "Status", "Message / Details"]
        rows = []
        filtered_results = [ vr for vr in validation_results if any(pattern in vr.rule_id for pattern in rule_id_patterns) ] # Semicolon removed

        if filtered_results:
            filtered_results.sort(key=lambda vr: vr.rule_id)
            for vr in filtered_results:
                details = vr.details or {}
                msg = "[Error formatting msg]"
                try:
                    msg_template = str(vr.message)
                    # Check if message is callable (lambda)
                ...
                # Initialize details_str before the if block
                details_str = ""
                # --- CORRECTED BLOCK START ---
                if not vr.passed and details:
                    try:
                        # Attempt JSON dump first for cleaner output
                        details_str = f" Details: {json.dumps(details, indent=None, ensure_ascii=False)}"[:150]
                    except Exception:
                        # Fallback to simple string conversion if JSON fails
                        details_str = f" Details: {str(details)}"[:150]
                # --- CORRECTED BLOCK END ---
        ...
        lines.extend(self._format_plain_text_table(headers, rows)) # Semicolon removed
        lines.append("```")
        return lines
    ...
    def _build_qa_section_7_prod_readiness(
        ...
        lines = ["", "7. PRODUCTION READINESS", ""]; lines.append("```markdown")
        # Ensure ValidationSeverity is accessible
        all_results = validation_results + [ vr for hop in self.orchestrator.hop_checkpoints for vr in hop.validation_results ]
        unique_results_dict = {};
        ...
        headers = ["Check", "Value", "Status"]; rows = [ ["Production Ready", str(prod_ready).upper(), "✅ PASS" if prod_ready else "❌ FAIL"], ["Critical Failures", len(critical_failures), "✅" if not critical_failures else "❌"], ["High Failures", len(high_failures), "✅" if not high_failures else "❌"] ]
        lines.extend(self._format_plain_text_table(headers, rows)); lines.append("```")
        if not prod_ready:
            ...
            if critical_failures:
                ...
                for f in critical_failures:
                    details = f.details or {}; msg = "[Error formatting msg]"
                    try:
    ...
    def _build_qa_section_8_pairwise_similarity(self) -> List[str]:
        ...
        # 1. Strict Duplicates
        pairwise_pass, pairwise_details, pairwise_max_sim, pairwise_violations, pairwise_notes, pairwise_status = True, "Analysis not performed or no data.", 0.0, 0, "", "⚠️ WARN"
        if similarity_matrix_data:
            duplicates_found = similarity_matrix_data.get('duplicates_found', [])
            duplicates_count = len(duplicates_found)
            pairwise_details = f"{similarity_matrix_data.get('total_comparisons', 0)} Comparisons"; pairwise_max_sim = similarity_matrix_data.get('max_similarity', 0.0); pairwise_violations = duplicates_count
            if duplicates_count == 0: pairwise_status, pairwise_notes = "✅ PASS", "No strict duplicates found."
            else:
                pairwise_status, pairwise_pass, overall_pass = "❌ FAIL", False, False; failed_examples = duplicates_found[:2]
                pairwise_notes = "; ".join([f"{ex.get('bullet_1', '?')} vs {ex.get('bullet_2', '?')}: {ex.get('similarity', 0.0):.4f}" for ex in failed_examples if isinstance(ex, dict)])
                if len(duplicates_found) > 2: pairwise_notes += "..."
        rows.append([ "Strict Duplicates (>= 0.90)", pairwise_details, f"{pairwise_max_sim:.4f}", str(pairwise_violations), pairwise_status, pairwise_notes ])

        # 2. Overview vs. Bullet
        overview_pass, overview_details, overview_max_sim, overview_violations_count, overview_notes, overview_status = True, "Analysis not performed.", 0.0, 0, "", "⚠️ WARN"
        excluded_overview_labels = [ResumeSection.K2_UNIFY_OVERVIEW.name, ResumeSection.K3_IBM_OVERVIEW.name] # Assuming ResumeSection enum is accessible
        if overview_similarity_data is not None:
            relevant_overview_data = [ result for result in overview_similarity_data if result and result.get("section") not in excluded_overview_labels ]
            if not relevant_overview_data: overview_details, overview_status, overview_notes = "0 Relevant Sections Checked", "✅ PASS", "No relevant Overview vs Bullet data to analyze."
            else:
                violations = []; max_sim_overall = 0.0; overview_details = f"{len(relevant_overview_data)} Sections Checked"
                for result in relevant_overview_data:
                    section_label = result.get('section', 'Unknown'); max_sim_overall = max(max_sim_overall, result.get('max_similarity', 0.0)); threshold_violations = result.get('threshold_violations', [])
                    if threshold_violations:
                        overview_pass, overall_pass = False, False
                        for v in threshold_violations:
                            if isinstance(v, dict): violations.append(f"{section_label}[{v.get('bullet_index', '?')}]: {v.get('similarity', 0.0):.4f}")
                overview_max_sim = max_sim_overall; overview_violations_count = len(violations)
                if not violations: overview_status, overview_notes = "✅ PASS", "No significant overlap found."
                else:
                    overview_status = "❌ FAIL"; overview_notes = "; ".join(violations[:2])
                    if len(violations) > 2: overview_notes += "..."
        rows.append([ "Overview vs. Bullet (>= 0.60, Excl. K2/K3)", overview_details, f"{overview_max_sim:.4f}", str(overview_violations_count), overview_status, overview_notes ])

        # 3. Exec Summary vs. Section
        exec_summary_pass, exec_summary_details, exec_summary_max_sim, exec_summary_violations_count, exec_summary_notes, exec_summary_status = True, "Analysis not performed.", 0.0, 0, "", "⚠️ WARN"
        threshold = 0.70
        if executive_summary_similarity_data is not None:
            if not executive_summary_similarity_data: exec_summary_details, exec_summary_status, exec_summary_notes = "0 Sections Compared", "✅ PASS", "No Exec Summary vs Section data to analyze."
            else:
                violations_details = []; max_sim_overall = 0.0; exec_summary_details = f"{len(executive_summary_similarity_data)} Sections Compared"
                staging_buffer = getattr(self.orchestrator, 'staging_buffer', None) # Get buffer if available
                for result in executive_summary_similarity_data:
                    section_label = result.get('section_label', 'Unknown'); item_count = result.get('item_count', 0); precalc_max_sim = result.get('max_similarity', 0.0)
                    max_sim_overall = max(max_sim_overall, precalc_max_sim); max_sim_for_section, index_of_max = precalc_max_sim, -1
                    if staging_buffer and item_count > 1 and precalc_max_sim >= threshold:
                        exec_summary_text_val = staging_buffer.get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, "") # Assuming ResumeSection accessible
                        if exec_summary_text_val:
                            section_content_items = []; section_enum = None
                            try:
                                for enum_member in ResumeSection: # Assuming ResumeSection accessible
                                     if section_label in enum_member.name or section_label in enum_member.value: section_enum = enum_member; break
                                if section_enum:
                                     content_raw = staging_buffer.get(section_enum.value)
                                     if isinstance(content_raw, list): section_content_items = [self._get_text_from_item(item) for item in content_raw]
                                     elif isinstance(content_raw, str): section_content_items = [content_raw]
                            except Exception as map_e: self.logger.warning(f"Could not map label '{section_label}' or get content: {map_e}")
                            if section_content_items:
                                dd = getattr(self.orchestrator, 'dup_detector', None) or DuplicateDetector() # Assuming DuplicateDetector accessible
                                try:
                                     similarities = [dd._calculate_cosine_similarity(exec_summary_text_val, item_text) for item_text in section_content_items]
                                     if similarities: max_sim_for_section = max(similarities); index_of_max = similarities.index(max_sim_for_section)
                                except Exception as calc_e: self.logger.warning(f"Recalculation error for Exec Summary vs {section_label}: {calc_e}")
                    if max_sim_for_section >= threshold:
                        exec_summary_pass, overall_pass = False, False; item_ref = f"[{index_of_max}]" if index_of_max != -1 else ""
                        violations_details.append((max_sim_for_section, f"{section_label}{item_ref}"))
                exec_summary_max_sim = max_sim_overall; exec_summary_violations_count = len(violations_details)
                if not violations_details: exec_summary_status, exec_summary_notes = "✅ PASS", f"Exec Summary distinct (Max sim < {threshold:.2f})."
                else:
                    exec_summary_status = "❌ FAIL"; violations_details.sort(key=lambda x: x[0], reverse=True)
                    exec_summary_notes = "; ".join([f"{label}: {score:.4f}" for score, label in violations_details[:2]])
                    if len(violations_details) > 2: exec_summary_notes += "..."
        rows.append([ f"Exec Summary vs. Section (>= {threshold:.2f})", exec_summary_details, f"{exec_summary_max_sim:.4f}", str(exec_summary_violations_count), exec_summary_status, exec_summary_notes ])
        # 4. Overall Status
        overall_status_str = "✅ PASS" if overall_pass else "❌ FAIL"; overall_notes = "All similarity checks passed." if overall_pass else "One or more similarity checks failed."; rows.append([ "Overall Similarity Status", "", "", "", overall_status_str, overall_notes ])
        alignments, col_widths = ['L', 'L', 'R', 'R', 'L', 'L'], [45, 25, 15, 18, 8, 50]; lines.extend(self._format_plain_text_table(headers, rows, alignments=alignments, col_widths=col_widths, wrap_text=True)); lines.append("```")
        return lines

# Section 9: Structural Validation (Specific Method - Was 10 - CORRECTED)
    def _build_qa_section_9_structural_validation(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        title = "9. STRUCTURAL VALIDATION (Content Presence)"
        description = "Validates the presence of all required structural elements in the generated content."
        rule_id_patterns = ["STRUCTURE_", "VG_COVER_LETTER_SIGNATURE_VALID"]
        lines = ["", title, "", description, ""]
        lines.append("```markdown")
        headers = ["Rule ID", "Status", "Message / Details"]
        rows = []
...
# Section 10: Cover Letter QA (Specific Method - Was 11 - CORRECTED)
    def _build_qa_section_10_cover_letter_qa(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        title = "10. COVER LETTER QA"
        description = "Specific validation checks for the generated Cover Letter content and structure."
        rule_id_patterns = ["COVER_LETTER", "VG_COVER_LETTER_SIGNATURE_VALID"]
        lines = ["", title, "", description, ""]
        lines.append("```markdown")
        headers = ["Rule ID", "Status", "Message / Details"]
        rows = []
    ...
    # Section 11: JD Enforcement (Renumbered from 12)
    def _build_qa_section_11_jd_enforcement(self) -> List[str]:
        lines = ["", "11. JD ENFORCEMENT VALIDATION", ""] # Updated title
        lines.append("```markdown")
        headers = ["Gate", "Rule", "Status", "Details"]
        enforcement_results = getattr(self.orchestrator.jd_enforcer, 'enforcement_results', [])
        rows = [[res.gate_id, res.rule.name, "PASS" if res.passed else "FAIL", res.details] for res in enforcement_results] if enforcement_results else [["N/A", "N/A", "WARN", "No JD enforcement results recorded."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        lines.append("```")
        return lines

# Section 12: Final Format (Renumbered from 13 - CORRECTED SYNTAX)
    def _build_qa_section_12_final_format(
        self,
        validation_results: List[ValidationResult],
        file_contents: Dict[str, str]
    ) -> List[str]:
        lines = ["", "12. FINAL OUTPUT FORMATTING SUMMARY", ""] # Updated title
        ...
        if resume_content:
            ...
            output_definitions.append(("**Resume**", "K.0 Headline Format", "Plain Text below Name", "PASS" if headline_pass else "FAIL", "" if headline_pass else "Headline missing, uses Markdown, missing pipes, or incorrect spacing."))
            ...
            comma_msg = "[Error formatting msg]"
            if not comma_pass and comma_check_result:
                # --- Corrected Try/Except Indentation ---
                try:
                    comma_msg = comma_check_result.message(defaultdict(lambda: '[N/A]', **(comma_check_result.details or {}))) if callable(comma_check_result.message) else str(comma_check_result.message)
                except Exception:
                    pass # Keep default message on error
            elif not comma_pass:
                comma_msg = "Check missing or error"
            else:
                comma_msg = ""
            output_definitions.append(("**Resume**", "Headline Commas", "VG_HEADLINE_NO_COMMAS", "PASS" if comma_pass else "FAIL", comma_msg))
            # Assuming visual rules passed
            ...
            output_definitions.append(("**Resume**", "Line Breaks", "Sufficient Blank Lines", "PASS" if lb_pass else "WARN", "" if lb_pass else f"Found {lb_count} blank lines, expected ~{expected_min_lbs}+."))
        ...
        if skills_content:
            ...
            if len(skills_items) > 1:
                ...
                for k in range(len(item_indices) - 1):
                    ...
                    if end_idx != start_idx + 2 or (start_idx + 1 < len(skills_lines_raw) and skills_lines_raw[start_idx + 1].strip() != ""):
                        double_newline_pass = False
                        break
            output_definitions.append(("**Skills**", "Line Breaks", "Double Newlines Between Skills", "PASS" if double_newline_pass else "FAIL", "" if double_newline_pass else "Missing double newlines between skills."))
        ...
        if cl_content:
            ...
            cl_struct_msg = "[Error formatting msg]"
            if not cl_struct_pass and cl_struct_result:
                # --- Corrected Try/Except Indentation ---
                try:
                    cl_struct_msg = cl_struct_result.message(defaultdict(lambda: '[N/A]', **(cl_struct_result.details or {}))) if callable(cl_struct_result.message) else str(cl_struct_result.message)
                except Exception:
                    pass
            elif not cl_struct_pass:
                cl_struct_msg = "Check missing or error"
            else:
                cl_struct_msg = ""
            output_definitions.append(("**Cover L.**", "Overall Structure", "Standard Letter Format Present", "PASS" if cl_struct_pass else "FAIL", cl_struct_msg))
            ...
            cl_sig_msg = "[Error formatting msg]"
            if not cl_sig_valid_pass and cl_sig_valid_result:
                # --- Corrected Try/Except Indentation ---
                try:
                    cl_sig_msg = cl_sig_valid_result.message(defaultdict(lambda: '[N/A]', **(cl_sig_valid_result.details or {}))) if callable(cl_sig_valid_result.message) else str(cl_sig_valid_result.message)
                except Exception:
                    pass
            elif not cl_sig_valid_pass:
                cl_sig_msg = "Check missing or error"
            else:
                cl_sig_msg = ""
        ...
        if qa_content:
            ...
            if hop8_checkpoint:
                qa_table_format_result = next((vr for vr in hop8_checkpoint.validation_results if vr.rule_id == "QA_TABLE_FORMAT_INVALID"), None)
            table_pass = qa_table_format_result.passed if qa_table_format_result else True
            table_msg = "[Error formatting msg]"
            if not table_pass and qa_table_format_result:
                # --- Corrected Try/Except Indentation ---
                try:
                    table_msg = qa_table_format_result.message(defaultdict(lambda: '[N/A]', **(qa_table_format_result.details or {}))) if callable(qa_table_format_result.message) else str(qa_table_format_result.message)
                except Exception:
                    pass
            elif not table_pass:
                table_msg = "Check missing or error"
            else:
                table_msg = ""
            output_definitions.append(("**QA Report**", "Table Format", "Uses Pre-formatted Tables", "PASS" if table_pass else "FAIL", table_msg))
            section_headers_present = all(f"\n{i}. " in qa_content for i in range(1, len(self.QA_REPORT_SECTIONS) + 1))
            output_definitions.append(("**QA Report**", "Headers", "Consistent Section Structure", "PASS" if section_headers_present else "FAIL", "" if section_headers_present else f"Missing expected section headers (1-{len(self.QA_REPORT_SECTIONS)})."))
        ...
        if app_content:
            ...
            try:
                ...
                # Append validity check result *only* if it fails within the try block
                output_definitions.append(("**App Trkr**", "JSON Validity", "Valid JSON Structure", "FAIL", json_fail_reason))
            # If JSON was valid, append PASS outside the try-except
            if json_valid:
            ...
            # Assuming APP_TRACKER_SCHEMA_DATA is accessible
            if json_valid and isinstance(parsed_json_data, dict) and APP_TRACKER_SCHEMA_DATA:
                schema_keys = set(APP_TRACKER_SCHEMA_DATA.keys()); actual_keys = set(parsed_json_data.keys()); schema_pass = schema_keys == actual_keys
                missing_keys, extra_keys = schema_keys - actual_keys, actual_keys - schema_keys; schema_fail_reason = ""
            ...
            elif not APP_TRACKER_SCHEMA_DATA: schema_fail_reason = "App Tracker Schema data not loaded."
    ...
    def _validate_qa_report_formatting(self, report_text: str) -> ValidationResult:
        pre_formatted_check_passed = True; pre_formatted_check_messages = []; sections = re.split(r'(?m)^(\d{1,2}\.\s+[A-Z\s&()-]+)$', report_text); section_blocks = {}
        if len(sections) > 1:
            for i in range(1, len(sections), 2):
                title = sections[i].strip(); content = sections[i+1] if (i+1) < len(sections) else ""
                try: section_index = int(title.split('.')[0]); section_blocks[section_index] = (title, content)
                except ValueError: self.logger.warning(f"Could not parse section index from QA header: '{title}'")
        # Determine sections expected to have ```markdown blocks
        sections_with_tables = [ idx for idx, cfg in enumerate(self.QA_REPORT_SECTIONS, 1) if cfg['method'] != '_build_qa_section_1_signal_quality' ] # Section 1 has custom format

        for section_index, (title, section_block) in section_blocks.items():
            if section_index not in sections_with_tables: continue
            md_blocks = re.findall(r"```(?:markdown)?(.*?)```", section_block, re.DOTALL)
            if not md_blocks:
                pre_formatted_check_passed = False; msg = f"QA Section {section_index} ({title}) missing expected ``` block."
                if msg not in pre_formatted_check_messages: pre_formatted_check_messages.append(msg); continue # Correct indentation
            for block in md_blocks:
                ...
                if has_pipe and has_separator:
                    pre_formatted_check_passed = False; msg = f"QA Section {section_index} ({title}) appears to contain Markdown table syntax inside ``` block."
                    if msg not in pre_formatted_check_messages: pre_formatted_check_messages.append(msg); break

        return ValidationResult( rule_id="QA_TABLE_FORMAT_INVALID", passed=pre_formatted_check_passed, severity=ValidationSeverity.HIGH if not pre_formatted_check_passed else ValidationSeverity.INFO, message="; ".join(pre_formatted_check_messages) if not pre_formatted_check_passed else "All QA tables use pre-formatted text.", details={"failed_sections": pre_formatted_check_messages} if not pre_formatted_check_passed else {} )
    ...
    def _format_plain_text_table(
        ...
        str_rows = [[str(cell) for cell in row] for row in rows]; str_headers = [str(h) for h in headers]
        ...
        if col_widths and len(col_widths) == num_cols:
            ...
            if widths[-1] == 0:
                max_last_col = 0
                if str_headers: max_last_col = max(max_last_col, max(len(line) for line in str_headers[-1].split('\n')) if '\n' in str_headers[-1] else len(str_headers[-1]))
                for row in str_rows:
                    if len(row) == num_cols: max_last_col = max(max_last_col, len(row[-1]))
                widths[-1] = max(1, max_last_col)
        else:
            widths = [0] * num_cols
            if str_headers:
                for i, header in enumerate(str_headers):
                     max_header_line = max(len(line) for line in header.split('\n')) if header and '\n' in header else len(header or "")
        ...
        if wrap_text:
             for row in str_rows:
                 # Ensure row has the expected number of columns, padding if necessary
            ...
            for row in str_rows:
                ...
                for i in range(num_cols):
                    ...
                    # Apply formatter (truncates if content > width)
    ...
    def _wrap_cell_text(self, text: str, width: int) -> List[str]:
        ...
        if not text: return [""]
        wrapped_lines = textwrap.wrap( text, width=width, break_long_words=True, replace_whitespace=True, drop_whitespace=True, break_on_hyphens=True )
        return wrapped_lines if wrapped_lines else [""]

    def _get_text_from_item(self, item: Any) -> str:
        if isinstance(item, dict): return item.get('text', item.get('bullet_text', ''))
    ...
    # --- Helper: ASCII Bar Chart Formatter ---
    def _format_ascii_bar_chart(
        self,
        label: str,
        value: float,
        ...
        # Corrected indentation for the entire method
        display_value = min(max(value, 0.0), 1.0)
        filled_length = int(round(bar_length * display_value))
        bar = '█' * filled_length + ' ' * (bar_length - filled_length)
        score_text = f"Raw: {value:.2f}" if score_display_mode == "raw" else f"{value:.1%}"
        ...
        elif isinstance(temperature, str) and temperature != 'N/A':
            temp_str = f"({temperature})"

        if target_min is not None and target_max is not None:
            target_str = f"(Tgt: {target_min:.2f}-{target_max:.2f})" if score_display_mode == "raw" else f"(Tgt: {target_min:.0%}-{target_max:.0%})"
        else:
            target_str = "(Target: N/A)"

        label_width, bar_width, score_width, target_width, status_width, temp_width = 25, bar_length + 2, 12, 18, 7, 10
        total_width = 95
        ...
        return f"{label_formatted} [{bar}] {score_text:<{score_width}} {target_str:<{target_width}} {status:<{status_width}} {temp_str:<{temp_width}}{padding}"
