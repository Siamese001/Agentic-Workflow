...
# (count_words_ms_word_style, count_words_in_list_ms_word_style, _count_sentences, calculate_signal_score - these remain the same as provided before)
...
def calculate_signal_score(text_content, thematic_analysis):
    ...
    try:
        # Ensure competitive_intelligence exists and is not None
        ...
        # Fallback if thematic_analysis structure is unexpected
        logging.warning("Error accessing keywords in thematic_analysis for signal score calculation.")
        return 0.0

    if not all_jd_words:
        # logging.debug("No JD keywords found in thematic_analysis for signal score calculation.")
        return 0.0 # Return 0 if no keywords extracted
    ...
    score = len(matches) / 10.0 # Base score on differentiator matches (max 1.0 if 10+ match)

    # Boost score for primary theme keywords (additively)
    primary_matches = words_in_text.intersection(primary_words)
    score += len(primary_matches) * 0.1 # Add 0.1 for each primary keyword match
...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_8_overview_similarity(self) -> List[str]:
        """Builds Section 8 of the QA report: Exec Summary vs. Bullet Similarity.""" # Keep original name for now
        lines = ["", "8. EXEC SUMMARY VS. BULLET SIMILARITY CHECK", ""] # Keep original name for now
        if self.overview_similarity_data: # Use pre-computed data
            ...
            lines.append("Exec Summary vs. Bullet similarity analysis was not performed (missing data).")
        return lines
