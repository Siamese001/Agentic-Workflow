...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_5_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        # Log the dictionary of averages being used for this QA section build
        logging.info(f"Using Master Averages for QA Sec 5: {master_avg_lengths}") # <<< ADDED LOG
        ...
        for section_data in self.QA_PROVENANCE_SECTIONS: # Iterate through configured sections
            ...
            # --- Process Overview ---
            # (Existing overview processing logic...)
            ...
            if bullets_enum:
                ...
                # Log which average is being used for this section
                logging.debug(f"Processing Section '{name}', using avg_key='{avg_key}', avg_len={avg_len:.1f}") # <<< ADDED LOG
                ...
                if isinstance(bullets, list) and bullets:
                    for i, bullet_item in enumerate(bullets):
                        ...
                        # (Existing logic to get bullet_text, word_count, provenance...)
                        ...
                        min_target = round(avg_len * (1 - tolerance)) #
                        max_target = round(avg_len * (1 + tolerance)) #
                        # Log the details for this specific bullet's calculation
                        logging.debug(f"  Bullet {i+1}: WordCount={word_count}, Avg={avg_len:.1f}, Tol={tolerance:.0%}, Target={min_target}-{max_target}") # <<< ADDED LOG
                        ...
                        rows.append([
                            ...
                            provenance,
                            str(word_count),
                            target_range_str, # This is the calculated range being displayed
                            status,
