"""
Resume Generation Engine v5.72 - WORD COUNT COMPLIANCE HARDENED PATCH

v5.72 CHANGES - WORD COUNT COMPLIANCE (HARDENED):
✅ PATCH: SECTION 7 WORD COUNT TABLE GENERATION
   - _generate_qa_report() Section 7 refactored for robust word count extraction
   - section_key_map maps rule IDs and message fragments to buffer keys
   - get_section_info_robust() intelligently identifies sections from validation results
   - Direct buffer access via staging_buffer.get(section_buffer_key) ensures accurate counts
   - Recalculation of actual word counts from generated content (not validation result details)
   - Proper target range lookup from SectionRegistry metadata
   - Bullet vs Overview disambiguation via message content matching
   - Duplicate section prevention with processed_sections_in_report tracking
   - Dynamic column formatting for clean table alignment
   - All 9+ resume sections now report precise word counts with proper buffer sourcing
...
- If JD emphasizes "backend engineering" → emphasize technical architecture and scalability"""
        ...
        # 4. Call the LLM API with v5.71 Reasoning Configuration
        try:
            ...
            base_system = "You are an expert resume editor. You rewrite content to align with job descriptions, never inventing new facts. All achievements must be traceable to the source material."
            ...
            response = client.messages.create(
                ...
                max_tokens=api_params["max_tokens"],           # ← FROM REASONING CONFIG (v5.71)
                temperature=api_params["temperature"],         # ← FROM REASONING CONFIG (v5.71)
                system=enhanced_system,                         # ← WITH REASONING DIRECTIVES (v5.71)
                messages=[
    ...
    def _generate_k2_skills(
        ...
        """
        v5.70 PATCH 2 (NEW METHOD): Generate 12 high-signal,
        1-3 word skills with robust LLM output parsing.
        
        Hardens against:
        - Comma-separated lists instead of newlines
...
"""
            ...
            response = client.messages.create(
                ...
                max_tokens=200,
                temperature=0.2,
                system="You are an expert HR data analyst. You generate 1-3 word skills for HR databases. You follow formatting instructions perfectly.",
                messages=[{"role": "user", "content": prompt}]
            )
            
            skills_text = response.content[0].text.strip()
            
            # 4. HARDENED PARSING: Handle newlines, commas, and bullets
            skills_list_final = []
...
class WorkflowOrchestrator:
    ...
    def _generate_qa_report(
        ...
        """
        Generate comprehensive QA report (9-section format).
        v5.70 PATCH 3: Removes "=" separators and uses dynamic table formatting.
        Returns: (validation_results, qa_report_text)
        """
        validation_results_out = []
        
        # Build ASCII bar chart helper
        def ascii_bar(actual: float, target: float, width: int = 50) -> str:
            actual_fill = int(actual * width)
            target_pos = int(target * width)
            ...
            if target_pos < width:
                ...
                bar = bar[:width-1] + "|" # Place at end
            
            return bar
        
        report_lines = []
        report_lines.append("RESUME QA REPORT v5.70 (Hardened)")
        ...
        # Per-section signal scoring (v5.64)
        section_config = {
            # High-value sections (Target: 0.90)
            "K.1": {"name": "Exec Summary", "weight": 0.20, "target": 0.90},
            "K.4": {"name": "Headline", "weight": 0.10, "target": 0.90},
            "K.5A": {"name": "Unify Bullets", "weight": 0.20, "target": 0.90},
            "K.8": {"name": "Competencies", "weight": 0.15, "target": 0.90},
            # Mid-value section (Target: 0.75)
            "K.6A": {"name": "IBM Bullets", "weight": 0.15, "target": 0.75},
            # Low-value sections (Target: 0.50)
            "K.5B": {"name": "Unify Intro", "weight": 0.05, "target": 0.50},
            "K.6B": {"name": "IBM Intro", "weight": 0.05, "target": 0.50},
            "K.7A": {"name": "EY Bullets", "weight": 0.05, "target": 0.50},
            "K.10A": {"name": "Early Career", "weight": 0.05, "target": 0.50},
        }

        total_weighted_score = 0.0
        total_weighted_target = 0.0 # New
        total_weight = 0.0
        section_bar_lines = []

        for key, config in section_config.items():
            content = staging_buffer.get(key)
            actual_score = self._calculate_signal_score(content, thematic_analysis)
            target_score = config.get("target", 0.90) # Get target, default to 0.90
            
            total_weighted_score += actual_score * config["weight"]
            total_weighted_target += target_score * config["weight"] # New
            total_weight += config["weight"]
            
            # New bar_line with Target
            bar_line = (
                f"{key} {config['name']:<15} "
                f"Act: {int(actual_score*100):>3} / Tgt: {int(target_score*100):>3} "
                f"[{ascii_bar(actual_score, target_score, 30)}]"
            )
            section_bar_lines.append(bar_line)

        # Calculate weighted averages
        weighted_avg_score = total_weighted_score / total_weight if total_weight > 0 else 0.0
        weighted_avg_target = total_weighted_target / total_weight if total_weight > 0 else 0.0 # New

        report_lines.append(f"Overall Signal Score: {weighted_avg_score:.2f} (Target: {weighted_avg_target:.2f})")
        report_lines.append(f"Signal Bar (Actual vs. Target |): {ascii_bar(weighted_avg_score, weighted_avg_target, 50)} ({int(weighted_avg_score*100)}%)")
        report_lines.append("")
        report_lines.append("Per-Section Signal Quality:")
        report_lines.append("-" * 80)
        report_lines.extend(section_bar_lines)
        report_lines.append("")
        ...
        # SECTION 4: AI Detection Defense (K.5B vs K.6B Similarity Analysis)
        report_lines.append("4. AI DETECTION DEFENSE (Content Similarity Analysis)")
        report_lines.append("")
        # HARDENED v5.66: Populate from deduplication analysis
        if self.overview_similarity_data:
            report_lines.append("| Section Pair | Similarity Score | Threshold | Status | Notes |")
            report_lines.append("|:---|:---:|:---:|:---:|:---|")
            
            for pair_name, data in self.overview_similarity_data.items():
                score = data.get('similarity_score', 0.0)
                threshold = 0.75
                status = "✓ Pass" if data.get('passed', False) else "✗ Fail"
                notes = f"{data['section1']} vs {data['section2']}"
                report_lines.append(f"| {pair_name} | {score:.2%} | {threshold:.0%} | {status} | {notes} |")
        else:
            report_lines.append("⚠ AI Detection Defense: No similarity data available (deduplication analysis not executed)")
        
        
        # HARDENED v5.66: Populate from deduplication analysis
        if self.overview_similarity_data:
            report_lines.append("| Comparison | Similarity | Threshold | Status | Notes |")
            report_lines.append("|:---|:---:|:---:|:---:|:---|")
            for name, data in self.overview_similarity_data.items():
                score = data.get('score', 0.0)
                status = "✓ Pass" if data.get('passed', False) else "✗ Fail"
                notes = f"{data.get('section1', 'N/A')} vs {data.get('section2', 'N/A')}"
                report_lines.append(f"| {name} | {score:.2%} | 75% | {status} | {notes} |")
        else:
            report_lines.append("⚠ No deduplication data (analysis not executed or no duplicates found)")
        ...
        # SECTION 5: Deduplication Matrix (Pairwise Content Similarity)
        report_lines.append("5. DEDUPLICATION MATRIX (Pairwise Similarity Analysis)")
        report_lines.append("")
        # HARDENED v5.66: Populate from deduplication analysis
        if self.similarity_matrix_data:
            # Build matrix table
            all_sections = set()
            for section1, pairs in self.similarity_matrix_data.items():
                all_sections.add(section1)
                all_sections.update(pairs.keys())
            all_sections = sorted(list(all_sections))
            
            if len(all_sections) > 0:
                # Header row
                header = "| Section | " + " | ".join([s[:12] for s in all_sections]) + " |"
                report_lines.append(header)
                
                # Separator
                sep = "|:---|" + "|:---:|" * len(all_sections) + "|"
                ...
                for section1 in all_sections:
                    ...
                    for section2 in all_sections:
                        if section1 == section2:
                            row += " - |"
                        elif section1 in self.similarity_matrix_data and section2 in self.similarity_matrix_data[section1]:
                            sim_data = self.similarity_matrix_data[section1][section2]
                            sim = sim_data['similarity']
                            flag = "⚠" if sim_data['flagged'] else "✓"
                            row += f" {flag} {sim:.0%} |"
                        else:
                            row += " N/A |"
                    report_lines.append(row)
        else:
            report_lines.append("⚠ Deduplication Matrix: No similarity data available (deduplication analysis not executed)")
        
    
    # HARDENED v5.66: Populate from deduplication matrix
    if self.similarity_matrix_data:
        # Build abbreviated matrix
        all_secs = sorted(set(list(self.similarity_matrix_data.keys()) + 
                             [sec for pairs in self.similarity_matrix_data.values() for sec in pairs.keys()]))
        if all_secs:
            # Header with section abbreviations
            abbrev = {sec: sec.split('.')[-1][:8] for sec in all_secs}
            header = "| Sec | " + " | ".join(abbrev[s][:6] for s in all_secs) + " |"
            report_lines.append(header)
            report_lines.append("|:---|" + "|:---:|" * len(all_secs) + "|")
            
            # Data rows
            for sec1 in all_secs:
                row = f"| {abbrev[sec1][:6]} |"
                for sec2 in all_secs:
                    if sec1 == sec2:
                        row += " - |"
                    elif sec1 in self.similarity_matrix_data and sec2 in self.similarity_matrix_data[sec1]:
                        sim = self.similarity_matrix_data[sec1][sec2]['similarity']
                        flag = "⚠" if self.similarity_matrix_data[sec1][sec2]['flagged'] else "✓"
                        row += f" {flag}{sim:.0%} |"
                    else:
                        row += " - |"
                report_lines.append(row)
    else:
        report_lines.append("⚠ No deduplication matrix (analysis not executed)")
        ...
        # SECTION 7: Word Count Compliance
        report_lines.append("7. WORD COUNT COMPLIANCE (v5.72 Hardened)")
        report_lines.append("")
        report_lines.append("| Section                     | Actual | Target  | Status | Message |")
        report_lines.append("|:----------------------------|:-------|:--------|:-------|:--------|")

        # Include relevant validation results for word counts, tolerances, and headline structure
        word_count_validation_results = [
            vr for vr in validation_results if
            'WORD_COUNT' in vr.rule_id or
            'TOLERANCE' in vr.rule_id or
            'HEADLINE_CONSTRAINTS' in vr.rule_id or
            # Add specific rule IDs if sections like Certs (K.11) have dedicated rules
            'VG_WORD_COUNT_K11' in vr.rule_id # Example if K.11 has its own rule
        ]

        # --- BEGIN PATCH v5.72 ---
        # Map rule IDs/message fragments to staging buffer keys and display names
        # Ensures consistent identification and allows direct counting
        section_key_map = {
            "VG_TOTAL_WORD_COUNT": ("Total Resume", "TOTAL"), # Special case handled separately for actual count
            "VG_WORD_COUNT_K1": ("K.1 Exec Summary", "K.1"),
            "HEADLINE_CONSTRAINTS": ("Headline", "K.4"), # Checks structure, reports K.4 word count
            "Unify Consulting - Bullets": ("Unify Bullets", "K.5A"), # Identified via message content
            "Unify Consulting - Overview": ("Unify Overview", "K.5B"), # Identified via message content
            "IBM - Bullets": ("IBM Bullets", "K.6A"), # Identified via message content
            "IBM - Overview": ("IBM Overview", "K.6B"), # Identified via message content
            "TraderSense - Bullets": ("TraderSense Bullets", "K.7.5A"), # Identified via message content
            "TraderSense - Overview": ("TraderSense Overview", "K.7.5B"), # Identified via message content
            "EY - Bullets": ("EY Bullets", "K.7A"), # Identified via message content
            "EY - Overview": ("EY Overview", "K.7B"), # Identified via message content
            "Competencies": ("Competencies", "K.8"), # Identified via message content
            "Early Career - Bullets": ("Early Career Bullets", "K.10A"), # Identified via message content
            "Early Career - Overview": ("Early Career Overview", "K.10B"), # Identified via message content
            "VG_WORD_COUNT_K11": ("Certifications", "K.11") # Example using a specific rule ID for K.11
            # Note: The keys (e.g., "Unify Consulting - Bullets") rely on matching text within the
            # validation result's message attribute if a specific rule_id isn't used.
        }

        # Helper to get section display name and buffer key from a validation result
        def get_section_info_robust(wcr):
            # Prioritize matching specific rule IDs
            for rule_id_prefix, (display_name, buffer_key) in section_key_map.items():
                if rule_id_prefix in wcr.rule_id:
                    return display_name, buffer_key
            # Fallback to matching message content for tolerance rules etc.
            for message_fragment, (display_name, buffer_key) in section_key_map.items():
                 # Avoid matching TOTAL based on message fragment
                if buffer_key != "TOTAL" and message_fragment.lower() in wcr.message.lower():
                    # Refine matching for bullets vs overview based on message context
                    if "bullet" in wcr.message.lower() and "bullet" not in display_name.lower():
                        continue # Skip if message mentions bullets but map entry is for overview
                    if "overview" in wcr.message.lower() and "overview" not in display_name.lower():
                        continue # Skip if message mentions overview but map entry is for bullets
                    return display_name, buffer_key
            return wcr.rule_id, None # Fallback if no match found

        processed_sections_in_report = set() # Track sections already added to the report

        # Recalculate total word count directly from buffer for verification (optional)
        # actual_total_recalculated = sum(
        #     count_words_clean(v) if isinstance(v, str) else count_words_in_list_clean(v) if isinstance(v, list) else 0
        #     for k, v in staging_buffer.data.items() if k.startswith('K.') # Only count K-sections
        # )

        for wcr in word_count_validation_results:
            status_text = "Pass" if wcr.passed else "Fail"
            details = wcr.details if hasattr(wcr, 'details') and isinstance(wcr.details, dict) else {}

            section_display_name, section_buffer_key = get_section_info_robust(wcr)

            # Avoid adding the same section row multiple times if covered by different rules
            if section_display_name in processed_sections_in_report:
                continue
            processed_sections_in_report.add(section_display_name)

            actual_count_str = "N/A"
            target_str = "N/A"

            # 1. Get ACTUAL count
            if section_buffer_key == "TOTAL":
                # For Total, trust the dedicated validation result's detail
                actual_count_str = str(details.get('total', 'ERR'))
                target_str = "899-999" # Hardcoded target for total
            elif section_buffer_key:
                # For specific sections, recalculate from the buffer
                content = staging_buffer.get(section_buffer_key)
                actual_count = 0
                if isinstance(content, str):
                    actual_count = count_words_clean(content)
                    # Special handling for headline character count reporting if needed
                    if section_buffer_key == 'K.4': actual_count_str = f"{actual_count}w" # Indicate words
                elif isinstance(content, list):
                    actual_count = count_words_in_list_clean(content)
                actual_count_str = str(actual_count) # Default is word count

                # 2. Get TARGET range from validation details or rule specifics
                if 'min_allowed' in details and 'max_allowed' in details:
                    target_str = f"{details['min_allowed']}-{details['max_allowed']}"
                elif 'VG_WORD_COUNT_K1' in wcr.rule_id: # K.1 specific target
                    target_str = "100-150"
                elif 'HEADLINE_CONSTRAINTS' in wcr.rule_id: # Headline specific target
                     # Get word range from SectionRegistry as fallback
                     meta = SectionRegistry.get_section('K.4')
                     target_str = f"{meta.word_count_min if meta else 10}-{meta.word_count_max if meta else 20}w"
                # Add specific target lookups for K.11 or others if needed based on rule IDs

            # 3. Sanitize Message
            # Use the original validation message, but clean it
            message = wcr.message.replace('|', ';').strip()
            # If the status is Fail, ensure the message reflects the *actual* count just calculated
            if not wcr.passed and section_buffer_key != "TOTAL":
                 # Simple replacement, could be made more robust
                 message = re.sub(r': \d+ words', f': {actual_count_str} words', message)
                 message = re.sub(r': \d+w', f': {actual_count_str}', message)


            report_lines.append(f"| {section_display_name:<27} | {actual_count_str:>6} | {target_str:>7} | {status_text:<6} | {message} |")
