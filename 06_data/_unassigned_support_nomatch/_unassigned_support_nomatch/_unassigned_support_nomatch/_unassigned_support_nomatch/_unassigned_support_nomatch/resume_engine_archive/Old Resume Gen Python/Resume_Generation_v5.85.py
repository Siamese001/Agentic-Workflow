...
- If JD emphasizes "backend engineering" → emphasize technical architecture and scalability"""
        ...
        try:
            ...
            if not api_key:
                raise HopExecutionError("K.1: GEMINI_API_KEY not set. Cannot generate executive summary.")
            ...
            if word_count < 80 or word_count > 170:
                raise HopExecutionError(f"K.1: Generated word count {word_count} is outside the 100-150 target range.")
            
            return tailored_summary
            
        except Exception as e:
            raise HopExecutionError(f"K.1: LLM generation failed: {e}")
...
"""
            ...
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise HopExecutionError("K.2: GEMINI_API_KEY not set. Cannot generate skills.")
            
            if not GEMINI_AVAILABLE:
                raise HopExecutionError("K.2: google-generativeai library not available.")
            ...
            if len(skills_list_final) < 10:  # Allow for 10-12
                raise HopExecutionError(f"K.2: LLM failed to generate at least 10 valid skills. Output: {skills_text}")
                
            return skills_list_final[:12]  # Return exactly 12

        except Exception as e:
            raise HopExecutionError(f"K.2: Skills generation failed: {e}")
...
"""
        ...
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise HopExecutionError("K.4: GEMINI_API_KEY not set. Cannot generate headline.")
            ...
            if word_count < 10 or word_count > 25:
                raise HopExecutionError(f"K.4: Generated word count {word_count} is outside the 15-20 target range.")
            
            return headline
            
        except Exception as e:
            raise HopExecutionError(f"K.4: Headline generation failed: {e}")
...
- Match metrics and achievements to job requirements"""
        ...
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise HopExecutionError("K.5A: GEMINI_API_KEY not set. Cannot select bullets.")
            ...
            if len(selected_bullets) != 7:
                raise HopExecutionError(f"K.5A: LLM returned {len(selected_bullets)} bullets instead of 7.")
            ...
            raise HopExecutionError(f"K.5A: Bullet selection failed: {e}")
...
4. Return ONLY the 6 selected bullets, one per line, no numbers"""
        ...
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise HopExecutionError("K.6A: GEMINI_API_KEY not set. Cannot select bullets.")
            ...
            if len(selected_bullets) != 6:
                raise HopExecutionError(f"K.6A: LLM returned {len(selected_bullets)} bullets instead of 6.")
            ...
            raise HopExecutionError(f"K.6A: Bullet selection failed: {e}")
...
5. Return ONLY the 6 selected competencies, one per line, no numbers or bullets"""
        ...
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise HopExecutionError("K.8: GEMINI_API_KEY not set. Cannot select competencies.")
            ...
            if len(selected_comps) != 6:
                raise HopExecutionError(f"K.8: LLM returned {len(selected_comps)} competencies instead of 6.")
            ...
            raise HopExecutionError(f"K.8: Competency selection failed: {e}")
...
{signature}"""
        ...
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise HopExecutionError("K.9: GEMINI_API_KEY not set. Cannot generate cover letter.")
            ...
            raise HopExecutionError(f"K.9: Cover letter generation failed: {e}")
