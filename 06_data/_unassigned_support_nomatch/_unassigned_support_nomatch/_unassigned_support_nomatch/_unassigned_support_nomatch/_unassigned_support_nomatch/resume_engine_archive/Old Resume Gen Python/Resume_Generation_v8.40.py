"""
Resume Generation Engine v8.40
...
class RetrievalSource:
    ...
    source_tag: str  # v8.10: For weighted synthesis (e.g., "SOURCE_COMPANY_BLOG")
...
class ThematicAnalysis:
    ...
    problem_solution_narratives: Optional[Dict[str, Any]] = None # v8.10: Approach 3
...
"""
    ...
    def _analyze_with_resilient_web_search(
        ...
        if partial_result.full_success:
            ...
            raise HopExecutionError("RAG analysis failed to achieve 100% success across all three phases.")
...
class ArtistGenerator:
    ...
    def _generate_artist_output(
        ...
        # Extract differentiator keywords and role archetype for consistent use
        differentiator_keywords = thematic_analysis.competitive_intelligence.differentiator_keywords[:8] if hasattr(thematic_analysis, 'competitive_intelligence') else []
        role_archetype = thematic_analysis.role_classification.get('role_archetype', 'Technical_IC') if hasattr(thematic_analysis, 'role_classification') else 'Technical_IC'

        output = {}
        output[ResumeSection.K0_NAME.value] = self._copy_k0_name()
        output[ResumeSection.K0_HEADLINE.value] = self._generate_k0_headline(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
        output[ResumeSection.K0_CONTACT.value] = self._copy_k0_contact()
        output[ResumeSection.K1_EXECUTIVE_SUMMARY.value] = self._generate_k1_executive_summary(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
        output[ResumeSection.K5_UNIFY_BULLETS.value] = self._generate_k5_unify_bullets(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
        output[ResumeSection.K5_UNIFY_OVERVIEW.value] = self._generate_k5_unify_overview(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
        output[ResumeSection.K6_IBM_BULLETS.value] = self._generate_k6_ibm_bullets(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
        output[ResumeSection.K6_IBM_OVERVIEW.value] = self._generate_k6_ibm_overview(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
        ...
        output[ResumeSection.K10_COMPETENCIES.value] = self._generate_k10_competencies(
            enriched_scaffold, job_description, thematic_analysis, previous_failures,
            differentiator_keywords=differentiator_keywords, role_archetype=role_archetype
        )
    ...
    def _generate_k1_executive_summary(
        ...
        thematic_analysis: ThematicAnalysis,
        previous_failures: List[ValidationResult] = None,
        differentiator_keywords: List[str] = None,
        ...
        differentiators = differentiator_keywords if differentiator_keywords else []
...
The target role is a PRE-SALES / Go-To-Market role. The summary MUST prioritize themes of revenue generation, strategic partnerships, and driving market adoption. Frame achievements in the context of winning new business and expanding market share."""
        ...
        # 4. Build the prompt
        prompt = f"""You are an expert resume writer crafting an executive summary.
...
**TASK:**
1. Write a 4-5 sentence executive summary (100-150 words).
...
"""
        ...
        # 5. Call LLM with HIGH SIGNAL and HIGH TEMP
        try:
            ...
            # 1. GET HIGH-SIGNAL REASONING CONFIG
            reasoning_config = ReasoningConfig.K1_EXECUTIVE_SUMMARY_CONFIG
            
            # 2. GET API PARAMS (this sets high-signal prompt AND default low temp)
            api_params = reasoning_config_to_api_params(reasoning_config)
            
            # 3. FORCE HIGHEST TEMPERATURE (OVERRIDE)
            # This is the critical change you requested.
            api_params["generation_config"].temperature = 1.0
            
            # 4. ENHANCE SYSTEM PROMPT
            base_system = "You are an expert resume writer crafting an executive summary."
            enhanced_system = enhance_system_prompt_with_reasoning(base_system, reasoning_config, "K.1")
            ...
            print(f"Warning: LLM generation for K.1 Executive Summary failed: {e}. Returning placeholder.")
            return "[Placeholder for K.1 Executive Summary - LLM call failed]"
    ...
    def _call_llm_for_text(self, prompt: str, section_name: str) -> str:
        """Helper for simple LLM text generation with fallback."""
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                raise ValueError("GEMINI_API_KEY not set")
            ...
            print(f"Warning: LLM generation for {section_name} failed: {e}. Returning placeholder.")
            return f"[Placeholder for {section_name} - LLM call failed]"
...
"""
    ...
    def _generate_k0_headline(
        ...
        # Get key themes
        primary_theme = thematic_analysis.primary_theme.get('name', 'AI Leadership')
        differentiators = differentiator_keywords[:3] if differentiator_keywords else []
        role_level = thematic_analysis.role_classification.get('seniority', 'senior')
...
"""
    ...
    def _generate_k5_unify_bullets(
        ...
        bullets_text = '\n'.join([f"{i+1}. {bullet['bullet_text']}" for i, bullet in enumerate(enriched_scaffold['experience_sections'][0]['bullets'])])
...
- Match metrics and achievements to job requirements"""
    ...
    def _generate_k5_unify_overview(
        ...
        primary_theme = thematic_analysis.primary_theme.get('name', 'AI Leadership')
        differentiators = differentiator_keywords[:5] if differentiator_keywords else []
...
"""
    ...
    def _generate_k6_ibm_bullets(
        ...
        # 3. Build prompt
        bullets_text = '\n'.join([f"{i+1}. {bullet['bullet_text']}" for i, bullet in enumerate(enriched_scaffold['experience_sections'][1]['bullets'])])
...
"""
    ...
    def _generate_lightly_customized_bullets(
        ...
        thematic_analysis: ThematicAnalysis,
        section_name: str # For logging/ReasoningConfig
    ) -> List[Dict]:
        """
        Lightly rewrite a small, fixed list of bullets using an LLM.
        Used for older roles (EY, Early Career) where selection isn't the goal,
...
"""
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                print(f"Warning: No GEMINI_API_KEY, returning original {section_name} bullets.")
                return [{"text": b, "provenance": BulletProvenance.VERIFIED.value} for b in source_bullets]
            ...
            api_params = reasoning_config_to_api_params(reasoning_config)
            enhanced_system = enhance_system_prompt_with_reasoning(model.system_instruction, reasoning_config, section_name)
            ...
            if len(rewritten_bullets) != len(source_bullets):
                print(f"Warning: {section_name} LLM returned {len(rewritten_bullets)} bullets, expected {len(source_bullets)}. Returning original.")
                return [{"text": b, "provenance": BulletProvenance.VERIFIED.value} for b in source_bullets]

            return [{"text": b, "provenance": BulletProvenance.TAILORED.value, "word_count": count_words_clean(b)} for b in rewritten_bullets]
            
        except Exception as e:
            print(f"Warning: {section_name} LLM bullet generation failed: {e}. Returning original bullets.")
            return [{"text": b, "provenance": BulletProvenance.VERIFIED.value} for b in source_bullets]
    ...
    def _generate_k8_ey_bullets(
        ...
        return self._generate_lightly_customized_bullets(source_bullets, thematic_analysis, "K.8A")
...
"""
        try:
            ...
            reasoning_config = ReasoningConfig.K8_EY_OVERVIEW_CONFIG
            api_params = reasoning_config_to_api_params(reasoning_config)
            enhanced_system = enhance_system_prompt_with_reasoning(model.system_instruction, reasoning_config, "K.8B")
    ...
    def _generate_k9_early_career_bullets(
        ...
        return self._generate_lightly_customized_bullets(source_bullets, thematic_analysis, "K.9A")
...
"""
        try:
            ...
            api_params = reasoning_config_to_api_params(reasoning_config)
            enhanced_system = enhance_system_prompt_with_reasoning(model.system_instruction, reasoning_config, "K.9B")
...
- Prioritize competencies that are explicitly mentioned or strongly implied in the job description."""
    ...
    def _generate_k13_cover_letter(
        ...
        primary_theme = thematic_analysis.primary_theme.get('name', 'this strategic opportunity')
        ...
        narratives = thematic_analysis.problem_solution_narratives or {}
        problem_context = "; ".join(narratives.get("common_problems", []))
        
        # Gather top achievements for the "Proof" section
        top_achievements = []
        for section in enriched_scaffold.get('experience_sections', [])[:2]: # Unify & IBM
...
class PreFlightValidator:
    ...
    def _register_rules(self):
        ...
        rules = [
            ...
            ValidationRule(
                ...
                severity=ValidationSeverity.CRITICAL,
                validator=lambda d: self.WORD_COUNT_MIN <= d['total_words'] <= self.WORD_COUNT_MAX,
                error_message=lambda d: f"Total resume: {d['total_words']} words (target: {self.WORD_COUNT_MIN}-{self.WORD_COUNT_MAX})",
                category="word_count"
            ),
            ValidationRule(
                rule_id="VG_WORD_COUNT_K1",
                severity=ValidationSeverity.CRITICAL,
                validator=lambda d: 100 <= count_words_clean(d['staging_buffer'].get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, '')) <= 150,
                error_message=lambda d: f"K.1: {count_words_clean(d['staging_buffer'].get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, ''))} words (target: 100-150)",
                category="word_count"
            ),
            # --- Distribution Rules ---
            ValidationRule(
                ...
                severity=ValidationSeverity.HIGH,
                validator=lambda d: SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_combined_percent'][0] <= d['unify_ibm_percent'] <= SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_combined_percent'][1],
                error_message=lambda d: f"Unify+IBM: {d['unify_ibm_percent']:.1f}% of total (target: {SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_combined_percent'][0]}-{SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_combined_percent'][1]}%)",
                category="distribution"
            ),
            ValidationRule(
                rule_id="UNIFY_IBM_RATIO",
                severity=ValidationSeverity.HIGH,
                validator=lambda d: d['ibm_words'] > 0 and SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_ratio'][0] <= (d['unify_words'] / d['ibm_words']) <= SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_ratio'][1],
                error_message=lambda d: f"Unify/IBM ratio: {(d['unify_words'] / d['ibm_words']):.2f} (target: {SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_ratio'][0]}-{SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_ratio'][1]})" if d['ibm_words'] > 0 else "IBM section has 0 words",
                category="distribution"
            ),
            ...
            ValidationRule(
                ...
                severity=ValidationSeverity.CRITICAL,
                validator=lambda d: d['staging_buffer'].is_locked(),
            ...
            ValidationRule(
                rule_id="COVER_LETTER_SIGNATURE",
                severity=ValidationSeverity.HIGH,
                validator=lambda d: d['staging_buffer'].get(ResumeSection.K13_COVER_LETTER.value, '').strip().endswith(d['expected_signature']),
                error_message="Cover letter signature is missing or malformed.",
                category="structure"
            ),
            ValidationRule(
                rule_id="CONTENT_NO_PLACEHOLDERS",
                severity=ValidationSeverity.HIGH,
                validator=lambda d: not re.search(r'\[placeholder for', json.dumps(d['staging_buffer'].data), re.IGNORECASE),
                error_message=lambda d: (
                    f"Found placeholder text in content: "
                    f"{re.findall(r'(\\[placeholder for.*?\\])', json.dumps(d['staging_buffer'].data), re.IGNORECASE)[:3]}"
                ),
                category="content"
            ),
            ValidationRule(
                rule_id="VG_K1_DIFFERENTIATOR_KEYWORDS",
                severity=ValidationSeverity.CRITICAL,
                validator=lambda d: len([kw for kw in d['differentiators'] if kw.lower() in d['staging_buffer'].get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, '').lower()]) == len(d['differentiators']) if d['differentiators'] else True,
                error_message=lambda d: f"K.1 Summary contains {len([kw for kw in d['differentiators'] if kw.lower() in d['staging_buffer'].get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, '').lower()])}/{len(d['differentiators'])} top differentiator keywords.",
                category="content"
            ),
            ValidationRule(
                rule_id="JD_KEYWORD_ENFORCEMENT",
                severity=ValidationSeverity.HIGH,
                validator=lambda d: len(d['jd_keywords_found']) >= 5,
                error_message=lambda d: f"JD Keyword Enforcement: Found {len(d['jd_keywords_found'])}/{len(d['jd_keywords_tracked'])} keywords in resume (min 5 required).",
                category="content"
            ),
            # v8.10: Harden validator to check for new narrative data (Approach 3)
            ValidationRule(
                rule_id="NARRATIVE_MINING_PRESENCE",
                severity=ValidationSeverity.HIGH,
                validator=lambda d: (
                    d['thematic_analysis'].problem_solution_narratives is not None and
                    isinstance(d['thematic_analysis'].problem_solution_narratives.get("common_problems"), list) and
                    len(d['thematic_analysis'].problem_solution_narratives.get("common_problems", [])) > 0
                ),
                error_message="Phase 4 Narrative Mining data (problem_solution_narratives) is missing or incomplete in ThematicAnalysis.",
                category="content"
            ),
            ValidationRule(
                rule_id="COVER_LETTER_JD_RELEVANCE",
                severity=ValidationSeverity.HIGH,
                validator=lambda d: d.get('cover_letter_jd_similarity', 0.0) >= 0.35,
                error_message=lambda d: (
                    f"Cover letter relevance to JD is too low. "
                    f"Similarity: {d.get('cover_letter_jd_similarity', 0.0):.2f} (target: >= 0.35)"
                ),
                category="content"
            ),
        ]
    ...
    def _prepare_validation_data(
        ...
        unify_words = count_words_comprehensive(staging_buffer.get(ResumeSection.K5_UNIFY_OVERVIEW.value, '')) + count_words_comprehensive(staging_buffer.get(ResumeSection.K5_UNIFY_BULLETS.value, []))
        ibm_words = count_words_comprehensive(staging_buffer.get(ResumeSection.K6_IBM_OVERVIEW.value, '')) + count_words_comprehensive(staging_buffer.get(ResumeSection.K6_IBM_BULLETS.value, []))
        ...
        cover_letter_jd_similarity = self._calculate_cosine_similarity(
            cover_letter_text,
            job_description
        )
    ...
    def _calculate_cosine_similarity(self, text1: str, text2: str) -> float:
        ...
        if SKLEARN_AVAILABLE:
            try:
                ...
                return 0.0 # Fallback on sklearn error
        else:
...
class FileRenderer:
    ...
    def render(
        ...
        try:
            ...
            # Modular rendering for each artifact
            file_paths['resume_md'], file_contents['resume_md'] = self._render_resume_artifact(staging_buffer, company_name, job_title)
            file_paths['skills'], file_contents['skills'] = self._render_skills_artifact(staging_buffer, company_name, job_title, job_description)
            file_paths['cover_letter'], file_contents['cover_letter'] = self._render_cover_letter_artifact(staging_buffer, company_name, job_title)
            file_paths['qa_report'], file_contents['qa_report'] = self._render_qa_report_artifact(company_name, job_title)
            file_paths['app_tracker'], file_contents['app_tracker'] = self._render_app_tracker_artifact(company_name, job_title, file_paths)

            # Final validation result for the rendering hop
            validation_results.append(ValidationResult(
                rule_id="FILE_RENDER",
                passed=True,
                severity=ValidationSeverity.INFO,
...
class WorkflowOrchestrator:
    ...
    def _execute_hop_7_5_deduplication(self, staging_buffer: ImmutableStagingBuffer):
        ...
        dedup_input_data = {
            "sections": {
                ResumeSection.K5_UNIFY_BULLETS.value: staging_buffer.get(ResumeSection.K5_UNIFY_BULLETS.value),
                ResumeSection.K6_IBM_BULLETS.value: staging_buffer.get(ResumeSection.K6_IBM_BULLETS.value),
            },
    ...
    def execute_workflow(
        ...
        print("=" * 80)
        print("RESUME GENERATION ENGINE v8.40 - GEMINI API")
        print("=" * 80)
        ...
        try:
            ...
            for attempt in range(1, 6):
                ...
                artist_output_raw, current_hop_results = artist.generate(
                    enriched_scaffold,
                    job_description,
                    thematic_analysis,
                    feedback_results,
                ...
                hop3_results.extend(current_hop_results)
                
                llm_calls_made = len([k for k in artist_output_raw.keys() if artist_output_raw.get(k)])
                
                # --- START PATCH v8.31 ---
                
                # 2. Create a temporary buffer for this attempt's RAW content
                staging_buffer_temp = ImmutableStagingBuffer()
                for key, value in artist_output_raw.items():
                    staging_buffer_temp.set(key, value)

                # 3. Sanitize the content *inside* the loop
                print(f"    Attempt {attempt}: Sanitizing content...")
                sanitizer = TextSanitizer()
                sanitization_results, sanitized_data = sanitizer.sanitize_buffer(staging_buffer_temp)
                
                # 4. Create a *new* temp buffer with the SANITIZED data and lock it
                sanitized_buffer_temp = ImmutableStagingBuffer()
                for key, value in sanitized_data.items():
                    sanitized_buffer_temp.set(key, value)
                sanitized_buffer_temp.lock() 

                # 5. Validate the *sanitized* content
                print(f"    Attempt {attempt}: Validating sanitized content...")
                preflight = PreFlightValidator(self.master_resume)
                validation_results, all_passed = preflight.validate(sanitized_buffer_temp, thematic_analysis, job_description)
                
                # --- END PATCH v8.31 ---

                if all_passed or attempt == 5:
                    # If it passed (or we're out of tries), this is our final output
                    artist_output = sanitized_data # Use the SANITIZED data
                    hop3_results.extend(sanitization_results) # Add sanitization results to the hop's log
                    break
            ...
            # This now populates with the *already sanitized* artist_output from the loop
            for key, value in artist_output.items():
                staging_buffer.set(key, value)
            ...
            print("\n[HOP-4.5] Text Sanitization...")
            # This step is now redundant but kept for structural integrity.
            # It will run on already-sanitized data and report 0 changes.
            sanitizer = TextSanitizer()
            hop45_results, sanitized_data = sanitizer.sanitize_buffer(staging_buffer)
            
            # Update buffer (redundant, but harmless)
            for key, value in sanitized_data.items():
                staging_buffer.set(key, value)
            print("  ✓ Staging buffer updated with sanitized content (0 new changes expected)")
            ...
            print("\n[HOP-5] Pre-flight Validation...")
            # This validation now runs on the *same* sanitized content
            # that already passed inside the HOP-3 loop.
            validator = PreFlightValidator(self.master_resume)
            hop5_results, all_validations_passed = validator.validate(staging_buffer, thematic_analysis, job_description)
            
            hop5_checkpoint = self._create_checkpoint("HOP-5", "Pre-flight Validation", hop5_results, {"all_passed": all_validations_passed})
            ...
            if gate_decision == GateDecision.HALT:
                ...
                # Note: This block should now only be reached if the loop *itself* fails 5 times.
                return {
                    "status": "HALTED",
            ...
            print("\n[HOP-7] Rendering Output Files...")
            renderer = FileRenderer(self.master_resume)
            file_paths, hop7_results, file_contents = renderer.render(
                staging_buffer,
                company_name,
                job_title,
                thematic_analysis,
            ...
            # Find the gate decision from the last checkpoint
    ...
    def _execute_gate_0_jd_validation(self, job_description: str):
        """Executes GATE-0: JD Input Validation."""
        print("\n[GATE-0] JD Input Validation...")
        try:
            jd_validation = self.jd_enforcer.validate_jd_input(job_description, "GATE-0")
            failed_validations = [r for r in jd_validation if not r.passed]
    ...
    def _execute_gate_1_jd_parsing(self, thematic_analysis: ThematicAnalysis):
        """Executes GATE-1: JD Parsing Validation."""
        print("\n[GATE-1] JD Parsing Validation...")
        parsed_jd_for_validation = asdict(thematic_analysis)
        self.jd_enforcer.validate_jd_parsing(parsed_jd_for_validation, "GATE-1")

    def _execute_gate_2_thematic_validation(self, thematic_analysis: ThematicAnalysis):
        """Executes GATE-2: Thematic Analysis Validation."""
        print("\n[GATE-2] Thematic Analysis Validation...")
        self.jd_enforcer.validate_thematic_analysis(thematic_analysis, "GATE-2")

    def _execute_gate_3_enrichment_validation(self, enriched_scaffold: Dict):
        """Executes GATE-3: Enrichment Validation."""
        print("\n[GATE-3] Enrichment Validation...")
        self.jd_enforcer.validate_enrichment(enriched_scaffold, "GATE-3")

    def _execute_gate_4_artist_input_validation(self, enriched_scaffold: Dict, thematic_analysis: ThematicAnalysis):
        """Executes GATE-4: Artist Input Validation."""
        print("\n[GATE-4] Artist Input Validation...")
        self.jd_enforcer.validate_artist_inputs(enriched_scaffold, thematic_analysis, "GATE-4")

    def _execute_gate_5_preflight_validation(self, staging_buffer: ImmutableStagingBuffer):
        """Executes GATE-5: Pre-flight JD Validation."""
        print("\n[GATE-5] Pre-flight JD Validation...")
        self.jd_enforcer.validate_preflight(staging_buffer, "GATE-5")

    def _execute_gate_7_file_output_validation(self, file_paths: Dict[str, str]):
        """Executes GATE-7: File Output Validation."""
        print("\n[GATE-7] File Output Validation...")
        self.jd_enforcer.validate_file_output(file_paths, "GATE-7")

    def _execute_gate_8_qa_report_validation(self, qa_report_text: str):
        """Executes GATE-8: QA Report Validation."""
        print("\n[GATE-8] QA Report Validation...")
        self.jd_enforcer.validate_qa_report({"report": qa_report_text}, "GATE-8")

    def _execute_hop_3_5_skills_validation(self, artist_output: Dict):
        """Executes HOP-3.5: HR-Validated Skills Generation Validation."""
        if ResumeSection.K2_SKILLS.value in artist_output:
            k2_skills = artist_output.get(ResumeSection.K2_SKILLS.value, [])
            print("\n[HOP-3.5 VALIDATION] HR-Validated 12 Skills...")
            hop35_checkpoint = self._create_checkpoint(
    ...
    def _execute_hop_3_artist_generation_with_retry(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        """Executes HOP-3: Content Generation with a feedback/retry loop."""
        print("\n[HOP-3] Content Generation...")
        artist = ArtistGenerator(self.master_resume)
        artist_output = None
        feedback_results = None
        ...
        self._execute_gate_4_artist_input_validation(enriched_scaffold, thematic_analysis)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            sanitizer = TextSanitizer()
            sanitization_results, sanitized_data = sanitizer.sanitize_buffer(temp_buffer)
            ...
            if all_passed or attempt == max_attempts:
                artist_output = sanitized_data
                hop3_results.extend(sanitization_results)
                llm_calls_made = len([k for k in artist_output_raw.keys() if artist_output_raw.get(k)])
                hop_checkpoint = self._create_checkpoint(
                    "HOP-3", f"Artist Generation (attempt {attempt})", hop3_results, artist_output,
                    metadata={"llm_api_calls": llm_calls_made}
                )
                self.hop_checkpoints.append(hop_checkpoint)
                self._check_hop_status(hop_checkpoint)
    ...
    def _build_coc_ledger(
        ...
        return {
            ...
            "version": "v8.40",
            "architecture": "Job_Workflow_v8.40_Comprehensive_Redesign",
    ...
    def _build_qa_section_1_signal_quality(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis) -> List[str]:
        ...
        for label, (section_enum, target_score, weight) in signal_sections.items():
            ...
            if content:
                ...
                total_weight += weight
                lines.append(self._format_ascii_bar_chart(label=label, value=score, target=target_score))
            else:
                lines.append(f"{label:<25} [SKIPPED - No Content]")
        ...
        if total_weight > 0:
            ...
            lines.append("-" * 80)
            lines.append(self._format_ascii_bar_chart(label="Total Signal Score", value=average_signal, target=overall_target))
        return lines
    ...
    def _build_qa_section_4_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        """Builds Section 4 of the QA report: Bullet Provenance & Word Count."""
        lines = ["", "4. BULLET PROVENANCE & WORD COUNT", ""]
            ...
            if isinstance(bullets, list) and bullets and isinstance(bullets[0], dict):
                for i, bullet_data in enumerate(bullets):
                    rows.append([
                        ...
                        str(bullet_data.get('word_count', 'N/A')), bullet_data.get('text', '')[:60] + "..."
                    ])
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines

    def _build_qa_section_5_authenticity(self, validation_results: List[ValidationResult]) -> List[str]:
        """Builds Section 5 of the QA report: Content Authenticity (Hallucination)."""
        lines = ["", "5. CONTENT AUTHENTICITY (Hallucination Detection)", ""]
        ...
        rows = [[vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message] for vr in auth_results] if auth_results else [["HALLUCINATION_CHECK", "PASS", "No hallucination checks run or all passed."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines

    def _build_qa_section_6_ai_defense(self) -> List[str]:
        """Builds Section 6 of the QA report: AI Detection Defense."""
        lines = ["", "6. AI DETECTION DEFENSE (Overview vs. Bullet Similarity)", ""]
        if self.overview_similarity_data:
            ...
            passed = max_sim < 0.6
            rows = [[sim_data.get("section", "N/A"), f"{max_sim:.2f}", "< 0.60", "✓ PASS" if passed else "✗ FAIL"]]
            lines.extend(self._format_plain_text_table(headers, rows))
        else:
            lines.append("AI Detection Defense analysis was not performed (missing data).")
        return lines

    def _build_qa_section_7_deduplication(self) -> List[str]:
        """Builds Section 7 of the QA report: Deduplication Matrix."""
        lines = ["", "7. DEDUPLICATION MATRIX (Pairwise Similarity Analysis)", ""]
        if self.similarity_matrix_data:
            ...
            lines.append("Deduplication Matrix analysis was not performed (missing data).")
        return lines

    def _build_qa_section_8_pipeline_health(self) -> List[str]:
        """Builds Section 8 of the QA report: Pipeline Health."""
        lines = ["", "8. PIPELINE HEALTH (Resource Consumption)", ""]
    ...
    def _build_qa_section_10_structural(self, validation_results: List[ValidationResult]) -> List[str]:
        """Builds Section 10 of the QA report: Structural Validation."""
        lines = ["", "10. STRUCTURAL VALIDATION", ""]
    ...
    def _build_qa_section_9_word_count(self, validation_results: List[ValidationResult]) -> List[str]:
        """Builds Section 9 of the QA report: Word Count Compliance."""
        lines = ["", "9. WORD COUNT COMPLIANCE", ""]
        wc_results = [vr for vr in validation_results if "WORD_COUNT" in vr.rule_id]
        headers = ["Rule ID", "Status", "Message"]
        rows = [[vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message] for vr in wc_results] if wc_results else [["N/A", "INFO", "No word count validation results found."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines
    ...
    def _build_qa_section_10_structural(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        struct_results = [vr for vr in validation_results if "STRUCTURE" in vr.rule_id or "HEADLINE" in vr.rule_id]
        headers = ["Rule ID", "Status", "Message"]
        rows = [[vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message] for vr in struct_results] if struct_results else [["N/A", "INFO", "No structural validation results found."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines

    def _build_qa_section_11_prod_readiness(self, validation_results: List[ValidationResult]) -> List[str]:
        """Builds Section 11 of the QA report: Production Readiness."""
    ...
    def _build_qa_section_12_cover_letter(self, validation_results: List[ValidationResult]) -> List[str]:
        """Builds Section 12 of the QA report: Cover Letter QA."""
        lines = ["", "12. COVER LETTER QA", ""]
        cl_results = [vr for vr in validation_results if "COVER_LETTER" in vr.rule_id]
        headers = ["Rule ID", "Status", "Message"]
        rows = [[vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message] for vr in cl_results] if cl_results else [["N/A", "INFO", "No cover letter validation results found."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines

    def _build_qa_section_13_jd_enforcement(self) -> List[str]:
        """Builds Section 13 of the QA report: JD Enforcement Validation."""
    ...
    def _generate_qa_report(
        ...
        """Generate full 12-section QA report by calling modular section builders."""
        validation_results_out = []
        report_lines = [
            "=" * 80,
            "RESUME QA REPORT (v8.40 - HARDENED)",
            f"Generated: {datetime.now().isoformat()}",
            "=" * 80,
        ]

        # Build each section of the report by calling its dedicated helper
        report_lines.extend(self._build_qa_section_1_signal_quality(staging_buffer, thematic_analysis))
        report_lines.extend(self._build_qa_section_2_thematic_compliance(thematic_analysis))
        report_lines.extend(self._build_qa_section_3_hop_summary())
        report_lines.extend(self._build_qa_section_4_provenance(staging_buffer))
        report_lines.extend(self._build_qa_section_5_authenticity(validation_results))
        report_lines.extend(self._build_qa_section_6_ai_defense())
        report_lines.extend(self._build_qa_section_7_deduplication())
        report_lines.extend(self._build_qa_section_8_pipeline_health())
        report_lines.extend(self._build_qa_section_9_word_count(validation_results))
        report_lines.extend(self._build_qa_section_10_structural(validation_results))
        report_lines.extend(self._build_qa_section_11_prod_readiness(validation_results))
        report_lines.extend(self._build_qa_section_12_cover_letter(validation_results))
        report_lines.extend(self._build_qa_section_13_jd_enforcement())
