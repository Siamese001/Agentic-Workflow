...
class ArtistGenerator:
    ...
    v11.0 REDESIGN: This class is now architected to consume the *full*
    ThematicAnalysis object from HOP-0, including Secondary Themes
    and Authenticity Patterns, in all relevant generation prompts.
    """
    ...
    def generate(
        ...
        try:
            artist_output = self._generate_artist_output(k1_temp_override=k1_temp_override)
    ...
    def _generate_artist_output(self, k1_temp_override: Optional[float] = None) -> Dict:
        """
...
"""
    ...
    def _generate_k1_executive_summary(self, temperature_override: Optional[float] = None) -> str:
        ...
        top_bullets = []
        for exp in self.master_resume['professional_experience'][:2]:
        ...
        differentiators = self.thematic_analysis.competitive_intelligence.get_top_differentiators(5)
        role_archetype = self.thematic_analysis.role_classification.get('role_archetype', 'Technical_IC')
        
        # 3. Get Authenticity Patterns
        auth_patterns = self.thematic_analysis.authenticity_patterns.get('patterns', {})
        ...
        if self.previous_failures:
            for failure in self.previous_failures:
                if failure.rule_id in ["VG_WORD_COUNT_K1", "VG_SENTENCE_COUNT_K1"]:
                    feedback_instruction = (
                        "\n**CRITICAL FEEDBACK FROM PREVIOUS ATTEMPT:**\n"
                        "Your previous summary FAILED validation. It did not meet requirements.\n"
                        f"**FAILURE DETAILS:** {failure.message}\n"
                        "You MUST generate a new summary that is strictly between 120 and 150 words AND 5 to 6 sentences.\n\n"
                    )
                    break
...
6. Return ONLY the summary paragraph.
"""

        # 7. Call LLM
        reasoning_config = ReasoningConfig.K1_EXECUTIVE_SUMMARY_CONFIG
        final_temp = temperature_override if temperature_override is not None else 1.0
        base_system = "You are an expert resume writer. You synthesize multiple data signals (themes, keywords, authentic voice) into a cohesive summary."
...
"""
    ...
    def _generate_k0_headline(self) -> str:
        ...
        # Get signals
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'AI Leadership')
        differentiators = self.thematic_analysis.competitive_intelligence.get_top_differentiators(3)
...
"""
        ...
        base_system = "You are an expert at crafting professional resume headlines. You synthesize job themes and authentic voice."
...
**Instructions:**
1. Select the TOP {verbatim_count} bullets that best match ALL the job themes (Primary and Secondary).
2. Prioritize bullets with strong alignment to the job description and keywords.
3. DO NOT modify the bullet text.
4. Return ONLY the {verbatim_count} selected bullets, one per line, with no numbers or formatting.
"""
            system_prompt_select = "You select verbatim bullets based on job fit, considering primary and secondary themes."
            ...
            if "[Placeholder" not in response_select:
                ...
                final_bullets.extend(master_bullets_structured[:verbatim_count])
        ...
        if synthetic_count > 0:
            context_bullets_text = '\n'.join([f"- {b['bullet_text']}" for b in final_bullets])
            synthetic_bullets = self._generate_synthetic_bullets(
                count=synthetic_count,
                company_name=company_name,
                job_description=self.job_description,
                thematic_analysis=self.thematic_analysis, # Pass full object
        ...
        section_name_for_avg = "Unify"
        if "IBM" in company_name: section_name_for_avg = "IBM"
        ...
        # --- 8. Final Reordering (Now with Secondary Themes) ---
...
**Instructions:**
1. Reorder the provided bullets based on their relevance to ALL themes (Primary and Secondary).
2. DO NOT modify the bullet text.
3. Return ONLY the {total_expected_count} reordered bullets, one per line.
"""
        system_prompt_reorder = "You reorder bullets based on job fit using all provided themes. Never modify text."
        ...
        if "[Placeholder" in response_reorder:
            ...
            return final_bullets[:total_expected_count]
        ...
        for text in reordered_texts:
            if text in generated_map and text not in used_from_generated:
                ...
                logging.warning(f"{section_id} Reordering returned unexpected or duplicate bullet text. Skipping.")
        
        if len(final_ordered_bullets) != total_expected_count:
             logging.warning(f"{section_id} Reordering validation failed. Returning bullets in generated order.")
             return final_bullets[:total_expected_count]
    ...
    def _generate_tailored_overview_for_experience(
        ...
        differentiators = self.thematic_analysis.competitive_intelligence.get_top_differentiators(3)
        auth_patterns = self.thematic_analysis.authenticity_patterns.get('patterns', {})
        auth_tone_example = auth_patterns.get('executive_summary_patterns', ["professional and consultative"])[0]
...
6. Output must be a single paragraph, approximately {word_count_range[0]}-{word_count_range[1]} words.
{extra_prompt_instruction}
7. Return ONLY the rewritten overview text with no preamble or explanation.
"""
        system_prompt = "You are an expert resume editor. You rewrite professional overviews to align with job descriptions, prioritizing authenticity and natural language. Never invent new facts."
...
**Instructions:**
1. Rewrite the bullet to be between **{min_wc} and {max_wc} words**.
2. **CRITICAL: DO NOT change, invent, or remove any facts, metrics (e.g., "$5M", "30%"), or proper nouns.**
3. Maintain an authentic, professional tone, guided by the verbs.
4. If the original is too long, concisely rephrase it.
5. If the original is too short, add professional phrasing (e.g., "Led initiative to...") *without* adding new facts.
6. Return ONLY the single rewritten bullet point, with no preamble.
"""
            system_prompt = f"You are a resume word count editor. You rewrite bullets to {min_wc}-{max_wc} words, preserving all facts."
            ...
            # Final check on the rewrite
            rewritten_wc = count_words_ms_word_style(rewritten_text)
            if not (min_wc <= rewritten_wc <= max_wc):
                 logging.warning(f"{section_id}_RewriteWC returned non-compliant count ({rewritten_wc}). Returning original.")
                 return original_bullet_text
...
"""
    ...
    def _generate_k10_competencies(self) -> List[Dict]:
        ...
        all_competencies_structured = [{
            "bullet_text": c.replace("• ", "").strip(), # Use 'bullet_text' for compatibility
            "provenance": BulletProvenance.Verbatim.value,
        ...
        if verbatim_count > 0:
            ...
            prompt_select = f"""Select the most relevant competencies for this job.
...
**Instructions:**
1. Select the TOP {verbatim_count} competencies that best match ALL the job themes (Primary and Secondary).
2. DO NOT modify the competency text.
3. Return ONLY the {verbatim_count} selected competencies, one per line.
"""
            system_prompt_select = "You select verbatim competencies based on job fit, considering primary and secondary themes."
            ...
            if "[Placeholder" not in response_select:
                selected_texts = [line.strip() for line in response_select.split('\n') if line.strip()]
                # Use the bullet validation helper
                ...
                final_competencies.extend(all_competencies_structured[:verbatim_count])

        # 5. Select and Customize Competencies
        if customized_count > 0:
            used_texts = {b['bullet_text'] for b in final_competencies}
            available_for_custom = [b for b in all_competencies_structured if b['bullet_text'] not in used_texts]
            random.shuffle(available_for_custom)
            candidates_for_custom = available_for_custom[:customized_count]
            ...
            if candidates_for_custom:
                customized_comps = self._generate_lightly_customized_bullets(
                    ...
                    thematic_analysis=self.thematic_analysis # Pass all signals
                )
                # Convert 'text' back to 'bullet_text' for consistency
                # Also ensure the original dict structure is preserved/re-created
                final_competencies.extend([
                    {
                        "bullet_text": c['text'], 
                        "provenance": c['provenance'], 
                        "word_count": c['word_count']
                    } for c in customized_comps
                ])
            else:
        ...
        if synthetic_count > 0:
            context_bullets_text = '\n'.join([f"- {b['bullet_text']}" for b in final_competencies])
            synthetic_comps = self._generate_synthetic_bullets(
                count=synthetic_count,
                company_name="Candidate", # Generic company
                job_description=self.job_description,
                thematic_analysis=self.thematic_analysis,
                context_bullets=context_bullets_text,
            ...
            # Ensure the dict structure matches
            final_competencies.extend([
                {
                    "bullet_text": c['text'], 
                    "provenance": c['provenance'], 
                    "word_count": c['word_count']
                } for c in synthetic_comps
            ])
        ...
        if len(final_competencies) < total_expected_count:
            ...
            used_texts = {b['bullet_text'] for b in final_competencies}
            padding = [b for b in all_competencies_structured if b['bullet_text'] not in used_texts][:needed]
            final_competencies.extend(padding)
             ...
             final_competencies.sort(key=lambda b: 0 if b['provenance'] != 'Verbatim' else 1)
        ...
        # 9. Final reformatting to match expected output structure
        # The validator returns dicts with 'text', but K.10 renderer expects dicts with 'text'
        # We need to make sure the key 'text' is used, not 'bullet_text'
        # The rewrite helper already uses 'text', so we just need to rename 'bullet_text'
        output_list = []
        for item in final_validated_comps:
            if 'text' not in item and 'bullet_text' in item:
                item['text'] = item['bullet_text']
                del item['bullet_text']
    ...
    def _generate_k13_cover_letter(self) -> str:
        ...
        # Signal 4: Authenticity Patterns
        auth_patterns = self.thematic_analysis.authenticity_patterns.get('patterns', {})
        auth_tone_example = auth_patterns.get('executive_summary_patterns', ["professional and consultative"])[0]

        # Get top achievements for the "Proof" section
        top_achievements = []
...
- **Competitive Differentiators:** Key skills are: {', '.join(differentiators)}.
...
- Frame them as a mini-story: "At [Company], we faced [Problem]. I led [Action], incorporating '{differentiators[0]}' and '{differentiators[1]}'{secondary_theme_context}, which resulted in [Quantifiable Result]."
...
Return the complete cover letter text. Start with the date and end *exactly* with the signature block provided.
...
{signature}"""
        ...
        try:
            ...
            genai.configure(api_key=api_key)
            model = genai.GenerativeModel('gemini-1.5-flash', system_instruction=f"You are an expert executive ghostwriter. Your writing style is: {auth_tone_example}. You follow all formatting and content instructions perfectly.")
...
def calculate_signal_score(text_content, thematic_analysis: ThematicAnalysis):
    ...
    try:
        if hasattr(thematic_analysis, 'competitive_intelligence') and thematic_analysis.competitive_intelligence:
            differentiators = set(thematic_analysis.competitive_intelligence.differentiator_keywords or [])
        else:
            differentiators = set()
        ...
        logging.warning("Error accessing keywords in thematic_analysis for signal score calculation.")
        return 0.0
...
def _calculate_master_avg_bullet_length(master_resume: Dict) -> Dict[str, float]:
    """
    ...
    for key, (company_name_part, bullets_key) in experience_sections_to_avg.items():
        total_words = 0
        bullet_count = 0
        exp = next((e for e in master_resume.get("professional_experience", []) if company_name_part in e.get("company", "")), None)
        if exp:
...
class PreFlightValidator:
    ...
    def validate(
        ...
        # Calculate master average lengths using the corrected helper
    ...
    # (These methods remain the same as provided before, including the new _validate_authenticity_signal)
    ...
    def _validate_unify_ibm_ratio(self, data: Dict) -> bool:
        if data['ibm_words'] == 0:
            data["error_details"] = {"ratio": "N/A (IBM words = 0)", "min": SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_ratio'][0], "max": SECTION_CONSTRAINTS_V521['word_distribution']['unify_ibm_ratio'][1]}
    ...
    def _validate_cover_letter_narrative(self, data: Dict) -> bool:
        ...
        # Expecting Date, Recipient, Salutation, P1, P2, P3, Closing, Signature Block (at least 5 splits -> 6+ parts)
        if len(paragraphs) < 6:
        ...
        # Assuming standard structure: Date[0], Recipient[1], Salutation[2], P1[3], P2[4], P3[5], Closing[6], Signature[7+]
    ...
    # --- Placeholder Formatting Validators (Checked in QA Section 16) ---
    ...
    def _validate_experience_render_format(self, data: Dict) -> bool:
        ...
        data["error_details"] = {"details": "Checked during rendering/QA-16"}
        return True # Checked visually/post-render
    # --- End Placeholder Formatting Validators ---
    ...
    def _calculate_master_avg_bullet_length(self) -> Dict[str, float]:
        ...
        competencies_strings = [c for c in competencies_list if isinstance(c, str)]
        avg_lengths["Competencies"] = (sum(count_words_ms_word_style(b) for b in competencies_strings) / len(competencies_strings)) if competencies_strings else 28.0 # Specific default for competencies
...
class WorkflowOrchestrator:
    """
    Manages the execution of the multi-hop resume generation workflow.
    Coordinates data flow, validation, and output generation.
    v11.0: Incorporates redesigned HOP-3 signal consumption and updated QA reporting.
    v10.14->v11.0 Refactor: Replaced QA Section 2 with Section 10 (Signal Flow Map).
    v11.0.1 Refactor: Renumbered QA sections 1-9, 10-15 per user request. New Signal Map is Section 2.
    """
    ...
    # --- Methods for executing Hops ---
    # (These _execute_hop_ methods remain unchanged from v10.14)
    ...
    def _execute_hop_3_artist_generation(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        max_attempts = 5
        attempt = 0
        k1_temperature_schedule = [1.0, 0.9, 0.8, 0.7, 0.5]

        for attempt in range(1, max_attempts + 1):
            self.logger.info(f"  Attempt {attempt}/{max_attempts}...")
            attempt_start_time = time.time()
            current_k1_temp = k1_temperature_schedule[attempt - 1]
            if attempt > 1:
            ...
            try:
                current_artist_output, current_generation_results = artist.generate(
                    ...
                    attempt=attempt,
                    k1_temp_override=current_k1_temp
                )
        ...
        combined_results = generation_hop_results + final_validation_results
        ...
        if not all_passed:
            ...
            hop_checkpoint.status = HopStatus.FAIL
            hop_checkpoint.error_message = f"Content validation failed after {max_attempts} attempts."
            self.validation_results.extend(final_validation_results or [])
    ...
    # --- QA Report Section Builders (Renumbered) ---
    ...
    def _build_qa_section_2_signal_flow_map(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis) -> List[str]:
        """
        [v11.0 NEW -> Renumbered to Section 2] Builds QA Section 2: RAG Signal Flow Map.
        """
        lines = ["", "2. HOP-0 RAG SIGNAL FLOW MAP (Consumed Signals)", ""] # <<< Renumbered
    ...
    def _build_qa_section_3_hop_summary(self) -> List[str]:
        ...
        lines = ["", "3. HOP-BY-HOP EXECUTION SUMMARY", ""] # <<< Renumbered
        lines.append("```markdown")
    ...
    def _build_qa_section_4_distribution(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "4. WORD COUNT DISTRIBUTION", ""] # <<< Renumbered
        dist_results = [vr for vr in validation_results if "DISTRIBUTION" in vr.rule_id or "RATIO" in vr.rule_id]
        lines.append("```markdown")
        headers = ["Rule ID", "Value", "Target Range", "Status", "Message"]
        rows = []
        for vr in dist_results:
            details = vr.details or {}
            value_str = "N/A"
            if 'percent' in details: value_str = f"{details['percent']:.1f}%"
            elif 'ratio' in details: value_str = f"{details.get('ratio', 'N/A')}" # Handle potential N/A from validator
            target_str = f"{details.get('min', '?')}-{details.get('max', '?')}"
            if '%' in value_str: target_str += "%"
            msg = vr.message(vr.details) if callable(vr.message) and not vr.passed else vr.message if not vr.passed else ""
            rows.append([vr.rule_id, value_str, target_str, "PASS" if vr.passed else "FAIL", msg])
    ...
    def _build_qa_section_5_word_count(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "5. WORD COUNT COMPLIANCE (Summary)", ""] # <<< Renumbered
        # Include all relevant word/sentence count rules
        wc_rule_ids = [
             "VG_TOTAL_WORD_COUNT", "VG_WORD_COUNT_K1", "VG_SENTENCE_COUNT_K1",
             "VG_HEADLINE_WORD_COUNT", "VG_BULLET_WORD_COUNT_TOLERANCE",
             "COVER_LETTER_STRUCTURE" # Paragraph counts
        ]
        wc_results = [vr for vr in validation_results if vr.rule_id in wc_rule_ids]

        lines.append("```markdown")
        headers = ["Rule ID", "Status", "Details / Message"]
        rows = []

        if wc_results:
            wc_results.sort(key=lambda vr: vr.rule_id)
            for vr in wc_results:
                msg = vr.message(vr.details) if callable(vr.message) else vr.message
                rows.append([vr.rule_id, "PASS" if vr.passed else "FAIL", msg if not vr.passed else ""])
        else:
            rows.append(["N/A", "INFO", "No word/sentence count results found."])

        expected_rows = len(wc_results) if wc_results else 1
        if len(rows) != expected_rows:
             lines.insert(2, f"ERROR   TRUNCATION_DETECTED   FAIL   Expected {expected_rows} wc rows, got {len(rows)}")
    ...
    def _build_qa_section_6_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "6. BULLET PROVENANCE & WORD COUNT", ""] # <<< Renumbered
        lines.append("```markdown")
        ...
        for section_data in self.QA_PROVENANCE_SECTIONS:
            ...
            if section_data.get("overview_enum"):
                ...
                if overview_text:
                    ...
                    # Use getattr to safely access constraint attributes
                    min_target = getattr(self.constraints, section_data.get("overview_min_attr", "DEFAULT_MIN"), 0)
                    max_target = getattr(self.constraints, section_data.get("overview_max_attr", "DEFAULT_MAX"), 999)
                    status, target_range_str = self._check_word_count(word_count, min_target, max_target)
                    rows.append([name, "Overview", "Customized", str(word_count), target_range_str, status, overview_text[:60] + "..."])
            ...
            if bullets_enum:
                ...
                if isinstance(bullets, list) and bullets:
                    for i, bullet_item in enumerate(bullets):
                        ...
                        elif isinstance(bullet_item, str): # Handle K7 case
                        ...
                        max_target = round(avg_len * (1 + tolerance))
                        status, target_range_str = self._check_word_count(word_count, min_target, max_target)
                        rows.append([name, str(i + 1), provenance, str(word_count), target_range_str, status, bullet_text[:60] + "..."])
    ...
    def _build_qa_section_7_authenticity(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "7. CONTENT AUTHENTICITY (Hallucination & Signal Check)", ""] # <<< Renumbered
    ...
    def _build_qa_section_8_overview_similarity(self) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "8. OVERVIEW VS. BULLET SIMILARITY CHECK", ""] # <<< Renumbered
    ...
    def _build_qa_section_9_pairwise_similarity(self) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "9. PAIRWISE BULLET SIMILARITY (Deduplication)", ""] # <<< Renumbered
        lines.append("```markdown")
    ...
    def _build_qa_section_10_pipeline_health(self) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "10. PIPELINE HEALTH (Resource Consumption)", ""] # <<< Renumbered (was 11)
        lines.append("```markdown")
    ...
    def _build_qa_section_11_structural(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "11. STRUCTURAL VALIDATION", ""] # <<< Renumbered (was 12)
        struct_rule_ids = [
            "STRUCTURE_", "HEADLINE", # Prefixes and keywords
            "VG_RESUME_HEADER_H2", "VG_EDU_CERTS_FORMAT",
            "VG_EXPERIENCE_BULLET_STYLE", "VG_COMPETENCIES_FORMATTING",
            "VG_EXPERIENCE_RENDER_FORMAT",
            "COVER_LETTER_SIGNATURE", "VG_COVER_LETTER_FULL_STRUCTURE",
            "VG_COVER_LETTER_SIGNATURE_MULTILINE"
        ]
        ...
        if struct_results:
             ...
             rows = [["N/A", "INFO", "No structural validation results found."]]
             expected_rows = 1
    ...
    def _build_qa_section_12_prod_readiness(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "12. PRODUCTION READINESS", ""] # <<< Renumbered (was 13)
        lines.append("```markdown")
    ...
    def _build_qa_section_13_cover_letter(self, validation_results: List[ValidationResult]) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "13. COVER LETTER QA", ""] # <<< Renumbered (was 14)
    ...
    def _build_qa_section_14_jd_enforcement(self) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "14. JD ENFORCEMENT VALIDATION", ""] # <<< Renumbered (was 15)
        lines.append("```markdown")
    ...
    def _build_qa_section_15_final_format(
        self,
        validation_results: List[ValidationResult],
        file_contents: Dict[str, str]
    ) -> List[str]:
        # (Implementation unchanged, title updated)
        lines = ["", "15. FINAL OUTPUT ARTIFACTS", ""] # <<< Renumbered (was 16)
        lines.append("```markdown")
        headers = ["Output Artifact", "Formatting Requirement", "Status", "Failure Explanation"]
        output_definitions = []

        # Resume Checks
        resume_content = file_contents.get('resume_md', '')
        output_definitions.append(("**Resume**", "Raw Text / Markdown", "PASS" if resume_content else "FAIL", "" if resume_content else "Content missing."))
        if resume_content:
            name_pass = resume_content.startswith("## ")
            output_definitions.append(("", "K.0 Name (## Format)", "PASS" if name_pass else "FAIL", "" if name_pass else "Name does not start with H2 (##)."))
            ...
            output_definitions.append(("", "K.0 Headline (Plain Text)", "PASS" if headline_pass else "FAIL", "" if headline_pass else "Headline missing, uses Markdown, or missing pipes."))
            lb_pass = bool(re.search(r"^## .+\n\n\S.+\n", resume_content))
            output_definitions.append(("", "Line Break Between Name/Headline", "PASS" if lb_pass else "FAIL", "" if lb_pass else "Missing/incorrect blank line."))
            h2_headers = ["## EXECUTIVE SUMMARY", "## PROFESSIONAL EXPERIENCE", "## EDUCATION", "## CERTIFICATIONS & CREDENTIALS", "## STRATEGIC & TECHNICAL COMPETENCIES"]
            missing_headers = [h for h in h2_headers if h not in resume_content]
            h2_pass = not missing_headers
            output_definitions.append(("", "H2 Headers (##) Used", "PASS" if h2_pass else "FAIL", "" if h2_pass else f"Missing: {missing_headers}"))
            exp_comp_loc_pass = bool(re.search(r"\n\*\*.*\s+\|\s+.*\*\*\n", resume_content))
            output_definitions.append(("", "Experience: `**Comp \| Loc**` format", "PASS" if exp_comp_loc_pass else "FAIL", "" if exp_comp_loc_pass else "Missing bold Company/Location."))
            exp_title_date_pass = bool(re.search(r"\n\*\*.*\s+\|\s+.*–.*\*\*\n", resume_content))
            output_definitions.append(("", "Experience: `**Title \| Dates**` format", "PASS" if exp_title_date_pass else "FAIL", "" if exp_title_date_pass else "Missing bold Title/Dates."))
            prof_exp_section_match = re.search(r"## PROFESSIONAL EXPERIENCE\n\n(.*?)(?:\n## |\Z)", resume_content, re.DOTALL)
            prof_exp_content = prof_exp_section_match.group(1) if prof_exp_section_match else ""
            exp_bullets_pass = bool(re.search(r"\n\* ", prof_exp_content))
            output_definitions.append(("", "Experience: `* ` Bullets Used", "PASS" if exp_bullets_pass else "FAIL", "" if exp_bullets_pass else "Experience bullets not using '* '."))
            comp_section_match = re.search(r"## STRATEGIC & TECHNICAL COMPETENCIES\n\n(.*?)(?:\n## |\Z)", resume_content, re.DOTALL)
            comp_content = comp_section_match.group(1) if comp_section_match else ""
            comp_bullets_pass = bool(re.search(r"^\* ", comp_content, re.MULTILINE))
            output_definitions.append(("", "Competencies: `* ` Bullets Used", "PASS" if comp_bullets_pass else "FAIL", "" if comp_bullets_pass else "Competencies not using '* '."))
            edu_section_match = re.search(r"## EDUCATION\n\n(.*?)(?:\n## |\Z)", resume_content, re.DOTALL)
            edu_content = edu_section_match.group(1) if edu_section_match else ""
            edu_no_bullet_pass = edu_content and not bool(re.search(r"^[\*•-] ", edu_content, re.MULTILINE))
            output_definitions.append(("", "Education: No Bullets", "PASS" if edu_no_bullet_pass else "FAIL", "" if edu_no_bullet_pass else "Education uses bullets."))
            cert_section_match = re.search(r"## CERTIFICATIONS & CREDENTIALS\n\n(.*?)(?:\n## |\Z)", resume_content, re.DOTALL)
            cert_content = cert_section_match.group(1) if cert_section_match else ""
            cert_no_bullet_pass = cert_content and not bool(re.search(r"^[\*•-] ", cert_content, re.MULTILINE))
            output_definitions.append(("", "Certifications: No Bullets", "PASS" if cert_no_bullet_pass else "FAIL", "" if cert_no_bullet_pass else "Certifications uses bullets."))
            lb_count = resume_content.count('\n\n')
            expected_min_lbs = 10
            lb_pass = lb_count >= expected_min_lbs
            output_definitions.append(("", "Proper Line Breaks", "PASS" if lb_pass else "WARN", "" if lb_pass else f"Found {lb_count} blank lines, expected ~{expected_min_lbs}+. Review visually."))

        # Skills Checks
        skills_content = file_contents.get('skills', '')
        output_definitions.append(("**Skills**", "Raw Text", "PASS" if skills_content else "FAIL", "" if skills_content else "Content missing."))
        if skills_content:
            skills_lines = [line.strip() for line in skills_content.strip().split('\n') if line.strip()]
            bullet_pass = all(line.startswith("• ") for line in skills_lines)
            output_definitions.append(("", "Bulleted list (`• `) Used", "PASS" if bullet_pass else "FAIL", "" if bullet_pass else "Skills not using '• ' format."))
            newline_pass = skills_content.strip().count('\n') >= (len(skills_lines) - 1) if len(skills_lines) > 1 else True
            output_definitions.append(("", "Newlines Between Skills", "PASS" if newline_pass else "FAIL", "" if newline_pass else "Missing newlines between skills."))

        # Cover Letter Checks
        cl_content = file_contents.get('cover_letter', '')
        output_definitions.append(("**Cover Letter**", "Raw Text", "PASS" if cl_content else "FAIL", "" if cl_content else "Content missing."))
        if cl_content:
            date_pass = bool(re.match(r"\w+ \d{1,2}, \d{4}", cl_content.strip()))
            recipient_pass = "Hiring Manager" in cl_content and "[Company Name]" in cl_content
            body_paras_pass = cl_content.count('\n\n') >= 4
            closing_pass = "\n\nSincerely,\n\n" in cl_content
            sig_template_parts = [p.strip() for p in COVER_LETTER_SIGNATURE_TEMPLATE.split('\n') if p.strip()]
            sig_block_present = all(part.format(name="", email="", phone="", linkedin="") in cl_content for part in sig_template_parts if "{" not in part)
            format_pass = date_pass and recipient_pass and body_paras_pass and closing_pass and sig_block_present
            output_definitions.append(("", "Standard Letter Format Present", "PASS" if format_pass else "FAIL", "" if format_pass else f"Missing elements (Date:{date_pass}, Recip:{recipient_pass}, Paras:{body_paras_pass}, Close:{closing_pass}, Sig:{sig_block_present})"))
            sig_block_match = re.search(r"Sincerely,\n\n(.*?)\Z", cl_content, re.DOTALL)
            sig_block_content = sig_block_match.group(1).strip() if sig_block_match else ""
            multiline_pass = sig_block_content.count('\n') >= 3
            output_definitions.append(("", "Signature Renders Multi-line", "PASS" if multiline_pass else "FAIL", "" if multiline_pass else "Signature block missing expected newlines."))

        # QA Report Checks
        qa_content = file_contents.get('qa_report', '')
        output_definitions.append(("**QA Report**", "Fenced Markdown (` ```markdown ... ``` `)", "PASS" if qa_content else "FAIL", "" if qa_content else "Content missing."))
        if qa_content:
            fence_pass = "```markdown" in qa_content and qa_content.strip().endswith("```")
            output_definitions.append(("", "Uses Fenced Markdown", "PASS" if fence_pass else "FAIL", "" if fence_pass else "Missing markdown fences."))
            table_pass = qa_content.count("```markdown") >= 2
            output_definitions.append(("", "Uses Pre-formatted Tables (Code Blocks)", "PASS" if table_pass else "WARN", "" if table_pass else "Could not verify presence of pre-formatted tables."))
            # Updated Headers for renumbered QA Report Structure
            section_headers = ["1. SIGNAL QUALITY", "2. HOP-0 RAG SIGNAL FLOW MAP", "3. HOP-BY-HOP", "14. JD ENFORCEMENT", "15. FINAL OUTPUT ARTIFACTS"]
            missing_qa_headers = [h for h in section_headers if h not in qa_content]
            structure_pass = not missing_qa_headers
            output_definitions.append(("", "Consistent Section Structure", "PASS" if structure_pass else "FAIL", "" if structure_pass else f"Missing headers: {missing_qa_headers}"))
            ...
            output_definitions.append(("", "**Section 1: Uses ASCII Bar Chart**", "PASS" if sec1_bar_pass else "FAIL", "" if sec1_bar_pass else "Missing ASCII bar chart."))
            sec1_details_pass = '(Tgt:' in sec1_content and 'Signal' in sec1_content and ('(T:' in sec1_content or '(Avg T:' in sec1_content)
            output_definitions.append(("", "**Section 1: Includes Targets/Status/Temp**", "PASS" if sec1_details_pass else "FAIL", "" if sec1_details_pass else "Missing Target/Status/Temp indicators."))
            sec1_avg_pass = 'Total Weighted Score' in sec1_content and '(Avg T:' in sec1_content
            output_definitions.append(("", "**Section 1: Includes Weighted Average**", "PASS" if sec1_avg_pass else "FAIL", "" if sec1_avg_pass else "Missing Weighted Average or Avg Temp."))
            sec1_fence_pass = bool(sec1_content_match)
            output_definitions.append(("", "**Section 1: Uses Code Fence**", "PASS" if sec1_fence_pass else "FAIL", "" if sec1_fence_pass else "Section 1 not within ```markdown fence."))

        # App Tracker Checks
        app_content = file_contents.get('app_tracker', '')
        output_definitions.append(("**App Tracker**", "**JSON Block (Unfenced)**", "PASS" if app_content else "FAIL", "" if app_content else "Content missing."))
        if app_content:
            is_fenced = app_content.strip().startswith("```json") and app_content.strip().endswith("```")
            fence_check_pass = not is_fenced
            if not fence_check_pass:
                 output_definitions[-1] = ("**App Tracker**", "**JSON Block (Unfenced)**", "FAIL", "App Tracker incorrectly includes ```json fences.")
            ...
            output_definitions.append(("", "Valid JSON Structure", "PASS" if json_valid else "FAIL", "" if json_valid else json_fail_reason))
            ...
            if json_valid and isinstance(parsed_json_data, dict):
                ...
                schema_fail_reason = ""
                if missing_keys: schema_fail_reason += f"Missing keys: {missing_keys}. "
                if extra_keys: schema_fail_reason += f"Extra keys: {extra_keys}."
                if not schema_fail_reason: schema_fail_reason = "Keys match schema."
            output_definitions.append(("", "Adheres to `APP_TRACKER_SCHEMA_V4`", "PASS" if schema_pass else "FAIL", schema_fail_reason.strip()))

        # --- Final Formatting ---
        final_rows = [[item[0], item[1], item[2], item[3]] for item in output_definitions]
        lines.extend(self._format_plain_text_table(headers, final_rows, alignments=['L', 'L', 'L', 'L']))
        lines.append("```")

        return lines

    # --- START REPLACEMENT: Renumbered QA_REPORT_SECTIONS Config ---
    # Configuration for the QA report structure with correct numbering.
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_3_hop_summary", "args": []}, # Was 3
        {"method": "_build_qa_section_4_distribution", "args": ["validation_results"]}, # Was 4
        {"method": "_build_qa_section_5_word_count", "args": ["validation_results"]}, # Was 5
        {"method": "_build_qa_section_6_provenance", "args": ["staging_buffer"]}, # Was 6
        {"method": "_build_qa_section_7_authenticity", "args": ["validation_results"]}, # Was 7
        {"method": "_build_qa_section_8_overview_similarity", "args": []}, # Was 8
        {"method": "_build_qa_section_9_pairwise_similarity", "args": []}, # Was 9
        {"method": "_build_qa_section_10_pipeline_health", "args": []}, # Was 11
        {"method": "_build_qa_section_11_structural", "args": ["validation_results"]}, # Was 12
        {"method": "_build_qa_section_12_prod_readiness", "args": ["validation_results"]}, # Was 13
        {"method": "_build_qa_section_13_cover_letter", "args": ["validation_results"]}, # Was 14
        {"method": "_build_qa_section_14_jd_enforcement", "args": []}, # Was 15
        {"method": "_build_qa_section_15_final_format", "args": ["validation_results", "file_contents"]}, # Was 16
    ]
    # --- END REPLACEMENT: Renumbered QA_REPORT_SECTIONS Config ---
    ...
    def _generate_qa_report(
        ...
        v11.0.1 Refactor: Uses renumbered QA_REPORT_SECTIONS config.
        """
        ...
        for section_config in self.QA_REPORT_SECTIONS: # Uses updated config
            ...
            try:
                ...
                for name in arg_names:
                     # Ensure the *most current* version of file_contents is passed, esp. to section 15
                ...
                # Perform the pre-formatting check (excluding section 15 itself which contains the content)
                if method_name != "_build_qa_section_15_final_format":
                    section_text_block = "\n".join(section_lines)
    ...
    # (These _format_ methods remain unchanged from v10.14)
    ...
    def _format_ascii_bar_chart(self, label: str, value: float, target_min: float, target_max: float, temperature: float = 0.0, bar_length: int = 10, is_summary: bool = False) -> str:
        ...
        target_str = f"(Tgt: {target_min:.0%}-{target_max:.0%})" if not is_summary else f"(Tgt: {target_min:.0%})" # Simpler target for summary
    ...
    def _invoke_deduplication_analysis(self, staging_buffer: ImmutableStagingBuffer) -> bool:
        """
        Gathers data and calls DuplicateDetector methods for QA sections 8, 9, and 10.
        ...
        # --- Execute Analyses ---
        try:
            # 1. Pairwise Matrix (for QA Section 9)
        ...
        try:
            # 2. Overview vs. Bullet Similarity (for QA Section 8)
