...
class TextSanitizer:
    ...
    def _sanitize_text(self, text: str) -> str:
        ...
        # 0. First apply general text sanitization from TextUtils
...
class TextUtils:
    ...
    def format_validation_message(validation_result, context_cache: dict = None) -> str:
        ...
        """
        try:
            # Get details from result or use empty dict
            details = getattr(validation_result, 'details', {}) or {}
            
            # Merge with context cache if provided
            if context_cache:
                details = {**context_cache.get(validation_result.rule_id, {}), **details}
            
            # Get the message template/callable
...
class QAReportGenerator:
    ...
    def _build_qa_section_8_similarity_summary(
        ...
        overall_notes = "Similarity validation gate passed." if overall_pass else "Similarity validation gate failed."
        rows.append([
        ...
        # Simplified reference to production readiness section
        prod_ready = overall_pass and not any(vr for vr in validation_results if not vr.passed and vr.severity in [ValidationSeverity.CRITICAL, ValidationSeverity.HIGH])
        if not prod_ready:
            lines.append("")
    ...
    def _build_qa_section_10_jd_enforcement(self) -> List[str]:
        ...
        # For this version of the code, enforcement results would be in the checkpoints
        ...
        # Check HOP-2 checkpoint for GATE-3
        ...
        # Check HOP-3 checkpoint for GATE-4
        ...
        # Check HOP-5 checkpoint for GATE-5
    ...
    def _safe_format_validation_message(self, validation_result: ValidationResult) -> str:
        ...
        """
        # Pass the context cache if available
