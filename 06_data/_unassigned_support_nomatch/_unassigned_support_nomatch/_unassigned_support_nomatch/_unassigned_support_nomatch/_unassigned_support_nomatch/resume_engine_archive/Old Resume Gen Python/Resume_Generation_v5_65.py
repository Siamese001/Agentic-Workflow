"""
Resume Generation Engine v5.64 - COMPREHENSIVE QA FIXES

v5.64 DUAL QA HARDENING:
...
class WorkflowOrchestrator:
    ...
    def _generate_qa_report(
        ...
        section_config = {
            "K.1": {"name": "Exec Summary", "weight": 0.20},
            "K.4": {"name": "Headline", "weight": 0.10},
            "K.5B": {"name": "Unify Intro", "weight": 0.05},
            "K.5A": {"name": "Unify Bullets", "weight": 0.20},
            "K.6B": {"name": "IBM Intro", "weight": 0.05},
            "K.6A": {"name": "IBM Bullets", "weight": 0.15},
            "K.7A": {"name": "EY Bullets", "weight": 0.05},
            "K.10A": {"name": "Early Career", "weight": 0.05},
            "K.8": {"name": "Competencies", "weight": 0.15},
        }
        ...
        for key, config in section_config.items():
            ...
            bar_line = (
                ...
                f"Act: {int(actual_score*100):>3} "
                f"[{ascii_bar(actual_score, 30)}]"
            )
            section_bar_lines.append(bar_line)

        weighted_avg_score = total_weighted_score / total_weight if total_weight > 0 else 0.0

        report_lines.append(f"Overall Signal Score: {weighted_avg_score:.2f}")
        report_lines.append(f"Signal Bar: {ascii_bar(weighted_avg_score, 50)} ({int(weighted_avg_score*100)}%)")
        report_lines.append("")
