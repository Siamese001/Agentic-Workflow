...
__version__ = "14.07" # <-- Increment version for tracking changes
try:
...
"""
    ...
    def _build_context_k1_summary(self, spec: Dict) -> Dict:
        ...
        # --- FIX: Define EXEC_SUMMARY_WORD_COUNT_MIN/MAX in ContentConstraintsConfig ---
        # Assuming they exist based on the v14.05 definition structure (add if missing)
        min_exec_wc = getattr(self.constraints, 'EXEC_SUMMARY_WORD_COUNT_MIN', 100) # Example fallback
        max_exec_wc = getattr(self.constraints, 'EXEC_SUMMARY_WORD_COUNT_MAX', 150) # Example fallback
...
def count_words_ms_word_style(text: str) -> int:
    ...
    if not text:
        return 0
    # Finds sequences of word characters (\w includes letters, numbers, underscore), hyphens, and apostrophes.
    # This is a robust way to count words, treating "state-of-the-art" and "it's" as single words.
...
class ValidationContext:
    ...
    def _calculate_k1_word_count_details(self):
        ...
        # This rule is not currently in RULES_CONFIG, but the logic is here if needed.
        details = self._calculate_metric_details(
            section_enum=ResumeSection.K1_EXECUTIVE_SUMMARY,
            metrics_to_calc=[('word_count', count_words_ms_word_style)],
            constraints={} # No min/max defined in config for this
        )
...
class PreFlightValidator:
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        rule_map = {
            ...
            # "VG_BULLET_WORD_COUNT_TOLERANCE": "COMPLEX_PER_SECTION", # This rule is removed
    ...
    def _register_rules(self):
        ...
        # --- START: Refactored Rule Registration ---
        ...
        # --- END: Refactored Rule Registration ---
    ...
    def _validate_section_presence(self, context: ValidationContext, section_enum: ResumeSection) -> bool:
        ...
        if content is None:
            return False
        if isinstance(content, str):
            return content.strip() not in ["", "HEADER_PLACEHOLDER"]
...
class FileRenderer:
    ...
    def _render_resume_markdown(self, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        This method is now fully data-driven, using a dispatch table.
        """
        output_lines = []

        for config in self.RESUME_RENDER_CONFIG:
            render_type = config["type"]
            handler = self._RENDER_DISPATCH.get(render_type)

            if handler:
                # Add newline before headers unless it's the first line
                if render_type == "header" and output_lines:
                    output_lines.append("") # Add a blank line for spacing
                
                rendered_part = handler(config, staging_buffer)
                if rendered_part:
                    output_lines.append(rendered_part)
            else:
                self.logger.warning(f"No render handler found for type '{render_type}'. Skipping.")

        # Join lines ensuring proper spacing and single trailing newline
        return "\n".join(output_lines).strip() + "\n"

    # --- START: New Data-Driven Handler Methods ---
    ...
    def _handle_render_list(self, config: Dict, staging_buffer: ImmutableStagingBuffer) -> str:
        ...
        if content:
            render_method = getattr(self, config["render_method"])
            return render_method(content, item_prefix=config.get("item_prefix", ""))
        return ""
    # --- END: New Data-Driven Handler Methods ---
    ...
    def _render_list_section(self, content_list: List[Union[str, Dict]], item_prefix: str = "") -> str:
        """Generic method to render a list of items, which can be strings or dicts."""
        lines = []
        if not isinstance(content_list, list):
            self.logger.warning(f"Expected a list for _render_list_section, but got {type(content_list)}. Skipping.")
            return ""

        for item in content_list:
            text_to_render = ""
            if isinstance(item, str):
                text_to_render = item.strip()
            elif isinstance(item, dict):
                # Special handling for education format
                if 'degree' in item or 'institution' in item:
                    ...
                    text_to_render = ", ".join(line_parts)
                    notes = item.get('notes', '').strip()
                    if notes: text_to_render += f" ({notes})"
                else: # Generic dict handling
                    text_to_render = item.get('text', str(item)).strip()
    ...
    def _render_skills(self, staging_buffer: ImmutableStagingBuffer, job_description: Optional[str] = None) -> str:
        ...
        for skill in skills_list:
            if isinstance(skill, str):
                cleaned = skill.strip()
                wc = count_words_ms_word_style(cleaned) # Use the consistent, centralized counter
...
# Assuming these are defined elsewhere in the file or imported:
# ResumeSection, ImmutableStagingBuffer, ThematicAnalysis, ValidationResult,
# ValidationSeverity, __version__, WorkflowOrchestrator, calculate_signal_score,
# HopStatus, HopCheckpoint, GateDecision, HopExecutionError, StagingBufferError,
# EnhancedJobDescriptionAnalyzer, ClerkExtractor, DataEnricher, ArtistGenerator,
# PreFlightValidator, GateDecisionEngine, FileRenderer, AppTrackerQAValidator,
# MASTER_RESUME_DATA, APP_TRACKER_SCHEMA_DATA, count_words_ms_word_style, BulletProvenance
...
class QAReportGenerator:
    ...
    # --- START NEW: Generic Table Builder ---
    ...
    # --- Unchanged Section Builders (1, 2, 3, 4, 5, 7, 8, 9, 12, 13) ---
    def _build_qa_section_1_signal_quality(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis, validation_results: List[ValidationResult]) -> List[str]:
        ...
        lines.append("-" * 80)
        lines.append(self._format_ascii_bar_chart("Weighted Average", avg_score, 0.70, 0.90, avg_temp_val, is_summary=True))
        lines.append("-" * 80)
        ...
        if signal_fail_rule and not signal_fail_rule.passed:
             ...
             try:
                 ...
                 fail_msg = msg_template.format_map(defaultdict(lambda: '[N/A]', **details)) if not callable(signal_fail_rule.message) else signal_fail_rule.message(defaultdict(lambda: '[N/A]', **details))

                 # fail_msg = signal_fail_rule.message(defaultdict(lambda: '[N/A]', **details)) if callable(signal_fail_rule.message) else str(signal_fail_rule.message)
             except Exception as e:
    ...
    def _build_qa_section_3_hop_summary(self) -> List[str]:
        ...
        if len(rows) != expected_rows:
             # Insert error message before formatting the table if rows are missing
             # Correctly insert before the table formatting call
             lines.insert(len(lines) -1, f"ERROR   TRUNCATION_DETECTED   FAIL   Expected {expected_rows} hop rows, got {len(rows)}") # Should insert before ``` ideally, but here is okay too.
        lines.extend(self._format_plain_text_table(headers, rows))
        lines.append("```")
        return lines
    ...
    def _build_qa_section_7_prod_readiness(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        if not prod_ready:
            ...
            if critical_failures:
                ...
                for f in critical_failures[:3]: # Show top 3
                     ...
                     try:
                         ...
                         # msg = f.message(defaultdict(lambda: '[N/A]', **details)) if callable(f.message) else str(f.message)
            ...
            if high_failures:
                ...
                for f in high_failures[:3]: # Show top 3
                     details = f.details or {}
                     msg = "[Error formatting msg]"
                     try:
                         # Ensure message template is treated as a string before formatting attempt
    ...
    # --- Formatting Helpers (Moved from Orchestrator) ---
    ...
    def _format_ascii_bar_chart(self, label: str, value: float, target_min: float, target_max: float, temperature: Optional[Union[float, str]] = None, bar_length: int = 10, is_summary: bool = False) -> str:
        ...
        value = min(max(value, 0.0), 1.0) # Clamp value between 0 and 1
        ...
        if isinstance(temperature, (float, int)):
             ...
             temp_str = f"({temperature})" # Use 'N/A' or other string directly
        ...
        # Calculate padding dynamically based on total width (e.g., 80)
    ...
    # --- START Moved Helper: _get_text_from_item ---
    def _get_text_from_item(self, item: Any) -> str:
        """Helper to safely extract text from a string or a bullet dictionary."""
        if isinstance(item, dict):
    ...
    # --- START Moved Helper: _check_word_count ---
    def _check_word_count(self, count: int, min_target: int, max_target: int) -> Tuple[str, str]:
        """Checks word count against range, returns status and target string."""
        status = "PASS" if min_target <= count <= max_target else "FAIL"
        target_range_str = f"{min_target}-{max_target}"
...
Excellent written and verbal communication skills, including interacting with and presenting to senior leadership, externally and internally.
    ...
    # --- FIX: Use MASTER_RESUME_DATA instead of MASTER_RESUME_JSON ---
    ...
    if result.get('status') == 'SUCCESS':
        ...
        if file_contents_dict:
            file_paths_dict = result.get('file_paths', {}) # Get file paths dict once
            if file_paths_dict:
