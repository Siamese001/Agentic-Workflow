...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_16_final_format(
        ...
        Shows a summary table with detailed formatting checks and the full content of each generated output.
        """
        ...
        # Output 1: Resume (Raw)
        lines.append("#### Output 1: Resume (Raw Text)\n")
        lines.append(file_contents.get('resume_md', "*Resume content not generated.*"))
        lines.append("\n---") # Separator

        # Output 2: Skills (Raw)
        lines.append("#### Output 2: Skills (Raw Text)\n")
        lines.append(file_contents.get('skills', "*Skills content not generated.*"))
        lines.append("\n---") # Separator

        # Output 3: Cover Letter (Raw)
        lines.append("#### Output 3: Cover Letter (Raw Text)\n")
        lines.append(file_contents.get('cover_letter', "*Cover Letter content not generated.*"))
        lines.append("\n---") # Separator

        # Output 4: QA Report (Fenced Markdown) - Swapped
        lines.append("#### Output 4: QA Report (Fenced Markdown)\n")
        lines.append("```markdown")
        qa_content_actual = file_contents.get('qa_report', "*QA Report content not available.*")
        lines.append(qa_content_actual if qa_content_actual else "")
        lines.append("```")
        lines.append("\n---") # Separator

        # Output 5: App Tracker (JSON Block - Unfenced) - Swapped
        lines.append("#### Output 5: App Tracker (JSON Block - Unfenced)\n")
        app_tracker_content = file_contents.get('app_tracker', '{}')
        try:
            # Attempt to parse and re-stringify for consistent indentation
            # v10.1: Handle potential fences before parsing/dumping
            content_to_dump = app_tracker_content.strip()
            if content_to_dump.startswith("```json") and content_to_dump.endswith("```"):
                 content_to_dump = content_to_dump[7:-3].strip()
            parsed_json = json.loads(content_to_dump)
            # IMPORTANT: Render without fences
            lines.append(json.dumps(parsed_json, indent=2))
        except json.JSONDecodeError:
            # If it's not valid JSON, append as is (without fences)
            lines.append(app_tracker_content if app_tracker_content else "{}")
