...
class DataEnricher:
    ...
    def _canonicalize_verbs(self, text: str) -> List[str]:
        """[NEW] Extract and canonicalize verbs from text."""
        canonical = []
        text_lower = text.lower()
    ...
    def _check_for_forbidden_verbs(self, text: str) -> List[str]:
        """[NEW] Check for forbidden verbs in the text."""
        found_verbs = []
        text_lower = text.lower()
...
"""
    ...
    def _generate_k5_unify_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        return self._generate_tailored_bullets_for_experience(
            ...
            section_id=ResumeSection.K5_UNIFY_BULLETS, # Pass Enum
            temperature_override=temperature_override
        )
    ...
    def _generate_k6_ibm_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        return self._generate_tailored_bullets_for_experience(
            ...
            section_id=ResumeSection.K6_IBM_BULLETS, # Pass Enum
            temperature_override=temperature_override
        )
    ...
    def _generate_k10_competencies(self, temperature_override: Optional[float] = None) -> List[Dict]:
        ...
        try:
            ...
            generated_bullets = self._generate_tailored_bullets_for_experience(
                ...
                section_id=ResumeSection.K10_COMPETENCIES, # Pass Enum
                temperature_override=temperature_override
            )
        finally:
    ...
    def _generate_k8_ey_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.8 EY Bullets (Customized only), validates against hardcoded range."""
        ...
        customized_bullets = self._generate_lightly_customized_bullets(
            ...
            section_id=ResumeSection.K8_EY_BULLETS.value, # Use string value for logging in helper
            thematic_analysis=self.thematic_analysis,
            temperature_override=temperature_override
        )

        # Validate word counts using the hardcoded range
        target_range = self.BULLET_WORD_COUNT_RANGES[ResumeSection.K8_EY_BULLETS]
        min_target, max_target = target_range
        ...
        return self._validate_and_potentially_rewrite_bullets(
             ...
             section_id_for_logging=ResumeSection.K8_EY_BULLETS.name, # Use Enum name
             temperature_override=temperature_override
        )
    ...
    def _generate_k9_early_career_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.9 Early Career Bullets (Customized only), validates against hardcoded range."""
        ...
        customized_bullets = self._generate_lightly_customized_bullets(
            source_bullets_text=bullets_to_customize,
            section_id=ResumeSection.K9_EARLY_CAREER_BULLETS.value, # Use string value for logging in helper
            thematic_analysis=self.thematic_analysis,
            temperature_override=temperature_override
        )

        # Validate word counts using the hardcoded range
        target_range = self.BULLET_WORD_COUNT_RANGES[ResumeSection.K9_EARLY_CAREER_BULLETS]
        min_target, max_target = target_range
        ...
        return self._validate_and_potentially_rewrite_bullets(
             ...
             min_target=min_target,
             max_target=max_target,
             section_id_for_logging=ResumeSection.K9_EARLY_CAREER_BULLETS.name, # Use Enum name
             temperature_override=temperature_override
        )
...
class PreFlightValidator:
    ...
    RULES_CONFIG = [
        ...
        # --- START CHANGE: Updated Rule Config for Bullet Word Count ---
        {
            "rule_id": "VG_BULLET_WORD_COUNT_RANGE", # Renamed rule ID for clarity
            "severity": ValidationSeverity.HIGH,
            "category": "word_count",
            "validator": "_validate_bullet_word_count_range", # Renamed validator method
            "error_message": "Bullet word counts outside hardcoded ranges: {violations}" # Updated message
        },
        # --- END CHANGE ---
        # --- Distribution Rules ---
        {
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        rule_map = {
            ...
            "VG_BULLET_WORD_COUNT_RANGE": "COMPLEX_PER_SECTION", # Renamed rule ID
    ...
    def _register_rules(self):
        ...
        for config in self.RULES_CONFIG:
            ...
            def create_error_message_lambda(template, rule_id):
                format_args = {}
                # --- START CHANGE: Remove tolerance from lambda ---
                # No longer need tolerance in the bullet word count message
    ...
    def validate(
        ...
        # --- START CHANGE: Removed calculation of section_averages ---
        # section_averages = calculate_master_avg_bullet_length(self.master_resume) # REMOVED
        # --- END CHANGE ---

        data = self._prepare_validation_data(
            staging_buffer, thematic_analysis, job_description, actual_counts #, section_averages # REMOVED
        )
    ...
    def _prepare_validation_data(
        ...
        return {
            ...
            # "section_averages": section_averages, # REMOVED
        }
    # --- END MODIFIED METHOD ---
    ...
    # --- START MODIFIED METHOD ---
    def _validate_bullet_word_count_range(self, data: Dict) -> bool: # Renamed method
        """
        [v12.90 REWRITE for Hardcoded Ranges]
        ...
        # --- Use Hardcoded Ranges (same as ArtistGenerator) ---
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            # Determine the correct key to look up the hardcoded range
            range_key = "Default" # Fallback key
            if section_enum == ResumeSection.K5_UNIFY_BULLETS: range_key = "Unify"
            elif section_enum == ResumeSection.K6_IBM_BULLETS: range_key = "IBM"
            elif section_enum == ResumeSection.K10_COMPETENCIES: range_key = "Competencies"
            elif section_enum == ResumeSection.K8_EY_BULLETS: range_key = "EY"
            elif section_enum == ResumeSection.K9_EARLY_CAREER_BULLETS: range_key = "EarlyCareer"

            # Get the hardcoded min/max targets
            min_wc, max_wc = HARDCODED_BULLET_RANGES.get(range_key)
            ...
            for i, bullet in enumerate(bullets):
                ...
                if not (min_wc <= actual_wc <= max_wc):
                    ...
                    # Update violation message to reflect hardcoded range
...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_4_word_count_distribution(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        # Find the overall COVER_LETTER_STRUCTURE result
        ...
        for rule_id, section_label in rules_to_include.items():
            ...
            # Handle Cover Letter Paragraphs specially
            if rule_id.startswith("CL_P"):
                ...
                if cl_structure_result: # If the main structure rule exists
                    # Get details from the structure rule's details dictionary
                    ...
                    # Determine status based on the overall structure rule pass/fail
                    # and check if individual count is within range if details are present
                    status = "N/A"
                    msg = ""
                    if not cl_structure_result.passed:
                        status = "FAIL"
                        msg = cl_structure_result.message # Use the main rule's message
            ...
            # Handle other rules
                ...
                if 'min' in details and 'max' in details:
                    ...
                    if '%' in value_str: target_str += "%"
                         ...
                         # Handle float ratios if applicable
                ...
                # If the validation result for the rule_id wasn't found at all
    ...
    def _build_qa_section_5_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        # --- Use Hardcoded Ranges (same as ArtistGenerator and PreFlightValidator) ---
        HARDCODED_BULLET_RANGES = {
            "Unify": (25, 38),
            "IBM": (22, 34),
            "EY": (25, 40),
        ...
        # Define sections to check (matches PreFlightValidator.BULLET_WORD_COUNT_SECTIONS_TO_CHECK)
        sections_to_process = [
            ...
            # Add TraderSense for completeness, though it uses Verbatim only
        ...
        for section_data in sections_to_process:
            ...
            if bullets_enum:
                # --- Get the correct hardcoded range ---
                ...
                if isinstance(bullets, list) and bullets:
                    for i, bullet_item in enumerate(bullets):
                        ...
                        elif isinstance(bullet_item, str): # Handle simple lists (like K.7)
                            bullet_text = bullet_item
                            word_count = count_words_ms_word_style(bullet_text)
                            # Assume Verbatim if it's just a string from K.7
    ...
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_7_exec_summary_similarity", "args": ["validation_results"]}, # Changed args
...
# --- START OVERWRITE: `if __name__ == '__main__':` block (Cache Clearing) ---
if __name__ == '__main__':
    ...
    try:
        # Instantiate RAGConfig to access directory paths
        ...
        if cache_dir_to_clear and os.path.exists(cache_dir_to_clear):
            ...
            if not os.path.exists(cache_dir_to_clear):
                logger.info(f"  ✓ RAG cache directory cleared successfully.")
            else:
                logger.warning(f"  ⚠️ Could not completely remove RAG cache directory (might be in use or permissions issue).")
        else:
            logger.info(f"RAG cache directory ({cache_dir_to_clear}) not found or not specified. Skipping clear.")

        # Clear Telemetry Log Directory
        if telemetry_dir_to_clear and os.path.exists(telemetry_dir_to_clear):
            ...
            if not os.path.exists(telemetry_dir_to_clear):
                logger.info(f"  ✓ Telemetry log directory cleared successfully.")
            else:
                logger.warning(f"  ⚠️ Could not completely remove Telemetry log directory (might be in use or permissions issue).")
        else:
            logger.info(f"Telemetry log directory ({telemetry_dir_to_clear}) not found or not specified. Skipping clear.")
...
Datadog values people from all walks of life. We understand not everyone will meet all the above qualifications on day one. That's okay. If you’re passionate about technology and want to grow your skills, we encourage you to apply.
    ...
    # Logging is now configured within the orchestrator's __init__
    ...
    if result.get('status') == 'SUCCESS':
        ...
        # --- START FIX: Ensure file_contents exists before iterating ---
        file_contents_dict = result.get('file_contents', {})
        if file_contents_dict: # Check if the dictionary exists and is not empty
            for file_type, content in file_contents_dict.items():
                # --- START FIX: Ensure file_paths exists before accessing ---
                file_paths_dict = result.get('file_paths', {})
                if file_paths_dict: # Check if the dictionary exists
                    file_name = file_paths_dict.get(file_type)
                    if file_name and content is not None: # Check content is not None
                        try:
                             ...
                             logger.error(f"Error writing file {file_name} - invalid content type ({type(content)}): {e}")

                    elif not file_name:
                         logger.warning(f"File path not found for type '{file_type}' in results.")
                    # Implicitly handles content being None by the 'if file_name and content is not None' check
                else:
                     logger.warning("File paths dictionary ('file_paths') not found in results. Cannot save files.")
                     break # Exit loop if file_paths is missing
    ...
    print("\n--- Workflow Finished ---")
# --- END OVERWRITE: `if __name__ == '__main__':` block (Cache Clearing) ---