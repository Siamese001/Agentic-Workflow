...
"""
    ...
    def _analyze_with_resilient_web_search(
        ...
        """
        v11.40 Final Fix: Added recursive conversion before caching & fixed indentation.
        """
        import logging
        logger = logging.getLogger(__name__)

        # (Telemetry init, cache check, web_rag check remain the same)
        ...
        if not self.web_rag or not self.rag_mission:
             # This check should ideally happen in the calling 'analyze' method
             # If reached here, it implies RAG was intended but failed init or pre-analysis
             logger.warning("Web RAG or RAG Mission not available. Falling back to local NLP.")
             return self._analyze_local_nlp(job_description) # Fallback if checks fail


        # Initialize RAG result tracker
        ...
        try:
            ...
            # Safely access nested keys for search calls made
            ...
            if telemetry:
                ...
                telemetry.phase1_attempts = 1 # Simplified, executor tracks actual
                telemetry.total_search_calls += searches_performed_p1
            logger.info(f"Phase 1: SUCCESS ({self.search_calls_made} searches so far)")
        except Exception as e:
            logger.warning(f"Phase 1: FAILED - {e}", exc_info=False) # Keep exc_info False unless debugging phase logic
        ...
        if partial_result.full_success:
            logger.info("✓ Strategy 1: Full 4-phase RAG successful") # Updated comment
            # Call synthesis function which now includes robust conversion internally
            analysis = self._synthesize_thematic_analysis(
                ...
                partial_result.phase4_result, # Pass phase 4 result
                job_description
            )
            if telemetry:
        ...
        elif partial_result.any_success: # Should not be reached with current fail-fast logic
            logger.error(f"✗ RAG analysis was only partially successful ({partial_result.success_rate:.0%}). Halting workflow (Logic Error).")
            # Fail fast if partial success occurs
            raise HopExecutionError("RAG analysis failed to achieve 100% success across all four phases.")

        else: # All phases failed
        ...
        def _force_plain_types_for_cache(item: Any) -> Any:
            """Recursively converts mappings/sequences/objects into plain dicts/lists for caching."""
            if isinstance(item, dict):
                return {k: _force_plain_types_for_cache(v) for k, v in item.items()}
            elif isinstance(item, Mapping):
                return {k: _force_plain_types_for_cache(v) for k, v in item.items()}
            elif isinstance(item, Sequence) and not isinstance(item, (str, bytes)):
                return [_force_plain_types_for_cache(elem) for elem in item]
            elif is_dataclass(item) and not isinstance(item, type):
                try:
                    dict_data = asdict(item)
                    return {k: _force_plain_types_for_cache(v) for k, v in dict_data.items()}
                except Exception: return item # Fallback
            elif hasattr(item, 'items') and callable(getattr(item, 'items')) and not isinstance(item, type):
                 try: return {k: _force_plain_types_for_cache(v) for k, v in item.items()}
                 except Exception: return item # Fallback
            else: return item

        try:
            # Convert the final 'analysis' object before trying to cache
            analysis_dict_for_cache = _force_plain_types_for_cache(analysis)
            # Ensure the top level is a dict
            if not isinstance(analysis_dict_for_cache, dict):
                 logger.warning("Top-level conversion failed, trying asdict before cache.")
                 analysis_dict_for_cache = asdict(analysis) # This might still fail if nested mappingproxy exists

            # Cache the fully converted dictionary
            if self.cache_manager and analysis_dict_for_cache and isinstance(analysis_dict_for_cache, dict): # Add isinstance check
                ...
                logger.debug("Analysis result cached successfully.")
                 ...
                 logger.warning(f"Skipping cache: Conversion to plain dict failed or result is not a dict (Type: {type(analysis_dict_for_cache)}).")

        except Exception as cache_e:
             # Log the specific error during conversion/caching but don't halt workflow
        ...
        if telemetry and self.telemetry_logger:
            ...
            telemetry.total_api_calls = self.search_calls_made # Approximate total calls triggered
            self.telemetry_logger.log(telemetry)

        logger.info(f"Analysis complete. Total searches: {self.search_calls_made}")
        return analysis # Return the original 'analysis' object (ThematicAnalysis instance)
...
# Inside class EnhancedJobDescriptionAnalyzer:
    def _synthesize_thematic_analysis(
        ...
        v11.40 Final Fix: Manually reconstruct dicts from RAG results.
        """
        import logging
        logger = logging.getLogger(__name__)
    ...
    def _analyze_local_nlp(self, job_description: str) -> 'ThematicAnalysis':
        ...
        # --- START FIX: Ensure authenticity_patterns['patterns'] is a dictionary ---
        local_auth_patterns = {
            "status": "FALLBACK",
    ...
    # [Rest of the methods within EnhancedJobDescriptionAnalyzer remain unchanged]
    # ... (Keep the existing _extract_keywords, _calculate_theme_scores, etc. methods below this)
...
"""
                ...
                if "[Placeholder" not in response_select:
                    ...
                    final_bullets.extend(verbatim_bullets) # Extend the list initialized earlier
                else:
                    logging.warning(f"{section_id} Verbatim selection failed. Using first {verbatim_count} master bullets.")
                    # Ensure structure consistency when padding
        ...
        if customized_count > 0:
            used_texts = {b.get('bullet_text', b.get('text')) for b in final_bullets} # Check both keys
            ...
            if candidates_for_custom:
                ...
                final_bullets.extend(customized_bullets) # These have 'text' key
            else:
                 logging.warning(f"{section_id} Not enough unique bullets to customize.")

        # --- 5. Generate Synthetic Bullets ---
        if synthetic_count > 0:
            # Safely get text from bullets which might have 'text' or 'bullet_text'
            ...
            final_bullets.extend(synthetic_bullets) # These have 'text' key
        ...
        if len(final_bullets) < total_expected_count:
            ...
            used_texts = {b.get('text', b.get('bullet_text')) for b in final_bullets} # Check both keys
            padding_candidates = [b for b in master_bullets_structured if isinstance(b, dict) and b.get('bullet_text') not in used_texts]
            # Ensure structure consistency when padding
            padding = [{'bullet_text': b['bullet_text'], **b} for b in padding_candidates[:needed]]
            final_bullets.extend(padding)
             ...
             # Prioritize non-Verbatim during trimming
        ...
        # --- START FIX: Call the global function correctly ---
        # The global function is defined later in the file and takes master_resume
        master_avg_lengths = calculate_master_avg_bullet_length(self.master_resume)
        # --- END FIX ---
        section_name_for_avg = "Unify" # Default key
        # Map company name/section ID to the keys used in master_avg_lengths
        ...
        # Competencies handled separately
        ...
        final_bullets = self._validate_and_potentially_rewrite_bullets(
            ...
            section_name_for_avg=section_name_for_avg, # Pass the mapped key
            section_id_for_logging=section_id
        )

        # --- 8. Final Reordering (Now with Secondary Themes) ---
        # Safely get text from bullets for reordering prompt
...
"""
        ...
        # Create map using both possible text keys
        generated_map = {b.get('text', b.get('bullet_text')): b for b in final_bullets}
        ...
        for text in reordered_texts:
            if text in generated_map and text not in used_from_generated:
                ...
                # Try a slightly fuzzy match in case of minor LLM rephrasing during reorder
                matched = False
                for gen_text, gen_bullet in generated_map.items():
                     # Initialize self.dup_detector if it hasn't been
                     if not hasattr(self, 'dup_detector') or self.dup_detector is None:
                         # Need access to orchestrator or create new instance
                         # For now, let's create a temporary one if needed
                         # This assumes DuplicateDetector is defined globally or imported
                         temp_detector = DuplicateDetector()
                         similarity = temp_detector._calculate_cosine_similarity(text, gen_text)
                     else:
                         similarity = self.dup_detector._calculate_cosine_similarity(text, gen_text)

                     if gen_text not in used_from_generated and similarity > 0.95:
                          final_ordered_bullets.append(gen_bullet)
                          used_from_generated.add(gen_text)
                          matched = True
                          logging.warning(f"{section_id} Reordering returned slightly modified text. Matched via similarity.")
                          break
                if not matched:
...
# Inside class ArtistGenerator:
    def _generate_k10_competencies(self) -> List[Dict]:
        ...
        all_competencies_structured = [{
            # Use 'bullet_text' key for consistency with other bullet processing
            "bullet_text": re.sub(r'^[•*]\s*', '', c).strip(), # Remove potential leading bullets
            "provenance": BulletProvenance.Verbatim.value,
            "word_count": count_words_ms_word_style(c)
        } for c in all_competencies_text if isinstance(c, str)] # Ensure items are strings
        ...
        final_competencies = [] # Initialize here
...
"""
                ...
                if "[Placeholder" not in response_select:
                    ...
                    final_competencies.extend(verbatim_comps) # Extend the list initialized earlier
                else:
        ...
        if customized_count > 0:
            used_texts = {b.get('bullet_text', b.get('text')) for b in final_competencies} # Check both keys
            ...
            if candidates_for_custom:
                ...
                final_competencies.extend(customized_comps) # These have 'text' key
            else:
        ...
        if synthetic_count > 0:
            ...
            final_competencies.extend(synthetic_comps) # These have 'text' key
        ...
        if len(final_competencies) < total_expected_count:
            ...
            used_texts = {b.get('text', b.get('bullet_text')) for b in final_competencies} # Check both keys
            padding_candidates = [b for b in all_competencies_structured if isinstance(b, dict) and b.get('bullet_text') not in used_texts]
            # Ensure consistent structure
            padding = [{'bullet_text': c['bullet_text'], **c} for c in padding_candidates[:needed]]
        ...
        # Call the global function correctly
        master_avg_lengths = calculate_master_avg_bullet_length(self.master_resume)
        # --- END FIX ---
        final_validated_comps = self._validate_and_potentially_rewrite_bullets(
            selected_bullets_structured=final_competencies,
            master_avg_lengths=master_avg_lengths,
            section_name_for_avg="Competencies", # Use the specific key
            section_id_for_logging="K.10_COMPETENCIES"
        )

        # 9. Final reformatting: Ensure output uses "text" key needed by renderer
        output_list = []
        for item in final_validated_comps:
            text_key = 'text'
            bullet_text_key = 'bullet_text'
            # Prioritize 'text' if it exists, otherwise use 'bullet_text'
            final_text = item.get(text_key, item.get(bullet_text_key, ''))
            # Create a new dict with the 'text' key and other metadata
            output_item = {
                'text': final_text, # Ensure 'text' key exists
                'provenance': item.get('provenance'),
                'word_count': item.get('word_count', count_words_ms_word_style(final_text))
                # Add other keys if necessary, e.g., 'canonical_verbs'
            }
            # Add bullet_text as well if it's different and exists, or just copy text
            if bullet_text_key in item and item[bullet_text_key] != final_text:
                output_item[bullet_text_key] = item[bullet_text_key]
                 ...
                 output_item[bullet_text_key] = final_text # Ensure bullet_text exists for consistency
...
"""
        ...
        # --- START FIX: Use _call_gemini_api ---
        try:
            # Use a default or specific reasoning config if desired
            reasoning_config = ReasoningConfig.DEFAULT # Or define a K13 config
            base_system = f"You are an expert executive ghostwriter. Your writing style is: {auth_tone_example}. You follow all formatting and content instructions perfectly."
            ...
            # Check if generation failed (returned placeholder)
            ...
            # Basic validation: Check if essential parts are present (Date, Signature)
            # More robust validation happens in PreFlightValidator
            if not cover_letter_text.strip().startswith(today) or signature not in cover_letter_text:
                 logging.warning("Generated cover letter missing date or signature. Attempting to fix or using fallback.")
                 # Attempt simple fix: Prepend date, append signature if missing
                 if not cover_letter_text.strip().startswith(today):
                     # Remove potential LLM preamble before adding date
                     cl_body_match = re.search(r"Dear Hiring Manager,", cover_letter_text)
                     if cl_body_match:
                         cl_body = cover_letter_text[cl_body_match.start():]
                         cover_letter_text = f"{today}\n\nHiring Manager\n[Company Name]\n\n{cl_body}"
                     else: # Cannot reliably fix, use fallback
                          return self._generate_fallback_cover_letter()

                 if signature not in cover_letter_text:
                     # Remove potential LLM postamble
                     sincerely_match = cover_letter_text.rfind("Sincerely,")
                     if sincerely_match != -1:
                         cover_letter_text = cover_letter_text[:sincerely_match] + signature
                     else: # Cannot reliably fix, use fallback
                          return self._generate_fallback_cover_letter()


            return cover_letter_text.strip() # Return cleaned text

        except Exception as e:
            # Catch errors from _call_gemini_api or subsequent processing
...
# Insert this class overwrite starting around line 3936 in your file
...
class PreFlightValidator:
    ...
    # --- START FIX: Correct indentation for methods ---
    ...
    # --- Individual Validator Methods (Now correctly indented) ---
    ...
    def _calculate_master_avg_bullet_length(self) -> Dict[str, float]:
        ...
        Helper to calculate average bullet length for sections in master resume using MS Word style counting.
        """
        logging.info("Calculating Master Resume Average Bullet Lengths...") # <<< ADDED LOG
        avg_lengths = {}
        # Corrected structure access based on MASTER_RESUME_JSON
        experience_sections = self.master_resume.get("professional_experience", [])
        experience_sections_to_avg = {
        ...
        for key, (company_name_part, bullets_key) in experience_sections_to_avg.items():
            ...
            logging.info(f"  - Master Avg for '{key}': {calculated_avg:.1f} words ({bullet_count} bullets)") # <<< ADDED LOG
        ...
        logging.info(f"  - Master Avg for 'Competencies': {calculated_avg_comp:.1f} words ({len(competencies_strings)} items)") # <<< ADDED LOG

        # Add a default for any other section needed
        avg_lengths["Default"] = default_avg
        # Log the default average
        logging.info(f"  - Master Avg for 'Default': {default_avg:.1f} words") # <<< ADDED LOG
        return avg_lengths
...
class WorkflowOrchestrator:
    ...
    v11.0.2 Refactor: Merged QA Sections 4 & 5 into new Section 4, removed bullet tolerance row, renumbered 5-15.
    """
...
if __name__ == '__main__':
    ...
    my_company_name = "DataRobot"
    my_job_title = "VP AI Technical Success"
    my_job_description = """
    As the Vice President of AI Technical Success at DataRobot, you will lead a world-class team of customer-facing data scientists and AI engineers.
    Your primary mission is to ensure our customers achieve maximum value from our AI platform, driving adoption, retention, and expansion.
    You will be responsible for scaling the technical success function, developing best practices for GenAI and LLM implementation, and serving as a trusted advisor to our most strategic enterprise clients.
    This is a post-sales leadership role that requires a deep understanding of enterprise AI/ML deployment, customer relationship management, and strategic thinking.
    You will partner closely with Sales, Product, and Engineering to champion the voice of the customer and influence the product roadmap.
    Key responsibilities include P&L management for the professional services team, building strategic partnerships with cloud providers, and ensuring the successful delivery of AI solutions.
    """
