...
class PhaseExecutor:
    ...
    def execute_with_retry(
        ...
        # All attempts failed
        logger.error(f"{phase_name}: All retries exhausted")
        if last_exception:
    ...
    def _validate_phase_result(self, result: Dict[str, Any], phase_name: str) -> bool:
        ...
        if "phase1" in phase_name.lower() or "thematic" in phase_name.lower():
            # FIX: Only require the main 'thematic_analysis' key.
            # 'role_classification' is secondary and .get() is used downstream.
            return "thematic_analysis" in result

        elif "phase2" in phase_name.lower() or "authenticity" in phase_name.lower():
            # FIX: Only require the main 'authenticity_patterns' key.
            # 'pattern_confidence' is secondary and .get() is used downstream.
            return "authenticity_patterns" in result

        elif "phase4" in phase_name.lower() or "narrative" in phase_name.lower(): # v8.10: Approach 3
            # This check is already correct (only one key)
        ...
        return True # Default to True if phase_name doesn't match known phases   
...
"""
    ...
    def _build_context_k0_headline(self, spec: Dict) -> Dict:
        feedback = self._get_feedback_instruction(["H3_K0_HEADLINE_WORD_COUNT", "H3_K0_HEADLINE_COMPONENT_WC", "H3_K0_HEADLINE_NO_TITLES", "H3_K0_HEADLINE_NO_COMMAS"])
        return {
    ...
    def _build_context_k1_summary(self, spec: Dict) -> Dict:
        ...
        feedback = self._get_feedback_instruction(["H3_K1_SENTENCE_COUNT", "H3_K1_WORD_COUNT", "H3_K1_DIFFERENTIATOR_RANGE"])
...
class PreFlightValidator:
    ...
    def _register_rules(self):
        ...
        all_rules_config.append(self._mk_method("H5_CONTENT_NO_PROMPT_CONTAMINATION", ValidationSeverity.HIGH, "content", "_validate_no_prompt_contamination", lambda ctx: f"Found prompt contamination keywords in content: {ctx._cache.get('CONTENT_NO_PROMPT_CONTAMINATION', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("H3_GLOBAL_CONTENT_NO_CONVERSATIONAL_FILLERS", ValidationSeverity.HIGH, "content", "_validate_no_conversational_fillers", lambda ctx: f"Found conversational filler phrases in content: {ctx._cache.get('CONTENT_NO_CONVERSATIONAL_FILLERS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("H5_STRUCTURE_NO_EMPTY_LIST_ITEMS", ValidationSeverity.MEDIUM, "structure", "_validate_no_empty_list_items", lambda ctx: f"Found empty list items in sections: {ctx._cache.get('STRUCTURE_NO_EMPTY_LIST_ITEMS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("H5_STRUCTURE_MARKDOWN_HEADER_SPACING", ValidationSeverity.MEDIUM, "structure", "_validate_markdown_header_spacing", lambda ctx: f"Found markdown headers with missing spaces: {ctx._cache.get('STRUCTURE_MARKDOWN_HEADER_SPACING', {}).get('violations', 'N/A')}"))
...
class WorkflowOrchestrator:
    ...
    def execute_workflow(
        ...
        # v14.50 FIX: Initialize thematic_analysis to prevent UnboundLocalError
