"""
Resume Generation Engine v4.4 - MINIMAL OUTPUT VERSION
=======================================================
...
class MasterResume:
    ...
    SCHEMA_VERSION = "master_resume_v4.4_minimal"
...
class PerSectionTolerance:
    ...
    section_baselines: Dict[str, int] = field(default_factory=lambda: {
        ...
        "unify_bullets": 203,
        "ibm_bullets": 194,
        "tradersense": 28,  # Adjusted from 65
        "ey_bullets": 67,
        "early_career": 42,
        "education": 21,
        "competencies": 120  # Adjusted from 240
    })
...
class SignalElasticityModel:
    ...
    def __init__(self):
        self.saturation_points = {
            ...
            "unify_bullets": 240,
            "ibm_bullets": 225,
            "tradersense": 35,  # Adjusted
            "ey_bullets": 70,
            "early_career": 45,
            "education": 22,
            "competencies": 150  # Adjusted
        }
...
class BaselineResumeMetricsV4:
    ...
    UNIFY_IBM_RATIO_MIN = 0.80
    UNIFY_IBM_RATIO_TARGET = 1.25
    UNIFY_IBM_RATIO_MAX = 1.60
    UNIFY_IBM_RATIO_SOFT_MIN = 0.90
    UNIFY_IBM_RATIO_SOFT_MAX = 1.50
...
class ResumeGenerationEngineV4:
    ...
    def _generate_sections(self, role_profile: str, targets: Dict[str, int]) -> Dict[str, any]:
        """Generate resume sections with word count targets."""
        sections = {}
        ...
        sections["unify_bullets"] = {
            ...
            "word_count": sum(b["word_count"] for b in unify_bullets),
        ...
        sections["ibm_bullets"] = {
            ...
            "word_count": sum(b["word_count"] for b in ibm_bullets),
    ...
    def _generate_output2_word_count(self, sections: Dict[str, any]) -> str:
        ...
        for section_key, display_name in section_display.items():
            if section_key in sections:
                wc = sections[section_key]["word_count"]
                min_w, max_w = self.tolerance.get_band(section_key)
                signal = sections[section_key]["signal_score"]
                elasticity = self.elasticity.calculate_elasticity(section_key, wc, signal)
                status = "✓ PASS" if self.tolerance.validate_section(section_key, wc) else "✗ FAIL"
                
                lines.append(f"│ {display_name:16} │ {wc:7} │ {min_w:3}–{max_w:<12} │ {signal:9.2f} │ {elasticity:9.3f} │ {status:6} │")
        
        lines.append("└" + "─" * 18 + "┴" + "─" * 9 + "┴" + "─" * 18 + "┴" + "─" * 11 + "┴" + "─" * 11 + "┴" + "─" * 8 + "┘")
        lines.append("")
        
        total_words = sum(s["word_count"] for s in sections.values())
        baseline = BaselineResumeMetricsV4.TOTAL_WORD_COUNT_TARGET
        lines.append(f"TOTAL WORDS: {total_words} (Baseline: {baseline}, within budget)")
