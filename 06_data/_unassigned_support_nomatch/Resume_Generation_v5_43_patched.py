"""
Resume Generation Engine v5.43 - JD INTEGRATION & OUTPUT HARDENING

v5.43 CHANGES - CRITICAL FIXES:
✓ FIXED: JobDescriptionAnalyzer now uses real JD parsing (not mock)
✓ FIXED: Integrated JDParser class into main workflow (HOP-0)
✓ NEW: Output display enforcement (plain text, unfenced markdown, fenced JSON)
✓ NEW: ASCII bar graph in Output 5 for section-level signal analysis
✓ NEW: Section 1.5 in QA report showing Expected vs Actual signal by section
✓ FIXED: App Tracker file path references (resume_md instead of resume_json)
✓ FIXED: QA Report section numbering (corrected duplicate Section 8 to Section 9)
✓ NEW: display_outputs_to_console() method for hardened output formatting
✓ NEW: _calculate_section_signals() for per-section signal measurement
✓ Result: 100% JD integration, no truncation, proper formatting

v5.42 CHANGES - EMBEDDED MASTER RESUME UPDATE:
✓ UPDATED: Embedded MASTER_RESUME_JSON with actual Master_Resume_V2.15.json data
✓ FIXED: Contact information (correct email, phone, linkedin)
✓ FIXED: Education (MS Biostatistics Columbia, BA Biology Brown)
✓ FIXED: Certifications (AWS ML Engineer, Databricks, Fellow of Society of Actuaries)
✓ FIXED: Professional experience (accurate titles, dates, locations, bullets)
✓ FIXED: Strategic and technical competencies (actual competency text)
✓ Result: Embedded resume now matches external Master_Resume_V2.15.json
...
v5.40 CHANGES - CRITICAL ARCHITECTURE FIX:
✓ FIXED: "Only resume should be master resume" principle enforced
  - REMOVED: load_master_resume() function and file loading
  - REMOVED: External file dependency on Master_Resume_V2_15.json
  - KEPT: Embedded master resume as SOLE source of truth
  - Result: Self-contained script, single MASTER_RESUME_JSON assignment
✓ PRINCIPLE: One embedded master resume, no file loading, no overwriting
  - Users modify the embedded resume directly in the code
  - No confusion about multiple data sources
  - No file loading failures
  - No overwriting bugs

v5.38 CHANGES (maintained):
✓ Real Claude API integration (not mock)
✓ Comprehensive API error handling

All v5.36 functionality maintained:
✓ Master resume as true source of truth
✓ Structured experience_sections data flow
✓ Dynamic job metadata extraction
✓ Zero fabrication - all bullets from master resume
✓ Comprehensive hyphenation rules (44+ terms)
✓ 7-layer text sanitization suite
✓ Enhanced QA validation with bullet provenance tracking
✓ JD alignment scoring for skills/competencies
...
class ArtistGenerator:
    ...
    def _call_claude_api(
        ...
        """
        Call Claude API with given prompt using actual Anthropic API.
        
        v5.40: Real API integration (not mock).
        ...
        """
        # v5.41: Route to Gemini if configured
        if LLM_PROVIDER == "gemini":
            return self._call_gemini_api(prompt, system_prompt, temperature, max_tokens, reasoning_config)
    ...
    def _call_gemini_api(
        ...
        v5.41: New Gemini integration.
        
        Args:
            prompt: User prompt
            system_prompt: System instruction (combined with prompt for Gemini)
            temperature: Sampling temperature (0.0-2.0 for Gemini)
            max_tokens: Maximum tokens to generate
            reasoning_config: Ignored for Gemini (Claude-specific)
        ...
        # Get model and prepare URL
        model = LLM_MODELS.get("gemini", "gemini-2.0-flash-exp")
    ...
    def _generate_k1_executive_summary(
        ...
        v5.36: Updated to use experience_sections structure.
        Uses JD analysis and master resume context.
        Target: 100-150 words
        """
        # v5.36: Extract master resume context from experience_sections
        master_bullets = []
        for exp_section in enriched_scaffold.get('experience_sections', []):
            company = exp_section.get('company', '')
            for bullet_data in exp_section.get('bullets', []):
...
Generate the headline now. Return ONLY the headline text."""
    ...
    def _generate_k5a_bullets(
        ...
        # v5.36: Get Unify bullets from experience_sections
        unify_bullets = []
        for exp_section in enriched_scaffold.get('experience_sections', []):
            if 'Unify Consulting' in exp_section.get('company', ''):
                for bullet_data in exp_section.get('bullets', []):
...
Generate the overview now. Return ONLY the overview text."""
    ...
    def _generate_k6a_bullets(
        ...
        # v5.36: Get IBM bullets from experience_sections
        ibm_bullets = []
        for exp_section in enriched_scaffold.get('experience_sections', []):
            if 'IBM' in exp_section.get('company', ''):
                for bullet_data in exp_section.get('bullets', []):
                    ibm_bullets.append({
                        'text': bullet_data.get('bullet_text', ''),
                        'metrics': bullet_data.get('quantified_metrics', []),
...
..."""
    ...
    def _generate_k9_cover_letter(
        ...
        # v5.36: Extract achievements from experience_sections
        achievement_bullets = []
        for exp_section in enriched_scaffold.get('experience_sections', []):
            for bullet_data in exp_section.get('bullets', [])[:4]:  # Max 4 from each section
                achievement_bullets.append({'text': bullet_data.get('bullet_text', '')})
...
{self._format_bullets_for_prompt(achievement_bullets[:12])}
</candidate_achievements>
...
class TextSanitizer:
    """
    HOP-4.5: Enhanced Text Sanitization (v5.35)
    Applies comprehensive hyphenation rules and 7-layer sanitization suite:
    1. Unnatural hyphen removal
    2. Natural hyphen preservation
    3. Unicode normalization
    4. Punctuation spacing correction
    5. Markdown artifact removal
    6. Corporate jargon simplification
    7. Filler word reduction
    """
