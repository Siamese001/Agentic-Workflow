"""
Resume Generation Engine v14.31

Enhanced Logging & Observability Update

This version implements 5 comprehensive logging enhancements for improved debugging,
monitoring, and workflow observability:

1. Enhanced API Call Logging (Retries, Parameters, Latency):
   - Logs key generation_config parameters (temp, max_tokens) before each API call
   - Logs finish_reason, block_reason, and call duration on completion
   - Logs specific error types and problematic text snippets for JSON parse failures
   - Distinguishes between timeout, JSON parse, and HopExecutionError types
   
2. Configuration Context Logging:
   - Logs all reasoning parameters (cot, tot_b, tot_d, sc, reflexion, max_loops)
   - Logs calculated intensity scores and reasoning levels
   - Logs temperature and token allocations for each API call
   - Makes configuration decisions transparent and traceable
   
3. Data Flow & Transformation Snapshots:
   - Logs key summary data after each HOP completion
   - HOP-0: Signal score, primary theme, keywords, differentiator counts
   - HOP-1: Bullets extracted, hallucination issue counts
   - HOP-2: Sections enriched, duplicate detection results
   - HOP-3: Sections generated, validation status per attempt
   - HOP-4.5: Staging buffer keys, sanitization counts
   
4. Intermediate Validation & Decision Logging:
   - Logs which bullets fail word count checks before rewrite
   - Logs old/new word counts for successful rewrites
   - Logs which specific sections failed validation in HOP-3 retry loop
   - Logs sections being added back to generation queue
   - Logs critical/high failure counts in GateDecisionEngine
   
5. Structured Logging (JSON Format):
   - New JSONFormatter class for machine-readable logs
   - setup_structured_logging() function for configurable log format
   - Enable via ENABLE_JSON_LOGGING environment variable
   - Supports extra context fields (hop_id, section_id, attempt, duration_ms, etc.)
   - Compatible with log analysis tools (Logstash, Splunk, jq)
...
Usage:
    ...
    orchestrator = WorkflowOrchestrator(master_resume, enable_json_logging=True)
    
    # Or via environment variable
    export ENABLE_JSON_LOGGING=true
    python Resume_Generation_v14_31.py

Dependencies: Same as v14.30
Author: Enhanced logging by Claude (Anthropic)
Date: 2025
"""
...
class JSONFormatter(logging.Formatter):
    ...
    Enhancement #5: Makes logs machine-readable for analysis.
    """
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, 'hop_id'):
            log_data['hop_id'] = record.hop_id
        ...
        if hasattr(record, 'api_call_count'):
            log_data['api_call_count'] = record.api_call_count
...
def setup_structured_logging(enable_json: bool = False, log_level: int = logging.INFO):
    """
    Setup logging with optional JSON formatting.
    Enhancement #5: Enables structured logging for better observability.
    
    Args:
        enable_json: If True, use JSONFormatter. If False, use standard format.
...
def reasoning_config_to_api_params(reasoning_config: ReasoningConfig) -> dict:
    ...
    try:
        ...
        logger.debug(
            ...
            f"intensity={intensity:.1f}, level={level}, temp={temperature}, "
            f"allocated_max_tokens={allocated_max_tokens}, final_max_tokens={final_max_tokens}"
        )
...
class GeminiWebSearchClient:
    ...
    def _make_api_call(
        ...
        v14.31 Enhancement #1: Enhanced API call logging with parameters and latency.
        """
...
"""
    ...
    def _analyze_with_resilient_web_search(
        ...
        if not self.web_rag or not self.rag_mission:
             # v14.29: Dead code removed - this should be unreachable
    ...
    def _synthesize_thematic_analysis(
        ...
        # v14.29: Fixed serialization using asdict recursively
        ...
        logger.debug("Converting phase results to dictionaries using asdict...")
        phase1_dict = safe_to_dict(phase1)
        phase2_dict = safe_to_dict(phase2)
        phase3_dict = safe_to_dict(phase3)
        phase4_dict = safe_to_dict(phase4)
        logger.debug("Phase conversion complete.")
...
class HallucinationDetector:
    """
    A configurable, regex-based detector for common LLM "boasts" or
    "hallucinations" in generated marketing or descriptive text.

    It is intended as a fast, heuristic-based safety net, not a
    perfect detector. It returns structured findings for the caller
    to act upon (e.g., log a warning, request regeneration).
    """

    # --- Pre-compiled Regex Patterns ---
    
    # Matches words like "best", "greatest", "unmatched", "world-class", etc.
    # \b ensures we match whole words.
    SUPERLATIVES_REGEX = re.compile(
        r"\b(best|greatest|most|fastest|largest|easiest|top-tier|"
        r"world-class|unmatched|unrivaled|unbeatable|revolutionary)\b",
        re.IGNORECASE
    )
    
    # Matches patterns like "1000%", "1,500.5%", "5000 %" growth/increase/etc.
    IMPLAUSIBLE_GROWTH_REGEX = re.compile(
        # Matches "1,000" or "1000" or "1500.5"
        r"(\d{1,3}(,\d{3})*|\d+)(\.\d+)?\s*%",
        re.IGNORECASE
    )
    
    # Matches short timeframes like "in just 5 days", "within a week", "in only 2 months"
    SHORT_TIMEFRAME_REGEX = re.compile(
        r"\b(in|within)\s+(just|only\s+)?(\d+|a few|several)\s+(days?|weeks?|months?)\b",
        re.IGNORECASE
    )

    def __init__(self, 
                 superlative_threshold: int = 2,
                 growth_percent_threshold: float = 1000.0):
        """
        Initialize the detector with configurable thresholds.

        Args:
            superlative_threshold: Flag text if it contains this many
                                     or more superlative words.
            growth_percent_threshold: The percentage point at which growth
                                        is considered "implausible".
        """
        self.superlative_threshold = superlative_threshold
        self.growth_percent_threshold = growth_percent_threshold

    def detect(self, text: str) -> List[Dict[str, Any]]:
        """
        Runs all configured checks against the input text.

        Args:
            text: The generated text to analyze.

        Returns:
            A list of "findings" (dictionaries). An empty list
            means no issues were detected.
        """
        findings = []

        # Run each specific check
        superlative_finding = self._check_excessive_superlatives(text)
        if superlative_finding:
            findings.append(superlative_finding)
            
        growth_finding = self._check_implausible_growth(text)
        if growth_finding:
            findings.append(growth_finding)

        # --- Future checks can be easily added here ---
        # e.g., abs_claim_finding = self._check_absolute_claims(text)
        # if abs_claim_finding:
        #     findings.append(abs_claim_finding)

        return findings

    def _check_excessive_superlatives(self, text: str) -> Optional[Dict[str, Any]]:
        """Finds text with too many "boastful" words."""
        matches = self.SUPERLATIVES_REGEX.findall(text)
        
        if len(matches) >= self.superlative_threshold:
            return {
                "type": "Excessive Superlatives",
                "count": len(matches),
                "threshold": self.superlative_threshold,
                "matches_found": list(set(matches)), # Show unique words found
                "message": f"Detected {len(matches)} superlatives, "
                           f"exceeding threshold of {self.superlative_threshold}."
            }
        return None

    def _check_implausible_growth(self, text: str) -> Optional[Dict[str, Any]]:
        """
        Finds text claiming very high growth (e.g., >1000%) combined
        with a stated short timeframe.
        """
        growth_match = self.IMPLAUSIBLE_GROWTH_REGEX.search(text)
        time_match = self.SHORT_TIMEFRAME_REGEX.search(text)

        # We must find BOTH a high growth figure AND a short timeframe
        # to trigger this. This reduces false positives.
        if not (growth_match and time_match):
            return None

        try:
            # Extract and parse the percentage value
            percent_str = growth_match.group(1).replace(",", "")
            percent_val = float(percent_str)
        except Exception:
            # Failed to parse, ignore it
            return None

        # Check if the parsed value exceeds our threshold
        if percent_val >= self.growth_percent_threshold:
            return {
                "type": "Implausible Growth Claim",
                "value": f"{percent_val}%",
                "threshold": self.growth_percent_threshold,
                "timeframe_context": time_match.group(0).strip(),
                "message": f"Detected implausible growth claim: {percent_val}% "
                           f"in a short timeframe ('{time_match.group(0).strip()}')."
            }
        return None
...
class DuplicateDetector:
    ...
    def compute_overview_bullet_similarity(
            ...
            results = {
                "section": section_id,
                "overview_length": text_utils.count_words(overview_text) if overview_text else 0, # USE MS WORD STYLE
    ...
    def _call_gemini_api(self, prompt: str, reasoning_config: ReasoningConfig, section_id: str, system_prompt: str, temperature_override: Optional[float] = None) -> Tuple[str, int]:
        ...
        try:
            ...
            if temperature_override is not None:
                ...
                # Enhancement #5: Example of structured logging with extra context
                logging.info(
...
"""
    ...
    def _post_process_k10_skills(self, skills_text: str, section_enum: ResumeSection) -> List[str]:
        ...
        try:
            ...
            for skill in skills_intermediate:
                word_count = text_utils.count_words(skill) # Use helper
                if 1 <= word_count <= 3:
    ...
    def _post_process_narrative(self, narrative_text: str, section_enum: ResumeSection) -> str:
        ...
        final_wc = text_utils.count_words(narrative_text); final_sc = text_utils.count_sentences(narrative_text)
...
"""
        ...
        final_wc = text_utils.count_words(synthesized_overview); final_sc = text_utils.count_sentences(synthesized_overview)
...
"""
            ...
            rewritten_wc = text_utils.count_words(rewritten_text)
    ...
    def _validate_and_potentially_rewrite_bullets(self, selected_bullets_structured: List[Dict], min_target: int, max_target: int, section_id_str: str, temperature_override: Optional[float] = None) -> Tuple[List[Dict], int]:
        ...
        for i, bullet_data in enumerate(selected_bullets_structured):
            ...
            word_count = bullet_data.get('word_count', text_utils.count_words(original_text)) # Use stored or calculate
            if not original_text: raise HopExecutionError(f"Empty bullet in {section_id_str}[{i}].")

            if not (min_target <= word_count <= max_target):
                # Enhancement #4: Log specific bullet details before rewrite
                logging.warning(
                    f"  WC Check FAIL for {section_id_str}[{i}]: Count={word_count} (Target: {min_target}-{max_target}). "
                    f"Attempting rewrite for bullet: '{original_text[:50]}...'"
                )
                try:
                    ...
                    total_rewrite_calls += rewrite_calls; rewritten_word_count = text_utils.count_words(rewritten_text)
                    # Enhancement #4: Log successful rewrite details
                    logging.info(
...
"""
            ...
            result_list = [{"text": b, "provenance": BulletProvenance.Customized.value, "word_count": text_utils.count_words(b)} for b in rewritten_bullets_text]
...
"""
            ...
            result_list = [{"text": b, "provenance": BulletProvenance.Synthetic.value, "word_count": text_utils.count_words(b)} for b in synthetic_bullets_text]
            return result_list, total_calls
        except HopExecutionError as he: raise he
    ...
    def _generate_tailored_bullets_for_experience(
        ...
        for bullet_text in master_bullets_source:
             if bullet_text and bullet_text.strip():
                 ...
                 master_bullets_structured.append({"bullet_text": cleaned_text, "text": cleaned_text, "provenance": BulletProvenance.Verbatim.value, "word_count": text_utils.count_words(cleaned_text)})
            ...
            if is_competencies:
                for item in final_bullets:
                    if isinstance(item, dict) and 'text' in item:
                        ...
                        item['word_count'] = text_utils.count_words(cleaned_text)
...
class TextUtils:
    ...
    def strip_markdown_fences(text: str) -> str:
        ...
        Handles both opening (```language) and closing (```) fences.
        """
        if not text:
            return text
    ...
    @staticmethod
    def strip_list_prefix(text: str, prefixes: list = None) -> str:
        """
        Remove common list prefixes from text.
        Default prefixes: '* ', '• ', '- ', numbered lists like '1. '
        """
        if not text:
            return text
        
        if prefixes is None:
            prefixes = [r'^\s*\*\s+', r'^\s*•\s+', r'^\s*-\s+', r'^\s*\d+\.\s+']
    ...
    def format_validation_message(validation_result, context_cache: dict = None) -> str:
        """
        Safely format a validation result message.
        Handles both string templates and callable message functions.
        Uses defaultdict to handle missing keys gracefully.
        
        Args:
            validation_result: ValidationResult object with message and optional details
            context_cache: Optional dict of cached values for message formatting
...
class ValidationContext:
            ...
            if key_enum not in [ResumeSection.K0_NAME, ResumeSection.K0_CONTACT] and \
               not key.endswith("_HEADER"):
                ...
                if isinstance(value, str):
                    total += text_utils.count_words(value) # Use helper
                elif isinstance(value, list):
                    total += sum(text_utils.count_words(item.get('text', str(item))) if isinstance(item, dict) else text_utils.count_words(str(item)) for item in value)
    ...
    def _calculate_unify_words(self):
        ...
        overview_wc = text_utils.count_words(unify_overview) # Use helper
        bullets_wc = sum(text_utils.count_words(b.get('text', '')) for b in unify_bullets if isinstance(b, dict))
    ...
    def _calculate_ibm_words(self):
        ...
        overview_wc = text_utils.count_words(ibm_overview) # Use helper
        bullets_wc = sum(text_utils.count_words(b.get('text', '')) for b in ibm_bullets if isinstance(b, dict))
        return overview_wc + bullets_wc
    ...
    def _calculate_k1_sentence_count_details(self):
        ...
        details = self._calculate_metric_details(
            ...
            metrics_to_calc=[('sentence_count', text_utils.count_sentences)], # Use helper
    ...
    def _calculate_k1_word_count_details(self):
        details = self._calculate_metric_details(
            section_enum=ResumeSection.K1_EXECUTIVE_SUMMARY,
            metrics_to_calc=[('word_count', text_utils.count_words)], # Use helper
    ...
    def _calculate_cover_letter_structure_details(self):
        ...
        try:
             ...
             if p1_idx < closing_idx and p1_idx < len(paras): p1_wc = text_utils.count_words(paras[p1_idx]) # Use helper
             if p2_idx < closing_idx and p2_idx < len(paras): p2_wc = text_utils.count_words(paras[p2_idx]) # Use helper
             if p3_idx < closing_idx and p3_idx < len(paras): p3_wc = text_utils.count_words(paras[p3_idx]) # Use helper
...
class PreFlightValidator:
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        rule_map = {
            ...
            "CONTENT_NO_PROMPT_CONTAMINATION": "GLOBAL",
            "CONTENT_NO_CONVERSATIONAL_FILLERS": "GLOBAL",
            "STRUCTURE_NO_EMPTY_LIST_ITEMS": "GLOBAL",
            "STRUCTURE_MARKDOWN_HEADER_SPACING": "GLOBAL",
    ...
    def _register_rules(self):
        ...
        # Add new regex guardrail rules
        all_rules_config.append(self._mk_method("CONTENT_NO_PROMPT_CONTAMINATION", ValidationSeverity.HIGH, "content", "_validate_no_prompt_contamination", lambda ctx: f"Found prompt contamination keywords in content: {ctx._cache.get('CONTENT_NO_PROMPT_CONTAMINATION', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("CONTENT_NO_CONVERSATIONAL_FILLERS", ValidationSeverity.HIGH, "content", "_validate_no_conversational_fillers", lambda ctx: f"Found conversational filler phrases in content: {ctx._cache.get('CONTENT_NO_CONVERSATIONAL_FILLERS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("STRUCTURE_NO_EMPTY_LIST_ITEMS", ValidationSeverity.MEDIUM, "structure", "_validate_no_empty_list_items", lambda ctx: f"Found empty list items in sections: {ctx._cache.get('STRUCTURE_NO_EMPTY_LIST_ITEMS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("STRUCTURE_MARKDOWN_HEADER_SPACING", ValidationSeverity.MEDIUM, "structure", "_validate_markdown_header_spacing", lambda ctx: f"Found markdown headers with missing spaces: {ctx._cache.get('STRUCTURE_MARKDOWN_HEADER_SPACING', {}).get('violations', 'N/A')}"))
            ...
            def create_error_message_lambda(template, rule_id_for_cache):
                def error_lambda(ctx: ValidationContext):
                    try:
                        ...
                        else: # Use TextUtils for consistent formatting
                            from collections import defaultdict
                            mock_result = type("MockResult", (), {"rule_id": rule_id_for_cache, "message": template, "details": details})()
                            return text_utils.format_validation_message(mock_result, {rule_id_for_cache: details})
                    except Exception as e:
    ...
    def _validate_bullet_word_count_range(self, context: ValidationContext) -> bool: # Validates that all generated bullets fall within their section-specific word count ranges.
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            for i, bullet in enumerate(bullets):
                ...
                if isinstance(bullet, dict): bullet_text = bullet.get('text', ''); actual_wc = bullet.get('word_count', text_utils.count_words(bullet_text))
                elif isinstance(bullet, str): bullet_text = bullet; actual_wc = text_utils.count_words(bullet_text)
    ...
    def _validate_headline_format_component_wc(self, context: ValidationContext) -> bool:
        ...
        for i, comp in enumerate(components):
            word_count = text_utils.count_words(comp)
    ...
    def _validate_no_prompt_contamination(self, context: ValidationContext) -> bool:
        ...
        if found_snippets:
            context._cache["CONTENT_NO_PROMPT_CONTAMINATION"] = {"violations": ", ".join(found_snippets[:3]), "failed_sections": failed_sections}
            return False
        return True
    ...
    def _validate_no_conversational_fillers(self, context: ValidationContext) -> bool:
        ...
        if found_snippets:
            context._cache["CONTENT_NO_CONVERSATIONAL_FILLERS"] = {"violations": ", ".join(found_snippets[:3]), "failed_sections": failed_sections}
            return False
        return True
    ...
    def _validate_no_empty_list_items(self, context: ValidationContext) -> bool:
        ...
        if found_snippets:
            context._cache["STRUCTURE_NO_EMPTY_LIST_ITEMS"] = {"violations": ", ".join(found_snippets), "failed_sections": failed_sections}
            return False
        return True
    ...
    def _validate_markdown_header_spacing(self, context: ValidationContext) -> bool:
        ...
        if found_snippets:
            context._cache["STRUCTURE_MARKDOWN_HEADER_SPACING"] = {"violations": ", ".join(found_snippets), "failed_sections": failed_sections}
            return False
        return True
    ...
    def _render_skills(self, staging_buffer: ImmutableStagingBuffer, job_description: Optional[str] = None) -> str:
        ...
        for skill in skills_list:
            if isinstance(skill, str):
                cleaned = skill.strip(); wc = text_utils.count_words(cleaned)
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict, test_mode: bool = False, enable_json_logging: bool = False):
        """
        ...
        Args:
            ...
            test_mode: If True, skip API key check
            enable_json_logging: Enhancement #5 - Enable structured JSON logging
        """
        self.master_resume = master_resume
        ...
        # Enhancement #5: Use new structured logging setup
        setup_structured_logging(enable_json=enable_json_logging, log_level=logging.INFO)
        self.logger = logging.getLogger(__name__)
        
        if enable_json_logging:
            self.logger.info("Structured JSON logging enabled", extra={"component": "orchestrator"})
    ...
    def _execute_hop_3_artist_generation(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Tuple[Dict, int]:
        ...
        for attempt, temperature in enumerate(temperature_schedule, 1):
            ...
            if all_passed:
                ...
                for section_enum in all_llm_sections:
                 ...
                 # Enhancement #4: Log sections being added back to generation queue
                 if failed_llm_sections_this_attempt:
                     self.logger.info(
                         f"    Sections to regenerate in next attempt: "
                         f"{', '.join([s.name for s in failed_llm_sections_this_attempt])}"
                     )
        ...
        self.logger.info(f"  [HOP-3] Final Locked Temperatures: {locked_section_temps}")
...
class QAReportGenerator:
    ...
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_3_word_count_distribution", "args": ["validation_results"]},  # Renumbered from 4
        {"method": "_build_qa_section_4_provenance", "args": ["staging_buffer"]},  # Renumbered from 5
        {"method": "_build_qa_section_5_content_authenticity", "args": ["validation_results"]},  # Renumbered from 6
        {"method": "_build_qa_section_6_prod_readiness", "args": ["validation_results"]},  # Renumbered from 7
        {"method": "_build_qa_section_7_structural_validation", "args": ["validation_results"]},  # Renumbered from 9
        {"method": "_build_qa_section_8_jd_enforcement", "args": []},  # Renumbered from 10
        {"method": "_build_qa_section_9_final_format", "args": ["validation_results", "file_contents"]},  # Renumbered from 11
    ]
    ...
    def _build_qa_section_3_word_count_distribution(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        lines = ["", "**3. WORD COUNT & DISTRIBUTION COMPLIANCE**", ""]
        lines.append("Combines overall resume word count, key section constraints (Headline, Exec Summary, Overviews, Narratives, Cover Letter paragraphs), and distribution metrics (Unify/IBM). Bullet length validation is in Section 4.")
        lines.append("")
        lines.append("```markdown")
    ...
    def _build_qa_section_4_provenance(
        self,
        staging_buffer: ImmutableStagingBuffer
    ) -> List[str]:
            ...
            if isinstance(items, list) and items:
                for i, item_data in enumerate(items):
                    ...
                    if isinstance(item_data, dict):
                        ...
                        word_count = item_data.get('word_count', text_utils.count_words(item_text)) # Use helper
                        provenance = item_data.get('provenance', 'N/A')
                    elif isinstance(item_data, str):
                        item_text = item_data
                        word_count = text_utils.count_words(item_text) # Use helper
    ...
    def _build_qa_section_5_content_authenticity(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        title = "**5. CONTENT AUTHENTICITY (Hallucination, Verbs, Phrasing, Signal Check)**"
    ...
    def _build_qa_section_6_prod_readiness(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        lines = ["", "**6. PRODUCTION READINESS**", ""]
        lines.append("```markdown")
    ...
    def _build_qa_section_7_structural_validation(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        title = "**7. STRUCTURAL VALIDATION (Content Presence & Basic Format)**"
    ...
    def _build_qa_section_8_jd_enforcement(self) -> List[str]:
        """Build JD enforcement summary."""
        lines = ["", "**8. JD ENFORCEMENT VALIDATION**", ""]
        lines.append("")
        
        enforcer = self.orchestrator.jd_enforcer

        # Display enforcer gate results stored in checkpoints/validation results
        # Assuming they were stored during HOP-0 through HOP-5 execution
        gate_results = []
        ...
        if hop0_chk and hop0_chk.validation_results:
            for vr in hop0_chk.validation_results:
                if vr.rule_id.startswith("GATE-"):
                    status = "PASS" if vr.passed else "FAIL"
                    gate_results.append((vr.rule_id, status, vr.message[:60] if hasattr(vr, 'message') else ""))

        hop2_chk = next((chk for chk in self.orchestrator.hop_checkpoints if chk.hop_id == "HOP-2"), None)
        if hop2_chk and hop2_chk.validation_results:
            for vr in hop2_chk.validation_results:
                if vr.rule_id.startswith("GATE-"):
                    status = "PASS" if vr.passed else "FAIL"
                    gate_results.append((vr.rule_id, status, vr.message[:60] if hasattr(vr, 'message') else ""))

        hop3_chk = next((chk for chk in self.orchestrator.hop_checkpoints if chk.hop_id == "HOP-3"), None)
        if hop3_chk and hop3_chk.validation_results:
            for vr in hop3_chk.validation_results:
                if vr.rule_id.startswith("GATE-"):
                    status = "PASS" if vr.passed else "FAIL"
                    gate_results.append((vr.rule_id, status, vr.message[:60] if hasattr(vr, 'message') else ""))

        hop5_chk = next((chk for chk in self.orchestrator.hop_checkpoints if chk.hop_id == "HOP-5"), None)
        if hop5_chk and hop5_chk.validation_results:
            for vr in hop5_chk.validation_results:
                if vr.rule_id.startswith("GATE-"):
                    status = "PASS" if vr.passed else "FAIL"
                    gate_results.append((vr.rule_id, status, vr.message[:60] if hasattr(vr, 'message') else ""))
        ...
        if gate_results:
            ...
            lines.append("No JD Enforcement gate results found in checkpoints.")

        return lines

    def _build_qa_section_9_final_format(
        self,
        validation_results: List[ValidationResult],
        file_contents: Dict[str, str]
    ) -> List[str]:
        lines = ["", "**9. FINAL OUTPUT FORMATTING SUMMARY**", ""]
    ...
    def _validate_qa_report_formatting(self, report_text: str) -> ValidationResult:
        """
        Validates QA report formatting to ensure tables use pre-formatted text blocks.
        
        Strategy:
        1. Split report into sections using numbered headers (e.g., "1. TITLE")
        2. For sections expected to contain tables, verify they have ``` code blocks
        3. Within those blocks, detect and flag Markdown table syntax (|---|) 
        4. Goal: Tables should be plain text with spacing, not Markdown pipes
        
        Returns ValidationResult indicating pass/fail with specific section errors.
        """
    ...
    def _format_plain_text_table(
        ...
        """
        Renders structured data as visually-aligned plain-text table for code blocks.
        
        Key features:
        - Auto-calculates or uses provided column widths
        - Supports multi-line headers
        - Handles text alignment (L/R/C) using Python format strings
        - Optional text wrapping with textwrap for long content
        - Uses spaces (not pipes) for column separation
        - Adds separator line (---) below headers
        
        Returns list of strings (lines) ready for ``` markdown ``` blocks.
        """
    ...
    def _safe_format_validation_message(self, validation_result: ValidationResult) -> str:
        """
        Safely formats a validation result message.
        Delegates to TextUtils.format_validation_message.
        """
        context_cache = getattr(self, '_context_cache', None)
        return text_utils.format_validation_message(validation_result, context_cache)
...
if __name__ == '__main__':
    # Enhancement #5: Use structured logging setup with env var control
    enable_json = os.environ.get('ENABLE_JSON_LOGGING', 'false').lower() == 'true'
    setup_structured_logging(enable_json=enable_json, log_level=logging.INFO)
    logger = logging.getLogger(__name__) # Define logger
    
    if enable_json:
        logger.info("Running with JSON structured logging", extra={"mode": "main"})
