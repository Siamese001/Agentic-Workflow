        ...
        for i, bullet_data in enumerate(selected_bullets_structured):
            ...
            if not (min_target <= word_count <= max_target):
                ...
                try:
                    ...
                    total_rewrite_calls += rewrite_calls; rewritten_word_count = text_utils.count_words(rewritten_text); logging.info(f"    Rewrite SUCCESSFUL for {section_id_str}[{i}]. New count: {rewritten_word_count}")
...
class PreFlightValidator:
    ...
    def _register_rules(self):
        ...
        all_rules_config.append(_mk_method("CONTENT_NO_PROMPT_CONTAMINATION", ValidationSeverity.HIGH, "content", "_validate_no_prompt_contamination", lambda ctx: f"Found prompt contamination keywords in content: {ctx._cache.get('CONTENT_NO_PROMPT_CONTAMINATION', {}).get('violations', 'N/A')}"))
        all_rules_config.append(_mk_method("CONTENT_NO_CONVERSATIONAL_FILLERS", ValidationSeverity.HIGH, "content", "_validate_no_conversational_fillers", lambda ctx: f"Found conversational filler phrases in content: {ctx._cache.get('CONTENT_NO_CONVERSATIONAL_FILLERS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(_mk_method("STRUCTURE_NO_EMPTY_LIST_ITEMS", ValidationSeverity.MEDIUM, "structure", "_validate_no_empty_list_items", lambda ctx: f"Found empty list items in sections: {ctx._cache.get('STRUCTURE_NO_EMPTY_LIST_ITEMS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(_mk_method("STRUCTURE_MARKDOWN_HEADER_SPACING", ValidationSeverity.MEDIUM, "structure", "_validate_markdown_header_spacing", lambda ctx: f"Found markdown headers with missing spaces: {ctx._cache.get('STRUCTURE_MARKDOWN_HEADER_SPACING', {}).get('violations', 'N/A')}"))
