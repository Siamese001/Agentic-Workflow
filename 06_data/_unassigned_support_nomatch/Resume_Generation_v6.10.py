...
- Match metrics and achievements to job requirements"""
    ...
    def _generate_k5b_overview(
        ...
        unify_exp = next((exp for exp in self.master_resume['professional_experience'] if 'Unify' in exp['company']), None)
        return unify_exp['overview'] if unify_exp else ""
...
4. Return ONLY the 6 selected bullets, one per line, no numbers"""
    ...
    def _generate_k7b_ey_overview(
        ...
        ey_exp = next((exp for exp in self.master_resume['professional_experience'] if 'Ernst & Young' in exp['company'] or 'EY' in exp['company']), None)
