"""
Resume Generation Engine v5.38 - REAL CLAUDE API INTEGRATION

v5.38 CHANGES FROM v5.37 - CRITICAL API FIX:
✓ FIXED: _call_claude_api now makes REAL API calls to Anthropic Claude API
  - Replaced hardcoded mock responses with actual HTTP requests
  - Uses claude-sonnet-4-20250514 model
  - Implements retry logic with exponential backoff
  - Adds proper error handling and timeout management
  - Requires ANTHROPIC_API_KEY environment variable
✓ FIXED: Generated content now unique per job description (not hardcoded)
  - Executive summary, headline, bullets, etc. now dynamically generated
  - Content tailored to actual JD requirements (e.g., DataRobot VP Pre-Sales)
  - No more "Amit Ayer" or "Acme Corp" hardcoded responses
...
TESTING IMPACT:
- All E2E test cases now validate against REAL Claude API responses
- Content uniqueness verified via MD5 hash comparison
- DataRobot JD properly parsed and used in prompt generation
- Complete workflow executes with live API integration

v5.37 CHANGES:
✓ Fixed input pipeline: DataRobot JD correctly parsed at HOP-0
✓ Live prompts created with JD-specific requirements
✓ (BUG) _call_claude_api still mocked - FIXED in v5.38
...
# v5.38: REMOVED hardcoded fallback that was overwriting loaded master resume
# Master resume is now loaded ONLY from file at line 1053
# No fallback - fails fast if Master_Resume_V2_15.json not found
        {
            "degree": "MBA",
...
class ArtistGenerator:
    ...
    def _call_claude_api(
        ...
        v5.38: FIXED - Now makes real API calls instead of returning mock data.
...
"""
        ...
        for i, exp in enumerate(master_experience[:5]):  # Render first 5 experiences
            if i < len(staging_keys):
                ...
                md += f"""#### {exp.get('company', '')}
**{exp.get('title', '')}** | {exp.get('location', '')} | {exp.get('start_date', '')} - {exp.get('end_date', '')}

{staging_buffer.get(overview_key, '')}
...
"""
    ...
    def _render_word_table(self, staging_buffer: ImmutableStagingBuffer) -> Dict:
        """
        Render word count table with ACTUAL and BASELINE word counts.
        ...
        sections.append({
            ...
            "within_tolerance": abs(total_variance_pct) <= 20
        })
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict):
        ...
        # v5.38: Validate API key is set before workflow starts
        if not os.environ.get("ANTHROPIC_API_KEY"):
            logger.warning(
                "=" * 80 + "\n" +
                "WARNING: ANTHROPIC_API_KEY environment variable not set!\n" +
                "The workflow will fail at HOP-3 (Artist Generation).\n" +
    ...
    def execute_workflow(
        ...
        print("=" * 80)
        print("RESUME GENERATION ENGINE v5.38 - REAL CLAUDE API")
        print("=" * 80)
