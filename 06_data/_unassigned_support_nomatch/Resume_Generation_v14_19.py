...
__version__ = "14.19" # <-- Increment version for tracking changes

# Third-party - Google AI (conditional)
try:
...
class ReasoningConfig:
    ...
    # --- Corrected ClassVar Declarations ---
    ...
    K2_UNIFY_BULLETS_CONFIG: ClassVar['ReasoningConfig']      # Was K5_UNIFY_BULLETS_CONFIG
    K2_UNIFY_OVERVIEW_CONFIG: ClassVar['ReasoningConfig']     # Was K5_UNIFY_OVERVIEW_CONFIG
    K3_IBM_BULLETS_CONFIG: ClassVar['ReasoningConfig']        # Was K6_IBM_BULLETS_CONFIG
    K3_IBM_OVERVIEW_CONFIG: ClassVar['ReasoningConfig']       # Was K6_IBM_OVERVIEW_CONFIG
    K4_TRADERSENSE_NARRATIVE_CONFIG: ClassVar['ReasoningConfig'] # New or Renamed
    K5_EY_NARRATIVE_CONFIG: ClassVar['ReasoningConfig']       # Was K8_EY... or New
    K6_EARLY_CAREER_NARRATIVE_CONFIG: ClassVar['ReasoningConfig'] # Was K9_EARLY_CAREER... or New
    # K7_EDUCATION - Typically no complex reasoning needed
    # K8_CERTIFICATIONS - Typically no complex reasoning needed
    K9_COMPETENCIES_CONFIG: ClassVar['ReasoningConfig']   # Was K10_COMPETENCIES_CONFIG
    K10_SKILLS_CONFIG: ClassVar['ReasoningConfig']        # Was K2_SKILLS_CONFIG
    K11_COVER_LETTER_CONFIG: ClassVar['ReasoningConfig']    # Added for Cover Letter
...
# --- NEW: Instantiate the intensity config ---
REASONING_INTENSITY_CONFIG = ReasoningIntensityConfig()

# --- NEW: Instantiate the prompt addendum config ---
...
def _calculate_reasoning_intensity(params: Dict) -> Tuple[float, str]:
    ...
    # --- START REFACTOR: Use configurable weights and thresholds ---
...
class WebSearchRAG:
    ...
    def __init__(self, client: GeminiWebSearchClient, config: RAGConfig = RAGConfig()):
        ...
        # --- NEW: Configurable Peer Companies ---
...
class EnhancedJobDescriptionAnalyzer:
    ...
    def __init__(
        ...
        # --- NEW: Configurable Industry Keywords ---
        self.INDUSTRY_KEYWORDS = {
            "Financial Technology": ['fintech', 'banking'],
            "Healthcare": ['healthcare', 'medical'],
            "Retail/E-Commerce": ['retail', 'e-commerce'],
            "Software/SaaS": ['saas', 'software'],
        }
...
"""
    ...
    def _classify_role(self, keywords: Dict[str, int], jd_text: str) -> Dict:
        ...
        # --- START REFACTOR: Infer industry from configurable list ---
...
# PASTE THIS AT LINE ~5650 (BEFORE _count_sentences function)
# ============================================================================
...
def _count_sentences(text: str) -> int:
    """
    DEPRECATED: Use text_utils.count_sentences() instead.
    This alias maintained for backward compatibility.
    """
    return text_utils.count_sentences(text)
...
# ============================================================================
# Validation Context (HELPER FOR HOP-5 - REFACTORED FOR NEW SIMILARITY RULES)
# ============================================================================
...
class ValidationContext:
    ...
    # ========================================================================
    # VALIDATION RULE HELPER FUNCTIONS - Reduce repetition
    # ========================================================================
    ...
    # --- Calculation Methods (Updated for K0-K11 Enums and Similarity) ---
    ...
    # --- START: New Method for Narrative vs Master Similarity Calculation ---
...
# ============================================================================
# HOP-5: VALIDATION GATES (FINAL - Redundant Check Removed)
# ============================================================================
# Assume ValidationContext, ValidationRule, ValidationEngine, ValidationSeverity,
# ContentConstraintsConfig, SignalControlConfig, ResumeSection,
# ArtistGenerator, DuplicateDetector are defined/imported correctly.
...
class PreFlightValidator:
    ...
    # Bullet word count sections to check (Unchanged)
    ...
    PROVENANCE_SPLIT_TARGETS = ArtistGenerator.PROVENANCE_SPLIT_TARGETS
    ...
    # Required sections for presence validation (Unchanged)
    REQUIRED_SECTIONS = [
    ...
    # --- RULES_CONFIG WITH NEW SIMILARITY RULES ---
    RULES_CONFIG = [
        # --- RAG Quality ---
        {
        ...
        # --- Word Count & Sentence Count Rules (Unchanged from previous final version) ---
        {
         ...
         { # K.1 Sentences
            ...
            "validator": lambda ctx: ctx.k1_sentence_count_details['min'] <= ctx.k1_sentence_count_details['sentence_count'] <= ctx.k1_sentence_count_details['max'],
        ...
        { # K.1 Words
            ...
            "validator": lambda ctx: ctx.k1_word_count_details['min'] <= ctx.k1_word_count_details['word_count'] <= ctx.k1_word_count_details['max'],
        ...
        { # K.2 Unify Overview Words
            ...
            "validator": lambda ctx: ctx.k2_overview_details['min_wc'] <= ctx.k2_overview_details['word_count'] <= ctx.k2_overview_details['max_wc'],
        ...
        { # K.2 Unify Overview Sentences
            ...
            "validator": lambda ctx: ctx.k2_overview_details['min_sc'] <= ctx.k2_overview_details['sentence_count'] <= ctx.k2_overview_details['max_sc'],
        ...
        { # K.3 IBM Overview Words
            ...
            "validator": lambda ctx: ctx.k3_overview_details['min_wc'] <= ctx.k3_overview_details['word_count'] <= ctx.k3_overview_details['max_wc'],
        ...
        { # K.3 IBM Overview Sentences
            ...
            "validator": lambda ctx: ctx.k3_overview_details['min_sc'] <= ctx.k3_overview_details['sentence_count'] <= ctx.k3_overview_details['max_sc'],
        ...
        # --- Narrative Block Rules (Unchanged from previous final version) ---
        { # K.4 Narrative Words
            "rule_id": "VG_NARRATIVE_WORD_COUNT_K4", "severity": ValidationSeverity.HIGH, "category": "word_count",
            "validator": lambda ctx: ctx.k4_narrative_details['min_wc'] <= ctx.k4_narrative_details['word_count'] <= ctx.k4_narrative_details['max_wc'],
        ...
        { # K.4 Narrative Sentences
            ...
            "error_message": lambda ctx: f"K.4 TraderSense Narrative: {ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K4']['sentence_count']} sentences (target range: {ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K4']['target_sc']-1}-{ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K4']['target_sc']+1})"
        },
        { # K.5 Narrative Words
            "rule_id": "VG_NARRATIVE_WORD_COUNT_K5", "severity": ValidationSeverity.HIGH, "category": "word_count",
            "validator": lambda ctx: ctx.k5_narrative_details['min_wc'] <= ctx.k5_narrative_details['word_count'] <= ctx.k5_narrative_details['max_wc'],
        ...
        { # K.5 Narrative Sentences
            ...
            "error_message": lambda ctx: f"K.5 EY Narrative: {ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K5']['sentence_count']} sentences (target range: {ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K5']['target_sc']-1}-{ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K5']['target_sc']+1})"
        },
        { # K.6 Narrative Words
            "rule_id": "VG_NARRATIVE_WORD_COUNT_K6", "severity": ValidationSeverity.HIGH, "category": "word_count",
            "validator": lambda ctx: ctx.k6_narrative_details['min_wc'] <= ctx.k6_narrative_details['word_count'] <= ctx.k6_narrative_details['max_wc'],
        ...
        { # K.6 Narrative Sentences
            ...
            "error_message": lambda ctx: f"K.6 Early Career Narrative: {ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K6']['sentence_count']} sentences (target range: {ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K6']['target_sc']-1}-{ctx._cache['VG_NARRATIVE_SENTENCE_COUNT_K6']['target_sc']+1})"
        },
        { # Bullet WC Range
        ...
        # --- Distribution Rules (K.2/K.3) (Unchanged from previous final version) ---
        {
        ...
        # --- Structure & Formatting Rules (Unchanged from previous final version) ---
        {
        ...
        },
        {
            ...
            "validator": lambda ctx: ',' not in ctx.headline_details.get('headline', ''),
        ...
        # --- Content & Signal Rules (Unchanged from previous final version) ---
        {
        ...
        },
        {
            ...
            "validator": "_validate_no_intro_phrases",
        ...
        },
        {
            ...
            "validator": "_validate_per_section_signal_raw",
        ...
        },
        {
            ...
            "validator": lambda ctx: ctx._calculate_cover_letter_narrative(ctx)['valid'],
        ...
        # --- NEW Cross Section Similarity Rule ---
        {
            "rule_id": "VG_CROSS_SECTION_SIMILARITY",
            "severity": ValidationSeverity.HIGH, # Set severity to HIGH to trigger HALT
            "category": "content",
            "validator": "_validate_cross_section_similarity", # New method name
            "error_message": lambda ctx: f"High similarity (>=0.65) found between sections: {'; '.join(ctx._cache.get('VG_CROSS_SECTION_SIMILARITY', {}).get('failures', []))}"
        },
        # --- NEW Narrative vs Master Similarity Rule ---
        {
            "rule_id": "VG_NARRATIVE_VS_MASTER_SIMILARITY",
            "severity": ValidationSeverity.HIGH, # Set severity to HIGH to trigger HALT
            "category": "content",
            "validator": "_validate_narrative_vs_master_similarity", # New method name
            "error_message": lambda ctx: f"Narrative similarity to master highlights outside range (0.40-0.70): {'; '.join(ctx._cache.get('VG_NARRATIVE_VS_MASTER_SIMILARITY', {}).get('failures', []))}"
        },
    ]
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        rule_map = {
            ...
            "VG_CROSS_SECTION_SIMILARITY": "GLOBAL", # NEW
            "VG_NARRATIVE_VS_MASTER_SIMILARITY": "GLOBAL", # NEW
            # Visual Rules
    ...
    # --- NEW: Cross-Section Similarity Validator ---
    ...
    # --- NEW: Narrative vs Master Similarity Validator ---
    ...
    def _validate_headline_format_no_titles(self, context: ValidationContext) -> bool:
        """Validates headline structure AND checks for forbidden titles."""
        ...
        if sections_under_test:
            ...
            for rule_id, section_map in self.RULE_TO_SECTION_MAP.items():
                if section_map == "GLOBAL" or section_map == "VISUAL" or section_map in sections_under_test:
                    ...
                    # Narrative vs Master affects K4, K5, K6
...
class GateDecisionEngine:
    ...
    def decide(
        ...
        # Decision logic
        if len(critical_failures) > 0:
            return (GateDecision.HALT, f"HALT: {len(critical_failures)} CRITICAL failures detected")
        elif len(high_failures) > 0:
            return (GateDecision.HALT, f"HALT: {len(high_failures)} HIGH severity failures detected (zero tolerance)")
        else:
            return (GateDecision.PROCEED, "PROCEED: All validations passed")

# ============================================================================
# WORKFLOW ORCHESTRATOR (STATEFUL RETRY VERSION W/ REFACTORED QA & HOP-5 SIMILARITY)
# ============================================================================
# Assume necessary classes are defined/imported:
# EnhancedJobDescriptionAnalyzer, ClerkExtractor, DataEnricher, ArtistGenerator,
# ImmutableStagingBuffer, TextSanitizer, PreFlightValidator, GateDecisionEngine,
# FileRenderer, QAReportGenerator, HopCheckpoint, HopStatus, GateDecision,
# ValidationResult, ValidationSeverity, ResumeSection, ThematicAnalysis, RAGConfig,
# ContentConstraintsConfig, ARTIST_SPECS_DATA, MASTER_RESUME_DATA, HopExecutionError
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict, test_mode: bool = False):
        ...
        # --- REMOVED HOP-7.5 related attributes ---
        # self.dup_detector = None # dup_detector is now created on-demand if needed
    ...
    # --- HOP Methods (_execute_hop_0 to _execute_hop_2 remain the same) ---
    ...
    def _handle_workflow_termination(
        ...
        qa_gen_results, qa_report_text, final_file_contents = [], "[QA Report Generation Failed Post-Error]", {}
    ...
    def execute_workflow(
        ...
        try:
            # --- Execute Core Workflow Steps (Helper 1) ---
            (
                thematic_analysis, staging_buffer, hop5_results, file_paths, file_contents,
                total_api_calls, gate_decision
            ...
            # --- Handle Controlled Workflow Halts (Helper 2) ---
            ...
            # --- Handle Unexpected Workflow Failures (Helper 2) ---
            final_val_results_for_error = self.validation_results if self.validation_results else hop5_results
            return self._handle_workflow_termination(
    ...
    # --- Helper Methods (_create_jd_analyzer, _create_checkpoint, _check_hop_status, _build_coc_ledger) ---
    # These remain unchanged from the previous version provided, including the CoC ledger logic.
    # Make sure they are correctly defined within the class scope.
    ...
    def _create_checkpoint( self, hop_id: str, hop_name: str, validation_results: List[ValidationResult], output_data: Any, start_time: datetime, metadata: Optional[Dict[str, Any]] = None, error_message: Optional[str] = None ) -> HopCheckpoint:
        ...
        output_hash = None
        # Calculate output hash if possible
        if output_data is not None:
            try:
                ...
                def default_serializer(o):
                    ...
                    # Check if directly serializable before giving up
                    try:
                        json.dumps(o)
                        return o
                    except TypeError:
                ...
                if isinstance(output_data, ImmutableStagingBuffer):
                    serializable_output = output_data.data
                     ...
                     # Ensure ThematicAnalysis and nested objects are serializable
        ...
        # Ensure final_temperatures values are basic types
        ...
        # Create the HopCheckpoint object
        # Ensure HopCheckpoint is defined/imported
    ...
    def _build_coc_ledger( self, workflow_start: datetime, workflow_end: datetime, thematic_analysis: Optional[ThematicAnalysis], total_api_calls: int ) -> Dict:
        ...
        for hc in self.hop_checkpoints:
             try:
                 ...
                 # Convert severity enums within validation results
                 ...
                 # Append minimal info on serialization failure
...
# ============================================================================
# QA REPORT GENERATOR CLASS (FINAL CORRECTED v14.11 Structure)
# ============================================================================
import textwrap
import re
import logging
...
class QAReportGenerator:
    ...
    def _build_qa_section_1_signal_quality(
        ...
        for label, (section_enum, target_min_raw, target_max_raw, weight, _) in raw_target_config.items():
            ...
            lines.append(self._format_ascii_bar_chart(label, raw_score, target_min_raw, target_max_raw, temp_used_str, score_display_mode="raw"))
            total_weighted_raw_score += raw_score * weight; total_section_weight += weight; total_weighted_min_target += target_min_raw * weight; total_weighted_max_target += target_max_raw * weight
        ...
        lines.append("-" * 95)
        lines.append(self._format_ascii_bar_chart("Weighted Average", avg_raw_score, avg_min_target, avg_max_target, avg_temp_val_str, is_summary=True, score_display_mode="raw"))
        lines.append("-" * 95)

        # --- Skills Score (Informational) ---
    ...
    # Section 6: Content Authenticity (Specific Method)
    ...
    # Section 8: Similarity Summary (Relies ONLY on HOP-5 Validation Results)
    ...
    # Section 9: Structural Validation (Specific Method)
    ...
    # Section 10: Cover Letter QA (Specific Method)
    def _build_qa_section_10_cover_letter_qa(
        self,
        validation_results: List[ValidationResult]
    ) -> List[str]:
        title = "**10. COVER LETTER QA**" # Updated title
        description = "Specific validation checks for the generated Cover Letter content, structure, and relevance."
        # Include all rules containing "COVER_LETTER"
        rule_id_patterns = ["COVER_LETTER"]
        lines = ["", title, "", description, ""]
        lines.append("```markdown")
        headers = ["Rule ID", "Status", "Message / Details"]
        rows = []
        ...
        if filtered_results:
            ...
            for vr in filtered_results:
                ...
                details_str = ""
                # Add details snippet on failure
                if not vr.passed and details:
                    try:
                        details_json = json.dumps(details, indent=None, ensure_ascii=False)
                        details_str = f" Details: {details_json}"[:150] # Truncate
    ...
    # Section 11: JD Enforcement (Renumbered)
    ...
    # Section 12: Final Format (Renumbered)
    ...
    # --- Helper: ASCII Bar Chart Formatter (Assumed to be correct from previous versions) ---
    def _format_ascii_bar_chart(self, label: str, value: float, target_min: Optional[float], target_max: Optional[float], temperature: Optional[Union[float, str]] = None, bar_length: int = 10, is_summary: bool = False, score_display_mode: str = "percentage", status_override: Optional[str] = None) -> str:
        # Clamp value between 0.0 and 1.0 for bar calculation, but display actual value
        ...
        if status_override:
            ...
            status = "INFO" # Default if no targets or value
        ...
        if isinstance(temperature, (float, int)):
            ...
            temp_str = f"({temperature})" # Use string directly if it's not N/A
        ...
        total_width = 95 # Target total width for alignment
        # Format parts with fixed widths
        label_formatted = f"{label:<{label_width}}"[:label_width] # Truncate if needed
        bar_formatted = f"[{bar}]"
        score_formatted = f"{score_text:<{score_width}}"
        target_formatted = f"{target_str:<{target_width}}"
        status_formatted = f"{status:<{status_width}}"
        temp_formatted = f"{temp_str:<{temp_width}}"

        # Combine parts and add padding if necessary (though fixed widths should handle alignment)
        line = f"{label_formatted} {bar_formatted} {score_formatted} {target_formatted} {status_formatted} {temp_formatted}"
        return line.rstrip() # Remove potential trailing whitespace
