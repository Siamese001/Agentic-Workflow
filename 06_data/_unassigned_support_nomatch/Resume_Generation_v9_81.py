...
__version__ = "9.80" # Final Hardening: Corrected Competencies formatting and fixed QA Table 16 rendering.
...
def _build_qa_section_6_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        # Define the sections to process with all required data
        sections_to_process = [
            {
                "name": "Unify",
                "overview_enum": ResumeSection.K5_UNIFY_OVERVIEW,
                "bullets_enum": ResumeSection.K5_UNIFY_BULLETS,
                "avg_bullet_len": master_avg_lengths.get("Unify", 25.0),
                "overview_range": (25, 40) # (min, max) from _generate_k5_unify_overview
            },
            {
                "name": "IBM",
                "overview_enum": ResumeSection.K6_IBM_OVERVIEW,
                "bullets_enum": ResumeSection.K6_IBM_BULLETS,
                "avg_bullet_len": master_avg_lengths.get("IBM", 25.0),
                "overview_range": (25, 35) # from _generate_k6_ibm_overview
            },
            {
                "name": "EY",
                "overview_enum": ResumeSection.K8_EY_OVERVIEW,
                "bullets_enum": ResumeSection.K8_EY_BULLETS,
                "avg_bullet_len": 25.0, # Default
                "overview_range": (25, 35) # from _generate_k8_ey_overview
            },
            {
                "name": "EarlyCareer",
                "overview_enum": ResumeSection.K9_EARLY_CAREER_OVERVIEW,
                "bullets_enum": ResumeSection.K9_EARLY_CAREER_BULLETS,
                "avg_bullet_len": 25.0, # Default
                "overview_range": (18, 30) # from _generate_k9_early_career_overview
            },
            {
                "name": "Competencies",
                "overview_enum": None, # No overview
                "bullets_enum": ResumeSection.K10_COMPETENCIES,
                "avg_bullet_len": master_avg_lengths.get("Competencies", 28.0),
                "overview_range": None
            }
        ]
        ...
        for section_data in sections_to_process:
            ...
            avg_len = section_data["avg_bullet_len"]
            bullets = staging_buffer.get(bullets_enum.value, [])

            if isinstance(bullets, list) and bullets:
                # Check if list items are dicts (expected) or strings (old format)
                is_dict_list = isinstance(bullets[0], dict)
                ...
                for i, bullet_item in enumerate(bullets):
                    ...
                    else: # Fallback for simple string list
