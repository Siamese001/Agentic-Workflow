...
class ArtistGenerator:
    ...
    # Define base starting temperatures (Not strictly needed if schedules cover attempt 1)
    # Define temperature schedules based on attempt number (index 0 = attempt 1, index 1 = attempt 2, etc.)
    TEMPERATURE_SCHEDULES: ClassVar[Dict[ResumeSection, List[float]]] = {
        # K.1 Executive Summary Schedule
        ResumeSection.K1_EXECUTIVE_SUMMARY: [1.0, 0.9, 0.8, 0.7, 0.6],
        # K.13 Cover Letter Schedule
        ResumeSection.K13_COVER_LETTER: [1.0, 0.8, 0.6, 0.4, 0.2],
        # Default schedule for all other sections
        **{section: [0.8, 0.6, 0.4, 0.2, 0.1] for section in ResumeSection if section not in [
            ...
            # Exclude sections that are copied or placeholders
            # Assuming these don't need a schedule:
            # ResumeSection.K0_NAME, ResumeSection.K0_CONTACT,
            # ResumeSection.K7_TRADERSENSE_BULLETS, ResumeSection.K7_TRADERSENSE_OVERVIEW,
    ...
    ARTIST_GENERATION_CONFIG = [
        ...
        # Sections generated by LLM (will use temperature schedules)
    ...
    def generate(
        ...
        Args:
            ...
            temperature_overrides: Dictionary mapping ResumeSection to specific
        ...
        try:
            # Pass the overrides dictionary to the internal generation method
            artist_output = self._generate_artist_output(
                attempt=attempt, # Pass attempt number for default temp calculation
            ...
            # Catch errors during generation itself (e.g., K13 raising HopExecutionError)
            logging.error(f"Artist generation failed during attempt {attempt}: {e}", exc_info=True)
            validation_results.append(ValidationResult(
            ...
            # Return empty dict on hard failure
            return {}, validation_results

    def _generate_artist_output(
        self,
        attempt: int, # Need attempt number to calculate default temps
        temperature_overrides: Optional[Dict[ResumeSection, float]] = None
        ) -> Dict:
        """
        Generate complete artist output with all K.X sections.
        Determines the correct temperature for each section based on overrides
        or the defined schedules and the current attempt number.
        """
        output = {}
        # Ensure overrides is a dict, even if empty
        ...
        for config in self.ARTIST_GENERATION_CONFIG:
            ...
            if method_name.startswith("_copy_") or method_name == "_generate_dummy_header":
                 try:
                     ...
                     logging.error(f"Error calling copy/dummy method {method_name} for {section_enum.value}: {e}", exc_info=True)
                     output[section_enum.value] = f"[Placeholder for {section_enum.value} due to error: {e}]"
                 continue # Move to next section in config
            ...
            if section_enum in effective_overrides:
                # Use the override if this section failed previously
                final_temp = effective_overrides[section_enum]
                logging.info(f"  {section_enum.name}: Applying temperature override: {final_temp:.1f}")
            elif schedule:
                # Use the scheduled temperature for the current attempt
                # Get temp based on attempt (adjust index, clamp to schedule length)
                ...
                # Fallback if section somehow missing from schedule (shouldn't happen for generated sections)
                final_temp = 0.5 # Safe default
                logging.warning(f"  {section_enum.name}: Temperature schedule not found! Using default {final_temp:.1f}")

            # --- Call the generation method with the determined temperature ---
            try:
                method = getattr(self, method_name)
                # Pass the final determined temperature to the specific method
                output[section_enum.value] = method(temperature_override=final_temp)

            except HopExecutionError as he:
                 # Catch specific errors raised by methods (like K13)
                 logging.error(f"Generation HALTED for section {section_enum.value} with {method_name} (Temp: {final_temp}): {he}", exc_info=False) # No need for full stack trace here
                 output[section_enum.value] = f"[Placeholder for {section_enum.value} due to HALT: {he}]"
                 # Re-raise the error to halt the entire HOP-3 generation process
                 raise he
            except Exception as e:
                 # Catch other unexpected errors during generation
                 logging.error(f"Error generating section {section_enum.value} with {method_name} (Temp: {final_temp}): {e}", exc_info=True)
                 output[section_enum.value] = f"[Placeholder for {section_enum.value} due to generation error: {e}]"
                 # Optionally re-raise here too if any generation error should halt everything
                 # raise HopExecutionError(f"Unexpected error during {section_enum.value} generation: {e}") from e


        # Ensure all sections exist in the output, even if None or Placeholder
    ...
    # --- LLM Call Helper (Assuming _call_gemini_api exists and handles temp override) ---
    ...
    #    ... (Implementation provided previously, ensure it correctly uses temperature_override)

    # --- LLM Generation Methods (Accepting temperature_override) ---

    def _generate_k0_headline(self, temperature_override: Optional[float] = None) -> str:
        """Generates K.0 Headline. Accepts temperature_override."""
        # ... (Get signals: current_title, primary_theme, differentiators, role_level, target_role_title, auth_tone_example)
        ...
        differentiators = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:3]
        ...
        feedback_instruction = "" # Add feedback logic if needed for this section
        # ... (Build prompt including CRITICAL INSTRUCTIONS and word count constraint)
        prompt = f"""{feedback_instruction}Create a professional resume headline...
...
**Instructions:**
1. ...position...'{primary_theme}'.
2. **DO NOT** include formal job titles...
3. Include 2-3 relevant keywords... {', '.join(differentiators)}.
4. Format: "[Functional Expertise Area] | [Key Strength 1] | [Key Strength 2]"
5. **DO NOT** use commas.
6. **ABSOLUTELY CRITICAL:** The headline MUST be **strictly between {self.constraints.HEADLINE_WORD_COUNT_MIN} and {self.constraints.HEADLINE_WORD_COUNT_MAX} words** total...
7. ...professional and confident... **Authentic Tone Example**.
**CRITICAL INSTRUCTION FOR THIS JOB:** ... POST-SALES ... ADOPTION, RETENTION, EXPANSION ...
**Good Example Format:** "AI Technical Success Leadership | Post-Sales Strategy | GenAI Adoption & Scalability"
8. Return ONLY the headline text...
"""
        ...
        headline = self._call_gemini_api(
            ...
            temperature_override=temperature_override # Pass temperature
        )
        ...
        if "[Placeholder" in headline or not (self.constraints.HEADLINE_WORD_COUNT_MIN <= word_count <= self.constraints.HEADLINE_WORD_COUNT_MAX):
            logging.warning(f"K.0 Headline failed validation (words: {word_count}). Using fallback.")
            # ... (Fallback generation logic) ...
            fb_theme = primary_theme if primary_theme != 'AI Leadership' else "AI Technical Success"
            ...
            fallback_headline = f"{fb_theme} Leadership | {fb_diff1} | {fb_diff2}" # Trim if needed...
            return fallback_headline # Simplified fallback return

        return headline

    def _generate_k1_executive_summary(self, temperature_override: Optional[float] = None) -> str:
        """Generates K.1 Executive Summary. Accepts temperature_override."""
        # ... (Get signals: top_bullets, primary_theme, secondary_themes, differentiators, role_archetype, auth_patterns)
        top_bullets = [] # Get from master_resume
        # ... (signal extraction logic) ...
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'AI Leadership')
        secondary_themes = [t.get('name', '') for t in self.thematic_analysis.secondary_themes[:3] if t.get('name')]
        differentiators = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:5]
        role_archetype = self.thematic_analysis.role_classification.get('role_archetype', 'Technical_IC')
        auth_patterns_data = self.thematic_analysis.authenticity_patterns or {}
        # ... (auth pattern extraction) ...

        # ... (Build archetype_instruction) ...
        archetype_instruction = "" # Based on role_archetype

        # ... (Build feedback_instruction based on self.previous_failures for VG_SENTENCE_COUNT_K1) ...
        feedback_instruction = ""
        if self.previous_failures:
            # ... (logic to find VG_SENTENCE_COUNT_K1 failure message) ...
            ...
            if sentence_fail:
                 ...
                 feedback_instruction = f"\n**CRITICAL FEEDBACK:** Previous summary FAILED sentence count ({msg}). MUST be {self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MIN}-{self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MAX} sentences.\n\n"

        # ... (Build prompt including NON-NEGOTIABLE sentence count) ...
        prompt = f"""{feedback_instruction}You are an expert resume writer...
...
**TASK:**
1. Write a {self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MIN}-{self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MAX} sentence summary...
# ... (rest of prompt) ...
"""
    ...
    def _generate_k2_skills(self, temperature_override: Optional[float] = None) -> List[str]:
        """Generates K.2 Skills. Accepts temperature_override."""
        # ... (Get signals: primary_theme, secondary_themes, differentiators) ...
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'AI Leadership')
        secondary_themes = [t.get('name', '') for t in self.thematic_analysis.secondary_themes[:4]]
        differentiators = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:10]

        # ... (Build prompt for 12 skills, 1-3 words each) ...
        prompt = f"""You are an expert HR data analyst... extract the 12 most critical skills... 1-3 word keywords...
...
**TASK:**
1. Identify the **12 most important skills**...
2. ... 90%+ signal...
3. Format **each** skill as a 1-3 word keyword...
4. ... common terms found in HR recruiting databases.
5. Return **only** the 12 skills, each on a new line.
6. Do NOT add bullets, numbers, or any other commentary...
"""
        ...
        # ... (Parse and validate output, return list or error string) ...
        if "[Placeholder" in skills_text: return [f"Error: LLM failed. Details: {skills_text}"]
        # (Parsing logic using re.split and word count check)
        skills_list_final = [] # Populate this list
        # ... (parsing loop) ...
        if len(skills_list_final) < 10: return [f"Error: Failed generation. Output: {skills_text}"]
    ...
    def _generate_tailored_overview_for_experience(
        ...
        """Generates tailored overviews. Accepts temperature_override."""
        # ... (Find source_overview) ...
        ...
        if not source_overview: return ""

        # ... (Get signals: primary_theme, secondary_themes, differentiators, auth_tone_example) ...
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'AI Leadership')
        secondary_themes = [t.get('name', '') for t in self.thematic_analysis.secondary_themes[:2] if t.get('name')]
        differentiators = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:3]
        # ... (auth tone extraction) ...

        min_wc, max_wc = word_count_range
        feedback_instruction = "" # Add feedback logic if needed
        # ... (Build prompt including CRITICAL word count constraint) ...
        prompt = f"""{feedback_instruction}You are an expert resume writer. Rewrite the overview...
**Original Overview:**
{source_overview}
...
- Secondary Themes: {', '.join(secondary_themes)}
- Non-Negotiable Keywords: {', '.join(differentiators)}
**Authentic Tone Guide:** "{auth_tone_example}"
**Instructions:**
1. Rewrite... align with themes.
2. **Subtly** emphasize...
3. **PRIORITY:** Maintain authentic voice...
4. **AVOID keyword stuffing.**
5. DO NOT invent new facts...
6. **ABSOLUTELY CRITICAL:** MUST be strictly between **{min_wc} and {max_wc} words**...
{extra_prompt_instruction}
7. Return ONLY the rewritten overview...
"""
        ...
        if "[Placeholder" in tailored_overview or not (min_wc <= word_count <= max_wc):
            logging.warning(f"{section_id} overview failed validation (words: {word_count}). Using original.")
    ...
    def _generate_k8_ey_overview(self, temperature_override: Optional[float] = None) -> str:
        return self._generate_tailored_overview_for_experience(
            ...
            extra_prompt_instruction="**CRITICAL:** Do NOT mention specific company names from early career...",
            temperature_override=temperature_override
        )
    ...
    def _generate_k9_early_career_overview(self, temperature_override: Optional[float] = None) -> str:
        return self._generate_tailored_overview_for_experience(
            ...
            extra_prompt_instruction="**CRITICAL:** Do NOT mention 'Ernst & Young' or 'EY'...",
            temperature_override=temperature_override
        )

    # --- Bullet Generation Methods ---
    ...
    def _validate_llm_bullet_selection(self, selected_bullets_text: List[str], master_bullets_structured: List[Dict], expected_count: int, section_id: str) -> List[Dict]:
        ...
        # Returns list of original dicts from master_bullets_structured that match selected_bullets_text
        # Falls back to master_bullets_structured[:expected_count] if validation fails
        if len(selected_bullets_text) != expected_count:
            ...
            return [{'bullet_text': b['bullet_text'], **b} for b in master_bullets_structured[:expected_count]] # Ensure consistent structure
        validated_bullets = []
        ...
        for selected_text in selected_bullets_text:
            if selected_text in master_texts:
                ...
                if original_bullet:
                    validated_bullets.append(original_bullet) # Append original dict
            else:
                logging.warning(f"{section_id} LLM returned unknown bullet: '{selected_text[:50]}...'. Skipping.")
        if len(validated_bullets) != expected_count:
             logging.warning(f"{section_id} failed validation. Using original {expected_count} bullets.")
             return [{'bullet_text': b['bullet_text'], **b} for b in master_bullets_structured[:expected_count]] # Ensure consistent structure
    ...
    def _rewrite_bullet_for_word_count(self, original_bullet_text: str, target_word_count_range: Tuple[int, int], section_id: str, temperature_override: Optional[float] = None) -> str:
        """Rewrites a bullet for word count. Accepts temperature_override."""
        # ... (Get signals: auth_verbs) ...
        auth_patterns_data = self.thematic_analysis.authenticity_patterns or {}
        auth_patterns = auth_patterns_data.get('patterns', {})
        auth_verbs = auth_patterns.get('achievement_verb_patterns', [])

        min_wc, max_wc = target_word_count_range
        prompt = f"""You are a concise resume editor... rewrite... ({min_wc}-{max_wc} words)...
**Original Bullet:**
"{original_bullet_text}"
**Authentic Voice Guide:**
- Use strong action verbs like: {', '.join(auth_verbs[:10])}
**Instructions:**
1. **ABSOLUTELY CRITICAL:** Rewrite... strictly between **{min_wc} and {max_wc} words**...
2. **CRITICAL:** DO NOT change... facts, metrics...
3. Maintain authentic, professional tone...
4. If too long, concisely rephrase...
5. If too short, add professional phrasing...
6. Return ONLY the single rewritten bullet...
"""
    ...
    def _validate_and_potentially_rewrite_bullets(
        ...
        temperature_override: Optional[float] = None # Pass temp for rewrite
    ) -> List[Dict]:
        """
        [v12.04 REWRITE] Checks word count, attempts rewrite if needed, falls back to original generated.
        """
        final_bullets = []
        ...
        for bullet_data in selected_bullets_structured:
            if not isinstance(bullet_data, dict): continue # Skip non-dicts
            ...
            if not original_text: continue # Skip empty

            if not (min_target <= word_count <= max_target):
                logging.warning(f"  Word count ({word_count}) outside target ({min_target}-{max_target}) for {section_id_for_logging}. Rewriting...")
                rewritten_text = self._rewrite_bullet_for_word_count(
                    ...
                    section_id=f"{section_id_for_logging}_RewriteWC",
                    temperature_override=temperature_override # Pass temp for rewrite
                )
                ...
                if rewritten_text != original_text and min_target <= rewritten_word_count <= max_target:
                    logging.info(f"    Rewrite SUCCESSFUL. New count: {rewritten_word_count}")
                ...
                else: # Rewrite failed or still outside range
                    status = "Rewrite Failed" if rewritten_text == original_text else "Rewrite Out Of Range"
                    logging.warning(f"    {status} ({rewritten_word_count} words). Keeping original generated bullet.")
                    final_bullets.append({
                        "text": original_text, "provenance": original_provenance,
                        "word_count": word_count, "word_count_status": "OutsideTarget"
                    })
            else: # Within range initially
                final_bullets.append({
                    "text": original_text, "provenance": original_provenance,
    ...
    def _generate_lightly_customized_bullets(
        ...
        """Lightly rewrites bullets. Accepts temperature_override."""
        try:
            if not source_bullets_text: return []
            # ... (Get signals: primary_theme, auth_verbs, auth_metrics) ...
            ...
            prompt = f"""You are an expert resume editor. Lightly rewrite...
**Original Bullets:**
{bullets_text}
**Target Job Theme:** {primary_theme}
**Target Job Description:** {self.job_description[:1000]}
**Authentic Voice Guide:**
- Verbs: {', '.join(auth_verbs[:10])}
- Metrics: {', '.join(auth_metrics[:5])}
**Instructions:**
1. Rewrite each bullet... concise and impactful.
2. **Subtly** emphasize concepts... '{primary_theme}'.
3. **CRITICAL:** Adopt the **Authentic Voice**...
4. **AVOID forcing keywords.**
5. DO NOT invent new facts...
6. Maintain same number of bullets...
7. Return ONLY rewritten bullets, each on a new line, starting with '• '.
"""
            system_prompt = "You are an expert resume editor..."
            ...
            # ... (Fallback and parsing logic remains) ...
            if "[Placeholder" in response_text: # Fallback
                return [{"text": b, "provenance": BulletProvenance.Verbatim.value, "word_count": count_words_ms_word_style(b)} for b in source_bullets_text]
            # (Parse, check count, return list of dicts)
            rewritten_bullets = [line.replace("• ", "").strip() for line in response_text.split('\n') if line.strip()]
            if len(rewritten_bullets) != len(source_bullets_text): # Count check fallback
                 return [{"text": b, "provenance": BulletProvenance.Verbatim.value, "word_count": count_words_ms_word_style(b)} for b in source_bullets_text]
            ...
            logging.warning(f"{section_id} rewrite failed: {e}. Returning original.", exc_info=True)
            return [{"text": b, "provenance": BulletProvenance.Verbatim.value, "word_count": count_words_ms_word_style(b)} for b in source_bullets_text]
    ...
    def _generate_synthetic_bullets(
        ...
        """Generates synthetic bullets. Accepts temperature_override."""
        # ... (Get signals: primary_theme, secondary_themes, differentiators, auth_verbs, auth_metrics) ...
        primary_theme = thematic_analysis.primary_theme.get('name', 'AI Leadership')
...
**Context:**
- Company: {company_name}
- Role Focus: {primary_theme}
- Other achievements: {context_bullets}
**Target Job Analysis:**
- Primary Theme: {primary_theme}
- Secondary Themes: {', '.join(secondary_themes)}
- Target Keywords: {', '.join(differentiators)}
- Target JD: {job_description[:1500]}
**Authentic Voice Guide:**
- Verbs: {', '.join(auth_verbs[:10])}
- Metrics: {', '.join(auth_metrics[:5])}
**Task:**
1. Generate {count} new, distinct bullet points...
2. MUST align with **Primary/Secondary Themes**.
3. Single, concise sentence starting with action verb.
4. MUST adopt **Authentic Voice Guide**.
5. Ensure realistic and consistent... NOT copied.
6. May **subtly incorporate one** target keyword... if natural.
7. Prioritize **believable, authentic achievement**...
...
"""
        ...
        if "[Placeholder" not in response_text:
            ...
            if len(synthetic_bullets_text) != count:
                logging.warning(f"{section_id} Failed count (got {len(synthetic_bullets_text)}). Returning empty.")
                return []
            ...
            logging.warning(f"{section_id} Synthetic generation failed. Returning empty.")
            return []
    ...
    def _generate_tailored_bullets_for_experience(
            ...
            reasoning_config: ReasoningConfig, section_id: str,
            temperature_override: Optional[float] = None # Added temperature
    ) -> List[Dict]:
        """
        Generic method to generate bullets for an experience section, applying V/C/S split.
        Accepts and passes temperature_override.
        """
        # ... (1. Find experience section in master_resume) ...
        # ... (2. Get signals from thematic_analysis) ...
        # ... (Get master_bullets_structured from enriched_scaffold) ...
        # ... (Calculate counts: total_expected, verbatim, customized, synthetic) ...

        final_bullets = []

        # --- 3. Select Verbatim Bullets ---
        if verbatim_count > 0 and master_bullets_structured:
            # ... (Build prompt_select) ...
            prompt_select = "..." # (Prompt to select verbatim bullets)
            system_prompt_select = "..."
            # Selection usually doesn't need temp adjustment, but can add if desired
            response_select = self._call_gemini_api(prompt_select, reasoning_config, f"{section_id}_SelectV", system_prompt_select)
            # ... (Validate response_select and add to final_bullets or fallback) ...
        ...
        if customized_count > 0:
            ...
            if candidates_for_custom:
                customized_bullets = self._generate_lightly_customized_bullets(
                    ...
                    temperature_override=temperature_override # Pass down
                )
        ...
        # --- 7. Word Count Validation & Rewrite (Uses IMPROVED logic) ---
        ...
        final_bullets = self._validate_and_potentially_rewrite_bullets(
            ...
            temperature_override=temperature_override # Pass temp for potential rewrite
        )

        # --- 8. Final Reordering ---
        # ... (Build prompt_reorder) ...
        prompt_reorder = "..." # (Prompt to reorder bullets)
        ...
        # ... (Validate response_reorder and build final_output_list or fallback) ...
        # (Build final_output_list ensuring consistent dict structure)
        if "[Placeholder" in response_reorder: # Fallback
            # Return final_bullets directly, ensuring consistent structure
            return [{'text': b.get('text', b.get('bullet_text','')), **b} for b in final_bullets[:total_expected_count]]
        # (Validation logic for reordering)
        # ...build final_ordered_bullets...
        # If validation fails, return fallback based on final_bullets

        # Return final list with consistent keys
        return [{'text': b.get('text', b.get('bullet_text','')),
                 'provenance': b.get('provenance'),
                 'word_count': b.get('word_count')} for b in final_ordered_bullets] # Simplified return
    ...
    def _generate_k5_unify_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        return self._generate_tailored_bullets_for_experience(
            ...
            section_id="K.5_UNIFY_BULLETS", temperature_override=temperature_override
        )
    ...
    def _generate_k6_ibm_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        return self._generate_tailored_bullets_for_experience(
            ...
            section_id="K.6_IBM_BULLETS", temperature_override=temperature_override
        )

    def _generate_k8_ey_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        # EY uses customize only
        ey_exp = next((exp for exp in self.master_resume['professional_experience'] if 'Ernst & Young' in exp['company']), None)
        if not ey_exp: return []
        source_bullets = ey_exp.get('highlights', [])[:self.PROVENANCE_SPLIT_TARGETS[ResumeSection.K8_EY_BULLETS]['Customized']]
        customized_bullets = self._generate_lightly_customized_bullets(
        ...
        # Apply word count validation/rewrite
        master_avg_lengths = calculate_master_avg_bullet_length(self.master_resume)
    ...
    def _generate_k9_early_career_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        # Early Career uses customize only
        early_exp = next((exp for exp in self.master_resume['professional_experience'] if 'Early Career' in exp['company']), None)
        if not early_exp: return []
        source_bullets = early_exp.get('highlights', [])[:self.PROVENANCE_SPLIT_TARGETS[ResumeSection.K9_EARLY_CAREER_BULLETS]['Customized']]
        customized_bullets = self._generate_lightly_customized_bullets(
            source_bullets_text=source_bullets,
    ...
    def _generate_k10_competencies(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.10 Competencies using V/C/S split. Accepts temperature_override."""
        # This reuses _generate_tailored_bullets_for_experience logic, adapted for competencies
    ...
    # --- Cover Letter Generation (No Fallback) ---
    def _generate_k13_cover_letter(self, temperature_override: Optional[float] = None) -> str:
        """
        [v11.0 REDESIGN & ... & v12.05 NO_FALLBACK]
        Generates K.13 cover letter. Raises HopExecutionError on failure.
        Accepts temperature_override.
        """
        from datetime import datetime
        today = datetime.now().strftime("%B %d, %Y")

        # --- 1. GATHER SIGNALS ---
        # ... (Signal gathering logic unchanged) ...
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'this opportunity')
        # ... (rest of signal gathering) ...
        ...
        feedback_instruction = ""
        # ... (Feedback logic for COVER_LETTER_STRUCTURE word count failure) ...
        prompt = f"""{feedback_instruction}You are an executive ghostwriter...
# ... (Full prompt structure including P1/P2/P3 word count constraints) ...
...
"""
        ...
        try:
            ...
            # ... (Structure fix logic for date/signature) ...
            if not cover_letter_text.strip().startswith(today):
                 # Try fixing...
                 if fix_fails:
                      logging.error("K.13 failed structure fix (date/salutation).")
                      raise HopExecutionError("K.13 failed structure fix (date/salutation).")
            # ... (Signature fix logic) ...
            if not cover_letter_text.strip().endswith(signature.split('\n')[-1].strip()):
                 # Try fixing...
                 if fix_fails:
                      logging.error("K.13 failed structure fix (signature).")
                      raise HopExecutionError("K.13 failed structure fix (signature).")

            return cover_letter_text.strip()

        except Exception as e:
            # Catch any error during generation or fixing
            logging.error(f"K.13 generation process failed: {e}", exc_info=True)
            # Raise HopExecutionError to halt the workflow
            raise HopExecutionError(f"K.13 generation failed: {e}") from e

    # Fallback method is now removed
    # def _generate_fallback_cover_letter(self) -> str: pass
