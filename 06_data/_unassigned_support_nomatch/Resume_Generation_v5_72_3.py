...
class WorkflowOrchestrator:
    ...
    if self.similarity_matrix_data:
        ...
        # SECTION 7: Word Count Compliance
        report_lines.append("7. WORD COUNT COMPLIANCE (v5.70 Hardened)")
        report_lines.append("")
        # Modified: Table format with Section | Actual | Target | Status | Message
        ...
        # Helper function to extract section name from rule_id or message
        def get_section_name(wcr):
            if "VG_TOTAL_WORD_COUNT" in wcr.rule_id: return "Total Resume"
            if "VG_WORD_COUNT_K1" in wcr.rule_id: return "K.1 Exec Summary"
            if "Unify Consulting" in wcr.message:
                return "Unify Overview" if "overview" in wcr.message else "Unify Bullets"
            if "IBM" in wcr.message:
                return "IBM Overview" if "overview" in wcr.message else "IBM Bullets"
            if "EY" in wcr.message: return "EY Bullets"
            if "Early Career" in wcr.message: return "Early Career"
            if "Competencies" in wcr.message: return "Competencies"
            if "Headline" in wcr.message: return "Headline"
            return wcr.rule_id
        
        for wcr in word_count_results:
            status_text = "Pass" if wcr.passed else "Fail"
            details = wcr.details if hasattr(wcr, 'details') and wcr.details else {}
            
            section = get_section_name(wcr)
            
            # Extract Actual and Target from details
            if isinstance(details, dict) and 'tailored_words' in details:
                actual = str(details['tailored_words'])
                target = f"{details.get('min_allowed', 'N/A')}-{details.get('max_allowed', 'N/A')}"
            elif isinstance(details, dict) and 'total' in details:
                actual = str(details['total'])
                target = "899-999"
            elif isinstance(details, dict) and 'char_count' in details:
                actual = f"{details['char_count']} chars"
                target = "60-90 chars"
            elif isinstance(details, dict) and 'min_allowed' in details:
                actual = "N/A"
                target = f"{details.get('min_allowed', 'N/A')}-{details.get('max_allowed', 'N/A')}"
            elif "K.1" in section:
                try:
                    actual = str(wcr.message.split(' ')[1])
            ...
            message = wcr.message.replace('|', ';')
            
            report_lines.append(f"| {section} | {actual} | {target} | {status_text} | {message} |")
