"""
Resume Generation Engine v5.10 - WORD COUNT TABLE + APP TRACKER
================================================================================
PATCH NOTES v5.10 (October 2025):
✓ Output 4: Word count comparison table (replaces CoC Ledger)
✓ Output 5: Application tracker (restored from v5.8)
✓ Shows baseline vs current word counts per section with tolerance bands
✓ 5-output structure: Resume, Cover Letter, QA Report, Word Table, App Tracker
...
✓ 5 Output Files (Resume, Cover Letter, QA Report, Word Table, App Tracker)
...
Version: 5.10
Date: October 2025
Architecture: Job_Workflow v1.9.2 + v5.4 + v5.6 RAG + v5.7 Constraints + v5.10 Word Table + App Tracker
Output Files: 5 (Resume .txt, Cover Letter .txt, QA Report .md, Word Table .txt, App Tracker .json)
"""
...
SECTION_CONSTRAINTS_V57 = {
    ...
    "output_files": {
        "count": 5,  # v5.10: Resume, Cover Letter, QA Report, Word Table, App Tracker
        "required": ["resume", "cover_letter", "qa_report", "word_table", "app_tracker"]
    }
}
...
class FileRenderer:
    ...
    def render_and_verify(
        ...
        try:
            ...
            files_to_write = [
                ...
                (app_tracker_filename, json.dumps(app_tracker_data, indent=2), "R8-011")
            ]
    ...
    def _build_word_count_table(
        ...
        """Build word count comparison table showing baseline vs current per section."""
        lines = []
        lines.append("=" * 100)
        lines.append("OUTPUT 4: WORD COUNT COMPARISON TABLE (BASELINE vs CURRENT)")
        lines.append("=" * 100)
        lines.append("")
        lines.append("Per-Section Tolerance Bands + Signal Scores + Elasticity Metrics")
        lines.append("")
        
        # Get section data from staging buffer
        sections = {}
        
        # Extract word counts from staging buffer
        unify_bullets = staging_buffer.get('K.5a', [])
        ibm_bullets = staging_buffer.get('K.6a', [])
        ey_bullets = staging_buffer.get('K.7a', [])
        early_bullets = staging_buffer.get('K.7b', [])
        
        # Count words in bullets
        def count_words_in_bullets(bullets):
            if isinstance(bullets, str):
                return len(bullets.split())
            elif isinstance(bullets, list):
                return sum(len(str(b).split()) for b in bullets)
            return 0
        
        sections['unify'] = {
            'baseline': 240,  # From master resume
            'current': count_words_in_bullets(unify_bullets),
            'signal': thematic_analysis.primary_theme.get('relevance_score', 0.0) if hasattr(thematic_analysis, 'primary_theme') else 0.8,
            'min_tolerance': 216,
            'max_tolerance': 264
        }
        ...
        sections['ibm'] = {
            'baseline': 200,
            'current': count_words_in_bullets(ibm_bullets),
            'signal': thematic_analysis.secondary_themes[0].get('relevance_score', 0.0) if hasattr(thematic_analysis, 'secondary_themes') and len(thematic_analysis.secondary_themes) > 0 else 0.75,
            'min_tolerance': 180,
            'max_tolerance': 220
        }
        ...
        sections['ey'] = {
            'baseline': 80,
            'current': count_words_in_bullets(ey_bullets),
            'signal': 0.65,
            'min_tolerance': 72,
            'max_tolerance': 88
        }
        ...
        sections['early'] = {
            'baseline': 60,
            'current': count_words_in_bullets(early_bullets),
            'signal': 0.60,
            'min_tolerance': 54,
            'max_tolerance': 66
        }
        ...
        # Table header
        lines.append("┌" + "─" * 18 + "┬" + "─" * 10 + "┬" + "─" * 10 + "┬" + "─" * 18 + "┬" + "─" * 11 + "┬" + "─" * 11 + "┬" + "─" * 8 + "┐")
        lines.append("│ Section          │ Baseline │ Current  │ Tolerance Band   │ Signal    │ Elasticity│ Status │")
        lines.append("├" + "─" * 18 + "┼" + "─" * 10 + "┼" + "─" * 10 + "┼" + "─" * 18 + "┼" + "─" * 11 + "┼" + "─" * 11 + "┼" + "─" * 8 + "┤")
        ...
        for section_name, data in sections.items():
            ...
            min_tol = data['min_tolerance']
            max_tol = data['max_tolerance']
            signal = data['signal']
            
            # Calculate elasticity (simplified)
            elasticity = (current - baseline) / baseline if baseline > 0 else 0.0
            
            # Determine status
            status = "✓ PASS" if min_tol <= current <= max_tol else "✗ FAIL"
            
            lines.append(
                f"│ {section_name:16} │ {baseline:8} │ {current:8} │ "
                f"{min_tol:3}–{max_tol:<12} │ {signal:9.2f} │ {elasticity:+9.3f} │ {status:6} │"
            )
        ...
        # Summary statistics
        total_baseline = sum(s['baseline'] for s in sections.values())
        total_current = sum(s['current'] for s in sections.values())
        
        lines.append(f"TOTAL WORDS:")
        lines.append(f"  Baseline: {total_baseline}")
        lines.append(f"  Current:  {total_current}")
        lines.append(f"  Delta:    {total_current - total_baseline:+d} ({((total_current - total_baseline) / total_baseline * 100):+.1f}%)")
        lines.append("")
        
        # Ratio analysis
        unify_words = sections['unify']['current']
        ibm_words = sections['ibm']['current']
        if ibm_words > 0:
            ratio = unify_words / ibm_words
            lines.append(f"UNIFY/IBM RATIO: {ratio:.2f} (Target: 1.1-1.3)")
