# File: agent_swarm_v9_5.py
# Overwrites: agent_swarm_v9_0.py
...
# v9.5: Stub for Neo4j/Graph Tool
...
class PIISanitizerAgent:
    ...
    def run(self, master_resume: Dict[str, Any]) -> Dict[str, Any]:
...
class RAG_SearchAgent(BaseAgent):
    ...
    def _generate_thought(self, query: str, iteration: int, current_results: List) -> str:
        # v9.5: This is now a real LLM call
    ...
    def _select_action(self, thought: str) -> Dict:
        # v9.5: Parse the thought for a tool call.
    ...
    def _tool_master_resume_search(self, tool_input: Dict) -> List[Dict]:
        ...
        master_resume = self.blackboard.master_resume # v9.5: This is the SANITIZED version
        all_text_chunks = []
    ...
    def _tool_graph_search(self, tool_input: Dict) -> List[Dict]:
        ...
        # v9.5: Calls the stubbed Neo4j client
...
class ProvenanceRouterAgent(BaseAgent):
    """v9.5: Replaces BulletSwarmAgent. Implements 2/3/2 & 2/2/2 logic."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
    ...
    def run(self) -> Dict[str, Any]: # v9.5: Return dict for provenance
        ...
        try:
            ...
            provenance_log["outputs"]["synthetic"] = synthetic_bullets # v9.5
...
class BulletCritiqueAgent(BaseAgent):
    """v9.5: New agent to critique the assembled bullet list."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
...
class DraftingConductorAgent(BaseAgent):
    """v9.5: Replaces AdversarialDraftingRouter. (Req #1)
    ...
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        ...
        # v9.5: Initialize all drafting experts
        self.drafters = {
...
class QAConductorAgent(BaseAgent):
    """v9.5: Replaces AtomicQASwarmLLM (Req #1)
...
class PreferenceCaptureAgent(BaseAgent):
    ...
    def __init__(self, blackboard: WorkflowBlackboard, log_path: str, debug_mode=False):
        ...
        self.client_name = "google"
self.log_path = log_path # v9.5: Added log path
...
"""
            ...
            # v9.5: Close the HIL loop by logging the preference
            try:
...
def get_blackboard(state: 'GraphState') -> WorkflowBlackboard:
    ...
    return WorkflowBlackboard(
        ...
        workflow_id=state["workflow_id"] # v9.5: Pass workflow_id
    )
...
def run_rag_stack(state: 'GraphState') -> Dict[str, Any]:
    ...
    # v9.5: Add to Provenance Ledger (Item #2)
    provenance_entry = {
        "step": "rag_stack",
        "queries": queries,
...
def run_bullet_stack(state: 'GraphState') -> Dict[str, Any]:
    ...
    result = bullet_agent.run() # v9.5: result is a dict
...
def run_bullet_critique(state: 'GraphState') -> Dict[str, Any]:
...
def run_drafting_stack(state: 'GraphState') -> Dict[str, Any]:
    ...
    # v9.5: This node is now split into two: run_bullet_stack and run_drafting_stack
...
def run_replanner(state: 'GraphState') -> Dict[str, Any]:
    ...
    # v9.5: Determine what failed (Bullets or QA)
...
workflow.add_edge("replanner", "bullet_stack") # v9.5: Re-plan must re-gen bullets
