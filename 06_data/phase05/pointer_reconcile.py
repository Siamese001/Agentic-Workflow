...
def load_pointer_map() -> Optional[PointerMap]:
    ...
    try:
        ...
        logging.debug(f"[WARN] Failed to load pointer map: {e}")
        return None
...
def run_reconciliation() -> ReconciliationResult:
    ...
    logging.debug("=== POINTER RECONCILIATION ===")
    logging.debug(f"Project root: {PROJECT_ROOT}")
    logging.debug(f"Pointer map: {POINTER_MAP_PATH}")
    
    # Load pointer map
    pointer_map = load_pointer_map()
    
    if pointer_map is None or not pointer_map.moves:
        logging.debug("[INFO] No moves recorded in pointer map. Nothing to reconcile.")
    ...
    logging.debug(f"Loaded {len(pointer_map.moves)} moves from pointer map")
    ...
    logging.debug(f"Found {len(pointer_files)} pointer files to scan")
    ...
    for pointer_path in pointer_files:
        ...
        if error:
            ...
            updated_count += 1
            logging.debug(f"  Updated: {pointer_path.relative_to(CACHE_ROOT)}")
        else:
            unchanged_count += 1
    ...
    logging.debug(f"Total pointers scanned: {result.total_pointers_scanned}")
    logging.debug(f"Pointers updated: {result.pointers_updated}")
    logging.debug(f"Pointers unchanged: {result.pointers_unchanged}")
    logging.debug(f"Errors: {len(result.errors)}")
...
def main() -> int:
    ...
    try:
        if args.record_move:
            ...
            record_move(src, dst)
            logging.debug(f"Recorded move: {src} → {dst}")
            return 0
        
        if args.show_map:
            pointer_map = load_pointer_map()
            if pointer_map:
                logging.debug(json.dumps(asdict(pointer_map), indent=2))
            else:
                logging.debug("No pointer map found")
            return 0
        
        if args.clear_map:
            if POINTER_MAP_PATH.exists():
                POINTER_MAP_PATH.unlink()
                logging.debug("Pointer map cleared")
            else:
                logging.debug("No pointer map to clear")
            return 0
        ...
        logging.debug(f"[ERROR] Pointer reconciliation failed: {e}")
        import traceback
        traceback.print_exc()
        return 1
