"""
Resume Generation Engine v14.36

API Client Initialization Fix: Proper Error Handling for Failed Gemini Client

This version fixes an AttributeError where code attempted to call methods on a None
GeminiWebSearchClient. The issue occurred when the GenerativeModel initialization
failed but the exception was caught and self.client was set to None, allowing the
object to be created in a broken state.

Key Changes from v14.35:
1. Fixed GeminiWebSearchClient.__init__ to raise exception instead of continuing
   - Changed line 1698 from setting self.client = None to re-raising exception
   - Ensures failed initialization prevents object creation
   - Prevents downstream AttributeError: 'NoneType' object has no attribute 'search_and_analyze'
2. This allows the proper exception handling in EnhancedJobDescriptionAnalyzer
   to catch the failure and set self.gemini_client = None appropriately

The issue manifested as:
  AttributeError: 'NoneType' object has no attribute 'search_and_analyze'
  when self.client was None but methods tried to use it

All other functionality from v14.35 is retained:
- Fixed WorkflowOrchestrator self.logger initialization
- Explicit API Key Configuration at Module Initialization
- Enhanced API Call Logging
- Configuration Context Logging
- Data Flow & Transformation Snapshots
- Intermediate Validation & Decision Logging

Usage:
    orchestrator = WorkflowOrchestrator(master_resume)
    result = orchestrator.execute_workflow(job_description, company, title)

Dependencies: Same as v14.35
Author: Updated by Claude (Anthropic)
Date: 2025
"""
...
class GeminiWebSearchClient:
    def __init__(self, config: RAGConfig = RAGConfig()):
        """Initializes the client using globally configured Gemini API."""
...
"""
    ...
    def _build_context_narrative(self, spec: Dict) -> Dict:
        ...
        if section_enum == ResumeSection.K4_TRADERSENSE_NARRATIVE:
            ...
            focus_instruction = "Focus on transferable executive themes and how this consulting experience built foundational capabilities relevant to current target role requirements."
            ...
            combined_signals = list(set(rag_keywords + rag_signals))[:7]
            focus_instruction = "Emphasize how early quantitative and actuarial work built analytical foundations that enabled transition to technology career."
        return {
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict):
        ...
        """
        # Initialize logger FIRST before any log calls
        logging.basicConfig(
            level=logging.INFO,
