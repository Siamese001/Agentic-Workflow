"""
Resume Generation Engine v5.45 - QA HARDENING & WORD COUNT ENFORCEMENT

v5.45 CHANGES - CRITICAL QUALITY FIXES:
✓ FIXED: Word count enforcement in HOP-3 generation - now uses count_words_clean() in prompts
✓ FIXED: QA output formatting - structured sections with clear separators
✓ FIXED: Variance signal visibility - added Status column (✓/✗) to Output 4
✓ NEW: QAFormatter class - structured markdown formatting for all 14 QA sections
✓ NEW: Post-generation validation loop - ensures word counts match targets (±5%)
✓ NEW: Delta column in Output 4 - shows absolute word difference per section
✓ NEW: Summary header in Output 4 - "X passed, Y failed" at top of table
✓ NEW: Test suite - test_word_count_accuracy() and test_qa_structure()
✓ Result: Accurate generation matching validation, readable QA reports, clear failure signals

BREAKING CHANGES:
- HOP-3 prompts now explicitly reference count_words_clean()
- QA report format changed - no more mixed JSON/dict repr
- Output 4 has new columns: Status, Delta (before Variance %)
- Failed sections now prefixed with ✗ FAIL

All v5.44 functionality maintained with hardened quality controls.

BUILD: October 19, 2025
FILE SIZE: 7,400+ lines (larger than v5.44)
...
# ============================================================================
# v5.45 QA FORMATTER - STRUCTURED OUTPUT GENERATION
# ============================================================================

class QAFormatter:
    """
    v5.45: Structured QA report formatting with clear section separation.
    
    Each section uses consistent markdown formatting:
    - 40-char separator lines (not 80+)
    - Clean tables for tabular data
    - Status indicators (✓/✗) for all checks
    - Summary headers showing pass/fail counts
    """
    
    @staticmethod
    def format_word_count_table(word_count_data: List[Dict]) -> str:
        """
        Format word count compliance table with status column.
        
        v5.45: New format with Status, Delta columns for clear variance signals.
        
        Args:
            word_count_data: List of dicts with keys: name, baseline, actual, variance, status
        ...
        # Count pass/fail
        passed = sum(1 for item in word_count_data if item['status'] == 'PASS')
        ...
        # Table header
        lines.append("| Status | Section | Baseline | Actual | Delta | Var % | Range |")
        lines.append("|--------|---------|----------|--------|-------|-------|-------|")
        
        # Table rows
        for item in word_count_data:
            status_icon = "✓ PASS" if item['status'] == 'PASS' else "✗ FAIL"
            delta = item['actual'] - item['baseline']
            delta_str = f"+{delta}" if delta > 0 else str(delta)
            
            # Format variance percentage
            var_str = f"{item['variance']:+.1f}%"
            
            # Calculate allowed range
            tolerance = item.get('tolerance_pct', 20.0)
            min_allowed = int(item['baseline'] * (1 - tolerance/100))
            max_allowed = int(item['baseline'] * (1 + tolerance/100))
            range_str = f"{min_allowed}-{max_allowed}"
            
            lines.append(
                f"| {status_icon} | {item['name']:30s} | "
                f"{item['baseline']:4d} | {item['actual']:4d} | "
                f"{delta_str:5s} | {var_str:7s} | {range_str:9s} |"
            )
        
        return "\n".join(lines)
    
    @staticmethod
    def format_section_with_separator(section_number: int, title: str, content: List[str]) -> str:
        """
        Format a QA section with numbered header and separator.
        
        Args:
            section_number: Section number (1-14)
            title: Section title
            content: List of content lines
            
        Returns:
            Formatted section with separator
        """
        lines = []
        lines.append("─" * 40)
    ...
    @staticmethod
    def format_similarity_matrix(matrix_data: Dict) -> str:
        """Format deduplication matrix section."""
        lines = []
        
        lines.append(f"Total Comparisons: {matrix_data['total_comparisons']}")
        lines.append(f"Sections Analyzed: {', '.join(matrix_data['sections_analyzed'])}")
        lines.append(f"Max Similarity: {matrix_data['max_similarity']:.4f}")
        lines.append(f"Duplicates Found (≥0.9): {len(matrix_data['duplicates_found'])}")
        lines.append("")
        
        if matrix_data['duplicates_found']:
            lines.append("✗ DUPLICATE BULLETS DETECTED:")
            for dup in matrix_data['duplicates_found']:
                lines.append(f"  {dup['bullet_1']} ↔ {dup['bullet_2']}: {dup['similarity']:.4f}")
        else:
            lines.append("✓ NO DUPLICATES DETECTED (all similarities < 0.9)")
        
        lines.append("")
        
        # Top 10 similarities
        sorted_checks = sorted(
            matrix_data['pairwise_checks'],
            key=lambda x: x['similarity'],
            reverse=True
        )[:10]
        
        if sorted_checks:
            lines.append("## Top 10 Highest Similarities:")
            lines.append("")
            for i, check in enumerate(sorted_checks, 1):
                cross_flag = " [CROSS-SECTION]" if check['cross_section'] else ""
                lines.append(f"{i:2d}. {check['bullet_1']:15s} ↔ {check['bullet_2']:15s}: {check['similarity']:.4f}{cross_flag}")
        
        return "\n".join(lines)
    
    @staticmethod
    def format_pipeline_health(hop_checkpoints: List) -> str:
        """Format pipeline health section."""
        lines = []
        for hop in hop_checkpoints:
            status_icon = "✓" if hop.status.value == "PASS" else "✗"
            lines.append(f"{status_icon} {hop.hop_id}: {hop.hop_name} - {hop.status.value}")
        return "\n".join(lines)
    
    @staticmethod
    def format_production_readiness(validation_results: List) -> str:
        """Format production readiness section."""
        lines = []
        ...
        critical_failures = len([
            vr for vr in validation_results 
            if not vr.passed and vr.severity.value == "CRITICAL"
        ])
        
        lines.append(f"Total Validation Checks: {total_checks}")
        ...
        if critical_failures == 0:
            ...
            lines.append("✗ STATUS: CRITICAL FAILURES - REVIEW REQUIRED")
...
def test_word_count_accuracy():
    """Test count_words_clean() accuracy with various markdown inputs."""
    print("\n" + "=" * 60)
    print("TEST: Word Count Accuracy")
    print("=" * 60)
    
    test_cases = [
        # (input_text, expected_count, description)
        ("## Heading\n\n**Bold text** and *italic*", 5, "Markdown formatting removal"),
        ("• First point\n• Second point", 4, "Bullet point removal"),
        ("Senior **pre-sales** leader with 10+ years.", 7, "Complex markdown"),
        ("LLM & Generative AI: Advanced expertise", 5, "Colon removal in competencies"),
        ("", 0, "Empty string"),
        ("   ", 0, "Whitespace only"),
    ]
    ...
    for text, expected, description in test_cases:
        actual = count_words_clean(text)
        if actual == expected:
            print(f"✓ PASS: {description} ({expected} words)")
            passed += 1
        else:
...
def test_qa_output_structure():
    """Test QA formatter produces well-structured output."""
    print("\n" + "=" * 60)
    print("TEST: QA Output Structure")
    print("=" * 60)
    
    # Mock word count data
    mock_data = [
        {'name': 'Executive Summary', 'baseline': 125, 'actual': 106, 'variance': -15.2, 'status': 'FAIL', 'tolerance_pct': 20.0},
        {'name': 'Headline', 'baseline': 11, 'actual': 10, 'variance': -9.1, 'status': 'PASS', 'tolerance_pct': 20.0},
        {'name': 'Unify Consulting - Bullets', 'baseline': 214, 'actual': 226, 'variance': 5.6, 'status': 'PASS', 'tolerance_pct': 20.0},
    ]
    
    formatter = QAFormatter()
    output = formatter.format_word_count_table(mock_data)
    
    # Validate structure
    checks = [
        ("Summary:" in output, "Has summary header"),
        ("| Status |" in output, "Has table header"),
        ("✓ PASS" in output, "Has pass indicator"),
        ("✗ FAIL" in output, "Has fail indicator"),
        ("| Delta |" in output, "Has delta column"),
        ("2 passed, 1 failed" in output, "Correct pass/fail count"),
    ]
...
def run_all_tests():
    """Run all v5.45 test cases."""
    print("\n" + "=" * 60)
    print("RUNNING v5.45 TEST SUITE")
    print("=" * 60)
    
    results = []
    results.append(("Word Count Accuracy", test_word_count_accuracy()))
    results.append(("QA Output Structure", test_qa_output_structure()))
