...
class ArtistGenerator:
    ...
    def generate(
        ...
        attempt: int = 1,
        k1_temp_override: Optional[float] = None  # *** ADDED ***
    ) -> Tuple[Dict, List[ValidationResult]]:
        """
    ...
    def _generate_artist_output(self, k1_temp_override: Optional[float] = None) -> Dict:
        ...
        for config in self.ARTIST_GENERATION_CONFIG:
            ...
            try:
                ...
                # --- START FIX: Pass override ONLY to K.1 ---
    ...
    def _call_gemini_api(self, prompt: str, reasoning_config: ReasoningConfig, section_id: str, system_prompt: str, temperature_override: Optional[float] = None) -> str:
        ...
        try:
            ...
            # --- START FIX: Apply temperature override ---
            # The override from the orchestrator (via _generate_k1)
            # takes highest priority over all other settings.
            if temperature_override is not None:
                generation_config.temperature = temperature_override
                if section_id == "K.1": # Log only for the section we're fixing
            ...
            if sc_count > 1:
                ...
                # --- START FIX: Apply SC default temp ONLY if no override ---
                # Force high temperature for diverse samples
                if temperature_override is None: # No override, use SC default
...
"""
                ...
                # --- SINGLE-CALL (SC=1) ---
                # The temperature is already set correctly, either by
                # the override or by the default reasoning_config_to_api_params.
                response = model.generate_content(
                    f"{enhanced_system}\n\n{prompt}",
                    generation_config=generation_config
                )
...
The target role is a PRE-SALES / Go-To-Market role. The summary MUST prioritize themes of revenue generation, strategic partnerships, and driving market adoption. Frame achievements in the context of winning new business and expanding market share."""
        ...
        if self.previous_failures:
            for failure in self.previous_failures:
                # *** MODIFIED: Check for both word and sentence count failures ***
...
"""
        ...
        # --- START FIX: Apply temperature override logic ---
        # Default to 1.0 for first creative attempt if no override is given
        ...
        # --- MODIFIED: Pass the final temperature to the API call ---
...
def calculate_signal_score(text_content, thematic_analysis):
...
# <<< MODIFICATION: Renamed and logic updated for range checking >>>
    ...
    def _calculate_master_avg_bullet_length(self) -> Dict[str, float]:
        ...
        v10.02 FIX: Corrected to use count_words_ms_word_style (was: count_words_clean).
        """
        avg_lengths = {}
...
class WorkflowOrchestrator:
    ...
    def _execute_hop_3_artist_generation(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        # *** ADDED: Progressive temperature schedule for K.1 compliance ***
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # --- START FIX: Get temp for this attempt ---
            current_k1_temp = k1_temperature_schedule[attempt - 1]
            if attempt > 1:
                print(f"    Applying K.1 Temperature Override: {current_k1_temp}")
            
            # Pass the temperature override to the artist
            current_artist_output, current_hop_results = artist.generate(
                feedback_results=feedback_results,
                attempt=attempt,
                k1_temp_override=current_k1_temp  # Pass the override
            )
            # --- END FIX ---
    ...
    def _format_ascii_bar_chart(self, label: str, value: float, target_min: float, target_max: float, temperature: Optional[float] = None, max_value: float = 1.0, width: int = 30, is_summary: bool = False) -> str:
        """
        Formats a value as an ASCII horizontal bar chart for the QA report.
        v9.96: Moves temperature to the end and supports weighted average.
        """
        if value < 0: value = 0
        ...
        # New contiguous target string
        target_str = f"(Tgt:{target_min:.0%}-{target_max:.0%})"
        
        # New temperature string (at the end)
        temp_label = "Avg T" if is_summary else "T"
        
        temp_str = ""
        if temperature is not None:
             if is_summary:
                 # Show 2 decimal places for weighted average
                 temp_str = f"({temp_label}:{temperature:.2f})"
             else:
                 # Show 1 decimal place for individual sections
                 temp_str = f"({temp_label}:{temperature:.1f})"

        # New format: Label [Bar] Signal% Target Status (Temp)
        # Adjusted spacing to align temperature column
        return f"{label:<25} [{bar:<{width}}] {value:.1%} Signal {target_str} {status:<10} {temp_str}"
    ...
    def _build_qa_section_1_signal_quality(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis) -> List[str]:
        ...
        # Use the config from PreFlightValidator
        ...
        total_weighted_temp, temp_weight = 0.0, 0.0 # <<< NEW: For weighted avg temp
        ...
        for label, (section_enum, target_min_score, target_max_score, weight, reasoning_config) in section_configs.items():
            ...
            if content:
                ...
                # Determine the *creative* temperature
                if sc_count > 1:
                    # If SC is active, the *creative drafts* were generated at T=0.9
                    temp = 0.9
                else:
                    ...
                    if label == "Executive Summary":
                        temp = 1.0 # Account for manual K.1 override
                # --- END MODIFICATION ---
                
                # <<< NEW: Accumulate weighted temperature >>>
        ...
        if total_weight > 0:
            ...
            # <<< NEW: Calculate weighted average temp >>>
            ...
            # <<< MODIFICATION: Pass average_temp to the formatter >>>
            summary_bar = self._format_ascii_bar_chart(
                ...
                temperature=average_temp, # Pass calculated average temp
                is_summary=True # Add flag to change "T" to "Avg T"
            )
            lines.append("-" * 80)
    ...
    def _build_qa_section_16_final_format(
        self,
        validation_results: List[ValidationResult],
        file_contents: Dict[str, str] # Added file_contents argument
    ) -> List[str]:
        """
        v10.0: Builds Section 16 of the QA report: Final Output Artifacts.
        Shows a summary table with detailed formatting checks.
        v10.02 FIX: Removed raw output dumps as requested by user.
        """
        lines = ["", "16. FINAL OUTPUT ARTIFACTS", ""]

        # --- Summary Table ---
        lines.append("```markdown")
        # v10.1: Updated headers for clarity
        headers = ["Output Artifact", "Formatting Requirement", "Status", "Failure Explanation"]

        # Define the outputs and their keys in file_contents
        # Perform QA checks directly here based on rendered content
        output_definitions = []

        # Resume Checks
        resume_content = file_contents.get('resume_md', '')
        # v10.1: Simplified initial check
        ...
        if resume_content:
            ...
            # K.0 Headline Check
            # v10.1: Improved regex to handle potential extra newlines
            headline_match = re.search(r"^## .+\n+(\S.+)\n", resume_content) # Find first non-blank line after Name
            headline_text = headline_match.group(1).strip() if headline_match else ""
            headline_pass = headline_match is not None and not headline_text.startswith("#") and "|" in headline_text # Check for plain text and pipe separators
            output_definitions.append(("", "K.0 Headline (Plain Text)", "PASS" if headline_pass else "FAIL", "" if headline_pass else "Headline is missing, uses Markdown header, or missing pipes."))
            # Line Break Check
            # v10.1: More robust check for exactly one blank line
            lb_pass = bool(re.search(r"^## .+\n\n\S.+\n", resume_content)) # Checks for the single blank line
            output_definitions.append(("", "Line Break Between Name/Headline", "PASS" if lb_pass else "FAIL", "" if lb_pass else "Missing/incorrect blank line between Name and Headline."))
            # H2 Headers Check
            ...
            output_definitions.append(("", "H2 Headers (##) Used", "PASS" if h2_pass else "FAIL", "" if h2_pass else f"Missing headers: {missing_headers}"))
            # Experience Format Checks
            # v10.1: Use regex for more precise checks of the **Comp | Loc** format
            exp_comp_loc_pass = bool(re.search(r"\n\*\*.*\s+\|\s+.*\*\*\n", resume_content)) # Checks for bolded line with a pipe
            output_definitions.append(("", "Experience: `**Comp \| Loc**` format", "PASS" if exp_comp_loc_pass else "FAIL", "" if exp_comp_loc_pass else "Experience section missing bold Company/Location format."))
            # v10.1: Use regex for more precise checks of the **Title | Dates** format
            exp_title_date_pass = bool(re.search(r"\n\*\*.*\s+\|\s+.*–.*\*\*\n", resume_content)) # Checks for bolded line with pipe and en dash
            output_definitions.append(("", "Experience: `**Title \| Dates**` format", "PASS" if exp_title_date_pass else "FAIL", "" if exp_title_date_pass else "Experience section missing bold Title/Dates format."))
            # v10.1: Check specifically within PROF EXP section
            ...
            exp_bullets_pass = bool(re.search(r"\n\* ", prof_exp_content)) # Check for asterisk bullets within this section
            output_definitions.append(("", "Experience: `* ` Bullets Used", "PASS" if exp_bullets_pass else "FAIL", "" if exp_bullets_pass else "Experience bullets do not use '* ' format."))
            # Competencies Bullets Check
            ...
            comp_bullets_pass = bool(re.search(r"^\* ", comp_content, re.MULTILINE)) # Check beginning of lines in this section
            output_definitions.append(("", "Competencies: `* ` Bullets Used", "PASS" if comp_bullets_pass else "FAIL", "" if comp_bullets_pass else "Competencies do not use '* ' format."))
            # Education/Certs No Bullets Check
            ...
            output_definitions.append(("", "Education: No Bullets", "PASS" if edu_no_bullet_pass else "FAIL", "" if edu_no_bullet_pass else "Education section incorrectly uses bullets."))
            ...
            output_definitions.append(("", "Certifications: No Bullets", "PASS" if cert_no_bullet_pass else "FAIL", "" if cert_no_bullet_pass else "Certifications section incorrectly uses bullets."))
            # Proper Line Breaks (Check for blank lines between sections and within experience)
            lb_count = resume_content.count('\n\n')
            # v10.1: Adjust heuristic based on expected structure
            expected_min_lbs = 5 + 5 # 5 sections + 5 experience entries approx
            lb_pass = lb_count >= expected_min_lbs
            output_definitions.append(("", "Proper Line Breaks", "PASS" if lb_pass else "WARN", "" if lb_pass else f"Potentially insufficient line breaks (found {lb_count}, expected ~{expected_min_lbs}+). Review visually."))

        # Skills Checks
        skills_content = file_contents.get('skills', '')
        output_definitions.append(("**Skills**", "Raw Text", "PASS" if skills_content else "FAIL", "" if skills_content else "Content missing."))
        if skills_content:
            # v10.1 Handle potential leading/trailing whitespace before splitting
            skills_lines = [line.strip() for line in skills_content.strip().split('\n') if line.strip()]
            bullet_pass = all(line.startswith("• ") for line in skills_lines)
            output_definitions.append(("", "Bulleted list (`• `) Used", "PASS" if bullet_pass else "FAIL", "" if bullet_pass else "Skills do not consistently use '• ' format."))
            # v10.1 More robust newline check allowing single or double newlines between items
            newline_pass = skills_content.strip().count('\n') >= len(skills_lines) - 1 if skills_lines else True # Check for at least single newline
            output_definitions.append(("", "Newlines Between Skills", "PASS" if newline_pass else "FAIL", "" if newline_pass else "Missing newlines between skills.")) # Simplified requirement name
        ...
        if cl_content:
            ...
            date_pass = bool(re.match(r"\w+ \d{1,2}, \d{4}", cl_content))
            # v10.1 Allow flexibility in recipient line
            recipient_pass = "Hiring Manager" in cl_content # Simplified check
            body_paras_pass = cl_content.count('\n\n') >= 4 # Date, Recipient, P1, P2, P3, Signature block
            closing_pass = "\nSincerely,\n\n" in cl_content
            sig_block_present = "Amit Ayer" in cl_content and "amitayer1@gmail.com" in cl_content # Check specific details
            format_pass = date_pass and recipient_pass and body_paras_pass and closing_pass and sig_block_present
            output_definitions.append(("", "Standard Letter Format Present", "PASS" if format_pass else "FAIL", "" if format_pass else f"Missing standard format elements (Date:{date_pass}, Recipient:{recipient_pass}, Paras:{body_paras_pass}, Closing:{closing_pass}, Sig:{sig_block_present})"))
            # Multi-line Signature Check
            # v10.1 Check actual newlines in the signature block based on expected content
            ...
            multiline_pass = sig_block_content.count('\n') >= 3 # Name, Email, Phone, LinkedIn
            output_definitions.append(("", "Signature Renders Multi-line", "PASS" if multiline_pass else "FAIL", "" if multiline_pass else "Signature block does not contain expected newlines."))
        ...
        if qa_content:
            # v10.1 Check fences more reliably, ignoring potential initial lines
            fence_pass = "```markdown" in qa_content and qa_content.strip().endswith("```")
            output_definitions.append(("", "Uses Fenced Markdown", "PASS" if fence_pass else "FAIL", "" if fence_pass else "QA Report missing markdown fences."))
            table_pass = "```markdown\n" in qa_content and qa_content.count('```') >= 4 # Check for code blocks used for tables
            output_definitions.append(("", "Uses Pre-formatted Tables (Code Blocks)", "PASS" if table_pass else "WARN", "" if table_pass else "Could not verify presence of pre-formatted tables. Review visually."))
            section_headers = ["1. SIGNAL QUALITY", "2. THEMATIC COMPLIANCE", "3. HOP-BY-HOP", "15. JD ENFORCEMENT", "16. FINAL OUTPUT ARTIFACTS"]
            missing_qa_headers = [h for h in section_headers if h not in qa_content]
            structure_pass = not missing_qa_headers
            output_definitions.append(("", "Consistent Section Structure", "PASS" if structure_pass else "FAIL", "" if structure_pass else f"Missing expected section headers: {missing_qa_headers}"))
            # Section 1 Specific Checks
            ...
            output_definitions.append(("", "**Section 1: Uses ASCII Bar Chart**", "PASS" if sec1_bar_pass else "FAIL", "" if sec1_bar_pass else "Section 1 missing ASCII bar chart characters."))
            sec1_details_pass = '(Tgt:' in sec1_content and 'Signal' in sec1_content and '(T:' in sec1_content
            output_definitions.append(("", "**Section 1: Includes Targets/Status/Temp**", "PASS" if sec1_details_pass else "FAIL", "" if sec1_details_pass else "Section 1 missing Target/Status/Temperature indicators."))
            sec1_avg_pass = 'Total Weighted Score' in sec1_content and '(Avg T:' in sec1_content
            output_definitions.append(("", "**Section 1: Includes Weighted Average**", "PASS" if sec1_avg_pass else "FAIL", "" if sec1_avg_pass else "Section 1 missing Weighted Average summary line or Avg Temp."))
            sec1_fence_pass = bool(sec1_content_match) # Check if the regex found the fenced block
            output_definitions.append(("", "**Section 1: Uses Code Fence**", "PASS" if sec1_fence_pass else "FAIL", "" if sec1_fence_pass else "Section 1 content is not within a ```markdown code fence."))
        ...
        if app_content:
            ...
            if not fence_check_pass:
                 # Overwrite the status and explanation of the previous row
                 output_definitions[-1] = ("**App Tracker**", "**JSON Block (Unfenced)**", "FAIL", "App Tracker incorrectly includes ```json fences.")
            # Validate JSON Structure
            json_valid = False
            parsed_json_data = None
            try:
                # v10.1 Handle potential fences before parsing
                content_to_parse = app_content.strip()
                if is_fenced:
                    content_to_parse = content_to_parse[7:-3].strip() # Remove fences
                parsed_json_data = json.loads(content_to_parse)
                json_valid = True
            except json.JSONDecodeError:
                json_valid = False
            output_definitions.append(("", "Valid JSON Structure", "PASS" if json_valid else "FAIL", "" if json_valid else "App Tracker content is not valid JSON."))
            # Adheres to Schema (Check keys)
            schema_pass = False
            if json_valid and parsed_json_data: # Check only if JSON is valid
                schema_keys = set(APP_TRACKER_SCHEMA_V4.keys())
                actual_keys = set(parsed_json_data.keys())
                schema_pass = schema_keys == actual_keys
                missing_keys = schema_keys - actual_keys
                extra_keys = actual_keys - schema_keys
                schema_fail_reason = ""
                if missing_keys: schema_fail_reason += f"Missing keys: {missing_keys}. "
                if extra_keys: schema_fail_reason += f"Extra keys: {extra_keys}."
            else:
                 schema_fail_reason = "Cannot check schema, JSON invalid."

            output_definitions.append(("", "Adheres to `APP_TRACKER_SCHEMA_V4`", "PASS" if schema_pass else "FAIL", "" if schema_pass else schema_fail_reason.strip()))

        # Convert definition list to rows for formatting
        final_rows = [[item[0], item[1], item[2], item[3]] for item in output_definitions]
        lines.extend(self._format_plain_text_table(headers, final_rows, alignments=['L', 'L', 'L', 'L'])) # v10.1 Specify alignments
        lines.append("```") # End summary table markdown

        # --- START FIX: Remove all raw output appends ---
        # (All code from lines 4487-4542 has been deleted)
        # --- END FIX ---
