            ...
            for bullet in section.get("bullets", []):
                ...
                if forbidden_found:
                    validation_results.append(ValidationResult(
                        ...
                        passed=False, # Changed from MEDIUM to HIGH
                        severity=ValidationSeverity.HIGH,
...
class PreFlightValidator:
    ...
    RULES_CONFIG = [
        ...
        {
            "rule_id": "COVER_LETTER_SIGNATURE", "severity": ValidationSeverity.HIGH, "category": "structure", "validator": lambda ctx: ctx.expected_signature and ctx.staging_buffer.get(ResumeSection.K13_COVER_LETTER.value, '').strip().endswith(ctx.expected_signature),
        ...
        {
            "rule_id": "VG_COVER_LETTER_SIGNATURE_MULTILINE", "severity": ValidationSeverity.CRITICAL, "category": "structure", "validator": lambda ctx: ctx.expected_signature and '\n' in ctx.expected_signature and ctx.expected_signature in ctx.staging_buffer.get(ResumeSection.K13_COVER_LETTER.value, ''),
    ...
    def _validate_k1_sentence_count(self, context: ValidationContext) -> bool:
        summary_text = context.staging_buffer.get(ResumeSection.K1_EXECUTIVE_SUMMARY.value, '')
        sentence_count = _count_sentences(summary_text)
        context._cache["VG_SENTENCE_COUNT_K1"] = {
    ...
    def _validate_no_placeholders(self, context: ValidationContext) -> bool:
        try:
            buffer_data = context.staging_buffer.data # Get a copy
            ...
            context._cache["CONTENT_NO_PLACEHOLDERS"] = {"placeholders": [f"Error during check: {e}"]}
    ...
    def _validate_cover_letter_narrative(self, context: ValidationContext) -> bool:
        ...
        differentiators = context.top_differentiators or []
        ...
        if len(paragraphs) < 6:
             context._cache["COVER_LETTER_NARRATIVE_INTEGRITY"] = {"hook": False, "proof": False, "vision": False, "message": f"Expected at least 6 paragraphs/blocks, found {len(paragraphs)}"}
             return False
    ...
    def _validate_cover_letter_full_structure(self, context: ValidationContext) -> bool:
        cover_letter = context.staging_buffer.get(ResumeSection.K13_COVER_LETTER.value, '')
        expected_signature_block = context.expected_signature # Get expected sig from prep data
    ...
    def _validate_authenticity_signal(self, context: ValidationContext) -> bool:
        ...
        try:
            ...
            if not isinstance(target_verbs, list):
                ...
                context._cache["VG_AUTHENTICITY_SIGNAL_CHECK"] = {"details": "Target authenticity verbs from HOP-0 analysis is not a list."}
            ...
            if not target_verbs:
                ...
                context._cache["VG_AUTHENTICITY_SIGNAL_CHECK"] = {"details": "No authenticity verbs found in HOP-0 analysis to check against."}
            ...
            if not full_text.strip():
                 context._cache["VG_AUTHENTICITY_SIGNAL_CHECK"] = {"details": "No generated text content found to check verbs against."}
            ...
            if found_verbs_count < 2:
                context._cache["VG_AUTHENTICITY_SIGNAL_CHECK"] = {"details": f"Found only {found_verbs_count} matching auth-verbs. Expected 2+. (Targets: {target_verbs[:5]})"}
            ...
            context._cache["VG_AUTHENTICITY_SIGNAL_CHECK"] = {"details": f"Validation logic failed: {e}"}
    ...
    def _validate_exec_summary_vs_sections(self, context: ValidationContext) -> bool:
        ...
        if not exec_summary_text:
            ...
            context._cache["VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY"] = {"failures": "Executive Summary text not found for comparison."}
        ...
        for bullets_section_enum, (label, threshold) in sections_to_compare.items():
            ...
            if overview_section_enum:
                overview_text = context.staging_buffer.get(overview_section_enum.value, '')
            ...
            items = context.staging_buffer.get(bullets_section_enum.value, [])
            ...
            if section_content_parts:
                ...
                if hasattr(context, 'dup_detector') and context.dup_detector:
                     similarity = context.dup_detector._calculate_cosine_similarity(exec_summary_text, combined_section_text)
                else:
...
class WorkflowOrchestrator:
    ...
    # --- START NEW: Helper for _generate_qa_report ---
    def _build_qa_report_sections(
        ...
        """
        [NEW] Iterates through QA_REPORT_SECTIONS config to build all report sections.
        """
        report_lines = []
    ...
    def _validate_qa_report_formatting(self, report_text: str) -> ValidationResult:
        """
        [NEW] Validates that the generated QA report uses pre-formatted text tables.
        """
        pre_formatted_check_passed = True
        pre_formatted_check_messages = []

        # Split the report into sections to check each one
        sections = re.split(r'\n\d+\.\s', report_text)
        for i, section_block in enumerate(sections[1:], 1): # Skip header
            # Section 1 (Signal Quality) is allowed to not have tables
            if i == 1 and "SIGNAL QUALITY" in section_block:
                continue

            md_blocks = re.findall(r"```markdown(.*?)```", section_block, re.DOTALL)
            for block in md_blocks:
                if any("|" in line and "---" in block for line in block.split('\n')):
                    pre_formatted_check_passed = False
                    msg = f"QA Section {i} appears to contain Markdown table syntax."
