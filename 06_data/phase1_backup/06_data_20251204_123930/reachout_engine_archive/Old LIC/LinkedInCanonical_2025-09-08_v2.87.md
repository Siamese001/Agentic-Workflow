# LinkedInCanonical — 2025-09-10 v2.87 (FULL, ZLO-Consolidated)

Lineage: v2.73 -> v2.75 -> v2.80 -> v2.81 -> v2.82 -> v2.83 -> v2.84 -> v2.84a -> v2.85 -> v2.86 -> **v2.87**

v2.87 is a versioned zero-loss overwrite that adds explicit **CXO** routing to the C Level archetype and clarifies DRQ scope. All v2.86 hardenings and restorations remain intact.
...
- Enforce formatting (URL, Message Type, Subject, Body, Signature), structure (Capability Frame, Transition, Insights, Bridge, Bullets, CTA), archetype rules, sector framing, evidence, redundancy controls, and all v2.87 RAG and mapping checks.
...
1) **CXO token added to C Level** - The literal token **CXO** now routes to C Level and triggers DRQ.  
2) **Precedence clarified** - When CXO appears with Executive tokens, C Level routing takes precedence.  
3) **Executive DRQ scope clarified** - DRQ applies only to C Level; Executive runs require a RAG Enrichment Summary.

---
## DIFF CHECKLIST (v2.86 -> v2.87)

| Invariant or Change | Status |
|---|---|
| CXO added as C Level token | ADDED ✅ |
| CXO precedence over Executive tokens | ADDED ✅ |
| DRQ scope clarified in Executive QA Grid | ADDED ✅ |
| All v2.86 QA rows, prompts, rubrics, blockers retained | PASS ✅ |
| No regression in Short or TA flows | PASS ✅ |
