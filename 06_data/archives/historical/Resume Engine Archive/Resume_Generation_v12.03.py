...
# /Resume_Generation_v12.02.py (Section: ContentConstraintsConfig)
@dataclass
...
# /Resume_Generation_v12.02.py (Section: ArtistGenerator._generate_k1_executive_summary)
    def _generate_k1_executive_summary(self, temperature_override: Optional[float] = None) -> str:
        """
        [v11.0 REDESIGN & v12.00 FIX & v_USER_UPDATE]
        ...
        FIX: Strengthened sentence count instructions.
        USER_UPDATE: Removed word count constraint, adjusted sentence range to 7-9.
        """
        ...
        # 5. Build Feedback Instruction (Updated to remove word count feedback)
        feedback_instruction = ""
        if self.previous_failures:
            failed_rules = [f.rule_id for f in self.previous_failures]
            messages = []
            for f in self.previous_failures:
                 # Check ONLY sentence count failures now
                 if f.rule_id == "VG_SENTENCE_COUNT_K1":
            ...
            if messages:
                feedback_instruction = (
                    ...
                    "Your previous summary FAILED validation on sentence count.\n"
        ...
        # 6. Build the new, comprehensive prompt (Updated NON-NEGOTIABLE REQUIREMENTS)
