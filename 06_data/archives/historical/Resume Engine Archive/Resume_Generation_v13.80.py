...
# ============================================================================
# REASONING CONFIGURATION INSTANTIATION (RENUMBERED)
# ============================================================================
...
ReasoningConfig.K5_EY_NARRATIVE_CONFIG = ReasoningConfig(cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=3, reflexion=True)
...
ReasoningConfig.K10_SKILLS_CONFIG = ReasoningConfig(cot_min_paths=2, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=True)

ReasoningConfig.K11_COVER_LETTER_CONFIG = ReasoningConfig(cot_min_paths=3, tot_branches=2, min_tot_depth=2, self_consistency=4, reflexion=True, max_reflexion_loops=2)
...
class ArtistGenerator:
    ...
    def __init__(self, master_resume: Dict, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis, previous_failures: List[ValidationResult] = None):
        ...
        self.previous_failures = previous_failures or [] # Used only in initial attempt context
    ...
    # Provenance targets configuration (EY/Early Career targets no longer used directly for generation)
    PROVENANCE_SPLIT_TARGETS = {
        ...
        # ResumeSection.K8_EY_BULLETS: {'Customized': 2}, # No longer used for generation
        # ResumeSection.K9_EARLY_CAREER_BULLETS: {'Customized': 1}, # No longer used for generation
    }

    # --- START UPDATE: ARTIST_GENERATION_CONFIG ---
    ARTIST_GENERATION_CONFIG = [
        ...
        {"section": ResumeSection.K7_TRADERSENSE_BULLETS, "method_name": "_copy_k7_tradersense_bullets"}, # Keep copying TS bullets
        {"section": ResumeSection.K7_TRADERSENSE_OVERVIEW, "method_name": "_copy_k7_tradersense_overview"}, # Keep copying TS overview
        ...
        # --- NEW Methods for EY and Early Career ---
        {"section": ResumeSection.K8_EY_BULLETS, "method_name": "_generate_k8_ey_narrative"}, # Store narrative under K8_BULLETS key
        {"section": ResumeSection.K9_EARLY_CAREER_BULLETS, "method_name": "_generate_k9_early_career_narrative"}, # Store narrative under K9_BULLETS key
        # --- Removed old EY/Early Career entries ---
        ...
        # --- Continue with other generated sections ---
    ...
    # Hardcoded Bullet Word Count Ranges (Still used for K5, K6, K10 validation)
    BULLET_WORD_COUNT_RANGES = {
        ...
        # Removed K8/K9 ranges as they now use narrative blocks
        # ResumeSection.K8_EY_BULLETS: (25, 40),
        # ResumeSection.K9_EARLY_CAREER_BULLETS: (25, 40),
    }

    # --- _call_gemini_api (Unchanged from previous approval, returns Tuple[str, int]) ---
    def _call_gemini_api(self, prompt: str, reasoning_config: ReasoningConfig, section_id: str, system_prompt: str, temperature_override: Optional[float] = None) -> Tuple[str, int]: # Return text and call count
        ...
        Returns a tuple: (final_text, call_count)
        """
        ...
        try:
            ...
            # Configure API Key (Assumes done globally elsewhere)
            # genai.configure(api_key=api_key)
            # Initialize model (Ideally done once)
            model = genai.GenerativeModel('gemini-2.5-pro')

            api_params = reasoning_config_to_api_params(reasoning_config)
            generation_config = api_params["generation_config"]
            sc_count = api_params.get('sc', 1) # Self-consistency count
            ...
            if sc_count > 1:
                ...
                # Ensure high temperature for diverse SC candidates unless overridden explicitly
                if temperature_override is None: generation_config.temperature = 0.9
                generation_config.candidate_count = sc_count # Set candidate count for API
                candidate_responses = []
                try:
                    ...
                    # Extract text from candidates robustly
                    if hasattr(response, 'candidates') and response.candidates:
                         for c in response.candidates:
                             if hasattr(c, 'content') and hasattr(c.content, 'parts'):
                                  for part in c.content.parts:
                                       if hasattr(part, 'text') and part.text:
                                            candidate_responses.append(part.text) # Keep raw text
                    ...
                    if not candidate_responses:
                        ...
                        raise HopExecutionError(f"{section_id} SC API call returned no valid text candidates. Finish: {finish_reason}, Block: {block_reason}")
                ...
                synthesis_prompt = f"""You are a senior editor...
...
""" # (Synthesis prompt detail omitted for brevity)
                for i, res in enumerate(candidate_responses):
                    synthesis_prompt += f"\n---\n**DRAFT {i+1}:**\n{res}\n---\n"
                synthesis_prompt += "\n**FINAL SYNTHESIZED ANSWER (ensure strict adherence to original constraints, NO markdown fences ```):**\n"
                ...
                try:
                    ...
                    raw_text = getattr(synthesis_response, 'text', None) # Get raw text safely
                    if not raw_text:
                        ...
                        raise HopExecutionError(f"{section_id} SC synthesis produced no text. Block: {synth_block_reason}")
            ...
            # Single Candidate Logic
                ...
                try:
                    ...
                    raw_text = getattr(response, 'text', None) # Get raw text safely
                    if not raw_text:
                         block_reason = getattr(response.prompt_feedback, 'block_reason', None) if hasattr(response, 'prompt_feedback') else None
                         raise HopExecutionError(f"{section_id} generation returned no text. Block: {block_reason}")
    ...
    # --- generate method (Unchanged from previous approval, returns Tuple[Dict, List[ValidationResult], int]) ---
    def generate(
        self,
    ...
    # --- _generate_artist_output (Updated to handle new K8/K9 methods) ---
    def _generate_artist_output(
        ...
        Passes current output dict to overview generators. Aggregates and returns total API calls.
        UPDATED: Handles new K8/K9 narrative generation methods.
        """
        output = {}
        total_api_calls = 0
        ...
        for config in self.ARTIST_GENERATION_CONFIG:
            ...
            if section_enum not in sections_to_generate:
                # Run idempotent copy/dummy methods anyway
            ...
            try:
                ...
                # New Narrative methods (K8, K9)
                elif method_name in ["_generate_k8_ey_narrative", "_generate_k9_early_career_narrative"]:
                     generated_content, section_api_calls = method(temperature_override=final_temp) # Pass only temp
                # Default case for most generation methods
                else:
                ...
                # Basic check for placeholders (should ideally be caught by validation later)
                if isinstance(generated_content, str) and "[Placeholder" in generated_content:
                    ...
                    # raise HopExecutionError(f"{section_enum.value} generation returned placeholder unexpectedly.") # Optionally halt here
    ...
    def _copy_k7_tradersense_bullets(self) -> List[str]: # Returns list of strings
        tradersense_exp = next((exp for exp in self.master_resume['professional_experience'] if 'TraderSense' in exp['company']), None)
        # Return bullets directly from 'highlights' which now contains the 5 approved strings
        return tradersense_exp.get('highlights', []) if tradersense_exp else []
    def _copy_k7_tradersense_overview(self) -> str: # Keep copying overview if it exists
        tradersense_exp = next((exp for exp in self.master_resume['professional_experience'] if 'TraderSense' in exp['company']), None)
        # Return overview if present (even though master was modified, keep this robust)
    ...
    # --- LLM Generation Methods (with added negative constraint) ---
    ...
    def _generate_k0_headline(self, temperature_override: Optional[float] = None) -> Tuple[str, int]: # Return tuple
        ...
        feedback_instruction = "" # Add feedback logic if needed from self.previous_failures
...
6.  **Do NOT start the headline or any component with phrases like 'At [Company]', 'As [Title]', etc.** # <-- ADDED CONSTRAINT
...
"""
    ...
    def _generate_k1_executive_summary(self, temperature_override: Optional[float] = None) -> Tuple[str, int]: # Return tuple
        ...
        archetype_map = {
            "Executive_Leader": "an executive leader", "Technical_IC": "a hands-on technical expert",
            "Post-Sales_Customer_Success": "a customer success leader", "Pre-Sales_GTM": "a pre-sales GTM strategist",
        ...
        # Feedback from previous attempts (using self.previous_failures)
        feedback_instruction = ""
        # (Existing feedback logic based on self.previous_failures remains here)
        ...
        word_count_failures = [f for f in self.previous_failures if f.rule_id == "VG_WORD_COUNT_K1"]
...
7.  **Do NOT start any sentence with phrases like 'At [Company]', 'As [Title]', 'In my role as...', etc. Focus directly on the capability or value.** # <-- ADDED CONSTRAINT
{feedback_instruction}
...
6.  **Do NOT include introductory phrases like 'Skills include:' etc.** # <-- ADDED CONSTRAINT (Implicitly covered by #5, but explicit)
...
"""
            ...
            skills_list_final = []
            # Split by newline, filter empty, remove leading/trailing whitespace and potential list markers
            skills_intermediate = [re.sub(r'^[•*\-\d\.]+\s*', '', s).strip() for s in skills_text.split('\n') if s.strip()]
            malformed_count = 0
            for skill in skills_intermediate:
                word_count = len(skill.split()) # Simple split for word count check
                if 1 <= word_count <= 3:
                    skills_list_final.append(skill)
                else:
                    logging.warning(f"K.2: Discarding malformed skill '{skill}' (words: {word_count})")
                    malformed_count += 1
            ...
            if malformed_count > 0:
                 # This check might be redundant if the length check already failed, but keep for clarity
    ...
    # --- Overview Generation (K5, K6 - with added negative constraint) ---
    def _generate_tailored_overview_for_experience(
        ...
        """
        Generates tailored overviews (K5, K6) by synthesizing provided bullet points AND
        incorporating high-level themes (Leadership, Strategy, Tech Depth) from HOP-0.
        Raises error on API failure. Returns (text, call_count).
        """
        ...
        for i, bullet_data in enumerate(generated_bullets):
             ...
             elif isinstance(bullet_data, str): # Handle case if list contains strings
                  text = bullet_data
             if not text:
                  logging.warning(f"Skipping empty bullet at index {i} during overview synthesis for {section_id}")
                  continue
             bullet_texts.append(f"* {text.strip()}") # Ensure bullets start with '* '

        if not bullet_texts:
            raise HopExecutionError(f"Cannot generate overview for {section_id}: All provided bullets were empty or invalid.")

        bullet_summary_input = "\n".join(bullet_texts)
        min_wc, max_wc = word_count_range

        # Extract Themes from HOP-0
        ...
        # Determine theme relevance (logic unchanged)
        ...
        theme_instructions = []
        if include_leadership_theme: theme_instructions.append("* **Leadership:** Reflect management, team building, or leading initiatives.")
        if include_strategic_theme: theme_instructions.append("* **Strategic Vision:** Mention strategy, long-term impact, transformations, or partnerships.")
        if include_technical_theme: theme_instructions.append("* **Technical Depth:** Hint at architectural oversight, solution design, or technical guidance.")
...
7.  **Do NOT start the overview with phrases like 'At [Company]', 'As [Title]', etc.** # <-- ADDED CONSTRAINT
...
"""
        ...
        # Basic post-check for prompt artifacts
        ...
        # Word/Sentence count validation happens in HOP-5, but log warning if clearly off
        final_wc = count_words_ms_word_style(synthesized_overview)
        final_sc = _count_sentences(synthesized_overview)
        ...
        if not (1 <= final_sc <= 2):
             logging.warning(f"{section_id} generated overview sentence count ({final_sc}) is outside target (1-2). Text: '{synthesized_overview}'")
    ...
    def _generate_k5_unify_overview(self, current_output: Dict, temperature_override: Optional[float] = None) -> Tuple[str, int]: # Return tuple
        # Retrieves K5 bullets from current_output state passed by _generate_artist_output
        ...
        if not generated_bullets or not isinstance(generated_bullets, list):
             # Log detailed error including type and preview if possible
             content_type = type(generated_bullets).__name__
             content_preview = str(generated_bullets)[:100] if generated_bullets is not None else "None"
             raise HopExecutionError(f"Cannot generate {ResumeSection.K5_UNIFY_OVERVIEW.value}: Corresponding bullets not found or invalid type ({content_type}). Preview: {content_preview}")
    ...
    def _generate_k6_ibm_overview(self, current_output: Dict, temperature_override: Optional[float] = None) -> Tuple[str, int]: # Return tuple
        # Retrieves K6 bullets from current_output state
        generated_bullets = current_output.get(ResumeSection.K6_IBM_BULLETS.value)
        if not generated_bullets or not isinstance(generated_bullets, list):
             content_type = type(generated_bullets).__name__
             content_preview = str(generated_bullets)[:100] if generated_bullets is not None else "None"
             raise HopExecutionError(f"Cannot generate {ResumeSection.K6_IBM_OVERVIEW.value}: Corresponding bullets not found or invalid type ({content_type}). Preview: {content_preview}")
    ...
    # --- START NEW: K8 EY Narrative Generation ---
    def _generate_k8_ey_narrative(self, temperature_override: Optional[float] = None) -> Tuple[str, int]:
        """
        Generates the 3-sentence narrative block for EY (K.8).
        Target: 40-60 words total. Weighting: 70% RAG/K.0 Themes, 30% Master Resume content.
        Returns (narrative_text, call_count).
        """
        min_wc = self.constraints.EY_NARRATIVE_WORD_COUNT_MIN
        max_wc = self.constraints.EY_NARRATIVE_WORD_COUNT_MAX
        section_id = ResumeSection.K8_EY_BULLETS.value # Store under this key

        # --- Gather Context ---
        # RAG-0 / K.0 Themes
        primary_theme = self.thematic_analysis.primary_theme.get('name', 'key capabilities')
        rag_keywords = []
        if hasattr(self.thematic_analysis, 'competitive_intelligence'):
            rag_keywords = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:5]
        # K.0 Themes
        k0_themes = ["Leadership", "Strategic Vision", "Executive Communication", "Cross-functional Alignment"] # Add relevant K.0 concepts

        # Master Resume Content (EY Bullets - use the updated list)
        ey_exp = next((exp for exp in self.master_resume['professional_experience'] if 'Ernst & Young' in exp['company']), None)
        master_bullets_text = []
        if ey_exp and 'highlights' in ey_exp:
            master_bullets_text = ey_exp['highlights']
        if not master_bullets_text:
            raise HopExecutionError(f"Cannot generate {section_id}: Master resume bullets for EY not found.")
        master_context = "\n".join([f"- {b}" for b in master_bullets_text])

        # --- Build Prompt ---
        prompt = f"""Generate a concise narrative block (exactly 3 sentences, total {min_wc}-{max_wc} words) summarizing the Ernst & Young Principal experience.

**Focus (70%):** Emphasize how this early experience built foundational capabilities in **{', '.join(k0_themes)}** and relates to current target themes like **{', '.join(rag_keywords)}**. Use past tense.

**Context (30%):** Ground the narrative in the technical achievements from the master resume bullets below, incorporating key details like team size, client type, regulatory areas (CCAR, Solvency II), and quantified impact ($16M, 38%, 19%).

**Master Resume Bullets (Context Only):**
{master_context}

**ABSOLUTELY CRITICAL:**
1.  Output EXACTLY 3 sentences.
2.  Total word count MUST be strictly between {min_wc} and {max_wc} words.
3.  Weight the content roughly 70% towards transferable executive themes/RAG signals and 30% towards the specific technical context.
4.  Use PAST TENSE.
5.  **Do NOT start sentences with phrases like 'At Ernst & Young', 'As Principal', etc.** # <-- ADDED CONSTRAINT
6.  Output ONLY the 3 sentences as a single paragraph. Do NOT use markdown fences or bullet points.

**Generated Narrative ({min_wc}-{max_wc} words total, 3 sentences):**
"""
        reasoning_config = ReasoningConfig.K8_EY_OVERVIEW_CONFIG # Reuse overview config for narrative
        base_system = "You are an expert resume writer crafting concise narratives that bridge past experience to current executive capabilities."
        ...
        # Basic post-check (Validation in HOP-5 will be more thorough)
        final_wc = count_words_ms_word_style(narrative_text)
        final_sc = _count_sentences(narrative_text)
        if not (min_wc <= final_wc <= max_wc):
             logging.warning(f"{section_id} narrative word count ({final_wc}) outside target ({min_wc}-{max_wc}).")
        if final_sc != 3:
             logging.warning(f"{section_id} narrative sentence count ({final_sc}) is not 3.")

        return narrative_text, call_count
    # --- END NEW K8 ---

    # --- START NEW: K9 Early Career Narrative Generation ---
    def _generate_k9_early_career_narrative(self, temperature_override: Optional[float] = None) -> Tuple[str, int]:
        """
        Generates the 3-sentence narrative block for Early Career (K.9).
        Target: 50-70 words total. Weighting: 1/3 Technical, 2/3 Storytelling/RAG bridge.
        No senior K.0 themes. Returns (narrative_text, call_count).
        """
        ...
        section_id = ResumeSection.K9_EARLY_CAREER_BULLETS.value # Store under this key

        # --- Gather Context ---
        # RAG-0 Signals (Keywords relevant to tech career bridge)
        rag_keywords = []
        if hasattr(self.thematic_analysis, 'competitive_intelligence'):
            rag_keywords = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:5]
        rag_signals = ["scalable systems", "data analysis", "cloud platforms", "technology partnerships", "solution architecture"] # Example signals based on JD
        combined_signals = list(set(rag_keywords + rag_signals))[:7] # Combine and limit

        # Master Resume Content (Early Career Overview & Highlight)
        ec_exp = next((exp for exp in self.master_resume['professional_experience'] if 'Early Career' in exp['company']), None)
        master_overview = ec_exp.get('overview', '') if ec_exp else ''
        master_highlights = ec_exp.get('highlights', []) if ec_exp else []
        if not master_overview and not master_highlights:
             raise HopExecutionError(f"Cannot generate {section_id}: Master resume content for Early Career not found.")
        # Combine overview and highlights for context
        master_context = master_overview + "\n" + "\n".join([f"- {h}" for h in master_highlights])

        # --- Build Prompt ---
        prompt = f"""Generate a concise narrative block (exactly 3 sentences, total {min_wc}-{max_wc} words) describing the Early Career actuarial experience and bridging it to a technology career.

**Weighting:**
* **1/3 Focus:** Ground the narrative in the specific technical work done (stochastic models, derivatives, annuities, distributed computing, valuations). Use context below.
...
**Master Resume Context (Use for 1/3 technical grounding):**
{master_context}
...
6.  **Do NOT start sentences with phrases like 'During my early career', 'As an analyst', etc.** # <-- ADDED CONSTRAINT
7.  Output ONLY the 3 sentences as a single paragraph. Do NOT use markdown fences or bullet points.

**Generated Narrative ({min_wc}-{max_wc} words total, 3 sentences):**
"""
        # Use simple reasoning for this less critical section
    ...
    # --- Bullet Generation Helpers (with added negative constraint) ---
    ...
    def _validate_llm_bullet_selection(self, selected_bullets_text: List[str], master_bullets_structured: List[Dict], expected_count: int, section_id: str) -> List[Dict]:
        ...
        for selected_text in selected_bullets_text:
            ...
            # Try matching even if LLM added/removed minor punctuation like a period
            matched_bullet = master_texts_map.get(cleaned_text) or master_texts_map.get(cleaned_text.rstrip('.'))
            if matched_bullet:
                 original_text = matched_bullet['bullet_text'].strip() # Use original text for set check
                 if original_text in returned_texts_set:
                      msg = f"{section_id} LLM returned duplicate bullet (based on original text): '{original_text[:50]}...'"
                      logging.error(msg)
                      raise HopExecutionError(msg)
                 validated_bullets.append(matched_bullet) # Append the original structured bullet
                 returned_texts_set.add(original_text)
            else:
                # Log details to help diagnose matching failure
                nearby_keys = [k[:50] for k in master_texts_map.keys() if k.startswith(cleaned_text[:10])]
                msg = f"{section_id} LLM returned bullet not found in master list or significantly modified: '{cleaned_text[:50]}...'. Nearby master keys: {nearby_keys}"
                logging.error(msg)
                raise HopExecutionError(msg)
        if len(validated_bullets) != expected_count:
             # This check should be redundant if validation above is correct, but keep as safety net
...
3. **Do NOT start the rewritten bullet with phrases like 'At [Company]', 'As [Title]', etc.** # <-- ADDED CONSTRAINT

REWRITTEN BULLET ({min_wc}-{max_wc} words):
"""
            reasoning_config = ReasoningConfig.DEFAULT # Simple rewrite task
            ...
            if not (min_wc <= rewritten_wc <= max_wc):
                 ...
                 raise HopExecutionError(msg) # Fail fast

            return rewritten_text, total_calls

        except HopExecutionError as he: raise he # Propagate API/validation errors
        except Exception as e: raise HopExecutionError(f"{section_id}_RewriteWC failed unexpectedly: {e}") from e


    def _validate_and_potentially_rewrite_bullets(
        self,
        selected_bullets_structured: List[Dict], # List of dicts with 'text', 'provenance', 'word_count'
        min_target: int,
        max_target: int,
        section_id_for_logging: str, # e.g., "K.5 Unify"
        temperature_override: Optional[float] = None
    ) -> Tuple[List[Dict], int]: # Return tuple (final_list, total_calls)
        """
        Checks word count against range, attempts rewrite if needed using _rewrite_bullet_for_word_count.
        Raises HopExecutionError on failure. Returns (final_list, total_rewrite_calls).
        """
        final_bullets = []
        ...
        for i, bullet_data in enumerate(selected_bullets_structured):
            ...
            original_provenance = bullet_data.get('provenance', BulletProvenance.Verbatim.value) # Default if missing
            # Recalculate word count for safety, fallback to stored if calculation fails
            try: word_count = count_words_ms_word_style(original_text)
            except: word_count = bullet_data.get('word_count', 0)
            ...
            if not (min_target <= word_count <= max_target):
                ...
                try:
                    # _rewrite_bullet_for_word_count raises error on failure
                    ...
                    rewritten_word_count = count_words_ms_word_style(rewritten_text) # Use recalculated count
                    logging.info(f"    Rewrite SUCCESSFUL for {section_id_for_logging}[{i}]. New count: {rewritten_word_count}")

                    # Update provenance if original was Verbatim
                    ...
                    # Append rewritten bullet data
                    final_bullets.append({
                        "text": rewritten_text,
                        "provenance": new_provenance,
                        "word_count": rewritten_word_count,
                        "original_text_if_rewritten": original_text # Store original for reference
                    })
    ...
    ) -> Tuple[List[Dict], int]: # Return tuple (list_of_dicts, call_count)
        """
        Lightly rewrites bullets based on thematic analysis. Raises HopExecutionError on failure.
        Returns list of dicts: [{"text": ..., "provenance": "Customized", "word_count": ...}]
        """
        total_calls = 0
        try:
            if not source_bullets_text: return [], 0

            # Gather context keywords
            ...
            context_keywords = list(set(primary_theme_kw + diff_kw))[:7] # Top 7 combined for subtle emphasis
...
7.  **CRITICAL: Do NOT start rewritten bullets with phrases like 'At [Company]', 'As [Title]', etc.** # <-- ADDED CONSTRAINT

REWRITTEN BULLETS:
"""
            reasoning_config = ReasoningConfig.DEFAULT # Simple rewrite
            ...
            if len(rewritten_bullets_text) != len(source_bullets_text):
                msg = f"{section_id} LLM returned {len(rewritten_bullets_text)} customized bullets, expected {len(source_bullets_text)}. Output preview: {response_text[:100]}..."
                logging.error(msg)
                raise HopExecutionError(msg) # Fail fast
        ...
        except Exception as e: raise HopExecutionError(f"{section_id} customization failed unexpectedly: {e}") from e


    def _generate_synthetic_bullets(
        self,
        count: int, company_name: str, job_description: str,
        thematic_analysis: ThematicAnalysis, context_bullets: str, # Context = string of existing bullets
        reasoning_config: ReasoningConfig, section_id: str,
        temperature_override: Optional[float] = None
    ) -> Tuple[List[Dict], int]: # Return tuple (list_of_dicts, call_count)
        """
        Generates plausible, synthetic resume bullets. Raises HopExecutionError on failure.
        Returns list of dicts: [{"text": ..., "provenance": "Synthetic", "word_count": ...}]
        """
        total_calls = 0
        try:
            if count <= 0: return [], 0
...
7.  **CRITICAL: Do NOT start generated bullets with phrases like 'At [Company]', 'As [Title]', etc.** # <-- ADDED CONSTRAINT
...
"""
        ...
        except Exception as e: raise HopExecutionError(f"{section_id} synthetic generation failed unexpectedly: {e}") from e


    # --- Generic Bullet Generation Helper (Calls V/C/S helpers) ---
    def _generate_tailored_bullets_for_experience(
            self,
            company_name: str,
            # section_index: int, # No longer needed
            provenance_targets: Dict[str, int],
            reasoning_config: ReasoningConfig, section_id: ResumeSection,
            temperature_override: Optional[float] = None
    ) -> Tuple[List[Dict], int]: # Return tuple (list_of_dicts, total_calls)
        """
        Generic method to generate bullets (Verbatim, Customized, Synthetic) for K.5, K.6, K.10.
        ...
        if not exp_section: raise HopExecutionError(f"Master data not found for '{company_name}' needed by {section_id.name}")

        # Determine source key ('bullet_pool' preferred, fallback to 'highlights')
        ...
        if not isinstance(master_bullets_source, list): raise HopExecutionError(f"{section_id.name} master '{master_bullets_key}' is not a list for '{company_name}'.")

        # Create structured master list (text, provenance, word_count)
        ...
        for bullet_text in master_bullets_source:
             if isinstance(bullet_text, str) and bullet_text.strip():
                 ...
                 master_bullets_structured.append({
                     "bullet_text": cleaned_text, "text": cleaned_text, # Include both for compatibility
                     "provenance": BulletProvenance.Verbatim.value,
                     "word_count": count_words_ms_word_style(cleaned_text)
                 })
        ...
        if not master_bullets_structured and (verbatim_count > 0 or customized_count > 0):
             raise HopExecutionError(f"{section_id.name} Cannot select/customize bullets: No valid master bullets found for '{company_name}'.")

        # --- 2. Select Verbatim Bullets (if needed) ---
        verbatim_bullets_selected = [] # Store selected verbatim dicts
        if verbatim_count > 0:
            ...
            # Prepare prompt for LLM selection (more robust than random)
            master_bullets_text_list = [b['bullet_text'] for b in master_bullets_structured]
            keywords_for_prompt = []
            if hasattr(self.thematic_analysis, 'competitive_intelligence'):
                 keywords_for_prompt = getattr(self.thematic_analysis.competitive_intelligence, 'differentiator_keywords', [])[:10]
...
"""
            ...
            try:
                # API call returns (text, calls)
                response_select, calls_v_select = self._call_gemini_api(prompt_select, ReasoningConfig.DEFAULT, f"{section_id.name}_SelectV", system_prompt_select, temperature_override=temperature_override)
                ...
                # Validate selection against master list (raises HopExecutionError on failure)
                verbatim_bullets_selected = self._validate_llm_bullet_selection(selected_texts, master_bullets_structured, verbatim_count, f"{section_id.name}_SelectV")
                final_bullets.extend(verbatim_bullets_selected) # Add validated dicts
            except HopExecutionError as he: raise he # Propagate selection/validation errors

        # --- 3. Generate Customized Bullets (if needed) ---
        if customized_count > 0:
            logging.info(f"    Customizing {customized_count} bullets...")
            # Select unique bullets not used for verbatim
            ...
            if len(available_for_custom) < customized_count:
                raise HopExecutionError(f"{section_id.name} Cannot customize {customized_count} bullets: Only {len(available_for_custom)} unique master bullets remaining after verbatim selection.")

            # Select candidates (e.g., randomly or using relevance)
            random.shuffle(available_for_custom) # Simple random selection for now
        ...
        if synthetic_count > 0:
            ...
            try:
                ...
                synthetic_bullets, calls_s = self._generate_synthetic_bullets(
                    ...
                    context_bullets=context_bullets_text, reasoning_config=reasoning_config, # Use section's reasoning
                    section_id=f"{section_id.name}_SynthS",
                    temperature_override=temperature_override
                )
                total_calls_for_section += calls_s
                final_bullets.extend(synthetic_bullets) # Add synthetic dicts
            except HopExecutionError as he: raise he # Propagate synthetic generation errors

        # --- 5. Final Count Check ---
        if len(final_bullets) != total_expected_count:
            # This indicates an internal logic error if previous steps passed
            raise HopExecutionError(f"{section_id.name} Internal Error: Generated incorrect total bullet count ({len(final_bullets)}), expected {total_expected_count}.")

        # --- 6. Word Count Validation & Potential Rewrite ---
        target_range = self.BULLET_WORD_COUNT_RANGES.get(section_id)
        if target_range is None:
             raise HopExecutionError(f"Configuration Error: Hardcoded word count range not found for section {section_id.name}.")
        min_target, max_target = target_range

        logging.info(f"    Validating word counts for {len(final_bullets)} generated bullets ({min_target}-{max_target})...")
        try:
            # Helper validates and potentially rewrites, returns (validated_list, rewrite_calls)
            final_bullets_validated, calls_rewrite = self._validate_and_potentially_rewrite_bullets(
                selected_bullets_structured=final_bullets, # Pass the list of dicts
                min_target=min_target, max_target=max_target,
                section_id_for_logging=section_id.name, # Pass Enum name
                temperature_override=temperature_override
            )
        ...
        # --- 7. Reorder Bullets (Optional but recommended) ---
        logging.info(f"    Reordering {len(final_bullets)} bullets for impact...")
        # Prepare text list for reordering prompt
...
"""
        ...
        try:
            ...
            # Parse reordered text (one bullet per line)
            reordered_texts_raw = [line.strip() for line in response_reorder.split('\n') if line.strip()]
            # Clean potential numbering added by LLM
            ...
            if len(reordered_texts) != total_expected_count:
                 raise HopExecutionError(f"{section_id.name} Reordering failed: Expected {total_expected_count} bullets, got {len(reordered_texts)}. Preview: {reordered_texts_raw[:3]}")

            final_ordered_bullets_dicts = []
            # Map original texts (from validated list) to their dicts for easy lookup
            original_texts_map = {b.get('text'): b for b in final_bullets}
            used_original_texts = set()

            for reordered_text in reordered_texts:
                 # Find the original dict matching the reordered text
                 matched_dict = original_texts_map.get(reordered_text)
                 if matched_dict:
                     # Check for duplicates introduced by reordering error
                     if reordered_text in used_original_texts:
                          raise HopExecutionError(f"{section_id.name} Reordering failed: Duplicate bullet found in output: '{reordered_text[:50]}...'")
                     final_ordered_bullets_dicts.append(matched_dict)
                     used_original_texts.add(reordered_text)
                 else:
                     # LLM modified the text during reordering - critical failure
                     raise HopExecutionError(f"{section_id.name} Reordering failed: LLM modified bullet text. Expected verbatim. Got: '{reordered_text[:50]}...'")

            # Final check to ensure all original bullets were returned exactly once
            if len(final_ordered_bullets_dicts) != total_expected_count:
                 raise HopExecutionError(f"{section_id.name} Reordering failed: Final count mismatch after matching ({len(final_ordered_bullets_dicts)} vs {total_expected_count}).")

            logging.info(f"  ✓ Reordering complete for {section_id.name}.")
            return final_ordered_bullets_dicts, total_calls_for_section

        except HopExecutionError as he: raise he # Propagate reorder/validation errors
        except Exception as e: raise HopExecutionError(f"{section_id.name} Reordering failed unexpectedly: {e}") from e

    # --- Methods calling the Generic Bullet Generator ---
    def _generate_k5_unify_bullets(self, temperature_override: Optional[float] = None) -> Tuple[List[Dict], int]: # Return tuple
        # Use targets defined in PROVENANCE_SPLIT_TARGETS
        targets = self.PROVENANCE_SPLIT_TARGETS.get(ResumeSection.K5_UNIFY_BULLETS, {})
        if not targets: raise HopExecutionError("Provenance targets missing for K.5 Unify")
        return self._generate_tailored_bullets_for_experience(
            ...
            section_id=ResumeSection.K5_UNIFY_BULLETS,
            temperature_override=temperature_override
        )

    def _generate_k6_ibm_bullets(self, temperature_override: Optional[float] = None) -> Tuple[List[Dict], int]: # Return tuple
        targets = self.PROVENANCE_SPLIT_TARGETS.get(ResumeSection.K6_IBM_BULLETS, {})
        if not targets: raise HopExecutionError("Provenance targets missing for K.6 IBM")
        return self._generate_tailored_bullets_for_experience(
            company_name="IBM",
            provenance_targets=targets,
            reasoning_config=ReasoningConfig.K6_IBM_BULLETS_CONFIG or ReasoningConfig.DEFAULT,
            section_id=ResumeSection.K6_IBM_BULLETS,
            temperature_override=temperature_override
        )
    ...
    def _generate_k10_competencies(self, temperature_override: Optional[float] = None) -> Tuple[List[Dict], int]: # Return tuple
        ...
        # Define a temporary 'company' name for sourcing competencies
        fake_company_name = "Strategic Competencies Section"
        competencies_list_source = self.master_resume.get("strategic_and_technical_competencies", [])
        if not competencies_list_source:
             raise HopExecutionError("Master resume missing 'strategic_and_technical_competencies'.")

        # Temporarily modify master resume structure for the helper function (use deepcopy)
        ...
        temp_master_resume["professional_experience"].append({
             "company": fake_company_name,
             "bullet_pool": competencies_list_source # Use competencies as the 'bullet_pool'
        })
        original_master_resume = self.master_resume # Backup original
        self.master_resume = temp_master_resume # Temporarily swap

        total_calls = 0
        generated_competencies = []
        try:
            targets = self.PROVENANCE_SPLIT_TARGETS.get(ResumeSection.K10_COMPETENCIES, {})
            if not targets: raise HopExecutionError("Provenance targets missing for K.10 Competencies")

            # Call the generic bullet generator
            generated_competencies, calls = self._generate_tailored_bullets_for_experience(
                company_name=fake_company_name, # Use the fake name
                provenance_targets=targets,
                reasoning_config=ReasoningConfig.K10_COMPETENCIES_CONFIG or ReasoningConfig.DEFAULT,
                section_id=ResumeSection.K10_COMPETENCIES,
                temperature_override=temperature_override
            )
            total_calls = calls
        finally:
             self.master_resume = original_master_resume # Restore original master resume

        # Post-processing: Remove potential bolding/prefixes added during generation
        for item in generated_competencies:
             if isinstance(item, dict) and 'text' in item:
                 # Remove leading "* **...:** " patterns
                 cleaned_text = re.sub(r'^\*\s*\*\*(.*?):\*\*\s*', '', item['text']).strip()
                 # Remove just leading bullet if missed above
    ...
    # --- K8/K9 Bullets/Overviews REMOVED, replaced by Narrative methods ---
    # def _generate_k8_ey_bullets(...) -> REMOVED
    # def _generate_k8_ey_overview(...) -> REMOVED
    # def _generate_k9_early_career_bullets(...) -> REMOVED
    # def _generate_k9_early_career_overview(...) -> REMOVED


    # --- Cover Letter Generation (with added negative constraint) ---
    def _generate_k13_cover_letter(self, temperature_override: Optional[float] = None) -> Tuple[str, int]: # Return tuple
        """Generates K.13 cover letter. Raises HopExecutionError on failure."""
        ...
        # Ensure COVER_LETTER_SIGNATURE_TEMPLATE exists and is formatted correctly
        try:
             ...
             raise HopExecutionError("COVER_LETTER_SIGNATURE_TEMPLATE not defined.")
        except KeyError as e:
             raise HopExecutionError(f"Missing key in COVER_LETTER_SIGNATURE_TEMPLATE format: {e}")


        # Get experience snippets (use generated content if available, fallback to enriched)
        exp_snippets = ""
        # Try getting generated Unify/IBM content first
        unify_overview = self.enriched_scaffold.get(ResumeSection.K5_UNIFY_OVERVIEW.value, "") # Use enriched as fallback? Or require generated?
        unify_bullets = self.enriched_scaffold.get(ResumeSection.K5_UNIFY_BULLETS.value, [])
        ibm_overview = self.enriched_scaffold.get(ResumeSection.K6_IBM_OVERVIEW.value, "")
        ibm_bullets = self.enriched_scaffold.get(ResumeSection.K6_IBM_BULLETS.value, [])
        ...
        if not exp_snippets: # Fallback if specific sections missing
             exp_snippets = "Candidate has extensive experience in relevant areas.\n"
...
INSTRUCTIONS:
1.  **Standard Letter Format:** Include today's date ({datetime.now().strftime("%B %d, %Y")}), recipient address placeholder ([Company Name]), salutation ("Dear Hiring Manager,"), 3 body paragraphs, closing ("Sincerely,"), and the EXACT signature block provided below.
...
5.  **Signature:** Append the following signature block EXACTLY as shown, including line breaks (ensure template forces line breaks, e.g., using double spaces):
{expected_signature}
...
4. **Do NOT start paragraphs or key sentences with phrases like 'At [Company]', 'As [Title]', 'In my previous role...', etc. Focus directly on the connection to the target role.** # <-- ADDED CONSTRAINT

COVER LETTER:
"""
        total_calls = 0
        try:
            reasoning_config = ReasoningConfig.DEFAULT # Cover letter is creative but structured
            ...
            # --- Post-generation Structure Fixes (Attempt to ensure format) ---
            ...
            # Use rstrip comparison to handle potential trailing whitespace variations
            if not fixed_text.rstrip().endswith(expected_signature.rstrip()):
                 logging.warning("K.13: Signature block missing or malformed at end. Attempting to append/fix.")
                 # Remove any potential duplicate/partial signature before appending
                 fixed_text = re.sub(r'\n*Sincerely,?\n*.*$', '', fixed_text.rstrip(), flags=re.MULTILINE)
                 fixed_text += f"\n\n{closing}\n\n{expected_signature}"
                 # Final check
                 if not fixed_text.rstrip().endswith(expected_signature.rstrip()):
            ...
            # Final basic checks (Validation handles more detailed checks)
            if "[Placeholder" in fixed_text or "[Your Name]" in fixed_text:
                 raise HopExecutionError("K.13 generation failed (placeholder detected after fixes).")
            # Check structure again loosely after fixes
            if not all(x in fixed_text for x in [current_date_str, recipient_placeholder, salutation, closing, expected_signature]):
                 logging.warning("K.13: Structure may still be incomplete after fixes.")


            return fixed_text.strip(), total_calls

        except HopExecutionError as he: raise he # Propagate known errors
        except Exception as e: raise HopExecutionError(f"K.13 generation failed unexpectedly: {e}") from e
...
# Assume necessary classes like ImmutableStagingBuffer, ThematicAnalysis,
# ResumeSection, ContentConstraintsConfig, SignalControlConfig,
# count_words_ms_word_style, _count_sentences are defined elsewhere and imported.
# Also assume DuplicateDetector is defined elsewhere.
...
class ValidationContext:
    ...
    def _calculate_expected_signature(self):
        ...
        if 'COVER_LETTER_SIGNATURE_TEMPLATE' not in globals():
             # Provide a fallback or raise an error if template is missing
    ...
    # --- START ADDED METHODS for K.8/K.9 Narrative Checks ---
    def _calculate_k8_narrative_details(self):
        """Calculates details for K.8 EY Narrative rules."""
        # Narrative is stored under the K.8_EY_BULLETS key
        text = self.staging_buffer.get(ResumeSection.K8_EY_BULLETS.value, '')
        word_count = count_words_ms_word_style(text)
        sentence_count = _count_sentences(text)
        details = {
            'word_count': word_count,
        ...
        # Cache details for specific rule IDs
        self._cache["VG_NARRATIVE_WORD_COUNT_K8"] = details
        self._cache["VG_NARRATIVE_SENTENCE_COUNT_K8"] = details
        return details

    def _calculate_k9_narrative_details(self):
        """Calculates details for K.9 Early Career Narrative rules."""
        # Narrative is stored under the K.9_EARLY_CAREER_BULLETS key
        text = self.staging_buffer.get(ResumeSection.K9_EARLY_CAREER_BULLETS.value, '')
        word_count = count_words_ms_word_style(text)
        sentence_count = _count_sentences(text)
        details = {
            'word_count': word_count,
        ...
        # Cache details for specific rule IDs
        self._cache["VG_NARRATIVE_WORD_COUNT_K9"] = details
        self._cache["VG_NARRATIVE_SENTENCE_COUNT_K9"] = details
        return details
    # --- END ADDED METHODS ---
...
# Assume other necessary imports like Enum, dataclass, field, Dict, List, Optional, Any, Tuple, Set, Union,
# datetime, hashlib, json, logging, os, copy, re, defaultdict are already present.
# Also assume helper classes like HopCheckpoint, HopStatus, GateDecision, ValidationResult,
# ValidationSeverity, EnhancedJobDescriptionAnalyzer, ClerkExtractor, DataEnricher,
# ArtistGenerator, ImmutableStagingBuffer, TextSanitizer, PreFlightValidator,
# GateDecisionEngine, FileRenderer, ThematicAnalysis, RAGConfig, ContentConstraintsConfig,
# SignalControlConfig, BulletProvenance, HopExecutionError etc. are defined.
...
class WorkflowOrchestrator:
    ...
    # Method for Section 1 (_build_qa_section_1_signal_quality) remains UNCHANGED (detailed ASCII)
    ...
    def _build_qa_section_4_word_count_distribution(self, validation_results: List[ValidationResult]) -> List[str]:
        ...
        rules_to_check = [
            ...
            "VG_SENTENCE_COUNT_K1", "VG_WORD_COUNT_K5_OVERVIEW", "VG_SENTENCE_COUNT_K5_OVERVIEW",
            "VG_WORD_COUNT_K6_OVERVIEW", "VG_SENTENCE_COUNT_K6_OVERVIEW",
            "WORD_DISTRIBUTION_UNIFY_IBM", "UNIFY_IBM_RATIO", "COVER_LETTER_STRUCTURE",
        ]
        failures = []
        checks_run = 0
        checks_passed = 0
    ...
    def _build_qa_section_8_pairwise_similarity(self) -> List[str]:
        ...
        if self.overview_similarity_data is not None:
            checks_run += 1
            excluded_labels = ["Unify Overview", "IBM Overview"]
            relevant_data = [r for r in self.overview_similarity_data if r.get("section") not in excluded_labels]
    ...
    def _build_qa_section_13_final_format(
        ...
        formatting_rule_ids = [
             "QA_TABLE_FORMAT_INVALID",
             "APP_TRACKER_VALIDATION" # Represents JSON validity, schema, etc.
             # Add specific rules like VG_RESUME_HEADER_H2 etc. if they are run post-rendering
        ]
        for rule_id in formatting_rule_ids:
             result = next((vr for vr in validation_results if vr.rule_id == rule_id), None)
             if result:
                 checks_run += 1
                 if result.passed: checks_passed += 1
    ...
    def _format_ascii_bar_chart(self, label: str, value: float, target_min: float, target_max: float, temperature: Optional[Union[float, str]] = None, bar_length: int = 10, is_summary: bool = False) -> str:
        # ... (Keep existing method content, ensure temp is handled if None/str) ...
    ...
    def _format_plain_text_table(
        ...
        # ... (Keep existing method content with robust str conversion) ...
    ...
    def _invoke_deduplication_analysis(self, staging_buffer: ImmutableStagingBuffer) -> bool:
        ...
        section_map = { # Simplified mapping
            ...
            ResumeSection.K8_EY_BULLETS: "EY Narrative", # Use K8 Bullets key for Narrative
            ResumeSection.K9_EARLY_CAREER_BULLETS: "Early Career Narrative", # Use K9 Bullets key for Narrative
        ...
        for section_enum, label in section_map.items():
            ...
            # Overviews (Unify, IBM, TraderSense)
            if "Overview" in label:
                if isinstance(content, str) and content.strip():
                    overview_text = content.strip()
                    sections_for_matrix[label] = [overview_text]
                    # Only add to pairs if NOT Unify/IBM for check later
                    if label not in ["Unify Overview", "IBM Overview"]:
                        overview_bullet_pairs[label] = {"overview": overview_text, "bullets": []}
                    exec_summary_comparison_data[label] = overview_text
                continue

            # Bullets (Unify, IBM, TraderSense, Competencies)
            if label in ["Unify Bullets", "IBM Bullets", "TraderSense Bullets", "Competencies"]:
                bullet_texts = []
                if isinstance(content, list):
                    for item in content:
                ...
                # Add bullets to corresponding overview pair (if pair exists)
                base_label = label.replace(" Bullets", "").replace(" Competencies", "")
                overview_key = f"{base_label} Overview"
                if overview_key in overview_bullet_pairs: # Only adds if overview exists and isn't Unify/IBM
                     overview_bullet_pairs[overview_key]["bullets"].extend(bullet_texts)

                # Exclude Unify/IBM Bullets from Pairwise Matrix
                if label not in ["Unify Bullets", "IBM Bullets"]:
                    sections_for_matrix[label] = bullet_texts
                else: self.logger.debug(f"Excluding {label} from pairwise similarity matrix.")
                continue

            # Narrative Blocks (EY, Early Career) stored under Bullets keys
            if label in ["EY Narrative", "Early Career Narrative"]:
                 if isinstance(content, str) and content.strip():
        ...
        try:
            ...
            for label, data in overview_bullet_pairs.items(): # Only contains non-Unify/IBM pairs now
            ...
            self.logger.error(f"Error computing overview vs bullet similarity: {e}", exc_info=False)
            self.overview_similarity_data = None
