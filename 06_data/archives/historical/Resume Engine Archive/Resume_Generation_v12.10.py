...
# ============================================================================
# HOP-3: ARTIST GENERATOR (LLM Calls) - FAIL FAST VERSION
# ============================================================================
...
class ArtistGenerator:
    ...
    # --- START MODIFICATION: Set universal temperature schedule ---
    # Apply the user-requested schedule ([1.0, 0.8, 0.6, 0.4, 0.2]) to all sections.
    UNIVERSAL_SCHEDULE: ClassVar[List[float]] = [1.0, 0.8, 0.6, 0.4, 0.2]
    TEMPERATURE_SCHEDULES: ClassVar[Dict[ResumeSection, List[float]]] = {
        section: UNIVERSAL_SCHEDULE for section in ResumeSection
    }
    # --- END MODIFICATION ---
    ...
    def generate(
        ...
        try:
            ...
            # Only add success result if no exception was raised
            validation_results.append(ValidationResult(
    ...
    def _generate_artist_output(
        ...
        for config in self.ARTIST_GENERATION_CONFIG:
            ...
            if method_name.startswith("_copy_") or method_name == "_generate_dummy_header":
                 ...
                 except Exception as e: # Should generally not happen for copy/dummy
                     logging.error(f"Error in copy/dummy method {method_name} for {section_enum.value}: {e}", exc_info=True)
                     # Raise error even for these simple methods if they fail unexpectedly
            ...
            else: # Should not happen for LLM sections, but fail safe
            ...
            # NOTE: The called method is now expected to raise HopExecutionError on failure
            try:
                method = getattr(self, method_name)
                output[section_enum.value] = method(temperature_override=final_temp)
                # Check for explicit placeholders returned by _call_gemini_api if it didn't raise
                # (This is a safeguard, _call_gemini_api should raise now)
                if isinstance(output[section_enum.value], str) and "[Placeholder" in output[section_enum.value]:
                 ...
                 # Log and re-raise to halt the entire generation process for this attempt
                 logging.error(f"Generation HALTED at section {section_enum.value} ({method_name}): {he}", exc_info=False)
                 raise he # Propagate the error up to self.generate()
            except Exception as e:
                 # Catch other unexpected errors during a specific section's generation
                 logging.error(f"Unexpected Error generating section {section_enum.value} with {method_name} (Temp: {final_temp}): {e}", exc_info=True)
                 # Raise as HopExecutionError to halt
                 raise HopExecutionError(f"Unexpected error during {section_enum.value} generation: {e}") from e

        # Ensure all sections keys exist (value might be None if copy failed silently before, though unlikely now)
        for section_enum in ResumeSection:
            if section_enum.value not in output:
                 output[section_enum.value] = None # Or raise error if all must be present
        return output
    ...
    def _generate_k0_headline(self, temperature_override: Optional[float] = None) -> str:
        ...
        """
        # ... (Get signals - UNCHANGED) ...
        # ... (Build feedback_instruction - UNCHANGED) ...
        # ... (Build prompt - UNCHANGED, includes word count constraint) ...
        prompt = f"""... ABSOLUTELY CRITICAL: ... strictly between {self.constraints.HEADLINE_WORD_COUNT_MIN} and {self.constraints.HEADLINE_WORD_COUNT_MAX} words total..."""

        reasoning_config = ReasoningConfig.K0_HEADLINE_CONFIG
        base_system = "You are an expert resume headline crafter..."
        ...
        # --- REMOVED FALLBACK & Internal Check ---
        # Word count check is now handled by the PreFlightValidator in HOP-5.
        # This function simply returns the generated text.
    ...
    def _generate_k1_executive_summary(self, temperature_override: Optional[float] = None) -> str:
        ...
        """
        # ... (Get signals - UNCHANGED) ...
        # ... (Build archetype_instruction - UNCHANGED) ...
        # ... (Build feedback_instruction based on self.previous_failures for VG_SENTENCE_COUNT_K1 - UNCHANGED) ...
        # ... (Build prompt - UNCHANGED, includes NON-NEGOTIA...
        prompt = f"""... NON-NEGOTIABLE REQUIREMENTS ... SENTENCE COUNT: MUST have strictly between {self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MIN} and {self.constraints.EXEC_SUMMARY_SENTENCE_COUNT_MAX} sentences..."""

        reasoning_config = ReasoningConfig.K1_EXECUTIVE_SUMMARY_CONFIG
        base_system = "You are an expert resume writer..."
    ...
    def _generate_k2_skills(self, temperature_override: Optional[float] = None) -> List[str]:
        ...
        try:
            # ... (Get signals - UNCHANGED) ...
            # ... (Build prompt for 12 skills, 1-3 words - UNCHANGED) ...
            prompt = "..."

            reasoning_config = ReasoningConfig.K2_SKILLS_CONFIG
            base_system = "You are an expert HR data analyst..."
            # API call raises HopExecutionError on failure
            ...
            if len(skills_list_final) < 10: # Stricter check? Maybe 12?
            ...
            if malformed_count > 2: # Allow a couple of errors, but fail if too many
    ...
    def _generate_tailored_overview_for_experience(
        ...
        v12.05: No internal fallback on word count failure. Raises error on API failure.
        """
        # ... (Find source_overview - UNCHANGED) ...
        ...
        if not source_overview:
             # If source is missing, we cannot generate - raise error
             raise HopExecutionError(f"Source overview not found for {company_name} in {section_id}.")

        # ... (Get signals - UNCHANGED) ...
        min_wc, max_wc = word_count_range
        feedback_instruction = "" # Add feedback logic if needed for word count
        # ... (Build prompt - UNCHANGED, includes CRITICAL word count constraint) ...
        prompt = f"""... ABSOLUTELY CRITICAL: ... strictly between {min_wc} and {max_wc} words..."""

        system_prompt = "You are an expert resume editor..."
        # API call raises HopExecutionError on failure
        ...
        return tailored_overview # Return generated text directly

    # --- Methods calling _generate_tailored_overview_for_experience (Unchanged signatures) ---
    def _generate_k5_unify_overview(self, temperature_override: Optional[float] = None) -> str:
        # Calls _generate_tailored_overview_for_experience, propagates error if raised
        return self._generate_tailored_overview_for_experience(
            company_name="Unify", word_count_range=(self.constraints.UNIFY_OVERVIEW_WORD_COUNT_MIN, self.constraints.UNIFY_OVERVIEW_WORD_COUNT_MAX),
            reasoning_config=ReasoningConfig.K5_UNIFY_OVERVIEW_CONFIG or ReasoningConfig.DEFAULT, section_id="K.5_UNIFY_OVERVIEW",
            temperature_override=temperature_override
        )
    
    def _generate_k6_ibm_overview(self, temperature_override: Optional[float] = None) -> str:
        """Generates K.6 IBM overview."""
        return self._generate_tailored_overview_for_experience(
            company_name="IBM",
    ...
    def _generate_k9_early_career_overview(self, temperature_override: Optional[float] = None) -> str:
        """Generates K.9 Early Career overview."""
        return self._generate_tailored_overview_for_experience(
    ...
    def _validate_llm_bullet_selection(self, selected_bullets_text: List[str], master_bullets_structured: List[Dict], expected_count: int, section_id: str) -> List[Dict]:
        ...
        for selected_text in selected_bullets_text:
            ...
            if cleaned_text in master_texts:
                ...
                # Try finding a near match (optional, can be strict)
                # If strict matching is required:
                msg = f"{section_id} LLM returned bullet not exactly in master list: '{cleaned_text[:50]}...'"
                logging.error(msg)
                raise HopExecutionError(msg)

        # Final count check in case of duplicates removed during strict matching
    ...
    def _rewrite_bullet_for_word_count(self, original_bullet_text: str, target_word_count_range: Tuple[int, int], section_id: str, temperature_override: Optional[float] = None) -> str:
        ...
        try:
            ...
            # ... (Get signals: auth_verbs - UNCHANGED) ...
            # ... (Build prompt - UNCHANGED, includes CRITICAL word count constraint) ...
            prompt = f"""... ABSOLUTELY CRITICAL: ... strictly between {min_wc} and {max_wc} words..."""

            system_prompt = f"You are a resume word count editor..."
            # API call raises HopExecutionError on failure
            ...
            # --- REMOVED FALLBACK - Now check compliance and fail if needed ---
            ...
            if not (min_wc <= rewritten_wc <= max_wc):
                 ...
                 # Raise error instead of returning original
                 raise HopExecutionError(msg)

            return rewritten_text # Return compliant rewritten text
    ...
    def _validate_and_potentially_rewrite_bullets(
        ...
        for i, bullet_data in enumerate(selected_bullets_structured):
            ...
            if not (min_target <= word_count <= max_target):
                ...
                try:
                    # _rewrite_bullet_for_word_count now raises error if it fails
    ...
    def _generate_lightly_customized_bullets(
        ...
        """
        try:
            if not source_bullets_text: return [] # Okay to return empty if source is empty
            # ... (Get signals - UNCHANGED) ...
            ...
            # --- REMOVED FALLBACK - Now check count and fail if needed ---
            ...
            # Return successfully generated bullets
    ...
    def _generate_synthetic_bullets(
        ...
        try:
            ...
            # ... (Build prompt - UNCHANGED) ...
            prompt = "..."
            system_prompt = "You generate plausible, synthetic resume bullets..."
            # API call raises HopExecutionError on failure
            response_text = self._call_gemini_api(
                prompt, reasoning_config, section_id, system_prompt,
                temperature_override=temperature_override
    ...
    def _generate_tailored_bullets_for_experience(
        ...
        v12.05: Uses fail-fast helpers.
        """
        # ... (1. Find experience section, raise error if not found) ...
        experience_section = next((exp for exp in self.master_resume['professional_experience'] if company_name in exp['company']), None)
        if not experience_section: raise HopExecutionError(f"{company_name} not found for {section_id}")
        # ... (2. Get signals) ...
        # ... (Get master_bullets_structured, raise error if invalid) ...
        ...
        if not isinstance(bullet_source, list):
             raise HopExecutionError(f"{section_id} master bullet source is not a list.")
        for bullet_text in bullet_source:
             if isinstance(bullet_text, str):
                 master_bullets_structured.append({
                     "bullet_text": bullet_text,
                     "text": bullet_text, # Add 'text' key for consistency
                     "provenance": BulletProvenance.Verbatim.value,
        ...
        final_bullets = [] # Stores dicts {text: ..., provenance: ..., word_count: ...}

        # --- 3. Select Verbatim Bullets (Fail Fast) ---
        if verbatim_count > 0:
            if not master_bullets_structured:
                 # Raise error if trying to select from nothing
                 raise HopExecutionError(f"{section_id} Cannot select Verbatim bullets: No master bullets available.")
            # ... (Build prompt_select) ...
            prompt_select="..." # Placeholder for prompt logic
            system_prompt_select="..." # Placeholder for prompt logic
            try:
                # API call raises error on failure
                ...
                # Validator raises error on failure
                ...
                raise he # Propagate failure

        # --- 4. Select and Customize Bullets (Fail Fast) ---
        if customized_count > 0:
            used_texts = {b.get('bullet_text') for b in final_bullets} # Use original key here
            ...
            # Helper raises error on failure
            ...
            final_bullets.extend(customized_bullets) # Add newly generated dicts
        ...
        if synthetic_count > 0:
            ...
            final_bullets.extend(synthetic_bullets) # Add newly generated dicts

        # --- 6. Final Padding / Trimming (Should be less needed, but check count) ---
        total_expected_count = sum(provenance_targets.values())
        if len(final_bullets) != total_expected_count:
            # This indicates a logic error in V/C/S counts or selection
        ...
        # Padding/Trimming logic removed as failure should occur earlier if counts mismatch

        # --- 7. Word Count Validation & Rewrite (Fail Fast) ---
        master_avg_lengths = calculate_master_avg_bullet_length(self.master_resume)
        section_name_for_avg = "Default" # Determine based on company/section_id
        ...
        elif section_id == "K.10_COMPETENCIES": section_name_for_avg = "Competencies"
        
        # Validator helper raises error on failure
        ...
        current_bullets_text = '\n'.join([f"{i+1}. {bullet.get('text', '')}" for i, bullet in enumerate(final_bullets)]) # Use 'text' key now
        prompt_reorder = f"""Reorder the following bullets...
**Bullets to Reorder ({company_name}):**
{current_bullets_text}
# ... (rest of reorder prompt) ..."""
        system_prompt_reorder = "You reorder bullets..."
        try:
            # API call raises error on failure
            ...
            # --- Validation of Reordering (Strict) ---
            ...
            original_texts_map = {b.get('text'): b for b in final_bullets} # Map generated text to dict
            ...
            for reordered_text in reordered_texts:
                 ...
                 if cleaned_reordered in original_texts_map and cleaned_reordered not in used_original_texts:
                     ...
                     # LLM modified text or returned duplicates
            ...
            # Final check if all original bullets were used exactly once
            if len(final_ordered_bullets_dicts) != total_expected_count:
                raise HopExecutionError(f"{section_id} Reordering validation failed: Final count mismatch after matching.")

            return final_ordered_bullets_dicts # Return the list of validated, reordered dicts
    ...
    # --- Methods calling _generate_tailored_bullets_for_experience (Unchanged signatures) ---
    def _generate_k5_unify_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        # Calls _generate_tailored_bullets_for_experience, propagates error if raised
    ...
    def _generate_k6_ibm_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.6 IBM bullets (V/C/S)."""
        return self._generate_tailored_bullets_for_experience(
    ...
    def _generate_k10_competencies(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.10 Competencies (V/C/S) from 'strategic_and_technical_competencies'."""
        # Find the "company" name for competencies in the master resume (it's not in experience)
        # We need to adapt _generate_tailored_bullets to handle this special case
        # For now, let's just make it work by faking the experience section
        # This is a bit of a hack, ideally _generate_tailored_bullets would be more generic
        
        # Create a fake experience section for competencies
        ...
        try:
            generated_bullets = self._generate_tailored_bullets_for_experience(
                company_name=fake_company_name, section_index=-1, # Use index that finds the fake one
             ...
             self.master_resume = original_master_resume # Restore original

        return generated_bullets

    def _generate_k8_ey_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.8 EY bullets (customize only). Raises error on failure."""
    ...
    def _generate_k9_early_career_bullets(self, temperature_override: Optional[float] = None) -> List[Dict]:
        """Generates K.9 Early Career bullets (customize only). Raises error on failure."""
        ...
        customized_bullets = self._generate_lightly_customized_bullets(
            source_bullets_text=source_bullets[:target_count],
            section_id=ResumeSection.K9_EARLY_CAREAR_BULLETS.value,
            thematic_analysis=self.thematic_analysis,
            temperature_override=temperature_override
        )
        # Validator raises error on failure
        master_avg_lengths = calculate_master_avg_bullet_length(self.master_resume)
        return self._validate_and_potentially_rewrite_bullets(
             customized_bullets, master_avg_lengths, "EarlyCareer", ResumeSection.K9_EARLY_CAREAR_BULLETS.value, temperature_override
        )

    # --- Cover Letter Generation (Fail Fast Version) ---
    def _generate_k13_cover_letter(self, temperature_override: Optional[float] = None) -> str:
        ...
        """
        # ... (Signal gathering - UNCHANGED) ...
        # ... (Prompt construction - UNCHANGED) ...
        prompt = "..." # Placeholder for prompt logic
        try:
            reasoning_config = ReasoningConfig.DEFAULT
            base_system = f"You are an expert executive ghostwriter..."
            # API call raises error on failure
            ...
            # Check for placeholder explicitly (safeguard)
            if "[Placeholder" in cover_letter_text:
                raise HopExecutionError(f"K.13 generation failed (placeholder).")

            # Attempt structure fix, raise error on failure
            # ... (Structure fix logic) ...
            structure_fix_fails_date = False # Placeholder
            structure_fix_fails_signature = False # Placeholder
            
            if structure_fix_fails_date:
                raise HopExecutionError("K.13 failed structure fix (date/salutation).")
            if structure_fix_fails_signature:
                raise HopExecutionError("K.13 failed structure fix (signature).")

            return cover_letter_text.strip()

        except HopExecutionError as he: # Catch API or structure fix errors
            logging.error(f"K.13 generation process failed: {he}")
            raise he # Propagate error to halt workflow
        except Exception as e: # Catch other unexpected errors
...
class PreFlightValidator:
    ...
    # /Resume_Generation_v12.02.py (Section: PreFlightValidator - RULES_CONFIG update)
...
# /Resume_Generation_v12.02.py (Section: PreFlightValidator - commented method)
    # def _validate_k1_word_count(self, data: Dict) -> bool: # Removed Constraint
...
class WorkflowOrchestrator:
    ...
    def _execute_hop_3_artist_generation(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        v12.10: Removed all special temperature override logic to use the UNIVERSAL_SCHEDULE.
        """
        ...
        # --- START MODIFICATION: Removed all special temp schedules ---
        # k1_temperature_schedule = [1.0, 0.8, 0.6, 0.4, 0.2] # <--- REMOVED
        # current_k13_temp = 1.0 # <--- REMOVED
        # k13_temperature_schedule = [1.0, 0.9, 0.8, 0.7, 0.6] # <--- REMOVED
        # --- END MODIFICATION ---
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # --- START MODIFICATION: Removed all override logic ---
            # current_k1_temp = k1_temperature_schedule[min(attempt - 1, len(k1_temperature_schedule) - 1)] # <--- REMOVED
            # k1_temp_to_use = current_k1_temp # <--- REMOVED
            # k1_failed_previously = any(...) # <--- REMOVED
            # if attempt > 1 and k1_failed_previously: # <--- REMOVED
            #     self.logger.info(f"    Applying K.1 Temperature Override: {current_k1_temp}") # <--- REMOVED
            # else: # <--- REMOVED
            #     k1_temp_to_use = None # <--- REMOVED
            # k13_temp_to_use = current_k13_temp # <--- REMOVED
            # self.logger.info(f"    Using K.13 Temperature: {k13_temp_to_use}") # <--- REMOVED
            # --- END MODIFICATION ---

            try:
                # --- START MODIFICATION: Simplified artist.generate() call ---
                current_artist_output, current_generation_results = artist.generate(
                    feedback_results=feedback_results,
                    attempt=attempt
                    # temperature_overrides=... # <--- REMOVED
                )
                # --- END MODIFICATION ---
            ...
            if all_passed or attempt == max_attempts:
                ...
                # --- START MODIFICATION: Removed K.13 temp reduction logic ---
                # cl_structure_failure = next(...) # <--- REMOVED
                # if cl_structure_failure: # <--- REMOVED
                # ... (all logic inside) ...
                # else: # <--- REMOVED
                #     self.logger.info(f"    K.13 structure passed... K.13 temp remains {current_k13_temp}.") # <--- REMOVED
                # --- END MODIFICATION ---
