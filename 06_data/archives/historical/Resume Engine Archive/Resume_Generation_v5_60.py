...
class FileRenderer:
    ...
    def render(
        ...
        try:
            ...
            skills_json = self._render_skills(staging_buffer, job_description)
    ...
    def _render_skills(self, staging_buffer: ImmutableStagingBuffer, job_description: str = None) -> Dict:
        """
        Render skills with JD alignment scoring (v5.57).
        Returns JSON with skills ordered by JD relevance.
        v5.58: Fixed JDAlignmentScorer instantiation bug.
        v5.60: CRITICAL FIX - Load 12 competencies from MASTER_RESUME_JSON instead of K.11 certifications.
        
        Root Cause (v5.59 bug):
        - K.11 staging buffer key was hard-coded to certifications (4 items)
        - generate_k11_skills() returns only certifications list
        - JDAlignmentScorer was scoring certifications instead of competencies
        
        Fix (v5.60):
        - Bypass K.11 staging buffer entirely
        - Load 6 strategic + 6 technical competencies directly from MASTER_RESUME_JSON
        - Pass all 12 competencies to JDAlignmentScorer for proper scoring
        """
        ...
        if job_description and skills:
            try:
                ...
                for i, skill in enumerate(skills):  # All 12 competencies
                    # Use score_competency (more sophisticated than score_skill)
                ...
                for i, skill_data in enumerate(scored_skills):
                    skill_data['rank'] = i + 1
                
                return {
                    "skills_ordered_by_jd_relevance": scored_skills,
                    "jd_alignment_methodology": "JDAlignmentScorer v5.60 (fixed to use 12 master competencies)",
                    "total_skills_analyzed": len(skills),
                    "competency_source": "MASTER_RESUME_JSON.competencies (strategic + technical)"
                }
            except Exception as e:
            ...
            return {
                ...
                "note": "No JD provided - competencies unscored"
            }
        else:
            # No competencies found (should never happen with valid MASTER_RESUME_JSON)
            return {
                "skills": [],
                "error": "No competencies found in MASTER_RESUME_JSON"
            }
