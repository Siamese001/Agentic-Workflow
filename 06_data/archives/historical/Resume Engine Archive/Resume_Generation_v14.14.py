...
__version__ = "14.14" # <-- Increment version for tracking changes
try:
...
# ============================================================================
# HOP-5: VALIDATION GATES (MODIFIED FOR CROSS-SECTION SIMILARITY)
# ============================================================================
...
class PreFlightValidator:
    ...
    # Bullet word count sections to check
    ...
    # --- RULES_CONFIG without VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY ---
    RULES_CONFIG = [
        # --- RAG Quality ---
        {
        ...
        # ... (All other rules EXCEPT VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY remain the same) ...
         { # K.1 Sentences
        ...
        { # K.4 Narrative Sentences
            ...
            "validator": lambda ctx: ctx.k4_narrative_details['sentence_count'] == ctx.k4_narrative_details['target_sc'],
        ...
        { # K.5 Narrative Sentences
            ...
            "validator": lambda ctx: ctx.k5_narrative_details['sentence_count'] == ctx.k5_narrative_details['target_sc'],
        ...
        { # K.6 Narrative Sentences
            ...
            "validator": lambda ctx: ctx.k6_narrative_details['sentence_count'] == ctx.k6_narrative_details['target_sc'],
        ...
        # --- NEW Cross Section Similarity Rule ---
        {
            ...
            "error_message": lambda ctx: f"High similarity (>=0.65) found between sections: {ctx._cache.get('VG_CROSS_SECTION_SIMILARITY', {}).get('failures_str', 'N/A')}"
        },
        # --- REMOVED VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY rule ---
    ]
    ...
    def _initialize_rule_map(self) -> Dict[str, Union[ResumeSection, str]]:
        ...
        REMOVED VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY.
        """
        logger = logging.getLogger(__name__)
        rule_map = {
            # Global Rules
            ...
            "VG_RAG_MIN_QUALITY": "GLOBAL", "VG_CROSS_SECTION_SIMILARITY": "GLOBAL",
            # Visual Rules
            ...
            # --- VG_EXEC_SUMMARY_VS_SECTION_SIMILARITY REMOVED ---
            # K.2 Unify
    ...
    def _register_rules(self):
        ...
        # ... (Content remains the same, ensures all rules in RULES_CONFIG are registered) ...
        logger = logging.getLogger(__name__)
    ...
    def _validate_cross_section_similarity(self, context: ValidationContext) -> bool:
        """
        Checks similarity between key generated text sections (K1, K2_O, K3_O, K4, K5, K6, K9).
        Fails if any pair similarity is >= 0.65.
        """
        logger = logging.getLogger(__name__)
        threshold = 0.65
        valid = True
        failures = []
        failed_sections = set() # Track sections involved in failures
        ...
        # Get content for all sections first
        content_map = {}
        for section_enum in sections_to_compare:
            content = context.staging_buffer.get(section_enum.value)
            # Handle list content (like competencies K9) by joining
            if isinstance(content, list):
                # Safely get text from potential dicts or strings in list
                texts = [str(item.get('text', str(item))) if isinstance(item, dict) else str(item)
                         for item in content]
                content_map[section_enum] = " ".join(texts) # Join list items into single string
            elif isinstance(content, str):
                content_map[section_enum] = content
            else:
                content_map[section_enum] = "" # Use empty string if missing or wrong type

        # Initialize DuplicateDetector
        try:
            dd = DuplicateDetector()
        except NameError:
             logger.error("DuplicateDetector class not found for similarity check.")
             context._cache["VG_CROSS_SECTION_SIMILARITY"] = {"failures_str": "Similarity check skipped: Detector unavailable.", "failed_sections": set()}
             return False # Fail if detector unavailable
        ...
        for i in range(len(sections_to_compare)):
            for j in range(i + 1, len(sections_to_compare)):
                ...
                text1 = content_map.get(enum1, "")
                text2 = content_map.get(enum2, "")

                if not text1 or not text2:
                    continue # Skip if one section is empty

                try:
                    similarity = dd._calculate_cosine_similarity(text1, text2)
                    if similarity >= threshold:
                        valid = False
                        failures.append(f"{enum1.name} vs {enum2.name}: {similarity:.3f}")
                        failed_sections.add(enum1)
                        failed_sections.add(enum2)
                except Exception as e:
                    logger.warning(f"Error calculating similarity between {enum1.name} and {enum2.name}: {e}")
                    # Optionally treat calculation error as a validation failure

        if not valid:
            context._cache["VG_CROSS_SECTION_SIMILARITY"] = {
                "failures_str": "; ".join(failures),
                "failed_sections": failed_sections # Store the set of failed enums
            }
        return valid

    # --- REMOVED _validate_exec_summary_vs_sections method ---

    # (Other validation methods: _validate_section_presence, _validate_cover_letter_full_structure, etc.)
    # ... (Include the correctly indented versions of all other _validate_... methods here) ...
    ...
    def _validate_headline_format_no_titles(self, context: ValidationContext) -> bool:
        ...
        if not headline or '|' not in headline: details['error'] = "Missing pipes"; context._cache["VG_HEADLINE_NO_TITLES"] = details; context._cache["VG_HEADLINE_COMPONENT_WC"] = details; return False
        components = [c.strip() for c in headline.split('|')]
        if len(components) != 3: details['error'] = f"Expected 3 components, found {len(components)}"; context._cache["VG_HEADLINE_NO_TITLES"] = details; context._cache["VG_HEADLINE_COMPONENT_WC"] = details; return False
        forbidden_titles = ['director', 'vp', 'manager', 'lead', 'head', 'chief', 'principal', 'senior', 'executive']
        forbidden_found = []; component_wc_violations = []
        ...
        for i, comp in enumerate(components):
            ...
            if not (min_comp_wc <= word_count <= max_comp_wc): component_wc_violations.append(f"Comp[{i+1}]: {word_count} words (Tgt: {min_comp_wc}-{max_comp_wc})")
        ...
        details_wc = details.copy(); details_wc['min'] = min_comp_wc; details_wc['max'] = max_comp_wc; details_wc['wc_violations'] = component_wc_violations; context._cache["VG_HEADLINE_COMPONENT_WC"] = details_wc
        ...
        if sections_under_test:
            ...
            for rule_id, section_map in self.RULE_TO_SECTION_MAP.items():
                if section_map == "GLOBAL" or section_map == "VISUAL" or section_map in sections_under_test:
                    ...
                    # --- Include new rule based on sections it compares ---
        ...
        if not all_passed:
            for vr in final_results_for_run:
                if not vr.passed and vr.severity.value >= ValidationSeverity.HIGH.value:
                    ...
                    if isinstance(section_map, ResumeSection): failed_sections_enums.add(section_map)
                    elif section_map == "COMPLEX_PER_SECTION" or section_map == "GLOBAL": # Include GLOBAL for similarity check
                        # Add sections identified by the complex/global validator's cached details
                        cached_details = context._cache.get(vr.rule_id, {})
                        failed_in_cache = cached_details.get("failed_sections", set())
                        if isinstance(failed_in_cache, set): failed_sections_enums.update(failed_in_cache)
...
# ============================================================================
# QA REPORT GENERATOR CLASS (FINAL CORRECTED VERSION - UNTRUNCATED)
# ============================================================================
import textwrap
import re
import logging
...
# PreFlightValidator, ArtistGenerator, DuplicateDetector, HopCheckpoint, HopStatus, GateDecision
# classes are defined/imported correctly.
...
# Section 1: Signal Quality (CORRECTED SYNTAX)
    def _build_qa_section_1_signal_quality(
        ...
        if jd_rule:
            ...
            details = jd_rule.details or {}; kw_min, kw_max, kw_found = details.get('min', '?'), details.get('max', '?'), details.get('found', '?')
        ...
        cl_enum = ResumeSection.K11_COVER_LETTER; cl_content = staging_buffer.get(cl_enum.value); cl_raw_score = 0.0; cl_target_min_raw, cl_target_max_raw = 0.80, 1.10
        if cl_content:
            # --- CORRECTED INDENTATION ---
            try:
    ...
    def _build_qa_section_2_signal_flow_map(
        ...
        for row_cfg in rows_config:
            section_label = row_cfg["label"]; signal_presence = row_cfg["signals"]
            content = staging_buffer.get(row_cfg["section"].value); row_data = [section_label]
            ...
            if signal_presence[1] == "Diff_Pct" and content:
                ...
                diff_count = 0;
                if valid_diffs: summary_lower = str(content).lower(); diff_count = sum(1 for kw in valid_diffs if kw.lower() in summary_lower)
                row_data.append(f"{diff_count}/{len(valid_diffs)} Found")
            else: row_data.append("N/A")
            row_data.append("✓" if signal_presence[2] == "Archetype" else "N/A"); row_data.append("✓" if signal_presence[3] == "Narrative" else "N/A"); row_data.append("✓" if signal_presence[4] == "Secondary" else "N/A"); row_data.append("✓" if signal_presence[5] == "Auth" else "N/A"); table_rows.append(row_data)
        ...
        for rule_id, section_label in rules_to_include.items():
            ...
            if rule_id.startswith("CL_P"):
                ...
                if cl_structure_result:
                    ...
                    if not cl_structure_result.passed and cl_paragraph_details.get('error'): status, msg = "FAIL", cl_paragraph_details.get('error')
                ...
                if not result.passed:
                    try:
                        ...
                        if callable(result.message): msg = result.message(defaultdict(lambda: '[N/A]', **details))
    ...
    def _build_qa_section_5_provenance(
        ...
        for section_data in sections_to_process:
            ...
            if not target_range:
                self.logger.warning(f"Word count range missing for {bullets_enum.name}. Using default.")
            ...
            if isinstance(bullets, list) and bullets:
                for i, bullet_item in enumerate(bullets):
                    ...
                    if isinstance(bullet_item, dict):
                        ...
                        word_count = bullet_item.get('word_count', count_words_ms_word_style(bullet_text)) # Use helper
    ...
    def _build_qa_section_6_content_authenticity(
        ...
        rows = []
        filtered_results = [ vr for vr in validation_results if any(pattern in vr.rule_id for pattern in rule_id_patterns) ]
            ...
            if critical_failures:
                ...
                for f in critical_failures:
                    ...
                    try:
                        ...
                        else: msg = msg_template.format_map(defaultdict(lambda: '[N/Warning]', **details))
    ...
    # --- UPDATED Section 8: Similarity Summary ---
    def _build_qa_section_8_similarity_summary(
        self,
        validation_results: List[ValidationResult] # Now uses validation results from HOP-5
    ) -> List[str]:
        """Builds QA Section 8: Content Similarity Summary."""
        lines = ["", "8. CONTENT SIMILARITY SUMMARY", ""]
        lines.append("Checks for excessive similarity between key generated sections and identifies strict duplicates based on HOP-5 validation results and HOP-7.5 analysis data.")
        lines.append("")
        lines.append("```markdown")
        headers = ["Check Type", "Details", "Status", "Notes/Examples"]
        rows = []
        overall_pass = True # Assume PASS initially

        # --- 1. Cross-Section Similarity (HOP-5 Validation Result) ---
        cross_section_rule = next((vr for vr in validation_results if vr.rule_id == "VG_CROSS_SECTION_SIMILARITY"), None)
        cross_section_status = "⚠️ WARN" # Default if rule missing
        cross_section_notes = "Validation rule VG_CROSS_SECTION_SIMILARITY result missing."

        if cross_section_rule:
            if cross_section_rule.passed:
                ...
                details = cross_section_rule.details or {}
                # Try to format the message using the lambda if available
                try:
                    msg_template = str(cross_section_rule.message)
                    if callable(cross_section_rule.message):
                        cross_section_notes = cross_section_rule.message(defaultdict(lambda: '[N/A]', **details))
                    else: # Fallback
                        cross_section_notes = msg_template.format_map(defaultdict(lambda: '[N/A]', **details))
                except Exception as e:
                    self.logger.warning(f"Error formatting message for VG_CROSS_SECTION_SIMILARITY: {e}")
                    cross_section_notes = f"High similarity found: {details.get('failures_str', 'Unknown failures')}" # Fallback
        else:
             overall_pass = False # Treat missing check as non-passing for overall

        rows.append([
            "Cross-Section Overlap (>= 0.65)",
            "K1/K2_O/K3_O/K4/K5/K6/K9", # Sections checked
            cross_section_status,
            cross_section_notes
        ])

        # --- 2. Strict Duplicates (HOP-7.5 Analysis Data) ---
        similarity_matrix_data = getattr(self.orchestrator, 'similarity_matrix_data', None)
        strict_dup_status = "⚠️ WARN"
        strict_dup_details = "Analysis not performed."
        strict_dup_notes = ""
        ...
        if similarity_matrix_data:
            ...
            strict_dup_details = f"{similarity_matrix_data.get('total_comparisons', 0)} Comparisons"
            if duplicates_count == 0:
                strict_dup_status = "✅ PASS"
                strict_dup_notes = "No strict duplicates (>= 0.90) found."
            else:
                strict_dup_status = "❌ FAIL"
                overall_pass = False # Mark overall as failed
                failed_examples = duplicates_found[:2]
                strict_dup_notes = "; ".join([f"{ex.get('bullet_1', '?')} vs {ex.get('bullet_2', '?')}: {ex.get('similarity', 0.0):.4f}" for ex in failed_examples if isinstance(ex, dict)])
                if len(duplicates_found) > 2: strict_dup_notes += "..."
        rows.append([
        ...
        rows.append([
            "Overall Similarity Status", "", overall_status_str, overall_notes
        ])

        # --- Format and return table ---
        alignments = ['L', 'L', 'L', 'L']
        col_widths = [45, 30, 8, 80] # Adjusted widths
        lines.extend(self._format_plain_text_table(headers, rows, alignments=alignments, col_widths=col_widths, wrap_text=True))
        lines.append("```")
        return lines
    # --- END UPDATED Section 8 ---

    # Section 9: Structural Validation (Specific Method - Was 10)
    ...
    # Section 10: Cover Letter QA (Specific Method - Was 11)
    ...
    # Section 12: Final Format (Renumbered from 13)
    def _build_qa_section_12_final_format(
        ...
        if app_content:
            ...
            json_valid_status_appended = False
            try:
        ...
        for section_index, (title, section_block) in section_blocks.items():
            ...
            for block in md_blocks:
                ...
                if has_pipe and has_separator:
                    ...
                    if msg not in pre_formatted_check_messages: pre_formatted_check_messages.append(msg)
                    break # Found error in this section, move to next section_index
    ...
    def _format_plain_text_table(self, headers: List[str], rows: List[List[Any]], alignments: Optional[List[str]] = None, col_widths: Optional[List[int]] = None, wrap_text: bool = False) -> List[str]:
    ...
    def _format_ascii_bar_chart(self, label: str, value: float, target_min: Optional[float], target_max: Optional[float], temperature: Optional[Union[float, str]] = None, bar_length: int = 10, is_summary: bool = False, score_display_mode: str = "percentage", status_override: Optional[str] = None) -> str:
        ...
        elif target_min is not None and target_max is not None: status = "PASS" if target_min <= value <= target_max else "FAIL"
        else: status = "INFO"
        temp_str = "(T: N/A)";
        if isinstance(temperature, (float, int)): temp_str = f"(T: {temperature:.1f})" if not is_summary else f"(Avg T: {temperature:.1f})"
        elif isinstance(temperature, str) and temperature != 'N/A': temp_str = f"({temperature})"
        if target_min is not None and target_max is not None: target_str = f"(Tgt: {target_min:.2f}-{target_max:.2f})" if score_display_mode == "raw" else f"(Tgt: {target_min:.0%}-{target_max:.0%})"
        else: target_str = "(Target: N/A)"
        label_width, bar_width, score_width, target_width, status_width, temp_width = 25, bar_length + 2, 12, 18, 7, 10
        total_width = 95; current_width = label_width + bar_width + score_width + target_width + status_width + temp_width; padding = " " * max(0, total_width - current_width)
