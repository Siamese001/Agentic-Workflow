"""
Resume Generation Engine v9.10
...
class HopExecutionError(Exception):
    """Custom exception for non-recoverable errors within a workflow hop."""
    pass
# ============================================================================
# DATACLASSES
# ============================================================================
...
class ArtistGenerator:
    ...
    def _generate_artist_output(self) -> Dict:
        ...
        output[ResumeSection.K7_TRADERSENSE_BULLETS.value] = self._copy_k7_tradersense_bullets()
        output[ResumeSection.K7_TRADERSENSE_OVERVIEW.value] = self._copy_k7_tradersense_overview()
        output[ResumeSection.K8_EY_BULLETS.value] = self._generate_k8_ey_bullets()
        output[ResumeSection.K8_EY_OVERVIEW.value] = self._generate_k8_ey_overview()
        output[ResumeSection.K9_EARLY_CAREER_BULLETS.value] = self._generate_k9_early_career_bullets()
        output[ResumeSection.K9_EARLY_CAREER_OVERVIEW.value] = self._generate_k9_early_career_overview()
        ...
        output[ResumeSection.K2_SKILLS.value] = self._generate_k2_skills()
        output[ResumeSection.K13_COVER_LETTER.value] = self._generate_k13_cover_letter()
        return output
...
**TASK:**
1. Write a 4-5 sentence executive summary, aiming for 130-140 words to fully incorporate the job's key themes. (Final output must be 110-150 words).
    ...
    def _render_experience_section(self, master_exp: Dict, overview: str, bullets: List[str]) -> str:
        ...
        for bullet in bullets:
            if isinstance(bullet, dict):
                bullet_lines.append(f"• {bullet.get('text', str(bullet))}")
...
class WorkflowOrchestrator:
    ...
    def _execute_hop_3_artist_generation_with_retry(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        artist_output = None
        feedback_results = None
        all_hop_results = [] # Accumulates results from all attempts
        max_attempts = 5
        ...
        for attempt in range(1, max_attempts + 1):
            all_passed, sanitized_output, raw_output, current_hop_results, feedback_results = \
                self._execute_hop_3_generation_attempt(
                    attempt, enriched_scaffold, job_description, thematic_analysis, feedback_results
                )
            
            all_hop_results.extend(current_hop_results)
            
            if all_passed or attempt == max_attempts:
                artist_output = sanitized_output # This is the final, sanitized data
                
                # Create checkpoint
                llm_calls_made = len([k for k in raw_output.keys() if raw_output.get(k)])
                hop_checkpoint = self._create_checkpoint(
                    "HOP-3", f"Artist Generation (attempt {attempt})", all_hop_results, artist_output,
                    metadata={"llm_api_calls": llm_calls_made}
                )
                self.hop_checkpoints.append(hop_checkpoint)
                self._check_hop_status(hop_checkpoint)
    ...
    def _build_qa_section_5_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]: # Renumbered to 5
        """Builds Section 5 of the QA report: Bullet Provenance & Word Count."""
        lines = ["", "5. BULLET PROVENANCE & WORD COUNT", ""] # Renumbered to 5
    ...
    def _build_qa_section_6_authenticity(self, validation_results: List[ValidationResult]) -> List[str]: # Renumbered to 6
        """Builds Section 6 of the QA report: Content Authenticity (Hallucination)."""
        lines = ["", "6. CONTENT AUTHENTICITY (Hallucination Detection)", ""] # Renumbered to 6
        auth_results = [vr for vr in validation_results if "HALLUCINATION" in vr.rule_id] # Fetch relevant results
        if not auth_results:
    ...
    def _build_qa_section_7_overview_similarity(self) -> List[str]: # Renumbered to 7
        """
        Builds Section 7 of the QA report: Overview vs. Bullet Similarity.
        v9.11: Renamed from _build_qa_section_6_ai_defense and expanded.
        """
        lines = ["", "7. OVERVIEW VS. BULLET SIMILARITY CHECK", ""] # Renumbered to 7
    ...
    def _build_qa_section_10_pipeline_health(self) -> List[str]: # Renumbered to 10
        """Builds Section 10 of the QA report: Pipeline Health."""
        lines = ["", "10. PIPELINE HEALTH (Resource Consumption)", ""] # Renumbered to 10
        headers = ["Hop ID", "Hop Name", "Status", "RAG API Calls", "LLM API Calls"] # Define columns
        rows = []
    ...
    def _build_qa_section_4_word_count(self, validation_results: List[ValidationResult]) -> List[str]: # Renumbered to 4
        """Builds Section 4 of the QA report: Word Count Compliance."""
        lines = ["", "4. WORD COUNT COMPLIANCE", ""] # Renumbered to 4
        wc_results = [vr for vr in validation_results if "WORD_COUNT" in vr.rule_id] # Fetch relevant results
        headers = ["Rule ID", "Status", "Message"] # Define columns
        rows = [[vr.rule_id, "PASS" if vr.passed else "FAIL", vr.message if not vr.passed else ""] for vr in wc_results] if wc_results else [["N/A", "INFO", "No word count validation results found."]]
        lines.extend(self._format_plain_text_table(headers, rows))
        return lines

    def _build_qa_section_11_structural(self, validation_results: List[ValidationResult]) -> List[str]: # Renumbered to 11
        """Builds Section 11 of the QA report: Structural Validation."""
        lines = ["", "11. STRUCTURAL VALIDATION", ""] # Renumbered to 11
        struct_results = [vr for vr in validation_results if "STRUCTURE" in vr.rule_id or "HEADLINE" in vr.rule_id] # Fetch relevant results
        headers = ["Rule ID", "Status", "Message"] # Define columns
        rows = []
        if struct_results:
    ...
    def _build_qa_section_12_prod_readiness(self, validation_results: List[ValidationResult]) -> List[str]: # Renumbered to 12
        """Builds Section 12 of the QA report: Production Readiness."""
        lines = ["", "12. PRODUCTION READINESS", ""] # Renumbered to 12
        ...
        headers = ["Check", "Value", "Status"] # Define columns
        rows = [
    ...
    QA_REPORT_SECTIONS = [
        ...
        {"method": "_build_qa_section_3_hop_summary", "args": []},                             # Section 3
        {"method": "_build_qa_section_4_word_count", "args": ["validation_results"]},          # Section 4 (Moved WC here)
        {"method": "_build_qa_section_5_provenance", "args": ["staging_buffer"]},              # Section 5 (Provenance)
        {"method": "_build_qa_section_6_authenticity", "args": ["validation_results"]},        # Section 6 (Was 5)
        {"method": "_build_qa_section_7_overview_similarity", "args": []},                     # Section 7 (Was 6)
        {"method": "_build_qa_section_8_exec_summary_overlap", "args": []},                    # Section 8 (Was 7)
        {"method": "_build_qa_section_9_bullet_deduplication", "args": []},                    # Section 9 (Was 7B)
        {"method": "_build_qa_section_10_pipeline_health", "args": []},                        # Section 10 (Was 8)
        {"method": "_build_qa_section_11_structural", "args": ["validation_results"]},         # Section 11 (Was 10)
        {"method": "_build_qa_section_12_prod_readiness", "args": ["validation_results"]},     # Section 12 (Was 11)
        {"method": "_build_qa_section_13_cover_letter", "args": ["validation_results"]},       # Section 13 (Was 12)
        {"method": "_build_qa_section_14_jd_enforcement", "args": []},                         # Section 14 (Was 13)
    ]
