# File: run_batch_v9_8.py
# Overwrites: run_batch_v9_7.py
...
logger = logging.getLogger("batch_runner_v9_8")
try:
...
SUMMARY_FILE = "batch_summary_v9_8.csv"
...
def process_single_job(job_path: str) -> Dict[str, Any]:
    ...
    logger.info(f"--- Starting v9.8 job: {os.path.basename(job_path)} ---")
    ...
    try:
        ...
        logger.info(f"--- Finished v9.8 job: {company} - {title} ---")
...
def run_batch():
    ...
    if not job_files:
        logger.info("v9.8 Batch process starting. No jobs found.")
        return

    logger.info(f"v9.8 Batch process starting. Found {len(job_files)} jobs.")
    ...
    logger.info(f"v9.8 Batch process complete.")
