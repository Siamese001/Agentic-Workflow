...
# Inside class EnhancedJobDescriptionAnalyzer:
    def _synthesize_thematic_analysis(
        ...
        v11.30 Fix: Force conversion to plain dict via JSON roundtrip.
        """
        import logging
        import json # Ensure json is imported
        from dataclasses import asdict # Still needed for fallback/conversion if direct JSON fails

        logger = logging.getLogger(__name__)
        logger.info("Synthesizing RAG results with weighted analysis...")

        # --- START FIX: Force conversion via JSON ---
        def force_to_dict(data: Any, phase_label: str) -> Dict:
            """Attempts to force data into a plain Python dictionary."""
            if isinstance(data, dict):
                 # If already dict, assume it's potentially nested non-dicts
                 try:
                     # Roundtrip through JSON to force conversion of nested objects
                     json_str = json.dumps(data, default=lambda o: asdict(o) if hasattr(o, '__dataclass_fields__') else str(o))
                     return json.loads(json_str)
                 except Exception as e:
                     logger.error(f"Failed JSON roundtrip for phase '{phase_label}' (already dict): {e}")
                     return {} # Fallback
            elif hasattr(data, '__dict__') or hasattr(data, 'items') or is_dataclass(data):
                 # Try converting known object types or dataclasses first
                 try:
                     # Roundtrip through JSON to force conversion
                     json_str = json.dumps(data, default=lambda o: asdict(o) if hasattr(o, '__dataclass_fields__') else str(o))
                     return json.loads(json_str)
                 except Exception as e:
                     logger.error(f"Failed JSON roundtrip for phase '{phase_label}' (object): {e}")
                     return {} # Fallback
            else:
                logger.warning(f"Unexpected data type for phase '{phase_label}': {type(data)}. Cannot reliably convert to dict.")
                return {} # Fallback

        phase1_dict = force_to_dict(phase1, "Phase 1")
        phase2_dict = force_to_dict(phase2, "Phase 2")
        phase3_dict = force_to_dict(phase3, "Phase 3")
        phase4_dict = force_to_dict(phase4, "Phase 4")

        # Add debug logging after conversion
        logger.debug(f"Type after JSON conversion - phase1_dict: {type(phase1_dict)}")
        if isinstance(phase1_dict, dict):
            p1_themes_check = phase1_dict.get("thematic_analysis")
            logger.debug(f"Type after JSON conversion - p1_themes_check: {type(p1_themes_check)}")
            if isinstance(p1_themes_check, dict):
                p1_primary_check = p1_themes_check.get("primary_theme")
                logger.debug(f"Type after JSON conversion - p1_primary_check: {type(p1_primary_check)}")
        # --- END FIX ---
        ...
        if not isinstance(p1_themes, dict): p1_themes = {} # Ensure dict
        p1_primary = p1_themes.get("primary_theme", {})
        if not isinstance(p1_primary, dict):
             logger.error(f"FATAL: p1_primary is not a dict even after JSON conversion! Type: {type(p1_primary)}")
             raise AttributeError("Internal error: primary_theme structure invalid after conversion.")
        ...
        for kw in p1_primary.get("keywords", []): # This line was failing
        ...
        # --- Rest of synthesis logic remains unchanged ---
        # (It relies on the *_dict variables being actual dictionaries now)
        ...
        competitive_intel = CompetitiveIntelligence(
            ...
            differentiator_keywords_raw=top_differentiators,
