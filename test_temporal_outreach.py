...
def test_temporal_vetting():
    ...
    try:
        ...
        for tz, utc_time, expected_send in test_cases:
            ...
            logger.info(f"{status} {tz}: UTC {utc_time} -> Local {result['lead_local_time']} -> {result['decision']}")
    ...
    try:
        ...
        try:
            ...
            if result1['status'] == "SENT_COMPLIANT":
                logger.info("✅ Test 1 PASSED: Email sent during business hours")
            else:
                logger.error(f"❌ Test 1 FAILED: Expected SENT_COMPLIANT, got {result1['status']}")
            ...
            if result2['status'] == "TEMPORAL_DELAY":
                ...
                logger.info(f"   Next optimal send time: {result2.get('next_optimal_send_time', 'N/A')}")
            else:
                logger.error(f"❌ Test 2 FAILED: Expected TEMPORAL_DELAY, got {result2['status']}")
...
def test_time_bound_benchmarking():
    ...
    try:
        ...
        if result['status'] == "success":
            ...
            logger.info(f"   Time window: {result.get('time_window_start', 'N/A')}")
            return True
        else:
            logger.error(f"❌ Salary benchmarking failed: {result.get('message', 'Unknown error')}")
            return False
