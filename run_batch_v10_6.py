# File: run_batch_v10_6.py
# Version: 10.6 (Refactored)
#
# v10.6 REFACTOR CHANGES:
# - UPDATED: All versioning to v10_6 (e.g., core_v10_6, ConfigV10_6).
# - UPDATED: create_workflow_context call updated to v10_6.
#
# v10.6 MAJOR CHANGES:
...
try:
    ...
    logging.getLogger("batch_runner_v10_6").warning("Meta-learning module (run_learning_v10_6.py) not found.")

logger = logging.getLogger("batch_runner_v10_6")

BATCH_QUEUE_DIR = "batch_queue"
BATCH_COMPLETE_DIR = "batch_complete"
SUMMARY_FILE = "batch_summary_v10_6.csv"
...
# ============================================================================
# ROW 6: ASYNC BATCH PROCESSING ENGINE (v10.6)
# ============================================================================

async def process_single_job_async(
    job_file: str,
    master_resume_path: str,
    context: WorkflowContext, # v10.6: This is the JOB-SPECIFIC context
    ...
    try:
        ...
        # v10.6 REFACTOR: Call centralized cleanup helper
...
async def run_batch_async(config: ConfigV10_6): # v10.6
    ...
    if not job_files:
        logger.info("v10.6 Batch process starting. No jobs found.")
        return
    
    logger.info(f"===== v10.6 Async Batch Process Starting =====")
    
    # v10.6 (Fix #25): Backpressure Check
    ...
    async def process_with_semaphore(job_file):
        ...
        async with semaphore:
            # v10.6: Create a job-specific context by injecting
            ...
            # v10.6: Manually inject the circular dependency
    ...
    if META_LEARNER_AVAILABLE and config.meta_loop_config.enable_meta_learning:
        logger.info("Triggering meta-learning loop (v10.6)...")
        try:
            # v10.6: Pass the config object to the meta-learner
            await run_meta_learning(config)
        except Exception as e:
            logger.error(f"Meta-learning failed: {e}")
    
    logger.info("v10.6 Batch process complete.")

def run_batch():
    """Synchronous wrapper for async batch processing"""
    # v10.6: Instantiate ConfigV10_6 here, ONCE.
    try:
        config = ConfigV10_6("master_config_v10_6.json")
    except Exception as e:
        print(f"FATAL: Failed to load master_config_v10_6.json: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    # v10.6: Inject the config object
