...
class YAMLHardeningValidatorRefined:
    ...
    def load_yaml(self):
        ...
        try:
            ...
            print("⚠️  Will proceed without baseline comparison")
    ...
    def compute_metrics(self, data=None):
        ...
        metrics = {
            ...
            'structural_domain_violations': defaultdict(int),  # Refined K50
            'layer_keys': set(),
            'phase_keys': set(),
            'all_keys': [],
            'file_paths': [],
        ...
        def traverse(obj, path="", depth=0):
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    # REFINED K50: Only flag structural domain violations
                    # Check if this key is a structural domain violation
                    # Rule: Domain token as a path segment below root is a violation
                    # Exception: Semantic uses in filenames are allowed
                    if depth > 0 and not key.endswith(('.py', '.json', '.yaml', '.yml', '.md', '.txt')):
                        for domain in domains:
                            ...
                            elif key.startswith(f'{domain}_') or key.endswith(f'_{domain}'):  # Domain prefix/suffix as structural label
                                metrics['structural_domain_violations'][domain] += 1
    ...
    def validate_k50(self):
        """Validate H4 - Domain token deduplication (REFINED)"""
        results = {}
        
        # REFINED K50: Only structural domain violations should be 0
        # Semantic uses in filenames are allowed
    ...
    def run_full_validation(self):
        ...
        print("=== YAML SSoT HARDENING VALIDATION (REFINED) ===")
        print("✓ Refined K50: Only structural domain violations flagged")
        print("✓ Semantic domain words in filenames are allowed")
    ...
    def generate_report(self):
        ...
        print("\n" + "="*80)
        print("YAML SSoT HARDENING VALIDATION REPORT (REFINED)")
        print("="*80)
        ...
        before_single = self.metrics_before.get('count_single_child_dirs', 'N/A')
        after_single = self.metrics_after.get('count_single_child_dirs', 0)
        single_status = '✅ REDUCED' if before_single != 'N/A' and after_single < before_single else '⚠️ NEEDS BASELINE'
        print(f"| Single child dirs | {before_single} | {after_single} | {single_status} |")
        
        # Structural domain violations (refined K50)
        print(f"\n## STRUCTURAL DOMAIN VIOLATIONS (REFINED K50)")
        print("| Domain | Violations | Status |")
        print("|--------|------------|--------|")
        for domain, count in self.metrics_after['structural_domain_violations'].items():
        ...
        hardenings = [
            ...
            ("H4", "Domain token deduplication (REFINED)", "K50"),
...
if __name__ == '__main__':
    validator = YAMLHardeningValidatorRefined('unified_structure_subatomic.yaml')
