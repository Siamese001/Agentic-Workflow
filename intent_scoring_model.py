...
class IntentScoringModel:
    ...
    def _extract_features(
        ...
        company_growth_stage = news_context.get("growth_stage", "mature").lower()
        
        # Temporal features
        days_since_last_contact = engagement_data.get("days_since_last_contact", 30)
        contact_frequency = engagement_data.get("contact_frequency", "low").lower()
        
        # Content features
        personalization_score = personalization_data.get("personalization_score", 0.5)
        template_relevance = personalization_data.get("template_relevance", 0.5)
    ...
    def _score_engagement(self, features: IntentFeatures) -> float:
        ...
        if features.previous_interactions > 0:
            interaction_score = min(features.previous_interactions / 10, 1.0) * 20
    ...
    def score_lead_intent(
        ...
        if logger:
            logger.info(f"✅ Intent scored: {overall_score:.1f}/100 ({priority} priority)")
