...
class PII_SanitizerSpecialistAgent(LICAgentBase):
    ...
    pii_patterns: dict[str, Any] = field(default_factory=lambda: {'EMAIL': re.compile('\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b'), 'PHONE': re.compile('\\b(?:\\+?1[ -]?)?\\(?\\d{3}\\)?[ -]?\\d{3}[ -]?\\d{4}\\b'), 'NAME': re.compile('\\b[A-Z][a-z]+ [A-Z][a-z]+\\b')})

    def __post_init__(self) -> None:
        """Initialize Sovereign Capabilities."""
        super().__post_init__()
    PII_PATTERNS = {'EMAIL': re.compile('\\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}\\b'), 'PHONE': re.compile('\\b(?:\\+?1[ -]?)?\\(?\\d{3}\\)?[ -]?\\d{3}[ -]?\\d{4}\\b'), 'NAME': re.compile('\\b[A-Z][a-z]+ [A-Z][a-z]+\\b')}
...
class BiasDetectorSpecialist(LICAgentBase, SubatomicTestingMixin):
    ...
    def scan_content(self, text: str) -> dict[str, Any]:
        ...
        additional_patterns = [p for p in bias_patterns if p.lower() in text.lower()]
        patterns_found.extend(additional_patterns)
        result = {'has_bias': len(patterns_found) > 0, 'bias_detected': len(patterns_found) > 0, 'patterns': patterns_found, 'sensitivity_level': self.sensitivity_level}
...
class PromptInjectionDetectorSpecialist(LICAgentBase, SubatomicTestingMixin):
    ...
    attack_patterns: list[str] = field(default_factory=lambda: ['ignore previous instructions', 'system prompt', 'jailbreak'])
    ...
    async def run_async(self, user_input: str, workflow_id: str) -> dict[str, Any]:
        ...
        prompt = await _format_prompt_with_defaults(prompt_template, {'user_input': user_input}, self.budget_manager, client.goal_state, client.top_failures)
        response = await client.chat_completion_async(messages=[{'role': 'user', 'content': prompt}], temperature=self.config.model_config.prompt_injection_model.temperature, response_format='json_object')
...
class ConstitutionalReviewerAgent(LICAgentBase, SubatomicTestingMixin):
    ...
    async def run_async(self, final_draft: str, workflow_id: str) -> ConstitutionalReviewResult:
        ...
        prompt = await _format_prompt_with_defaults(prompt_template, {'final_draft': final_draft, 'constitution': constitution_text}, self.budget_manager, client.goal_state, client.top_failures)
        response = await client.chat_completion_async(messages=[{'role': 'user', 'content': prompt}], temperature=self.config.model_config.constitutional_review_model.temperature, response_format='json_object')
