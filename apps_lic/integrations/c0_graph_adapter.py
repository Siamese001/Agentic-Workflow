"""apps_lic C0.3 graph adapter — W4N chroma-graphrag-lic-rg-research-f4a2e9.
...
W4N (no-core track) invariants enforced here:
  ...
  - ``build_lic_graph_traverse_input()`` returns a typed dict; live
    ``GraphTraverseInput`` construction is deferred (C0.3 adapter registry not
    wired under no-core plan).
  - Terminal routes carry no graph policy; this adapter is only reached on
    R3_SIMPLE_GROUNDED_READ (non-terminal grounded route).
  - apps_lic semantic cache remains disabled (W2N invariant preserved).
"""
...
# ---------------------------------------------------------------------------
# W4N policy constants — must match apps_lic/config/domain_contract/route_profiles.yaml
# ---------------------------------------------------------------------------

_GRAPH_SOURCE = "apps_lic.knowledge_graph.v1"
_PROJECTION_VERSION = "v1.0.0"
_SNAPSHOT_POINTER = "apps_lic::snapshot::placeholder"
...
def build_lic_graph_traverse_input(
    ...
    """Build a GraphTraverseInput-compatible dict for apps_lic.
...
class LicGraphAdapter:
    """apps_lic knowledge-graph adapter for C0.3.
    ...
    def resolve_anchor(
        ...
        return UnresolvedAnchorResolution(
            ...
            reason="apps_lic graph adapter: stub — no live backend connected (W4N)",
        )
...
def get_graph_adapter() -> GraphTraversalAdapter:
    ...
    Returns a fresh ``LicGraphAdapter`` instance each call.
    """
