"""Implementation for campaign_rag_impl."""
...
class SignalQualityScorer:
    ...
    """
    SOURCE_WEIGHTS = {'STRATEGIC_BRIEF': 2.5, 'RECIPIENT_LINKEDIN_ABOUT': 2.0, 'RECIPIENT_RECENT_POST': 1.8, 'COMPANY_BLOG_ANNOUNCEMENT': 1.5, 'COMPANY_LINKEDIN_PAGE': 1.3, 'NEWS_ARTICLE_COMPANY': 1.2, 'COMPETITOR_COMPARISON': 0.9, 'GENERIC_INDUSTRY_TREND': 0.6, 'SENDER_PROFILE_ONLY': 0.3, 'MASTER_RESUME': 2.0, 'SENDER_KNOWLEDGE_BASE': 1.8}
    MINIMUM_SIGNAL_THRESHOLD = 0.7

    def calculate_signal_score(self, rag_results: List[RAGResult], message_content: str) -> Tuple[float, Dict[str, int]]:
        """
...
class ClaimConfidenceScorer:
    ...
    def score_claim(self, claim_text: str, rag_results: List[RAGResult], embedding_similarity_threshold: float=0.75) -> MessageClaim:
        """
        ...
        return MessageClaim(text=claim_text, confidence=confidence, supporting_sources=supporting, source_weights=weights)

    def score_message_claims(self, message: str, rag_results: List[RAGResult]) -> Tuple[List[MessageClaim], float]:
        """
    ...
    def validate_claims(self, claims: List[MessageClaim], aggregate_confidence: float) -> Tuple[bool, str]:
        """
...
class RAGReflexionSystem:
    ...
    def critique_rag_sufficiency(self, rag_results: List[RAGResult], recipient_archetype: Archetype, iteration: int) -> RAGCritique:
        """
        ...
        return RAGCritique(confidence_score=confidence, gaps_identified=gaps, refinement_tasks=refinement_tasks, reasoning=reasoning, is_sufficient=is_sufficient)
    ...
    def _calculate_confidence(self, rag_results: List[RAGResult], gaps: List[str]) -> float:
        ...
        diversity_bonus = min(0.3, num_source_types * 0.1)
...
class RecipientAgent:
    ...
    async def validate_entity(self, entity_name: str, entity_context: str, mission: OutreachMission) -> Dict[str, object]:
        ...
        search_results = await loop.run_in_executor(None, self.search_client.search, query, 2)
        ...
        return {'rag_results': rag_results, 'is_validated': is_validated, 'staleness_warning': staleness_warning}
    ...
    async def run_refinement_task(self, task: str, mission: OutreachMission) -> Dict[str, object]:
        ...
        loop = asyncio.get_event_loop()
        search_results = await loop.run_in_executor(None, self.search_client.search, task, 2)
    ...
    def _process_search_results(self, search_results: list, entity_name: str, recipient_specific: bool) -> List[RAGResult]:
        ...
        for item in search_results:
            ...
            if 'github.com' in link:
                ...
                source_type = 'RECIPIENT_LINKEDIN_ABOUT'
            rag_results.append(RAGResult(source=link, source_type=source_type, text=text, extracted_keywords=keywords, source_weight=1.5, age_days=30, recipient_specific=recipient_specific, confidence=0.8))
        return rag_results
...
class OrganizationAgent:
    ...
    def _process_search_results(self, search_results: list, entity_name: str) -> List[RAGResult]:
        ...
        for item in search_results:
            ...
            rag_results.append(RAGResult(source=link, source_type=source_type, text=text, extracted_keywords=keywords, source_weight=1.3, age_days=30, recipient_specific=False, confidence=0.75))
        return rag_results
...
class InternalAgent:
    ...
    def get_internal_context(self, mission: OutreachMission) -> Dict[str, object]:
        ...
        if brief_entities:
            pass
        return {'prior_applications': prior_applications, 'rag_results': rag_results, 'brief_entities': brief_entities}
    ...
    def _load_strategic_brief(self) -> Tuple[List[RAGResult], List[Dict[str, str]]]:
        ...
        try:
            ...
            for para in paragraphs[:50]:
                ...
                rag_results.append(RAGResult(source=pdf_path, source_type='STRATEGIC_BRIEF', text=para, extracted_keywords=keywords, source_weight=2.5, age_days=0, recipient_specific=True, confidence=1.0))
    ...
    def _extract_entities_from_text(self, text: str) -> List[Dict[str, str]]:
        ...
        """
        entities = []
        person_pattern = '([A-Z][a-z]+ [A-Z][a-z]+)(?:\\s+as\\s+|\\,\\s+)([A-Z][^\\.]+)'
        for match in re.finditer(person_pattern, text):
            entities.append({'type': 'person', 'name': match.group(1), 'context': match.group(2)[:100]})
        initiative_pattern = '"([^"]{10,50})"'
        for match in re.finditer(initiative_pattern, text):
            phrase = match.group(1)
            if any((word[0].isupper() for word in phrase.split())):
                entities.append({'type': 'initiative', 'name': phrase, 'context': ''})
        unique_entities = {}
        for entity in entities:
            unique_entities[entity['name']] = entity
        return list(unique_entities.values())[:10]
    ...
    def _load_resume_as_rag(self) -> List[RAGResult]:
        ...
        for exp in resume_data.get('professional_experience', []):
            ...
            for bullet in exp.get('bullet_pool', []):
                ...
                rag_results.append(RAGResult(source=f'master_resume_{company}', source_type='MASTER_RESUME', text=bullet, extracted_keywords=keywords, source_weight=2.0, age_days=0, recipient_specific=False, confidence=1.0))
        return rag_results
    ...
    def _load_kb_as_rag(self) -> List[RAGResult]:
        ...
        for vp in kb_data.get('core_value_propositions', []):
            ...
            keywords = list(set(keywords[:15]))
            rag_results.append(RAGResult(source='sender_knowledge_base', source_type='SENDER_KNOWLEDGE_BASE', text=vp, extracted_keywords=keywords, source_weight=1.8, age_days=0, recipient_specific=False, confidence=1.0))
        for product in kb_data.get('whitelisted_products', []):
            ...
            keywords = [w.strip('.,!?') for w in text.split() if len(w) > 4]
            keywords = list(set(keywords[:15]))
            rag_results.append(RAGResult(source='sender_knowledge_base', source_type='SENDER_KNOWLEDGE_BASE', text=text, extracted_keywords=keywords, source_weight=1.8, age_days=0, recipient_specific=False, confidence=1.0))
...
class S2_SupervisorAgent:
    ...
    async def orchestrate_research(self, mission: OutreachMission, profile_analysis: ProfileAnalysis, refinement_context: Optional[List]=None) -> Tuple[ResearchContext, ProfileAnalysis]:
        """
        ...
        if brief_entities:
            for entity in brief_entities[:5]:
                if entity['type'] == 'person':
                    validation = await self.recipient_agent.validate_entity(entity['name'], entity['context'], mission)
        ...
        while reflexion_iterations < 2:
            critique = self.rag_reflexion.critique_rag_sufficiency(rag_results, profile_analysis.archetype, iteration=reflexion_iterations + 1)
        ...
        context = ResearchContext(recipient_insights=[f"Title: {mission.recipient_profile.get('title')}", f"Company: {mission.recipient_profile.get('company')}", f'Archetype: {profile_analysis.archetype.value}'], company_context=[f"Company: {mission.job_description.get('company')}", f"Job: {mission.job_description.get('title')}"], recent_activity=[], rag_results=rag_results, reflexion_iterations=reflexion_iterations, prior_applications=prior_applications, mission_context={'job_title': mission.job_description.get('title', ''), 'company': mission.job_description.get('company', ''), 'sender_teams': mission.sender_profile.get('teams', [])}, sender_context=[], sender_grounding=sender_grounding)
        corrected_profile_analysis = None
    ...
    async def _run_adversarial_check(self, context: ResearchContext) -> List[str]:
        rag_summary = '\n'.join([f'- {r.source_type}: {r.text[:100]}...' for r in context.rag_results[:10]])
    ...
    def _extract_sender_grounding(self, rag_results: List[RAGResult], mission: OutreachMission) -> SenderGroundingWhitelists:
    ...
    def _critique_archetype_classification(self, provisional_analysis: ProfileAnalysis, context: ResearchContext) -> ProfileAnalysis:
        ...
        if provisional_analysis.archetype != Archetype.C_LEVEL:
            if any((term in all_text for term in ['strategic vision', 'board member', 'company direction'])):
                ...
                provisional_analysis.confidence = 0.9
