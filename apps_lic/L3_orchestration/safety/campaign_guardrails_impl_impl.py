"""Implementation for campaign_guardrails_impl."""
...
class BiasDetectorAgent(BaseAgent):
    ...
    def run(self, text: str, workflow_id: str='') -> Dict[str, object]:
        ...
        if workflow_id:
            self.log_feedback(workflow_id, 'bias_detection', 'warning' if result['bias_detected'] else 'success', {'patterns_found': len(result.get('patterns', []))})
        return result
