"""GPT-5.5 X1D judge adapter for apps_lic.

The Exit engine remains deterministic: this module only produces
``X1DJudgeResult`` artifacts for W5/W7 to consume.
"""
...
DEFAULT_GPT_TRANSPORT_MODEL_ID = resolve_x1d_judge_transport_model_id()
DEFAULT_GPT_MAX_TOKENS = 1200


class GPTX1DTransport(Protocol):
...
class X1DGPTJudgeRequest:
    ...
    max_tokens: int = DEFAULT_GPT_MAX_TOKENS
...
def build_gpt_x1d_judge_request(
    *,
    ...
    system_prompt = (
        ...
        "not objects. Use compact snake_case reason strings. You cannot override "
        "deterministic X2 gates, C0 evidence, no-send policy, or Exit clearance."
    )
    user_prompt = json.dumps(
        {
            ...
            "instructions": (
                ...
                "claims, sales-pitch tone, over-specificity without evidence, and weak "
                "LinkedIn fit. Respond only as JSON."
            ),
        },
        sort_keys=True,
    )
    return X1DGPTJudgeRequest(
...
def parse_gpt_x1d_response(
    ...
    *,
    ...
    return X1DJudgeResult(
        ...
        required_repairs=_string_tuple(payload.get("required_repairs")),
    )


def run_gpt_x1d_judges(
    request: WholeMessageGenerationRequest,
    candidate: WholeMessageCandidate,
    *,
    transport: GPTX1DTransport,
    transport_model_id: str = DEFAULT_GPT_TRANSPORT_MODEL_ID,
) -> tuple[X1DJudgeResult, ...]:
    if not isinstance(transport, OpenAIGPTX1DTransport) or not getattr(transport, "live_gpt_transport", False):
        return tuple(
            X1DJudgeResult(
                judge_id=profile.judge_id,
                model=profile.model,
                ...
                threshold=profile.threshold,
                availability_status=JUDGE_UNAVAILABLE,
                issues=("non_live_gpt_transport_rejected",),
            )
    ...
    for profile in required_x1d_profiles(request):
        judge_request = build_gpt_x1d_judge_request(
            request=request,
            candidate=candidate,
            profile=profile,
            transport_model_id=transport_model_id,
        ...
        try:
            ...
            continue
        results.append(parse_gpt_x1d_response(raw, profile=profile, trust_transport_proof=True))
    return tuple(results)


class OpenAIGPTX1DTransport:
    """Optional live transport backed by the OpenAI API."""

    live_gpt_transport = True

    def __init__(
        self,
        *,
        api_key: str = "",
        model_id: str = DEFAULT_GPT_TRANSPORT_MODEL_ID,
    ...
    def __call__(self, payload: Mapping[str, Any]) -> Mapping[str, Any] | str:
        if self._client_injected:
            return {
                ...
                "passed": False,
                "availability_status": JUDGE_UNAVAILABLE,
                "issues": ["injected_openai_client_not_live"],
            }
        ...
        if client is None:
            return {
                ...
                "passed": False,
                "availability_status": JUDGE_UNAVAILABLE,
                "issues": ["gpt_transport_unavailable"],
            }
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": str(payload.get("user_prompt") or "")},
                ],
                temperature=float(payload.get("temperature") or 0.10),
                max_tokens=int(payload.get("max_tokens") or DEFAULT_GPT_MAX_TOKENS),
            )
        except Exception as exc:  # guardian: allow-broad-exception -- OpenAI SDK exposes provider/network subclasses that vary by version; surface as unavailable artifact
            return {
                "score": 0.0,
                "passed": False,
                "availability_status": JUDGE_UNAVAILABLE,
                "issues": [f"gpt_transport_error:{type(exc).__name__}"],
            }
        ...
        if not parsed:
            return {
                ...
                "transport_call_id": str(getattr(response, "id", "") or "openai_chat_completion_response"),
                "model": DEFAULT_X1D_JUDGE_MODEL,
                "provider": DEFAULT_X1D_JUDGE_PROVIDER,
            }
        parsed["transport_provenance"] = LIVE_GPT_API_CALL
        parsed["transport_provider"] = OPENAI_RESPONSES_API
        parsed["transport_call_id"] = str(getattr(response, "id", "") or "openai_chat_completion_response")
