...
class PersonaPlanner:
    ...
    def __init__(self, telemetry_bus: Any | None=None) -> None:
        ...
        self.executive_persona = {'tone_style': 'concise_executive', 'detail_level': 'low', 'risk_tolerance': 'low', 'drift_threshold': 0.2, 'communication_style': 'formal', 'decision_maker_type': 'analytical', 'time_preference': 'immediate'}
        self.senior_ta_persona = {'tone_style': 'technical_detailed', 'detail_level': 'high', 'risk_tolerance': 'medium', 'drift_threshold': 0.3, 'communication_style': 'technical', 'decision_maker_type': 'analytical', 'time_preference': 'considered'}
        self.recruiter_persona = {'tone_style': 'friendly_recruiter', 'detail_level': 'medium', 'risk_tolerance': 'medium', 'drift_threshold': 0.4, 'communication_style': 'professional', 'decision_maker_type': 'collaborative', 'time_preference': 'considered'}
        self.default_persona = {'tone_style': 'neutral', 'detail_level': 'medium', 'risk_tolerance': 'medium', 'drift_threshold': 0.3, 'communication_style': 'professional', 'decision_maker_type': 'collaborative', 'time_preference': 'deliberate'}
        self.seniority_adjustments = {'C_LEVEL': {'detail_level': 'low', 'risk_tolerance': 'low', 'time_preference': 'immediate', 'drift_threshold': 0.15}, 'SENIOR': {'detail_level': 'medium', 'risk_tolerance': 'medium', 'time_preference': 'considered', 'drift_threshold': 0.25}, 'MID_LEVEL': {'detail_level': 'high', 'risk_tolerance': 'medium', 'time_preference': 'deliberate', 'drift_threshold': 0.35}, 'JUNIOR': {'detail_level': 'high', 'risk_tolerance': 'high', 'time_preference': 'deliberate', 'drift_threshold': 0.4}}
        self.industry_adjustments = {'technology': {'detail_level': 'high', 'communication_style': 'technical'}, 'finance': {'risk_tolerance': 'low', 'communication_style': 'formal'}, 'healthcare': {'risk_tolerance': 'low', 'detail_level': 'medium'}, 'consulting': {'communication_style': 'professional', 'detail_level': 'high'}, 'sales': {'tone_style': 'friendly_recruiter', 'time_preference': 'immediate'}}

    def plan(self, *, Archetype: str, recipient_profile: dict[str, object], grounding_plan: Any | None=None, outreach_context: dict[str, object]=None) -> PersonaPlan:
        ...
        industry_adjusted: Any = self._apply_industry_adjustments(seniority_adjusted, recipient_profile, outreach_context)
        ...
        metadata: Any = {'Archetype': Archetype, 'base_persona': base_persona['tone_style'], 'seniority': recipient_profile.get('seniority', 'unknown'), 'industry': recipient_profile.get('industry', 'unknown'), 'confidence_score': confidence_score, 'adjustments_applied': self._count_adjustments(base_persona, final_persona)}
        plan: Any = PersonaPlan(Archetype=Archetype, tone_style=final_persona['tone_style'], detail_level=final_persona['detail_level'], risk_tolerance=final_persona['risk_tolerance'], drift_threshold=final_persona['drift_threshold'], communication_style=final_persona['communication_style'], decision_maker_type=final_persona['decision_maker_type'], time_preference=final_persona['time_preference'], confidence_score=confidence_score, metadata=metadata)
    ...
    def _get_base_persona(self, Archetype: str) -> dict[str, object]:
        ...
        archetype_map = {'C_LEVEL': self.executive_persona, 'EXECUTIVE': self.executive_persona, 'SENIOR_TA': self.senior_ta_persona, 'RECRUITER': self.recruiter_persona}
    ...
    def _apply_industry_adjustments(self, persona: dict[str, object], profile: dict[str, object], context: dict[str, object]) -> dict[str, object]:
        ...
        industry = profile.get('industry', '').lower() or context.get('industry', '').lower() or profile.get('company_industry', '').lower()
        adjustments = {}
    ...
    def _apply_grounding_refinements(self, persona: dict[str, object], grounding_plan: Any | None) -> dict[str, object]:
    ...
    def _safe_record_telemetry(self, plan: PersonaPlan) -> None:
        ...
        try:
            if self.telemetry_bus:
                self.telemetry_bus.record('persona_plan_created', {'Archetype': plan.Archetype, 'tone_style': plan.tone_style, 'detail_level': plan.detail_level, 'risk_tolerance': plan.risk_tolerance, 'confidence_score': plan.confidence_score})
    ...
    def get_persona_summary(self, plan: PersonaPlan) -> dict[str, object]:
        ...
        return {'plan_id': f'persona_{plan.Archetype}_{plan.tone_style}', 'Archetype': plan.Archetype, 'tone_style': plan.tone_style, 'detail_level': plan.detail_level, 'risk_tolerance': plan.risk_tolerance, 'communication_style': plan.communication_style, 'decision_maker_type': plan.decision_maker_type, 'time_preference': plan.time_preference, 'drift_threshold': plan.drift_threshold, 'confidence_score': plan.confidence_score, 'adjustments_count': plan.metadata.get('adjustments_applied', 0)}
