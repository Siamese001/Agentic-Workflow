...
class HOPOrchestratorAgent(V2AgentBase, SubatomicTestingMixin, MCPHardenedMixin, HealerMixin):
    ...
    def __init__(self, llm_client: Optional[Any] = None, mission_id: str = "default") -> None:
    ...
    def run_mission(self, mission_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.registry.add_trace("ORCHESTRATOR_START", {"mission_id": mission_input.get("mission_id")})
        ...
        try:
            ...
            for hop in ["HOP1", "HOP2", "HOP3", "HOP4"]:
                ...
                if step_count > self.GLOBAL_STEP_LIMIT:
                    self.registry.add_trace("CRITICAL_FAILURE", {"reason": "Global step limit exceeded"})
            ...
            while iteration < max_iterations:
                ...
                if not self._check_retry_limits(gate):
                    self.registry.add_trace("ORCHESTRATOR_LIMIT_EXCEEDED", {
                        "action": gate["action"],
                        "iteration": iteration
                    })
                    break
    ...
    def _handle_retry(self, gate: Dict, current_buffer: ImmutableStagingBuffer) -> ImmutableStagingBuffer:
        """
        ...
        """
        action = gate["action"]
        self.registry.add_trace("ORCHESTRATOR_RETRY", {"action": action, "reason": gate.get("reason")})
    ...
    def _check_retry_limits(self, gate: Dict) -> bool:
        ...
        retry_count = sum(1 for t in traces if t.get("type") == "ORCHESTRATOR_RETRY" and t.get("details", {}).get("action") == action)
