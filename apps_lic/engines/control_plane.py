...
class PolicyDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if self.metadata is None:
            self.metadata = {}
_PII_PATTERNS = ('ssn', 'social security', 'credit card', 'passport', 'date of birth', 'phone number', 'email address', '@gmail', '@yahoo', '@hotmail')
...
class ControlPlane:
    ...
    def _evaluate(self, content: str, context: dict[str, Any] | None, is_input: bool) -> PolicyDecision:
        ...
        if detected_pii:
            ...
            self._block_count += 1
            logger.warning('ControlPlane: PII detected in %s content: %s', 'input' if is_input else 'output', detected_pii)
            return PolicyDecision(action=PolicyAction.BLOCK, is_safe=False, warnings=warnings, errors=errors, metadata={'is_input': is_input, 'decision_id': self._decision_count, 'pii': detected_pii})
            ...
            try:
                ...
                if isinstance(shield_result, dict):
                    if shield_result.get('blocked'):
                        ...
                        self._block_count += 1
                        return PolicyDecision(action=PolicyAction.BLOCK, is_safe=False, warnings=warnings, errors=errors, metadata={'is_input': is_input, 'decision_id': self._decision_count, 'shield': shield_result})
        ...
        return PolicyDecision(action=action, is_safe=True, warnings=warnings, errors=errors, metadata={'is_input': is_input, 'decision_id': self._decision_count})
