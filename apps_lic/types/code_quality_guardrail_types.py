...
class CodeQualityGuardrail:
    ...
    def __init__(self):
        ...
        self.enabled_rules: list[str] = ['formatting', 'duplication', 'unused_code', 'dependencies', 'git_hygiene']
    ...
    async def validate(self, code: str, file_path: str | None=None) -> QualityResult:
        ...
        return QualityResult(valid=not any((i.severity == 'error' for i in issues)), issues=issues, metrics={'line_count': len(code.splitlines()), 'issue_count': len(issues)})
    ...
    def _check_formatting(self, code: str, file_path: str | None) -> list[CodeIssue]:
        ...
        for i, line in enumerate(lines, 1):
            if len(line) > self.max_line_length:
                issues.append(CodeIssue(rule='formatting', severity='warning', message=f'Line exceeds {self.max_line_length} characters ({len(line)})', file_path=file_path, line_number=i, suggestion='Consider breaking this line'))
        if len(lines) > self.max_file_length:
            issues.append(CodeIssue(rule='formatting', severity='warning', message=f'File exceeds {self.max_file_length} lines ({len(lines)})', file_path=file_path, suggestion='Consider splitting into multiple files'))
        for i, line in enumerate(lines, 1):
            if line != line.rstrip():
                issues.append(CodeIssue(rule='formatting', severity='info', message='Trailing whitespace', file_path=file_path, line_number=i))
        return issues
        ...
        for i in range(len(lines) - self.min_duplicate_lines):
            ...
            if block_hash in self.code_hashes:
                if file_path not in self.code_hashes[block_hash]:
                    issues.append(CodeIssue(rule='duplication', severity='warning', message='Duplicate code block detected', file_path=file_path, line_number=i + 1, suggestion='Consider extracting to shared function'))
            else:
        ...
        for i, line in enumerate(lines, 1):
            for pattern in self.unused_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    issues.append(CodeIssue(rule='unused_code', severity='info', message='Potential cleanup needed', file_path=file_path, line_number=i, suggestion='Review and remove if no longer needed'))
        return issues
    ...
    def validate_commit_message(self, message: str) -> QualityResult:
        ...
        for pattern in self.bad_commit_patterns:
            if re.match(pattern, message.lower().strip()):
                issues.append(CodeIssue(rule='git_hygiene', severity='error', message='Commit message too short or unclear', suggestion='Use descriptive commit messages'))
        if len(message.strip()) < 10:
            issues.append(CodeIssue(rule='git_hygiene', severity='warning', message='Commit message is too short', suggestion='Add more context to the commit message'))
    ...
    def validate_dependencies(self, dependencies: list[str], used: set[str]) -> QualityResult:
        ...
        for dep in unused:
            issues.append(CodeIssue(rule='dependencies', severity='warning', message=f'Unused dependency: {dep}', suggestion='Consider removing from dependencies'))
        return QualityResult(valid=len(issues) == 0, issues=issues, metrics={'total_dependencies': len(dependencies), 'unused_dependencies': len(unused)})

    def get_statistics(self) -> dict[str, Any]:
        """Get code quality statistics."""
        return {'checks_performed': self.checks_performed, 'issues_found': self.issues_found, 'enabled_rules': self.enabled_rules}
