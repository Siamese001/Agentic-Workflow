...
class StackModernizationAgent:
    ...
    def __init__(self):
        ...
        self.legacy_markers = {'infrastructure': {'on-prem': 'Mature on-premise infrastructure', 'data center': 'Established data center operations', 'hybrid cloud': 'Hybrid cloud environment', 'vmware': 'VMware virtualization platform', 'physical servers': 'Physical server infrastructure', 'mainframe': 'Legacy Mainframe Infrastructure'}, 'data': {'oracle': 'Oracle database systems', 'stored procedures': 'Stored procedure architecture', 'db2': 'IBM Db2 Database', 'hadoop': 'Hadoop data processing', 'data warehouse': 'Traditional data warehouse', 'etl': 'ETL pipeline architecture'}, 'application': {'monolith': 'Monolithic application architecture', 'java 8': 'Java 8 application stack', 'soap': 'SOAP web services', 'xml': 'XML-based data exchange', 'jsp': 'JSP presentation layer', 'cobol': 'Legacy COBOL Systems'}, 'search': {'elasticsearch': 'Elasticsearch (keyword-only)', 'solr': 'Apache Solr search', 'keyword search': 'Keyword-based search', 'full-text search': 'Traditional full-text search'}}
        self.pain_point_mappings = {'infrastructure': ['Slow deployment cycles', 'Limited scalability', 'High operational overhead', 'Difficulty in disaster recovery'], 'data': ['Data silos and duplication', 'Slow query performance', 'Complex data transformations', 'Limited real-time capabilities'], 'application': ['Tight coupling between components', 'Difficult to scale individual features', 'Slow innovation cycles', 'High risk deployments'], 'search': ['Limited semantic understanding', 'Poor relevance for complex queries', 'Difficulty handling unstructured data', 'Limited multilingual support']}
        self.transformation_playbooks = {'search_migration': {'legacy': ['elasticsearch', 'solr', 'keyword search'], 'modern': 'Vector-based Semantic RAG', 'thesis': 'Moving from keyword matching to Semantic RAG to improve retrieval accuracy by 40%', 'strategy': 'Implement Strangler Fig Pattern: gradually replace keyword endpoints with vector search while maintaining 100% uptime'}, 'monolith_decomposition': {'legacy': ['monolith', 'java 8', 'soap'], 'modern': 'Event-Driven Agentic Architecture', 'thesis': 'Decoupling complex workflows into an Event-Driven Agentic Architecture for scalability', 'strategy': 'Apply Parallel Run pattern: run new agent system alongside monolith, gradually migrating workflows'}, 'data_modernization': {'legacy': ['oracle', 'data warehouse', 'etl'], 'modern': 'Knowledge Graph + Lakehouse', 'thesis': 'Unlocking siloed data by overlaying a Knowledge Graph for AI context', 'strategy': 'Implement CDC (Change Data Capture) for real-time sync while maintaining warehouse for reporting'}, 'cloud_migration': {'legacy': ['on-prem', 'data center', 'physical servers'], 'modern': 'Cloud-Native Microservices', 'thesis': 'Transitioning to cloud-native architecture for elastic scalability and reduced TCO', 'strategy': 'Use Lift-and-Shift followed by modernization, with parallel environments to ensure zero downtime'}}
    ...
    def diagnose_stack(self, job_description: str) -> LegacyDiagnostic:
        ...
        try:
            ...
            diagnostic = LegacyDiagnostic(detected_legacy_tech=unique_tech, implied_pain_points=unique_pain[:5], modernization_score=score)
    ...
    def generate_thesis(self, diagnostic: LegacyDiagnostic) -> MigrationThesis:
        ...
        try:
            if not diagnostic.detected_legacy_tech:
                return MigrationThesis(current_state_diagnosis='Modern technology stack with opportunities for optimization', target_state_vision='Enhanced AI capabilities with advanced architectures', bridge_strategy='Incremental improvements and strategic AI integration')
            ...
            return MigrationThesis(current_state_diagnosis=f"Mature infrastructure with {', '.join(diagnostic.detected_legacy_tech[:2])}", target_state_vision='Modern, cloud-native AI architecture', bridge_strategy='Gradual migration using Strangler Fig Pattern to ensure business continuity')
        # guardian: allow-silent-swallow
        except Exception as e:
            logger.error(f'Error generating thesis: {str(e)}')
            return MigrationThesis(current_state_diagnosis='Legacy technology stack', target_state_vision='Modern architecture', bridge_strategy='Safe, incremental migration approach')
    ...
    def _create_thesis_from_playbook(self, playbook: dict[str, str], diagnostic: LegacyDiagnostic) -> MigrationThesis:
        ...
        try:
            ...
            return MigrationThesis(current_state_diagnosis=current, target_state_vision=target, bridge_strategy=strategy)
        # guardian: allow-silent-swallow
        except Exception as e:
...
def analyze_modernization_opportunity(job_description: str) -> dict[str, Any]:
    ...
    return {'legacy_score': diagnostic.modernization_score, 'detected_tech': diagnostic.detected_legacy_tech, 'thesis': thesis.dict(), 'has_opportunity': diagnostic.is_highly_legacy}
