...
class MockIntelProvider(IntelProvider):
    ...
    def __init__(self):
        ...
        self.mock_competitors = {'technology': ['OpenAI', 'Anthropic', 'Google', 'Microsoft', 'Meta'], 'finance': ['Stripe', 'Square', 'PayPal', 'Adyen', 'Braintree'], 'healthcare': ['Tempus', 'Flatiron', 'Verily', 'IBM Watson', 'Philips'], 'retail': ['Amazon', 'Shopify', 'BigCommerce', 'Magento', 'WooCommerce'], 'biotech': ['Ginkgo Bioworks', 'Recursion', 'Twist Bioscience', 'Benchling'], 'agritech': ['Indigo Ag', 'Farmers Business Network', 'John Deere AI', 'Blue River']}
        self.mock_moves = {'OpenAI': [CompetitorMove(competitor_name='OpenAI', recent_launch='GPT-4 Turbo with 128K context', source_url='https://openai.com/blog', date='2 months ago'), CompetitorMove(competitor_name='OpenAI', recent_launch='Assistants API for agent building', source_url='https://openai.com/blog', date='1 month ago')], 'Anthropic': [CompetitorMove(competitor_name='Anthropic', recent_launch='Claude 3 with improved reasoning', source_url='https://anthropic.com', date='3 months ago')], 'Google': [CompetitorMove(competitor_name='Google', recent_launch='Gemini Pro with multimodal capabilities', source_url='https://deepmind.google', date='2 months ago')], 'Meta': [CompetitorMove(competitor_name='Meta', recent_launch='Llama 3 open source model', source_url='https://ai.meta.com', date='1 month ago')], 'Microsoft': [CompetitorMove(competitor_name='Microsoft', recent_launch='Copilot Studio for custom AI agents', source_url='https://microsoft.com/ai', date='3 months ago')]}
...
class CompetitorReconAgent:
    ...
    def __post_init__(self):
        ...
        self.skill_feature_map = {'llm': ['GPT', 'LLM', 'language model', 'chatbot', 'assistant'], 'vector search': ['vector search', 'embedding', 'retrieval', 'RAG'], 'computer vision': ['vision', 'image', 'video', 'computer vision'], 'recommendation': ['recommendation', 'personalization', 'ranking'], 'nlp': ['NLP', 'text processing', 'sentiment', 'classification'], 'mlops': ['MLOps', 'deployment', 'monitoring', 'pipeline'], 'agents': ['agent', 'autonomous', 'workflow', 'automation'], 'multimodal': ['multimodal', 'vision-language', 'cross-modal']}
        logger.info('Initialized CompetitorReconAgent')

    def generate_fomo_hook(self, target_company: str, industry: str, candidate_skills: list[str]) -> StrategicHook | None:
            ...
            for move in moves:
                ...
                for skill in skills:
                    ...
                    if skill_lower in move_text:
                        matches.append({'move': move, 'skill': skill, 'relevance': 0.9, 'match_type': 'direct'})
                        continue
                    if skill_lower in self.skill_feature_map:
                        features = self.skill_feature_map[skill_lower]
                        for feature in features:
                            if feature in move_text:
                                matches.append({'move': move, 'skill': skill, 'relevance': 0.7, 'match_type': 'feature'})
                                break
    ...
    def _create_speed_hook(self, move: CompetitorMove, target_company: str, skills: list[str]) -> StrategicHook:
