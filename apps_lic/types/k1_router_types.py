...
class K1Router:
    ...
    def __init__(self, config: dict[str, Any]=None):
        ...
        self.archetype_tokens: dict[str, list[str]] = self.config.get('archetype_tokens', {'C_LEVEL': ['CEO', 'CTO', 'CFO', 'CIO', 'COO', 'PRESIDENT', 'DIRECTOR'], 'EXECUTIVE': ['VP', 'VICE PRESIDENT', 'SENIOR', 'LEAD', 'HEAD', 'MANAGER'], 'SENIOR_TA': ['SENIOR', 'PRINCIPAL', 'STAFF', 'ENGINEER', 'DEVELOPER'], 'RECRUITER': ['RECRUITER', 'TALENT', 'HR', 'SOURCING']})
        self.cxo_precedence_tokens: list[str] = self.config.get('cxo_precedence_tokens', ['CEO', 'CTO', 'CFO', 'CIO', 'COO', 'PRESIDENT', 'FOUNDER', 'CHIEF'])
    ...
    def execute_routing(self, context: dict[str, Any]) -> K1Output:
        ...
        logger.info(f'Gate 4: Archetype = {archetype_result.archetype} (confidence={archetype_result.confidence:.2f}, CXO_precedence={archetype_result.cxo_precedence_triggered})')
        route_result = self._select_route(lifecycle=lifecycle, premium_available=premium_available, route_override=route_override, archetype=archetype_result.archetype)
        ...
        output = K1Output(archetype=archetype_result, route=route_result, entrance_gates_passed=entrance_gates_passed, metadata={'router_id': 'K1Router', 'lifecycle': lifecycle, 'contact_name': contact_name, 'contact_title': contact_title})
    ...
    def _classify_archetype(self, title: str, about: str='') -> ArchetypeClassificationResult:
        ...
        for token in self.cxo_precedence_tokens:
            if token.upper() in combined_text:
                ...
                return ArchetypeClassificationResult(archetype='C_LEVEL', confidence=1.0, matched_tokens=matched_tokens, cxo_precedence_triggered=True, manual_override_required=False)
        for token in self.archetype_tokens.get('C_LEVEL', []):
            if token.upper() in combined_text:
                matched_tokens.append(token)
        if matched_tokens:
            return ArchetypeClassificationResult(archetype='C_LEVEL', confidence=0.95, matched_tokens=matched_tokens, cxo_precedence_triggered=False, manual_override_required=False)
        matched_tokens = []
        for token in self.archetype_tokens.get('EXECUTIVE', []):
            if token.upper() in combined_text:
                matched_tokens.append(token)
        if matched_tokens:
            return ArchetypeClassificationResult(archetype='EXECUTIVE', confidence=0.9, matched_tokens=matched_tokens, cxo_precedence_triggered=False, manual_override_required=False)
        matched_tokens = []
        for token in self.archetype_tokens.get('SENIOR_TA', []):
            if token.upper() in combined_text:
                matched_tokens.append(token)
        if matched_tokens:
            return ArchetypeClassificationResult(archetype='SENIOR_TA', confidence=0.9, matched_tokens=matched_tokens, cxo_precedence_triggered=False, manual_override_required=False)
        matched_tokens = []
        for token in self.archetype_tokens.get('RECRUITER', []):
            if token.upper() in combined_text:
                matched_tokens.append(token)
        if matched_tokens:
            return ArchetypeClassificationResult(archetype='RECRUITER', confidence=0.85, matched_tokens=matched_tokens, cxo_precedence_triggered=False, manual_override_required=False)
        logger.warning('No archetype tokens matched - defaulting to EXECUTIVE')
        return ArchetypeClassificationResult(archetype='EXECUTIVE', confidence=0.5, matched_tokens=[], cxo_precedence_triggered=False, manual_override_required=True)

    def _select_route(self, lifecycle: str, premium_available: bool, route_override: str | None, archetype: str) -> RouteSelectionResult:
        ...
        if route_override:
            ...
            if selected_route == 'INMAIL' and (not premium_available):
                return RouteSelectionResult(route=selected_route, premium_available=premium_available, premium_routing_mismatch=True, blocking_reason='INMAIL route selected but Premium InMail not available. Operator response to Gate 3A conflicts with route selection.')
            return RouteSelectionResult(route=selected_route, premium_available=premium_available, premium_routing_mismatch=False)
