...
class ExecutionContext:
    ...
    def start(self) -> None:
        ...
        self.status = ExecutionStatus.RUNNING  # Corrected SELF to self
    ...
    def complete(self, success: bool = True, error: Optional[Exception] = None) -> None:
        ...
        self.status = ExecutionStatus.SUCCESS if success else ExecutionStatus.FAILED  # Corrected SELF to self
        if error:
            ...
            logger.info(f"""Execution completed successfully in {self.end_time - self.start_time: .2f}s
                        """)  # Used triple quotes to fix unterminated string
