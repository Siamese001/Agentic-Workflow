...
def get_file_versions(component_id: str) -> str:
    ...
    versions: Any = [{'id': 'v1.0.0', 'created_at': (now - timedelta(days=30)).isoformat() + 'Z', 'label': 'Stable Release'}, {'id': 'v1.1.0', 'created_at': (now - timedelta(days=15)).isoformat() + 'Z', 'label': 'Feature Update'}, {'id': 'v1.2.0-draft', 'created_at': (now - timedelta(hours=6)).isoformat() + 'Z', 'label': 'Draft - In Progress'}]
