...
async def main():
    ...
    if not all([sender_profile, recipient_profile, job_description]):
        print("FATAL: mission_input.json is missing one or more required top-level keys: 'sender_profile', 'recipient_profile', 'job_description'")
        sys.exit(1)
    mission = OutreachMission(mission_id=str(uuid4()), sender_profile=sender_profile, recipient_profile=recipient_profile, job_description=job_description, connection_status=recipient_profile.get('connection_status', 'not_connected'), prior_message_count=recipient_profile.get('prior_message_count', 0))
    print(f"\n{'=' * 80}")
