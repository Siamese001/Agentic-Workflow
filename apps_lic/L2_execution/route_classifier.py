"""Route Classifier Agent - Routing & Archetype Detection (K.1)

This agent classifies LinkedIn outreach routes and archetypes with strict precedence rules.
Enforces CXO precedence and premium gate validation.

Layer: L2_execution
Responsibilities:
- Classify route (INMAIL, CONNECTION_REQ, etc.)
- Detect archetype (C_LEVEL, RECRUITER, etc.)
- Enforce CXO precedence for executive titles
- Block non-premium InMails
...
class RouteClassifier:
    ...
    CXO_TITLE_PATTERNS = [
        'Chief', 'CEO', 'CFO', 'CTO', 'COO', 'CMO', 'CISO', 'CIO', 'CPO',
        'Head of', 'VP', 'Vice President', 'SVP', 'EVP'
    ]
    
    RECRUITER_TITLE_PATTERNS = [
        'Recruiter', 'Talent Acquisition', 'Talent Partner', 'Hiring Manager',
        'HR', 'Human Resources', 'People Operations', 'Staffing'
    ]
    
    def __init__(
        self,
        config: Optional[RouteClassifierConfig] = None,
    ...
    def classify(
        ...
        if not premium_gate_result.passed:
            ...
            override_result = ValidationResult(
                gate_id='VG_ROUTE_OVERRIDE',
                passed=True,
                severity='INFO',
                message=f"Route overridden to CONNECTION_REQ due to premium gate failure",
                details={'original_route': route.value, 'new_route': 'CONNECTION_REQ'}
            )
        ...
        return ClassificationResult(
            route=route,
            archetype=archetype,
            confidence=0.95 if archetype != ArchetypeType.UNKNOWN else 0.5,
    ...
    def _validate_premium_gate(
        ...
        if route == RouteType.INMAIL and not is_premium:
            return ValidationResult(
                gate_id='VG_PREMIUM_GATE',
                passed=False,
                severity='BLOCK',
                message="BLOCKED: INMAIL route requires premium account",
                details={'route': route.value, 'is_premium': is_premium}
            )
        ...
        return ValidationResult(
            ...
            passed=True,
            severity='INFO',
            message=f"Premium gate passed for route {route.value}",
            signature=f"PREMIUM:OK:{route.value}"
        )
    
    def _validate_cxo_precedence(
        self,
        title: str,
        archetype: ArchetypeType
    ) -> ValidationResult:
        ...
        if has_cxo_pattern:
            if archetype not in [ArchetypeType.C_LEVEL, ArchetypeType.VP_LEVEL]:
                return ValidationResult(
                    gate_id='VG_CXO_PRECEDENCE',
                    passed=False,
                    severity='BLOCK',
                    message=f"BLOCKED: CXO title '{title}' mapped to non-executive archetype {archetype.value}",
                    details={'title': title, 'archetype': archetype.value}
                )
        ...
        return ValidationResult(
            ...
            passed=True,
            severity='INFO',
            message="CXO precedence rule satisfied",
            signature=f"CXO:OK:{archetype.value}"
        )
