"""Message Assembler Agent - Final QA & Assembly (K.7)

This agent performs final message assembly with 4-block QA order validation.
Enforces signature immutability and exact QA block ordering.

Layer: L2_execution
Responsibilities:
- Assemble final message from body + CTA
- Execute final signature immutability check
...
{contact}"""
    ...
    QA_BLOCK_ORDER = [
        QABlockType.LINKEDIN_QA_GRID,
        QABlockType.AI_FILTER_CANONICAL,
        QABlockType.MESSAGE_SPECIFIC_RAG_QA,
        QABlockType.EVIDENCE_PACK
    ]
    
    def __init__(
        self,
        config: Optional[MessageAssemblerConfig] = None,
    ...
    def assemble_final_message(
        self,
        message_body: str,
        cta: str,
        qa_data: Dict[str, Any],
        sender_info: Dict[str, str]
    ) -> MessageAssemblerResult:
        """
        ...
        final_message = self._assemble_message(
        ...
        metadata = {
            ...
            'qa_blocks_count': len(qa_blocks),
            'signature_lines': len(signature.split('\n'))
        }
    ...
    def _generate_signature(self, sender_info: Dict[str, str]) -> str:
        ...
        return self.CANONICAL_SIGNATURE_TEMPLATE.format(
            name=sender_info.get('name', 'John Doe'),
            title=sender_info.get('title', 'Technology Executive'),
            contact=sender_info.get('contact', 'john.doe@email.com | (555) 123-4567')
        )
    
    def _validate_signature_immutability(
        self,
        signature: str,
        sender_info: Dict[str, str]
    ) -> ValidationResult:
        ...
        if len(lines) != self.config.canonical_signature_lines:
            return ValidationResult(
                gate_id='VG_SIGNATURE_IMMUTABILITY',
                passed=False,
                severity='BLOCK',
                message=f"BLOCKED: Signature has {len(lines)} lines (expected {self.config.canonical_signature_lines})",
                details={'line_count': len(lines), 'expected': self.config.canonical_signature_lines}
            )
        ...
        if not lines[0].startswith('Best regards'):
            return ValidationResult(
                ...
                passed=False,
                severity='BLOCK',
                message="BLOCKED: Signature line 1 must start with 'Best regards'",
                details={'actual_line_1': lines[0]}
            )
        ...
        return ValidationResult(
            ...
            passed=True,
            severity='INFO',
            message="Signature immutability validated - canonical 4-line format",
            signature=f"SIG:OK:4LINES"
        )
    ...
    def _generate_qa_blocks(self, qa_data: Dict[str, Any]) -> List[QABlock]:
        ...
        linkedin_qa = QABlock(
            block_type=QABlockType.LINKEDIN_QA_GRID,
            title="LinkedIn QA Grid",
            content=self._format_linkedin_qa_grid(qa_data.get('linkedin_qa', {})),
            order=1
        )
        qa_blocks.append(linkedin_qa)
        
        ai_filter = QABlock(
            block_type=QABlockType.AI_FILTER_CANONICAL,
            title="AI Filter Canonical",
            content=self._format_ai_filter(qa_data.get('ai_filter', {})),
            order=2
        )
        qa_blocks.append(ai_filter)
        
        rag_qa = QABlock(
            block_type=QABlockType.MESSAGE_SPECIFIC_RAG_QA,
            title="Message-Specific RAG QA Table",
            content=self._format_rag_qa_table(qa_data.get('rag_qa', {})),
            order=3
        )
        qa_blocks.append(rag_qa)
        
        evidence = QABlock(
            block_type=QABlockType.EVIDENCE_PACK,
            title="Evidence Pack",
            content=self._format_evidence_pack(qa_data.get('evidence', {})),
            order=4
        )
    ...
    def _validate_qa_block_order(self, qa_blocks: List[QABlock]) -> ValidationResult:
        ...
        if len(qa_blocks) != self.config.required_qa_blocks:
            return ValidationResult(
                gate_id='VG_QA_BLOCK_ORDER',
                passed=False,
                severity='BLOCK',
                message=f"BLOCKED: Expected {self.config.required_qa_blocks} QA blocks, got {len(qa_blocks)}",
                details={'expected': self.config.required_qa_blocks, 'actual': len(qa_blocks)}
            )
        ...
        for i, (block, expected_type) in enumerate(zip(qa_blocks, self.QA_BLOCK_ORDER)):
            if block.block_type != expected_type:
                return ValidationResult(
                    ...
                    passed=False,
                    severity='BLOCK',
                    message=f"BLOCKED: QA block {i+1} is {block.block_type.value}, expected {expected_type.value}",
                    details={
                        'position': i + 1,
                        'actual': block.block_type.value,
            ...
            if block.order != i + 1:
                return ValidationResult(
                    ...
                    passed=False,
                    severity='BLOCK',
                    message=f"BLOCKED: QA block order mismatch at position {i+1}",
                    details={'position': i + 1, 'block_order': block.order}
                )
        ...
        return ValidationResult(
            ...
            passed=True,
            severity='INFO',
            message="QA block order validated - exact sequence maintained",
            signature=f"QAORDER:OK:4BLOCKS",
            details={'block_sequence': [b.block_type.value for b in qa_blocks]}
        )
