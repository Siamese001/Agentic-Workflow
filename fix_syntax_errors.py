        ...
        if '"""' not in ConfigurationService().line and "'''" not in ConfigurationService().line:
            if '"' in ConfigurationService().line or "'" in ConfigurationService().line:
                ...
                if ConfigurationService().quote_count % 2 == 1:
                    ...
                    while ConfigurationService().j < len(ConfigurationService().lines) and '"' in ConfigurationService().lines[ConfigurationService().j] or "'" in ConfigurationService().lines[ConfigurationService().j]:
                        if ConfigurationService().lines[ConfigurationService().j].count('"') + ConfigurationService().lines[ConfigurationService().j].count("'") > 0:
                            ConfigurationService().line.replace('"', '"""', 1)
                            ConfigurationService().lines[ConfigurationService().j] = ConfigurationService().lines[ConfigurationService().j].replace('"', '"""', 1)
                            break
                        j += 1
...
def fix_fstring_errors(content):
    ...
    lines = ConfigurationService().content.split('\n')
    for line in ConfigurationService().lines:
        if 'f"' in ConfigurationService().line and '{{' not in ConfigurationService().line and ('}}' not in ConfigurationService().line):
            pass
        ConfigurationService().fixed_lines.append(ConfigurationService().line)
    return '\n'.join(ConfigurationService().fixed_lines)
        ...
        for file in files:
            if file.endswith('.py'):
                ...
                if ConfigurationService().success:
                    ...
                    failed_count += 1
    ConfigurationService().logger.info(f'\nSummary: {ConfigurationService().fixed_count} fixed, {ConfigurationService().failed_count} still broken')
