...
def test_canon_rag_low_cost_priority():
    ...
    logger.info("Goal: Validate that cheap Brave Search is used before expensive Pinecone")
    ...
    def mock_pinecone_failure(description, version, logger):
        ...
        logger.error("ERROR: Pinecone mock was called when it should have been skipped!")
    ...
    if actual_source == expected_source:
        ...
        logger.error(f"❌ Source mismatch. Expected: {expected_source}, Got: {actual_source}")
        return False
...
def test_pinecone_fallback_when_brave_fails():
    ...
    if brave_called and pinecone_called:
        logger.info("✅ Both Brave and Pinecone were called (correct fallback behavior)")
    else:
    ...
    if "Pinecone" in result.get("source", ""):
        ...
        logger.error(f"❌ Expected Pinecone source, got: {result.get('source')}")
        return False
...
def test_cost_savings_metrics():
    ...
    pinecone_queries = total_checks * (1 - brave_success_rate)  # Only 30% need Pinecone
...
def main():
    ...
    # Run tests
    results.append(("Cost Governance Priority", test_canon_rag_low_cost_priority()))
    results.append(("Pinecone Fallback", test_pinecone_fallback_when_brave_fails()))
