...
def find_duplicates():
    ...
    for class_name, paths in duplicates.items():
        ...
        has_backup = any(
            ".sovereign_healing_backup" in str(p)
            or "hygiene_surgery" in str(p)
            or ".backup" in str(p)
            for p in paths
        )
    ...
    if backup_duplicates:
        ...
        for class_name, paths in sorted(backup_duplicates.items()):
            ...
            for path in sorted(paths):
                is_backup = ".sovereign_healing_backup" in str(path) or "hygiene_surgery" in str(
                    path
                )
    ...
    if true_duplicates:
        ...
        for class_name, paths in sorted(true_duplicates.items()):
            ...
            for path in paths:
                ...
                if "L2_execution" in str(path) and "L2" in class_name:
                    canonical_candidates.append(path)
                elif "L3_orchestration" in str(path) and "L3" in class_name:
                    canonical_candidates.append(path)
                elif "L4_state" in str(path) and "L4" in class_name:
                    canonical_candidates.append(path)
                elif "L5_safety" in str(path) and "L5" in class_name:
                    canonical_candidates.append(path)
                elif "L6_observability" in str(path) and "L6" in class_name:
                    canonical_candidates.append(path)
