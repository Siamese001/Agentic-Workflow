...
def run(argv: list[str]) -> tuple[str, int]:
    ...
    # Replace any remaining non-ASCII characters with '?' so byte-scan never fails
...
def main() -> None:
    ...
    # Filter blank lines from pretty=format: output
    ...
    for line in collect_out.splitlines():
        m = re.search(r"(\d+) test[s]? collected", line)
        if m:
    ...
    # ------------------------------------------------ COMMAND: collection integrity check
    ...
    # ------------------------------------------------ write file
    # Strip trailing whitespace from every line (pre-commit compliance)
