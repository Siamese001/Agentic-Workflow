...
# ---------------------------------------------------------------------------
# Persistence (SQLite — stores serialised graph + metadata)
# ---------------------------------------------------------------------------
...
def _load() -> tuple[nx.DiGraph, dict[str, str], list] | None:
    ...
    if row is None:
        return None
    g = pickle.loads(row[0])  # noqa: S301
...
class DepGraph:
    ...
    def fan_in_top(self, n: int = 20) -> list[tuple[str, int]]:
        ...
        return sorted(
            ((node, self._g.in_degree(node)) for node in self._g.nodes),
            key=lambda x: x[1],
            reverse=True,
        )[:n]
    ...
    def fan_out_top(self, n: int = 15) -> list[tuple[str, int]]:
        ...
        return sorted(
            ((node, self._g.out_degree(node)) for node in self._g.nodes),
            key=lambda x: x[1],
            reverse=True,
        )[:n]
