...
_log = logging.getLogger("adg_mcp.debug")
...
def main() -> int:
    """Initialize and run the debug MCP server over stdio."""
    try:
        ...
        _init_service()
    except Exception as exc:  # guardian: allow-broad-exception -- debug launcher must preserve a durable traceback instead of failing silently
        _log.exception("FATAL during debug launcher startup: %s", exc)
        return 1
