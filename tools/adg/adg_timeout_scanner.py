...
class TimeoutViolation:
    ...
    def __init__(self, file_path: str, line_number: int, symbol: str, 
                 violation_type: str, context: str = ""):
    ...
    def __repr__(self):
        return f"TimeoutViolation({self.file_path}:{self.line_number} - {self.symbol} [{self.violation_type}])"
...
class ADGTimeoutScanner:
    ...
    def scan_subprocess_calls(self, directories: List[str] | None = None) -> List[TimeoutViolation]:
        ...
        try:
            ...
            warnings.warn(f"ADG scan failed, falling back to regex: {e}")
    ...
    def _analyze_subprocess_call(self, call: FileMatch) -> TimeoutViolation | None:
        ...
        try:
            ...
            warnings.warn(f"Failed to analyze call {call.file_path}:{call.line_number}: {e}")
                        ...
                        for pattern in subprocess_patterns:
                            ...
                            for match in matches:
                                ...
                                if "timeout=" not in call_text:
                                    ...
                                    violations.append(TimeoutViolation(
    ...
    def scan_while_true_loops(self, directories: List[str] | None = None) -> List[TimeoutViolation]:
        ...
        try:
            ...
            warnings.warn(f"ADG loop scan failed, falling back to regex: {e}")
            violations = self._fallback_loop_scan(directories)
        
        return violations
    
    def _analyze_loop_for_timeout(self, loop: FileMatch) -> TimeoutViolation | None:
        ...
        try:
            ...
            if loop.line_number and loop.line_number <= len(lines):
                ...
                context_start = max(0, loop.line_number - 50)
                context_end = min(len(lines), loop.line_number + 10)
            ...
            warnings.warn(f"Failed to analyze loop {loop.file_path}:{loop.line_number}: {e}")
...
def main():
    ...
    parser.add_argument("--directory", action="append", help="Directories to scan (can be used multiple times)")
    ...
    if args.format == "json":
        ...
        print(f"ADG Timeout Scanner Results")
        print(f"=========================")
