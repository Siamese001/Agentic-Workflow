#!/usr/bin/env python3
"""
Capability Extraction Engine - Phase 3 Implementation
...
class CapabilityExtractor:
    ...
    def extract_capability(self, candidate: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            log_entry = {
                ...
                "functions_extracted": len([f for f in candidate["functions"] if not f["is_private"]]),
                "classes_extracted": len([c for c in candidate["classes"] if not c["is_private"]]),
                "status": "extracted"
            }
    ...
    def _determine_primary_capability(self, candidate: Dict[str, Any]) -> str:
        ...
        pattern_counts = {pattern: len(funcs) for pattern, funcs in candidate["capability_patterns"].items()}
