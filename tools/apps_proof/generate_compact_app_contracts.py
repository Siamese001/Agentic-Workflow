...
APPS_SPEC: Dict[str, Dict[str, Any]] = {
    ...
    "apps_rfp": {
        "domain": "rfp_response_generation",
        "task_class": "rfp_response",
        "task_class_kind": "generation",
        "task_description": "Generate a structured RFP response from RFP doc + company KB + past wins library.",
        "risk_tier": "elevated",
        "output_type": "structured_record",
        "side_effect_class": "read_only",
        "freshness_class": "bounded",
        "orchestration_kind": "dag",
        "hitl_required": False,
        "dimensions": [
            ("requirement_coverage", "Every RFP requirement addressed.", 0.25, "deterministic", 0.95, True, True),
            ("factual_grounding", "Claims cite company KB or past wins.", 0.20, "deterministic", 0.95, True, True),
            ("compliance_with_rfp_constraints", "Page limits, format, required sections per RFP.", 0.15, "deterministic", 0.95, False, True),
            ("competitive_positioning", "Clear differentiation vs common competitors.", 0.10, "hybrid", 0.55, False, False),
            ("win_theme_alignment", "Response aligns with identified win themes.", 0.10, "llm_as_judge", 0.55, False, False),
            ("no_fabricated_credentials", "Every claimed capability/cert is in company KB.", 0.10, "deterministic", 0.99, True, True),
            ("pricing_integrity", "No unauthorized pricing commitments.", 0.10, "deterministic", 0.99, True, True),
        ],
        "negatives": [
            ("fabricated_capability", "no_fabricated_credentials", "Response claims a certification not in company KB."),
            ("missed_requirement", "requirement_coverage", "RFP has N requirements; response only addresses N-1."),
        ],
        "forbidden_inputs": ["competitor_rfp_response_text"],
        "prohibited_outputs": ["unauthorized_pricing_commitment", "fabricated_capability_claim", "compliance_violation"],
    },
    "apps_qna": {
