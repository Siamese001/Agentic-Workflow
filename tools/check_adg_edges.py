...
P0_RELS = [
    "emits_replay_key", "emits_determinism_digest", "applies_guardrail",
    "snapshots_state", "signs_execution_trace", "records_execution_trace",
]
cur.execute(
    "SELECT relation_type, symbol, line_no FROM edges WHERE source_file = ?",
    (target,)
)
rows = cur.fetchall()
