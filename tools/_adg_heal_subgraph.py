...
# Node compact format: n=adg_name, p=path, l=layer, c=criticality, t=type, k=kind
nodes = fg["nodes"]  # dict: str(id) -> {n, p, l, c, t, k}
edges = fg["edges"]  # list of {s, d, r, k, f, ln, sym}

# Normalise: edges use integer src/dst ids, nodes dict is keyed by str
# Build int->node lookup
id_to_node = {int(k): v for k, v in nodes.items()}

print(f"Nodes: {len(nodes)}  Edges: {len(edges)}")

# Healing/routing module patterns to include (matched against 'n' field)
HEAL_PATTERNS = [
    "healing_tier_router",
...
# Intra-subgraph edges (imports/calls between heal nodes)
...
# External callers of the healing dispatcher/router
...
# What does execute_ssot import from L2 healing?
...
# Summary of edge kinds touching heal subgraph
