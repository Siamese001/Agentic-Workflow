...
dispatcher_ids = {int(k) for k, v in nodes.items() if 'healing_tier_dispatcher' in (v.get('n') or '').lower() or 'healing_tier_router' in (v.get('n') or '').lower()}
for e in edges:
...
l2_heal_ids = {int(k) for k, v in nodes.items() if 'L2_execution/healers' in (v.get('n') or '') or 'L2_execution/healers' in (v.get('p') or '')}
for e in edges:
...
viols = [e for e in edges if e.get('k', '') in ('GV_violates', 'violation') and (e['s'] in heal_ids or e['d'] in heal_ids)]
for e in viols:
    sn = id_to_node.get(e['s'], {}).get('p') or id_to_node.get(e['s'], {}).get('n', '?')
    dn = id_to_node.get(e['d'], {}).get('p') or id_to_node.get(e['d'], {}).get('n', '?')
