...
class MCPSmokeTester:
        ...
        try:
            ...
            try:
                import_result = subprocess.run(
                    ['python', '-c', f'import sys; sys.path.insert(0, r"{script_dir}"); import {module_name}; print("Import successful")'],
                    capture_output=True,
                    text=True,
                    timeout=10,
                    cwd=cwd
    ...
    def generate_smoke_test_report(self):
        ...
        for status, count in status_counts.items():
            icon = '✅' if status in ['success', 'timeout_success', 'python_ready', 'available'] else '⚠️' if status in ['warning', 'skipped'] else '❌'
...
def main():
    ...
    critical_issues = len([r for r in results.values() if r['status'] in ['missing', 'syntax_error', 'import_error', 'error']])
