...
cursor.execute('SELECT COUNT(*) FROM edges WHERE relation_type = "records_execution_trace" AND src_id IN (SELECT id FROM nodes WHERE adg_name LIKE "%test%")')
test_execution_count = cursor.fetchone()[0]
print(f'Test execution traces: {test_execution_count}')

# Check what layer tests belong to
cursor.execute('SELECT DISTINCT to_name FROM edges WHERE relation_type = "belongs_to_layer" AND src_id IN (SELECT id FROM nodes WHERE adg_name LIKE "%test%" LIMIT 1)')
