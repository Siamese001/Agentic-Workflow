...
QUERIES: list[tuple[str, str, str]] = [
    ("How does Late Chunking pool token embeddings into chunk vectors?",
     "late_chunking.py", "semantic"),
    ...
    ("SeniorLibrarianReranker relevance coverage authority",
     "senior_librarian_reranker.py", "exact"),
...
def main() -> int:
    ...
    for query, expected_file, category in QUERIES:
        relevant = [
            ...
            if expected_file in (m or {}).get("source_path", "")
        ]
        if not relevant:
