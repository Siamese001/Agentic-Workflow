...
class MCPTester:
    ...
    def test_node_server(self, name, config):
        ...
        try:
            ...
            if result.returncode == 0 or 'timeout' in result.stderr.lower() or 'server' in result.stderr.lower():
                return {
                    'status': 'success',
                    'time': startup_time,
    ...
    def get_performance_metrics(self):
        ...
        node_servers = [(name, r) for name, r in self.results.items()
                       if r.get('status') in ['success', 'timeout_success']]
        python_servers = [(name, r) for name, r in self.results.items()
                         if r.get('status') == 'python_ready']

        metrics = {
            'total_servers': len(self.results),
            'working_servers': len([r for r in self.results.values() if r.get('status') not in ['skipped', 'error', 'missing']]),
...
def main():
    ...
    success_rate = metrics['working_servers'] / metrics['total_servers'] if metrics['total_servers'] > 0 else 0
