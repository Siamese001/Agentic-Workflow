...
OUT_REPORT = Path(f"docs/reports/sc1_subtype_triage_{datetime.now(UTC):%Y%m%d}.md")
...
def main() -> int:
    ...
    lines.append("")
    try:
        cur.execute("SELECT name FROM sqlite_master WHERE name='mv_hotspot_centrality' AND type='table'")
        if cur.fetchone():
            ...
            for mod in top_mods:
                try:
                    col = (
                        "file_path"
                        if "file_path" in hc_cols
                        else (
                            "module"
                            if "module" in hc_cols
                            else ("adg_name" if "adg_name" in hc_cols else None)
                        )
                    )
                    if col:
                        cur.execute(
    ...
    lines.append("")
    lines.append(
        f"_Generated by `tools/debug/_sc1_subtype_classifier.py` on {datetime.now(UTC).isoformat()}._"
    )
