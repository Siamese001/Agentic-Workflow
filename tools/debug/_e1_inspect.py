...
cur.execute(
    "SELECT name FROM sqlite_master WHERE type='view' AND (name LIKE '%duplic%' OR name LIKE '%adapter%')"
)
...
for pat in ["%redis%client%", "%chroma%", "%sqlite%client%", "%sqlite%adapter%", "%redis%adapter%"]:
    cur.execute(
        "SELECT file_path, layer, adg_name FROM nodes WHERE file_path LIKE ? AND file_path NOT LIKE '%archive%' AND file_path NOT LIKE '%__pycache__%' GROUP BY file_path ORDER BY file_path",
        (pat,),
    )
    rows = cur.fetchall()
    if rows:
