...
print(f"snap: {p.name}\n")
for mv in [
    "mv_l2_phase_coverage", "mv_exit_disposition_coverage",
    "mv_trace_replay_eval_gaps", "mv_eval_coverage_by_path",
    "mv_handoff_witness_tiers", "mv_cross_cutting_witness_tiers",
    "mv_graph_reverse_dependency_hotspots", "mv_hotspot_centrality",
]:
...
for r in c.execute("SELECT name FROM sqlite_master WHERE type IN ('table','view')"):
    n = r[0]
    if "antipattern" in n.lower() or "defect" in n.lower() or "violation" in n.lower() or "burndown" in n.lower():
