...
AGENTS = [
    ...
    ("LocationHealerAgent", "agentic_core.L5_safety.reasoning.LocationHealerAgent", "UnifiedAgent"),
    ("GovernanceAgent", "agentic_core.L5_safety.reasoning.GovernanceAgent", "direct-path"),
]
...
for class_name, mod_path, util_path in AGENTS:
    ...
    mod_re = re.compile(rf"(?:from\s+{re.escape(mod_path)}\s+import|import\s+{re.escape(mod_path)}\b)")
    results = []
