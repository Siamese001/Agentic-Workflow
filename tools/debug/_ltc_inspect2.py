...
print("symbols in module:",
    c.execute("SELECT COUNT(*) FROM nodes WHERE adg_name LIKE ?",
              (like,)).fetchone())

print("fan-in to all those symbols:",
    c.execute("SELECT COUNT(*) FROM edges WHERE dst_id IN "
              "(SELECT id FROM nodes WHERE adg_name LIKE ?)",
              (like,)).fetchone())
...
for r in c.execute(
    ...
    "GROUP BY e.relation_type ORDER BY 2 DESC", (like,)):
    print(r)

print("\ndistinct caller files:",
    c.execute("SELECT COUNT(DISTINCT n.resolved_path) FROM edges e "
...
for r in c.execute(
    ...
    "JOIN nodes n ON n.id=e.src_id "
    "WHERE e.dst_id IN (SELECT id FROM nodes WHERE adg_name LIKE ?) "
    "GROUP BY n.resolved_path ORDER BY c DESC LIMIT 10", (like,)):
    print(r)
...
for r in c.execute(
    ...
    "GROUP BY n.adg_name ORDER BY c DESC LIMIT 10", (like,)):
    print(r)
