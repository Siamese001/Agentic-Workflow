...
def _run_pre(payload: dict) -> subprocess.CompletedProcess:
    script = repo / ".claude" / "governance" / "scripts" / "pre_mcp_gate.py"
    env = os.environ.copy()
...
def _load_post_module():
    spec = importlib.util.spec_from_file_location(
        ...
        str(repo / ".claude" / "governance" / "scripts" / "post_agent_mcp_preflight_audit.py"),
    )
...
spec = importlib.util.spec_from_file_location(
    "pre_mcp_gate", str(repo / ".claude" / "governance" / "scripts" / "pre_mcp_gate.py")
)
