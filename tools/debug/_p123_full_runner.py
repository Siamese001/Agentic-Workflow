...
def run_script(script: str) -> dict:
    full = ROOT / script
    if not full.exists():
        return {"status": "missing", "exit": None, "gross": None, "net": None,
                "exempt": None, "note": "script not found"}
    ...
    return {"status": "passed" if result.returncode == 0 else "failed",
            "exit": result.returncode, "output": preserved}
...
for tier, gate_id, modname, cls in CLASS_GATES:
    try:
        ...
        print(f"{tier:<5}{gate_id:<24}{res.status:<10}{'—':>8}{net:>8}{'—':>8}  {','.join(GateCls.source_views)[:40]}")
        ...
        print(f"{tier:<5}{gate_id:<24}{'ERROR':<10}{'—':>8}{'—':>8}{'—':>8}  {type(e).__name__}: {str(e)[:40]}")
...
for tier, gate_id, script in SCRIPT_GATES:
    ...
    if info["status"] == "missing":
        all_results.append({"tier": tier, "gate_id": gate_id, "status": "missing",
                            "script": script})
    ...
    row = {"tier": tier, "gate_id": gate_id, "status": info["status"],
           "exit": info["exit"], **counts, "script": script,
           "last_line": last_line}
    ...
    print(f"{tier:<5}{gate_id:<24}{info['status']:<10}{info['exit']:>5}{str(g):>8}{str(n):>8}{str(b):>8}  {last_line[:50]}")
