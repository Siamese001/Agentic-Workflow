...
class TestSequentialThinkingDominance:
    ...
    def test_sequential_thinking_always_first(self):
        ...
        assert boosted[0]["name"] == "sequential_thinking", "CRITICAL FAIL: Sequential thinking is not first!"

        # Chat tools MUST be at the end
        chat_positions = [
            i for i, t in enumerate(boosted) if any(pat in t["name"] for pat in SUPPRESSED_TOOLS)
        ]
        other_positions = [
            i for i, t in enumerate(boosted) if not any(pat in t["name"] for pat in SUPPRESSED_TOOLS)
        ]
...
class TestEnvironmentVariables:
    ...
    def test_cascade_chat_fallback_disabled(self, monkeypatch):
        ...
        assert fallback == "disabled", f"CRITICAL: CASCADE_CHAT_FALLBACK is '{fallback}', must be 'disabled'!"
    ...
    def test_sequential_thinking_priority_absolute(self, monkeypatch):
        ...
        assert priority == "0", f"CRITICAL: Priority is '{priority}', must be '0' for absolute dominance!"
    ...
    def test_kimi_k2_5_dominance_enabled(self, monkeypatch):
        ...
        assert dominance == "enabled", f"CRITICAL: KIMI_K2_5_DOMINANCE is '{dominance}', must be 'enabled'!"
    ...
    def test_reasoning_mode_sequential_only(self, monkeypatch):
        ...
        assert mode == "sequential-only", f"CRITICAL: Reasoning mode is '{mode}', must be 'sequential-only'!"
...
class TestHardenedConfiguration:
    ...
    def test_max_thoughts_increased(self, monkeypatch):
        ...
        assert max_thoughts == "25", f"Max thoughts is '{max_thoughts}', expected '25' for hardened config!"
    ...
    def test_token_budget_increased(self, monkeypatch):
        ...
        assert budget == "50000", f"Token budget is '{budget}', expected '50000' for hardened config!"
    ...
    def test_aggressive_mode_enabled(self, monkeypatch):
        ...
        assert mode == "enabled", f"Aggressive mode is '{mode}', must be 'enabled'!"
    ...
    def test_suppress_chat_on_planning(self, monkeypatch):
        ...
        assert suppress == "true", f"Chat suppression is '{suppress}', must be 'true'!"
