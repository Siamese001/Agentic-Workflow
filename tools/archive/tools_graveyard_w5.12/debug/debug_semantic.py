...
cursor = conn.execute(
    'SELECT edge_kind, relation_type, semantic_type FROM edges WHERE edge_kind = "reads_runtime_state" LIMIT 5'
)
...
print("\n=== reads_policy_state ===")
cursor = conn.execute(
    'SELECT edge_kind, relation_type, semantic_type FROM edges WHERE edge_kind = "reads_policy_state" LIMIT 5'
)
samples = cursor.fetchall()
for edge_kind, rel_type, sem_type in samples:
    print(f"  edge_kind: {edge_kind}, relation_type: {rel_type}, semantic_type: {sem_type}")
...
print("\n=== layer_membership ===")
cursor = conn.execute(
    'SELECT edge_kind, relation_type, semantic_type FROM edges WHERE edge_kind = "layer_membership" LIMIT 5'
)
samples = cursor.fetchall()
for edge_kind, rel_type, sem_type in samples:
    print(f"  edge_kind: {edge_kind}, relation_type: {rel_type}, semantic_type: {sem_type}")
