...
for mod_path in sorted(prod_paths):
    ...
    if test_dir.exists():
        for f in test_dir.iterdir():
            if f.name.startswith(f"test_{stem}") and f.suffix == ".py" and not f.name.endswith("_adg.py"):
...
redundant = [e for e in overlap_both if e["foundational_asserts"] >= 5]
print(f"\n  Redundant ADG stubs (foundational has >=5 asserts): {len(redundant)}")

# True overlap = both have meaningful depth
both_deep = [e for e in overlap_both if e["adg_asserts"] >= 5 and e["foundational_asserts"] >= 5]
...
for e in sorted(redundant, key=lambda x: -x["foundational_asserts"])[:20]:
    print(
        f"  fan_in={e['fan_in']:>3}  adg={e['adg_asserts']:>3} asserts  "
...
for threshold in [1, 2, 3, 5, 10]:
    ...
    print(
        f"  {threshold:>10}  {len(above):>8}  {100 * len(above) / total:>6.1f}%  {len(have_f):>11}  {gap:>6}"
    )
