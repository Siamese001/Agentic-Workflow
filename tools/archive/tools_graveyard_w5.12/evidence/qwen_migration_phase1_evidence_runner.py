...
def run_cmd(args: list[str]) -> tuple[int, str, str]:
    ...
    result = subprocess.run(
        args, cwd=REPO_ROOT, capture_output=True, text=True, shell=False, encoding="utf-8", errors="replace"
    )
...
def build_evidence(code_commit: str, evidence_commit: str | None) -> list[str]:
    ...
    demo_script = "\n".join(
        [
            ...
            "    enforce_output_cap, VLLMOutputCapExceeded, VLLM_MAX_TOKENS_ABSOLUTE, TaskClass",
            ")",
            "cap = enforce_output_cap(9999, TaskClass.HEALING_JSON_ARTIFACT.value)",
            "print(f'healing_json_artifact: requested=9999, enforced={cap}')",
            "cap2 = enforce_output_cap(9999, TaskClass.PATCH_SUGGESTION.value)",
            "print(f'patch_suggestion: requested=9999, enforced={cap2}')",
            "cap3 = enforce_output_cap(9999, TaskClass.MULTI_FILE_SUMMARY.value)",
            "print(f'multi_file_summary: requested=9999, enforced={cap3}')",
            "print(f'VLLM_MAX_TOKENS_ABSOLUTE={VLLM_MAX_TOKENS_ABSOLUTE}')",
            "try:",
            "    enforce_output_cap(500, 'unknown_class')",
            "except VLLMOutputCapExceeded as e:",
            "    print(f'undefined_class raised VLLMOutputCapExceeded: reason={e.reason}')",
        ]
    )
