...
def run_cmd(argv: list[str]) -> tuple[int, str, str]:
    ...
    r = subprocess.run(
        argv, cwd=str(REPO_ROOT), capture_output=True, encoding="utf-8", errors="replace", shell=False
    )
...
def main() -> None:
    ...
    assertion_lines.append(
        hard_assert(
            len(v_porcelain) == 0, "OK: len(porcelain_stdout) == 0", f"porcelain not empty: {v_porcelain}"
        )
    )
    assertion_lines.append(
        hard_assert(
