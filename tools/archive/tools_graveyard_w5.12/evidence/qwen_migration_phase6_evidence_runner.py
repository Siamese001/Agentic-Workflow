...
def main():
    ...
    seen_targets = set()
    test_targets.append(
        ["python", "-m", "pytest", "-q", "tests/unit_min_deps/test_vllm_replay_with_violations.py"]
    )
...
def execute_proofs():
    ...
    assert result_fail1.telemetry.failure_type == "INVARIANT_VIOLATION", (
        "FAIL: failure_type must be INVARIANT_VIOLATION (exact)"
    )
    assert result_fail1.invariant_violations[0].invariant_id == fail_violation.invariant_id, (
        "FAIL: invariant_id must match"
    )
    ...
    result_tampered = VLLMGatewayCallResult(
        ...
        local_request=None,
        telemetry=result_fail1.telemetry,
    ...
    with patch(
        ...
        try:
            ...
            sys.exit(1)
        except AssertionError:  # guardian: AssertionError should be handled with specific context
