...
def main() -> None:
    ...
    h()
    pytest_cmd = ['python', '-m', 'pytest', '-q', '--color=no', 'tests/architecture/test_wave1_phase1_2_sovereignty.py']
    ...
    h('|------|----------|-------------|-----------|----------|------|')
    rows = [('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'success', 'bare external SDK import', 'flagged', 'test_import_openai_flagged_as_direct_provider'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'success', 'vllm top-level', 'flagged', 'test_import_vllm_flagged_as_direct_provider'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'boundary', 'vllm.submodule prefix match', 'flagged', 'test_import_vllm_submodule_flagged_as_direct_provider'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'negative', 'agentic_core.*.vllm_* internal', 'not flagged', 'test_import_agentic_core_vllm_type_not_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'negative', 'agentic_core.* all internals', 'not flagged', 'test_import_agentic_core_never_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'success', 'anthropic top-level', 'flagged', 'test_import_anthropic_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'success', 'litellm top-level', 'flagged', 'test_import_litellm_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'negative', 'stdlib import', 'not flagged', 'test_import_stdlib_not_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.ImportFrom)', 'success', 'from openai import', 'flagged', 'test_from_openai_import_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.ImportFrom)', 'boundary', 'from vllm import', 'flagged', 'test_from_vllm_import_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.ImportFrom)', 'negative', 'from agentic_core...vllm_types import', 'not flagged', 'test_from_agentic_core_vllm_types_not_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.ImportFrom)', 'negative', 'vllm_infrastructure_fingerprint regression', 'not flagged', 'test_from_agentic_core_vllm_infra_fingerprint_not_flagged'), ('semantic_gap_analyzer.py', 'analyze_file (ast.Import)', 'boundary', 'lazy google.generativeai in function body', 'flagged (AST walks all nodes)', 'test_google_generativeai_lazy_import_still_detected'), ('semantic_gap_analyzer.py', '_detect_upward_imports', 'negative', 'L2 file imports L1 (rank 1 < 2)', 'violation reported', 'test_detect_upward_imports_l2_importing_l1_is_upward'), ('semantic_gap_analyzer.py', '_detect_upward_imports', 'contract', 'L1 imports L0 flagged by lower-rank rule', 'reported', 'test_detect_upward_imports_l1_importing_l0_is_flagged'), ('semantic_gap_analyzer.py', '_detect_upward_imports', 'boundary', 'non-layer file returns empty list', 'empty', 'test_detect_upward_imports_no_layer_returns_empty'), ('semantic_gap_analyzer.py', '_detect_upward_imports', 'boundary', 'empty imported_layer_refs', 'empty', 'test_detect_upward_imports_no_refs_returns_empty'), ('semantic_gap_analyzer.py', '_detect_upward_imports', 'boundary', 'same-layer import (L2->L2)', 'not flagged', 'test_detect_upward_imports_same_layer_not_upward'), ('semantic_gap_analyzer.py', '_detect_upward_imports', 'boundary', 'L2 imports L3 (rank 3 > 2)', 'not flagged', 'test_detect_upward_imports_l2_importing_l3_is_not_flagged'), ('semantic_gap_analyzer.py', 'DIRECT_PROVIDER_IMPORT_PATTERNS', 'contract', 'all patterns are strings', 'non-empty strings', 'test_direct_provider_patterns_are_top_level_package_names'), ('semantic_gap_analyzer.py', 'DIRECT_PROVIDER_IMPORT_PATTERNS', 'invariant', 'no agentic_core in patterns', 'invariant holds', 'test_direct_provider_patterns_does_not_contain_agentic_core'), ('agentic_core (real)', 'L0/L1/L3/L4/L5/L6 files', 'codebase-invariant', 'no provider SDK imports outside L2', 'zero violations', 'test_no_real_provider_imports_outside_l2'), ('agentic_core (real)', 'L2_execution adapter files', 'codebase-success', 'known adapters have correct SDKs', 'healing_provider_adapters + qwen_vllm', 'test_l2_real_provider_imports_are_in_expected_files'), ('agentic_core (real)', 'vllm_* type files', 'regression', '8 false-positive files produce zero flags', 'zero', 'test_internal_vllm_type_modules_produce_no_direct_provider_gap')]
    for row in rows:
    ...
    h('|---------|---------|-------------|----------|-------------|--------------|-----------------|-----------------|')
    h('| ast.Import provider detection | analyze_file AST walk | test_import_openai_flagged, test_import_vllm_flagged, test_import_anthropic_flagged, test_import_litellm_flagged | test_import_vllm_submodule_flagged, test_google_generativeai_lazy_import | test_import_agentic_core_vllm_type_not_flagged, test_import_agentic_core_never_flagged, test_import_stdlib_not_flagged | - | idempotent: same file same result | read-only |')
    h('| ast.ImportFrom provider detection | analyze_file AST walk | test_from_openai_import_flagged, test_from_vllm_import_flagged | test_from_vllm_import_flagged | test_from_agentic_core_vllm_types_not_flagged, test_from_agentic_core_vllm_infra_fingerprint_not_flagged | - | idempotent | read-only |')
    h('| _detect_upward_imports | _detect_upward_imports(path, analysis) | test_detect_upward_imports_l2_importing_l1_is_upward | test_detect_upward_imports_no_layer_returns_empty, test_detect_upward_imports_no_refs_returns_empty, test_detect_upward_imports_same_layer_not_upward, test_detect_upward_imports_l2_importing_l3_is_not_flagged | - | - | test_detect_upward_imports_l1_importing_l0_is_flagged | read-only |')
    h('| Codebase invariants | full L0-L6 rglob scan | test_no_real_provider_imports_outside_l2, test_l2_real_provider_imports_are_in_expected_files | test_internal_vllm_type_modules_produce_no_direct_provider_gap | - | - | deterministic file scan | read-only |')
    h()
    h('## DEFECT_MODEL')
    h()
    h('| Defect Mechanism | Covered By |')
    h('|-----------------|------------|')
    h("| Substring match 'vllm' in internal module path causes false positive | test_import_agentic_core_vllm_type_not_flagged, test_from_agentic_core_vllm_infra_fingerprint_not_flagged, test_internal_vllm_type_modules_produce_no_direct_provider_gap |")
    h('| Missing dotted-prefix match drops google.generativeai detection | test_google_generativeai_lazy_import_still_detected |')
    h('| Guard omission: internal imports escape agentic_core.* guard | test_import_agentic_core_never_flagged, test_from_agentic_core_vllm_types_not_flagged |')
    h('| Off-by-one: same-layer imports wrongly flagged as upward | test_detect_upward_imports_same_layer_not_upward |')
    h('| Hidden fallback: missing layer returns non-empty violation list | test_detect_upward_imports_no_layer_returns_empty |')
    h('| Duplicate mutation: non-L2 provider SDK imports go undetected | test_no_real_provider_imports_outside_l2 |')
    h()
    content = '\n'.join(evidence_lines) + '\n'
