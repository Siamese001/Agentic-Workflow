...
class EvidenceContractV2:
    ...
    def validate_hash_loop_prevention(self, code_commit: str) -> None:
        ...
        if code_commit == current_head:
            raise ValueError(
                f"CODE_COMMIT ({code_commit}) == current HEAD ({current_head}). This would create a hash loop. Use a commit from before the evidence changes."
            )
    ...
    def validate_evidence_contract_structure(
        self, code_commit: str, evidence_commit: str | None = None, require_evidence_commit: bool = False
    ) -> None:
    ...
    def build_evidence_sections(
        self, code_commit: str, evidence_commit: str | None = None, inspected_files: list[str] | None = None
    ) -> dict:
