...
def run_subprocess_command(argv: list[str], cwd: str=None) -> str:
    ...
    result = subprocess.run(argv, shell=False, capture_output=True, text=True, encoding='utf-8', errors='replace', cwd=cwd or os.getcwd())
...
def execute_shadow_router_proofs():
    ...
    route_decision = RouteDecisionArtifact(trace_id='phase9-pass-001', timestamp='2024-01-01T00:00:00Z', route_path=RoutePath.STANDARD_VALIDATION, risk_score=0.3, budget_est=100.0, rationale_enum=RoutingRationale.STANDARD_VALIDATION, policy_config_hash='pass123')
    ...
    assert shadow_decision.observed_route == RoutePath.STANDARD_VALIDATION, 'PASS: Shadow must observe correctly'
    ...
    print('FAIL SCENARIO:')
    # guardian: allow-magic-config
    suboptimal_decision = RouteDecisionArtifact(trace_id='phase9-fail-001', timestamp='2024-01-01T00:00:00Z', route_path=RoutePath.HUMAN_ESCALATION, risk_score=0.1, budget_est=200.0, rationale_enum=RoutingRationale.HUMAN_ESCALATION, policy_config_hash='fail456')
    shadow_decision_fail = classifier.observe_routing_decision(suboptimal_decision)
    drift_detected = shadow_decision_fail.observed_route != shadow_decision_fail.shadow_route and shadow_decision_fail.drift_score > 0.0
    ...
    # guardian: allow-magic-config
    control_decision = RouteDecisionArtifact(trace_id='phase9-control-001', timestamp='2024-01-01T00:00:00Z', route_path=RoutePath.LOW_RISK_BYPASS, risk_score=0.1, budget_est=50.0, rationale_enum=RoutingRationale.LOW_RISK_BYPASS, policy_config_hash='control789')
...
def main():
    ...
    pytest_output = run_subprocess_command(['python', '-m', 'pytest', '-q', 'tests/unit_min_deps/test_shadow_router_classifier.py'])
    print(pytest_output)
    print()
