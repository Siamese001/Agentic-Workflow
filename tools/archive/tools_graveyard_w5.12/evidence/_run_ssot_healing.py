...
_LOG_RE = re.compile(
    "^(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2},\\d{3})\\s+(INFO|WARNING|ERROR|CRITICAL|DEBUG)\\s+(\\S+)\\s+(.*)$"
)
...
def _build_report(run_meta: dict, stdout_text: str, stderr_text: str) -> dict:
            ...
            rca.append(
                {
                    ...
                    "agent": "LocationAgent",
                    "root_cause": "super().__post_init__() in LocationHealerAgent reaches SovereignBaseAgent which had no __post_init__ defined",
                    "status": "REMEDIATED",
                    "fix_applied": "Added __post_init__ + _sovereign_init to SovereignBaseAgent; updated .core_golden_seal Merkle hash",
    ...
    phase_events = [
        {"timestamp": e["timestamp"], "level": e["level"], "message": e["message"]}
        for e in entries
        if re.search("Phase [1-5]", e["message"])
    ]
    summary = {
...
def main() -> int:
    ...
    proc = subprocess.run(
        cmd, capture_output=True, text=True, encoding="utf-8", errors="replace", env=env, cwd=str(_REPO_ROOT)
    )
