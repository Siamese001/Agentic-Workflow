    ...
    for line in text.splitlines():
        ...
        if in_table and s.startswith("| REQ-"):
            ...
            if len(cells) >= 7:
                ...
                rows.append(
                    {"id": rid, "domain": domain, "severity": severity, "layers": elayers, "eclass": eclass}
                )
    return rows
...
def main():
    ...
    w(
        f"ARITHMETIC_CHECK: {total_pass} + {total_partial} + {total_fail} + {total_struct} = {arith_sum} ({('== TOTAL OK' if arith_ok else '!= TOTAL MISMATCH')})"
    )
    w("```")
    w("")
    w("---")
