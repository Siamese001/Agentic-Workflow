...
def main():
    ...
    commands = [
        ([sys.executable, "-m", "pytest", "-q", "--color=no"], "Full Test Suite"),
        (
            [sys.executable, "ops_scripts/ci/check_evidence_contract_v2.py", "--paths", DOCS_REPORTS_PLANS],
            "Evidence Contract v2 Checker",
        ),
        ([sys.executable, "ops_scripts/ci/check_tooling_apps_boundary.py"], "Tooling/Apps Boundary Guard"),
        ([sys.executable, "ops_scripts/ci/run_contract_gates.py"], "Contract Gates Runner"),
    ]
    failed_commands = []
