...
def _run(argv: list[str], title: str, lines: list[str], *, required: bool = True) -> bool:
    ...
    result = subprocess.run(
        argv, shell=False, encoding="utf-8", errors="replace", capture_output=True, cwd=str(REPO_ROOT)
    )
...
def main() -> int:
    ...
    pytest_result = subprocess.run(
        pytest_argv, shell=False, encoding="utf-8", errors="replace", capture_output=True, cwd=str(REPO_ROOT)
    )
    ...
    if bad_bytes:
        print(
            f"FAIL: evidence file contains {len(bad_bytes)} non-ASCII bytes at positions {bad_bytes[:5]}",
            file=sys.stderr,
        )
        return 1
    if not all_passed:
