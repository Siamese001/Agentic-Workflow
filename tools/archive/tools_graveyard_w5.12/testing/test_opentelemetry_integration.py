...
def test_opentelemetry_runtime_adg_integration():
    ...
    try:
        ...
        with end_to_end_tracer.trace_orchestrator("e2e-mission", {"pipeline": "test"}) as root_span:
            with end_to_end_tracer.trace_cognitive(
                "e2e-planning", reasoning_mode="chain_of_thought"
            ) as planning_span:
                time.sleep(0.001)
        ...
        results["e2e_success"] = (
            len(e2e_spans) >= 4  # orchestrator + cognitive + action + 2 tools
            and len(e2e_snapshot.nodes) >= 4
            and len(e2e_snapshot.edges) >= 4
            and e2e_payload is not None
            and len(e2e_payload) > 0
        )

        if results["e2e_success"]:
            print(
                f"✅ End-to-end pipeline successful: {len(e2e_spans)} spans → {len(e2e_snapshot.nodes)} nodes → {len(e2e_snapshot.edges)} edges"
            )
        else:
