...
check_result(
    ...
    result,
    lambda r: print(
        f"  ✅ OK - Nodes: {r['data'].get('node_count')}, Edges: {r['data'].get('edge_count')}, Fresh: {r['data'].get('is_fresh')}"
    ),
)
...
check_result(
    "[3/17] adg_snapshot()", adg_snapshot(), lambda r: print(f"  ✅ OK - Data keys: {list(r.keys())}")
)
...
check_result(
    "[5/17] redis_scan()",
    redis_scan(pattern="adg:*", max_keys=50),
    lambda r: print(
        f"  ✅ OK - Matched: {r.get('matched_keys')} keys, Prefixes: {list(r.get('prefix_summary', {}).keys())[:3]}"
    ),
)
...
check_result(
    ...
    redis_lrange("adg:violations", start=0, stop=5),
