            ...
            if isinstance(data, dict):
                ...
                for k in top_keys:
                    ...
                    if isinstance(v, list):
                        print(f"    [{k}]: list({len(v)})  first item keys: {(list(v[0].keys())[:6] if v and isinstance(v[0], dict) else '?')}")
...
new_files = {'file_graph': sorted(glob.glob(Path(ADG_DIR) / 'adg_file_graph_*.json'))[-1], 'governance_graph': sorted(glob.glob(Path(ADG_DIR) / 'adg_governance_graph_*.json'))[-1], 'symbol_graph': sorted(glob.glob(Path(ADG_DIR) / 'adg_symbol_graph_*.json'))[-1], 'test_graph': sorted(glob.glob(Path(ADG_DIR) / 'adg_test_graph_*.json'))[-1], 'full': sorted(glob.glob(Path(ADG_DIR) / 'adg_full_*.json'))[-1], 'sqlite': sorted(glob.glob(Path(ADG_DIR) / 'adg_indexed_*.sqlite'))[-1]}
for label, fpath in new_files.items():
    ...
    if fpath.endswith('.json'):
        try:
            with open(fpath, encoding='utf-8') as f:
                snippet = f.read(4096)
            data = json.loads(snippet + '"}' if not snippet.rstrip().endswith('}') and (not snippet.rstrip().endswith(']')) else snippet)
        # guardian: allow-silent-swallow
        except Exception:
            try:
                with open(fpath, encoding='utf-8') as f:
