    ...
    def validate_silent_swallowers(self):
        ...
        try:
            ...
            if report_file.exists():
                ...
                medium_severity = len(
                    [v for v in report.get("violations", []) if v.get("severity") == "MEDIUM"]
                )
    ...
    def generate_comprehensive_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "critical_issues": [
                    cat
                    for cat, details in compliance_details.items()
            ...
            if not details["compliant"]:
                if category == "layer_gravity":
                    recommendations.append(
                        "Fix remaining layer gravity violations using tools/fix_layer_gravity_violations.py"
                    )
                elif category == "silent_swallowers":
                    recommendations.append(
                        "Apply silent swallower fixes using tools/fix_high_severity_silent_swallowers.py"
                    )
                elif category == "test_enforcement":
                    recommendations.append(
                        "Fix test enforcement violations using tools/fix_test_enforcement_high.py"
                    )
                elif category == "adg_separation":
                    recommendations.append(
                        "Deploy clean static/runtime ADG separation using tools/deploy_clean_adg_separation.py"
                    )
...
def main():
    ...
    print(
        f"📁 Categories compliant: {report['summary']['compliant_categories']}/{report['summary']['total_categories']}"
    )
