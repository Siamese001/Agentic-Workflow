...
if "GovernedPayload" not in c:
    new = safe_insert_before(
        c,
        "from agentic_core.L0_routing.types.determinism_types",
...
if "_emit_reads_through" in c and "lifecycle_trace_contract" not in c:
    ...
    for i, line in enumerate(lines):
        if line.strip().startswith("def _require_safe_segment"):
            lines.insert(
                i, "from agentic_core.runtime.contracts.lifecycle_trace_contract import _emit_reads_through\n"
            )
            break
    write(rel, "\n".join(lines))
    fixes_applied += 1
