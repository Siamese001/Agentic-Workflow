"""Add remaining manual CamelCase aliases where automatic snake_case conversion failed."""

import ast

fp = r"C:\Git\Agentic-Workflow\apps_shared\types\sovereign_severity_types.py"
src = open(fp, encoding="utf-8").read()
tree = ast.parse(src)

# Manual mapping: CamelCase -> actual class name
manual = {
    "SafetyProfile": "safety_profile_types",
    "LLMResponse": "llm_response",
    "RAGState": "rag_state",
    "EnforcementRAGConfig": "enforcement_rag_config",
    "RAGCritique": "rag_critique",
    "EnforcementRAGResult": "enforcement_rag_result",
    "QAReport": "qa_report",
    "CTABrief": "cta_brief",
    "APICallMetrics": "api_call_metrics",
    "CTAFormat": "cta_format",
    "RGCreativeBrief": "rg_creative_brief",
}

# Find end line of each target class
class_ends = {}
for node in ast.walk(tree):
    if isinstance(node, ast.ClassDef) and node.name in manual.values():
        class_ends[node.name] = node.end_lineno

lines = src.split("\n")
# Insert aliases bottom-up
insertions = []
for camel, snake in manual.items():
    if snake in class_ends:
        insertions.append((class_ends[snake], f"{camel} = {snake}"))

insertions.sort(key=lambda x: x[0], reverse=True)
for line_no, alias in insertions:
    lines.insert(line_no, alias)

new_src = "\n".join(lines)
try:
    ast.parse(new_src)
    open(fp, "w", encoding="utf-8").write(new_src)
    print(f"Inserted {len(insertions)} aliases. Syntax OK.")
except SyntaxError as e:  # guardian: allow-silent-swallow - acceptable exception handling
    print(f"SYNTAX ERROR: {e}")
