...
def main() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["python", "ops_scripts/ci/check_anti_patterns.py"], capture_output=True, text=True, cwd=REPO
        )
        ...
        subprocess.run(
            [
                "cmd",
                "/c",
                "set ALLOW_LANDMINE_BASELINE_WRITE=1&& python ops_scripts/ci/check_anti_patterns.py --write-baseline",
            ],
            cwd=REPO,
        )
        result = subprocess.run(
