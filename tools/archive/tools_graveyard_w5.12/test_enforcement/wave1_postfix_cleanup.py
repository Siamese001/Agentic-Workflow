...
def cleanup_file(filepath: pathlib.Path) -> dict:
    ...
    source = re.sub(r'^\s+assert _AVAILABLE\s*$', '        pass  # Import verified at module level', source, flags=re.MULTILINE)
    ...
    for i, fp in enumerate(test_files):
        ...
        if "_AVAILABLE" in source and "def " in source:
            ...
            for line in source.splitlines():
                stripped = line.strip()
                if "_AVAILABLE" in stripped and not stripped.startswith("#") and not stripped.startswith('"""'):
