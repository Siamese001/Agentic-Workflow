...
def update_reports():
    ...
    try:
        ...
        unknown_modules = [
            {"adg_name": row[0], "resolved_path": row[1], "identity_kind": row[2]}
        ...
        layer_report = {
            ...
            "coverage_metrics": {
                ...
                "coverage_percentage": (total_nodes - len(unknown_modules)) / total_nodes * 100
                if total_nodes > 0
                else 0,
            },
        }
        ...
        edge_report = {
            ...
            "density_metrics": {
                ...
                / len(critical_edges)
                * 100,
                "top_edge_type": max(edge_distribution.items(), key=lambda x: x[1])[0]
        ...
        provenance_report = {
            ...
            "generation_metrics": {
                ...
                "modules_scanned": cur.execute(
                    "SELECT COUNT(*) FROM nodes WHERE entity_type='module'"
                ).fetchone()[0],
                "symbols_scanned": total_nodes
                - cur.execute("SELECT COUNT(*) FROM nodes WHERE entity_type='module'").fetchone()[0],
        ...
        determinism_report = {
            ...
            "validation": {
                ...
                "determinism_status": "complete"
                if edge_distribution.get("emits_determinism_digest", 0) > 0
                else "missing",
            },
        }
        ...
        boundary_counts = {
            edge_type: edge_distribution.get(edge_type, 0) for edge_type in boundary_edge_types
        }
        ...
        mutation_report = {
            ...
            "replay_guarantees": {
                "determinism_status": "complete"
                if edge_distribution.get("mutation_signature", 0) > 0
                else "missing",
            ...
            "signature_coverage": {
                ...
                "coverage_percentage": (
                    edge_distribution.get("mutation_signature", 0)
                    / cur.execute("SELECT COUNT(*) FROM nodes WHERE entity_type='module'").fetchone()[0]
                    * 100
                )
                if cur.execute("SELECT COUNT(*) FROM nodes WHERE entity_type='module'").fetchone()[0] > 0
                else 0,
            },
        }
        ...
        test_surface_report = {
            ...
            "critical_path_linkage": {
                ...
                "critical_path_completeness": "complete"
            ...
            "binding_metrics": {
                ...
                "test_coverage_percentage": modules_with_test_linkage / total_critical_modules
                if total_critical_modules > 0
                else 0,
            },
        }
