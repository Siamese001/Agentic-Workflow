...
class MCPSmokeTester:
    ...
    async def _test_server(self, server_name: str, server_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            results["tests"]["functionality"] = await self._test_server_functionality(
        ...
        try:
            ...
            process = subprocess.run(
                [
                    "python",
                    "-c",
                    "import subprocess; print(subprocess.run(['echo', 'test'], capture_output=True, text=True).stdout)",
                ],
                capture_output=True,
                text=True,
                timeout=5,
