...
MULTILINE_FROZENSET_PAT = re.compile(
    "^(?P<var>[A-Za-z_][A-Za-z0-9_]*)(?P<annot>[^=]*)=\\s*frozenset\\(\\s*\\{", re.MULTILINE
)
MULTILINE_SET_PAT = re.compile("^(?P<var>[A-Za-z_][A-Za-z0-9_]*)(?P<annot>[^=]*)=\\s*\\{", re.MULTILINE)
INLINE_SET_PAT = re.compile(
    "^(?P<var>[A-Za-z_][A-Za-z0-9_]*)(?P<annot>[^=]*)=\\s*[\\[\\{](?P<items>[^\\]\\}]+)[\\]\\}]", re.MULTILINE
)
...
def _remove_ssot_block(lines: list[str]) -> list[str]:
    ...
    while i < len(lines):
        if re.match(
            "from agentic_core\\.L5_safety\\.config\\.structure_blueprint\\.ssot import", lines[i].strip()
        ):
            j = i
...
def fix_import_order(path: pathlib.Path) -> bool:
    ...
    for i, line in enumerate(lines):
        if re.match(
            "from agentic_core\\.L5_safety\\.config\\.structure_blueprint\\.ssot import", line.strip()
        ):
            first_import_idx = i
            break
...
def replace_hardcoded_literal(path: pathlib.Path, lineno: int, varname: str) -> bool:
    ...
    inline_match = re.match(
        "^(\\s*" + re.escape(varname) + "[^=]*=\\s*)(frozenset\\(\\s*)?\\{([^}]+)\\}(\\s*\\))?", line
    )
    if inline_match:
