...
APPROVED_SKIP_PATTERNS: dict[str, str] = {'tests/hardening/conftest.py': 'zero-skip enforcement gate: detects skips and converts to CI failure', 'tests/_config/import_strict_mode.py': 'controlled import-strict ramp: skip is documented degraded-mode fallback', 'tests/performance/test_adg_runtime_acceleration.py': 'performance gate: env-gated skip for slow/GPU-only environments', 'tests/integration_full_deps/': 'integration_full_deps: requires live external packages not present in unit CI', 'tests/sovereign_hardening/test_ssot_pipeline_protocol.py': 'sovereign_hardening: environment-gated skip for missing optional infra', 'tests/unit/test_ai_checking_ai_hardenings.py': 'ai_checking hardenings: skips when AI service is unavailable in unit CI', 'tests/hardening/test_all_gap_fixes.py': 'gap-fix hardening: skipif guards for missing optional dependencies', 'tests/unit/test_sovereign_seal_state.py': 'sovereign seal: env-gated skipif for missing crypto dependency', 'tests/integration_full_deps/test_seed_pack_full_build_b5.py': 'full-build integration: skipif for missing full-stack dependencies'}
    ...
    for f in sorted(TESTS_DIR.rglob('conftest.py')):
        ...
        for lineno, code in hits:
            if rationale:
                ...
                violations += 1
    return violations
APPROVED_CI_SUPPRESSIONS: frozenset[str] = frozenset({'--ignore=tests/integration/test_redis_integration.py', '--ignore=tests/integration/test_imports_no_mro_error.py', '--ignore=tests/evaluation'})

def scan_ci_config(out: list[str]) -> int:
    """Scan pytest.ini, pyproject.toml, and CI workflows for marker suppression."""
    danger = re.compile('(-m\\s+[\\"\'`]?not\\s+|--ignore=tests|--deselect|addopts.*-k\\s|norecursedirs.*tests)', re.IGNORECASE)
    count = 0
    config_files: list[pathlib.Path] = [ROOT / 'pytest.ini', ROOT / 'pyproject.toml', ROOT / 'tox.ini', ROOT / 'setup.cfg']
...
def main() -> int:
    ...
    print()
    categories: dict[str, list[str]] = {'XFAIL_NO_STRICT': [], 'VIOLATION_SKIP': [], 'CONFTEST_SKIP': [], 'CI_SUPPRESSION': [], 'APPROVED_SKIP': [], 'CONFTEST_APPROVED': [], 'CI_SUPPRESSION_APPROVED': []}
    for line in report:
