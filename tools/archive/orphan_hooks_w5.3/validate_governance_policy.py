...
GOVERNANCE_REQUIREMENTS: dict[str, dict[str, dict[str, str]]] = {
    ...
    "ops_scripts/ci/check_anti_patterns.py": {
        "baseline_protection": {
            ...
            "pattern": "ALLOW_LANDMINE_BASELINE_WRITE",
            "policy_file": "docs/rules/governance.md",
    ...
    for req_name, req_config in requirements.items():
        ...
        if re.search(pattern, content):
            if not policy_sections.get(required_section):
                violations.append(
                    f"{req_name}: {req_config['description']}. Missing section '{required_section}' in {policy_file}"
                )
    return violations
...
def validate_governance_consistency() -> list[str]:
    ...
    if precommit.exists() and _file_changed(precommit):
        ...
        for hook_id in manual_ids:
            matching_headers = [
                h for h in policy_sections.keys() if re.search(f"\\({re.escape(hook_id)}\\)", h)
            ]
            if not matching_headers:
                violations.append(
                    f".pre-commit-config.yaml: hook '{hook_id}' moved to manual stage but governance.md lacks a section header containing '({hook_id})'"
                )
                continue
