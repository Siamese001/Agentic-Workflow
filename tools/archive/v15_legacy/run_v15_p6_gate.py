...
V15_TYPE_FILES = ['agentic_core/L0_routing/types/determinism_types.py', 'agentic_core/L0_routing/types/determinism_contracts_types.py', 'agentic_core/L0_routing/types/governance_types.py', 'agentic_core/L0_routing/types/traceability_types.py', 'agentic_core/L0_routing/types/crypto_trust_types.py', 'agentic_core/L0_routing/types/boundary_types.py']
...
class P6EvidenceCollector:
    ...
    def _check_ssot_binding_type(self):
        ...
        if has_class and has_node_id and has_blueprint and has_resolved:
            self.checks_passed.append({'check': 'ssot_binding_type', 'detail': 'SSOTBinding found with node_id, blueprint_entry, resolved'})
        else:
            self.violations.append({'check': 'ssot_binding_type', 'detail': f'class={has_class}, node_id={has_node_id}, blueprint_entry={has_blueprint}, resolved={has_resolved}'})
    ...
    def _check_context_retrieval_request(self):
        ...
        if has_class and has_trace and has_query:
            self.checks_passed.append({'check': 'context_retrieval_request', 'detail': 'ContextRetrievalRequest found for L0→L4 boundary'})
        else:
            self.violations.append({'check': 'context_retrieval_request', 'detail': f'class={has_class}, trace_id={has_trace}, query_hash={has_query}'})
    ...
    def _check_layer_boundary_imports(self):
        ...
        for rel_path in V15_TYPE_FILES:
            ...
            try:
                ...
                self.violations.append({'check': 'layer_boundary_imports', 'file': rel_path, 'detail': 'SyntaxError — cannot parse'})
                continue
            inversions = []
                    ...
                    if isinstance(node, ast.ImportFrom) and node.module:
                        ...
                        if m:
                            ...
                            if LAYER_ORDER.get(imported_layer, 0) > LAYER_ORDER.get('L0', 0):
                                inversions.append({'line': node.lineno, 'module': node.module, 'layer': imported_layer})
            if not inversions:
                self.checks_passed.append({'check': 'layer_boundary_imports', 'file': rel_path, 'detail': 'No layer inversions in L0 type file'})
            else:
                self.violations.append({'check': 'layer_boundary_imports', 'file': rel_path, 'detail': f'{len(inversions)} layer inversions found', 'inversions': inversions})
    ...
    def _check_p6_artifact_immutability(self):
        ...
        if total_dc > 0 and frozen_count == total_dc:
            self.checks_passed.append({'check': 'p6_artifact_immutability', 'detail': f'All {total_dc} dataclasses in boundary_types are frozen=True'})
        elif total_dc > 0:
            self.violations.append({'check': 'p6_artifact_immutability', 'detail': f'{frozen_count}/{total_dc} dataclasses are frozen'})
    ...
    def _check_all_type_files_exist(self):
        ...
        if not missing:
            self.checks_passed.append({'check': 'type_files_complete', 'detail': f'All {len(V15_TYPE_FILES)} V15 type files present'})
        else:
