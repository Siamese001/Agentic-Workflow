...
class P4EvidenceCollector:
    ...
    def _check_trace_id_pattern_exported(self):
        ...
        if not p4_path.exists():
            self.violations.append(
                {"check": "trace_id_pattern_exported", "detail": "traceability_types.py not found"}
            )
            return
        ...
        if has_pattern and has_regex:
            ...
            self.violations.append(
                {"check": "trace_id_pattern_exported", "detail": f"pattern={has_pattern}, regex={has_regex}"}
            )
    ...
    def _check_validate_trace_id_callable(self):
        ...
        try:
            ...
            raise
            self.violations.append(
                {"check": "validate_trace_id_callable", "detail": f"Import/call failed: {e}"}
            )
    ...
    def _check_generate_trace_id_callable(self):
        ...
        try:
            ...
            if pattern.match(tid):
                self.checks_passed.append(
                    {"check": "generate_trace_id_compliant", "detail": f"generate_trace_id produced: {tid}"}
                )
            else:
                self.violations.append(
                    {"check": "generate_trace_id_compliant", "detail": f"Non-compliant output: {tid}"}
                )
        except Exception as e:  # guardian: allow-broad-exception -- intentional error boundary, re-raises all caught exceptions to caller
            raise
            self.violations.append(
                {"check": "generate_trace_id_callable", "detail": f"Import/call failed: {e}"}
            )
    ...
    def _check_trace_id_in_manifest(self):
        ...
        if not p2_path.exists():
            self.violations.append(
                {"check": "trace_id_in_manifest", "detail": "determinism_types.py not found"}
            )
            return
        content = p2_path.read_text(encoding="utf-8")
        if "correlation_id: str" in content:
            self.checks_passed.append(
                {"check": "trace_id_in_manifest", "detail": "SurgicalManifest.correlation_id present"}
            )
        else:
            self.violations.append(
                {
    ...
    def _check_trace_id_in_gateway_execute(self):
        ...
        if "trace_id=trace_id" in content:
            self.checks_passed.append(
                {"check": "trace_id_in_gateway_execute", "detail": "trace_id passed to gateway.execute()"}
            )
        else:
            self.violations.append(
                {"check": "trace_id_in_gateway_execute", "detail": "trace_id not passed to gateway.execute()"}
            )
    ...
    def _check_trace_id_immutability_contract(self):
        ...
        if "@dataclass(frozen=True)" in content and "class SurgicalManifest" in content:
            self.checks_passed.append(
                {"check": "manifest_immutability", "detail": "SurgicalManifest is frozen=True (immutable)"}
            )
        else:
            self.violations.append(
                {"check": "manifest_immutability", "detail": "SurgicalManifest not marked frozen=True"}
            )
