...
def _archive_old_artifacts(adg_dir: Path, current_ts: str, keep_runs: int = 1) -> None:
    ...
    for ts in to_archive:
        ...
        try:
            ...
            print(f"[ADG] Archive: failed to create archive dir for {ts}: {e}")    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
            continue

        # Check if this run has a zip file (preferred storage)
        zip_files = [f for f in files if f.name.startswith("adg_run_") and f.suffix == ".zip"]    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
...
def _stable_digest(payload: object) -> str:
    text = json.dumps(payload, sort_keys=True, separators=(",", ":"), default=str)    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime
    ...
    proof["determinism_status"] = (
        ...
        if all(
            ...
            for key in (
                ...
                "edge_row_digest_match",    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
            )
        )
        else "partial"
    )
...
def _cleanup_validation_files(adg_dir: Path, current_ts: str) -> None:
    ...
    # Remove all MANIFEST files (low value)    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
...
def _archive_individual_files(files: list[Path], archive_month_dir: Path) -> tuple[int, int, int]:
    """Archive individual files (legacy fallback for orphaned runs).    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
