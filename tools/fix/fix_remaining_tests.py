...
def aggressive_clean(filepath):
    ...
    while i < len(lines):
        ...
        if in_class and not in_function:
            ...
            if any(stripped.startswith(x) for x in ['# Arrange', '# Act', '# Assert', 'result = None', 'assert ', 'input_data = ', 'runtime_context = ']):
                    ...
                    while i < len(lines):
                        ...
                        if lines[i].strip() == '' or curr_indent <= indent_level:
                            if lines[i].strip() not in ['', 'pass'] and not lines[i].strip().startswith('#'):
                                if curr_indent <= indent_level and (lines[i].strip().startswith('def ') or lines[i].strip().startswith('class ')):
                                    break
                        i += 1
                    continue
