#!/usr/bin/env python3
"""Automated migration tool to move top-level app imports into test functions/fixtures.

Target: Convert risky collection-phase imports to deferred execution-phase imports.
Focus: agentic_core, apps_*, system_learning, infrastructure imports in test files.
"""
...
class TestImportMigrator:
        ...
        for imp in imports:
            if isinstance(imp, ast.Import):
                for alias in imp.names:
                    ...
                    import_map[name] = ast.unparse(imp)
    ...
    def _generate_migrated_content(self, content: str, imports: list[ast.AST],
        ...
        for i, line in enumerate(lines):
            if not TARGET_IMPORT_RE.match(line):
                new_lines.append(line)
            else:
                # Replace with a comment indicating migration
                new_lines.append(f"#  # MOVED: {line.strip()}")

        # Add imports back into functions
        for func_name, used_names in usage.items():
            ...
            if func_start is not None:
                ...
                for name in used_names:
                    # Find the original import statement
                    for imp in imports:
                        if isinstance(imp, ast.Import):
                            for alias in imp.names:
