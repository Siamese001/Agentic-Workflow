...
def main():
    ...
    mandatory = {dep.split(">=")[0].split("==")[0].split("<=")[0].split("[")[0].strip().lower() for dep in project.get("dependencies", [])}

    optionals = project.get("optional-dependencies", {})
    optional_groups = {}
    for group, deps in optionals.items():
        optional_groups[group] = {dep.split(">=")[0].split("==")[0].split("<=")[0].split("[")[0].strip().lower() for dep in deps}
    ...
    try:
        result = subprocess.run([sys.executable, "tools/find_imports.py"], capture_output=True, text=True, check=True)
    ...
    for imp, locs in found_imports.items():
        ...
        if imp in ["client", "data", "execute_ssot", "mcp0_git_add_or_commit",
                  "mcp0_git_branch", "mcp0_git_log_or_diff", "mcp0_git_push",
                  "mcp0_git_status", "mcp11_delete", "mcp11_get", "mcp11_set",
                  "mcp4_fetch", "mcp6_get_file_info", "mcp6_list_directory",
                  "mcp6_read_text_file", "mcp8_add_observations", "mcp8_create_entities",
                  "mcp8_search_nodes", "mcp_time_client", "runtime", "services",
                  "shared", "territory_ssot_definitions", "test_migration_guardian",
                  "test_runtime_adg_integration", "test_shadow_router_classifier",
                  "test_vllm_canonical_payload_lock", "test_vllm_replay_tamper_roundtrip",
                  "wave6a_validation_enforcer", "wave_state_manager", "p0_microwave_wirer",
                  "pre_execution_validator", "implement_unified_memory", "idempotent_wave_template",
                  "guardian_sweep", "fast_file_analysis", "broken_module", "archives", "config",
