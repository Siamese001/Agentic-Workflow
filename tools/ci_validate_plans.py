...
class CIPlanValidator:
    ...
    def validate_all_plans(self) -> Dict[str, Any]:
        ...
        self.results["metrics"] = {
            "avg_lines": sum(r["metrics"]["lines"] for r in [self.validate_single_plan(p) for p in plans]) // len(plans) if plans else 0,
            "total_waves": sum(r["metrics"]["waves"] for r in [self.validate_single_plan(p) for p in plans]),
            "total_tokens_estimated": sum(r["metrics"]["tokens_estimated"] for r in [self.validate_single_plan(p) for p in plans])
        }
    ...
    def save_report(self, output_path: str = None):
        ...
        if output_path is None:
            output_path = self.repo_root / "artifacts" / "ci" / "plan_validation_report.md"
...
def main():
    ...
    # Print final status
    print("=" * 80)
    print(f"CI VALIDATION COMPLETE")
