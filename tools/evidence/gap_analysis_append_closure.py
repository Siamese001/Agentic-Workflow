...
def parse_baseline():
    ...
    text = REQ_MD.read_text(encoding="utf-8")
    m_total = re.search(r"TOTAL_ROWS\s*=\s*(\d+)", text)
    m_corpus = re.search(r"CORPUS_ROWS\s*=\s*(\d+)", text)
    m_ext = re.search(r"EXT_ROWS\s*=\s*(\d+)", text)
    ...
    return {
        "integrity_total": int(m_total.group(1)) if m_total else None,
        "integrity_corpus": int(m_corpus.group(1)) if m_corpus else None,
        "integrity_ext": int(m_ext.group(1)) if m_ext else None,
        "actual_table": actual_table,
        "actual_corpus": corpus_actual,
        "actual_ext": ext_actual,
        "ext_prefixes": {k: v for k, v in sorted(pfx.items()) if k != "REQ-nnn"},
    }


# ── Parse existing ledger from evidence file ────────────────────────────────
                ...
                rows.append(
                    {
                        ...
                        "matches": cells[4] if len(cells) > 4 else "",
                    }
                )
...
def main():
    ...
    # xfail negative control
    # guardian: allow-magic-config
    xfail_cmd, xfail_lines, xfail_total, xfail_files = py_grep_raw(r"xfail.*strict.*True", max_lines=20)

    # ── CRITICAL PARTIAL isolation ──────────────────────────────────────
    crit_partial_rows = [r for r in ledger if r["severity"] == "CRITICAL" and r["status"] == "PARTIAL"]

    # Classify by missing layer category
    ...
    w(
        f"  INTEGRITY_BLOCK_CLAIM = {baseline['integrity_total']}  (STALE -- {discrepancy} ext rows missing from table)"
    )
    w("")
    ...
    w(
        f"ARITHMETIC_CHECK: {total_pass} + {total_partial} + {total_fail} + {total_struct}"
        f" = {arith_sum} ({'== TOTAL OK' if arith_ok else '!= TOTAL MISMATCH'})"
    )
    w("```")
    w("")
    ...
    spot_checks = [
        ("REQ-036", "replay_envelope|ReplayGuard|replay_validator", "Replay binding"),
        ("REQ-045", "embedding_input_guard|embedding_sovereignty_guard", "Embedding guard"),
        ("REQ-011", "SovereignLLMGateway", "Gateway enforcement"),
        ("REQ-413", "provider_binding_determinism", "Provider binding determinism"),
    ]
    w("### D4. Spot-Check Evidence for Key CRITICAL Items")
    w("")
    for rid, pat, desc in spot_checks:
        cmd_d, raw_d, mc_d, files_d = py_grep_raw(pat, max_lines=10)  # guardian: allow-magic-config
    ...
    w(f"  CRITICAL_PASS = {crit_pass}")
    w(
        f"  PASS_RATE = {crit_pass}/{crit_pass + crit_partial + crit_fail} = {100 * crit_pass / (crit_pass + crit_partial + crit_fail):.1f}%"
    )
    w("")
    ...
    if verdict in ("BLOCKED", "CONDITIONAL"):
        ...
        if discrepancy > 0:
            w(
                f"  {3 if crit_fail > 0 else 2}. Write {discrepancy} missing extension rows to requirements table"
            )
    ...
    w(
        f"  - CRITICAL_PASS = {crit_pass} ({100 * crit_pass / (crit_pass + crit_partial + crit_fail):.1f}% of CRITICAL requirements)"
    )
    ...
    # ── Strip existing closure blocks if present, then append ───────────
