...
def main() -> None:
    ...
    h('')
    h('## Scope')
    h('Phase 4 of Qwen vLLM migration: seal deterministic replay by adding infrastructure fingerprint capture, canonical hashing, and replay validation harnesses, wired through the Phase 3 adapter/controller telemetry path. Preserves Phase 1-3 routing/backpressure invariants. No model tier changes.')
    h('')
    h('## CODE_COMMIT')
    h(code_commit)
    h('')
    ...
    out, rc = run([sys.executable, '-m', 'pytest', '-q', '--color=no', 'tests/agentic_core/L2_execution/types/test_vllm_infrastructure_fingerprint.py'])
    fence(out)
    if rc != 0:
    ...
    out, rc = run([sys.executable, '-m', 'pytest', '-q', '--color=no', 'tests/agentic_core/L2_execution/types/test_vllm_replay_validator.py'])
    fence(out)
    if rc != 0:
    ...
    phase_files = ['agentic_core/L2_execution/types/vllm_infrastructure_fingerprint_types.py', 'agentic_core/L2_execution/types/vllm_gateway_integration_types.py', 'agentic_core/L2_execution/types/vllm_gateway_adapter_types.py', 'agentic_core/L2_execution/types/vllm_replay_validator_types.py', 'tests/agentic_core/L2_execution/types/test_vllm_infrastructure_fingerprint.py', 'tests/agentic_core/L2_execution/types/test_vllm_replay_validator.py']
