...
def py_find_files(name_pattern, root=None, max_results=20):
    ...
    return (f"py_find(r'{name_pattern}')", results)
PROD_PREFIXES = ('agentic_core/', 'apps_', 'system_learning/', 'L6_observability/', 'ops_scripts/', 'tools/', '.github/')
...
class _FileAstRecord:
    ...
    __slots__ = ('path', 'imports', 'from_imports', 'classes', 'functions', 'base_classes', 'decorators', 'calls', 'raised', 'markers')
...
_ENFORCEMENT_CLASS_SUFFIXES = ('Guard', 'Validator', 'Gate', 'Enforcer', 'Contract', 'Prohibition', 'Invariant')
_ENFORCEMENT_FUNC_PREFIXES = ('verify', 'enforce', 'check_', 'deny', 'allow', 'validate_', 'guard_', 'gate_')
_GOVERNANCE_EXCEPTIONS = {'SovereigntyError', 'ToolNotAllowedError', 'BudgetExhaustedError', 'MutationProhibitionError', 'EmbeddingInputViolation', 'PolicyViolation'}
...
_SCANNER_CALLS = {'ast.parse', 'ast.walk', 'ast.get_source_segment', 'libcst.parse_module', 'inspect.getsource'}
...
_REPLAY_CLASSES = {'ReplayValidator', 'ReplayGuard', 'ReplayEnvelope', 'ReplayGuardStore', 'ReplayGuardRecord'}
...
def ast_explain_replay(records):
    ...
    for rec in records:
        ...
        prefix_fn = [fn for fn in rec.functions if fn.startswith(('replay_', 'determinism_')) and fn not in rp_fn]
        if prefix_fn:
...
def _run_self_validation(ast_index, ci_index):
    ...
    """
    results = []
    scanner_files = [r for r in ast_index.all_records.values() if r.imports & _SCANNER_IMPORTS and r.calls & _SCANNER_CALLS]
    ...
    results.append(('CI_RUNS_PYTEST', t3_pass, f'CI workflows: {len(ci_index.workflow_names)}, runs_pytest={ci_index.runs_pytest}'))
    return results
    ...
    for line in lines:
        ...
        if in_table and s.startswith('| REQ-'):
            ...
            if len(cells) >= 7:
                ...
                rows.append({'id': rid, 'domain': domain, 'requirement': req_text, 'enforcement': enf_text, 'severity': severity, 'layers': elayers, 'eclass': eclass})
    return rows
...
def get_search_terms(req):
    ...
    SPECIFIC = {'REQ-011': ['SovereignLLMGateway'], 'REQ-012': ['model.*literal.*outside|system_invariant_scanner'], 'REQ-013': ['EmbeddingServiceFactory|embedding_factory'], 'REQ-022': ['InstructionPacket.*signature|instruction_packet.*verify'], 'REQ-024': ['SandboxEnvelope.*signature|sandbox_envelope.*verify'], 'REQ-029': ['UniversalWriteGateway|write_gateway\\.py'], 'REQ-037': ['xfail.*strict.*True|negative.*control.*tamper'], 'REQ-038': ['route_healing_tier'], 'REQ-045': ['embedding_input_guard|embedding_sovereignty_guard'], 'REQ-114': ['datetime\\.now|time\\.time\\(\\)|uuid4'], 'REQ-243': ['WaveAuditSummary'], 'REQ-244': ['WaveAuditSummary'], 'REQ-413': ['provider_binding_determinism'], 'REQ-414': ['network_egress_guard'], 'REQ-415': ['provider_substitution_prohibition'], 'REQ-416': ['critical_dual_enforcement'], 'REQ-417': ['runtime_mutation_guard'], 'REQ-PT-001': ['slot.*order.*S0.*I0.*C0.*U0|prompt_assembler.*slot'], 'REQ-PT-002': ['slot.*S0|S0.*L5|prompt.*slot.*own|slot_order'], 'REQ-PT-005': ['PromptBundle|prompt.*bundle|prompt.*artifact'], 'REQ-PT-007': ['U0.*override|slot.*ownership|user.*slot.*restrict|slot_order'], 'REQ-PT-009': ['prompt.*deterministic|prompt_assembler.*canonical'], 'REQ-PT-011': ['validate_slot_order|SlotOrderViolation|xfail.*strict.*True.*slot|negative.*control.*prompt|slot.*tamper'], 'REQ-PT-012': ['prompt_hash.*stable|dual.*run.*prompt'], 'REQ-EM-004': ['embedding.*disabled.*fail|EMBEDDING_ENABLED.*fail'], 'REQ-RAGX-005': ['chunk.*hash|chunk.*sha256|chunk.*integrit|retriev.*verify'], 'REQ-RAGX-006': ['ExternalKnowledge|knowledge.*access.*viol|external.*retriev.*guard'], 'REQ-CTX-002': ['PreGuard|pre.*guard.*snapshot|context.*snapshot|ContextSnapshot'], 'REQ-COG-001': ['PolicyAlignment|policy.*alignment|PolicyViolation'], 'REQ-COG-002': ['prompt.*augmentation.*MRO|dependency.*facts.*augment'], 'REQ-HEALX-001': ['HealingProviderInvoker|FakeInvoker|DefaultInvoker'], 'REQ-HEALX-002': ['InvocationRecord.*tier.*model_id'], 'REQ-DPO-001': ['rlhf_optimizer.*clamp|DPO.*0\\.1.*2\\.0'], 'REQ-DPO-002': ['DPOPair|dpo.*proposal|dpo.*ChangePackage|dpo_pair_generator'], 'REQ-MEMX-001': ['episodic.*memory|EpisodicMemory|memory.*proposal'], 'REQ-MEMX-003': ['memory.*append|memory.*immutable|ChangePackage.*memory|episodic.*version'], 'REQ-WLD-001': ['citation.*byte|CitationBundle.*verify|byte.*hash.*citation|citation_hash'], 'REQ-WLD-002': ['ghost.*mutation|mutation.*detect|GhostMutation|ghost_mutation_detector']}
    if rid in SPECIFIC:
        return SPECIFIC[rid]
    DOMAIN = {'Layer Sovereignty': ['mutation_prohibition|layer.*sovereign|L[0-6].*Base'], 'Gateway': ['SovereignLLMGateway|network_egress_guard|provider_substitution'], 'META-INVARIANT': ['fail.closed|SovereigntyError|no.*silent.*fallback'], 'Canonicalization': ['canonical_json|sort_keys|HMAC'], 'Packet': ['InstructionPacket|instruction_packet'], 'Replay': ['replay_guard|replay_envelope|replay_validator|replay_key'], 'Envelope': ['SandboxEnvelope|sandbox_envelope'], 'Budget': ['budget_enforcer|ToolBudget'], 'Tools': ['ToolCall|ToolResult|ToolTranscript'], 'Mutation': ['UniversalWriteGateway|write_gateway|ToolNotAllowedError'], 'Artifact': ['ExecutionTrace|execution_trace'], 'Determinism': ['determinism_guard|digest_calculator|determinism\\.py'], 'Determinism Canon': ['semantic_clock|SemanticClock|uuid4.*forbidden'], 'Healing': ['healing_tier_router|healing_tier_dispatcher|escalation_context'], 'Embeddings': ['embedding_factory|embedding_input_guard|embedding_sovereignty'], 'RAG': ['SeedEmbeddingPack|matrix_hash|EmbeddingResult'], 'RAG Custody': ['CitationBundle|RetrievalQuery|citation_enforcement'], 'Meta-Learning': ['ChangePackage|proposal_only|VersionStore|meta_learning'], 'Guardian': ['guardrail|guardian.*guard|HARD.*STOP'], 'HIL': ['HumanDecisionArtifact|reviewer_sig|MODIFY_DIFF'], 'Incident': ['CognitiveDiffBundle|ForensicTraceBuffer'], 'Vigilance': ['TieredVigilance|EmergencyFreeze'], 'Prompt Governance': ['prompt_governance|prompt_assembler|PromptBundleArtifact'], 'Prompt Taxonomy': ['prompt_governance|prompt_assembler|slot_order|PromptBundleArtifact'], 'Auth': ['capability_chokepoint|capability_token'], 'Kill-Switch': ['EMBEDDING_ENABLED|ApprovalGate|tiering_allowlist'], 'Sovereignty': ['mutation_prohibition|SovereigntyError|eval.*exec'], 'Governance': ['governance_contracts|SovereigntyError|boundary.*validation'], 'Seam': ['safety_enforcement_seam|seam.*allowlist|orchestration_protocols'], 'CI': ['guardian.*tests\\.yml|ssot.*verify\\.yml|sovereignty.*enforcement\\.yml'], 'CI Ratchet': ['guardian.*tests\\.yml|ssot.*enforcement\\.yml|determinism.*guard\\.yml'], 'Boundary': ['boundary_verifier|health.*check'], 'Discovery': ['agent_discovery|integrity_hash|ZOMBIE'], 'Trace': ['ExecutionTrace|HashChainAuditLog|hash_chain_audit'], 'Evidence': ['EvidencePack|evidence.*bundle'], 'Override': ['PolicyUpdateProposal'], 'Surgical': ['SurgicalManifest|SurgicalHealingAdapter|node_id.*blueprint'], 'SSOT': ['ssot_guardrail|ssot_structure_validation|structure_blueprint'], 'SSOT Enforcement': ['ssot_guardrail|ssot_structure_validation'], 'Capability Tokens': ['capability_token|CapabilityToken'], 'Side-Effect Registry': ['tool_policy_enforcer|write_set_enforcer|side.effect'], 'Promotion State': ['promotion_authority|phase_lock_store|activation_flags'], 'Emergency Freeze': ['EmergencyFreeze|freeze_state'], 'Artifact Legality': ['artifact.*emission|flow.*violation|artifact.*type.*registry'], 'Sovereignty Matrix': ['sovereignty.*matrix|layer.*permission'], 'Phase Lock': ['phase_lock_store|phase.*gate'], 'TraceID Canon': ['trace_id_generator|CC3AL1|TraceID'], 'Canonical Hashing': ['canonical_hash|SHA.256|canonicalize'], 'HMAC Custody': ['key_derivation|key_source|hmac'], 'Signature Enclave': ['signature_verifier|SignatureEnclave'], 'Semantic Clock': ['semantic_clock|SemanticClock|vector.*clock'], 'Knowledge Supervisor': ['knowledge_integrity|retraining|KnowledgeAudit'], 'Guardian Meta': ['guardian.*tests|invariant.*coverage'], 'L0 Seam': ['importlib.*seam|seam.*allowlist'], 'Incident Telemetry': ['TieredVigilance|telemetry_recorder|ForensicTraceBuffer'], 'Cognitive Diff': ['cognitive_diff|CognitiveDiff'], 'Boundary Snapshot': ['filesystem_hash|git_state_hash|snapshot'], 'Budget Routing': ['budget_enforcer|RouteRecovery|BudgetGuard'], 'Law Slot Handler': ['LawSlotHandler|capability.*depletion'], 'MRO Integrity': ['mro_signature|verify_mro|MRO'], 'Structure Blueprint': ['structure_blueprint|blueprint.*hash'], 'Structural Lock': ['structure_blueprint|dynamic.*class.*injection'], 'Quorum Governance': ['quorum|N.of.M.*signature'], 'Rollback Integrity': ['rollback|RollbackArtifact'], 'Audit Completeness': ['WaveAuditSummary'], 'Human Override': ['HumanDecision|reviewer_sig|override.*TTL'], 'Policy Exception': ['PolicyException|exception.*scope'], 'Artifact Registry': ['ArtifactID|artifact.*registry'], 'Drift Escalation': ['DriftEscalation|drift.*escalat'], 'Cross-Wave Integrity': ['prev_wave_hash|cross.*wave'], 'Provider Binding Determinism': ['provider_binding_determinism'], 'Network Egress Guard': ['network_egress_guard'], 'Provider Substitution Prohibition': ['provider_substitution_prohibition'], 'CRITICAL Dual Enforcement Guarantee': ['critical_dual_enforcement'], 'Dynamic Runtime Mutation Prohibition': ['runtime_mutation_guard'], 'Embedding Utilization': ['embedding_factory|embedder.*metadata'], 'Agentic RAG Schema': ['RetrievalQuery|CitationBundle|citation_enforcement'], 'Context Control': ['context.*budget|PreGuardSnapshot|context_curator'], 'Application Boundary': ['apps_taxonomy_guard|apps_.*MUST NOT'], 'Telemetry': ['telemetry_recorder|INCIDENT.*telemetry'], 'Policy/HIL': ['HumanDecision|PolicyException|reviewer_sig'], 'Shared Memory': ['episodic.*memory|proposal_only.*memory|memory.*collision'], 'World-Check': ['byte_sha256|ghost.*mutation|context_set_hash'], 'DPO/RLHF Bounds': ['rlhf_optimizer|dpo_pair_generator|DPOPair'], 'Cognitive Safety': ['PolicyAlignmentCheck|PolicyViolationArtifact'], 'Healing Seam': ['HealingProviderInvoker|FakeInvoker|DefaultInvoker']}
    if domain in DOMAIN:
...
def classify(req, cmd_desc, raw_lines, match_count, matched_files, ast_index=None, ci_index=None):
    ...
    prov = {'layers_present': {}, 'ast_predicates': {}, 'file_categories': {'prod': [], 'test': [], 'tool': [], 'ci_test': []}, 'filtered_self_refs': len(self_refs), 'ast_records_count': 0}
    if match_count == 0:
    ...
    prov['layers_present'] = {'Runtime': enf_hit or has_core, 'AST': scan_hit or has_core, 'Schema': schema_hit, 'Replay': replay_hit, 'Signature': sig_hit, 'CI': ci_hit}
...
def main():
    ...
    cmd, stdout, stderr, rc = run_cmd([PY, '-c', "f=open('docs/reports/plans/Agentic Master Requirements.md','r',encoding='utf-8');lines=f.readlines(); f.close();c=sum(1 for l in lines if l.strip().startswith('| REQ-'));print('REQ_TABLE_ROWS:', c)"])
    w('```')
    w(f'$ {cmd}')
    w(stdout.strip())
    w(f'EXIT CODE: {rc}')
    ...
    cmd, stdout, stderr, rc = run_cmd([PY, '-c', "import os; items=sorted(os.listdir('.')); [print(f'  {i}/' if os.path.isdir(i) else f'  {i}') for i in items if not i.startswith('.')]"])
    w('```')
    w(f'$ {cmd}')
    w(stdout.strip())
    w(f'EXIT CODE: {rc}')
    ...
    cmd, stdout, stderr, rc = run_cmd([PY, '-c', "import os; wf='.github/workflows'; fs=sorted(os.listdir(wf)) if os.path.isdir(wf) else []; print(f'count: {len(fs)}'); [print(f'  {f}') for f in fs]"])
    w('```')
    w(f'$ {cmd}')
    w(stdout.strip())
    w(f'EXIT CODE: {rc}')
    ...
    surfaces = ['agentic_core/L2_execution/enforcement', 'agentic_core/L5_safety/enforcement', 'agentic_core/L0_routing/enforcement', 'agentic_core/L4_state/enforcement', 'agentic_core/prompt_governance', 'agentic_core/embeddings', 'agentic_core/replay', 'agentic_core/seams', 'agentic_core/security', 'agentic_core/L2_execution/determinism', 'agentic_core/L2_execution/healers']
    w('```')
    for sf in surfaces:
        full = REPO / sf
        if full.exists():
    ...
    for i, req in enumerate(rows):
        ...
        status, missing, summary, prov = classify(req, '', [], all_match_count, all_files, ast_index=ast_idx, ci_index=ci_idx)
        status_ctr[status] += 1
        ...
        w(f"| {rid} | {severity} | {status} | {(missing if missing else '—')} | {all_match_count} | {summary} |")
        det = []
        det.append(f'### {rid} ({severity}) — {status}')
        det.append(f"**Domain:** {req['domain']}")
        det.append(f"**Requirement:** {req['requirement'][:200]}{('...' if len(req['requirement']) > 200 else '')}")
    ...
    w()
    crit_deep = [('REQ-037', 'Negative control XFAIL(strict=True)', ['xfail.*strict.*True', 'negative.*control', 'tamper.*vector']), ('REQ-011', 'All LLM calls via SovereignLLMGateway', ['SovereignLLMGateway']), ('REQ-016', 'Fail-closed no silent fallback', ['fail.closed', 'SovereigntyError', 'silent.*fallback']), ('REQ-045', 'Embeddings C0 informational only', ['embedding_input_guard', 'embedding_sovereignty_guard']), ('REQ-093', 'Prompt governance chokepoint', ['prompt_assembler', 'prompt_governance.*validator']), ('REQ-114', 'No wall-clock in determinism paths', ['datetime\\.now', 'time\\.time\\(\\)', 'uuid4']), ('REQ-131', 'CI fail on CRITICAL violation by Req ID', ['CI.*fail.*CRITICAL|Req.*ID.*failure.*list']), ('REQ-243', 'WaveAuditSummary emitted per wave', ['WaveAuditSummary']), ('REQ-COG-001', 'PolicyAlignmentCheck before response', ['PolicyAlignmentCheck', 'PolicyViolationArtifact']), ('REQ-PT-011', 'Negative control for slot order tamper', ['xfail.*strict.*True.*slot', 'slot.*order.*tamper'])]
    ...
    for pat_label, pat in [('datetime.now in agentic_core/', 'datetime\\.now'), ('time.time() in agentic_core/', 'time\\.time\\(\\)'), ('uuid4 in agentic_core/', 'uuid4')]:
