...
def find_existing_lifecycle_import_line(lines: list[str]) -> int | None:
    ...
    Returns 0-indexed line number of the 'from agentic_core.L_CONTRACTS.lifecycle_trace_contract import'
    line, or None.
    """
...
def wire_module(module_rel: str) -> None:
    ...
    if existing_import_idx is not None:
        ...
        if len(symbols_to_import) == 1:
            import_block.append(
                f"from agentic_core.L_CONTRACTS.lifecycle_trace_contract import {symbols_to_import[0]}"
            )
        else:
            import_block.append("from agentic_core.L_CONTRACTS.lifecycle_trace_contract import (")
            for sym in symbols_to_import:
