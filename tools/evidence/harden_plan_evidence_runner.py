...
def run_command(argv: list[str], cwd: Path) -> tuple[str, int]:
    ...
    try:
        result = subprocess.run(argv, cwd=cwd, shell=False, encoding='utf-8', errors='replace', capture_output=True, timeout=DEFAULT_TIMEOUT)
...
def main():
    ...
    evidence_lines.append('')
    output, exit_code = run_command(['git', 'show', '--name-only', '--pretty=format:', code_commit], repo_root)
    evidence_lines.append('```')
