...
def execute_canonical_payload_proofs():
    ...
    assert canonical_payload_hash_1 == canonical_payload_hash_2, (
        "PASS: canonical_payload_hash must be identical"
    )

    payload_digest_deterministic = (
        replay_hash_1 == replay_hash_2 and canonical_payload_hash_1 == canonical_payload_hash_2
    )
    ...
    drift_detected = (
        mutated_replay_hash != original_replay_hash and mutated_canonical_hash != original_canonical_hash
    )
    ...
    assert disabled_report["canonical_payload_hash"] == test_canonical_hash, (
        "NEGATIVE: Canonical hash must be accessible"
    )
    ...
    print(
        f"  canonical_payload_accessible={disabled_report['canonical_payload_hash'] == test_canonical_hash}"
    )
...
def main():
    ...
    test_targets.append(
        ["python", "-m", "pytest", "-q", "tests/unit_min_deps/test_vllm_canonical_payload_lock.py"]
    )
