"""Fort Knox v2 — CI Gate Binding Report Emitter (runtime-acceptance subject).

Companion to emit_ci_gate_binding_report.py. Statically proves that
`scripts/verify_runtime_certification_acceptance.py` is registered in at
least one fail-closed GitHub Actions workflow and emits a per_req
binding payload for RTC-REQ-034 (the row whose evidence chain depends
on that verifier).
...
this emitter as the source of its ci_gate evidence.

Logic mirrors the CSV-gate emitter; the only constants that differ are
SUBJECT_REL, OUTPUT_REL, and COVERED_REQS.
"""
...
OUTPUT_REL = "artifacts/certification/ci_gate_binding_report_runtime_acceptance.json"
...
def main() -> int:
    ...
    if not WORKFLOWS_DIR.exists():
        print(f"[emit_ci_gate_binding_report_runtime_acceptance] FAIL: workflows dir missing: {WORKFLOWS_DIR}",
              file=sys.stderr)
        return 1
    ...
    report = {
        ...
        "per_req": per_req,
        "generated_at_utc": now,
        "generated_by_command": "tools/cert/emit_ci_gate_binding_report_runtime_acceptance.py",
    }
    ...
    print(f"[emit_ci_gate_binding_report_runtime_acceptance] subject={SUBJECT_REL}")
