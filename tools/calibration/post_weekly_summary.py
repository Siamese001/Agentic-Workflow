...
This module produces the JSON shape Codex pastes into a Notion `API-post-page`
...
deliberate, because:
  ...
  1. MCP serialization rules (constitutional §25) require Notion calls to be
     ...
     Codex picks it up next session and dispatches it as the sole tool call.

  2. The auto-router is documented in AGENTS.md "Auto-Routing Rules" — when
     the weekly report changes, Codex should post the dashboard summary to
     the MCP Registry Notes field on the affected ledger rows. This script
     shapes the payload; the routing decision belongs to Codex.
