    ...
    def _generate_next_steps(self, status, metrics):
        ...
        if status['overall_complete']:
            ...
            if metrics['silent_swallowers']['remaining_high'] > 0:
                steps.append(f"Fix {metrics['silent_swallowers']['remaining_high']} remaining HIGH severity silent swallower violations")
            if metrics['test_enforcement']['remaining_high'] > 0:
                steps.append(f"Fix {metrics['test_enforcement']['remaining_high']} remaining HIGH severity test violations")
            if metrics['silent_swallowers']['remaining_medium'] > 0:
                steps.append(f"Fix {metrics['silent_swallowers']['remaining_medium']} MEDIUM severity violations")
...
def main():
    ...
    if not report['completion_status']['overall_complete']:
        ...
        if 'silent_swallowers' in remaining:
            ...
            print(f"   Silent swallowers: {ss['high_remaining']} HIGH, {ss['medium_remaining']} MEDIUM, {ss['low_remaining']} LOW")
