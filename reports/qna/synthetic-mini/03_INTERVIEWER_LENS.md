...
## Purpose
This card encodes who Jane Doe is and how to answer in a way that lands with their bias. It does not answer questions on its own; it tunes other route answers.
...
## Lens
Trust and platform reuse over feature velocity.

## Public signals
- Spoke at AI Platform Summit 2025 on governed agents.


## Hot buttons (topics they will probe hard)
- LLM as a Judge
- Text-to-SQL safety
- MLflow lifecycle


## Tuning rules for answers
- When answering Jane Doe, lead with the trust and platform reuse over feature velocity..
- Skip introductory framing — they want the technical core in the first sentence.
- Always name a specific control point or artifact (registry, contract, gate, eval).
- It is acceptable to say "I would have to look that up" rather than improvise on a stack they know cold.


## What to avoid with this interviewer
- Do not bluff on `LLM as a Judge` — they will catch it.
- Do not bluff on `Text-to-SQL safety` — they will catch it.
- Do not bluff on `MLflow lifecycle` — they will catch it.


## When this card applies
- All routes when Jane Doe is the speaker (or the panel lead).
- For panel mode with multiple interviewers, the runtime hint `lens: jane_doe` activates this card for the next answer.
