...
def test_openai_client() -> dict[str, object]:
    ...
    results = {"provider": "OpenAI", TESTS_DIR: {}, "overall": False}

    if not os.getenv("OPENAI_API_KEY"):
        results[TESTS_DIR]["initialization"] = {"passed": False, "error": "OPENAI_API_KEY not set"}
        return results
    ...
    try:
        ...
        response = client.chat_completion(
            [{"role": "user", "content": "Say 'OpenAI test passed' in 5 words"}], max_tokens=10
        )
        ...
        structured = client.structured_completion(
            [{"role": "user", "content": "Return status as 'ok' and provider as 'openai'"}], schema
        )
        ...
        # Test streaming
        chunks = client.stream_completion([{"role": "user", "content": "Count from 1 to 3"}])

        results[TESTS_DIR]["streaming"] = {"passed": len(chunks) > 0, "chunks": len(chunks)}
...
def test_anthropic_client() -> dict[str, object]:
    ...
    results = {"provider": "Anthropic", TESTS_DIR: {}, "overall": False}
    ...
    try:
        ...
        tool_result = client.tool_use_message(
            [
                {
                    "role": "user",
                    "content": [{"type": "text", "text": "Use test_tool with message 'hello'"}],
                }
            ],
            tools,
        )
...
def test_vertex_client() -> dict[str, object]:
    ...
    results = {"provider": "Google Vertex", TESTS_DIR: {}, "overall": False}
    ...
    try:
        ...
        response = client.generate_content("Say 'Vertex test passed' in 5 words", max_tokens=10)
        ...
        grounded = client.grounded_response("What is Google Vertex AI?", grounding_threshold=0.7)

        results[TESTS_DIR]["grounded_response"] = {
            "passed": len(grounded["content"]) > 0,
            "has_grounding": grounded["grounding_metadata"] is not None,
            "grounding_score": grounded["grounding_metadata"]["grounding_score"]
            if grounded["grounding_metadata"]
            else None,
        }

        # Test safe response
        safe = client.safe_response("Write a professional greeting", safety_threshold="BLOCK_NONE")
...
def test_multi_provider_router() -> dict[str, object]:
    ...
    try:
        ...
        failover_result = router.chat_completion(
            [{"role": "user", "content": "Simple test message"}], strategy="round_robin"
        )
        ...
        structured_result = router.structured_completion(
            [{"role": "user", "content": "Return status as 'ok'"}], schema
        )
...
def test_reference_clients() -> dict[str, object]:
    ...
    results = {"provider": "Reference Clients", TESTS_DIR: {}, "overall": False}
    ...
    try:
        ...
        results["overall"] = all(test.get("passed", False) for test in results[TESTS_DIR].values())
        ...
        try:
            ...
            for test_name, test_result in result[TESTS_DIR].items():
                if "error" in test_result and not test_result.get("passed", True):
                    logger.error(
                        f"    Failed: {test_name} - {test_result.get('error', 'Unknown error')}"
                    )
