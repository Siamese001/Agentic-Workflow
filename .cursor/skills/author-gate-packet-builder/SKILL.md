...
> ⛔ **Pipeline Completion Invariant**: the emitted `AUTHOR_GATE_PACKET:` block **MUST** be followed by `ask_user_question` **in the same Cursor Agent response**. Emitting the packet and ending the response without `ask_user_question` is a critical violation. Enforcement: `post_cursor_agent_author_gate_pipeline_audit.py`. See plan `author-gate-ui-renderer-hardening-a7f3c2`.
...
}' | python .cursor/skills/author-gate-packet-builder/emit_packet.py
```
...
echo '{"decision_type": "refactor_scope", "normalized_intent": "..."}' | \
  python .cursor/skills/author-gate-packet-builder/precedent_injector.py
```
