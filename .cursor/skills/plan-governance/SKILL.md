...
> **Invariants (glob/on-demand rules):** `plan-location.mdc` (path, format floor, Notion status discipline), `plan-update-enforcement.mdc` (authorization invariant), `notion-plans-taxonomy.mdc` (status taxonomy).  
...
See `notion-plan-identity-verification.mdc` for the invariant; use `tools/notion/` helpers where available.
