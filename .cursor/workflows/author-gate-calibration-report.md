---
description: Generate weekly Author-Gate calibration report — firing rate, FP rate, flip-readiness. Invokes `.cursor/scripts/generate_calibration_report.py` and writes `docs/reports/author-gate/<YYYY-Www>.md`.
---
...
python .cursor/scripts/generate_calibration_report.py

# Previous week
python .cursor/scripts/generate_calibration_report.py --week-offset 1

# Print only, don't write
python .cursor/scripts/generate_calibration_report.py --no-write
```
...
- `.cursor/state/refactor_decisions/refactor_decision_ledger.sqlite` — decisions + outcomes
- `artifacts/cursor/author_gate_violations.jsonl` — gate events (shadow warnings and blocks)
...
1. Run: `python .cursor/scripts/generate_calibration_report.py --week-offset 1`
2. Read Flip Readiness → if **GO**, proceed; else stop and tune
3. Edit `.cursor/schemas/author_gate_triggers.yaml` → `enforcement: block`
4. Run: `python .cursor/scripts/pre_author_gate.py --self-test`
...
- Schema: `.cursor/schemas/author_gate_triggers.yaml` (`shadow_launch_date`, `shadow_min_days`)
- Rule: `.cursor/rules/author-gate-enforcement.md`
- Plan: `.cursor/plans/harness-enforcement-rename-a8f21c.md` (W3 shadow mode)
