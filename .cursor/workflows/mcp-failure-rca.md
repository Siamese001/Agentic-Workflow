...
> **Load-order note:** The numeric tool prefixes Cursor assigns (e.g. `mcp8_redis_health`) shift whenever a server is added or removed from `mcp.json`. Use the server names above as the stable identifiers; resolve the live prefix from the tool list visible in your session.
...
Then restart the MCP server by asking Cursor to reload MCP connections (Cmd/Ctrl+Shift+P → "Reload MCP"). Re-run `adg_health` — if ok, return to original prompt.
...
```
Restart the ADG MCP server in Cursor (Ctrl+Shift+P → Reload MCP).
...
- `True` → Redis is up, MCP config is broken → check `.cursor/mcp.json` redis entry
...
| Any npx-based MCP hangs (filesystem, memory, deepwiki, brave) | Same root cause — `npx` vs `npx.cmd` on Windows | Restart MCP in Cursor |
...
- Skill: `.cursor/skills/structured-reasoning/SKILL.md`
...
**C) Restart MCP in Cursor:** Ctrl+Shift+P → "Reload MCP".
