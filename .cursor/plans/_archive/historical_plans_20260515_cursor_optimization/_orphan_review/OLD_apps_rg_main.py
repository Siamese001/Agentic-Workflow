...
is unavailable ΓÇö never blocks execution on ADG failure.
"""
...
def _adg_bootstrap() -> None:
    try:
        ...
        if report.route_mode == "HUMAN_REVIEW":
            _log.error("[ADG] route_mode=HUMAN_REVIEW ΓÇö manual review required before dispatch")
            sys.exit(1)
...
def _apps_rg_emission_config(
    ...
    Uses the shared ``apps_shared.spine_emission`` helper ΓÇö the same one
...
def main() -> None:
    ...
    # L0 INTAKE ΓÇö 3 required inputs per apps_rg contract
    # 1. Company (target_company)
    # 2. JD ΓÇö Job Description JSON file path
    # 3. Research Briefing ΓÇö if missing, route to apps_research; if apps_research fails, apps_rg fails
    ...
    if not args.target_role:
        try:
            ...
            if not args.target_role:
                args.target_role = input("JD has no 'title' field ΓÇö enter target role: ").strip()
                if not args.target_role:
                    parser.error("target role is required (from JD.title or prompt)")
            else:
                print(f"       ΓåÆ role from JD: {args.target_role}", file=sys.stderr)
    ...
    if jd_path != jd_path_default:
        ...
        print(f"       ΓåÆ copied JD to canonical path: {jd_path_default}", file=sys.stderr)

    # 3. Research Briefing ΓÇö check prerequisite, route to apps_research if missing
    ...
    if brief_exists:
        ...
        if choice in ("", "y", "yes"):
            print(f"       ΓåÆ Invoking apps_research for company='{args.target_company}'...", file=sys.stderr)
            ...
            try:
                ...
                if result.returncode != 0 or not Path(brief_path).exists():
                    print(f"ERROR: apps_research failed (exit {result.returncode}) ΓÇö apps_rg cannot proceed", file=sys.stderr)
                    sys.exit(1)
                print(f"       ΓåÆ apps_research succeeded; briefing at {brief_path}", file=sys.stderr)
                args.manual_brief = brief_path
            except _subprocess.TimeoutExpired:
                print("ERROR: apps_research timed out after 5 minutes ΓÇö apps_rg fails", file=sys.stderr)
                sys.exit(1)
            except Exception as exc:
                print(f"ERROR: apps_research invocation failed: {exc} ΓÇö apps_rg fails", file=sys.stderr)
                sys.exit(1)
        else:
        ...
        with gr.span("apps_rg.entrypoint"):
            ...
            if not args.skip_r1b_check and args.target_company and args.target_role:
                with gr.span("L0.r1b_cache_check"):
                    ...
                    if r1b_hit:
                        # R1B hit ΓÇö terminal return with cached output
                        _log.info("[apps_rg] R1B cache hit ΓÇö terminal return")
                        ...
                        # Terminal ΓÇö skip L2 execution
                ...
                with gr.span("L0.briefing_prerequisite"):
                    ...
                    if not briefing_check.is_valid:
                        if briefing_check.requires_apps_research:
                            ...
                            _log.info(
                                "[apps_rg] Briefing prerequisite not met (%s) ΓÇö "
                            ...
                            # Continue to L2 ΓÇö apps_research will be invoked
                            # as part of the managed workflow or narrative pass
                        else:
                            # Fail closed ΓÇö cannot proceed
                            _log.error(
                                "[apps_rg] Briefing prerequisite failed (%s: %s) ΓÇö "
                                "failing closed",
                                briefing_check.result.value,
            ...
            # W2.P1 of plan apps-runtime-domain-enforcement-a7e9d4 ΓÇö
    ...
    try:
        ...
        if status == "HUMAN_REVIEW":
            ...
            if provenance.get("valid") is False:
                ...
                # HUMAN_REVIEW for other reasons ΓÇö still mark but log generically
...
def _run_post_pipeline(args) -> int:
    ...
    if not input_resume.exists():
        _log.error("[apps_rg] %s missing ΓÇö narrative pass aborted", input_resume)
        return 1
    ...
    if result.returncode != 0:
        _log.error("[apps_rg] narrative pass exit=%s ΓÇö DOCX export skipped", result.returncode)
