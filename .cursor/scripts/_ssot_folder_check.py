...
def _check_scripts_root(rel: str) -> Violation | None:
    ...
    return Violation(
        ...
        message=(
            ...
            f"scripts/proof/. See .cursor/rules/ssot-folder-enforcement.md."
        ),
    )
...
def _check_repo_root_py(rel: str) -> Violation | None:
    ...
    return Violation(
        ...
        message=(
            ...
            f"Move to {suggested}. "
            f"See .cursor/rules/ssot-folder-enforcement.md."
        ),
    )
        ...
        if rel.endswith(".py"):
            ...
            return Violation(
                ...
                message=(
                    ...
                    f"{suggested}. See .cursor/rules/ssot-folder-enforcement.md."
                ),
            )
    return None
...
def _check_hook_script_misroute(rel: str) -> Violation | None:
    """Hook-prefix file landing outside .cursor/scripts/."""
    if not rel.endswith(".py"):
        return None
    if rel.startswith(".cursor/scripts/"):
        return None
    filename = PurePosixPath(rel).name
    if re.match(r"^(pre|post)_(read|write|run|user_prompt|mcp_tool_use|"
        ...
        return Violation(
            ...
            suggested=".cursor/scripts/",
            message=(
                f"Filename '{filename}' looks like a Cursor hook script "
                f"(pre_*/post_* prefix) but is being written outside "
                f".cursor/scripts/. Hook scripts MUST live in "
                f".cursor/scripts/ to be discoverable by .cursor/hooks.json. "
                f"See .cursor/rules/ssot-folder-enforcement.md."
            ),
        )
    return None
