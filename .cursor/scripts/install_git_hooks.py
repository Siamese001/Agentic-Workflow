...
Currently installs:
    - post-commit: invokes .cursor/scripts/post_commit_outcome_binder.py --head

Idempotent: re-running refreshes the hook to latest content.

Usage:
    python .cursor/scripts/install_git_hooks.py           # install
    python .cursor/scripts/install_git_hooks.py --check   # verify installed
    python .cursor/scripts/install_git_hooks.py --uninstall
...
{MARKER}
# Installed by .cursor/scripts/install_git_hooks.py
...
set +e
python .cursor/scripts/post_commit_outcome_binder.py --head >/dev/null 2>&1 &
