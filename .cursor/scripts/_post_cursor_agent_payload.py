"""Shared payload extractor for post_cursor_agent_response hooks.
...
This helper is the SSOT extractor. All post_cursor_agent_*.py hooks MUST call
``extract_response_text(sys.stdin.read())`` rather than rolling their own
parser. A CI gate (`ops_scripts/ci/check_post_cursor_agent_payload.py`) blocks
any post_cursor_agent_*.py that reimplements the logic.
"""
