...
This script is an exact analog of ``.cursor/scripts/defer.py`` for the
...
-----
    ...
    python .cursor/scripts/capture_author_gate.py "DECISION_CAPTURED: type=refactor_scope, repo_area=.cursor, selected=W1, outcome=executed, confidence=0.92, gap=0.18"

    # From a file (one or more markers, one per line):
    python .cursor/scripts/capture_author_gate.py --file markers.txt

    # From stdin:
    echo "DECISION_CAPTURED: ..." | python .cursor/scripts/capture_author_gate.py --stdin
...
HOOK_SCRIPT = REPO_ROOT / ".cursor" / "scripts" / "post_cursor_agent_author_gate_capture.py"
