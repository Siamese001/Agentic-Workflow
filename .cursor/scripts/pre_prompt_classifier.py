...
[pre_prompt_classifier] STRUCTURED REASONING REQUIRED ({tier}):
  ...
  Rule: .cursor/rules/sequential-thinking-enforcement.md
...
def check_plan_exists(tier: str) -> bool:
    """Return True if a plan file exists in .cursor/plans/ for T2/T3."""
    ...
    if tier in ("T2", "T3"):
        if not check_plan_exists(tier):
            print(
                ...
                "in .cursor/plans/ — consider creating a plan per constitutional §10.",
                file=sys.stderr,
            )
