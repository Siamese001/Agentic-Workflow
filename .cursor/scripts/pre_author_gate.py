...
features of the pending action against .cursor/schemas/author_gate_triggers.yaml.
...
Fresh invocation modes:
    python .cursor/scripts/pre_author_gate.py                 # hook mode
    python .cursor/scripts/pre_author_gate.py --self-test     # validate triggers.yaml
    python .cursor/scripts/pre_author_gate.py --dry-run       # evaluate without exiting 2
    python .cursor/scripts/pre_author_gate.py --reset-session # clear denial counters
...
TRIGGERS_PATH = REPO_ROOT / ".cursor" / "schemas" / "author_gate_triggers.yaml"
...
LOOKUP_SKILL_PATH = REPO_ROOT / ".cursor" / "skills" / "refactor-decision-memory" / "lookup_refactor_decisions.py"
...
def _is_sensitive_path(file_path: str) -> bool:
    ...
    for pattern in SENSITIVE_PATH_PATTERNS:
        ...
        if normalized.startswith(pattern) or fnmatch.fnmatch(normalized, pattern):
            return True
        # W2: Handle absolute Windows paths (e.g., C:/path/.cursor/rules/...)
        ...
        if pattern_no_slash in normalized:
            ...
            # e.g., ".cursor/rules" should match "/path/.cursor/rules/file"
            # but not "/path/x.cursor/rules" (partial match)
