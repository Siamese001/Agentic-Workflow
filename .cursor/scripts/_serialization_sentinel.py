...
``mcp-serialization.md``). Designed to run inside Cursor pre-* gate hooks,
...
Bypass: ``MCP_SERIAL_BYPASS=1`` (matches ``post_cursor_agent_mcp_serialization_audit.py``).

Sunset: honors ``.cursor/config/mcp_serialization_ttl.json`` ``retired_after``
date — same TTL contract as the post-cursor_agent audit hook.
"""
...
_artifacts = _repo_root / "artifacts" / "cursor"
_ttl_config = _repo_root / ".cursor" / "config" / "mcp_serialization_ttl.json"
