#!/usr/bin/env python3
"""
post_cursor_agent_plan_evidence_gate.py — Stop-equivalent plan evidence gate (P2).

Reads the Cursor Agent response from stdin. When the response edited or created
any ``.cursor/plans/*.md`` file, runs the canonical graph-layer evidence
...
task produces them?" — Windsurf has no Stop hook, but post_cursor_agent_response
...
_PLANS_DIR = _ROOT / ".windsurf" / "plans"
