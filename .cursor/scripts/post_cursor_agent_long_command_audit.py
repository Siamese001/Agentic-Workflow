...
``artifacts/cursor/long_command_violations.jsonl`` so session-level
timeout discipline becomes measurable even though the tool schema has no
native ``timeout`` field.

Companion rule: ``.cursor/rules/constitutional.md`` §14 (subprocess
timeout) + §11 (terminal lifecycle). Workflow:
``.cursor/workflows/timeout-progress-enforcement.md``.
"""
...
def main() -> int:
    ...
    if violations:
        ...
        print(
            ...
            f"artifacts/cursor/long_command_violations.jsonl",
            file=sys.stderr,
        )
