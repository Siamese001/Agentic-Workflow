"""post_cursor_agent_read_budget_audit.py - advisory read-budget cap.
...
`post_cursor_agent_grep_budget_audit.py` exactly in shape.
...
This exists because:
  ...
  3. Advisory-only: this hook NEVER blocks; it logs. Deterministic-but-soft,
     mirroring `post_cursor_agent_grep_budget_audit.py`.
...
Violations log: `artifacts/cursor/read_budget_violations.jsonl`
"""
...
def _read_response_text() -> str:
    ...
    Same shape as post_cursor_agent_grep_budget_audit.py.
    """
    try:
