#!/usr/bin/env python3
"""
post_cursor_agent_writeback_audit.py — Windsurf post_cursor_agent_response writeback discipline audit.

Reads the cursor agent response payload from stdin. Detects signals indicating a
...
Policy SSOT: .cursor/rules/memory-notion-writeback.md
Entity/row templates: .cursor/skills/writeback-discipline/
...
Detection signals (any one fires → writeback due):
    ...
    3. Response mentions editing `.cursor/scripts/*_gate.py`
    4. Response resolved a scored `ask_user_question` (DECISION_CAPTURED marker)
    5. Response mentions creating/editing `.cursor/plans/*-<6hex>.md`
...
SIGNALS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    (
        ...
        re.compile(r".cursor[/\\]scripts[/\\][a-z_]+_gate\.py", re.IGNORECASE),
...
def main() -> int:
    ...
    try:
        ...
        emit_ledger_event(
            ...
            repo_area=".cursor/scripts/post_cursor_agent_writeback_audit.py",
        )
