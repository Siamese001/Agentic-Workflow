...
+-------------------------------------------------------------+                                                                                v                                        |
                     ...
                     | [ FINAL ANSWER ] |                                                                  +-------------------------------------------------------------------------+ (Insufficient / Re-plan)
                     +------------------+                                                                                |
                                                                                                                         | (Sufficient Info)
                                                                                                                         v
                                                                                                           +-------------------------------------------------------------------------+
...
========================================================================================================================================================================================================================================================
 ...
 [ L1: COGNITIVE COMPUTE ]              [ L4: MODEL & SIGNAL STATE BUS ]            [ L4: INFRASTRUCTURE ADAPTERS ]           [ EXTERNAL INFRASTRUCTURE ]            [ L1: REASONING & RE-PLANNING ]           [ L6: OBSERVABILITY ]
 (High-Inference Logic Engine)           (The Persistent Knowledge Substrate)        (The Physical I/O Surface)                (Hardware & Data Persistence)          (Final Cognitive Alignment)               (The Telemetry Engine)
+-------------------------------+      +-------------------------------------+      +-------------------------------+          +---------------------------+          +-------------------------------+         +-------------------------------+
...
| - [EMBEDDING MODEL] Inference | <--- | [READ: Embedder_ID / Weights] ----- |      | - Enforces Rate Limits        |          | - Pinecone / Milvus       |          | - [CROSS-ENCODER] Inference   | <---    | [READ: Hit Rate / Latency] -- |
...
+-------------------------------+      +-------------------------------------+      +-------------------------------+          +---------------------------+          +-------------------------------+
             |                                          |                                           |                                      |                                      ^
             | [L1 WRITES Fused Set]                    | [INFRA WRITES candidate vectors]          | [L4 WRITES fetch request]            | [L4 WRITES context bundle]           | [L1 WRITES Materialization Request]
             v                                          v                                           v                                      v                                      |
+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
========================================================================================================================================================================================================================================================
| STEP | OPERATION             | LAYER | DATA MOVEMENT (READ / WRITE)                                                                                                       | ARCHITECTURAL INVARIANT                                 |
|------|-----------------------|-------|------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------|
| 1    | Query Encoding        | L1    | L1 READS Model Weights from L4; L1 WRITES query_vector to context.                                                                 | L1 performs all high-inference compute.       |
| 2    | Gateway Validation    | L4    | L4 READS Auth/Budgets; L4 WRITES authorized fetch request to Infrastructure.                                                       | L4 Gateway enforces authority boundaries.     |
| 3    | Raw Search            | INFRA | INFRA READS search intent; INFRA WRITES raw candidate vectors/IDs to L4.                                                           | Hardware provides raw data persistence.       |
| 4    | Similarity & Fusion   | L1    | L1 READS Raw Vectors & IDF Stats from L4; L1 WRITES Fused Rank Set.                                                                | L1 executes all mathematical scoring logic.   |
| 5    | High-Res Reranking    | L1    | L1 READS Cross-Encoder weights from L4; L1 WRITES Surgical Materialization Request.                                                | Reranking optimizes token efficiency in L1.    |
| 6    | Materialization       | L4    | L4 READS raw bytes from S3/SQL; L4 WRITES context_bundle.json to L1 Reasoning.                                                     | L4 abstracts physical storage complexity.     |
| 7    | Observability Loop    | L6    | L6 READS retrieval signals from L4; L6 WRITES Quality/Drift metrics into L4 for Run t+1.                                           | L6 monitors quality without blocking flow.    |
========================================================================================================================================================================================================================================================