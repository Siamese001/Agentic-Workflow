...
PHASE 2 STRICT MODE (OPTION A):
  ...
  • STRUCTURAL DIFF INVARIANT:
      ...
      - Phase 2 verifies that the SSoT subtree for TARGET ROOT and the live
        filesystem are structurally compatible.
      - If non-trivial mismatches are found, Phase 2 marks validation as failed
        and still remains read-only (no plan or a plan with "no-op" status).
...
def build_semantic_operations(
    ...
    for d in diffs:
        ...
        rel_under_root = d.live_path[len(target_root) + 1 :] if len(d.live_path) > len(target_root) + 1 else ""
