# File: run_batch_v10_3.py
# Version: 10.3 (Instructional Injection)
#
# v10.3 MAJOR CHANGES:
# - Enforced Strict Dependency Injection: Removed the global `CONFIG`
#   singleton instance.
# - Eliminated Service Locator Pattern: `run_batch()` now creates the `ConfigV10_3`
#   object and passes it to `run_batch_async`.
# - True Composition Root: `run_batch_async` now instantiates all 10+
#   core services (CacheManager, PromptTemplateManager, ResponseValidator,
#   ContextBudgetManager, etc.) ONCE and shares them.
# - Job-Specific Contexts: The parallel worker loop now correctly injects
#   the SHARED services into a new `job_context` for each job,
#   using the updated `WorkflowContext` constructor.
...
# v10.3: Import from new main/core
# v10.3: setup_logging is now in main
from main_v10_3 import setup_logging, load_job_input
...
try:
    # v10.3: Import new meta-learner
    from run_learning_v10_3 import run_meta_learning
    META_LEARNER_AVAILABLE = True
except ImportError:
    META_LEARNER_AVAILABLE = False
    logging.getLogger("batch_runner_v10_3").warning("Meta-learning module (run_learning_v10_3.py) not found.")

logger = logging.getLogger("batch_runner_v10_3")

# v10.3: DELETED the global CONFIG singleton instance.

BATCH_QUEUE_DIR = "batch_queue"
BATCH_COMPLETE_DIR = "batch_complete"
SUMMARY_FILE = "batch_summary_v10_3.csv"
...
# ============================================================================
# ROW 6: ASYNC BATCH PROCESSING ENGINE (Preserved)
# ============================================================================

class CircuitBreaker:
    """Circuit breaker for batch processing. Used by Conductors."""
...
async def run_batch_async(config: ConfigV10_3): # v10.3: Config is injected
    ...
    if not job_files:
        logger.info("v10.3 Batch process starting. No jobs found.")
        return
    
    logger.info(f"===== v10.3 Async Batch Process Starting =====")
    ...
    logger.info("Initialized shared ChromaDB client (v10.3)")
    ...
    # ... (CSV writing and summary logic preserved) ...
    
    batch_summary = batch_aggregator.get_batch_summary()
    logger.info(f"BATCH PROCESSING COMPLETE (v10.3)")
    ...
    if META_LEARNER_AVAILABLE and config.meta_loop_config.enable_meta_learning:
        logger.info("Triggering meta-learning loop (v10.3)...")
        try:
    ...
    logger.info("v10.3 Batch process complete.")
