...
2) LinkedIn QA Grid (3 columns) — RESULT GLYPH ENFORCEMENT
| # | Check                                                 | Result |
|---|-------------------------------------------------------|--------|
| 1 | Message type and path (correct)                       | ✅     |
| 2 | Region and URL validation                             | ✅     |
| 3 | Why Company present                                   | ✅     |
| 4 | Why Role or mandate present                           | ✅     |
| 5 | Two dated or attributable insights                    | ✅     |
| 6 | Quantified resume mappings                            | ✅     |
| 7 | Structure and signature enforcement                   | ✅     |
| 8 | Dash policy (AI FILTER vNext3) & percent normalization| ✅     |

- NOTE: All "Result" PASS cells must render exclusively with the green check glyph `✅`. Any textual "PASS" will cause an immediate BLOCK.
...
Short messages only: Immediately after the Outreach Message body, render one extra line: `Chars: <N>` where `<N>` is the **LinkedIn-compatible** body count (see Short (NEW) rules).
...
- **NEW v1.6 enforcement** for both lines above (NEW full message): append “My resume is attached for your convenience.” as the final sentence of the body before the ask.
...
- **Global Abbreviation Scope (NEW v1.6)**:
...
**App Tracker Field & Validation Enforcement (NEW v1.7)**
1) **Base Resume Validation (allowed set only)**  
   - Permissible selections:  
     • Chief AI Officer Resume — textdoc_id: **68b04f184ce48191bed00bbc3256f072**  
     • Professional Services AI Resume — textdoc_id: **68b05008bff0819186bdd34d0dc43d8f**  
   - Any other selection (including null/blank) → BLOCK.

2) **Date Format Enforcement (global)**  
   - All date-class fields must be strictly **MM/DD/YYYY** (zero-padded month and day).  
   - Any deviation (e.g., ISO 8601, textual months, missing zero pad) → BLOCK.

3) **Outreach Channel Enumeration (strict)**  
   - Field **Outreach Channel** must exactly match one of the enumerated values defined in the App Tracker QA Spec (resolve from MSC pointer: textdoc_id **68b7ab7aae00819182ba5f679e0034cd**).  
   - Any free-text or non-enumerated value → BLOCK.

4) **Interviewer & Recruiter Field Population (explicit only)**  
   - Populate **Hiring Recruiter** and **Interviewer** fields **only** upon explicit user instruction/confirmation.  
   - Any auto-population or inference without explicit confirmation → BLOCK.

5) **Application Date & Pipeline Status (explicit confirmation)**  
   - **Application Date** must be explicitly validated (chat history or provided docs). Never default to the communication/send date.  
   - **Pipeline Status** must reflect an explicitly validated state transition.  
   - Ambiguous or missing validation for either field → BLOCK.

**Standard BLOCK message template (v1.7 additions)**  
Return only this JSON on any of the five failures above:
{
  "status": "error",
  "failed_checks": ["specific_rule_violated"],
  "details": { "field": "<field_name>", "expected": "<explicit_requirement>", "observed": "<actual_detected>" },
  "required_correction": "Explicit instructions for correction."
}
...
- If NEW:
  ...
  • If SINGLE: default Short (NEW); if (3A)=YES, route to EXISTING shell with salutation override **and resume-attachment sentence**.
- If EXISTING:
...
5) Reasoning — MESSAGE LENGTH & ABBREVIATION RULES (NEW v1.6)
...
Then render LinkedIn QA Grid (with glyph enforcement), AI Filter Canonical Table, and the SHORT (NEW) Message-Specific RAG QA Table.
...
**NEW v1.6 — NEW full-message edge case rule:**  
...
- **Short (NEW) message audit (mandatory, v1.6 additions):**
  - final_linkedin_char_count
  - abbreviation_mappings {original:abbrev}
...
- **NEW v1.6:** For any **NEW full-message** routed into Recruiter/Contact shells, omission of the sentence  
...
===============================================================================
END OF LINKEDIN OUTREACH - CANONICAL v1.7
===============================================================================
