...
- Dash policy harmonized with AI FILTER vNext: absolute ban on dash-like characters in external-facing content, with a small, auditable **Exception Registry** (e.g., phone number in signature). Percent normalization (“percent” → “%”) preserved.
- Verb-tense enforcement added across shells: present/future for proposed contributions; past only for verified outcomes/credibility clauses.
- **Operator Prompts Enforcement — Mandatory Sequence**: Prompts 1 → 2 → 2A (if applicable) → 3 → 4 → 5 → 6 must be asked verbatim, answered, and logged in order. Any deviation or bypass causes immediate BLOCK.
...
| 8 | Dash policy (AI FILTER vNext) & percent normalization| PASS/FAIL |
...
"Is Premium InMail/full-message available to this NEW contact? Reply YES or NO."
   - If YES: choose one — Recruiter→ExistingShell or Contact→ExistingShell.
     Salutation override (required): remove any “Thanks for connecting/Thanks again for connecting” line; use a direct open (e.g., “I’m reaching out regarding the [Role Name] at [Company].”).
...
3) "Are you sending to a Single contact or Multiple contacts? Reply SINGLE or MULTIPLE."
   ...
   - MULTIPLE (EXISTING only): provide a batch envelope and K lines (see next block). NEW supports SINGLE only.
...
6) Dash Exception Registry check (auto if signature will render):
   - "Confirm Exception Registry entry for the phone number signature? YES/NO."
   - If NO: BLOCK with `dash_policy_violation` and instructions to register the exact token.

--------------------------------------------------------------------------------
OPERATOR PROMPTS ENFORCEMENT — MANDATORY SEQUENCE (Strict)
--------------------------------------------------------------------------------
Enforcement policy:
- The operator prompts above must be presented verbatim, in the exact numeric order (1 → 2 → 2A [if applicable] → 3 → 4 → 5 → 6).
- Each prompt response must be explicitly logged and validated before moving to the next.
- Any attempt to skip, reorder, or bypass prompts results in immediate BLOCK.

Prompt execution flow (MANDATORY):

(1) "Select the Message Type: Short (NEW) | Recruiter (EXISTING) | Contact-LightRAG (EXISTING) | Executive-RobustRAG (EXISTING). Confirm?"
   - Record message_type selection explicitly. BLOCK if invalid or missing.

(2) "Is this outreach for a NEW contact or an EXISTING contact? Reply NEW or EXISTING."
   - If response NEW → proceed to Prompt 2A.
   - If response EXISTING → skip directly to Prompt 3.
   - BLOCK if response not exactly "NEW" or "EXISTING".

(2A) NEW full-message eligibility check (MANDATORY EDGE CASE CHECK):
   "Is Premium InMail/full-message available to this NEW contact? Reply YES or NO."
   - YES → select one: Recruiter→ExistingShell or Contact→ExistingShell.
     Enforce salutation override strictly ("Thanks for connecting" variants strictly prohibited).
   - NO → default strictly to Short (NEW).
   - BLOCK if response not exactly "YES" or "NO".

(3) "Are you sending to a Single contact or Multiple contacts? Reply SINGLE or MULTIPLE."
   - SINGLE → proceed immediately.
   - MULTIPLE → enforce batching rules strictly (EXISTING only; K=2..5). BLOCK for NEW.
   - BLOCK if response not exactly "SINGLE" or "MULTIPLE".

(4) Role-detector confirmation (NON-BYPASSABLE CHECK):
   - Infer category from title/profile: Recruiter | Business-side Director/VP+ | Executive VP+.
   - If mismatch detected vs selected Message Type:
     Prompt verbatim: "Detected {inferred_category} vs selected {message_type}. Confirm override? YES/NO."
       - NO or missing response → BLOCK immediately.
       - YES → proceed with logged override.
   - Log inferred_category, message_type, override_response explicitly.

(5) Evidence rendering toggle (MANDATORY FOR CONTACT/EXECUTIVE; OPTIONAL OTHERWISE):
   - "Render 2-row evidence pack after QA tables? YES/NO (default NO)."
   - Log toggle_response explicitly; BLOCK if not exactly "YES", "NO", or null (implying NO).

(6) Dash Exception Registry check (MANDATORY IF SIGNATURE RENDERS):
   - Prompt verbatim: "Confirm Exception Registry entry for the phone number signature? YES/NO."
   - If "NO" or no response provided → immediate BLOCK with dash_policy_violation.
   - Log registry_check_response explicitly; proceed ONLY on explicit "YES".

RUNTIME BLOCKING CONDITIONS (NEW):
- BLOCK if Prompt 1 is skipped or missing.
- BLOCK if Prompt 2 response missing or invalid.
- BLOCK if Prompt 2A (when required) is skipped or invalid.
- BLOCK if Prompt 3 response missing, invalid, or MULTIPLE for NEW.
- BLOCK if Prompt 4 mismatch detected without explicit YES override.
- BLOCK if Prompt 5 invalid (must be YES/NO/null).
- BLOCK if Prompt 6 is missing or explicitly NO.

INTERNAL AUDIT LOGGING (REQUIRED ON EACH RUN):
- message_type_selected (Prompt 1)
- new_or_existing_response (Prompt 2)
- premium_eligibility_response (Prompt 2A, if applicable)
- single_or_multiple_response (Prompt 3)
- inferred_category (Prompt 4)
- override_response (Prompt 4, if applicable)
- evidence_toggle_response (Prompt 5)
- dash_registry_confirmation (Prompt 6)
- Each prompt's response timestamp logged in ISO8601 format.

--------------------------------------------------------------------------------
BATCH ENVELOPE FORMAT (EXISTING only, MULTIPLE mode)
--------------------------------------------------------------------------------
- Envelope header (one line, verbatim): `BATCH | <MessageType> | K=<2..5>`
- Then exactly K minimal prompt lines for the same `<MessageType>` (no partial sets, no mixing types).
- Reject duplicate LinkedIn URLs within the same batch.
- Block if K outside 2..5, header malformed, or any line fails validation.
...
- QA linkage: AI FILTER vNext governs dash policy, evidence, structure/readability, and Final QA table semantics across artifacts (this doc renders the 13-cell canonical table).
...
5) Reasoning
- **Prompt Sequence Gate:** require recorded, valid responses for prompts 1 → 2 → 2A (if applicable) → 3 → 4 → 5 → 6 before routing. Any skip, reorder, or invalid response → BLOCK.
...
- In MULTIPLE, reject duplicate LinkedIn URLs.
- Enforce K in 2..5 for EXISTING only; for NEW, K must be 1.
- For EXISTING, run dedup vs prior outreach (avoid repeating “I applied for [Role]” unless materially changed).
- Role-detector mismatch without explicit operator confirmation → BLOCK.

6) Output (strict)
- If SINGLE: route = {message_type, mode=SINGLE}. Proceed to selected shell.
- If MULTIPLE: route = {message_type, mode=MULTIPLE, K}. Process K lines serially.
...
- Normalize “percent” → “%”.
- Enforce **Dash Absolute Ban** for external body text; if any dash-like character would appear, rewrite to avoid it. If a signature is appended in short form (discouraged), ensure registry entry first.
- **Verb-tense enforcement:** use present or future for proposed contributions (e.g., “increase,” “will improve”). Past tense allowed only for verifiable facts in brief credibility clauses. Fail → block with `verb_tense_violation`.
...
- **Verb-tense enforcement:** capability frame may cite past outcomes; the three bullets must be present/future-oriented contributions (e.g., “standardize,” “deploy,” “implement”). Fail → block with `verb_tense_violation`.
...
- **Verb-tense enforcement:** insights are descriptive (present/past, sourceable); proposed actions/achievements should be framed in present/future. Fail → block with `verb_tense_violation`.
...
- **Verb-tense enforcement:** insights may be past; proposals/tactics must be present/future when describing what you will do. Fail → block with `verb_tense_violation`.
...
===============================================================================
Block if any of:
- **Operator Prompts sequence failures:** Prompt 1 skipped/missing; Prompt 2 missing/invalid; Prompt 2A required but skipped/invalid; Prompt 3 missing/invalid or MULTIPLE for NEW; Prompt 4 mismatch without explicit YES override; Prompt 5 invalid (must be YES/NO/null); Prompt 6 missing or NO.
...
===============================================================================
END OF LINKEDIN OUTREACH - CANONICAL v1.1
===============================================================================
