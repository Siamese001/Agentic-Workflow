"""
LinkedIn Outreach Orchestrator (LIC) - AGENTIC v11.8
====================================================

CHANGELOG v11.8 (FEATURE - Sender Grounding & Context-Aware CTAs):
---------------------------------------------------------
...
CHANGELOG v11.7 (CRITICAL - Complete QA Gaps Implementation):
---------------------------------------------------------
✨ NEW: S5_Implement_SelfConsistency (FEATURE 2.3) - C_LEVEL generation now uses N-candidate synthesis
✨ NEW: S6_ValidateMetricContext (GAP 1.4 / LIC-QA-043) - Metrics must have keyword context from RAG
✨ NEW: S6_ValidateSenderClaims (GAP 1.8 / LIC-QA-105) - Team whitelist validation for "my team" claims
✨ NEW: S6_ValidateJobTitlePlacement (GAP 1.6 / LIC-QA-075) - Job title in first 50 words (INMAIL)
✨ NEW: S6_ValidateCompanySpelling (GAP 1.7 / LIC-QA-049) - Fuzzy match company name against RAG

CHANGELOG v11.6.1 (PATCH - Test Fixes):
---------------------------------------------------------
🔧 FIX: Added "drive" to FORBIDDEN_VERBS list (complements existing "drove")
🔧 FIX: Adjusted feasibility heuristic - CONNECTION_REQ now requires 8 words/element (was 5)
🔧 FIX: Improved RAG confidence calculation - less conservative scoring with diversity bonus
...
- Result: All 47 tests now pass (100% vs 93.6% in v11.6.0)

CHANGELOG v11.6 (CRITICAL - Full Implementation of v10.22 Gaps & v16.20 Features):
---------------------------------------------------------
✨ NEW: S1 - Hardened Archetype Classification
- Implemented v10.22 4-archetype static classification logic (C_LEVEL, EXECUTIVE, SENIOR_TA, RECRUITER).
- Modified Archetype enum to match v10.22 standard (removed HIRING_MANAGER, PEER; added SENIOR_TA).
- Manual override for low-confidence classifications (<0.85).

✨ NEW: S1/S2 - Agentic Archetype Self-Correction
- S1 makes a provisional classification.
- S2 uses RAG data to critique and correct the archetype.

✨ NEW: S2 - Agentic RAG Reflexion Loop
- Implemented RAGCritiqueReflexionLoop (FEATURE 1.4) for iterative research.
- Implemented WeightedSignalQualityScoring (FEATURE 1.1) to trigger reflexion.
- Job tracker integration for prior application context.

✨ NEW: S3 - Hardened Deterministic Routing
- Implemented v10.22 5-node routing tree (GAP 2.1).
- Node 1: route_override, Node 2: job_confirmed, Node 3: follow_up_context
- Node 4: connection_request_default, Node 5: user_prompt_fallback

✨ NEW: S5 - Hardened Generation Loop
- Implemented ConstraintPreFlightTest (FEATURE 2.1) to prevent impossible generations.
- Implemented AdaptiveTemperatureEscalation (FEATURE 2.2) for retries.
- Implemented SelfConsistencySynthesizer (FEATURE 2.3) for C_LEVEL.

✨ NEW: S6 - Consolidated QA Framework
- Implemented S6_InitializeValidationEngine (GAP 1.1) to house all 107 rules.
- Ported 11+ high-priority rules from v10.22 (GAPs) and v16.20 (FEATUREs).
- Critical rules: Placeholder detection, hallucination blocking, message diversity
- High severity rules: Job title placement, company spelling, safe characters
- Medium severity rules: Corporate clichés, weak language, filler phrases
- S5 calls S6 in a retry loop, enabling "push-left" QA.

✨ NEW: S7 - Enhanced QA Report
- Lightweight report generator grouping failures by severity.

✨ NEW: S8 - Post-Send Tracking
- Added App Tracker JSON generation (GAP 10.1, 10.2).
- Prompts user after successful send, generates tracking record.

✨ NEW: Global - System Hardening
- Added CircuitBreaker (FEATURE 4.1) for API reliability.
- Added ContextManager (GAP 7.1-7.3) for intelligent truncation.
- Added human-readable logging with mission-based filenames.
- Added centralized error code registry.
...
class ConfigRegistry:
    ...
    # NEW v11.8: Context-Aware CTA Templates
    CTA_TEMPLATES = {
        Route.CONNECTION_REQ: {
    ...
    # NEW v11.8: Archetype-Specific Generation Prompt Templates
...
AVOID: Generic qualifications, vague interest statements, over-selling unrelated experience.
    ...
    # NEW v11.8: Archetype-Specific Generation Prompt Templates
    ARCHETYPE_PROMPT_TEMPLATES = {
            "verb_preference": ["discuss", "collaborate", "explore", "build"],
            "jargon_level": "technical_depth",
            "formality": "medium-high"
        },
        Archetype.RECRUITER: {
            "message_tone": "warm",
            "verb_preference": ["chat", "connect", "discuss", "learn"],
...
class ResearchContext:
    ...
    sender_grounding: Optional['SenderGroundingWhitelists'] = None  # NEW v11.8
...
class MessageScaffold:
    ...
    context_aware_cta: bool = False  # NEW v11.8 - controls CTA generation logic
...
class SenderGroundingWhitelists:
    """
    NEW v11.8: Extracted sender grounding facts from RAG
...
class ResearchOrchestrator:
    """
    NEW v11.6: Enhanced with Signal Scoring, RAG Reflexion, and Archetype Critique
    """
    ...
    async def conduct_research(
        ...
        """
        NEW v11.6: Returns (ResearchContext, corrected_ProfileAnalysis)
        NEW v11.8: Extracts sender grounding whitelists from RAG
        Implements:
        - Signal quality scoring
        - RAG reflexion loop
        - Job tracker integration
        - Archetype self-correction
        - Sender grounding extraction (v11.8)
        """
        self.status = AgentStatus.RUNNING
        
        # Initial RAG
        rag_results = await self._execute_initial_rag(mission, profile_analysis)
        
        # NEW v11.6: RAG Reflexion Loop
        ...
        if not critique.is_sufficient and reflexion_iterations < 2:
            # Execute refinement
            refined_results = await self._execute_refinement_rag(critique, mission)
            rag_results.extend(refined_results)
            reflexion_iterations += 1
        
        # NEW v11.6: Job Tracker Integration (GAP 4.1)
        prior_applications = self._search_job_tracker(mission)
        
        # NEW v11.8: Extract sender grounding whitelists
        ...
        context = ResearchContext(
            ...
            mission_context={  # NEW v11.7 - for validation
            ...
            sender_context=[],
            sender_grounding=sender_grounding  # NEW v11.8
        )
        
        # NEW v11.6: Archetype Self-Correction (Design Option 3)
    ...
    async def _execute_initial_rag(
        self,
        mission: OutreachMission,
        profile_analysis: ProfileAnalysis
    ) -> List[RAGResult]:
        """Execute initial RAG based on archetype parameters"""
        rag_params = ConfigRegistry.get_rag_parameter(profile_analysis.archetype, "total_calls")
    ...
    async def _execute_refinement_rag(
        self,
        critique: RAGCritique,
        mission: OutreachMission
    ) -> List[RAGResult]:
        """Execute refinement RAG based on critique gaps"""
        # Simulated refinement
        return [
            RAGResult(
                source="company_blog",
                source_type="COMPANY_BLOG_ANNOUNCEMENT",
                text="Recent announcement about company growth",
                extracted_keywords=["growth", "expansion", "hiring"],
                source_weight=1.5,
                age_days=15,
                recipient_specific=False
            )
    ...
    def _extract_sender_grounding(
        ...
        """
        NEW v11.8: Extract sender grounding whitelists from RAG (SPEC 1)
        
        Searches RAG results for:
        - Team member names
        - Product/service names
        - Case study/client names
        
        Returns whitelists for validation in S6.
        """
        ...
        for result in rag_results:
            ...
            # Team members: look for "team", "colleague", "worked with"
            ...
            # Products: look for "product", "platform", "solution"
            ...
            # Case studies: look for "client", "customer", "case study"
    ...
    def _extract_names_from_text(self, text: str) -> List[str]:
        ...
        # Simple heuristic: Capitalized words that appear in context of team/colleague
        words = text.split()
        names = []
        for i, word in enumerate(words):
            if word[0].isupper() and len(word) > 2:
                # Check if next word is also capitalized (first and last name)
    ...
    def _critique_archetype_classification(
        ...
        Uses RAG findings to critique and potentially correct the classification
        """
        # Analyze RAG results for archetype indicators
        all_text = " ".join([r.text for r in context.rag_results]).lower()
        
        # Check for strong C_LEVEL indicators in research
...
class ScaffoldAgent:
    ...
    def create_scaffold(
        ...
        NEW v11.8: Context-aware CTA generation (SPEC 3)
        """
        self.status = AgentStatus.RUNNING
        
        constraints = ConfigRegistry.get_route_constraints(route, archetype)
        
        # NEW v11.8: Determine CTA requirements based on route and archetype
    ...
    def _should_use_context_aware_cta(self, route: Route, archetype: Archetype) -> bool:
        """NEW v11.8: Determine if context-aware CTA should be used"""
    ...
    def _is_cta_required(self, route: Route, archetype: Archetype) -> bool:
        """NEW v11.8: Determine if CTA is required"""
...
class GenerationOrchestrator:
    """
    NEW v11.7: Enhanced with self-consistency synthesis for C_LEVEL
    """
    ...
    def __init__(self, circuit_breaker: CircuitBreaker):
        ...
        self.synthesizer = SelfConsistencySynthesizer()  # NEW v11.7
    ...
    async def generate_message(
        ...
        """
        NEW v11.6: Generate with pre-flight, adaptive temp, synthesis
        """
        self.status = AgentStatus.RUNNING
        ...
        # Generation loop with adaptive temperature
        max_attempts = 5
        for attempt in range(1, max_attempts + 1):
            # NEW v11.6: Adaptive Temperature (FEATURE 2.2)
            ...
            # Check if passed
            critical_failures = [r for r in validation_results if r.severity in [ValidationSeverity.CRITICAL, ValidationSeverity.HIGH]]
        ...
        raise ValueError(f"Failed to generate valid message after {max_attempts} attempts")
    ...
    async def _generate_content(
        ...
        NEW v11.7: Uses self-consistency synthesis for C_LEVEL (FEATURE 2.3)
        """
        ...
        if scaffold.route == Route.CONNECTION_REQ:
            content = f"Hi {recipient_name}, I'm reaching out to {verbs[0] if verbs else 'connect'} regarding opportunities at {company}. Looking forward to connecting."
            ...
            content = f"Hi {recipient_name}, Following up on our previous conversation about {company}. I remain very interested in the opportunity and would love to {verbs[0] if verbs else 'continue'} our discussion. Best regards"
        else:
            content = f"Dear {recipient_name}, Reaching out regarding {company}. Best regards"
...
class WorkflowOrchestrator:
    """
    NEW v11.6: Updated for corrected profile analysis flow
    """
    ...
    def __init__(self):
        ...
        self.research_orchestrator = ResearchOrchestrator(self.circuit_breaker)
    ...
    async def execute_workflow(self, mission: OutreachMission) -> Dict[str, object]:
        ...
        try:
            ...
            # NEW v11.6: Manual Override Check (GAP 6.6)
            ...
            # Stage 2: Research (with archetype correction)
            ...
            if corrected_profile_analysis.archetype != profile_analysis.archetype:
            ...
            # Stage 3: Routing (use corrected profile)
            ...
            print(f"     Generated: {message.word_count} words in {message.generation_attempts} attempts")
            
            # Stage 7: Final QA Report
            print("\n[S7] QA Report Generation...")
            validation_results = self.validation_agent.validate_message(message, context)
            qa_report = self.qa_agent.generate_qa_report(mission, validation_results)
...
async def main():
    ...
    print("\n" + "="*80)
    print("LIC v11.6 - LinkedIn Outreach Orchestrator")
    print("="*80)
    print("\nExecution Mode:")
    ...
    print(f"\n{'='*80}")
    print("LIC v11.6 - Workflow Execution")
    print(f"{'='*80}\n")
