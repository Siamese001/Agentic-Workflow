# File: run_learning_v7_5.py
# Overwrites: run_learning_v7_0.py
# Version: 7.5 (HIL Meta-Learning)
#
# v7.5 (Phase 2 HIL) CHANGES:
# - Imports from core_v7_5.
# - 'MetaPlannerAgent' is now a "proposer." It no longer writes
#   to the live rules registry.
# - It now writes *proposals* to 'proposed_rules.jsonl'
#   (as configured in master_config_v7_5.json).
# - This defuses Critique #4 (The "Time Bomb").
...
from archives.legacy_resume_gen.Older Microservices Models.v7.5.core_v7_5 import CONFIG, setup_logging

logger = logging.getLogger("meta_learner_v7_5")
...
def run_meta_learning():
    """
    Runs the asynchronous meta-learning loop (v7.5).
    """
    try:
    ...
    logger.info(f"===== Starting v7.5 Meta-Learning Loop ({datetime.now().isoformat()}) =====")
    
    if not CONFIG.meta_loop_config.enable_meta_learning:
        logger.info("Meta-learning is disabled in master_config_v7_5.json. Exiting.")
        return
    ...
    if not patterns:
        ...
        logger.info("===== v7.5 Meta-Learning Loop Complete =====")
        return
