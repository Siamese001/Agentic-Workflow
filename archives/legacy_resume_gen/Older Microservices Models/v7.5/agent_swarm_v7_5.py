# File: agent_swarm_v7_5.py
# Overwrites: agent_swarm_v7_0.py
# Version: 7.5 (LangGraph Refactor + HIL)
#
# v7.5 (Phase 2 HIL) CHANGES:
# - Imports from core_v7_5 and GraphState.
# - Added new 'PreferenceCaptureAgent' class.
# - 'get_graph_app' now accepts 'enable_hil' flag.
# - 'get_graph_app' adds new nodes: 'human_review_pause' and 'capture_preferences'.
# - 'check_qa_results' conditional edge now routes to 'human_review_pause' on success.
# - 'workflow.compile()' now uses 'interrupt_before' to create the HIL pause.
...
# ============================================================================
import copy
import difflib  # <-- NEW IMPORT (v7.5)
import hashlib
import json
import logging
import os
...
# ============================================================================
# PART 5: V7.5 PREFERENCE LEARNING AGENT (NEW)
# ============================================================================

class PreferenceCaptureAgent(BaseAgent):
    """
    v7.5 (Phase 2): Compares the AI draft to the human-edited draft
    and generates a "preference insight" for future runs.
    """
    
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "PreferenceCaptureAgent"
        # This agent can be just Python, or it can use an LLM for a "smarter" diff.
        # For now, we use a simple, cheap Python diff.
    
    def run(self, ai_draft: str, human_draft: str) -> dict:
        self.log_info("Running Step 5: Preference Capture...")
        
        if ai_draft == human_draft:
            self.log_info("No changes detected. Human approved AI draft as-is.")
            return {"insight": "No changes made by HIL."}

        # Use difflib to find the changes
        diff = list(difflib.unified_diff(
            ai_draft.splitlines(keepends=True),
            human_draft.splitlines(keepends=True),
            fromfile="ai_draft.txt",
            tofile="human_draft.txt",
            n=1  # Context lines
        ))
        
        if not diff:
            # Should be caught by the first check, but as a fallback
            return {"insight": "No functional changes detected."}

        # For v7.5, the insight is just the raw diff.
        # A v8.0+ would feed this diff to an LLM to generate a
        # natural language "preference" (e.g., "User prefers metrics in bold").
        insight_text = "".join(diff)
        self.log_info(f"Captured insight (diff):\n{insight_text}")
        
        # TODO: Implement saving this to the strategic_insights_collection
        # For now, it's saved to the GraphState.
        
        return {
            "insight": "Human made edits.",
            "raw_diff": insight_text
        }
...
def check_qa_results(state: 'GraphState') -> str:
    ...
    if qa_passed:
        ...
        return "human_review_pause"  # <-- MODIFIED
...
def get_graph_app(checkpointer: 'RedisSaver', enable_hil: bool = True) -> 'CompiledGraph':
    """
    Builds and compiles the persistent v7.5 StateGraph.
    v7.5: Adds an 'enable_hil' flag to allow batch processing (v7.0 logic).
    """
    from archives.legacy_resume_gen.Older Microservices Models.v10.7.vendor.langgraph.graph import StateGraph, END
    from archives.legacy_resume_gen.Older Microservices Models.v7.5.core_v7_5 import GraphState
    ...
    if enable_hil and CONFIG.hil_config.enable_content_approval:
        ...
        workflow.add_conditional_edges(
            ...
            check_qa_results,
            {
                "human_review_pause": "human_review_pause",  # <-- MODIFIED
                "FAIL_HALT": END,
                "REPLANNER": "replanner"
            }
        )
        ...
        workflow.add_conditional_edges(
            ...
            check_qa_results,
            {
                "human_review_pause": END,  # <-- MODIFIED (Success routes to END)
                "FAIL_HALT": END,
                "REPLANNER": "replanner"
            }
        )
