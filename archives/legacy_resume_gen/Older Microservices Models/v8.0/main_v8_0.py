...
# Import from consolidated modules (v8.0)

# v8.0: Import from the new graph builder
...
# Version info
__version__ = "8.0.0-agentic-upgrade"
...
def print_summary(results: Dict[str, object], workflow_id: str, start_time: datetime):
    ...
    print("\n" + "=" * 80)
    print(f"WORKFLOW EXECUTION SUMMARY (v8.0)")
    print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f'V8.0 LangGraph Workflow (Version: {__version__})',
...
Examples:
  ...
  python {script_path} --debug                   # Enable debug mode
    ...
    try:
        ...
        logger.info(f"🚀 Starting v8.0 (Agentic Upgrade) Workflow. ID: {workflow_id}")
        ...
        if args.output_dir:
            ...
            if final_draft:
                draft_file = output_path / f"{base_filename}_final_draft_v8.0.txt"
