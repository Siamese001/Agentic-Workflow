...
# v8.0: Pointing CONFIG_PATH to master_config_v8_0.json
# v8.0: Added new prompt constants for v8.0 agentic agents (ReAct, MoE conductors, bullet generation)
#
...
# Load Configuration
CONFIG_PATH = PROJECT_ROOT / "master_config_v8_0.json"
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            if not self.config_path.exists():
                core_logger.critical(f"FATAL: master_config_v7_5.json not found at {self.config_path}.")
                raise FileNotFoundError(f"master_config_v7_5.json not found at {self.config_path}.")
...
def _get_prompt_template(key: str) -> str:
    ...
    if not hasattr(PROMPT_TEMPLATES, key):
        ...
        raise KeyError(f"Prompt template key '{key}' not found in master_config_v7_5.json!")
