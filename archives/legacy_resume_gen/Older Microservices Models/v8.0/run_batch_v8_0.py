# File: run_batch_v8_0.py
# Overwrites: run_batch_v7_5.py
# Version: 8.0 (Agentic Upgrade)
#
# v8.0 (Agentic Upgrade) CHANGES:
# - Imports from v8_0 modules.
...
from archives.legacy_resume_gen.Older Microservices Models.v8.0.main_v8_0 import setup_logging, load_job_input
from archives.legacy_resume_gen.Older Microservices Models.v8.0.core_v8_0 import CONFIG, CircuitBreakerOpenError

# v8.0: Import the graph builder and checkpointer
from archives.legacy_resume_gen.Older Microservices Models.v8.0.agent_swarm_v8_0 import get_graph_app
from langgraph.checkpoint.redis import RedisSaver

# v8.0: Import the meta-learner
try:
    from archives.legacy_resume_gen.Older Microservices Models.v8.0.run_learning_v8_0 import run_meta_learning
    META_LEARNER_AVAILABLE = True
except ImportError:
    META_LEARNER_AVAILABLE = False


logger = logging.getLogger("batch_runner_v8_0")
try:
...
SUMMARY_FILE = "batch_summary_v8_0.csv"

# --- v8.0: Graph Setup (will be re-initialized in each process) ---
checkpointer = None
app = None
...
def get_compiled_graph():
    ...
    v8.0: Passes 'enable_hil=False' to the graph builder.
    """
...
def process_single_job(job_path: str) -> Dict[str, object]:
    ...
    logger.info(f"--- Starting v8.0 job: {os.path.basename(job_path)} ---")
    ...
    try:
        ...
        logger.info(f"--- Finished v8.0 job: {company} - {title} ---")
...
def run_batch():
    ...
    if not job_files:
        logger.info("v8.0 Batch process starting. No jobs found in queue.")
        return

    logger.info(f"v8.0 Batch process starting. Found {len(job_files)} jobs.")
    ...
    logger.info(f"v8.0 Batch process complete.")
    ...
    if CONFIG.meta_loop_config.enable_meta_learning:
        ...
        if META_LEARNER_AVAILABLE:
            ...
            logger.warning("Meta-learning is enabled, but 'run_learning_v8_0' could not be imported.")
    else:
