...
# --- GAP 2 FIX: Simulated Vector Search (No longer mock) ---

class GraphSearchTool(BaseTool):
    """
    GAP 2 FIX: Searches the resume graph/database.
    This is a SIMULATED vector search, not a mock keyword search.
    It builds a simple TF-IDF vector and uses cosine similarity.
    """
    tool_name = "search_resume_database"

    def _embed_text(self, text: str) -> Counter:
        """Create a simple TF vector (word count)"""
        if not text:
            return Counter()
        words = re.findall(r'\b\w+\b', text.lower())
        # Simple stopword removal
        stopwords = {"the", "a", "is", "in", "for", "of", "and", "to"}
        return Counter(w for w in words if w not in stopwords)

    def _get_cosine_similarity(self, vec1: Counter, vec2: Counter) -> float:
        """Calculate cosine similarity between two TF vectors"""
        intersection = set(vec1.keys()) & set(vec2.keys())
        dot_product = sum(vec1[x] * vec2[x] for x in intersection)

        mag1 = math.sqrt(sum(vec1[x]**2 for x in vec1.keys()))
        mag2 = math.sqrt(sum(vec2[x]**2 for x in vec2.keys()))
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        ...
        # This simulates querying a vector DB
        master_resume_str = self.context.redis_client.get(f"workflow:{workflow_id}:master_resume")
        if not master_resume_str:
            self.log_warning("No master resume found in Redis for search")
            return {"search_results": []}
        
        try:
            experience = json.loads(master_resume_str).get("experience", [])
            query_vector = self._embed_text(query)
            
            results = []
            for i, exp in enumerate(experience):
                # Combine all text in the experience block for embedding
                exp_text = f"{exp.get('title', '')} {exp.get('company', '')} {' '.join(exp.get('bullet_pool', []))}"
                exp_vector = self._embed_text(exp_text)
                
                score = self._get_cosine_similarity(query_vector, exp_vector)
                
                if score > 0.1: # Similarity threshold
                    results.append({"score": score, "experience": exp, "id": i})
            
            # Sort by score descending
            sorted_results = sorted(results, key=lambda x: x["score"], reverse=True)
            final_experiences = [r["experience"] for r in sorted_results]
            
            self.log_feedback(workflow_id, "graph_search", "success", {"query": query, "results_found": len(final_experiences)})
            return {"search_results": final_experiences[:5]} # Return top 5
            
        except Exception as e:
            self.log_error(f"Failed to run vector search: {e}")
            return {"search_results": []}
