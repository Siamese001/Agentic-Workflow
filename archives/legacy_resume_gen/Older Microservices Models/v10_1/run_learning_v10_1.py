...
#
# GAP 4 FIX (DI):
# - Removed global CONFIG import.
# - run_meta_learning now acts as the "Composition Root" for this workflow.
# - It accepts a 'config' object and CREATES/INJECTS all dependencies
#   into its own WorkflowContext.
...
from archives.legacy_resume_gen.Older Microservices Models.v10_1.main_v10_1 import setup_logging
...
class AsyncLogSummarizerAgent(BaseAgent):
    ...
    async def run_async(self, raw_logs: Dict[str, str], workflow_id: str) -> Dict[str, object]:
        ...
        # GAP 4 FIX: Use self.config
        model_config = self.config.model_config.qa_model
...
class AsyncPatternFinderAgent(BaseAgent):
    ...
    async def run_async(self, log_summary: Dict[str, object], workflow_id: str) -> List[Dict]:
        ...
        # GAP 4 FIX: Use self.config
        model_config = self.config.model_config.strategy_model
...
class AsyncProposalDrafterAgent(BaseAgent):
    ...
    async def run_async(self, hypothesis: Dict[str, object], workflow_id: str) -> Dict[str, object]:
        ...
        # GAP 4 FIX: Use self.config
        model_config = self.config.model_config.prompt_engineer_model
...
class AsyncProposalCritiqueAgent(BaseAgent):
    ...
    async def run_async(self, proposal: Dict[str, object], patterns: List[Dict], workflow_id: str) -> Dict[str, object]:
        ...
        # GAP 4 FIX: Use self.config
        model_config = self.config.model_config.critique_model
...
def build_meta_learning_graph(context: WorkflowContext, checkpointer: RedisSaver):
    ...
    async def write_rules_node(state: dict) -> dict:
        ...
        # GAP 4 FIX: Pass config to manager
...
async def run_meta_learning(config: ConfigV10_1):
    """
    Runs v10.1 async meta-learning graph with hot-reload
    GAP 4 FIX: This function is now the "Composition Root" for meta-learning.
    ...
    try:
        ...
        # .invoke is synchronous, but the nodes inside are async
        # We run the whole sync invoke in an async-friendly thread
...
if __name__ == "__main__":
    # GAP 4 FIX: Load config first, then pass it
    config = ConfigV10_1()
    try:
