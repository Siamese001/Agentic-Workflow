...
# Version: 5.6 (Agentic Executor - Blackboard Data Models)
...
class RAG_Blackboard:
    """
    Shared state for the agentic RAG pipeline (v5.5).
    Agents read from and write to this object.
    """
    workflow_id: str
    strategy: 'StrategyBrief'
    
    # Populated by QueryGenerator
    ...
    # Populated by ChunkingAgent
    chunks: List[str] = field(default_factory=list)
    # Populated by RankingAgent
    ranked_chunks: List[Tuple[str, float]] = field(default_factory=list)
    # Populated by FilterAgent
    filtered_chunks: List[Tuple[str, float]] = field(default_factory=list)
    # Populated by CrossReferenceAgent
    annotated_chunks: List[Dict[str, Any]] = field(default_factory=list)
    # Populated by RAG_DraftingAgent
    synthesis_draft: str = ""
    # Populated by RAG_CritiqueAgent
    critiques: List[str] = field(default_factory=list)
    # Populated by RAG_DraftingAgent (refine step)
    final_analysis: Optional[ThematicAnalysis] = None
...
# ============================================================================
# V5.6: AGENTIC WORKFLOW DATA MODELS
# ============================================================================

@dataclass
class StrategyBlackboard:
    """
    V5.6: Blackboard for Strategy Stack.
    Holds intermediate state as atomic Strategy agents execute.
    """
    raw_jd: str
    structured_jd: Dict[str, Any] = field(default_factory=dict)
    brainstormed_themes: List[str] = field(default_factory=list)
...
class RAGBlackboard:
    """
    V5.6: Blackboard for RAG Pipeline.
    Replaces and extends RAG_Blackboard with finer-grained state tracking.
    """
...
class PlanStep:
    """
    V5.6: Represents a single step in the workflow plan.
    """
    agent: str
...
class Plan:
    """
    V5.6: The dynamic workflow plan.
    Can be modified by RePlanner during execution.
    """
...
class WorkflowBlackboard:
    """
    V5.6: Central blackboard for entire workflow.
    ...
    # Sub-blackboards for different stacks
    strategy_board: StrategyBlackboard = field(default_factory=lambda: StrategyBlackboard(raw_jd=""))
    rag_board: RAGBlackboard = field(default_factory=RAGBlackboard)
    
    # The current plan
    plan: Optional[Plan] = None
    
    # Generated artifacts (keyed by section name)
