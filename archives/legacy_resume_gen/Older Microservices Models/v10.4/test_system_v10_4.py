...
# v10.4: FIX - Removed global pytestmark to fix 29 warnings
...
def mock_prompt_manager():
    ...
    # v10.4: FIX - Include all possible keys to prevent KeyErrors
    mock.get_template.side_effect = lambda name: f"Mock template for {name}: {{style_guide}} {{draft}} {{strategy}} {{job_description}} {{section_text}} {{critique}} {{bullets}} {{master_resume}} {{draft_text}} {{required_tone}} {{experience}} {{query}} {{candidates}} {{instruction}} {{context}} {{content}} {{job_title}} {{company}} {{branch_num}} {{total_branches}}"
    return mock
...
def mock_response_validator():
    ...
    def validate_side_effect(content, model):
        try:
            ...
            # v10.4: FIX - Check if model is a type (like StrategyPlan)
            if isinstance(model, type) and issubclass(model, BaseModel):
                 return model.model_validate(content), None
            # v10.4: FIX - Handle dicts (like in the original test)
            elif model == dict:
                return content, None
            # v10.4: FIX - Handle tuples of types
...
def base_state():
    """
    v10.4: FIX - Creates a base MainGraphState dict with all required nested keys
    to prevent the 11 KeyError failures.
    """
    ...
    # Return the dict representation, as this is what LangGraph passes
...
async def test_conductor_circuit_breaker_opens(mock_workflow_context, mock_llm_client):
    ...
    # v10.4: FIX - Pass the Pydantic model
...
async def test_async_bullet_generator(mock_workflow_context, mock_llm_client, sample_master_resume, base_state):
    ...
    # v10.4: FIX - Reconstruct the Pydantic model from the state dict
    ...
    result = await agent.run_async(
        ...
        strategy=strategy_model, # v10.4: Pass strategy
...
async def test_contract_tool_fails_on_missing_input(mock_workflow_context, mock_llm_client):
    ...
    # v10.4: FIX - Re-wire validator to *actually* fail
...
async def test_contract_qa_conductor_uses_budget_manager(mock_workflow_context, mock_llm_client, base_state):
    ...
    with patch.object(conductor.budget_manager, 'prune', side_effect=lambda doc, limit: doc) as mock_prune:
        ...
        # v10.4: FIX - Reconstruct the Pydantic model from the state dict
        # This is what the graph *actually* does
        ...
        mock_prune.assert_any_call(json.dumps(base_state['resume']['master_resume']), 4000)
...
async def test_self_consistency_zero_temp(mock_workflow_context, mock_llm_client, base_state):
    ...
    # v10.4: FIX - Reconstruct the Pydantic model
...
@pytest.mark.asyncio
async def test_graph_compiles_correctly(mock_workflow_context, base_state):
...
async def test_orchestration_qa_retry_logic(mock_workflow_context, base_state):
    ...
    with patch('agent_orchestration_v10_4.run_sanitize_pii', new_callable=AsyncMock) as mock_sanitize, \
        ...
        # v10.4: FIX - Return the Pydantic model *dumped* as a dict
        mock_ambiguity.return_value = {"hil": {"ambiguity_report": {"ambiguity_detected": False, "confidence": 1.0, "reason": "N/A", "question_for_human": "N/A"}}}
...
async def test_integration_state_accumulation(mock_workflow_context, base_state):
    ...
    with patch('agent_orchestration_v10_4.run_sanitize_pii', new_callable=AsyncMock) as mock_sanitize, \
        ...
        # v10.4: FIX - Return the Pydantic model *dumped* as a dict
        mock_ambiguity.return_value = {"hil": {"ambiguity_report": {"ambiguity_detected": True, "confidence": 1.0, "reason": "N/A", "question_for_human": "N/A"}}}
...
async def test_mock_detection_tool_empty_return(mock_workflow_context, mock_llm_client):
    ...
    # v10.4: FIX - Re-wire validator to *actually* validate
    real_validator = ResponseValidator()
    mock_workflow_context.response_validator.validate.side_effect = real_validator.validate
...
# v10.4: FIX - Removed @pytest.mark.asyncio
def test_mock_detection_bm25_tool_handles_missing_library(mock_workflow_context):
    ...
    with patch('agent_stacks_v10_4.BM25_AVAILABLE', False):
        ...
        # v10.4: FIX - Must be async
        result = asyncio.run(tool.run_async({"query": "test"}, "test-wf"))
