# File: agent_swarm_v9_6.py
# Overwrites: agent_swarm_v9_5.py
# Version: 9.6 (Adversarial)
#
# v9.5 CHANGES:
# - Added PIISanitizerAgent (Item #1)
# - Updated RAG/Bullet agents to populate provenance_ledger (Item #2)
...
from archives.legacy_resume_gen.Older Microservices Models.v9.6.core_v9_6 import BaseAgent, get_model_client, CONFIG, HopExecutionError, MechanicalFailureError, SemanticFailureError, FactualFailureException, ValidationSeverity, ValidationResult, ReasoningConfig, ReasoningStrategy, ThematicAnalysis, RAG_Blackboard, RAGMission, RAGPhase, StrategyBrief, ReflectionIteration, ReflectionResult, ReflectionStatus, ToolCall, ToolType, ReActTrace, MoEExpertResult, MoEDecision, ConductorBranch, ConductorDecision, WorkflowBlackboard, WorkflowPlan, WorkflowStep, GraphState, DEFAULT_GENERATION_TEMPERATURE, text_utils, fence_data, STRATEGY_THEME_CLASSIFICATION_SYSTEM_PROMPT, STRATEGY_THEME_CLASSIFICATION_USER_PROMPT, RAG_QUERY_GEN_SYSTEM_PROMPT, RAG_QUERY_GEN_USER_PROMPT, RAG_THOUGHT_SYSTEM_PROMPT, RAG_CRITIQUE_STEP_SYSTEM_PROMPT, RAG_CRITIQUE_SYSTEM_PROMPT, RAG_CRITIQUE_USER_PROMPT, BULLET_CUSTOMIZER_SYSTEM_PROMPT, BULLET_CUSTOMIZER_USER_PROMPT, BULLET_SYNTHETIC_SYSTEM_PROMPT, BULLET_SYNTHETIC_USER_PROMPT, DRAFTING_CONDUCTOR_SYSTEM_PROMPT, DRAFTING_CONDUCTOR_USER_PROMPT, DRAFTING_STRATEGIST_SYSTEM_PROMPT, DRAFTING_REDTEAM_SYSTEM_PROMPT, DRAFTING_REFINER_SYSTEM_PROMPT, DRAFTING_METRICS_SYSTEM_PROMPT, DRAFTING_USER_PROMPT, REPLANNER_SYSTEM_PROMPT, REPLANNER_USER_PROMPT, QA_CONDUCTOR_SYSTEM_PROMPT, QA_CONDUCTOR_USER_PROMPT, QA_CLAIM_VALIDATOR_SYSTEM_PROMPT, QA_TONE_VALIDATOR_SYSTEM_PROMPT, QA_ALIGNMENT_VALIDATOR_SYSTEM_PROMPT, QA_ENTAILMENT_VALIDATOR_SYSTEM_PROMPT, QA_NARRATIVE_VALIDATOR_SYSTEM_PROMPT, QA_ADVERSARIAL_VALIDATOR_SYSTEM_PROMPT, QA_JD_SKILLS_VALIDATOR_SYSTEM_PROMPT, QA_SIGNAL_SCORE_VALIDATOR_SYSTEM_PROMPT, QA_BIAS_VALIDATOR_SYSTEM_PROMPT, QA_TENURE_VALIDATOR_SYSTEM_PROMPT, QA_MISSED_OPPORTUNITY_SYSTEM_PROMPT, QA_GENERIC_USER_PROMPT, STRATEGY_CRITIQUE_SYSTEM_PROMPT, RAG_QUERY_ADVERSARY_SYSTEM_PROMPT, BULLET_SYNTHETIC_FACTCHECK_SYSTEM_PROMPT, QA_PLAN_CRITIQUE_SYSTEM_PROMPT, REPLANNER_CRITIQUE_SYSTEM_PROMPT
    ...
    # v9.6 Adversarial Prompts
    STRATEGY_CRITIQUE_SYSTEM_PROMPT,
    RAG_QUERY_ADVERSARY_SYSTEM_PROMPT,
    BULLET_SYNTHETIC_FACTCHECK_SYSTEM_PROMPT,
    QA_PLAN_CRITIQUE_SYSTEM_PROMPT,
    REPLANNER_CRITIQUE_SYSTEM_PROMPT
)

# v9.6: Stub for Neo4j/Graph Tool
...
# ============================================================================
# V9.5: PII SANITIZER (Item #1)
# ============================================================================

class PIISanitizerAgent:
    """
    v9.6: (Item #1)
    A simple, regex-based agent to sanitize PII from the master_resume.json.
    This is NOT a BaseAgent, it's a utility class.
    """
    def __init__(self):
        self.logger = logging.getLogger(f"{__name__}.PIISanitizerAgent")
        self.replacements = {}

    def run(self, master_resume: Dict[str, object]) -> Dict[str, object]:
        self.logger.info("Sanitizing master_resume PII...")
        
        # We need a deep copy to avoid modifying the original dict
        sanitized_resume = copy.deepcopy(master_resume)
        
        try:
            owner = sanitized_resume.get("owner", {})
            contact = owner.get("contact", {})

            if "name" in owner:
                owner["name"] = "[CANDIDATE_NAME]"
            if "phone" in contact:
                contact["phone"] = "[CANDIDATE_PHONE]"
            if "email" in contact:
                contact["email"] = "[CANDIDATE_EMAIL]"
            
            self.logger.info("PII sanitization complete.")
            return sanitized_resume
        
        except Exception as e:
            self.logger.error(f"Failed to sanitize PII: {e}")
            return master_resume # Return original on failure
...
# ============================================================================
# V9.6: ADVERSARIAL CRITIQUE AGENTS
# ============================================================================

class StrategyCritiqueAgent(BaseAgent):
    """v9.6: Adversarially reviews the strategy brief."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "StrategyCritiqueAgent"
        self.client = get_model_client("google", "gemini-2.5-flash")

    def run(self, strategy_brief: dict) -> dict:
        self.log_info("Running adversarial critique on strategy...")
        try:
            prompt = STRATEGY_CRITIQUE_SYSTEM_PROMPT.format(strategy_brief=json.dumps(strategy_brief))
            ...
            if not response.get("critique_passed", False):
                self.log_warning(f"Strategy Critique FAILED: {response.get('reason')}")
            return response
        except Exception as e:
            self.log_error(f"StrategyCritiqueAgent failed: {e}")
            return {"critique_passed": False, "reason": str(e)}


# ============================================================================
# PART 2: RAG STACK AGENTS (v7.0 Step 2 -> v9.0 Upgrade)
# ============================================================================

class RAG_QueryGeneratorAgent(BaseAgent):
    """
    v9.5 Step 2: RAG - QueryGen
...
class QueryAdversaryAgent(BaseAgent):
    """v9.6: Adversarially reviews the RAG queries."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "QueryAdversaryAgent"
        self.client = get_model_client("google", "gemini-2.5-flash")

    def run(self, queries: List[str], strategy_brief: dict) -> List[str]:
        self.log_info("Running adversarial critique on RAG queries...")
        try:
            prompt = RAG_QUERY_ADVERSARY_SYSTEM_PROMPT.format(
                strategy_brief=json.dumps(strategy_brief),
                queries=json.dumps(queries)
            )
            messages = [{"role": "user", "content": prompt}]
            response = self.client.chat_completion(messages=messages, response_format="json_object")
            improved_queries = response.get("queries", queries)
            if improved_queries != queries:
                self.log_info("Query Adversary modified the query list.")
            return improved_queries
        except Exception as e:
            self.log_error(f"QueryAdversaryAgent failed: {e}")
...
class RAG_SearchAgent(BaseAgent):
    ...
    def run(self, queries: List[str]) -> Dict[str, object]:
        ...
        for query in queries:
            ...
            for iteration in range(self.max_react_iterations):
                ...
                react_traces.append({"type": "thought", "content": thought}) # v9.5
                ...
                react_traces.append({"type": "action", "tool": tool_name, "input": tool_input}) # v9.5
                ...
                react_traces.append({"type": "observation", "content": str(observation)[:200]}) # v9.5
                ...
                # 4. (v9.5) CRITIQUE: Self-correction loop
                if self._critique_step(query, current_context, react_traces):
    ...
    def _generate_thought(self, query: str, iteration: int, current_results: List) -> str:
        # v9.6: This is now a real LLM call
    ...
    def _select_action(self, thought: str) -> Dict:
        # v9.6: Parse the thought for a tool call.
    ...
    def _critique_step(self, query: str, context: List, react_traces: List) -> bool:
        # v9.6: Internal critique loop
        ...
        react_traces.append({"type": "critique", "content": critique, "satisfied": satisfied}) # v9.5
        self.log_info(f"  Critique: {critique} (Satisfied: {satisfied})")
        return satisfied
    ...
    def _tool_master_resume_search(self, tool_input: Dict) -> List[Dict]:
        ...
        master_resume = self.blackboard.master_resume # v9.6: This is the SANITIZED version
        all_text_chunks = []
    ...
    def _tool_graph_search(self, tool_input: Dict) -> List[Dict]:
        ...
        # v9.6: Calls the stubbed Neo4j client
...
class PromptStackAgent(BaseAgent):
    """
    v9.5 Step 3: Drafting - Prompt Stack
    ...
    def run(self) -> str:
        ...
        try:
            ...
            prompt = DRAFTING_USER_PROMPT.format(
                ...
                generated_bullets=json.dumps(self.blackboard.get_artifact("generated_bullets"), indent=2)
            )
...
# ============================================================================
# PART 4 (v9.5): NEW BULLET STACK (Req #2)
# ============================================================================

class CustomizedBulletDrafterAgent(BaseAgent):
    """v9.0: (LLM-based) Rewrites existing bullets to match strategy."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
...
class SyntheticBulletDrafterAgent(BaseAgent):
    """v9.0: (LLM-based) Generates new bullets from scratch."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
...
class SyntheticFactCheckAgent(BaseAgent):
    """v9.6: Adversarially reviews synthetic bullets for plausibility."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "SyntheticFactCheckAgent"
        self.client = get_model_client("google", "gemini-2.5-flash")

    def run(self, synthetic_bullets: List[str], master_resume: dict) -> List[str]:
        self.log_info("Running adversarial fact-check on synthetic bullets...")
        try:
            prompt = BULLET_SYNTHETIC_FACTCHECK_SYSTEM_PROMPT.format(
                master_resume=json.dumps(master_resume),
                synthetic_bullets=json.dumps(synthetic_bullets)
            )
            messages = [{"role": "user", "content": prompt}]
            response = self.client.chat_completion(messages=messages, response_format="json_object")
            factchecked_bullets = response.get("factchecked_bullets", [])
            if len(factchecked_bullets) < len(synthetic_bullets):
                self.log_warning(f"SyntheticFactCheckAgent rejected {len(synthetic_bullets) - len(factchecked_bullets)} bullets.")
            return factchecked_bullets
        except Exception as e:
            self.log_error(f"SyntheticFactCheckAgent failed: {e}")
            return [] # Return no bullets on failure


class ProvenanceRouterAgent(BaseAgent):
    """v9.6: Replaces BulletSwarmAgent. Implements 2/3/2 & 2/2/2 logic."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        ...
        self.fact_check_agent = SyntheticFactCheckAgent(blackboard, debug_mode) # v9.6
    ...
    def run(self) -> Dict[str, object]: # v9.6: Return dict for provenance
        self.log_info("Running Step 3: Advanced Bullet Swarm (v9.5)...")
        try:
            strategy = self.blackboard.get_artifact("strategy_brief")
            if not strategy:
                raise HopExecutionError("Missing strategy_brief artifact.")
            ...
            provenance_log = {"agent": self.agent_name, "plan": plan, "outputs": {}} # v9.5
            ...
            provenance_log["outputs"]["verbatim"] = verbatim_bullets # v9.5

            # 2. Get Customized bullets (LLM)
            custom_bullets = self.custom_drafter.run(remaining_pool, strategy, plan["custom"])
            provenance_log["outputs"]["custom"] = custom_bullets # v9.5

            # 3. Get Synthetic bullets (LLM)
            synthetic_bullets = self.synthetic_drafter.run(strategy, plan["synthetic"])
            provenance_log["outputs"]["synthetic_original"] = synthetic_bullets # v9.6

            # 4. v9.6: Fact-check synthetic bullets
            fact_checked_bullets = self.fact_check_agent.run(synthetic_bullets, master_resume)
            provenance_log["outputs"]["synthetic_fact_checked"] = fact_checked_bullets # v9.6

            final_bullets = verbatim_bullets + custom_bullets + fact_checked_bullets
            ...
            return {"bullets": final_bullets, "provenance": provenance_log} # v9.5
...
class BulletCritiqueAgent(BaseAgent):
    """v9.6: New agent to critique the assembled bullet list."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "BulletCritiqueAgent"
        self.client = get_model_client("google", "gemini-2.5-flash")

    def run(self, generated_bullets: List[str], strategy_brief: dict) -> dict:
        self.log_info(f"Running BulletCritiqueAgent...")
        if not generated_bullets or not strategy_brief:
            raise HopExecutionError("Missing bullets or strategy for critique.")

        try:
            user_prompt = BULLET_CRITIQUE_USER_PROMPT.format(
                strategy_brief=json.dumps(strategy_brief),
                generated_bullets=json.dumps(generated_bullets)
            )
            messages = [{"role": "system", "content": BULLET_CRITIQUE_SYSTEM_PROMPT}, {"role": "user", "content": user_prompt}]
            
            response = self.client.chat_completion(messages=messages, response_format="json_object")
            
            self.log_info(f"Bullet critique complete. Passed: {response.get('passed')}")
            return response

        except Exception as e:
            self.log_error(f"Error during bullet critique: {e}")
            raise HopExecutionError(f"BulletCritiqueAgent failed: {e}")


# ============================================================================
# PART 5 (v9.5): NEW DRAFTING STACK (Req #1)
# ============================================================================

class DraftingConductorAgent(BaseAgent):
    """v9.6: Replaces AdversarialDraftingRouter. (Req #1)
    ...
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        ...
        # v9.6: Initialize all drafting experts
        self.drafters = {
    ...
    def run(self) -> str:
        self.log_info("Running Step 3: Dynamic MoE Drafting (v9.5)...")
...
# ============================================================================
# PART 6: REPLANNER AGENT (v9.5 Upgrade)
# ============================================================================

class WorkflowRePlannerAgent(BaseAgent):
    """
    v9.5 Step 3 (SC Paths): Workflow Re-Planner
    ...
    def run(self, failure_context: dict) -> dict:
        self.log_info("Running Step 3 (SC Paths): Workflow Re-Planner (v9.5)...")
        if self.client is None:
        ...
        try:
            ...
            messages = [
                ...
                {"role": "user", "content": REPLANNER_USER_PROMPT.format(failure_context=context_str)}
            ]
...
class RePlanCritiqueAgent(BaseAgent):
    """v9.6: Adversarially reviews the replan."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "RePlanCritiqueAgent"
        self.client = get_model_client("google", "gemini-2.5-flash")

    def run(self, plan: dict, failure_context: dict) -> dict:
        self.log_info("Running adversarial critique on replan...")
        try:
            prompt = REPLANNER_CRITIQUE_SYSTEM_PROMPT.format(
            ...
            response = self.client.chat_completion(messages=messages, response_format="json_object")
            if not response.get("critique_passed", False):
                self.log_warning(f"Re-Plan Critique FAILED: {response.get('reason')}")
            return response
        except Exception as e:
            self.log_error(f"RePlanCritiqueAgent failed: {e}")
            return {"critique_passed": False, "reason": str(e)}



# ============================================================================
# PART 7 (v9.5): NEW QA STACK (Req #1)
# ============================================================================

class QAConductorAgent(BaseAgent):
    """v9.6: Replaces AtomicQASwarmLLM (Req #1)
    ...
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        ...
        self.conductor_client = get_model_client("google", "gemini-2.5-flash")
        self.plan_critique_agent = QAPlanCritiqueAgent(blackboard, debug_mode) # v9.6
    ...
    def run(self, final_draft: str) -> List[dict]:
        self.log_info("Running Step 4: Dynamic MoE QA Swarm (v9.5)...")
        ...
        # 2. v9.6: Critique the plan
        critiqued_plan_response = self.plan_critique_agent.run(plan, final_draft, strategy)
        critiqued_plan = critiqued_plan_response.get("plan", plan)
        if critiqued_plan != plan:
            self.log_warning(f"QA Plan Critique modified plan. Original: {plan}, New: {critiqued_plan}")
        plan = critiqued_plan # Use the critiqued plan
        
        # 3. Execute the dynamic plan (in parallel)
        for agent_name in plan:
        ...
        self.log_info(f"Dynamic MoE QA Swarm (v9.0) complete. Ran {len(all_results)} checks.")
        self.blackboard.update_artifact("qa_llm_results", all_results)
        return all_results



class QAPlanCritiqueAgent(BaseAgent):
    """v9.6: Adversarially reviews the QA Conductor's plan."""
    def __init__(self, blackboard: WorkflowBlackboard, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "QAPlanCritiqueAgent"
        self.client = get_model_client("google", "gemini-2.5-flash")

    def run(self, plan: List[str], final_draft: str, strategy_brief: dict) -> dict:
        self.log_info("Running adversarial critique on QA plan...")
        try:
            prompt = QA_PLAN_CRITIQUE_SYSTEM_PROMPT.format(
                final_draft=final_draft[:2000], # Truncate
                strategy_brief=json.dumps(strategy_brief),
                plan=json.dumps(plan)
            )
            messages = [{"role": "user", "content": prompt}]
            ...
            self.log_error(f"QAPlanCritiqueAgent failed: {e}")
            return {"plan": plan} # Return original plan on failure



class AtomicQASwarmLogic(BaseAgent):
    """
    v9.5 Step 4: Atomic QA Swarm (Logic/Python)
...
class PreferenceCaptureAgent(BaseAgent):
    ...
    """
    def __init__(self, blackboard: WorkflowBlackboard, log_path: str, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.agent_name = "PreferenceCaptureAgent"
        self.model_name = "gemini-2.5-flash"
        self.client_name = "google"
        self.log_path = log_path # v9.6: Added log path
...
"""
            ...
            # v9.6: Close the HIL loop by logging the preference
            try:
                ...
                self.log_error(f"Failed to write preference log: {log_e}")
...
def get_blackboard(state: 'GraphState') -> WorkflowBlackboard:
    ...
    return WorkflowBlackboard(
        ...
        workflow_id=state["workflow_id"] # v9.6: Pass workflow_id
    )
...
def run_rag_stack(state: 'GraphState') -> Dict[str, object]:
    ...
    # 2. v9.6: Query Adversary
    adversary_agent = QueryAdversaryAgent(blackboard, debug_mode=True)
    critiqued_queries = adversary_agent.run(queries, state["artifacts"].get("strategy_brief", {}))
    
    # 2. Search
    search_agent = RAG_SearchAgent(blackboard, debug_mode=True)
    search_results = search_agent.run(queries=critiqued_queries) # v9.6: Use critiqued queries
    ...
    # v9.6: Add to Provenance Ledger (Item #2)
    provenance_entry = {
        "step": "rag_stack",
        "queries": queries,
        "critiqued_queries": critiqued_queries,
        "traces": search_results.get("traces", []),
        "critique": critique_result
    }
    
    return {"artifacts": {
        "rag_queries": critiqued_queries, # Store the ones we used
        "rag_search_results": search_results.get("results", []),
        "rag_critique": critique_result,
    }, "provenance_ledger": [provenance_entry]} # v9.5

def run_bullet_stack(state: 'GraphState') -> Dict[str, object]:
    """v9.5 Node: Runs the new Advanced Bullet Swarm (Req #2)"""
    blackboard = get_blackboard(state)
    
    # 1. ProvenanceRouterAgent
    bullet_agent = ProvenanceRouterAgent(blackboard, debug_mode=True) # v9.5
    result = bullet_agent.run() # v9.6: result is a dict
    bullets = result.get("bullets", [])
    
    # v9.6: Add to Provenance Ledger (Item #2)
    provenance_entry = result.get("provenance", {})
    
    return {
        "artifacts": {"generated_bullets": bullets},
        "provenance_ledger": state["provenance_ledger"] + [provenance_entry] # v9.5
    }

def run_bullet_critique(state: 'GraphState') -> Dict[str, object]:
    """v9.5 Node: Runs the new BulletCritiqueAgent"""
    blackboard = get_blackboard(state)
    agent = BulletCritiqueAgent(blackboard, debug_mode=True)
    
    critique_results = agent.run(
        generated_bullets=state["artifacts"].get("generated_bullets", []),
        strategy_brief=state["artifacts"].get("strategy_brief", {})
    )
    return {"artifacts": {"bullet_critique_results": critique_results}}

def run_drafting_stack(state: 'GraphState') -> Dict[str, object]:
    """Node: Runs Step 3: Drafting (Prompt, Bullets, Adversarial)"""
    # v9.6: This node is now split into two: run_bullet_stack and run_drafting_stack
    ...
    drafting_agent = DraftingConductorAgent(blackboard, debug_mode=True) # v9.5
...
def run_qa_swarm(state: 'GraphState') -> Dict[str, object]:
    ...
    # 1. LLM QA
    qa_llm_agent = QAConductorAgent(blackboard, debug_mode=True) # v9.5
...
def run_replanner(state: 'GraphState') -> Dict[str, object]:
    ...
    # v9.6: Determine what failed (Bullets or QA)
    bullet_critique = state["artifacts"].get("bullet_critique_results", {})
    qa_results = state["artifacts"].get("validation_results", {})
    
    if not bullet_critique.get("passed", True): # Fails if 'passed' is False
        failure_context = bullet_critique
        logger.warning(f"⚠️ Bullet Critique Failed. Triggering Re-Planner.")
    elif not qa_results.get("overall_passed", True): # Fails if 'overall_passed' is False
        failure_context = qa_results.get("failed_checks", [])
        logger.warning(f"⚠️ QA Swarm Failed. Triggering Re-Planner.")
    else:
        logger.error("Replanner called without a failure. This should not happen.")
        return {"artifacts": state["artifacts"]}
        
    agent = WorkflowRePlannerAgent(blackboard, debug_mode=True) # v9.5 logic is now inside this agent
    plan = agent.run(failure_context=failure_context)
    ...
    for key in [
        "final_drafting_prompt", "generated_bullets", "bullet_critique_results",
        "final_draft",
        "qa_llm_results", "qa_logic_results", "validation_results", "recovery_plan",
        "replan_critique" # v9.6
    ]:
        artifacts.pop(key, None)
    
    artifacts["recovery_plan"] = plan
    artifacts["replan_critique"] = replan_critique # v9.6
    return {
        "artifacts": artifacts,
...
def run_preference_capture(state: 'GraphState') -> Dict[str, object]:
    ...
    blackboard = get_blackboard(state)
    agent = PreferenceCaptureAgent(
        blackboard,
log_path=CONFIG.meta_loop_config.preference_log_path, # v9.5
        debug_mode=True)
...
def check_bullet_critique(state: 'GraphState') -> str:
    """
    v9.5 Conditional Edge: Checks results from the BulletCritiqueAgent.
    """
    critique_results = state["artifacts"].get("bullet_critique_results", {})
    replan_count = state["replan_count"]
    max_replan = CONFIG.planner_config.max_replan_loops
    
    if critique_results.get("passed", False):
        logger.info("✅ Bullet Critique Passed. Routing to Drafting.")
        return "DRAFTING"
    elif replan_count >= max_replan:
        logger.error(f"❌ Bullet Critique Failed. Max replan loops ({max_replan}) reached. Halting.")
        return "FAIL_HALT"
    else:
        return "REPLANNER"
...
def get_graph_app(checkpointer: 'RedisSaver', enable_hil: bool = True) -> 'CompiledGraph':
    ...
    """
    from archives.legacy_resume_gen.Older Microservices Models.v10.7.vendor.langgraph.graph import StateGraph, END
    from archives.legacy_resume_gen.Older Microservices Models.v9.6.core_v9_6 import GraphState
    ...
    workflow.add_node("bullet_stack", run_bullet_stack) # v9.5
    workflow.add_node("bullet_critique", run_bullet_critique) # v9.5
    ...
    workflow.add_edge("rag_stack", "bullet_stack") # v9.5
    workflow.add_edge("bullet_stack", "bullet_critique") # v9.5
    
    # v9.6: Add Bullet Critique Loop
    workflow.add_conditional_edges(
        "bullet_critique",
        check_bullet_critique,
        {
            "DRAFTING": "drafting_stack",
            "FAIL_HALT": END,
            "REPLANNER": "replanner"
        }
    )
    ...
    workflow.add_edge("replanner", "bullet_stack") # v9.6: Re-plan must re-gen bullets
...
__all__ = [
    ...
    'ProvenanceRouterAgent', 'CustomizedBulletDrafterAgent', 'SyntheticBulletDrafterAgent', # v9.5
    'BulletCritiqueAgent', # v9.5
    'DraftingConductorAgent', # v9.5
    'WorkflowRePlannerAgent',
    'QAConductorAgent', 'AtomicQASwarmLogic' # v9.5
]
