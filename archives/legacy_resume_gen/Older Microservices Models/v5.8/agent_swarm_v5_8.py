# File: agent_swarm.py
# Version: 5.8.1 (MoE + Reflection + ReAct + Conductor + Hardening Patch)
...
from archives.legacy_resume_gen.Older Microservices Models.v5.8.validation_stack_v5_8 import ValidationEngine, ValidationContext, calculate_signal_score
...
class ConductorAgent:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        if not hasattr(self.config, 'strategies') or not self.config.strategies:
            raise ValueError("Conductor strategies are missing from master_config_v5_8.json.")
...
class WorkflowPlannerAgent:
    ...
    def __init__(self, strategy_type: str = "balanced"):
        ...
        if not self.default_plan_config:
            raise ValueError("planner_config.default_workflow_plan is missing from master_config_v5_8.json.")
