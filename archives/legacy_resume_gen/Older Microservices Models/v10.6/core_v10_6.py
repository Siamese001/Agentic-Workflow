...
# - FIXED: All v10_5 class names/imports updated to v10_6.
...
logger = logging.getLogger("core_v10_6")

# ============================================================================
# CONFIGURATION (v10.6: Fixed class name and paths)
# ============================================================================

class ConfigV10_6:
    """Configuration loader for v10.6"""
    
    def __init__(self, config_path: str = "master_config_v10_6.json"):
        with open(config_path, 'r') as f:
            self._config = json.load(f)
        
        # Validate schema version
        expected_schema = "master_config_v10.6"
...
# ============================================================================
# EXCEPTION HIERARCHY (v10.6: Preserved)
# ============================================================================
...
class CircuitBreaker:
    """v10.6: Circuit breaker for batch processing."""
...
# ============================================================================
# v10.6: PYDANTIC MODELS (Added Constitutional AI)
# ============================================================================
...
# ============================================================================
# v10.6: RESILIENCE & OBSERVABILITY (Preserved)
# ============================================================================

def exponential_backoff_retry(max_retries: int = 3, initial_delay: float = 1.0):
    """v10.6: Decorator for async node functions."""
...
def async_timeout(seconds: int):
    """v10.6: Decorator to enforce a timeout on an async node."""
    def decorator(func: Callable[..., Awaitable[Any]]) -> Callable[..., Awaitable[Any]]:
        @wraps(func)
        async def wrapper(*args, **kwargs) -> Any:
            try:
...
class ContextBudgetManager:
    """
    v10.6 (Fix #14): Manages context window limits using agentic pruning.
    """
    def __init__(self, 
                 config: ConfigV10_6,
    ...
    async def _prune_agentic(self, document: str, max_tokens: int) -> str:
        """v10.6 (Fix #14): Uses an LLM to prune text."""
        ...
        try:
            ...
            # v10.6 NOTE: We cannot use PromptTemplateManager here as it
    ...
    async def prune(self, document: str, max_tokens: Optional[int] = None) -> str:
        ...
        # v10.6 (Fix #14): Use agentic pruning
...
class MetricsCollector:
    ...
    def get_average_latency(self, agent_name: str, task_name: str) -> Optional[float]:
        """v10.6 (Fix #15): Gets average latency for a specific task."""
        latencies = [
...
def track_metrics(task_name: str):
    """v10.6: Decorator for agent/tool run/run_async methods."""
...
class SemanticValidator:
    """v10.6: Local, deterministic validation service."""
...
# ============================================================================
# v10.6: CENTRALIZED PROMPT FORMATTER (Fix #14, #19, #24)
# ============================================================================
...
async def _format_prompt_with_defaults(
    ...
    """
    v10.6: Centralized helper.
    Injects Goal State, Top Failures, and performs agentic pruning.
    """
    
    # v10.6 (Fix #19, #24): Inject Goal and Failures
    ...
    # v10.6 (Fix #14): Use agentic pruning for large context fields
    ...
    all_keys = {
        ...
        "final_draft": tool_input.get('final_draft', ''), # v10.6 (Fix #30)
        "constitution": tool_input.get('constitution', ''), # v10.6 (Fix #30)
    }
    
    return template.format(**all_keys)

# ============================================================================
# v10.6: PROMPT TEMPLATE MANAGER (Fix #17, #19, #20, #24, #30)
# ============================================================================

class PromptTemplateManager:
    """
    v10.6: Manages all 30+ system prompts.
    ...
    def __init__(self, feedback_reader: 'FeedbackLogReader'):
        ...
        # v10.6 (Fix #24): Get top failures on init
        self.top_failures = self._get_top_failures(feedback_reader)
        # v10.6 (Fix #19): Define global goal state
    ...
    def _get_top_failures(self, feedback_reader: 'FeedbackLogReader') -> List[str]:
        """v10.6 (Fix #24): Analyzes feedback log for top failure patterns."""
        try:
    ...
    def get_template(self, tool_name: str) -> str:
        ...
        # v10.6 (Fix #19, #24): Inject Goal State and Failures into *every* prompt
        injected_template = (
            f"{{goal_state}}\n"       # Fix #19
            f"{{top_failures}}\n"     # Fix #24
            f"-------------------\n"
    ...
    def _load_templates(self) -> Dict[str, str]:
        """
        v10.6 (Fix #17, #20): Defines all system prompts using Cognitive Modes.
        """
        templates = {
            # === DRAFTING TOOLS ===
...
""",
            ...
            "agentic_pruning": "MODE: ANALYTICAL. TASK: Summarize document to its essential points. Max chars: {max_chars}. DOCUMENT: {document} SUMMARY:", # v10.6 (Fix #14)
...
# ============================================================================
# v10.6: RESPONSE VALIDATOR (Preserved)
# ============================================================================

class ResponseValidator:
    """v10.6: Central utility to parse and validate LLM JSON."""
    def __init__(self):
...
# ============================================================================
# ROW 7: FEEDBACK LOG READER (v10.6: Added failure getter)
# ============================================================================
...
# ============================================================================
# ROW 5: CACHE MANAGER (v10.6: Fix #13 - Semantic Caching)
# ============================================================================
...
class CacheManager:
    def __init__(self, 
        ...
        self._semantic_hits = 0 # v10.6 (Fix #13)
        
        # v10.6 (Fix #13): Init Semantic Cache
    ...
    def _generate_tool_cache_key(self, tool_name: str, tool_input: Dict[str, object]) -> str:
        try:
            ...
            return f"tool_cache_v10_6:{hashlib.sha256(key_str.encode()).hexdigest()}"
...
# ============================================================================
# ROW 4: COST TRACKER (v10.6: Preserved)
# ============================================================================
...
# ============================================================================
# BASE AGENT CLASS (v10.6: Fix #15 - Cost/Latency Routing)
# ============================================================================

class BaseAgent:
    """Base class for all agents with v10.6 context injection"""
    ...
    def get_model_client(self, model_config_name: str) -> "AsyncBaseModelClient":
        """
        v10.6 (Fix #15): Gets model client.
        ...
        # v10.6 (Fix #19, #24): Inject prompt context
...
# ============================================================================
# v10.6: BASE TOOL INTERFACE (Preserved)
# ============================================================================
...
# ============================================================================
# ROW 6: ASYNC LLM CLIENTS (v10.6: Fix #29 - Idempotency)
# ============================================================================
...
class AsyncBaseModelClient:
    def __init__(self, 
        ...
        # v10.6: Injected by get_model_client
        self.goal_state: str = ""
    ...
    async def chat_completion_async(self, messages: List[Dict[str, str]], 
        ...
        if cached_response:
            # v10.6 (Fix #29): Idempotency Validation
...
# ============================================================================
# ROW 4: WORKFLOW CONTEXT (v10.6: DI Fixes)
# ============================================================================

class WorkflowContext:
    """
    v10.6: True Dependency Injection container.
    """
...
def create_workflow_context(config: ConfigV10_6, db: int = 0) -> WorkflowContext:
    ...
    # 4. v10.6 (Fix #14): Resolve circular dependency for ContextBudgetManager
...
def cleanup_workflow_chroma_collection(context: WorkflowContext):
    """v10.6 REFACTOR: Centralized ChromaDB cleanup logic."""
...
# v10.6 (Fix #10): Agent-to-Agent Communication State
@dataclass
class A2AMessage:
    sender: str
...
class MainGraphState:
    ...
    def from_dict(cls, data: Dict[str, object]) -> 'MainGraphState':
        ...
        state.a2a = A2AContext(**data.get("a2a", {})) # v10.6 (Fix #10)
