...
from archives.legacy_resume_gen.Older Microservices Models.v9.0.main_v9_0 import setup_logging, load_job_input
from archives.legacy_resume_gen.Older Microservices Models.v9.0.core_v9_0 import CONFIG, CircuitBreakerOpenError

# v9.0: Import the graph builder and checkpointer
from archives.legacy_resume_gen.Older Microservices Models.v9.0.agent_swarm_v9_0 import get_graph_app
from langgraph.checkpoint.redis import RedisSaver

# v9.0: Import the meta-learner
try:
    from archives.legacy_resume_gen.Older Microservices Models.v9.0.run_learning_v9_0 import run_meta_learning
    META_LEARNER_AVAILABLE = True
except ImportError:
    META_LEARNER_AVAILABLE = False


logger = logging.getLogger("batch_runner_v9_0")
try:
...
SUMMARY_FILE = "batch_summary_v9_0.csv"
...
def process_single_job(job_path: str) -> Dict[str, object]:
    ...
    logger.info(f"--- Starting v9.0 job: {os.path.basename(job_path)} ---")
    ...
    try:
        ...
        logger.info(f"--- Finished v9.0 job: {company} - {title} ---")
...
def run_batch():
    ...
    if not job_files:
        logger.info("v9.0 Batch process starting. No jobs found in queue.")
        return

    logger.info(f"v9.0 Batch process starting. Found {len(job_files)} jobs.")
    ...
    logger.info(f"v9.0 Batch process complete.")
    ...
    if CONFIG.meta_loop_config.enable_meta_learning:
        ...
        if META_LEARNER_AVAILABLE:
            ...
            logger.warning("Meta-learning is enabled, but 'run_learning_v9_0' could not be imported.")
    else:
