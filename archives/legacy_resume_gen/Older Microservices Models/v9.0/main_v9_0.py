...
# Import from consolidated modules (v9.0)

# v9.0: Import from the new graph builder
...
# Version info
__version__ = "9.0.0-agentic-loops"
...
def setup_logging(debug_mode: bool):
    ...
    """
    try:
        from archives.legacy_resume_gen.Older Microservices Models.v9.0.core_v9_0 import CONFIG
    except ImportError:
        print("CRITICAL: core_v9_0.py not found. Logging setup failed.", file=sys.stderr)
...
def print_summary(results: Dict[str, object], workflow_id: str, start_time: datetime):
    ...
    print("\n" + "=" * 80)
    print(f"WORKFLOW EXECUTION SUMMARY (v9.0)")
    print("=" * 80)
...
def main():
    """
    Main entry point for V8.5 Workflow.
    """
    script_path = os.path.relpath(__file__) if "__file__" in globals() else "main_v9_0.py"
    ...
    parser = argparse.ArgumentParser(
        description=f'V8.5 LangGraph Workflow (Version: {__version__})',
...
Examples:
  ...
  python {script_path} --debug                   # Enable debug mode
    ...
    parser.add_argument(
        '--version',
        action='version',
        version=f'V8.5 LangGraph Workflow - Version {__version__}'
    )
    ...
    try:
        ...
        logger.info(f"🚀 Starting v9.0 (Cleanup) Workflow. ID: {workflow_id}")
        ...
        if args.output_dir:
            ...
            if final_draft:
                draft_file = output_path / f"{base_filename}_final_draft_v9.0.txt"
