# File: prompts_RES.py
# Version: 17.00 - V2 Agentic Architecture Compatible
...
def build_crl_context_for_section(
    ...
    **kwargs
    ...
    if "HEADLINE" in section_name:
        ...
        # This case seems incomplete in original file, but we'll preserve it
        return _build_bullets_context_dict(thematic_analysis, enriched_scaffold, section_name)
    elif "NARRATIVE" in section_name:
        # This case seems incomplete in original file, but we'll preserve it
        return _build_narrative_context_dict(thematic_analysis, enriched_scaffold, section_name)
    elif "COMPETENCIES" in section_name:
        # This case seems incomplete in original file, but we'll preserve it
        return _build_competencies_context_dict(thematic_analysis, enriched_scaffold)
...
def _build_narrative_context_dict(
    ...
    **kwargs
    ...
    narrative_config = config.artist.narrative_config if hasattr(config, 'artist') and hasattr(config.artist, 'narrative_config') else {}
    
    # Convert string section_name to enum
    try:
        section_enum = ResumeSection[section_name]
    except KeyError:
        raise HopExecutionError(f"Invalid section_name '{section_name}' for narrative context")
    
    if section_enum not in narrative_config:
        raise HopExecutionError(f"Missing/invalid config for narrative generation: {section_enum.name}")
    ...
    section_config = narrative_config[section_enum]
    
    # --- FIX: Read constraints dynamically from the constraints object ---
    # The old logic assumed keys like "min_wc_key" existed, which they don't.
    min_wc = getattr(constraints, f"{section_enum.name}_WORD_COUNT_MIN", 40)
    max_wc = getattr(constraints, f"{section_enum.name}_WORD_COUNT_MAX", 60)
    # --- END FIX ---
    
    combined_signals = list(set(rag_keywords + section_config["rag_signals"]))[:7]
    focus_instruction = section_config["focus"]
    k0_themes = section_config["k0_themes"]
...
# --- REFACTOR: Added FALLBACK stubs for missing context builders ---
def _build_bullets_context_dict(thematic_analysis: 'ThematicAnalysis', scaffold: Dict, section_name: str) -> Dict:
    """Build context dictionary for bullets."""
    logging.warning(f"Using fallback context builder for {section_name}")
    return {"section_name": section_name, "primary_theme": "Fallback Theme"}

def _build_competencies_context_dict(thematic_analysis: 'ThematicAnalysis', scaffold: Dict) -> Dict:
    """Build context dictionary for competencies."""
    logging.warning(f"Using fallback context builder for K9_COMPETENCIES")
    return {"section_name": "K9_COMPETENCIES", "primary_theme": "Fallback Theme"}
# --- END REFACTOR ---
...
def build_hil_brief_prompt(
    ...
    """
    V2 NEW: Builds prompt for generating HIL escalation brief.
...
def build_bullet_generation_prompt(
    ...
    template = _get_prompt_template("artist_bullet_generation")
...
__version__ = "17.00"
__architecture__ = "v2"
__compatible_with__ = ["workflow_RES_v2.py", "governor.py"]
