...
"""
    ...
    def _rewrite_bullet_for_word_count(self, original_bullet_text: str, target_word_count_range: Tuple[int, int], section_id_str: str, temperature_override: Optional[float] = None, max_retries: int = 3) -> Tuple[str, int]:
        """
        Rewrites a bullet for word count with temperature-based retry.
        
        CRITICAL FIX (v15_63): Implements progressive temperature reduction for HIGH/CRITICAL QA rules.
        - Attempt 1: temp = 1.0 (or override if provided)
        - Attempt 2: temp = 0.7
        - Attempt 3: temp = 0.4
        
        This ensures compliance with: "All Critical and High QA rules require retry at lower 
        temperature levels up to maximum number of retries"
        ...
        # Define temperature schedule: start high, progressively reduce
        # If temperature_override provided, use it for first attempt, then reduce
        if temperature_override is not None:
            temperature_schedule = [temperature_override, 0.7, 0.4]
        else:
            temperature_schedule = [1.0, 0.7, 0.4]
...
4. Preserve all metrics, numbers, and specific achievements.
...
"""
                ...
                try:
                    rewritten_text, call_count = self._call_gemini_api(
                        prompt, reasoning_config, f"{section_id_str}_RewriteWC", 
                    ...
                    if min_wc <= rewritten_wc <= max_wc:
                        ...
                        logging.warning(
                            ...
                            f"{'Retrying with lower temperature...' if attempt < max_retries - 1 else 'No more retries.'}"
                        )
        ...
        for i, bullet_data in enumerate(selected_bullets_structured):
            ...
            if not (min_target <= word_count <= max_target):
                ...
                logging.warning(
                    ...
                    f"Attempting rewrite with temperature-based retry (temps: 1.0→0.7→0.4) for bullet: '{original_text[:50]}...'"
                )
                try:
                    # CRITICAL FIX (v15_63): Pass max_retries=3 to enable temperature-based retry
                    # This ensures compliance with HIGH/CRITICAL QA rule retry requirements
                    rewritten_text, rewrite_calls = self._rewrite_bullet_for_word_count(original_text, (min_target, max_target), f"{section_id_str}_RewriteWC_{i}", temperature_override, max_retries=3)
