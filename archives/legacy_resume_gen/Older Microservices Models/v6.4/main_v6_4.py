...
# Version: 6.4 (Parallel Batch Harness)
# REFACTORED: main() is now importable via `if __name__ == "__main__":` guard
# This allows WorkflowV64 to be safely imported by batch processing scripts.
...
# Version info
__version__ = "6.4.0-agentic-parallel"
...
class WorkflowV64:
    """
    Main workflow class for v6.4 Batch Harness.
    ...
    def __init__(self, config_overrides: Optional[Dict] = None):
        ...
        self.logger.info(f"Workflow v6.4 initialized (version: {__version__})")
    ...
    def run(
        ...
        """
        self.logger.info("=" * 80)
        self.logger.info(f"Starting Workflow v6.4 for {company_name} - {job_title}")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            self.logger.info("=" * 80)
            self.logger.info(f"Workflow v6.4 execution finished [workflow_id: {workflow_id}]")
            self.logger.info("=" * 80)
...
# This allows `WorkflowV64` to be safely imported
# by other scripts, like `run_batch_v6_4.py`.
# ============================================================================

def main():
    """
    Main entry point for V6.4 Batch Harness Workflow.
    REFACTORED: Reads defaults from CONFIG. Now importable for batch processing.
    """
    script_path = os.path.relpath(__file__) if "__file__" in globals() else "main_v6_4.py"
    ...
    parser = argparse.ArgumentParser(
        description=f'V6.4 MoE + Reflection + ReAct + Conductor System (Version: {__version__})',
...
Examples:
  ...
  python {script_path} --test                    # Run with test data
    ...
    parser.add_argument(
        '--version',
        action='version',
        version=f'V6.4 MoE + Reflection + ReAct + Conductor System - Version {__version__}'
    )
    ...
    try:
        ...
        # Initialize workflow
        workflow = WorkflowV64(config_overrides=config_overrides)
