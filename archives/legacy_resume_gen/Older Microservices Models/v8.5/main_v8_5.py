# File: main_v8_5.py
# Overwrites: main_v8_0.py
# Version: 8.5 (Cleanup)
#
# v8.5 (Cleanup) CHANGES:
# - Imports from v8_5 modules.
# - Updated version string to 8.5.0-cleanup.
...
# Import from consolidated modules (v8.5)

# v8.5: Import from the new graph builder
from archives.legacy_resume_gen.Older Microservices Models.v8.5.agent_swarm_v8_5 import get_graph_app
...
def setup_logging(debug_mode: bool):
    ...
    """
    try:
        from archives.legacy_resume_gen.Older Microservices Models.v8.5.core_v8_5 import CONFIG
    except ImportError:
        print("CRITICAL: core_v8_5.py not found. Logging setup failed.", file=sys.stderr)
...
def print_summary(results: Dict[str, object], workflow_id: str, start_time: datetime):
    ...
    print("\n" + "=" * 80)
    print(f"WORKFLOW EXECUTION SUMMARY (v8.5)")
    print("=" * 80)
...
def main():
    ...
    """
    script_path = os.path.relpath(__file__) if "__file__" in globals() else "main_v8_5.py"
...
Examples:
  ...
  python {script_path} --debug                   # Enable debug mode
    ...
    try:
        ...
        logger.info(f"🚀 Starting v8.5 (Cleanup) Workflow. ID: {workflow_id}")
        ...
        if args.output_dir:
            ...
            if final_draft:
                draft_file = output_path / f"{base_filename}_final_draft_v8.5.txt"
