# File: run_learning_v8_5.py
# Overwrites: run_learning_v8_0.py
# Version: 8.5 (Cleanup)
#
# v8.5 (Cleanup) CHANGES:
# - Imports from core_v8_5.
...
from archives.legacy_resume_gen.Older Microservices Models.v8.5.core_v8_5 import CONFIG, setup_logging

logger = logging.getLogger("meta_learner_v8_5")
...
def run_meta_learning():
    """
    Runs the asynchronous meta-learning loop (v8.5).
    """
    try:
    ...
    logger.info(f"===== Starting v8.5 Meta-Learning Loop ({datetime.now().isoformat()}) =====")
    
    if not CONFIG.meta_loop_config.enable_meta_learning:
        logger.info("Meta-learning is disabled in master_config_v8_5.json. Exiting.")
        return
    ...
    if not patterns:
        ...
        logger.info("===== v8.5 Meta-Learning Loop Complete =====")
        return
