...
class ValidationContext:
    ...
    def __init__(self, staging_buffer: ImmutableStagingBuffer, thematic_analysis: ThematicAnalysis, 
        ...
        if not hasattr(self.config, 'constraints') or not self.config.constraints:
            raise ValueError("FATAL: Validation constraints are missing from master_config_v6_2.json.")
        if not hasattr(self.config, 'signal_constraints') or not self.config.signal_constraints:
            raise ValueError("FATAL: Signal constraints are missing from master_config_v6_2.json.")
...
class ValidationEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        if not self._AGENT_DEFINITIONS or not vars(self._AGENT_DEFINITIONS):
            raise ValueError("FATAL: agent_definitions.validation_agents is missing from master_config_v6_2.json.")
    ...
    def _get_agent_config(self, agent_name: str) -> AtomicAgentConfig:
        ...
        if not hasattr(self._AGENT_DEFINITIONS, agent_name):
            raise ValueError(f"Agent definition '{agent_name}' not found in master_config_v6_2.json.")
    ...
    def _create_router(self, router_name: str) -> BaseMoERouter:
        ...
        if not hasattr(CONFIG.moe_config.routers, router_name):
            raise ValueError(f"MoE router config '{router_name}' not found in master_config_v6_2.json.")
    ...
    def validate_all(self, context: ValidationContext) -> Dict[str, object]:
        ...
        """
        self.logger.info("Starting v6.2 MoE validation...")
