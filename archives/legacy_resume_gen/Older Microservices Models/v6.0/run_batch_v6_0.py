# File: run_batch_v6_0.py
# NEW: Batch processing harness for v6.0
# Processes all job_input.json files in batch_queue/ folder
# Writes one-line summaries to batch_summary_v6_0.csv
...
from archives.legacy_resume_gen.Older Microservices Models.v6.0.main_v6_0 import WorkflowV60, setup_logging
from archives.legacy_resume_gen.Older Microservices Models.v6.0.core_v6_0 import CONFIG, CircuitBreakerOpenError

logger = logging.getLogger("batch_runner_v6_0")
setup_logging(debug_mode=False)

BATCH_QUEUE_DIR = "batch_queue"
BATCH_COMPLETE_DIR = "batch_complete"
SUMMARY_FILE = "batch_summary_v6_0.csv"
...
def run_batch():
    ...
    logger.info(f"v6.0 Batch process starting. Found {len(job_files)} jobs.")
    consecutive_failures = 0
    FAILURE_THRESHOLD = 3  # v6.0: Alerting - halt batch after 3 consecutive failures
    ...
    for job_file in job_files:
        ...
        try:
            ...
            logger.info(f"--- Starting v6.0 job: {company} - {title} ---")

            # --- v6.0: Cost Ceiling Check (Spell #6) - ACTIVATED ---
        ...
        # --- v6.0: Circuit Breaker Catch (Spell #5) ---
