...
class AgentSignal(Enum):
    """Signals used for inter-agent communication and Governor control flow."""
    PASS = "PASS"
    VETO = "VETO"
    RETRY = "RETRY"
...
class GovernanceState(Enum):
    """States for the Governor's main control loop."""
    INIT = auto()
    META_LEARNING = auto()
...
@dataclass
class VetoRecord:
    """Record of a VETO signal from a QA specialist."""
    source_agent: str
    target_draft_id: str
    reason: str
    severity: ValidationSeverity
...
class ExecutionStrategy:
    """Dynamic strategy defined by ChiefStrategistAgent."""
    name: str
    focus_areas: List['ResumeSection']
