...
def _build_skills_context_dict(
    ...
    **kwargs
    ...
    try:
        ...
        return {
            "job_description": fence_data(job_description[:2000], "UNTRUSTED_JOB_DESCRIPTION"), # T3: XML Fence
            "primary_theme": primary_theme_name,
...
def build_generation_prompt_with_reinforced_constraints(
    ...
    negative_constraints = [
        ...
        f"- DO NOT use forbidden verbs: {', '.join(forbidden_verbs[:5])}..."
    ]
    neg_constraint_block = "\nNEGATIVE CONSTRAINTS:\n" + "\n".join(negative_constraints) + "\n"
    # ----------------------------------------------------------------
    
    if attempt_number == 1:
        constraint_language = f"""{neg_constraint_block}
