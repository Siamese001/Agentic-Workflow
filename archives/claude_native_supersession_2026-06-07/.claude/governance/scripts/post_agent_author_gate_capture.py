#!/usr/bin/env python3
"""
post_agent_author_gate_capture.py — Cursor afterAgentResponse Author-Gate capture hook.
...
def _infer_layer(repo_area: str) -> str:
    ...
    if p.startswith("tools/"):
        return "tools"
    if p.startswith(".claude/") or p.startswith("docs/archive/windsurf/legacy-tree/") or p.startswith("ops_scripts/"):
        return "harness"
    return ""
...
def _extract_response_text(payload: object) -> str:
    ...
    if isinstance(payload, dict):
        # post_agent_response puts response text at tool_info.response (Windsurf docs)
