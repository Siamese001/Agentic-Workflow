...
def load_lic_capabilities() -> Dict[str, object]:
    ...
    try:
        ...
        print(f"⚠️  Warning: Could not load LIC capabilities ({e})")
...
def print_validation_summary(validations: list, title: str):
    ...
    if not validations:
        print(f"✅ {title}: No validations")
        return
    
    failed = [v for v in validations if not v.passed]
    passed = len(validations) - len(failed)
    
    print(f"📊 {title}: {passed}/{len(validations)} passed")
...
def demo_complete_workflow():
    ...
    print("✅ All 13 capability engines initialized successfully")
    ...
    print(f"📤 Sender: {sender_profile['name']} - {sender_profile['title']} at {sender_profile['company']}")
    print(f"📁 Job Context: {job_context['title']} at {job_context['company']}")
    ...
    for i, recipient_profile in enumerate(recipient_profiles, 1):
        ...
        print(f"🎯 Classification: {classification.recipient_type} (confidence: {classification.confidence:.2f})")
        print(f"📋 Analysis: {analysis['personalization_strategy']['approach']} approach")
        ...
        print(f"🛤️  Route: {route.value}")
        print(f"👤 Archetype: {context.archetype.value}")
        ...
        print(f"🔍 Evidence Retrieved: {len(rag_result.evidence)} items")
        print(f"📊 Confidence Score: {rag_result.confidence_score:.3f}")
        print(f"⏱️  Processing Time: {rag_result.processing_time_ms}ms")
        if rag_result.hyde_profile:
            print(f"💭 HyDE Profile Generated: {len(rag_result.hyde_profile)} chars")
        ...
        if components.get('subject_line'):
            print(f"📧 Subject: {components['subject_line']}")
        print(f"📞 CTA: {components['cta']}")
        print(f"✍️  Signature: {components['signature'].replace(chr(10), ' | ')}")
        ...
        print(f"📊 Word Count: {len(formatted_message.split())} words")
        ...
        if not critical_failures:
            print("✅ MESSAGE READY FOR DELIVERY")
        else:
            print("⚠️  MESSAGE NEEDS REVISION: " + str(len(critical_failures)) + " critical issues")
        
        print("📈 PROCESSING SUMMARY:")
        print("   • Total Validations: " + str(len(all_validations)))
        print("   • Passed: " + str(len([v for v in all_validations if v.passed])))
        print("   • Failed: " + str(len([v for v in all_validations if not v.passed])))
        print("   • Critical Issues: " + str(len(critical_failures)))
...
def main():
    ...
    try:
        ...
        print("🎉 OUTREACH ENGINE DEMO COMPLETED SUCCESSFULLY!")
        print("✅ All 13 capability modules demonstrated:")
        print("   • Models & Data Structures")
        print("   • Routing Engine")
        print("   • Configuration Management")
        print("   • RAG Pipeline v75 (6-stage)")
        print("   • Insights Engine")
        print("   • CTA Engine")
        print("   • Tone & Language Adaptation")
        print("   • Constraints & Validation")
        print("   • Entity Grounding Framework")
        print("   • Template Engine")
        print("   • K-Node Assembly")
        print("   • Seniority Classification")
        print("   • Message Schemas")
        print("\n🚀 Phase F LIC Capability Integration is COMPLETE!")
        print("📚 See outreach_engine/README.md for comprehensive documentation")
