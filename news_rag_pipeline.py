...
class NewsRAGPipeline:
    ...
    def __init__(self):
        ...
        if not self.brave_key:
            logger.warning("⚠️ BRAVE_SEARCH_API_KEY not found. News RAG will be disabled.")
    ...
    def _search_company_news(self, company: str, industry: str) -> List[NewsInsight]:
        ...
        try:
            ...
            logger.info(f"📰 Found {len(insights)} relevant news items for {company}")
            return insights
    ...
    def _search_industry_trends(self, industry: str) -> List[NewsInsight]:
        ...
        try:
            ...
            for result in results[:2]:
                ...
                insight.talking_points = [f"Industry trend: {insight.headline}"]
                insights.append(insight)
            
            logger.info(f"📈 Found {len(insights)} industry trends for {industry}")
            return insights
        ...
        for insight in insights[:2]:  # Top 2 insights
            for talking_point in insight.talking_points:
                ...
                if "product launch" in talking_point.lower():
                    points.append(f"Congrats on the recent launch - must be exciting to see it come to fruition!")
                elif "growth milestone" in talking_point.lower():
                    points.append(f"The recent growth milestone is impressive - speaks to the team's execution.")
                elif "strategic partnership" in talking_point.lower():
                    points.append(f"The strategic partnership announcement caught my eye - smart move for expansion.")
                elif "industry trend" in talking_point.lower():
                    points.append(f"The industry trends are fascinating - curious about your perspective on these shifts.")
                else:
                    points.append(f"The recent developments seem promising - would love to learn more.")
    ...
    def execute_news_rag(
        ...
        personalization_points = self._generate_personalization_points(insights)
