...
class TraceRegistry(MCPHardenedMixin):
    ...
    def get_by_type(self, trace_type: str) -> List[Dict[str, Any]]:
        ...
        return [
            {"trace_id": t.trace_id, "agent_name": t.agent_name, "action": t.action, 
             "status": t.status, "duration_ms": t.duration_ms}
            for t in self._traces if t.action == trace_type
        ]
    ...
    def get_latest(self, trace_type: str) -> Dict[str, Any] | None:
        ...
        if not matches:
            return None
        t = matches[-1]
        return {"trace_id": t.trace_id, "agent_name": t.agent_name, "action": t.action, 
                "status": t.status, "duration_ms": t.duration_ms}
