...
class DiffGeneratorArgs(BaseModel):
    ...
    original: str = Field(description="Original code/text content")
    modified: str = Field(description="Modified code/text content")
    format: DiffFormat = Field(default=DiffFormat.UNIFIED, description="Output format for the diff")
    context_lines: int = Field(default=3, description="Number of context lines around changes")
    original_name: str = Field(
        default="original", description="Name/path for original file in diff header"
    )
    modified_name: str = Field(
        default="modified", description="Name/path for modified file in diff header"
    )
...
def generate_unified_diff(
    ...
    return DiffResult(
        success=True, diff=diff_text, format="unified", stats=stats, patch_applicable=True
    )
...
def generate_context_diff(
    ...
    return DiffResult(
        success=True, diff=diff_text, format="context", stats=stats, patch_applicable=True
    )
...
def generate_ndiff(original: str, modified: str) -> DiffResult:
    """
...
def apply_patch(original: str, patch: str, reverse: bool = False) -> PatchResult:
    """
    ...
    if not patch.strip():
        return PatchResult(
            success=True, patched_content=original, warnings=["Empty patch - no changes applied"]
        )
    ...
    try:
        ...
        if not hunks:
            return PatchResult(
                success=False, patched_content=original, error="No valid hunks found in patch"
            )
        ...
        return PatchResult(
            success=False, patched_content=original, error=f"Patch application failed: {str(e)}"
        )
...
def validate_patch(original: str, patch: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not hunks:
            return {"valid": False, "error": "No valid hunks found in patch", "hunks": 0}
        ...
        return {"valid": False, "error": str(e), "hunks": 0}
...
def _calculate_diff_stats(original_lines: list[str], modified_lines: list[str]) -> dict[str, int]:
...
def _apply_hunk(lines: list[str], hunk: dict[str, Any]) -> tuple:
...
def generate_diff(args: DiffGeneratorArgs) -> dict[str, Any]:
    ...
    if args.format == DiffFormat.UNIFIED:
        result = generate_unified_diff(
            args.original, args.modified, args.original_name, args.modified_name, args.context_lines
        )
        ...
        result = DiffResult(
            success=False, diff="", format=str(args.format), error=f"Unknown format: {args.format}"
        )
...
def quick_html_diff(original: str, modified: str) -> str:
    ...
    args = DiffGeneratorArgs(original=original, modified=modified, format=DiffFormat.HTML)
    result = generate_diff(args)
    return result.get("diff", "")
