...
class GapClosureEngine(BaseRGEngine):
    ...
    async def execute(
        self, jd_keywords: list[str], candidate_skills: list[str]
    ) -> list[CompetencyItem]:
        """
    ...
    def _parse_output(self, text: str) -> list[CompetencyItem]:
        ...
        return [
            ...
            CompetencyItem(
                "Technical Leadership", "Directed engineering teams and architecture", 24
            ),
            CompetencyItem(
                "Stakeholder Management", "Managed relationships with key stakeholders", 23
            ),
            CompetencyItem(
                "Process Optimization", "Optimized workflows and delivery processes", 22
            ),
