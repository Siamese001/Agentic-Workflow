...
from FactChecker import HallucinationException
...
def _get_missing_keywords(JobDescription: str, user_skills_and_history: Dict[str, Any], search_records_tool: Any, Logger: Optional[Any] = None) -> Optional[list]:
    """Identifies Missing keywords using Pinecone."""
    pinecone_query = f"""
    JD Text: {JobDescription}
...
def _generate_skill_gap_report(user_name: str, JobDescription: str, missing_keywords: list) -> str:
...
---
Target Job Description: {JobDescription[:100]}...
---
...
def validate_resume_design_skills(job_url: str, resume_file_path: str, user_name: str, tools: Dict[str, Any], Logger: Optional[Any] = None) -> Dict[str, Any]:
    ...
    JobDescription = _fetch_job_description(job_url, tools['fetch'], Logger)
    ...
    missing_keywords = _get_missing_keywords(JobDescription, user_skills_and_history, tools['search_records'], Logger)
    if missing_keywords is None:
        return {"status": "error", "message": "L3 Pinecone lookup failed."}

    # --- Step 5: Validation & Cache (LLM + L4 Redis) ---
    skill_report_content = _generate_skill_gap_report(user_name, JobDescription, missing_keywords)
...
def _get_initial_context(JobDescription: str, user_name: str, Logger: Optional[Any] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        required_keywords_str = search_records(
            query=f"keywords for {JobDescription}", index="resume_keywords", top_k=5)
...
def _render_prompt(JobDescription: str, context: Dict[str, Any]) -> str:
...
def _generate_and_score_draft(rendered_prompt: str, user_name: str, JobDescription: str, Logger: Optional[Any] = None) -> Dict[str, Any]:
    """Generates a draft using governed caching and scores it."""
    try:
        cache_result = execute_governed_prompt_caching(
            user_name=user_name,
            job_description_hash=str(hash(JobDescription)),
        ...
        score = semantic_score_draft(draft_content, JobDescription)
...
def generate_optimized_draft(JobDescription: str, user_name: str, score_threshold: float = 0.90, max_iterations: int = 3, Logger: Optional[Any] = None) -> Dict[str, Any]:
    """
    ...
    context = _get_initial_context(JobDescription, user_name, Logger)
    if not context:
    ...
    for i in range(max_iterations):
        ...
        rendered_prompt = _render_prompt(JobDescription, context)

        # Generate and score draft
        generation_result = _generate_and_score_draft(rendered_prompt, user_name, JobDescription, Logger)
    ...
    try:
        ...
        FactChecker.validate_skills(final_draft)
