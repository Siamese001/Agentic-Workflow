...
class HardenedGeminiExecutor:
    ...
    def execute(self, prompt: str, model: str | None=None, temperature: float=0.7, max_tokens: int | None=None) -> dict[str, Any]:
        ...
        request = GenerationRequest(agent_id=self.agent_id, provider='google', model=effective_model, prompt=prompt)
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                try:
                    ...
                    loop.close()
                logger.debug('HardenedGeminiExecutor: success on attempt %d model=%s', attempt, effective_model)
                return {'content': response.content, 'model': effective_model, 'provider': 'google', 'success': True, 'attempt': attempt}
            # guardian: allow-silent-swallow
            except Exception as exc:
                last_exc = exc
                logger.warning('HardenedGeminiExecutor attempt %d/%d failed: %s', attempt, self.max_retries, exc)
        raise RuntimeError(f'HardenedGeminiExecutor: all {self.max_retries} attempts failed. Last: {last_exc}')
