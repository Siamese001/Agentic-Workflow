...
try:
    from agentic_core.interfaces.execution_contracts import AgentOutputContract, get_current_secret, wrap_output
...
try:
    ...
    class HealerMixin:
        ...
        def heal_repository(self, dry_run: bool=False, execute: bool=False, depth: int=0, max_depth: int=3, _call_path: set | None=None) -> dict[str, int]:
...
class BaseRGEngine(MCPHardenedMixin, HealerMixin, ABC):
    ...
    def execute_contracted(self, input_data: BaseModel, trace_id: str='') -> 'AgentOutputContract':
        ...
        return wrap_output(agent_id=self.AGENT_ID, trace_id=trace_id or self._current_trace_id, payload_model=result, secret=get_current_secret())
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {'engine': self.__class__.__name__, 'initialized': self._initialized, 'mixins_available': MIXINS_AVAILABLE, 'knowledge_available': self.knowledge is not None}
