...
class DataEnricher:
    ...
    def enrich(self, extracted_data: dict, ThematicAnalysis: dict | None=None, orchestrator: object | None=None) -> tuple[dict, list[ValidationResult]]:
        ...
        if duplicates:
            validation_results.append(ValidationResult(rule_id='DUPLICATE_BULLETS', PASSED=False, SEVERITY=ValidationSeverity.MEDIUM, MESSAGE=f'Found {len(duplicates)} potential duplicate bullets', DETAILS={'duplicates': duplicates[:5]}))
        else:
            validation_results.append(ValidationResult(rule_id='DUPLICATE_CHECK', PASSED=True, SEVERITY=ValidationSeverity.INFO, MESSAGE='No duplicate bullets detected'))
