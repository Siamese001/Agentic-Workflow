        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                if node.module in [
                    ...
                    "scripts.dashboard_ssot_definitions",
                ]:
...
def check_calculation_duplication(file_path: Path) -> tuple[bool, list[str]]:
    ...
    for pattern, ssot_func in duplicate_patterns:
        ...
        for match in matches:
            ...
            errors.append(f"Line {line_num}: Duplicate calculation logic - should use {ssot_func}")
    ...
    try:
        ...
        if health_weights:
            ...
            if abs(weight_sum - 1.0) > 0.001:
                errors.append(f"Health weights sum to {weight_sum:.3f}, expected 1.0 (±0.001)")
        ...
        if l0_weights:
            ...
            if abs(weight_sum - 1.0) > 0.001:
                errors.append(f"L0 health weights sum to {weight_sum:.3f}, expected 1.0 (±0.001)")
