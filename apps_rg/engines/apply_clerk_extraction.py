...
class ClerkExtractor:
    ...
    def _build_experience_sections(self) -> list[dict]:
        ...
        for exp in self.master_resume.get('experience', []):
            BULLETS = [{'bullet_text': text, 'quantified_metrics': self._extract_metrics(text), 'canonical_verbs': [], 'provenance': BulletProvenance.Verbatim.value} for text in exp.get('bullets', [])]
