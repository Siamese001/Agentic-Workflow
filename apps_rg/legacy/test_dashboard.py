...
class DashboardTestSuite:
    ...
    def test_territory_order(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if len(actual_order) < EXPECTED_TERRITORY_COUNT:
                return (
                    False,
                    f"Missing territories: expected {EXPECTED_TERRITORY_COUNT}, got {len(actual_order)}",
                )
    ...
    def test_data_consistency(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if total_row["Total"] != len(agents):
                return (
                    False,
                    f"Agent count mismatch: Dashboard={total_row['Total']}, Actual={len(agents)}",
                )
            ...
            if abs(total_row["Heal Cap %"] - actual_heal_pct) > 0.5:
                return (
                    False,
                    f"Heal Cap % mismatch: Dashboard={total_row['Heal Cap %']}%, Actual={actual_heal_pct}%",
                )
    ...
    def test_regeneration_stability(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if len(new_data) < expected_total_rows:
                return (
                    False,
                    f"Row count too low: expected {expected_total_rows}, got {len(new_data)}",
                )
    ...
    def test_visual_data_population(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if (
                "function generateMockAgentData(" in html
                and "generateMockAgentData_DEPRECATED" not in html
            ):
            ...
            fanin_match = re.search(
                r"function getMockFanInData\([^)]+\)\s*\{[^}]*return\s+(\d+)", html, re.DOTALL
            )
            if fanin_match:
            ...
            return (
                True,
                f"Visual data populated: {territory_count} territories, {agent_count} agents in sample, 0 mock calls",
            )
    ...
    def test_cell_by_cell_visual_inspection(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if total_row.get("Total") != expected_total_agents:
                issues.append(
                    f"TOTAL Total cell: expected {expected_total_agents}, got {total_row.get('Total')}"
                )

            if total_row.get("Heal Cap %") != expected_heal_cap:
                issues.append(
                    f"TOTAL Heal Cap % cell: expected {expected_heal_cap}, got {total_row.get('Heal Cap %')}"
                )
            ...
            if not sample_territory:
                ...
                for field in percentage_fields:
                    ...
                    if value is None:
                        ...
                        issues.append(
                            f"Territory '{territory_name}' cell '{field}' = {value} (out of range 0-100)"
                        )
            ...
            if real_data:
                ...
                if "healCap" not in sample_real_data:
                    issues.append(
                        f"realAgentData territory '{sample_real_territory}' missing healCap array for outlier badges"
                    )
                elif not isinstance(sample_real_data["healCap"], list):
                    issues.append(
                        f"realAgentData territory '{sample_real_territory}' healCap is not an array"
                    )
                else:
                    ...
                    if len(heal_cap_array) == 0:
                        issues.append(
                            f"realAgentData territory '{sample_real_territory}' healCap array is empty"
                        )
                    ...
                    for i, val in enumerate(heal_cap_array[:5]):  # Check first 5
                        if not (0 <= val <= 100):
                            issues.append(
                                f"realAgentData '{sample_real_territory}' healCap[{i}] = {val} (invalid)"
                            )
                ...
                if territory_name in real_data:
                    ...
                    for arr_name in required_arrays:
                        if arr_name not in territory_real:
                            issues.append(
                                f"Territory '{territory_name}' missing '{arr_name}' array for sparklines"
                            )
                        elif not isinstance(territory_real[arr_name], list):
                            issues.append(
                                f"Territory '{territory_name}' '{arr_name}' is not an array"
                            )
            ...
            if real_data:
                ...
                if "agents" not in sample_real_data:
                    issues.append(
                        f"realAgentData territory '{sample_real_territory}' missing agents array for drill-down"
                    )
                elif not isinstance(sample_real_data["agents"], list):
                    issues.append(
                        f"realAgentData territory '{sample_real_territory}' agents is not an array"
                    )
                else:
                    agents = sample_real_data["agents"]
                    if len(agents) == 0:
                        issues.append(
                            f"realAgentData territory '{sample_real_territory}' agents array is empty"
                        )
                    else:
                        ...
                        for field in required_agent_fields:
                            if field not in agent:
                                issues.append(
                                    f"Agent object missing '{field}' field for drill-down"
                                )
            ...
            if issues:
                return (
                    False,
                    f"Cell inspection failed: {len(issues)} issues found: {'; '.join(issues[:3])}...",
                )

            print(
                "✅ PASSED: Cell-by-cell inspection passed: TOTAL row verified, sample territory verified, outlier data verified, sparkline data verified, drill-down data verified"
            )
    ...
    def test_source_vs_rendered_data(self) -> tuple[bool, str]:
        ...
        try:
            ...
            expected_heal_cap = (
                sum(1 for a in agents if a.get("has_healing")) / expected_total * 100
                if expected_total > 0
                else 0
            )
            expected_heal_inv = (
                sum(1 for a in agents if a.get("invocation") == "Yes") / expected_total * 100
                if expected_total > 0
                else 0
            )
            ...
            if abs(rendered_heal_cap - expected_heal_cap) > 1.0:
                errors.append(
                    f"Heal Cap %: expected {expected_heal_cap:.1f}%, got {rendered_heal_cap}%"
                )

            # Check Heal Invocation % (allow ±1% for rounding)
            if abs(rendered_heal_inv - expected_heal_inv) > 1.0:
                errors.append(
                    f"Heal Invocation %: expected {expected_heal_inv:.1f}%, got {rendered_heal_inv}%"
                )
            ...
            return (
                True,
                f"Data matches source: {rendered_total} agents, Heal Cap {rendered_heal_cap:.1f}%, Heal Inv {rendered_heal_inv:.1f}%",
            )
    ...
    def test_tooltip_data_availability(self) -> tuple[bool, str]:
        ...
        try:
            ...
            dashboard_territories = [
                r["Territory"] for r in dashboard_data if r["Territory"] != "TOTAL"
            ]
            ...
            for territory in dashboard_territories:
                if territory not in real_territories:
                    ...
                    match = next(
                        (t for t in real_territories if t.lower() == territory.lower()), None
                    )
                    if not match:
            ...
            if issues:
                return (
                    False,
                    f"Tooltip data issues: {'; '.join(issues[:5])}{'...' if len(issues) > 5 else ''}",
                )
    ...
    def test_distribution_stats_display(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if "stats.min === stats.max && stats.min >= 99.9" not in js_content:
                issues.append(
                    "Missing correct condition: 'stats.min === stats.max && stats.min >= 99.9'"
                )

            # Check for uniform value indicator for < 100%
            if (
                "(all ${stats.min.toFixed(0)}%)" not in js_content
                and "(all ${stats.min.toFixed(0)}%)" not in js_content
            ):
            ...
            if "stats.min === stats.max || avg >= 99.9" in js_content:
                issues.append(
                    "Buggy condition still present: 'stats.min === stats.max || avg >= 99.9'"
                )
    ...
    def run_all_tests(self) -> bool:
        ...
        tests = [
            ...
            (
                "Cell-by-Cell Visual Inspection (MANDATORY)",
                self.test_cell_by_cell_visual_inspection,
            ),
