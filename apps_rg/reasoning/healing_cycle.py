...
class HealingCycle:
    ...
    async def execute(self, strategy: str) -> dict[str, Any]:
        ...
        try:
            ...
            if emitter:
                emitter.emit(trace_id=getattr(self.ctx, 'trace_id', 'unknown'), attempt_number=self.cycle_num, failure_class=strategy, healer_selected='HealingCycle', model_used='local', outcome=outcome)
            return {'status': 'success' if converged else 'partial', 'strategy': strategy, 'cycle_num': self.cycle_num, 'passed_agents': passed_agents, 'failed_agents': failed_agents, 'converged': converged, 'rollback_triggered': rollback_triggered}
        except Exception as exc:
            raise
            Logger.error('HealingCycle[%d] failed: %s', self.cycle_num, exc)
            if emitter:
                emitter.emit(trace_id=getattr(self.ctx, 'trace_id', 'unknown'), attempt_number=self.cycle_num, failure_class=strategy, healer_selected='HealingCycle', model_used='local', outcome='error', metadata={'error': str(exc)})
            return {'status': 'error', 'strategy': strategy, 'cycle_num': self.cycle_num, 'passed_agents': [], 'failed_agents': [], 'converged': False, 'rollback_triggered': False}
