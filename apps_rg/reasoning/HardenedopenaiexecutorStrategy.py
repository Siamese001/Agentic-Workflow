...
class HardenedOpenAIConfig:
    ...
    MODEL_LIMITS = {'gpt-4': 8192, 'gpt-4-32k': 32768, 'gpt-4-0613': 8192, 'gpt-4-32k-0613': 32768, 'gpt-4-turbo': 128000, 'gpt-4-turbo-2024-04-09': 128000, 'gpt-4o': 128000, 'gpt-4o-2024-08-06': 128000, 'gpt-4o-mini': 128000, 'gpt-3.5-turbo': 4096, 'gpt-3.5-turbo-16k': 16384, 'gpt-3.5-turbo-0613': 4096, 'gpt-3.5-turbo-16k-0613': 16384}

    # guardian: allow-magic-config
    def __init__(self, model: str='gpt-4o-2024-08-06', temperature: float=0.7, max_tokens: int=4096, timeout_s: int=60, max_retries: int=3, failure_threshold: int=5, reset_timeout_s: int=30):
        self.model = model
...
class HardenedOpenAIExecutor(HardeningMixin):
    ...
    def __init__(self, config: HardenedOpenAIConfig | None=None, telemetry: SystemTelemetry | None=None):
        ...
        super().__init__(component_name='openai_executor', failure_threshold=self.config.failure_threshold, reset_timeout_s=self.config.reset_timeout_s, max_retries=self.config.max_retries, telemetry=telemetry)
        self._client = None
        self._gateway = None
        self._setup_client()
    ...
    def _validate_token_budget(self, prompt: str) -> None:
        ...
        """
        self.validate_token_budget_tiktoken(prompt=prompt, model=self.config.model, max_tokens=self.config.max_context_tokens - self.config.max_tokens)

    def _build_messages(self, messages: list[AgentMessage], system_prompt: str | None=None) -> list[dict[str, str]]:
    ...
    async def run_llm(self, prompt: str, *, temperature: float | None=None, max_tokens: int | None=None, system_prompt: str | None=None, messages: list[AgentMessage] | None=None) -> str:
        ...
        async def _completion():
            response = await self._gateway.route_generation(GenerationRequest(agent_id='hardened_openai_executor', provider='openai', model=self.config.model, prompt=combined_prompt, temperature=temperature or self.config.temperature, max_tokens=max_tokens or self.config.max_tokens))
            return response.content or ''
        return await self.execute_hardened(operation='chat_completion', fn=_completion, validate_token_budget=lambda: self._validate_token_budget(combined_prompt), metadata={'model': self.config.model, 'temperature': temperature or self.config.temperature, 'max_tokens': max_tokens or self.config.max_tokens})
    ...
    async def run_llm_with_response(self, prompt: str, *, temperature: float | None=None, max_tokens: int | None=None, system_prompt: str | None=None, messages: list[AgentMessage] | None=None) -> AgentResponse:
        ...
        async def _completion():
            return await self._gateway.route_generation(GenerationRequest(agent_id='hardened_openai_executor', provider='openai', model=self.config.model, prompt=combined_prompt, temperature=temperature or self.config.temperature, max_tokens=max_tokens or self.config.max_tokens))
        raw_response = await self.execute_hardened(operation='chat_completion', fn=_completion, validate_token_budget=lambda: self._validate_token_budget(combined_prompt), metadata={'model': self.config.model, 'temperature': temperature or self.config.temperature, 'max_tokens': max_tokens or self.config.max_tokens})
        content = raw_response.content or '' if raw_response else ''
        usage = None
        return AgentResponse(content=content, model=raw_response.model if raw_response else self.config.model, usage=usage, finish_reason=None)
...
# guardian: allow-magic-config
def create_hardened_openai_executor(model: str='gpt-4o-2024-08-06', temperature: float=0.7, **kwargs) -> HardenedOpenAIExecutor:
