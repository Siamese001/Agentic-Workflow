...
class ThematicAnalysisNode:
    ...
    def __init__(self, config: dict[str, Any]=None):
        ...
        self.linkedin_config = {'minimum_profiles': 10, 'authenticity_transformation': {'avoid': ['Expert in', 'Skilled in'], 'prefer': ['Built', 'Engineered', 'Spearheaded']}}
    ...
    def analyze_thematic_resonance(self, job_description: str, company_name: str) -> ThematicAnalysisOutput:
        ...
        authenticity = AuthenticityPatterns(executive_summary_patterns=['Built and scaled', 'Led transformation'], achievement_verb_patterns=['Spearheaded', 'Engineered', 'Optimized'], metric_presentation_patterns=['resulting in X% improvement'], competency_phrasing_patterns=['Specialized in', 'Proficient with'])
        comp_intel = CompetitiveIntelligence(peer_jds_analyzed=[f'Competitor to {company_name}'], table_stakes_keywords=['leadership', 'strategy'], differentiator_keywords=['innovation', 'scale'])
        return ThematicAnalysisOutput(primary_theme=primary, secondary_themes=secondary, authenticity_patterns=authenticity, competitive_intelligence=comp_intel, company_name=company_name)
