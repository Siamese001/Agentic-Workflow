...
class RGFlowRouter:
    ...
    def __init__(self, config: dict[str, Any]=None):
        ...
        self.flow_configs = self.config.get('flow_configs', {'strategic_tailor_node': {'required_hops': ['HOP-1', 'HOP-2', 'HOP-3', 'HOP-4', 'HOP-5', 'HOP-6'], 'validation_required': True, 'retry_enabled': True}, 'tailor_existing': {'required_hops': ['HOP-1', 'HOP-2', 'HOP-3', 'HOP-4', 'HOP-5'], 'validation_required': True, 'retry_enabled': True}, 'generate_scratch': {'required_hops': ['HOP-1', 'HOP-2', 'HOP-3', 'HOP-4', 'HOP-5'], 'validation_required': True, 'retry_enabled': True}, 'enhance_current': {'required_hops': ['HOP-3', 'HOP-4', 'HOP-5'], 'validation_required': True, 'retry_enabled': False}})
    ...
    def execute_routing(self, context: dict[str, Any]) -> RGFlowOutput:
        ...
        output = RGFlowOutput(flow_result=flow_result, entrance_gates_passed=entrance_gates_passed, metadata={'router_id': 'RGFlowRouter', 'task_description': task_description[:100], 'has_master_resume': has_master_resume, 'job_description_length': len(job_description)})
    ...
    def _classify_flow(self, task_description: str, has_master_resume: bool) -> ResumeFlowResult:
        ...
        if any((keyword in task_lower for keyword in self.tailor_keywords)):
            if has_master_resume:
                return ResumeFlowResult(flow_type='tailor_existing', confidence=0.95, required_hops=self.flow_configs['tailor_existing']['required_hops'], validation_required=self.flow_configs['tailor_existing']['validation_required'], retry_enabled=self.flow_configs['tailor_existing']['retry_enabled'])
            else:
        ...
        if any((keyword in task_lower for keyword in self.generate_keywords)):
            return ResumeFlowResult(flow_type='generate_scratch', confidence=0.9, required_hops=self.flow_configs['generate_scratch']['required_hops'], validation_required=self.flow_configs['generate_scratch']['validation_required'], retry_enabled=self.flow_configs['generate_scratch']['retry_enabled'])
        if any((keyword in task_lower for keyword in self.enhance_keywords)):
            return ResumeFlowResult(flow_type='enhance_current', confidence=0.85, required_hops=self.flow_configs['enhance_current']['required_hops'], validation_required=self.flow_configs['enhance_current']['validation_required'], retry_enabled=self.flow_configs['enhance_current']['retry_enabled'])
        if has_master_resume:
            return ResumeFlowResult(flow_type='tailor_existing', confidence=0.7, required_hops=self.flow_configs['tailor_existing']['required_hops'], validation_required=self.flow_configs['tailor_existing']['validation_required'], retry_enabled=self.flow_configs['tailor_existing']['retry_enabled'])
        else:
            return ResumeFlowResult(flow_type='generate_scratch', confidence=0.7, required_hops=self.flow_configs['generate_scratch']['required_hops'], validation_required=self.flow_configs['generate_scratch']['validation_required'], retry_enabled=self.flow_configs['generate_scratch']['retry_enabled'])
    ...
    def _classify_flow_with_thematic_analysis(self, task_description: str, has_master_resume: bool, thematic_analysis: ThematicAnalysisOutput) -> ResumeFlowResult:
        ...
        if len(differentiators) > 3:
            return ResumeFlowResult(flow_type='strategic_tailor_node', confidence=0.98, required_hops=self.flow_configs['strategic_tailor_node']['required_hops'], validation_required=self.flow_configs['strategic_tailor_node']['validation_required'], retry_enabled=self.flow_configs['strategic_tailor_node']['retry_enabled'])
        if any((keyword in task_lower for keyword in self.tailor_keywords)):
            if has_master_resume:
                return ResumeFlowResult(flow_type='tailor_existing', confidence=0.95, required_hops=self.flow_configs['tailor_existing']['required_hops'], validation_required=self.flow_configs['tailor_existing']['validation_required'], retry_enabled=self.flow_configs['tailor_existing']['retry_enabled'])
            else:
    ...
    def _validate_routing_requirements(self, flow_result: ResumeFlowResult, context: dict[str, Any]) -> None:
        ...
        if flow_result.flow_type in ['tailor_existing', 'enhance_current']:
            if not context.get('has_master_resume', False):
                raise ValueError(f"ROUTING_VALIDATION_FAILED: Flow '{flow_result.flow_type}' requires master resume")
        job_description = context.get('job_description', '')
