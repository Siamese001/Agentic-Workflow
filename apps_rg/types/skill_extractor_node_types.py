...
class SkillExtractorNode:
    ...
    def __init__(self, config: dict[str, Any]=None):
        ...
        self.technical_skills = {'programming': ['python', 'java', 'javascript', 'typescript', 'c++', 'c#', 'go', 'rust', 'ruby', 'php', 'swift', 'kotlin', 'scala', 'perl', 'r', 'matlab'], 'web_development': ['html', 'css', 'react', 'angular', 'vue', 'node.js', 'express', 'django', 'flask', 'spring', 'laravel', 'rails', 'asp.net', 'next.js', 'gatsby'], 'databases': ['sql', 'mysql', 'postgresql', 'mongodb', 'redis', 'elasticsearch', 'oracle', 'sql server', 'cassandra', 'dynamodb', 'firebase'], 'cloud': ['aws', 'azure', 'gcp', 'google cloud', 'heroku', 'digitalocean', 'terraform', 'kubernetes', 'docker', 'jenkins', 'ci/cd', 'devops'], 'data_science': ['machine learning', 'data analysis', 'statistics', 'pandas', 'numpy', 'scikit-learn', 'tensorflow', 'pytorch', 'tableau', 'power bi', 'excel']}
        self.soft_skills = ['leadership', 'communication', 'teamwork', 'problem solving', 'critical thinking', 'adaptability', 'time management', 'project management', 'creativity', 'collaboration', 'attention to detail', 'analytical thinking', 'strategic planning', 'decision making', 'mentoring', 'public speaking', 'negotiation', 'conflict resolution', 'emotional intelligence']
        self.domain_skills = {'finance': ['financial analysis', 'investment banking', 'risk management', 'portfolio management', 'financial modeling', 'compliance', 'audit', 'tax planning', 'mergers and acquisitions'], 'healthcare': ['clinical research', 'medical terminology', 'patient care', 'healthcare compliance', 'medical coding', 'pharmacology', 'healthcare informatics', 'clinical trials'], 'technology': ['software development', 'system architecture', 'api design', 'microservices', 'agile methodology', 'scrum', 'code review', 'testing', 'debugging', 'optimization'], 'marketing': ['digital marketing', 'seo', 'sem', 'content marketing', 'social media marketing', 'brand management', 'market research', 'analytics', 'copywriting', 'campaign management']}
        self.tool_skills = {'development_tools': ['git', 'github', 'gitlab', 'jira', 'confluence', 'slack', 'vs code', 'intellij', 'eclipse', 'postman', 'docker', 'kubernetes', 'terraform', 'ansible'], 'design_tools': ['figma', 'sketch', 'adobe creative suite', 'photoshop', 'illustrator', 'xd', 'invision', 'zeplin', 'canva', 'gimp'], 'office_tools': ['microsoft office', 'word', 'excel', 'powerpoint', 'outlook', 'google workspace', 'docs', 'sheets', 'slides', 'notion', 'trello', 'asana']}
    ...
    def analyze_skills(self, job_description: str, candidate_profile: dict[str, Any]) -> SkillAnalysisOutput:
        ...
        output = SkillAnalysisOutput(extraction_result=jd_skills, gap_result=gap_result, match_result=match_result, recommendations=recommendations, metadata={'node_id': 'SkillExtractorNode', 'job_description_length': len(job_description), 'candidate_has_profile': bool(candidate_profile), 'analysis_timestamp': self._get_timestamp()})
    ...
    def _extract_skills_from_text(self, text: str) -> SkillExtractionResult:
        ...
        return SkillExtractionResult(technical_skills=list(set(technical_skills)), soft_skills=list(set(soft_skills)), domain_skills=list(set(domain_skills)), tool_skills=list(set(tool_skills)), confidence_score=confidence_score, source_text_length=len(text))
    ...
    def _analyze_skill_gaps(self, jd_skills: SkillExtractionResult, candidate_skills: SkillExtractionResult) -> SkillGapResult:
        ...
        """
        jd_all_skills = set(jd_skills.technical_skills + jd_skills.soft_skills + jd_skills.domain_skills + jd_skills.tool_skills)
        candidate_all_skills = set(candidate_skills.technical_skills + candidate_skills.soft_skills + candidate_skills.domain_skills + candidate_skills.tool_skills)
        ...
        if len(jd_all_skills) == 0:
            ...
            if gap_score >= 0.7:
                ...
                gap_severity = 'LOW'
        return SkillGapResult(missing_skills=sorted(missing_skills), existing_skills=sorted(existing_skills), gap_severity=gap_severity, gap_score=gap_score)
    ...
    def _match_skills(self, jd_skills: SkillExtractionResult, candidate_skills: SkillExtractionResult) -> SkillMatchResult:
        ...
        all_jd_skills = jd_skills.technical_skills + jd_skills.soft_skills + jd_skills.domain_skills + jd_skills.tool_skills
        ...
        return SkillMatchResult(matched_skills=[skill.title() for skill in all_matched_skills], partially_matched_skills=[], unmatched_skills=unmatched_skills, match_percentage=match_percentage, skill_categories={'technical': {'matched': [skill.title() for skill in matched_technical]}, 'soft': {'matched': [skill.title() for skill in matched_soft]}, 'domain': {'matched': [skill.title() for skill in matched_domain]}, 'tools': {'matched': [skill.title() for skill in matched_tool]}})
    ...
    def get_skill_categories(self) -> dict[str, list[str]]:
        ...
        """
        return {'technical': [skill for skills in self.technical_skills.values() for skill in skills], 'soft': self.soft_skills.copy(), 'domain': [skill for skills in self.domain_skills.values() for skill in skills], 'tools': [skill for tools in self.tool_skills.values() for skill in tools]}
    ...
    def validate_skill_extraction(self, text: str, expected_skills: list[str]) -> dict[str, Any]:
        ...
        extracted_skills = set(extraction_result.technical_skills + extraction_result.soft_skills + extraction_result.domain_skills + extraction_result.tool_skills)
        ...
        return {'precision': precision, 'recall': recall, 'f1_score': f1_score, 'true_positives': true_positives, 'false_positives': false_positives, 'false_negatives': false_negatives, 'extracted_skills': list(extracted_skills), 'missing_expected': list(expected_skills_set - extracted_skills)}
