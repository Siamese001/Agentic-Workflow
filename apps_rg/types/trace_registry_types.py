...
class TraceRegistry(MCPHardenedMixin):
    ...
    def add_trace(self, event_type: str, details: dict[str, Any]) -> None:
        ...
        """
        trace = AgentTrace(trace_id=f'legacy_{time.time()}', agent_name=details.get('agent', 'SYSTEM'), action=event_type, start_time=time.time(), end_time=time.time(), status='COMPLETE', metadata=details)
        self._traces.append(trace)
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {'total_spans': len(self._traces), 'completed': len(completed), 'failures': len(failures), 'avg_latency_ms': round(avg_latency, 2), 'total_tokens': sum((t.tokens_used for t in completed))}

    def get_traces(self) -> list[dict[str, Any]]:
        """Returns a list of trace dictionaries for backward compatibility."""
        return [{'trace_id': t.trace_id, 'agent_name': t.agent_name, 'action': t.action, 'status': t.status, 'duration_ms': t.duration_ms, 'tokens_used': t.tokens_used, 'error': t.error, 'metadata': t.metadata} for t in self._traces]
    ...
    def get_by_type(self, trace_type: str) -> list[dict[str, Any]]:
        ...
        return [{'trace_id': t.trace_id, 'agent_name': t.agent_name, 'action': t.action, 'status': t.status, 'duration_ms': t.duration_ms} for t in self._traces if t.action == trace_type]
    ...
    def get_latest(self, trace_type: str) -> dict[str, Any] | None:
        ...
        if not matches:
            return None
        t = matches[-1]
        return {'trace_id': t.trace_id, 'agent_name': t.agent_name, 'action': t.action, 'status': t.status, 'duration_ms': t.duration_ms}

    def _persist_trace(self, trace: AgentTrace) -> None:
        """Write trace details to disk."""
        try:
            if self.persistence_path:
                entry = {'timestamp': datetime.utcnow().isoformat(), 'agent': trace.agent_name, 'action': trace.action, 'status': trace.status, 'error': trace.error, 'duration': trace.duration_ms, 'tokens_used': trace.tokens_used}
