...
class RGSovereignAuditor:
    ...
    def __init__(self, base_path: str='apps_rg'):
        ...
        self.audit_results = {'sovereign_agents': [], 'stateless_tools': [], 'passive_data': [], 'legacy_debt': [], 'unknown': [], 'imposter_agent': [], 'unknown_ledger': [], 'nomenclature_fixes': [], 'migration_candidates': []}
    ...
    def analyze_file_structure(self, file_path: Path) -> dict:
        ...
        try:
            ...
            classification, details = self._classify_file_logic(file_path.name, content, classes, imports, functions)
            return {'classification': classification, 'details': details, 'classes': classes, 'imports': imports, 'functions': functions, 'line_count': len(content.split('\n'))}
    ...
    def _extract_classes(self, tree: ast.AST) -> list[dict]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                classes.append({'name': node.name, 'bases': bases, 'methods': methods, 'has_execute_or_process': has_execute_or_process, 'has_init': has_init, 'has_heal': has_heal, 'has_state': has_state, 'is_agent': 'Agent' in node.name, 'is_mixin': 'Mixin' in node.name, 'is_base': 'Base' in node.name, 'is_model': 'Model' in node.name or 'Schema' in node.name, 'is_enum': 'Enum' in node.name or node.name.endswith('Type'), 'line_count': node.end_lineno - node.lineno if hasattr(node, 'end_lineno') else 0})
        return classes
    ...
    def _extract_functions(self, tree: ast.AST) -> list[dict]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                functions.append({'name': node.name, 'args_count': len(node.args.args), 'is_generator': any((isinstance(item, ast.Yield) for item in ast.walk(node))), 'is_async': isinstance(node, ast.AsyncFunctionDef)})
        return functions

    def _classify_file_logic(self, filename: str, content: str, classes: list[dict], imports: list[str], functions: list[dict]) -> tuple[str, dict]:
        ...
        if any((indicator in filename for indicator in model_indicators)):
            ...
            all_passive = all((cls.get('is_model', False) or cls.get('is_enum', False) or (not cls.get('has_execute_or_process', False)) for cls in classes))
        ...
        inherits_from_agent_base = any((any((indicator in base for base in cls.get('bases', []))) for cls in classes for indicator in agent_base_indicators))
        agent_method_indicators = ['execute', '_process', 'run', 'run_phase', 'heal_repository', 'orchestrate', 'dispatch']
        has_agent_methods = any((any((method in cls.get('methods', []) for method in agent_method_indicators)) for cls in classes))
        has_agent_naming = filename.endswith('Agent.py') or filename.endswith('Engine.py') or any((cls.get('is_agent', False) for cls in classes)) or any(('Engine' in cls.get('name', '') for cls in classes))
        if (inherits_from_agent_base or has_agent_methods) and has_agent_naming:
            return ('SOVEREIGN_AGENT', {'reason': 'Inherits from agent base and has agent methods/naming', 'agent_classes': [cls['name'] for cls in classes if cls.get('is_agent', False) or 'Engine' in cls.get('name', '')], 'inherits_from_base': inherits_from_agent_base, 'has_agent_methods': has_agent_methods})
        ...
        if is_tool_by_name or (not has_stateful_classes and (not has_init_methods) and (len(classes) == 0)):
            return ('STATELESS_TOOL', {'reason': 'Stateless utility functions or tool-like naming', 'is_tool_by_name': is_tool_by_name, 'has_stateful_classes': has_stateful_classes})
        if 'Agent' in filename and (not inherits_from_agent_base) and (not has_agent_methods):
            return ('IMPOSTER_AGENT', {'reason': 'Filename suggests agent but no agent inheritance or methods found'})
        if 'Engine' in filename or 'engine' in filename.lower():
            if inherits_from_agent_base or has_agent_methods:
                return ('SOVEREIGN_AGENT', {'reason': 'Engine with agent base inheritance or methods', 'agent_classes': [cls['name'] for cls in classes], 'inherits_from_base': inherits_from_agent_base})
            else:
                return ('STATELESS_TOOL', {'reason': 'Engine without agent characteristics - likely a utility engine'})
        if classes:
    ...
    def audit_directory(self) -> dict:
        ...
        print('=' * 60)
        results = {'total_files': 0, 'by_directory': {}, 'classifications': self.audit_results.copy(), 'unknown_ledger': [], 'nomenclature_fixes': [], 'migration_candidates': []}
    ...
    def _analyze_subdirectory(self, subdir_path: Path, subdir_name: str) -> dict:
        ...
        subdir_results = {'file_count': len(python_files), 'files': {}, 'classifications': {'SOVEREIGN_AGENT': [], 'STATELESS_TOOL': [], 'PASSIVE_DATA': [], 'LEGACY_DEBT': [], 'IMPOSTER_AGENT': [], 'UNKNOWN': []}}
        ...
        for file_path, analysis in engines_files.items():
            if 'error' not in analysis:
                ...
                if classification in ['UNKNOWN', 'IMPOSTER_AGENT'] or not inherits_from_base:
                    unknown_ledger.append({'file': file_path, 'classification': classification, 'reason': analysis.get('details', {}).get('reason', 'Unknown'), 'classes': [cls['name'] for cls in classes], 'inherits_from_base': inherits_from_base})
        ...
        for _subdir_name, subdir_data in results['by_directory'].items():
            for file_path, analysis in subdir_data.get('files', {}).items():
                if 'error' not in analysis and analysis['classification'] == 'IMPOSTER_AGENT':
                    nomenclature_fixes.append({'file': file_path, 'current_name': Path(file_path).name, 'suggested_name': self._suggest_fixed_name(file_path, analysis), 'reason': analysis.get('details', {}).get('reason', 'Unknown')})
    ...
    def _generate_migration_candidates(self, results: dict):
        ...
        for file_path, analysis in engines_files.items():
            if 'error' not in analysis and analysis['classification'] == 'STATELESS_TOOL':
                migration_candidates.append({'file': file_path, 'current_location': 'engines/', 'target_location': 'shared/tools/', 'reason': analysis.get('details', {}).get('reason', 'Unknown')})
