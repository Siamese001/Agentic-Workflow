...
class ResumeGenerator:
    ...
    def __init__(self, llm_client: Any | None=None, provider: Any | None=None, creative_brief: Any | None=None, validation_rules: dict[str, Any] | None=None):
        """
    ...
    def _generate_with_gemini(self, prompt: str, temperature: float=0.7) -> str:
        ...
        try:
            ...
            request = GenerationRequest(agent_id='ResumeGenerator', provider='google', model='gemini-2.5-pro', prompt=prompt, temperature=temperature)
            response = gateway.generate(request)
            return response.text if hasattr(response, 'text') else str(response)
        # guardian: allow-silent-swallow
        except Exception as e:
            Logger.error('ResumeGenerator._generate_with_gemini: SovereignLLMGateway failed: %s; direct SDK fallback is NOT permitted — raising', e)
    ...
    def optimize_for_ats(self, resume_data: dict[str, Any], analysis: dict[str, Any]) -> dict[str, Any]:
        ...
        all_keywords = analysis.get('hard_skills', []) + analysis.get('soft_skills', []) + analysis.get('key_responsibilities', []) + analysis.get('cultural_indicators', [])
        unique_keywords = list({k.lower() for k in all_keywords})[:30]
        optimized['ats_keywords'] = unique_keywords
        section_mapping = {'professional_summary': 'summary', 'objective': 'summary', 'work_experience': 'experience', 'employment': 'experience', 'education_history': 'education', 'technical_skills': 'skills', 'technologies': 'skills'}
