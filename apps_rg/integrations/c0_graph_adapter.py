"""apps_rg C0.3 graph adapter — W4N chroma-graphrag-lic-rg-research-f4a2e9.
...
W4N (no-core track) invariants enforced here:
  ...
  - ``build_rg_graph_traverse_input()`` returns a typed dict; live
    ``GraphTraverseInput`` construction is deferred (C0.3 adapter registry not
    wired under no-core plan).
  - apps_rg is bounded to max_hops=1, max_nodes=32, max_edges=64 per no-core profile.
  - apps_rg/cache/r1b_adapter.py remains quarantined and untouched.
  - apps_rg semantic cache config is preserved from W2N (live_wiring_deferred).
"""
...
# ---------------------------------------------------------------------------
# W4N policy constants — must match apps_rg/config/domain_contract/route_profiles.yaml
# ---------------------------------------------------------------------------

_GRAPH_SOURCE = "apps_rg.resume_graph.v1"
_PROJECTION_VERSION = "v1.0.0"
_SNAPSHOT_POINTER = "apps_rg::snapshot::placeholder"
...
def build_rg_graph_traverse_input(
    ...
    """Build a GraphTraverseInput-compatible dict for apps_rg.
...
class RgGraphAdapter:
    """apps_rg resume-graph adapter for C0.3.
    ...
    W4N discipline:
      ...
      - apps_rg bounded to max_hops=1 per no-core route-profile policy.
    """
    ...
    def resolve_anchor(
        ...
        return UnresolvedAnchorResolution(
            ...
            reason="apps_rg graph adapter: stub — no live backend connected (W4N)",
        )
...
def get_graph_adapter() -> GraphTraversalAdapter:
    ...
    Returns a fresh ``RgGraphAdapter`` instance each call.
    """
    return RgGraphAdapter()
