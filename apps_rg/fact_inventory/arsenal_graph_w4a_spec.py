...
_AGENTIC_ROW_TEMPLATE: list[tuple[str, str, list[str], str, list[str], list[str]]] = [
    ...
    ("skill_layered_runtime_spine_design", "Layered runtime spine design", ["L0L6Spine", "LayerGravity", "ProfileResolver"], "Layered L0–L6 spine: L2 proposes, Exit clears, UWG commits, L4 stores, L6 learns after run boundary.", ["fact_engineering_platform_001"], [".claude/rules/000-agentic-core-operating-contract.mdc"]),
    ...
    ("skill_bounded_planning_contracts", "Bounded planning contracts", ["SRPlan", "PlanFirstExecuteSecond"], "Plan-first execute-second with explicit SR_INTAKE/SR_PLAN approval before edits on T2/T3 work.", [], [".claude/rules/sequential-thinking-enforcement.mdc"]),
    ...
    ("skill_planning_prior_and_policy_context_use", "Planning prior and policy context use", ["PolicyContext", "PrecedentLookup"], "Use policy context and precedent lookup before high-leverage decisions.", [], [".claude/skills/refactor-decision-memory/"]),
    # Domain 3
    ...
    ("skill_side_effect_bounded_action_design", "Side effect bounded action design", ["SideEffectBounds", "NoDirectWrite"], "Side-effect-bounded actions; no direct UWG/L4 write path from L2/L3/tools.", ["fact_engineering_platform_001"], [".claude/rules/000-agentic-core-operating-contract.mdc"]),
    ...
    ("skill_policy_bound_runtime_design", "Policy bound runtime design", ["PolicyBoundRuntime", "ConstitutionalRules"], "Policy-bound runtime aligned with constitutional rules and apps overlays.", ["fact_governance_003"], [".claude/rules/constitutional.mdc"]),
    ...
    ("skill_receipt_and_artifact_chain_design", "Receipt and artifact chain design", ["ReceiptChain", "ArtifactProvenance"], "Receipt and artifact chain design with provenance discipline.", ["fact_governance_001"], [".claude/rules/artifact-provenance-discipline.mdc"]),
    ("skill_no_bypass_proof_controls", "No bypass proof controls", ["NoBypassProof", "ProofContract"], "No-bypass proof controls: markers and narratives are not proof.", ["fact_governance_003"], [".claude/rules/002-pass-blocked-proof-contract.mdc"]),
    ...
    ("skill_learning_firewall_controls", "Learning firewall controls", ["LearningFirewall", "PromotionGate"], "Learning firewall: promotion gates block weakening X2/X3/judges.", ["fact_governance_003"], [".claude/rules/evaluation-promotion-gate.mdc"]),
    # Domain 13
    ("skill_hitl_escalation_architecture", "HITL escalation architecture", ["HITLEscalation", "AuthorGate"], "HITL escalation via Author-Gate packets and ask_user_question discipline.", ["fact_governance_002"], [".claude/rules/003-cursor-author-gate-hitl.mdc"]),
    ("skill_approval_gated_workflow_design", "Approval gated workflow design", ["ApprovalGated", "WaveLifecycle"], "Approval-gated workflows with wave lifecycle markers and human confirmation.", ["fact_governance_003"], [".claude/rules/wave-completion-discipline.mdc"]),
