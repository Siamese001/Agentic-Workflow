"""
08_scripts/logic/logic/validation/find_problems/diagnostics_ops/log_inspection.py
AUTO-HARDENED BY ZERO-LOSS MERGE ENGINE
L5 CANONICAL — WINDSURF Ω — 2025-12-07
MERKLE-INTENDED: bbb0534c864d3b5475ddab2a5d39d6c03070c9b779aab5a6c08ae8bcef80ef55
"""
...
class ContentConstraintsConfig:
    ...
    HEADLINE_MIN_CHARS: int = 60 # <<< Added from SECTION_CONSTRAINTS_V521
    HEADLINE_MAX_CHARS: int = 90 # <<< Added from SECTION_CONSTRAINTS_V521
    HEADLINE_COMPONENT_WORDS_MIN: int = 2 # <<< Added from SECTION_CONSTRAINTS_V521
    HEADLINE_COMPONENT_WORDS_MAX: int = 4 # <<< Added from SECTION_CONSTRAINTS_V521
    ...
    EY_OVERVIEW_WORD_COUNT_MIN: int = 28 # <<< Corrected: Added EY constraints
    EY_OVERVIEW_WORD_COUNT_MAX: int = 38 # <<< Corrected: Added EY constraints
    ...
    UNIFY_IBM_COMBINED_PERCENT_MIN: float = 35.0 # <<< Added from SECTION_CONSTRAINTS_V521
    UNIFY_IBM_COMBINED_PERCENT_MAX: float = 45.0 # <<< Added from SECTION_CONSTRAINTS_V521
    UNIFY_IBM_RATIO_MIN: float = 1.1 # <<< Added from SECTION_CONSTRAINTS_V521
    UNIFY_IBM_RATIO_MAX: float = 1.3 # <<< Added from SECTION_CONSTRAINTS_V521
...
class WorkflowOrchestrator:
    ...
    def _build_qa_section_5_provenance(self, staging_buffer: ImmutableStagingBuffer) -> List[str]:
        ...
        """
        # (Implementation unchanged, title updated)
        lines = ["", "5. BULLET PROVENANCE & WORD COUNT", ""] # <<< Renumbered (was 6)
        lines.append("```markdown")
        headers = ["Section", "Item", "Provenance", "Word Count", "Target Range", "Status", "Text Snippet"]
        rows = []
        # Ensure master average lengths are calculated using the correct method from the orchestrator
        master_avg_lengths = self._calculate_master_avg_bullet_length()
        tolerance = 0.20 # Defined in PreFlightValidator or class scope

        # Assume self.QA_PROVENANCE_SECTIONS is defined in the class, mapping section names to enums etc.
        # Example structure assumed for self.QA_PROVENANCE_SECTIONS:
        # QA_PROVENANCE_SECTIONS = [
        #     {"name": "Unify", "overview_enum": ResumeSection.K5_UNIFY_OVERVIEW, "bullets_enum": ResumeSection.K5_UNIFY_BULLETS, "overview_min_attr": "UNIFY_OVERVIEW_WORD_COUNT_MIN", "overview_max_attr": "UNIFY_OVERVIEW_WORD_COUNT_MAX"},
        #     {"name": "IBM", "overview_enum": ResumeSection.K6_IBM_OVERVIEW, "bullets_enum": ResumeSection.K6_IBM_BULLETS, "overview_min_attr": "IBM_OVERVIEW_WORD_COUNT_MIN", "overview_max_attr": "IBM_OVERVIEW_WORD_COUNT_MAX"},
        #     {"name": "TraderSense", "overview_enum": ResumeSection.K7_TRADERSENSE_OVERVIEW, "bullets_enum": ResumeSection.K7_TRADERSENSE_BULLETS, "overview_min_attr": "TRADERSENSE_OVERVIEW_WORD_COUNT_MIN", "overview_max_attr": "TRADERSENSE_OVERVIEW_WORD_COUNT_MAX"},
        #     {"name": "EY", "overview_enum": ResumeSection.K8_EY_OVERVIEW, "bullets_enum": ResumeSection.K8_EY_BULLETS, "overview_min_attr": "EY_OVERVIEW_WORD_COUNT_MIN", "overview_max_attr": "EY_OVERVIEW_WORD_COUNT_MAX"},
        #     {"name": "EarlyCareer", "overview_enum": ResumeSection.K9_EARLY_CAREER_OVERVIEW, "bullets_enum": ResumeSection.K9_EARLY_CAREER_BULLETS, "overview_min_attr": "EARLY_CAREER_OVERVIEW_WORD_COUNT_MIN", "overview_max_attr": "EARLY_CAREER_OVERVIEW_WORD_COUNT_MAX"},
        #     {"name": "Competencies", "bullets_enum": ResumeSection.K10_COMPETENCIES},
        # ]

        for section_data in self.QA_PROVENANCE_SECTIONS: # Iterate through configured sections
            name = section_data["name"] # Get the readable name (e.g., "Unify", "IBM")

            # --- Process Overview ---
            if section_data.get("overview_enum"):
                overview_enum = section_data["overview_enum"]
                overview_text = staging_buffer.get(overview_enum.value, "")
                if overview_text and isinstance(overview_text, str): # Check if text exists and is string
                    word_count = count_words_ms_word_style(overview_text) # Use consistent counting

                    # Safely get min/max targets from self.constraints using attribute names defined in QA_PROVENANCE_SECTIONS
                    min_target_attr = section_data.get("overview_min_attr", "DEFAULT_MIN_DOES_NOT_EXIST") # Default to non-existent attr
                    max_target_attr = section_data.get("overview_max_attr", "DEFAULT_MAX_DOES_NOT_EXIST")
                    min_target = getattr(self.constraints, min_target_attr, 0) # Fallback to 0 if attr missing
                    max_target = getattr(self.constraints, max_target_attr, 999) # Fallback to 999 if attr missing

                    status, target_range_str = self._check_word_count(word_count, min_target, max_target)
                    rows.append([
                        name,
                        "Overview",
                        "Customized", # Overviews are generally rewritten/customized
                        str(word_count),
                        target_range_str,
                        status,
                        overview_text[:60] + ("..." if len(overview_text) > 60 else "") # Add ellipsis only if truncated
                    ])

            # --- Process Bullets ---
            bullets_enum = section_data.get("bullets_enum")
            if bullets_enum:
                # Determine the correct key to fetch the average length for this section's bullets
                # The key should match the keys generated by _calculate_master_avg_bullet_length (e.g., "Unify", "IBM", "Competencies")
                ...
                bullets = staging_buffer.get(bullets_enum.value, []) # Get bullet list from buffer

                if isinstance(bullets, list) and bullets: # Check if it's a non-empty list
                    for i, bullet_item in enumerate(bullets):
                        ...
                        if isinstance(bullet_item, dict):
                            # Handle dictionary bullets (most common case)
                            # Check both 'text' (from rewrite) and 'bullet_text' (from master)
                            bullet_text = bullet_item.get('text', bullet_item.get('bullet_text',''))
                            # Use pre-calculated 'word_count' if available, otherwise calculate
                            ...
                            # Handle list of strings (e.g., K7 TraderSense fallback)
                            bullet_text = bullet_item
                            word_count = count_words_ms_word_style(bullet_text)
                            # Assign Verbatim provenance specifically for K7 strings, otherwise unknown
                            provenance = BulletProvenance.Verbatim.value if bullets_enum == ResumeSection.K7_TRADERSENSE_BULLETS else "N/A"
                        else:
                            # Log unexpected item type but continue gracefully
                            logging.warning(f"Unexpected item type in {bullets_enum.value}[{i}]: {type(bullet_item)}. Skipping.")
                            continue # Skip this item

                        # Calculate min/max target range based on the correct average length and tolerance
                        min_target = round(avg_len * (1 - tolerance))
                        max_target = round(avg_len * (1 + tolerance))

                        # Check status and get the target range string
                        status, target_range_str = self._check_word_count(word_count, min_target, max_target)

                        # Append row data to the list for the table
                        rows.append([
                            name,
                            str(i + 1), # Item number (1-based index)
                            provenance,
                            str(word_count),
                            target_range_str,
                            status,
                            bullet_text[:60] + ("..." if len(bullet_text) > 60 else "") # Add ellipsis only if truncated
                        ])
                # else: # Optionally log if bullets list is empty or not a list
                #    logging.debug(f"No valid bullet list found for {bullets_enum.value}")

        # Format the collected rows into a plain text table
        lines.extend(self._format_plain_text_table(headers, rows))
        lines.append("```")
        return lines
