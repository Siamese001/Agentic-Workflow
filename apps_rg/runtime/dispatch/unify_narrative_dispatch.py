...
def load_companion_bullets_text() -> str:
    path = resolve_latest_real_l2(REPO_ROOT, "unify_bullets")
...
def build_prompt_messages(runtime_payload: dict[str, Any], companion_text: str) -> list[dict[str, str]]:
    ...
    companion_block = (
        f"\nACCEPTED_UNIFY_BULLETS (read-only; do not recap each line):\n{companion_text}\n"
...
{fact_lines}
{companion_block}
Write one narrative_sentence only: strong platform-leadership arc for Unify Consulting, third person, one period at the end, under 52 words, no \"while also\" stack.
""".strip()
...
def run_dispatch(args: argparse.Namespace) -> int:
    ...
    bullets_l2 = resolve_latest_real_l2(REPO_ROOT, "unify_bullets")
    companion_ref = (
