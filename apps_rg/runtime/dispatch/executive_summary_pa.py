...
def format_srfs_style_only_quality_oneshot_block() -> str:
    ...
    return (
        ...
        "**Sentence 2 - governed runtime / architecture mechanics:**\n"
        "- Deterministic routing, orchestration, retrieval, validation, policy, traceability, auditability, reliability "
        "**when supported** - **no** revenue, margin, dollars, **team/org scale** (`8 to 28`, specialists), **or** "
        "credential tokens.\n"
        ...
        "- **Write an integrated credibility clause**, not a bare cert list: **pair** AWS / Databricks / FSA (or other "
        "proved tokens) with **applied depth** backed by the same ALLOWED_SOURCE_FACT_IDS (e.g. quantitative rigor, "
        "causal or statistical depth, distributed-systems engineering) **only when that substance appears in the "
        "selected fact blob**.\n"
        "- **Preferred shape (STYLE_ONLY_NOT_PROOF; emit words only when IDs support them):** "
        "``Pairs AWS, Databricks, and FSA credentials with applied depth in causal inference, statistics, and distributed "
        "systems engineering.``\n"
        ...
        "- S1: Engineering executive building production-grade governed AI platforms for regulated enterprise environments.\n"
        "- S2: Designs and operates governed runtime systems that combine deterministic routing, multi-agent orchestration, "
        "graph-aware retrieval, validation controls, policy enforcement, and traceability to improve reliability and "
        "auditability.\n"
        ...
        "- S5 (integrated credibility): **Pairs** proved AWS / Databricks / FSA anchors with applied quantitative or "
        "systems-engineering depth **only** when the same selected facts support both the tools and the depth clause - "
        "never a **Holds ...** inventory lead.\n"
