...
MIGRATED from apps_rg/runtime/entry/dispatch.py
...
# MIGRATED: Import from apps_rg.runtime.bindings instead of agentic_core
...
def _save_stage_output(run_dir: Path, stage: str, data: Any) -> None:
    ...
    except Exception:  # guardian: allow-broad-net -- stage persistence is best-effort; must never block the pipeline
...
def apps_rg_dispatch(envelope: RequestEnvelope) -> X3Disposition:
    ...
    if route.model_generation_required:
        ...
        if fec is None:
            ...
            except Exception:  # guardian: allow-broad-net -- resume text extraction is best-effort
                _resume_text = ""
