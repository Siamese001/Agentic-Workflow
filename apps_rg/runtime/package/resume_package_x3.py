"""Resume-wide package disposition (offline): rolls up generated lanes + assembly + DOCX proofs."""
...
CONSTRUCTOR_MODULE = "apps_rg.runtime.package.resume_package_x3"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Emit resume_package manifest + disposition + receipt (offline).")
    parser.add_argument(
        "--repo-root",
        type=Path,
        default=None,
        help="Repository root override (defaults to inferred workspace root)",
    )
    args = parser.parse_args(argv)

    rr = args.repo_root or repo_root_default()
    emitted = emit_resume_package_artifacts(resolve_resume_package_paths(repo_root=rr))
    dsp = emitted["resume_package_disposition"]
    sys.stdout.write(
        json.dumps({"final_x3_code": dsp.get("final_x3_code"), "deterministic_blocked": dsp.get("deterministic_blocked")}, indent=2)
        + "\n"
    )
    return 0 if isinstance(dsp, Mapping) else 1
