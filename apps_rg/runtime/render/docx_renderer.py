...
def main() -> None:
    paths = resolve_docx_renderer_paths()
    res = build_docx_from_final_resume(paths)
    print(
        f"DOCX_RENDER_DONE gates_all_pass={res['gates_all_pass']} docx="
        f"{paths.output_docx.relative_to(paths.repo_root).as_posix()}",
    )
