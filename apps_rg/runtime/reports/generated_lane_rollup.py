...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Aggregate per-lane runtime proof artifacts.")
    parser.add_argument(
        "--artifact-mode",
        choices=("real", "mock"),
        default="real",
    ...
    parser.add_argument(
        "--include-mock",
        action="store_true",
        help="Alias for --artifact-mode mock (roll up latest mock-bucket run per lane).",
    )
    args = parser.parse_args(argv)
    mode: Literal["real", "mock"] = "mock" if args.include_mock else args.artifact_mode  # type: ignore[assignment]

    ROLLUP_DIR.mkdir(parents=True, exist_ok=True)
    data = build_rollup(rollup_artifact_mode=mode)
    json_path = ROLLUP_DIR / "generated_lane_rollup.json"
    md_path = ROLLUP_DIR / "generated_lane_rollup.md"
