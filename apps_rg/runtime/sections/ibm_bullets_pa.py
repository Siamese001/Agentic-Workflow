...
def _legacy_i0(runtime_payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Example: a slot whose fact carries metric_raw='$10M new ARR' must state '$10M' (or '$10M ARR'); "
        "a slot bound to metric_ibm_20pct_joint_revenue_growth must state '20%'. "
        "This rule NEVER applies to the forbidden/HOLD figures above (25/30/35/40%, $15M, $30M): even if "
        "a fact carries one, write that bullet qualitatively with NO numeric figure. "
        ...
        "ITS OWN slot's selected_fact_plan fact (that slot's claim_text) — the X1D grader compares each "
        "bullet against its assigned source fact and decisively fails mismatches. NEVER borrow another "
        "slot's activity, technology, or metric (e.g. slot 001's '$10M ARR' may appear ONLY in bullet 001); "
        ...
        "commercial engagement (Salesforce-driven pipeline analytics, client portfolio growth, alliance "
