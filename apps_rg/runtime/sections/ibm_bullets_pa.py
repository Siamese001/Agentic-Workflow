...
def _legacy_i0(runtime_payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "LENGTH: bullet_text is EXACTLY ONE sentence of AT MOST 300 characters including spaces "
        "(hard cap — any bullet over 320 characters or with more than one sentence is rejected; keep it tight). "
        ...
        "METRIC SURFACING: when a bullet's selected_fact_plan fact has has_metric=true, a non-empty "
        "metric_raw, AND that figure is bound to an allowed_metric_outcome_id, weave the metric token "
        "INTO the single bullet sentence (e.g. metric_raw='$10M new ARR' -> '...generating $10M in new ARR "
        "across regulated portfolios'). Do NOT append a separate metric sentence like 'Delivered $10M at "
        "enterprise scale.' — that is a second sentence and is rejected. Mirror the figure exactly and set "
        "has_metric=true with bound metric_outcome_ids. If the figure is NOT bound to an "
        "allowed_metric_outcome_id, or is an unanchored/forbidden figure (e.g. 25/30/35/40%), write the bullet "
        "qualitatively with NO numeric figure and has_metric=false.\n"
        "GATED-METRIC BAN: NEVER emit a "
        "metric whose outcome_id is only *_gated / unconfirmed (e.g. the 10% FinOps savings figure mapped "
        "to metric_ibm_10pct_finops_savings_gated). If a savings/efficiency figure is not bound to a "
        "CONFIRMED promotable metric_outcome_id, write that bullet qualitatively with NO numeric figure.\n"
        ...
        "fact_engineering_platform_* (those are Unify facts). Do NOT state any percentage that is not bound to an "
        "allowed_metric_outcome_id — never write 25%, 30%, 35%, or 40%.\n\n"
        "# Positioning\n"
