...
def _legacy_i0(runtime_payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "METRIC SURFACING: when a bullet's "
        "selected_fact_plan fact has has_metric=true and a non-empty metric_raw, the bullet_text MUST "
        "literally include that metric token. Do NOT write a qualitative bullet that drops its own "
        "approved metric. Example: a slot whose fact carries metric_raw='$10M new ARR' must state "
        "'$10M' (or '$10M ARR'); a slot with metric_raw='30% cost optimization' must state '30%'. "
        "Mirror the metric_raw figure exactly; set has_metric=true and bind metric_outcome_ids.\n"
