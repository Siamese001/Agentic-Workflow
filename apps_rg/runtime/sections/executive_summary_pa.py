...
def format_graph_proof_pool_appendix(runtime_payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"- authority: in-memory augmented_skills_graph; proof_pool_type={pool_type}; "
        f"selection_id={sid or '(in-memory)'}\n"
        f"- HIGH executive_summary proof IDs only: [{id_tail}]\n"
        f"- excluded_counts: blocked={counts['blocked_facts_count']}; "
        ...
        "- Claims must cite ALLOWED_SOURCE_FACT_IDS verbatim; JD_TEXT/BRIEFING are targeting-only, not citations.\n"
        "- Omit unsupported JD themes and excluded MEDIUM/LOW/NEEDS_VERIFICATION rows unless promoted by human confirmation.\n"
        "- Cite every non-credential allowed fact; credential facts are optional context and belong mainly in credentials/competencies sections.\n"
    )
