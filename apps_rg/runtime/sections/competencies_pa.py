...
def build_competencies_assembly_input(
    ...
    *,
    ...
    u0 = (
        ...
        "- REQUIRED FAMILY COVERAGE: use the selected_competency_families from the graph evidence plan "
        "as the authoritative coverage set. Do not force a fixed SVP-Engineering taxonomy when the JD "
        "and briefing point to a different role family. For partnership-heavy roles, the graph-backed "
        "coverage should naturally emphasize partnerships/ecosystem execution, commercial productization, "
        "enterprise adoption, and leadership-adjacent terms when supported. Every family in "
        "selected_competency_families must show up somewhere in the 8 categories' terms unless the plan "
        "explicitly marks it as unavailable or unsupported. If the graph cannot support the family, fail "
        "the section at the judge instead of inventing coverage.\n"
    )
