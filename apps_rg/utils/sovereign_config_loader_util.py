...
class SovereignConfigLoader:
    ...
    def _get_default_scaffold() -> OrchestrationTopology:
        ...
        return OrchestrationTopology(phases={'HOP1': ['HOP1_CLERK'], 'HOP2': ['HOP2_ENRICH']}, agents={'HOP1_CLERK': AgentSpec(name='HOP1_CLERK', module_path='apps_rg.engines.hop1_clerk_engine.ClerkExtractionEngine', inputs=['master_resume'], outputs=['extraction_data']), 'HOP2_ENRICH': AgentSpec(name='HOP2_ENRICH', module_path='apps_rg.engines.hop2_enrichment_engine.EnrichmentEngine', inputs=['extraction_data'], outputs=['enriched_data'])})
