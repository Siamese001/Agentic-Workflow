...
class AgentExecutor:
    ...
    def execute(self, messages: list[AgentMessage], system_prompt: str | None=None, tools: list[dict[str, Any]] | None=None, **kwargs) -> AgentResponse:
    ...
    def _execute_internal(self, messages: list[AgentMessage], system_prompt: str | None, tools: list[dict[str, Any]] | None, **kwargs) -> AgentResponse:
        ...
        gateway_response = self._try_execute_via_gateway(formatted_messages, model, system_prompt, tools, **kwargs)
        if gateway_response is not None:
            return gateway_response
        logger.warning('[RG-GAP-04] SovereignLLMGateway unavailable; falling back to direct SDK for provider=%s', self.config.provider.value)
    ...
    def _try_execute_via_gateway(self, formatted_messages: list[dict[str, str]], model: str, system_prompt: str | None, tools: list[dict[str, Any]] | None, **kwargs) -> AgentResponse | None:
        ...
        try:
            ...
            request = GenerationRequest(agent_id=f'AgentExecutor.{self.config.provider.value}', provider=self.config.provider.value, model=model, prompt=prompt, system_prompt=_system, temperature=self.config.temperature, max_tokens=self.config.max_tokens)
            response = gateway.generate(request)
            text = response.text if hasattr(response, 'text') else str(response)
            return AgentResponse(content=text, finish_reason='stop', raw_response=response)
        except Exception as exc:
            logger.error('[RG-GAP-04] SovereignLLMGateway.generate failed for provider=%s: %s; falling back to direct SDK', self.config.provider.value, exc)
            return None

    def _format_messages(self, messages: list[AgentMessage], system_prompt: str | None) -> list[dict[str, str]]:
    ...
    def _execute_openai(self, messages: list[dict[str, str]], model: str, tools: list[dict[str, Any]] | None, **kwargs) -> AgentResponse:
        """Execute using OpenAI client."""
        client = self._get_client()
        params = {'model': model, 'messages': messages, 'temperature': self.config.temperature, 'max_tokens': self.config.max_tokens, **kwargs}
        if tools:
            params['tools'] = tools
        response = client.chat.completions.create(**params)
        message = response.choices[0].message
        return AgentResponse(content=message.content or '', finish_reason=response.choices[0].finish_reason, usage={'prompt_tokens': response.usage.prompt_tokens, 'completion_tokens': response.usage.completion_tokens, 'total_tokens': response.usage.total_tokens}, tool_calls=message.tool_calls if hasattr(message, 'tool_calls') else None, raw_response=response)

    def _execute_anthropic(self, messages: list[dict[str, str]], model: str, tools: list[dict[str, Any]] | None, **kwargs) -> AgentResponse:
        ...
        if messages and messages[0]['role'] == 'system':
            ...
            messages = messages[1:]
        params = {'model': model, 'messages': messages, 'temperature': self.config.temperature, 'max_tokens': self.config.max_tokens or 4096, **kwargs}
        if system:
        ...
        return AgentResponse(content=content, finish_reason=response.stop_reason, usage={'prompt_tokens': response.usage.input_tokens, 'completion_tokens': response.usage.output_tokens, 'total_tokens': response.usage.input_tokens + response.usage.output_tokens}, tool_calls=tool_calls if tool_calls else None, raw_response=response)

    def _execute_google(self, messages: list[dict[str, str]], model: str, tools: list[dict[str, Any]] | None, previous_interaction_id: str | None=None, **kwargs) -> AgentResponse:
        """Execute using Google GenAI client with Interactions API."""
        client = self._get_client()
        if hasattr(client, 'interactions'):
            return self._execute_google_interactions(client, messages, model, tools, previous_interaction_id, **kwargs)
        else:
            return self._execute_google_legacy(client, messages, model, **kwargs)

    def _execute_google_interactions(self, client, messages: list[dict[str, str]], model: str, tools: list[dict[str, Any]] | None, previous_interaction_id: str | None, **kwargs) -> AgentResponse:
        ...
        def _execute_with_retry():
            try:
                ...
                return AgentResponse(content=content, finish_reason=getattr(response, 'finish_reason', 'stop'), usage={}, interaction_id=getattr(response, 'id', None), raw_response=response)
            except Exception as e:
                logger.error(f'Google GenAI Interactions API error: {e}')
                raise
        return _execute_with_retry()

    def _execute_google_legacy(self, genai_module, messages: list[dict[str, str]], model: str, **kwargs) -> AgentResponse:
        ...
        return AgentResponse(content=str(getattr(response, 'text', '') or ''), finish_reason=getattr(response, 'candidates', [{}])[0].get('finish_reason', 'stop'), usage={}, raw_response=response)

    def _execute_litellm(self, messages: list[dict[str, str]], model: str, tools: list[dict[str, Any]] | None, **kwargs) -> AgentResponse:
    ...
    def execute_structured(self, messages: list[AgentMessage], response_model: Any, system_prompt: str | None=None, **kwargs) -> Any:
        ...
        formatted_messages = self._format_messages(messages, system_prompt)
        model = self.config.model or self._get_default_model()
        response = instructor_client.chat.completions.create(model=model, messages=formatted_messages, response_model=response_model, temperature=self.config.temperature, max_tokens=self.config.max_tokens, **kwargs)
        return response

    def _execute_google_structured(self, messages: list[AgentMessage], response_model: Any, system_prompt: str | None, **kwargs) -> Any:
        ...
        response = client.interactions.create(model=model, input=input_messages, config={'response_mime_type': 'application/json', 'response_schema': schema, 'temperature': self.config.temperature, 'max_output_tokens': self.config.max_tokens})
        content = ''
        if hasattr(response, 'candidates') and response.candidates:
            candidate = response.candidates[0]
            if hasattr(candidate, 'content') and candidate.content:
...
def create_agent_executor(provider: Provider=Provider.OPENAI, model: str | None=None, temperature: float=0.7, **kwargs) -> AgentExecutor:
