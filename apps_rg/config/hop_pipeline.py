...
HOP1..HOP7 config schemas already present in ``agent_spec_config.py``:
    ...
    HOP1  ClerkExtraction     — parse JD + master resume
    HOP2  DataEnrichment      — normalize extracted data
    HOP3  ResumeGeneration    — LLM synthesis of tailored resume
    HOP4  FactCheck           — cross-reference against master resume
    HOP5  BulletDiversityGate — thematic-spread gate (evaluator-style)
    HOP6  ContentOptimizer    — keyword/action-verb refinement
    HOP7  GenerationDiagnostics — final scorecard + QA report
...
``HopPipelineExecutor`` — useful for replay, composability, and
seal_step integration.

See ``apps_rg/engines/hop_pipeline_adapters.py`` for the BaseModel↔dict
