...
class CollateralRules(BaseModel):
    ...
    min_ltv: Optional[float] = Field(None, ge=0, le=1, description="Minimum LTV")
    max_ltv: Optional[float] = Field(None, ge=0, le=1, description="Maximum LTV")
...
class PolicyContext(BaseModel):
    ...
    min_dscr: Optional[float] = Field(None, gt=0, description="Minimum DSCR requirement")
    max_debt_to_ebitda: Optional[float] = Field(None, gt=0, description="Maximum Debt/EBITDA")
    min_fico: Optional[int] = Field(None, ge=300, le=850, description="Minimum FICO requirement")
    restricted_industries: List[str] = Field(default_factory=list, description="NAICS codes restricted")
    prohibited_jurisdictions: List[str] = Field(default_factory=list, description="Prohibited states/countries")
    max_single_customer_concentration_pct: Optional[float] = Field(None, ge=0, le=100, description="Max customer concentration")
    ...
    human_review_triggers: List[str] = Field(default_factory=list, description="Auto-trigger for human review")
