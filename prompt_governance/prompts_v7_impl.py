...
def build_librarian_mission_extraction_prompt(job_description: str, company_name: str, job_title: str) -> str:
    """
    ...
    return template.format(job_description=job_description, company_name=company_name, job_title=job_title)

def build_librarian_strategic_analysis_prompt(job_description: str, rag_mission: 'RAGMission', previous_context: Optional[str]=None) -> str:
    """
    ...
    return template.format(job_description=job_description, target_company_name=rag_mission.target_company_name, precise_role_title=rag_mission.precise_role_title, key_technologies=', '.join(rag_mission.key_technologies), core_responsibilities=', '.join(rag_mission.core_responsibilities), context_section=context_section)
...
def build_phase1_prompt(job_description: str, mission: 'RAGMission', master_resume_index: 'MasterResumeIndex', company_name: Optional[str]=None, librarian_context: Optional[str]=None) -> str:
    """
    ...
    return template.format(job_description=job_description[:1500], candidate_context=candidate_context, target_company_name=mission.target_company_name, tech_search_line=tech_search_line, key_technologies=', '.join(mission.key_technologies), librarian_section=librarian_section, strategic_context=strategic_context)

def build_phase2_prompt(job_description: str, mission: 'RAGMission', industry: str, librarian_context: Optional[str]=None) -> str:
    """
    ...
    return template.format(precise_role_title=mission.precise_role_title, industry=industry, signal_gap_keywords=', '.join(mission.signal_gap_keywords), target_company_name=mission.target_company_name, librarian_section=librarian_section, competitors_section=competitors_section)

def build_phase3_prompt(job_description: str, mission: 'RAGMission', master_resume_index: 'MasterResumeIndex', peer_companies: List[str], comp_config: 'CompetitiveAnalysisConfig', industry: str, librarian_context: Optional[str]=None) -> str:
    """
    ...
    search_pattern_instruction = comp_config.search_pattern.format(role_title=mission.precise_role_title, peer_company='<peer_company>')
    librarian_section = ''
    if librarian_context:
        librarian_section = f'\n**Librarian Intelligence:**\n{librarian_context}\n'
    ...
    return template.format(target_company_name=mission.target_company_name, precise_role_title=mission.precise_role_title, job_description=job_description[:1000], peer_companies=', '.join(peer_companies), peer_companies_json=json.dumps(peer_companies), achievements_context=achievements_context, industry=industry, search_pattern_instruction=search_pattern_instruction, min_peer_jds=comp_config.min_peer_jds, table_stakes_threshold=comp_config.table_stakes_threshold, differentiator_threshold=comp_config.differentiator_threshold, librarian_section=librarian_section, differentiators_section=differentiators_section)

def build_phase4_prompt(mission: 'RAGMission', librarian_context: Optional[str]=None) -> str:
    """
    ...
    if librarian_context:
        librarian_section = f'\n**Librarian Intelligence:**\n{librarian_context}\n'
    return template.format(precise_role_title=mission.precise_role_title, core_responsibilities=', '.join(mission.core_responsibilities), key_technologies=', '.join(mission.key_technologies), queries=', '.join(queries), pain_points_section=pain_points_section, librarian_section=librarian_section)

def build_macro_tot_generation_prompt(base_prompt: str, draft_number: int, total_drafts: int, variation_instruction: str='') -> str:
    """
    ...
    return template.format(base_prompt=base_prompt, draft_number=draft_number, total_drafts=total_drafts, variation_instruction=variation_instruction)

def build_evaluator_scoring_prompt(drafts: List[str], criteria: Dict[str, object], section_name: str) -> str:
    """
    ...
    return template.format(section_name=section_name, drafts_text=drafts_text, criteria_text=criteria_text, num_drafts=len(drafts))

def build_macro_tot_synthesis_prompt(original_prompt: str, scored_drafts: List[Tuple[str, float]], top_k: int=2) -> str:
    """
    ...
    return template.format(original_prompt=original_prompt, top_drafts_text=top_drafts_text, top_k=top_k)

def build_narrative_prompt(company_name: str, title: str, target_sc: int, min_wc: int, max_wc: int, master_context: str, combined_signals_str: str, focus_instruction: str, k0_themes_str: str, **kwargs: object) -> str:
    """Builds the prompt for generating K4/K5/K6 narratives."""
    template = _get_prompt_template('artist_narrative')
    return template.format(company_name=company_name, title=title, target_sc=target_sc, min_wc=min_wc, max_wc=max_wc, master_context=master_context, combined_signals_str=combined_signals_str, focus_instruction=focus_instruction, k0_themes_str=k0_themes_str)

def build_verbatim_bullet_selection_prompt(master_bullets_text_list: List[str], verbatim_count: int, thematic_analysis: 'ThematicAnalysis') -> str:
    ...
    return template.format(bullets_list=bullets_list, verbatim_count=verbatim_count, keywords_str=', '.join(keywords_for_prompt) or 'N/A')

def build_customized_bullet_prompt(source_bullets_text: List[str], thematic_analysis: 'ThematicAnalysis') -> str:
    ...
    return template.format(bullets_input=bullets_input, context_keywords=', '.join(context_keywords), bullet_count=len(source_bullets_text))

def build_synthetic_bullet_prompt(count: int, company_name: str, job_description: str, thematic_analysis: 'ThematicAnalysis', context_bullets: str) -> str:
    ...
    return template.format(company_name=company_name, primary_theme=primary_theme, context_keywords=', '.join(context_keywords), context_bullets=context_bullets, job_description=job_description[:500], count=count)

def build_bullet_reorder_prompt(current_bullets_text_input: str, company_name: str, bullet_count: int, thematic_analysis: 'ThematicAnalysis') -> str:
    """
    ...
    if comp_intel:
        ...
        if isinstance(kw_raw, list):
            keywords_for_prompt = kw_raw[:10]
    return template.format(current_bullets_text_input=current_bullets_text_input, company_name=company_name, keywords_str=', '.join(keywords_for_prompt) or 'N/A', bullet_count=bullet_count)

def build_bullet_rewrite_prompt(original_bullet: str, target_word_count_range: Tuple[int, int], **kwargs: Dict[str, any]) -> str:
...
def build_overview_generation_prompt(bullet_summary_input: str, word_count_range: Tuple[int, int], thematic_analysis: 'ThematicAnalysis', job_description: str, **kwargs: Dict[str, object]) -> str:
    ...
    return template.format(bullet_summary_input=bullet_summary_input, theme_prompt_section=theme_prompt_section, min_wc=min_wc, max_wc=max_wc)

def build_generation_prompt_with_reinforced_constraints(base_prompt: str, constraints: Dict[str, object], attempt_number: int) -> str:
    """
...
def build_validator_factual_check_prompt(section_name: str, section_content: str, thematic_analysis: 'ThematicAnalysis', job_description: str) -> str:
    """
    ...
    return template.format(section_name=section_name, section_content=section_content, primary_theme=primary_theme, differentiators_str=', '.join(differentiators) or 'N/A', job_description=job_description[:800])
