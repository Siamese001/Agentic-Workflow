...
def build_librarian_strategic_analysis_prompt(job_description: str,
    ...
    return template.format(job_description=job_description,
        ...
        key_technologies=',
        '.join(rag_mission.key_technologies),
        core_responsibilities=',
        '.join(rag_mission.core_responsibilities),
...
def _format_resume_index_summary(index: 'MasterResumeIndex') -> str:
    ...
    if hasattr(index, 'achievement_catalog') and index.achievement_catalog:
        ach_summary = [f"{a.get('value',
            'N/A')} {a.get('metric_type',
            'achievement')}" for a in index.achievement_catalog[:3]]
        if ach_summary:
...
def build_phase2_prompt(job_description: str,
    ...
    return template.format(precise_role_title=mission.precise_role_title,
        industry=industry,
        signal_gap_keywords=',
        '.join(mission.signal_gap_keywords),
        target_company_name=mission.target_company_name,
        librarian_section=librarian_section,
...
def build_phase4_prompt(mission: 'RAGMission', librarian_context: Optional[str]=None) -> str:
    ...
    return template.format(precise_role_title=mission.precise_role_title,
        core_responsibilities=',
        '.join(mission.core_responsibilities),
        key_technologies=',
        '.join(mission.key_technologies),
        queries=',
        '.join(queries),
...
def build_evaluator_scoring_prompt(drafts: List[str],
    ...
    return template.format(section_name=section_name,
...
def build_verbatim_bullet_selection_prompt(master_bullets_text_list: List[str],
    ...
    return template.format(bullets_list=bullets_list,
...
def build_customized_bullet_prompt(source_bullets_text: List[str],
    ...
    return template.format(bullets_input=bullets_input,
        context_keywords=',
...
def build_synthetic_bullet_prompt(count: int,
    ...
    context_bullets: str) -> str:
    """Builds the prompt for generating synthetic bullets."""
    template = _get_prompt_template('artist_synthetic_bullet')
    primary_theme = thematic_analysis.primary_theme.get('name',
        'key responsibilities') if thematic_analysis.primary_theme else 'key responsibilities'
    primary_theme_kw = []
    if thematic_analysis and thematic_analysis.primary_theme:
        kw_raw = thematic_analysis.primary_theme.get('keywords', [])
        if isinstance(kw_raw, list):
...
def build_bullet_reorder_prompt(current_bullets_text_input: str,
    ...
    if comp_intel:
        ...
        if isinstance(kw_raw, list):
            keywords_for_prompt = kw_raw[:10]
    return template.format(current_bullets_text_input=current_bullets_text_input,
        company_name=company_name,
        keywords_str=',
        '.join(keywords_for_prompt) or 'N/A',
...
def build_overview_generation_prompt(bullet_summary_input: str,
    ...
    **kwargs: Dict[str,
    ...
    include_leadership_theme = any((kw in job_desc_lower for kw in ['lead',
        'manage',
        'director',
        'vp',
        'executive']))
    include_strategic_theme = any((kw in job_desc_lower for kw in ['strategy',
        'roadmap',
        'vision',
        'partnership',
        'alliance']))
    include_technical_theme = any((kw in job_desc_lower for kw in ['technical',
        'architect',
        'engineer',
        'platform',
        'cloud',
...
def build_validator_factual_check_prompt(section_name: str,
    ...
    primary_theme = thematic_analysis.primary_theme.get('name',
        'N/A') if thematic_analysis.primary_theme else 'N/A'
    differentiators = []
    comp_intel = getattr(thematic_analysis, 'competitive_intelligence', None)
    if comp_intel:
