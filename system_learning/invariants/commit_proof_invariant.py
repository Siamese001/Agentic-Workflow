...
# Sentinel hash for the empty / placeholder bytes -- used to detect churn commits.
...
class CommitProofInvariant:
    ...
    def verify(self) -> None:
        ...
        # version_id must be 64-char lowercase hex (SHA-256)
        ...
        # implementation_hash must be non-empty hex
        ...
        # implementation_hash must not be a known churn/placeholder hash
        if self.implementation_hash in _CHURN_HASHES:
            raise CommitProofViolation(
                f"COMMIT_PROOF_VIOLATION: implementation_hash {self.implementation_hash!r} "
                "matches a known churn/placeholder hash -- commit is not bound to real content"
            )
    ...
    def from_package(
        ...
        # version_id must match SHA-256 of canonical_bytes
        if version_id != impl_hash:
            raise CommitProofViolation(
                f"COMMIT_PROOF_VIOLATION: version_id {version_id!r} does not match "
                f"SHA-256(canonical_bytes()) = {impl_hash!r} -- proof not bound to implementation"
            )
