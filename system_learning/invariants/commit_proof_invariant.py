...
class CommitProofInvariant:
    ...
    def verify(self) -> None:
        ...
        if not isinstance(self.version_id, str) or len(self.version_id) != 64:
            raise CommitProofViolation(f'COMMIT_PROOF_VIOLATION: version_id must be 64-char hex, got {self.version_id!r}')
        if not _HEX_RE.match(self.version_id):
            raise CommitProofViolation(f'COMMIT_PROOF_VIOLATION: version_id is not lowercase hex: {self.version_id!r}')
        if not isinstance(self.implementation_hash, str) or not self.implementation_hash:
            raise CommitProofViolation('COMMIT_PROOF_VIOLATION: implementation_hash must be a non-empty hex string')
        if not _HEX_RE.match(self.implementation_hash):
            raise CommitProofViolation(f'COMMIT_PROOF_VIOLATION: implementation_hash is not hex: {self.implementation_hash!r}')
        if self.implementation_hash in _CHURN_HASHES:
            raise CommitProofViolation(f'COMMIT_PROOF_VIOLATION: implementation_hash {self.implementation_hash!r} matches a known churn/placeholder hash -- commit is not bound to real content')
        if not isinstance(self.commit_timestamp_utc, int) or self.commit_timestamp_utc <= 0:
            raise CommitProofViolation(f'COMMIT_PROOF_VIOLATION: commit_timestamp_utc must be > 0, got {self.commit_timestamp_utc!r}')

    @classmethod
    def from_package(cls, version_id: str, package: object, commit_timestamp_utc: int) -> CommitProofInvariant:
        ...
        if version_id != impl_hash:
            raise CommitProofViolation(f'COMMIT_PROOF_VIOLATION: version_id {version_id!r} does not match SHA-256(canonical_bytes()) = {impl_hash!r} -- proof not bound to implementation')
        proof = cls(version_id=version_id, implementation_hash=impl_hash, commit_timestamp_utc=commit_timestamp_utc)
        proof.verify()
        return proof
...
def verify_commit_proof(version_id: str, package: object, commit_timestamp_utc: int) -> CommitProofInvariant:
    ...
    """
    return CommitProofInvariant.from_package(version_id=version_id, package=package, commit_timestamp_utc=commit_timestamp_utc)
