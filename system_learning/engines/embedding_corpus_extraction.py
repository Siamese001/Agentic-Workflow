...
def write_jsonl_records(path: Path, records: list[CorpusRecord]) -> None:
    ...
    with open(path, 'w', encoding='utf-8') as f:
        for record in sorted_records:
            output_record = {'text': record.text, 'trace_id': record.trace_id, 'content_hash': record.content_hash, 'namespace': record.namespace}
...
def extract_healing_context_records(source: list[dict]) -> list[CorpusRecord]:
    ...
    for i, item in enumerate(source):
        ...
        records.append(CorpusRecord(text=text, trace_id=trace_id, content_hash=content_hash, namespace='healing_contexts'))
    return records
...
def extract_telemetry_event_records(source: list[dict]) -> list[CorpusRecord]:
    ...
    for i, item in enumerate(source):
        ...
        if 'trace_id' in item:
            ...
            trace_id = content_hash[:16]
        records.append(CorpusRecord(text=text, trace_id=trace_id, content_hash=content_hash, namespace='telemetry_events'))
    return records
...
def extract_dpo_pair_records(source: list[dict]) -> list[CorpusRecord]:
    ...
    for i, item in enumerate(source):
        ...
        if 'trace_id' in item:
            ...
            trace_id = content_hash[:16]
        records.append(CorpusRecord(text=text, trace_id=trace_id, content_hash=content_hash, namespace='dpo_pairs'))
    return records
__all__ = ['CorpusRecord', 'canonical_record_json', 'compute_content_hash', 'write_jsonl_records', 'extract_healing_context_records', 'extract_telemetry_event_records', 'extract_dpo_pair_records']
