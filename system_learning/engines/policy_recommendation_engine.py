...
class PolicyRecommendation:
    ...
    def to_canonical_json(self) -> str:
        ...
        data = {'profile_id': self.profile_id, 'recommended_changes': {k: round(v, 6) for k, v in self.recommended_changes.items()}, 'rationale': self.rationale, 'confidence_score': round(self.confidence_score, 6)}
        return json.dumps(data, sort_keys=True, separators=(',', ':'))

class PolicyRecommendationEngine:
    """Generates deterministic, bounded policy recommendations from drift analysis."""

    def generate_recommendation(self, *, drift_summary: DriftSummary, active_profile: RetrievalProfile, now_utc: int) -> PolicyRecommendation:
        ...
        if drift_summary.drift_flag:
            ...
            if max_cutoff_reduction > 1e-06:
                ...
                rationale_parts.append(f'Lower similarity_cutoff from {active_profile.similarity_cutoff:.6f} to {new_cutoff:.6f} (drift_score={drift_summary.drift_score:.6f})')
            ...
            if max_cap_increase > 1e-06:
                ...
                rationale_parts.append(f'Increase influence_cap from {active_profile.influence_cap:.6f} to {new_cap:.6f} (drift_score={drift_summary.drift_score:.6f})')
        ...
        deterministic_digest = self._compute_digest(profile_id=active_profile.profile_id, drift_summary=drift_summary, recommended_changes=recommended_changes, rationale=rationale, confidence_score=confidence_score, now_utc=now_utc)
        return PolicyRecommendation(profile_id=active_profile.profile_id, recommended_changes=recommended_changes, rationale=rationale, confidence_score=round(confidence_score, 6), deterministic_digest=deterministic_digest)

    def _compute_digest(self, *, profile_id: str, drift_summary: DriftSummary, recommended_changes: dict[str, float], rationale: str, confidence_score: float, now_utc: int) -> str:
        """Compute deterministic SHA-256 digest of recommendation data."""
        data = {'profile_id': profile_id, 'drift_summary': {'profile_id': drift_summary.profile_id, 'batch_size': drift_summary.batch_size, 'mean_cosine': round(drift_summary.mean_cosine, 6), 'p95_cosine': round(drift_summary.p95_cosine, 6), 'drift_flag': drift_summary.drift_flag, 'drift_score': round(drift_summary.drift_score, 6)}, 'recommended_changes': {k: round(v, 6) for k, v in sorted(recommended_changes.items())}, 'rationale': rationale, 'confidence_score': round(confidence_score, 6), 'now_utc': now_utc, 'engine_version': 'W4-D-v1.0'}
...
class MemoryAwarePolicyRecommendationEngine(PolicyRecommendationEngine):
    ...
    def generate_recommendation(self, *, drift_summary: DriftSummary, active_profile: RetrievalProfile, now_utc: int) -> PolicyRecommendation:
        recommendation = super().generate_recommendation(drift_summary=drift_summary, active_profile=active_profile, now_utc=now_utc)
        try:
