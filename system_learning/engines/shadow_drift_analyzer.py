...
class DriftSummary:
    ...
    def to_canonical_json(self) -> str:
        ...
        data = {'profile_id': self.profile_id, 'batch_size': self.batch_size, 'mean_cosine': round(self.mean_cosine, 6), 'p95_cosine': round(self.p95_cosine, 6), 'drift_flag': self.drift_flag, 'drift_score': round(self.drift_score, 6)}
...
class ShadowDriftAnalyzer:
    ...
    def analyze_batch(self, *, shadow_records: list[dict[str, Any]], profile_id: str, now_utc: int) -> DriftSummary:
        ...
        if not shadow_records:
            return DriftSummary(profile_id=profile_id, batch_size=BATCH_SIZE, mean_cosine=1.0, p95_cosine=1.0, drift_flag=False, drift_score=0.0, deterministic_digest=self._compute_digest([], profile_id, now_utc))
        cosine_values = []
        for record in shadow_records:
            if 'primary_shadow_cosine' in record:
                cosine_values.append(float(record['primary_shadow_cosine']))
        if not cosine_values:
            return DriftSummary(profile_id=profile_id, batch_size=len(shadow_records), mean_cosine=1.0, p95_cosine=1.0, drift_flag=False, drift_score=0.0, deterministic_digest=self._compute_digest([], profile_id, now_utc))
        cosine_values.sort()
        ...
        summary = DriftSummary(profile_id=profile_id, batch_size=len(shadow_records), mean_cosine=mean_cosine, p95_cosine=p95_cosine, drift_flag=drift_flag, drift_score=drift_score, deterministic_digest=deterministic_digest, drift_threshold=self._drift_threshold)
    ...
    def _emit_to_registry(self, summary: DriftSummary) -> None:
        ...
        try:
            ...
            digest_payload = _json.dumps({'source': 'shadow', 'metric': 'p95_cosine', 'value': summary.p95_cosine, 'threshold': summary.drift_threshold, 'digest': summary.deterministic_digest}, sort_keys=True, separators=(',', ':'))
            deterministic_digest = hashlib.sha256(digest_payload.encode()).hexdigest()
            entry = DriftRegistryEntry(source='shadow', timestamp_iso=_json.dumps(summary.profile_id), metric_name='p95_cosine', current_value=summary.p95_cosine, threshold_value=summary.drift_threshold, drift_flag=summary.drift_flag, severity=severity, deterministic_digest=deterministic_digest)
            ...
            if summary.drift_flag:
                try:
                    ...
                    pkg = MetaLearningChangePackage.create(kind='drift_alert', payload={'source': 'shadow', 'metric_name': 'p95_cosine', 'current_value': summary.p95_cosine, 'threshold_value': summary.drift_threshold, 'severity': severity, 'drift_score': summary.drift_score, 'digest': summary.deterministic_digest, 'profile_id': summary.profile_id}, proposal_only=True)
                    bus.enqueue(pkg)
                # guardian: allow-silent-swallow
                except Exception:
                    pass
    ...
    def _compute_digest(self, cosine_values: list[float], profile_id: str, now_utc: int) -> str:
        ...
        data = {'profile_id': profile_id, 'now_utc': now_utc, 'cosine_values': [round(v, 6) for v in sorted(cosine_values)], 'drift_threshold': self._drift_threshold}
