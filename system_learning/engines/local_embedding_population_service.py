...
class LocalEmbeddingPopulationService:
    ...
    def populate_from_jsonl(
        ...
        *,
        ...
        """
        # Sort source files for deterministic ordering
        sorted_files = sorted(source_files, key=lambda p: str(p))

        # Collect all records with deterministic ordering
        all_records = []
        ...
        # Sort by global order: (file_path ASC, record_index ASC)
        ...
        for i, (_, record) in enumerate(all_records):
            # Canonicalize record before text extraction
            ...
            vectors.append(text)  # Store text for batch embedding
            metadatas.append(
                {
