...
class LocalFAISSStore:
    ...
    def __init__(self, base_path: Path, *, telemetry_callback: Callable[[str, dict[str, Any]], None] | None=None) -> None:
    ...
    def search(self, index_id: str, query_vector: list[float], top_k: int, cutoff: float) -> list[tuple[str, str, float]]:
    ...
    def finalize_build(self, index_id: str, *, built_at_utc: int, canonicalization_version: str, embedding_model_version: str, embedding_model_checksum: str) -> IndexBuildMetadata:
        ...
        faiss_ver = 'faiss-IndexFlatIP-v1' if _faiss_available() and 'faiss_index' in idx else 'memory-fallback-v1'
        metadata = IndexBuildMetadata(index_id=index_id, faiss_version=faiss_ver, build_seed=idx['seed'], canonicalization_version=canonicalization_version, embedding_model_version=embedding_model_version, embedding_model_checksum=embedding_model_checksum, built_at_utc=built_at_utc, index_version_hash=index_version_hash, vector_count=len(vectors), dimension=idx['dimension'])
        idx['metadata'] = metadata
        idx['version_hash'] = index_version_hash
        if self._telemetry_callback:
            self._telemetry_callback('faiss_index_rebuilt', {'index_id': index_id, 'vector_count': len(vectors), 'index_version_hash': index_version_hash})
        return metadata
    ...
    def rebuild(self, index_id: str, *, built_at_utc: int, canonicalization_version: str, embedding_model_version: str, embedding_model_checksum: str) -> IndexBuildMetadata:
    ...
    def _compute_version_hash(vectors: list, metadatas: list) -> str:
        ...
        for vec, meta in zip(vectors, metadatas):
            ...
            entry = {'content_hash': meta.get('content_hash', ''), 'trace_id': meta.get('trace_id', ''), 'vector_bytes': vector_bytes.hex()}
            entry_bytes = json.dumps(entry, sort_keys=True, separators=(',', ':'), ensure_ascii=True).encode('ascii')
    ...
    def persist_to_disk(self, index_id: str, dest_dir: Path, *, embedder_id: str, model_version: str) -> str:
        ...
        if index_id not in self._memory_indexes:
            raise IndexNotBuiltError(f'Index {index_id} not found; call begin_build/add_vectors/finalize_build first')
        ...
        index_data = {'schema_version': _SCHEMA_VERSION, 'index_id': index_id, 'dimension': dimension, 'vector_count': len(vectors), 'vectors': [list(v) for v in vectors], 'metadatas': metadatas}
        index_bytes = json.dumps(index_data, sort_keys=True, separators=(',', ':'), ensure_ascii=True).encode('ascii')
        sha256_index = hashlib.sha256(index_bytes).hexdigest()
        meta_data = {'dims': dimension, 'embedder_id': embedder_id, 'index_id': index_id, 'index_version_hash': version_hash, 'model_version': model_version, 'schema_version': _SCHEMA_VERSION, 'vector_count': len(vectors)}
        meta_bytes = json.dumps(meta_data, sort_keys=True, separators=(',', ':'), ensure_ascii=True).encode('ascii')
        sha256_meta = hashlib.sha256(meta_bytes).hexdigest()
        manifest_data = {'dims': dimension, 'embedder_id': embedder_id, 'model_version': model_version, 'schema_version': _SCHEMA_VERSION, 'sha256_index': sha256_index, 'sha256_meta_canonical': sha256_meta, 'vector_count': len(vectors)}
        manifest_bytes = json.dumps(manifest_data, sort_keys=True, separators=(',', ':'), ensure_ascii=True).encode('ascii')
        ...
        if self._telemetry_callback:
            self._telemetry_callback('faiss_index_persisted', {'index_id': index_id, 'vector_count': len(vectors), 'digest': digest, 'embedder_id': embedder_id, 'model_version': model_version})
        return digest

    def load_from_disk(self, index_id: str, source_dir: Path, *, expected_embedder_id: str | None=None) -> None:
        ...
        required = {'schema_version', 'embedder_id', 'model_version', 'dims', 'vector_count', 'sha256_index', 'sha256_meta_canonical'}
        ...
        if expected_embedder_id is not None and manifest['embedder_id'] != expected_embedder_id:
            raise EmbedderMismatchError(f"embedder_id mismatch: manifest has '{manifest['embedder_id']}' but runtime expects '{expected_embedder_id}' — index at {source_dir} was built with a different model and cannot be loaded")
        ...
        from system_learning.types.index_build_metadata_types import IndexBuildMetadata
        metadata = IndexBuildMetadata(index_id=index_id, faiss_version='disk-json-v1', build_seed=0, canonicalization_version=_SCHEMA_VERSION, embedding_model_version=manifest['model_version'], embedding_model_checksum=manifest['sha256_index'], built_at_utc=0, index_version_hash=manifest['sha256_index'], vector_count=len(vectors), dimension=dimension)
        self._memory_indexes[index_id] = {'dimension': dimension, 'seed': 0, 'vectors': vectors, 'metadatas': metadatas, 'metadata': metadata, 'version_hash': manifest['sha256_index']}
        if self._telemetry_callback:
            self._telemetry_callback('faiss_manifest_verified', {'index_id': index_id, 'vector_count': len(vectors), 'digest': manifest['sha256_index'], 'embedder_id': manifest['embedder_id'], 'model_version': manifest['model_version']})
...
__all__ = ['LocalFAISSStore', 'IndexNotBuiltError', 'IndexMetadataError', 'ManifestIntegrityError', 'EmbedderMismatchError']
