...
class DeterministicReplayEngine:
    ...
    def __init__(self):
        ...
        self._synthetic_cases = [{'query': 'machine learning fundamentals', 'corpus': [{'id': 'doc1', 'text': 'ML basics'}, {'id': 'doc2', 'text': 'Deep learning'}, {'id': 'doc3', 'text': 'Neural networks'}, {'id': 'doc4', 'text': 'Optimization'}, {'id': 'doc5', 'text': 'Data preprocessing'}]}, {'query': 'neural network architectures', 'corpus': [{'id': 'doc6', 'text': 'CNN architectures'}, {'id': 'doc7', 'text': 'RNN architectures'}, {'id': 'doc8', 'text': 'Transformer models'}, {'id': 'doc9', 'text': 'Attention mechanisms'}, {'id': 'doc10', 'text': 'GAN architectures'}]}, {'query': 'optimization algorithms', 'corpus': [{'id': 'doc11', 'text': 'Gradient descent'}, {'id': 'doc12', 'text': 'Adam optimizer'}, {'id': 'doc13', 'text': 'SGD with momentum'}, {'id': 'doc14', 'text': 'Learning rate schedules'}, {'id': 'doc15', 'text': 'Adaptive methods'}]}, {'query': 'data preprocessing techniques', 'corpus': [{'id': 'doc16', 'text': 'Normalization'}, {'id': 'doc17', 'text': 'Standardization'}, {'id': 'doc18', 'text': 'Feature scaling'}, {'id': 'doc19', 'text': 'Missing value imputation'}, {'id': 'doc20', 'text': 'Data augmentation'}]}, {'query': 'model evaluation metrics', 'corpus': [{'id': 'doc21', 'text': 'Accuracy metrics'}, {'id': 'doc22', 'text': 'Precision and recall'}, {'id': 'doc23', 'text': 'F1 score'}, {'id': 'doc24', 'text': 'ROC curves'}, {'id': 'doc25', 'text': 'Cross-validation'}]}]
    ...
    def replay(self, *, base_profile: RetrievalProfile, candidate_profile: RetrievalProfile) -> ReplayResult:
        ...
        changed_cases = sum((1 for case_id in base_outputs if base_outputs[case_id] != candidate_outputs[case_id]))
        replay_digest = self._compute_replay_digest(base_profile=base_profile, candidate_profile=candidate_profile, base_outputs=base_outputs, candidate_outputs=candidate_outputs, changed_cases=changed_cases)
        replay_digest_check = self._compute_replay_digest(base_profile=base_profile, candidate_profile=candidate_profile, base_outputs=base_outputs, candidate_outputs=candidate_outputs, changed_cases=changed_cases)
        if replay_digest != replay_digest_check:
            raise ValueError(f'Determinism self-check failed: {replay_digest} != {replay_digest_check}')
        result = ReplayResult(case_count=len(self._synthetic_cases), base_outputs=base_outputs, candidate_outputs=candidate_outputs, changed_cases=changed_cases, replay_digest=replay_digest)
        result.emit_digest()
        return result
    ...
    def _compute_replay_digest(self, *, base_profile: RetrievalProfile, candidate_profile: RetrievalProfile, base_outputs: dict[str, list[str]], candidate_outputs: dict[str, list[str]], changed_cases: int) -> str:
        ...
        """
        data = {'base_profile': json.loads(base_profile.to_canonical_json()), 'candidate_profile': json.loads(candidate_profile.to_canonical_json()), 'base_outputs': {k: sorted(v) for k, v in sorted(base_outputs.items())}, 'candidate_outputs': {k: sorted(v) for k, v in sorted(candidate_outputs.items())}, 'changed_cases': changed_cases, 'case_count': len(self._synthetic_cases), 'replay_version': 'W11-v1.0'}
