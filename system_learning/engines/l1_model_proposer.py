...
class L1ModelProposer:
    ...
    def propose(self, snapshot: Any, metrics: Any, config: Any, now_utc: int, history: Any, cooldown: Any, sample: Any) -> L1ModelChangePackage | None:
        ...
        if new_temp == current_temp:
            return None
        return L1ModelChangePackage(surface_name='l1_model_temperature', parameter='temperature', old_value=current_temp, new_value=new_temp, justification=f'L1 confidence drift {confidence_drift:.3f} exceeds threshold {_CONFIDENCE_DRIFT_THRESHOLD}; {direction} temperature from {current_temp} to {new_temp}', snapshot_id=snapshot_id)
