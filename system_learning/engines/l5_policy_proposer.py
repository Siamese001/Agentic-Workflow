...
_FALSE_POSITIVE_THRESHOLD = 0.15  # Propose relaxation when FP rate > 15%
_FALSE_NEGATIVE_THRESHOLD = 0.10  # Propose tightening when FN rate > 10%
_MIN_OBSERVATIONS = 5
_MAX_DELTA = 0.05
_DEFAULT_DELTA = 0.02
...
class L5PolicyChangePackage:
    ...
    surface_name: str
    direction: str  # "relax" or "tighten"
    delta: float
    justification: str
    snapshot_id: str
...
class L5PolicyProposer:
    ...
    def propose(
        ...
        if fp_rate > _FALSE_POSITIVE_THRESHOLD:
            ...
            justification = (
                f"L5 false-positive rate {fp_rate:.3f} exceeds threshold "
                f"{_FALSE_POSITIVE_THRESHOLD}; proposing relaxation by {delta}"
            )
        elif fn_rate > _FALSE_NEGATIVE_THRESHOLD:
            direction = "tighten"
            delta = min(_DEFAULT_DELTA, _MAX_DELTA)
            justification = (
                f"L5 false-negative rate {fn_rate:.3f} exceeds threshold "
                f"{_FALSE_NEGATIVE_THRESHOLD}; proposing tightening by {delta}"
            )
        else:
            return None
...
def extract_l5_metrics_from_healing_actions(
    ...
    l5_actions = [
        ...
        if any(a.get("agent", "").startswith(pfx) for pfx in _L5_AGENT_PREFIXES)
    ]

    if not l5_actions:
        return {
            "l5_false_positive_rate": 0.0,
            "l5_false_negative_rate": 0.0,
            "l5_observation_count": 0,
        }

    total = len(l5_actions)
    # False positive: agent flagged a violation but healing was skipped/unnecessary
    false_positives = sum(
        1 for a in l5_actions
        if a.get("status") in ("skipped", "plan_only", "unnecessary")
    )
    # False negative: agent missed a violation (detected by a later re-scan)
    false_negatives = sum(
        1 for a in l5_actions
        if a.get("status") in ("missed", "false_negative")
    )
