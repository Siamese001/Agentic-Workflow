...
_LOW_RECALL_THRESHOLD = 0.60
_HIGH_NOISE_THRESHOLD = 0.40  # precision below this = too much noise
_TOP_K_MIN = 3
_TOP_K_MAX = 20
_SIMILARITY_CUTOFF_MIN = 0.30
...
class RAGParameterProposer:
    ...
    def propose(
        ...
        current_cutoff = config.get("similarity_cutoff", 0.70)

        # Low recall → lower similarity cutoff to retrieve more
        if recall < _LOW_RECALL_THRESHOLD:
            ...
            if new_cutoff != current_cutoff:
                return RAGChangePackage(
                    ...
                    justification=(
                        f"RAG recall {recall:.3f} < {_LOW_RECALL_THRESHOLD}; "
                        f"lowering cutoff from {current_cutoff} to {new_cutoff:.4f}"
                    ),
                    snapshot_id=snapshot_id,
                )

        # Low precision → raise similarity cutoff to reduce noise
        if precision < _HIGH_NOISE_THRESHOLD:
            ...
            if new_cutoff != current_cutoff:
                return RAGChangePackage(
                    ...
                    new_value=round(new_cutoff, 4),
                    justification=(
                        f"RAG precision {precision:.3f} < {_HIGH_NOISE_THRESHOLD}; "
                        f"raising cutoff from {current_cutoff} to {new_cutoff:.4f}"
                    ),
                    snapshot_id=snapshot_id,
                )
