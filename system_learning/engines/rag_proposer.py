...
class RAGChangePackage:
    ...
    def canonical_bytes(self) -> bytes:
        data = {'surface_name': self.surface_name, 'parameter': self.parameter, 'old_value': self.old_value, 'new_value': self.new_value, 'justification': self.justification, 'snapshot_id': self.snapshot_id}
...
class RAGParameterProposer:
    ...
    def propose(self, snapshot: Any, metrics: Any, config: Any, now_utc: int, history: Any, cooldown: Any, sample: Any) -> RAGChangePackage | None:
        ...
        if recall < _LOW_RECALL_THRESHOLD:
            ...
            if new_cutoff != current_cutoff:
                return RAGChangePackage(surface_name='rag_similarity_cutoff', parameter='similarity_cutoff', old_value=current_cutoff, new_value=round(new_cutoff, 4), justification=f'RAG recall {recall:.3f} < {_LOW_RECALL_THRESHOLD}; lowering cutoff from {current_cutoff} to {new_cutoff:.4f}', snapshot_id=snapshot_id)
        if precision < _HIGH_NOISE_THRESHOLD:
            new_cutoff = min(current_cutoff + _SIMILARITY_DELTA, _SIMILARITY_CUTOFF_MAX)
            if new_cutoff != current_cutoff:
                return RAGChangePackage(surface_name='rag_similarity_cutoff', parameter='similarity_cutoff', old_value=current_cutoff, new_value=round(new_cutoff, 4), justification=f'RAG precision {precision:.3f} < {_HIGH_NOISE_THRESHOLD}; raising cutoff from {current_cutoff} to {new_cutoff:.4f}', snapshot_id=snapshot_id)
        return None
