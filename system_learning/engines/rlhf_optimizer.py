...
class RLHFOptimizer(Protocol):
    ...
    def propose_from_dpo(self, *, dpo_batch_bytes: bytes, current_threshold_config_bytes: bytes, embedding_context_hash: str | None=None) -> ChangePackage:
...
class DefaultDeterministicRLHFOptimizer:
    ...
    def __init__(self, *, min_threshold: float=0.1, max_threshold: float=2.0, approve_relax_delta: float=0.1, reject_tighten_delta: float=-0.1):
    ...
    def propose_from_dpo(self, *, dpo_batch_bytes: bytes, current_threshold_config_bytes: bytes, embedding_context_hash: str | None=None) -> ChangePackage:
        ...
        try:
            ...
            return ChangePackage(source='rlhf_optimizer', target='threshold_config', changes=b'{}', confidence=0.0, reason=('malformed_dpo_batch',), timestamp_utc=0, embedding_context_hash=embedding_context_hash, authority_sensitivity='MEDIUM', target_surface='threshold_config')
        try:
            current_config = json.loads(current_threshold_config_bytes.decode('utf-8'))
        except (json.JSONDecodeError, UnicodeDecodeError):
            return ChangePackage(source='rlhf_optimizer', target='threshold_config', changes=b'{}', confidence=0.0, reason=('malformed_threshold_config',), timestamp_utc=0, authority_sensitivity='MEDIUM', target_surface='threshold_config')
        adjustments = {}
        reasons = []
        if 'pairs' in dpo_data:
            sorted_pairs = sorted(dpo_data['pairs'], key=lambda p: (p.get('score', 0.0), p.get('timestamp_utc', 0), p['example_id']['control_hash'], p['example_id']['candidate_hash']), reverse=True)
        ...
        return ChangePackage(source='rlhf_optimizer', target='threshold_config', changes=changes_bytes, confidence=confidence, reason=tuple(reasons) if reasons else ('no_adjustments',), timestamp_utc=0, embedding_context_hash=embedding_context_hash, authority_sensitivity='MEDIUM', target_surface='threshold_config')
