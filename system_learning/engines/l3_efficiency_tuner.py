...
_SLOW_TERRITORY_THRESHOLD_MS = 30_000  # 30 seconds
_SLOW_AGENT_THRESHOLD_MS = 10_000  # 10 seconds
...
class EfficiencyReport:
    ...
    def canonical_bytes(self) -> bytes:
        data = {
            ...
            "bottlenecks": [
                json.loads(b.canonical_bytes().decode("utf-8"))
...
class L3EfficiencyTuner:
    ...
    def analyze(
        ...
        *,
        ...
        # Check territory-level slowness
            ...
            if time_ms > self._slow_territory_ms:
                bottlenecks.append(
                    EfficiencyBottleneck(
                        ...
                        territory=territory,
                        recommendation=f"Territory '{territory}' took {time_ms:.0f}ms "
                        f"(threshold: {self._slow_territory_ms:.0f}ms). "
                        f"Consider parallelizing or reducing agent count.",
                    )
                )

        # Check agent-level slowness
                ...
                if time_ms > self._slow_agent_ms:
                    bottlenecks.append(
                        EfficiencyBottleneck(
                            ...
                            territory=territory,
                            recommendation=f"Agent '{agent_name}' in '{territory}' took "
                            f"{time_ms:.0f}ms (threshold: {self._slow_agent_ms:.0f}ms). "
                            f"Consider caching or optimizing scan scope.",
                        )
                    )
...
def extract_timings_from_runtime_state(
    ...
    # Extract from execution log if available
