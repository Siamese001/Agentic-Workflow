...
class EfficiencyBottleneck:
    ...
    def canonical_bytes(self) -> bytes:
        data = {'component': self.component, 'metric_name': self.metric_name, 'observed_value_ms': self.observed_value_ms, 'threshold_ms': self.threshold_ms, 'territory': self.territory, 'recommendation': self.recommendation}
...
class EfficiencyReport:
    ...
    def canonical_bytes(self) -> bytes:
        data = {'snapshot_id': self.snapshot_id, 'bottlenecks': [json.loads(b.canonical_bytes().decode('utf-8')) for b in self.bottlenecks], 'total_territories': self.total_territories, 'total_agents_executed': self.total_agents_executed, 'avg_territory_time_ms': self.avg_territory_time_ms}
        return json.dumps(data, separators=(',', ':'), sort_keys=True).encode('utf-8')
...
class L3EfficiencyTuner:
    ...
    def __init__(self, slow_territory_threshold_ms: float=_SLOW_TERRITORY_THRESHOLD_MS, slow_agent_threshold_ms: float=_SLOW_AGENT_THRESHOLD_MS) -> None:
        self._slow_territory_ms = slow_territory_threshold_ms
        self._slow_agent_ms = slow_agent_threshold_ms

    def analyze(self, *, snapshot_id: str, territory_timings: dict[str, float], agent_timings: dict[str, dict[str, float]]) -> EfficiencyReport:
        ...
        for territory, time_ms in sorted(territory_timings.items()):
            if time_ms > self._slow_territory_ms:
                bottlenecks.append(EfficiencyBottleneck(component='L3_orchestration', metric_name='territory_processing_time', observed_value_ms=time_ms, threshold_ms=self._slow_territory_ms, territory=territory, recommendation=f"Territory '{territory}' took {time_ms:.0f}ms (threshold: {self._slow_territory_ms:.0f}ms). Consider parallelizing or reducing agent count."))
        for territory, agents in sorted(agent_timings.items()):
            for agent_name, time_ms in sorted(agents.items()):
                ...
                if time_ms > self._slow_agent_ms:
                    bottlenecks.append(EfficiencyBottleneck(component=agent_name, metric_name='agent_execution_time', observed_value_ms=time_ms, threshold_ms=self._slow_agent_ms, territory=territory, recommendation=f"Agent '{agent_name}' in '{territory}' took {time_ms:.0f}ms (threshold: {self._slow_agent_ms:.0f}ms). Consider caching or optimizing scan scope."))
        total_territories = len(territory_timings)
        times = list(territory_timings.values())
        avg_time = sum(times) / len(times) if times else 0.0
        return EfficiencyReport(snapshot_id=snapshot_id, bottlenecks=tuple(bottlenecks), total_territories=total_territories, total_agents_executed=total_agents, avg_territory_time_ms=round(avg_time, 2))

def extract_timings_from_runtime_state(state: dict[str, Any]) -> tuple[dict[str, float], dict[str, dict[str, float]]]:
...
__all__ = ['L3EfficiencyTuner', 'EfficiencyReport', 'EfficiencyBottleneck', 'extract_timings_from_runtime_state']
