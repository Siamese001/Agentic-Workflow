        ...
        for fingerprint in sorted_fingerprints:
            for drift_event in sorted_drift_events:
                if self._should_correlate(fingerprint, drift_event.policy_id):
                    row = CorrelatedRow(fingerprint=fingerprint, policy_id=drift_event.policy_id, drift_type=drift_event.drift_type, severity=drift_event.severity)
                    rows.append(row)
        sorted_rows = sorted(rows, key=lambda r: (r.fingerprint, r.policy_id, r.drift_type))
        canonical_data = {'rows': [{'fingerprint': r.fingerprint, 'policy_id': r.policy_id, 'drift_type': r.drift_type, 'severity': r.severity} for r in sorted_rows]}
