...
from system_learning.engines.policy_recommendation_engine import PolicyRecommendation, PolicyRecommendationEngine
...
def _analyze_shadow_drift_and_write(profile_id: str, now_utc: int, l4_writer: L4StateWriter) -> DriftSummary | None:
    ...
    if not _shadow_telemetry_batch:
        return None
    drift_summary = _shadow_drift_analyzer.analyze_batch(shadow_records=_shadow_telemetry_batch, profile_id=profile_id, now_utc=now_utc)
    try:
        summary_json = drift_summary.to_canonical_json().encode('utf-8')
        l4_writer.write_l4c_shadow_drift(payload_bytes=summary_json, component_name='meta-learning', created_utc=now_utc)
...
def _generate_policy_recommendation_and_write(drift_summary: DriftSummary, active_profile: RetrievalProfile, now_utc: int, l4_writer: L4StateWriter) -> PolicyRecommendation | None:
    ...
    if drift_summary is None:
        return None
    recommendation = _policy_recommendation_engine.generate_recommendation(drift_summary=drift_summary, active_profile=active_profile, now_utc=now_utc)
    try:
        recommendation_json = recommendation.to_canonical_json().encode('utf-8')
        l4_writer.write_l4c_policy_recommendation(payload_bytes=recommendation_json, component_name='meta-learning', created_utc=now_utc)
    # guardian: allow-silent-swallow
    except Exception as _l4_err:
        logger.warning('[MetaLearning] L4C policy_recommendation write failed: %s', _l4_err)
    return recommendation

def _create_proposal_and_write(policy_recommendation: PolicyRecommendation, active_profile: RetrievalProfile, now_utc: int, l4_writer: L4StateWriter) -> RetrievalProfileProposal | None:
    ...
    if policy_recommendation is None:
        return None
    proposal = _proposal_manager.create_proposal(recommendation=policy_recommendation, active_profile=active_profile, now_utc=now_utc)
    try:
        proposal_json = proposal.to_canonical_json().encode('utf-8')
        l4_writer.write_l4c_retrieval_profile_proposal(payload_bytes=proposal_json, component_name='meta-learning', created_utc=now_utc)
    # guardian: allow-silent-swallow
    except Exception as _l4_err:
...
def _analyze_historical_patterns(deps: PipelineDependencies, aggregate_snapshot: Any, *, now_utc: int=0, detection_signal_bytes: bytes | None=None, drift_snapshot_bytes: bytes | None=None) -> Any:
    ...
    try:
        if hasattr(aggregate_snapshot, 'canonical_bytes'):
            ...
            return deps.pattern_analysis_engine.analyze(healing_snapshot_bytes=healing_snapshot_bytes, detection_signal_bytes=detection_signal_bytes, drift_snapshot_bytes=drift_snapshot_bytes, now_utc=now_utc)
        ...
        if hasattr(aggregate_snapshot, 'outcomes'):
            for outcome in aggregate_snapshot.outcomes:
                if hasattr(outcome, 'failure_signature'):
                    ...
                    meta = {'healer_name': getattr(outcome, 'healer_name', 'unknown'), 'failure_type': getattr(outcome, 'failure_type', 'unknown'), 'component': getattr(outcome, 'component', 'unknown')}
                    metadata.append(meta)
        if not historical_embeddings or len(historical_embeddings) < 10:
            return None
        # guardian: allow-magic-config
        pattern_summary = deps.pattern_analysis_engine.analyze(historical_embeddings=historical_embeddings, metadata=metadata, min_cluster_size=3)
...
def _retrieve_semantic_context(rca_report: Any, pattern_report: Any, now_utc: int) -> dict[str, Any]:
    ...
    if embedding_service.is_disabled():
        ...
        return {'embedding_enabled_at_time': False, 'embedding_replay_key': None, 'embedding_artifact_hash': None, 'embedding_topk_hashes': [], 'embedding_topk_scores_round6': [], 'retrieval_profile_id': retrieval_profile.profile_id, 'vector_source': 'disabled', 'wc_determinism_digest': _wc_dig, **shadow_telemetry}
    query_components = []
    ...
    if retrieval_profile.shadow_embedder_id is not None:
        ...
        cosine_sim = round(float(np.dot(query_vector, shadow_vector) / (np.linalg.norm(query_vector) * np.linalg.norm(shadow_vector))), 6)
        shadow_telemetry = {'shadow_embedder_id': retrieval_profile.shadow_embedder_id, 'primary_embedding_norm': primary_norm, 'shadow_embedding_norm': shadow_norm, 'primary_shadow_cosine': cosine_sim}
    ...
    try:
        ...
        if results is None:
            ...
            return {'embedding_enabled_at_time': True, 'embedding_replay_key': getattr(embedding_service, 'replay_key', None), 'embedding_artifact_hash': None, 'embedding_topk_hashes': [], 'embedding_topk_scores_round6': [], 'retrieval_profile_id': retrieval_profile.profile_id, 'vector_source': _vector_source, 'wc_determinism_digest': _wc_dig, **shadow_telemetry}
        ...
        _wc_dig = _wc_digest(failure_signature, _vector_source, retrieval_profile.profile_id, len(topk_hashes))
        return {'embedding_enabled_at_time': True, 'embedding_replay_key': getattr(embedding_service, 'replay_key', None), 'embedding_artifact_hash': artifact_hash, 'embedding_topk_hashes': topk_hashes, 'embedding_topk_scores_round6': topk_scores, 'retrieval_profile_id': retrieval_profile.profile_id, 'vector_source': _vector_source, 'wc_determinism_digest': _wc_dig, **shadow_telemetry}
    except (ImportError, AttributeError, ValueError) as e:
        print(f'Embedding retrieval failed: {e}')
        _wc_dig = _wc_digest('ERROR', 'error', retrieval_profile.profile_id, 0)
        return {'embedding_enabled_at_time': True, 'embedding_replay_key': None, 'embedding_artifact_hash': 'RETRIEVAL_FAILED', 'embedding_topk_hashes': [], 'embedding_topk_scores_round6': [], 'retrieval_profile_id': retrieval_profile.profile_id, 'vector_source': 'error', 'wc_determinism_digest': _wc_dig, **shadow_telemetry}

def run_pipeline(now_utc: int, window_start_utc: int, window_end_utc: int, cfg: PipelineConfig, deps: PipelineDependencies) -> tuple[Any, ...]:
    ...
    snapshot = create_snapshot(engine_version=cfg.engine_version, config_surface_version=cfg.config_surface_version, audit_window_start_utc=window_start_utc, audit_window_end_utc=window_end_utc, telemetry_bytes=b'placeholder', policy_config_bytes=b'placeholder', routing_config_bytes=b'placeholder', model_config_bytes=b'placeholder', semantic_clock_bytes=b'placeholder', semantic_clock=semantic_clock)
    from system_learning.engines.rca_engine import analyze_failures
    rca_report = analyze_failures(snapshot_id=snapshot.snapshot_id, audit_slice=audit_slice, window_start_utc=window_start_utc, window_end_utc=window_end_utc)
    if deps.failure_fingerprinter is not None and hasattr(rca_report, 'failure_events'):
        fingerprints = [deps.failure_fingerprinter.fingerprint(ev).fingerprint_hex for ev in rca_report.failure_events or []]
    ...
    if deps.risk_correlator is not None and hasattr(rca_report, 'fingerprints') and hasattr(snapshot, 'drift_events'):
        correlated_risk = deps.risk_correlator.build(rca_report.fingerprints or [], snapshot.drift_events or [])
    ...
    proposer_map = {'L0': deps.l0_proposer, 'RAG': deps.rag_proposer, 'L1': deps.l1_proposer, 'L5': deps.l5_proposer}
    ...
    for key in proposer_order:
        ...
        pkg = proposer.propose(snapshot=snapshot, metrics=None, config=current_configs, now_utc=now_utc, history=None, cooldown=cfg.cooldown_policy, sample=cfg.sample_policy)
        if pkg is not None:
    ...
    if deps.resource_predictor_bytes is not None:
        try:
            ...
            resource_proposal = ChangePackage(source='phase9_resource_predictor', target='resource_envelope', changes=deps.resource_predictor_bytes, confidence=prediction_data.get('confidence', 0.5), reason=tuple(prediction_data.get('reasons', [])), timestamp_utc=now_utc)
    ...
    if deps.rollback_refinement_decision_bytes is not None:
        try:
            ...
            from system_learning.types.proposal_types import ChangePackage
            rollback_proposal = ChangePackage(source='phase9_rollback_refiner', target='rollback_strategy', changes=deps.rollback_refinement_decision_bytes, confidence=0.8, reason=tuple(decision_data.get('reasons', [])), timestamp_utc=now_utc)
    ...
    if deps.dpo_batch_bytes is not None and deps.rlhf_optimizer is not None:
        try:
            ...
            current_threshold_config_bytes = _json_dpo.dumps(current_configs, separators=(',', ':'), sort_keys=True).encode('utf-8')
            dpo_proposal = deps.rlhf_optimizer.propose_from_dpo(dpo_batch_bytes=deps.dpo_batch_bytes, current_threshold_config_bytes=current_threshold_config_bytes)
    ...
    for pkg in proposals:
        ...
        if hasattr(deps.config_provider, 'get_last_update_utc') and hasattr(cfg.cooldown_policy, 'min_seconds_between_updates'):
            last_update_utc = deps.config_provider.get_last_update_utc(surface_name)
            if last_update_utc is not None:
                assert_cooldown_ok(last_update_utc=last_update_utc, now_utc=now_utc, cooldown_policy=cfg.cooldown_policy)
        else:
            last_update_utc = None
        if hasattr(cfg.sample_policy, 'min_observations'):
            _audit_text = audit_slice.decode('utf-8', errors='replace') if isinstance(audit_slice, (bytes, bytearray)) else str(audit_slice)
        ...
        if hasattr(deps.config_provider, 'get_param_history') and hasattr(cfg.oscillation_policy, 'window'):
            ...
            if len(param_history) > 0:
                freeze_decision = compute_freeze_decision(values=param_history, last_update_utc=last_update_utc or 0, now_utc=now_utc, policy=cfg.oscillation_policy)
                if freeze_decision.should_freeze:
                    raise ValidationError(f'Oscillation detected for {surface_name}: freeze until {freeze_decision.freeze_until_utc}')
    ...
    if deps.arbitration_engine is not None and deps.arbitration_policy is not None and validated_proposals:
        candidates = [ArbitrationCandidate(id=getattr(p, 'proposal_id', str(i)), score=getattr(p, 'score', 0.0), cost=getattr(p, 'cost', 1.0), kind=getattr(p, 'kind', 'generic'), payload=p.to_dict() if hasattr(p, 'to_dict') else {}) for i, p in enumerate(validated_proposals)]
        decision = deps.arbitration_engine.arbitrate(candidates, deps.arbitration_policy)
        winner_ids = set(decision.winner_ids)
        validated_proposals = [p for i, p in enumerate(validated_proposals) if getattr(p, 'proposal_id', str(i)) in winner_ids]
    intake_record = None
    if deps.healing_outcome_intake_adapter is not None:
        _window_records = deps.healing_outcome_intake_adapter.get_recent_records(window_start_utc, window_end_utc)
        if _window_records:
            ...
            for _rec in _window_records:
                for _s in _rec.snapshot:
                    for _ in range(_s.success_count):
                        _window_aggregator.ingest(_WHE(healer_id=_s.healer_id, tier=_s.tier, failure_type=_s.failure_type, success=True, timestamp_utc=now_utc))
                    for _ in range(_s.failure_count):
                        _window_aggregator.ingest(_WHE(healer_id=_s.healer_id, tier=_s.tier, failure_type=_s.failure_type, success=False, timestamp_utc=now_utc))
            intake_record = deps.healing_outcome_intake_adapter.build_record(aggregator=_window_aggregator, created_utc=now_utc, source='meta-learning-pipeline-window')
            deps.healing_outcome_intake_adapter.persist_record(intake_record)
        else:
            intake_record = None
    if deps.healing_config_optimizer is not None and intake_record is not None and hasattr(intake_record, 'snapshot'):
        aggregate_snapshot = deps.healing_config_optimizer.create_snapshot_from_intake(intake_record, created_utc=now_utc)
        if deps.l4_state_writer is not None:
            try:
                payload_bytes = aggregate_snapshot.canonical_bytes()
                deps.l4_state_writer.write_l4b_healing_snapshot(payload_bytes=payload_bytes, component_name='meta-learning', created_utc=now_utc)
    ...
    pattern_report = _analyze_historical_patterns(deps, _8_5_aggregate_snapshot, now_utc=now_utc, detection_signal_bytes=_detection_signal_bytes_86, drift_snapshot_bytes=_drift_snapshot_bytes_86)
    embedding_metadata = _retrieve_semantic_context(rca_report=rca_report, pattern_report=pattern_report, now_utc=now_utc)
    if _8_5_aggregate_snapshot is not None:
        from system_learning.engines.retrieval_profile_manager import get_active_retrieval_profile
        active_profile = get_active_retrieval_profile(now_utc)
        drift_summary = _analyze_shadow_drift_and_write(profile_id=active_profile.profile_id, now_utc=now_utc, l4_writer=deps.l4_state_writer)
        if drift_summary is not None:
            drift_summary.emit_digest()
        policy_recommendation = _generate_policy_recommendation_and_write(drift_summary=drift_summary, active_profile=active_profile, now_utc=now_utc, l4_writer=deps.l4_state_writer)
        if policy_recommendation is not None:
            policy_recommendation.emit_digest()
        profile_proposal = _create_proposal_and_write(policy_recommendation=policy_recommendation, active_profile=active_profile, now_utc=now_utc, l4_writer=deps.l4_state_writer)
        if profile_proposal is not None:
            profile_proposal.emit_digest()
        if deps.healing_config_optimizer is not None:
            if hasattr(deps.healing_config_optimizer, 'propose_threshold_adjustments_with_patterns_and_embeddings'):
                threshold_proposal = deps.healing_config_optimizer.propose_threshold_adjustments_with_patterns_and_embeddings(_8_5_aggregate_snapshot, pattern_report, embedding_metadata)
            elif hasattr(deps.healing_config_optimizer, 'propose_threshold_adjustments_with_patterns'):
                threshold_proposal = deps.healing_config_optimizer.propose_threshold_adjustments_with_patterns(_8_5_aggregate_snapshot, pattern_report)
            else:
                threshold_proposal = deps.healing_config_optimizer.propose_threshold_adjustments(_8_5_aggregate_snapshot)
        else:
            threshold_proposal = None
        if threshold_proposal is not None and embedding_metadata:
            _artifact_hash = embedding_metadata.get('embedding_artifact_hash') or embedding_metadata.get('content_hash')
        ...
        if threshold_proposal is not None and hasattr(threshold_proposal, 'adjustments') and threshold_proposal.adjustments:
    ...
    if _vs_present and (not _ag_present):
        raise PipelineError('partial injection: approval_gate required when version_store is present; both must be injected together when proposal_only=False')
    if _ag_present and (not _vs_present):
        raise PipelineError('partial injection: version_store required when approval_gate is present; both must be injected together when proposal_only=False')
    if not _vs_present:
