...
class HealingConfidenceScorer:
    ...
    def __init__(self):
        ...
        # Outcome base scores (higher is better)
    ...
    def score(self, attempts: Sequence[HealingAttempt]) -> HealingConfidenceReport:
        ...
        # Sort attempts by attempt_id for determinism
    ...
    def _calculate_confidence(self, attempt: HealingAttempt) -> float:
        ...
        # Apply severity penalty (monotone: higher severity = lower confidence)
        severity_penalty = min(attempt.severity * 0.1, 0.5)  # Max 0.5 penalty

        # Apply cost penalty (monotone: higher cost = lower confidence)
        cost_penalty = min(attempt.cost * 0.05, 0.3)  # Max 0.3 penalty

        # Calculate final confidence with monotonic constraints
        confidence = base_score - severity_penalty - cost_penalty

        # Apply monotonic guards
        # SUCCESS attempts should never be lower than PARTIAL attempts
        if attempt.outcome == "SUCCESS":
            min_confidence = self._outcome_scores["PARTIAL"] - 0.1
            confidence = max(confidence, min_confidence)

        # FAIL attempts should never be higher than PARTIAL attempts
