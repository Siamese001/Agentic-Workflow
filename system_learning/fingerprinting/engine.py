...
class FailureFingerprinter:
    ...
    def _normalize_event(self, event: FailureEvent) -> FailureEvent:
        ...
        return FailureEvent(exc_type=normalized_exc_type, error_code=normalized_error_code, component=normalized_component, symbols=normalized_symbols, metadata=normalized_metadata)
    ...
    def _normalize_metadata(self, metadata: dict[str, Any]) -> dict[str, str]:
        ...
        if not metadata:
            return {}
        allowlist = {'message', 'code', 'status', 'severity', 'category', 'retry_count', 'timeout', 'version', 'phase'}
        normalized = {}
