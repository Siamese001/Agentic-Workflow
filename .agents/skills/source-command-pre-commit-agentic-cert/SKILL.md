---
name: "source-command-pre-commit-agentic-cert"
description: "|"
---

# source-command-pre-commit-agentic-cert

Use this skill when the user asks to run the migrated source command `pre-commit-agentic-cert`.
...
Migrated from source command `pre-commit-agentic-cert` into a Codex skill. Invoke it as `$source-command-pre-commit-agentic-cert` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
