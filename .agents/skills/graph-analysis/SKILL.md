...
For ALL dependency analysis: `mcp__adg_sqlite__adg_nodes_by_file` → `mcp__adg_sqlite__adg_edge_fanin` / `mcp__adg_sqlite__adg_edge_fanout`.
...
```
Step 1: nodes = mcp__adg_sqlite__adg_nodes_by_file(file_path="path/to/file.py")
...
Step 3: Run fan-in on EACH node:
         - mcp__adg_sqlite__adg_edge_fanin(tgt_id=<module_node_id>, relation_type="imports")
           → catches file-level imports (from X import Y, import X)
         - mcp__adg_sqlite__adg_edge_fanin(tgt_id=<symbol_node_id>, relation_type="imports")
...
Review unsupported Claude skill fields manually: `enforcement_layer`, `enforcement_timing`, `enforcement_type`, `metadata`.
