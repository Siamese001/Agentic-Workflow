---
name: "source-command-agent-deletion-gate"
description: "Authorization gate before deleting any *Agent.py file - invoke when deleting agents"
---

# source-command-agent-deletion-gate

Use this skill when the user asks to run the migrated source command `agent-deletion-gate`.
...
Migrated from source command `agent-deletion-gate` into a Codex skill. Invoke it as `$source-command-agent-deletion-gate` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
