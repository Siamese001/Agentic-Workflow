---
name: "source-command-adg-redis-refresh"
description: "Regenerate ADG and reload Redis hot cache - run after any code refactoring or structural change"
---

# source-command-adg-redis-refresh

Use this skill when the user asks to run the migrated source command `adg-redis-refresh`.
...
Migrated from source command `adg-redis-refresh` into a Codex skill. Invoke it as `$source-command-adg-redis-refresh` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
