---
name: "source-command-mcp-failure-rca"
...
# source-command-mcp-failure-rca

Use this skill when the user asks to run the migrated source command `mcp-failure-rca`.
...
> **Tool-naming note:** In Claude Code, MCP tools are named `mcp__<server>__<tool>` (e.g. `mcp__redis__redis_health`) — stable, derived from the server key in `.mcp.json`. Use the server names above as the stable identifiers.
...
Then reconnect the MCP server: run `/mcp` to reconnect (or restart the Claude Code session). Re-run `adg_health` — if ok, return to original prompt.
...
```
Reconnect the ADG MCP server: run `/mcp` (or restart the Claude Code session).
...
- `True` → Redis is up, MCP config is broken → check `.mcp.json` redis entry
...
| Any npx-based MCP hangs (filesystem, memory, deepwiki, brave) | Same root cause — `npx` vs `npx.cmd` on Windows | Reconnect via `/mcp` |
...
**Why**: stdio transport fragility on Windows, zombie node.exe processes, no reliable timeout, oversized opaque tool surface, architectural mismatch with Claude Code's native reasoning.
...
**C) Reconnect MCP:** run `/mcp` to reconnect (or restart the Claude Code session).
...
Migrated from source command `mcp-failure-rca` into a Codex skill. Invoke it as `$source-command-mcp-failure-rca` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
