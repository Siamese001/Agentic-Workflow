---
name: "source-command-timeout-progress-enforcement"
description: "Enforce timeout and progress reporting requirements for all queries"
---

# source-command-timeout-progress-enforcement

Use this skill when the user asks to run the migrated source command `timeout-progress-enforcement`.
...
Migrated from source command `timeout-progress-enforcement` into a Codex skill. Invoke it as `$source-command-timeout-progress-enforcement` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
