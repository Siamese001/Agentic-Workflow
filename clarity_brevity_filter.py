...
class ClarityBrevityFilter:
    ...
    def _simplify_jargon(self, text: str) -> Tuple[str, List[str]]:
        ...
        for jargon, simple in self.jargon_map.items():
            ...
            if re.search(pattern, filtered, re.IGNORECASE):
                filtered = re.sub(pattern, simple, filtered, flags=re.IGNORECASE)
    ...
    def _improve_sentence_structure(self, text: str) -> Tuple[str, List[str]]:
        ...
        for sentence in sentences:
            ...
            if word_count > self.max_sentence_length and sentence.endswith(('.', '!', '?')):
                ...
                split_points = [' but ', ' and ', ' however, ', ' therefore, ', ' meanwhile, ']
                
                for split_point in split_points:
                    if split_point in sentence.lower():
                        parts = re.split(split_point, sentence, flags=re.IGNORECASE)
                        if len(parts) == 2:
                            improved_sentences.extend([parts[0].strip() + '.', parts[1].strip() + '.'])
                            changes.append(f"Split long sentence ({word_count} words)")
                            break
                else:
    ...
    def _calculate_scores(self, original: str, filtered: str) -> Tuple[float, float, int, float]:
        ...
        if original_words > 0:
            ...
            brevity_score = min(1.0, word_reduction / (original_words * 0.3))  # 30% reduction = perfect score
        else:
            word_reduction = 0
            brevity_score = 1.0
        ...
        readability_improvement = max(0, original_readability - filtered_readability)
        
        # Score based on how close to target reading level
        if filtered_readability <= self.target_reading_level:
            clarity_score = 1.0
        else:
            clarity_score = max(0, 1 - (filtered_readability - self.target_reading_level) / 10)
    ...
    def filter_content(
        ...
        if logger:
            ...
            logger.info(f"   Readability improvement: {readability_improvement:.1f} grade levels")
