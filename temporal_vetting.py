...
def vet_lead_optimal_time(lead_timezone: str, current_send_time_utc: str, tools: Dict[str, Any], logger: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        conversion_result_str = convert_time(source_timezone, current_send_time_utc, lead_timezone)
        
        # Parse the ISO format result to extract HH:MM
        converted_time = datetime.fromisoformat(conversion_result_str.replace('Z', '+00:00'))
        ...
        if logger:
            logger.info(f"Lead Local Time: {lead_local_time}. Decision: {decision}.")

    except Exception as e:
        if logger:
            logger.error(f"Time conversion/vetting failed: {e}. Defaulting to DELAY for safety.")
    ...
    try:
        ...
        if logger:
            logger.info(f"Audit: Temporal Vetting - TZ={lead_timezone}, LocalTime={lead_local_time}, Decision={decision}")
    except:
        if logger:
...
if __name__ == "__main__":
    ...
    print("=== Temporal Vetting Test Results ===")
    for tz, utc_time in test_cases:
        result = vet_lead_optimal_time(tz, utc_time)
        print(f"\nTimezone: {tz}")
        print(f"UTC Time: {utc_time}")
        print(f"Local Time: {result['lead_local_time']}")
