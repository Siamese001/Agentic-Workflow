...
class MiscLiquidationScanner:
    ...
    EXISTING_CATEGORIES = {
        "audit": {
            "keywords": ["audit", "drift", "variance", "compliance", "ssot", "violation", "integrity"],
            "headers": ["# Audit", "## Violations", "## Drift", "## Compliance"],
            "json_keys": ["violations", "drift_metrics", "compliance_score"]
        },
        "assessments": {
            "keywords": ["assessment", "analysis", "gap", "architecture", "strategic", "recommendation"],
    ...
    def generate_mapping_table(self) -> str:
        ...
        lines = [
            ...
            f"**Target Directory:** `docs/reports/misc/`",
        ...
        if "unclassified" in self.classifications:
            ...
            lines.append(f"### ⚠️ Unclassified Files")
            lines.append("")
            lines.append(f"**File Count:** {len(unclassified)}")
            lines.append("")
            lines.append("**Recommendation:** Manual review or create `docs/reports/misc_archive/` for truly uncategorizable files")
            lines.append("")
            lines.append("**Sample Files:**")
        ...
        for category in ["legacy", "governance", "prototypes"]:
            if category in self.classifications and len(self.classifications[category]) > 0:
                purpose = self.NEW_CATEGORIES[category]["keywords"][0].title() + " artifacts and documentation"
...
def main():
    ...
    # Print summary
    for category, files in sorted(scanner.classifications.items(), key=lambda x: len(x[1]), reverse=True):
