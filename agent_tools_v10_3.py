# File: agent_tools_v10_3.py
# Version: 10.3 (Instructional Injection)
#
# Description:
# v10.3: DESTRUCTIVE OVERWRITE based on Instructional_Injection_Enhanced_v4.md
# - Eradicated Hardcoded Prompts: All 15+ hardcoded prompt templates
#   have been removed from this file.
# - Centralized Prompts: All tools now call `self.prompt_manager.get_template()`
#   to retrieve prompts (with few-shot examples) from `core_v10_3.py`.
# - Mandated Schema Validation: All tools now use `self.validator.validate()`
#   and import/return specific Pydantic models (e.g., `DraftStrategyOutput`)
#   from `core_v10_3.py`. All manual JSON parsing is GONE.
# - Injected Style & Voice: `DraftingRefinerTool` now accepts a `style_guide`.
...
# v10.3: Import from new stacks (BaseTool's definition will be in v10.3)
from agent_stacks_v10_3 import BaseTool, BiasDetectorAgent # Import from stacks

logger = logging.getLogger("agent_tools_v10_3")

# ============================================================================
# ROW 7: DRAFTING STACK (ReAct Conductor with REAL Tools)
# ============================================================================

# --- Drafting Tools (v10.3: Validated, Prompts Centralized) ---
...
class DraftingStrategistTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        self.log_info(f"Tool: Reviewing draft strategy (v10.3)...")
...
class DraftingRedTeamTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        self.log_info(f"Tool: Red teaming draft (v10.3)...")
...
class DraftingRefinerTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        self.log_info(f"Tool: Refining section (v10.3)...")
...
class DraftingMetricsTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        self.log_info(f"Tool: Adding metrics (v10.3)...")
...
# --- QA Tools (v10.3: Validated, Prompts Centralized) ---
...
class QABaseValidatorTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        self.log_info(f"Tool: Running {self.tool_name} (v10.3)...")
...
class QAClaimValidatorTool(QABaseValidatorTool):
    ...
    def format_prompt(self, template: str, tool_input: Dict[str, Any]) -> str:
        ...
        return template.format(master_resume=master_resume, draft_text=draft_text)
...
class QAToneValidatorTool(QABaseValidatorTool):
    ...
    def format_prompt(self, template: str, tool_input: Dict[str, Any]) -> str:
        ...
        return template.format(
            required_tone=json.dumps(tool_input.get('strategy', {}).get('tone')),
...
class QANarrativeThreadTool(QABaseValidatorTool):
    ...
    def format_prompt(self, template: str, tool_input: Dict[str, Any]) -> str:
        ...
        return template.format(draft_text=draft_text)
...
class QAAdversarialReviewerTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        self.log_info(f"Tool: Running Adversarial Review (v10.3)...")
        ...
        prompt_template = self.prompt_manager.get_template(self.tool_name)
        prompt = prompt_template.format(draft_text=draft_text)
...
class QABiasDetectorTool(BaseTool):
    ...
    async def run_async(self, tool_input: Dict[str, Any], workflow_id: str) -> Dict[str, Any]:
        # This tool imports BiasDetectorAgent from agent_stacks_v10_3.py
