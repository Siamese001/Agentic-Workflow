    ...
    try:
        ...
        for var, allowed in design_spec["allowed_values"].items():
            if var in figma_variables and figma_variables[var] not in allowed:
                violations.append(f"Invalid value for {var}: {figma_variables[var]}")
        
        # Check for extra variables (warning only)
        extra_vars = set(figma_variables.keys()) - set(design_spec["required_variables"]) - set(design_spec["allowed_values"].keys())
        if extra_vars:
...
def test_time_aware_design_updates():
    ...
    try:
        ...
        if design_update["timestamp"]:
            ...
            # Parse ISO time
            try:
                parsed_time = datetime.fromisoformat(current_time.replace('Z', '+00:00'))
...
def main():
    ...
    # Run tests
    results.append(("Figma Variable Extraction", test_figma_variable_extraction()))
    results.append(("Design Context Retrieval", test_design_context_retrieval()))
    ...
    results.append(("Design Compliance Validation", test_design_compliance_validation()))
    results.append(("Time-Aware Design Updates", test_time_aware_design_updates()))
