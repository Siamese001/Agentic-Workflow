# File: agent_swarm_v9_8.py
# Overwrites: agent_swarm_v9_7.py
...
class BiasDetectorAgent(BaseAgent):
    """P0: Detects bias in resume content."""
    ...
    def run(self, content: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_info(f"Bias score: {content_data.get('bias_score', 0.0)}")
            return content_data
...
class PIISanitizerAgent(BaseAgent):
    """P0: Sanitizes PII from resume data."""
    ...
    def run(self, resume_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            content_data = response.get("content", {})
            if content_data.get("pii_found", False):
                self.log_warning(f"PII sanitized. Redactions: {content_data.get('redaction_count', 0)}")
                try:
                    sanitized = json.loads(content_data.get("sanitized_content", "{}"))
                    self.blackboard["pii_map"] = content_data.get("pii_map", {})
                    return sanitized
        ...
        try:
            ...
            if content_data.get("ambiguity_detected", False):
                ...
                if confidence > CONFIG.agent_stacks.ambiguity_confidence_threshold:
                    ...
                    # Create HIL request
                    from core_v9_8 import HILRequest, AmbiguityType
                    request = HILRequest(
...
def update_cost_tracking(state: MainGraphState) -> Dict:
    ...
    from core_v9_8 import COST_TRACKER
...
def register_default_tools():
    ...
    from core_v9_8 import ToolDefinition, TOOL_REGISTRY
