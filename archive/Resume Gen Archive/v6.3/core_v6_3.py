...
# Load Configuration
CONFIG_PATH = PROJECT_ROOT / "master_config_v6_3.json"

class Configuration:
    """
    Central configuration class.
    Reads all settings from master_config_v6_3.json.
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if not self.config_path.exists():
            core_logger.critical(f"FATAL: master_config_v6_3.json not found at {self.config_path}.")
            raise FileNotFoundError(f"master_config_v6_3.json not found at {self.config_path}.")
        try:
...
class ResumeSection:
    """
    Dynamically provides resume sections from the master_config_v6_3.json.
...
class JDEnforcementRule:
    """
    Dynamically provides JD enforcement rules from master_config_v6_3.json.
...
def _get_prompt_template(key: str) -> str:
    ...
    if not hasattr(PROMPT_TEMPLATES, key):
        ...
        raise KeyError(f"Prompt template key '{key}' not found in master_config_v6_3.json!")
