# File: run_batch_v8_5.py
# Overwrites: run_batch_v8_0.py
# Version: 8.5 (Cleanup)
#
# v8.5 (Cleanup) CHANGES:
# - Imports from v8_5 modules.
...
from main_v8_5 import setup_logging, load_job_input
from core_v8_5 import CONFIG, CircuitBreakerOpenError

# v8.5: Import the graph builder and checkpointer
from agent_swarm_v8_5 import get_graph_app
from langgraph.checkpoint.redis import RedisSaver

# v8.5: Import the meta-learner
try:
    from run_learning_v8_5 import run_meta_learning
    META_LEARNER_AVAILABLE = True
except ImportError:
    META_LEARNER_AVAILABLE = False


logger = logging.getLogger("batch_runner_v8_5")
try:
...
SUMMARY_FILE = "batch_summary_v8_5.csv"

# --- v8.5: Graph Setup (will be re-initialized in each process) ---
checkpointer = None
app = None
...
def get_compiled_graph():
    ...
    v8.5: Passes 'enable_hil=False' to the graph builder.
    """
...
def process_single_job(job_path: str) -> Dict[str, Any]:
    ...
    logger.info(f"--- Starting v8.5 job: {os.path.basename(job_path)} ---")
    ...
    try:
        ...
        logger.info(f"--- Finished v8.5 job: {company} - {title} ---")
...
def run_batch():
    ...
    if not job_files:
        logger.info("v8.5 Batch process starting. No jobs found in queue.")
        return

    logger.info(f"v8.5 Batch process starting. Found {len(job_files)} jobs.")
    ...
    logger.info(f"v8.5 Batch process complete.")
    ...
    if CONFIG.meta_loop_config.enable_meta_learning:
        ...
        if META_LEARNER_AVAILABLE:
            ...
            logger.warning("Meta-learning is enabled, but 'run_learning_v8_5' could not be imported.")
    else:
