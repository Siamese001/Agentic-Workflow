...
# - FIXED: Changed config file name to master_config_v10_6.json.
...
def setup_logging(config: ConfigV10_6, debug_mode: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.StreamHandler(sys.stdout) # v10.6: Log to stdout for streaming
        ]
    )
    
    # Configure metrics logger
    metrics_log_path = config.logging_config.metrics_log_path
    metrics_logger = logging.getLogger("core_v10_6.MetricsCollector")
...
async def run_workflow_async(
    ...
    try:
        ...
        # v10.6 (Fix #9): Use astream_events for real-time streaming
        current_node = ""
        print("\n--- Workflow Stream (v10.6) ---", flush=True)
        ...
        async for event in app.astream_events(state_dict, run_config, version="v1"):
            ...
            # v10.6 (Fix #9): Handle real-time token streaming
        ...
        # v10.6 (Fix #30): Check for constitutional failure
        ...
        logger.info(f"--- Workflow Metrics Summary (v10.6) ---")
        ...
        logger.info(f"===== v10.6 Workflow Complete =====")

def main():
    """Main CLI entry point"""
    parser = argparse.ArgumentParser(description="Resume Generation Engine v10.6")
