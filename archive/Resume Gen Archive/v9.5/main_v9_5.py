...
# Version info
__version__ = "9.5.0-privacy-provenance"
...
def setup_logging(debug_mode: bool):
    ...
    """
    try:
from core_v9_5 import CONFIG as LOG_CONFIG
    except ImportError:
print("CRITICAL: core_v9_5.py not found. Logging setup failed.", file=sys.stderr)
...
def print_summary(results: Dict[str, Any], workflow_id: str, start_time: datetime):
    ...
    print("\n" + "=" * 80)
print(f"WORKFLOW EXECUTION SUMMARY (v9.5)")
    print("=" * 80)
...
def main():
    """
    Main entry point for V9.5 Workflow.
    """
script_path = os.path.relpath(__file__) if "__file__" in globals() else "main_v9_5.py"
...
description=f'V9.5 LangGraph Workflow (Version: {__version__})',
...
Examples:
  ...
  python {script_path} --debug                   # Enable debug mode
    ...
    parser.add_argument(
        '--version',
        action='version',
version=f'V9.5 LangGraph Workflow - Version {__version__}'
    )
    ...
    try:
        ...
        logger.info(f"🚀 Starting v9.5 (Privacy & Provenance) Workflow. ID: {workflow_id}")
        ...
        if args.output_dir:
            ...
            if final_draft:
                draft_file = output_path / f"{base_filename}_final_draft_v9.5.txt"
