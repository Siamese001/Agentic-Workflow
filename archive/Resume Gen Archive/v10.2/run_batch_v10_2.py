# File: run_batch_v10_2.py
# Version: 10.2 (ChromaDB Semantic RAG)
#
# v10.2 MAJOR CHANGES:
# - Imports and versioning updated to v10.2
# - Added chromadb import and client initialization
# - Injected chromadb_client into WorkflowContext for batch jobs
# - Added ChromaDB cleanup step to process_single_job_async
...
try:
    ...
    logging.getLogger("batch_runner_v10_2").warning("Meta-learning module (run_learning_v10_2.py) not found. Batch will run without meta-learning trigger.")

logger = logging.getLogger("batch_runner_v10_2")
...
SUMMARY_FILE = "batch_summary_v10_2.csv"
...
async def run_batch_async():
    ...
    if not job_files:
        logger.info("v10.2 Batch process starting. No jobs found.")
        return
    
    logger.info(f"===== v10.2 Async Batch Process Starting =====")
    ...
    logger.info("Initialized shared ChromaDB client for semantic RAG")
    
    # 3. Initialize Core Services (will be shared by all jobs)
    ...
    # 4. Initialize Shared WorkflowContext
    context = WorkflowContext(
        config=CONFIG,
    ...
    logger.info("Initialized shared WorkflowContext with v10.2 dependencies")
    ...
    async def process_with_semaphore(job_file):
        ...
        async with semaphore:
            ...
            # All jobs share Redis/ChromaDB, but not the context object itself
            job_context = WorkflowContext(
                config=CONFIG,
                redis_client=redis_client,
                chromadb_client=chromadb_client, # v10.2: Inject shared client
                cache_manager=cache_manager,
                cost_tracker=cost_tracker,
                feedback_reader=feedback_reader,
                rules_loader=rules_loader
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"BATCH PROCESSING COMPLETE (v10.2)")
    ...
    if META_LEARNER_AVAILABLE and CONFIG.meta_loop_config.enable_meta_learning:
        logger.info("Triggering meta-learning loop (v10.2)...")
        try:
            # v10.2: Pass config to meta-learner
    ...
    logger.info("v10.2 Batch process complete.")
