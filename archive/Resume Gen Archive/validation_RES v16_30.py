...
# FIX: Import text_utils, DuplicateDetector, and calculate_signal_score from utils
from utils_RES import text_utils, TextUtils, DuplicateDetector, calculate_signal_score

# Load data required by AppTrackerQAValidator
try:
    from utils import _load_json_data
...
class ValidationContext:
    ...
    def _calculate_unify_ibm_percent(self):
        ...
        details = {'unify_ibm_percent': percent, 'min': self.constraints.UNIFY_IBM_COMBINED_PERCENT_MIN, 'max': self.constraints.UNIFY_IBM_COMBINED_PERCENT_MAX}
    ...
    def _calculate_unify_ibm_ratio(self):
        ...
        details = {'unify_ibm_ratio': ratio, 'min': self.constraints.UNIFY_IBM_RATIO_MIN, 'max': self.constraints.UNIFY_IBM_RATIO_MAX}
...
class PreFlightValidator:
    ...
    def RULES_CONFIG(self):
        return [
            ...
            self._mk_range("H3_K1_WORD_COUNT", ValidationSeverity.CRITICAL, "word_count", lambda ctx: ctx.k1_word_count_details, "K.1 Exec Summary words", 'min', 'max', 'word_count'),
            {
            ...
            self._mk_method("H3_GLOBAL_BULLET_WORD_COUNT_HIGH", ValidationSeverity.HIGH, "word_count",
            ...
            
            {
                ...
                "error_message": lambda ctx: f"K.2(Unify)+K.3(IBM): {ctx.unify_ibm_percent:.1f}% of total (target: {ctx.get_details_for_rule('H5_GLOBAL_WORD_DISTRIBUTION_UNIFY_IBM')['min']}-{ctx.get_details_for_rule('H5_GLOBAL_WORD_DISTRIBUTION_UNIFY_IBM')['max']}%)"
            },
            {
                ...
                "error_message": lambda ctx: f"K.2(Unify)/K.3(IBM) ratio: {ctx.unify_ibm_ratio:.2f} (target: {ctx.get_details_for_rule('H5_GLOBAL_UNIFY_IBM_RATIO')['min']}-{ctx.get_details_for_rule('H5_GLOBAL_UNIFY_IBM_RATIO')['max']})"
            },
            {
            ...
            self._mk_method("H3_K11_COVER_LETTER_FULL_STRUCTURE", ValidationSeverity.HIGH, "structure", "_validate_cover_letter_full_structure", "K.11 Cover letter missing expected structure components (Date, Recipient, Salutation, Body Para 1/2/3, Closing, Signature)."),
            ...
            self._mk_method("H5_CONTENT_NO_PLACEHOLDERS", ValidationSeverity.HIGH, "content", "_validate_no_placeholders", lambda ctx: f"Found placeholder text in content: {ctx.get_details_for_rule('H5_CONTENT_NO_PLACEHOLDERS').get('placeholders', 'N/A')}"),
            self._mk_method("H3_CONTENT_NO_FORBIDDEN_VERBS", ValidationSeverity.HIGH, "content", "_validate_forbidden_verbs", lambda ctx: f"Forbidden verbs found in generated content: {ctx.get_details_for_rule('H3_CONTENT_NO_FORBIDDEN_VERBS').get('violations', 'N/A')}"),
            self._mk_method("H3_CONTENT_NO_INTRO_PHRASES", ValidationSeverity.HIGH, "content", "_validate_no_intro_phrases", lambda ctx: f"Banned introductory phrases found: {ctx.get_details_for_rule('H3_CONTENT_NO_INTRO_PHRASES').get('violations', 'N/A')}"),
    ...
    def _register_rules(self):
        ...
        for config in all_rules_config:
            ...
            # Create a closure for the error message lambda to capture the rule_id
            def create_error_message_lambda(template, rule_id_for_cache):
                def error_lambda(ctx: ValidationContext):
                    try:
                        # Ensure the details for this rule are calculated and cached
                        if rule_id_for_cache not in ctx._cache:
                             # Try to trigger the calculation
                            ...
                            # For complex rules, the validator method populates the cache
                            # If it's still not there, getter(ctx) will trigger it
    ...
    def _get_bullet_word_counts(self, context: ValidationContext) -> List[Tuple[int, str, ResumeSection]]:
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            if not isinstance(bullets, list): 
                logging.warning(f"Expected list for {section_key} bullets. Skipping."); 
                continue
    ...
    def _validate_bullet_word_count_CRITICAL(self, context: ValidationContext) -> bool:
        violations = []
        failed_sections = set()
        all_counts = self._get_bullet_word_counts(context)
        ...
        if violations:
            context._cache["H3_GLOBAL_BULLET_WORD_COUNT_CRITICAL"] = { "violations": ", ".join(violations), "failed_sections": failed_sections }
            return False
        return True
    ...
    def _validate_bullet_word_count_HIGH(self, context: ValidationContext) -> bool:
        ...
        if violations:
            context._cache["H3_GLOBAL_BULLET_WORD_COUNT_HIGH"] = { "violations": ", ".join(violations), "failed_sections": failed_sections }
            return False
        return True
    ...
    def _validate_bullet_word_count_MEDIUM(self, context: ValidationContext) -> bool:
        ...
        if violations:
            context._cache["H3_GLOBAL_BULLET_WORD_COUNT_MEDIUM"] = { "violations": ", ".join(violations), "failed_sections": failed_sections }
            return False
        return True
    ...
    def _validate_bullet_word_count_LOW(self, context: ValidationContext) -> bool:
        ...
        if violations:
            context._cache["H3_GLOBAL_BULLET_WORD_COUNT_LOW"] = { "violations": ", ".join(violations), "failed_sections": failed_sections }
            return False
        return True
    # --- END TIERED METHODS ---
