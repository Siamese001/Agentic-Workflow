"""
Pytest suite for Resume_Generation_v15_68.py intermediate HOP output JSON persistence validation.

Tests v15_68 features for automatic JSON serialization of HOP outputs:
1. Validates _save_hop_output_json method creates correct directory structure
2. Ensures all major HOPs save their primary output objects as JSON files
3. Confirms JSON files use correct naming convention
4. Validates robust serialization of dataclasses, enums, and ImmutableStagingBuffer
5. Ensures workflow doesn't halt if JSON saving fails

Also includes inherited tests from v15_67 for narrative generation fix,
and v15_66 tests for MasterResumeIndex integration throughout RAG pipeline.

Run with: pytest test_resume_generation_v15_68.py -v
"""
...
from Resume_Generation_v15_68 import (
