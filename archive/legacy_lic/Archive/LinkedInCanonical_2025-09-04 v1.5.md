...
5) Reasoning — MESSAGE LENGTH & ABBREVIATION RULES (NEW)
...
- **Abbreviation Policy (MANDATORY):** apply the following abbreviations where appropriate to meet the strict message length:
  - Generative → Gen
  - Engineering → Eng
  - Vice President → VP
  - Machine Learning → ML
...
- The system may propose additional widely-recognized abbreviations only when strictly required to meet the character constraint; any unusual/unclear abbreviation triggers BLOCK.
- Compose one paragraph (WhyCompany + WhyRole + FitLine) aligned to selected imperatives.
- **Final Validation — Strict Character Check:** before rendering, compute final character count (count Unicode codepoints). If count <290 or count >310 → immediate BLOCK with:
  {
    "status": "error",
    "failed_checks": ["Short (NEW) message length violation"],
    "details": {"required_length_range": "290–310", "actual_length": <computed>}
  }
- **Audit Logging for Short (NEW):**
  - Log final character count.
  - Log abbreviation mappings used (original → abbreviation).
  - Log final validation timestamp (ISO8601).
...
- Where `<N>` is the audited final character count (must be between 290 and 310 inclusive).
...
- **Short (NEW) message audit (mandatory):**
  - final_character_count; abbreviation_mappings {original:abbrev}; final_validation_timestamp.
...
===============================================================================
END OF LINKEDIN OUTREACH - CANONICAL v1.5
===============================================================================
