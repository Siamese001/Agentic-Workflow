...
- GPT-5 runtime compliance overlay enforced. Dash policy aligned to AI FILTER vNext3; phone number is auto-whitelisted.
...
- Short (NEW) audit: final_linkedin_char_count; abbreviation_mappings; length_violation_attempts; regeneration_attempts; final_validation_timestamp; character_counter_normalized_linkedin.
...
-------------------------------------------------------------------------------
IMPLEMENTATION APPENDIX — LINKEDIN CHARACTER COUNTER v2.1 (NEW)
-------------------------------------------------------------------------------
Purpose: deterministic LinkedIn-compatible normalization + Unicode code-point counting.
...
Counting:
  - len(normalized_string) measured as Unicode codepoints.
...
def count_linkedin_chars(s):
    s_norm = normalize_for_linkedin(s)
    return len(s_norm)
...
Integration tests:
  TST-120: full Short (NEW) render end-to-end: prompts → char counter → QA titles -> success or explicit block JSON
...
Never-recur lines:
  ...
  • "CharCounter v2.1 deterministically normalizes and counts LinkedIn-compliant characters; mismatch blocks and suggests safe edits."

===============================================================================
END OF LINKEDIN OUTREACH - CANONICAL v2.3.1
===============================================================================
