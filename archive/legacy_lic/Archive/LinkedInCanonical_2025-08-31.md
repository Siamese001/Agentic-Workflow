## PATCH LOG — Non-Destructive Patches (2025-08-31)
- change_type: non_destructive_patch
- reason: Enforce pre-run gate, role-detector mismatch block, signature enforcement, recruiter vs executive resume-line rules
- sections_affected: [TASK - START PROMPT, OUTPUT - EXECUTIVE (EXISTING), OUTPUT - RECRUITER (EXISTING), AUDIT AND QA]
- timestamp: 2025-09-01T00:53:29.671273
...
**Role-Detector (automatic):** infer category from title/profile; BLOCK on mismatch until confirmed.
### IF NEW
...
**Resume Line Requirement:** The exact sentence "My resume is attached for your convenience." must appear once (BLOCK if missing/altered).
**Signature Enforcement:** Same as Executive path (BLOCK on deviation).
**Dash Policy:** BAN em dashes and "--"; only '-' and '–' for ranges.
...
**Resume Line:** Do NOT include "My resume is attached for your convenience." (BLOCK if present)
**Dash Policy:** BAN em dashes and "--"; only '-' and '–' for ranges.
**Percent Symbol:** Normalize 'percent' → '%'.
...
**Pre-run Audit Fields (MANDATORY):** log is_existing, message_type, contact_category (user), inferred_category (role-detector), role_detector_match (true/false), contact_url, timestamp. BLOCK if any missing.
**Tests:** gate present, mismatch flag, signature block, resume-line presence/absence, dash policy, percent normalization.
