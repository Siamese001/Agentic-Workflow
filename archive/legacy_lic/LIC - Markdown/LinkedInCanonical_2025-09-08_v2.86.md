Understood. Full, zero-loss LIC v2.86 below — single continuous fenced block.

````markdown
# LinkedInCanonical — 2025-09-09 v2.86 (FULL, ZLO-Consolidated)

Lineage: v2.73 -> v2.75 -> v2.80 -> v2.81 -> v2.82 -> v2.83 -> v2.84 -> v2.84a -> v2.85 -> **v2.86**

v2.86 is a versioned zero-loss overwrite that **fully decouples mandatory profile-derived RAG enrichment from JD availability** for Senior TA. Profile/About RAG is now enforced unconditionally; JD-driven semantic matching is a separate step with its own blocker. All v2.85 hardenings and prior restorations remain intact.
...
- Enforce formatting (URL, Message Type, Subject, Body, Signature), structure (Capability Frame, Transition, Insights, Bridge, Bullets, CTA), archetype rules, sector framing, evidence, redundancy controls, and all v2.86 RAG and mapping checks.
...
- **Profile/About RAG is mandatory and runs independently of JD availability for Senior TA.** JD-only insights are forbidden.
...
**Senior TA sequencing clarifier (v2.86):**
...
- Step C: If JD is unavailable, emit **BLOCK_JD_SNIPPET_MISSING** while preserving the separate PASS/FAIL state of profile-RAG checks.
...
#### LinkedIn QA Grid — Full message types (v2.86)
| Test | Result |
|---|---|
...
| **Mandatory profile-derived RAG executed independently of JD availability (Senior TA + Exec where applicable)** | ✅/❌ |
...
#### Senior TA QA Grid — Hardened and Decoupled (v2.86)
| Test | Result |
|---|---|
...
#### App Tracker QA Grid (unchanged from v2.85)
| Test | Result |
|---|---|
...
**New v2.85 global blockers (retained):**
...
**New v2.86 blocker:**
- **BLOCK_JD_SNIPPET_MISSING**   // JD not supplied; JD semantic matching cannot be executed. Profile-RAG enforcement remains independent and must still PASS/FAIL on its own.
...
### MPV-5 EXECUTIVE (Hardened, RAG-Anchored, Credential-Balance) \[v2.86]

\[Identical to v2.85; present above mechanics unchanged.]

### MPV-5 C-SUITE (use as-is, DRQ assumed complete)  \[v2.86]

\[Identical to v2.85; present above mechanics unchanged.]

### MPV-5 SENIOR TA (Hardened, Role-Aligned, Profile-RAG) \[v2.86]

\[Identical to v2.85 structure with explicit note: Profile/About RAG step executes even when JD is missing; JD semantic matching is a separate gated step that may trigger **BLOCK\_JD\_SNIPPET\_MISSING**.]

---

## CHANGELOG (v2.86)

1. **Mandatory RAG Enrichment Decoupled** — Senior TA profile/About RAG is required regardless of JD availability; immediate BLOCK if profile-RAG missing or incomplete.
2. **Semantic Matching Clarified** — JD semantic matching is explicitly separated from the profile-RAG step; added dedicated **BLOCK\_JD\_SNIPPET\_MISSING** when JD is unavailable.

---

## DIFF CHECKLIST (v2.85 -> v2.86)

| Invariant or Change                                    | Status  |
| ------------------------------------------------------ | ------- |
| Profile-RAG for Senior TA enforced regardless of JD    | ADDED ✅ |
| Immediate BLOCK on missing/incomplete profile-RAG      | ADDED ✅ |
| JD semantic matching separated and gated               | ADDED ✅ |
| New blocker: BLOCK\_JD\_SNIPPET\_MISSING               | ADDED ✅ |
| All v2.85 QA rows, prompts, rubrics, blockers retained | PASS ✅  |

```

Sources for continuity with your prior baselines. :contentReference[oaicite:0]{index=0} :contentReference[oaicite:1]{index=1}
```
