# LinkedInCanonical — 2025-09-08 v2.9.2 (Full Overwrite from v2.9.1 + ND Patch: About Supplement + CTA Time-Bound + Insight Spacing + C-Suite Deep RAG)
Generated: 2025-09-08T01:41:33

# LinkedInCanonical — 2025-09-08 v2.9 (Full Overwrite from v2.8 + ND Patch: Sector Framing Enforcement at Insight Authoring)
Generated: 2025-09-08T23:59:00-04:00

# LinkedInCanonical — 2025-09-08 v2.8 (Full Overwrite from v2.7 + ND Patch: Dash Scanner, QA Order, RAG Map + Bullet→Company→Resume QA Table)
Generated: 2025-09-08T23:59:00-04:00

# LinkedInCanonical — 2025-09-08 v2.7 (Full Overwrite from v2.6 + ND Patch: Cross-Archetype QA & CTA Clarification)
Generated: 2025-09-08T01:10:00-04:00
...
# LinkedInCanonical — 2025-09-06 v2.8 (ND Patch: AI Filter v8 Update)
...
6) **QA & Evidence (order fixed):**  
   ...
   6.2) **AI Filter v8 (Canonical — GPT-5 Zero-Loss Overwrite) [Global MSC Index #8]
- Renderer must BLOCK if AI Filter v8 (13 checks) is not fully rendered green.
**  
...
**AI Filter v8 (Canonical — GPT-5 Zero-Loss Overwrite) [Global MSC Index #8]
- Renderer must BLOCK if AI Filter v8 (13 checks) is not fully rendered green.
** (sample)
| Criterion                       | Result |
|---------------------------------|--------|
...
reason:
  ...
  - Ensure strict linkage to AI Filter v8 (Canonical — GPT-5 Zero-Loss Overwrite) [Global MSC Index #8]
- Renderer must BLOCK if AI Filter v8 (13 checks) is not fully rendered green.
...
- Background compliance layers (AI Filter v8 (Canonical — GPT-5 Zero-Loss Overwrite) [Global MSC Index #8]
- Renderer must BLOCK if AI Filter v8 (13 checks) is not fully rendered green.
...
- When evidence is limited, label as candidate-provided with date and lower the claim strength per AI Filter v8.
...
Renderer must:
  ...
  5) Run QA: structure → overlap guards → AI Filter v8 (13 checks) → LinkedIn QA Grid → RAG QA Table → Evidence Pack → Scoring Grid (all 10/10).
...
===============================================================================
NON-DESTRUCTIVE PATCH — LinkedInCanonical v2.5 (Decision Tree Shorthand Inputs)
Applied: 2025-09-07T18:25:00-04:00 (America/New_York)
===============================================================================

## Adjustment — Entrance Gate and Operator Prompts
- Prompts now accept shorthand entries (E/N, Y/N, S/M) alongside canonical NEW/EXISTING, YES/NO, SINGLE/MULTIPLE.

## Adjustment — Block Conditions
- New block: BLOCK-SHORTHAND-INVALID if input is not in {NEW, EXISTING, N, E, SINGLE, MULTIPLE, S, M, YES, NO, Y, N}.
- Fallback auto-normalization: map single-letter input to long-form before routing.

## Adjustment — Implementation Appendix (Validation Logic)
- Input normalization map added:
...
- Telemetry additions: operator_input_raw, operator_input_normalized, normalization_applied (true/false).

===============================================================================
END OF PATCH — Integrated into v2.6 Full
===============================================================================



===============================================================================
NON-DESTRUCTIVE PATCH — LinkedInCanonical v2.6 (Cross-Archetype QA & CTA Clarification)
Target: LinkedInCanonical_2025-09-07_v2.6_full.md
change_type: non_destructive_patch
timestamp: 2025-09-08T01:10:00-04:00 (America/New_York)
===============================================================================

sections_affected:
  - Full Message — Executive
  - Full Message — Recruiter / Senior TA / Contact
  - QA Blocks (LinkedIn QA Grid)
  - CTA Enforcement
  - RAG Enforcement
  - Implementation Appendix (Renderer Logic & QA Validation)

reason:
  - Address RCA-identified gaps: subject prefix bug, missing informal intro, sector omission, weak attribution, weak RAG mapping, generic CTA.
  - Extend remediation checks across **Executive, Recruiter, Senior TA, and Contact** message types where relevant.
  - Enforce explicit company reference (e.g., “executive leadership roles at Wells Fargo”) in Executive CTAs.

-------------------------------------------------------------------------------
## Adjustment — Subject Line (All Archetypes)
- Renderer must strip any literal “Subject:” prefix before rendering.
- QA Grid row: “Subject line visible without prefix or fencing” → ✅/❌
- Block code: `BLOCK-SUBJECT-PREFIX`.

-------------------------------------------------------------------------------
## Adjustment — Greeting & Intro (All Archetypes)
- Greeting must be informal and personalized:  
  - **Hi [Name],** followed by exactly one blank line.  
  - Opener must mention recipient **role/title** and **company priorities** drawn from RAG.  
- QA Grid rows:  
  - Informal greeting present with recipient’s name → ✅/❌  
  - Intro references recipient’s role/title and company priorities → ✅/❌  
- Block codes:  
  - `BLOCK-GREETING-FORCED` — too stiff.  
  - `BLOCK-NAME-OMITTED` — recipient name missing.  
  - `BLOCK-ROLE-OMITTED` — recipient role/title missing.

-------------------------------------------------------------------------------
## Adjustment — Insights (Executive / Contact / Recruiter / TA)
- Must include sector framing (“in banking,” “in financial services,” or relevant vertical).  
- Each insight must tie explicitly to a company objective (via RAG).  
- QA Grid rows:  
  - Insights reference sector → ✅/❌  
  - Insights mapped to company objective → ✅/❌  
- Block codes:  
  - `BLOCK-SECTOR-OMITTED`  
  - `BLOCK-RAG-MAP-MISSING`.

-------------------------------------------------------------------------------
## Adjustment — Bullets (All Archetypes except Short)
- Bullets must explicitly attribute leadership to candidate (“I led…”, “I drove…”).  
- Each bullet must map to a company strategic objective (RAG).  
- QA Grid rows:  
  - Outcomes attributed to candidate leadership → ✅/❌  
  - Bullets mapped to company objectives (claim→objective mapping table required) → ✅/❌  
- Block codes:  
  - `BLOCK-OUTCOME-ATTRIBUTION`  
  - `BLOCK-RAG-MAP-MISSING`.

-------------------------------------------------------------------------------
## Adjustment — CTA Enforcement
- **Executive Archetype:**  
  Must include explicit company anchor, e.g.:  
  “I would value a conversation exploring executive leadership roles **at [Company]** where my AI governance and transformation experience could directly support [Company]’s strategic objectives.”  
  Block if “at [Company]” is omitted.  
  - Block code: `BLOCK-CTA-COMPANY-EXEC`.  

- **Recruiter / Senior TA Archetypes:**  
  CTA must reference alignment with **roles you’re recruiting at [Company]**.  
  - Block code: `BLOCK-CTA-COMPANY-REC`.  

- **Contact Archetype:**  
  CTA must reference alignment with **the contact’s role or objectives at [Company]**.  
  - Block code: `BLOCK-CTA-COMPANY-CONTACT`.

-------------------------------------------------------------------------------
## Adjustment — RAG Enforcement (All Archetypes)
- Renderer must generate claim→RAG map for all insights/bullets.  
- Format: `claim | company_objective | rag_source_pointer | resume_source_pointer`  
- Evidence Pack must contain at least one external (market/industry) source and one internal (resume/track record) source.  
- Missing any mapping or balance triggers `BLOCK-EVIDENCE-MAP` or `BLOCK-EVIDENCE-BALANCE`.




===============================================================================
NON DESTRUCTIVE PATCH — LinkedInCanonical v2.7 (Dash Scanner, QA Order, RAG Map + RAG→Company→Resume QA Table)
Target: LinkedInCanonical_2025-09-08_v2.7_full.md
change_type: non_destructive_patch
timestamp: 2025-09-08T23:59:00-04:00 (America/New_York)
===============================================================================

sections_affected:
  * QA Blocks (LinkedIn QA Grid, Message-Specific RAG QA Table)
  * Visible Output Contract
  * Block Codes and Telemetry

reason:
  * Surface a dedicated table mapping bullets → company strategic objectives → resume outcomes.
  * Ensure explicit, auditable ties between candidate metrics and Wells Fargo’s (or any company’s) stated priorities.
  * Preserve ordering: LinkedIn QA Grid → RAG Mapping Table → Evidence Pack → Scoring Grid → AI Filter v8 (last).
  * Strengthen audit overlay with mapping_present flag.

-------------------------------------------------------------------------------
QA BLOCKS — APPEND
-------------------------------------------------------------------------------
After the LinkedIn QA Grid, insert a **Message-Specific Bullet→Company→Resume Mapping Table** with the following canonical structure:

| Bullet | Company Objective (Strategic Priority) | Resume Outcome (project files) |

Renderer must ensure:
- Each bullet line in the message body appears in this table.
- Each row explicitly links to a company objective (from RAG) and a resume proof (with metric).
- Table rows must preserve the candidate attribution (“I led…”, “I drove…”).
- Table must appear before Evidence Pack and Scoring Grid.

Example (Executive archetype, Wells Fargo):
| Bullet                                        | Company Objective (Wells Fargo strategic priority)                                                                   | Resume Outcome (project files)                                                                                  |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Accelerated AI solution deployment by **35%** | Supports Wells Fargo’s strategic priority of **rapid digital transformation aligned with regulatory frameworks**.    | Documented on your resume as accelerating deployment cycles by **35%** via standardized cloud frameworks.       |
| Improved governance maturity by **40%**       | Aligns directly with Wells Fargo’s emphasis on **strengthening regulatory compliance and data governance maturity**. | Specifically cited on your resume as increasing governance maturity by **40%**, enhancing compliance readiness. |
| Reduced ML model latency by **30%**           | Aligns with Wells Fargo’s strategic priority of **enhancing digital responsiveness and operational speed**.          | Verified resume outcome: reducing ML latency by **30%**, enabling faster, data-driven decision-making.          |

-------------------------------------------------------------------------------
VISIBLE OUTPUT CONTRACT — ORDER (UPDATED)
-------------------------------------------------------------------------------
After message body:
  1) LinkedIn QA Grid
  2) Bullet→Company→Resume Mapping Table (new, required)
  3) Evidence Pack
  4) Scoring Grid
  5) AI Filter v8 Canonical (13 checks) — must always be last

-------------------------------------------------------------------------------
BLOCK CODES — ADDITIONS
-------------------------------------------------------------------------------
- BLOCK-RAG-MAP-MISSING: any bullet omitted from mapping table.
- BLOCK-RAG-MAP-INCOMPLETE: mapping table missing company objective or resume source column entry.
- BLOCK-QA-ORDER-VIOLATION: AI Filter v8 not rendered last.

-------------------------------------------------------------------------------
TELEMETRY — ADDITIONS
-------------------------------------------------------------------------------
Audit overlay must log:
- mapping_table_present = true/false
- mapping_row_count = integer
- mapping_complete = true/false (all bullets covered with company objective + resume source)

===============================================================================
END NON DESTRUCTIVE PATCH — LinkedInCanonical v2.7 (Dash Scanner, QA Order, RAG Map + RAG→Company→Resume QA Table)
===============================================================================
...
===============================================================================
NON-DESTRUCTIVE PATCH — LinkedInCanonical v2.8  
Title: Sector Framing Enforcement at Insight Authoring (Executive/Contact/TA/Recruiter)  
Target: LinkedInCanonical_2025-09-08_v2.8_full.md  
change_type: non_destructive_patch  
timestamp: 2025-09-07T23:59:00-04:00 (America/New_York)  
actor_id: amitayer1@gmail.com
===============================================================================

## RATIONALE
LIC v2.6 introduced a binding requirement that **Insights must include sector framing** such as “in financial services” or “in insurance” and must map to a company objective:contentReference[oaicite:0]{index=0}. A recent run produced Insights without sector language, which should have triggered `BLOCK-SECTOR-OMITTED`. This patch wires sector enforcement into the **authoring stage** and adds a pre-compose gate so messages cannot render without sector framing. AI Filter v8 sequencing remains last in order and continues to govern final release:contentReference[oaicite:1]{index=1}.

---

## SECTIONS MODIFIED

### 1) Full Message — Body Standard (Insights authoring rule):contentReference[oaicite:2]{index=2}
**Add under Executive, Contact, Recruiter, Senior TA Insights bullet:**

- **Sector Framing (mandatory):** Each Insights section must explicitly reference the target vertical. Accepted forms:  
  - “in financial services,” “in insurance,” “across insurance and financial services,” or a more specific line-of-business term (for example, “in P&C insurance underwriting”).  
- **Authoring guard:** The renderer must inject a sector placeholder if missing **before** composing numbered insight lines.  
  - Executive template prefix: “Two strategic insights I have gained in financial services and insurance are:”  
  - Contact template prefix: “Two tactical observations from my work in financial services and insurance are:”  
  - Recruiter or Senior TA template prefix: “Two themes from my background in financial services and insurance are:”

Failure to detect sector text prior to numbering triggers an immediate block. See Block Conditions update below.

---

### 2) QA Blocks — LinkedIn QA Grid (pre-compose gating):contentReference[oaicite:3]{index=3}
**Amend and promote the following row to pre-compose gate:**

| Test                                       | Result |
|--------------------------------------------|--------|
| **Insights reference sector (pre-compose)** | ✅/❌ |

- If ❌ at pre-compose, **halt** body generation and return `BLOCK-SECTOR-OMITTED` with a one-line fix hint:  
  “Insert explicit sector framing, for example: ‘in financial services and insurance’.”

**Note:** This row remains in the final QA Grid for visibility, but enforcement occurs **before** message body is rendered.

---

### 3) Block Conditions (additions):contentReference[oaicite:4]{index=4}
- `BLOCK-SECTOR-OMITTED` — Sector text absent at Insights authoring stage.  
  **Fix hint:** “Start the Insights section with ‘in financial services and insurance’ or an equivalent sector phrase.”

---

### 4) Visible Output Contract — Sequencing reminder:contentReference[oaicite:5]{index=5}:contentReference[oaicite:6]{index=6}
Retain the existing downstream order and keep AI Filter v8 last:

1) LinkedIn QA Grid  
2) Bullet→Company→Resume Mapping Table  
3) Evidence Pack  
4) Scoring Grid  
5) **AI Filter v8 Canonical (13 checks)**

This patch only adds a **pre-compose** sector gate prior to rendering the message body. Final AI Filter checks remain the terminal release guard:contentReference[oaicite:7]{index=7}.

---

### 5) Implementation Appendix — Renderer & Validation Notes:contentReference[oaicite:8]{index=8}
- **Renderer step:**  
  1) Detect sector token set: {financial services, insurance, P&C, life and annuities, reinsurance}.  
  2) If none found, inject the template prefix per archetype and re-check.  
  3) Only then compose Insight lines “1.” and “2.”  
- **Regex acceptance example:**  
  `\b(in\s+(financial\s+services|insurance|property\s*&\s*casualty|life\s*&\s*annuities|reinsurance))\b`  
- **Mapping coupling:** The RAG mapping table must continue to link each Insight to a company objective row. If mapping is missing, use existing `BLOCK-RAG-MAP-MISSING`.

---

### 6) Telemetry & Audit Overlay (add fields):contentReference[oaicite:9]{index=9}
Append fields to the run audit overlay:

- `sector_detected_precompose` = true|false  
- `sector_inserted_auto` = true|false  
- `sector_phrase_used` = string

Values must be set before body render. If `sector_detected_precompose=false` and auto-insert fails, emit `BLOCK-SECTOR-OMITTED`.

---

## COMPATIBILITY NOTES
- This patch aligns with LIC v2.6 sector requirements and prevents late-stage misses by **blocking before composition**:contentReference[oaicite:10]{index=10}.  
- AI Filter v8 remains the final release gate and is unaffected in ordering:contentReference[oaicite:11]{index=11}.

===============================================================================
END NON-DESTRUCTIVE PATCH — LinkedInCanonical v2.8 (Sector Framing Enforcement)
===============================================================================



===============================================================================
NON-DESTRUCTIVE PATCH — LinkedInCanonical v2.9.1 
(About Supplement + CTA Time-Bound + Insight Spacing + C-Suite Deep RAG)
Target: LinkedInCanonical_2025-09-08_v2.9.1_full.md
change_type: non_destructive_patch
timestamp: 2025-09-08T21:15:00-04:00 (America/New_York)
actor_id: amitayer1@gmail.com
===============================================================================

## 1. About Section — Supplementation with RAG
### Adjustment — Full Message Body Standard
- When operator provides LinkedIn “About” text in Entrance Gate step (3), renderer must:
  • Parse and store About section.  
  • Auto-derive 1–2 **keywords/themes** (skills, industries, differentiators).  
  • Supplement Capability Frame and/or Insights with at least one element from the About text *plus* RAG-retrieved JD/sector/company priorities.  
- If About section omitted or empty: renderer continues using RAG + Resume only.  
- QA Grid row added:  
  | Test | Result |  
  |------|--------|  
  | About section themes integrated into Capability Frame/Insights | ✅/❌ |

### Block Conditions
- `BLOCK-ABOUT-SUPPLEMENT-MISSING`: About provided but no content surfaced in Capability Frame or Insights.  

---

## 2. CTA Enforcement — Crisp, Actionable, Time-Bound
### Adjustment — CTA Enforcement (All Archetypes)
- Every CTA must be:
  • Single sentence.  
  • Explicit, role-aligned, and include a **concrete next step**.  
  • Include **time-bound phrasing** customized by archetype.  

📌 LIC CTA Defaults (Actionable + Time-Bound)

| Archetype | NEW (InMail) CTA | EXISTING (Reply/Follow-up) CTA |
|-----------|------------------|--------------------------------|
| Executive (C-Suite / VP+) | “I would welcome a short discussion on how my experience could contribute to UBS’s AI Engagement & Adoption priorities, and learn more about STAAT’s current focus areas **over the next week**.” | “Would you be open to scheduling a short call **next week** to explore how my AI leadership background could support UBS’s executive priorities in STAAT?” |
| Senior TA | “I would value a conversation **this week** about executive leadership roles you are currently recruiting for at UBS, where my background in AI and transformation could be a strong fit. My resume is attached for your convenience.” | “Could we schedule a brief call **over the next week** to discuss the executive leadership openings you are managing at UBS, and how my background may align? My resume is attached for your convenience.” |
| Recruiter | “I would value a discussion **this week** about the senior-level opportunities you are currently managing at UBS that may align with my experience. My resume is attached for your convenience.” | “Would you be open to a short call **next week** to review the senior-level roles you’re recruiting for at UBS and how my background may fit? My resume is attached for your convenience.” |
| Contact (Business-side, Non-Exec) | “I would welcome a conversation **this week** to explore how my background in AI adoption and governance could support your team’s priorities at UBS.” | “Could we arrange a brief call **next week** to discuss how my background in AI adoption and governance could directly support your team’s current initiatives?” |

### QA Grid Rows Added
| Test | Result |
|------|--------|
| CTA includes explicit next step (call/discussion/connection) | ✅/❌ |
| CTA includes time-bound phrasing aligned to archetype | ✅/❌ |

### Block Conditions
- `BLOCK-CTA-NEXTSTEP-MISSING` — CTA vague, no explicit action.  
- `BLOCK-CTA-TIMEBOUND-MISSING` — CTA present but no timeframe phrase.  
- `BLOCK-CTA-TIMEBOUND-MISALIGNED` — Wrong timeframe for archetype.  

---

## 3. Insight Spacing — Blank Row Requirement
### Adjustment — Full Message Body Standard
- All message types (Executive, Senior TA, Recruiter, Contact, NEW, EXISTING) must include **exactly one blank line between the intro sentence (transition into Insights) and the first numbered insight (1.)**.  
- QA Grid row added:  
  | Test | Result |  
  |------|--------|  
  | Blank line present between intro sentence and Insight 1 | ✅/❌ |

### Block Conditions
- `BLOCK-INSIGHT-SPACING-MISSING`: No blank line detected between intro and numbered insights.  
- `BLOCK-INSIGHT-SPACING-EXCESS`: More than one blank line detected.  

---

## 4. C-Suite Deep RAG Enforcement
### Adjustment — Executive (C-Suite) Path
- When composing Insights for C-Suite archetype (CEO, CEO-1), the renderer must perform **deep research query RAG**:  
  • Must aggregate from ≥3 independent high-confidence sources (e.g., JD, company filings, press releases, analyst commentary).  
  • Shallow single-hook or surface-level RAG is prohibited.  
  • Insights must reflect synthesized sector + company-level themes derived from this deep RAG.  
- QA Grid row added:  
  | Test | Result |  
  |------|--------|  
  | C-Suite Insights sourced from deep research query RAG (≥3 sources) | ✅/❌ |

### Block Conditions
- `BLOCK-RAG-DEPTH-MISSING`: Executive message attempted without deep RAG.  

---

## 5. Audit Overlay — Telemetry Fields
- `about_supplement_used` = true/false  
- `about_keywords_extracted` = [array]  
- `cta_nextstep_present` = true/false  
- `cta_timebound_present` = true/false  
- `cta_timebound_phrase` = string  
- `cta_timebound_aligned` = true/false  
- `insight_spacing_valid` = true/false  
- `rag_depth_sufficient` = true/false (C-Suite only)  

===============================================================================
END OF NON-DESTRUCTIVE PATCH — LinkedInCanonical v2.9.1 
(About + CTA + Insight Spacing + C-Suite Deep RAG)
===============================================================================
