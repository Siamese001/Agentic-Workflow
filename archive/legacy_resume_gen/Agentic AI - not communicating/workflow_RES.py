...
try:
    ...
    logging.warning("Warning: chromadb package not installed - Librarian agent disabled")

# Check for Code Interpreter availability
CODE_INTERPERTER_AVAILABLE = True  # Assuming CodeInterpreterTool is always available from utils_RES
...
try:
    # --- FIX: Use _load_json_config ---
    ARTIST_SPECS_DATA = _load_json_config(str(DATA_DIR / "artist_specs.json"), "Artist Specs")
...
try:
    # --- FIX: Use _load_json_config ---
    APP_TRACKER_SCHEMA_DATA = _load_json_config(str(DATA_DIR / "app_tracker_schema.json"), "App Tracker Schema")
except (FileNotFoundError, json.JSONDecodeError) as e:
...
class DataEnricher:
    ...
    def enrich(
     ...
     orchestrator=None
        ...
        # --- FIX: Change method name to 'find_duplicates_in_list' ---
        # The 'DuplicateDetector' class expects a list of strings
...
class ArtistGenerator:
    ...
    def __init__(self, master_resume: Dict, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis, artist_specs: Dict, app_config: 'AppConfig', **kwargs):
        """
        ...
        Args:
            ...
            app_config: Full AppConfig object (provides access to rag, artist, constraints, etc.)
        ...
        # Store full config for access to all sub-configs
        self.config = app_config
        
        # Extract commonly-used sub-configs for convenience (maintains backward compatibility)
        self.artist_config = app_config.artist
        ...
        # --- NEW: Load prompts from central JSON ---
        self.PROMPT_TEMPLATES = PROMPT_TEMPLATES_GLOBAL
        if not self.PROMPT_TEMPLATES:
            raise HopExecutionError("ArtistGenerator: prompts.json failed to load.")

        # --- NEW: Code Interpreter Tool ---
        self.code_interpreter = CodeInterpreterTool()

        # --- NEW: Internal dispatch map for "Clean R&R" ---
        self.GENERATION_DISPATCH = {
            ...
            "_generate_section_generic": self._generate_section_generic,
            "_generate_section_macro_tot": self._generate_section_macro_tot,
    ...
    def _parse_specs(self, raw_specs: Dict) -> Dict['ResumeSection', Dict[str, Any]]:
        ...
        CORRECTED VERSION - Fixed indentation bug that made validation code unreachable.
        """
        try:
            reconstructed_specs = {}
...
"""
    ...
    def _call_gemini_api(self, prompt: str, reasoning_config: ReasoningConfig, section_id: str, system_prompt: str, temperature_override: Optional[float] = None) -> Tuple[str, int]:
        ...
        try:
            ...
            try:
                # --- FIX: Use the model from config, not a hardcoded string ---
                model = genai.GenerativeModel(self.config.rag.model)
            ...
            if sc_count > 1:
                ...
                try:
                    ...
                    if hasattr(response, 'candidates') and response.candidates:
                        for c in response.candidates:
                            ...
                            # --- START FIX: Correctly aggregate parts for each candidate ---
                            if hasattr(c, 'content') and hasattr(c.content, 'parts'):
                                # Combine all text parts for this single candidate
                                full_candidate_text = "".join(
                                    part.text for part in c.content.parts if hasattr(part, 'text') and part.text
                                )
                                if full_candidate_text:
                                    candidate_responses.append(full_candidate_text)
                            # --- END FIX ---
                ...
                # --- FIX: Log the *actual* number of candidates synthesized ---
...
"""
        ...
        for section_enum in ordered_sections:
            ...
            if generation_method_name in ["_copy_from_master", "_copy_k0_contact", "_generate_dummy_header"]:
                ...
                try:
                    ...
                    elif generation_method_name == "_generate_section_macro_tot":
                        method_args["spec"] = spec
    ...
    def _build_context_k10_skills(self, spec: Dict, section_enum: Optional[ResumeSection] = None) -> Dict:
        try:
            # --- START FIX: Provide the context keys expected by 'artist_skills' prompt ---
            
            # 1. Get data from ThematicAnalysis
            ...
            if self.thematic_analysis:
                ...
                if self.thematic_analysis.primary_theme and isinstance(self.thematic_analysis.primary_theme, dict):
                    primary_theme_name = self.thematic_analysis.primary_theme.get('name', 'N/A')
                    key_tech_list = self.thematic_analysis.primary_theme.get('keywords', [])
                
                # Get Secondary Theme Keywords (as Required Skills)
                if self.thematic_analysis.secondary_themes and isinstance(self.thematic_analysis.secondary_themes, list) and len(self.thematic_analysis.secondary_themes) > 0:
                    req_skills_list = self.thematic_analysis.secondary_themes[0].get('keywords', [])

            # 2. Return the four keys the prompt expects
            return {
                "job_description": self.job_description[:2000], # Truncate for prompt safety
    ...
    def _post_process_k10_skills(self, skills_text: str, section_enum: ResumeSection) -> List[str]:
        try:
            ...
            skill_count = len(skills_list_final)
            if not (self.constraints.SKILLS_COUNT_MIN <= skill_count <= self.constraints.SKILLS_COUNT_MAX):
                raise HopExecutionError(f"{section_enum.value} generation failed: Expected {self.constraints.SKILLS_COUNT_MIN}-{self.constraints.SKILLS_COUNT_MAX} valid skills, found {skill_count}. Preview: {skills_text[:100]}...")
    ...
    # --- NEW: Macro-ToT Generator ---
    def _generate_section_macro_tot(self, spec: Dict, section_enum: ResumeSection, temperature_override: Optional[float]) -> Tuple[List[str], int]:
        """
        NEW: Runs the "Macro-ToT" loop, generating 3 drafts for evaluation.
        """
        logging.info(f"  Running Macro-ToT generation for {section_enum.name} (3 drafts)...")
        drafts = []
        total_calls = 0
        
        macro_tot_config = spec.get("macro_tot_config", {})
        num_drafts = macro_tot_config.get("num_drafts", 3)

        for i in range(num_drafts):
            # Vary temperature slightly for creativity
            temp_override = (temperature_override or 0.8) + (i * 0.1)  # e.g., 0.8, 0.9, 1.0
            logging.info(f"    Generating draft {i+1}/{num_drafts} (Temp: {temp_override:.1f})...")
            
            draft_output, call_count = self._generate_section_generic(
                spec=spec,
                section_enum=section_enum,
                temperature_override=temp_override
            )
            drafts.append(draft_output)
            total_calls += call_count

        logging.info(f"  ✓ Macro-ToT complete for {section_enum.name}. Generated {len(drafts)} drafts.")
        # This section now returns a LIST of drafts.
        # The Inspector (HOP-5) will be responsible for picking the winner.
        return drafts, total_calls
    ...
    def _generate_section_generic(self, spec: Dict, section_enum: ResumeSection, temperature_override: Optional[float]) -> Tuple[Any, int]:
        ...
        # --- REFACTORED: Load prompt from central template store ---
        prompt_key = spec.get("prompt_template")  # artist_specs uses "prompt_template" as the key
        if not prompt_key: raise HopExecutionError(f"Prompt key missing in spec for {section_enum.name}")
        prompt_template = self.PROMPT_TEMPLATES.get(prompt_key)
        if not prompt_template: raise HopExecutionError(f"Prompt template '{prompt_key}' not found in prompts.json for {section_enum.name}")
...
"""
                ...
                enhanced_prompt = prompts_RES.build_generation_prompt_with_reinforced_constraints(
                    base_prompt,
...
class WorkflowOrchestrator:
    ...
    def __init__(
        ...
        # --- NEW: Initialize Agentic Tools ---
        # --- FIX: Pass correct path from config to Librarian ---
        self.librarian = LibrarianAgent(storage_path=str(config.rag.chroma_persist_dir)) if CHROMADB_AVAILABLE else None
        self.code_interpreter = CodeInterpreterTool() if CODE_INTERPERTER_AVAILABLE else None
    ...
    def _execute_hop_0_jd_analysis(self, force_rerun: bool = False) -> None:
        ...
        try:
            ...
            # --- FIX: Add the missing 'job_title' argument ---
            thematic_analysis = jd_analyzer.analyze_job_description(
                job_description=self.job_input['job_description'],
                company_name=self.job_input.get('company_name', 'Unknown'),
                job_title=self.job_input.get('job_title', 'Unknown'), # <-- ADD THIS LINE
                master_resume_index=None,  # This needs to be built from master_resume
                comp_config=self.config.comp_config
            )
    ...
    def _execute_hop_3_artist_generation(self) -> None:
        ...
        try:
            ...
            # --- START FIX ---
            # Get all sections the artist knows how to generate
            ...
            temperature_overrides_pass1 = {
                section: DEFAULT_GENERATION_TEMPERATURE 
                for section in sections_to_generate_all
            }
            
            # Call generate with the correct arguments and unpack all 3 return values
            artist_output, validation_results, total_api_calls = artist.generate(
                sections_to_generate=sections_to_generate_all,
                temperature_overrides=temperature_overrides_pass1
            ) #
            # --- END FIX ---
            ...
            checkpoint = self._create_checkpoint(
                hop_id_str, hop_name,
                validation_results=validation_results, # <-- Use the actual validation results
    ...
    def _execute_hop_4_staging_and_sanitization(self) -> None:
        ...
        try:
            ...
            # 1. Fill the buffer with raw data objects
            staging_buffer = ImmutableStagingBuffer()
            for section_key, section_data in artist_output.items():
                # Pass the raw object (str, List[Dict], etc.) directly.
                # NEW: This now includes the LISTS of 3 drafts for K0, K1, K11
                staging_buffer.set(section_key, section_data)
            
            staging_buffer.lock()

            # 2. Sanitize the entire buffer (TextSanitizer handles recursion)
            sanitizer = TextSanitizer()
            validation_results, sanitized_data = sanitizer.sanitize_buffer(staging_buffer)
            
            # 3. Save the *sanitized* data
            self.state_serializer.save(hop_id_num, sanitized_data)
            
            checkpoint = self._create_checkpoint(
                hop_id_str, hop_name,
                validation_results=validation_results, # Pass sanitization V-results
                output_data={"sections_count": len(sanitized_data)},
                start_time=hop_start_time,
                metadata={"buffer_locked": True, "sanitization_fixes": sum(sanitizer.sanitization_counts.values())}
            )
            self._save_checkpoint_to_manifest(checkpoint)
            self.hop_checkpoints.append(checkpoint)
    ...
    def _execute_hop_5_validation(self) -> None:
        ...
        try:
            # --- START FIX ---
            # 1. Load data
            staging_buffer_data = self.state_serializer.load(4) # This data contains the *lists* of drafts
            thematic_analysis = self.state_serializer.load(0)

            # 2. Rebuild the locked buffer
            staging_buffer = ImmutableStagingBuffer()
            for k, v in staging_buffer_data.items():
                staging_buffer.set(k, v)
            staging_buffer.lock()
            
            # 3. Initialize the *real* validator
            validator = PreFlightValidator(
                master_resume=self.master_resume,
                app_config=self.config
            )
            
            # 4. Run the *real* validation
            # --- FIX: Capture all 4 return values and pass the 'macro_tot_drafts' argument ---
            validation_results, all_passed, failed_sections, winning_drafts = validator.validate(
                staging_buffer=staging_buffer,
                thematic_analysis=thematic_analysis,
                job_description=self.job_input.get('job_description', ''),
                macro_tot_drafts=staging_buffer_data # Pass the raw drafts dict from HOP-4
            )
            # --- END FIX ---
            
            self.validation_results = validation_results
            self.state_serializer.save(hop_id_num, validation_results) # Save HOP-5 results
            
            # --- FIX: CRITICAL: Overwrite HOP-4 data with the winning drafts ---
            # This ensures HOP-7 renders the *winners*, not the list of drafts
            self.state_serializer.save(4, winning_drafts)
            self.logger.info("  ✓ Saved winning drafts, overwriting HOP-4 state.")
            ...
            # --- NEW: Check for strategic/factual failures that should trigger Slow Loop ---
            strategic_failures = [
                vr for vr in validation_results 
                if not vr.passed 
                and vr.severity == ValidationSeverity.CRITICAL
                and ('strategic' in vr.rule_id.lower() or 'factual' in vr.rule_id.lower())
            ]
            
            if strategic_failures:
                failure_messages = [f"{vr.rule_id}: {vr.message}" for vr in strategic_failures]
                error_msg = f"Critical strategic/factual validation failures detected: {'; '.join(failure_messages)}"
                self.logger.error(error_msg)
                raise FactualFailureException(error_msg)
            
            checkpoint = self._create_checkpoint(
                hop_id_str, hop_name,
                validation_results=validation_results,
                output_data={"validation_count": len(validation_results), "passed": all_passed},
                start_time=hop_start_time,
                metadata={"failed_sections": [s.name for s in failed_sections]}
            )
            self._save_checkpoint_to_manifest(checkpoint)
            self.hop_checkpoints.append(checkpoint)
    ...
    def _execute_hop_6_gate_decision(self) -> None:
        ...
        try:
            # Load validation results from the state file
            validation_results = self.state_serializer.load(5)
            
            # Initialize the engine (it takes no arguments)
            gate_engine = GateDecisionEngine()
            
            # Call the 'decide' method (it returns a tuple)
            decision, reason = gate_engine.decide(validation_results)
            ...
            if decision == GateDecision.HALT:
                ...
                self._save_checkpoint_to_manifest(checkpoint)
                self.hop_checkpoints.append(checkpoint)
                # This is a CREATIVE/CONSTRAINT failure. HALT.
                raise HopExecutionError(f"Gate decision: HALT (Creative/Constraint Failure). Reason: {reason}")
    ...
    def _execute_hop_7_rendering(self) -> None:
        ...
        try:
            ...
            # This data is NOW the *updated* buffer with the *winning* drafts
            staging_buffer = ImmutableStagingBuffer()
            for k, v in staging_buffer_data.items():
                staging_buffer.set(k, v)
            staging_buffer.lock()
            ...
            # --- FIX: Add the missing 'config' argument ---
            ...
            # --- FIX: Add the missing 'thematic_analysis' argument ---
            file_paths, (validation_results_render, file_contents) = renderer.render(
                staging_buffer=staging_buffer,
                company_name=company_name,
                job_title=job_title,
                thematic_analysis=self.state_serializer.load(0), 
    ...
    def _execute_hop_8_qa_report(self) -> None:
        ...
        try:
            ...
            qa_results, qa_report_text = qa_generator.generate(
                staging_buffer, thematic_analysis, validation_results, file_contents, self.job_input.get('job_description', '')
            )
    ...
    def _create_jd_analyzer(self):
        ...
        # from rag_RES import EnhancedJobDescriptionAnalyzer # Old v16.20 call
    ...
    def _handle_workflow_termination(
        ...
        return {
            ...
            "coc_ledger": coc_ledger,
                        "hop_checkpoints": [asdict(hc) for hc in self.hop_checkpoints],
            "metadata": {
                "slow_loop_iterations": self.slow_loop_iteration
            }
        }
