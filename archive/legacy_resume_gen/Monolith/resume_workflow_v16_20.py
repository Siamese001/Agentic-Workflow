...
try:
    ...
    logging.warning("Warning: sklearn not installed. Some features disabled.")
...
class AppConfig:
    ...
    content_constraints: 'ContentConstraintsConfig' = field(default_factory=lambda: None)
    signal_constraints: 'SignalControlConfig' = field(default_factory=lambda: None)
    ...
    def _validate_row(self, idx: int, row: Dict):
        ...
        if versioned_resume:
            ...
            if not re.match(filename_pattern, versioned_resume):
             ...
             # --- SPEC 3.2: USE STRING FIELD HELPER ---
...
6. Required vs preferred skills"""
        ...
        if mission.key_technologies:
            safe_tech = mission.key_technologies[0]
...
class EnhancedJobDescriptionAnalyzer:
    ...
    def __init__(
        ...
        self.web_rag_config = web_rag_config or WebRagConfig()
...
CRITICAL: Return only the JSON object. Do not include any preamble, explanation, or markdown formatting outside of the JSON block. """
    ...
    def _synthesize_thematic_analysis(
        ...
        def get_status(phase_result_dict):
             ...
             if "thematic_analysis" in phase_result_dict and phase_result_dict["thematic_analysis"]: return "SUCCESS"
             if "authenticity_patterns" in phase_result_dict and phase_result_dict["authenticity_patterns"]: return "SUCCESS"
             if "competitive_analysis" in phase_result_dict and phase_result_dict["competitive_analysis"]: return "SUCCESS"
             if "problem_solution_narratives" in phase_result_dict and phase_result_dict["problem_solution_narratives"]: return "SUCCESS"
                    ...
                    try:
                        if 'severity' in src_data and isinstance(src_data['severity'], str):
                             try:
                                 ...
                                 src_data['severity'] = ValidationSeverity.INFO
...
class ConstraintFailureClassifier:
    ...
    # DEPRECATED: Removed should_reduce_temperature (Spec 1.3)
    # This logic is superseded by the "Halt on Creative Failure" approach in HOP-3
...
class TextSanitizer:
    ...
    def __init__(self, hyphenation_rules: Dict = None):
        ...
        self.sanitization_counts = {
            ...
            'forbidden_hyphen_minus_removed': 0,
            'invisible_chars_removed': 0,
            'natural_hyphens_preserved': 0,
        }
    ...
    def sanitize_buffer(self, staging_buffer: 'ImmutableStagingBuffer') -> Tuple[List['ValidationResult'], Dict]:
        ...
        total_fixes = sum(v for k, v in self.sanitization_counts.items() if 'preserved' not in k)
    ...
    def _sanitize_text(self, text: str) -> str:
        ...
        preserved_hyphen_minus_count_in_iteration = 0
...
class ValidationContext:
    ...
    def _calculate_unify_ibm_percent(self):
        ...
        self._cache["H5_GLOBAL_WORD_DISTRIBUTION_UNIFY_IBM"] = details
        return percent

    def _calculate_unify_ibm_ratio(self):
        ibm_w = self.ibm_words
        unify_w = self.unify_words
        ratio = unify_w / ibm_w if ibm_w > 0 else 0.0
        ...
        self._cache["H5_GLOBAL_UNIFY_IBM_RATIO"] = details
        return ratio
    ...
    def _calculate_headline_details(self):
        details = self._calculate_metric_details(
            ...
            metrics_to_calc=[('word_count', text_utils.count_words), ('headline', lambda t: t)],
    ...
    def _calculate_cover_letter_jd_similarity(self):
        ...
        if cover_letter_text and self.job_description:
            try:
                similarity = self.dup_detector._calculate_cosine_similarity(cover_letter_text, self.job_description)
            except Exception as e:
    ...
    def _calculate_k4_narrative_details(self):
        min_wc = getattr(ContentConstraintsConfig, 'TRADERSENSE_NARRATIVE_WORD_COUNT_MIN', 40)
        max_wc = getattr(ContentConstraintsConfig, 'TRADERSENSE_NARRATIVE_WORD_COUNT_MAX', 60)
        details = self._calculate_metric_details(
    ...
    def _calculate_cross_section_similarity_details(self) -> Dict:
        ...
        for i in range(len(sections_to_compare)):
            for j in range(i + 1, len(sections_to_compare)):
                ...
                if text1 and text2:
                    try:
                        similarity = self.dup_detector._calculate_cosine_similarity(text1, text2)
            ...
            if isinstance(narrative_text, str) and narrative_text.strip():
                ...
                if master_highlights:
                    ...
                    for highlight in master_highlights:
                        try:
                            similarity = self.dup_detector._calculate_cosine_similarity(narrative_text, highlight)
...
class PreFlightValidator:
    ...
    def _register_rules(self):
        ...
        all_rules_config.append(self._mk_method("H5_CONTENT_NO_PROMPT_CONTAMINATION", ValidationSeverity.HIGH, "content", "_validate_no_prompt_contamination", lambda ctx: f"Found prompt contamination keywords in content: {ctx._cache.get('H5_CONTENT_NO_PROMPT_CONTAMINATION', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("H3_GLOBAL_CONTENT_NO_CONVERSATIONAL_FILLERS", ValidationSeverity.HIGH, "content", "_validate_no_conversational_fillers", lambda ctx: f"Found conversational filler phrases in content: {ctx._cache.get('H3_GLOBAL_CONTENT_NO_CONVERSATIONAL_FILLERS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("H5_STRUCTURE_NO_EMPTY_LIST_ITEMS", ValidationSeverity.MEDIUM, "structure", "_validate_no_empty_list_items", lambda ctx: f"Found empty list items in sections: {ctx._cache.get('H5_STRUCTURE_NO_EMPTY_LIST_ITEMS', {}).get('violations', 'N/A')}"))
        all_rules_config.append(self._mk_method("H5_STRUCTURE_MARKDOWN_HEADER_SPACING", ValidationSeverity.MEDIUM, "structure", "_validate_markdown_header_spacing", lambda ctx: f"Found markdown headers with missing spaces: {ctx._cache.get('H5_STRUCTURE_MARKDOWN_HEADER_SPACING', {}).get('violations', 'N/A')}"))

        # --- SPEC 3.1: USE BULK REGISTRATION ---
            ...
            def create_error_message_lambda(template, rule_id_for_cache):
                def error_lambda(ctx: ValidationContext):
                    try:
                        ...
                        logger.error(f"Error formatting error message for rule {rule_id_for_cache}: {e}. Template type: '{type(template)}'. Details retrieved: {ctx._cache.get(rule_id_for_cache, 'Not Found')}", exc_info=False)
    ...
    def _validate_bullet_word_count_range(self, context: ValidationContext) -> bool:
        ...
        for section_enum in self.BULLET_WORD_COUNT_SECTIONS_TO_CHECK:
            ...
            target_range = self.validator_config.bullet_word_count_ranges.get(section_enum.name) if hasattr(self.validator_config, 'bullet_word_count_ranges') else None
            if target_range is None:
                logging.warning(f"WC range missing for {section_enum.name} in PreFlightValidator config. Skipping validation for this section.")
                continue
...
class QAReportGenerator:
    ...
    def generate(
        ...
        for output_name, rule_id in output_checks.items():
            ...
            status = ('PASS' if result and result.passed else 'FAIL') if result else 'PASS'
