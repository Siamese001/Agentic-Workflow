(comment
  ;; File: Resume Gen Agentic Pipeline v8.5.clj
  ;; Version: 8.5 (Code-Validated)
  ;; This file represents the v8.5 (Agentic Upgrade) architecture.
  ;; It has been code-reviewed and corrected against the v8.5 Python/JSON files
  ;; to ensure all agent definitions, scores, and logic checks are accurate.

  +------------------------------------------------------------------------------------------------------------+
  | 🚀 v8.5 PARALLEL BATCH HARNESS (run_batch_v8_5.py)                                                          |
  ...
  |  | | (v8.5 Cost Check)| | (v8.5 Cost Check)| | (v8.5 Cost Check)| | (v8.5 Cost Check)| simultaneously)|    |
  ...
  |  |                                      v                                                        |    |
  |  |   2. (finally) Write all results to `batch_summary_v8_5.csv` (Output 1) 📈                     |    |
  ...
  |  |   (Batch runner *automatically* calls `run_learning_v8_5.py`)                                    |    |
  |  +--------------------------------------------------------------------------------------------------+    |
  |                                                                                                            |
  + - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  | ♾️ v8.5 ASYNCHRONOUS META-LEARNING LOOP (Safer v7.5+ Logic)                                                |
  | (Triggered by `run_batch_v8_5.py` after batch is complete)                                                 |
  |                                                                                                            |
  | 1. `run_learning_v8_5.py` runs:                                                                            |
  |    +---------------------------+ (Reads failures from) -> +-------------------------+                     |
  ...
  +------------------------------------------------------------------------------------------------------------+
  | 🧠 v8.5 LANGGRAPH ORCHESTRATION (Replaces v6.5 Governor)                                                     |
  | (LangGraph executes a 5-step stateful flow with dynamic recovery loops)                                    |
  |                                                                                                            |
  | 1. 🚀 **CrewOrchestrator** (Receives "Neo4j VP" goal)                                                      |
  | 2. **CrewOrchestrator** (Calls) -> 🧠 **v8.5 LangGraph State Flow** |
  |                                                                                                            |
  |  +------------------------------------------------------------------------------------------------+  |
  |  | **v8.5 LangGraph State Flow** |  |
  |  |                                                                                                |  |
  |  |   1. 🧭 **Run Strategy** (Calls `ThemeClassifierAgent`)                                          |  |
  |  |                | (Writes `strategy_brief` to 📋 State)                                        |  |
  |  |                v                                                                               |  |
  |  |   2. 🔍 **Run RAG** (Calls `v8.5 RAG Stack` agents) [Score: 96]                                  |  |
  |  |                | (Writes `rag_critique`, `rag_search_results` to 📋 State)                     |  |
  |  |                v                                                                               |  |
  |  |   3. ✒️ **Run Bullet Stack** (Calls `v8.5 ProvenanceRouterAgent`) [Score: 92] (Corrected)        |  |
  |  |                | (Writes `generated_bullets` to 📋 State)                                      |  |
  |  |                v                                                                               |  |
  |  |   4. ✍️ **Run Drafting** (Calls `v8.5 DraftingConductorAgent`) [Score: 98]                      |  |
  |  |                | (Writes `final_draft` to 📋 State)                                            |  |
  |  |                v                                                                               |  |
  |  |   5. 🛡️ **Run QA** (Calls `v8.5 QAConductorAgent`, `AtomicQASwarmLogic`) [Score: 95]           |  |
  ...
  +------------------------------------------------------------------------------------------------------------+
  | 🧭 v8.5 (v7.0 Arch) STEP 1: STRATEGY (Unchanged)                                                            |
  |                                                                                                            |
  |    +--------------------------------------------------------------------------------------------------+    |
  ...
  +------------------------------------------------------------------------------------------------------------+
  | 🔍 v8.5 STEP 2: RAG STACK (Agentic Upgrade) [Score: 96]                                                      |
  |                                                                                                            |
  ...
  |    |       `--> [ 🔗 **graph_search** ] (v8.5 Graph Tool)                                            |
  |    |       `--> [ ✍️ **write_to_graph** ] (v8.5 Graph Tool)                                          |
  |    |                                                                                                |
  ...
  +------------------------------------------------------------------------------------------------------------+
  | ✒️ v8.5 STEP 3: BULLET STACK (Provenance) [Score: 92] (Corrected) (Replaces v6.5 BulletSwarm)              |
  |                                                                                                            |
  ...
  +------------------------------------------------------------------------------------------------------------+
  | ✍️ v8.5 STEP 4: DRAFTING (MoE Conductor) [Score: 98] (Replaces v6.5 AdversarialRouter)                       |
  |                                                                                                            |
  ...
  |   | * [ 📊 **MetricsSpecialist (Flash)** [75] ] (v8.5: Injects/Fixes metrics)          |   |
  |   +----------------------------------------------------------------------------------+   |
  ...
  +------------------------------------------------------------------------------------------------------------+
  | 🛡️ v8.5 STEP 5: QA STACK (MoE Conductor) [Score: 95] (Code-Corrected)                                       |
  |                                                                                                            |
  |   +---------------------------------------+  +---------------------------------------+   |
  ...
  |   | * 🔎 **MissedOpportunityAgent** [80] (v8.5) |  |                                 |   |
  |   +---------------------------------------+  +---------------------------------------+   |
  |                                                                                                            |
  +------------------------------------------------------------------------------------------------------------+
)