...
# Version: 6.0 (Telemetry, Circuit Breaker, Cost Tracking, Structured I/O)
# v6.0 CHANGES:
# - Added CircuitBreakerOpenError exception handling
# - Enhanced Configuration for telemetry and cost tracking
# - Added meta_loop_config and cost_config sections
# - Ready for JSON structured logging
...
# Load Configuration
CONFIG_PATH = PROJECT_ROOT / "master_config_v6_0.json"

class Configuration:
    """
    Central configuration class.
    Reads all settings from master_config_v6_0.json.
    v6.0: Enhanced with cost_config and meta_loop_config
    """
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if not self.config_path.exists():
            core_logger.critical(f"FATAL: master_config_v6_0.json not found at {self.config_path}.")
            raise FileNotFoundError(f"master_config_v6_0.json not found at {self.config_path}.")
        try:
...
class CircuitBreakerOpenError(Exception):
    """--- v6.0: Raised when circuit breaker is open and rejects requests ---"""
    pass
...
class ResumeSection:
    """
    Dynamically provides resume sections from the master_config_v6_0.json.
...
class JDEnforcementRule:
    """
    Dynamically provides JD enforcement rules from master_config_v6_0.json.
...
# REFACTORED: ReflectionStatus for v5.8+ agentic features
...
class CircuitBreakerConfig:
    """--- v6.0: Configuration for circuit breaker pattern ---"""
...
class WorkflowLogFilter(logging.Filter):
    """Filter workflow logs based on workflow_id."""
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        """Filter records by workflow_id."""
        return getattr(record, 'workflow_id', None) == self.workflow_id

def build_atomic_agent_prompt(
    agent_config: AtomicAgentConfig,
...
def _get_prompt_template(key: str) -> str:
    ...
    if not hasattr(PROMPT_TEMPLATES, key):
        ...
        raise KeyError(f"Prompt template key '{key}' not found in master_config_v6_0.json!")
