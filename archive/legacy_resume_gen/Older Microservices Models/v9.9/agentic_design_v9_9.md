# Agentic Architecture Design v9.9

**VERSION 9.9: SECURITY & ERROR HANDLING HARDENING**

This document reflects the v9.9 architecture, which implements critical security and error handling fixes based on the v9.8 analysis.

### Key v9.9 Changes:

1.  **🔒 Security (PII):** `PIISanitizerAgent` no longer uses an external API. It now uses **local Presidio processing** to prevent resume data leaks.
2.  **🔒 Security (Bias):** `BiasDetectorAgent` no longer uses an external API. It now uses **local regex patterns**.
3.  **📊 Error Handling:** Replaced broad `except Exception` with specific, custom exceptions (`ModelAPIError`, `JSONParsingError`, `FileIOError`) for reliable, fail-fast behavior and easier debugging.

## SECTION 1: AGENTIC CAPABILITY ASSESSMENT (TRANSPOSED) [v9.9]

**UPDATE:** This table is transposed. Stacks are rows.
Scores for v9.9 reflect P0, P1, and P2 enhancements, plus v9.9 security hardening.
...
| SafetyGuardStack | Observe & Modify Self | 98 | 0 | 3 | 0 | 0 | 3 | Gap: Needs to read `proposed_rules.jsonl` to update its constitution. |
| DynamicToolingStack | Read, Write & Act on Env | 98\* | 2 | 1 | 0 | 0 | 3 | Gap: Needs to read `feedback_log.jsonl` for dynamic tool selection. |
| HIL\_InteractionStack | Read, Write & Act on Env | 96\* | 3 | 1 | 0 | 0 | 4 | Gap: Deeper Human-Computer Interaction / collaborative reasoning. |
...
## SECTION 2: DETAILED PIPELINE FLOW (v9.9)

```
+------------------------------------------------------------------------------------------------------------+
| 🚀 v9.9 PARALLEL BATCH HARNESS (run_batch_v9_9.py)                                |
...
| |  |                                      v                                                        |
| |  |   2. (finally) Write all results to `batch_summary_v9_9.csv` (Output 1) 📈 |
...
|  | (Batch runner *automatically* calls `run_learning_v9_9.py`)                |
|  +--------------------------------------------------------------------------------------------------+    |
|                                                                                                            |
+------------------------------------------------------------------------------------------------------------+
...
+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
|
| ♾️ v9.9 AGENTIC META-LEARNING GRAPH (v9.6 Logic, v9.9 Model Info)               |
| (Triggered by `run_batch_v9_9.py` after batch is complete)                                                 |
|                                                                                                            |
| 1. `run_learning_v9_9.py` runs (which is a LangGraph app):                     |
|    +---------------------------+ (Reads) -> +-------------------------+                                   |
...
+------------------------------------------------------------------------------------------------------------+
| 🧠 v9.9 FULL LANGGRAPH ORCHESTRATION (P0 + P1 + P2 Enhancements, v9.9 Security)                             |
| (The main graph now includes parallel safety/cost, ToT strategy, local retries, and a full HIL stack)      |
|                                                                                                            |
| +---------------------------------------------------------------------------------------------------------+  |
| | 📦 **v9.9 LangGraph State Flow** |
| +---------------------------------------------------------------------------------------------------------+  |
| |                                                                                                         |  +----------------------------------+
| | 0. 🧼 **Run PII Scrubber** (LOCAL Presidio 🔒)                  |  | 🛡️ **PARALLEL SAFETYGUARDSTACK** (LOCAL Regex 🔒) |
...
+------------------------------------------------------------------------------------------------------------+
| 🧭 v9.9 STEP 1: STRATEGY (P1 HIL Upgrade)                                                                   |
...
+------------------------------------------------------------------------------------------------------------+
| 🔍 v9.9 STEP 2: RAG STACK (P1/P2 Enhancements) [Score: 98]                                                  |
|                                                                                                            |
...
+------------------------------------------------------------------------------------------------------------+
| ✒️ v9.9 STEP 3: BULLET STACK (v9.6 Adversarial Upgrade) [Score: 88]                                         |
...
+------------------------------------------------------------------------------------------------------------+
| ✍️ v9.9 STEP 5: DRAFTING (P1 "True Conductor" Upgrade) [Score: 99]                                          |
|                                                                                                            |
...
+------------------------------------------------------------------------------------------------------------+
| 🛡️ v9.9 STEP 6: QA STACK (P1 "True Conductor" Upgrade) [Score: 98]                                          |
|                                                                                                            |
|   +---------------------------------------+  +---------------------------------------+   |
...
|   | * ⚖️ **BiasScrubberAgent [60] (LOCAL Regex 🔒)**      |
