# File: run_batch_v7_5.py
# Overwrites: run_batch_v7_0.py
# Version: 7.5 (LangGraph + Redis + HIL Disabled)
#
# v7.5 (Phase 2 HIL) CHANGES:
# - Imports from v7_5.
# - 'get_compiled_graph' now passes 'enable_hil=False'
#   to compile a non-interactive graph for batch processing.
...
# Imports from its *own* versioned files (v7.5)
from main_v7_5 import setup_logging, load_job_input
from core_v7_5 import CONFIG, CircuitBreakerOpenError

# v7.5: Import the graph builder and checkpointer
from agent_swarm_v7_5 import get_graph_app
from langgraph.checkpoint.redis import RedisSaver

# v7.5: Import the meta-learner
try:
...
logger = logging.getLogger("batch_runner_v7_5")
try:
...
SUMMARY_FILE = "batch_summary_v7_5.csv"

# --- v7.5: Graph Setup (will be re-initialized in each process) ---
checkpointer = None
app = None
...
def get_compiled_graph():
    ...
    v7.5: Passes 'enable_hil=False' to the graph builder.
    """
...
def process_single_job(job_path: str) -> Dict[str, Any]:
    ...
    logger.info(f"--- Starting v7.5 job: {os.path.basename(job_path)} ---")
    ...
    try:
        ...
        logger.info(f"--- Finished v7.5 job: {company} - {title} ---")
...
def run_batch():
    ...
    if not job_files:
        logger.info("v7.5 Batch process starting. No jobs found in queue.")
        return

    logger.info(f"v7.5 Batch process starting. Found {len(job_files)} jobs.")
    ...
    logger.info(f"v7.5 Batch process complete.")
    ...
    if CONFIG.meta_loop_config.enable_meta_learning:
        ...
        if META_LEARNER_AVAILABLE:
            ...
            logger.warning("Meta-learning is enabled, but 'run_learning_v7_5' could not be imported.")
    else:
