# File: agent_swarm_v7_0.py
# Overwrites: agent_swarm_v6_5.py
# Version: 7.0 (LangGraph Refactor)
#
# v7.0 (LangGraph Refactor) CHANGES:
# - DELETED the 'Governor' class.
# - DELETED the 'CrewOrchestrator' class.
# - Kept all agent classes (ThemeClassifierAgent...AtomicQASwarmLogic) as-is.
# - Added new 'PART 8: V7.0 LANGGRAPH ORCHESTRATOR' section.
# - This new section defines node wrappers, the conditional edge, and
#   a 'get_graph_app' function to build the persistent graph.
...
# (!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
# (!!! The entire 'Governor' class from v6.5 is DELETED here !!!)
# (!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
...
# (!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!) 
# (!!! The 'CrewConfiguration' class from v6.5 is DELETED here      !!!) 
# (!!! The 'CrewOrchestrator' class from v6.5 is DELETED here       !!!) 
# (!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)


# ============================================================================
# PART 8: V7.0 LANGGRAPH ORCHESTRATOR (NEW)
# ============================================================================

# This section replaces the Governor and CrewOrchestrator.
# It defines the nodes and graph structure for LangGraph.

def get_blackboard(state: 'GraphState') -> WorkflowBlackboard:
    """Helper to construct the v6.5 blackboard for agents."""
    return WorkflowBlackboard(
        workflow_id=state["workflow_id"],
        master_resume=state["master_resume"],
        job_input=state["job_input"],
        artifacts=state["artifacts"],
        plan=None # v6.5/v7.0 is dynamic
    )
...
def run_rag_stack(state: 'GraphState') -> Dict[str, Any]:
    ...
    queries = query_agent.run(strategy_brief=state["artifacts"]["strategy_brief"])
    ...
    critique_result = critique_agent.run(
        strategy_brief=state["artifacts"]["strategy_brief"],
        rag_results=search_results.get("results", [])
    )
    
    # Return all artifacts from this step
    return {"artifacts": {
        "rag_queries": queries,
...
def run_drafting_stack(state: 'GraphState') -> Dict[str, Any]:
    ...
    # 2. Bullet Swarm
    bullet_agent = BulletSwarmAgent(blackboard, debug_mode=True)
    bullets = bullet_agent.run()
    
    # 3. Adversarial Drafting
    drafting_agent = AdversarialDraftingRouter(blackboard, debug_mode=True)
    final_draft = drafting_agent.run()
    
    return {"artifacts": {
        "final_drafting_prompt": prompt_result.get("final_prompt"),
...
def run_qa_swarm(state: 'GraphState') -> Dict[str, Any]:
    ...
    # 1. LLM QA
    qa_llm_agent = AtomicQASwarmLLM(blackboard, debug_mode=True)
...
def check_qa_results(state: 'GraphState') -> str:
    ...
    It routes to the Re-Planner, to END, or to a FAIL node.
    """
    ...
    if qa_passed:
        logger.info("✅ QA Swarm Passed. Ending workflow.")
        return "END"
...
def get_graph_app(checkpointer: 'RedisSaver') -> 'CompiledGraph':
    """
    ...
    workflow.add_edge("rag_stack", "drafting_stack")
    workflow.add_edge("drafting_stack", "qa_swarm")
    
    # 3. Add Conditional Edge (The QA Loop)
    workflow.add_conditional_edges(
        "qa_swarm",
        check_qa_results,
        {
            "END": END,
            "FAIL_HALT": END, # Can also be a dedicated failure node
            "REPLANNER": "replanner"
        }
    )
    
    # 4. Add Re-Planner Loop Edge
    # The replanner clears artifacts and sends back to drafting
    workflow.add_edge("replanner", "drafting_stack")
