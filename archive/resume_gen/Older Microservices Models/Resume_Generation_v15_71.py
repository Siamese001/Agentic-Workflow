...
# VERSION HISTORY
# v15_71 (2025-10-29):
#   CODE QUALITY: "Highest Signal Subject to Constraints" - Resurrect dormant high-temp optimization code
#            - FIXED: Added SECTION_SIGNAL_TARGETS_CONFIG to PreFlightValidator for per-section signal validation
#            - REMOVED: ClerkExtractor._extract_metrics (duplicate of EnhancedJobDescriptionAnalyzer._extract_metrics_from_text)
#            - REMOVED: Commented-out code markers that create noise (git history is the source of truth)
#            - PHILOSOPHY: ConstraintFailureClassifier, _get_feedback_instruction, _pre_flight_constraint_test
#              are NOT tech debt - they implement "highest temp subject to constraints" principle
#            - PHILOSOPHY: Fix broken signal enforcement (SECTION_SIGNAL_TARGETS_CONFIG) rather than deleting
#            - PHILOSOPHY: Remove true tech debt (duplicates, noise) to improve code signal quality
#            - IMPACT: _validate_per_section_signal_raw now functional with proper config
#            - IMPACT: Reduced duplicate metrics extraction logic (single source of truth)
#            - IMPACT: Cleaner codebase without distracting historical comment markers
#   Previous version (15_70) had configuration centralization but missing signal config
#
# v15_70 (2025-10-29):
#   CONFIGURATION CENTRALIZATION: Complete config refactoring for single source of truth
#            - NEW: FilePathsConfig dataclass for all file paths
#            - NEW: ArtistConfig dataclass for ArtistGenerator constants
#            - NEW: ValidatorConfig dataclass for PreFlightValidator constants
#            - NEW: WebRagConfig dataclass for WebSearchRAG constants
#            - NEW: EnricherConfig dataclass for DataEnricher constants
#            - NEW: AppConfig master dataclass combining all configuration
#            - NEW: Global CONFIG instance as single source of truth
#            - REFACTORED: ArtistGenerator now receives artist_config via dependency injection
#            - REFACTORED: PreFlightValidator now receives validator_config via dependency injection
#            - REFACTORED: WebSearchRAG now receives web_rag_config via dependency injection
#            - REFACTORED: DataEnricher now receives enricher_config via dependency injection
#            - REFACTORED: AppTrackerQAValidator now receives validator_config via dependency injection
#            - REFACTORED: WorkflowOrchestrator now manages and distributes CONFIG to all components
#            - ENHANCED: All configuration now in contiguous block at file top
#            - PHILOSOPHY: Destructive overwrite - no parallel configs, single source of truth
#            - PHILOSOPHY: Classes receive config, don't create it - clean separation of concerns
#            - IMPACT: All word counts, constraints, and constants now centrally managed
#            - IMPACT: No risk of ArtistGenerator and PreFlightValidator configs drifting
#            - BACKWARD COMPATIBLE: All functional behavior unchanged, only architecture improved
#   Previous version (15_69) had distributed configuration across multiple classes
#
# v15_69 (2025-10-29):
#   NEW FEATURE: Agentic RAG Loop with Critique & Refinement
#            - NEW: RAGCritique dataclass for evaluating retrieval quality with confidence scores
#            - NEW: RAGState dataclass for tracking evidence accumulation across iterations
#            - NEW: RAGEvidence dataclass for logging individual search actions and findings
#            - ENHANCED: ThematicAnalysis now includes evidence_log field for audit trail
#            - NEW: agentic_search_and_analyze() method for iterative RAG with self-critique
#            - NEW: _critique_rag_results() method for evaluating retrieval completeness
#            - NEW: _build_refinement_prompt() for generating targeted follow-up queries
#            - PHILOSOPHY: "Maximum Research Investment" - transform single-pass RAG into
#              iterative self-improving loop where each phase can validate and refine outputs
#            - PHILOSOPHY: Active Learning - LLM suggests specific follow-up queries to fill gaps
#            - PHILOSOPHY: Audit Trail - evidence_log tracks reasoning chain across iterations
#            - PHILOSOPHY: Quality Gating - confidence_score provides quantitative signal for
#              whether retrieval was sufficient before proceeding to next phase
#            - CONFIGURATION: Default max_iterations=3, confidence_threshold=0.7
#            - IMPACT: Phases can now automatically refine their research when initial results
#              are insufficient, accumulating evidence until confidence threshold is met
#            - BACKWARD COMPATIBLE: Existing search_and_analyze() unchanged; agentic loop is opt-in
#   Previous version (15_68) had single-pass RAG without self-critique or refinement
#
# v15_68 (2025-10-29):
#   NEW FEATURE: Intermediate HOP Output JSON Persistence
#            - NEW: _save_hop_output_json() helper method for saving HOP outputs as JSON files
#            - NEW: Automatic JSON serialization of data objects at the end of each HOP
#            - NEW: Directory structure: workflow_outputs/<workflow_id>/intermediate_hops/
#            - ENHANCED: All major HOPs now save their primary output object as separate JSON files
#            - HOPs with JSON output: -0.5 (RAGMission), 0 (ThematicAnalysis), 1 (ExtractedData),
#              2 (EnrichedScaffold), 3 (ArtistOutput), 4 (StagingBufferData), 4.5 (SanitizedBufferData),
#              5 (ValidationResults), 7 (FilePaths)
#            - ENHANCED: Robust serialization with dataclasses, enums, ImmutableStagingBuffer, datetime handling
#            - ENHANCED: Error handling ensures workflow doesn't halt if JSON saving fails
#            - ENHANCED: INFO logging for successful saves, WARNING for failures
#            - PHILOSOPHY: Debugging/visibility through explicit HOP output persistence
#            - IMPACT: Each workflow execution now produces complete audit trail of intermediate states
#   Previous version (15_67) did not persist intermediate HOP outputs as separate files
#
# v15_67 (2025-10-29):
#   CRITICAL FIX: Resolved HOP-3 Artist Generation failure in narrative section generation
#            - FIXED: Modified _generate_section_generic to pass section_enum to context builders
#            - FIXED: Updated all context builder signatures to accept optional section_enum parameter
#            - FIXED: _build_context_narrative now receives section_enum as parameter instead of from extra_args
#            - FIXED: Removed section_enum extraction from extra_args in _build_context_narrative
#            - IMPACT: Resolves workflow halt at K.4_TraderSense_Narrative generation
#            - ROOT CAUSE: section_enum was available in _generate_section_generic but not passed to builders
#            - SOLUTION: Direct parameter passing ensures context builders have required enum for config lookup
#   Previous version (15_66) had section_enum mismatch causing narrative generation failure
#
# v15_66 (2025-10-29):
#   HOP-0 INTEGRATION: MasterResumeIndex now leveraged throughout RAG pipeline
#            - NEW: WebSearchRAG now accepts master_resume_index parameter for candidate context
#            - ENHANCED: Phase 1 prompt includes candidate's top skills and recent experience context
#            - ENHANCED: Phase 3 prompt cross-references differentiators against candidate's achievements
#            - ENHANCED: _synthesize_thematic_analysis uses index for candidate-grounded keyword weighting
#            - ENHANCED: Keywords matched against skill_to_experiences get priority weighting boost
#            - ENHANCED: Differentiators validated against achievement_catalog for credibility
#            - ENHANCED: recency_scores influence final keyword prioritization
#            - PHILOSOPHY: Transform generic market research INTO candidate-specific strategic positioning
#            - IMPACT: ThematicAnalysis now deeply grounded in what candidate can *credibly support*
#   Previous version (15_65) built MasterResumeIndex but only used it in HOP -0.5 initialization
#
# v15_65 (2025-10-29):
#   HOP -0.5 ENHANCEMENT: Augmented pre-RAG analysis with two signal-boosting upgrades
#            - NEW: _extract_structured_requirements() for must-have vs nice-to-have skill classification
#            - NEW: _cluster_related_skills() for semantic skill grouping using TF-IDF
#            - NEW: _infer_implicit_skills() for domain-specific skill inference
#            - NEW: _build_master_resume_semantic_index() for pre-computed skill→experience mappings
#            - NEW: MasterResumeIndex dataclass containing achievement catalog and domain vocabularies
#            - ENHANCED: EnhancedJobDescriptionAnalyzer.__init__ now builds semantic index at startup
#            - ENHANCED: _execute_pre_rag_analysis now performs structured extraction and clustering
#            - PHILOSOPHY: Better ground truth extraction → better RAG retrieval → better final output
#            - IMPACT: Improves signal quality for all downstream HOP-0 phases (1-4) and HOP-1/HOP-2 retrieval
#   Previous version (15_64) had basic HOP -0.5 with simple entity extraction
#
# v15_64 (2025-10-29):
...
# ============================================================================
# --- MASTER CONFIGURATION BLOCK (START) ---
# ============================================================================

# --- Section 1: File Paths Configuration ---
@dataclass
...
# --- Section 2: Artist Generator Configuration ---
@dataclass
class ArtistConfig:
    """Configuration for ArtistGenerator - provenance splits, word counts, narrative config."""
...
# --- Section 3: Validator Configuration ---
@dataclass
class ValidatorConfig:
    """Configuration for PreFlightValidator and AppTrackerQAValidator."""
...
# --- Section 4: Web RAG Configuration ---
@dataclass
class WebRagConfig:
    """Configuration for WebSearchRAG."""
...
# --- Section 5: Data Enricher Configuration ---
@dataclass
class EnricherConfig:
    """Configuration for DataEnricher."""
...
# --- Section 6: Master Configuration Container ---
@dataclass
class AppConfig:
    ...
    content_constraints: 'ContentConstraintsConfig' = field(default_factory=lambda: None)  # Will be set after class definition
    signal_constraints: 'SignalControlConfig' = field(default_factory=lambda: None)  # Will be set after class definition
...
# ============================================================================
# --- MASTER CONFIGURATION BLOCK (END) ---
# ============================================================================
...
# Create global CONFIG instance after forward-declared classes are defined
# This will be properly initialized after ContentConstraintsConfig and SignalControlConfig are defined
CONFIG = None  # Will be set after config classes are defined
...
# Initialize global CONFIG now that all config classes are defined
CONFIG = AppConfig(
...
class ThematicAnalysis:
     ...
     evidence_log: List[Dict] = field(default_factory=list)  # NEW v15_69: Agentic RAG evidence trail
...
class AppTrackerQAValidator:
    ...
    def __init__(self, run_sha: str = "", actor_id: str = "", validator_config: ValidatorConfig = None):
        # v15_70: Use injected config or default
...
class SkillRequirement:
    """Structured representation of a skill requirement from JD."""
    skill: str
...
@dataclass
class SkillCluster:
    """Semantic cluster of related skills."""
    cluster_name: str
    skills: List[str]
    representative_skill: str  # The skill with highest TF-IDF in cluster
    confidence: float

@dataclass
class MasterResumeIndex:
    """Pre-computed semantic index of master resume for fast retrieval."""
    skill_to_experiences: Dict[str, List[Dict]]  # skill -> [experience_dicts with role, company, bullets]
    achievement_catalog: List[Dict]  # All quantified achievements [{metric, value, context, source_bullet}]
    domain_vocabularies: Dict[str, List[str]]  # domain -> [terminology]
    recency_scores: Dict[str, float]  # skill -> recency score (0.0-1.0)
    skill_vectors: Optional[Dict[str, Any]] = None  # skill -> TF-IDF vector (optional for advanced matching)

# ============================================================================
# NEW v15_69: AGENTIC RAG LOOP WITH CRITIQUE & REFINEMENT
# ============================================================================

@dataclass
class RAGEvidence:
    """Individual search action and its findings within an agentic RAG loop."""
    iteration: int
    action: str  # e.g., "initial_search", "refinement_query", "gap_fill"
    query_or_action: str  # The actual query/action taken
    findings_summary: str  # Brief summary of what was learned
    sources_count: int  # Number of sources retrieved
    confidence_contribution: float  # How much this action improved confidence (0.0-1.0)
    timestamp: str = field(default_factory=lambda: datetime.now().isoformat())

@dataclass
class RAGCritique:
    """Self-critique of RAG retrieval quality."""
    confidence_score: float  # Overall confidence in retrieval completeness (0.0-1.0)
    gaps_identified: List[str]  # Specific information gaps detected
    refinement_tasks: List[str]  # Suggested follow-up queries/actions to fill gaps
    reasoning: str  # Explanation of confidence score and gaps
    is_sufficient: bool  # Whether retrieval is sufficient to proceed
    
@dataclass
class RAGState:
    """State tracker for agentic RAG loop with evidence accumulation."""
    phase_name: str
    iteration: int
    evidence_log: List[RAGEvidence] = field(default_factory=list)
    cumulative_result: Optional[Dict[str, Any]] = None  # Latest merged result
...
class GeminiWebSearchClient:
    ...
    def agentic_search_and_analyze(
        ...
        Args:
            prompt: Initial search prompt
            phase_name: Name of the phase for logging
            max_iterations: Maximum refinement iterations (default: 3)
            confidence_threshold: Minimum confidence to proceed (default: 0.7)
            
        Returns:
            Tuple of (final_result, total_api_calls, RAGState with evidence log)
        """
        logger = logging.getLogger(__name__)
        ...
        # Iteration 1: Initial search with standard prompt
        ...
        initial_evidence = RAGEvidence(
            ...
            confidence_contribution=0.5  # Initial baseline
        )
        state.add_evidence(initial_evidence)
        
        # Critique initial results
        ...
        if critique.is_sufficient or critique.confidence_score >= confidence_threshold:
            ...
            # Attach evidence log to result if it has thematic_analysis
        ...
        for iteration in range(2, max_iterations + 1):
            ...
            try:
                ...
                # Merge results (prioritize new findings but preserve original structure)
                ...
                refinement_evidence = RAGEvidence(
                    ...
                    confidence_contribution=0.2  # Incremental improvement
                )
                state.add_evidence(refinement_evidence)
                
                # Re-critique merged results
        ...
        # Attach evidence log to final result
    ...
    def _critique_rag_results(
        ...
        """
        Evaluate RAG retrieval quality and identify gaps.
        
        Uses heuristics to assess completeness without additional API calls.
        """
        logger = logging.getLogger(__name__)
        ...
        # Heuristic 1: Number of searches performed
        ...
        # Heuristic 2: Number of unique sources
        if sources_count < 3:
        ...
        # Heuristic 3: Phase-specific validation
        if "phase1" in phase_name.lower() or "thematic" in phase_name.lower():
        ...
        # Heuristic 4: Source diversity (check for different domains)
        if sources:
        ...
        refinement_tasks = []
        if gaps:
            # Suggest specific follow-up queries based on gaps
    ...
    def _merge_rag_results(
        ...
        Strategy: Keep original structure, augment with additional data from refinement.
        """
        merged = copy.deepcopy(original)
        
        # Merge search_summary
        if 'search_summary' in refined:
            orig_summary = merged.get('search_summary', {})
            refined_summary = refined['search_summary']
            
            # Accumulate searches performed
            ...
            # Merge other summary fields if present
            for key in refined_summary:
                if key not in ['searches_performed', 'sources']:
                    # Accumulate counts, concatenate lists
...
class WebSearchRAG:
    ...
    def __init__(self, client: GeminiWebSearchClient, config: RAGConfig = RAGConfig(), master_resume_index: Optional['MasterResumeIndex'] = None, web_rag_config: WebRagConfig = None):
        self.client = client
        self.config = config
        self.master_resume_index = master_resume_index  # v15_66: Store for use in phases
...
"""
        ...
        # v15_66: Add candidate context from master_resume_index
        candidate_context = ""
        if self.master_resume_index and self.master_resume_index.skill_to_experiences:
            # Get top 8 skills by recency
            top_skills = sorted(
                self.master_resume_index.recency_scores.items(),
                key=lambda x: x[1],
                reverse=True
            )[:8]
            skills_list = [skill for skill, _ in top_skills]
            
            # Get sample recent experience
            recent_achievements = self.master_resume_index.achievement_catalog[:3] if self.master_resume_index.achievement_catalog else []
            achievements_summary = "; ".join([
                f"{a.get('value', 'N/A')} in {a.get('metric_type', 'unknown')}"
                for a in recent_achievements
            ]) if recent_achievements else "multiple quantified achievements"
            
            candidate_context = f"""
**CANDIDATE BACKGROUND CONTEXT (Use to inform theme identification):**
The candidate has demonstrated expertise in: {', '.join(skills_list[:5])}
Recent quantified achievements include: {achievements_summary}
When identifying themes and keywords, prioritize those where the candidate has proven experience.
"""
...
"""
        ...
        # v15_66: Add candidate achievements context for differentiator validation
        achievements_context = ""
        if self.master_resume_index and self.master_resume_index.achievement_catalog:
            # Get top 5 quantified achievements
            top_achievements = self.master_resume_index.achievement_catalog[:5]
            achievements_summary = "\n".join([
                f"- {a.get('value', 'N/A')} {a.get('metric_type', 'achievement')} ({a.get('context', '')[:80]}...)"
                for a in top_achievements
            ])
            
            achievements_context = f"""
**CANDIDATE ACHIEVEMENTS (Use to validate differentiators):**
The candidate has demonstrated these quantified results:
{achievements_summary}
When identifying differentiators, prioritize keywords the candidate can credibly support with achievements.
"""
...
class EnhancedJobDescriptionAnalyzer:
    ...
    def __init__(
        ...
        web_rag_config: Optional[WebRagConfig] = None  # v15.70: For WebSearchRAG config
    ):
        ...
        self.web_rag_config = web_rag_config or WebRagConfig()  # v15.70
        ...
        # v15_65: Build semantic index of master resume at initialization
        logger = logging.getLogger(__name__)
        ...
        if self.enable_web_search:
            try:
                ...
                # v15_66: Pass master_resume_index to WebSearchRAG for candidate-grounded analysis
                # v15_70: Pass web_rag_config to WebSearchRAG
    ...
    def _build_master_resume_semantic_index(self, master_resume: Dict) -> MasterResumeIndex:
        ...
        # Extract skills and achievements from each experience
        for exp_idx, exp in enumerate(experiences):
            role = exp.get("role", "")
            company = exp.get("company", "")
            end_date = exp.get("end_date", "")
            
            # Calculate recency score (1.0 = current, decays over time)
            try:
                if end_date and end_date.lower() != "present":
                    end_year = int(end_date.split()[-1]) if end_date.split() else current_year
                else:
                    end_year = current_year
                years_ago = current_year - end_year
                recency = max(0.0, 1.0 - (years_ago * 0.15))  # 15% decay per year
            except:
            ...
            for bullet in bullets:
                ...
                # Extract skills from bullet (simple keyword extraction)
                ...
                for skill in potential_skills:
                    ...
                    # Update recency score (take max across all mentions)
            ...
            # Build domain vocabularies from experience
        ...
        # Deduplicate domain vocabularies
        ...
        return MasterResumeIndex(
            ...
            skill_vectors=None  # Could add TF-IDF vectors here for advanced matching
        )
    
    def _extract_skills_from_text(self, text: str) -> List[str]:
        """Extract potential skill keywords from text."""
        # Common technical skills, platforms, methodologies
        skill_patterns = [
            r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b',  # Title case terms (Python, Machine Learning)
            r'\b[A-Z]{2,}\b',  # Acronyms (AWS, API, ML)
            r'\b\w+(?:-\w+)+\b'  # Hyphenated terms (co-sell, cross-functional)
        ]
        ...
        return list(set(skills))[:20]  # Limit to top 20 per bullet
    
    def _extract_metrics_from_text(self, text: str) -> List[Dict]:
        """Extract quantified achievements (metrics) from text."""
        metrics = []
        ...
        # Pattern: XX% growth/increase/improvement
        ...
        # Pattern: XXX+ (customers, users, etc.)
        count_pattern = r'(\d{2,})\+?\s+(\w+)'
        for match in re.finditer(count_pattern, text):
            if match.group(2).lower() in ["customers", "users", "employees", "partners", "deals"]:
                metrics.append({
                    "metric_type": "count",
                    "value": match.group(1),
                    "unit": match.group(2),
    ...
    def _infer_domain(self, role: str, company: str, bullets: List) -> Optional[str]:
        """Infer domain/industry from experience."""
        ...
        domain_keywords = {
            ...
            "fintech": ["financial", "banking", "payments", "trading", "blockchain"],
            "saas": ["saas", "software", "platform", "api", "integration"]
        }
    ...
    def _extract_structured_requirements(self, job_description: str) -> Tuple[List[SkillRequirement], List[SkillRequirement]]:
        """
        v15_65: UPGRADE #1 - Extract structured skill requirements from JD.
        Classifies skills as MUST_HAVE vs NICE_TO_HAVE based on language cues.
        """
        must_have_skills = []
        nice_to_have_skills = []
        ...
        current_section_type = "MUST_HAVE"  # Default
        ...
        for line in lines:
            ...
            # Extract skills from bullet points or sentences
            if line.strip() and (line.strip().startswith('-') or line.strip().startswith('•') or line.strip().startswith('*')):
                ...
                # Extract primary skill (first noun phrase or technology mentioned)
                potential_skills = self._extract_skills_from_text(skill_text)
                
                for skill in potential_skills[:2]:  # Max 2 skills per line
    ...
    def _cluster_related_skills(self, skills: List[str]) -> List[SkillCluster]:
        """
        v15_65: UPGRADE #1 - Semantically cluster related skills using TF-IDF similarity.
        """
        if len(skills) < 2:
            return []
        ...
        try:
            ...
            # Simple clustering: group skills with similarity > threshold
            threshold = 0.3
            ...
            for i, skill in enumerate(skills):
                ...
                if similar_indices:
                    ...
                    # Representative skill is the one with highest average similarity
    ...
    def _infer_implicit_skills(self, explicit_skills: List[str], domain_context: str) -> List[str]:
        """
        v15_65: UPGRADE #1 - Infer implicit skills based on domain context.
        E.g., "technology alliances" implies cloud platform knowledge, ecosystem partnerships, GTM strategy.
        """
        implicit_skills = []
        ...
        inference_rules = {
            ...
            "customer success": ["retention", "expansion", "onboarding", "customer health scoring"],
            "ai/ml": ["model training", "data pipelines", "MLOps", "model deployment"],
            "saas": ["subscription management", "churn reduction", "product adoption", "API integration"]
        }
        ...
        for skill in explicit_skills:
            skill_lower = skill.lower()
            # Check if this skill matches any inference rule
        ...
        # Deduplicate and remove skills already in explicit list
...
# Assuming EnhancedJobDescriptionAnalyzer class definition starts above this
    ...
    def _execute_pre_rag_analysis(self, job_description: str) -> RAGMission:
        ...
        v15_65: Enhanced with structured requirements extraction and skill clustering.
        """
        # Note: 'import logging' should ideally be at the top of the file
        logger = logging.getLogger(__name__)
        logger.info("Executing HOP -0.5: Pre-RAG Differential Analysis...")
        
        # v15_65: UPGRADE #1 - Extract structured requirements
        ...
        # v15_65: UPGRADE #1 - Cluster related skills
        ...
        # v15_65: UPGRADE #1 - Infer implicit skills
        ...
        try:
            ...
            # v15_65: Enhance key_technologies with clustered and implicit skills
            extracted_technologies = analysis_json["jd_entities"]["key_technologies"]
            
            # Add representative skills from clusters
            for cluster in skill_clusters[:3]:  # Top 3 clusters
                if cluster.representative_skill not in extracted_technologies:
                    extracted_technologies.append(cluster.representative_skill)
            
            # Add top implicit skills
            ...
            logger.info(f"    ✓ Enhanced key_technologies: {len(extracted_technologies)} total (was {len(analysis_json['jd_entities']['key_technologies'])})")
            ...
            mission = RAGMission(
                ...
                key_technologies=extracted_technologies,  # Enhanced version
    ...
    def _synthesize_thematic_analysis(
        ...
        # --- v15_66: Apply Candidate Experience Weighting ---
        # Boost keywords where candidate has demonstrated experience
        if self.master_resume_index:
            ...
            for kw in list(keyword_scores.keys()):
                ...
                # Check if keyword matches candidate's skills
                if kw_lower in self.master_resume_index.skill_to_experiences:
                    ...
                    # Get recency score for this skill
                    recency = self.master_resume_index.recency_scores.get(kw_lower, 0.5)
                    
                    # Apply boost: base 1.3x multiplier * recency (0.0-1.0) * experience count factor
                    experience_count = len(experiences)
                    count_factor = min(1.5, 1.0 + (experience_count * 0.1))  # Cap at 1.5x
                    ...
                    logger.debug(f"    Boosted '{kw}': {original_score:.2f} -> {keyword_scores[kw]:.2f} "
                               f"(recency={recency:.2f}, experiences={experience_count})")
                
                # Additional boost for keywords in achievement catalog
                elif self.master_resume_index.achievement_catalog:
                    for achievement in self.master_resume_index.achievement_catalog:
                        source_bullet = achievement.get('source_bullet', '').lower()
                        if kw_lower in source_bullet:
                            # Smaller boost for achievement mentions
                            keyword_scores[kw] *= 1.15
                            candidate_boost_applied += 1
                            logger.debug(f"    Achievement boost '{kw}': matched in quantified result")
                            break
...
class DataEnricher:
    ...
    v15.70: Receives enricher_config via dependency injection.
    """
...
class ArtistGenerator:
    ...
    def __init__(self, master_resume: Dict, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis, artist_specs: Dict, artist_config: ArtistConfig, content_constraints: ContentConstraintsConfig, **kwargs):
        ...
        self.constraints = content_constraints  # Injected config
        self.artist_config = artist_config  # Injected config
        ...
        # Convert string keys to ResumeSection enums for artist_config dictionaries
    ...
    def _convert_config_keys_to_enums(self, config_dict: Dict) -> Dict:
        """Convert string keys to ResumeSection enums."""
        result = {}
...
"""
    ...
    def _build_context_narrative(self, spec: Dict, section_enum: Optional[ResumeSection] = None) -> Dict:
        ...
        # Now use the section_enum argument directly instead of extracting from extra_args
    ...
    def _post_process_narrative(self, narrative_text: str, section_enum: ResumeSection) -> str:
        ...
        spec = self.SECTION_GENERATION_SPECS[section_enum]
        context = self._build_context_narrative(spec, section_enum=section_enum) # Rebuild context to get constraints
    ...
    def _generate_section_generic(self, spec: Dict, section_enum: ResumeSection, temperature_override: Optional[float]) -> Tuple[Any, int]:
        ...
        if "context_builder" in spec and spec["context_builder"]:
            ...
            if builder_method:
                 # Pass section_enum to the context builder
...
class ValidationContext:
    ...
    def _calculate_k1_differentiator_range(self) -> Dict:
    ...
    def _calculate_cover_letter_narrative(self) -> Dict:
        """Checks for the presence of 'Hook', 'Proof', and 'Vision' elements in the cover letter."""
...
class PreFlightValidator:
    ...
    def __init__(self, master_resume: Dict, validator_config: ValidatorConfig, content_constraints: ContentConstraintsConfig, signal_config: SignalControlConfig):
        """Initializes the validator and registers all rules with the ValidationEngine."""
        self.master_resume = master_resume
        self.engine = ValidationEngine()
        self.constraints = content_constraints  # Injected config
        self.signal_constraints = signal_config  # Injected config
        self.validator_config = validator_config  # Injected config
        
        # Convert string keys to ResumeSection enums for validator_config dictionaries
        ...
        # Convert string-based sets/dicts to enum-based
        ...
        # SECTION_SIGNAL_TARGETS_CONFIG for per-section signal score validation
        # Format: {label: (section_enum, target_min_raw, target_max_raw, _, _)}
    ...
    RULES_CONFIG = [
        ...
        _mk_method("H3_GLOBAL_PER_SECTION_SIGNAL_SCORE", ValidationSeverity.HIGH, "content", "_validate_per_section_signal_raw", lambda ctx: f"One or more sections outside target raw signal score range: {ctx._cache.get('VG_PER_SECTION_SIGNAL_SCORE', {}).get('failures', 'N/A')}"),
        {
            ...
            "validator": lambda ctx: ctx.constraints.K1_MIN_DIFFERENTIATORS <= ctx._calculate_k1_differentiator_range()['found'] <= ctx.signal_constraints.K1_MAX_DIFFERENTIATORS,
        ...
        _mk_method("H3_K11_COVER_LETTER_NARRATIVE_INTEGRITY", ValidationSeverity.HIGH, "content", lambda ctx: ctx._calculate_cover_letter_narrative()['valid'], lambda ctx: f"K.11 Cover letter may be missing narrative integrity. Hook: {ctx._cache.get('COVER_LETTER_NARRATIVE_INTEGRITY', {}).get('hook', '?')}, Proof: {ctx._cache.get('COVER_LETTER_NARRATIVE_INTEGRITY', {}).get('proof', '?')}, Vision: {ctx._cache.get('COVER_LETTER_NARRATIVE_INTEGRITY', {}).get('vision', '?')}"),
        {
...
# Inside class FileRenderer:
    ...
    def _render_app_tracker_artifact(self, file_paths: Dict[str, str], jd_url: str = "") -> Tuple[str, str, List[ValidationResult]]:
        ...
        validation_results = []
        try:
            # v15.70: Pass config to AppTrackerQAValidator
...
class WorkflowOrchestrator:
    ...
    def __init__(self, master_resume: Dict, config: AppConfig, test_mode: bool = False):
        ...
        Args:
            ...
            config: Application configuration (v15.70: centralized config)
            test_mode: If True, skips file handler creation (for testing)
        """
        ...
        self.master_resume = master_resume
        self.config = config  # v15.70: Store centralized config
        ...
        self.hash_chain = []
        self.constraints = config.content_constraints  # v15.70: Use config
    ...
    def _execute_hop_0_jd_analysis(self, job_description: str) -> Tuple[ThematicAnalysis, int]:
        ...
        try:
            ...
            # Save HOP -0.5 output as JSON (RAGMission from pre-RAG analysis)
    ...
    def _execute_hop_2_enrichment(self, extracted_data: Dict, thematic_analysis: ThematicAnalysis) -> Dict:
        ...
        enriched_scaffold = {}
        try:
            # v15.70: Pass config to DataEnricher
    ...
    def _execute_hop_3_artist_generation(self, enriched_scaffold: Dict, job_description: str, thematic_analysis: ThematicAnalysis) -> Tuple[Dict, int]:
        ...
        try:
            # v15.70: Pass config to ArtistGenerator
            artist = ArtistGenerator(
                ...
                artist_config=self.config.artist,  # v15.70: Inject config
                content_constraints=self.config.content_constraints  # v15.70: Inject config
            )
            # v15.70: Pass config to PreFlightValidator
    ...
    def _execute_hop_4_staging_and_sanitization(self, artist_output: Dict) -> ImmutableStagingBuffer:
        ...
        try:
            ...
            # Save HOP-4 output as JSON (before sanitization)
        ...
        try:
            ...
            # Save HOP-4.5 output as JSON (after sanitization and locking)
    ...
    def _create_jd_analyzer(self) -> EnhancedJobDescriptionAnalyzer:
        # v15.70: Use config from self.config
    ...
    def _save_hop_output_json(
        ...
        try:
            ...
            # Custom serializer for complex types (reusing pattern from _create_checkpoint)
            def default_serializer(o):
                if is_dataclass(o):
                    return asdict(o)
                    ...
                    return o.data  # Extract internal data dict
            ...
            # Do NOT halt workflow - saving is for debugging/visibility only
...
"""
    ...
    # v15.70: Pass global CONFIG to orchestrator
    orchestrator = WorkflowOrchestrator(MASTER_RESUME_DATA, config=CONFIG, test_mode=False)
