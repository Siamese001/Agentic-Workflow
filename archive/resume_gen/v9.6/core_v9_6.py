...
# v9.6: Pointing CONFIG_PATH to master_config_v9_6.json
#
# v9.0: Added BulletCritiqueAgent prompts.
...
# Load Configuration
CONFIG_PATH = PROJECT_ROOT / "master_config_v9_6.json"

class Configuration:
    """
    Central configuration class.
    Reads all settings from master_config_v9_6.json.
    ...
    def __init__(self, config_path: Optional[Path] = None, logger=None):
        ...
        self.caching_config = getattr(self, 'caching_config', self._get_default_caching_config()) # v9.5
    ...
    def _get_default_caching_config(self):
        """v9.5: Default caching config if missing"""
        ns = type('Namespace', (), {})()
        ns.enable_semantic_caching = False
        ns.cache_db = 1
        ns.cache_ttl_sec = 3600
        return ns
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            if not self.config_path.exists():
                core_logger.critical(f"FATAL: {self.config_path.name} not found at {self.config_path}.")
                raise FileNotFoundError(f"{self.config_path.name} not found at {self.config_path}.")
...
# ============================================================================
# V9.6: STUB IMPLEMENTATIONS FOR BASEAGENT, TRACER, CACHE
# (MOVED TO CORRECT LOCATION TO FIX NameError)
# ============================================================================

# v9.6: Initialize OpenTelemetry tracer
...
class CacheManager:
    """v9.6: Redis-based cache manager for semantic caching"""
    def __init__(self):
        self.enabled = False
        self.redis_client = None
        try:
            if CONFIG.caching_config.enable_semantic_caching:
                ...
                self.enabled = True
                self.ttl = CONFIG.caching_config.cache_ttl_sec
                core_logger.info("CacheManager initialized with Redis")
        except Exception as e:
            core_logger.warning(f"CacheManager init failed: {e}")
            self.enabled = False
...
class BaseAgent:
    """v9.6: Base class for all agents with logging and telemetry support"""
    def __init__(self, blackboard: 'WorkflowBlackboard', debug_mode: bool = False):
...
# v9.5: Stub get_model_client (to fix import) and implement Caching (Item #4)
def get_model_client(client_name: str, model_name: str, **kwargs):
    """
    Simulated factory for getting an LLM client.
    v9.5: Implements Redis-based semantic caching.
    """
    # This is a stub implementation. In a real app, this would
    # return an actual client (e.g., OpenAI(), Gemini(), etc.)
    
    class MockLLMClient:
        def __init__(self, model_name: str):
            self.model_name = model_name
            self.cache = get_cache_manager()
            core_logger.info(f"MockLLMClient initialized for {self.model_name}")

        def chat_completion(self, messages: List[Dict], response_format: str = "text", max_tokens: int = 2048):
            """Simulates a chat completion call with v9.5 caching."""
            
            cache_key = ""
            if self.cache.enabled:
                # Create a hash of the prompt for the cache key
                prompt_str = json.dumps(messages, sort_keys=True)
                cache_key = f"cache:{hashlib.md5(prompt_str.encode()).hexdigest()}"
                
                cached_response = self.cache.get(cache_key)
                if cached_response:
                    core_logger.info(f"CACHE HIT for {self.model_name}")
                    return json.loads(cached_response)
            
            core_logger.info(f"CACHE MISS for {self.model_name}. Calling mock LLM.")
            
            # Simulate a response based on format
            if response_format == "json_object":
                # Simulate a JSON response
                response_data = {"simulated": "json_response", "model": self.model_name}
                
                # Special simulation for BulletCritiqueAgent
                if "BulletCritiqueAgent" in str(messages):
                    response_data = {"passed": True, "critique": "Looks good."}
            else:
                # Simulate a text response
                response_data = f"Simulated text response from {self.model_name}"

            if self.cache.enabled:
                self.cache.set(cache_key, json.dumps(response_data))

            return response_data

    # Return a new instance of the mock client
    return MockLLMClient(model_name=model_name)


# v9.5: OpenTelemetry Configuration (Item #5)
def setup_telemetry():
    """Configures OpenTelemetry based on config."""
    if CONFIG.tracing_config.langsmith_enabled:
        core_logger.info("Telemetry enabled.")
        # This is where Jaeger/LangSmith exporters would be set up.
        # For now, it's handled by the global tracer setup.
    else:
        core_logger.info("Telemetry disabled.")
...
class ResumeSection:
    ...
    def __getattr__(self, name: str) -> str:
        ...
        if hasattr(self, '_sections') and self._sections and hasattr(self._sections, name):
...
# ============================================================================
# PART 5: V7.5 LANGGRAPH STATE
# ============================================================================

class GraphState(TypedDict):
    """
    The state object for the v7.5 LangGraph.
    ...
    # --- v7.5 HIL & Learning State ---
    original_draft: str  # The draft produced by the AI
    human_approved_draft: str  # The draft after HIL review
    preference_insight: Optional[Dict[str, Any]]  # The learned insight
    
    # --- v9.5 Provenance (Item #2) ---
    provenance_ledger: List[Dict[str, Any]]

class MetaGraphState(TypedDict):
    """
    v9.6: The state object for the MetaLearningLoop Graph.
    """
    raw_logs: Dict[str, str]
    log_summary: Dict[str, List]
    patterns: List[Dict]
...
def _get_prompt_template(key: str) -> str:
    ...
    if not hasattr(PROMPT_TEMPLATES, key):
        ...
        raise KeyError(f"Prompt template key '{key}' not found in {CONFIG_PATH.name}!")
    
    template = getattr(PROMPT_TEMPLATES, key)
    
    if not template or not isinstance(template, str):
        core_logger.error(f"Prompt template key '{key}' is empty or invalid in config!")
        raise ValueError(f"Prompt template '{key}' is empty or invalid in {CONFIG_PATH.name}!")
...
# ============================================================================
# PART 6: PROMPT CONSTANTS (v8.0)
# ============================================================================
# Load prompts from CONFIG for easy import by agents
# (This was moved here from the end of the file to fix the structure)

# v7.5 Prompts
STRATEGY_THEME_CLASSIFICATION_SYSTEM_PROMPT = CONFIG.prompts.get("strategy_theme_classification_system_prompt", "")
STRATEGY_THEME_CLASSIFICATION_USER_PROMPT = CONFIG.prompts.get("strategy_theme_classification_user_prompt", "")
RAG_QUERY_GEN_SYSTEM_PROMPT = CONFIG.prompts.get("rag_query_gen_system_prompt", "")
RAG_QUERY_GEN_USER_PROMPT = CONFIG.prompts.get("rag_query_gen_user_prompt", "")
RAG_CRITIQUE_SYSTEM_PROMPT = CONFIG.prompts.get("rag_critique_system_prompt", "")
RAG_CRITIQUE_USER_PROMPT = CONFIG.prompts.get("rag_critique_user_prompt", "")
DRAFTING_STRATEGIST_SYSTEM_PROMPT = CONFIG.prompts.get("drafting_strategist_system_prompt", "")
DRAFTING_REDTEAM_SYSTEM_PROMPT = CONFIG.prompts.get("drafting_redteam_system_prompt", "")
DRAFTING_REFINER_SYSTEM_PROMPT = CONFIG.prompts.get("drafting_refiner_system_prompt", "")
DRAFTING_USER_PROMPT = CONFIG.prompts.get("drafting_prompt", "")
REPLANNER_SYSTEM_PROMPT = CONFIG.prompts.get("replanner_system_prompt", "")
REPLANNER_USER_PROMPT = CONFIG.prompts.get("replanner_user_prompt", "")
QA_CLAIM_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_claim_validator_system_prompt", "")
QA_TONE_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_tone_validator_system_prompt", "")
QA_ALIGNMENT_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_alignment_validator_system_prompt", "")
QA_ENTAILMENT_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_entailment_validator_system_prompt", "")
QA_NARRATIVE_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_narrative_validator_system_prompt", "")
QA_ADVERSARIAL_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_adversarial_validator_system_prompt", "")
QA_JD_SKILLS_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_jd_skills_validator_system_prompt", "")
QA_SIGNAL_SCORE_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_signal_score_validator_system_prompt", "")
QA_BIAS_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_bias_validator_system_prompt", "")
QA_TENURE_VALIDATOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_tenure_validator_system_prompt", "")
QA_GENERIC_USER_PROMPT = CONFIG.prompts.get("qa_generic_user_prompt", "")

# v8.0 New Prompts
RAG_THOUGHT_SYSTEM_PROMPT = CONFIG.prompts.get("rag_thought_system_prompt", "")
RAG_CRITIQUE_STEP_SYSTEM_PROMPT = CONFIG.prompts.get("rag_critique_step_system_prompt", "")
BULLET_CUSTOMIZER_SYSTEM_PROMPT = CONFIG.prompts.get("bullet_customizer_system_prompt", "")
BULLET_CUSTOMIZER_USER_PROMPT = CONFIG.prompts.get("bullet_customizer_user_prompt", "")
BULLET_SYNTHETIC_SYSTEM_PROMPT = CONFIG.prompts.get("bullet_synthetic_system_prompt", "")
BULLET_SYNTHETIC_USER_PROMPT = CONFIG.prompts.get("bullet_synthetic_user_prompt", "")
BULLET_CRITIQUE_SYSTEM_PROMPT = CONFIG.prompts.get("bullet_critique_system_prompt", "")
BULLET_CRITIQUE_USER_PROMPT = CONFIG.prompts.get("bullet_critique_user_prompt", "")
DRAFTING_CONDUCTOR_SYSTEM_PROMPT = CONFIG.prompts.get("drafting_conductor_system_prompt", "")
DRAFTING_CONDUCTOR_USER_PROMPT = CONFIG.prompts.get("drafting_conductor_user_prompt", "")
DRAFTING_METRICS_SYSTEM_PROMPT = CONFIG.prompts.get("drafting_metrics_system_prompt", "")
QA_CONDUCTOR_SYSTEM_PROMPT = CONFIG.prompts.get("qa_conductor_system_prompt", "")
QA_CONDUCTOR_USER_PROMPT = CONFIG.prompts.get("qa_conductor_user_prompt", "")
QA_MISSED_OPPORTUNITY_SYSTEM_PROMPT = CONFIG.prompts.get("qa_missed_opportunity_system_prompt", "")

# v9.6 New Adversarial Prompts
STRATEGY_CRITIQUE_SYSTEM_PROMPT = CONFIG.prompts.get("strategy_critique_system_prompt", "")
RAG_QUERY_ADVERSARY_SYSTEM_PROMPT = CONFIG.prompts.get("rag_query_adversary_system_prompt", "")
BULLET_SYNTHETIC_FACTCHECK_SYSTEM_PROMPT = CONFIG.prompts.get("bullet_synthetic_factcheck_system_prompt", "")
QA_PLAN_CRITIQUE_SYSTEM_PROMPT = CONFIG.prompts.get("qa_plan_critique_system_prompt", "")
REPLANNER_CRITIQUE_SYSTEM_PROMPT = CONFIG.prompts.get("replanner_critique_system_prompt", "")

# v9.6 Meta Graph Prompts
META_LOG_READER_SYSTEM_PROMPT = CONFIG.prompts.get("meta_log_reader_system_prompt", "")
META_PATTERN_FINDER_SYSTEM_PROMPT = CONFIG.prompts.get("meta_pattern_finder_system_prompt", "")
META_HYPOTHESIS_GENERATOR_SYSTEM_PROMPT = CONFIG.prompts.get("meta_hypothesis_generator_system_prompt", "")
META_PROPOSAL_DRAFTER_SYSTEM_PROMPT = CONFIG.prompts.get("meta_proposal_drafter_system_prompt", "")
META_PROPOSAL_CRITIQUE_SYSTEM_PROMPT = CONFIG.prompts.get("meta_proposal_critique_system_prompt", "")


# Export key functions
__all__ = [
    # v9.6 Additions
    'BaseAgent', 'get_model_client', 'tracer',
    'MetaGraphState', # v9.6
