# File: run_learning_v9_7.py
# Overwrites: run_learning_v9_6.py
...
class PatternFinderAgent(BaseAgent):
    """Finds recurring failure patterns in feedback logs."""
    def __init__(self, blackboard: Dict, debug_mode=False):
        super().__init__(blackboard, debug_mode)
...
class HypothesisGeneratorAgent(BaseAgent):
    """Generates potential root causes for observed patterns."""
    def __init__(self, blackboard: Dict, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.client = get_model_client("google", "gemini-2.0-flash-exp")
...
class ProposalDrafterAgent(BaseAgent):
    """Drafts a specific change proposal from a hypothesis."""
    def __init__(self, blackboard: Dict, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.client = get_model_client("google", "gemini-2.0-flash-exp")
...
class MetaPlannerAgent:
    """Writes the final, approved proposal to disk."""
    def __init__(self):
...
class ProposalCritiqueAgent(BaseAgent):
    """Adversarially reviews the drafted proposal."""
    def __init__(self, blackboard: Dict, debug_mode=False):
        super().__init__(blackboard, debug_mode)
        self.client = get_model_client("google", "gemini-2.0-flash-exp")
...
def run_meta_learning():
    """Runs the asynchronous, agentic meta-learning graph (v9.7)."""
    try:
    ...
    logger.info(f"===== Starting v9.7 Meta-Learning Graph ({datetime.now().isoformat()}) =====")
    
    if not CONFIG.meta_loop_config.enable_meta_learning:
        logger.info("Meta-learning is disabled in master_config_v9_7.json. Exiting.")
        return
    ...
    try:
        ...
        logger.info(f"===== v9.7 Meta-Learning Graph Complete ({workflow_id}) =====")
