...
# Version: 16.11 (Refactored - Corrected)
...
# ==============================================================================
# NOTE: HopExecutionError and StagingBufferError removed from here
# They are now imported from models.py to avoid duplication
# ==============================================================================
    ...
    def _build_experience_sections(self) -> List[Dict]:
        ...
        for exp in self.master_resume.get("professional_experience", []):
            ...
            experience_sections.append({
                ...
                "dates": exp.get("dates", ""),
                "bullets": bullets,
                "overview_narrative": exp.get("overview", "")
            })
...
class DataEnricher:
    """
    Hop 2: Enriches master resume data with thematic analysis from JD.
    
    Responsibilities:
    - Parse and analyze the job description
    - Extract skills, themes, and requirements using RAG
    - Enrich bullets with relevance scoring
    - Create thematic analysis for downstream use
    """

    def __init__(self, job_description: str, extracted_data: Dict, config: AppConfig):
        self.job_description = job_description
        self.extracted_data = extracted_data
        self.config = config
        
        if not job_description or len(job_description.strip()) < 100:
            raise HopExecutionError("Job description is too short or empty (min 100 chars required)")

    def enrich(self) -> Tuple[Dict, ThematicAnalysis, List[ValidationResult]]:
        """
        Enriches the extracted data with JD analysis.
        
        Returns:
            Tuple of (enriched_scaffold, thematic_analysis, validation_results)
        """
        validation_results = []
        
        try:
            logger.info("  Starting JD analysis using RAG...")
            
            # Use the EnhancedJobDescriptionAnalyzer from rag.py
            analyzer = EnhancedJobDescriptionAnalyzer(
                master_resume_data=self.extracted_data,
                config=self.config
            )
            
            thematic_analysis = analyzer.analyze_job_description(self.job_description)
            
            # Validate that we got real data, not mock data
            if thematic_analysis.retrieval_method == "UNKNOWN":
                raise HopExecutionError("Thematic analysis failed: retrieval method is UNKNOWN")
            
            # Create enriched scaffold
            enriched_scaffold = copy.deepcopy(self.extracted_data)
            enriched_scaffold["thematic_analysis"] = thematic_analysis
            
            logger.info(f"  ✓ Thematic analysis complete (method: {thematic_analysis.retrieval_method})")
            
            return enriched_scaffold, thematic_analysis, validation_results
            
        except Exception as e:
            logger.error(f"DataEnricher failed: {e}", exc_info=True)
            raise HopExecutionError(f"DataEnricher failed: {e}")
...
# ==============================================================================
# NOTE: ValidationContext and PreFlightValidator are imported from validation.py
# Duplicate class declarations removed to avoid conflicts
# ==============================================================================
