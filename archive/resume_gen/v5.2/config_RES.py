# File: config_RES.py
# Version: 5.2 Standard (Centralized Configuration)
...
# --- WIZARD PATCH: Load Unified Master Config ---
MASTER_CONFIG_PATH = DATA_DIR / "master_config.json"
MASTER_CONFIG_DATA = {}
if MASTER_CONFIG_PATH.exists():
    try:
        with open(MASTER_CONFIG_PATH, 'r', encoding='utf-8') as f:
            MASTER_CONFIG_DATA = json.load(f)
        logger.info(f"✓ Loaded unified master_config.json")
    except Exception as e:
        logger.error(f"✗ Failed to load master_config.json: {e}")

# Mapping virtual filenames to keys in master_config.json
VIRTUAL_FILE_MAP = {
    "prompts.json": "prompts",
    "artist_specs.json": "artist_specs",
    "validator_rules.json": "validator_rules",
    "app_tracker_schema.json": "app_tracker_schema",
    "constraints.json": "constraints",
    "hyphenation_rules.json": "hyphenation_rules",
    "enricher_rules.json": "enricher_rules"
}
# ------------------------------------------------
...
def _load_json_config(file_path: str, config_name: str, required: bool = True) -> Dict:
    ...
    try:
        ...
        # --- WIZARD PATCH: Fallback to MASTER_CONFIG_DATA ---
        filename = os.path.basename(file_path)
        master_key = VIRTUAL_FILE_MAP.get(filename)
        
        if master_key and master_key in MASTER_CONFIG_DATA:
             logger.info(f"ℹ️ Loaded {config_name} from master_config.json (key: {master_key})")
             return MASTER_CONFIG_DATA[master_key]
             
        # If still not found, perform standard error handling
        if required:
            logger.error(f"✗ Required config file not found: {file_path} (and not in master_config)")
            raise
        else:
...
class ValidatorConfig:
    ...
    def from_json(cls, 
        ...
        # --- WIZARD PATCH: Removed loading of constraints.json. Use hardcoded defaults if not in rules ---
        ...
        return cls(
            ...
            # --- WIZARD PATCH: Load from rules_data, will default to empty if not there (relying on ContentConstraintsConfig) ---
...
# ============================================================================
# CONTENT CONSTRAINTS CONFIGURATION (LOADED FROM MASTER CONFIG)
# ============================================================================
...
class ContentConstraintsConfig:
    ...
    Centrally loaded from master_config.json to ensure a single source of truth.
    """
    ...
    @classmethod
    def from_master_config(cls, master_data: Dict) -> 'ContentConstraintsConfig':
        """Factory to load directly from master_config.json data."""
        c = master_data.get('constraints', {})
        return cls(
            HEADLINE_WORD_COUNT_MIN=c.get('headline', {}).get('word_count_min', 7),
            HEADLINE_WORD_COUNT_MAX=c.get('headline', {}).get('word_count_max', 12),
            HEADLINE_COMPONENT_WORDS_MIN=c.get('headline', {}).get('component_words_min', 2),
            HEADLINE_COMPONENT_WORDS_MAX=c.get('headline', {}).get('component_words_max', 4),
            EXECUTIVE_SUMMARY_WORD_COUNT_MIN=c.get('executive_summary', {}).get('word_count_min', 120),
            EXECUTIVE_SUMMARY_WORD_COUNT_MAX=c.get('executive_summary', {}).get('word_count_max', 150),
            EXECUTIVE_SUMMARY_SENTENCE_COUNT_MIN=c.get('executive_summary', {}).get('sentence_count_min', 7),
            EXECUTIVE_SUMMARY_SENTENCE_COUNT_MAX=c.get('executive_summary', {}).get('sentence_count_max', 9),
            BULLETS_WORD_COUNT_MIN=c.get('bullets', {}).get('word_count_min', 25),
            BULLETS_WORD_COUNT_MAX=c.get('bullets', {}).get('word_count_max', 40),
            BULLETS_WORD_COUNT_ACCEPTABLE_MIN=c.get('bullets', {}).get('word_count_acceptable_min', 21),
            BULLETS_WORD_COUNT_ACCEPTABLE_MAX=c.get('bullets', {}).get('word_count_acceptable_max', 44),
            COMPETENCIES_BULLET_COUNT_MIN=c.get('competencies', {}).get('bullet_count_min', 5),
            COMPETENCIES_BULLET_COUNT_MAX=c.get('competencies', {}).get('bullet_count_max', 7),
            SKILLS_COUNT_MIN=c.get('skills', {}).get('count_min', 8),
            SKILLS_COUNT_MAX=c.get('skills', {}).get('count_max', 12),
            SKILLS_WORD_COUNT_MIN=c.get('skills', {}).get('word_count_min', 1),
            SKILLS_WORD_COUNT_MAX=c.get('skills', {}).get('word_count_max', 3),
            COVER_LETTER_P1_WORD_COUNT_MIN=c.get('cover_letter', {}).get('p1_word_count_min', 80),
            COVER_LETTER_P1_WORD_COUNT_MAX=c.get('cover_letter', {}).get('p1_word_count_max', 120),
            COVER_LETTER_P2_WORD_COUNT_MIN=c.get('cover_letter', {}).get('p2_word_count_min', 90),
            COVER_LETTER_P2_WORD_COUNT_MAX=c.get('cover_letter', {}).get('p2_word_count_max', 130),
            COVER_LETTER_P3_WORD_COUNT_MIN=c.get('cover_letter', {}).get('p3_word_count_min', 80),
            COVER_LETTER_P3_WORD_COUNT_MAX=c.get('cover_letter', {}).get('p3_word_count_max', 120),
            COVER_LETTER_JD_RELEVANCE_THRESHOLD=c.get('cover_letter', {}).get('jd_relevance_threshold', 0.7),
            TOTAL_RESUME_WORD_COUNT_MIN=c.get('total_resume', {}).get('word_count_min', 900),
            TOTAL_RESUME_WORD_COUNT_MAX=c.get('total_resume', {}).get('word_count_max', 1200),
            UNIFY_OVERVIEW_WORD_COUNT_MIN=c.get('section_specific', {}).get('unify_overview', {}).get('word_count_min', 25),
            UNIFY_OVERVIEW_WORD_COUNT_MAX=c.get('section_specific', {}).get('unify_overview', {}).get('word_count_max', 40),
            IBM_OVERVIEW_WORD_COUNT_MIN=c.get('section_specific', {}).get('ibm_overview', {}).get('word_count_min', 25),
            IBM_OVERVIEW_WORD_COUNT_MAX=c.get('section_specific', {}).get('ibm_overview', {}).get('word_count_max', 40),
            TRADERSENSE_NARRATIVE_WORD_COUNT_MIN=c.get('section_specific', {}).get('tradersense_narrative', {}).get('word_count_min', 50),
            TRADERSENSE_NARRATIVE_WORD_COUNT_MAX=c.get('section_specific', {}).get('tradersense_narrative', {}).get('word_count_max', 70),
            EY_NARRATIVE_WORD_COUNT_MIN=c.get('section_specific', {}).get('ey_narrative', {}).get('word_count_min', 60),
            EY_NARRATIVE_WORD_COUNT_MAX=c.get('section_specific', {}).get('ey_narrative', {}).get('word_count_max', 80),
            EARLY_CAREER_NARRATIVE_WORD_COUNT_MIN=c.get('section_specific', {}).get('early_career_narrative', {}).get('word_count_min', 50),
            EARLY_CAREER_NARRATIVE_WORD_COUNT_MAX=c.get('section_specific', {}).get('early_career_narrative', {}).get('word_count_max', 70),
            K1_MIN_DIFFERENTIATORS=c.get('generation_requirements', {}).get('k1_min_differentiators', 4),
            MIN_JD_KEYWORDS=c.get('generation_requirements', {}).get('min_jd_keywords', 6),
            provenance_split_targets=c.get('provenance_split_targets', {})
        )
...
class Config:
    ...
    constraints: ContentConstraintsConfig = field(
        default_factory=lambda: ContentConstraintsConfig.from_master_config(MASTER_CONFIG_DATA) 
        if MASTER_CONFIG_DATA else ContentConstraintsConfig()
    )
    ...
    artist_specs: Dict = field(default_factory=lambda: _load_json_config(str(DATA_DIR / "artist_specs.json"), "Artist Specs", required=False))
    
    # --- WIZARD PATCH: Signature Template (moved from global to class) ---
    COVER_LETTER_SIGNATURE_TEMPLATE: str = COVER_LETTER_SIGNATURE_TEMPLATE
    # ---------------------------------------------------------------------
