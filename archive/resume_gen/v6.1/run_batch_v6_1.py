# File: run_batch_v6_1.py
# NEW: Batch processing harness for v6.1
# Processes all job_input.json files in batch_queue/ folder
# Writes one-line summaries to batch_summary_v6_1.csv
...
from main_v6_1 import WorkflowV60, setup_logging 
from core_v6_1 import CONFIG, CircuitBreakerOpenError

logger = logging.getLogger("batch_runner_v6_1")
setup_logging(debug_mode=False)

BATCH_QUEUE_DIR = "batch_queue"
BATCH_COMPLETE_DIR = "batch_complete"
SUMMARY_FILE = "batch_summary_v6_1.csv"
...
def run_batch():
    ...
    logger.info(f"v6.1 Batch process starting. Found {len(job_files)} jobs.")
    consecutive_failures = 0
    FAILURE_THRESHOLD = 3  # v6.1: Alerting - halt batch after 3 consecutive failures
    ...
    for job_file in job_files:
        ...
        try:
            ...
            logger.info(f"--- Starting v6.1 job: {company} - {title} ---")

            # --- v6.1: Cost Ceiling Check (Spell #6) - ACTIVATED ---
        ...
        # --- v6.1: Circuit Breaker Catch (Spell #5) ---
