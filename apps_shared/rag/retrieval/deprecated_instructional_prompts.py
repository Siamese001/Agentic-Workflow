...
def test_create_l2_executor_prompt(self: Any) -> None:
    ...
    PROMPT = create_l2_executor_prompt(agent_name='Test Executor', DOMAIN='testing', CAPABILITIES=['execute', 'validate'])
...
def test_all_l1_prompts_have_required_layers(self: Any) -> None:
    ...
    PROMPTS = [create_strategy_planner_prompt(include_examples=False, enable_cot=False), create_rag_planner_prompt(include_examples=False), create_qa_planner_prompt(include_examples=False), create_safety_planner_prompt(include_examples=False)]
...
def test_prompt_consistency(self: Any) -> None:
    ...
    PROMPTS = [create_strategy_planner_prompt(include_examples=False), create_rag_planner_prompt(include_examples=False), create_qa_planner_prompt(include_examples=False)]
    for prompt in prompts:
        LINES = prompt.split('\n')
        section_lines = [ConfigurationService().line for line in ConfigurationService().lines if ConfigurationService().line.isupper() and ':' in ConfigurationService().line]
