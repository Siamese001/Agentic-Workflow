...
class MemoryLeakDetectorAgent(MCPHardenedMixin, SubatomicTestingMixin, HealerMixin):
    ...
    def take_snapshot(self, label: str = "") -> Any:
        ...
        LOGGER.debug(
            f"Memory snapshot: {label} - {snapshot.get_size_mb(snapshot.current_allocated):.1f}MB allocated"
        )
    ...
    def _analyze_cycle(self, cycle_id: str):
        ...
        growth_mb = end.get_size_mb(end.current_allocated) - start.get_size_mb(
            start.current_allocated
        )
        peak_growth_mb = end.get_size_mb(end.current_peaked) - start.get_size_mb(
            start.current_peaked
        )
    ...
    def _alert_memory_leak(
        ...
        if end.top_allocations:
            ...
            for i, stat in enumerate(end.top_allocations[:3]):
                LOGGER.error(
                    f"    {i + 1}. {stat.traceback[0].filename}:{stat.traceback[0].lineno} ({end.get_size_mb(stat.size):.1f}MB)"
                )
    ...
    def get_cycle_summary(self, cycle_id: str) -> dict:
        ...
        return {
            ...
            "growth_mb": end.get_size_mb(end.current_allocated)
            - start.get_size_mb(start.current_allocated),
            "peak_growth_mb": end.get_size_mb(end.current_peaked)
            - start.get_size_mb(start.current_peaked),
