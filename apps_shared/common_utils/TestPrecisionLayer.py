    ...
    def test_adaptive_retrieval_gate(self):
        ...
        for query, expected_type, expected_retrieve in self.test_queries:
            ...
            if type_correct and retrieve_correct:
                ...
                if not type_correct:
                    print(f"   ⚠️  Type mismatch: expected {expected_type}, got {decision.query_type}")
                if not retrieve_correct:
                    print(f"   ⚠️  Retrieval mismatch: expected {expected_retrieve}, got {decision.should_retrieve}")
    ...
    def test_integration_scenario(self):
        ...
        for msg in conversation:
            if msg['role'] == 'user':
                ...
                print(f"  '{msg['content']}' -> {'RETRIEVE' if decision.should_retrieve else 'NO RETRIEVAL'}")
                print(f"    Reason: {decision.reason}")
                ...
                if decision.should_retrieve:
                    ...
                    print(f"    Retrieved chunks compressed by {(1-result.compression_ratio):.1%}")
