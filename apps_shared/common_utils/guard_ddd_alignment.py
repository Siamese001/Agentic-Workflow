...
def get_ddd_violations_detailed(root_path: str) -> list[dict]:
    ...
    for py_file in python_files:
        ...
        if any(
            ...
            for skip in [TESTS_DIR, "migrations", "__pycache__", ".venv", "venv"]
        ...
        try:
            ...
            )  # [ROBUSTNESS] Ignore encoding errors
        ...
        for node in ast.walk(tree):
            ...
            if (
                ...
                setters_or_mutators = [
                    m
                    for m in methods
                    if m.name.startswith(("set_", "update_", "add_", "remove_"))
                    or "mutat" in m.name.lower()
                ]
