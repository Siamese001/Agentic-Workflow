        ...
        if isinstance(node, ast.ClassDef):
            ...
            for item in node.body:
                if isinstance(item, ast.FunctionDef):
                    ...
                    class_methods[node.name].append(ClassMethod(
                        name=item.name,
                        lineno=item.lineno,
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef):
            ...
            if node.name in SUSPICIOUS_NAMES or node.name.startswith('_'):
                ...
                orphans.append(OrphanedFunction(
                    name=node.name,
                    lineno=node.lineno,
    ...
    for orphan in orphans:
        ...
        if matching_method:
            ...
            if orphan_normalized == method_normalized:
                ...
                details_list.append(f"Orphan `{orphan.name}` at line {orphan.lineno} is identical to class method at line {matching_method.lineno}")
            else:
