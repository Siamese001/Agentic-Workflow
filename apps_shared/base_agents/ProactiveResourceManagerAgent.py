...
class ProactiveResourceManagerAgent(MCPHardenedMixin, SubatomicTestingMixin, HealerMixin):
    ...
    def _check_budget_constraints(self, file_path: str) -> tuple[bool, str]:
        ...
        if self.global_healing_count >= self.thresholds.global_healing_budget:
            return (
                False,
                f"Global healing budget exhausted ({self.global_healing_count}/{self.thresholds.global_healing_budget})",
            )
        file_count: Any = self.file_healing_counts.get(file_path, 0)
        if file_count >= self.thresholds.max_healing_per_file:
            return (
                False,
                f"Per-file healing limit reached ({file_count}/{self.thresholds.max_healing_per_file})",
            )
        if self.active_healings >= self.thresholds.max_concurrent_heals:
            return (
                False,
                f"Max concurrent healings reached ({self.active_healings}/{self.thresholds.max_concurrent_heals})",
            )
        return (True, "OK")
    ...
    def _check_priority_and_success_rate(self, violation_key: int) -> tuple[bool, str]:
        ...
        if budget_utilization >= self.thresholds.budget_critical_threshold:
            ...
            if priority < 0.8:
                return (
                    False,
                    f"Budget critical ({budget_utilization:.1%}), only high-priority violations allowed",
                )
    ...
    def record_healing_attempt(
        self, file_path: str, violation_key: int, success: bool, rounds_taken: int
    ) -> Any:
        """
        ...
        if success:
            Logger.info(
                f"Healing success: {os.path.basename(file_path)} (Key {violation_key}, {rounds_taken} rounds)"
            )
        else:
    ...
    def _check_and_adjust_thresholds(self) -> Any:
        ...
        if recent_success_rate > 0.8:
            ...
            if self.thresholds.global_healing_budget != old_budget:
                Logger.info(
                    f"Increased global budget: {old_budget} → {self.thresholds.global_healing_budget}"
                )
            ...
            if self.thresholds.global_healing_budget != old_budget:
                Logger.warning(
                    f"Decreased global budget: {old_budget} → {self.thresholds.global_healing_budget}"
                )
    ...
    def add_to_queue(self, file_path: str, violation_key: int, violation_details: str) -> Any:
        ...
        Logger.debug(
            f"Added to queue: {os.path.basename(file_path)} (Key {violation_key}, Priority {priority:.2f})"
        )
            ...
            try:
                ...
                if len(self.success_history) >= 10:
                    ...
                    if success_rate > 0.85 and avg_rounds < 2:
                        new_budget: Any = min(
                            self.base_global_budget * 2, self.current_global_budget + 10
                        )
                    ...
                    if new_budget != old_budget:
                        ...
                        Logger.info(
                            f"L2 MUTATION: Budget adjusted {old_budget} → {new_budget} (success_rate={success_rate:.1%})"
                        )
                if self.l1_learner:
                    try:
                        rec: Any = None
                        if rec and rec.get("confidence", 0) > 0.5:
                            if rec["mutation"] == "increase_critical_budget":
                                ...
                                Logger.info(
                                    f"L2: Applied L1 Recommendation: +{increase} budget (confidence={rec['confidence']:.1%})"
                                )
                            elif rec["mutation"] == "decrease_budget":
                                decrease: Any = rec.get("suggested_global_decrease", 5)
                                safety_floor: Any = int(self.base_global_budget * 0.2)
                                old_budget: Any = self.current_global_budget
                                self.current_global_budget = max(
                                    safety_floor, self.current_global_budget - decrease
                                )
                                self.thresholds.global_healing_budget = self.current_global_budget
                                Logger.info(
                                    f"L2: Applied L1 Recommendation: -{decrease} budget (confidence={rec['confidence']:.1%})"
                                )
    ...
    async def persist_mutation_state(self) -> Any:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                "w", delete=False, dir=state_path.parent, encoding="utf-8"
            ) as tf:
    ...
    def get_recommendations(self) -> list[str]:
        ...
        if status["budget_utilization"] > 0.9:
            recommendations.append(
                "CRITICAL: Healing budget nearly exhausted. Consider increasing budget or focusing on high-priority violations."
            )
        if status["success_rate"] < 0.4 and len(self.success_history) >= 10:
            recommendations.append(
                "WARNING: Low healing success rate. Review healing strategies or reduce complexity of fixes."
            )
        if status["queue_length"] > 20:
            recommendations.append(
                "INFO: Large healing queue. Consider parallel healing or increasing max_concurrent_heals."
            )
        if status["active_healings"] >= self.thresholds.max_concurrent_heals:
            recommendations.append(
                "INFO: Max concurrent healings reached. Queue is processing at capacity."
            )
...
def create_proactive_resource_manager(
    thresholds: ResourceThreshold | None = None,
