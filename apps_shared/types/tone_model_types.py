...
class ToneAnalyzer:
    ...
    def __init__(self, min_sample_length: int=50):
        ...
        self.tone_keywords = {ToneType.AUTHORITATIVE: ['expert', 'leader', 'strategy', 'vision', 'decisive', 'results', 'execution', 'accountability', 'ownership', 'leadership'], ToneType.EMPATHETIC: ['understand', 'support', 'team', 'people', 'culture', 'values', 'together', 'collaborate', 'community', 'care'], ToneType.ANALYTICAL: ['data', 'analysis', 'metrics', 'research', 'study', 'findings', 'evidence', 'statistics', 'trends', 'insights'], ToneType.ENTHUSIASTIC: ['excited', 'amazing', 'awesome', 'love', 'fantastic', 'incredible', 'thrilled', 'passionate', 'energy', 'opportunity'], ToneType.DIRECT: ['action', 'implement', 'execute', 'deliver', 'achieve', 'complete', 'done', 'focus', 'priority', 'result']}
    ...
    def analyze_style(self, content_samples: list[str]) -> StyleProfile:
        ...
        try:
            ...
            if len(combined_text) < self.min_sample_length:
                logger.warning(f'Insufficient content length ({len(combined_text)}), returning neutral profile')
                return self._get_neutral_profile()
            ...
            profile = StyleProfile(primary_tone=primary_tone, formality_level=formality, emoji_frequency=emoji_freq, sentence_length_avg=metrics['avg_sentence_length'], vocabulary_complexity=vocab_complexity, confidence_level=metrics['confidence'])
            logger.info(f"Analyzed tone: {primary_tone.value} (confidence: {metrics['confidence']:.2f})", extra={'tone': primary_tone.value, 'confidence': metrics['confidence']})
            return profile
    ...
    def _get_neutral_profile(self) -> StyleProfile:
        ...
        return StyleProfile(primary_tone=ToneType.AUTHORITATIVE, formality_level=0.7, emoji_frequency=0.1, sentence_length_avg=15, vocabulary_complexity=0.5, confidence_level=0.3)
    ...
    def _calculate_metrics(self, text: str) -> dict[str, float]:
        ...
        try:
            ...
            return {'avg_sentence_length': avg_sentence_length, 'exclamation_ratio': exclamation_ratio, 'question_ratio': question_ratio, 'confidence': confidence}
        # guardian: allow-silent-swallow
        except Exception as e:
...
class ToneAdapter:
    ...
    def __init__(self):
        ...
        self.adaptation_rules = {ToneType.DIRECT: {'remove_patterns': ['\\b(?:in order to|in an effort to|for the purpose of)\\b', '\\b(?:very|quite|rather|extremely)\\s+\\w+', '\\b(?:it is important to note that|it should be mentioned that)\\b'], 'replacements': {'utilize': 'use', 'facilitate': 'help', 'leverage': 'use', 'optimize': 'improve', 'implement': 'do', 'strategize': 'plan'}}, ToneType.ENTHUSIASTIC: {'add_transitions': ['Absolutely!', 'Fantastic!', "What's great is", 'Excited to'], 'emoji_places': ['greeting', 'closing']}, ToneType.ANALYTICAL: {'add_transitions': ['Based on the data,', 'The evidence suggests,', 'Analysis indicates,'], 'require_evidence': True}, ToneType.EMPATHETIC: {'add_openers': ['I understand that', 'Recognizing that', 'Appreciating that'], 'soften_language': True}}
...
class ToneModel:
    ...
    def __init__(self):
        ...
        # guardian: allow-magic-config
        self.config_templates = {ToneType.AUTHORITATIVE: GenerationConfig(system_prompt_fragment='Use confident, expert language. Lead with insights. Be decisive and clear.', temperature_setting=0.4, banned_phrases=['maybe', 'perhaps', 'might', 'could', 'I think'], preferred_transitions=['Therefore', 'Consequently', 'Based on'], max_sentence_length=20), ToneType.EMPATHETIC: GenerationConfig(system_prompt_fragment='Use supportive, understanding language. Focus on people and values. Show genuine care.', temperature_setting=0.6, banned_phrases=['must', 'require', 'mandatory', 'failure'], preferred_transitions=['Understanding that', 'Recognizing that', 'Appreciating that'], max_sentence_length=18), ToneType.ANALYTICAL: GenerationConfig(system_prompt_fragment='Use data-driven, logical language. Provide evidence and reasoning. Be thorough.', temperature_setting=0.3, banned_phrases=['feel', 'believe', 'intuition', 'gut'], preferred_transitions=['According to', 'Based on', 'Analysis shows'], max_sentence_length=25), ToneType.ENTHUSIASTIC: GenerationConfig(system_prompt_fragment='Use energetic, positive language. Express excitement and possibility. Be engaging.', temperature_setting=0.7, banned_phrases=['problem', 'issue', 'challenge', 'difficult'], preferred_transitions=['Excited to', 'Thrilled about', "What's great is"], max_sentence_length=15), ToneType.DIRECT: GenerationConfig(system_prompt_fragment='Use concise, action-oriented language. Be clear and specific. No fluff.', temperature_setting=0.3, banned_phrases=['delve', 'tapestry', 'journey', 'utilize', 'facilitate'], preferred_transitions=['Next', 'Then', 'After that'], max_sentence_length=12)}
    ...
    def analyze_and_configure(self, content_samples: list[str], archetype: str | None=None) -> tuple[StyleProfile, GenerationConfig]:
        ...
        try:
            ...
            config = self.config_templates.get(profile.primary_tone, self.config_templates[ToneType.AUTHORITATIVE])
