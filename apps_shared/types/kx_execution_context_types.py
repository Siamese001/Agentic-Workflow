...
class KXNodeExecutor:
    ...
    def execute_node(self, node_key: str, context: KXExecutionContext, system_prompt: str | None=None) -> KXExecutionResult:
        """Execute a K.X node.
        ...
        with create_span(f'kx_node.{config.node_id}.{config.element}'):
            ...
            return KXExecutionResult(node_id=config.node_id, element=config.element, content=response.content, reasoning_trace=self._extract_reasoning_trace(response), rag_sources=rag_sources, validation_results=validation_results, usage=response.usage, metadata={'reasoning_strategy': config.reasoning_strategy.value, 'rag_enabled': config.rag_config.enabled if config.rag_config else False, 'validation_passed': all((v.get('passed', False) for v in validation_results))})
    ...
    def _execute_rag(self, config: KNodeConfig, context: KXExecutionContext) -> list[dict[str, Any]]:
        ...
        try:
            collection = create_chroma_collection(context.vector_store, context.metadata.get('collection_name', 'knowledge_base'))
            results = search_vectors_chroma(collection, query_embeddings=[query_embedding], n_results=config.rag_config.max_retrievers)
            sources = []
            if results and 'documents' in results:
                for i, doc in enumerate(results['documents'][0]):
                    ...
                    sources.append({'document': doc, 'metadata': results.get('metadatas', [[{}]])[0][i], 'distance': results.get('distances', [[0]])[0][i], 'weight': weight, 'weighted_score': weight / (1 + results.get('distances', [[0]])[0][i])})
    ...
    def _build_messages(self, config: KNodeConfig, context: KXExecutionContext, rag_sources: list[dict[str, Any]]) -> list[AgentMessage]:
        ...
        messages = []
        if rag_sources:
            rag_context = '\n\n'.join([f"Source {i + 1} ({src['metadata'].get('source_type', 'unknown')}):\n{src['document']}" for i, src in enumerate(rag_sources[:3])])
            messages.append(AgentMessage(role='user', content=f'Context from knowledge base:\n\n{rag_context}'))
    ...
    def _build_generation_prompt(self, config: KNodeConfig, context: KXExecutionContext) -> str:
        ...
        if context.source_data:
            source_info = '\n'.join([f'{key}: {value}' for key, value in context.source_data.items() if key != 'query'])
            if source_info:
                prompt_parts.append(f'\nSource Data:\n{source_info}')
    ...
    def _build_system_prompt(self, config: KNodeConfig, context: KXExecutionContext) -> str:
        ...
        Returns:
            System prompt
        """
        prompts = [f'You are an expert at generating {config.element}.', 'Follow all constraints and validation rules strictly.']
    ...
    def _validate_output(self, config: KNodeConfig, content: str, context: KXExecutionContext) -> list[dict[str, Any]]:
    ...
    def _apply_validation_rule(self, rule: str, content: str, config: KNodeConfig, context: KXExecutionContext) -> dict[str, Any]:
        ...
        if rule == 'non_empty':
            ...
            return {'rule': rule, 'passed': passed, 'message': f"Word count: {word_count}/{config.max_words or 'unlimited'}", 'word_count': word_count}
        elif rule == 'character_limit' or rule == 'character_limit_strict':
            char_count = len(content)
            passed = True
            if config.max_chars:
                passed = char_count <= config.max_chars
            return {'rule': rule, 'passed': passed, 'message': f"Character count: {char_count}/{config.max_chars or 'unlimited'}", 'char_count': char_count}
        else:
            return {'rule': rule, 'passed': True, 'message': f"Validation rule '{rule}' not implemented (assumed pass)"}

def execute_kx_node(node_key: str, agent_executor: AgentExecutor, source_data: dict[str, Any], vector_store: Any | None=None, cache_client: Any | None=None, engine: str='resume') -> KXExecutionResult:
    ...
    context = KXExecutionContext(node_config=config, agent_executor=agent_executor, vector_store=vector_store, cache_client=cache_client, source_data=source_data)
