...
class GoldenStateEvaluator:
    ...
    def __init__(self, dataset_path: Path | None=None, JudgeEvaluator: JudgeEvaluator | None=None, enable_logging: bool=True):
    ...
    async def evaluate_case(self, case: GoldenCase, output: GoldenOutput) -> EvaluationReport:
        ...
        judge_result = await self.JudgeEvaluator.evaluate(output=output.actual_output, expected=expected_str, context={'Task': case.mission, 'category': case.category})
