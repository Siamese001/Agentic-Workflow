...
class DomainConfig:
    ...
    metric_weights: dict[str, float] = field(default_factory=lambda: {'relevance': 0.3, 'authority': 0.2, 'specificity': 0.2, 'coherence': 0.2, 'accuracy': 0.1})
...
class ResumeValidator(DomainValidator):
    ...
    def validate_domain_content(self, content: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        results = {'has_achievements': self._has_achievements(content), 'has_metrics': self._has_metrics(content), 'action_verbs': self._count_action_verbs(content), 'bullet_quality': self._assess_bullet_quality(content), 'completeness': self._assess_completeness(content, context)}
        return results

    def extract_domain_metrics(self, content: str) -> dict[str, float]:
        """Extract resume-specific metrics."""
        return {'achievement_density': self._calculate_achievement_density(content), 'metric_usage': self._calculate_metric_usage(content), 'verb_diversity': self._calculate_verb_diversity(content), 'impact_score': self._calculate_impact_score(content)}

    def _has_achievements(self, content: str) -> bool:
        """Check if content has achievement statements."""
        achievement_patterns = ['\\bincreased\\b', '\\bdecreased\\b', '\\bsaved\\b', '\\bgenerated\\b', '\\breduced\\b', '\\boptimized\\b', '\\blead\\b.*\\bteam\\b']
        import re
    ...
    def _has_metrics(self, content: str) -> bool:
        ...
        import re
        metric_patterns = ['\\d+%', '\\$\\d+(?:,\\d{3})*(?:\\.\\d+)?', '\\d+(?:,\\d{3})*\\s*(?:employees|people|users|customers)', '\\d+(?:,\\d{3})*\\s*(?:hours|days|weeks|months)']
    ...
    def _count_action_verbs(self, content: str) -> int:
        ...
        action_verbs = {'led', 'managed', 'developed', 'created', 'implemented', 'optimized', 'reduced', 'increased', 'improved', 'achieved', 'delivered', 'launched', 'coordinated', 'directed', 'supervised', 'mentored', 'trained'}
    ...
    def _calculate_verb_diversity(self, content: str) -> float:
        ...
        unique_verbs = len({word.lower() for word in content.split() if word in ['led', 'managed', 'developed', 'created']})
        return unique_verbs / max(verbs, 1)

    def _calculate_impact_score(self, content: str) -> float:
        """Calculate overall impact score."""
        return self._calculate_achievement_density(content) * 0.4 + self._calculate_metric_usage(content) * 0.4 + self._assess_bullet_quality(content) * 0.2
...
class OutreachValidator(DomainValidator):
    ...
    def validate_domain_content(self, content: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        results = {'has_personalization': self._has_personalization(content, context), 'has_cta': self._has_call_to_action(content), 'tone_appropriate': self._assess_tone(content, context), 'value_proposition': self._has_value_proposition(content), 'recipient_relevance': self._assess_recipient_relevance(content, context)}
        return results

    def extract_domain_metrics(self, content: str) -> dict[str, float]:
        """Extract outreach-specific metrics."""
        return {'personalization_score': self._calculate_personalization_score(content), 'engagement_potential': self._calculate_engagement_potential(content), 'professionalism': self._calculate_professionalism(content), 'clarity': self._calculate_clarity(content)}
    ...
    def _has_call_to_action(self, content: str) -> bool:
        ...
        cta_phrases = ["let's discuss", 'would love to', 'looking forward to', 'please let me know', 'feel free to', 'would be happy to']
        content_lower = content.lower()
    ...
    def _has_value_proposition(self, content: str) -> bool:
        ...
        value_indicators = ['bring to', 'contribute to', 'help you', 'benefit', 'value', 'expertise', 'experience']
        content_lower = content.lower()
    ...
    def _calculate_engagement_potential(self, content: str) -> float:
        ...
        engaging_words = ['exciting', 'opportunity', 'innovative', 'breakthrough', 'transform', 'revolutionize', 'game-changing']
        content_lower = content.lower()
    ...
    def _calculate_professionalism(self, content: str) -> float:
        ...
        professional_words = ['expertise', 'experience', 'background', 'qualifications', 'accomplished', 'achieved', 'delivered', 'executed']
        content_lower = content.lower()
...
class SharedSignalInfrastructure:
    ...
    def __init__(self):
        ...
        self._validators: dict[EngineType, DomainValidator] = {EngineType.RESUME: ResumeValidator(), EngineType.OUTREACH: OutreachValidator()}
    ...
    def get_enhancer(self, engine_type: EngineType, domain_config: DomainConfig) -> signal_enhancer:
        ...
        if enhancer_key not in self._enhancers:
            enhancer = signal_enhancer(name=f'{engine_type.value}_enhancer', thresholds=domain_config.quality_thresholds)
    ...
    def assess_signal(self, content: str, engine_type: EngineType, domain_config: DomainConfig, context: dict[str, Any] | None=None) -> SignalAssessment:
    ...
    def create_domain_config(self, engine_type: EngineType, custom_thresholds: QualityThresholds | None=None, custom_weights: dict[str, float] | None=None) -> DomainConfig:
        ...
        if engine_type == EngineType.RESUME:
            validation_rules = {'require_achievements': True, 'require_metrics': True, 'min_bullet_points': 3, 'max_bullet_length': 200}
            custom_metrics = ['achievement_density', 'metric_usage', 'verb_diversity', 'impact_score']
        # guardian: allow-config-with-logic
        elif engine_type == EngineType.OUTREACH:
            validation_rules = {'require_personalization': True, 'require_cta': True, 'max_length': 500, 'min_recipient_references': 2}
            custom_metrics = ['personalization_score', 'engagement_potential', 'professionalism', 'clarity']
        else:
            validation_rules = {}
            custom_metrics = []
        return DomainConfig(engine_type=engine_type, quality_thresholds=thresholds, validation_rules=validation_rules, custom_metrics=custom_metrics, metric_weights=custom_weights or {})
    ...
    def get_cross_engine_insights(self) -> dict[str, Any]:
        ...
        insights['shared_patterns'] = {'threshold_comparison': all_thresholds, 'common_flags': self._find_common_flags(), 'quality_correlation': self._analyze_quality_correlation()}
    ...
    def _generate_cross_engine_recommendations(self, insights: dict[str, Any]) -> list[str]:
        ...
        for engine, engine_insights in insights['engines'].items():
            ...
            if avg_quality < 0.7:
                recommendations.append(f'Engine {engine} has low average quality ({avg_quality:.2f}). Consider reviewing domain-specific validation rules.')
        common_flags = insights['shared_patterns'].get('common_flags', {})
        if 'LOW_QUALITY' in common_flags and len(common_flags['LOW_QUALITY']) > 1:
            recommendations.append('Multiple engines experiencing LOW_QUALITY flags. Consider strengthening base validation criteria.')
...
def assess_resume_signal(content: str, context: dict[str, Any] | None=None, strict_mode: bool=True) -> SignalAssessment:
...
def assess_outreach_signal(content: str, context: dict[str, Any] | None=None, strict_mode: bool=True) -> SignalAssessment:
