...
class JudgeEvaluator:
    ...
    def __init__(self, llm_client: Callable[[str], Awaitable[str]] | None=None, criteria: list[JudgmentCriterion] | None=None, pass_threshold: float=0.7, enable_logging: bool=True, model_id: str | None=None, deterministic_anchor_tolerance: float=0.15):
        ...
        if self.enable_logging:
            logger.info('judge_evaluator_initialized', extra={'criteria_count': len(self.criteria), 'pass_threshold': pass_threshold, 'model_id': self.model_id})

    async def evaluate(self, output: str, expected: str | None=None, context: dict[str, Any] | None=None) -> JudgeEvaluationResult:
        ...
        result = JudgeEvaluationResult(overall_score=overall_score, verdicts=verdicts, passed=passed, threshold=self.pass_threshold, summary=summary, metadata={'criteria_count': len(self.criteria), 'output_length': len(output), 'model_id': self.model_id, 'heuristic_anchor': anchor_score, 'anchor_deviation': anchor_deviation, 'anchor_alert': anchor_alert, 'evaluation_path': 'llm' if self.llm_client else 'heuristic'})
        audit_entry = {'ts': time.time(), 'model_id': self.model_id, 'output_hash': hashlib.sha256(output.encode()).hexdigest()[:16], 'overall_score': overall_score, 'heuristic_anchor': anchor_score, 'anchor_deviation': anchor_deviation, 'anchor_alert': anchor_alert, 'passed': passed, 'evaluation_path': 'llm' if self.llm_client else 'heuristic'}
        self._audit_log.append(audit_entry)
        if self.enable_logging:
            if anchor_alert:
                logger.warning('judge_anchor_deviation', extra={'overall_score': overall_score, 'anchor_score': anchor_score, 'deviation': anchor_deviation, 'tolerance': self.deterministic_anchor_tolerance})
            logger.info('evaluation_completed', extra={'overall_score': overall_score, 'passed': passed, 'failing_criteria': [c.value for c in result.get_failing_criteria()], 'model_id': self.model_id, 'anchor_alert': anchor_alert})
        return result

    async def _evaluate_criterion(self, output: str, expected: str | None, context: dict[str, Any] | None, criterion: JudgmentCriterion) -> JudgeVerdict:
        ...
        if self.llm_client:
            ...
            # guardian: allow-silent-swallow
                ...
                if self.enable_logging:
                    logger.error('llm_evaluation_failed', extra={'criterion': criterion.value, 'error': str(e), 'model_id': self.model_id}, exc_info=True)
    ...
    def _build_evaluation_prompt(self, output: str, expected: str | None, context: dict[str, Any] | None, criterion: JudgmentCriterion) -> str:
    ...
    def _heuristic_evaluation(self, output: str, expected: str | None, criterion: JudgmentCriterion) -> JudgeVerdict:
...
# guardian: allow-magic-config
def create_judge_evaluator(llm_client: Callable[[str], Awaitable[str]] | None=None, pass_threshold: float=0.7) -> JudgeEvaluator:
