...
class BM25Scorer:
    ...
    def __init__(self, k1: float=1.2, b: float=0.75):
        ...
        """
        import warnings
        warnings.warn('BM25Scorer is deprecated. Use agentic_core.L4_state.memory.bm25_store.Bm25Store (backed by ASTAwareTokenizer) instead.', DeprecationWarning, stacklevel=2)
        self.k1 = k1
        self.b = b
...
class HybridScorer:
    ...
    def score_documents(self, query: str, top_k: int | None=None) -> list[ScoringResult]:
        ...
        for i, doc in enumerate(self.documents):
            ...
            final_score = self.weights.bm25_weight * bm25_score + self.weights.semantic_weight * semantic_score + self.weights.tfidf_weight * tfidf_score + self.weights.freshness_weight * freshness_score
            result = ScoringResult(document_id=doc['id'], bm25_score=bm25_score, semantic_score=semantic_score, tfidf_score=tfidf_score, freshness_score=freshness_score, final_score=final_score, metadata={'content_length': len(doc['content'])})
