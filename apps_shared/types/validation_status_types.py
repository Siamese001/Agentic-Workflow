...
try:
    ...
    raise ImportError("scikit-learn is required for this module. Install with: pip install -e '.[infra]'") from _err
...
class ValidationGateExecutor:
    ...
    def __init__(self, validation_gates: list[Any], word_count_constraints: dict[str, Any], differentiator_distribution: dict[str, int] | None=None, similarity_thresholds: dict[str, float] | None=None):
    ...
    def execute_gate(self, gate_id: str, content: str, k_node_id: str, execution_point: str, context: dict[str, Any] | None=None) -> ValidationResult:
        ...
        if not gate:
            ...
            return ValidationResult(status=ValidationStatus.PASS, gate_id=gate_id, execution_point=execution_point, message=f'Gate {gate_id} not configured - skipping')
        if gate.execution_point != execution_point:
            logger.warning(f'Gate {gate_id} execution point mismatch: expected {gate.execution_point}, got {execution_point}')
        failures = []
        for check in gate.checks:
            ...
            if failure:
                ...
                if gate.severity == 'CRITICAL' and gate.blocking:
                    ...
                    return ValidationResult(status=ValidationStatus.BLOCK, gate_id=gate_id, execution_point=execution_point, failures=[failure], action=ValidationAction.HALT, score=0.0, message=gate.halt_message or f'CRITICAL: {failure.message}')
        if failures:
            ...
            score = 1.0
        return ValidationResult(status=status, gate_id=gate_id, execution_point=execution_point, failures=failures, action=action, score=score, message=gate.halt_message if failures else None)

    def execute_all_gates(self, execution_point: str, content: str, k_node_id: str, context: dict[str, Any] | None=None) -> list[ValidationResult]:
    ...
    def _check_word_count(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        ...
        if failures:
            return RuleFailure(rule_id=check, rule_name='Word Count Validation', severity='CRITICAL', message='; '.join(failures), actual=[len(s.split()) for s in segments], expected={'min': min_words, 'max': max_words, 'scope': scope}, context={'segment_count': len(segments)})
        return None

    def _check_char_count(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        """Check character count."""
        if 'K.4' in k_node_id or 'headline' in check.lower():
            char_count = len(content)
            if char_count < 60:
                return RuleFailure(rule_id=check, rule_name='Character Count Minimum', severity='CRITICAL', message=f'Character count {char_count} < min 60', actual=char_count, expected={'min': 60, 'max': 90})
            if char_count > 90:
                return RuleFailure(rule_id=check, rule_name='Character Count Maximum', severity='CRITICAL', message=f'Character count {char_count} > max 90', actual=char_count, expected={'min': 60, 'max': 90})
        return None

    def _check_differentiators(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        ...
        if 'gap_coverage' in check.lower():
            ...
            if coverage < threshold:
                ...
                return RuleFailure(rule_id=check, rule_name='Gap Coverage Check', severity=severity, message=f'Gap coverage {coverage:.1%} < threshold {threshold:.1%}', actual=coverage, expected=threshold, context={'total_gap_keywords': len(jd_keyword_gap), 'covered_keywords': len(covered_keywords), 'missing_keywords': list(set(jd_keyword_gap) - covered_keywords)})
        ...
        if required_count:
            ...
            if found_count < required_count:
                return RuleFailure(rule_id=check, rule_name='Differentiator Distribution', severity='HIGH', message=f'Found {found_count} differentiators, required {required_count}', actual=found_count, expected=required_count, context={'differentiators': differentiators})
        return None

    def _check_similarity(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        ...
        if 'strictly' in check.lower() or '74' in check:
            if similarity >= threshold:
                return RuleFailure(rule_id=check, rule_name='Similarity Check (Strict)', severity='CRITICAL', message=f'Similarity {similarity:.2%} >= threshold {threshold:.2%} (must be strictly less)', actual=similarity, expected=f'< {threshold}', context={'target': target_name})
        elif similarity > threshold:
            return RuleFailure(rule_id=check, rule_name='Similarity Check', severity='HIGH', message=f'Similarity {similarity:.2%} > threshold {threshold:.2%}', actual=similarity, expected=f'<= {threshold}', context={'target': target_name})
        return None

    def _check_placeholders(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        """Check for placeholders."""
        placeholder_patterns = ['\\[NAME\\]', '\\[COMPANY\\]', '\\{name\\}', '\\{company\\}', '<NAME>', '<COMPANY>', 'PLACEHOLDER', 'TODO']
        found_placeholders = []
        ...
        if found_placeholders:
            return RuleFailure(rule_id=check, rule_name='Placeholder Detection', severity='CRITICAL', message=f"Placeholders detected: {', '.join(set(found_placeholders))}", actual=found_placeholders, expected='No placeholders')
        return None

    def _check_grounding(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        ...
        if ungrounded_claims:
            return RuleFailure(rule_id=check, rule_name='Claim Grounding Check', severity='CRITICAL', message=f'{len(ungrounded_claims)} ungrounded claims detected', actual=len(ungrounded_claims), expected=0, context={'ungrounded_claims': ungrounded_claims[:3]})
        return None

    def _check_variance(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        ...
        if std_dev > max_std_dev:
            return RuleFailure(rule_id=check, rule_name='Word Count Variance', severity='CRITICAL', message=f'Std dev {std_dev:.1f} > max {max_std_dev}', actual=std_dev, expected=f'<= {max_std_dev}', context={'word_counts': word_counts})
        return None

    def _check_plausibility(self, check: str, content: str, k_node_id: str, context: dict[str, Any]) -> RuleFailure | None:
        ...
        if authentic_count < min_authentic:
            return RuleFailure(rule_id=check, rule_name='Plausibility Check', severity='CRITICAL', message=f'Only {authentic_count} authentic competencies, required {min_authentic}', actual=authentic_count, expected=min_authentic)
        return None
