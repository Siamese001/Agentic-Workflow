...
severity_log_levels = {sovereign_severity.CRITICAL: logging.CRITICAL, sovereign_severity.ERROR: logging.ERROR, sovereign_severity.WARNING: logging.WARNING, sovereign_severity.INFO: logging.INFO, sovereign_severity.DEBUG: logging.DEBUG}
...
sovereign_event_categories = {'GOVERNANCE': [sovereign_event_type.AUDIT_STARTED, sovereign_event_type.AUDIT_COMPLETED, sovereign_event_type.SOVEREIGNTY_COMPROMISED, sovereign_event_type.SOVEREIGNTY_RESTORED, sovereign_event_type.SOVEREIGNTY_ACHIEVED, sovereign_event_type.SOVEREIGNTY_PERFECT], 'GUARDIAN': [sovereign_event_type.GUARDIAN_BLOCKED_COMMIT, sovereign_event_type.GUARDIAN_VIOLATION, sovereign_event_type.GUARDIAN_CLEAN], 'HEALING': [sovereign_event_type.HEALING_CYCLE_STARTED, sovereign_event_type.HEALING_ACTION_APPLIED, sovereign_event_type.HEALING_ACTION_FAILED, sovereign_event_type.HEALING_TRANSACTION_START, sovereign_event_type.HEALING_TRANSACTION_COMMIT, sovereign_event_type.HEALING_TRANSACTION_ROLLBACK, sovereign_event_type.HEALING_FIX_APPLIED, sovereign_event_type.HEALING_FIX_REVERTED, sovereign_event_type.HEALING_CYCLE_COMPLETE], 'REASONING': [sovereign_event_type.REASONING_START, sovereign_event_type.REASONING_END, sovereign_event_type.REASONING_STEP, sovereign_event_type.HYPOTHESIS_FORMED, sovereign_event_type.HYPOTHESIS_VALIDATED, sovereign_event_type.HYPOTHESIS_REJECTED, sovereign_event_type.DARK_REASONING_DETECTED], 'VIOLATION': [sovereign_event_type.VIOLATION_DETECTED, sovereign_event_type.SSOT_INLINE_MODEL, sovereign_event_type.SSOT_RAW_PROMPT, sovereign_event_type.SSOT_HARDCODED_CONFIG, sovereign_event_type.SSOT_UNDERSCORE_FIELD, sovereign_event_type.DDD_VIOLATION, sovereign_event_type.DDD_AGGREGATE_BYPASS, sovereign_event_type.DDD_UBIQUITOUS_LANGUAGE_MISSING, sovereign_event_type.LAYER_CROSS_IMPORT], 'SYSTEM': [sovereign_event_type.SYSTEM_BOOT, sovereign_event_type.CONSTITUTION_LOAD], 'MCP': [sovereign_event_type.MCP_INTEGRATION_STARTED, sovereign_event_type.MCP_INTEGRATION_SUCCESS, sovereign_event_type.MCP_INTEGRATION_FAILED]}
...
class research_result(BaseModel):
    ...
    confidence_level: Literal['high', 'medium', 'low'] = Field(..., description='Confidence in the research results')
...
class retry_policy(BaseModel):
    ...
    retryable_stages: list[micro_stage] = Field(default=[micro_stage.THINK, micro_stage.ACT, micro_stage.CRITIQUE])
...
class injection_pattern(BaseModel):
    ...
    enabled: bool = True
core_contracts_types_registry = {'Territory': Territory, 'AgentMessage': AgentMessage, 'ReadFileArgs': ReadFileArgs, 'WriteFileArgs': WriteFileArgs, 'MoveFileArgs': MoveFileArgs, 'ListFilesArgs': ListFilesArgs, 'ExecuteCommandArgs': ExecuteCommandArgs, 'DeleteFileArgs': DeleteFileArgs, 'CreateDirectoryArgs': CreateDirectoryArgs, 'AgentThoughtProcess': AgentThoughtProcess, 'CodeGenerationResult': CodeGenerationResult, 'ResearchResult': ResearchResult, 'ConsensusVerdict': ConsensusVerdict, 'ModelOpinion': ModelOpinion, 'AgentPlan': AgentPlan, 'ToneType': ToneType, 'StyleProfile': StyleProfile, 'GenerationConfig': GenerationConfig, 'micro_stage': micro_stage, 'HopState': HopState, 'RetryPolicy': RetryPolicy, 'MicroCheckpoint': MicroCheckpoint, 'StageTransition': StageTransition, 'InjectionType': InjectionType, 'InjectionScope': InjectionScope, 'InjectionPattern': InjectionPattern}
...
class hard_state:
    ...
    def add_trace(self, event: str, data: dict[str, Any]) -> 'HardState':
        ...
        new_trace = self.execution_trace + [{'event': event, 'timestamp': datetime.utcnow().isoformat(), 'data': data}]
        return HardState(execution_id=self.execution_id, workflow_id=self.workflow_id, node_id=self.node_id, security_scopes=self.security_scopes, file_paths=self.file_paths, schemas=self.schemas, execution_trace=new_trace, created_at=self.created_at)
...
class soft_state:
    ...
    def record_revision(self, key: str, old_value: Any, new_value: Any) -> None:
        ...
        self.revision_history.append({'key': key, 'old_value': old_value, 'new_value': new_value, 'timestamp': datetime.utcnow().isoformat()})
...
class thermal_config:
    ...
    def get_params_for_node(self, node_id: str) -> dict[str, float]:
        ...
        if node_id in self.node_overrides:
            return {'temperature': self.node_overrides[node_id].get('temperature', self.temperature), 'top_p': self.node_overrides[node_id].get('top_p', self.top_p), 'frequency_penalty': self.node_overrides[node_id].get('frequency_penalty', self.frequency_penalty), 'presence_penalty': self.node_overrides[node_id].get('presence_penalty', self.presence_penalty)}
        return {'temperature': self.temperature, 'top_p': self.top_p, 'frequency_penalty': self.frequency_penalty, 'presence_penalty': self.presence_penalty}

    def set_node_profile(self, node_id: str, profile: ThermalProfile) -> None:
        """Set a thermal profile for a specific node."""
        profile_configs = {ThermalProfile.CREATIVITY_MAX: {'temperature': 0.9, 'top_p': 0.95}, ThermalProfile.CREATIVITY_HIGH: {'temperature': 0.8, 'top_p': 0.9}, ThermalProfile.BALANCED: {'temperature': 0.7, 'top_p': 0.85}, ThermalProfile.STRUCTURED: {'temperature': 0.3, 'top_p': 0.7}, ThermalProfile.PRECISION: {'temperature': 0.1, 'top_p': 0.5}}
...
CORE_CONTRACTS_REGISTRY.update({'ThermalProfile': ThermalProfile, 'HardState': HardState, 'SoftState': SoftState, 'thermal_config': thermal_config, 'signed_claim': signed_claim, 'SignalContext': SignalContext, 'SafetyProfile': SafetyProfile, 'BudgetProfile': BudgetProfile, 'SimScenario': SimScenario, 'SimOutcome': SimOutcome, 'Hypothesis': Hypothesis, 'MetacognitionReport': MetacognitionReport, 'GoldenStateTestCase': GoldenStateTestCase, 'JudgeVerdict': JudgeVerdict, 'EvalResult': EvalResult, 'GoldenCase': GoldenCase, 'GoldenOutput': GoldenOutput})
...
class circuit_state(str, Enum):
    ...
    OPEN = 'open'
    HALF_OPEN = 'half_open'
CORE_CONTRACTS_REGISTRY.update({'LLMResponse': LLMResponse, 'MessageType': MessageType, 'ResidualAgentMessage': ResidualAgentMessage, 'AgentResponse': AgentResponse, 'ResidualValidationResult': ResidualValidationResult, 'ReasoningConfig': ReasoningConfig, 'HopStatus': HopStatus, 'GateDecision': GateDecision, 'ValidationSeverity': ValidationSeverity, 'WorkflowCheckpoint': WorkflowCheckpoint, 'ThematicAnalysis': ThematicAnalysis, 'RAGState': RAGState, 'CircuitState': CircuitState})
...
class enforcement_rag_config:
    ...
    source_weights: dict[str, float] = field(default_factory=lambda: {'SOURCE_JD': 1.8, 'SOURCE_COMPANY_BLOG': 1.5, 'SOURCE_TARGET_EMPLOYEE': 1.4, 'SOURCE_GARTNER_MQ': 1.2, 'SOURCE_PEER_JD': 0.8, 'SOURCE_GENERIC_PROFILE': 0.5, 'LOCAL_NLP': 0.2})
...
CORE_CONTRACTS_REGISTRY.update({'FilePathsConfig': FilePathsConfig, 'ArtistConfig': ArtistConfig, 'ValidatorConfig': ValidatorConfig, 'PromptsConfig': PromptsConfig, 'WebRagConfig': WebRagConfig, 'EnricherConfig': EnricherConfig, 'EnforcementRAGConfig': EnforcementRAGConfig, 'EnforcementReasoningConfig': EnforcementReasoningConfig, 'ContentConstraintsConfig': ContentConstraintsConfig, 'SignalControlConfig': SignalControlConfig, 'PromptAddendumConfig': PromptAddendumConfig, 'AppConfig': AppConfig})
...
class qa_report:
    ...
    passed: bool
    timestamp: str
CORE_CONTRACTS_REGISTRY.update({'OutreachMission': OutreachMission, 'ProfileAnalysis': ProfileAnalysis, 'MessageClaim': MessageClaim, 'RAGCritique': RAGCritique, 'EnforcementRAGResult': EnforcementRAGResult, 'SenderGroundingWhitelists': SenderGroundingWhitelists, 'ResearchContext': ResearchContext, 'MessageScaffold': MessageScaffold, 'GeneratedMessage': GeneratedMessage, 'EnforcementValidationResult': EnforcementValidationResult, 'QAReport': QAReport})
...
class integrity_gate_result:
    ...
    def add_violation(self, reason: str, detail: str) -> None:
        ...
        self.passed = False
CORE_CONTRACTS_REGISTRY.update({'ExecutiveProfile': ExecutiveProfile, 'FinancialMetric': FinancialMetric, 'TechnicalImplementation': TechnicalImplementation, 'StrategicLayer': StrategicLayer, 'TechnicalLayer': TechnicalLayer, 'LeadershipLayer': LeadershipLayer, 'CitationMap': CitationMap, 'DeepResearchOutput': DeepResearchOutput, 'ResearchHopResult': ResearchHopResult, 'IntegrityGateResult': IntegrityGateResult})
...
class greeting_template:
    ...
    template: str
    note: str
CORE_CONTRACTS_REGISTRY.update({'SubjectLineBrief': SubjectLineBrief, 'MessageBodyBrief': MessageBodyBrief, 'CTABrief': CTABrief, 'CreativeBrief': CreativeBrief, 'ArchetypeTemplate': ArchetypeTemplate, 'SignatureTemplate': SignatureTemplate, 'GreetingTemplate': GreetingTemplate})
...
class immutable_staging_buffer:
    ...
    def with_data(self, new_data: dict[str, Any]) -> 'ImmutableStagingBuffer':
        ...
        from datetime import datetime
        return ImmutableStagingBuffer(_data={**self.data, **new_data}, _version=self.version + 1, _timestamp=datetime.utcnow().isoformat(), _checksum=None)
...
CORE_CONTRACTS_REGISTRY.update({'APICallMetrics': APICallMetrics, 'ImmutableStagingBuffer': ImmutableStagingBuffer})
...
CORE_CONTRACTS_REGISTRY.update({'VoiceType': VoiceType, 'ProvenanceStrategy': ProvenanceStrategy, 'MessageRoute': MessageRoute, 'RecipientArchetype': RecipientArchetype, 'SignatureFormat': SignatureFormat, 'CTAFormat': CTAFormat})
...
class workflow_spec:
    ...
    version: str
    hops: list[HopSpec]
CORE_CONTRACTS_REGISTRY.update({'WordCountConstraint': WordCountConstraint, 'CharCountConstraint': CharCountConstraint, 'StructureConstraint': StructureConstraint, 'HeadlineBrief': HeadlineBrief, 'HopInput': HopInput, 'HopOutput': HopOutput, 'RetryPolicy': RetryPolicy, 'HopSpec': HopSpec, 'WorkflowSpec': WorkflowSpec})
...
CORE_CONTRACTS_REGISTRY.update({'ExperienceBulletsBrief': ExperienceBulletsBrief, 'LeadershipCompetenciesBrief': LeadershipCompetenciesBrief, 'CoverLetterBrief': CoverLetterBrief, 'OptimizedSkillsBrief': OptimizedSkillsBrief, 'RGCreativeBrief': RGCreativeBrief, 'ExecutiveSummaryBrief': ExecutiveSummaryBrief})
...
CORE_CONTRACTS_REGISTRY.update({'RouteConditions': RouteConditions, 'RouteConstraints': RouteConstraints, 'RouteConfig': RouteConfig, 'ArchetoneConfig': ArchetoneConfig, 'TemperatureConfig': TemperatureConfig})
import logging
...
class mission_plan(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'MissionPlan':
            ...
            logger.info(f'[BUILDER] Constructing MissionPlan {self._mission_id} | Phases: {len(self._phases)} | Priority: {self._priority}')
            return MissionPlan(mission_id=self._mission_id, cycle_id=self._cycle_id, priority=self._priority, objective=self._objective, phases=self._phases.copy(), risk_assessment=self._risk_assessment.copy())
            ...
            for phase_name in graph:
                if phase_name not in visited:
                    if has_cycle(phase_name):
                        raise ValueError(f'Sovereignty Breach: Dependency cycle detected in MissionPlan {self._mission_id} involving phases: {list(rec_stack if rec_stack else graph.keys())}')
...
class thought_chain(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'ThoughtChain':
            ...
            return ThoughtChain(chain_id=self._chain_id, goal=self._goal, steps=self._steps.copy(), active_hypotheses=self._hypotheses.copy(), revisions=self._revisions.copy(), final_conclusion=self._final_conclusion, success=self._success, duration_seconds=self._duration_seconds)
...
class constitutional_violation(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'ConstitutionalViolation':
            ...
            required = {'guardian': self._guardian, 'dimension': self._dimension, 'severity': self._severity, 'file_path': self._file_path, 'description': self._description}
            ...
            logger.warning(f"[AUDIT] Violation Detected: {self._violation_id} | Severity: {self._severity} | Dimension: {self._dimension} | Loc: {self._file_path}:{self._line_number or 'N/A'}")
            return ConstitutionalViolation(violation_id=self._violation_id, guardian=self._guardian, dimension=self._dimension, severity=self._severity, file_path=self._file_path, line_number=self._line_number, description=self._description, evidence=self._evidence or 'Not recorded', suggested_fix=self._suggested_fix)
...
class healing_action(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'HealingAction':
            ...
            required = {'strategy': self._strategy, 'action_type': self._action_type, 'target_file': self._target_file, 'reason': self._reason}
            for field, value in required.items():
                if not value:
            ...
            logger.info(f'[AUDIT] Healing Action Logged: {self._action_id} | Outcome: {status} | Strategy: {self._strategy} | Type: {self._action_type} | File: {self._target_file}')
            return HealingAction(action_id=self._action_id, strategy=self._strategy, action_type=self._action_type, target_file=self._target_file, target_line=self._target_line, reason=self._reason, success=self._success, error_message=self._error_message, backup_path=self._backup_path, transaction_id=self._transaction_id)
...
class healing_cycle(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'HealingCycle':
            ...
            logger.info(f'[AUDIT] Healing Cycle Concluded: {self._cycle_id} | Outcome: {status} | Delta: {self._trigger_score:.1f}% -> {self._target_score:.1f}% | Restored: {healed}/{len(self._actions)}')
            return HealingCycle(cycle_id=self._cycle_id, trigger_score=self._trigger_score, target_score=self._target_score, actions=self._actions.copy(), success=self._success if self._success is not None else False, duration_seconds=self._duration_seconds, healed_violations=healed, persistent_violations=persistent)
...
class healing_report(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'HealingReport':
            ...
            logger.info(f"[AUDIT] HealingReport Sealed: {self._report_id} | Outcome: {('SUCCESS' if self._success else 'PARTIAL')} | Remediation: {self._violations_fixed}/{self._violations_found}")
            return HealingReport(report_id=self._report_id, auditor_version=self._auditor_version, target_scope=self._target_scope, violations_found=self._violations_found, violations_fixed=self._violations_fixed, healing_actions=self._healing_actions.copy(), pre_healing_score=self._pre_healing_score, post_healing_score=self._post_healing_score, success=self._success, healing_strategies_used=self._strategies_used.copy())
...
class sovereign_event(sovereign_base_model_types):
    ...
    class Builder:
        ...
        def build(self) -> 'SovereignEvent':
            ...
            return SovereignEvent(event_id=self._event_id, event_type=self._event_type, severity=self._severity, source=self._source, dimension=self._dimension, payload=self._payload.copy(), correlation_id=self._correlation_id)
CORE_CONTRACTS_REGISTRY.update({'MissionPriority': MissionPriority, 'MissionStatus': MissionStatus, 'MissionPhase': MissionPhase, 'MissionPlan': MissionPlan, 'ThinkingStep': ThinkingStep, 'RevisionStep': RevisionStep, 'ThoughtChain': ThoughtChain, 'ConstitutionalViolation': ConstitutionalViolation, 'HealingAction': HealingAction, 'HealingCycle': HealingCycle, 'HealingReport': HealingReport, 'SovereignSeverity': SovereignSeverity, 'SovereignEventType': SovereignEventType, 'SovereignEvent': SovereignEvent})
if __name__ != '__main__':
    assert len(CORE_CONTRACTS_REGISTRY) == len(set(CORE_CONTRACTS_REGISTRY.values())), 'CRITICAL: Duplicate class definitions detected in CORE_CONTRACTS_REGISTRY'
if __name__ != '__main__':
    assert len(CORE_CONTRACTS_REGISTRY) == len(set(CORE_CONTRACTS_REGISTRY.values())), 'CRITICAL: Duplicate class definitions detected in CORE_CONTRACTS_REGISTRY'
if __name__ != '__main__':
    assert len(CORE_CONTRACTS_REGISTRY) == len(set(CORE_CONTRACTS_REGISTRY.values())), 'CRITICAL: Duplicate class definitions detected in CORE_CONTRACTS_REGISTRY'
