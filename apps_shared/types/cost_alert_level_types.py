...
class CostTracker:
    ...
    def __init__(self, default_budget_per_agent: float | None=None, alert_threshold_percent: float=0.8, enable_logging: bool=True):
    ...
    def record_cost(self, agent_id: str, spiffe_id: str, model_id: str, tokens: int, cost: float) -> None:
        ...
        record = {'spiffe_id': spiffe_id, 'model_id': model_id, 'tokens': tokens, 'cost': cost, 'timestamp': time.time()}
    ...
    def get_metrics(self, agent_id: str, period_hours: int=24) -> CostMetrics | None:
        ...
        metrics = CostMetrics(agent_id=agent_id, spiffe_id=spiffe_id, total_cost=total_cost, token_count=token_count, request_count=request_count, avg_cost_per_request=avg_cost, period_start=period_start, period_end=time.time(), model_breakdown=model_breakdown)
        return metrics
