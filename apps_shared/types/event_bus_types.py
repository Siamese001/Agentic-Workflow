...
class SystemEvent(BaseModel):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        """
        return {'id': self.id, 'trace_id': self.trace_id, 'type': self.type.value, 'source_component': self.source_component, 'payload': self.payload, 'timestamp': self.timestamp, 'correlation_id': self.correlation_id, 'causation_id': self.causation_id}
    ...
    def from_dict(cls, data: dict[str, Any]) -> 'SystemEvent':
        ...
        """
        return cls(id=data['id'], trace_id=data['trace_id'], type=EventType(data['type']), source_component=data['source_component'], payload=data['payload'], timestamp=data['timestamp'], correlation_id=data.get('correlation_id'), causation_id=data.get('causation_id'))
...
class MemoryEventBus(EventBus):
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        Returns:
            Health status
        """
        return {'status': 'healthy' if self._running else 'stopped', 'type': 'memory', 'channels': len(self._queues), 'subscribers': sum((len(subs) for subs in self._subscribers.values())), 'queue_sizes': {ch: q.qsize() for ch, q in self._queues.items()}, 'stats': self._stats.copy()}
    ...
    async def _safe_notify(self, callback: Callable[[SystemEvent], Awaitable[None]], event: SystemEvent) -> None:
...
class RedisEventBus(EventBus):
    ...
    def __init__(self, connection_string: str, consumer_group: str='agentic_workflow', consumer_name: str | None=None):
        ...
        self._stats = {'events_published': 0, 'events_processed': 0, 'subscriber_errors': 0, 'reconnections': 0, 'channels': 0}
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self.redis = redis.from_url(self.connection_string, decode_responses=True, retry_on_timeout=True, socket_keepalive=True, socket_keepalive_options={})
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            await self.redis.ping()
            return {'status': 'healthy', 'type': 'redis', 'connection': self.connection_string, 'consumer_group': self.consumer_group, 'channels': len(self._subscribers), 'stats': self._stats.copy()}
        # guardian: allow-silent-swallow
        except Exception as e:
    ...
    async def _reader_loop(self, channel: str) -> None:
        ...
        while self._running:
            try:
                messages = await self.redis.xreadgroup(self.consumer_group, self.consumer_name, {channel: '>'}, count=10, block=1000)
...
async def publish_event(event_type: EventType, source_component: str, payload: dict[str, Any], trace_id: str | None=None, correlation_id: str | None=None, causation_id: str | None=None) -> None:
    ...
    """
    event = SystemEvent(type=event_type, source_component=source_component, payload=payload, trace_id=trace_id or str(uuid.uuid4()), correlation_id=correlation_id, causation_id=causation_id)
...
def event_publisher(event_type: EventType, channel: str | None=None):
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            await publish_event(event_type, func.__module__ + '.' + func.__name__, {'status': 'started', 'args_count': len(args)}, trace_id=trace_id)
            try:
                result = await func(*args, **kwargs)
                await publish_event(event_type, func.__module__ + '.' + func.__name__, {'status': 'completed', 'success': True}, trace_id=trace_id, causation_id=trace_id)
                return result
            except Exception as e:
                raise
                await publish_event(EventType.ERROR_OCCURRED, func.__module__ + '.' + func.__name__, {'status': 'failed', 'error': str(e)}, trace_id=trace_id, causation_id=trace_id)
                raise
