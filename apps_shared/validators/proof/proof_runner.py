...
W1 surface (working today):
    ...
    python -m apps_shared.validators.proof.proof_runner --bypass-only \
...
# ``python -m apps_shared.validators.proof.proof_runner`` from the repo root.
_REPO_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from apps_shared.validators.proof.adg_queries import AppBypassReport
from apps_shared.validators.proof.app_inventory import discover_apps, required_apps
from apps_shared.validators.proof.bypass_validator import (
...
from apps_shared.validators.proof.scenario_base import run_app_scenario
from apps_shared.validators.proof.scenarios import SCENARIOS
from apps_shared.validators.proof.validators import validate_scenario
from apps_shared.validators.proof.negative_controls import run_negative_controls_for_all
from apps_shared.validators.proof.write_sovereignty import validate_write_sovereignty
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="apps_shared.validators.proof.proof_runner",
