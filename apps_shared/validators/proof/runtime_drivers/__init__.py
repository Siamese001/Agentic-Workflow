...
def get_driver(app_id: str):
    ...
    if app_id == "apps_underwriting_ai":
        from apps_shared.validators.proof.runtime_drivers.apps_underwriting_ai_driver import (
            AppsUnderwritingAIDriver,
        )
        return AppsUnderwritingAIDriver()
    if app_id == "apps_rfp":
        from apps_shared.validators.proof.runtime_drivers.apps_rfp_driver import AppsRfpDriver
        return AppsRfpDriver()
    if app_id == "apps_research":
        from apps_shared.validators.proof.runtime_drivers.apps_research_driver import AppsResearchDriver
        return AppsResearchDriver()
    if app_id == "apps_exec":
        from apps_shared.validators.proof.runtime_drivers.apps_exec_driver import AppsExecDriver
        return AppsExecDriver()
    if app_id == "apps_lic":
        from apps_shared.validators.proof.runtime_drivers.apps_lic_driver import AppsLicDriver
        return AppsLicDriver()
    if app_id == "apps_rg":
        from apps_shared.validators.proof.runtime_drivers.apps_rg_driver import AppsRgDriver
        return AppsRgDriver()
    if app_id == "apps_eval":
        from apps_shared.validators.proof.runtime_drivers.apps_eval_driver import AppsEvalDriver
        return AppsEvalDriver()
    if app_id == "apps_shared":
        from apps_shared.validators.proof.runtime_drivers.apps_shared_driver import AppsSharedDriver
