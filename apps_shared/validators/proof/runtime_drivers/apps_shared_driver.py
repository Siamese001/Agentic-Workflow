...
class AppsSharedDriver:
    ...
    def invoke(self, ctx) -> dict[str, str]:
        ...
        try:
            pc = importlib.import_module("apps_shared.validators.proof.proof_contracts")
            class_results = []
            ...
            contract_schema_report = {
                "substrate_id": fixture.get("substrate_id"),
                "module": "apps_shared.validators.proof.proof_contracts",
        ...
        for mod_path in (
            "apps_shared.validators.proof.scenario_base",
            "apps_shared.validators.proof.otel_export",
        ):
            try:
        ...
        try:
            v = importlib.import_module("apps_shared.validators.proof.validators")
            replay_helpers_results.append({
                "symbol": "apps_shared.validators.proof.validators._strip_volatile",
                "present": hasattr(v, "_strip_volatile"),
            })
            replay_helpers_results.append({
                "symbol": "apps_shared.validators.proof.validators.validate_replay",
                "present": hasattr(v, "validate_replay"),
            })
        except ImportError as exc:
            replay_helpers_results.append({
                "symbol": "apps_shared.validators.proof.validators",
                "present": False,
                "detail": f"ImportError: {exc!r}",
            })
        try:
            sb = importlib.import_module("apps_shared.validators.proof.scenario_base")
            replay_helpers_results.append({
                "symbol": "apps_shared.validators.proof.scenario_base.run_app_scenario",
                "present": hasattr(sb, "run_app_scenario"),
            })
        except ImportError as exc:
            replay_helpers_results.append({
                "symbol": "apps_shared.validators.proof.scenario_base",
                "present": False,
                "detail": f"ImportError: {exc!r}",
            })
