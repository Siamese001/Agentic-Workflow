...
class QualityAssessment(BaseModel):
    ...
    def composite_score(self) -> float:
        ...
        return self.relevance_score * weights['relevance'] + self.authority_score * weights['authority'] + self.specificity_score * weights['specificity'] + self.coherence_score * weights['coherence']
...
class SignalQualityPipeline:
    ...
    def __init__(self, relevance_threshold: float=0.3, authority_threshold: float=0.4, specificity_threshold: float=0.5, enable_coherence_check: bool=False):
        ...
        self.authority_tiers = {'tier_1': {'score': 1.0, 'sources': {'10-k', '10-q', 'official_report', 'sec_filing', 'annual_report', 'proxy_statement'}}, 'tier_2': {'score': 0.8, 'sources': {'linkedin', 'resume_v1', 'official_resume', 'company_profile', 'verified_profile'}}, 'tier_3': {'score': 0.5, 'sources': {'notes', 'blog', 'scratchpad', 'personal_notes', 'draft'}}, 'tier_4': {'score': 0.2, 'sources': {'unknown', 'unverified', 'cached', 'temp'}}}
        self.impact_words = {'grew', 'growth', 'increased', 'decreased', 'reduced', 'saved', 'generated', 'achieved', 'improved', 'optimized', 'accelerated', 'expanded', 'launched', 'delivered', 'completed', 'managed', 'led', 'built', 'created', 'drove', 'revenue', 'cost', 'savings', 'profit', 'margin', 'roi', 'efficiency'}
        self.metric_patterns = ['\\$\\d+(?:,\\d{3})*(?:\\.\\d+)?[kmb]?', '\\d+(?:,\\d{3})*(?:\\.\\d+)?%', '\\d+(?:,\\d{3})*(?:\\.\\d+)?[kmb]', '\\d+(?:,\\d{3})*(?:\\.\\d+)?x', '\\d+(?:,\\d{3})*(?:\\.\\d+)?\\s*(?:times|fold)', '\\b\\d+\\s*(?:years?|months?|weeks?|days?)\\b']
        logger.info(f'Initialized SignalQualityPipeline with thresholds: relevance={relevance_threshold}, authority={authority_threshold}, specificity={specificity_threshold}')

    def evaluate_signal(self, content: str, metadata: dict[str, str], query: str, doc_id: str | None=None) -> QualityAssessment:
        ...
        try:
            ...
            if assessment.authority_score < self.authority_threshold or assessment.relevance_score < self.relevance_threshold:
                assessment.is_pass = False
                assessment.add_flag('HARD_FAIL')
            logger.debug(f'Signal evaluation for doc {doc_id}: relevance={assessment.relevance_score:.2f}, authority={assessment.authority_score:.2f}, specificity={assessment.specificity_score:.2f}, flags={assessment.flags}, pass={assessment.is_pass}', extra={'doc_id': doc_id, 'flags': assessment.flags, 'is_pass': assessment.is_pass})
            return assessment
    ...
    def _normalize_text(self, text: str) -> list[str]:
        ...
        try:
            ...
            stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can', 'this', 'that'}
    ...
    def batch_evaluate(self, documents: list[tuple[str, dict[str, str], str]], filter_failed: bool=True) -> list[tuple[dict[str, str], QualityAssessment]]:
...
# guardian: allow-magic-config
def create_quality_pipeline(relevance_threshold: float=0.3, authority_threshold: float=0.4, specificity_threshold: float=0.5, strict_mode: bool=False) -> SignalQualityPipeline:
    ...
    """
    if strict_mode:
        return SignalQualityPipeline(relevance_threshold=THRESHOLD, authority_threshold=THRESHOLD, specificity_threshold=THRESHOLD, enable_coherence_check=True)
    return SignalQualityPipeline(relevance_threshold=relevance_threshold, authority_threshold=authority_threshold, specificity_threshold=specificity_threshold)

def filter_high_quality_signals(documents: list[tuple[str, dict[str, str], str]], strict_mode: bool=False) -> list[dict[str, str]]:
