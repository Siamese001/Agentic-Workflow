...
class QueryDecomposer(SimpleAgentBase):
    ...
    def __init__(self, model_name: str='gpt-4', max_sub_queries: int=4):
        ...
        try:
            ...
            self.gate = None
        self.complexity_indicators = {'comparison': re.compile('\\b(compare|vs|versus|against|difference|contrast)\\b', re.IGNORECASE), 'causation': re.compile('\\b(why|cause|reason|impact|effect)\\b', re.IGNORECASE), 'temporal': re.compile('\\b(before|after|during|when|timeline|history)\\b', re.IGNORECASE), 'aggregation': re.compile('\\b(sum|total|average|count|aggregate|combine)\\b', re.IGNORECASE), 'relationship': re.compile('\\b(relationship|correlation|between|and)\\b', re.IGNORECASE)}
    ...
    async def _call_llm(self, prompt: str, temperature: float=0.3) -> Any:
        ...
        try:
            ...
            # guardian: allow-magic-config
            response = await client.messages.create(model='claude-3-5-sonnet-20241022', max_tokens=200, temperature=temperature, messages=[{'role': 'user', 'content': prompt}])
    ...
    async def decompose(self, query: str) -> DecomposedQuery:
        ...
        if self.gate:
            ...
            if decision.query_type in ['CONVERSATIONAL', 'FACTUAL'] and (not decision.should_retrieve):
                ...
                return DecomposedQuery(original_query=query, sub_queries=[query], reasoning='Query is simple, no decomposition needed', complexity_score=1)
        complexity = self._calculate_complexity_score(query)
        if complexity <= 3:
            logger.info(f'Low complexity ({complexity}), returning original query')
            return DecomposedQuery(original_query=query, sub_queries=[query], reasoning='Query complexity is low, no decomposition needed', complexity_score=complexity)
        ...
        try:
            ...
            return DecomposedQuery(original_query=query, sub_queries=sub_queries, reasoning=reasoning, complexity_score=complexity)
        # guardian: allow-silent-swallow
        except Exception as e:
            logger.error(f'Failed to decompose query: {e}')
            return DecomposedQuery(original_query=query, sub_queries=[query], reasoning='Decomposition failed, using original query', complexity_score=complexity)

    async def execute_plan(self, decomposed_query: DecomposedQuery, search_function: callable, **kwargs) -> list[Any]:
