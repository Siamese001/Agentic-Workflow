...
class AdaptiveRetrievalGate:
    ...
    def __init__(self):
        ...
        self.patterns = {'conversational': re.compile('^(hi|hello|hey|thanks|thank you|ok|okay|bye|goodbye|yes|no|sure|got it|understood|cool|awesome|great|perfect)$', re.IGNORECASE), 'reference': re.compile('\\b(previous|last|that|above|mentioned|earlier|said|told|asked|discussed)\\b', re.IGNORECASE), 'self_reference': re.compile('\\b(who are you|what are you|what can you do|how do you work|your name|help)\\b', re.IGNORECASE), 'continuation': re.compile('^(and|but|so|then|also|plus|however|therefore|meanwhile)\\b', re.IGNORECASE)}
        self.complex_keywords = {'metrics', 'how to', 'latest', 'compare', 'strategy', 'plan', 'analyze', 'evaluate', 'recommend', 'implement', 'design', 'architecture', 'framework', 'best practices', 'guidelines', 'statistics', 'data', 'performance', 'optimization', 'trends', 'forecast', 'roadmap', 'timeline', 'requirements'}
        self.question_patterns = ['\\bwhat\\s+(is|are|were|do|does|did)\\b', '\\bwhen\\s+(was|were|is|are|did|do)\\b', '\\bwhere\\s+(is|are|was|were|did|do)\\b', '\\bwhich\\s+(is|are|was|were|did|do)\\b', '\\bwho\\s+(is|are|was|were|did|do)\\b', '\\bwhy\\s+(is|are|was|were|did|do|does)\\b', '\\bhow\\s+(can|could|should|would|will|do|does|did)\\b']
    ...
    def _calculate_complexity_score(self, query: str, query_type: str) -> float:
        ...
        """
        type_scores = {'CONVERSATIONAL': 0.0, 'SELF_REFERENCE': 0.0, 'REFERENCE': 0.1, 'CONTINUATION': 0.2, 'FACTUAL': 0.6, 'COMPLEX': 0.8}
    ...
    def should_retrieve(self, query: str, history: list[dict] | None=None) -> RetrievalDecision:
        ...
        query = query.strip()
        if not query:
            return RetrievalDecision(should_retrieve=False, reason='Empty query', query_type='EMPTY', confidence=1.0)
        ...
        if should_retrieve:
            if len(query.split()) < 4 and (not any((pattern.search(query) for pattern in self.compiled_questions))):
                ...
                confidence = 0.7
        logger.info(f'Retrieval decision: {should_retrieve} | Type: {query_type} | Reason: {reason} | Query: {query[:50]}...')
        return RetrievalDecision(should_retrieve=should_retrieve, reason=reason, query_type=query_type, confidence=confidence)
    ...
    def get_statistics(self, decisions: list[RetrievalDecision]) -> dict[str, float]:
        ...
        return {'total_queries': total, 'retrieval_rate': retrieve_count / total, 'type_distribution': {k: v / total for k, v in type_counts.items()}, 'avg_confidence': sum((d.confidence for d in decisions)) / total}
