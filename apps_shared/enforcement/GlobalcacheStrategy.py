...
class L1MemoryCache:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'size': len(self.cache), 'max_size': self.max_size, 'hits': self._hits, 'misses': self._misses, 'hit_rate': hit_rate}
...
class L2VectorStore:
    ...
    # guardian: allow-magic-config
    def search(self, query_embedding: list[float], threshold: float=0.92, max_results: int=5) -> list[tuple[CacheEntry, float]]:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        total_requests = self._hits + self._misses
        hit_rate = self._hits / total_requests if total_requests > 0 else 0.0
        return {'size': len(self.entries), 'max_size': self.max_size, 'hits': self._hits, 'misses': self._misses, 'hit_rate': hit_rate, 'embedding_dim': self.embeddings.shape[1] if self.embeddings.shape[0] > 0 else 0}
...
class GlobalCache:
    ...
    def __init__(self, l1_size: int=1000, l2_size: int=10000, semantic_threshold: float=0.92):
        ...
        logger.info(f'Initialized GlobalCache (L1: {l1_size}, L2: SSOT-backed, threshold: {semantic_threshold})')
    ...
    def get_semantic(self, query_text: str, threshold: float | None=None, max_results: int=1) -> list[Any]:
    ...
    def put(self, key: str, value: Any, text_for_embedding: str | None=None, ttl: int=3600, source_engine: str='UNKNOWN') -> None:
        """Put value in cache.
        ...
        entry = CacheEntry(key_hash=key_hash, value=value, embedding=embedding, ttl=ttl, source_engine=source_engine)
        self.l1.put(key_hash, entry)
        if embedding:
            self.l2.add(entry)
        if text_for_embedding:
            ...
            if hive is not None:
                try:
                    hive.learn(text_for_embedding, self._HIVE_NAMESPACE, {'value': value, 'key': key, 'source_engine': source_engine})
                # guardian: allow-silent-swallow
                except Exception as e:
...
# guardian: allow-magic-config
def cached(key_func: Callable[..., Any] | None=None, ttl: int=3600, semantic: bool=False, threshold: float=0.92):
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if semantic:
                cache.put(key, result, text_for_embedding=str(args[0]) if args else key, ttl=ttl, source_engine=func.__module__)
            else:
...
def cache_put(key: str, value: Any, text_for_embedding: str | None=None, ttl: int=3600, source_engine: str='UNKNOWN') -> None:
