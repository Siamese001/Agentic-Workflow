...
def _initialize_orchestrator() -> None:
    ...
    HardenedWorkflowOrchestrator(workflow_spec=workflow_spec, run_base_dir='./pipeline_runs', storage_path='./state_storage')
...
def _prepare_workflow_context() -> None:
    ...
    return {'target_role': TEST_CONFIG['target_role'], 'target_company': TEST_CONFIG['target_company'], 'job_url': TEST_CONFIG['job_url'], 'routing_tier': TEST_CONFIG['routing_tier']}
...
async def main() -> None:
    ...
    try:
        ...
        updated_context = orchestrator.initialize_or_resume_workflow(workflow_id=TEST_JOB_ID, total_k_nodes=5, context=context)
