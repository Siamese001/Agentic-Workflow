...
class InputSanitizer:
    ...
    SQL_INJECTION_PATTERN = re.compile('(\\b(SELECT|INSERT|UPDATE|DELETE|DROP|UNION|ALTER)\\b)', re.IGNORECASE)
    PATH_TRAVERSAL_PATTERN = re.compile('\\.\\./|\\.\\.\\\\')

    @classmethod
    # guardian: allow-magic-config
    def sanitize_string(cls, value: str, max_length: int=10000, strip_html: bool=True, strip_scripts: bool=True) -> str:
...
class SecurityAuditLog:
    ...
    def log_event(self, event_type: str, message: str, severity: str='info', metadata: dict[str, Any] | None=None) -> None:
        """Log a security event."""
        import time
        event = {'timestamp': time.time(), 'type': event_type, 'message': message, 'severity': severity, 'metadata': metadata or {}}
        self._events.append(event)
        log_method = getattr(self._logger, severity, self._logger.info)
        log_method(f'[{event_type}] {message}')

    def log_validation_failure(self, field: str, errors: list[str], metadata: dict[str, Any] | None=None) -> None:
        """Log a validation failure."""
        self.log_event(event_type='validation_failure', message=f"Validation failed for {field}: {', '.join(errors)}", severity='warning', metadata={'field': field, 'errors': errors, **(metadata or {})})

    def log_rate_limit(self, key: str, metadata: dict[str, Any] | None=None) -> None:
        """Log a rate limit event."""
        self.log_event(event_type='rate_limit', message=f'Rate limit exceeded for {key}', severity='warning', metadata={'key': key, **(metadata or {})})

    def log_suspicious_activity(self, activity: str, metadata: dict[str, Any] | None=None) -> None:
        """Log suspicious activity."""
        self.log_event(event_type='suspicious_activity', message=activity, severity='error', metadata=metadata)
