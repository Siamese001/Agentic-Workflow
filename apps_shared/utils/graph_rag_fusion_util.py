...
class GraphRAGFusion:
    ...
    def __init__(self, knowledge_graph: KnowledgeGraphAgent | None=None, vector_retriever: callable | None=None, enable_fusion: bool=True, confidence_threshold: float=DEFAULT_CONFIDENCE_THRESHOLD):
        ...
        self.stats = {'total_queries': 0, 'vector_only': 0, 'graph_only': 0, 'fusion_queries': 0, 'multi_hop_queries': 0, 'graph_fallbacks': 0}
        logger.info(f'Initialized GraphRAGFusion - Fusion: {enable_fusion}')

    async def query(self, natural_query: str, query_type: QueryType | None=None, max_results: int=DEFAULT_MAX_RESULTS) -> FusionResult:
    ...
    def _detect_query_type(self, query: str) -> QueryType:
        ...
        relationship_words = ['relationship', 'connection', 'between', 'related to', 'worked with', 'collaborated', 'team', 'together']
        multi_hop_words = ['path', 'journey', 'progression', 'through', 'via', 'leads to', 'resulted in', 'caused']
    ...
    async def _vector_only_query(self, query: str, max_results: int) -> FusionResult:
        ...
        try:
            if self.vector_retriever:
                ...
                vector_results = []
            return FusionResult(query=query, query_type=QueryType.VECTOR_ONLY, vector_results=vector_results, sources=['vector_search'], confidence=0.8)
    ...
    async def _graph_only_query(self, query: str, max_results: int) -> FusionResult:
        ...
        try:
            if self.knowledge_graph:
                ...
                graph_context = self.knowledge_graph.query_context(params.get('entity_pattern', '').replace('(?i).*', '').replace('.*', ''), hops=2, limit=max_results)
                return FusionResult(query=query, query_type=QueryType.GRAPH_ONLY, graph_results=graph_context, sources=['graph_search'], confidence=graph_context.confidence, metadata={'cypher_pattern': pattern_type})
            else:
                self.stats['graph_fallbacks'] += 1
                return FusionResult(query=query, query_type=QueryType.GRAPH_ONLY, sources=['graph_unavailable'], confidence=0.0)
        except Exception as e:
    ...
    async def _fusion_query(self, query: str, query_type: QueryType, max_results: int) -> FusionResult:
        ...
        fused_context = self._fuse_results(vector_result.vector_results, graph_result.graph_results, query_type)
        ...
        return FusionResult(query=query, query_type=query_type, vector_results=vector_result.vector_results, graph_results=graph_result.graph_results, fused_context=fused_context, sources=combined_sources, confidence=combined_confidence, metadata={'vector_confidence': vector_result.confidence, 'graph_confidence': graph_result.confidence, 'graph_metadata': graph_result.metadata})

    def _fuse_results(self, vector_results: list[dict[str, Any]], graph_context: GraphContext, query_type: QueryType) -> str:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        return {**self.stats, 'fusion_enabled': self.enable_fusion, 'graph_available': self.knowledge_graph is not None, 'vector_available': self.vector_retriever is not None}
...
async def graphrag_query(query: str, query_type: QueryType | None=None, max_results: int=DEFAULT_MAX_RESULTS, **kwargs) -> FusionResult:
