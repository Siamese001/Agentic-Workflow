...
class HealthReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'status': self.status.value, 'timestamp': self.timestamp, 'version': self.version, 'checks': [{'name': c.name, 'status': c.status.value, 'message': c.message, 'duration_ms': round(c.duration_ms, 2), 'metadata': c.metadata} for c in self.checks]}
...
class HealthChecker:
    ...
    def run_check(self, name: str) -> CheckResult | None:
        ...
        # guardian: allow-silent-swallow
            ...
            return CheckResult(name=name, status=HealthStatus.UNHEALTHY, message=f'Check failed with error: {e}', duration_ms=(time.perf_counter() - start) * 1000)
...
class CommonChecks:
    ...
    def env_var_check(var_name: str, required: bool=True) -> Callable[[], CheckResult]:
        ...
        def check() -> CheckResult:
            ...
            if value:
                return CheckResult(name=f'env:{var_name}', status=HealthStatus.HEALTHY, message=f'{var_name} is set')
            elif required:
                return CheckResult(name=f'env:{var_name}', status=HealthStatus.UNHEALTHY, message=f'{var_name} is not set')
            else:
                return CheckResult(name=f'env:{var_name}', status=HealthStatus.DEGRADED, message=f'{var_name} is not set (optional)')
        return check
    ...
    def redis_check(host: str='localhost', port: int=6379) -> Callable[[], CheckResult]:
        ...
        def check() -> CheckResult:
            try:
                ...
                client.ping()
                return CheckResult(name='redis', status=HealthStatus.HEALTHY, message=f'Redis connected at {host}:{port}')
            except ImportError:
                return CheckResult(name='redis', status=HealthStatus.DEGRADED, message='Redis client not installed')
            # guardian: allow-silent-swallow
            except Exception as e:
                return CheckResult(name='redis', status=HealthStatus.UNHEALTHY, message=f'Redis connection failed: {e}')
        return check
        ...
        def check() -> CheckResult:
            try:
                ...
                if free_gb >= min_free_gb:
                    return CheckResult(name='disk_space', status=HealthStatus.HEALTHY, message=f'{free_gb:.2f} GB free', metadata={'free_gb': round(free_gb, 2)})
                else:
                    return CheckResult(name='disk_space', status=HealthStatus.DEGRADED, message=f'Low disk space: {free_gb:.2f} GB free', metadata={'free_gb': round(free_gb, 2)})
            # guardian: allow-silent-swallow
            except Exception as e:
                return CheckResult(name='disk_space', status=HealthStatus.UNKNOWN, message=f'Could not check disk space: {e}')
        return check
        ...
        def check() -> CheckResult:
            try:
                ...
                if percent < max_percent:
                    return CheckResult(name='memory', status=HealthStatus.HEALTHY, message=f'Memory usage: {percent:.1f}%', metadata={'percent': percent})
                else:
                    return CheckResult(name='memory', status=HealthStatus.DEGRADED, message=f'High memory usage: {percent:.1f}%', metadata={'percent': percent})
            except ImportError:
                return CheckResult(name='memory', status=HealthStatus.UNKNOWN, message='psutil not installed')
            # guardian: allow-silent-swallow
            except Exception as e:
                return CheckResult(name='memory', status=HealthStatus.UNKNOWN, message=f'Could not check memory: {e}')
        return check
