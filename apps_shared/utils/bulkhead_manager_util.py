...
class Bulkhead:
    ...
    def __init__(self, name: str, config: BulkheadConfig, enable_circuit_breaker: bool=True):
        ...
        self.metrics = BulkheadMetrics(name=name, max_concurrency=config.max_concurrency, queue_size=config.queue_size)
        ...
        if enable_circuit_breaker:
            self._circuit_breaker_config = CircuitBreakerConfig(failure_threshold=max(3, config.max_concurrency // 2), timeout=DEFAULT_TIMEOUT, failure_rate_threshold=THRESHOLD)
    ...
    async def _get_circuit_breaker(self) -> CircuitBreaker | None:
        ...
        if self.circuit_breaker is None and hasattr(self, '_circuit_breaker_config'):
            ...
            self.circuit_breaker = await registry.get_circuit_breaker(f'bulkhead_{self.name}', self._circuit_breaker_config)
    ...
    async def execute(self, coro: Callable, *args, timeout: float | None=None, **kwargs) -> Any:
        ...
        try:
            if self.queue.full():
                ...
                raise ResourceExhaustedError(self.name, f'Queue full ({self.queue.qsize()}/{self.config.queue_size})')
            ...
            try:
                ...
                self.metrics.rejected_tasks = self._rejected_count
                if circuit_breaker:
                    circuit_breaker.record_failure(asyncio.TimeoutError(f'Timeout acquiring semaphore after {timeout}s'), timeout * 1000)
...
class BulkheadManager:
    ...
    def __init__(self):
        ...
        self._default_configs = {'RESUME_GENERATION': BulkheadConfig(max_concurrency=5, priority=TaskPriority.HIGH, queue_size=50), 'OUTREACH_GENERATION': BulkheadConfig(max_concurrency=10, priority=TaskPriority.MEDIUM, queue_size=100), 'BACKGROUND_ANALYSIS': BulkheadConfig(max_concurrency=2, priority=TaskPriority.LOW, queue_size=20), 'CRITICAL_OPERATIONS': BulkheadConfig(max_concurrency=3, priority=TaskPriority.CRITICAL, queue_size=10, timeout_seconds=60.0)}
    ...
    async def execute(self, bulkhead_name: str, coro: Callable, *args, timeout: float | None=None, **kwargs) -> Any:
    ...
    async def execute_for_engine(self, engine_type: EngineType, coro: Callable, *args, timeout: float | None=None, **kwargs) -> Any:
    ...
    def log_utilization(self) -> None:
        ...
        for name, bulkhead_metrics in metrics['bulkheads'].items():
            logger.info(f'{name}: {bulkhead_metrics.active_tasks}/{bulkhead_metrics.config.max_concurrency} ({bulkhead_metrics.utilization_percent:.1f}%) Queue: {bulkhead_metrics.queued_tasks}/{bulkhead_metrics.config.queue_size}')
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        return {'status': 'healthy' if not issues else 'degraded', 'bulkheads': len(self.bulkheads), 'issues': issues, 'metrics': self.get_all_metrics()}
...
def with_engine_bulkhead(engine_type: EngineType, timeout: float | None=None):
    ...
    Returns:
        Decorated function
    """
    bulkhead_name = {EngineType.RESUME: 'RESUME_GENERATION', EngineType.OUTREACH: 'OUTREACH_GENERATION'}[engine_type]
