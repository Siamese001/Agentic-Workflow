...
class ObservabilityPlanningOrchestrator:
    ...
    def _plan_tracing(self, request: dict[str, Any]) -> TraceConfiguration:
        ...
        return TraceConfiguration(service_name=service_name, sampling_rate=sampling_rate, include_payload=False, max_spans_per_trace=1000, export_batch_size=BATCH_SIZE)
    ...
    def _plan_alerts(self, request: dict[str, Any]) -> list[AlertRule]:
        ...
        service_type = request.get('service_type')
        alerts = []
        alerts.append(AlertRule(name=f'{service_name}_high_error_rate', condition='error_rate > 0.05', severity=AlertSeverity.HIGH, threshold=THRESHOLD, duration=300, notification_channels=['slack', 'email']))
        alerts.append(AlertRule(name=f'{service_name}_high_latency', condition='p95_latency > 1000', severity=AlertSeverity.MEDIUM, threshold=THRESHOLD, duration=600, notification_channels=['slack']))
        if service_type == 'api':
            alerts.append(AlertRule(name=f'{service_name}_api_availability', condition='availability < 0.99', severity=AlertSeverity.CRITICAL, threshold=THRESHOLD, duration=60, notification_channels=['pagerduty', 'slack', 'email']))
        elif service_type == 'worker':
            alerts.append(AlertRule(name=f'{service_name}_queue_backlog', condition='queue_size > 1000', severity=AlertSeverity.HIGH, threshold=THRESHOLD, duration=300, notification_channels=['slack', 'email']))
        return alerts

    def _estimate_resources(self, metrics: list[MetricDefinition], logs: LogConfiguration | None, traces: TraceConfiguration | None) -> dict[str, Any]:
...
def plan_observability(service_name: str, service_type: str, config: dict[str, Any] | None=None) -> dict[str, Any]:
...
if __name__ == '__main__':
    result = plan_observability(service_name='user_service', service_type='api', config={'log_level': 'info', 'tracing_sampling_rate': 0.1})
