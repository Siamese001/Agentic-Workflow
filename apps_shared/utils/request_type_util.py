...
class ObservabilityLoadPlanner:
    ...
    def plan_load(self, load_request: dict[str, Any]) -> ObservabilityLoadResult:
        ...
        try:
            ...
            load_plan = self._create_load_plan(load_request, request_type, data_source, metrics, log_queries, trace_queries)
            ...
            result = ObservabilityLoadResult(success=True, load_plan=load_plan, estimated_data_points=estimated_data_points, query_count=query_count, load_time_estimate=load_time, memory_estimate=memory_estimate, metadata={'planned_at': datetime.utcnow().isoformat(), 'plan_name': load_request.get('plan_name'), 'request_type': request_type.value, 'data_source': data_source.value, 'planner': 'ObservabilityLoadPlanner'})
            self.logger.info(f'Successfully planned observability load: {query_count} queries, ~{estimated_data_points} data points')
            return result
        except Exception as e:
            self.logger.error(f'observability load planning failed: {str(e)}')
            return ObservabilityLoadResult(success=False, errors=[str(e)], metadata={'failed_at': datetime.utcnow().isoformat(), 'planner': 'ObservabilityLoadPlanner'})
    ...
    def _parse_request_type(self, request: dict[str, Any]) -> RequestType:
        ...
        type_mapping = {'metric_query': RequestType.METRIC_QUERY, 'log_search': RequestType.LOG_SEARCH, 'trace_lookup': RequestType.TRACE_LOOKUP, 'aggregation': RequestType.AGGREGATION, 'anomaly_detection': RequestType.ANOMALY_DETECTION}
    ...
    def _parse_data_source(self, request: dict[str, Any]) -> DataSource:
        ...
        source_mapping = {'prometheus': DataSource.PROMETHEUS, 'elasticsearch': DataSource.ELASTICSEARCH, 'jaeger': DataSource.JAEGER, 'zipkin': DataSource.ZIPKIN, 'grafana': DataSource.GRAFANA, 'datadog': DataSource.DATADOG}
        ...
        for raw_metric in raw_metrics:
            if isinstance(raw_metric, dict):
                ...
                if 'aggregation' in raw_metric:
                    agg_mapping = {'sum': AggregationType.SUM, 'avg': AggregationType.AVG, 'min': AggregationType.MIN, 'max': AggregationType.MAX, 'count': AggregationType.COUNT, 'percentile': AggregationType.PERCENTILE}
                    aggregation = agg_mapping.get(raw_metric.get('aggregation'), AggregationType.AVG)
                metric = MetricDefinition(name=raw_metric.get('name', 'unnamed'), query=raw_metric.get('query', ''), labels=raw_metric.get('labels', {}), aggregation=aggregation, time_range=raw_metric.get('time_range', self.config.default_time_range), step=raw_metric.get('step', 60))
                metrics.append(metric)
        if len(metrics) > self.config.max_queries_per_plan:
            raise ValueError(f'Number of metrics ({len(metrics)}) exceeds maximum ({self.config.max_queries_per_plan})')
        return metrics
    ...
    def _parse_log_queries(self, request: dict[str, Any]) -> list[LogQuery]:
        ...
        for raw_query in raw_queries:
            if isinstance(raw_query, dict):
                query = LogQuery(index=raw_query.get('index', 'logs-*'), query=raw_query.get('query', '*'), filters=raw_query.get('filters', {}), time_range=raw_query.get('time_range', self.config.default_time_range), size=raw_query.get('size', 1000), sort_field=raw_query.get('sort_field', '@timestamp'), sort_order=raw_query.get('sort_order', 'desc'))
                queries.append(query)
        if len(queries) > self.config.max_queries_per_plan:
            raise ValueError(f'Number of log queries ({len(queries)}) exceeds maximum ({self.config.max_queries_per_plan})')
        return queries
    ...
    def _parse_trace_queries(self, request: dict[str, Any]) -> list[TraceQuery]:
        ...
        for raw_query in raw_queries:
            if isinstance(raw_query, dict):
                query = TraceQuery(service=raw_query.get('service'), operation=raw_query.get('operation'), trace_id=raw_query.get('trace_id'), tags=raw_query.get('tags', {}), time_range=raw_query.get('time_range', self.config.default_time_range), limit=raw_query.get('limit', 100))
                queries.append(query)
        if len(queries) > self.config.max_queries_per_plan:
            raise ValueError(f'Number of trace queries ({len(queries)}) exceeds maximum ({self.config.max_queries_per_plan})')
        return queries

    def _create_load_plan(self, request: dict[str, Any], request_type: RequestType, data_source: DataSource, metrics: list[MetricDefinition], log_queries: list[LogQuery], trace_queries: list[TraceQuery]) -> ObservabilityLoadPlan:
        """Create observability load plan from parsed components."""
        return ObservabilityLoadPlan(id=request.get('plan_id', f"plan_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"), name=request.get('plan_name', 'unnamed_plan'), request_type=request_type, data_source=data_source, metrics=metrics, log_queries=log_queries, trace_queries=trace_queries, enable_caching=request.get('enable_caching', True), cache_ttl=request.get('cache_ttl', 300), enable_sampling=request.get('enable_sampling', False), sample_rate=request.get('sample_rate', 1.0), metadata=request.get('metadata', {}))
...
def create_observability_load_planner(enable_metrics: bool=True, enable_logs: bool=True, enable_traces: bool=True, **kwargs: object) -> ObservabilityLoadPlanner:
    """Create a configured observability load planner."""
    config = ObservabilityLoadConfig(enable_metrics=enable_metrics, enable_logs=enable_logs, enable_traces=enable_traces, **kwargs)
    return ObservabilityLoadPlanner(config)

def plan_observability_load(plan_name: str, request_type: str, data_source: str='prometheus', metrics: list[dict[str, Any]] | None=None, log_queries: list[dict[str, Any]] | None=None, trace_queries: list[dict[str, Any]] | None=None, config: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    """
    request = {'plan_name': plan_name, 'request_type': request_type, 'data_source': data_source, 'metrics': metrics or [], 'log_queries': log_queries or [], 'trace_queries': trace_queries or []}
    ...
    return {'success': result.success, 'load_plan': {'id': result.load_plan.id, 'name': result.load_plan.name, 'request_type': result.load_plan.request_type.value, 'data_source': result.load_plan.data_source.value, 'metrics': [{'name': m.name, 'query': m.query, 'labels': m.labels, 'aggregation': m.aggregation.value if m.aggregation else None, 'time_range': m.time_range, 'step': m.step} for m in result.load_plan.metrics], 'log_queries': [{'index': q.index, 'query': q.query, 'filters': q.filters, 'time_range': q.time_range, 'size': q.size, 'sort_field': q.sort_field, 'sort_order': q.sort_order} for q in result.load_plan.log_queries], 'trace_queries': [{'service': q.service, 'operation': q.operation, 'trace_id': q.trace_id, 'tags': q.tags, 'time_range': q.time_range, 'limit': q.limit} for q in result.load_plan.trace_queries], 'enable_caching': result.load_plan.enable_caching, 'cache_ttl': result.load_plan.cache_ttl, 'enable_sampling': result.load_plan.enable_sampling, 'sample_rate': result.load_plan.sample_rate, 'metadata': result.load_plan.metadata} if result.load_plan else None, 'estimated_data_points': result.estimated_data_points, 'query_count': result.query_count, 'load_time_estimate': result.load_time_estimate, 'memory_estimate': result.memory_estimate, 'warnings': result.warnings, 'errors': result.errors, 'metadata': result.metadata}
...
class LoadDataPlanningPlanImpl(LoadDataPlanningPlanProcessor):
    ...
    def process(self, input_data: dict[str, object]) -> LoadDataPlanningPlanResult:
        ...
        result = LoadDataPlanningPlanResult(success=True, data={'processed': True, 'input': input_data}, safety_validated=True, timestamp=self._get_timestamp())
