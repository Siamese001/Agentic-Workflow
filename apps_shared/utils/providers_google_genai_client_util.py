...
def run_llm_google(model: str, prompt: str, *, temperature: float, max_tokens: int, timeout_s: int, use_interactions_api: bool=True) -> str:
    ...
    if use_interactions_api:
        try:
            ...
            response = client.interactions.create(model=model, input=input_messages, config={'temperature': temperature, 'max_output_tokens': max_tokens})
