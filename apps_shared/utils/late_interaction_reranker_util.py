...
class LateInteractionReranker:
    ...
    def rerank(self, query: str, documents: list[str], top_k: int | None=None, batch_size: int=32) -> list[str]:
        ...
        try:
            ...
            if scores is not None and len(scores) > 0:
                score_stats = {'min': float(min(scores)), 'max': float(max(scores)), 'mean': float(sum(scores) / len(scores))}
    ...
    def rerank_with_scores(self, query: str, documents: list[str], top_k: int | None=None, batch_size: int=32) -> list[tuple[str, float]]:
    ...
    def get_model_info(self) -> dict:
        ...
        """
        info = {'model_name': self.model_name, 'loaded': self._model_loaded, 'fallback_mode': self._fallback_mode, 'available': self.is_available}
        if self._model_loaded and (not self._fallback_mode):
            try:
                if hasattr(self._model, 'config'):
                    info.update({'max_seq_length': getattr(self._model.config, 'max_position_embeddings', 'unknown'), 'num_labels': getattr(self._model.config, 'num_labels', 'unknown')})
            # guardian: allow-silent-swallow
            except Exception:
                pass
        return info

def rerank_documents(query: str, documents: list[str], model_name: str='BAAI/bge-reranker-v2-m3', top_k: int=5) -> list[str]:
