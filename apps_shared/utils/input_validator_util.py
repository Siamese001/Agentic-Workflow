...
class InputValidator:
        ...
        if 'properties' in schema and isinstance(value, dict):
            for prop, prop_schema in schema['properties'].items():
                if prop in value:
                    ...
                    rule = ValidationRule(prop, self._get_validation_type_from_schema(prop_schema), required=schema.get('required', {}).get(prop, False))
    ...
    def _get_validation_type_from_schema(self, schema: dict[str, Any]) -> ValidationType:
        ...
        Returns:
            Validation type
        """
        type_map = {'string': ValidationType.STRING, 'integer': ValidationType.INTEGER, 'number': ValidationType.FLOAT, 'boolean': ValidationType.BOOLEAN, 'array': ValidationType.LIST, 'object': ValidationType.DICT}
    ...
    def _sanitize_value(self, value: Any, rule: ValidationRule) -> Any:
        ...
        if isinstance(value, str):
            ...
            if rule.max_length:
                value = value[:rule.max_length]
        return value
COMMON_RULES = {'hop_id': ValidationRule('hop_id', ValidationType.STRING, required=True, min_length=1, max_length=HOP_ID_MAX_LENGTH, pattern='^[a-zA-Z0-9_-]+$'), 'context_data': ValidationRule('context_data', ValidationType.DICT, required=False, max_length=CONTEXT_DATA_MAX_KEYS), 'retry_count': ValidationRule('retry_count', ValidationType.INTEGER, required=False, min_value=0, max_value=RETRY_COUNT_MAX), 'timeout': ValidationRule('timeout', ValidationType.FLOAT, required=False, min_value=TIMEOUT_MIN_SECONDS, max_value=TIMEOUT_MAX_SECONDS), 'json_payload': ValidationRule('json_payload', ValidationType.JSON, required=False, max_length=JSON_PAYLOAD_MAX_LENGTH), 'xml_content': ValidationRule('xml_content', ValidationType.XML, required=False, max_length=XML_CONTENT_MAX_LENGTH)}
