...
class ToneEnforcer:
    ...
    def __init__(self):
        ...
        self.voice_patterns = {ToneVoice.AUTHORITATIVE: {'verbs': ['led', 'drove', 'built', 'created', 'established', 'pioneered'], 'patterns': ['\\bled\\b', '\\bdrove\\b', '\\bbuilt\\b'], 'avoid': ['helped', 'assisted', 'participated']}, ToneVoice.COLLABORATIVE: {'verbs': ['partnered', 'enabled', 'supported', 'collaborated', 'facilitated'], 'patterns': ['\\bpartnered\\b', '\\benabled\\b', '\\bsupported\\b'], 'avoid': ['controlled', 'dictated', 'commanded']}, ToneVoice.TECHNICAL: {'terms': ['architecture', 'scalability', 'optimization', 'infrastructure'], 'patterns': ['\\bimplement\\b', '\\boptimize\\b', '\\barchitect\\b'], 'avoid': ['simple', 'easy', 'basic']}, ToneVoice.EXECUTIVE: {'verbs': ['strategized', 'orchestrated', 'spearheaded', 'directed'], 'patterns': ['\\bstrategized\\b', '\\borchestrated\\b'], 'avoid': ['executed', 'performed', 'completed']}, ToneVoice.CREATIVE: {'verbs': ['innovated', 'envisioned', 'imagined', 'designed'], 'patterns': ['\\binnovated\\b', '\\benvisioned\\b'], 'avoid': ['maintained', 'preserved', 'followed']}}
        self.passive_patterns = ['\\b(was|were|is|are|am|been|being)\\s+\\w+ed\\b', '\\b(was|were|is|are|am|been|being)\\s+\\w+en\\b', '\\b\\w+ed\\s+by\\s+the\\b', '\\b\\w+ed\\s+by\\s+\\w+\\b']
    ...
    def _create_default_profiles(self) -> dict[ToneVoice, ToneSettings]:
        ...
        return {ToneVoice.AUTHORITATIVE: ToneSettings(primary_voice=ToneVoice.AUTHORITATIVE, max_sentence_length=20, banned_words=['helped', 'assisted', 'participated', 'contributed'], required_keywords=['led', 'drove', 'built'], preferred_verbs={'leadership': ['led', 'directed', 'managed', 'orchestrated'], 'creation': ['built', 'created', 'developed', 'established'], 'impact': ['drove', 'achieved', 'delivered', 'produced']}, max_passive_voice_percent=10.0), ToneVoice.COLLABORATIVE: ToneSettings(primary_voice=ToneVoice.COLLABORATIVE, max_sentence_length=25, banned_words=['controlled', 'dictated', 'commanded', 'forced'], required_keywords=['partnered', 'enabled', 'collaborated'], preferred_verbs={'teamwork': ['partnered', 'collaborated', 'teamed', 'cooperated'], 'support': ['enabled', 'supported', 'facilitated', 'empowered'], 'guidance': ['mentored', 'guided', 'advised', 'coached']}, max_passive_voice_percent=15.0), ToneVoice.TECHNICAL: ToneSettings(primary_voice=ToneVoice.TECHNICAL, max_sentence_length=30, banned_words=['simple', 'easy', 'basic', 'just'], required_keywords=['implemented', 'optimized', 'architected'], preferred_verbs={'development': ['implemented', 'developed', 'programmed', 'coded'], 'architecture': ['architected', 'designed', 'structured', 'engineered'], 'optimization': ['optimized', 'enhanced', 'improved', 'refined']}, max_passive_voice_percent=20.0), ToneVoice.EXECUTIVE: ToneSettings(primary_voice=ToneVoice.EXECUTIVE, max_sentence_length=25, banned_words=['executed', 'performed', 'completed', 'finished'], required_keywords=['strategized', 'orchestrated', 'spearheaded'], preferred_verbs={'strategy': ['strategized', 'planned', 'envisioned', 'conceptualized'], 'leadership': ['orchestrated', 'spearheaded', 'directed', 'guided'], 'business': ['drove', 'grew', 'expanded', 'scaled']}, max_passive_voice_percent=5.0), ToneVoice.CREATIVE: ToneSettings(primary_voice=ToneVoice.CREATIVE, max_sentence_length=30, banned_words=['standard', 'typical', 'conventional', 'traditional'], required_keywords=['innovated', 'designed', 'created'], preferred_verbs={'innovation': ['innovated', 'pioneered', 'invented', 'conceived'], 'design': ['designed', 'crafted', 'shaped', 'formed'], 'vision': ['envisioned', 'imagined', 'conceptualized', 'dreamed']}, max_passive_voice_percent=25.0)}
    ...
    def _check_sentence_length(self, text: str, settings: ToneSettings) -> list[ToneViolation]:
        ...
        for sentence in sentences:
            ...
            if word_count > settings.max_sentence_length:
                violations.append(ToneViolation(type='sentence_length', severity='warning', message=f'Sentence too long: {word_count} words (max: {settings.max_sentence_length})', location=sentence[:50] + '...' if len(sentence) > 50 else sentence, suggestion='Consider breaking into shorter sentences'))
            elif word_count < settings.min_sentence_length:
                violations.append(ToneViolation(type='sentence_length', severity='info', message=f'Sentence too short: {word_count} words (min: {settings.min_sentence_length})', location=sentence, suggestion='Consider expanding with more detail'))
        return violations
    ...
    def _check_banned_words(self, text: str, settings: ToneSettings) -> list[ToneViolation]:
        ...
        for word in settings.banned_words:
            if word.lower() in text_lower:
                ...
                violations.append(ToneViolation(type='banned_word', severity='error', message=f"Banned word detected: '{word}'", location=match.group() if match else word, suggestion='Replace with stronger alternative'))
        return violations
    ...
    def _check_required_keywords(self, text: str, settings: ToneSettings) -> list[ToneViolation]:
        ...
        if missing_keywords:
            violations.append(ToneViolation(type='missing_keyword', severity='warning', message=f"Missing required keywords: {', '.join(missing_keywords)}", suggestion='Consider incorporating these keywords'))
        return violations
    ...
    def _check_voice_consistency(self, text: str, settings: ToneSettings) -> list[ToneViolation]:
        ...
        for avoid_word in patterns['avoid']:
            if avoid_word.lower() in text_lower:
                violations.append(ToneViolation(type='voice_inconsistency', severity='warning', message=f"Voice inconsistency: '{avoid_word}' doesn't match {voice.value} tone", suggestion=f"Use {voice.value} voice alternatives: {', '.join(patterns['verbs'][:3])}"))
        if preferred_found == 0 and len(patterns['patterns']) > 0:
            violations.append(ToneViolation(type='voice_weakness', severity='info', message=f'Weak {voice.value} voice: no preferred patterns detected', suggestion=f"Consider using: {', '.join(patterns['verbs'][:3])}"))
        return violations
    ...
    def _check_passive_voice(self, text: str, settings: ToneSettings) -> list[ToneViolation]:
        ...
        if passive_percent > settings.max_passive_voice_percent:
            violations.append(ToneViolation(type='passive_voice', severity='warning', message=f'Too much passive voice: {passive_percent:.1f}% (max: {settings.max_passive_voice_percent}%)', suggestion='Use more active voice construction'))
        return violations
        ...
        if settings.formality_level == 'formal':
            ...
            for pattern in informal_patterns:
                if re.search(pattern, text.lower()):
                    violations.append(ToneViolation(type='formality', severity='warning', message='Informal language detected in formal tone', suggestion='Use formal language'))
        elif settings.formality_level == 'professional':
            casual_patterns = ["\\by'all\\b", "\\bain't\\b", '\\bcuz\\b']
            for pattern in casual_patterns:
                if re.search(pattern, text.lower()):
                    violations.append(ToneViolation(type='formality', severity='error', message='Overly casual language detected', suggestion='Use professional language'))
        return violations
    ...
    def analyze_tone(self, text: str) -> ToneAnalysisResult:
        ...
        return ToneAnalysisResult(is_compliant=score >= 0.8, violations=violations, score=score, voice_detected=voice_detected, metrics=metrics)
    ...
    def _calculate_metrics(self, text: str, violations: list[ToneViolation]) -> dict[str, float]:
        ...
        return {'sentences': float(sentences), 'words': float(words), 'avg_sentence_length': words / sentences if sentences > 0 else 0.0, 'violations_by_type': {k: float(v) for k, v in violation_counts.items()}}
