...
class NegotiationConfig(BaseModel):
    ...
    response_timeout: float = Field(default_factory=lambda: float(os.getenv('NEGOTIATION_RESPONSE_TIMEOUT', '30.0')), ge=5.0, le=300.0)
    enable_persistence: bool = True
    auto_resolve_threshold: float = Field(default_factory=lambda: float(os.getenv('NEGOTIATION_AUTO_RESOLVE_THRESHOLD', '0.8')), ge=0.0, le=1.0)
...
class NodeNegotiator:
    ...
    def __init__(self, config: NegotiationConfig | None=None):
        ...
        self.stats = {'total_negotiations': 0, 'successful_negotiations': 0, 'escalated_negotiations': 0, 'average_rounds': 0.0}
        self._register_default_handlers()
        logger.info('Initialized NodeNegotiator')

    def _register_default_handlers(self) -> None:
        """Register default message handlers."""
        self.message_handlers.update({'CLARIFICATION_REQUEST': self._handle_clarification, 'CHANGE_REQUEST': self._handle_change_request, 'REJECTION': self._handle_rejection})

    async def send_feedback(self, from_hop: SubatomicHop, to_hop_id: str, message_type: str, payload: str, context: dict[str, Any] | None=None, priority: int=0) -> bool:
        ...
        """
        message = NegotiationMessage(from_hop=from_hop.config.hop_id, to_hop=to_hop_id, message_type=message_type, payload=payload, context=context or {}, priority=priority)
    ...
    async def request_change(self, downstream_hop: SubatomicHop, upstream_hop_id: str, requested_change: str, reason: str, context: dict[str, Any] | None=None) -> NegotiationResult:
        ...
        negotiation = self.active_negotiations[round_id]
        await self.send_feedback(from_hop=downstream_hop, to_hop_id=upstream_hop_id, message_type='CHANGE_REQUEST', payload=f'Please modify output: {requested_change}', context={'reason': reason, **(context or {})}, priority=5)
    ...
    async def _handle_clarification(self, message: NegotiationMessage, negotiation: NegotiationRound) -> None:
        ...
        response = NegotiationMessage(from_hop=message.to_hop, to_hop=message.from_hop, message_type='CLARIFICATION_RESPONSE', payload='Clarification: The output meets the specified format requirements', context={'original_message_id': message.message_id})
        negotiation.messages.append(response)

    async def _handle_change_request(self, message: NegotiationMessage, negotiation: NegotiationRound) -> None:
        ...
        if upstream_hop.state == HopState.COMPLETED:
            ...
            upstream_hop.context['negotiation_request'] = {'from_hop': message.from_hop, 'request': message.payload, 'context': message.context}
            if 'negotiation_log' not in upstream_hop.context:
                upstream_hop.context['negotiation_log'] = []
            upstream_hop.context['negotiation_log'].append({'timestamp': message.timestamp.isoformat(), 'from': message.from_hop, 'message': message.payload})
    ...
    def _get_or_create_round(self, hop1_id: str, hop2_id: str) -> str:
        ...
        for round_id, negotiation in self.active_negotiations.items():
            if hop1_id in negotiation.participants and hop2_id in negotiation.participants and (negotiation.status == 'ACTIVE'):
                return round_id
        round_id = f'neg_{int(time.time() * 1000)}_{hop1_id}_{hop2_id}'
        self.active_negotiations[round_id] = NegotiationRound(round_id=round_id, participants=[hop1_id, hop2_id])
        return round_id
    ...
    async def _wait_for_resolution(self, negotiation: NegotiationRound) -> NegotiationResult:
        ...
        return NegotiationResult(success=success, resolution_type='AGREEMENT' if success else 'TIMEOUT', negotiation_log=[msg.payload for msg in negotiation.messages], rounds_completed=rounds_completed)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {**self.stats, 'active_negotiations': len(self.active_negotiations), 'config': {'max_rounds': self.config.max_rounds, 'timeout': self.config.response_timeout}}
...
async def request_upstream_change(downstream_hop: SubatomicHop, upstream_hop_id: str, change_request: str, reason: str, **kwargs) -> NegotiationResult:
    ...
    return await negotiator.request_change(downstream_hop=downstream_hop, upstream_hop_id=upstream_hop_id, requested_change=change_request, reason=reason, context=kwargs)
...
async def send_clarification(from_hop: SubatomicHop, to_hop_id: str, question: str, **kwargs) -> bool:
    ...
    """
    negotiator = get_node_negotiator()
    return await negotiator.send_feedback(from_hop=from_hop, to_hop_id=to_hop_id, message_type='CLARIFICATION_REQUEST', payload=question, context=kwargs)
...
class NegotiatingHop(SubatomicHop):
    ...
    async def evaluate_downstream_feedback(self, downstream_output: Any, expected_criteria: list[str]) -> bool:
    ...
    async def request_upstream_modification(self, upstream_hop_id: str, modification: str, reason: str) -> NegotiationResult:
        ...
        return await request_upstream_change(downstream_hop=self, upstream_hop_id=upstream_hop_id, change_request=modification, reason=reason)
