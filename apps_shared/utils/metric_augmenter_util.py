...
class MetricAugmenter:
    ...
    def __init__(self, industry: str='technology'):
        ...
        self.metric_mappings = {'latency': ImpactCategory.RETENTION, 'speed': ImpactCategory.RETENTION, 'response_time': ImpactCategory.RETENTION, 'throughput': ImpactCategory.REVENUE, 'accuracy': ImpactCategory.REVENUE, 'f1': ImpactCategory.REVENUE, 'precision': ImpactCategory.REVENUE, 'recall': ImpactCategory.REVENUE, 'storage': ImpactCategory.OPEX, 'compute': ImpactCategory.OPEX, 'infrastructure': ImpactCategory.OPEX, 'cloud': ImpactCategory.OPEX, 'cost': ImpactCategory.OPEX, 'uptime': ImpactCategory.RISK, 'reliability': ImpactCategory.RISK, 'availability': ImpactCategory.RISK, 'migration': ImpactCategory.CAPEX, 'deployment': ImpactCategory.OPEX}
        self.impact_templates = {ImpactCategory.RETENTION: ['improving user retention by est. {value}%', 'reducing churn by approximately {value}%', 'enhancing customer satisfaction by est. {value}%'], ImpactCategory.REVENUE: ['generating est. ${value}M in additional revenue', 'enabling est. {value}% revenue growth', 'contributing to est. ${value}K in monthly revenue'], ImpactCategory.OPEX: ['reducing operational costs by est. {value}%', 'saving est. ${value}K monthly in cloud expenses', 'slashing infrastructure spend by est. {value}%'], ImpactCategory.RISK: ['mitigating est. ${value}M in potential downtime costs', 'reducing compliance risk by est. {value}%', 'preventing est. ${value} hours of system downtime'], ImpactCategory.CAPEX: ['deferring est. ${value}M in hardware purchases', 'enabling est. {value}% reduction in capital needs', 'optimizing asset utilization by est. {value}%']}
        self.industry_multipliers = {'technology': {'revenue': 1.2, 'cost': 1.0}, 'finance': {'revenue': 1.5, 'cost': 1.2}, 'healthcare': {'revenue': 1.3, 'cost': 1.1}, 'retail': {'revenue': 1.1, 'cost': 0.9}, 'manufacturing': {'revenue': 1.0, 'cost': 1.3}}
    ...
    def augment_bullet(self, bullet_text: str) -> AugmentedBullet:
        ...
        try:
            ...
            if not technical_metrics:
                return AugmentedBullet(original_text=bullet_text, technical_metric=None, business_impact=None, final_text=bullet_text)
            selected_metric = self._select_highest_impact_metric(technical_metrics)
            business_impact = self._estimate_impact(selected_metric['type'], selected_metric['value'], bullet_text)
            if not business_impact:
                return AugmentedBullet(original_text=bullet_text, technical_metric=selected_metric['type'], business_impact=None, final_text=bullet_text)
            final_text = self._create_augmented_text(bullet_text, business_impact)
            return AugmentedBullet(original_text=bullet_text, technical_metric=selected_metric['type'], business_impact=business_impact, final_text=final_text)
    ...
    def _select_highest_impact_metric(self, metrics: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        impact_priority = {'cost': 10, 'latency': 9, 'accuracy': 8, 'throughput': 7, 'uptime': 6, 'storage': 5, 'compute': 5, 'migration': 4, 'deployment': 3, 'refactoring': 2, 'optimization': 2}
    ...
    def _detect_metrics(self, text: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            detected_metrics = []
            patterns = {'latency': ['(\\d+)\\s*ms', '(\\d+)\\s*milliseconds?', 'reduced.*latency.*by\\s*(\\d+)', 'improved.*latency.*to\\s*(\\d+)'], 'accuracy': ['(\\d+(?:\\.\\d+)?)\\s*%?\\s*accuracy', 'accuracy.*to\\s*(\\d+(?:\\.\\d+)?)', 'f1.*score.*of\\s*(\\d+(?:\\.\\d+)?)', 'precision.*(\\d+(?:\\.\\d+)?)', 'recall.*(\\d+(?:\\.\\d+)?)'], 'throughput': ['(\\d+(?:k|m|g)?)\\s*(?:req\\/s|rps|requests?\\/second)', 'throughput.*by\\s*(\\d+)', 'processing.*(\\d+(?:k|m)?)\\s*messages?'], 'storage': ['(\\d+(?:gb|tb|pb))', 'storage.*by\\s*(\\d+)', 'reduced.*storage.*by\\s*(\\d+)'], 'compute': ['(\\d+)%?\\s*(?:cpu|compute)', 'nodes?.*(\\d+)', 'cluster.*(\\d+)', 'k8s|kubernetes'], 'cost': ['\\$(\\d+(?:k|m)?)', 'cost.*by\\s*(\\d+)', 'saving.*\\$(\\d+)', 'reduced.*spend.*by\\s*(\\d+)'], 'uptime': ['(\\d+\\.?\\d*)%?\\s*uptime', 'availability.*(\\d+\\.?\\d*)', 'sla.*(\\d+\\.?\\d*)']}
            for metric_type, metric_patterns in patterns.items():
                for pattern in metric_patterns:
                    ...
                    for match in matches:
                        ...
                        break
            keywords = {'migration': ['migration', 'migrated'], 'deployment': ['deployment', 'deployed'], 'refactoring': ['refactor', 'refactored'], 'optimization': ['optimiz', 'optimised']}
            for metric_type, keyword_list in keywords.items():
                if any((keyword in text_lower for keyword in keyword_list)):
                    detected_metrics.append({'type': metric_type, 'value': 'significant', 'pattern': 'keyword'})
