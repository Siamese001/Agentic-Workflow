...
class ETLPipeline:
    ...
    def hydrate_cache(self, min_success_count: int=10, max_patterns: int=50, project_filter: str | None=None) -> dict[str, Any]:
        """
        ...
        golden_patterns = self._fetch_golden_patterns(min_success_count=min_success_count, max_patterns=max_patterns, project_filter=project_filter)
        loaded_count = self._load_to_redis(golden_patterns)
        stats = {'fetched_from_pinecone': len(golden_patterns), 'loaded_to_redis': loaded_count, 'min_success_count': min_success_count, 'max_patterns': max_patterns, 'project_filter': project_filter}
        logger.info(f'cache hydration complete: {stats}')
        return stats

    def _fetch_golden_patterns(self, min_success_count: int, max_patterns: int, project_filter: str | None) -> list[CanonEntry]:
        ...
        try:
            ...
            results = index.query(vector=[0.0] * 768, top_k=max_patterns, include_metadata=True, filter=filter_dict)
            patterns = []
            for match in results['matches']:
                if match['score'] > 0:
                    metadata = match['metadata']
                    entry = CanonEntry(id=match['id'], code_snippet=metadata['code_snippet'], ast_structure=metadata['ast_structure'], embedding=match['values'] if 'values' in match else [0.0] * 768, metadata={'failure_count': metadata['failure_count'], 'success_count': metadata['success_count'], 'last_validated': metadata['last_validated'], 'project_context': metadata['project_context'], 'canon_rule_id': metadata['canon_rule_id']})
    ...
    def _load_to_redis(self, patterns: list[CanonEntry]) -> int:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                self.redis_index.load(documents=[{'id': fields['id'], 'embedding': fields['embedding'], 'failure_count': fields['failure_count'], 'success_count': fields['success_count'], 'project_context': fields['project_context'], 'canon_rule_id': fields['canon_rule_id'], 'last_validated': fields['last_validated']}])
                loaded_count += 1
            pipe.execute()
    ...
    # guardian: allow-magic-config
    def backfill_from_code(self, code_files: list[str], project_context: str='backfill', batch_size: int=100) -> dict[str, Any]:
        """
        ...
        stats = {'total_files': len(code_files), 'processed': processed, 'failed': failed, 'batch_size': batch_size}
    ...
    def _create_entry_from_code(self, code: str, project_context: str, file_path: str) -> CanonEntry:
        ...
        entry = CanonEntry(code_snippet=code, ast_structure=ast_structure, embedding=embedding, metadata={'failure_count': 0, 'success_count': 0, 'project_context': project_context, 'canon_rule_id': 'backfill'})
        return entry
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats['redis'] = {'connected_clients': redis_info.get('connected_clients', 0), 'used_memory': redis_info.get('used_memory_human', '0B'), 'keyspace_hits': redis_info.get('keyspace_hits', 0), 'keyspace_misses': redis_info.get('keyspace_misses', 0)}
        # guardian: allow-silent-swallow
        except Exception as e:
        ...
        try:
            ...
            stats['pinecone'] = {'vector_count': index_stats.get('total_vector_count', 0), 'dimension': index_stats.get('dimension', 0), 'index_fullness': index_stats.get('index_fullness', 0)}
        # guardian: allow-silent-swallow
        except Exception as e:
