...
class MemoryManagerAgent(MCPHardenedMixin, SubatomicTestingMixin, HealerMixin):
    ...
    def load_validation_results(self, session_id: str = None) -> dict[str, Any]:
        ...
        if session_id is None:
            ...
            results_files: Any = sorted(
                [f for f in get_json_files(self.results_dir) if f.name.startswith("results_")]
            )
    ...
    def heal_repository(
        ...
        try:
            ...
            if hasattr(self, "_validate_memory_integrity"):
                ...
                metrics["violations_found"] += integrity_results.get(
                    "violations_found", integrity_results.get("violations", 0)
                )
                metrics["violations_fixed"] += integrity_results.get(
                    "violations_fixed", integrity_results.get("fixed", 0)
                )

            # 2. Staleness Cleanup
            if hasattr(self, "_cleanup_stale_memories"):
                cleanup_results = self._cleanup_stale_memories(dry_run=dry_run)
                metrics["violations_found"] += cleanup_results.get(
                    "violations_found", cleanup_results.get("violations", 0)
                )
                metrics["violations_fixed"] += cleanup_results.get(
                    "violations_fixed", cleanup_results.get("fixed", 0)
                )
