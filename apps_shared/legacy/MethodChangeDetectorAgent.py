...
from agentic_core.base_agents.healer_mixin import HealerMixin, HealResult
    ...
    def detect_method_changes(self, file_path: str) -> list[MethodChange]:
        ...
        for key_id, data in history.items():
            ...
            try:
                ...
                continue
            before_methods: Any = (
                self._extract_methods(before_tree, before_code) if before_tree else {}
            )
                ...
                if not before_method_code and after_method_code:
                    ...
                    changes.append(
                        MethodChange(
                            file_path=file_path,
                            method_name=method_name,
                            before_code=before_method_code,
                            after_code="",
                            is_new=False,
                            is_modified=False,
                            is_deleted=True,
...
class RegressionTestGenerator:
    ...
    async def generate_test_code_and_file(
        self, change: MethodChange
    ...
    async def _synthesize_with_gemini(self, change: MethodChange, edge_cases: list[str]) -> str:
        ...
        response = await self.genai_client.models.generate_content_async(
            ...
            contents=prompt,
            generation_config=types.GenerationConfig(temperature=0.2, max_output_tokens=2048),
        )
...
class RegressionTestRunner:
    ...
    async def run_and_correct_test(
        self, change: MethodChange, test_file: Path, test_code: str
    ) -> tuple[bool, str | None]:
    ...
    async def _run_test(self, test_file: Path) -> tuple[bool, str | None]:
        ...
        try:
            result = subprocess.run(
                ["pytest", str(test_file), "-v"], capture_output=True, text=True, timeout=30
            )
    ...
    async def _self_correct(
        self, change: MethodChange, test_code: str, error_msg: str
    ) -> tuple[bool, str | None]:
        """
            ...
            if "code_regression" in analysis.lower():
                ...
                if hasattr(self.ctx, "signals"):
                    self.ctx.signals.add(
                        f"REGRESSION_DETECTED:{change.file_path}:{change.method_name}"
                    )
    ...
    async def _auto_fix_test(
        self, change: MethodChange, test_code: str, error_msg: str, analysis: str
    ) -> str | None:
        """
