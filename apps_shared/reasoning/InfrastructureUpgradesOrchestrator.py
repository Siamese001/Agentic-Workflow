...
class InfrastructureUpgradesOrchestrator:
    ...
    async def _setup_event_subscriptions(self) -> None:
        ...
        await self.infrastructure.event_bus.subscribe('events.content_generated', self._handle_content_generated)
    ...
    async def _handle_content_generated(self, event: SystemEvent) -> None:
        ...
        try:
            ...
            if content:
                ...
                if violations:
                    await self.infrastructure.event_bus.publish('events.fact_violations', SystemEvent(type=EventType.ERROR_OCCURRED, trace_id=event.trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'violations': [v.dict() for v in violations], 'content_length': len(content)}, causation_id=event.id))
    ...
    async def _verify_content_facts(self, content: str, trace_id: str) -> list[VerificationResult]:
        ...
        for sentence in sentences:
            ...
            if result.status == 'CONFLICT':
                ...
                await self.infrastructure.event_bus.publish('events.fact_conflict', SystemEvent(type=EventType.ERROR_OCCURRED, trace_id=trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'claim': sentence, 'correction': result.correction_suggestion, 'verified_value': result.verified_fact.value if result.verified_fact else None}))
        return violations

    async def generate_with_upgrades(self, task_type: TaskType, prompt: str, tone_voice: ToneVoice | None=None, verify_facts: bool=True, enforce_tone: bool=True, use_cache: bool=True, trace_id: str | None=None) -> dict[str, Any]:
        ...
        if use_cache:
            ...
            if cached_result:
                await self.infrastructure.event_bus.publish('events.cache_hit', SystemEvent(type=EventType.AGENT_THINKING, trace_id=trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'cache_key': cache_key}))
                return cached_result
        await self.infrastructure.event_bus.publish('events.upgraded_generation_started', SystemEvent(type=EventType.WORKFLOW_STARTED, trace_id=trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'task_type': task_type.value, 'verify_facts': verify_facts, 'enforce_tone': enforce_tone}))
        try:
            result = await self.infrastructure.execute_with_infrastructure(task_type, prompt, complexity_score=5, trace_id=trace_id)
            ...
            if enforce_tone and tone_voice:
                ...
                if tone_violations:
                    await self.infrastructure.event_bus.publish('events.tone_violation', SystemEvent(type=EventType.ERROR_OCCURRED, trace_id=trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'violations': [v.dict() for v in tone_violations], 'tone_voice': tone_voice.value}))
            fact_violations = []
            if verify_facts:
                fact_violations = await self._verify_content_facts(content, trace_id)
            if use_cache:
                cache_key = f'{task_type.value}:{prompt}'
                self.global_cache.put(cache_key, result, text_for_embedding=prompt, source_engine='InfrastructureUpgrades')
            await self.infrastructure.event_bus.publish('events.upgraded_generation_complete', SystemEvent(type=EventType.ARTIFACT_GENERATED, trace_id=trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'content_length': len(content), 'tone_violations': len(tone_violations), 'fact_violations': len(fact_violations), 'cached': use_cache}))
            return {'content': content, 'trace_id': trace_id, 'metadata': {'model_used': result['model_used'], 'tier': result['tier'], 'execution_time': result['execution_time'], 'tone_violations': len(tone_violations), 'fact_violations': len(fact_violations), 'cached': use_cache}, 'violations': {'tone': [v.dict() for v in tone_violations], 'facts': [v.dict() for v in fact_violations]}}
        except Exception as e:
            raise
            await self.infrastructure.event_bus.publish('events.upgraded_generation_failed', SystemEvent(type=EventType.ERROR_OCCURRED, trace_id=trace_id, source_component='InfrastructureUpgradesOrchestrator', payload={'error': str(e)}, causation_id=trace_id))
            raise
    ...
    def get_upgrades_stats(self) -> dict[str, Any]:
        ...
        """
        return {'fact_ledger': self.fact_ledger.get_stats() if self.fact_ledger else {}, 'global_cache': self.global_cache.get_stats() if self.global_cache else {}, 'tone_enforcer': {'profiles_loaded': len(self.tone_enforcer.profiles) if self.tone_enforcer else 0}}
...
async def generate_with_consistency(task_type: TaskType, prompt: str, tone_voice: ToneVoice | None=None, verify_facts: bool=True, enforce_tone: bool=True, trace_id: str | None=None) -> dict[str, Any]:
    ...
    return await orchestrator.generate_with_upgrades(task_type, prompt, tone_voice, verify_facts, enforce_tone, True, trace_id)
