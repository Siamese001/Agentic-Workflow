...
class InfrastructureOrchestrator:
    ...
    async def initialize(self) -> None:
        ...
        await initialize_system_health_checks(bulkhead_manager=self.bulkhead_manager, circuit_breaker_registry=await get_circuit_breaker_registry(), dead_letter_queue=await get_dead_letter_queue(), checkpoint_manager=None)
    ...
    async def _register_component_health_checks(self) -> None:
        ...
        class EventBusHealthChecker(HealthChecker):
            ...
            async def check_health(self) -> HealthCheckResult:
                ...
                return HealthCheckResult(component_name='event_bus', component_type=ComponentType.CUSTOM, status=status, message=f"Event bus is {health['event_bus']['status']}", timestamp=None, metrics=health)
        ...
        class ProvenanceHealthChecker(HealthChecker):
            ...
            async def check_health(self) -> HealthCheckResult:
                ...
                return HealthCheckResult(component_name='provenance_tracker', component_type=ComponentType.CUSTOM, status=status, message=f"Provenance tracker is {health['status']}", timestamp=None, metrics=health)
        ...
        class ModelRouterHealthChecker(HealthChecker):
            ...
            async def check_health(self) -> HealthCheckResult:
                ...
                return HealthCheckResult(component_name='model_router', component_type=ComponentType.CUSTOM, status=status, message=message, timestamp=None, metrics=stats)
    ...
    async def _handle_artifact_generated(self, event: SystemEvent) -> None:
        ...
        try:
            ...
            if artifact_id:
                ...
                if sources:
                    await self.provenance_tracker.record_generation(event.trace_id, artifact_id, output, model_version, payload.get('prompt'))
    ...
    async def _handle_error_occurred(self, event: SystemEvent) -> None:
        ...
        try:
            ...
            await dlq.add_failed_envelope(event, FailureReason.PROCESSING_ERROR, event.source_component, event.payload.get('error', 'Unknown error'))
        # guardian: allow-silent-swallow
        except Exception as e:
    ...
    async def _handle_agent_completed(self, event: SystemEvent) -> None:
        ...
        try:
            ...
            if model_name and usage:
                self.model_router.record_usage(model_name, usage.get('input_tokens', 0), usage.get('output_tokens', 0), usage.get('cost', 0.0))
        # guardian: allow-silent-swallow
        except Exception as e:
            logger.error(f'Failed to handle agent completed event: {e}')

    async def execute_with_infrastructure(self, task_type: TaskType, prompt: str, sources: list[tuple] | None=None, complexity_score: int=1, trace_id: str | None=None, priority: TaskPriority=TaskPriority.MEDIUM) -> dict[str, Any]:
        ...
        await self.event_bus.publish('events.workflow_started', SystemEvent(type=EventType.WORKFLOW_STARTED, trace_id=trace_id, source_component='InfrastructureOrchestrator', payload={'task_type': task_type.value, 'complexity_score': complexity_score}))
        try:
            if sources:
            ...
            tier = self.model_router._select_model_for_tier(self.model_router._determine_tier(self.model_router._task_profiles[task_type], complexity_score))
            client = await self.model_router.get_client(tier)
            result = await self.bulkhead_manager.execute(client.generate, prompt, bulkhead_name='model_generation', priority=priority)
            if sources:
                artifact_id = f'artifact_{int(time.time())}'
                lineage = await self.provenance_tracker.record_generation(trace_id, artifact_id, result, model_config['model'], prompt)
            await self.event_bus.publish('events.artifact_generated', SystemEvent(type=EventType.ARTIFACT_GENERATED, trace_id=trace_id, source_component='InfrastructureOrchestrator', payload={'artifact_id': artifact_id if sources else None, 'output': result, 'model_version': model_config['model'], 'prompt': prompt, 'sources': sources or []}, causation_id=trace_id))
            execution_time = time.time() - start_time
            return {'result': result, 'trace_id': trace_id, 'model_used': model_config['model'], 'tier': model_config['tier'], 'execution_time': execution_time, 'lineage': lineage.to_dict() if sources else None}
        except Exception as e:
            raise
            await self.event_bus.publish('events.error_occurred', SystemEvent(type=EventType.ERROR_OCCURRED, trace_id=trace_id, source_component='InfrastructureOrchestrator', payload={'error': str(e)}, causation_id=trace_id))
            dlq = await get_dead_letter_queue()
            await dlq.add_failed_envelope(SystemEvent(type=EventType.WORKFLOW_FAILED, trace_id=trace_id, source_component='InfrastructureOrchestrator', payload={'error': str(e)}), FailureReason.PROCESSING_ERROR, 'InfrastructureOrchestrator.execute_with_infrastructure', str(e))
            raise
    ...
    async def get_system_health(self) -> dict[str, Any]:
        ...
        health['infrastructure'] = {'event_bus': await self.event_bus.health_check(), 'provenance_tracker': self.provenance_tracker.get_stats(), 'model_router': self.model_router.get_stats(), 'bulkheads': self.bulkhead_manager.get_all_metrics()}
        return health
...
async def execute_task(task_type: TaskType, prompt: str, sources: list[tuple] | None=None, complexity_score: int=1, trace_id: str | None=None, priority: TaskPriority=TaskPriority.MEDIUM) -> dict[str, Any]:
    """Execute a task with full infrastructure support.
    ...
    return await orchestrator.execute_with_infrastructure(task_type, prompt, sources, complexity_score, trace_id, priority)
...
def with_infrastructure(task_type: TaskType, complexity_score: int=1, priority: TaskPriority=TaskPriority.MEDIUM):
