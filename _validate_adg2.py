...
print("\nSample anomalies:")
cur.execute(
    "SELECT DISTINCT anomaly_type, COUNT(*) FROM mv_snapshot_integrity_anomalies GROUP BY anomaly_type LIMIT 10"
)
...
cur.execute(
    "SELECT COUNT(DISTINCT exempt_node_id), COUNT(DISTINCT critical_node_id) FROM mv_exemptions_near_critical_paths"
)
...
cur.execute(
    "SELECT COUNT(*), gap_type FROM mv_trace_replay_eval_gaps GROUP BY gap_type ORDER BY COUNT(*) DESC"
)
for row in cur.fetchall():
    print(f"  {row[1]}: {row[0]}")
