...
def extract_tree_lines(md_text):
    ...
    return lines
# -------------------------------------------------------------
# Convert ASCII tree → nested dict structure
# -------------------------------------------------------------
def tree_to_dict(tree_lines):
    ...
    for i, raw_line in enumerate(tree_lines):
        ...
        # Remove tree characters (├──, └──, │) and get clean name
        ...
        # Count tree character groups (│   ├── = level 1, │   │   ├── = level 2, etc.)
        indent_level = 0
        if "│" in raw_line or "├" in raw_line or "└" in raw_line:
            # Each "│   " or "    " group represents one level
...
# -------------------------------------------------------------
# MAIN LOOP — process all 9 markdown files
# -------------------------------------------------------------
