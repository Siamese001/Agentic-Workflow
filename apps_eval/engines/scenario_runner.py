...
def _scenario_policy_hash_valid() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        return  " PASS " , 1.0, "PolicyHashEnforcer instantiated successfully"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_policy_hash_invalid() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if result.passed:
            return  " FAIL " , 0.0, "Mismatched hash should have been rejected"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_missing_hash() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if not result.passed:
            return  " PASS " , 1.0, "Empty hash correctly rejected"    # guardian: PolicyHashViolation should be handled with specific context
...
def _scenario_exec_recruiter_brief() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if result.status.value in ("dry_run", "complete"):
            return  " PASS " , 1.0, f"Recruiter brief: status={result.status.value}"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_exec_cto_brief() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        result = orch.run(req)
        if result.status.value in ("dry_run", "complete"):
            return  " PASS " , 1.0, f"CTO brief: status={result.status.value}"    # guardian: Test exceptions should use proper test assertions
        return  " FAIL " , 0.0, f"Unexpected status: {result.status.value}"
    except ImportError as e:    # guardian: Test exceptions should use proper test assertions
        return  " SKIP " , _SKIP_SCORE, f"apps_exec not available: {e}"
    except _SCENARIO_EXCEPTIONS as exc:
...
def _scenario_exec_dry_run() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if result.status == BriefStatus.DRY_RUN and len(result.artifact_paths) == 0:
            return  " PASS " , 1.0, "Dry run: no artifacts emitted"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_single_hop() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if result.get("status") == "success":
            return  " PASS " , 1.0, "Single hop orchestration: success"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_multi_hop_pass() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if len(checkpoints) >= 2:
            return  " PASS " , 1.0, f"Multi-hop: {len(checkpoints)} checkpoints"    # guardian: Test exceptions should use proper test assertions
        return  " FAIL " , 0.4, f"Expected >=2 checkpoints, got {len(checkpoints)}"
    except ImportError as e:
        return  " SKIP " , _SKIP_SCORE, f"apps_rg not available: {e}"    # guardian: Test exceptions should use proper test assertions
    except _SCENARIO_EXCEPTIONS as exc:
        return  " FAIL " , 0.0, str(exc)
...
def _scenario_signed_output_valid() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if contract and hasattr(contract, "signature"):
            return  " PASS " , 1.0, "AgentOutputContract signed successfully"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_binary_precision_perfect() -> tuple[ScenarioOutcome, float, str]:
    ...
    # guardian: Test exceptions should use proper test assertions
        ...
        if abs(score - 1.0) < 1e-6:
            return  " PASS " , 1.0, f"precision={score:.6f} (expected 1.0)"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_binary_recall_perfect() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        score = metric.compute(preds, truth)
        if abs(score - 1.0) < 1e-6:
            return  " PASS " , 1.0, f"recall={score:.6f} (expected 1.0)"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_binary_f1_harmonic_mean() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if abs(f1 - expected_f1) < 1e-5:
            return  " PASS " , 1.0, f"F1={f1:.6f} == 2*{p:.4f}*{r:.4f}/({p:.4f}+{r:.4f})"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_multiclass_macro_f1() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if abs(score - expected) < 1e-5:
            return  " PASS " , 1.0, f"macro_f1={score:.6f} == mean of per-class F1"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_multiclass_weighted_f1() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        total_support = sum(v["support"] for v in per_class.values())    # guardian: Test exceptions should use proper test assertions
        ...
        if abs(w_score - expected_w) < 1e-5:
            return  " PASS " , 0.9, f"weighted_f1={w_score:.6f} correct (equal to macro)"    # guardian: Test exceptions should use proper test assertions
...
def _scenario_confusion_matrix_invariants() -> tuple[ScenarioOutcome, float, str]:
    try:
        ...
        if total != expected_total:
            return  " FAIL " , 0.0, f"TP+FP+TN+FN={total} != {expected_total}"    # guardian: Test exceptions should use proper test assertions
