...
class SOTALayerTestSuite:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            if len(reranked) > 0:
                ...
                if top_doc == expected_doc:
                    logger.info(f"   ✅ Correct document ranked #1")
                else:
                    logger.info(f"   ⚠️  Expected different document at #1")
                    logger.info(f"      Expected: {expected_doc[:50]}...")
                    logger.info(f"      Got: {top_doc[:50]}...")

                # Show top 3 results
                logger.info(f"   Top 3 results:")
                for j, doc in enumerate(reranked[:3], 1):
                    logger.info(f"     {j}. {doc[:60]}...")
    ...
    def test_contrastive_semantic_cache(self):
        ...
        for i, test_case in enumerate(self.cache_test_queries, 1):
            ...
            if success:
                ...
                logger.info(f"⚠️  Failed to cache original query")
                continue
            ...
            for j, similar_query in enumerate(test_case['similar_queries'], 1):
                ...
                if cached_response:
                    ...
                    if cached_response == original_response:
                        logger.info(f"   ✅ Correct response retrieved")
                    else:
                        logger.info(f"   ⚠️  Different response retrieved")
                else:
                    logger.info(f"   ❌ Cache miss (threshold not met)")
    ...
    def test_cache_features(self):
        ...
        try:
            ...
            logger.info(f"   ✅ Exported cache to {temp_file}")
    ...
    def test_integration_scenario(self):
        ...
        if cached_result:
            logger.info(f"✅ Cache hit! Response served in {cache_time*1000:.2f}ms")
            logger.info(f"   Response: {cached_result[:100]}...")
            return
        else:
            logger.info(f"   Cache miss in {cache_time*1000:.2f}ms")
        ...
        if self.reranker.is_available:
            ...
            logger.info(f"   ✅ Reranked in {rerank_time:.3f}s")

            logger.info(f"   Top 3 reranked documents:")
            for i, doc in enumerate(reranked_docs, 1):
                logger.info(f"     {i}. {doc[:70]}...")
        else:
            logger.info(f"   ⚠️  Reranker not available, using original order")
        ...
        if cached_result:
            logger.info(f"   ✅ Similar query hit cache in {cache_time*1000:.2f}ms")
            logger.info(f"   Response: {cached_result[:100]}...")
        else:
    ...
    def test_fallback_modes(self):
        ...
        logger.info(f"   Put operation: {'Failed' if not put_result else 'Unexpectedly succeeded'}")
    ...
    def test_convenience_functions(self):
        ...
        if result:
            logger.info(f"   Top result: {result[0][:50]}...")
    ...
    def run_all_tests(self):
        ...
        logger.info("  • Redis-speed responses with semantic caching")
        logger.info("  • Lazy loading for fast startup")
        logger.info("  • Graceful fallbacks for missing dependencies")
        logger.info("  • Complete titanium RAG pipeline ready!")
