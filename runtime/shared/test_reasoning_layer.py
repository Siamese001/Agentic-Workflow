...
class ReasoningLayerTestSuite:
    ...
    def test_dynamic_hybrid_scorer(self):
        ...
        for test_case in self.alpha_test_queries:
            ...
            logger.info(f"Expected Alpha: {test_case['expected_alpha']} ({test_case['reason']})")
            logger.info("-" * 40)
    ...
    async def test_async_execution(self):
        ...
        logger.info(f"   Actual time (parallel): {elapsed:.3f}s")
        logger.info(f"   Speedup: {(len(decomposed.sub_queries) * 0.05) / elapsed:.1f}x")
    ...
    def run_all_tests(self):
        ...
        logger.info("  • Dynamic alpha adjustment based on query characteristics")
        logger.info("  • Parallel execution of sub-queries for efficiency")
        logger.info("  • Entity-aware scoring (IDs, technical terms, concepts)")
        logger.info("  • Seamless integration with Phase 1 Precision Layer")
