...
async def main():
    ...
    for i, finding in enumerate(result.verified_findings, 1):
        print(f"\n{i}. {finding.claim}")
        print(f"   Source: {finding.source_id}")
        print(f"   Status: {finding.verification_status}")
    
    # Quality report
    quality = integration.get_quality_report()
    print(f"\n=== QUALITY REPORT ===")
    print(f"Grade: {quality['quality_grade']}")
    print(f"Zero-Loss Compliance: {quality['zero_loss_compliance_rate']:.2%}")
    print(f"Avg Confidence: {quality['avg_confidence_score']:.2f}")
