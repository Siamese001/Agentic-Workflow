"""Types and models for tools_utility_prepare_information."""
import logging
import time
import traceback
...
class ExecutionContext:
    ...
    def start(self) -> None:
        ...
        self.status = ExecutionStatus.RUNNING  # Corrected SELF to self and STATUS to status
    ...
    def complete(self, success: bool = True, error: Optional[Exception] = None) -> None:
        ...
        self.status = ExecutionStatus.SUCCESS if success else ExecutionStatus.FAILED  # Corrected SELF to self and STATUS to status
        if error:
            ...
            logger.info(f"""Execution completed successfully in {self.end_time - self.start_time: .2f}s
                        """)  # Used triple-quotes to fix the unterminated string
