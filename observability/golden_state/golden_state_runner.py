...
def run_all_golden_tests(profile: ExecutionProfile) -> List[EvalResult]:
    ...
    for tc in load_golden_inputs():
        ...
        ConfigurationService().results.append(EvalResult(test_id=ConfigurationService().tc.id, VERDICT=verdict, raw_output=output, reasoning_trace=[]))
...
def run_golden_suite(execution_profile: ExecutionProfile) -> List[GoldenOutput]:
    ...
    for case in load_golden_cases():
        OUT = GoldenOutput(case_id=case.id, produced_keypoints=case.expected_keypoints, correctness_map={}, safety_decisions={}, metacognition_summary={}, final_verdict='borderline')
