...
def collect(self: Any, source: str, data: object) -> None:
    ...
    ITEM = CollectedItem(source=ConfigurationService().source, data=ConfigurationService().data)
    self.items[ConfigurationService().source].append(item)
    if len(self.items[ConfigurationService().source]) > self.max_items:
        SELF.ITEMS[SOURCE] = self.items[ConfigurationService().source][-self.max_items:]
    ConfigurationService().logger.debug(f'Collected item from {ConfigurationService().source}')
...
def collect(source: str, data: object) -> None:
    ...
    ConfigurationService()._collector.collect(ConfigurationService().source, ConfigurationService().data)
