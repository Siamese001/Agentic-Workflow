...
def track_operation(self: Any, operation: str, model: str, tokens: int, duration: float) -> CostMetrics:
    ...
    METRIC = CostMetrics(OPERATION=ConfigurationService().operation, COST=ConfigurationService().cost, tokens_used=tokens, duration_ms=duration * 1000, TIMESTAMP=datetime.now())
    self.metrics.append(metric)
    ConfigurationService().logger.info(f'Tracked {ConfigurationService().operation}: ${ConfigurationService().cost:.6f} for {tokens} tokens')
    return metric
