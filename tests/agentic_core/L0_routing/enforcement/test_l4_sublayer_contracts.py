...
class TestL4CLedgerEntry:
    def test_valid_write_entry(self):
        e = L4CLedgerEntry(
            correlation_id="corr-001", key="config.x", value=42, operation="write"
        )
        assert e.operation == "write"

    def test_valid_delete_entry(self):
        e = L4CLedgerEntry(
            correlation_id="corr-002", key="config.x", value=None, operation="delete"
        )
    ...
    def test_invalid_operation_raises(self):
        with pytest.raises(SubLayerViolation, match="operation must be"):
            L4CLedgerEntry(
                correlation_id="c1", key="k", value=None, operation="mutate"
            )
...
class TestL4CPersistentLedger:
    ...
    def test_commit_and_read(self):
        ledger = _FakeL4CLedger()
        ledger.commit(L4CLedgerEntry(
            correlation_id="c1", key="settings.x", value=99, operation="write"
        ))
