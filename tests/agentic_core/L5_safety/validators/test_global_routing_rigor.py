...
class TestGlobalRoutingRigor:
    ...
    def test_version_registry_schema_priority(self):
        ...
        vr_w = SOVEREIGN_TERRITORIES["agentic_core"]["ast_signals"][
    ...
    def test_domain_logic_weight_range(self):
        ...
        assert 15 <= cognition_w <= 18, (
            f"FAIL: Cognition weight {cognition_w} not in domain logic range 15-18"
        )
    ...
    def test_generic_utilities_weight_range(self):
        ...
        assert 5 <= execution_w <= 9, (
            f"FAIL: Execution weight {execution_w} not in generic utilities range 5-9"
        )
