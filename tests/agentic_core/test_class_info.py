...
def test_test_class_info_can_import():
    """Test that the module can be imported successfully."""
    try:
        mod = importlib.import_module("agentic_core.L0_routing.scripts.class_info")
        assert mod is not None
    except ImportError as e:
        pytest.skip(f"Cannot import module agentic_core.L0_routing.scripts.class_info: {e}")


def test_test_class_info_has_file_attribute():
    """Test that module has __file__ attribute."""
    try:
        mod = importlib.import_module("agentic_core.L0_routing.scripts.class_info")
        assert hasattr(mod, "__file__")
    except ImportError:
        pytest.skip("Cannot import module agentic_core.L0_routing.scripts.class_info")


def test_test_class_info_has_public_attributes():
