...
def test_test_fix_missing_invocation_util_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "fix_missing_invocation_util",
            str(
                Path(__file__).resolve().parents[4]
                / "dev_tools"
                / "l0_scripts"
                / "fix_missing_invocation_util.py",
            ),
        )
        ...
        pytest.skip(
            f"Cannot load dev_tools/l0_scripts/fix_missing_invocation_util.py: {e}",
        )
...
def test_test_fix_missing_invocation_util_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/fix_missing_invocation_util.py")
