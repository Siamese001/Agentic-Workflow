...
def test_manifest_integrity_lock(clean_manifest_environment):
    ...
    sys.path.insert(
        0,
        str(Path(__file__).parent.parent.parent / "agentic_core" / "L0_routing" / "security"),
    )
...
def test_sovereign_root_whitelist():
    ...
    with patch("agentic_core.L0_routing.scripts.compliance_gate.check_compliance") as mock_check:
...
def test_boot_sequence_integrity_failure():
    ...
    with patch("agentic_core.L0_routing.security.ManifestGuardian.verify_integrity", return_value=False):
        with patch(
            "agentic_core.L0_routing.boot.boot_sequence.ManifestGuardian.verify_integrity",
            return_value=False,
        ):
            # Mock the boot sequence to avoid import issues
            with patch("agentic_core.L0_routing.boot.boot_sequence.check_compliance", return_value=[]):
                with patch("agentic_core.L0_routing.boot.boot_sequence.AgentRegistry") as mock_registry:
                    mock_instance = Mock()
...
def test_boot_sequence_successful_boot():
    ...
    with patch("agentic_core.L0_routing.security.ManifestGuardian.verify_integrity", return_value=True):
        with patch(
            "agentic_core.L0_routing.boot.boot_sequence.ManifestGuardian.verify_integrity",
            return_value=True,
        ):
