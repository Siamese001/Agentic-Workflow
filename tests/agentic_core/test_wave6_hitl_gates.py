...
def test_hitl_decision_logger_exists():
    ...
    assert LOGGER_PATH.exists(), (
        f"hitl_decision_logger.py not found at {LOGGER_PATH} — "
        "HITL decisions cannot be recorded"
    )
...
def test_hitl_decision_logger_exports_log_fn():
    ...
    assert "log_hitl_decision" in fn_names, (
        "log_hitl_decision not defined in hitl_decision_logger.py"
    )
...
def test_location_healer_hitl_approval_fn_param():
    ...
    assert "hitl_approval_fn" in args, (
        "_heal_via_archiving() missing hitl_approval_fn parameter — "
        "archive deletions will bypass HITL gate"
    )
