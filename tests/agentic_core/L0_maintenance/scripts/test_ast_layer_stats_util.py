...
def test_test_ast_layer_stats_util_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "ast_layer_stats_util",
            str(Path(__file__).resolve().parents[4] / "dev_tools" / "l0_scripts" / "ast_layer_stats_util.py"),
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert mod is not None
    except ImportError as e:
        pytest.skip(f"Cannot load dev_tools/l0_scripts/ast_layer_stats_util.py: {e}")
...
def test_test_ast_layer_stats_util_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/ast_layer_stats_util.py")
