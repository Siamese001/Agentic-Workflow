...
def test_test_fix_remaining_imports_util_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "fix_remaining_imports_util",
            str(
                Path(__file__).resolve().parents[4]
                / "dev_tools"
                / "l0_scripts"
                / "fix_remaining_imports_util.py",
            ),
        )
        ...
        pytest.skip(
            f"Cannot load dev_tools/l0_scripts/fix_remaining_imports_util.py: {e}",
        )
...
def test_test_fix_remaining_imports_util_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/fix_remaining_imports_util.py")
