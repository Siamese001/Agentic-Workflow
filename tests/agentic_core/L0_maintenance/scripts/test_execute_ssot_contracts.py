...
EXECUTE_SSOT_PATH = REPO_ROOT / "agentic_core" / "L0_maintenance" / "scripts" / "execute_ssot.py"
ENTRYPOINT_PATH = REPO_ROOT / "agentic_core" / "L0_maintenance" / "scripts" / "execute_ssot_entrypoint.py"
...
def _load_module():
    ...
    try:
        return importlib.import_module(
            "agentic_core.L0_maintenance.scripts.execute_ssot",
        )
...
class TestEntrypointBoundaryLock:
    ...
    def test_entrypoint_without_legacy_returns_1(self):
        ...
        env = {**__import__("os").environ, "PYTHONPATH": str(REPO_ROOT)}
        result = subprocess.run(
            [sys.executable, "-m", "agentic_core.L0_maintenance.scripts.execute_ssot_entrypoint"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=str(REPO_ROOT),
    ...
    def test_entrypoint_has_plan_flag_in_help(self):
        ...
        env = {**__import__("os").environ, "PYTHONPATH": str(REPO_ROOT)}
        result = subprocess.run(
            [sys.executable, "-m", "agentic_core.L0_maintenance.scripts.execute_ssot_entrypoint", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=str(REPO_ROOT),
