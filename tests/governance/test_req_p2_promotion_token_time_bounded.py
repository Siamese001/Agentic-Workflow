...
class TestPromotionTokenTimeBounded:
    ...
    def test_token_time_window_boundaries(self):
        ...
        token = issue_promotion_token(
            target_namespace="time_boundaries",
            semantic_clock_tick=100,
            window_size=10
        )
    ...
    def test_token_time_window_with_validation(self):
        ...
        token = issue_promotion_token(
            target_namespace="time_validation",
            semantic_clock_tick=100,
            window_size=10
        )
    ...
    def test_token_time_window_persistence(self):
        ...
        # Given - Token with time window
        token = issue_promotion_token(
            target_namespace="time_persistence",
        ...
        assert retrieved.semantic_clock_window == (200, 250), \
            "Time window should be preserved"
    ...
    def test_token_time_window_different_start_times(self):
        ...
        early_token = issue_promotion_token(
            target_namespace="time_early",
            semantic_clock_tick=50,
            window_size=10
        )
        
        middle_token = issue_promotion_token(
            target_namespace="time_middle",
            semantic_clock_tick=100,
            window_size=10
        )
        
        late_token = issue_promotion_token(
            target_namespace="time_late",
            semantic_clock_tick=150,
            window_size=10
        )
        
        # When/Then - Each should have different windows
        assert early_token.semantic_clock_window == (50, 60), \
            "Early token window"
        assert middle_token.semantic_clock_window == (100, 110), \
            "Middle token window"
        assert late_token.semantic_clock_window == (150, 160), \
            "Late token window"
    ...
    def test_token_time_window_edge_cases(self):
        ...
        try:
            token = issue_promotion_token(
                target_namespace="time_negative",
                semantic_clock_tick=100,
                window_size=-5
            )
    ...
    def test_token_time_window_with_single_use(self):
        ...
        assert not token.validate_scope_and_use(), \
            "Second use should fail due to single-use"
    ...
    def test_token_time_window_monitoring(self):
        ...
        for i in range(5):
            token = issue_promotion_token(
                target_namespace=f"monitor_{i}",
                semantic_clock_tick=current_tick + i * 10,
                window_size=15
            )
            tokens.append(token)
