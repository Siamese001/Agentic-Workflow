...
def test_agent_ids_unique_and_stable():
    ...
    from agentic_core.agents.agent_registry import get_all_agent_ids, AGENT_REGISTRY
...
def test_deterministic_agents_have_no_allowed_models():
    ...
    for agent_id in deterministic_agents:
        ...
        assert profile.allowed_models == (), f"Deterministic agent '{agent_id}' must have empty allowed_models"
...
def test_scanner_finds_violations():
    ...
    result = subprocess.run(
        [sys.executable, str(scanner_path), "tests/governance"],
        capture_output=True,
        text=True,
        timeout=30
    )
...
def test_full_agent_registry_system_scan():
    ...
    assert "agent registry violations" in result.stdout or "No agent registry violations found" in result.stdout, "Scanner should report results"
