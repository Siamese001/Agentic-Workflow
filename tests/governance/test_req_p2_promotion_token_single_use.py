...
class TestPromotionTokenSingleUse:
    ...
    def test_token_single_use_basic(self):
        ...
        token = issue_promotion_token(
            target_namespace="single_use_basic",
    ...
    def test_nonce_tracking_across_tokens(self):
        ...
        token1 = issue_promotion_token(
            target_namespace="nonce_test1",
            semantic_clock_tick=100
        )
        
        token2 = issue_promotion_token(
            target_namespace="nonce_test2",
            semantic_clock_tick=100
        )
    ...
    def test_nonce_uniqueness(self):
        ...
        for i in range(10):
            token = issue_promotion_token(
                target_namespace=f"unique_test_{i}",
                semantic_clock_tick=100
            )
            tokens.append(token)
    ...
    def test_nonce_persistence_across_validations(self):
        ...
        # Given - Issue token
        token = issue_promotion_token(
            target_namespace="persistence_test",
            semantic_clock_tick=100
        )
    ...
    def test_single_use_with_issuer_validation(self):
        ...
        token = self.issuer.issue_promotion_token(
            target_namespace="issuer_single_use",
            semantic_clock_tick=100
        )
        
        # When - Validate through issuer first time
        first_validation = self.issuer.validate_token(
            token, "issuer_single_use", 100
        )
        ...
        second_validation = self.issuer.validate_token(
            token, "issuer_single_use", 100
        )
    ...
    def test_single_use_with_different_validation_contexts(self):
        ...
        issuer_result = self.issuer.validate_token(
            token, "context_test", 100
        )
    ...
    def test_single_use_prevents_replay_attack(self):
        ...
        token = issue_promotion_token(
            target_namespace="replay_attack_test",
            semantic_clock_tick=100
        )
    ...
    def test_single_use_statistics_and_monitoring(self):
        ...
        for i in range(5):
            token = issue_promotion_token(
                target_namespace=f"stats_test_{i}",
                semantic_clock_tick=100
            )
            tokens.append(token)
    ...
    def test_single_use_error_handling(self):
        ...
        assert not token.validate_scope_and_use(), \
            "Should handle already used nonce gracefully"
        
        # Clear and try again
        PromotionTokenStore.clear_all()
        new_token = issue_promotion_token(
            target_namespace="error_test2",
            semantic_clock_tick=100
        )
        
        assert new_token.validate_scope_and_use(), \
