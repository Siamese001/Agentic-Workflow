...
class CrossLayerCallValidator:
    ...
    def validate_cross_layer_call(
        ...
        if source_schema.version != source_version:
            raise ValueError(f"Source layer version mismatch: expected {source_schema.version}, got {source_version}")
        
        if target_schema.version != target_version:
            raise ValueError(f"Target layer version mismatch: expected {target_schema.version}, got {target_version}")
        
        # Check compatibility
        if not self._is_compatible(target_layer, target_version, target_schema.version):
            raise ValueError(f"Target layer version {target_version} not compatible with {target_schema.version}")
