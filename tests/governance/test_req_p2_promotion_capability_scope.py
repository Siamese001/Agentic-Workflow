...
from promotion_token import (
    PromotionToken, PromotionTokenStore, PromotionTokenIssuer,
    issue_promotion_token, get_token_issuer
)
...
class TestPromotionCapabilityScope:
    ...
    def test_token_limited_to_pointer_updates(self):
        ...
        assert token.allowed_action == "pointer_update", \
            "Token should only allow pointer_update action"
        
        # Should not allow other actions
        assert token.allowed_action != "file_write", \
            "Token should not allow file_write"
        assert token.allowed_action != "config_change", \
            "Token should not allow config_change"
    
    def test_token_namespace_scope_validation(self):
        """Test token namespace scope validation."""
        # Given - Token for specific namespace
        token = issue_promotion_token(
            target_namespace="specific_namespace",
            semantic_clock_tick=100
        )
        
        # When/Then - Should validate for correct namespace
        assert token.is_valid_for_namespace("specific_namespace"), \
            "Should be valid for correct namespace"
        
        # Should not validate for wrong namespace
        assert not token.is_valid_for_namespace("wrong_namespace"), \
            "Should not be valid for wrong namespace"
        
        assert not token.is_valid_for_namespace(""), \
            "Should not be valid for empty namespace"
    ...
    def test_token_single_use_enforcement(self):
        ...
        # Given - Issue token
        token = issue_promotion_token(
            target_namespace="single_use_test",
            semantic_clock_tick=100
        )
        ...
        assert PromotionTokenStore.is_nonce_used(token.single_use_nonce), \
    ...
    def test_token_replay_digest_binding(self):
        ...
        assert token.replay_digest_binding == replay_digest, \
            "Token should be bound to replay digest"
        
        # Different replay digest should create different token
        token2 = issue_promotion_token(
            target_namespace="replay_test",
            semantic_clock_tick=100,
            replay_digest="different_digest"
        )
    ...
    def test_token_issuer_validation(self):
        ...
        token = self.issuer.issue_promotion_token(
            target_namespace="issuer_test",
            semantic_clock_tick=current_tick
        )
        
        # Then - Should validate through issuer
        assert self.issuer.validate_token(token, "issuer_test", current_tick), \
            "Issuer should validate token"
    ...
    def test_token_revocation(self):
        ...
        assert not PromotionTokenStore.revoke_token("non_existent"), \
            "Should not revoke non-existent token"
    ...
    def test_multiple_tokens_different_scopes(self):
        ...
        token1 = issue_promotion_token(
            target_namespace="namespace1",
            semantic_clock_tick=100
        )
        
        token2 = issue_promotion_token(
            target_namespace="namespace2",
            semantic_clock_tick=150
        )
        
        # When/Then - Tokens should have different scopes
        assert token1.target_namespace != token2.target_namespace, \
            "Tokens should have different namespaces"
        ...
        token3 = issue_promotion_token(
            target_namespace="namespace3",
            semantic_clock_tick=200
        )
        
        assert not token3.is_valid_for_namespace("namespace1"), \
            "Token3 should not validate for namespace1"
        assert not token3.is_valid_for_namespace("namespace2"), \
            "Token3 should not validate for namespace2"
