...
class TestCrossWaveLinkage:
    ...
    def test_consecutive_wave_linkage(self):
        ...
        wave15_hash = hashlib.sha256("wave15_data".encode()).hexdigest()
        ...
        wave15_summary = WaveAuditSummary(
            ...
            artifacts_hash=hashlib.sha256("wave15_artifacts".encode()).hexdigest(),
        ...
        wave16_summary = WaveAuditSummary(
            wave_id=16,
            wave_hash=hashlib.sha256("wave16_data".encode()).hexdigest(),
            prev_wave_hash=wave15_hash,  # Should match Wave 15's hash
            timestamp=1234567891.0,
            artifacts_hash=hashlib.sha256("wave16_artifacts".encode()).hexdigest(),
        ...
        assert wave16_summary.prev_wave_hash == wave15_summary.wave_hash, \
            "Wave 16 should reference Wave 15's hash"
        assert wave16_summary.wave_id == wave15_summary.wave_id + 1, \
            "Waves should be consecutive"
    ...
    def test_tampered_linkage_detection(self):
        ...
        correct_hash = hashlib.sha256("correct_data".encode()).hexdigest()
    ...
    def test_wave_hash_consistency(self):
        ...
        wave_summary = WaveAuditSummary(
            ...
            prev_wave_hash="prev_hash",
            timestamp=1234567890.0,
            artifacts_hash=hashlib.sha256("artifacts".encode()).hexdigest(),
            guardian_signature="guardian_sig"
        )
        
        # When/Then - Hash should match content
        assert wave_summary.wave_hash == expected_hash, \
            "Wave hash should match content hash"
...
def test_req254_wave_hash_tamper_detection():
    ...
    legitimate_hash = hashlib.sha256("legitimate_data".encode()).hexdigest()
    
    # Create tampered wave
    tampered_summary = WaveAuditSummary(
        wave_id=16,
        wave_hash=hashlib.sha256("tampered_data".encode()).hexdigest(),
        prev_wave_hash=legitimate_hash,
        timestamp=1234567890.0,
        artifacts_hash=hashlib.sha256("tampered_artifacts".encode()).hexdigest(),
        guardian_signature="forged_signature"  # Tampered signature
    )
    
    # Should detect tampering
    assert not test._verify_wave_linkage(tampered_summary, legitimate_hash), \
        "Should detect tampered wave"
