...
class test_terminal_sweep_compliance:
    ...
    def test_legacy_archival_verification(self, disable_path_shield):
        ...
        assert not (engines_path / "OutreachEngineRefactored.py").exists(), (
            "OutreachEngineRefactored.py should be in legacy/"
        )
    ...
    def test_specialist_node_upgrade_lead_quality(self):
        ...
        assert hasattr(LeadQualitySpecialist, "heal_repository"), (
            "LeadQualitySpecialist must have heal_repository"
        )
        assert hasattr(LeadQualitySpecialist, "_run_self_tests"), (
            "LeadQualitySpecialist must have _run_self_tests"
        )
    ...
    def test_test_script_quarantine(self, disable_path_shield):
        ...
        assert len(engine_tests) == 0, (
            f"Found {len(engine_tests)} test files still in engines/: {[f.name for f in engine_tests]}"
        )
