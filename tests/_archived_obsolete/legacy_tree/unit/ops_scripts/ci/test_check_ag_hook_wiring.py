...
def _make_hooks(
    ...
    return {
        "hooks": {
            "post_agent_response": post_cascade or [],
...
def _hook(script: str, show_output: bool = True) -> dict:
    return {
        "command": f"python .codex/governance/scripts/_legacy_windsurf/{script}",
...
def _full_valid_hooks() -> dict:
    ...
    return _make_hooks(
        post_cascade=[
            _hook("post_agent_heartbeat.py"),
            _hook("post_agent_author_gate_miss_detector.py", show_output=True),
            _hook("post_agent_author_gate_ui_audit.py", show_output=True),
            _hook("post_agent_ask_user_question_packet_audit.py", show_output=True),
        ],
        pre_prompt=[
...
class TestWire1ReminderHook:
    ...
    def test_fail_when_reminder_absent(self, mod):
        hooks = _make_hooks(
            post_cascade=[_hook("post_agent_author_gate_miss_detector.py")],
...
class TestWire2MissDetector:
    ...
    def test_fail_when_absent(self, mod):
        hooks = _make_hooks(
            post_cascade=[
                _hook("post_agent_author_gate_ui_audit.py"),
                _hook("post_agent_ask_user_question_packet_audit.py"),
            ],
    ...
    def test_fail_when_show_output_false(self, mod):
        hooks = _make_hooks(
            post_cascade=[
                _hook("post_agent_author_gate_miss_detector.py", show_output=False),
                _hook("post_agent_author_gate_ui_audit.py"),
                _hook("post_agent_ask_user_question_packet_audit.py"),
            ],
            pre_prompt=[_hook("pre_user_prompt_author_gate_reminder.py")],
...
class TestWire3UiAudit:
    ...
    def test_fail_when_absent(self, mod):
        hooks = _make_hooks(
            post_cascade=[
                _hook("post_agent_author_gate_miss_detector.py"),
                _hook("post_agent_ask_user_question_packet_audit.py"),
            ],
            pre_prompt=[_hook("pre_user_prompt_author_gate_reminder.py")],
        )
    ...
    def test_fail_when_show_output_false(self, mod):
        hooks = _make_hooks(
            post_cascade=[
                _hook("post_agent_author_gate_miss_detector.py"),
                _hook("post_agent_author_gate_ui_audit.py", show_output=False),
                _hook("post_agent_ask_user_question_packet_audit.py"),
            ],
            pre_prompt=[_hook("pre_user_prompt_author_gate_reminder.py")],
        )
    ...
    def test_fail_when_show_output_false(self, mod):
        hooks = _make_hooks(
            post_cascade=[
                _hook("post_agent_author_gate_miss_detector.py"),
                _hook("post_agent_author_gate_ui_audit.py"),
                _hook("post_agent_ask_user_question_packet_audit.py", show_output=False),
            ],
            pre_prompt=[_hook("pre_user_prompt_author_gate_reminder.py")],
        )
        violations = mod.evaluate(hooks)
...
class TestNoAgHooks:
    def test_no_violations_when_no_ag_hooks_present(self, mod):
        hooks = _make_hooks(
            post_cascade=[_hook("post_agent_heartbeat.py")],
            pre_prompt=[_hook("pre_prompt_classifier.py")],
        )
        assert mod.evaluate(hooks) == []

    def test_no_violations_when_post_agent_empty(self, mod):
...
class TestMainExitCodes:
    ...
    def test_exits_0_advisory_on_violation(self, mod, tmp_path):
        bad_hooks = _make_hooks(
            post_cascade=[_hook("post_agent_author_gate_ui_audit.py")],
            pre_prompt=[],
        )
        hooks_path = tmp_path / "hooks.json"
    ...
    def test_report_written_on_violation(self, mod, tmp_path):
        bad_hooks = _make_hooks(
            post_cascade=[_hook("post_agent_author_gate_ui_audit.py", show_output=False)],
            pre_prompt=[],
        )
        hooks_path = tmp_path / "hooks.json"
        hooks_path.write_text(json.dumps(bad_hooks), encoding="utf-8")
