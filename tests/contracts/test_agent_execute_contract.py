"""Phase 3 — Execute Contract Gate.

Enforces:
1. Agent class defines execute(self, ...).
2. Signature includes **kwargs (or matches canonical signature).
3. Body is not a stub (pass, ..., raise NotImplementedError, docstring-only).
4. Body contains at least one executable statement.
...
# ── Gate logic ─────────────────────────────────────────────────────────────────
def _check_execute_contract(filepath: Path, tree: ast.Module) -> list[str]:
    ...
    if agent_cls is None:
        ...
        if len(agent_classes) == 1:
            ...
            issues.append("no_agent_class_for_execute_check")
            return issues

    # 1. Must define execute(self, ...)
    execute_method = find_method(agent_cls, "execute")
    if execute_method is None:
        issues.append("missing_execute_method")
        return issues

    # 2. Check for **kwargs in signature
    args = execute_method.args
    has_kwargs = args.kwarg is not None
    has_varargs = args.vararg is not None
    if not has_kwargs and not has_varargs:
        # Check if it has *args, **kwargs implicitly via parameters
        if not any(True for _ in args.kwonlyargs):
            issues.append("execute_missing_kwargs_or_varargs")
...
# ── Tests ──────────────────────────────────────────────────────────────────────
def test_execute_contract_no_new_violations():
    """No new execute contract violations beyond known debt."""
    ...
    for f in collect_reasoning_agent_files():
        ...
        if exempt:
            continue
        issues = _check_execute_contract(f, tree)
        if issues:
    ...
    if new_violations:
        ...
        pytest.fail(f"New execute contract violations:\n{details}")


def test_execute_contract_debt_ceiling():
    ...
    for f in collect_reasoning_agent_files():
        ...
        if exempt:
            continue
        if _check_execute_contract(f, tree):
            count += 1
    ceiling = len(KNOWN_DEBT)
    assert count <= ceiling, f"Execute contract debt grew: actual={count}, ceiling={ceiling}"
