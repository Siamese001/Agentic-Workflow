...
class TestRGProductionFlow:
    ...
    def test_end_to_end_resume_lifecycle(self):
        ...
        assert hasattr(next_hop, 'flow_result'), "Router should return RGFlowOutput with flow_result"
        assert next_hop.flow_result.flow_type in ["generate_base_node", "strategic_tailor_node", "generate_scratch"]
        ...
        experience_result = section_node.generate_experience_section(
            profile, 
            thematic_output
        )
        ...
        assert validation_signature is not None, "BROKEN LINK: Signature not propagated from Enforcer!"
