"""Enhanced behavioral tests for agentic_core.L2_execution.types.vllm_token_budget_types."""
...
def test_taskclass_is_instantiable():
    """TaskClass can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_localtier_is_callable():
    """LocalTier is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
