...
def run_parallel(runs: int = 3):
    ...
    for run_num in range(1, runs + 1):
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            print(f"{'Runtime (s)':<25} | {mod.get('time', 0):.2f}{'':<26} | {mono.get('time', 0):.2f}{'':<26}")
            print(f"{'Phases Executed':<25} | {mod.get('phase_count', 0)}/5{'':<25} | {mono.get('phase_count', 0)}/5{'':<25}")
            print(f"{'Phases List':<25} | {', '.join(mod.get('phases', []))[:28]:<30} | {', '.join(mono.get('phases', []))[:28]:<30}")
            ...
            print(f"{'Output Size (chars)':<25} | {mod.get('stdout_chars', 0):<30} | {mono.get('stdout_chars', 0):<30}")
            print(f"{'Result':<25} | {'✓ PASS' if mod.get('success') else '✗ FAIL':<30} | {'✓ PASS' if mono.get('success') else '✗ FAIL':<30}")
    ...
    mono_times = [r['monolithic']['time'] for r in all_results if 'monolithic' in r and 'time' in r['monolithic']]
    ...
    if mod_times and mono_times:
        ...
        print(f"{'Avg Runtime (s)':<25} | {sum(mod_times)/len(mod_times):.2f}{'':<26} | {sum(mono_times)/len(mono_times):.2f}{'':<26}")
