...
class TestAggressiveFaultToleranceE2E:
    ...
    def test_classification_permission_denied_handling(
        ...
        """AGGRESSIVE: Handle permission denied errors gracefully using mocking."""
        test_file = tmp_path / "no_read.py"
        test_file.write_text("class Agent: pass")

        # Mock open to simulate permission denied (cross-platform)
        original_open = open
        def mock_open_permission_error(filepath, *args, **kwargs):
            if str(filepath) == str(test_file):
        ...
        with mock.patch("builtins.open", mock_open_permission_error):
            ...
            result = classification_kernel_module.classify_file_standalone(test_file)
            # Should return a string classification (possibly an error indicator)
            assert isinstance(result, str)
