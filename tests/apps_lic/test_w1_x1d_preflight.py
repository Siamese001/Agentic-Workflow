...
def test_w1_config_freezes_live_claude_preflight_policy() -> None:
    ...
    assert preflight["required_transport_model_id"] == DEFAULT_GPT_TRANSPORT_MODEL_ID
    ...
    assert "OPENAI_API_KEY" in preflight["live_mode_requires"]
    assert "openai_sdk" in preflight["live_mode_requires"]
...
def test_fake_x1d_preflight_mode_is_deterministic_but_cannot_clear() -> None:
    ...
    receipt = run_gpt_x1d_preflight(
        mode=X1D_MODE_FAKE,
        env={},
        transport=transport,
        openai_sdk_available_override=False,
    )
...
def test_live_x1d_preflight_missing_key_or_sdk_fails_closed() -> None:
    missing_key = run_gpt_x1d_preflight(
        mode=X1D_MODE_LIVE,
        env={},
        openai_sdk_available_override=True,
    )
    ...
    missing_sdk = run_gpt_x1d_preflight(
        mode=X1D_MODE_LIVE,
        api_key="sk-ant-test",
        env={},
        openai_sdk_available_override=False,
...
def test_live_x1d_preflight_blocks_wrong_model_and_non_live_transport() -> None:
    wrong_model = run_gpt_x1d_preflight(
        mode=X1D_MODE_LIVE,
        api_key="sk-ant-test",
        env={},
    ...
    non_live = run_gpt_x1d_preflight(
        mode=X1D_MODE_LIVE,
        api_key="sk-ant-test",
        env={},
...
def test_live_x1d_preflight_blocks_injected_anthropic_client() -> None:
    receipt = run_gpt_x1d_preflight(
        ...
        api_key="sk-ant-test",
        env={},
        transport=OpenAIGPTX1DTransport(api_key="sk-ant-test", client=object()),
        openai_sdk_available_override=True,
    )
...
def test_unparseable_claude_response_parser_persists_digest() -> None:
    ...
    parsed = parse_gpt_x1d_response("not-json", profile=profile)
...
def test_manual_live_shaped_judge_receipts_cannot_clear_x1d() -> None:
    ...
    assert f"non_live_gpt_judge:{JUDGE_EVIDENCE_SUPPORT}" in blocked.reason_codes
...
def test_live_gpt_x1d_preflight_opt_in_environment_marker() -> None:
    if os.environ.get("APPS_LIC_RUN_LIVE_GPT_X1D") != "1":
        pytest.skip("Set APPS_LIC_RUN_LIVE_GPT_X1D=1 to call live GPT.")

    receipt = run_gpt_x1d_preflight(mode=X1D_MODE_LIVE)

    assert receipt.preflight_status == PREFLIGHT_READY
    assert receipt.clearance_allowed is True
    assert receipt.transport_provenance == LIVE_GPT_API_CALL
