"""
File: tests/integration/agentic_core/L0_maintenance/test_ssot_phases_final.py
Description: Final corrected aggressive integration testing for Phase 2 (Write) and Phase 3 (Validation) logic.
Updated to work with @standard_heal decorator schema and _raw_result access.
"""
...
class TestPhaseExecutionSafety:
    ...
    def mock_components(self, tmp_path):
        ...
        agents = {"FixerAgent": MagicMock(), "AnotherAgent": MagicMock()}
    ...
    def sample_violations(self):
        ...
        return [
            ...
            },
            {
                ...
                "type": "HIERARCHY",
                "suggested_agent": "AnotherAgent",  # Different agent to avoid cycle
                "severity": "high",
            },
        ]
    ...
    def test_phase2_hard_gate_budget_exhausted(self, mock_components, sample_violations):
        ...
        plan = {"violations_found": sample_violations}
    ...
    def test_phase2_dry_run_safety(self, mock_components, sample_violations):
        ...
        # Clear any existing call path to avoid cycle detection
        ...
        assert result["status"] == "FAIL"  # Dry run returns FAIL due to decorator logic
        assert result["skipped"] == 2

        # Verify dry run behavior
        raw = result["_raw_result"]
        assert len(raw["modifications"]) == 2
        for mod in raw["modifications"]:
            assert mod["action"] == "would_fix"
            # Dry run modifications don't have 'success' field
    ...
    def test_phase2_confidence_threshold_blocking(self, mock_components):
        ...
        assert raw["failures"][0]["confidence"] == 0.3
    ...
    def test_phase2_agent_not_found_handling(self, mock_components):
        ...
        raw = result["_raw_result"]
        assert len(raw["failures"]) == 1
        assert "Agent NonExistentAgent not found" in raw["failures"][0]["error"]
    ...
    def test_phase3_ast_validation_catch_syntax_error(self, mock_components, tmp_path):
        ...
        # Check custom fields in _raw_result
        raw = result["_raw_result"]
        assert len(raw["remaining_violations"]) > 0

        violation = raw["remaining_violations"][0]
...
""")
        ...
        raw = result["_raw_result"]
        violations = raw["remaining_violations"]
        for v in violations:
...
""")
    ...
    def test_phase3_dry_run_skip(self, mock_components):
        ...
        raw = result["_raw_result"]
        assert raw["message"] == "Dry run - validation skipped"
    ...
    def test_phase3_large_file_protection(self, mock_components, tmp_path):
        ...
        # Large files are treated as errors (file too large for AST analysis)
        assert result["status"] == "FAIL"
        assert result["violations_found"] == 1  # Large file counts as a violation
        assert result["violations_fixed"] == 0
        assert result["errors"] == 1  # File too large error

        raw = result["_raw_result"]
        assert len(raw["remaining_violations"]) == 1
        assert raw["remaining_violations"][0]["type"] == "SYNTAX_ERROR"  # Treated as syntax error
    ...
    def test_reconciliation_manifest_telemetry(self, mock_components):
        ...
        manifest.add_modification({"action": "fix_applied", "target": "file1.py", "success": True})

        manifest.add_modification({"action": "fix_failed", "target": "file2.py", "success": False})

        # Add a failure
        manifest.add_failure({"violation": {"file": "file3.py"}, "reason": "Budget exceeded"})
    ...
    def test_phase2_mixed_success_failure_scenario(self, mock_components):
        ...
        """
        engine, state, agents = mock_components
        fixer_mock = agents["FixerAgent"]
        another_mock = agents["AnotherAgent"]

        # Clear any existing call path to avoid cycle detection
        engine._call_path.clear()

        # Mock agents: one succeeds, one fails
        fixer_mock.heal.return_value = {"status": "success", "changes": 2}
        another_mock.heal.side_effect = Exception("Agent failure")
        ...
        # Verify HEAL_RESULT_SCHEMA compliance
        assert result["violations_found"] == 2
        assert result["violations_fixed"] == 1  # One succeeded, one failed
        assert result["status"] == "ERROR"  # Any execution failure results in ERROR status
        assert result["errors"] == 1  # One failure
        assert result["skipped"] == 0

        # Verify mixed result
        raw = result["_raw_result"]
        assert len(raw["modifications"]) == 1  # One successful modification
        assert len(raw["failures"]) == 1  # One execution failure

        # Check successful modification
        success_mod = raw["modifications"][0]
        ...
        manifest = raw["manifest"]
        assert manifest["violations"]["attempted"] == 1  # One successful attempt
        assert manifest["violations"]["fixed"] == 1
        assert manifest["violations"]["failed"] == 1  # 1 execution failure only
