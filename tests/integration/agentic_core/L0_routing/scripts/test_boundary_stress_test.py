...
class BoundaryStressTest:
    ...
    def test_c_cli_flag_interaction(self) -> dict:
        ...
        try:
            ...
            has_auto_accept = (
                "--yes" in stdout or "auto-accepting" in stdout.lower() or "batch accept" in stdout.lower()
            )
    ...
    def generate_findings_report(self) -> str:
        ...
        structural_behavior = (
            "Automatic" if self.results["test_a_structural"]["status"] == "PASS" else "Prompted (FAIL)"
        )
        archival_behavior = (
            "Prompted" if self.results["test_b_archival"]["status"] == "PASS" else "Automatic (FAIL)"
        )
        cli_behavior = (
            "Override" if self.results["test_c_cli_interaction"]["status"] == "PASS" else "Ignored (FAIL)"
        )
