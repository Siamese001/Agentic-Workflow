...
class TestFetchMCPIntegration:
    ...
    @pytest.mark.asyncio
    async def test_fetch_mcp_enabled(self):
    ...
    @pytest.mark.asyncio
    async def test_fetch_client_singleton(self):
        """Verify singleton pattern for Fetch client."""
        client1 = get_fetch_client()
        client2 = get_fetch_client()
        assert client1 is client2, "Should return same singleton instance"
    
    @pytest.mark.asyncio
    async def test_fetch_mcp_routing(self):
    ...
    @pytest.mark.asyncio
    async def test_fetch_client_methods_available(self):
        """Test all required MCP methods are available."""
        client = get_fetch_client()
        
        # Verify all required methods exist
        required_methods = ['get_clean_content', 'fetch_raw_html', 'fetch_youtube_transcript', 'fetch_multiple_urls', 'health_check']
...
class TestGuardianEnforcement:
    ...
    def test_guardian_blocks_requests_import(self):
        ...
        # Create temp file with requests import
    ...
    def test_guardian_blocks_httpx_import(self):
        """Verify guardian blocks direct httpx imports."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with httpx import
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
    ...
    def test_guardian_blocks_urllib_import(self):
        """Verify guardian blocks direct urllib imports."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with urllib import
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
    ...
    def test_guardian_blocks_requests_get(self):
        """Verify guardian blocks direct requests.get() calls."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with requests.get()
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            f.write("import requests\n")
    ...
    def test_guardian_blocks_httpx_async_client(self):
        """Verify guardian blocks direct httpx.AsyncClient() calls."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with httpx.AsyncClient()
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            f.write("import httpx\n")
    ...
    def test_guardian_allows_fetch_mcp(self):
        """Verify guardian allows Fetch MCP usage."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with Fetch MCP import
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
...
class TestL2ExecutionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_fetch_client_initialization(self):
    ...
    @pytest.mark.asyncio
    async def test_clean_content_method_signature(self):
    ...
    @pytest.mark.asyncio
    async def test_raw_html_method_signature(self):
    ...
    @pytest.mark.asyncio
    async def test_youtube_transcript_method_signature(self):
...
class TestContentIngestion:
    ...
    @pytest.mark.asyncio
    async def test_markdown_conversion_enabled(self):
    ...
    @pytest.mark.asyncio
    async def test_max_content_length_configured(self):
        ...
        assert config.FETCH_MAX_CONTENT_LENGTH <= 10_000_000, "Max content length should be reasonable"
...
class TestSovereigntyProtection:
    """Test sovereignty protection for content ingestion."""
    
    @pytest.mark.asyncio
    async def test_l3_routing_enforced(self):
        """Test all fetch operations route through L3."""
        client = get_fetch_client()
        
        # Verify router exists and is properly configured
    ...
    @pytest.mark.asyncio
    async def test_mcp_tools_used(self):
        """Test correct MCP tools are used."""
        # Verify the client uses the correct MCP tool names
        client = get_fetch_client()
        
        # The client should use mcp3_fetch_url and mcp3_fetch_youtube_transcript
        # This is verified by checking the implementation uses these tool names
