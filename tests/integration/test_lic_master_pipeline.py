...
class MockHOP2Agent:
    ...
    def run_phase(self, buffer: ImmutableStagingBuffer, registry: TraceRegistry) -> None:
        ...
        if self.fail_first_time and self.call_count == 1:
            ...
            buffer.write_once("hop2_research", {
                "evidence_pack": [
                    {"artifact_id": "abc123def456", "summary": "Strategic AI Growth", "source": "vector_db", "confidence": 0.9},
                    {"artifact_id": "xyz789ghi012", "summary": "Two strategic insights I have gleaned", "source": "rag", "confidence": 0.85}
                ],
...
class MockHOP3Agent:
    ...
    def run_phase(self, buffer: ImmutableStagingBuffer, registry: TraceRegistry) -> None:
        buffer.write_once("hop3_grounding", {
...
class TestLICMasterPipeline:
    ...
    def test_full_mission_cxo_inmail_success(self, full_system_mocks):
        ...
        assert any(t["type"] == "CXO_PRECEDENCE_TRIGGERED" for t in traces), "CXO precedence must be triggered for CTO"

        # 2. Verify HOP-5: Specialist Assembly (K.3 Phrase Check)
        report = final_status.get("report", {})
        payload = report.get("payload", {})
        assert "Two strategic insights I have gleaned" in payload.get("message", ""), "Strategic phrase must be in message"

        # 3. Verify HOP-9: Integrity & Delivery
        assert payload.get("delivery_route") == "INMAIL", "Route must be INMAIL for premium + not connected"
    ...
    def test_factual_retry_loop_recovery(self, system_with_factual_fail_mock):
        ...
        assert "FACTUAL_LOOP_TRIGGERED" in traces, "Factual loop must be triggered on alignment failure"
    ...
    def test_signature_immutability_enforcement(self, orchestrator_with_mocks):
        ...
        if "```" in message:
            ...
            assert any("linkedin.com" in line for line in signature_lines), "Signature must contain LinkedIn URL"
    ...
    def test_trace_registry_observability(self, orchestrator_with_mocks):
        ...
        assert len(phase_starts) >= 8, f"Expected at least 8 PHASE_START traces, got {len(phase_starts)}"
