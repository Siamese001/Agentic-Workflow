...
# ---------------------------------------------------------------------------
# W1 — MutationRecord: determinism, tamper-evidence, replay-key chaining
# ---------------------------------------------------------------------------
...
class TestW1MutationRecordCreative:
    ...
    def test_mutation_hash_is_collision_resistant(self):
        ...
        base_kwargs = dict(actor_id="a", run_id="r", operation="write", path="artifacts/x.json",
                           data="hello", replay_key="k1")
        r1 = MutationRecord.build(**base_kwargs)
        for field_name, new_val in [
            ("actor_id", "b"),
            ("run_id", "s"),
            ("operation", "delete"),
            ("path", "artifacts/y.json"),
            ("data", "world"),
            ("replay_key", "k2"),
        ]:
            ...
            assert r1.mutation_hash != r2.mutation_hash, (
                f"Collision on field '{field_name}': same hash for different {field_name}"
            )
    ...
    def test_verify_mutation_record_detects_tampering(self):
        ...
        r = MutationRecord.build(actor_id="actor", run_id="r1", operation="write",
                                 path="artifacts/out.json", data="payload")
        assert UniversalWriteGateway.verify_mutation_record(r)
        # Tamper: change actor_id without recomputing hash
    ...
    def test_mutation_hash_chain_across_writes(self):
        ...
        hashes = []
        for i in range(5):
            rec = uwg.record_mutation(
                path="artifacts/chain.json",
                operation="write",
                data=f"payload-{i}",
                replay_key=f"rk-{i}",
            )
    ...
    def test_mutation_ledger_is_append_only_under_concurrent_access(self):
        ...
        def write_worker(i):
            try:
                rec = uwg.record_mutation(path="artifacts/concurrent.json",
                                          operation="write", data=f"worker-{i}")
...
# ---------------------------------------------------------------------------
# W2 — AgentDispatchRegistry: ledger integrity, enforce mode gate, token shapes
# ---------------------------------------------------------------------------
...
class TestW2DispatchRegistryCreative:
    ...
    def test_mode_transition_shim_to_enforce_blocks_new_dispatches(self):
        ...
        from agentic_core.L3_orchestration.registry.agent_dispatch_registry import (
            AgentDispatchRegistry, DispatchDeniedError,
        )
        from agentic_core.L2_execution.enforcement.guardrail_gate import GuardrailGate
        from agentic_core.L3_orchestration.registry.agent_capability_registry import (
            AgentCapabilityRegistry, AgentCapabilitySpec,
        )

        # Registry with a capability spec that does NOT include "Worker"
        cap_registry = AgentCapabilityRegistry()
        cap_registry.register(AgentCapabilitySpec(
            agent_name="Orch", layer="L3", capabilities=["orchestrate"], handoff_targets=[]
        ))
        gate = GuardrailGate(strict_mode=True)
        registry = AgentDispatchRegistry(
        ...
        w = Worker()
        # In shim mode: allowed despite undeclared handoff
        ...
        # Now the same dispatch must be denied
    ...
    def test_stats_reflect_blocked_plus_permitted(self):
        ...
        gate = GuardrailGate(strict_mode=False)  # non-strict: DENY won't raise
        ...
        reg.dispatch(caller="bad", target_instance=t, method="run")  # guardrail deny, warn continues
...
# ---------------------------------------------------------------------------
# W3 — Guardrail: flapping, audit completeness, verdict propagation
# ---------------------------------------------------------------------------
    ...
    def test_applies_guardrail_context_manager_blocks_body_on_deny(self):
        ...
        with pytest.raises(GuardrailViolationError):
            with gate.applies_guardrail("write:secret", "secrets/x.json"):
                executed.append(True)  # must never run
...
# ---------------------------------------------------------------------------
# W4 — Providers: injection isolation, digest determinism, replay key stability
# ---------------------------------------------------------------------------
    ...
    def test_monotonic_clock_never_goes_backwards(self):
        ...
        for i in range(1, len(readings)):
            assert readings[i] > readings[i - 1], (
                f"Clock went backwards at index {i}: {readings[i-1]} -> {readings[i]}"
            )
...
# ---------------------------------------------------------------------------
# W5 — TraceContext: signing, nested frames, bijection with dispatches
# ---------------------------------------------------------------------------
...
class TestW5TraceContextCreative:
    ...
    def test_sign_changes_when_entries_change(self):
        ...
        with TraceContext.run_frame("run-B") as ctx_b:
            ctx_b.record(layer="L3", module="M", operation="op_2")  # different op
            d_b = ctx_b.sign()
    ...
    def test_assert_transcripted_passes_when_entry_exists(self):
        ...
        with TraceContext.run_frame("run-cov") as ctx:
            ...
            ctx.assert_transcripted("reason")  # must not raise
...
class TestW6RunStateAuthorityCreative:
    ...
    def test_conflict_detection_prevents_stale_read_overwrite(self):
        ...
        # Original reader now tries to overwrite — conflict must be detected
        assert rsa.detect_conflict("slot", version_at_read) is True
        assert rsa.detect_conflict("slot", 2) is False  # current version
    ...
    def test_run_scope_child_has_independent_version_vectors(self):
        ...
        # Parent's version unchanged by child writes
    ...
    def test_backend_fallback_does_not_overwrite_committed_value(self):
        ...
        assert val == "local_value"  # local wins
        assert ver == 1


# ---------------------------------------------------------------------------
# W7 — Burndown tracker: importability, no circular deps, gate logic
# ---------------------------------------------------------------------------
...
class TestW7BurndownTrackerCreative:
    ...
    def test_check_phase_logic_with_synthetic_totals(self):
        ...
        # 7a: dead_imports <= 2000 AND unresolved <= 50
        ...
        # final: dead_imports must be 0, antipattern <= 200
    ...
    def test_baseline_file_loads_without_error(self):
        ...
        # Must have modes section with gate modes
        assert "modes" in b or "counts" in b or len(b) > 0


# ---------------------------------------------------------------------------
# XWAVE — Full cross-wave pipeline: W1+W2+W3+W4+W5+W6 in one orchestration run
# ---------------------------------------------------------------------------
...
class TestXWaveFullPipeline:
    ...
    def test_full_orchestration_run_all_waves_compose(self):
        ...
        # W4: inject deterministic clock and random
        ...
        with TraceContext.run_frame(RUN_ID) as ctx:
            # W6: commit initial state
            rsa.commit("run_status", "started")
            ctx.record(layer="L6", module="RunStateAuthority", operation="state:run_status=started",
                       metadata={"run_id": RUN_ID})

            # W2+W3: dispatch wave phases via registry
            phases = ["wave1", "wave2", "wave3", "wave4", "wave5", "wave6"]
            results = []
            for phase in phases:
                result = registry.dispatch(
                    caller="XWaveOrchestrator",
                    target_instance=agent,
                    method="execute",
                    args=(phase,),
                )
                results.append(result)
                # W6: record phase completion in state authority
                rsa.commit(f"phase_{phase}", "complete")

            # W1: record mutations and snapshot state
            mut = uwg.record_mutation(
                path="artifacts/xwave_output.json",
                operation="write",
                data=json.dumps(results),
                replay_key="xwave-rk-001",
            )
        ...
        # W2: all 6 phase dispatches succeeded
        assert len(results) == 6
        assert all(r["status"] == "done" for r in results)

        # W2+W3: all dispatch records have guardrail_verdict=allow
        ledger = registry.get_dispatch_ledger()
        assert len(ledger) == 6
        assert all(r.guardrail_verdict == "allow" for r in ledger)

        # W1: mutation record is tamper-evident
        assert mut.actor_id == "test-orch"
        assert mut.run_id == RUN_ID

        # W1: UWG snapshot captured all phases
        assert snap["state"]["run_id"] == RUN_ID

        # W6: all phases recorded in state authority
        stats = rsa.get_stats()
        assert stats["total_commits"] == 7  # run_status + 6 phases
        assert stats["total_snapshots"] == 1

        # W5: trace has entries for 6 dispatches + 1 state record = 7
    ...
    def test_two_identical_runs_produce_identical_trace_digests(self):
        ...
        # The two run digests will differ because run_id is embedded in metadata.
        # But the STRUCTURE (layer, module, operation pattern) must be identical.
        # We verify by comparing entry operation lists.
        # (Full digest equality would require identical run_ids — tested separately below)
    ...
    def test_same_run_id_produces_identical_digests(self):
        ...
        assert digests[0] == digests[1], (
            f"Same run_id + frozen clock must produce identical digest. "
            f"Got: {digests[0][:16]}... vs {digests[1][:16]}..."
        )
    ...
    def test_w1_replay_key_matches_w4_clock_derived_key(self):
        ...
        # Build mutation record using that replay key
        ...
        # Re-derive the replay_key with the same frozen clock — must match
    ...
    def test_w6_state_w1_snapshot_content_hashes_are_independent(self):
        ...
        # W1 UWG snapshot: hashes the raw state dict directly
        ...
        # W6 RSA snapshot: hashes {run_id, label, state, versions} payload
        rsa = RunStateAuthority(run_id="r1")
        for k, v in state.items():
            rsa.commit(k, v)
        rsa_snap = rsa.snapshot("test")
        version_vectors = {k: 1 for k in state}  # each key committed once
        rsa_payload = _json.dumps(
            {"run_id": "r1", "label": "test", "state": state, "versions": version_vectors},
        ...
        # The two systems produce different hashes for the same logical state (independent)
        assert uwg_snap["content_hash"][:16] != rsa_snap.content_hash, (
            "UWG and RSA produced identical hashes — they should be independent systems"
        )
