...
class TestPineconeSovereignty:
    """Test suite for Pinecone sovereignty verification."""
    
    @pytest.mark.asyncio
    async def test_pinecone_mcp_client_exists(self):
    ...
    @pytest.mark.asyncio
    async def test_pinecone_mcp_client_singleton(self):
        """Verify singleton pattern for Pinecone MCP client."""
        client1 = get_pinecone_mcp_client()
        client2 = get_pinecone_mcp_client()
        assert client1 is client2, "Should return same singleton instance"
    
    @pytest.mark.asyncio
    async def test_pinecone_mcp_has_search_method(self):
    ...
    @pytest.mark.asyncio
    async def test_pinecone_mcp_has_upsert_method(self):
    ...
    @pytest.mark.asyncio
    async def test_pinecone_mcp_has_router(self):
...
class TestGuardianEnforcement:
    ...
    def test_guardian_blocks_pinecone_import(self):
        ...
        # Create temp file with direct pinecone import
    ...
    def test_guardian_blocks_pinecone_instantiation(self):
        """Verify guardian blocks direct Pinecone() instantiation."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with Pinecone instantiation
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            f.write("import os\n")
            f.write("from pinecone import Pinecone\n")
    ...
    def test_guardian_blocks_index_access(self):
        """Verify guardian blocks direct pc.Index() calls."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with Index access
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
    ...
    def test_guardian_blocks_hardcoded_index_name(self):
        """Verify guardian blocks hardcoded index names."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with hardcoded index name
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
    ...
    def test_guardian_allows_pinecone_mcp(self):
        """Verify guardian allows Pinecone MCP usage."""
        from agentic_core.L0_maintenance.scripts.guard_no_hardcoded_config import check_file
        from pathlib import Path
        import tempfile
        
        # Create temp file with Pinecone MCP import
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
...
class TestL4StateIntegration:
    ...
    @pytest.mark.asyncio
    async def test_pinecone_mcp_routing(self):
    ...
    @pytest.mark.asyncio
    async def test_pinecone_mcp_methods_available(self):
        """Test all required MCP methods are available."""
        client = get_pinecone_mcp_client()
        
        # Verify all required methods exist
        required_methods = ['search', 'upsert', 'inference_embed', 'describe_index_stats', 'health_check']
...
class TestAgentLogicSovereignty:
    """Test Agent Logic sovereignty - ensuring no direct SDK usage."""
    
    def test_no_direct_pinecone_in_agent_logic(self):
        """Verify agent_logic.py doesn't use direct Pinecone SDK."""
        from pathlib import Path
        
        agent_logic_path = Path("c:/Git/Agentic-Workflow/agentic_core/L1_cognition/thought_engine/agent_logic.py")
        
        if agent_logic_path.exists():
            content = agent_logic_path.read_text()
            
            # Check for sovereignty breaches
    ...
    def test_agent_logic_uses_mcp_pattern(self):
        ...
        if agent_logic_path.exists():
            ...
            # If it uses Pinecone, it should use MCP client
            if "pinecone" in content.lower():
                # Should use MCP client pattern
                assert ("pinecone_mcp_client" in content or 
                       "SovereignPineconeMCPClient" in content or
                       "get_pinecone_mcp_client" in content), \
                       "Should use Pinecone MCP client if using Pinecone"
...
class TestMCPClientFeatures:
    ...
    @pytest.mark.asyncio
    async def test_upsert_method_signature(self):
    ...
    @pytest.mark.asyncio
    async def test_inference_embed_method_signature(self):
