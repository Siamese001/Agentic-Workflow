...
class TestHealingTransactionAtomicity:
    """Verify HealingTransaction backup/commit/rollback preserves data integrity."""
    
    def test_successful_commit_clears_backups(
        ...
        """
        # Arrange
        test_file = tmp_sovereign_workspace / "test_module.py"
        original_content = "def original_function():\n    pass\n"
        test_file.write_text(original_content)
        original_hash = file_hash_tracker(test_file)
    ...
    def test_rollback_restores_exact_content(
        ...
        """
        # Arrange
        test_file = tmp_sovereign_workspace / "critical_module.py"
        ...
        corrupted_hash = file_hash_tracker(test_file)
    ...
    def test_partial_failure_triggers_full_rollback(
        self, tmp_sovereign_workspace, healing_transaction_mock, file_hash_tracker
    ):
        """
        ...
        """
        # Arrange
        file1 = tmp_sovereign_workspace / "module1.py"
        file2 = tmp_sovereign_workspace / "module2.py"
        file1.write_text("# Module 1\nclass A:\n    pass\n")
        file2.write_text("# Module 2\nclass B:\n    pass\n")
        
        hash1_original = file_hash_tracker(file1)
        hash2_original = file_hash_tracker(file2)
    ...
    def test_no_backup_no_rollback_corruption(
        self, tmp_sovereign_workspace, healing_transaction_mock
    ):
        """
        ...
        """
        # Arrange
        test_file = tmp_sovereign_workspace / "no_backup.py"
        test_file.write_text("# Original\n")
        
        # Act - modify without backup
        test_file.write_text("# Modified without backup\n")
        healing_transaction_mock.rollback()
        
        # Assert - should remain modified since no backup exists
        assert "Modified without backup" in test_file.read_text()
        assert healing_transaction_mock.rolled_back is True
    
    @pytest.mark.parametrize("file_size", [100, 10000, 100000])
    def test_large_file_rollback_integrity(
        self, tmp_sovereign_workspace, healing_transaction_mock, file_hash_tracker, file_size
    ):
        """
        ...
        """
        # Arrange
        test_file = tmp_sovereign_workspace / f"large_{file_size}.py"
        original_content = "# " + ("x" * file_size) + "\n"
        test_file.write_text(original_content)
        original_hash = file_hash_tracker(test_file)
    ...
    def test_transaction_operations_audit_trail(
        self, tmp_sovereign_workspace, healing_transaction_mock, audit_log_tracker
    ):
        """
        ...
        """
        # Arrange
        file1 = tmp_sovereign_workspace / "audit1.py"
        file2 = tmp_sovereign_workspace / "audit2.py"
        ...
        # Assert
        operations = healing_transaction_mock.get_operations()
        assert len(operations) == 3
        assert operations[0][0] == "backup"
        assert operations[1][0] == "backup"
        assert operations[2][0] == "commit"
        
        audit_entries = audit_log_tracker.get_entries("backup")
        assert len(audit_entries) == 2
    
    def test_nested_transaction_not_supported(
        self, tmp_sovereign_workspace, healing_transaction_mock
    ):
        """
        ...
        """
        # Arrange
        test_file = tmp_sovereign_workspace / "nested.py"
        test_file.write_text("# Original\n")
        ...
        # Second backup overwrites in mock (no nesting)
        healing_transaction_mock.backup(test_file)
        test_file.write_text("# Modified 2\n")
        
        healing_transaction_mock.rollback()
        
        # Assert - should restore to "Modified 1" (last backup)
...
class TestZeroLossGuarantee:
    ...
    def test_binary_file_preservation(
        self, tmp_sovereign_workspace, healing_transaction_mock, file_hash_tracker
    ):
        """
        ...
        """
        # Arrange
        binary_file = tmp_sovereign_workspace / "data.bin"
        binary_content = bytes([0x00, 0xFF, 0xAA, 0x55] * 100)
        binary_file.write_bytes(binary_content)
        original_hash = hashlib.sha256(binary_content).hexdigest()
        
        # Act
        healing_transaction_mock.backup(binary_file)
        binary_file.write_bytes(bytes([0x00] * 400))
        healing_transaction_mock.rollback()
        
        # Assert
        restored_hash = hashlib.sha256(binary_file.read_bytes()).hexdigest()
