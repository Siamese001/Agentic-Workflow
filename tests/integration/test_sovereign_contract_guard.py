...
class SovereignContractGuard:
    ...
    def __init__(self, json_output_path: Optional[str] = None):
        ...
        self.json_output_path = json_output_path or f"sovereign_contract_guard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def mro_audit(self, file_path: Path, class_name: str) -> MROResult:
        ...
        try:
            ...
            if result.mro_valid:
                ...
                logger.warning(f"⚠ MRO issues for {class_name}: Shadowing={result.shadowing_detected}, Missing={result.missing_mixins}")
        ...
        try:
            ...
            try:
                ...
                if isinstance(heal_result, dict):
                    ...
                    logger.info(f"✓ Mock execution successful for {class_name}: returned dict with {len(heal_result)} keys")
                else:
                    result.result_content = {"raw_output": str(heal_result)}
                    logger.warning(f"⚠ Mock execution for {class_name} returned non-dict: {type(heal_result)}")
            ...
            json_results = {
                ...
                "failed_agents": {
                    "import_failures": [
                        ...
                        for r in self.import_results if not r.success
                    ],
                    "signature_violations": [
                        ...
                        for r in self.signature_results if not r.signature_valid
                    ],
                    "mro_violations": [
                        {
                            "class_name": r.class_name,
                        ...
                        for r in self.mro_results if not r.mro_valid
                    ],
                    "execution_failures": [
                        {
                            "class_name": r.class_name,
                        ...
                        for r in self.mock_results if not (r.execution_success and r.result_dict)
                    ]
                },
                "compliance_status": {
    ...
    def generate_summary(self, total_classes: int) -> Dict[str, Any]:
        ...
        missing_subatomic = sum(1 for r in self.import_results 
