...
def test_agent_purpose():
    ...
    lic_count = sum(
        1 for indicator in lic_specific_indicators if indicator.lower() in content.lower()
    )
    ...
    if shared_count > lic_count:
        ...
        test_fail(
            "SCOPE", f"Agent appears LIC-specific (lic: {lic_count} >= shared: {shared_count})"
        )
...
def test_functional_analysis():
    ...
    generic_count = sum(
        ...
        if v and "Generic" in k or "Scans entire" in k or "Detects" in k
    )
    ...
    if generic_count > specific_count:
        test_pass(
            "FUNCTION",
            f"Agent is GENERIC utility (generic: {generic_count} > specific: {specific_count})",
        )
        print("  📊 VERDICT: Should be in apps_shared (generic code quality tool)")
    else:
        test_fail(
            "FUNCTION",
            f"Agent may be app-specific (specific: {specific_count} >= generic: {generic_count})",
        )
