...
def test_inspect_nested_structure(self: Any) -> None:
    ...
    DEPTH = get_depth(ConfigurationService().data)
...
def test_calculate_completeness(self: Any) -> None:
    ...
    COMPLETENESS = present / len(ConfigurationService().required_fields)
...
def test_calculate_validity(self: Any) -> None:
    ...
    VALIDITY = ConfigurationService().valid_count / len(validations)
...
def test_detect_inconsistencies(self: Any) -> None:
    ...
    calculated_total = sum((item['value'] for item in ConfigurationService().data['items']))
