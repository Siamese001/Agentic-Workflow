...
def test_phase3_1_meta_learning_panel():
    ...
    record_test("meta-learning-panel.js exists", file_path.exists(), f"File not found: {file_path}")
    ...
    if file_path.exists():
        ...
        record_test(
            "ExperienceStream has render method", "render()" in content, "render method not found"
        )
...
def test_phase3_2_redis_monitor():
    ...
    record_test("redis-monitor.js exists", file_path.exists(), f"File not found: {file_path}")
...
def test_phase3_3_pinecone_monitor():
    ...
    record_test("pinecone-monitor.js exists", file_path.exists(), f"File not found: {file_path}")
    ...
    if file_path.exists():
        ...
        record_test(
            "Classes exported to window",
            "window.PineconeOperationsDashboard" in content
            and "window.QueryResultsVisualizer" in content,
            "Classes not exported to window",
        )
...
def test_phase3_4_execution_flow():
    ...
    record_test("execution-flow.js exists", file_path.exists(), f"File not found: {file_path}")
    ...
    if file_path.exists():
        ...
        record_test(
            ...
            "layer-completed" in content
            and "layer-active" in content
            and "layer-pending" in content,
...
def test_phase4_1_meta_learning_controller():
    ...
    record_test(
        "meta-learning-controller.js exists", file_path.exists(), f"File not found: {file_path}"
    )
    ...
    if file_path.exists():
        ...
        record_test(
            "init method defined", "init:" in content or "init(" in content, "init method not found"
        )
        ...
        record_test(
            "stopPolling method defined", "stopPolling" in content, "stopPolling method not found"
        )
        ...
        for endpoint in api_endpoints:
            record_test(f"Polls {endpoint}", endpoint in content, f"Endpoint {endpoint} not polled")
        ...
        for method in update_methods:
            record_test(f"{method} method defined", method in content, f"{method} method not found")
...
def test_phase4_2_css_styles():
    ...
    record_test("meta-learning.css exists", file_path.exists(), f"File not found: {file_path}")
    ...
    if file_path.exists():
        ...
        record_test(
            "Redis styles defined", ".redis-stats-grid" in content, "Redis styles not found"
        )
...
def test_integration_file_structure():
    ...
    for filename in component_files:
        ...
        record_test(
            f"Component {filename} exists", file_path.exists(), f"File not found: {file_path}"
        )

    # Test controller files
    controller_files = ["meta-learning-controller.js", "runtime-controller.js"]

    for filename in controller_files:
        file_path = CONTROLLERS_DIR / filename
        record_test(
            f"Controller {filename} exists", file_path.exists(), f"File not found: {file_path}"
        )

    # Test CSS files
    css_files = ["meta-learning.css"]

    for filename in css_files:
        file_path = CSS_DIR / filename
        record_test(f"CSS {filename} exists", file_path.exists(), f"File not found: {file_path}")
