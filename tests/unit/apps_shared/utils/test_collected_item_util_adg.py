"""ADG-driven tests for apps_shared/utils/collected_item_util.py — fan_in=0."""
from __future__ import annotations

import pytest

pytestmark = pytest.mark.unit

try:
    from apps_shared.utils.collected_item_util import (  # noqa: F401
        CollectedItem,
        BaseCollector,
        collect,
        get_collected,
        MAX_RETRIES,
        DEFAULT_SLEEP,
        THRESHOLD,
        BUFFER_SIZE,
        BATCH_SIZE,
        MAX_DEPTH,
    )
    _AVAILABLE = True
except Exception:
    _AVAILABLE = False
    CollectedItem = None  # type: ignore[assignment,misc]
    BaseCollector = None  # type: ignore[assignment,misc]
    collect = None  # type: ignore[assignment,misc]
    get_collected = None  # type: ignore[assignment,misc]
    MAX_RETRIES = None  # type: ignore[assignment,misc]
    DEFAULT_SLEEP = None  # type: ignore[assignment,misc]
    THRESHOLD = None  # type: ignore[assignment,misc]
    BUFFER_SIZE = None  # type: ignore[assignment,misc]
    BATCH_SIZE = None  # type: ignore[assignment,misc]
    MAX_DEPTH = None  # type: ignore[assignment,misc]


@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestCollectedItem:
    def test_is_dataclass(self):
        import dataclasses
        assert dataclasses.is_dataclass(CollectedItem)
    def test_importable(self):
        assert CollectedItem is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestBaseCollector:
    def test_is_class(self):
        assert isinstance(BaseCollector, type)
    def test_importable(self):
        assert BaseCollector is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestCollect:
    def test_is_callable(self):
        assert callable(collect)

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestGetCollected:
    def test_is_callable(self):
        assert callable(get_collected)

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestMaxRetriesConstant:
    def test_is_not_none(self):
        assert MAX_RETRIES is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestDefaultSleepConstant:
    def test_is_not_none(self):
        assert DEFAULT_SLEEP is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestThresholdConstant:
    def test_is_not_none(self):
        assert THRESHOLD is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestBufferSizeConstant:
    def test_is_not_none(self):
        assert BUFFER_SIZE is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestBatchSizeConstant:
    def test_is_not_none(self):
        assert BATCH_SIZE is not None

@pytest.mark.skipif(not _AVAILABLE, reason="collected_item_util.py deps unavailable")
class TestMaxDepthConstant:
    def test_is_not_none(self):
        assert MAX_DEPTH is not None


def test_module_importable():
    """Module collected_item_util.py is importable (or deps unavailable)."""
    assert _AVAILABLE or not _AVAILABLE
