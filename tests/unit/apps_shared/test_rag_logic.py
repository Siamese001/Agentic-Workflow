...
class TestEnhancedSemanticCache:
    ...
    def test_generate_fingerprint_basic(self):
        ...
        fp1 = self.cache.generate_fingerprint("Hello world", "gpt-4", 0.7, "You are helpful")
        fp2 = self.cache.generate_fingerprint("Hello world", "gpt-4", 0.7, "You are helpful")
        fp3 = self.cache.generate_fingerprint("Hello world", "gpt-4", 0.8, "You are helpful")  # Different temp
    ...
    def test_generate_fingerprint_temperature_sensitivity(self):
        ...
        fp_low = self.cache.generate_fingerprint(prompt, model, 0.1, system)
        fp_medium = self.cache.generate_fingerprint(prompt, model, 0.7, system)
        fp_high = self.cache.generate_fingerprint(prompt, model, 1.0, system)
    ...
    def test_generate_fingerprint_model_sensitivity(self):
        ...
        fp_gpt3 = self.cache.generate_fingerprint(prompt, "gpt-3.5-turbo", temp, system)
        fp_gpt4 = self.cache.generate_fingerprint(prompt, "gpt-4", temp, system)
    ...
    def test_generate_fingerprint_system_prompt_sensitivity(self):
        ...
        fp_system1 = self.cache.generate_fingerprint(prompt, model, temp, "Be helpful")
        fp_system2 = self.cache.generate_fingerprint(prompt, model, temp, "Be concise")
    ...
    def test_lookup_returns_none_on_temperature_change(self):
        ...
        fp_low_temp = self.cache.generate_fingerprint(prompt, model, 0.7, system)
        
        # Store data with low temp fingerprint
        self.cache.store(fp_low_temp, {'response': 'Paris'})
        
        # Generate fingerprint with temperature 0.9
        fp_high_temp = self.cache.generate_fingerprint(prompt, model, 0.9, system)
    ...
    def test_fingerprint_consistency(self):
        ...
        fp1 = self.cache.generate_fingerprint(prompt, model, 0.7, system)
        fp2 = self.cache.generate_fingerprint(prompt, model, 0.7, system)
        fp3 = self.cache.generate_fingerprint(prompt, model, 0.7, system)
        
        assert fp1 == fp2 == fp3
        
        # Verify whitespace is handled (stripped)
        fp_stripped = self.cache.generate_fingerprint(
            prompt.strip(),
            model.strip(),
            0.7,
            system.strip()
...
class TestRAGIntegration:
    ...
    def test_hybrid_scorer_with_cache(self):
        ...
        fingerprint = cache.generate_fingerprint(query, "gpt-4", 0.7)
