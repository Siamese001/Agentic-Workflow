"""Enhanced behavioral tests for agentic_core.knowledge.document_loaders.html_loader."""
...
def test_htmldocumentloader_is_instantiable():
    """HTMLDocumentLoader can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_htmlparser_is_instantiable():
    """HTMLParser can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
