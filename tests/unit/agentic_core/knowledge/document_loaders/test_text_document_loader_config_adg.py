"""Enhanced behavioral tests for agentic_core.knowledge.document_loaders.text_document_loader_config."""
...
def test_textdocumentloader_is_instantiable():
    """TextDocumentLoader can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
