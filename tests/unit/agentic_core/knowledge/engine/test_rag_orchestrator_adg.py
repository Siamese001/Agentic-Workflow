...
def test_clear_embedding_cache_is_callable(mod):
    """clear_embedding_cache is accessible and callable."""
    func = getattr(mod, "clear_embedding_cache", None)
    assert func is not None, "clear_embedding_cache must be defined in {MODULE_PATH}"
    assert callable(func), "clear_embedding_cache must be callable"
...
def test_get_rag_manager_is_callable(mod):
    """get_rag_manager is accessible and callable."""
    func = getattr(mod, "get_rag_manager", None)
    assert func is not None, "get_rag_manager must be defined in {MODULE_PATH}"
    assert callable(func), "get_rag_manager must be callable"


def test_timeout_is_callable(mod):
    """timeout is accessible and callable."""
    func = getattr(mod, "timeout", None)
    assert func is not None, "timeout must be defined in {MODULE_PATH}"
    assert callable(func), "timeout must be callable"
