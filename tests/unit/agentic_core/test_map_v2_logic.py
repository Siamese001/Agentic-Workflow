...
def test_tc_1_5_2_orphan_reduction(migration_data):
    ...
    rg_conftest = next((p for c, p in migration_data.items() if "tests\\apps_rg\\conftest.py" in c or "tests/apps_rg/conftest.py" in c), None)
    
    if rg_conftest:
        assert "agentic_core" not in rg_conftest, "apps_rg/conftest.py incorrectly mapped to agentic_core"
        assert "tests/unit/apps_rg/conftest.py" in rg_conftest or "tests/integration/apps_rg/conftest.py" in rg_conftest, \
...
def test_tc_1_5_3_root_conftest_handling(migration_data):
    ...
    """
    root_conftest = next((p for c, p in migration_data.items() if c.strip() == "tests\\conftest.py" or c.strip() == "tests/conftest.py"), None)
    
    if root_conftest:
        assert "agentic_core" not in root_conftest, "Root conftest incorrectly sucked into agentic_core gravity."
        assert "tests/fixtures/root_conftest.py" in root_conftest, f"Root conftest target invalid: {root_conftest}"
...
def test_tc_1_5_4_strict_mirror_structure(migration_data):
    ...
    assert not violations, f"Proposed paths violate Strict Mirror Structure:\n" + "\n".join(violations[:5])
