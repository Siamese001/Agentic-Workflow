...
class TestUnifiedWorkManifest(unittest.TestCase):
    def setUp(self):
        ...
        self.agent.run_audit = MagicMock(return_value={"violations": [], "stats": {"violations_found": 0}})

    @patch('agentic_core.L5_safety.validators.HierarchyAgent.HierarchyAgent.scan_root_violations')
    @patch('agentic_core.L5_safety.validators.SystemArchitectAgent.SystemArchitectAgent.validate_core_architecture')
    def test_complete_telemetry_ingestion(self, mock_arch, mock_hierarchy):
        ...
        manifest = self.agent.comprehensive_territory_audit(target_territories=["prompt_governance"])
        ...
        self.assertEqual(manifest["stats"]["violations_found"], 2, "Total count must match sum of all agents")
