...
class TestDetailedReporting(unittest.TestCase):
    def setUp(self):
        ...
        self.state_mgr.state = {
            "compliance_report": {
                ...
                "stats": {"drift_detected": 1, "violations_found": 2, "errors": 0, "violations_fixed": 1}
            },
            "compliance_scores": {"prompt_governance": 0.85},
            "decisions_made": [
                {"confidence": 0.65, "decision": True, "reason": "Fix Naming - LOW CONFIDENCE (0.65) - LLM Override"},
                {"confidence": 0.92, "decision": True, "reason": "Hierarchy Healing - HIGH CONFIDENCE (0.92)"}
            ]
        }
    ...
    def test_compliant_status_logic(self):
        ...
        with patch('sys.stdout') as mock_stdout:
            ...
            self.state_mgr.state["compliance_report"]["violations"] = [{"file": "test.py", "type": "TEST"}]
            result = execute_phase5_final_impl({}, "test_territory", self.state_mgr)
            self.assertEqual(result["meta"]["status"], "NON-COMPLIANT")
