"""Enhanced behavioral tests for agentic_core.adg.runtime.io_interception."""
...
def test_iointerceptionevent_is_instantiable():
    """IOInterceptionEvent can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
