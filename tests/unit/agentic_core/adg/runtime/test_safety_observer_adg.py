"""Enhanced behavioral tests for agentic_core.adg.runtime.safety_observer."""
...
def test_guardrailexecution_is_instantiable():
    """GuardrailExecution can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
