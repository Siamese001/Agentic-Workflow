"""Enhanced behavioral tests for agentic_core.adg.runtime.capability_budget."""
...
def test_budgetevent_is_instantiable():
    """BudgetEvent can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_budgetexceedederror_is_instantiable():
    """BudgetExceededError can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
