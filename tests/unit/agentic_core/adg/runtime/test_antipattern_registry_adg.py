"""Enhanced behavioral tests for agentic_core.adg.runtime.antipattern_registry."""
...
def test_antipatterncategory_is_instantiable():
    """AntipatternCategory can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_antipatternrecord_is_instantiable():
    """AntipatternRecord can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_antipatternregistry_is_instantiable():
    """AntipatternRegistry can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
