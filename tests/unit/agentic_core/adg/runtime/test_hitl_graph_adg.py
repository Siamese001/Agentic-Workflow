"""Enhanced behavioral tests for agentic_core.adg.runtime.hitl_graph."""
...
def test_hitlcheckpoint_is_instantiable():
    """HITLCheckpoint can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
