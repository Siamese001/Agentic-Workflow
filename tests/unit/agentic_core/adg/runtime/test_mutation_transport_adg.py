"""Enhanced behavioral tests for agentic_core.adg.runtime.mutation_transport."""
...
def test_commitphase_is_instantiable():
    """CommitPhase can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
