"""Enhanced behavioral tests for agentic_core.adg.runtime.jit_context."""
...
def test_contextsnapshot_is_instantiable():
    """ContextSnapshot can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
