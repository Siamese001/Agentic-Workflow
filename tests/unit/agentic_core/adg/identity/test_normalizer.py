"""Enhanced behavioral tests for agentic_core.adg.identity.normalizer."""
...
def test_identityconfidence_is_instantiable():
    """IdentityConfidence can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_build_identity_index_is_callable():
    """build_identity_index is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
