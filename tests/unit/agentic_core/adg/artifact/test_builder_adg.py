"""Enhanced behavioral tests for agentic_core.adg.artifact.builder_types."""
...
def test_blindspotreport_is_instantiable():
    """BlindSpotReport can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
