"""Proof-evidence test for 10C-REQ-079 (03_L0_Route_Decision).
...
Source        : 03_Route_Decision_Switching.md :: D4 R4 OUTSIDE HELP

Canonical requirement (paraphrased): Outside help path R4 MUST: Route to external action dispatch packet only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-079]: RouteContract emitted without satisfying 'Outside help path R4 MUST: Route to external action dispatch packet on...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-79-001",
        "trace_id": "trace-79-001",
        "request_id": "req-79-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-79-h",
        "blueprint_hash": "blueprint-79-h",
        "replay_key": "replay-79-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-79-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-079: per ledger row, a RouteContract that violates the canonical
