...
def test_test_complexity_can_import():
    """Test that the module can be imported successfully."""
    try:
        spec = importlib.util.spec_from_file_location(
            "complexity",
            str(Path(__file__).resolve().parents[4] / "dev_tools" / "l0_scripts" / "complexity.py"),
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert mod is not None
    except Exception as e:
        pytest.skip(f"Cannot load dev_tools/l0_scripts/complexity.py: {e}")


def test_test_complexity_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/complexity.py")


def test_test_complexity_has_public_attributes():
