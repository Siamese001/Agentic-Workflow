...
def test_run_drift_detection_guardian_is_callable(mod):
    """run_drift_detection_guardian is accessible and callable."""
    func = getattr(mod, "run_drift_detection_guardian", None)
    assert func is not None, "run_drift_detection_guardian must be defined in {MODULE_PATH}"
    assert callable(func), "run_drift_detection_guardian must be callable"


def test_scan_archived_files_at_root_is_callable(mod):
    """scan_archived_files_at_root is accessible and callable."""
    func = getattr(mod, "scan_archived_files_at_root", None)
    assert func is not None, "scan_archived_files_at_root must be defined in {MODULE_PATH}"
    assert callable(func), "scan_archived_files_at_root must be callable"
