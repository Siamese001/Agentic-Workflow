"""Enhanced behavioral tests for agentic_core.L0_routing.scripts.file_analysis."""
...
def test_fileanalysis_is_instantiable():
    """FileAnalysis can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_analyze_class_is_callable():
    """analyze_class is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_analyze_function_is_callable():
    """analyze_function is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
