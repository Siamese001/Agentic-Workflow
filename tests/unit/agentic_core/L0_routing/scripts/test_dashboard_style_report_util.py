...
def test_test_dashboard_style_report_util_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "dashboard_style_report_util",
            str(
                Path(__file__).resolve().parents[4]
                / "dev_tools"
                / "l0_scripts"
                / "dashboard_style_report_util.py",
            ),
        )
        ...
        pytest.skip(
            f"Cannot load dev_tools/l0_scripts/dashboard_style_report_util.py: {e}",
        )
...
def test_test_dashboard_style_report_util_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/dashboard_style_report_util.py")
