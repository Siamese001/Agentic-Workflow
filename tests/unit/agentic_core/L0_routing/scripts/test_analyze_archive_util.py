...
def test_test_analyze_archive_util_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "analyze_archive_util",
            str(Path(__file__).resolve().parents[4] / "dev_tools" / "l0_scripts" / "analyze_archive_util.py"),
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert mod is not None
    except Exception as e:
        pytest.skip(f"Cannot load dev_tools/l0_scripts/analyze_archive_util.py: {e}")
...
def test_test_analyze_archive_util_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/analyze_archive_util.py")
