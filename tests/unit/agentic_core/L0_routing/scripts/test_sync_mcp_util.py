...
def test_test_sync_mcp_util_can_import():
    """Test that the module can be imported successfully."""
    try:
        mod = load_dev_script("sync_mcp_util.py")
        assert mod is not None
    except Exception as e:
        pytest.skip(f"Cannot load dev_tools/l0_scripts/sync_mcp_util.py: {e}")


def test_test_sync_mcp_util_has_file_attribute():
    """Test that module has __file__ attribute."""
    try:
        mod = load_dev_script("sync_mcp_util.py")
        assert hasattr(mod, "__file__")
    except Exception as e:
        pytest.skip("Cannot load dev_tools/l0_scripts/sync_mcp_util.py")


def test_test_sync_mcp_util_has_public_attributes():
