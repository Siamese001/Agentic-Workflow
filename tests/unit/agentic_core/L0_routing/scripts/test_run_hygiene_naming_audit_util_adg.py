"""Behavioral contract tests for agentic_core.L0_routing.scripts.run_hygiene_naming_audit_util."""
...
def test_load_hygiene_guardian_is_callable(mod):
    """load_hygiene_guardian is accessible and callable."""
    func = getattr(mod, "load_hygiene_guardian", None)
    assert func is not None, "load_hygiene_guardian must be defined in {MODULE_PATH}"
    assert callable(func), "load_hygiene_guardian must be callable"
