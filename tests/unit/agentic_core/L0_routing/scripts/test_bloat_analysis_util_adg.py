"""Enhanced behavioral tests for agentic_core.L0_routing.scripts.bloat_analysis_util."""
...
def test_datetime_is_instantiable():
    """datetime can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_find_deprecated_markers_is_callable():
    """find_deprecated_markers is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
