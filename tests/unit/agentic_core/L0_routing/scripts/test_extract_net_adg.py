"""Behavioral contract tests for agentic_core.L0_routing.scripts.extract_net."""
from __future__ import annotations

import importlib
import pytest

MODULE_PATH = "agentic_core.L0_routing.scripts.extract_net"
...
def test_extract_net_incremental_is_callable(mod):
    """extract_net_incremental is accessible and callable."""
    func = getattr(mod, "extract_net_incremental", None)
    assert func is not None, "extract_net_incremental must be defined in {MODULE_PATH}"
    assert callable(func), "extract_net_incremental must be callable"
