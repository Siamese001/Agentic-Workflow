...
def test_test_analyze_extract_can_import():
    """Test that the module can be imported successfully."""
    try:
        spec = importlib.util.spec_from_file_location(
            "analyze_extract",
            str(Path(__file__).resolve().parents[4] / "dev_tools" / "l0_scripts" / "analyze_extract.py"),
        )
...
def test_test_analyze_extract_has_file_attribute():
...
def test_test_analyze_extract_has_public_attributes():
