...
def test_test_dashboard_verifier_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "dashboard_verifier",
            str(Path(__file__).resolve().parents[4] / "dev_tools" / "l0_scripts" / "dashboard_verifier.py"),
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert mod is not None
    except Exception as e:
        pytest.skip(f"Cannot load dev_tools/l0_scripts/dashboard_verifier.py: {e}")
...
def test_test_dashboard_verifier_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/dashboard_verifier.py")
