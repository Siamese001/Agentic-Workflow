"""Enhanced behavioral tests for agentic_core.L0_routing.scripts.drift."""
...
def test_driftdetector_is_instantiable():
    """DriftDetector can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_scan_repository_is_callable():
    """scan_repository is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
