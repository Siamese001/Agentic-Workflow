"""ADG-driven tests for agentic_core/L0_routing/scripts/gatekeeper_lock_util.py — fan_in=0."""
...
@pytest.mark.skipif(not _AVAILABLE, reason="gatekeeper_lock_util.py deps unavailable")
class TestCheckEnvBypass:
    def test_is_callable(self):
        assert callable(check_env_bypass)

@pytest.mark.skipif(not _AVAILABLE, reason="gatekeeper_lock_util.py deps unavailable")
class TestCheckCommitMessageOverride:
    def test_is_callable(self):
...
def test_module_importable():
    """Module gatekeeper_lock_util.py is importable (or deps unavailable)."""
