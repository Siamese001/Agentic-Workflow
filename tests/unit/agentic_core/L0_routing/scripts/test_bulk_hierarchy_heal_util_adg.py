"""Behavioral contract tests for agentic_core.L0_routing.scripts.bulk_hierarchy_heal_util."""
from __future__ import annotations

import importlib
import pytest

MODULE_PATH = "agentic_core.L0_routing.scripts.bulk_hierarchy_heal_util"
...
def test_log_move_is_callable(mod):
    """log_move is accessible and callable."""
    func = getattr(mod, "log_move", None)
    assert func is not None, "log_move must be defined in {MODULE_PATH}"
    assert callable(func), "log_move must be callable"
...
def test_safe_shutil_rmtree_is_callable(mod):
    """safe_shutil_rmtree is accessible and callable."""
    func = getattr(mod, "safe_shutil_rmtree", None)
    assert func is not None, "safe_shutil_rmtree must be defined in {MODULE_PATH}"
    assert callable(func), "safe_shutil_rmtree must be callable"
