...
def test_file_hash_is_callable(mod):
    """file_hash is accessible and callable."""
    func = getattr(mod, "file_hash", None)
    assert func is not None, "file_hash must be defined in {MODULE_PATH}"
    assert callable(func), "file_hash must be callable"


def test_find_in_current_is_callable(mod):
    """find_in_current is accessible and callable."""
    func = getattr(mod, "find_in_current", None)
    assert func is not None, "find_in_current must be defined in {MODULE_PATH}"
    assert callable(func), "find_in_current must be callable"
