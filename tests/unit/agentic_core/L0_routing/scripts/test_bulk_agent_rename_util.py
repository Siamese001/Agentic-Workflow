...
def test_test_bulk_agent_rename_util_can_import():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "bulk_agent_rename_util",
            str(
                Path(__file__).resolve().parents[4]
                / "dev_tools"
                / "l0_scripts"
                / "bulk_agent_rename_util.py",
            ),
        )
        ...
        pytest.skip(f"Cannot load dev_tools/l0_scripts/bulk_agent_rename_util.py: {e}")
...
def test_test_bulk_agent_rename_util_has_file_attribute():
    ...
    try:
        ...
        pytest.skip("Cannot load dev_tools/l0_scripts/bulk_agent_rename_util.py")
