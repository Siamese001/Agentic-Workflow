"""Enhanced behavioral tests for agentic_core.L0_routing.scripts.extract_unique_content_util."""
...
def test_analyze_archive_file_is_callable():
    """analyze_archive_file is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_build_codebase_index_is_callable():
    """build_codebase_index is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
