"""Enhanced behavioral tests for agentic_core.L0_routing.scripts.reasoning."""
...
def test_chainofthoughtstrategy_is_instantiable():
    """ChainOfThoughtStrategy can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
