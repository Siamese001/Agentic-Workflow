"""Enhanced behavioral tests for agentic_core.L0_routing.types.guardian_registry_types."""
...
def test_guardianspec_is_instantiable():
    """GuardianSpec can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_guardiantier_is_instantiable():
    """GuardianTier can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
