"""Enhanced behavioral tests for agentic_core.L0_routing.types.governance_types."""
...
def test_changeaction_is_instantiable():
    """ChangeAction can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_evidencepack_is_instantiable():
    """EvidencePack can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
