"""Enhanced behavioral tests for agentic_core.L0_routing.enforcement.mutation_prohibition."""
...
def test_protectedrootblockevent_is_instantiable():
    """ProtectedRootBlockEvent can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
