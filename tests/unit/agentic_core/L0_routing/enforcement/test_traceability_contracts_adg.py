"""Enhanced behavioral tests for agentic_core.L0_routing.enforcement.traceability_contracts."""
...
def test_advisoryviolationerror_is_instantiable():
    """AdvisoryViolationError can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_build_cognitive_diff_bundle_is_callable():
    """build_cognitive_diff_bundle is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_build_error_signature_is_callable():
    """build_error_signature is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
...
def test_build_plan_provenance_is_callable():
    """build_plan_provenance is callable."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        func = getattr(mod, func_name)
    except (ImportError, AttributeError) as e:
