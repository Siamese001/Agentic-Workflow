"""Proof-evidence test for 10C-REQ-080 (03_L0_Route_Decision).
...
Source        : 03_Route_Decision_Switching.md :: R5 SAFE DEFAULT

Canonical requirement (paraphrased): Safe default path R5 MUST: Provide abstain clarify bounded backup when no other path matches with no-silent-fallback rule

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-080]: RouteContract emitted without satisfying 'Safe default path R5 MUST: Provide abstain clarify bounded backup when...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-80-001",
        "trace_id": "trace-80-001",
        "request_id": "req-80-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-80-h",
        "blueprint_hash": "blueprint-80-h",
        "replay_key": "replay-80-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-80-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-080: per ledger row, a RouteContract that violates the canonical
