...
def test_namespace_lookup_falls_back_to_global_when_absent() -> None:
    ...
    assert (
        routing_calibration.get_similarity_threshold(namespace="nonexistent_ns")
        == 0.98
    )
...
""".strip(),
    ...
    assert (
        routing_calibration.get_similarity_threshold(namespace="tenant_alpha")
        == 0.92
    )
    # Unknown namespace falls back to global.
    assert (
        routing_calibration.get_similarity_threshold(namespace="tenant_omega")
        == 0.85
    )
