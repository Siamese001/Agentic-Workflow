"""Behavioral contract tests for agentic_core.L0_routing.reasoning.SSOTFolderCleanupAgent."""
from __future__ import annotations

import importlib
import pytest

MODULE_PATH = "agentic_core.L0_routing.reasoning.SSOTFolderCleanupAgent"
...
def test_ssotfoldercleanupagent_is_instantiable(mod):
    """SSOTFolderCleanupAgent is accessible and is a type."""
    cls = getattr(mod, "SSOTFolderCleanupAgent", None)
    assert cls is not None, "SSOTFolderCleanupAgent must be defined in {MODULE_PATH}"
    assert isinstance(cls, type), "SSOTFolderCleanupAgent must be a class"
