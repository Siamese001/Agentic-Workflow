"""Proof-evidence test for 10C-REQ-076 (03_L0_Route_Decision).
...
Source        : 03_Route_Decision_Switching.md :: D1 R1A EXACT CACHE

Canonical requirement (paraphrased): Exact cache path R1A MUST: Check exact cache key by policy allow zero new thinking on hit evict stale items if invalid enable deep pipeline ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-076]: RouteContract emitted without satisfying 'Exact cache path R1A MUST: Check exact cache key by policy allow zero ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-76-001",
        "trace_id": "trace-76-001",
        "request_id": "req-76-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-76-h",
        "blueprint_hash": "blueprint-76-h",
        "replay_key": "replay-76-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-76-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-076: per ledger row, a RouteContract that violates the canonical
