"""Proof-evidence test for 10C-REQ-170 (03_L0_Route_Decision).
...
Source        : agentic_process_mapping_v29.md :: [3] ROUTE DECISION

Canonical requirement (paraphrased): Routing MUST: Have authority to determine execution path cache RAG action fallback

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-170]: RouteContract emitted without satisfying 'Routing MUST: Have authority to determine execution path cache RAG act...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-170-001",
        "trace_id": "trace-170-001",
        "request_id": "req-170-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-170-h",
        "blueprint_hash": "blueprint-170-h",
        "replay_key": "replay-170-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-170-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-170: per ledger row, a RouteContract that violates the canonical
