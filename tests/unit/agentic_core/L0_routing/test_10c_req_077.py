"""Proof-evidence test for 10C-REQ-077 (03_L0_Route_Decision).
...
Source        : 03_Route_Decision_Switching.md :: D2 R1B SEMANTIC CACHE

Canonical requirement (paraphrased): Semantic cache path R1B MUST: Compare query_vec to cached vector approve support threshold check freshness policy valid prevent unsafe seman...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-077]: RouteContract emitted without satisfying 'Semantic cache path R1B MUST: Compare query_vec to cached vector appro...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-77-001",
        "trace_id": "trace-77-001",
        "request_id": "req-77-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-77-h",
        "blueprint_hash": "blueprint-77-h",
        "replay_key": "replay-77-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-77-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-077: per ledger row, a RouteContract that violates the canonical
