"""Behavioral contract tests for agentic_core.L0_routing.engines.assembly_stage."""
from __future__ import annotations

import importlib
import pytest

MODULE_PATH = "agentic_core.L0_routing.engines.assembly_stage"
...
def test_airlockassembler_is_instantiable(mod):
    """AirlockAssembler is accessible and is a type."""
    cls = getattr(mod, "AirlockAssembler", None)
    assert cls is not None, "AirlockAssembler must be defined in {MODULE_PATH}"
    assert isinstance(cls, type), "AirlockAssembler must be a class"
