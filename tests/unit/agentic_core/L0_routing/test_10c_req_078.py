"""Proof-evidence test for 10C-REQ-078 (03_L0_Route_Decision).
...
Source        : 03_Route_Decision_Switching.md :: D3 R3 GROUNDED

Canonical requirement (paraphrased): Grounded path R3 MUST: Route to C0 context engine for evidence class support target citation requirement grounded answer only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-078]: RouteContract emitted without satisfying 'Grounded path R3 MUST: Route to C0 context engine for evidence class s...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-78-001",
        "trace_id": "trace-78-001",
        "request_id": "req-78-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-78-h",
        "blueprint_hash": "blueprint-78-h",
        "replay_key": "replay-78-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-78-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-078: per ledger row, a RouteContract that violates the canonical
