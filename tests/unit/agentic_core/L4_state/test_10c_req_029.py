"""Proof-evidence test for 10C-REQ-029 (00B_L4_State_Archive_and_UWG).
...
Source        : 00C_index_materialization_runtime_handoff.md :: C1. VECTOR STORE

Canonical requirement (paraphrased): Vector store MUST be fast math shelf storing UUID->fact_vec with sidecar UUID->metadata_refs using engines ChromaDB pgvector FAISS

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-029]: CommitRequest emitted without satisfying 'Vector store MUST be fast math shelf storing UUID->fact_vec with sidec...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-29-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-29-h",
        "policy_hash": "policy-29-h",
        "diff_payload_hash": "diff-29-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-029: per ledger row, a CommitRequest that violates the canonical
