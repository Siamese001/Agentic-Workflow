"""Proof-evidence test for 10C-REQ-019 (00B_L4_State_Archive_and_UWG).
...
Source        : 00A_ingestion_embedding_implementation.md :: A8. CANONICAL SIDECAR WRITE

Canonical requirement (paraphrased): Canonical sidecar write MUST store: ChunkManifest ParentChildIndex sparse terms provenance in canonical metadata store

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-019]: CommitRequest emitted without satisfying 'Canonical sidecar write MUST store: ChunkManifest ParentChildIndex spa...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-19-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-19-h",
        "policy_hash": "policy-19-h",
        "diff_payload_hash": "diff-19-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-019: per ledger row, a CommitRequest that violates the canonical
