"""Proof-evidence test for 10C-REQ-152 (00B_L4_State_Archive_and_UWG).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: RULE DRAFTING 6

Canonical requirement (paraphrased): C6 draft MUST: Derive fix target structure improvements controls rollback floor staff propose only no live changes

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-152]: CommitRequest emitted without satisfying 'C6 draft MUST: Derive fix target structure improvements controls rollb...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-152-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-152-h",
        "policy_hash": "policy-152-h",
        "diff_payload_hash": "diff-152-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-152-001",
        "trace_id": "trace-152-001",
        "request_id": "req-152-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-152-h",
        "blueprint_hash": "blueprint-152-h",
        "replay_key": "replay-152-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-152-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-152: per ledger row, a CommitRequest that violates the canonical
