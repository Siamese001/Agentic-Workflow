"""Proof-evidence test for 10C-REQ-127 (00B_L4_State_Archive_and_UWG).
...
Source        : C4_State_Sovereignty_Universal_Write_Governance.md :: REFRESH READ SURFACES

Canonical requirement (paraphrased): C4 refresh MUST: Execute alias swap clear retrieval caches ensure very next request sees updated state

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-127]: CommitRequest emitted without satisfying 'C4 refresh MUST: Execute alias swap clear retrieval caches ensure very...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-127-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-127-h",
        "policy_hash": "policy-127-h",
        "diff_payload_hash": "diff-127-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-127-001",
        "trace_id": "trace-127-001",
        "request_id": "req-127-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-127-h",
        "blueprint_hash": "blueprint-127-h",
        "replay_key": "replay-127-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-127-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-127: per ledger row, a CommitRequest that violates the canonical
