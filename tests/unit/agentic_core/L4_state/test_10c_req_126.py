"""Proof-evidence test for 10C-REQ-126 (00B_L4_State_Archive_and_UWG).
...
Source        : C4_State_Sovereignty_Universal_Write_Governance.md :: COMMIT + CHAIN APPEND

Canonical requirement (paraphrased): C4 commit MUST: Perform durable ledger write hash-chain audit log update sync to permanent L4 archive

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-126]: CommitRequest emitted without satisfying 'C4 commit MUST: Perform durable ledger write hash-chain audit log upda...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-126-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-126-h",
        "policy_hash": "policy-126-h",
        "diff_payload_hash": "diff-126-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-126-001",
        "trace_id": "trace-126-001",
        "request_id": "req-126-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-126-h",
        "blueprint_hash": "blueprint-126-h",
        "replay_key": "replay-126-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-126-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-126: per ledger row, a CommitRequest that violates the canonical
