"""Proof-evidence test for 10C-REQ-124 (00B_L4_State_Archive_and_UWG).
...
Source        : C4_State_Sovereignty_Universal_Write_Governance.md :: CHECK CATALOG RULES

Canonical requirement (paraphrased): C4 catalog MUST: Verify RBAC blast radius structure constraints perform before-after diff validation of knowledge base

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-124]: CommitRequest emitted without satisfying 'C4 catalog MUST: Verify RBAC blast radius structure constraints perfor...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-124-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-124-h",
        "policy_hash": "policy-124-h",
        "diff_payload_hash": "diff-124-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-124-001",
        "trace_id": "trace-124-001",
        "request_id": "req-124-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-124-h",
        "blueprint_hash": "blueprint-124-h",
        "replay_key": "replay-124-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-124-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-124: per ledger row, a CommitRequest that violates the canonical
