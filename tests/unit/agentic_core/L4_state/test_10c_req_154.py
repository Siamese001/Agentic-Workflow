"""Proof-evidence test for 10C-REQ-154 (00B_L4_State_Archive_and_UWG).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: KNOWLEDGE EXTRACTION 8

Canonical requirement (paraphrased): C6 extract MUST: Classify destination type route to rubric desk rules catalog priors

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-154]: CommitRequest emitted without satisfying 'C6 extract MUST: Classify destination type route to rubric desk rules ...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-154-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-154-h",
        "policy_hash": "policy-154-h",
        "diff_payload_hash": "diff-154-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-154-001",
        "trace_id": "trace-154-001",
        "request_id": "req-154-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-154-h",
        "blueprint_hash": "blueprint-154-h",
        "replay_key": "replay-154-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-154-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-154: per ledger row, a CommitRequest that violates the canonical
