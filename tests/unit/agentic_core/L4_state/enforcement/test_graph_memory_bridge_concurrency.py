...
class TestGraphMemoryBridgeConcurrency:
            ...
            try:
                for i in range(10):
                    ...
                    if bridge.create_agent_entity(entity_name, 'test_type'):
                        entity_count += 1
                results[instance_id] = {'entity_count': entity_count, 'stats': bridge.stats.copy(), 'registered_count': len(bridge._registered_entities), 'validation': bridge.validate_state_isolation()}
            except Exception as e:
    ...
    def test_concurrent_cleanup_operations(self):
        ...
        def create_and_cleanup(bridge_id):
            ...
            cleanup_results.append({'bridge_id': bridge_id, 'pre_clean_entities': pre_cleanup['registered_entities_count'], 'post_clean_entities': post_cleanup['registered_entities_count'], 'is_clean_after': post_cleanup['is_clean']})
        ...
        def use_context_manager(instance_id):
            try:
                with GraphMemoryBridge() as bridge:
                    ...
                    context_results.append({'instance_id': instance_id, 'entities_created': validation['registered_entities_count'], 'stats': bridge.stats.copy()})
