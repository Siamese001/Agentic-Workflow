...
@pytest.mark.skipif(not _AVAILABLE, reason="ghost_mutation_detector.py deps unavailable")
class TestGhostMutationDetectorImportability:
    def test_module_importable(self) -> None:
        """ADG contract: ghost_mutation_detector.py must be importable."""
        assert _AVAILABLE

    def test_ghostmutationviolation_is_type(self) -> None:
        assert GhostMutationViolation is not None

    def test_reconciliationresult_is_type(self) -> None:
        assert ReconciliationResult is not None

    def test_detect_ghost_mutations_callable(self) -> None:
        assert callable(detect_ghost_mutations)
