"""Proof-evidence test for 10C-REQ-177 (00B_L4_State_Archive_and_UWG).

Severity      : CRITICAL
Surface       : 00B_L4_State_Archive_and_UWG
Layer owner   : L4 UWG
Artifact      : CommitRequest
OTEL span     : uwg.commit.committed
Source        : agentic_process_mapping_v38.md + best-practice gap analysis 2026-04-29 :: HITL-NEW-D UWG dual-control for regulated/irreversible commits

Canonical requirement (paraphrased): UWG MUST require two distinct authenticated identities to admit a CommitRequest whose commit_class is in {regulated, irreversible_state}. Th...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-177]: CommitRequest emitted without satisfying 'UWG MUST require two distinct authenticated identities to admit a Comm...
...
EXPECTED_SPAN = "uwg.commit.committed"
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-177-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-177-h",
        "policy_hash": "policy-177-h",
        "diff_payload_hash": "diff-177-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-177-001",
        "trace_id": "trace-177-001",
        "request_id": "req-177-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-177-h",
        "blueprint_hash": "blueprint-177-h",
        "replay_key": "replay-177-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-177-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The uwg.commit.committed span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-177: per ledger row, a CommitRequest that violates the canonical
