"""Proof-evidence test for 10C-REQ-185 (00B_L4_State_Archive_and_UWG).

Severity      : CRITICAL
Surface       : 00B_L4_State_Archive_and_UWG
Layer owner   : L4 state + L6 observability
Artifact      : CommitRequest
OTEL span     : uwg.commit.validated
Source        : best-practice gap analysis 2026-04-30 wave2 C2 context engineering :: C2-NEW-F session-to-persistent memory promotion

Canonical requirement (paraphrased): L4 state MUST implement a MemoryPromotionGate that evaluates session-scoped scratchpad entries and semantic cache items for promotion to the...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-185]: CommitRequest emitted without satisfying 'L4 state MUST implement a MemoryPromotionGate that evaluates session-s...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-185-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-185-h",
        "policy_hash": "policy-185-h",
        "diff_payload_hash": "diff-185-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-185-001",
        "trace_id": "trace-185-001",
        "request_id": "req-185-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-185-h",
        "blueprint_hash": "blueprint-185-h",
        "replay_key": "replay-185-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-185-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-185: per ledger row, a CommitRequest that violates the canonical
