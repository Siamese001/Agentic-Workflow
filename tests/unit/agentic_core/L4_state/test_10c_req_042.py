"""Proof-evidence test for 10C-REQ-042 (00B_L4_State_Archive_and_UWG).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [7] CANONICAL STORE

Canonical requirement (paraphrased): Canonical store MUST store: raw text and metadata alongside sparse index

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-042]: CommitRequest emitted without satisfying 'Canonical store MUST store: raw text and metadata alongside sparse ind...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-42-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-42-h",
        "policy_hash": "policy-42-h",
        "diff_payload_hash": "diff-42-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-042: per ledger row, a CommitRequest that violates the canonical
