"""Proof-evidence test for 10C-REQ-150 (00B_L4_State_Archive_and_UWG).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: CASE FILE COMPILATION 4

Canonical requirement (paraphrased): C6 case MUST: Generate unique incident ID time attach context logs package RCA packet

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-150]: CommitRequest emitted without satisfying 'C6 case MUST: Generate unique incident ID time attach context logs pac...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-150-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-150-h",
        "policy_hash": "policy-150-h",
        "diff_payload_hash": "diff-150-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-150-001",
        "trace_id": "trace-150-001",
        "request_id": "req-150-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-150-h",
        "blueprint_hash": "blueprint-150-h",
        "replay_key": "replay-150-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-150-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-150: per ledger row, a CommitRequest that violates the canonical
