"""Proof-evidence test for 10C-REQ-140 (00B_L4_State_Archive_and_UWG).

Severity      : CRITICAL
Surface       : 00B_L4_State_Archive_and_UWG
Layer owner   : L5/L3/L6
Artifact      : CommitRequest
OTEL span     : uwg.commit.validated
Source        : C3_Healing_Remediation_Escalation.md :: ZERO-LOSS FAILURE CONTAINMENT

Canonical requirement (paraphrased): C3 zero-loss MUST: Freeze immediately UWG locks pending diffs status suspended audit handoff L4 note L6 threshold tune

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-140]: CommitRequest emitted without satisfying 'C3 zero-loss MUST: Freeze immediately UWG locks pending diffs status s...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-140-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-140-h",
        "policy_hash": "policy-140-h",
        "diff_payload_hash": "diff-140-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-140-001",
        "trace_id": "trace-140-001",
        "request_id": "req-140-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-140-h",
        "blueprint_hash": "blueprint-140-h",
        "replay_key": "replay-140-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-140-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-140: per ledger row, a CommitRequest that violates the canonical
