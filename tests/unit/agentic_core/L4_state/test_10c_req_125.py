"""Proof-evidence test for 10C-REQ-125 (00B_L4_State_Archive_and_UWG).
...
Source        : C4_State_Sovereignty_Universal_Write_Governance.md :: CLAIM WRITE LOCK

Canonical requirement (paraphrased): C4 lock MUST: Prevent ghost writes overlapping mutations claim exclusive write-access to knowledge substrate

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-125]: CommitRequest emitted without satisfying 'C4 lock MUST: Prevent ghost writes overlapping mutations claim exclusi...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-125-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-125-h",
        "policy_hash": "policy-125-h",
        "diff_payload_hash": "diff-125-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-125-001",
        "trace_id": "trace-125-001",
        "request_id": "req-125-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-125-h",
        "blueprint_hash": "blueprint-125-h",
        "replay_key": "replay-125-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-125-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-125: per ledger row, a CommitRequest that violates the canonical
