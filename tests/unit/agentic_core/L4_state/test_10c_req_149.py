"""Proof-evidence test for 10C-REQ-149 (00B_L4_State_Archive_and_UWG).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: ARCHIVE FREEZE 1-3

Canonical requirement (paraphrased): C6 freeze MUST: Capture patron trails freeze active rules seal source state produce master archive payload

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-149]: CommitRequest emitted without satisfying 'C6 freeze MUST: Capture patron trails freeze active rules seal source ...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-149-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-149-h",
        "policy_hash": "policy-149-h",
        "diff_payload_hash": "diff-149-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-149-001",
        "trace_id": "trace-149-001",
        "request_id": "req-149-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-149-h",
        "blueprint_hash": "blueprint-149-h",
        "replay_key": "replay-149-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-149-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-149: per ledger row, a CommitRequest that violates the canonical
