"""Proof-evidence test for 10C-REQ-018 (00B_L4_State_Archive_and_UWG).
...
Source        : 00A_ingestion_embedding_implementation.md :: A7. VECTOR INDEX WRITE

Canonical requirement (paraphrased): Vector index write MUST store: UUID->vector in vector database with sidecar UUID->metadata_refs

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-018]: CommitRequest emitted without satisfying 'Vector index write MUST store: UUID->vector in vector database with si...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-18-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-18-h",
        "policy_hash": "policy-18-h",
        "diff_payload_hash": "diff-18-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-018: per ledger row, a CommitRequest that violates the canonical
