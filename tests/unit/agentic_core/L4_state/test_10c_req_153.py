"""Proof-evidence test for 10C-REQ-153 (00B_L4_State_Archive_and_UWG).

Severity      : CRITICAL
Surface       : 00B_L4_State_Archive_and_UWG
Layer owner   : L5/L4/UWG
Artifact      : CommitRequest
OTEL span     : uwg.commit.validated
Source        : C6_Evaluation_Learning_Promotion_System.md :: COMMANDANT GAUNTLET 7

Canonical requirement (paraphrased): C6 gauntlet MUST: Shadow replay regression checks safety validation SME sign-off promotion readiness sovereign approve/veto

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-153]: CommitRequest emitted without satisfying 'C6 gauntlet MUST: Shadow replay regression checks safety validation SM...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-153-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-153-h",
        "policy_hash": "policy-153-h",
        "diff_payload_hash": "diff-153-h",
        "serial_seqno": 1,
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-153-001",
        "trace_id": "trace-153-001",
        "request_id": "req-153-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-153-h",
        "blueprint_hash": "blueprint-153-h",
        "replay_key": "replay-153-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-153-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-153: per ledger row, a CommitRequest that violates the canonical
