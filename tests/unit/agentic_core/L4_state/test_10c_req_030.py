"""Proof-evidence test for 10C-REQ-030 (00B_L4_State_Archive_and_UWG).
...
Source        : 00C_index_materialization_runtime_handoff.md :: C2. CANONICAL METADATA STORE

Canonical requirement (paraphrased): Canonical metadata store MUST hold: ChunkManifest raw text + enriched json + hash ParentChild graph edges AccessCtrl ACL tenant freshness sc...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-030]: CommitRequest emitted without satisfying 'Canonical metadata store MUST hold: ChunkManifest raw text + enriched ...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-30-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-30-h",
        "policy_hash": "policy-30-h",
        "diff_payload_hash": "diff-30-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-030: per ledger row, a CommitRequest that violates the canonical
