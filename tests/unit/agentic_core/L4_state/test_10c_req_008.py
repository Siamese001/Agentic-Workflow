"""Proof-evidence test for 10C-REQ-008 (00B_L4_State_Archive_and_UWG).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.7] MULTI-VECTOR MASTER ARCHIVES

Canonical requirement (paraphrased): Ingestion MUST publish five storage surfaces: dense search index sparse keyword index canonical raw store parent-child index unified metadat...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-008]: CommitRequest emitted without satisfying 'Ingestion MUST publish five storage surfaces: dense search index spars...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-8-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-8-h",
        "policy_hash": "policy-8-h",
        "diff_payload_hash": "diff-8-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-008: per ledger row, a CommitRequest that violates the canonical
