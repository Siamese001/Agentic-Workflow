"""Proof-evidence test for 10C-REQ-123 (00B_L4_State_Archive_and_UWG).
...
Source        : C4_State_Sovereignty_Universal_Write_Governance.md :: VERIFY THE BOSS

Canonical requirement (paraphrased): C4 verify MUST: Validate signature compliance_hash policy_hash check capability tokens for write authorization

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-123]: CommitRequest emitted without satisfying 'C4 verify MUST: Validate signature compliance_hash policy_hash check c...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-123-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-123-h",
        "policy_hash": "policy-123-h",
        "diff_payload_hash": "diff-123-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-123: per ledger row, a CommitRequest that violates the canonical
