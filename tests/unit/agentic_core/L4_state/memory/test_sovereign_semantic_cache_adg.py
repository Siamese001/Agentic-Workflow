...
@pytest.mark.skipif(not _AVAILABLE, reason="sovereign_semantic_cache.py deps unavailable")
class TestSovereignSemanticCacheImportability:
    def test_module_importable(self) -> None:
        """ADG contract: sovereign_semantic_cache.py must be importable."""
        assert _AVAILABLE

    def test_sovereignsemanticcache_is_type(self) -> None:
        assert SovereignSemanticCache is not None

    def test_get_redis_client_callable(self) -> None:
