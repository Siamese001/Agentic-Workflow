"""Proof-evidence test for 10C-REQ-010 (00B_L4_State_Archive_and_UWG).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.9] PUBLISH RETRIEVAL ASSETS

Canonical requirement (paraphrased): Ingestion MUST publish retrieval assets: raw_text_vector contextual_text_vector sparse keyword surfaces canonical raw chunks parent-child li...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-010]: CommitRequest emitted without satisfying 'Ingestion MUST publish retrieval assets: raw_text_vector contextual_te...
...
def _valid_artifact() -> dict:
    return {
        "commit_request_id": "cr-10-001",
        "writer_identity": "uwg-clerk",
        "blueprint_hash": "blueprint-10-h",
        "policy_hash": "policy-10-h",
        "diff_payload_hash": "diff-10-h",
        "serial_seqno": 1,
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-010: per ledger row, a CommitRequest that violates the canonical
