"""Enhanced behavioral tests for agentic_core.base_agents.L5SafetyBase."""
...
def test_l5safetybase_is_instantiable():
    """L5SafetyBase can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
