"""Enhanced behavioral tests for agentic_core.base_agents.L1CognitionBase."""
...
def test_l1cognitionbase_is_instantiable():
    """L1CognitionBase can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
