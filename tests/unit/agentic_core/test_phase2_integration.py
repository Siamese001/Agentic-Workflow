...
class TestGatewayExecuteEndToEnd:
    ...
    def test_gateway_accepts_manifest_without_hash_fields(self):
        ...
        result = gw.execute(
            manifest, _noop_heal, _noop_state_hash, trace_id="e2e-legacy", agent_id="CodeHealerAgent"
        )
    ...
    def test_gateway_accepts_manifest_with_correct_hashes(self):
        ...
        result = gw.execute(
            manifest, _noop_heal, _noop_state_hash, trace_id="e2e-correct", agent_id="CodeHealerAgent"
        )
        assert result.success is True
