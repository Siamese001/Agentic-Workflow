...
def test_mirror_structure_exactness():
    ...
    critical_pairs = [
        # L5 Safety
        (
            "agentic_core/L5_safety/guardrails.py",
            "tests/unit/agentic_core/L5_safety/test_guardrails.py",
        ),
        # L0 Maintenance
        (
            "agentic_core/L0_maintenance/arch_guard.py",
            "tests/unit/agentic_core/L0_maintenance/test_arch_guard.py",
        ),
        # L6 Observability (Explicit check)
        (
            "agentic_core/L6_observability/trace.py",
            "tests/unit/agentic_core/L6_observability/test_trace.py",
        ),
    ]
    ...
    for src_rel, test_rel in critical_pairs:
        ...
        if src.exists():
            if not expected_test.exists():
                ...
                failures.append(
                    f"Broken Mirror: {src.name} -> Found at {loc}, Expected {expected_test}"
                )

    assert not failures, "Mirror Parity Failed:\n" + "\n".join(failures)
