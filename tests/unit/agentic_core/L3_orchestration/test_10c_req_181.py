"""Proof-evidence test for 10C-REQ-181 (03_L3_Orchestration).

Severity      : CRITICAL
Surface       : 03_L3_Orchestration
Layer owner   : L3 orchestration + L4 state
Artifact      : WorkflowContract
OTEL span     : l3.workflow.contract_emitted
Source        : best-practice gap analysis 2026-04-30 wave2 C2 context engineering :: C2-NEW-B structured agent scratchpad

Canonical requirement (paraphrased): L3 orchestration loop MUST provide a typed AgentScratchpad persisted outside the context window. The scratchpad MUST support keyed read/writ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-181]: WorkflowContract emitted without satisfying 'L3 orchestration loop MUST provide a typed AgentScratchpad persiste...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-181-h",
        "blueprint_hash": "blueprint-181-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-181-001",
        "trace_id": "trace-181-001",
        "request_id": "req-181-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-181-h",
        "blueprint_hash": "blueprint-181-h",
        "replay_key": "replay-181-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-181-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-181: per ledger row, a WorkflowContract that violates the canonical
