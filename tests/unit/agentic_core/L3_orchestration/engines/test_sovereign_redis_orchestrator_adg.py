...
@pytest.mark.skipif(not _AVAILABLE, reason="sovereign_redis_orchestrator.py deps unavailable")
class TestSovereignRedisOrchestratorImportability:
    def test_module_importable(self) -> None:
        """ADG contract: sovereign_redis_orchestrator.py must be importable."""
        assert _AVAILABLE

    def test_sovereignredisorchestrator_is_type(self) -> None:
        assert SovereignRedisOrchestrator is not None

    def test_get_sovereign_redis_orchestrator_callable(self) -> None:
        assert callable(get_sovereign_redis_orchestrator)
