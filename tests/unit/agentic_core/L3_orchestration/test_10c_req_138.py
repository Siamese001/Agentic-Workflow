"""Proof-evidence test for 10C-REQ-138 (03_L3_Orchestration).
...
Source        : C3_Healing_Remediation_Escalation.md :: SOVEREIGN GATEWAY

Canonical requirement (paraphrased): C3 sovereign MUST: Perform provider-only operations mandatory invocation record sealed repair artifact

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-138]: WorkflowContract emitted without satisfying 'C3 sovereign MUST: Perform provider-only operations mandatory invoc...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-138-h",
        "blueprint_hash": "blueprint-138-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-138-001",
        "trace_id": "trace-138-001",
        "request_id": "req-138-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-138-h",
        "blueprint_hash": "blueprint-138-h",
        "replay_key": "replay-138-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-138-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-138: per ledger row, a WorkflowContract that violates the canonical
