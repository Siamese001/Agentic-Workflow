"""Proof-evidence test for 10C-REQ-136 (03_L3_Orchestration).
...
Source        : C3_Healing_Remediation_Escalation.md :: LOCAL HEAL FIRST

Canonical requirement (paraphrased): C3 MUST: Attempt deterministic rule fix schema repair known type casting flag for LLM path if exceeds local rules

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-136]: WorkflowContract emitted without satisfying 'C3 MUST: Attempt deterministic rule fix schema repair known type ca...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-136-h",
        "blueprint_hash": "blueprint-136-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-136-001",
        "trace_id": "trace-136-001",
        "request_id": "req-136-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-136-h",
        "blueprint_hash": "blueprint-136-h",
        "replay_key": "replay-136-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-136-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-136: per ledger row, a WorkflowContract that violates the canonical
