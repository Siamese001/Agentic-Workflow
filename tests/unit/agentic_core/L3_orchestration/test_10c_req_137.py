"""Proof-evidence test for 10C-REQ-137 (03_L3_Orchestration).
...
Source        : C3_Healing_Remediation_Escalation.md :: SCORE HEAL CONFIDENCE

Canonical requirement (paraphrased): C3 MUST: Score heal confidence High->Local Agent Medium->Qwen_vLLM Low->Gemini_2.5_Pro

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-137]: WorkflowContract emitted without satisfying 'C3 MUST: Score heal confidence High->Local Agent Medium->Qwen_vLLM ...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-137-h",
        "blueprint_hash": "blueprint-137-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-137-001",
        "trace_id": "trace-137-001",
        "request_id": "req-137-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-137-h",
        "blueprint_hash": "blueprint-137-h",
        "replay_key": "replay-137-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-137-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-137: per ledger row, a WorkflowContract that violates the canonical
