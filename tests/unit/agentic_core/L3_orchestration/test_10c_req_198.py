"""Proof-evidence test for 10C-REQ-198 (03_L3_Orchestration).

Severity      : CRITICAL
Surface       : 03_L3_Orchestration
Layer owner   : L3 orchestration + L6 observability
Artifact      : WorkflowContract
OTEL span     : l3.workflow.contract_emitted
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C9-NEW-A workflow pattern selection rationale logging

Canonical requirement (paraphrased): Every L3 run MUST log a WorkflowPatternSelectionReceipt declaring which pattern was chosen from {prompt_chain, router, parallelization, orch...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-198]: WorkflowContract emitted without satisfying 'Every L3 run MUST log a WorkflowPatternSelectionReceipt declaring w...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-198-h",
        "blueprint_hash": "blueprint-198-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-198-001",
        "trace_id": "trace-198-001",
        "request_id": "req-198-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-198-h",
        "blueprint_hash": "blueprint-198-h",
        "replay_key": "replay-198-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-198-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-198: per ledger row, a WorkflowContract that violates the canonical
