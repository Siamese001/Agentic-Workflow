"""Proof-evidence test for 10C-REQ-189 (03_L3_Orchestration).
...
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C3-NEW-B sub-agent return summarization contract

Canonical requirement (paraphrased): Sub-agent results returned to the parent agent MUST conform to a SubAgentResultEnvelope: {outcome_status, summary_text (bounded to max_retur...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-189]: WorkflowContract emitted without satisfying 'Sub-agent results returned to the parent agent MUST conform to a Su...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-189-h",
        "blueprint_hash": "blueprint-189-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-189-001",
        "trace_id": "trace-189-001",
        "request_id": "req-189-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-189-h",
        "blueprint_hash": "blueprint-189-h",
        "replay_key": "replay-189-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-189-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-189: per ledger row, a WorkflowContract that violates the canonical
