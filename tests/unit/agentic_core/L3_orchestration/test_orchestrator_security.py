...
class TestOrchestratorSecurity(unittest.TestCase):
    ...
    def test_factory_resolves_symlinks(self):
        ...
        self.assertTrue(agent_default.project_root.is_absolute(), "Default project root must be absolute")
        self.assertEqual(agent_default.project_root, self.root, "Default project root must be resolved")
    ...
    def test_module_whitelist_blocks_system_imports(self):
        ...
        for malicious_module in malicious_modules:
            with self.subTest(module=malicious_module):
                ...
                self.assertFalse(is_allowed, f"Whitelist FAILED: System module '{malicious_module}' was allowed!")
    ...
    def test_module_whitelist_allows_valid_core(self):
        ...
        for valid_module in valid_modules:
            with self.subTest(module=valid_module):
                ...
                self.assertTrue(is_allowed, f"Whitelist FAILED: Valid module '{valid_module}' was blocked!")
    ...
    def test_module_whitelist_blocks_edge_cases(self):
        ...
        for suspicious_module in suspicious_modules:
            with self.subTest(module=suspicious_module):
                ...
                self.assertFalse(is_allowed, f"Security FAILED: Suspicious module '{suspicious_module}' was allowed!")
    ...
    def test_project_root_hardening(self):
        ...
        self.assertTrue(agent.project_root.is_absolute(), "Direct instantiation must have absolute project_root")
        self.assertEqual(agent.project_root, self.root, "Direct instantiation must resolve to current directory")
    ...
    def test_import_validation_security_integration(self):
        ...
        with patch('agentic_core.L3_orchestration.OrchestratorAgent.get_agent_paths') as mock_get_paths:
    ...
    def test_whitelist_configuration_integrity(self):
        ...
        self.assertIsInstance(ALLOWED_MODULE_PREFIXES, tuple, "ALLOWED_MODULE_PREFIXES must be a tuple for security")
        
        # Check that all required prefixes are present
        required_prefixes = ["agentic_core", "apps_shared", "apps_lic", "apps_rg"]
        for prefix in required_prefixes:
            self.assertIn(prefix, ALLOWED_MODULE_PREFIXES, f"Required prefix '{prefix}' missing from whitelist")
        ...
        for prefix in ALLOWED_MODULE_PREFIXES:
            self.assertTrue(prefix.strip(), f"Whitelist prefix '{prefix}' appears to be whitespace-only")
