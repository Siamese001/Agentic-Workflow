...
@pytest.mark.skipif(not _AVAILABLE, reason="tool_call_store.py deps unavailable")
class TestToolCallStoreImportability:
    def test_module_importable(self) -> None:
        """ADG contract: tool_call_store.py must be importable."""
        assert _AVAILABLE

    def test_toolcallstore_is_type(self) -> None:
        assert ToolCallStore is not None

    def test_get_tool_call_store_callable(self) -> None:
        assert callable(get_tool_call_store)

    def test_record_tool_call_callable(self) -> None:
