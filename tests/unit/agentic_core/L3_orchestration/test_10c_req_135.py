"""Proof-evidence test for 10C-REQ-135 (03_L3_Orchestration).

Severity      : CRITICAL
Surface       : 03_L3_Orchestration
Layer owner   : L3 healing
Artifact      : WorkflowContract
OTEL span     : l3.workflow.contract_emitted
Source        : C3_Healing_Remediation_Escalation.md :: FAILURE SIGNAL

Canonical requirement (paraphrased): C3 MUST: Build failure signal from context only no external hallucinated state metadata check_id retry_count error_code lineage_hash

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-135]: WorkflowContract emitted without satisfying 'C3 MUST: Build failure signal from context only no external halluci...
...
def _valid_artifact() -> dict:
    return {
        ...
        "policy_hash": "policy-135-h",
        "blueprint_hash": "blueprint-135-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-135-001",
        "trace_id": "trace-135-001",
        "request_id": "req-135-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-135-h",
        "blueprint_hash": "blueprint-135-h",
        "replay_key": "replay-135-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-135-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-135: per ledger row, a WorkflowContract that violates the canonical
