"""L7_AUDITABILITY emit verification for integrated_r4_lic_pipeline_run.

Tests that the R4 lic entrypoint produces the four canonical L7 artifacts:
...
def _minimal_raw_request() -> dict:
    """Return a minimal valid raw request for R4 lic pipeline."""
    return {
        "request_id": "test-req-lic-001",
        "trace_id": "test-trace-lic-001",
        "recipient_class": "hiring_manager",
        "channel": "email",
        "outreach_mode": "warm_intro",
    }
...
class TestR4LicL7Emit:
    """Verify L7_AUDITABILITY artifacts are emitted by R4 lic pipeline."""
    ...
    def test_l7_artifacts_emitted(self, temp_artifact_dir: Path):
        ...
        with patch(
            ...
            result = run_integrated_r4_lic_pipeline(
