"""ADG importability contract for agentic_core/runtime/types/circuit_breaker_types.py.

Auto-generated stub — covers GT_covers edge for ADG reachability.
Behavioral tests belong in test_circuit_breaker_types.py (no _adg suffix).
"""
from __future__ import annotations

import pytest

try:
    from agentic_core.runtime.types.circuit_breaker_types import (  # noqa: F401
        BUFFER_SIZE,
        DEFAULT_SLEEP,
        MAX_RETRIES,
        THRESHOLD,
        CircuitBreakerOpenError,
        CircuitBreakerState,
        get_breaker,
    )
    _AVAILABLE = True
except ImportError:
    _AVAILABLE = False
    CircuitBreakerState = None  # type: ignore[assignment,misc]
    CircuitBreakerOpenError = None  # type: ignore[assignment,misc]
    get_breaker = None  # type: ignore[assignment,misc]
    MAX_RETRIES = None  # type: ignore[assignment,misc]
    DEFAULT_SLEEP = None  # type: ignore[assignment,misc]
    THRESHOLD = None  # type: ignore[assignment,misc]
    BUFFER_SIZE = None  # type: ignore[assignment,misc]

@pytest.mark.skipif(not _AVAILABLE, reason="circuit_breaker_types.py deps unavailable")
class TestCircuitBreakerTypesImportability:
    def test_module_importable(self) -> None:
        """ADG contract: circuit_breaker_types.py must be importable."""
        assert _AVAILABLE

    def test_circuitbreakerstate_is_type(self) -> None:
        assert CircuitBreakerState is not None

    def test_circuitbreakeropenerror_is_type(self) -> None:
        assert CircuitBreakerOpenError is not None

    def test_get_breaker_callable(self) -> None:
        assert callable(get_breaker)

    def test_max_retries_defined(self) -> None:
        assert MAX_RETRIES is not None

    def test_default_sleep_defined(self) -> None:
        assert DEFAULT_SLEEP is not None
