...
""")
    ...
    assert new_checksum != SEALED_CHECKSUM, \
        "Adding unauthorized agent should change checksum"
...
def test_clean_boot_execution(sealed_manifest_environment):
    ...
    with patch('agentic_core.L0_maintenance.boot.boot_sequence.AgentRegistry') as mock_registry:
        with patch('agentic_core.L0_maintenance.boot.boot_sequence.check_compliance') as mock_compliance:
            # Setup mocks
            mock_instance = Mock()
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('agentic_core.L0_maintenance.boot.boot_sequence.AgentRegistry') as mock_registry:
            with patch('agentic_core.L0_maintenance.boot.boot_sequence.check_compliance') as mock_compliance:
                ...
                mock_instance = Mock()
                mock_instance.discover_all.return_value = [Mock() for _ in range(1000)]  # 1000 agents
                mock_registry.return_value = mock_instance
                mock_compliance.return_value = []
