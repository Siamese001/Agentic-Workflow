...
class TestGlobalCanonKeyPurge:
    ...
    def test_territory_based_healing_intact(self):
        ...
        assert DEFAULT_CORE_HEALING_TERRITORY is not None, "DEFAULT_CORE_HEALING_TERRITORY missing"
        assert DEFAULT_APP_HEALING_TARGET is not None, "DEFAULT_APP_HEALING_TARGET missing"
        assert len(CORE_TERRITORY_KEYWORDS) > 0, "CORE_TERRITORY_KEYWORDS empty"
        assert len(SOVEREIGN_REGISTRY) > 0, "SOVEREIGN_REGISTRY empty"
        ...
        assert isinstance(AST_DOMAIN_HIT_THRESHOLD, float), "AST_DOMAIN_HIT_THRESHOLD must be float"
        assert isinstance(TERRITORY_MISMATCH_THRESHOLD, float), (
            "TERRITORY_MISMATCH_THRESHOLD must be float"
        )
        assert AST_DOMAIN_HIT_THRESHOLD > 0, "AST_DOMAIN_HIT_THRESHOLD must be positive"
        assert TERRITORY_MISMATCH_THRESHOLD > 0, "TERRITORY_MISMATCH_THRESHOLD must be positive"
    ...
    def test_void_compliance_refactored(self):
        ...
        with open(void_compliance_path) as f:
            content = f.read()
    ...
    def test_import_error_messages(self):
        ...
        try:
            ...
            assert any(
                term in error_str for term in ["canon_key", "not found", "attribute", "module"]
