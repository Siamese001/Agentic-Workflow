...
def test_sovereign_base_agent_inheritance():
    ...
    class ValidAgent:
        ...
        instance = Mock()
        class_ref = type(
            "ValidTestAgent", (SovereignBaseAgent,), {"__mro__": (str, SovereignBaseAgent, object)}
        )
...
def test_orphaned_agent_detection():
    ...
    class OrphanedAgent:
        ...
        layer = "L1_planning"
        instance = Mock()
        class_ref = type("OrphanedTestAgent", (object,), {"__mro__": (str, object)})
...
def test_unknown_layer_detection():
    ...
    class UnknownLayerAgent:
        ...
        class_ref = type(
            "UnknownLayerAgent",
            (SovereignBaseAgent,),
            {"__mro__": (str, SovereignBaseAgent, object)},
        )
...
def test_multiple_violations():
    ...
    class OrphanedAgent:
        ...
        layer = "unknown"
        instance = Mock()
        class_ref = type("OrphanedAgent", (object,), {"__mro__": (str, object)})
    ...
    class ValidAgent:
        ...
        instance = Mock()
        class_ref = type(
            "ValidAgent", (SovereignBaseAgent,), {"__mro__": (str, SovereignBaseAgent, object)}
        )
...
def test_boot_sequence_successful_boot():
    ...
    with patch.object(ManifestGuardian, "verify_integrity", return_value=True):
        with patch(
            "agentic_core.L0_maintenance.scripts.compliance_gate.check_compliance", return_value=[]
        ):
            boot = BootSequence(strict_mode=True)
            result = boot.execute_boot()
            ...
            assert len(result["compliance_violations"]) == 0
            assert all(
                phase in result["phases_completed"]
                for phase in [
                    "integrity_check",
                    "discovery",
                    "compliance",
