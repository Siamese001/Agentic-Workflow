...
class TestIntegrationParity(unittest.TestCase):
    ...
    def test_meta_learning_recent_experiences_limit(self):
        ...
        # Add 7 experiences
        for i in range(7):
            mgr.update_meta_learning({"experience": f"Experience {i}", "total_experiences": i})
    ...
    def test_l3_orchestrator_fallback_on_import_error(self):
        ...
        with patch.dict(
            sys.modules, {"agentic_core.L3_orchestration.ConsolidatedOrchestratorAgent": None}
        ):
    ...
    def test_l3_orchestrator_success_delegation(self):
        ...
        with patch.dict(sys.modules, modules):
            success, result = try_summon_orchestrator(
                self.project_root, ["target1", "target2"], True
            )
    ...
    def test_l3_orchestrator_exception_handling(self):
        ...
        mock_orch_module = MagicMock()
        mock_orch_module.get_consolidated_orchestrator.side_effect = Exception(
            "L3 initialization failed"
        )
    ...
    def test_meta_learning_strategy_weights_persistence(self):
        ...
        mgr.update_meta_learning({"strategy_weights": new_weights, "total_experiences": 10})
