...
class TestGlobalIntelligenceRigor:
    ...
    def test_safety_outranks_cognition(self):
        ...
        assert safety_w > cognition_w, (
            "FATAL: Cognitive reasoning signals are too heavy relative to safety."
        )
    ...
    def test_orchestration_outranks_execution(self):
        ...
        assert orch_w > exec_w, (
            "FATAL: Low-level tool logic is shadowing strategic workflow engines."
        )
    ...
    def test_prompt_utility_wins_over_generic_l0(self):
        ...
        pg_script_w = SOVEREIGN_TERRITORIES["agentic_core"]["ast_signals"][
            "agentic_core/prompt_governance/scripts"
        ]["weight"]
        # Generic maintenance baseline is 9
        assert pg_script_w > 9, (
            "CRITICAL: Specialized prompt utilities will be misrouted to L0 maintenance."
        )
    ...
    def test_state_integrity_priority(self):
        ...
        assert state_w > version_w, (
            "FAIL: State integrity contexts are losing priority to registry manifests."
        )
    ...
    def test_registry_manifest_dna_identification(self):
        ...
        assert "checksum_manifest" in signals["json_keys"], (
            "FAIL: DNA signal missing for registry artifacts."
        )
    ...
    def test_no_weight_collisions_in_conflict_zone(self):
        ...
        assert len(conflict_zone) == len(set(conflict_zone)), (
            f"COLLISION: Found identical weights: {conflict_zone}"
        )
