...
ADG import-hygiene is covered separately by test_canonical_serializer_util_adg.py.
...
@pytest.mark.skipif(not _AVAILABLE, reason="canonical_serializer_util.py deps unavailable")
...
class TestCanonicalHashFunction:
    ...
    def test_has_return_annotation(self):
        import inspect
        sig = inspect.signature(canonical_hash)
        assert sig.return_annotation is not inspect.Parameter.empty
