...
@pytest.mark.skipif(not _AVAILABLE, reason="evaluation_dataset_schema.py deps unavailable")
class TestEvaluationExample:
    def test_is_dataclass(self):
        import dataclasses
        assert dataclasses.is_dataclass(EvaluationExample)
    def test_importable(self):
        assert EvaluationExample is not None

@pytest.mark.skipif(not _AVAILABLE, reason="evaluation_dataset_schema.py deps unavailable")
class TestEvaluationDataset:
    def test_is_dataclass(self):
        import dataclasses
        assert dataclasses.is_dataclass(EvaluationDataset)
    def test_importable(self):
...
def test_module_importable():
    """Module evaluation_dataset_schema.py is importable (or deps unavailable)."""
