...
try:
    ...
    audit_registry_linkages = None  # type: ignore[assignment,misc]
...
@pytest.mark.skipif(not _AVAILABLE, reason="audit_registry_linkages.py deps unavailable")
class TestLoadRegistry:
    def test_is_callable(self):
        assert callable(load_registry)

@pytest.mark.skipif(not _AVAILABLE, reason="audit_registry_linkages.py deps unavailable")
class TestExtractSchemaFromTemplate:
    def test_is_callable(self):
        assert callable(extract_schema_from_template)

@pytest.mark.skipif(not _AVAILABLE, reason="audit_registry_linkages.py deps unavailable")
class TestAuditRegistryLinkages:
    def test_is_callable(self):
        assert callable(audit_registry_linkages)
...
def test_module_importable():
    """Module audit_registry_linkages.py is importable (or deps unavailable)."""
