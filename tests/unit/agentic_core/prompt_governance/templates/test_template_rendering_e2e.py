"""End-to-end tests: every Jinja template renders through SovereignPromptRenderer.

This file proves that templates are not just wired but actually *renderable*
with proper context dictionaries. It catches:
  - Missing variables (StrictUndefined failures)
  - Broken {% include %} chains
  - Jinja syntax errors
  - Template-not-found errors from wrong FileSystemLoader paths
  - Schema header parse failures
"""
...
TEMPLATES_DIR = PG_ROOT / "templates"
META_PROMPTS_DIR = PG_ROOT / "meta_prompts"
ADVERSARIAL_DIR = PG_ROOT / "security" / "adversarial"
...
def _subatomic_context() -> dict[str, Any]:
    """Full context dict for templates that {% include 'subatomic_healing_context.jinja' %}."""
    return {
        "behavioral_status": "partial",
        "canon_key": "18",
        "file_path": "agentic_core/L5_safety/reasoning/GravityLeakRepairAgent.py",
        "file_violations": 3,
        "healing_round": 5,
        "past_fixes": "renamed X -> y in round 3",
        "persistent_keys": ["13", "18", "19"],
        "primary_key": "18",
        "recently_converged": ["14", "15"],
        "surgery_flags": ["RUN_HIERARCHY_HEALING"],
        "task_violations": "- snake_case violation on line 42\n- missing type hint on line 88",
        "top_subatomic_fixes": ["naming", "type_hints", "docstrings"],
        "total_violations": 12,
        "code_block": "def MyFunction(x):\n    return x + 1",
    }


# ---------------------------------------------------------------------------
# E2E Rendering: INSTRUCTIONAL templates (templates/)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# E2E Rendering: META-PROMPT templates (meta_prompts/)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# E2E Rendering: ADVERSARIAL templates (security/adversarial/)
# ---------------------------------------------------------------------------
