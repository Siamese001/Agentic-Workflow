"""Proof-evidence test for 10C-REQ-109 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : 06_Shadow_Evaluation_System_Learning.md :: S2D HUMAN CALIBRATION

Canonical requirement (paraphrased): Shadow eval S2D MUST: Perform SME adjudication spot checks grader calibration to calibrate B/C/D evaluators

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-109]: L6EvalRecord emitted without satisfying 'Shadow eval S2D MUST: Perform SME adjudication spot checks grader calib...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-109-001",
        "trace_id": "trace-109-001",
        "request_id": "req-109-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-109-h",
        "blueprint_hash": "blueprint-109-h",
        "replay_key": "replay-109-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-109-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-109: per ledger row, a L6EvalRecord that violates the canonical
