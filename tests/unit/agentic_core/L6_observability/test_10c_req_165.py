"""Proof-evidence test for 10C-REQ-165 (Cross_Cutting_Observability_Replay_Audit).
...
Source        : Contextual Refinement in Transformer Models.md :: DIVERGENCE POINT 1: DIRECTIONALITY

Canonical requirement (paraphrased): Model distinction: Encoder-only bidirectional (retrieval) vs Decoder-only causal left-context (generation) are architecturally different and...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-165]: OTEL emitted without satisfying 'Model distinction: Encoder-only bidirectional (retrieval) vs Decoder-only causa...
...
def _valid_artifact() -> dict:
    return {
        "trace_id": "trace-165-001",
        "replay_key": "replay-165-k",
        "owner_surface": OWNER_SURFACE,
        "w3c_traceparent": "00-000000000000000000000000000000165-00000000000000165-01",
        "w3c_tracestate": "vendor=v1",
        "replay_key_audit_present": True,
        "policy_hash": "policy-165-h",
        "blueprint_hash": "blueprint-165-h",
    }
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-165: per ledger row, a OtelTraceAuditRecord that violates the canonical
