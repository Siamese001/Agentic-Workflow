"""Proof-evidence test for 10C-REQ-129 (06_L6_Observability_and_System_Learning).
...
Source        : C2_Observability_Telemetry_Control_Signals.md :: S1 TIME AUDIT

Canonical requirement (paraphrased): C2 S1 MUST: Verify stamps check order and latency detect clock drift

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-129]: L6EvalRecord emitted without satisfying 'C2 S1 MUST: Verify stamps check order and latency detect clock drift' -...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-129-001",
        "trace_id": "trace-129-001",
        "request_id": "req-129-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-129-h",
        "blueprint_hash": "blueprint-129-h",
        "replay_key": "replay-129-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-129-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-129: per ledger row, a L6EvalRecord that violates the canonical
