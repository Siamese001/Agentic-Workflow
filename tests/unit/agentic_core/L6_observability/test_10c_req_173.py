"""Proof-evidence test for 10C-REQ-173 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : agentic_process_mapping_v29.md :: [6] SHADOW EVALUATION

Canonical requirement (paraphrased): Shadow eval MUST: Perform async telemetry review RCA system evolution update policies catalogs through master clerk only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-173]: L6EvalRecord emitted without satisfying 'Shadow eval MUST: Perform async telemetry review RCA system evolution u...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-173-001",
        "trace_id": "trace-173-001",
        "request_id": "req-173-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-173-h",
        "blueprint_hash": "blueprint-173-h",
        "replay_key": "replay-173-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-173-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-173: per ledger row, a L6EvalRecord that violates the canonical
