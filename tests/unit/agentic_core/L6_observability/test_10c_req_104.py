"""Proof-evidence test for 10C-REQ-104 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : 06_Shadow_Evaluation_System_Learning.md :: S1A GATHER LOGS

Canonical requirement (paraphrased): Shadow eval S1A MUST: Gather telemetry intake exit outcomes trace capture

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-104]: L6EvalRecord emitted without satisfying 'Shadow eval S1A MUST: Gather telemetry intake exit outcomes trace captu...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-104-001",
        "trace_id": "trace-104-001",
        "request_id": "req-104-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-104-h",
        "blueprint_hash": "blueprint-104-h",
        "replay_key": "replay-104-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-104-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-104: per ledger row, a L6EvalRecord that violates the canonical
