"""Proof-evidence test for 10C-REQ-147 (06_L6_Observability_and_System_Learning).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: L6 ANALYSIS CORE

Canonical requirement (paraphrased): C6 L6 MUST: Analyze outcomes B trajectory C governance regressions D human calibration F

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-147]: L6EvalRecord emitted without satisfying 'C6 L6 MUST: Analyze outcomes B trajectory C governance regressions D hu...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-147-001",
        "trace_id": "trace-147-001",
        "request_id": "req-147-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-147-h",
        "blueprint_hash": "blueprint-147-h",
        "replay_key": "replay-147-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-147-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-147: per ledger row, a L6EvalRecord that violates the canonical
