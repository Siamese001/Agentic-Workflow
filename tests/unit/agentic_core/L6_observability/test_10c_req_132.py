"""Proof-evidence test for 10C-REQ-132 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : C2_Observability_Telemetry_Control_Signals.md :: S4 PACKET SEAL

Canonical requirement (paraphrased): C2 S4 MUST: Normalize metrics seal execution environment final provenance

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-132]: L6EvalRecord emitted without satisfying 'C2 S4 MUST: Normalize metrics seal execution environment final provenan...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-132-001",
        "trace_id": "trace-132-001",
        "request_id": "req-132-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-132-h",
        "blueprint_hash": "blueprint-132-h",
        "replay_key": "replay-132-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-132-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-132: per ledger row, a L6EvalRecord that violates the canonical
