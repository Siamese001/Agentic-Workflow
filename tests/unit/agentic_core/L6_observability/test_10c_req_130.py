...
Source        : C2_Observability_Telemetry_Control_Signals.md :: S2 ISOLATION CHECK

Canonical requirement (paraphrased): C2 S2 MUST: Verify seeds check isolation verify replay strictness

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-130]: L6EvalRecord emitted without satisfying 'C2 S2 MUST: Verify seeds check isolation verify replay strictness' -- m...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-130-001",
        "trace_id": "trace-130-001",
        "request_id": "req-130-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-130-h",
        "blueprint_hash": "blueprint-130-h",
        "replay_key": "replay-130-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-130-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-130: per ledger row, a L6EvalRecord that violates the canonical
