"""Proof-evidence test for 10C-REQ-118 (Cross_Cutting_Observability_Replay_Audit).

Severity      : CRITICAL
Surface       : Cross_Cutting_Observability_Replay_Audit
Layer owner   : L5/L3/L0/L2
Artifact      : OtelTraceAuditRecord
OTEL span     : obs.audit_event.emitted
Source        : C1_Deterministic_Replay_Execution_Integrity.md :: REPLAY MODE PROPAGATION

Canonical requirement (paraphrased): C1 MUST: Propagate replay mode inject Freeze signal across L0->L3->L5->L2 halt wall-clock updates ensure same snapshot seen by all component...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-118]: OTEL emitted without satisfying 'C1 MUST: Propagate replay mode inject Freeze signal across L0->L3->L5->L2 halt ...
...
def _valid_artifact() -> dict:
    return {
        "trace_id": "trace-118-001",
        "replay_key": "replay-118-k",
        "owner_surface": OWNER_SURFACE,
        "w3c_traceparent": "00-000000000000000000000000000000118-00000000000000118-01",
        "w3c_tracestate": "vendor=v1",
        "replay_key_audit_present": True,
        "policy_hash": "policy-118-h",
        "blueprint_hash": "blueprint-118-h",
    }
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-118: per ledger row, a OtelTraceAuditRecord that violates the canonical
