"""Proof-evidence test for 10C-REQ-148 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: E SIGNAL AGGREGATOR

Canonical requirement (paraphrased): C6 E MUST: Aggregate signals compile unified score bundle severity classification drift flags

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-148]: L6EvalRecord emitted without satisfying 'C6 E MUST: Aggregate signals compile unified score bundle severity clas...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-148-001",
        "trace_id": "trace-148-001",
        "request_id": "req-148-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-148-h",
        "blueprint_hash": "blueprint-148-h",
        "replay_key": "replay-148-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-148-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-148: per ledger row, a L6EvalRecord that violates the canonical
