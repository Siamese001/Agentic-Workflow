"""Proof-evidence test for 10C-REQ-106 (06_L6_Observability_and_System_Learning).
...
Source        : 06_Shadow_Evaluation_System_Learning.md :: S2A OUTCOME EVALS

Canonical requirement (paraphrased): Shadow eval S2A MUST: Evaluate task completion groundedness citation support abstain correctness escalation correctness answer relevance

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-106]: L6EvalRecord emitted without satisfying 'Shadow eval S2A MUST: Evaluate task completion groundedness citation su...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-106-001",
        "trace_id": "trace-106-001",
        "request_id": "req-106-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-106-h",
        "blueprint_hash": "blueprint-106-h",
        "replay_key": "replay-106-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-106-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-106: per ledger row, a L6EvalRecord that violates the canonical
