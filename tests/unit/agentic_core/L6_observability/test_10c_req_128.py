"""Proof-evidence test for 10C-REQ-128 (06_L6_Observability_and_System_Learning).
...
Source        : C2_Observability_Telemetry_Control_Signals.md :: L6 READ SURFACES

Canonical requirement (paraphrased): C2 L6 MUST: Read sealed execution trace exit dispositions L4 telemetry shelf historical baseline metrics prior run logs

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-128]: L6EvalRecord emitted without satisfying 'C2 L6 MUST: Read sealed execution trace exit dispositions L4 telemetry ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-128-001",
        "trace_id": "trace-128-001",
        "request_id": "req-128-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-128-h",
        "blueprint_hash": "blueprint-128-h",
        "replay_key": "replay-128-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-128-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-128: per ledger row, a L6EvalRecord that violates the canonical
