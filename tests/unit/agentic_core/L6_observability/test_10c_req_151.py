"""Proof-evidence test for 10C-REQ-151 (06_L6_Observability_and_System_Learning).
...
Source        : C6_Evaluation_Learning_Promotion_System.md :: INVESTIGATION 5

Canonical requirement (paraphrased): C6 investigate MUST: Map failure flow classify root cause separate weak signal from stable trend

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-151]: L6EvalRecord emitted without satisfying 'C6 investigate MUST: Map failure flow classify root cause separate weak...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-151-001",
        "trace_id": "trace-151-001",
        "request_id": "req-151-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-151-h",
        "blueprint_hash": "blueprint-151-h",
        "replay_key": "replay-151-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-151-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-151: per ledger row, a L6EvalRecord that violates the canonical
