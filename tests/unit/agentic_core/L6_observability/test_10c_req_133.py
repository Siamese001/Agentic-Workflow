"""Proof-evidence test for 10C-REQ-133 (06_L6_Observability_and_System_Learning).
...
Source        : C2_Observability_Telemetry_Control_Signals.md :: BUS D BUS E Live Control

Canonical requirement (paraphrased): C2 MUST: Emit BUS D/E real-time signals Deny Re-enter Escalation triggers for HITL

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-133]: L6EvalRecord emitted without satisfying 'C2 MUST: Emit BUS D/E real-time signals Deny Re-enter Escalation trigge...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-133-001",
        "trace_id": "trace-133-001",
        "request_id": "req-133-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-133-h",
        "blueprint_hash": "blueprint-133-h",
        "replay_key": "replay-133-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-133-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-133: per ledger row, a L6EvalRecord that violates the canonical
