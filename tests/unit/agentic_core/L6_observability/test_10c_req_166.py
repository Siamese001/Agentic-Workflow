"""Proof-evidence test for 10C-REQ-166 (Cross_Cutting_Observability_Replay_Audit).

Severity      : CRITICAL
Surface       : Cross_Cutting_Observability_Replay_Audit
Layer owner   : architecture design
Artifact      : OtelTraceAuditRecord
OTEL span     : obs.audit_event.emitted
Source        : Contextual Refinement in Transformer Models.md :: DIVERGENCE POINT 2: THE CRUCIAL FORK

Canonical requirement (paraphrased): Divergence: Encoder-only produces pooled chunk vector for retrieval vs Decoder-only produces next-token probabilities for generation MUST NO...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-166]: OTEL emitted without satisfying 'Divergence: Encoder-only produces pooled chunk vector for retrieval vs Decoder-...
...
EXPECTED_SPAN = "obs.audit_event.emitted"
...
def _valid_artifact() -> dict:
    return {
        "trace_id": "trace-166-001",
        "replay_key": "replay-166-k",
        "owner_surface": OWNER_SURFACE,
        "w3c_traceparent": "00-000000000000000000000000000000166-00000000000000166-01",
        "w3c_tracestate": "vendor=v1",
        "replay_key_audit_present": True,
        "policy_hash": "policy-166-h",
        "blueprint_hash": "blueprint-166-h",
    }
...
def test_span_shape_positive() -> None:
    """The obs.audit_event.emitted span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-166: per ledger row, a OtelTraceAuditRecord that violates the canonical
