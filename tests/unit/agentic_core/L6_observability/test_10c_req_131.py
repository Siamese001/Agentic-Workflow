"""Proof-evidence test for 10C-REQ-131 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : C2_Observability_Telemetry_Control_Signals.md :: S3 DRIFT DETECTION

Canonical requirement (paraphrased): C2 S3 MUST: Detect budget usage thrash spikes unusual patterns

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-131]: L6EvalRecord emitted without satisfying 'C2 S3 MUST: Detect budget usage thrash spikes unusual patterns' -- must...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-131-001",
        "trace_id": "trace-131-001",
        "request_id": "req-131-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-131-h",
        "blueprint_hash": "blueprint-131-h",
        "replay_key": "replay-131-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-131-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-131: per ledger row, a L6EvalRecord that violates the canonical
