"""Proof-evidence test for 10C-REQ-200 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C10-NEW-B per-workflow cost attribution

Canonical requirement (paraphrased): Cost telemetry MUST be attributed at finer granularity than run-total: per {workflow_pattern, sub_agent_id, provider, tool_name}. Each LLM c...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-200]: L6EvalRecord emitted without satisfying 'Cost telemetry MUST be attributed at finer granularity than run-total: ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-200-001",
        "trace_id": "trace-200-001",
        "request_id": "req-200-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-200-h",
        "blueprint_hash": "blueprint-200-h",
        "replay_key": "replay-200-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-200-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-200: per ledger row, a L6EvalRecord that violates the canonical
