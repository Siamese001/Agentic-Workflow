"""Proof-evidence test for 10C-REQ-190 (06_L6_Observability_and_System_Learning).
...
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C5-NEW-A eval-set version pinning + drift gate

Canonical requirement (paraphrased): Every eval set (golden, adversarial, regression) MUST carry a content-addressed version_hash computed from sorted (prompt, expected, rubric)...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-190]: L6EvalRecord emitted without satisfying 'Every eval set (golden, adversarial, regression) MUST carry a content-a...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-190-001",
        "trace_id": "trace-190-001",
        "request_id": "req-190-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-190-h",
        "blueprint_hash": "blueprint-190-h",
        "replay_key": "replay-190-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-190-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-190: per ledger row, a L6EvalRecord that violates the canonical
