"""Proof-evidence test for 10C-REQ-107 (06_L6_Observability_and_System_Learning).
...
Source        : 06_Shadow_Evaluation_System_Learning.md :: S2B TRAJECTORY EVALS

Canonical requirement (paraphrased): Shadow eval S2B MUST: Evaluate tool selection/ordering arg correctness retry thrash budget policy compliance trajectory integrity

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-107]: L6EvalRecord emitted without satisfying 'Shadow eval S2B MUST: Evaluate tool selection/ordering arg correctness ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-107-001",
        "trace_id": "trace-107-001",
        "request_id": "req-107-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-107-h",
        "blueprint_hash": "blueprint-107-h",
        "replay_key": "replay-107-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-107-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-107: per ledger row, a L6EvalRecord that violates the canonical
