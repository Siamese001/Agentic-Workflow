"""Proof-evidence test for 10C-REQ-108 (06_L6_Observability_and_System_Learning).

Severity      : CRITICAL
Surface       : 06_L6_Observability_and_System_Learning
Layer owner   : L6 shadow eval
Artifact      : L6EvalRecord
OTEL span     : l6.learning.proposal_emitted
Source        : 06_Shadow_Evaluation_System_Learning.md :: S2C GOVERNANCE REGRESSIONS

Canonical requirement (paraphrased): Shadow eval S2C MUST: Detect exact match drift schema/state drift API drift rubric/grader drift gate regression

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-108]: L6EvalRecord emitted without satisfying 'Shadow eval S2C MUST: Detect exact match drift schema/state drift API d...
...
EXPECTED_SPAN = "l6.learning.proposal_emitted"
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-108-001",
        "trace_id": "trace-108-001",
        "request_id": "req-108-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-108-h",
        "blueprint_hash": "blueprint-108-h",
        "replay_key": "replay-108-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-108-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The l6.learning.proposal_emitted span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-108: per ledger row, a L6EvalRecord that violates the canonical
