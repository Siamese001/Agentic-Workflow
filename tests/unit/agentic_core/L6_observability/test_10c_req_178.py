"""Proof-evidence test for 10C-REQ-178 (06_L6_Observability_and_System_Learning).
...
Source        : agentic_process_mapping_v38.md + best-practice gap analysis 2026-04-29 :: HITL-NEW-E human-on-the-loop parallel surface (advisory only)

Canonical requirement (paraphrased): A parallel human-on-the-loop surface MUST consume BUS D/E HITL trigger signals (REQ-133) and surface in-flight Tier-T1/T2 runs to a non-bloc...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-178]: L6EvalRecord emitted without satisfying 'A parallel human-on-the-loop surface MUST consume BUS D/E HITL trigger ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-178-001",
        "trace_id": "trace-178-001",
        "request_id": "req-178-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-178-h",
        "blueprint_hash": "blueprint-178-h",
        "replay_key": "replay-178-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-178-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-178: per ledger row, a L6EvalRecord that violates the canonical
