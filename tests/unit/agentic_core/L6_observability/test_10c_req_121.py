"""Proof-evidence test for 10C-REQ-121 (06_L6_Shadow_Evaluation_System_Learning).
...
Source        : C1_Deterministic_Replay_Execution_Integrity.md :: SEAL DETERMINISM DIGEST

Canonical requirement (paraphrased): C1 seal MUST: Produce exactly one stable proof W<n>-DETERMINISM-DIGEST invariant same input+envelope+clock+reads produces same digest

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-121]: L6EvalRecord emitted without satisfying 'C1 seal MUST: Produce exactly one stable proof W<n>-DETERMINISM-DIGEST ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-121-001",
        "trace_id": "trace-121-001",
        "request_id": "req-121-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-121-h",
        "blueprint_hash": "blueprint-121-h",
        "replay_key": "replay-121-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-121-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-121: per ledger row, a L6EvalRecord that violates the canonical
