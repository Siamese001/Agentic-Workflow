"""Proof-evidence test for 10C-REQ-194 (Cross_Cutting_Observability_Replay_Audit).

Severity      : CRITICAL
Surface       : Cross_Cutting_Observability_Replay_Audit
Layer owner   : L6 observability + all layers
Artifact      : OtelTraceAuditRecord
OTEL span     : obs.tracecontext.propagated
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C7-NEW-A W3C traceparent cross-system propagation

Canonical requirement (paraphrased): Every outbound call across a system boundary (MCP server, sub-agent dispatch, external HTTP, LLM provider invocation) MUST include the W3C T...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-194]: OTEL emitted without satisfying 'Every outbound call across a system boundary (MCP server, sub-agent dispatch, e...
...
from tests.fixtures.proof_evidence.runtime_artifact_validators import (
    ArtifactShapeError,
    assert_otel_replay_key_audit_present,
    validate_artifact_shape,
)

REQ_ID = "10C-REQ-194"
OWNER_SURFACE = "Cross_Cutting_Observability_Replay_Audit"
EXPECTED_SPAN = "obs.tracecontext.propagated"
ARTIFACT_TYPE = "OtelTraceAuditRecord"
...
EXTRA_SPAN_ATTRS = tuple(
    a for a in 'req_id|run_id|trace_id|request_id|owner_surface|policy_hash|blueprint_hash|replay_key|traceparent|tracestate'.split("|")
...
def _valid_artifact() -> dict:
    return {
        "trace_id": "trace-194-001",
        "replay_key": "replay-194-k",
        "owner_surface": OWNER_SURFACE,
        "w3c_traceparent": "00-000000000000000000000000000000194-00000000000000194-01",
        "w3c_tracestate": "vendor=v1",
        "replay_key_audit_present": True,
        "policy_hash": "policy-194-h",
        "blueprint_hash": "blueprint-194-h",
    }
...
def test_artifact_shape_positive() -> None:
    """A well-formed OtelTraceAuditRecord must validate cleanly + satisfy boundary invariant."""
    record = _valid_artifact()
    validate_artifact_shape(ARTIFACT_TYPE, record)
    assert_otel_replay_key_audit_present(record)


def test_span_shape_positive() -> None:
    """The obs.tracecontext.propagated span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-194: per ledger row, a OtelTraceAuditRecord that violates the canonical
    requirement MUST raise. Drift inducer: replay_key_audit_present.
    """
    record = _valid_artifact()
    record["replay_key_audit_present"] = False
    with pytest.raises(ArtifactShapeError):
        assert_otel_replay_key_audit_present(record)


def test_negative_control_missing_required_field() -> None:
    """A OtelTraceAuditRecord missing the required field 'trace_id' MUST fail shape validation."""
    record = _valid_artifact()
    del record['trace_id']
