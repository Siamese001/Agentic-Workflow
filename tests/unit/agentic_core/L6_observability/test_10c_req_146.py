"""Proof-evidence test for 10C-REQ-146 (06_L6_Observability_and_System_Learning).

Severity      : CRITICAL
Surface       : 06_L6_Observability_and_System_Learning
Layer owner   : L6 evaluation
Artifact      : L6EvalRecord
OTEL span     : l6.eval.record_sealed
Source        : C6_Evaluation_Learning_Promotion_System.md :: PHASE 1 CURRENT RUN EXIT

Canonical requirement (paraphrased): C6 phase 1 MUST: Live exit review validate env integrity schema compliance mutation policy replay completeness real ink write requests

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-146]: L6EvalRecord emitted without satisfying 'C6 phase 1 MUST: Live exit review validate env integrity schema complia...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-146-001",
        "trace_id": "trace-146-001",
        "request_id": "req-146-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-146-h",
        "blueprint_hash": "blueprint-146-h",
        "replay_key": "replay-146-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-146-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-146: per ledger row, a L6EvalRecord that violates the canonical
