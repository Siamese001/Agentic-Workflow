"""Proof-evidence test for 10C-REQ-105 (06_L6_Observability_and_System_Learning).
...
Source        : 06_Shadow_Evaluation_System_Learning.md :: S1B NORMALIZE EVIDENCE

Canonical requirement (paraphrased): Shadow eval S1B MUST: Align formats preserve lineage correlate dispositions

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-105]: L6EvalRecord emitted without satisfying 'Shadow eval S1B MUST: Align formats preserve lineage correlate disposit...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-105-001",
        "trace_id": "trace-105-001",
        "request_id": "req-105-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-105-h",
        "blueprint_hash": "blueprint-105-h",
        "replay_key": "replay-105-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-105-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-105: per ledger row, a L6EvalRecord that violates the canonical
