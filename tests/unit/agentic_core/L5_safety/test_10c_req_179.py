"""Proof-evidence test for 10C-REQ-179 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : HITL L5 + L6 shadow eval
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : agentic_process_mapping_v38.md + best-practice gap analysis 2026-04-29 :: HITL-NEW-F HITLDecisionReceipt for closed-loop calibration

Canonical requirement (paraphrased): Every HITL cycle MUST emit a HITLDecisionReceipt artifact containing: decision_id, run_id, risk_tier, reviewer_id, verdict in {APPROVE, MODI...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-179]: L5CertificationResult emitted without satisfying 'Every HITL cycle MUST emit a HITLDecisionReceipt artifact cont...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-179-001",
        "certification_class": "policy",
        "policy_hash": "policy-179-h",
        "blueprint_hash": "blueprint-179-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-179-001",
        "trace_id": "trace-179-001",
        "request_id": "req-179-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-179-h",
        "blueprint_hash": "blueprint-179-h",
        "replay_key": "replay-179-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-179-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-179: per ledger row, a L5CertificationResult that violates the canonical
