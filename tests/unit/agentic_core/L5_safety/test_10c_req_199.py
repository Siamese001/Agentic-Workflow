"""Proof-evidence test for 10C-REQ-199 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L5 safety + L3 orchestration
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C10-NEW-A per-request cost ceiling with circuit breaker

Canonical requirement (paraphrased): Each ingressed request MUST carry a per_request_cost_ceiling_usd derived from (env_class, risk_tier, customer_tier). Cost is accumulated acr...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-199]: L5CertificationResult emitted without satisfying 'Each ingressed request MUST carry a per_request_cost_ceiling_u...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-199-001",
        "certification_class": "policy",
        "policy_hash": "policy-199-h",
        "blueprint_hash": "blueprint-199-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-199-001",
        "trace_id": "trace-199-001",
        "request_id": "req-199-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-199-h",
        "blueprint_hash": "blueprint-199-h",
        "replay_key": "replay-199-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-199-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-199: per ledger row, a L5CertificationResult that violates the canonical
