"""Proof-evidence test for 10C-REQ-158 (00A_L5_Governance_Safety).
...
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G4 BUILD ACCESS TICKET

Canonical requirement (paraphrased): C7 G4 MUST: Generate capability_token sandbox_envelope with bounded scope expiration timeout parameters

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-158]: L5CertificationResult emitted without satisfying 'C7 G4 MUST: Generate capability_token sandbox_envelope with bo...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-158-001",
        "certification_class": "policy",
        "policy_hash": "policy-158-h",
        "blueprint_hash": "blueprint-158-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-158-001",
        "trace_id": "trace-158-001",
        "request_id": "req-158-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-158-h",
        "blueprint_hash": "blueprint-158-h",
        "replay_key": "replay-158-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-158-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-158: per ledger row, a L5CertificationResult that violates the canonical
