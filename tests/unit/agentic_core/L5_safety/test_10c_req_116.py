"""Proof-evidence test for 10C-REQ-116 (00A_L5_Governance_Safety).
...
Source        : C0_Governance_Safety_Enforcement.md :: G7 SOVEREIGN EGRESS

Canonical requirement (paraphrased): Governance G7 MUST: Perform symbolic to provider mapping audit prompt injection detection replay sealing with fail-closed only policy

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-116]: L5CertificationResult emitted without satisfying 'Governance G7 MUST: Perform symbolic to provider mapping audit...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-116-001",
        "certification_class": "policy",
        "policy_hash": "policy-116-h",
        "blueprint_hash": "blueprint-116-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-116-001",
        "trace_id": "trace-116-001",
        "request_id": "req-116-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-116-h",
        "blueprint_hash": "blueprint-116-h",
        "replay_key": "replay-116-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-116-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-116: per ledger row, a L5CertificationResult that violates the canonical
