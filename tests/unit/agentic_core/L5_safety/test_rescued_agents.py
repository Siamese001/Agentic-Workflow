...
def test_schema_evolver():
    ...
    schema_file = (
        PROJECT_ROOT / "agentic_core" / "L4_state" / "ValidationContext" / "SchemaEvolverAgent.py"
    )
    ...
    if "_discover_schemas()" in source or "_discover_schemas(" in source:
        ...
        if heal_match and "_discover_schemas" in heal_match.group(1):
            test_pass(
                "SCH-02", "Registry population - _discover_schemas() is wired in heal_repository"
            )
        else:
    ...
    if "generate_drift_report" in source:
        ...
        if heal_match and "generate_drift_report" in heal_match.group(1):
            test_pass(
                "SCH-03", "Drift report - generate_drift_report() is wired in heal_repository"
            )
        else:
...
def test_predictive_cost_auditor():
    ...
    if heal_is_method:
        test_pass(
            "COST-02", "Class integration - heal_repository is an instance method with 'self'"
        )
    else:
    ...
    if heal_match:
        ...
        if has_audit and has_report:
            test_pass(
                "COST-03", "Report output - _audit_files() and _generate_cost_report() are wired"
            )
        else:
            missing = []
            if not has_audit:
...
def test_deadlock_detector():
    ...
    has_timeout = (
        "from agentic_core.L5_safety.validators.decorators import" in source and "timeout" in source
    )
    ...
    if heal_match:
        ...
        if has_stale_check and has_violations:
            test_pass(
                "DEAD-02",
                "Stale detection - checks monitored_tasks for TIMEOUT status and counts violations",
            )
        else:
...
def test_secure_checkpoint_manager():
    ...
    heal_match = re.search(
        r"class SecureCheckpointManagerAgent.*?def heal_repository\(.*?\n(.*?)(?=\n    def |\nclass |\Z)",
        source,
        re.DOTALL,
    )
    ...
    if heal_is_method and has_cleanup_wired:
        test_pass(
            "CHK-02",
            "Signature check - heal_repository is instance method with cleanup_old_checkpoints wired",
        )
    else:
        issues = []
...
def test_scripts_planning_orchestrator():
    ...
    if heal_match:
        ...
        if "_validate_tasks" in heal_body and "diagnostic_task" in heal_body:
            test_pass(
                "PLAN-01", "Health check - _validate_tasks diagnostic is wired in heal_repository"
            )
        else:
    ...
    if "def heal_repository(" in source and "dry_run: bool" in source and "execute: bool" in source:
        test_pass(
            "PLAN-04", "Discovery - heal_repository has proper signature with dry_run/execute"
        )
    else:
...
def test_dag_mutator():
    ...
    if heal_match:
        ...
        if "_validate_mutation" in heal_body and "test_graph" in heal_body:
            test_pass(
                "DAG-01", "Diagnostic wiring - _validate_mutation is wired in heal_repository"
            )
        else:
...
def test_peer_intelligence_auditor():
    ...
    if heal_match:
        ...
        if "_validate_search_count" in heal_body and "dummy_hops" in heal_body:
            test_pass(
                "PEER-01", "Diagnostic wiring - _validate_search_count is wired in heal_repository"
            )
        else:
    ...
    if "def heal_repository(" in source and "dry_run: bool" in source and "execute: bool" in source:
        test_pass(
            "PEER-04", "Discovery - heal_repository has proper signature with dry_run/execute"
        )
    else:
...
def test_import_healer():
    ...
    if heal_match:
        ...
        if "heal_all_imports_in_directory" in heal_body:
            test_pass(
                "IMP-04", "Discovery - heal_all_imports_in_directory is wired in heal_repository"
            )
        else:
