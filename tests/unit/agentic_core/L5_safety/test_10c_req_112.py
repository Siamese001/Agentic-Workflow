"""Proof-evidence test for 10C-REQ-112 (00A_L5_Governance_Safety).
...
Source        : C0_Governance_Safety_Enforcement.md :: G3 STRUCTURE CHECK

Canonical requirement (paraphrased): Governance G3 MUST: Perform layer isolation depth verification placement connectivity audit

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-112]: L5CertificationResult emitted without satisfying 'Governance G3 MUST: Perform layer isolation depth verification...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-112-001",
        "certification_class": "policy",
        "policy_hash": "policy-112-h",
        "blueprint_hash": "blueprint-112-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-112-001",
        "trace_id": "trace-112-001",
        "request_id": "req-112-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-112-h",
        "blueprint_hash": "blueprint-112-h",
        "replay_key": "replay-112-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-112-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-112: per ledger row, a L5CertificationResult that violates the canonical
