"""Proof-evidence test for 10C-REQ-155 (00A_L5_Governance_Safety).
...
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G1 WHAT KIND OF POWER?

Canonical requirement (paraphrased): C7 G1 MUST: Classify access type read tool model network memory write

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-155]: L5CertificationResult emitted without satisfying 'C7 G1 MUST: Classify access type read tool model network memor...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-155-001",
        "certification_class": "policy",
        "policy_hash": "policy-155-h",
        "blueprint_hash": "blueprint-155-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-155-001",
        "trace_id": "trace-155-001",
        "request_id": "req-155-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-155-h",
        "blueprint_hash": "blueprint-155-h",
        "replay_key": "replay-155-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-155-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-155: per ledger row, a L5CertificationResult that violates the canonical
