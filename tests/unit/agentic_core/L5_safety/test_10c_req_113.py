"""Proof-evidence test for 10C-REQ-113 (00A_L5_Governance_Safety).
...
Source        : C0_Governance_Safety_Enforcement.md :: G4 REGISTRY VALIDATION

Canonical requirement (paraphrased): Governance G4 MUST: Perform identity check allowed model verification execution mode digest integrity check

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-113]: L5CertificationResult emitted without satisfying 'Governance G4 MUST: Perform identity check allowed model verif...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-113-001",
        "certification_class": "policy",
        "policy_hash": "policy-113-h",
        "blueprint_hash": "blueprint-113-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-113-001",
        "trace_id": "trace-113-001",
        "request_id": "req-113-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-113-h",
        "blueprint_hash": "blueprint-113-h",
        "replay_key": "replay-113-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-113-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-113: per ledger row, a L5CertificationResult that violates the canonical
