"""Proof-evidence test for 10C-REQ-192 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L5 safety
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C6-NEW-A defense-in-depth guardrail ordering contract

Canonical requirement (paraphrased): Guardrails MUST execute in a deterministic chain: (1) input sanitization, (2) policy classification, (3) injection detection, (4) authority ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-192]: L5CertificationResult emitted without satisfying 'Guardrails MUST execute in a deterministic chain: (1) input sa...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-192-001",
        "certification_class": "policy",
        "policy_hash": "policy-192-h",
        "blueprint_hash": "blueprint-192-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-192-001",
        "trace_id": "trace-192-001",
        "request_id": "req-192-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-192-h",
        "blueprint_hash": "blueprint-192-h",
        "replay_key": "replay-192-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-192-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-192: per ledger row, a L5CertificationResult that violates the canonical
