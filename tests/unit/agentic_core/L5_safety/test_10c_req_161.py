"""Proof-evidence test for 10C-REQ-161 (00A_L5_Governance_Safety).
...
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G7 INVOCATION RECORD

Canonical requirement (paraphrased): C7 G7 MUST: Record usage who used what provider tool compute cost append to audit log seal replay envelope

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-161]: L5CertificationResult emitted without satisfying 'C7 G7 MUST: Record usage who used what provider tool compute c...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-161-001",
        "certification_class": "policy",
        "policy_hash": "policy-161-h",
        "blueprint_hash": "blueprint-161-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-161-001",
        "trace_id": "trace-161-001",
        "request_id": "req-161-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-161-h",
        "blueprint_hash": "blueprint-161-h",
        "replay_key": "replay-161-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-161-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-161: per ledger row, a L5CertificationResult that violates the canonical
