"""Proof-evidence test for 10C-REQ-110 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : C0 governance
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : C0_Governance_Safety_Enforcement.md :: G1 GOVERNANCE INVOCATION

Canonical requirement (paraphrased): Governance G1 MUST: Select triage mode static check pre-flight runtime check active human re-entry with initial integrity verification ingre...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-110]: L5CertificationResult emitted without satisfying 'Governance G1 MUST: Select triage mode static check pre-flight...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-110-001",
        "certification_class": "policy",
        "policy_hash": "policy-110-h",
        "blueprint_hash": "blueprint-110-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-110-001",
        "trace_id": "trace-110-001",
        "request_id": "req-110-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-110-h",
        "blueprint_hash": "blueprint-110-h",
        "replay_key": "replay-110-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-110-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-110: per ledger row, a L5CertificationResult that violates the canonical
