"""Proof-evidence test for 10C-REQ-120 (00A_L5_Governance_Safety).
...
Source        : C1_Deterministic_Replay_Execution_Integrity.md :: REPLAY GUARD

Canonical requirement (paraphrased): C1 replay guard MUST: Wrap every tool/model invocation prevent non-deterministic data leaks intercept wall clock raw random uuid4 live netwo...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-120]: L5CertificationResult emitted without satisfying 'C1 replay guard MUST: Wrap every tool/model invocation prevent...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-120-001",
        "certification_class": "policy",
        "policy_hash": "policy-120-h",
        "blueprint_hash": "blueprint-120-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-120-001",
        "trace_id": "trace-120-001",
        "request_id": "req-120-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-120-h",
        "blueprint_hash": "blueprint-120-h",
        "replay_key": "replay-120-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-120-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-120: per ledger row, a L5CertificationResult that violates the canonical
