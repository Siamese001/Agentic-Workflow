"""Proof-evidence test for 10C-REQ-101 (00A_L5_Governance_Safety).
...
Source        : 05_Live_Runtime_Exit_Control.md :: H2 MATERIALIZE PACKET

Canonical requirement (paraphrased): HITL H2 MUST: Materialize packet with reason_code evidence policy state bounded packet only for human review

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-101]: L5CertificationResult emitted without satisfying 'HITL H2 MUST: Materialize packet with reason_code evidence pol...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-101-001",
        "certification_class": "policy",
        "policy_hash": "policy-101-h",
        "blueprint_hash": "blueprint-101-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-101-001",
        "trace_id": "trace-101-001",
        "request_id": "req-101-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-101-h",
        "blueprint_hash": "blueprint-101-h",
        "replay_key": "replay-101-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-101-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-101: per ledger row, a L5CertificationResult that violates the canonical
