"""Proof-evidence test for 10C-REQ-119 (00A_L5_Governance_Safety).
...
Source        : C1_Deterministic_Replay_Execution_Integrity.md :: DETERMINISM SURFACE

Canonical requirement (paraphrased): C1 determinism MUST: Enforce Run Clock Only Seeded Only Stable IDs Only Photocopy Calls One Snapshot Only Proposal Only no wall clock no raw...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-119]: L5CertificationResult emitted without satisfying 'C1 determinism MUST: Enforce Run Clock Only Seeded Only Stable...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-119-001",
        "certification_class": "policy",
        "policy_hash": "policy-119-h",
        "blueprint_hash": "blueprint-119-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-119-001",
        "trace_id": "trace-119-001",
        "request_id": "req-119-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-119-h",
        "blueprint_hash": "blueprint-119-h",
        "replay_key": "replay-119-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-119-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-119: per ledger row, a L5CertificationResult that violates the canonical
