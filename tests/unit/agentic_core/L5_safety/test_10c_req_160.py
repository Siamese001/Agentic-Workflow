"""Proof-evidence test for 10C-REQ-160 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : C7 capability
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G6 SOVEREIGN EGRESS GATE

Canonical requirement (paraphrased): C7 G6 MUST: Map symbolic request to specific provider enforce No silent fallback ensure exactly one approved path

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-160]: L5CertificationResult emitted without satisfying 'C7 G6 MUST: Map symbolic request to specific provider enforce ...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-160-001",
        "certification_class": "policy",
        "policy_hash": "policy-160-h",
        "blueprint_hash": "blueprint-160-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-160-001",
        "trace_id": "trace-160-001",
        "request_id": "req-160-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-160-h",
        "blueprint_hash": "blueprint-160-h",
        "replay_key": "replay-160-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-160-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-160: per ledger row, a L5CertificationResult that violates the canonical
