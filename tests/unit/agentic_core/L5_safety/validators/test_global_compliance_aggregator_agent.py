"""
Unit tests for GlobalComplianceAggregatorAgent - Validator in L5.
...
class TestGlobalComplianceAggregatorAgent:
    """Unit tests for GlobalComplianceAggregatorAgent."""

    @pytest.fixture
    def agent_class(self):
        """Import agent class with mocked dependencies."""
        try:
            from agentic_core.L5_safety.validators.GlobalComplianceAggregatorAgent import (
                GlobalComplianceAggregatorAgent,
            )

            return GlobalComplianceAggregatorAgent
        except (ImportError, NameError, AttributeError, TypeError) as e:
            pytest.skip(f"Cannot import GlobalComplianceAggregatorAgent: {e}")

    def test_class_exists(self, agent_class):
        """Verify GlobalComplianceAggregatorAgent exists and is importable."""
        assert agent_class is not None, "GlobalComplianceAggregatorAgent should exist"
    ...
    def test_has_aggregate_results_method(self, agent_class):
        """Verify agent has aggregate_results method."""
        assert hasattr(agent_class, "aggregate_results"), "Should have aggregate_results method"
    ...
    def test_no_network_calls_on_import(self):
        ...
        with patch("requests.get", track_call), patch("requests.post", track_call):
            try:
                from agentic_core.L5_safety.validators.GlobalComplianceAggregatorAgent import (
                    GlobalComplianceAggregatorAgent,  # noqa: F401
                )
