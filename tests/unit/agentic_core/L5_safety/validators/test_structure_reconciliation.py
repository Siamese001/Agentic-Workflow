...
class TestStructureReconciliation:
    ...
    def test_apps_shared_subfolder_map_reconciliation(self):
        ...
        # Check for absence of evicted folders
        assert "base_agents" not in map_folders, (
            "APPS_SHARED_SUBFOLDER_MAP contains evicted 'base_agents'"
        )
        assert "common_utils" not in map_folders, (
            "APPS_SHARED_SUBFOLDER_MAP contains evicted 'common_utils'"
        )
    ...
    def test_no_zombie_references_in_subfolder_metadata(self):
        ...
        assert "base_agents" not in metadata or "apps_shared" not in str(
            metadata.get("base_agents", {})
        ), "SUBFOLDER_METADATA contains reference to evicted base_agents"
        assert "common_utils" not in metadata or "apps_shared" not in str(
            metadata.get("common_utils", {})
