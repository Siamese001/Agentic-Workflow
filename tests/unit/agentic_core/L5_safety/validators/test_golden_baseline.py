...
def test_baseline_capture_integrity(gold_master_env):
    ...
    """
    manifest_path = (
        gold_master_env / "agentic_core" / "config" / "baselines" / "golden_baseline.json"
    )
...
def test_multiple_file_drift(gold_master_env):
    ...
    (gold_master_env / "agentic_core" / "TestAgent1.py").write_text(
        "class Test1: pass", encoding="utf-8"
    )
    (gold_master_env / "agentic_core" / "TestAgent2.py").write_text(
        "class Test2: pass", encoding="utf-8"
    )
    ...
    (gold_master_env / "agentic_core" / "TestAgent1.py").write_text(
        "class Test1: modified", encoding="utf-8"
    )
    (gold_master_env / "agentic_core" / "TestAgent2.py").write_text(
        "class Test2: changed", encoding="utf-8"
    )
