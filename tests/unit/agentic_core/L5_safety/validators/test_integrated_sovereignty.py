...
def test_pascal_sovereignty_deletes_identical_collision(sovereign_env):
    ...
    assert (l5 / "helper_utils.py").exists(), "Canonical helper_utils.py should remain"
...
def test_unified_healing_mission(sovereign_env):
    ...
    assert not (l5 / "HelperUtils.py").exists(), "Identical collision should be deleted"
...
def test_zero_violation_audit_pass(sovereign_env):
    ...
    validator = PascalSovereigntyAgent(project_root=sovereign_env, dry_run=True, validate_only=True)
    result = validator.run()

    # After healing, validation should pass with 0 violations
    total_violations = sum(validator.stats["violations"].values())
    assert total_violations == 0, f"After healing, should have 0 violations, got {total_violations}"
...
def test_dry_run_mode(sovereign_env):
    ...
    l5 = sovereign_env / "agentic_core" / "L5_safety" / "validators"
    assert (l5 / "SubatomicTestingMixin.py").exists(), "Violator should still exist in dry_run mode"
