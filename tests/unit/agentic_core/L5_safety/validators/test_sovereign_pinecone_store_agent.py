"""
Unit tests for SovereignPineconeStoreAgent - Orchestrator in L5.


    ADAPTER: Legacy Interface -> New MCP Client.
    Maintains backward compatibility for 'add_text
...
class TestSovereignPineconeStoreAgent:
    """Unit tests for SovereignPineconeStoreAgent."""

    @pytest.fixture
    def agent_class(self):
        """Import agent class with mocked dependencies."""
        try:
            from agentic_core.L5_safety.validators.SovereignPineconeStoreAgent import (
                SovereignPineconeStoreAgent,
            )

            return SovereignPineconeStoreAgent
        except (ImportError, NameError, AttributeError, TypeError) as e:
            pytest.skip(f"Cannot import SovereignPineconeStoreAgent: {e}")

    def test_class_exists(self, agent_class):
        """Verify SovereignPineconeStoreAgent exists and is importable."""
        assert agent_class is not None, "SovereignPineconeStoreAgent should exist"
    ...
    def test_has_similarity_search_method(self, agent_class):
        """Verify agent has similarity_search method."""
        assert hasattr(agent_class, "similarity_search"), "Should have similarity_search method"
    ...
    def test_no_network_calls_on_import(self):
        ...
        with patch("requests.get", track_call), patch("requests.post", track_call):
            try:
                from agentic_core.L5_safety.validators.SovereignPineconeStoreAgent import (
                    SovereignPineconeStoreAgent,  # noqa: F401
                )
