...
class TestAntiPatternScanner:
    def test_creates_with_default_settings(self, tmp_path):
        ...
        assert len(scanner.scan_dirs) > 0  # Should have default scan dirs
        assert len(scanner.exclude_patterns) > 0  # Should have default excludes
        assert scanner.composite is not None
        assert len(scanner.composite.detectors) > 0  # Should have detectors
...
""")
        ...
        assert report.total_violations >= 1  # Should detect violation in file2
    ...
    def test_get_enforcement_action(self, tmp_path):
        ...
        assert action == "warn"  # Default enforcement level
