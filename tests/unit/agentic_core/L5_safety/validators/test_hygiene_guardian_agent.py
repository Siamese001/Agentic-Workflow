"""
Unit tests for HygieneGuardianAgent - Healer in L5.
...
class TestHygieneGuardianAgent:
    ...
    def agent_class(self):
        ...
        try:
            from agentic_core.L5_safety.enforcement.hygiene_guardian_config_script import (
                HygieneGuardianAgent,
            )

            return HygieneGuardianAgent
        except (ImportError, NameError, AttributeError, TypeError) as e:
            pytest.skip(f"Cannot import HygieneGuardianAgent: {e}")

    def test_class_exists(self, agent_class):
        """Verify HygieneGuardianAgent exists and is importable."""
        assert agent_class is not None, "HygieneGuardianAgent should exist"
