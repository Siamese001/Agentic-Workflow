        ...
        for core_dir in core_dirs:
            ...
            for file_path in core_dir.rglob("*"):
                if file_path.is_file():
                    ...
                    if (('canon' in filename and 
                         ('key' in filename or 'validator' in filename or 'agent' in filename or 'scheduler' in filename)) or
                        filename.startswith('canon_') or
                        'canonkey' in filename or
                        ('check_key' in filename and filename.endswith('.py'))):
            ...
            if doc_path.is_file():
                ...
                try:
                    ...
                    for i, line in enumerate(lines):
                        line = line.strip()
                        if ('canon key' in line.lower() and 
                            not line.startswith('#') and
                            not line.startswith('<!--') and
                            'deprecated' not in line.lower() and
                            'removed' not in line.lower()):
            ...
            for config_file in config_dir.rglob("*"):
                ...
                try:
                    ...
                    if 'canon' in content.lower() and 'key' in content.lower():
                        ...
                        for i, line in enumerate(lines):
                            line = line.strip()
                            if ('canon' in line.lower() and 'key' in line.lower() and
                                not line.startswith('#') and
                                not line.startswith('"') and
                                not line.startswith("'")):
                ...
                try:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if stripped.startswith('from ') or stripped.startswith('import '):
                            ...
                            if any(module in stripped for module in [
                                'canon_agents_core',
                                ...
                                if any(skip in str(py_file) for skip in ['maintenance', 'scripts', 'legacy', 'test']):
                                    continue
                                orphaned_deps.append(f"{py_file.relative_to(PROJECT_ROOT)}:{i+1}: {stripped}")
                except Exception:
                    continue
