"""Proof-evidence test for 10C-REQ-139 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : HITL healing
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : C3_Healing_Remediation_Escalation.md :: SECURE READING ROOM

Canonical requirement (paraphrased): C3 secure room MUST: Allow bounded packet ONLY no free-form bypass to live ops decision APPROVE/MODIFY/REJECT

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-139]: L5CertificationResult emitted without satisfying 'C3 secure room MUST: Allow bounded packet ONLY no free-form by...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-139-001",
        "certification_class": "policy",
        "policy_hash": "policy-139-h",
        "blueprint_hash": "blueprint-139-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-139-001",
        "trace_id": "trace-139-001",
        "request_id": "req-139-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-139-h",
        "blueprint_hash": "blueprint-139-h",
        "replay_key": "replay-139-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-139-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-139: per ledger row, a L5CertificationResult that violates the canonical
