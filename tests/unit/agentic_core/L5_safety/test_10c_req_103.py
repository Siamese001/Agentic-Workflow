"""Proof-evidence test for 10C-REQ-103 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L5 re-clearance
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : 05_Live_Runtime_Exit_Control.md :: H5 HITL RE-CLEARANCE

Canonical requirement (paraphrased): HITL H5 re-clearance MUST: Route REJECT->DENY/STOP/RETURN_TO_L1 MODIFY_DIFF->L5 Re-clear->Context Re-hydrate->RESTART APPROVE->L5 Confirmati...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-103]: L5CertificationResult emitted without satisfying 'HITL H5 re-clearance MUST: Route REJECT->DENY/STOP/RETURN_TO_L...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-103-001",
        "certification_class": "policy",
        "policy_hash": "policy-103-h",
        "blueprint_hash": "blueprint-103-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-103-001",
        "trace_id": "trace-103-001",
        "request_id": "req-103-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-103-h",
        "blueprint_hash": "blueprint-103-h",
        "replay_key": "replay-103-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-103-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-103: per ledger row, a L5CertificationResult that violates the canonical
