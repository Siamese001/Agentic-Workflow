"""Proof-evidence test for 10C-REQ-187 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L2 execution + L5 safety
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C1-NEW-B tool-error self-correction schema

Canonical requirement (paraphrased): Tool failures MUST return a structured ToolErrorPacket with fields: error_class in {INVALID_ARGS, UNAUTHORIZED, RATE_LIMITED, NOT_FOUND, TRA...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-187]: L5CertificationResult emitted without satisfying 'Tool failures MUST return a structured ToolErrorPacket with fi...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-187-001",
        "certification_class": "policy",
        "policy_hash": "policy-187-h",
        "blueprint_hash": "blueprint-187-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-187-001",
        "trace_id": "trace-187-001",
        "request_id": "req-187-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-187-h",
        "blueprint_hash": "blueprint-187-h",
        "replay_key": "replay-187-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-187-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-187: per ledger row, a L5CertificationResult that violates the canonical
