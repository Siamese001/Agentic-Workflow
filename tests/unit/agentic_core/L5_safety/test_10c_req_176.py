"""Proof-evidence test for 10C-REQ-176 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : HITL L5
Artifact      : L5CertificationResult
OTEL span     : l5.hitl.escalation_recorded
Source        : agentic_process_mapping_v38.md + best-practice gap analysis 2026-04-29 :: HITL-NEW-C durable HITL suspend/resume primitive

Canonical requirement (paraphrased): HITL stage H2 MUST support a durable suspend primitive: a serializable HITLPendingState containing the bounded review packet, TTL, escalatio...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-176]: L5CertificationResult emitted without satisfying 'HITL stage H2 MUST support a durable suspend primitive: a seri...
...
EXPECTED_SPAN = "l5.hitl.escalation_recorded"
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-176-001",
        "certification_class": "policy",
        "policy_hash": "policy-176-h",
        "blueprint_hash": "blueprint-176-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-176-001",
        "trace_id": "trace-176-001",
        "request_id": "req-176-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-176-h",
        "blueprint_hash": "blueprint-176-h",
        "replay_key": "replay-176-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-176-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The l5.hitl.escalation_recorded span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-176: per ledger row, a L5CertificationResult that violates the canonical
