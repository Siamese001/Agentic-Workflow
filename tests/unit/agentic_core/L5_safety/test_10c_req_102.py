"""Proof-evidence test for 10C-REQ-102 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : HITL human
Artifact      : L5CertificationResult
OTEL span     : l5.hitl.escalation_recorded
Source        : 05_Live_Runtime_Exit_Control.md :: H3 HUMAN REVIEW CORE

Canonical requirement (paraphrased): HITL H3/H4 MUST: Allow human inspect evidence proposed action replay with decision APPROVE MODIFY_DIFF REJECT + rationale

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-102]: L5CertificationResult emitted without satisfying 'HITL H3/H4 MUST: Allow human inspect evidence proposed action ...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-102-001",
        "certification_class": "policy",
        "policy_hash": "policy-102-h",
        "blueprint_hash": "blueprint-102-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-102-001",
        "trace_id": "trace-102-001",
        "request_id": "req-102-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-102-h",
        "blueprint_hash": "blueprint-102-h",
        "replay_key": "replay-102-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-102-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-102: per ledger row, a L5CertificationResult that violates the canonical
