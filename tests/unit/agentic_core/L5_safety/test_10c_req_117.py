"""Proof-evidence test for 10C-REQ-117 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L5/L3/L0
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : C1_Deterministic_Replay_Execution_Integrity.md :: BUILD REPLAY ENVELOPE

Canonical requirement (paraphrased): C1 MUST: Build replay envelope generating unique replay_key tied to active policy_hash bind capability_token to run_id for state tracking

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-117]: L5CertificationResult emitted without satisfying 'C1 MUST: Build replay envelope generating unique replay_key ti...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-117-001",
        "certification_class": "policy",
        "policy_hash": "policy-117-h",
        "blueprint_hash": "blueprint-117-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-117-001",
        "trace_id": "trace-117-001",
        "request_id": "req-117-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-117-h",
        "blueprint_hash": "blueprint-117-h",
        "replay_key": "replay-117-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-117-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-117: per ledger row, a L5CertificationResult that violates the canonical
