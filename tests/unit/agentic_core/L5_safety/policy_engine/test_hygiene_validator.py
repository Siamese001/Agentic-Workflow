...
def run_self_tests() -> dict[str, Any]:
    """Run the StructureValidatorAgent's internal self-tests."""
    from agentic_core.L5_safety.policy_engine.StructureValidatorAgent import (
        StructureValidatorAgent,
    )

    validator = StructureValidatorAgent(project_root=PROJECT_ROOT)
...
def test_gap4_duplicate_detection() -> dict[str, Any]:
    ...
    validator = StructureValidatorAgent(project_root=fixtures_dir)
...
def main():
    parser = argparse.ArgumentParser(description="Test StructureValidatorAgent")
    parser.add_argument("--self-test", action="store_true", help="Run only self-tests")
    parser.add_argument(
        "--duplicates-only", action="store_true", help="Run only duplicate detection test"
    )
    parser.add_argument(
        "--orphans-only", action="store_true", help="Run only orphan detection test"
    )
    parser.add_argument("--markers-only", action="store_true", help="Run only marker scanning test")
    parser.add_argument(
        "--output-dir", type=str, default="test_results", help="Output directory for JSON results"
    )
    ...
    print("=" * 60)
    print("StructureValidatorAgent Test Suite (Phase 2)")
    print("=" * 60)
    ...
    if args.duplicates_only or not any([args.self_test, args.orphans_only, args.markers_only]):
        ...
        try:
            ...
            if dup_results.get("status") == "PASS":
                print(
                    f"  ✓ GAP-4 PASSED: Duplicate files detected ({dup_results.get('duplicates_found')} groups)"
                )
    ...
    if args.orphans_only or not any([args.self_test, args.duplicates_only, args.markers_only]):
        ...
        try:
            ...
            if orphan_results.get("status") == "PASS":
                print(
                    f"  ✓ Orphan detection PASSED: {orphan_results.get('orphans_found')} orphans found"
                )
    ...
    if args.markers_only or not any([args.self_test, args.duplicates_only, args.orphans_only]):
        ...
        try:
            ...
            if marker_results.get("status") == "PASS":
                print(
                    f"  ✓ Marker scanning PASSED: {marker_results.get('total_markers_found')} markers found"
                )
