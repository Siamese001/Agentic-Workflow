"""
Unit tests for DependencyPruningAgent - Healer in L5.

L5 Safety agent that detects and removes unused Python dependencies.

    This batch agent uses 'dep
...
class TestDependencyPruningAgent:
    ...
    def agent_class(self):
        ...
        try:
            from agentic_core.L5_safety.enforcement.DependencyPruningAgent import (
                DependencyPruningAgent,
            )

            return DependencyPruningAgent
        except (ImportError, NameError, AttributeError, TypeError) as e:
            pytest.skip(f"Cannot import DependencyPruningAgent: {e}")

    def test_class_exists(self, agent_class):
        """Verify DependencyPruningAgent exists and is importable."""
        assert agent_class is not None, "DependencyPruningAgent should exist"
