"""
Unit tests for SelfUpdatingSafetyEngineAgent - Guardian in L5.


    Safety engine that learns and adapts to new threats.

    Features:
    - Automatic threat patt
...
class TestSelfUpdatingSafetyEngineAgent:
    """Unit tests for SelfUpdatingSafetyEngineAgent."""
    
    @pytest.fixture
    def agent_class(self):
        """Import agent class with mocked dependencies."""
        try:
            from agentic_core.L5_safety.guardrails.SelfUpdatingSafetyEngineAgent import SelfUpdatingSafetyEngineAgent
            return SelfUpdatingSafetyEngineAgent
        except (ImportError, NameError, AttributeError, TypeError) as e:
            pytest.skip(f"Cannot import SelfUpdatingSafetyEngineAgent: {e}")
    
    def test_class_exists(self, agent_class):
        """Verify SelfUpdatingSafetyEngineAgent exists and is importable."""
        assert agent_class is not None, "SelfUpdatingSafetyEngineAgent should exist"
    ...
    def test_has_detect_threats_method(self, agent_class):
        """Verify agent has detect_threats method."""
        assert hasattr(agent_class, 'detect_threats'), "Should have detect_threats method"
