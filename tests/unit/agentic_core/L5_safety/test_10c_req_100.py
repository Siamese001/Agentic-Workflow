"""Proof-evidence test for 10C-REQ-100 (00A_L5_Governance_Safety).
...
Source        : 05_Live_Runtime_Exit_Control.md :: H1 HITL AIRLOCK FREEZE

Canonical requirement (paraphrased): HITL H1 freeze MUST: Set authority_state=FROZEN write_auth=NONE

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-100]: L5CertificationResult emitted without satisfying 'HITL H1 freeze MUST: Set authority_state=FROZEN write_auth=NON...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-100-001",
        "certification_class": "policy",
        "policy_hash": "policy-100-h",
        "blueprint_hash": "blueprint-100-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-100-001",
        "trace_id": "trace-100-001",
        "request_id": "req-100-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-100-h",
        "blueprint_hash": "blueprint-100-h",
        "replay_key": "replay-100-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-100-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-100: per ledger row, a L5CertificationResult that violates the canonical
