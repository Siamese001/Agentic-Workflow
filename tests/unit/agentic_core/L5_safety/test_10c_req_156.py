"""Proof-evidence test for 10C-REQ-156 (00A_L5_Governance_Safety).
...
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G2 REGISTRY + ALLOWED SET

Canonical requirement (paraphrased): C7 G2 MUST: Validate identity enforce allowed_models execution_mode locks registry digest integrity match ACL verification

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-156]: L5CertificationResult emitted without satisfying 'C7 G2 MUST: Validate identity enforce allowed_models execution...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-156-001",
        "certification_class": "policy",
        "policy_hash": "policy-156-h",
        "blueprint_hash": "blueprint-156-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-156-001",
        "trace_id": "trace-156-001",
        "request_id": "req-156-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-156-h",
        "blueprint_hash": "blueprint-156-h",
        "replay_key": "replay-156-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-156-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-156: per ledger row, a L5CertificationResult that violates the canonical
