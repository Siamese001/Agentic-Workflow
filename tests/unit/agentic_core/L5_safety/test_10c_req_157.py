"""Proof-evidence test for 10C-REQ-157 (00A_L5_Governance_Safety).
...
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G3 CHOOSE THE LANE

Canonical requirement (paraphrased): C7 G3 MUST: Route to local tool external model memory network or Universal Write Gate

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-157]: L5CertificationResult emitted without satisfying 'C7 G3 MUST: Route to local tool external model memory network ...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-157-001",
        "certification_class": "policy",
        "policy_hash": "policy-157-h",
        "blueprint_hash": "blueprint-157-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-157-001",
        "trace_id": "trace-157-001",
        "request_id": "req-157-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-157-h",
        "blueprint_hash": "blueprint-157-h",
        "replay_key": "replay-157-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-157-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-157: per ledger row, a L5CertificationResult that violates the canonical
