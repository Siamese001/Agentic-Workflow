"""Proof-evidence test for 10C-REQ-159 (00A_L5_Governance_Safety).
...
Source        : C7_Capability_Tool_Model_Access_Control_Plane.md :: G5 INTERCEPT THE CALL

Canonical requirement (paraphrased): C7 G5 MUST: Validate argument shape route target perform injection checks assess risk tiering against current policy

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-159]: L5CertificationResult emitted without satisfying 'C7 G5 MUST: Validate argument shape route target perform injec...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-159-001",
        "certification_class": "policy",
        "policy_hash": "policy-159-h",
        "blueprint_hash": "blueprint-159-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-159-001",
        "trace_id": "trace-159-001",
        "request_id": "req-159-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-159-h",
        "blueprint_hash": "blueprint-159-h",
        "replay_key": "replay-159-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-159-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-159: per ledger row, a L5CertificationResult that violates the canonical
