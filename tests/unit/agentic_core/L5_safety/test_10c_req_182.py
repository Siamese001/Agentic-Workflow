"""Proof-evidence test for 10C-REQ-182 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L1 cognition + L5 safety
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : best-practice gap analysis 2026-04-30 wave2 C2 context engineering :: C2-NEW-C trust-level propagation through PA

Canonical requirement (paraphrased): Every ContextItem and VerifiedChunk entering the PA pipeline MUST carry a trust_level in {system, policy, user, tool_output, external_retrie...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-182]: L5CertificationResult emitted without satisfying 'Every ContextItem and VerifiedChunk entering the PA pipeline M...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-182-001",
        "certification_class": "policy",
        "policy_hash": "policy-182-h",
        "blueprint_hash": "blueprint-182-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-182-001",
        "trace_id": "trace-182-001",
        "request_id": "req-182-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-182-h",
        "blueprint_hash": "blueprint-182-h",
        "replay_key": "replay-182-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-182-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-182: per ledger row, a L5CertificationResult that violates the canonical
