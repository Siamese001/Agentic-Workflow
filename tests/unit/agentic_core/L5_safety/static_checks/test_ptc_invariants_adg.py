...
@pytest.mark.skipif(not _AVAILABLE, reason="ptc_invariants.py deps unavailable")
class TestPtcInvariantsImportability:
    def test_module_importable(self) -> None:
        """ADG contract: ptc_invariants.py must be importable."""
        assert _AVAILABLE

    def test_ptcinvariantvisitor_is_type(self) -> None:
        assert PTCInvariantVisitor is not None

    def test_scan_file_for_ptc_invariants_callable(self) -> None:
        assert callable(scan_file_for_ptc_invariants)

    def test_scan_repository_for_ptc_invariants_callable(self) -> None:
        assert callable(scan_repository_for_ptc_invariants)
