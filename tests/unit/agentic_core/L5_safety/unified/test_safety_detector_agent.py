"""
Unit tests for SafetyDetectorAgent - Validator in L5.
...
class TestSafetyDetectorAgent:
    """Unit tests for SafetyDetectorAgent."""

    @pytest.fixture
    def agent_class(self):
        """Import agent class with mocked dependencies."""
        try:
            from agentic_core.L5_safety.unified.safety_detector_agent_types import SafetyDetectorAgent

            return SafetyDetectorAgent
        except (ImportError, NameError, AttributeError, TypeError) as e:
            pytest.skip(f"Cannot import SafetyDetectorAgent: {e}")

    def test_class_exists(self, agent_class):
        """Verify SafetyDetectorAgent exists and is importable."""
        assert agent_class is not None, "SafetyDetectorAgent should exist"
    ...
    def test_no_network_calls_on_import(self):
        ...
        with patch("requests.get", track_call), patch("requests.post", track_call):
            try:
                from agentic_core.L5_safety.unified.safety_detector_agent_types import (
                    SafetyDetectorAgent,  # noqa: F401
                )
