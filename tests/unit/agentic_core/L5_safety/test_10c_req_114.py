"""Proof-evidence test for 10C-REQ-114 (00A_L5_Governance_Safety).
...
Source        : C0_Governance_Safety_Enforcement.md :: G5 CLASSIFY SHAPE

Canonical requirement (paraphrased): Governance G5 MUST: Perform execution type check dual-tag clash detection resource intent vs capability alignment

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-114]: L5CertificationResult emitted without satisfying 'Governance G5 MUST: Perform execution type check dual-tag clas...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-114-001",
        "certification_class": "policy",
        "policy_hash": "policy-114-h",
        "blueprint_hash": "blueprint-114-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-114-001",
        "trace_id": "trace-114-001",
        "request_id": "req-114-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-114-h",
        "blueprint_hash": "blueprint-114-h",
        "replay_key": "replay-114-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-114-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-114: per ledger row, a L5CertificationResult that violates the canonical
