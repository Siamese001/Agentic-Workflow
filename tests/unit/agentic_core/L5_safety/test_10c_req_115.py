"""Proof-evidence test for 10C-REQ-115 (00A_L5_Governance_Safety).
...
Source        : C0_Governance_Safety_Enforcement.md :: G6 POLICY CHOKEPOINT

Canonical requirement (paraphrased): Governance G6 MUST: Perform risk tiering explicit tool validation plan-to-action alignment check

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-115]: L5CertificationResult emitted without satisfying 'Governance G6 MUST: Perform risk tiering explicit tool validat...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-115-001",
        "certification_class": "policy",
        "policy_hash": "policy-115-h",
        "blueprint_hash": "blueprint-115-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-115-001",
        "trace_id": "trace-115-001",
        "request_id": "req-115-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-115-h",
        "blueprint_hash": "blueprint-115-h",
        "replay_key": "replay-115-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-115-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-115: per ledger row, a L5CertificationResult that violates the canonical
