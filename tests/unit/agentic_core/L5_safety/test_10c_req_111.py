"""Proof-evidence test for 10C-REQ-111 (00A_L5_Governance_Safety).
...
Source        : C0_Governance_Safety_Enforcement.md :: G2 AUTHORITY CONTEXT

Canonical requirement (paraphrased): Governance G2 MUST: Load active policy sets structure maps registry rules for specific tenant bind governing Charter to request folder

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-111]: L5CertificationResult emitted without satisfying 'Governance G2 MUST: Load active policy sets structure maps reg...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-111-001",
        "certification_class": "policy",
        "policy_hash": "policy-111-h",
        "blueprint_hash": "blueprint-111-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-111-001",
        "trace_id": "trace-111-001",
        "request_id": "req-111-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-111-h",
        "blueprint_hash": "blueprint-111-h",
        "replay_key": "replay-111-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-111-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-111: per ledger row, a L5CertificationResult that violates the canonical
