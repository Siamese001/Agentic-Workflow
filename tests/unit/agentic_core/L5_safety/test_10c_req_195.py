"""Proof-evidence test for 10C-REQ-195 (00A_L5_Governance_Safety).

Severity      : CRITICAL
Surface       : 00A_L5_Governance_Safety
Layer owner   : L6 observability + L5 safety
Artifact      : L5CertificationResult
OTEL span     : l5.certification.evidence_emitted
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C7-NEW-B PII and secret scrubbing at telemetry egress

Canonical requirement (paraphrased): Before any OTEL span or log record leaves the process boundary (exporter to collector, log shipper to remote sink), a deterministic Scrubbin...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-195]: L5CertificationResult emitted without satisfying 'Before any OTEL span or log record leaves the process boundary...
...
def _valid_artifact() -> dict:
    return {
        "certification_id": "cert-195-001",
        "certification_class": "policy",
        "policy_hash": "policy-195-h",
        "blueprint_hash": "blueprint-195-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-195-001",
        "trace_id": "trace-195-001",
        "request_id": "req-195-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-195-h",
        "blueprint_hash": "blueprint-195-h",
        "replay_key": "replay-195-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-195-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-195: per ledger row, a L5CertificationResult that violates the canonical
