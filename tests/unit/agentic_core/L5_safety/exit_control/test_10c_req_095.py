"""Proof-evidence test for 10C-REQ-095 (05_Exit_Evaluation_and_Control).
...
Source        : 05_Live_Runtime_Exit_Control.md :: X1A WHAT ARE TODAY'S RULES RUBRICS?

Canonical requirement (paraphrased): Exit eval X1A MUST: Check active baselines user ask/success current policy refs

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-095]: X3DispositionPacket emitted without satisfying 'Exit eval X1A MUST: Check active baselines user ask/success curr...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-95-001",
        "trace_id": "trace-95-001",
        "request_id": "req-95-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-95-h",
        "blueprint_hash": "blueprint-95-h",
        "replay_key": "replay-95-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-95-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-095: per ledger row, a X3DispositionPacket that violates the canonical
