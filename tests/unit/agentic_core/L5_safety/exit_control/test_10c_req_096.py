"""Proof-evidence test for 10C-REQ-096 (05_Exit_Evaluation_and_Control).
...
Source        : 05_Live_Runtime_Exit_Control.md :: X1B DID WE ANSWER IT?

Canonical requirement (paraphrased): Exit eval X1B MUST: Check prompt/format fit answer/artifact fit paperwork complete schema complete

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-096]: X3DispositionPacket emitted without satisfying 'Exit eval X1B MUST: Check prompt/format fit answer/artifact fit ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-96-001",
        "trace_id": "trace-96-001",
        "request_id": "req-96-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-96-h",
        "blueprint_hash": "blueprint-96-h",
        "replay_key": "replay-96-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-96-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-096: per ledger row, a X3DispositionPacket that violates the canonical
