"""Proof-evidence test for 10C-REQ-098 (05_Exit_Evaluation_and_Control).

Severity      : CRITICAL
Surface       : 05_Exit_Evaluation_and_Control
Layer owner   : L5 exit control
Artifact      : X3DispositionPacket
OTEL span     : exit.x1.gates_evaluated
Source        : 05_Live_Runtime_Exit_Control.md :: X1D IS THE ANSWER GOOD?

Canonical requirement (paraphrased): Exit eval X1D MUST: Check groundedness citation support answer relevance abstain correctness escalation correctness

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-098]: X3DispositionPacket emitted without satisfying 'Exit eval X1D MUST: Check groundedness citation support answer r...
...
EXPECTED_SPAN = "exit.x1.gates_evaluated"
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-98-001",
        "trace_id": "trace-98-001",
        "request_id": "req-98-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-98-h",
        "blueprint_hash": "blueprint-98-h",
        "replay_key": "replay-98-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-98-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The exit.x1.gates_evaluated span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-098: per ledger row, a X3DispositionPacket that violates the canonical
