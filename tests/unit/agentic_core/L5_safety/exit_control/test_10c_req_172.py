"""Proof-evidence test for 10C-REQ-172 (05_Exit_Evaluation_and_Control).
...
Source        : agentic_process_mapping_v29.md :: [5] EXIT EVAL

Canonical requirement (paraphrased): Exit eval MUST: Review sealed work decide response human escalation durable commit with L5 safety officer

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-172]: X3DispositionPacket emitted without satisfying 'Exit eval MUST: Review sealed work decide response human escalat...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-172-001",
        "trace_id": "trace-172-001",
        "request_id": "req-172-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-172-h",
        "blueprint_hash": "blueprint-172-h",
        "replay_key": "replay-172-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-172-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-172: per ledger row, a X3DispositionPacket that violates the canonical
