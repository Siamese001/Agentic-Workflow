"""Proof-evidence test for 10C-REQ-097 (05_Exit_Evaluation_and_Control).
...
Source        : 05_Live_Runtime_Exit_Control.md :: X1C IS IT SAFE TO LEAVE?

Canonical requirement (paraphrased): Exit eval X1C MUST: Check policy pass/fail revealing secrets mutation authorization env integrity replay env completeness

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-097]: X3DispositionPacket emitted without satisfying 'Exit eval X1C MUST: Check policy pass/fail revealing secrets mut...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-97-001",
        "trace_id": "trace-97-001",
        "request_id": "req-97-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-97-h",
        "blueprint_hash": "blueprint-97-h",
        "replay_key": "replay-97-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-97-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-097: per ledger row, a X3DispositionPacket that violates the canonical
