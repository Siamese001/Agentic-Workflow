...
class TestUniversalHealing(unittest.TestCase):
    ...
    def test_pascal_healing_trigger_with_confidence(self):
        ...
        self.mock_decision_engine.should_proceed_with_healing.return_value = (
            True,
        ...
        try:
            ...
            with patch("agentic_core.L0_maintenance.scripts.execute_ssot.logger") as mock_logger:
                ...
                pascal_proceed, pascal_reason = (
                    self.mock_decision_engine.should_proceed_with_healing(pascal_confidence)
                )
                ...
                if pascal_proceed and not dry_run:
                    ...
                    if hasattr(pascal, "heal_repository"):
                        ...
                        self.mock_state_mgr.complete_agent(
                ...
                self.mock_decision_engine.should_proceed_with_healing.assert_called_once_with(
                    mock_confidence
                )
                mock_pascal_instance.heal_repository.assert_called_once_with(
                    target_territory="prompt_governance", dry_run=False, auto_approve=True
                )
                self.mock_state_mgr.complete_agent.assert_called_with(
                    "PascalSovereigntyAgent", True, "Healed: 5"
                )
        ...
        for i, case in enumerate(test_cases):
            with self.subTest(case=i):
                confidence = decision_engine.calculate_healing_confidence(
                    case["violations_count"], case["violation_types"], case["territory"]
                )
    ...
    def test_dry_run_safety(self):
        ...
        pascal_proceed, pascal_reason = self.mock_decision_engine.should_proceed_with_healing(
            pascal_confidence
        )
        ...
        self.mock_state_mgr.add_event.assert_called_with(
            "info", "Sovereignty healing skipped - Dry run mode"
        )
    ...
    def test_territory_scope_lock(self):
        ...
        for territory in territories:
            with self.subTest(territory=territory):
                ...
                mock_pascal_instance.heal_repository.assert_called_once_with(
                    target_territory=territory, dry_run=False, auto_approve=True
                )
    ...
    def test_low_confidence_blocking(self):
        ...
        self.mock_decision_engine.should_proceed_with_healing.return_value = (
            False,
            "LOW CONFIDENCE (0.30) - LLM Disabled",
        )
        ...
        if not pascal_proceed:
            self.mock_state_mgr.add_event(
                "warning", f"Sovereignty healing skipped - {pascal_reason}"
            )

        # Verify heal_repository was NOT called
        mock_pascal_instance.heal_repository.assert_not_called()
        self.mock_state_mgr.add_event.assert_called_with(
            "warning", "Sovereignty healing skipped - LOW CONFIDENCE (0.30) - LLM Disabled"
        )
    ...
    def test_certificate_includes_pascal_agent(self):
        ...
        try:
            ...
            cert = execute_phase5_final_impl(
                self.mock_agents, "test_territory", self.mock_state_mgr
            )
