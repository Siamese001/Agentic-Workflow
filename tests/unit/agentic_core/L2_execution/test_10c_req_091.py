"""Proof-evidence test for 10C-REQ-091 (04_L2_Execute).
...
Source        : 04_Live_Task_Dispatch_Execution.md :: E2 VALIDATE WORK ORDER CHECK

Canonical requirement (paraphrased): Validation MUST: Check integrity signature chain cap scope env budget schema side-effect class mutation type sanity with sealed rejection on...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-091]: ExecutionResult emitted without satisfying 'Validation MUST: Check integrity signature chain cap scope env budge...
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-91-001",
        "blueprint_hash": "blueprint-91-h",
        "policy_hash": "policy-91-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-91-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-91-001",
        "trace_id": "trace-91-001",
        "request_id": "req-91-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-91-h",
        "blueprint_hash": "blueprint-91-h",
        "replay_key": "replay-91-k",
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-091: per ledger row, a ExecutionResult that violates the canonical
