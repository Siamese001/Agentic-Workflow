"""Proof-evidence test for 10C-REQ-093 (04_L2_Execute).

Severity      : CRITICAL
Surface       : 04_L2_Execute
Layer owner   : L2 execution
Artifact      : ExecutionResult
OTEL span     : l2.execution.heal
Source        : 04_Live_Task_Dispatch_Execution.md :: E4 HEAL LOOP FIXING MISTAKES

Canonical requirement (paraphrased): Heal loop MUST: Log reason_code require parent_packet_id localize failure bounded SSOT repair verify hash/replay integrity increment repair ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-093]: ExecutionResult emitted without satisfying 'Heal loop MUST: Log reason_code require parent_packet_id localize fa...
...
EXPECTED_SPAN = "l2.execution.heal"
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-93-001",
        "blueprint_hash": "blueprint-93-h",
        "policy_hash": "policy-93-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-93-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-93-001",
        "trace_id": "trace-93-001",
        "request_id": "req-93-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-93-h",
        "blueprint_hash": "blueprint-93-h",
        "replay_key": "replay-93-k",
...
def test_span_shape_positive() -> None:
    """The l2.execution.heal span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-093: per ledger row, a ExecutionResult that violates the canonical
