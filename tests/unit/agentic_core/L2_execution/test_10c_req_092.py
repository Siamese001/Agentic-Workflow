"""Proof-evidence test for 10C-REQ-092 (04_L2_Execute).
...
Source        : 04_Live_Task_Dispatch_Execution.md :: E3 EXECUTE DOING THE WORK

Canonical requirement (paraphrased): Execution MUST: Increment attempt count invoke tool/model with timeout/circuit breaker capture output classify result as SUCCESS SOFT_REPAIR...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-092]: ExecutionResult emitted without satisfying 'Execution MUST: Increment attempt count invoke tool/model with timeo...
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-92-001",
        "blueprint_hash": "blueprint-92-h",
        "policy_hash": "policy-92-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-92-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-92-001",
        "trace_id": "trace-92-001",
        "request_id": "req-92-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-92-h",
        "blueprint_hash": "blueprint-92-h",
        "replay_key": "replay-92-k",
...
def test_span_shape_positive() -> None:
    """The l2.execution.attempt span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-092: per ledger row, a ExecutionResult that violates the canonical
