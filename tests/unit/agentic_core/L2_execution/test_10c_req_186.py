"""Proof-evidence test for 10C-REQ-186 (04_L2_Execute).
...
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C1-NEW-A tool-response token cap + pagination

Canonical requirement (paraphrased): Every typed tool MUST declare max_response_tokens in its contract. The L2 tool invoker MUST truncate responses exceeding the cap and return ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-186]: ExecutionResult emitted without satisfying 'Every typed tool MUST declare max_response_tokens in its contract. T...
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-186-001",
        "blueprint_hash": "blueprint-186-h",
        "policy_hash": "policy-186-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-186-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-186-001",
        "trace_id": "trace-186-001",
        "request_id": "req-186-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-186-h",
        "blueprint_hash": "blueprint-186-h",
        "replay_key": "replay-186-k",
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-186: per ledger row, a ExecutionResult that violates the canonical
