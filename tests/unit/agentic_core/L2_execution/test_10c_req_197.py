"""Proof-evidence test for 10C-REQ-197 (04_L2_Execute).
...
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C8-NEW-B MCP tool deprecation lifecycle

Canonical requirement (paraphrased): Each tool exposed by an MCP server MUST be able to declare deprecation metadata: {deprecated: bool, replacement_tool_name, removal_date, dep...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-197]: ExecutionResult emitted without satisfying 'Each tool exposed by an MCP server MUST be able to declare deprecati...
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-197-001",
        "blueprint_hash": "blueprint-197-h",
        "policy_hash": "policy-197-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-197-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-197-001",
        "trace_id": "trace-197-001",
        "request_id": "req-197-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-197-h",
        "blueprint_hash": "blueprint-197-h",
        "replay_key": "replay-197-k",
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-197: per ledger row, a ExecutionResult that violates the canonical
