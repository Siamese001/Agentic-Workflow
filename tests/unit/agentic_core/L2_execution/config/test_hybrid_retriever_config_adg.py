...
@pytest.mark.skipif(not _AVAILABLE, reason="hybrid_retriever_config deps unavailable")
class TestHybridRetrieverConfigImportability:
    def test_module_importable(self) -> None:
        """ADG contract: agentic_core/L2_execution/config/hybrid_retriever_config.py must be importable."""
        assert _AVAILABLE
