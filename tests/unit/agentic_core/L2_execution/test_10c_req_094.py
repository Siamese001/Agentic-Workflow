"""Proof-evidence test for 10C-REQ-094 (04_L2_Execute).
...
Source        : 04_Live_Task_Dispatch_Execution.md :: E5 SEAL THE FINAL FOLDER

Canonical requirement (paraphrased): L2 seal MUST: Package payload attach traces/lineage attach replay receipts/counters seal L2 artifact with terminal class no durable commit

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-094]: ExecutionResult emitted without satisfying 'L2 seal MUST: Package payload attach traces/lineage attach replay re...
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-94-001",
        "blueprint_hash": "blueprint-94-h",
        "policy_hash": "policy-94-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-94-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-94-001",
        "trace_id": "trace-94-001",
        "request_id": "req-94-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-94-h",
        "blueprint_hash": "blueprint-94-h",
        "replay_key": "replay-94-k",
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-094: per ledger row, a ExecutionResult that violates the canonical
