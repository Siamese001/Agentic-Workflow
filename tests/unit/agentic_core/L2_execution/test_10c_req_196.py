"""Proof-evidence test for 10C-REQ-196 (04_L2_Execute).

Severity      : CRITICAL
Surface       : 04_L2_Execute
Layer owner   : L2 execution + L4 state
Artifact      : ExecutionResult
OTEL span     : l2.execution.sealed
Source        : best-practice gap analysis 2026-04-30 wave3-10 hybrid audit :: C8-NEW-A MCP discovery cache TTL + invalidation contract

Canonical requirement (paraphrased): MCP client wrappers MUST cache the (tools, resources, prompts) discovery response keyed by server_id + server_schema_hash, with TTL <= 5 min...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-196]: ExecutionResult emitted without satisfying 'MCP client wrappers MUST cache the (tools, resources, prompts) disco...
...
def _valid_artifact() -> dict:
    return {
        "execution_id": "exec-196-001",
        "blueprint_hash": "blueprint-196-h",
        "policy_hash": "policy-196-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-196-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-196-001",
        "trace_id": "trace-196-001",
        "request_id": "req-196-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-196-h",
        "blueprint_hash": "blueprint-196-h",
        "replay_key": "replay-196-k",
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-196: per ledger row, a ExecutionResult that violates the canonical
