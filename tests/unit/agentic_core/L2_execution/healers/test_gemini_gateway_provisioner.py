...
class GeminiConfigFromEnvTest(unittest.TestCase):
    def setUp(self) -> None:
        self._saved = {k: os.environ.get(k) for k in (ENV_GEMINI_API_KEY, ENV_GEMINI_FLASH_OVERRIDE)}
    ...
    def test_returns_config_when_api_key_set(self) -> None:
        os.environ[ENV_GEMINI_API_KEY] = "test-key-xyz"
    ...
    def test_env_override_for_flash_model(self) -> None:
        os.environ[ENV_GEMINI_API_KEY] = "k"
...
class ProvisionRouterTest(unittest.TestCase):
    def setUp(self) -> None:
        self._saved_key = os.environ.get(ENV_GEMINI_API_KEY)
        os.environ.pop(ENV_GEMINI_API_KEY, None)

    def tearDown(self) -> None:
        if self._saved_key is None:
            os.environ.pop(ENV_GEMINI_API_KEY, None)
        else:
            os.environ[ENV_GEMINI_API_KEY] = self._saved_key
    ...
    def test_provision_attaches_gateway_when_key_present(self) -> None:
        os.environ[ENV_GEMINI_API_KEY] = "test-key"
        router = HealingRouter()
