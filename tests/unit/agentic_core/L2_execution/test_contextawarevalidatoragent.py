#!/usr/bin/env python3
"""
Unit tests for ContextAwareValidatorAgent.
...
class TestContextAwareValidatorAgent:
    """Test suite for ContextAwareValidatorAgent."""

    def test_class_exists(self):
        """Verify the class can be imported."""
        try:
            from agentic_core.L2_execution.ToolRegistry.ContextAwareValidatorAgent import ContextAwareValidatorAgent
            assert ContextAwareValidatorAgent is not None
    ...
    def test_class_is_agent(self):
        ...
        try:
            from agentic_core.L2_execution.ToolRegistry.ContextAwareValidatorAgent import ContextAwareValidatorAgent
            # Check it's a class
            assert isinstance(ContextAwareValidatorAgent, type)
        except ImportError:
    ...
    def test_has_healing_capability(self):
        ...
        try:
            ...
            has_heal = hasattr(ContextAwareValidatorAgent, 'heal_repository') or \
                       any('heal' in str(m).lower() for m in dir(ContextAwareValidatorAgent))
    ...
    def test_key_methods_exist(self):
        ...
        try:
            ...
            methods = [m for m in dir(ContextAwareValidatorAgent) if not m.startswith('_')]
