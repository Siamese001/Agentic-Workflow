"""Proof-evidence test for 10C-REQ-171 (04_L2_Execute).
...
Source        : agentic_process_mapping_v29.md :: [4] L2 EXECUTE

Canonical requirement (paraphrased): L2 execution MUST: Execute tools actions interface external systems produce output with no routing no human no durable write

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-171]: ExecutionResult emitted without satisfying 'L2 execution MUST: Execute tools actions interface external systems ...
...
def _valid_artifact() -> dict:
    return {
        ...
        "blueprint_hash": "blueprint-171-h",
        "policy_hash": "policy-171-h",
        "tool_calls": [],
        "side_effects_proposed": [],
        "replay_key": "replay-171-k",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-171-001",
        "trace_id": "trace-171-001",
        "request_id": "req-171-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-171-h",
        "blueprint_hash": "blueprint-171-h",
        "replay_key": "replay-171-k",
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-171: per ledger row, a ExecutionResult that violates the canonical
