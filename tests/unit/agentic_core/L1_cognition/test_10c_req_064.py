"""Proof-evidence test for 10C-REQ-064 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: THE THINKING DESK L1 REASONING LOOP

Canonical requirement (paraphrased): L1 reasoning invariant: Internal non-linearity stays within L1 only L1 can draft inspect refine simplify or abstain but cannot execute retri...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-064]: L1PlanContract emitted without satisfying 'L1 reasoning invariant: Internal non-linearity stays within L1 only L...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-64-001",
        "trace_id": "trace-64-001",
        "request_id": "req-64-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-64-h",
        "blueprint_hash": "blueprint-64-h",
        "replay_key": "replay-64-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-64-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-064: per ledger row, a L1PlanContract that violates the canonical
