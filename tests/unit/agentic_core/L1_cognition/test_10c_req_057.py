"""Proof-evidence test for 10C-REQ-057 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: I2 WHAT ARE THEIR RULES?

Canonical requirement (paraphrased): Rule parsing MUST: Extract hard constraints soft constraints scope exclusions must/should/avoid classifications

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-057]: L1PlanContract emitted without satisfying 'Rule parsing MUST: Extract hard constraints soft constraints scope ex...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-57-001",
        "trace_id": "trace-57-001",
        "request_id": "req-57-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-57-h",
        "blueprint_hash": "blueprint-57-h",
        "replay_key": "replay-57-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-57-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-057: per ledger row, a L1PlanContract that violates the canonical
