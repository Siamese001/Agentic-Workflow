"""Proof-evidence test for 10C-REQ-058 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: I3 SPECIFIC DETAILS

Canonical requirement (paraphrased): Detail parsing MUST: Extract entities actors exact numbers variables required output format explicit deliverable

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-058]: L1PlanContract emitted without satisfying 'Detail parsing MUST: Extract entities actors exact numbers variables ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-58-001",
        "trace_id": "trace-58-001",
        "request_id": "req-58-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-58-h",
        "blueprint_hash": "blueprint-58-h",
        "replay_key": "replay-58-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-58-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-058: per ledger row, a L1PlanContract that violates the canonical
