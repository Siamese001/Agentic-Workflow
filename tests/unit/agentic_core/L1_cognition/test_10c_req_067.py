"""Proof-evidence test for 10C-REQ-067 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: P3 PICK THE AISLES

Canonical requirement (paraphrased): Route proposal MUST: Identify proposed routes only assess grounding likelihood assess external action likelihood note no route authority at ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-067]: L1PlanContract emitted without satisfying 'Route proposal MUST: Identify proposed routes only assess grounding l...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-67-001",
        "trace_id": "trace-67-001",
        "request_id": "req-67-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-67-h",
        "blueprint_hash": "blueprint-67-h",
        "replay_key": "replay-67-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-67-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-067: per ledger row, a L1PlanContract that violates the canonical
