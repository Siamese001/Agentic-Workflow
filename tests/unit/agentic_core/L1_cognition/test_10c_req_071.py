"""Proof-evidence test for 10C-REQ-071 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: V3 DOES IT MAKE SENSE?

Canonical requirement (paraphrased): Logic validation MUST: Check dependencies resolve coherent sub-task order admits unknowns

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-071]: L1PlanContract emitted without satisfying 'Logic validation MUST: Check dependencies resolve coherent sub-task o...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-71-001",
        "trace_id": "trace-71-001",
        "request_id": "req-71-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-71-h",
        "blueprint_hash": "blueprint-71-h",
        "replay_key": "replay-71-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-71-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-071: per ledger row, a L1PlanContract that violates the canonical
