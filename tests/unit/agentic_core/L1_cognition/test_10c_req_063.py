"""Proof-evidence test for 10C-REQ-063 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: M4 PRE-APPROVED TEMPLATES

Canonical requirement (paraphrased): Template load MUST: Load bounded plan archetypes safe decomposition habits stopping rules lowest viable agency from L4 read-only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-063]: L1PlanContract emitted without satisfying 'Template load MUST: Load bounded plan archetypes safe decomposition h...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-63-001",
        "trace_id": "trace-63-001",
        "request_id": "req-63-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-63-h",
        "blueprint_hash": "blueprint-63-h",
        "replay_key": "replay-63-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-63-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-063: per ledger row, a L1PlanContract that violates the canonical
