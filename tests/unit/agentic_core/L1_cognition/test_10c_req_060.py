"""Proof-evidence test for 10C-REQ-060 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: M1 STANDARD CHECKLISTS

Canonical requirement (paraphrased): Memory load MUST: Load task schemas routing heuristics output contracts normal plan patterns from L4 read-only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-060]: L1PlanContract emitted without satisfying 'Memory load MUST: Load task schemas routing heuristics output contrac...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-60-001",
        "trace_id": "trace-60-001",
        "request_id": "req-60-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-60-h",
        "blueprint_hash": "blueprint-60-h",
        "replay_key": "replay-60-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-60-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-060: per ledger row, a L1PlanContract that violates the canonical
