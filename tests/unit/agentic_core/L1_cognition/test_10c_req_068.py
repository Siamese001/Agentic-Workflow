"""Proof-evidence test for 10C-REQ-068 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: P4 WRITE THE DRAFT PLAN

Canonical requirement (paraphrased): Plan draft MUST: Produce proposed_route query_spec task_spec route_risk confidence grounding_required missing_info/clarify_markers answer_su...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-068]: L1PlanContract emitted without satisfying 'Plan draft MUST: Produce proposed_route query_spec task_spec route_ri...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-68-001",
        "trace_id": "trace-68-001",
        "request_id": "req-68-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-68-h",
        "blueprint_hash": "blueprint-68-h",
        "replay_key": "replay-68-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-68-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-068: per ledger row, a L1PlanContract that violates the canonical
