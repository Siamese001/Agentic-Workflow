"""Proof-evidence test for 10C-REQ-168 (01_U0_Request_Intake).
...
Source        : agentic_process_mapping_v29.md :: [1] REQUEST INTAKE

Canonical requirement (paraphrased): Request intake MUST: Perform ingress validation auth identity rate limiting with NO semantic routing

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-168]: ValidatedRequest emitted without satisfying 'Request intake MUST: Perform ingress validation auth identity rate ...
...
def _valid_artifact() -> dict:
    return {
        "request_id": "req-168-001",
        "session_id": "sess-168-001",
        "trace_root": "trace-168-001",
        "tenant": "tenant-A",
        "transport": "https",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-168-001",
        "trace_id": "trace-168-001",
        "request_id": "req-168-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-168-h",
        "blueprint_hash": "blueprint-168-h",
        "replay_key": "replay-168-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-168-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-168: per ledger row, a ValidatedRequest that violates the canonical
