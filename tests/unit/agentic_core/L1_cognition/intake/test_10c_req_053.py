"""Proof-evidence test for 10C-REQ-053 (01_U0_Request_Intake).

Severity      : CRITICAL
Surface       : 01_U0_Request_Intake
Layer owner   : L5/gateway
Artifact      : ValidatedRequest
OTEL span     : u0.intake.schema_validated
Source        : 01_request_intake.md :: E4 DID YOU FILL OUT THE FORM?

Canonical requirement (paraphrased): Schema validation MUST: Validate schema/envelope presence check required fields reject malformed accept supported shapes only attach rejecti...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-053]: ValidatedRequest emitted without satisfying 'Schema validation MUST: Validate schema/envelope presence check req...
...
EXPECTED_SPAN = "u0.intake.schema_validated"
...
def _valid_artifact() -> dict:
    return {
        "request_id": "req-53-001",
        "session_id": "sess-53-001",
        "trace_root": "trace-53-001",
        "tenant": "tenant-A",
        "transport": "https",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-53-001",
        "trace_id": "trace-53-001",
        "request_id": "req-53-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-53-h",
        "blueprint_hash": "blueprint-53-h",
        "replay_key": "replay-53-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-53-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The u0.intake.schema_validated span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-053: per ledger row, a ValidatedRequest that violates the canonical
