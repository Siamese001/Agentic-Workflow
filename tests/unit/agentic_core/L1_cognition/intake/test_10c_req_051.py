"""Proof-evidence test for 10C-REQ-051 (01_U0_Request_Intake).

Severity      : CRITICAL
Surface       : 01_U0_Request_Intake
Layer owner   : L5/gateway
Artifact      : ValidatedRequest
OTEL span     : u0.intake.identity_bound
Source        : 01_request_intake.md :: E2 CHECKING LIBRARY CARDS

Canonical requirement (paraphrased): Auth validation MUST: Verify auth/identity bind caller/tenant establish access-rights baseline check banned/allowed class set region/scope b...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-051]: ValidatedRequest emitted without satisfying 'Auth validation MUST: Verify auth/identity bind caller/tenant estab...
...
EXPECTED_SPAN = "u0.intake.identity_bound"
...
def _valid_artifact() -> dict:
    return {
        "request_id": "req-51-001",
        "session_id": "sess-51-001",
        "trace_root": "trace-51-001",
        "tenant": "tenant-A",
        "transport": "https",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-51-001",
        "trace_id": "trace-51-001",
        "request_id": "req-51-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-51-h",
        "blueprint_hash": "blueprint-51-h",
        "replay_key": "replay-51-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-51-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The u0.intake.identity_bound span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-051: per ledger row, a ValidatedRequest that violates the canonical
