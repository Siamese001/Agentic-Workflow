"""Proof-evidence test for 10C-REQ-055 (01_U0_Request_Intake).

Severity      : CRITICAL
Surface       : 01_U0_Request_Intake
Layer owner   : L5/gateway
Artifact      : ValidatedRequest
OTEL span     : u0.intake.stamped
Source        : 01_request_intake.md :: E6 STAMPING THE TICKET

Canonical requirement (paraphrased): Stamping MUST: Produce validated_request stamp request_id trace_root caller_scope_baseline ingress_time tenancy mark safe for staff reading

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-055]: ValidatedRequest emitted without satisfying 'Stamping MUST: Produce validated_request stamp request_id trace_roo...
...
EXPECTED_SPAN = "u0.intake.stamped"
...
def _valid_artifact() -> dict:
    return {
        "request_id": "req-55-001",
        "session_id": "sess-55-001",
        "trace_root": "trace-55-001",
        "tenant": "tenant-A",
        "transport": "https",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-55-001",
        "trace_id": "trace-55-001",
        "request_id": "req-55-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-55-h",
        "blueprint_hash": "blueprint-55-h",
        "replay_key": "replay-55-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-55-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The u0.intake.stamped span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-055: per ledger row, a ValidatedRequest that violates the canonical
