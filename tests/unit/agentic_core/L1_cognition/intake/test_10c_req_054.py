"""Proof-evidence test for 10C-REQ-054 (01_U0_Request_Intake).

Severity      : CRITICAL
Surface       : 01_U0_Request_Intake
Layer owner   : L5/gateway
Artifact      : ValidatedRequest
OTEL span     : u0.intake.payload_normalized
Source        : 01_request_intake.md :: E5 FIXING MESSY HANDWRITING

Canonical requirement (paraphrased): Normalization MUST: Normalize encoding produce bounded clean payload tidy malformed spacing normalize delimiters casing strip unsupported tr...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-054]: ValidatedRequest emitted without satisfying 'Normalization MUST: Normalize encoding produce bounded clean payloa...
...
EXPECTED_SPAN = "u0.intake.payload_normalized"
...
def _valid_artifact() -> dict:
    return {
        "request_id": "req-54-001",
        "session_id": "sess-54-001",
        "trace_root": "trace-54-001",
        "tenant": "tenant-A",
        "transport": "https",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-54-001",
        "trace_id": "trace-54-001",
        "request_id": "req-54-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-54-h",
        "blueprint_hash": "blueprint-54-h",
        "replay_key": "replay-54-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-54-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The u0.intake.payload_normalized span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-054: per ledger row, a ValidatedRequest that violates the canonical
