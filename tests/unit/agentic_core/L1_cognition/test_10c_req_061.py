"""Proof-evidence test for 10C-REQ-061 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: M2 SAFETY POLICY

Canonical requirement (paraphrased): Policy load MUST: Load compliance bounds escalation thresholds disallowed actions policy-safe bounds from L4 read-only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-061]: L1PlanContract emitted without satisfying 'Policy load MUST: Load compliance bounds escalation thresholds disall...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-61-001",
        "trace_id": "trace-61-001",
        "request_id": "req-61-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-61-h",
        "blueprint_hash": "blueprint-61-h",
        "replay_key": "replay-61-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-61-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-061: per ledger row, a L1PlanContract that violates the canonical
