"""Proof-evidence test for 10C-REQ-059 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: I4 WHAT KIND OF JOB IS THIS?

Canonical requirement (paraphrased): Job classification MUST: Classify job type summarize/compare/analyze/plan/act before planning work class drives plan structure

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-059]: L1PlanContract emitted without satisfying 'Job classification MUST: Classify job type summarize/compare/analyze/...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-59-001",
        "trace_id": "trace-59-001",
        "request_id": "req-59-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-59-h",
        "blueprint_hash": "blueprint-59-h",
        "replay_key": "replay-59-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-59-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-059: per ledger row, a L1PlanContract that violates the canonical
