"""Proof-evidence test for 10C-REQ-174 (02_L1_Reasoning_Plan).
...
Source        : agentic_process_mapping_v38.md + best-practice gap analysis 2026-04-29 :: HITL-NEW-A risk-tier classifier

Canonical requirement (paraphrased): L1 plan output contract MUST emit risk_tier in {T1_auto, T2_approve, T3_hard_stop} deterministically derived from (side_effect_class x rever...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-174]: L1PlanContract emitted without satisfying 'L1 plan output contract MUST emit risk_tier in {T1_auto, T2_approve, ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-174-001",
        "trace_id": "trace-174-001",
        "request_id": "req-174-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-174-h",
        "blueprint_hash": "blueprint-174-h",
        "replay_key": "replay-174-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-174-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-174: per ledger row, a L1PlanContract that violates the canonical
