"""Proof-evidence test for 10C-REQ-072 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: V4 CAN IT BE SIMPLER?

Canonical requirement (paraphrased): Simplicity validation MUST: Check lowest viable agency no over-complication erase and restart if weak

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-072]: L1PlanContract emitted without satisfying 'Simplicity validation MUST: Check lowest viable agency no over-compli...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-72-001",
        "trace_id": "trace-72-001",
        "request_id": "req-72-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-72-h",
        "blueprint_hash": "blueprint-72-h",
        "replay_key": "replay-72-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-72-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-072: per ledger row, a L1PlanContract that violates the canonical
