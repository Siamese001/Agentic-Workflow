"""Proof-evidence test for 10C-REQ-180 (02_L1_Reasoning_Plan).

Severity      : CRITICAL
Surface       : 02_L1_Reasoning_Plan
Layer owner   : L1 cognition + L3 orchestration
Artifact      : L1PlanContract
OTEL span     : l1.plan.contract_emitted
Source        : best-practice gap analysis 2026-04-30 wave2 C2 context engineering :: C2-NEW-A tool-result clearing gate

Canonical requirement (paraphrased): PA.5 budget enforcement MUST trigger tool-result clearing (replace consumed tool outputs with lightweight stubs preserving tool_call_id and ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-180]: L1PlanContract emitted without satisfying 'PA.5 budget enforcement MUST trigger tool-result clearing (replace co...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-180-001",
        "trace_id": "trace-180-001",
        "request_id": "req-180-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-180-h",
        "blueprint_hash": "blueprint-180-h",
        "replay_key": "replay-180-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-180-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-180: per ledger row, a L1PlanContract that violates the canonical
