"""Proof-evidence test for 10C-REQ-184 (02_L1_Reasoning_Plan).

Severity      : CRITICAL
Surface       : 02_L1_Reasoning_Plan
Layer owner   : L3 orchestration + L1 cognition
Artifact      : L1PlanContract
OTEL span     : l1.plan.contract_emitted
Source        : best-practice gap analysis 2026-04-30 wave2 C2 context engineering :: C2-NEW-E citation chain across compaction

Canonical requirement (paraphrased): CompactionResult MUST include a citation_chain field of type list[CitationMapping] where each CitationMapping carries (original_chunk_id, co...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-184]: L1PlanContract emitted without satisfying 'CompactionResult MUST include a citation_chain field of type list[Cit...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-184-001",
        "trace_id": "trace-184-001",
        "request_id": "req-184-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-184-h",
        "blueprint_hash": "blueprint-184-h",
        "replay_key": "replay-184-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-184-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-184: per ledger row, a L1PlanContract that violates the canonical
