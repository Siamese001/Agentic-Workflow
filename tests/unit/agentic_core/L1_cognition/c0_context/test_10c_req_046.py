"""Proof-evidence test for 10C-REQ-046 (03A_C0_Context_Engine).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [11] GOVERNANCE FILTERS

Canonical requirement (paraphrased): Governance filters MUST: Check ACL and freshness on hybrid-merged candidates before hydration

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-046]: FinalEvidenceContract emitted without satisfying 'Governance filters MUST: Check ACL and freshness on hybrid-mer...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-46-001",
        "evidence_chain": [
            {"ref": "evidence-46-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-46-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-46-1", "evidence_ref": "evidence-46-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-46-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-46-001",
        "trace_id": "trace-46-001",
        "request_id": "req-46-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-46-h",
        "blueprint_hash": "blueprint-46-h",
        "replay_key": "replay-46-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-46-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-046: per ledger row, a FinalEvidenceContract that violates the canonical
