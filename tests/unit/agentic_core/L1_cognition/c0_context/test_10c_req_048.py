"""Proof-evidence test for 10C-REQ-048 (03A_C0_Context_Engine).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [13] C0 OUTPUT

Canonical requirement (paraphrased): C0 output MUST: Yield verified chunks with grounding ready for prompt assembly

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-048]: FinalEvidenceContract emitted without satisfying 'C0 output MUST: Yield verified chunks with grounding ready for...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-48-001",
        "evidence_chain": [
            {"ref": "evidence-48-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-48-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-48-1", "evidence_ref": "evidence-48-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-48-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-48-001",
        "trace_id": "trace-48-001",
        "request_id": "req-48-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-48-h",
        "blueprint_hash": "blueprint-48-h",
        "replay_key": "replay-48-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-48-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-048: per ledger row, a FinalEvidenceContract that violates the canonical
