"""Proof-evidence test for 10C-REQ-082 (03A_C0_Context_Engine).
...
Source        : 03_Route_Decision_Switching.md :: C0.2 EVIDENCE FETCH

Canonical requirement (paraphrased): C0.2 evidence fetch MUST: Execute dense recall query_vec vs stored vectors sparse recall exact term/code/schema match metadata hydration par...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-082]: FinalEvidenceContract emitted without satisfying 'C0.2 evidence fetch MUST: Execute dense recall query_vec vs st...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-82-001",
        "evidence_chain": [
            {"ref": "evidence-82-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-82-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-82-1", "evidence_ref": "evidence-82-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-82-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-82-001",
        "trace_id": "trace-82-001",
        "request_id": "req-82-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-82-h",
        "blueprint_hash": "blueprint-82-h",
        "replay_key": "replay-82-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-82-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-082: per ledger row, a FinalEvidenceContract that violates the canonical
