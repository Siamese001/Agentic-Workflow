"""Proof-evidence test for 10C-REQ-013 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/chunking
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.chunk.sealed
Source        : 00A_ingestion_embedding_implementation.md :: A2. CHUNKING

Canonical requirement (paraphrased): Chunking MUST implement strategies: section-aware semantic-object overlap-window event-boundary chunking with chunk_001...chunk_n output

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-013]: ChunkSealedEnvelope emitted without satisfying 'Chunking MUST implement strategies: section-aware semantic-objec...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-13-001",
        "trace_id": "trace-13-001",
        "request_id": "req-13-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-13-h",
        "blueprint_hash": "blueprint-13-h",
        "replay_key": "replay-13-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-13-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-013: per ledger row, a ChunkSealedEnvelope that violates the canonical
