"""Proof-evidence test for 10C-REQ-040 (Offline_Ingestion_Index_Build).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [5] INVERTED INDEX BUILD

Canonical requirement (paraphrased): Inverted index build MUST map: term/phrase -> postings list [chunk_id field position weight] creating searchable inverted index

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-040]: ChunkSealedEnvelope emitted without satisfying 'Inverted index build MUST map: term/phrase -> postings list [chu...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-40-001",
        "trace_id": "trace-40-001",
        "request_id": "req-40-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-40-h",
        "blueprint_hash": "blueprint-40-h",
        "replay_key": "replay-40-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-40-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-040: per ledger row, a ChunkSealedEnvelope that violates the canonical
