"""Proof-evidence test for 10C-REQ-036 (Offline_Ingestion_Index_Build).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [1] NORMALIZE

Canonical requirement (paraphrased): Sparse index build MUST normalize text copy before term extraction

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-036]: ChunkSealedEnvelope emitted without satisfying 'Sparse index build MUST normalize text copy before term extracti...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-36-001",
        "trace_id": "trace-36-001",
        "request_id": "req-36-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-36-h",
        "blueprint_hash": "blueprint-36-h",
        "replay_key": "replay-36-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-36-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-036: per ledger row, a ChunkSealedEnvelope that violates the canonical
