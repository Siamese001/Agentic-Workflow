"""Proof-evidence test for 10C-REQ-026 (Offline_Ingestion_Index_Build).
...
Source        : 00B_token_vector_mechanics.md :: B6. POOLING PROJECTION

Canonical requirement (paraphrased): Pooling MUST execute: Mean pool or CLS pool or model-specific projection converting token hidden states -> one chunk-level semantic vector o...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-026]: ChunkSealedEnvelope emitted without satisfying 'Pooling MUST execute: Mean pool or CLS pool or model-specific pr...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-26-001",
        "trace_id": "trace-26-001",
        "request_id": "req-26-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-26-h",
        "blueprint_hash": "blueprint-26-h",
        "replay_key": "replay-26-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-26-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-026: per ledger row, a ChunkSealedEnvelope that violates the canonical
