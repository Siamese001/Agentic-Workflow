"""Proof-evidence test for 10C-REQ-003 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/lifecycle
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.chunk.sealed
Source        : 00_ingestion_pipeline_index_build.md :: [00.3] LIFECYCLE + STATE SYNC

Canonical requirement (paraphrased): Ingestion MUST implement lifecycle sync: dedupe checksum version compare tombstone stale data preserve graph lineage trigger reindex

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-003]: ChunkSealedEnvelope emitted without satisfying 'Ingestion MUST implement lifecycle sync: dedupe checksum version...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-3-001",
        "trace_id": "trace-3-001",
        "request_id": "req-3-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-3-h",
        "blueprint_hash": "blueprint-3-h",
        "replay_key": "replay-3-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-3-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-003: per ledger row, a ChunkSealedEnvelope that violates the canonical
