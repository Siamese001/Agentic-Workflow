"""Proof-evidence test for 10C-REQ-002 (Offline_Ingestion_Index_Build).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.2] CANONICAL RAW UNIT

Canonical requirement (paraphrased): Ingestion MUST create canonical raw unit with raw text source identifier parent identifier visual modality flag

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-002]: ChunkSealedEnvelope emitted without satisfying 'Ingestion MUST create canonical raw unit with raw text source id...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-2-001",
        "trace_id": "trace-2-001",
        "request_id": "req-2-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-2-h",
        "blueprint_hash": "blueprint-2-h",
        "replay_key": "replay-2-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-2-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-002: per ledger row, a ChunkSealedEnvelope that violates the canonical
