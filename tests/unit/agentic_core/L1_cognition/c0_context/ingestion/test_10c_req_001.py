"""Proof-evidence test for 10C-REQ-001 (Offline_Ingestion_Index_Build).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.1] INTAKE

Canonical requirement (paraphrased): Ingestion pipeline MUST detect content modality and route to text extractor or visual detector

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-001]: ChunkSealedEnvelope emitted without satisfying 'Ingestion pipeline MUST detect content modality and route to tex...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-1-001",
        "trace_id": "trace-1-001",
        "request_id": "req-1-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-1-h",
        "blueprint_hash": "blueprint-1-h",
        "replay_key": "replay-1-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-1-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-001: per ledger row, a ChunkSealedEnvelope that violates the canonical
