"""Proof-evidence test for 10C-REQ-022 (Offline_Ingestion_Index_Build).
...
Source        : 00B_token_vector_mechanics.md :: B2. TOKENIZATION

Canonical requirement (paraphrased): Tokenization MUST convert: text -> tokens -> token_ids with truncation padding special tokens applied producing discrete model inputs

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-022]: ChunkSealedEnvelope emitted without satisfying 'Tokenization MUST convert: text -> tokens -> token_ids with trun...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-22-001",
        "trace_id": "trace-22-001",
        "request_id": "req-22-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-22-h",
        "blueprint_hash": "blueprint-22-h",
        "replay_key": "replay-22-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-22-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-022: per ledger row, a ChunkSealedEnvelope that violates the canonical
