"""Proof-evidence test for 10C-REQ-007 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/embedding + L2 execution
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.chunk.sealed
Source        : 00_ingestion_pipeline_index_build.md :: [MODEL DISTINCTIONS]

Canonical requirement (paraphrased): Embedding model maps text to semantic space offline for document-side vectors; LLM predicts next token at runtime; Different objectives diff...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-007]: ChunkSealedEnvelope emitted without satisfying 'Embedding model maps text to semantic space offline for document...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-7-001",
        "trace_id": "trace-7-001",
        "request_id": "req-7-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-7-h",
        "blueprint_hash": "blueprint-7-h",
        "replay_key": "replay-7-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-7-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-007: per ledger row, a ChunkSealedEnvelope that violates the canonical
