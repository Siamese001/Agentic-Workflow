"""Proof-evidence test for 10C-REQ-015 (Offline_Ingestion_Index_Build).
...
Source        : 00A_ingestion_embedding_implementation.md :: A4. METADATA BINDING

Canonical requirement (paraphrased): Metadata binding MUST attach: doc_id chunk_id parent_id ACL tenant freshness schema_version provenance lineage producing ChunkManifest + Par...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-015]: ChunkSealedEnvelope emitted without satisfying 'Metadata binding MUST attach: doc_id chunk_id parent_id ACL tena...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-15-001",
        "trace_id": "trace-15-001",
        "request_id": "req-15-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-15-h",
        "blueprint_hash": "blueprint-15-h",
        "replay_key": "replay-15-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-15-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-015: per ledger row, a ChunkSealedEnvelope that violates the canonical
