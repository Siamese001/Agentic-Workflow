"""Proof-evidence test for 10C-REQ-014 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/enrichment
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.chunk.sealed
Source        : 00A_ingestion_embedding_implementation.md :: A3. SEMANTIC ENRICHMENT

Canonical requirement (paraphrased): Semantic enrichment MUST transform chunk into knowledge object with title summary concepts query_expansion lineage_hints producing raw text ...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-014]: ChunkSealedEnvelope emitted without satisfying 'Semantic enrichment MUST transform chunk into knowledge object w...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-14-001",
        "trace_id": "trace-14-001",
        "request_id": "req-14-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-14-h",
        "blueprint_hash": "blueprint-14-h",
        "replay_key": "replay-14-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-14-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-014: per ledger row, a ChunkSealedEnvelope that violates the canonical
