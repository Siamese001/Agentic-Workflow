"""Proof-evidence test for 10C-REQ-041 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/sparse_index
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.index.materialized
Source        : 00D_sparse_index_hybrid_merge.md :: [6] SPARSE STORE

Canonical requirement (paraphrased): Sparse store MUST store: postings lists for lexical retrieval

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-041]: ChunkSealedEnvelope emitted without satisfying 'Sparse store MUST store: postings lists for lexical retrieval' -...
...
EXPECTED_SPAN = "ingest.index.materialized"
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-41-001",
        "trace_id": "trace-41-001",
        "request_id": "req-41-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-41-h",
        "blueprint_hash": "blueprint-41-h",
        "replay_key": "replay-41-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-41-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The ingest.index.materialized span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-041: per ledger row, a ChunkSealedEnvelope that violates the canonical
