"""Proof-evidence test for 10C-REQ-017 (Offline_Ingestion_Index_Build).
...
Source        : 00A_ingestion_embedding_implementation.md :: A6. VECTOR RECORD ASSEMBLY

Canonical requirement (paraphrased): Vector record assembly MUST build row: UUID + fact_vec + chunk pointer + metadata pointer with invariant: Vector store is fast math shelf NO...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-017]: ChunkSealedEnvelope emitted without satisfying 'Vector record assembly MUST build row: UUID + fact_vec + chunk p...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-17-001",
        "trace_id": "trace-17-001",
        "request_id": "req-17-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-17-h",
        "blueprint_hash": "blueprint-17-h",
        "replay_key": "replay-17-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-17-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-017: per ledger row, a ChunkSealedEnvelope that violates the canonical
