"""Proof-evidence test for 10C-REQ-037 (Offline_Ingestion_Index_Build).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [2] EXTRACT

Canonical requirement (paraphrased): Extract MUST extract: IDs fields terms phrases symbols from normalized text

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-037]: ChunkSealedEnvelope emitted without satisfying 'Extract MUST extract: IDs fields terms phrases symbols from norm...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-37-001",
        "trace_id": "trace-37-001",
        "request_id": "req-37-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-37-h",
        "blueprint_hash": "blueprint-37-h",
        "replay_key": "replay-37-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-37-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-037: per ledger row, a ChunkSealedEnvelope that violates the canonical
