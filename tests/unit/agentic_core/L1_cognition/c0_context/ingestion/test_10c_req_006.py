"""Proof-evidence test for 10C-REQ-006 (Offline_Ingestion_Index_Build).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.6] DUAL REPRESENTATION

Canonical requirement (paraphrased): Ingestion MUST generate dual representations: raw_text_vector literal semantic map AND contextual_text_vector semantic overlay map from enri...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-006]: ChunkSealedEnvelope emitted without satisfying 'Ingestion MUST generate dual representations: raw_text_vector li...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-6-001",
        "trace_id": "trace-6-001",
        "request_id": "req-6-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-6-h",
        "blueprint_hash": "blueprint-6-h",
        "replay_key": "replay-6-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-6-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-006: per ledger row, a ChunkSealedEnvelope that violates the canonical
