"""Proof-evidence test for 10C-REQ-012 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/ingestion
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.chunk.sealed
Source        : 00A_ingestion_embedding_implementation.md :: A1. EXTRACTION

Canonical requirement (paraphrased): Extraction MUST convert raw bytes/pages/records into canonical source text with document text source_id source metadata

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-012]: ChunkSealedEnvelope emitted without satisfying 'Extraction MUST convert raw bytes/pages/records into canonical s...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-12-001",
        "trace_id": "trace-12-001",
        "request_id": "req-12-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-12-h",
        "blueprint_hash": "blueprint-12-h",
        "replay_key": "replay-12-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-12-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-012: per ledger row, a ChunkSealedEnvelope that violates the canonical
