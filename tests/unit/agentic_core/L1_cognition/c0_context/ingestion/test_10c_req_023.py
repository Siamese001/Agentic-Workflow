"""Proof-evidence test for 10C-REQ-023 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/embedding
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.bootstrap.completed
Source        : 00B_token_vector_mechanics.md :: B3. MODEL CHECKPOINT RESOLUTION

Canonical requirement (paraphrased): Model checkpoint resolution MUST locate: e.g. BAAI/bge-m3 from Hugging Face Hub or local cache at ~/.cache/huggingface/ producing pretrained...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-023]: ChunkSealedEnvelope emitted without satisfying 'Model checkpoint resolution MUST locate: e.g. BAAI/bge-m3 from H...
...
EXPECTED_SPAN = "ingest.bootstrap.completed"
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-23-001",
        "trace_id": "trace-23-001",
        "request_id": "req-23-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-23-h",
        "blueprint_hash": "blueprint-23-h",
        "replay_key": "replay-23-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-23-v")
    return attrs
...
def test_span_shape_positive() -> None:
    """The ingest.bootstrap.completed span must carry all required attrs."""
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-023: per ledger row, a ChunkSealedEnvelope that violates the canonical
