"""Proof-evidence test for 10C-REQ-021 (Offline_Ingestion_Index_Build).
...
Source        : 00B_token_vector_mechanics.md :: B1. TOKENIZER LOAD

Canonical requirement (paraphrased): Tokenizer load MUST load model-compatible tokenizer same model family as embedding encoder with token_ids + attention_mask output contract

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-021]: ChunkSealedEnvelope emitted without satisfying 'Tokenizer load MUST load model-compatible tokenizer same model f...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-21-001",
        "trace_id": "trace-21-001",
        "request_id": "req-21-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-21-h",
        "blueprint_hash": "blueprint-21-h",
        "replay_key": "replay-21-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-21-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-021: per ledger row, a ChunkSealedEnvelope that violates the canonical
