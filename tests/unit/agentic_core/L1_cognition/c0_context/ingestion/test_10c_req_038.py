"""Proof-evidence test for 10C-REQ-038 (Offline_Ingestion_Index_Build).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [3] TOKENIZE

Canonical requirement (paraphrased): Tokenize MUST tokenize: phrases symbols into searchable tokens

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-038]: ChunkSealedEnvelope emitted without satisfying 'Tokenize MUST tokenize: phrases symbols into searchable tokens' ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-38-001",
        "trace_id": "trace-38-001",
        "request_id": "req-38-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-38-h",
        "blueprint_hash": "blueprint-38-h",
        "replay_key": "replay-38-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-38-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-038: per ledger row, a ChunkSealedEnvelope that violates the canonical
