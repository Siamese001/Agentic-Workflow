"""Proof-evidence test for 10C-REQ-027 (Offline_Ingestion_Index_Build).
...
Source        : 00B_token_vector_mechanics.md :: B7. NORMALIZATION

Canonical requirement (paraphrased): Normalization MUST L2 normalize vector for cosine-friendly search producing stable retrieval-ready embedding

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-027]: ChunkSealedEnvelope emitted without satisfying 'Normalization MUST L2 normalize vector for cosine-friendly searc...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-27-001",
        "trace_id": "trace-27-001",
        "request_id": "req-27-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-27-h",
        "blueprint_hash": "blueprint-27-h",
        "replay_key": "replay-27-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-27-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-027: per ledger row, a ChunkSealedEnvelope that violates the canonical
