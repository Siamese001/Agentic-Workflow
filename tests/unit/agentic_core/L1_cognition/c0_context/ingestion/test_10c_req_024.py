"""Proof-evidence test for 10C-REQ-024 (Offline_Ingestion_Index_Build).
...
Source        : 00B_token_vector_mechanics.md :: B4. WEIGHT LOAD RAM ALLOCATION

Canonical requirement (paraphrased): Weight load MUST import trained neural network weights into RAM making model executable for forward pass

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-024]: ChunkSealedEnvelope emitted without satisfying 'Weight load MUST import trained neural network weights into RAM ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-24-001",
        "trace_id": "trace-24-001",
        "request_id": "req-24-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-24-h",
        "blueprint_hash": "blueprint-24-h",
        "replay_key": "replay-24-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-24-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-024: per ledger row, a ChunkSealedEnvelope that violates the canonical
