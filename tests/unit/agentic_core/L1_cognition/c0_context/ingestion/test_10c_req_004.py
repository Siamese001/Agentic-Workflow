"""Proof-evidence test for 10C-REQ-004 (Offline_Ingestion_Index_Build).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.4] CHUNKING POLICY

Canonical requirement (paraphrased): Ingestion MUST implement corpus-aware chunking with four strategies: policy/long docs section-aware incident/trace event-boundary code/confi...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-004]: ChunkSealedEnvelope emitted without satisfying 'Ingestion MUST implement corpus-aware chunking with four strateg...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-4-001",
        "trace_id": "trace-4-001",
        "request_id": "req-4-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-4-h",
        "blueprint_hash": "blueprint-4-h",
        "replay_key": "replay-4-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-4-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-004: per ledger row, a ChunkSealedEnvelope that violates the canonical
