"""Proof-evidence test for 10C-REQ-039 (Offline_Ingestion_Index_Build).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [4] WEIGHT

Canonical requirement (paraphrased): Weight MUST apply: title boost field boost to extracted terms

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-039]: ChunkSealedEnvelope emitted without satisfying 'Weight MUST apply: title boost field boost to extracted terms' -...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-39-001",
        "trace_id": "trace-39-001",
        "request_id": "req-39-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-39-h",
        "blueprint_hash": "blueprint-39-h",
        "replay_key": "replay-39-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-39-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-039: per ledger row, a ChunkSealedEnvelope that violates the canonical
