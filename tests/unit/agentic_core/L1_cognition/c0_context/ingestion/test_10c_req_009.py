"""Proof-evidence test for 10C-REQ-009 (Offline_Ingestion_Index_Build).

Severity      : CRITICAL
Surface       : Offline_Ingestion_Index_Build
Layer owner   : knowledge/evaluation
Artifact      : ChunkSealedEnvelope
OTEL span     : ingest.chunk.sealed
Source        : 00_ingestion_pipeline_index_build.md :: [00.8] INDEX EVAL + FEEDBACK

Canonical requirement (paraphrased): Ingestion MUST implement evaluation feedback: Recall@K NDCG MRR citation_precision support_rate drift/staleness detection reindex_trigger em...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-009]: ChunkSealedEnvelope emitted without satisfying 'Ingestion MUST implement evaluation feedback: Recall@K NDCG MRR ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-9-001",
        "trace_id": "trace-9-001",
        "request_id": "req-9-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-9-h",
        "blueprint_hash": "blueprint-9-h",
        "replay_key": "replay-9-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-9-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-009: per ledger row, a ChunkSealedEnvelope that violates the canonical
