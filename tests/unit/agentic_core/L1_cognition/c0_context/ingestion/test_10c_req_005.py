"""Proof-evidence test for 10C-REQ-005 (Offline_Ingestion_Index_Build).
...
Source        : 00_ingestion_pipeline_index_build.md :: [00.5] METADATA BINDING

Canonical requirement (paraphrased): Ingestion MUST bind metadata BEFORE embedding: ACL tenant_id confidentiality_tier freshness_band effective_date expiry_date embedding_schema...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-005]: ChunkSealedEnvelope emitted without satisfying 'Ingestion MUST bind metadata BEFORE embedding: ACL tenant_id con...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-5-001",
        "trace_id": "trace-5-001",
        "request_id": "req-5-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-5-h",
        "blueprint_hash": "blueprint-5-h",
        "replay_key": "replay-5-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-5-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-005: per ledger row, a ChunkSealedEnvelope that violates the canonical
