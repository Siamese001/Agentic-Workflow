"""Proof-evidence test for 10C-REQ-025 (Offline_Ingestion_Index_Build).
...
Source        : 00B_token_vector_mechanics.md :: B5. FORWARD PASS

Canonical requirement (paraphrased): Forward pass MUST execute: token_ids + attention_mask + weights -> context hidden states with every token getting learned contextual represe...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-025]: ChunkSealedEnvelope emitted without satisfying 'Forward pass MUST execute: token_ids + attention_mask + weights ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-25-001",
        "trace_id": "trace-25-001",
        "request_id": "req-25-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-25-h",
        "blueprint_hash": "blueprint-25-h",
        "replay_key": "replay-25-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-25-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-025: per ledger row, a ChunkSealedEnvelope that violates the canonical
