"""Proof-evidence test for 10C-REQ-016 (Offline_Ingestion_Index_Build).
...
Source        : 00A_ingestion_embedding_implementation.md :: A5. EMBEDDING GENERATION

Canonical requirement (paraphrased): Embedding generation MUST execute: tokenizer -> model weights -> encoder forward pass -> pooling/projection -> chunk_vector with optional se...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-016]: ChunkSealedEnvelope emitted without satisfying 'Embedding generation MUST execute: tokenizer -> model weights ->...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-16-001",
        "trace_id": "trace-16-001",
        "request_id": "req-16-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-16-h",
        "blueprint_hash": "blueprint-16-h",
        "replay_key": "replay-16-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-16-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-016: per ledger row, a ChunkSealedEnvelope that violates the canonical
