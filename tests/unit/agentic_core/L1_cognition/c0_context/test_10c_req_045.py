"""Proof-evidence test for 10C-REQ-045 (03A_C0_Context_Engine).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [10] HYBRID MERGE

Canonical requirement (paraphrased): Hybrid merge MUST: Combine sparse and dense results via union and rerank with sparse wins on IDs exact matches

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-045]: FinalEvidenceContract emitted without satisfying 'Hybrid merge MUST: Combine sparse and dense results via union ...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-45-001",
        "evidence_chain": [
            {"ref": "evidence-45-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-45-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-45-1", "evidence_ref": "evidence-45-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-45-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-45-001",
        "trace_id": "trace-45-001",
        "request_id": "req-45-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-45-h",
        "blueprint_hash": "blueprint-45-h",
        "replay_key": "replay-45-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-45-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-045: per ledger row, a FinalEvidenceContract that violates the canonical
