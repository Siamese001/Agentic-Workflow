"""Proof-evidence test for 10C-REQ-081 (03A_C0_Context_Engine).
...
Source        : 03_Route_Decision_Switching.md :: C0.1 RETRIEVAL PLAN

Canonical requirement (paraphrased): C0.1 retrieval plan MUST: Bind source_scope freshness_window access/security_filters retrieval_mode with ACL prefilter version tenant bind r...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-081]: FinalEvidenceContract emitted without satisfying 'C0.1 retrieval plan MUST: Bind source_scope freshness_window a...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-81-001",
        "evidence_chain": [
            {"ref": "evidence-81-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-81-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-81-1", "evidence_ref": "evidence-81-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-81-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-81-001",
        "trace_id": "trace-81-001",
        "request_id": "req-81-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-81-h",
        "blueprint_hash": "blueprint-81-h",
        "replay_key": "replay-81-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-81-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-081: per ledger row, a FinalEvidenceContract that violates the canonical
