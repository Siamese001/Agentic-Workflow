"""Proof-evidence test for 10C-REQ-142 (03A_C0_Context_Engine).
...
Source        : C5_Retrieval_Prompt_Assembly.md :: C0.1 RETRIEVAL PLAN C5

Canonical requirement (paraphrased): C5 C0.1 MUST: Scope source freshness ACL retrieval mode with C0 retrieves only never routes never executes

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-142]: FinalEvidenceContract emitted without satisfying 'C5 C0.1 MUST: Scope source freshness ACL retrieval mode with C...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-142-001",
        "evidence_chain": [
            {"ref": "evidence-142-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-142-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-142-1", "evidence_ref": "evidence-142-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-142-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-142-001",
        "trace_id": "trace-142-001",
        "request_id": "req-142-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-142-h",
        "blueprint_hash": "blueprint-142-h",
        "replay_key": "replay-142-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-142-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-142: per ledger row, a FinalEvidenceContract that violates the canonical
