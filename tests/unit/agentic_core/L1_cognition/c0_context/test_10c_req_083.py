"""Proof-evidence test for 10C-REQ-083 (03A_C0_Context_Engine).

Severity      : CRITICAL
Surface       : 03A_C0_Context_Engine
Layer owner   : C0 context engine
Artifact      : FinalEvidenceContract
OTEL span     : c0.evidence.contract_emitted
Source        : 03_Route_Decision_Switching.md :: C0.3 EVIDENCE SHAPING

Canonical requirement (paraphrased): C0.3 evidence shaping MUST: Deduplicate parent-child expand preserve provenance rerank by support retain contradictions preserve citations s...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-083]: FinalEvidenceContract emitted without satisfying 'C0.3 evidence shaping MUST: Deduplicate parent-child expand pr...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-83-001",
        "evidence_chain": [
            {"ref": "evidence-83-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-83-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-83-1", "evidence_ref": "evidence-83-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-83-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-83-001",
        "trace_id": "trace-83-001",
        "request_id": "req-83-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-83-h",
        "blueprint_hash": "blueprint-83-h",
        "replay_key": "replay-83-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-83-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-083: per ledger row, a FinalEvidenceContract that violates the canonical
