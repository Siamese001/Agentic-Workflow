"""Proof-evidence test for 10C-REQ-032 (03A_C0_Context_Engine).
...
Source        : 00C_index_materialization_runtime_handoff.md :: C4. C0 RETRIEVAL PLAN

Canonical requirement (paraphrased): Retrieval plan C0.1 MUST bind: source_scope ACL freshness tenant version retrieval_mode with strict rule: retrieval authority ONLY no routin...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-032]: FinalEvidenceContract emitted without satisfying 'Retrieval plan C0.1 MUST bind: source_scope ACL freshness tena...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-32-001",
        "evidence_chain": [
            {"ref": "evidence-32-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-32-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-32-1", "evidence_ref": "evidence-32-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-32-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-32-001",
        "trace_id": "trace-32-001",
        "request_id": "req-32-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-32-h",
        "blueprint_hash": "blueprint-32-h",
        "replay_key": "replay-32-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-32-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-032: per ledger row, a FinalEvidenceContract that violates the canonical
