"""Proof-evidence test for 10C-REQ-047 (03A_C0_Context_Engine).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [12] HYDRATE

Canonical requirement (paraphrased): Hydrate MUST: Fetch raw text citations lineage edges from canonical store for governance-filtered candidates

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-047]: FinalEvidenceContract emitted without satisfying 'Hydrate MUST: Fetch raw text citations lineage edges from cano...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-47-001",
        "evidence_chain": [
            {"ref": "evidence-47-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-47-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-47-1", "evidence_ref": "evidence-47-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-47-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-47-001",
        "trace_id": "trace-47-001",
        "request_id": "req-47-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-47-h",
        "blueprint_hash": "blueprint-47-h",
        "replay_key": "replay-47-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-47-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-047: per ledger row, a FinalEvidenceContract that violates the canonical
