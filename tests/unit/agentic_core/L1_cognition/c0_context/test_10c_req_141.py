"""Proof-evidence test for 10C-REQ-141 (03A_C0_Context_Engine).
...
Source        : C5_Retrieval_Prompt_Assembly.md :: OFFLINE CATALOG BUILD

Canonical requirement (paraphrased): C5 offline MUST: Chunk tag index meaning vectors keyword lexical lineage graph

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-141]: FinalEvidenceContract emitted without satisfying 'C5 offline MUST: Chunk tag index meaning vectors keyword lexic...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-141-001",
        "evidence_chain": [
            {"ref": "evidence-141-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-141-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-141-1", "evidence_ref": "evidence-141-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-141-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-141-001",
        "trace_id": "trace-141-001",
        "request_id": "req-141-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-141-h",
        "blueprint_hash": "blueprint-141-h",
        "replay_key": "replay-141-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-141-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-141: per ledger row, a FinalEvidenceContract that violates the canonical
