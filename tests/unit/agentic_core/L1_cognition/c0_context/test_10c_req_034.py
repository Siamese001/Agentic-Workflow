"""Proof-evidence test for 10C-REQ-034 (03A_C0_Context_Engine).
...
Source        : 00C_index_materialization_runtime_handoff.md :: C6. EVIDENCE SHAPING

Canonical requirement (paraphrased): Evidence shaping C0.3 MUST: Deduplicate rerank by support preserve provenance citations retain contradictions preserve citations produce top...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-034]: FinalEvidenceContract emitted without satisfying 'Evidence shaping C0.3 MUST: Deduplicate rerank by support pres...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-34-001",
        "evidence_chain": [
            {"ref": "evidence-34-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-34-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-34-1", "evidence_ref": "evidence-34-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-34-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-34-001",
        "trace_id": "trace-34-001",
        "request_id": "req-34-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-34-h",
        "blueprint_hash": "blueprint-34-h",
        "replay_key": "replay-34-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-34-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-034: per ledger row, a FinalEvidenceContract that violates the canonical
