"""Proof-evidence test for 10C-REQ-044 (03A_C0_Context_Engine).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [9] DENSE PATH

Canonical requirement (paraphrased): Query-time dense path MUST handle: semantic queries paraphrase queries using vector database dependency

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-044]: FinalEvidenceContract emitted without satisfying 'Query-time dense path MUST handle: semantic queries paraphrase...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-44-001",
        "evidence_chain": [
            {"ref": "evidence-44-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-44-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-44-1", "evidence_ref": "evidence-44-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-44-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-44-001",
        "trace_id": "trace-44-001",
        "request_id": "req-44-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-44-h",
        "blueprint_hash": "blueprint-44-h",
        "replay_key": "replay-44-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-44-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-044: per ledger row, a FinalEvidenceContract that violates the canonical
