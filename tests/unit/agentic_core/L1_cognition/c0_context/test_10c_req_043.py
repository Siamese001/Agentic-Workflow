"""Proof-evidence test for 10C-REQ-043 (03A_C0_Context_Engine).
...
Source        : 00D_sparse_index_hybrid_merge.md :: [8] SPARSE PATH

Canonical requirement (paraphrased): Query-time sparse path MUST handle: literal queries quoted queries ID searches using postings lists dependency

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-043]: FinalEvidenceContract emitted without satisfying 'Query-time sparse path MUST handle: literal queries quoted que...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-43-001",
        "evidence_chain": [
            {"ref": "evidence-43-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-43-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-43-1", "evidence_ref": "evidence-43-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-43-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-43-001",
        "trace_id": "trace-43-001",
        "request_id": "req-43-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-43-h",
        "blueprint_hash": "blueprint-43-h",
        "replay_key": "replay-43-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-43-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-043: per ledger row, a FinalEvidenceContract that violates the canonical
