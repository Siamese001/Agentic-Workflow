"""Proof-evidence test for 10C-REQ-143 (03A_C0_Context_Engine).
...
Source        : C5_Retrieval_Prompt_Assembly.md :: C0.2 EVIDENCE FETCH C5

Canonical requirement (paraphrased): C5 C0.2 MUST: Fetch evidence dense sparse cache lookups metadata hydration parent-child expansion

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-143]: FinalEvidenceContract emitted without satisfying 'C5 C0.2 MUST: Fetch evidence dense sparse cache lookups metada...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-143-001",
        "evidence_chain": [
            {"ref": "evidence-143-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-143-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-143-1", "evidence_ref": "evidence-143-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-143-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-143-001",
        "trace_id": "trace-143-001",
        "request_id": "req-143-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-143-h",
        "blueprint_hash": "blueprint-143-h",
        "replay_key": "replay-143-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-143-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-143: per ledger row, a FinalEvidenceContract that violates the canonical
