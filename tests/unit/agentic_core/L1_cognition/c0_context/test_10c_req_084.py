"""Proof-evidence test for 10C-REQ-084 (03A_C0_Context_Engine).
...
Source        : 03_Route_Decision_Switching.md :: C0.4 EVIDENCE CONTRACT

Canonical requirement (paraphrased): C0.4 evidence contract MUST: Produce support scoring verified chunks cited spans source_ids dates coverage_score gaps contradiction_status a...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-084]: FinalEvidenceContract emitted without satisfying 'C0.4 evidence contract MUST: Produce support scoring verified ...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-84-001",
        "evidence_chain": [
            {"ref": "evidence-84-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-84-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-84-1", "evidence_ref": "evidence-84-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-84-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-84-001",
        "trace_id": "trace-84-001",
        "request_id": "req-84-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-84-h",
        "blueprint_hash": "blueprint-84-h",
        "replay_key": "replay-84-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-84-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-084: per ledger row, a FinalEvidenceContract that violates the canonical
