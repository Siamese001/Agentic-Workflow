"""Proof-evidence test for 10C-REQ-144 (03A_C0_Context_Engine).
...
Source        : C5_Retrieval_Prompt_Assembly.md :: C0.3 EVIDENCE SHAPING C5

Canonical requirement (paraphrased): C5 C0.3 MUST: Shape evidence deduplicate expand rerank preserve provenance retain contradictions

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-144]: FinalEvidenceContract emitted without satisfying 'C5 C0.3 MUST: Shape evidence deduplicate expand rerank preserv...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-144-001",
        "evidence_chain": [
            {"ref": "evidence-144-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-144-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-144-1", "evidence_ref": "evidence-144-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-144-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-144-001",
        "trace_id": "trace-144-001",
        "request_id": "req-144-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-144-h",
        "blueprint_hash": "blueprint-144-h",
        "replay_key": "replay-144-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-144-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-144: per ledger row, a FinalEvidenceContract that violates the canonical
