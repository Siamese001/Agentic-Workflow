"""Proof-evidence test for 10C-REQ-031 (03A_C0_Context_Engine).
...
Source        : 00C_index_materialization_runtime_handoff.md :: C3. PUBLISHED RETRIEVAL SURFACES

Canonical requirement (paraphrased): Published retrieval surfaces MUST include: Dense vectors Sparse lexical Lineage graph Canonical raw truth all unified as retrieval substrate

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-031]: FinalEvidenceContract emitted without satisfying 'Published retrieval surfaces MUST include: Dense vectors Spars...
...
def _valid_artifact() -> dict:
    return {
        "contract_id": "fec-31-001",
        "evidence_chain": [
            {"ref": "evidence-31-1", "doc_id": "doc-A", "span": "p1"},
            {"ref": "evidence-31-2", "doc_id": "doc-B", "span": "p2"},
        ],
        "citation_anchors": [
            {"claim_id": "claim-31-1", "evidence_ref": "evidence-31-1"},
        ],
        "support_targets": ["PA", "L2"],
        "owner_surface": OWNER_SURFACE,
        "assembly_hash": "asm-31-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-31-001",
        "trace_id": "trace-31-001",
        "request_id": "req-31-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-31-h",
        "blueprint_hash": "blueprint-31-h",
        "replay_key": "replay-31-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-31-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-031: per ledger row, a FinalEvidenceContract that violates the canonical
