"""Proof-evidence test for 10C-REQ-062 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: M3 PAST EXAMPLES

Canonical requirement (paraphrased): Examples load MUST: Load prior good answers SOPs exemplars valid success states clear success models from L4 read-only

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-062]: L1PlanContract emitted without satisfying 'Examples load MUST: Load prior good answers SOPs exemplars valid succ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-62-001",
        "trace_id": "trace-62-001",
        "request_id": "req-62-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-62-h",
        "blueprint_hash": "blueprint-62-h",
        "replay_key": "replay-62-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-62-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-062: per ledger row, a L1PlanContract that violates the canonical
