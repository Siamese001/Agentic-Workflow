...
@pytest.mark.skipif(not _AVAILABLE, reason="pitch_engine.py deps unavailable")
class TestPitchResult:
    def test_is_dataclass(self):
        import dataclasses
        assert dataclasses.is_dataclass(PitchResult)
    def test_importable(self):
        assert PitchResult is not None

@pytest.mark.skipif(not _AVAILABLE, reason="pitch_engine.py deps unavailable")
class TestPitchGenerator:
    def test_is_class(self):
        assert isinstance(PitchGenerator, type)
    def test_importable(self):
...
def test_module_importable():
    """Module pitch_engine.py is importable (or deps unavailable)."""
