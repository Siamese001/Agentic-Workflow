"""Proof-evidence test for 10C-REQ-074 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: L1 PLAN OUTPUT CONTRACT

Canonical requirement (paraphrased): L1 output contract MUST: Produce proposed_route query_spec task_spec route_risk confidence grounding_required declared_assumptions unresolve...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-074]: L1PlanContract emitted without satisfying 'L1 output contract MUST: Produce proposed_route query_spec task_spec ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-74-001",
        "trace_id": "trace-74-001",
        "request_id": "req-74-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-74-h",
        "blueprint_hash": "blueprint-74-h",
        "replay_key": "replay-74-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-74-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-074: per ledger row, a L1PlanContract that violates the canonical
