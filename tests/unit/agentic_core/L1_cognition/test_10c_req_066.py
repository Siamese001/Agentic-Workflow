"""Proof-evidence test for 10C-REQ-066 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: P2 PUT IN ORDER

Canonical requirement (paraphrased): Ordering MUST: Determine sequential vs parallel execution identify what unlocks what build dependency graph set stopping points

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-066]: L1PlanContract emitted without satisfying 'Ordering MUST: Determine sequential vs parallel execution identify wh...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-66-001",
        "trace_id": "trace-66-001",
        "request_id": "req-66-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-66-h",
        "blueprint_hash": "blueprint-66-h",
        "replay_key": "replay-66-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-66-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-066: per ledger row, a L1PlanContract that violates the canonical
