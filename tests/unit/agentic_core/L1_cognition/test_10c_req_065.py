"""Proof-evidence test for 10C-REQ-065 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: P1 BREAK INTO BABY STEPS

Canonical requirement (paraphrased): Planning MUST: Decompose into atomic work units with sub-goal boundaries missing-info markers explicit unknowns

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-065]: L1PlanContract emitted without satisfying 'Planning MUST: Decompose into atomic work units with sub-goal boundar...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-65-001",
        "trace_id": "trace-65-001",
        "request_id": "req-65-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-65-h",
        "blueprint_hash": "blueprint-65-h",
        "replay_key": "replay-65-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-65-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-065: per ledger row, a L1PlanContract that violates the canonical
