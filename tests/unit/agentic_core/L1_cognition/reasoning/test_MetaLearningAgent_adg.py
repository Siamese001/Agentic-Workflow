...
@pytest.mark.skipif(not _AVAILABLE, reason="MetaLearningAgent.py deps unavailable")
class TestMetalearningagentImportability:
    def test_module_importable(self) -> None:
        """ADG contract: MetaLearningAgent.py must be importable."""
        assert _AVAILABLE

    def test_experiencerecord_is_type(self) -> None:
        assert ExperienceRecord is not None

    def test_metalearningagent_is_type(self) -> None:
