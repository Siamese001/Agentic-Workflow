"""Proof-evidence test for 10C-REQ-070 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: V2 IS IT SAFE?

Canonical requirement (paraphrased): Safety validation MUST: Check within policy bounds escalates if needed ensures no forbidden action proposed

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-070]: L1PlanContract emitted without satisfying 'Safety validation MUST: Check within policy bounds escalates if neede...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-70-001",
        "trace_id": "trace-70-001",
        "request_id": "req-70-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-70-h",
        "blueprint_hash": "blueprint-70-h",
        "replay_key": "replay-70-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-70-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-070: per ledger row, a L1PlanContract that violates the canonical
