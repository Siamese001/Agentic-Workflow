"""Proof-evidence test for 10C-REQ-085 (03B_PA_Prompt_Assembly).
...
Source        : 03_Route_Decision_Switching.md :: PA.1 LOAD STATIC BLOCKS

Canonical requirement (paraphrased): PA.1 load static MUST: Load system template policy refs output schema persona mode S0 invariants role mixin blocks

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-085]: CompiledPromptArtifact emitted without satisfying 'PA.1 load static MUST: Load system template policy refs outpu...
...
def _valid_artifact() -> dict:
    return {
        "assembly_hash": "asm-85-h",
        "instruction_blocks": ["sys", "user"],
        "evidence_refs": ["ev-85-1"],
        "citation_anchors": [{"claim_id": "c1", "ref": "ev-85-1"}],
        "contradiction_flags": [],
        "slot_order_hash": "slot-85-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-85-001",
        "trace_id": "trace-85-001",
        "request_id": "req-85-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-85-h",
        "blueprint_hash": "blueprint-85-h",
        "replay_key": "replay-85-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-85-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-085: per ledger row, a CompiledPromptArtifact that violates the canonical
