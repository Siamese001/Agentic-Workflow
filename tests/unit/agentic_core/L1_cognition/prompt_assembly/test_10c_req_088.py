"""Proof-evidence test for 10C-REQ-088 (03B_PA_Prompt_Assembly).

Severity      : CRITICAL
Surface       : 03B_PA_Prompt_Assembly
Layer owner   : prompt assembly
Artifact      : CompiledPromptArtifact
OTEL span     : pa.prompt.compiled_artifact_emitted
Source        : 03_Route_Decision_Switching.md :: PA.4 PROMPT CONTRACT

Canonical requirement (paraphrased): PA.4 prompt contract MUST: Produce PromptEnvelope PromptAssemblyStatus citations_required authority_slot_order signed artifact with HMAC rep...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-088]: CompiledPromptArtifact emitted without satisfying 'PA.4 prompt contract MUST: Produce PromptEnvelope PromptAssem...
...
def _valid_artifact() -> dict:
    return {
        "assembly_hash": "asm-88-h",
        "instruction_blocks": ["sys", "user"],
        "evidence_refs": ["ev-88-1"],
        "citation_anchors": [{"claim_id": "c1", "ref": "ev-88-1"}],
        "contradiction_flags": [],
        "slot_order_hash": "slot-88-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-88-001",
        "trace_id": "trace-88-001",
        "request_id": "req-88-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-88-h",
        "blueprint_hash": "blueprint-88-h",
        "replay_key": "replay-88-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-88-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-088: per ledger row, a CompiledPromptArtifact that violates the canonical
