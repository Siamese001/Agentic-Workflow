"""Proof-evidence test for 10C-REQ-087 (03B_PA_Prompt_Assembly).
...
Source        : 03_Route_Decision_Switching.md :: PA.3 TOKEN BUDGETER

Canonical requirement (paraphrased): PA.3 token budgeter MUST: Trim stratify if needed reserve safety/schema space preserve instruction order trigger overflow->refine/abstain va...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-087]: CompiledPromptArtifact emitted without satisfying 'PA.3 token budgeter MUST: Trim stratify if needed reserve saf...
...
def _valid_artifact() -> dict:
    return {
        "assembly_hash": "asm-87-h",
        "instruction_blocks": ["sys", "user"],
        "evidence_refs": ["ev-87-1"],
        "citation_anchors": [{"claim_id": "c1", "ref": "ev-87-1"}],
        "contradiction_flags": [],
        "slot_order_hash": "slot-87-h",
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-87-001",
        "trace_id": "trace-87-001",
        "request_id": "req-87-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-87-h",
        "blueprint_hash": "blueprint-87-h",
        "replay_key": "replay-87-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-87-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-087: per ledger row, a CompiledPromptArtifact that violates the canonical
