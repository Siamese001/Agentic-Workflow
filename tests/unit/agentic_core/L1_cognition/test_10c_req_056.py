"""Proof-evidence test for 10C-REQ-056 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: I1 WHAT DO THEY WANT?

Canonical requirement (paraphrased): Intent parsing MUST: Parse primary objective requested end-state answer/plan/artifact success condition

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-056]: L1PlanContract emitted without satisfying 'Intent parsing MUST: Parse primary objective requested end-state answ...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-56-001",
        "trace_id": "trace-56-001",
        "request_id": "req-56-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-56-h",
        "blueprint_hash": "blueprint-56-h",
        "replay_key": "replay-56-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-56-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-056: per ledger row, a L1PlanContract that violates the canonical
