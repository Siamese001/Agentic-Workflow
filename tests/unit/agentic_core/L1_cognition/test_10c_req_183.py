"""Proof-evidence test for 10C-REQ-183 (02_L1_Reasoning_Plan).

Severity      : CRITICAL
Surface       : 02_L1_Reasoning_Plan
Layer owner   : L1 cognition + L6 observability
Artifact      : L1PlanContract
OTEL span     : l1.plan.contract_emitted
Source        : best-practice gap analysis 2026-04-30 wave2 C2 context engineering :: C2-NEW-D context utilization telemetry

Canonical requirement (paraphrased): Each LLM call MUST emit an OTEL span with attribute set: context.total_tokens, context.slot_fill_pct (per slot S0..H0 + R0), context.trim_st...

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-183]: L1PlanContract emitted without satisfying 'Each LLM call MUST emit an OTEL span with attribute set: context.tota...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-183-001",
        "trace_id": "trace-183-001",
        "request_id": "req-183-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-183-h",
        "blueprint_hash": "blueprint-183-h",
        "replay_key": "replay-183-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-183-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-183: per ledger row, a L1PlanContract that violates the canonical
