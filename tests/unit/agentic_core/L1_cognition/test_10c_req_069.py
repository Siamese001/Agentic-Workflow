"""Proof-evidence test for 10C-REQ-069 (02_L1_Reasoning_Plan).
...
Source        : 02_L1_Reasoning_Plan_Generation.md :: V1 DID WE LISTEN?

Canonical requirement (paraphrased): Validation MUST: Check answers actual goal respects all constraints has right deliverable/format

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-069]: L1PlanContract emitted without satisfying 'Validation MUST: Check answers actual goal respects all constraints h...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-69-001",
        "trace_id": "trace-69-001",
        "request_id": "req-69-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-69-h",
        "blueprint_hash": "blueprint-69-h",
        "replay_key": "replay-69-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-69-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-069: per ledger row, a L1PlanContract that violates the canonical
