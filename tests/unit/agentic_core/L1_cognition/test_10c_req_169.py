"""Proof-evidence test for 10C-REQ-169 (02_L1_Reasoning_Plan).
...
Source        : agentic_process_mapping_v29.md :: [2] L1 REASONING

Canonical requirement (paraphrased): L1 reasoning MUST: Parse intent synthesize validate produce formalized plan dispatch to routing read-only from L4

Negative control text (from ledger row, paraphrased): Row-specific [10C-REQ-169]: L1PlanContract emitted without satisfying 'L1 reasoning MUST: Parse intent synthesize validate produce formalize...
...
def _valid_span_attrs() -> dict:
    attrs = {
        "req_id": REQ_ID,
        "run_id": "run-169-001",
        "trace_id": "trace-169-001",
        "request_id": "req-169-001",
        "owner_surface": OWNER_SURFACE,
        "policy_hash": "policy-169-h",
        "blueprint_hash": "blueprint-169-h",
        "replay_key": "replay-169-k",
    }
    # Add any extra row-required span attrs with deterministic placeholders.
    for a in EXTRA_SPAN_ATTRS:
        attrs.setdefault(a, f"{a}-169-v")
    return attrs
...
def test_negative_control_specific_violation() -> None:
    """10C-REQ-169: per ledger row, a L1PlanContract that violates the canonical
