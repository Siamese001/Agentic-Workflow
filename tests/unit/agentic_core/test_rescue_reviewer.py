...
def test_test_rescue_reviewer_can_import():
    """Test that the module can be imported successfully."""
    try:
        mod = load_dev_script("rescue_reviewer.py")
        assert mod is not None
    except ImportError as e:
        pytest.skip(f"Cannot load dev_tools/l0_scripts/rescue_reviewer.py: {e}")


def test_test_rescue_reviewer_has_file_attribute():
    """Test that module has __file__ attribute."""
    try:
        mod = load_dev_script("rescue_reviewer.py")
        assert hasattr(mod, "__file__")
    except ImportError:
        pytest.skip("Cannot load dev_tools/l0_scripts/rescue_reviewer.py")


def test_test_rescue_reviewer_has_public_attributes():
