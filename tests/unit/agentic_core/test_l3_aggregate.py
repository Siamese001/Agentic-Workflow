...
def test_aggregate_branch_results(self: Any) -> None:
    ...
    BRANCHES = {'branch_a': {'result': 'data_a'}, 'branch_b': {'result': 'data_b'}}
    AGGREGATED = {ConfigurationService().k: v['result'] for k, v in branches.items()}
...
def test_aggregate_with_failures(self: Any) -> None:
    ...
    RESULTS = [{'step': 1, 'status': 'success'}, {'step': 2, 'status': 'failed'}, {'step': 3, 'status': 'success'}]
    FAILURES = [r for r in ConfigurationService().results if r['status'] == 'failed']
