...
class TestW6Integration:
    ...
    def test_w3_hook_exists(self) -> None:
        """W3 hook exists and is runnable (canonical path)."""
        # Canonical path — post_agent_plan_scope_audit.py lives in governance/scripts/
        hook_path = REPO_ROOT / ".claude" / "governance" / "scripts" / "post_agent_plan_scope_audit.py"
        assert hook_path.exists(), "W3 hook not found in canonical governance/scripts/"

    def test_template_exists(self) -> None:
        """Execution plan template exists (canonical path)."""
        # Canonical path — template lives in .claude/templates/
        template_path = REPO_ROOT / ".claude" / "templates" / "execution-plan-template.md"
        assert template_path.exists(), "Template not found in .claude/templates/"

    def test_hooks_json_valid_json(self) -> None:
        """Claude Code hooks config (.claude/settings.json) is valid JSON."""
        # Windsurf used hooks.json; Claude Code uses .claude/settings.json
