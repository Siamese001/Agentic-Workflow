...
The gate is a live Stop hook under .codex/hooks/. It imports lib.codex_hook_common
(so .codex/hooks must be on sys.path) and reuses detect() from the advisory audit.
...
def gate_mod(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    # .codex/hooks must be importable for `from lib.codex_hook_common import ...`.
