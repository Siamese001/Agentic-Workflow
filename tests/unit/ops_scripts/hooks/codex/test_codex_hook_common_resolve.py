"""Unit tests for the shared Stop-payload response resolver (codex_hook_common).
...
Stop hooks share so they actually see the final assistant turn. codex_hook_common has no
...
CHC_PATH = REPO_ROOT / ".codex" / "hooks" / "lib" / "codex_hook_common.py"


@pytest.fixture()
def chc():
    spec = importlib.util.spec_from_file_location("_codex_hook_common_test", CHC_PATH)
