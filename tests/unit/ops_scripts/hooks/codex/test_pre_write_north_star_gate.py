...
_GATE_PATH = _REPO_ROOT / ".codex" / "hooks" / "pre_write_north_star_gate.py"
...
def test_non_edit_tool_allowed():
    code, _ = gate.evaluate({"tool_name": "Read", "tool_input": {"file_path": ".codex/rules/x.md"}},
...
def test_block_message_shows_earned_confidence():
    code, reason = gate.evaluate(_edit(".codex/hooks/new_hook.py"), state=_STATE, env=_NO_BYPASS)
    assert code == 2
...
def test_auto_disabled_when_shipped():
    ...
    code, _ = gate.evaluate(_edit(".codex/rules/x.md"), state=shipped, env=_NO_BYPASS)
    assert code == 0


def test_disabled_state_allows():
    off = {"lanes_passing": 1, "lanes_total": 11, "enabled": False}
    code, _ = gate.evaluate(_edit(".codex/rules/x.md"), state=off, env=_NO_BYPASS)
    assert code == 0


def test_empty_state_failsoft_allows():
    code, _ = gate.evaluate(_edit(".codex/rules/x.md"), state={}, env=_NO_BYPASS)
    assert code == 0


def test_bypass_env_allows():
    code, _ = gate.evaluate(_edit(".codex/rules/x.md"), state=_STATE, env={"NORTH_STAR_GATE_BYPASS": "1"})
    assert code == 0


def test_mode_off_allows():
    code, _ = gate.evaluate(_edit(".codex/rules/x.md"), state=_STATE, env={"NORTH_STAR_GATE_ENFORCE": "off"})
    assert code == 0
