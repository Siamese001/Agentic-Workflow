...
Covers (claude-enforcement-runtime-truth-hardening W2):
  * lib.claude_hook_common.write_failopen_receipt — schema, non-dict payload, write-failure
...
CHC_PATH = HOOKS_DIR / "lib" / "claude_hook_common.py"
...
class TestBudgetGate:
    ...
    def gate(self, monkeypatch, tmp_path):
        ...
        settings.write_text(
            json.dumps(
                {"hooks": {"PreToolUse": [{"matcher": "Grep", "hooks": [{"type": "command", "command": 'python "$CLAUDE_PROJECT_DIR/.claude/hooks/before_grep.py"'}]}]}}
            ),
            encoding="utf-8",
        )
    ...
    def test_critical_target_missing_errors(self, gate) -> None:
        gate.SETTINGS.write_text(
            json.dumps(
                {"hooks": {"PreToolUse": [{"matcher": "X", "hooks": [{"type": "command", "command": 'python "$CLAUDE_PROJECT_DIR/.claude/hooks/does_not_exist.py"'}]}]}}
            ),
            encoding="utf-8",
        )
