...
def test_quiet_mode_output():
    ...
    try:
        ...
        assert result.stdout.strip() == "", (
            f"FAILED: Quiet mode produced output: '{result.stdout.strip()}'"
        )
...
def test_argparse_help_functionality():
    ...
    assert "Hardened Cache Purge Utility" in result.stdout, (
        "FAILED: Description not in help output."
    )
...
def test_extended_flag_with_quiet():
    ...
    try:
        ...
        assert result.stdout.strip() == "", (
            "FAILED: --quiet should suppress output even with --all."
        )
