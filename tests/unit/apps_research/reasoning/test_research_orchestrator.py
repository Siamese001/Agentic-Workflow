"""
Unit tests for ResearchOrchestrator — apps_research.
...
class _ResearchStatusNS:
    """Namespace shim: ResearchStatus is a Literal alias, not an Enum; the orchestrator uses .ATTR syntax."""
...
def _make_orchestrator(*, qwen_enabled: bool = True, qwen_available: bool = True):
    """Build a ResearchOrchestrator with all external I/O mocked."""
    with _routing_module_patch(lambda _: _RoutingDecision(_Provider.LOCAL_VLLM)):
        from apps_research.reasoning.ResearchOrchestrator import ResearchOrchestrator  # noqa: PLC0415

        with (
            patch("apps_research.reasoning.ResearchOrchestrator._QWEN_AVAILABLE", qwen_available),
            patch("apps_research.reasoning.ResearchOrchestrator.AppsQwenGateway", Mock()),
            patch("apps_research.reasoning.ResearchOrchestrator.apps_qwen_telemetry", None),
            patch(
                "apps_research.reasoning.ResearchOrchestrator.ResearchAssemblyEngine",
                Mock(
                    return_value=Mock(
                        execute=Mock(return_value=Mock(sections=[], comparison_matrix=[], source_register=[]))
                    )
                ),
            ),
            patch(
                "apps_research.reasoning.ResearchOrchestrator.ResearchGateValidator",
                Mock(
                    return_value=Mock(
            ...
            orch = ResearchOrchestrator(qwen_enabled=qwen_enabled, dry_run=True)
    return orch
...
class TestResearchOrchestratorLocalFirstRouting:
    ...
    def test_run_sets_qwen_inference_result_when_local_vllm_selected(self):
        ...
        mock_qwen_result = {"success": True, "content": "Research synthesis text", "confidence": 0.85}
        ...
        with (
            ...
            patch("apps_research.reasoning.ResearchOrchestrator.ResearchStatus", _ResearchStatusNS),
            patch(
                "apps_research.reasoning.ResearchOrchestrator.ResearchRunSummary", Mock(return_value=Mock())
            ),
            patch.object(orch, "synthesize_research_with_qwen", new=AsyncMock(return_value=mock_qwen_result)),
        ):
    ...
    def test_run_leaves_qwen_inference_result_none_when_opus_escalation(self):
        ...
        with (
            ...
            patch.object(orch, "synthesize_research_with_qwen", new=AsyncMock()) as mock_infer,
        ):
            result = asyncio.run(orch.run(_make_request()))
    ...
    def test_qwen_init_error_field_set_on_gateway_failure(self):
        ...
        with _routing_module_patch(lambda _: _RoutingDecision(_Provider.LOCAL_VLLM)):
            ...
            with (
                patch("apps_research.reasoning.ResearchOrchestrator._QWEN_AVAILABLE", True),
                patch("apps_research.reasoning.ResearchOrchestrator.AppsQwenGateway", broken_gateway),
                patch("apps_research.reasoning.ResearchOrchestrator.apps_qwen_telemetry", None),
                patch(
                    "apps_research.reasoning.ResearchOrchestrator.ResearchAssemblyEngine",
                ...
                orch = ResearchOrchestrator(qwen_enabled=True)
    ...
    def test_routing_context_uses_phase1_constants(self):
        ...
        with (
            ...
            patch.object(
                orch, "synthesize_research_with_qwen", new=AsyncMock(return_value={"success": True})
            ),
        ):
            asyncio.run(orch.run(_make_request()))
...
class TestResearchOrchestratorAdapterDisposition:
    ...
    def test_adapter_allow_calls_qwen(self):
        ...
        orch = _make_orchestrator()
        mock_qwen_result = {"success": True, "content": "synthesis"}
    ...
    def test_adapter_evaluate_called_with_correct_task_class(self):
        """Contract: adapter.evaluate() receives task_class=research_synthesis."""
        orch = _make_orchestrator()
        ...
        with (
            ...
            patch.object(orch, "synthesize_research_with_qwen", new=AsyncMock(return_value={})),
        ):
            asyncio.run(orch.run(_make_request()))

        call_kwargs = mock_adapter.evaluate.call_args
        assert call_kwargs.kwargs["task_class"] == "research_synthesis"
    ...
    def test_adapter_circuit_breaker_failure_recorded_on_qwen_error(self):
        ...
        with (
            ...
            patch.object(
                orch, "synthesize_research_with_qwen", new=AsyncMock(side_effect=RuntimeError("Qwen down"))
            ),
        ):
