...
def mock_security_validation():
    with patch("agentic_core.base_agents.SovereignBaseAgent.SovereignBaseAgent._security_hardening_validation") as mock:
...
def test_circular_dependency_detection_scoped(tmp_path):
    ...
    assert "agentic_core.circular.a -> agentic_core.circular.b -> agentic_core.circular.a" in report["circular_dependencies"][0]
