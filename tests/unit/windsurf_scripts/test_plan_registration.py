...
# Canonical path — _plan_registration.py lives in governance/scripts/, not _legacy_windsurf/
HELPER_PATH = (
...
def pr(tmp_path, monkeypatch):
    ...
    # Canonical module also has NEW_PLANS_DIR and PLAN_DIRS — patch both so
    # list_on_disk_plans() only sees the tmp_path plans dir, not live plans/.
    if hasattr(mod, "NEW_PLANS_DIR"):
        monkeypatch.setattr(mod, "NEW_PLANS_DIR", plans)
    if hasattr(mod, "PLAN_DIRS"):
        monkeypatch.setattr(mod, "PLAN_DIRS", [plans])
    return mod
...
def test_parse_multiple_markers(pr):
    ...
    # Canonical module derives path as "plans/{slug}.md" (not the legacy windsurf archive path)
