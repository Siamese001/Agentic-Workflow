...
HELPER_DIR = Path(__file__).resolve().parents[3] / ".claude" / "governance/scripts" / "_legacy_windsurf"
...
def test_hook_prefix_misroute_blocked(path: str) -> None:
    ...
    assert v.suggested == ".claude/governance/scripts/_legacy_windsurf/"


def test_hook_prefix_in_canonical_folder_passes() -> None:
    assert helper.decide(".claude/governance/scripts/_legacy_windsurf/post_cursor_agent_new.py", exists=False) is None
    assert helper.decide(".claude/governance/scripts/_legacy_windsurf/pre_write_extra.py", exists=False) is None
