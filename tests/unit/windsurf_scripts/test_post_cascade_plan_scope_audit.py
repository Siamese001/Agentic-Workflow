"""
Unit tests for post_cascade_plan_scope_audit.py
...
MODULE_PATH = REPO_ROOT / ".windsurf" / "scripts" / "post_cascade_plan_scope_audit.py"

spec = importlib.util.spec_from_file_location(
    "post_cascade_plan_scope_audit", MODULE_PATH
)
_module = importlib.util.module_from_spec(spec)
sys.modules["post_cascade_plan_scope_audit"] = _module
spec.loader.exec_module(_module)

from post_cascade_plan_scope_audit import (
    main,
    get_config,
...
class TestAdvisoryMode:
    ...
    @patch('post_cascade_plan_scope_audit.detect_changed_files')
    @patch('post_cascade_plan_scope_audit.detect_active_plan')
    @patch('post_cascade_plan_scope_audit.extract_markers_from_text')
    @patch('post_cascade_plan_scope_audit.check_scope_authorization')
    @patch('post_cascade_plan_scope_audit.write_audit_record')
...
class TestJSONLLogging:
    ...
    def test_log_record_structure(self, tmp_path):
        ...
        with patch('post_cascade_plan_scope_audit.LOG_PATH', log_file):
            mock_result = MagicMock()
            mock_result.authorized = False
            mock_result.reason = "MISSING_AUTHORIZATION_DECISION"
