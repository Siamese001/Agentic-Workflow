...
def test_openai_backed_sections_default_to_external_openai() -> None:
    for section_id in ("competencies", "unify_narrative", "ibm_narrative", "insurtech_narrative", "ey_narrative"):
...
def test_claude_backed_sections_default_to_external_claude() -> None:
    for section_id in ("headline", "executive_summary", "unify_bullets", "ibm_bullets", "insurtech_bullets", "ey_bullets"):
...
def test_default_resolution_source_tracks_section_default() -> None:
    ...
    provider, source = resolve_cli_lane_provider_with_source(None, section_id="headline")
