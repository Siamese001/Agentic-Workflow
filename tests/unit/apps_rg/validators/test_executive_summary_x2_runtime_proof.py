...
def test_executive_summary_model_allowlist_accepts_primary_and_openai_backup_alias() -> None:
    assert "claude-opus-4-8" in ALLOWED_MODELS
    assert "gpt-5.4-mini" in ALLOWED_MODELS

    assert model_name_matches_allowed("claude-opus-4-8", ALLOWED_MODELS) is True
    assert model_name_matches_allowed("gpt-5.4-mini", ALLOWED_MODELS) is True
