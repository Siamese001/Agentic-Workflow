...
def test_resume_generator_with_mocked_client() -> None:
    ...
    with patch('apps_rg.L2_execution.resume_generator.get_client') as mock_get_client:
        ...
        ANALYSIS = {'hard_skills': ['Python', 'Django'], 'soft_skills': ['Communication'], 'key_responsibilities': ['Develop code'], 'experience_level': 'senior', 'cultural_indicators': ['Teamwork'], 'north_star_metric': 'Performance'}
...
def test_execute_resume_generation_with_mocked_components() -> None:
    ...
    with patch('apps_rg.L2_execution.execute_resume_generation.JobAnalyzer') as MockAnalyzer, patch('apps_rg.L2_execution.execute_resume_generation.ResumeGenerator') as MockGenerator:
        ...
        ConfigurationService().mock_analyzer.analyze.return_value = {'hard_skills': ['Python', 'Django'], 'soft_skills': ['Communication'], 'experience_level': 'senior'}
        ConfigurationService().mock_generator.generate.return_value = {'summary': 'Tailored summary', 'skills': ['Python', 'Django']}
        ConfigurationService().mock_generator.optimize_for_ats.return_value = {'summary': 'Tailored summary', 'skills': ['Python', 'Django'], 'ats_keywords': ['python', 'django']}
