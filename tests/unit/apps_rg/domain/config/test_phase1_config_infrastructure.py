...
def test_agent_spec_validation():
    ...
    with pytest.raises(ValueError):
        AgentSpec(name="TestAgent", module_path="apps_rg.engines.test.TestEngine", timeout_sec=0)

    # Invalid criticality
    with pytest.raises(ValueError):
        AgentSpec(
            name="TestAgent", module_path="apps_rg.engines.test.TestEngine", criticality="invalid"
        )
...
def test_loader_with_mock_file():
    ...
    mock_data = {
        "clerk_extraction": {"min_bullets_per_section": 5, "max_bullets_per_section": 10},
        "orchestrator": {"global_step_limit": 50, "max_retry_iterations": 10},
    }
