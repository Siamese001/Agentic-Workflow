...
class TestHardenedCoreSynthesis:
    ...
    def test_domain_exceptions_exist(self):
        ...
        try:
            from agentic_core.domain.exceptions import SovereignError, HealerError, CircularDependencyError, ConfigurationError, StructuralError, HygieneError
    ...
    def test_sovereign_base_agent_is_dataclass(self):
        ...
        try:
            ...
            if not sovereign_path.exists():
                # Try absolute path
                sovereign_path = Path("c:/Git/Agentic-Workflow/agentic_core/base_agents/SovereignBaseAgent.py")
            ...
            assert "_security_hardening_validation" in content, "Missing _security_hardening_validation method"
    ...
    def test_structural_healing_mixin_exists(self):
        ...
        try:
            ...
            assert hasattr(StructuralHealingMixin, '_salvaged_file_relocation'), "Missing _salvaged_file_relocation method"
            assert hasattr(StructuralHealingMixin, '_analyze_file_structure'), "Missing _analyze_file_structure method"
        ...
        try:
            ...
            for name, func in methods:
                if name.startswith('_') and not name.startswith('__'): 
                    continue
                if name in ['validate_canon_key', 'check_key_00_no_hardcoded_secrets', '__eq__', '__hash__', '__repr__']:  # Skip validation and dataclass methods
                    continue
    ...
    def test_circular_dependency_firewall(self):
        ...
        try:
            ...
            if len(violations) > 0:
                print(f"WARNING: Found {len(violations)} dependency leaks (expected during transition): {violations[:5]}")
            
            # Only fail if critical base_agents have violations
            critical_violations = [v for v in violations if any(f in v for f in ['healer_mixin.py', 'SovereignBaseAgent.py'])]
    ...
    def test_security_validation_methods(self):
        ...
        try:
            ...
            assert "def _security_hardening_validation" in content, "Missing _security_hardening_validation method"
