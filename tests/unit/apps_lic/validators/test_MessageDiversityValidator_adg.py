...
try:
    from apps_lic.validators.MessageDiversityValidator import (  # noqa: F401
        HealerMixin,
...
@pytest.mark.skipif(not _AVAILABLE, reason="MessageDiversityValidator.py deps unavailable")
...
class TestMessageDiversityValidator:
    def test_is_dataclass(self):
        import dataclasses
        assert dataclasses.is_dataclass(MessageDiversityValidator)
    def test_importable(self):
...
def test_module_importable():
    """Module MessageDiversityValidator.py is importable (or deps unavailable)."""
