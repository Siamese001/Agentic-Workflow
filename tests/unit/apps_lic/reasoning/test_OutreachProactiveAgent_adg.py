...
try:
    from apps_lic.reasoning.OutreachProactiveAgent import (  # noqa: F401
        BATCH_SIZE,
        BUFFER_SIZE,
        DEFAULT_SLEEP,
        MAX_DEPTH,
...
@pytest.mark.skipif(not _AVAILABLE, reason="OutreachProactiveAgent.py deps unavailable")
...
class TestOutreachProactiveAgent:
    def test_is_dataclass(self):
        import dataclasses
        assert dataclasses.is_dataclass(OutreachProactiveAgent)
    def test_importable(self):
...
def test_module_importable():
    """Module OutreachProactiveAgent.py is importable (or deps unavailable)."""
