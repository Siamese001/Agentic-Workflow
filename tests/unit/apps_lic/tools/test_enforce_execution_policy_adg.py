...
@pytest.mark.skipif(not _AVAILABLE, reason="enforce_execution_policy.py deps unavailable")
class TestEnforceExecutionPolicy:
    def test_is_callable(self):
        assert callable(enforce_execution_policy)

@pytest.mark.skipif(not _AVAILABLE, reason="enforce_execution_policy.py deps unavailable")
class TestGetEnforceExecutionPolicyConfig:
    def test_is_callable(self):
        assert callable(get_enforce_execution_policy_config)
...
def test_module_importable():
    """Module enforce_execution_policy.py is importable (or deps unavailable)."""
