"""
Unit tests for ExecOrchestrator — apps_exec.
...
# ---------------------------------------------------------------------------
# Stub VLLMGatewayAdapter (avoids importing the real adapter chain which
# requires GPU-library stubs and circuit-breaker singletons)
# ---------------------------------------------------------------------------


def _make_adapter_stub(*, route_to_gemini: bool = False):
    """Return a mock VLLMGatewayAdapter with a canned evaluate() result."""
    mock_telemetry = Mock()
...
class _BriefStatusNS:
    """Namespace shim: BriefStatus is a Literal alias, not an Enum; the orchestrator uses .ATTR syntax."""
...
_EXEC_PATCHES = [
    ("apps_exec.reasoning.ExecOrchestrator.BriefStatus", _BriefStatusNS),
    (
        "apps_exec.reasoning.ExecOrchestrator.IngestionEngine",
        Mock(return_value=Mock(execute=Mock(return_value=Mock(documents=[], skipped_paths=[])))),
    ),
    (
        "apps_exec.reasoning.ExecOrchestrator.CapabilityExtractionEngine",
        Mock(return_value=Mock(execute=Mock(return_value=Mock(capabilities=[])))),
    ),
    (
        "apps_exec.reasoning.ExecOrchestrator.BriefAssemblyEngine",
        Mock(return_value=Mock(execute=Mock(return_value=Mock(sections=[])))),
    ),
    (
        "apps_exec.reasoning.ExecOrchestrator.StyleGateValidator",
        Mock(
            return_value=Mock(
                validate_sections=Mock(return_value=Mock(passed=True, quality_score=0.9, violations=[]))
            )
        ),
    ),
]
...
def _make_orchestrator(*, qwen_enabled: bool = True, qwen_available: bool = True):
    """Build an ExecOrchestrator with all external I/O mocked."""
    with _routing_module_patch(lambda _: _RoutingDecision(_Provider.LOCAL_VLLM)):
        from apps_exec.reasoning.ExecOrchestrator import ExecOrchestrator  # noqa: PLC0415

        with (
            patch("apps_exec.reasoning.ExecOrchestrator._QWEN_AVAILABLE", qwen_available),
            patch("apps_exec.reasoning.ExecOrchestrator.AppsQwenGateway", Mock()),
            patch("apps_exec.reasoning.ExecOrchestrator.apps_qwen_telemetry", None),
            patch(
                "apps_exec.reasoning.ExecOrchestrator.IngestionEngine",
                Mock(return_value=Mock(execute=Mock(return_value=Mock(documents=[], skipped_paths=[])))),
            ),
            ...
            patch(
                ...
                Mock(
                    return_value=Mock(
                        validate_sections=Mock(
                            return_value=Mock(passed=True, quality_score=0.9, violations=[])
                        )
                    )
                ),
            ),
        ):
            orch = ExecOrchestrator(qwen_enabled=qwen_enabled, dry_run=True)
    return orch


def _make_request():
    from apps_exec.types.exec_types import ExecBriefRequest  # noqa: PLC0415

    return ExecBriefRequest(source_dirs=["docs/architecture"], dry_run=True)


class TestExecOrchestratorLocalFirstRouting:
    ...
    def test_run_sets_qwen_inference_result_when_local_vllm_selected(self):
        ...
        mock_qwen_result = {"success": True, "content": "Execution plan text", "confidence": 0.9}
        ...
        with (
            ...
            patch("apps_exec.reasoning.ExecOrchestrator.BriefStatus", _BriefStatusNS),
            patch("apps_exec.reasoning.ExecOrchestrator.RunSummary", Mock(return_value=Mock())),
            patch.object(orch, "plan_execution_with_qwen", new=AsyncMock(return_value=mock_qwen_result)),
        ):
    ...
    def test_run_leaves_qwen_inference_result_none_when_opus_escalation(self):
        ...
        with (
            ...
            patch("apps_exec.reasoning.ExecOrchestrator.BriefStatus", _BriefStatusNS),
            patch("apps_exec.reasoning.ExecOrchestrator.RunSummary", Mock(return_value=Mock())),
            patch.object(orch, "plan_execution_with_qwen", new=AsyncMock()) as mock_infer,
        ):
            result = asyncio.run(orch.run(_make_request()))
    ...
    def test_qwen_init_error_field_set_on_gateway_failure(self):
        ...
        with _routing_module_patch(lambda _: _RoutingDecision(_Provider.LOCAL_VLLM)):
            ...
            with (
                patch("apps_exec.reasoning.ExecOrchestrator._QWEN_AVAILABLE", True),
                patch("apps_exec.reasoning.ExecOrchestrator.AppsQwenGateway", broken_gateway),
                patch("apps_exec.reasoning.ExecOrchestrator.apps_qwen_telemetry", None),
                patch("apps_exec.reasoning.ExecOrchestrator.IngestionEngine", Mock(return_value=Mock())),
                patch(
                    "apps_exec.reasoning.ExecOrchestrator.CapabilityExtractionEngine",
                    Mock(return_value=Mock()),
                ),
                patch("apps_exec.reasoning.ExecOrchestrator.BriefAssemblyEngine", Mock(return_value=Mock())),
                patch("apps_exec.reasoning.ExecOrchestrator.StyleGateValidator", Mock(return_value=Mock())),
            ):
                orch = ExecOrchestrator(qwen_enabled=True)
    ...
    def test_routing_context_uses_phase1_constants(self):
        ...
        with (
            ...
            patch("apps_exec.reasoning.ExecOrchestrator.BriefStatus", _BriefStatusNS),
            patch("apps_exec.reasoning.ExecOrchestrator.RunSummary", Mock(return_value=Mock())),
            patch.object(orch, "plan_execution_with_qwen", new=AsyncMock(return_value={"success": True})),
        ):
            asyncio.run(orch.run(_make_request()))
...
class TestExecOrchestratorAdapterDisposition:
    ...
    def test_adapter_allow_calls_qwen(self):
        ...
        orch = _make_orchestrator()
        mock_qwen_result = {"success": True, "content": "plan"}
    ...
    def test_adapter_evaluate_called_with_correct_task_class(self):
        """Contract: adapter.evaluate() receives task_class=execution_planning."""
        orch = _make_orchestrator()
        ...
        with (
            ...
            patch("apps_exec.reasoning.ExecOrchestrator.BriefStatus", _BriefStatusNS),
            patch("apps_exec.reasoning.ExecOrchestrator.RunSummary", Mock(return_value=Mock())),
            patch.object(orch, "plan_execution_with_qwen", new=AsyncMock(return_value={})),
        ):
            asyncio.run(orch.run(_make_request()))

        call_kwargs = mock_adapter.evaluate.call_args
        assert call_kwargs.kwargs["task_class"] == "execution_planning"
    ...
    def test_adapter_circuit_breaker_failure_recorded_on_qwen_error(self):
        ...
        with (
            ...
            patch("apps_exec.reasoning.ExecOrchestrator.RunSummary", Mock(return_value=Mock())),
            patch.object(
                orch, "plan_execution_with_qwen", new=AsyncMock(side_effect=RuntimeError("Qwen down"))
            ),
        ):
