...
# ===========================================================================
# W12 -- apps_rfp migration to APP_OVERLAY_STATIC_EVIDENCE
# ===========================================================================


def test_apps_rfp_live_classifies_as_overlay_static_evidence() -> None:
    """apps_rfp/spine_manifest.yaml + integrations/spine_handoff.py must
    combine to classify as APP_OVERLAY_STATIC_EVIDENCE for R3_grounded_read.
    ...
    """
    repo_root = Path(__file__).resolve().parents[4]
    apps_rfp = repo_root / "apps_rfp"
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
    runtime_mode, evidence, sc = _classify(apps_rfp)
    assert sc["manifest_present"] is True, "apps_rfp/spine_manifest.yaml missing"
    assert "R3_grounded_read" in sc["manifest_claimed_routes"], (
        f"manifest claimed_routes={sc['manifest_claimed_routes']}"
    )
    assert sc["manifest_missing_contracts"] == [], (
...
def test_apps_rfp_surfaces_full_R3_contract_chain() -> None:
    """All 8 R3 contracts (with PromptEnvelope ↔ CompiledPromptArtifact equivalence)
    must be detected as direct imports in apps_rfp."""
    repo_root = Path(__file__).resolve().parents[4]
    apps_rfp = repo_root / "apps_rfp"
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
    _runtime_mode, _evidence, sc = _classify(apps_rfp)
    detected = set(sc["distinct_contracts"])
    for required in (
        "ValidatedRequest",
        "L1PlanContract",
...
def test_apps_rfp_is_not_build_time_compiler() -> None:
    """apps_rfp must NOT declare or be classified as build_time_compiler."""
    repo_root = Path(__file__).resolve().parents[4]
    apps_rfp = repo_root / "apps_rfp"
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
...
def test_apps_rfp_is_not_formal_exception() -> None:
    """apps_rfp is not exempt; it uses the standard GovernedAppRunner
    substrate. The scanner must NOT classify it as
    FORMAL_EXCEPTION_STATIC_EVIDENCE."""
    repo_root = Path(__file__).resolve().parents[4]
    apps_rfp = repo_root / "apps_rfp"
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
    runtime_mode, _evidence, sc = _classify(apps_rfp)
    assert runtime_mode != "FORMAL_EXCEPTION_STATIC_EVIDENCE"
    assert sc["manifest_has_formal_exception"] is False
    assert sc["manifest_exception_reason_code"] == ""


def test_apps_rfp_does_not_require_commit_request() -> None:
    """The pre-migration audit proved no durable-write surface exists in
    apps_rfp (portal submission explicitly out of scope per SVP review and
    SLO). R3_grounded_read intentionally excludes CommitRequest, and
    apps_rfp must NOT be promoted to R3R4_managed_workflow."""
    repo_root = Path(__file__).resolve().parents[4]
    apps_rfp = repo_root / "apps_rfp"
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
    _runtime_mode, _evidence, sc = _classify(apps_rfp)
    assert "CommitRequest" not in sc["manifest_required_contracts"], (
        f"apps_rfp must not require CommitRequest; "
        f"required={sc['manifest_required_contracts']}"
    )
    assert "CommitRequest" not in sc["manifest_missing_contracts"]
    # Reinforce: route stays R3_grounded_read.
...
def test_apps_rfp_is_not_R3R4_managed_workflow() -> None:
    """apps_rfp must NOT be classified as R3R4_managed_workflow. The
    pre-migration audit found no durable-write surface, no CommitRequest,
    no StateDiffCandidate, no portal submission (explicitly out of scope),
    and no L4/UWG admission. Adding R3R4 would be contract theater that
    contradicts apps_rfp/SVP_ENGINEERING_REVIEW.md and apps_rfp/SLO.md."""
    repo_root = Path(__file__).resolve().parents[4]
    apps_rfp = repo_root / "apps_rfp"
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
    ...
    # importing it without a runtime path would be contract theater.
    assert "CommitRequest" not in sc["distinct_contracts"], (
        f"apps_rfp spine_handoff.py must not import CommitRequest; "
        f"distinct_contracts={sc['distinct_contracts']}"
    )


def test_apps_rfp_hitl_absent_does_not_change_route_shape() -> None:
    """Unlike apps_lic and apps_exec, GovernedRfpRun does NOT declare
    HITL_ENABLED (defaults False per GovernedAppRunner). The scanner must
    classify apps_rfp on the same shape as its R3 siblings regardless --
    HITL is orthogonal to route classification."""
    from tools.analysis.apps_spine_coverage import (
        ROUTE_TYPE_CONTRACT_REQUIREMENTS,
    )
    repo_root = Path(__file__).resolve().parents[4]
    ...
    if not apps_rfp.is_dir():
        pytest.skip("apps_rfp not present in this checkout")
    runtime_mode, _evidence, sc = _classify(apps_rfp)
    # Same R3 bucket as HITL-enabled apps_lic / apps_exec.
    assert sc["manifest_claimed_routes"] == ["R3_grounded_read"]
    assert runtime_mode == "APP_OVERLAY_STATIC_EVIDENCE"
    # The R3 chain alone does not require CommitRequest.
    r3_required = ROUTE_TYPE_CONTRACT_REQUIREMENTS["R3_grounded_read"]
...
def test_apps_rfp_spine_handoff_module_imports_cleanly() -> None:
    """The apps_rfp spine_handoff module must import without error,
    expose all 8 R3 contract types via R3_CONTRACT_SURFACE, and NOT
    expose CommitRequest as part of the surface or __all__."""
    from apps_rfp.integrations import spine_handoff
    ...
    for name, cls in surface.items():
        assert cls is not None, f"surface entry {name!r} is None"
        assert isinstance(cls, type), f"surface entry {name!r} is not a class"
    valid = spine_handoff.validate_rfp_r3_contract_surface()
    assert valid == {n: True for n in expected_names}
    # The module's __all__ must not advertise CommitRequest.
    assert "CommitRequest" not in spine_handoff.__all__
