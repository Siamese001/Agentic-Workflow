...
def _discover_repo_root(start: Path) -> Path:
    ...
    for candidate in (start, *start.parents):
        if (candidate / "artifacts").exists() or (candidate / ".git").exists():
            return candidate
...
def _find_latest_sqlite() -> Path | None:
    ...
    candidates = [Path(p) for p in glob.glob(_SQLITE_GLOB)]
    if not candidates:
        return None
...
def live_conn():
    ...
    conn = sqlite3.connect(f"file:{LIVE_SQLITE}?mode=ro", uri=True, timeout=30)
    yield conn
    conn.close()
