...
def mock_memory_store():
    ...
    store.query_by_company.return_value = [
        {"text": "Company Context A - detailed information about strategic priorities and initiatives for 2025", "metadata": {"source_weight": 0.8, "age_days": 10}},
        {"text": "Company Context B - additional context about roadmap and platform", "metadata": {"source_weight": 0.7, "age_days": 15}},
        {"text": "Company Context C - more strategic information", "metadata": {"source_weight": 0.75, "age_days": 20}}
    ]
    store.query_by_executive.return_value = [
        {"text": "Executive Context A - recent LinkedIn posts and presentations about leadership vision", "metadata": {"source_weight": 0.9, "age_days": 5}},
        {"text": "Executive Context B - background and career history with detailed achievements", "metadata": {"source_weight": 0.85, "age_days": 10}}
    ]
    store.get_strategic_briefs.return_value = [
        {"text": "Strategic Brief - Comprehensive analysis of company direction, key initiatives, market positioning, competitive landscape, and growth strategy for the upcoming fiscal year with detailed roadmap and milestones", "metadata": {"source_weight": 1.0, "age_days": 5}}
    ]
    return store
...
def mock_search_client():
    ...
    client.search = MagicMock(return_value=[
        {"snippet": "New News Article", "link": "http://news.com", "title": "News"}
    ])
    return client
...
class TestHOP2SlowPath:
    ...
    def test_cache_miss_rag_activation(self, mock_memory_store, mock_search_client, resources, input_data):
        """
        ...
        rag_texts = [r["text"] for r in result["rag_results"] if r.get("metadata", {}).get("SourceType") == "STRATEGIC_BRIEF"]
