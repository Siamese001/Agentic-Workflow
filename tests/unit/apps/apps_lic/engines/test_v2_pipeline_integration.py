...
def orchestrator(tmp_path):
    ...
    mock_store.query_by_company.return_value = [
        {"text": "Strategic AI roadmap and machine learning initiatives", "metadata": {"source_weight": 1.0, "age_days": 1}}
    ]
    mock_store.get_strategic_briefs.return_value = [
        {"text": "Strategic Roadmap 2026 focusing on intelligent systems", "metadata": {"source_weight": 1.0}}
    ]
    ...
    orch.register_agent("HOP2", HOP2ResearchAgent(memory_store=mock_store, search_client=mock_search))
    ...
    with patch("apps_lic.shared.v2_patterns.agent_base.load_agent_specs") as mock_specs:
        mock_config = MagicMock()
        mock_config.qa_report_agent.output_directory = str(tmp_path)
        mock_config.qa_report_agent.scoring_weights = {
            "research": 0.3, "alignment": 0.2, "validation": 0.3, "generation": 0.2
        }
...
class TestV2PipelineIntegrity:
    ...
    def test_full_linear_success(self, orchestrator):
        ...
        with patch("pathlib.Path.exists", return_value=True), \
             patch("builtins.open", MagicMock(return_value=MagicMock(__enter__=MagicMock(return_value=MagicMock(read=MagicMock(return_value='{"capabilities": ["AI", "ML"]}')))))):
    ...
    def test_hop1_hybrid_override(self, orchestrator):
        ...
        orchestrator.llm.analyze = MagicMock(return_value={
            "archetype": "EXECUTIVE",
            "confidence": 0.9,
        ...
        with patch("pathlib.Path.exists", return_value=True), \
             patch("builtins.open", MagicMock(return_value=MagicMock(__enter__=MagicMock(return_value=MagicMock(read=MagicMock(return_value='{}')))))):
            result = orchestrator.run_mission(mission_input)
    ...
    def test_orchestrator_halt_on_failure(self, orchestrator):
        ...
        if result["status"] == "FAILED":
            assert "error" in result or "ORCHESTRATOR_ERROR" in [t["type"] for t in result["traces"]]
