"""Tests for the post_cascade_mcp_orphan_reap Windsurf hook.

Invariants the hook must uphold:
    1. ALWAYS exit 0 (must never block Cascade).
...
HOOK_PATH = (
    REPO_ROOT / ".windsurf" / "scripts" / "post_cascade_mcp_orphan_reap.py"
)
...
def _load_hook(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    spec = importlib.util.spec_from_file_location(
        "post_cascade_mcp_orphan_reap", HOOK_PATH
    )
...
def test_bypass_env_skips_detector(
...
def test_missing_detector_skips_silently(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    mod, log = _load_hook(tmp_path, monkeypatch)
...
def test_clean_scan_logged(
    ...
    fake = mock.MagicMock(
        stdout=detector_out, stderr="", returncode=0
    )
...
def test_orphans_reaped_logged(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    mod, log = _load_hook(tmp_path, monkeypatch)
    monkeypatch.delenv("MCP_ORPHAN_REAP_BYPASS", raising=False)
    detector_out = json.dumps({
        ...
        "orphan_count": 3,
        "orphans": [
            {"pid": 111, "started": "2026-04-23T10:00:00",
             "cmdline": "python tools.adg.mcp.server"},
            {"pid": 222, "started": "2026-04-23T10:00:01",
             "cmdline": "node filesystem_mcp_launcher.js"},
            {"pid": 333, "started": "2026-04-23T10:00:02",
             "cmdline": "npx notion-mcp-server"},
        ],
    })
    fake = mock.MagicMock(
        stdout=detector_out, stderr="", returncode=0
...
def test_detector_called_with_kill_json_gap(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    mod, log = _load_hook(tmp_path, monkeypatch)
    monkeypatch.delenv("MCP_ORPHAN_REAP_BYPASS", raising=False)
...
def test_detector_timeout_still_exits_zero(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    mod, log = _load_hook(tmp_path, monkeypatch)
    monkeypatch.delenv("MCP_ORPHAN_REAP_BYPASS", raising=False)
...
def test_detector_spawn_error_still_exits_zero(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    mod, log = _load_hook(tmp_path, monkeypatch)
    monkeypatch.delenv("MCP_ORPHAN_REAP_BYPASS", raising=False)
...
def test_malformed_detector_json_still_exits_zero(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    mod, log = _load_hook(tmp_path, monkeypatch)
    monkeypatch.delenv("MCP_ORPHAN_REAP_BYPASS", raising=False)
...
def test_hook_is_registered_in_hooks_json() -> None:
    """Regression guard: the hook must be wired into post_cascade_response."""
    hooks_path = REPO_ROOT / ".windsurf" / "hooks.json"
    data = json.loads(hooks_path.read_text(encoding="utf-8"))
    entries = data["hooks"]["post_cascade_response"]
    commands = [e["command"] for e in entries]
    assert any("post_cascade_mcp_orphan_reap.py" in c for c in commands), (
        "post_cascade_mcp_orphan_reap.py must be registered in "
        ".windsurf/hooks.json post_cascade_response chain"
    )
