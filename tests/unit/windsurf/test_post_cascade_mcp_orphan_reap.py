...
HOOK_PATH = REPO_ROOT / ".windsurf" / "scripts" / "post_cascade_mcp_orphan_reap.py"


def _load_hook(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Import the hook module with LOG_PATH redirected to tmp_path."""
    spec = importlib.util.spec_from_file_location("post_cascade_mcp_orphan_reap", HOOK_PATH)
