"""Tests for post_cascade_heartbeat + pre_user_prompt_hook_health_check (W7.1)."""
...
def heartbeat_mod(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    mod = _load("post_cascade_heartbeat_t", "post_cascade_heartbeat.py")
...
def test_heartbeat_main_writes_record(heartbeat_mod) -> None:
    ...
    assert record["hook"] == "post_cascade_heartbeat"
...
def test_heartbeat_disable_env_var(heartbeat_mod, monkeypatch: pytest.MonkeyPatch) -> None:
    mod, hb_path = heartbeat_mod
    monkeypatch.setenv("POST_CASCADE_HEARTBEAT_DISABLE", "1")
    rc = mod.main()
    assert rc == 0
...
def test_health_reports_ok_when_fresh(health_mod) -> None:
    ...
    now = 1_000_000.0
    hb_path.write_text(
        json.dumps({"timestamp_unix": now - 60.0, "hook": "post_cascade_heartbeat"}) + "\n",
        encoding="utf-8",
    )
...
def test_health_reports_stale_when_old(health_mod) -> None:
    ...
    now = 1_000_000.0
    hb_path.write_text(
        json.dumps({"timestamp_unix": now - 10_000.0, "hook": "post_cascade_heartbeat"}) + "\n",
        encoding="utf-8",
    )
    result = mod.check_heartbeat_health(path=hb_path, now=now, stale_seconds=3600)
...
def test_health_disable_env_var_silences(
    ...
    mod, _ = health_mod
    monkeypatch.setenv("POST_CASCADE_HEARTBEAT_HEALTH_DISABLE", "1")
    rc = mod.main()
    assert rc == 0
    err = capsys.readouterr().err
