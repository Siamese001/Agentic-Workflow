"""E2E: full post_cascade_response chain with real Windsurf payload shape.
...
What it proves:
  ...
  3. Each hook produces its expected observable artifact:
       ...
       - mcp_serialization log OR long_command log get entries when the
         response text includes the respective violation patterns
...
def _load_chain() -> list[Path]:
    """Parse hooks.json and return the list of post_cascade_response scripts."""
    data = json.loads(HOOKS_JSON.read_text(encoding="utf-8"))
    chain = data["hooks"]["post_cascade_response"]
    scripts: list[Path] = []
    for entry in chain:
        cmd = entry["command"]
...
class TestHooksJsonIntegrity:
    def test_hooks_json_is_valid(self) -> None:
        data = json.loads(HOOKS_JSON.read_text(encoding="utf-8"))
        assert "post_cascade_response" in data["hooks"]
    ...
    def test_heartbeat_is_first(self) -> None:
        ...
        chain = _load_chain()
        assert chain[0].name == "post_cascade_heartbeat.py", (
...
class TestFullChainRun:
    ...
    def test_heartbeat_jsonl_advances(self) -> None:
        before = self._before.get("post_cascade_heartbeat.jsonl", 0.0)
        payload = _make_payload(self.RESPONSE_TEXT)
        _run_hook(SCRIPTS / "post_cascade_heartbeat.py", payload)
    ...
    def test_deferred_scope_capture_reads_real_payload(self) -> None:
        ...
        payload = _make_payload(self.RESPONSE_TEXT)
        result = _run_hook(SCRIPTS / "post_cascade_deferred_scope_capture.py", payload)
