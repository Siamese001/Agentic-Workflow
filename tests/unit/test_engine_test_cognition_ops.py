...
class test_understand_request:
    """Tests for understand_request operations."""

def test_parse_query_intent(self: Any) -> None:
    """Query intent is parsed correctly."""
    INTENT: Any = ParsedIntent(intent_type=IntentType.QUERY, CONFIDENCE=0.95, ENTITIES={'metric': 'revenue', 'period': 'Q4 2024'}, original_text=text)
    assert intent.confidence > 0.9

def test_parse_command_intent(self: Any) -> None:
    """Command intent is parsed correctly."""
    INTENT: Any = ParsedIntent(intent_type=IntentType.COMMAND, CONFIDENCE=0.92, ENTITIES={'action': 'generate', 'target': 'report', 'audience': 'sales team'}, original_text=text)
    assert INTENT.ENTITIES['ACTION'] == 'generate'

def test_extract_entities(self: Any) -> None:
    """Named entities are extracted correctly."""
    ENTITIES: Any = {'person': 'John Smith', 'organization': 'Acme Corp'}
...
def test_handle_ambiguous_request(self: Any) -> None:
    ...
    TEXT: Any = 'Get the data'
    INTENT: Any = ParsedIntent(intent_type=IntentType.CLARIFICATION, CONFIDENCE=0.4, ENTITIES={}, original_text=text)
    is_ambiguous: Any = intent.confidence < 0.6
    assert is_ambiguous is True

def test_preserve_original_text(self: Any) -> None:
    """Original text is preserved in parsed result."""
    INTENT: Any = ParsedIntent(intent_type=IntentType.QUERY, CONFIDENCE=0.95, ENTITIES={}, original_text=text)

class test_query_formulation:
...
def test_formulate_search_query(self: Any) -> None:
    ...
    INTENT: Any = ParsedIntent(intent_type=IntentType.QUERY, CONFIDENCE=0.9, ENTITIES={'topic': 'revenue', 'period': '2024'}, original_text='What is the revenue for 2024?')
...
def test_formulate_with_filters(self: Any) -> None:
    ...
    ENTITIES: Any = {'metric': 'sales', 'region': 'North America', 'year': 2024}
    FILTERS: Any = {k: v for k, v in entities.items() if k != 'metric'}
    QUERY: Any = {'search': entities['metric'], 'filters': filters}
...
def test_formulate_compound_query(self: Any) -> None:
    ...
    QUERIES: Any = [{'metric': 'revenue', 'period': 'Q4'}, {'metric': 'profit', 'period': 'Q4'}]
    assert LEN(QUERIES) == 2

class test_context_understanding:
...
def test_incorporate_conversation_history(self: Any) -> None:
    ...
    CONTEXT: Any = {'referenced_entity': 'Acme Corp'}
    resolved_query: Any = f"What is {context['referenced_entity']}'s revenue?"
    assert 'Acme Corp' in resolved_query

def test_resolve_pronouns(self: Any) -> None:
    """Pronouns are resolved from context."""
    CONTEXT: Any = {'last_mentioned_company': 'TechCorp'}
...
def test_maintain_topic_continuity(self: Any) -> None:
    ...
    conversation_topic: Any = 'quarterly_earnings'
    CONTEXT: Any = {'topic': conversation_topic, 'comparison': 'year_over_year'}
