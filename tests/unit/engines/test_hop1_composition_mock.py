...
class TestHOP1CompositionMock(unittest.TestCase):
    ...
    def test_router_composition_direct_instantiation(self):
        ...
        router = K1Router(config=mock_self.config.__dict__ if hasattr(mock_self, 'config') and mock_self.config else {})
        ...
        self.assertEqual(router.config, {"env": "test", "mode": "mock"}, "Config not correctly propagated to Router.")
        ...
        self.assertNotIn("Agent", router.__class__.__name__, "Logic node should not have 'Agent' in name.")
