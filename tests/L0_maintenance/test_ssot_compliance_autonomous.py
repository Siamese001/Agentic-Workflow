...
from agentic_core.L0_maintenance.scripts.execute_ssot_compliance_protocol import (
    ...
    execute_phase5_final_validation,
    execute_territory_compliance,
    main
)
...
class TestAutonomousDecisionEngine:
    ...
    def test_decision_tracking(self, decision_engine):
        ...
        confidence = ConfidenceScore(value=0.9, reasoning="High", factors={})
        decision_engine.should_proceed_with_healing(
            confidence,
            {'violation_types': ['SHALLOW']}
        )
...
class TestPhase1Discovery:
    ...
    def test_phase1_mixed_violation_types(self, mock_agents, decision_engine):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('pathlib.Path.rglob', return_value=[Path(f'test{i}.py') for i in range(1, 5)]):
                drift_report, location_violations = execute_phase1_discovery(
                    mock_agents,
                    'prompt_governance',
                    decision_engine
...
class TestPhase2Alignment:
    ...
    def test_phase2_high_confidence_healing(self, mock_agents, decision_engine):
        ...
        mock_hierarchy.heal_hierarchy.return_value = {
            'total_healed': 3,
            'errors': []
        }
        mock_agents['hierarchy'].return_value = mock_hierarchy
...
class TestIntegration:
    ...
    @patch('agentic_core.L0_maintenance.scripts.execute_ssot_compliance_protocol.execute_phase0_validation')
    @patch('agentic_core.L0_maintenance.scripts.execute_ssot_compliance_protocol.execute_territory_compliance')
    ...
    def test_main_autonomous_mode(self, mock_phase0):
        ...
        with patch('agentic_core.L0_maintenance.scripts.execute_ssot_compliance_protocol.execute_territory_compliance') as mock_exec:
