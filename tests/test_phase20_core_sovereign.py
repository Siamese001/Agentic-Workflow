...
class TestCoreLogicSynthesis:
                        ...
                        if hasattr(node, 'names'):
                            for alias in node.names:
                                if any(zone in str(alias.name) for zone in forbidden_zones):
                                    pytest.fail(f"Circular Dependency: {f.name} imports {alias.name}")
    ...
    def test_healer_logic_synthesis(self):
        ...
        for method in synthesized_methods:
            assert f"SYNTHESIZED from {method}.py" in content, f"Missing synthesized logic from {method}"
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        ...
                        if class_name in class_names:
                            pytest.fail(f"Duplicate class '{class_name}' found in {f.name} and {class_names[class_name]}")
