...
def test_orphan_reduction():
    ...
    root_orphans = 0
    if ORPHAN_ROOT.exists():
        root_orphans = len([f for f in ORPHAN_ROOT.iterdir() if f.is_file() and f.name.endswith(".py")])
    
    assert root_orphans == 0, f"Still have unstructured orphans in root directory ({root_orphans})."
    print(f"✅ SUCCESS: Flat unit tests eliminated and orphans are properly structured.")
