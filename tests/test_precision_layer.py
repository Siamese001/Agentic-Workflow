...
class PrecisionLayerTestSuite:
    ...
    def test_contextual_compressor(self):
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            logger.info(f"   Compressed length: {result.compressed_length} chars")
            ...
            if test_case['min_ratio'] <= result.compression_ratio <= test_case['max_ratio']:
                logger.info(f"   ✅ Compression ratio within expected range")
            else:
                logger.info(f"   ⚠️  Compression ratio outside expected range "
    ...
    def test_adaptive_retrieval_gate(self):
        ...
        for query, expected_type, expected_retrieve in self.test_queries:
            ...
            logger.info(f"   Should Retrieve: {decision.should_retrieve}")
            ...
            if type_correct and retrieve_correct:
                logger.info(f"   ✅ Correct decision")
                correct_decisions += 1
            else:
                if not type_correct:
    ...
    1%}")
        ...
        logger.info(f"Average compression time: {avg_compression_time*1000:.2f}ms")

        if avg_compression_time < 0.05:  # 50ms threshold
            logger.info("✅ Compression meets performance requirement (< 50ms)")
        else:
            logger.info("⚠️  Compression exceeds performance requirement (> 50ms)")
    ...
    def test_convenience_functions(self):
        ...
        logger.info(f"   Preview: {compressed[:150]}...")
    ...
    def run_all_tests(self):
        ...
        logger.info("  • Improved efficiency with adaptive retrieval gating")
        logger.info("  • Performance: < 50ms compression time")
        logger.info("  • Safety: Fallback to original text if over-compressed")
