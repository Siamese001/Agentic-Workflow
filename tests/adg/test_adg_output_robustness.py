...
def _make_entity(adg_name, entity_type="module", layer="L2",
                 identity_kind="repo_module", confidence="HIGH",
                 resolved_path=""):
...
def _full_artifact():
    ...
    a.entities = [
        ...
        _make_entity("ADG::Module::tests/test_x.py", layer="L_TEST",
                     resolved_path="tests/test_x.py"),
        _make_entity("ADG::Symbol::json.loads", entity_type="symbol",
                     layer="L_EXTERNAL", identity_kind="external_module",
                     resolved_path=""),
    ]
    a.relations = [
        _make_relation("ADG::Module::x.py", "imports",
                       "ADG::Module::y.py", edge_kind="import"),
        _make_relation("ADG::Module::x.py", "calls",
                       "ADG::Symbol::json.loads", edge_kind="call"),
        _make_relation("ADG::Module::tests/test_x.py", "covers",
                       "ADG::Module::x.py", edge_kind="test_coverage"),
        _make_relation("ADG::Module::x.py", "violates",
                       "ADG::Module::y.py", edge_kind="layer_violation"),
    ]
    a.blind_spots = BlindSpotReport()
    a.structural_metrics = StructuralMetrics(
...
class TestLayerSplitterEdgeCases:
    ...
    def test_unknown_rel_type_dropped_from_all_planes(self):
        ...
        a.relations.append(
            _make_relation("ADG::Module::a.py", unknown_type, "ADG::Module::b.py")
        )
    ...
    def test_dangling_node_reference_creates_stub(self):
        ...
        a.relations.append(
            _make_relation("ADG::Module::a.py", "imports", "ADG::Module::ghost.py")
        )
        planes = split_artifact(a)
...
class TestLayerSplitterZeroOverlapInvariant:
    ...
    def test_split_artifact_has_no_test_graph_attribute(self):
        ...
        assert not hasattr(planes, "test_graph"), \
            "SplitArtifact.test_graph was removed in the ADG redesign"
...
class TestMultiWriterStateTransitions:
    ...
    def test_write_split_planes_false_skips_plane_files(self, tmp_path):
        ...
        paths = write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="20260101T000000Z",
            write_split_planes=False, write_sqlite=True,
        )
    ...
    def test_write_sqlite_false_skips_sqlite_file(self, tmp_path):
        ...
        paths = write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="20260101T000000Z",
            write_split_planes=True, write_sqlite=False,
        )
        assert paths.snapshot.exists()
    ...
    def test_create_latest_symlinks_true_creates_latest_files(self, tmp_path):
        ...
        assert len(latest_files) >= 2, \
            f"Expected ≥2 LATEST files, got: {latest_files}"
    ...
    def test_latest_files_overwritten_on_second_run(self, tmp_path):
        ...
        write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="20260102T000000Z",
            create_latest_symlinks=True,
        )
    ...
    def test_no_ts_creates_no_latest_files(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="",
            create_latest_symlinks=True,
        )
        latest_files = list(tmp_path.glob("adg_LATEST*"))
    ...
    def test_both_flags_false_only_snapshot_written(self, tmp_path):
        ...
        paths = write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="",
            write_split_planes=False, write_sqlite=False,
        )
        assert paths.snapshot.exists()
        assert not paths.sqlite.exists()
        assert not paths.file_graph.exists()
...
class TestMultiWriterSnapshotEdgeCases:
    ...
    def test_snapshot_has_all_required_top_level_keys(self, tmp_path):
        ...
        snap = json.loads(paths.snapshot.read_text())
        required = {
            "schema_version", "commit_sha", "scanner_digest", "artifact_digest",
            "counts", "graph_plane_counts", "by_layer", "blind_spots",
            "top_fan_in_hotspots", "top_fan_out_hotspots",
        }
...
class TestSQLiteWriterEdgeCases:
    ...
    def test_sqlite_recreated_if_already_exists(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="run1", write_sqlite=True
        )
        mtime_1 = paths.sqlite.stat().st_mtime_ns
        # Second write with different artifact, same path strategy — must not fail
        paths2 = write_all_artifacts(
            _minimal_artifact(), out_dir=tmp_path, ts="run1", write_sqlite=True
        )
    ...
    def test_sqlite_empty_artifact_produces_zero_rows(self, tmp_path):
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(
            _empty_artifact(), out_dir=tmp_path, ts="", write_sqlite=True
        )
    ...
    def test_sqlite_meta_schema_version_is_v4(self, tmp_path):
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(
            _minimal_artifact(), out_dir=tmp_path, ts="", write_sqlite=True
        )
        conn = sqlite3.connect(str(paths.sqlite))
    ...
    def test_sqlite_meta_total_rows_consistent(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="", write_sqlite=True
        )
        conn = sqlite3.connect(str(paths.sqlite))
        actual_nodes = conn.execute("SELECT COUNT(*) FROM nodes").fetchone()[0]
        actual_edges = conn.execute("SELECT COUNT(*) FROM edges").fetchone()[0]
        meta_nodes = int(conn.execute(
            "SELECT value FROM meta WHERE key='total_nodes'"
        ).fetchone()[0])
        meta_edges = int(conn.execute(
            "SELECT value FROM meta WHERE key='total_edges'"
        ).fetchone()[0])
        conn.close()
    ...
    def test_sqlite_symbol_column_empty_string_by_default(self, tmp_path):
        ...
        null_symbols = conn.execute(
            "SELECT COUNT(*) FROM edges WHERE symbol IS NULL"
        ).fetchone()[0]
        conn.close()
    ...
    def test_sqlite_foreign_key_src_dst_reference_valid_nodes(self, tmp_path):
        ...
        orphan_src = conn.execute(
            ...
            "(SELECT 1 FROM nodes n WHERE n.id = e.src_id)"
        ).fetchone()[0]
        orphan_dst = conn.execute(
            "SELECT COUNT(*) FROM edges e WHERE NOT EXISTS "
            "(SELECT 1 FROM nodes n WHERE n.id = e.dst_id)"
        ).fetchone()[0]
        conn.close()
...
class TestArtifactPathsContract:
    ...
    def test_artifact_paths_has_no_full_attribute(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(_minimal_artifact(), out_dir=tmp_path, ts="")
        assert not hasattr(paths, "full"), \
            "ArtifactPaths.full was removed; SQLite is now the canonical store"
    ...
    def test_artifact_paths_has_no_test_graph_attribute(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(_minimal_artifact(), out_dir=tmp_path, ts="")
        assert not hasattr(paths, "test_graph"), \
            "ArtifactPaths.test_graph was removed in the ADG redesign"
    ...
    def test_size_report_missing_file_shows_missing(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(
            _minimal_artifact(), out_dir=tmp_path, ts="",
            write_sqlite=False,
        )
        sizes = paths.size_report()
    ...
    def test_size_report_five_keys_exactly(self, tmp_path):
        ...
        assert set(sizes.keys()) == {
            "snapshot", "sqlite", "file_graph", "symbol_graph", "governance_graph"
        }
...
class TestRegressionGuards:
    ...
    def test_importing_test_graph_rels_raises_import_error(self):
        ...
        with pytest.raises(ImportError):
            from agentic_core.adg.artifact.layer_splitter import _TEST_GRAPH_RELS  # noqa: F401
    ...
    def test_cli_report_dict_no_full_key(self, tmp_path, capsys):
        ...
        rc = main(["--repo-root", str(ROOT), "build-artifacts",
...
class TestFailClosed:
    ...
    def test_plane_json_files_are_valid_json(self, tmp_path):
        ...
        from agentic_core.adg.artifact.multi_writer import write_all_artifacts
        paths = write_all_artifacts(
            _full_artifact(), out_dir=tmp_path, ts="", write_split_planes=True
        )
    ...
    def test_plane_json_has_schema_version_field(self, tmp_path):
        ...
        for path in (paths.file_graph, paths.symbol_graph, paths.governance_graph):
            ...
            assert data.get("schema_version") == "4.0.0", \
                f"{path.name} missing schema_version=4.0.0"


class TestMatrixWriteFlags:
    """Matrix: all combinations of write_split_planes × write_sqlite."""

    @pytest.mark.parametrize("write_planes,write_sq", [
        (True, True),
        (True, False),
        (False, True),
        (False, False),
    ...
    def test_flag_matrix_snapshot_always_written(self, tmp_path, write_planes, write_sq):
        ...
        paths = write_all_artifacts(
            _full_artifact(), out_dir=sub, ts="",
            write_split_planes=write_planes, write_sqlite=write_sq,
        )
    ...
    def test_flag_matrix_adg_full_never_written(self, tmp_path, write_planes, write_sq):
        ...
        sub = tmp_path / f"p{write_planes}_s{write_sq}"
        write_all_artifacts(
            _full_artifact(), out_dir=sub, ts="ts",
            write_split_planes=write_planes, write_sqlite=write_sq,
        )
    ...
    def test_flag_matrix_test_graph_never_written(self, tmp_path, write_planes, write_sq):
        ...
        assert not list(sub.glob("adg_test_graph*.json")), \
            "adg_test_graph.json must never be written"
