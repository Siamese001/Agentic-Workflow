...
Test inventory
--------------
Test 1: End-to-end artifact generation — all 7 files exist with expected sizes
...
Test 4: Split plane isolation — each plane contains only its designated edges
...
class TestArtifactGeneration:
    """Test 1 — all 7 artifact files exist with sensible sizes."""
    ...
    def test_tier1_snapshot_is_lightweight(self):
        ...
        # Must NOT contain raw entities/relations arrays
    ...
    def test_tier1_snapshot_has_counts(self):
        ...
        counts = data["counts"]
        # Keys are total_entities / total_relations (see multi_writer._build_snapshot)
        assert counts.get("total_entities", 0) > 0
        assert counts.get("total_relations", 0) > 0

    def test_tier2_full_exists(self):
        p = _require("adg_full_2026*.json", "Tier-2 full artifact")
        assert p.exists()

    def test_tier2_full_uses_schema_v4(self):
        p = _require("adg_full_2026*.json", "Tier-2 full artifact")
        data = json.loads(p.read_text())
        assert data.get("schema_version") == "4.0.0"

    def test_tier2_full_has_compact_nodes_and_edges(self):
        p = _require("adg_full_2026*.json", "Tier-2 full artifact")
        data = json.loads(p.read_text())
    ...
    def test_tier3_sqlite_exists(self):
    ...
    def test_test_graph_exists(self):
        p = _require("adg_test_graph_*.json", "test_graph")
        assert p.exists()
    ...
    def test_e7_graphsnap_exists(self):
        p = _require("adg_graphsnap_*.json", "E7 graph snapshot")
        assert p.exists()

    def test_seven_artifacts_have_matching_timestamps(self):
        """All 7 artifacts from the same run share a timestamp suffix."""
        full = _latest("adg_full_2026*.json")
        if full is None:
            pytest.skip("No full artifact found")
        # Extract timestamp: adg_full_20260311T160428Z.json → 20260311T160428Z
        ts = full.stem.split("_", 2)[-1]
        for pattern, label in [
            ...
            (f"adg_test_graph_{ts}.json", "test_graph"),
...
class TestSQLiteQueryValidation:
    ...
    def test_node_count_matches_full_artifact(self, conn):
        full = _latest("adg_full_2026*.json")
        if full is None:
            pytest.skip("No full artifact")
        data = json.loads(full.read_text())
        expected_nodes = len(data["nodes"])
        actual_nodes = conn.execute("SELECT COUNT(*) FROM nodes").fetchone()[0]
        assert actual_nodes == expected_nodes, (
            f"SQLite nodes={actual_nodes} ≠ full artifact nodes={expected_nodes}"
        )

    def test_edge_count_matches_full_artifact(self, conn):
        full = _latest("adg_full_2026*.json")
        if full is None:
            pytest.skip("No full artifact")
        data = json.loads(full.read_text())
...
class TestSplitPlaneIsolation:
    ...
    def planes(self):
        return {
            ...
            "test": json.loads(_require("adg_test_graph_*.json", "test_graph").read_text()),
    ...
    def test_test_graph_has_covers(self, rel_types):
        assert "covers" in rel_types["test"], "test_graph missing 'covers' edges"

    def test_test_graph_has_no_imports(self, rel_types):
        assert "imports" not in rel_types["test"], "test_graph must not contain 'imports' edges"
    ...
    def test_edge_counts_cover_all_relations(self):
        """Every edge in the full artifact must appear in at least one plane.

        Some relation types (e.g. 'in_cycle') are intentionally routed to
        multiple planes (file_graph AND governance_graph) so the sum can
        exceed the full count. We verify coverage instead of exact equality.
        """
        full = _latest("adg_full_2026*.json")
        if full is None:
            pytest.skip("No full artifact")
        full_data = json.loads(full.read_text())
        total_full = len(full_data["edges"])

        # Collect all relation types present in any plane
        all_plane_rels: set[str] = set()
        for pat in [
            "adg_file_graph_*.json",
            "adg_symbol_graph_*.json",
            "adg_test_graph_*.json",
            "adg_governance_graph_*.json",
        ]:
            p = _latest(pat)
            if p:
                plane_data = json.loads(p.read_text())
                # Resolve compact edge relation types via nodes
                all_plane_rels.update(e["r"] for e in plane_data["edges"])

        # Every relation type in full must be covered by at least one plane
        full_rel_types = {e["r"] for e in full_data["edges"]}
        uncovered = full_rel_types - all_plane_rels
        assert not uncovered, f"{len(uncovered)} relation types unassigned to any plane: {sorted(uncovered)}"

    def test_no_cross_contamination_between_any_pair(self):
        """Relation types exclusive to one plane must not appear in another plane's frozenset.

        Note: 'in_cycle' is intentionally shared (file + governance). We test
        the declared frozenset constants, not the live data.
        """
        ...
        # belongs_to_layer is file-only
        assert "belongs_to_layer" not in _SYMBOL_GRAPH_RELS
        assert "belongs_to_layer" not in _TEST_GRAPH_RELS
        assert "belongs_to_layer" not in _GOVERNANCE_GRAPH_RELS
        # covers_module, covers_symbol are test-only
        assert "covers_module" not in _FILE_GRAPH_RELS
        assert "covers_module" not in _SYMBOL_GRAPH_RELS
        assert "covers_module" not in _GOVERNANCE_GRAPH_RELS
        # bypasses_uwg, layer_authority_violation are governance-only
        assert "bypasses_uwg" not in _FILE_GRAPH_RELS
        assert "bypasses_uwg" not in _SYMBOL_GRAPH_RELS
        assert "bypasses_uwg" not in _TEST_GRAPH_RELS
        # calls, instantiates are symbol-only
        assert "calls" not in _FILE_GRAPH_RELS
        assert "calls" not in _TEST_GRAPH_RELS
...
class TestIncrementalScanAccuracy:
    ...
    def test_incremental_using_live_snapshot(self):
        """Incremental scan against the real live snapshot produces valid stats."""
        from agentic_core.adg.extraction.incremental import incremental_scan

        snapshot = _latest("adg_full_2026*.json")
        if snapshot is None:
            pytest.skip("No full artifact for snapshot")
        ...
        # Affected must be ≤ total (sanity)
        assert stats.affected_modules <= stats.total_modules
        # At least one module was affected
    ...
    def test_incremental_skips_majority_of_modules(self):
        ...
        if snapshot is None:
            pytest.skip("No full artifact for snapshot")
        # Only works with a v4.0.0 NormalizedGraph snapshot
        snap_data = json.loads(snapshot.read_text())
        if snap_data.get("schema_version") != "4.0.0":
            pytest.skip("Snapshot is not v4.0.0 — re-run generate_full_adg.py first")
...
class TestCLIIntegration:
    ...
    def test_build_artifacts_writes_to_custom_dir(self, tmp_path):
        ...
        # Run build-artifacts to the temp dir (uses cached scan)
        rc = main(["--repo-root", str(ROOT), "build-artifacts", "--output-dir", str(tmp_path)])
        assert rc == 0

        # All 7 output files should exist
        assert any(tmp_path.glob("adg_snapshot_*.json")), "snapshot missing"
        assert any(tmp_path.glob("adg_full_*.json")), "full missing"
        ...
        assert any(tmp_path.glob("adg_test_graph_*.json")), "test_graph missing"
    ...
    def test_incremental_scan_explicit_changed_files(self, capsys):
        ...
        if snapshot is None:
            pytest.skip("No full artifact for snapshot")
        # Verify the snapshot is a valid NormalizedGraph v4 (not a legacy file)
        import json as _json

        snap_data = _json.loads(snapshot.read_text())
        if snap_data.get("schema_version") != "4.0.0":
            pytest.skip(
                f"Snapshot is not v4.0.0 ({snap_data.get('schema_version')}) — re-run generate_full_adg.py"
            )
...
class TestSizeAndPerformance:
    """Test 8 — Tier-2 is measurably smaller than a legacy verbose artifact."""

    def test_tier2_smaller_than_latest_legacy_artifact(self):
        """adg_full schema-v4 must be smaller than any schema-v3 artifact of same run count."""
        v4 = _latest("adg_full_2026*.json")
        if v4 is None:
            pytest.skip("No Tier-2 artifact found")

        v4_size = v4.stat().st_size
        # Legacy schema-v3 artifacts had "schema_version": "2.0" or "3.0.0"
        # The largest legacy file in the repo was ~60 MB
        # Tier-2 should be under 35 MB for the same scan
        assert v4_size < 35 * 1024 * 1024, (
            f"Tier-2 too large: {v4_size / 1024 / 1024:.1f} MB (expected < 35 MB)"
        )
    ...
    def test_tier2_has_correct_entity_and_relation_counts(self):
        v4 = _require("adg_full_2026*.json", "Tier-2 full artifact")
        data = json.loads(v4.read_text())
        node_count = len(data["nodes"])
        edge_count = len(data["edges"])
        # Known live counts from the generated artifact
    ...
    def test_split_planes_individually_smaller_than_full(self):
        full = _latest("adg_full_2026*.json")
        if full is None:
            pytest.skip("No full artifact")
        ...
        for pat in [
            ...
            if p:
                assert p.stat().st_size < full_size, (
                    ...
                    f"than full artifact ({full_size / 1024:.0f} KB)"
                )
