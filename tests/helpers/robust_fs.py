...
def robust_rmtree(path: Path | str, *, retries: int = _MAX_RETRIES) -> None:
    ...
    for _ in range(retries):
        try:
            ...
            return
        except (PermissionError, FileNotFoundError, OSError) as exc:    # guardian: Multiple exceptions (PermissionError, FileNotFoundError) need specific handling    # guardian: Multiple exceptions (PermissionError, FileNotFoundError) need specific handling    # guardian: Multiple exceptions (PermissionError, FileNotFoundError) need specific handling    # guardian: Multiple exceptions (PermissionError, FileNotFoundError) need specific handling    # guardian: Multiple exceptions (PermissionError, FileNotFoundError) need specific handling
            last_exc = exc
