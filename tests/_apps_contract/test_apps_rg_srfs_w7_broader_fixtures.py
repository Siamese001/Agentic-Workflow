...
def test_w7_representative_offline_runs_section_metric_receipt(
    section: str,
    w7_realistic_nested_path: Path,
    ...
    rec_path = run_dir / "section_metric_receipt.json"
    assert rec_path.is_file(), section
    rec = json.loads(rec_path.read_text(encoding="utf-8"))
    for k in W6_FIELDS:
        assert k in rec, f"{section}:{k}"
...
def test_w7_cli_headline_smoke_offline_stub(
    w7_realistic_bare_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    # Non-interactive; avoid wizard / prompts.
    env.pop("APPS_RG_INTERACTIVE_STDIN", None)
    cmd = [
        sys.executable,
        "-m",
        "apps_rg",
    ...
    receipt = art / "section_metric_receipt.json"
    assert receipt.is_file(), (cp.stdout, cp.stderr)
    rec = json.loads(receipt.read_text(encoding="utf-8"))
    assert rec.get("proof_pool_type") == "selected_role_fact_set"
    assert rec.get("full_resume_srfs_supported") is False
