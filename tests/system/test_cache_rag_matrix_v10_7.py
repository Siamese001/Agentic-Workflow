...
@pytest.mark.parametrize("provider,model,temps", [
    ("openai","gpt-test",[0.0,0.1,0.2]),
    ("anthropic","claude-3",[0.0,0.3]),
    ("gemini","1.5-pro",[0.2,0.5]),
])
def test_cache_roundtrip_variants(cache_manager: CacheManager, provider, model, temps):
    for t in temps:
        key = f"{provider}-{model}-{t}"
        asyncio.run(cache_manager.set_llm_cache(provider, model, key, t, {"content": key}))
        got = asyncio.run(cache_manager.get_llm_cache(provider, model, key, t))
