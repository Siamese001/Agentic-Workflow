...
class ColoredProgress:
    ...
    def scan_skipped_tests() -> tuple[int, int]:
        ...
        if HAS_TQDM:
            iterator = tqdm(
                test_files,
                desc=f"{ColoredProgress.CYAN}Scanning tests{ColoredProgress.RESET}",
                bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}",
            )
        else:
            iterator = test_files
            print(
                f"{ColoredProgress.CYAN}Scanning {len(test_files)} test files...{ColoredProgress.RESET}"
            )
    ...
    def remove_skips_with_progress(dry_run: bool = False) -> tuple[int, int]:
        ...
        if HAS_TQDM:
            ...
            iterator = test_files
            print(
                f"{ColoredProgress.GREEN}Processing {len(test_files)} files...{ColoredProgress.RESET}"
            )
        ...
        for py_file in iterator:
            try:
                ...
                if HAS_TQDM:
                    tqdm.write(
                        f"{ColoredProgress.RED}Error in {py_file.name}: {e}{ColoredProgress.RESET}"
                    )
    ...
    def print_summary(files: int, skips: int, action: str = "found"):
        ...
        color = (
            ColoredProgress.GREEN
            if skips < 200
            else ColoredProgress.YELLOW
            if skips < 400
            else ColoredProgress.RED
        )
