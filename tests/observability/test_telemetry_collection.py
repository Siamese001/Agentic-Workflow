...
class TelemetryCollector:
    ...
    def record_test_execution(self, test_metrics: TestExecutionMetrics):
        ...
        with self.collection_lock:
            ...
            self.record_metric("test_execution_time", MetricType.TIMER, test_metrics.execution_time,
                             tags={"test_type": test_metrics.test_type, "success": str(test_metrics.success)})
            self.record_metric("test_success", MetricType.COUNTER, 1.0 if test_metrics.success else 0.0,
                             tags={"test_type": test_metrics.test_type})
            
            if not test_metrics.success:
                self.record_metric("test_failures", MetricType.COUNTER, 1.0,
                                 tags={"test_type": test_metrics.test_type})
    ...
    def record_quality_metrics(self, quality_metrics: QualityMetrics):
        ...
        with self.collection_lock:
            ...
            self.record_metric("quality_overall_score", MetricType.GAUGE, quality_metrics.overall_score,
            ...
            for component, score in quality_metrics.component_scores.items():
                self.record_metric("quality_component_score", MetricType.GAUGE, score,
                                 tags={"domain": quality_metrics.domain, "component": component})
            
            self.record_metric("quality_evaluation_time", MetricType.TIMER, quality_metrics.evaluation_time,
    ...
    def export_metrics(self) -> Dict[str, Any]:
        ...
        with self.collection_lock:
            return {
                ...
                "gauges": self.gauges,
                "histograms": {k: self.get_metric_statistics(k.split(":")[0], MetricType.HISTOGRAM) 
                              for k in self.histograms.keys()},
                "timers": {k: self.get_metric_statistics(k.split(":")[0], MetricType.TIMER) 
...
class TestTelemetryCollection:
    ...
    def test_metric_recording_and_aggregation(self):
        ...
        self.collector.record_metric("test_counter", MetricType.COUNTER, 1.0, tags={"test": "unit"})
        self.collector.record_metric("test_counter", MetricType.COUNTER, 2.0, tags={"test": "unit"})
        self.collector.record_metric("test_gauge", MetricType.GAUGE, 42.5, tags={"test": "unit"})
        self.collector.record_metric("test_histogram", MetricType.HISTOGRAM, 1.0, tags={"test": "unit"})
        self.collector.record_metric("test_histogram", MetricType.HISTOGRAM, 3.0, tags={"test": "unit"})
        self.collector.record_metric("test_timer", MetricType.TIMER, 0.1, tags={"test": "unit"})
    ...
    def test_metric_export_and_analysis(self):
        ...
        test_metrics = TestExecutionMetrics("export_test", "unit", 0.12, True, memory_usage=80.0)
        quality_metrics = QualityMetrics("job_matching", 0.75, {"alignment": 0.8, "coverage": 0.7}, 0.002, "good")
        
        self.collector.record_test_execution(test_metrics)
        self.collector.record_quality_metrics(quality_metrics)
        self.collector.record_metric("custom_metric", MetricType.GAUGE, 42.0, tags={"type": "custom"})
...
class TestObservabilityIntegration:
    ...
    def test_e2e_workflow_observability(self):
        ...
        workflow_success = all(mock_e2e_phase(name, duration, success) for name, duration, success in phases)
        ...
        phase_metrics = {tm.test_name: tm for tm in collector.test_metrics}
        assert "e2e_planning_phase" in phase_metrics
        assert "e2e_execution_phase" in phase_metrics
        
        # Execution phase should take longest
        execution_time = phase_metrics["e2e_execution_phase"].execution_time
        planning_time = phase_metrics["e2e_planning_phase"].execution_time
    ...
    def test_quality_scoring_observability(self):
        ...
        resume_scores = [qm.overall_score for qm in collector.quality_metrics if qm.domain == "resume_analysis"]
        job_scores = [qm.overall_score for qm in collector.quality_metrics if qm.domain == "job_matching"]
        ...
        gauge_keys = [k for k in collector.gauges.keys() if "quality_overall_score" in k]
        assert len(gauge_keys) >= 2  # resume_analysis + job_matching
        
        # Check timer metrics for evaluation time
        timer_stats = collector.get_metric_statistics("quality_evaluation_time", MetricType.TIMER)
    ...
    def test_observability_dashboard_simulation(self):
        ...
        dashboard_kpis = {
            ...
            # Test type breakdown
            "unit_tests": len([tm for tm in collector.test_metrics if tm.test_type == "unit"]),
            "integration_tests": len([tm for tm in collector.test_metrics if tm.test_type == "integration"]),
            "e2e_tests": len([tm for tm in collector.test_metrics if tm.test_type == "e2e"]),
            "golden_tests": len([tm for tm in collector.test_metrics if tm.test_type == "golden"]),
            
            # Performance metrics
            "fastest_test": min(tm.execution_time for tm in collector.test_metrics),
            "slowest_test": max(tm.execution_time for tm in collector.test_metrics),
        ...
        assert float(dashboard_kpis["fastest_test"]) < float(dashboard_kpis["slowest_test"])
