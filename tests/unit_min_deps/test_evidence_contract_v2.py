...
def test_accepts_valid_code_commit():
    ...
    with patch.object(sys, "argv", ["test_evidence_contract_v2.py", "--code-commit", "a" * 40]):
...
def test_validate_scope_containment_violations():
    ...
    files_out_of_scope = ["docs/technical/drill-down.md", "scripts/deploy.sh", "temp/file.txt"]
...
def test_validate_scope_containment_allowed():
    ...
    files_in_scope = ["apps_lic/engines/test.py", "apps_rg/engines/test.py", "apps_lic/subdir/file.py"]
...
def test_build_evidence_sections():
    ...
    with (
        patch.object(contract, "get_changed_files") as mock_get_files,
        ...
        sections = contract.build_evidence_sections(
            "a" * 40, evidence_commit="b" * 40, inspected_files=["inspected.py"]
        )
...
def test_validate_evidence_contract_structure():
    ...
    with (
        patch.object(contract, "validate_commit_hash") as mock_hash,
