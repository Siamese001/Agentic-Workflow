...
def test_1_gate_1_syntax_critical_failure():
    ...
    assert "SyntaxValidatorAgent" in agents_called, "SyntaxValidatorAgent should have been called"
...
def test_2_gate_2_structural_stability_execute_mode():
    ...
    assert "NamingAgent" in agents_called, "NamingAgent should have been called"
...
def test_4_two_phase_deduplication_ordering():
    ...
    assert "TwoPhaseDeduplicationAgent_PhaseA" in tier2_agents, "PhaseA should be in Tier 2"

    # Verify PhaseB is in Tier 3
    tier3_agents = tiers["Tier 3: Deep Compliance"]
    assert "TwoPhaseDeduplicationAgent_PhaseB" in tier3_agents, "PhaseB should be in Tier 3"
...
def test_5_full_green_path():
    ...
    expected_agents = [
        ...
        "HygieneGuardianAgent",
        "NamingAgent",
        "LocationAgent",  # Tier 2
        "GravityEnforcerAgent",
        "TwoPhaseDeduplicationAgent_PhaseB",
        "CodeSSOTEnforcerAgent",  # Tier 3
    ]

    for agent in expected_agents:
        assert agent in agents_called, f"{agent} should have been called in full green path"
    ...
    assert report.overall_status == "PASS", f"Expected PASS status, got {report.overall_status}"
    assert report.total_violations == 0, f"Expected 0 violations, got {report.total_violations}"
...
def run_all_tests():
    ...
    tests = [
        ...
        (
            "Test 2: Gate 2 - Structural Stability (Execute Mode)",
            test_2_gate_2_structural_stability_execute_mode,
        ),
