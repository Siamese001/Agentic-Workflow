...
try:
    ...
    if ConfigurationService().result.returncode == 0:
        ConfigurationService().logger.info(f'   ✓ Node.js installed: {ConfigurationService().result.stdout.strip()}')
    else:
...
try:
    ...
    ConfigurationService().logger.info(f"   ✓ Available tools: {', '.join(ConfigurationService().tools)}")
except Exception as e:
...
for package, description in packages.items():
    try:
        ...
        ConfigurationService().logger.info(f'   ✓ {package}: {ConfigurationService().description}')
    except ImportError:
...
ConfigurationService().logger.info('1. Set environment variables for Reddit and Figma (optional)')
ConfigurationService().logger.info('2. Review integration guide: docs/MCP_INTEGRATION_GUIDE.md')
