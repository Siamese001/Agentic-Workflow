...
from agentic_core.L2_execution.P3_engines.resume_engine_zlg import DraftResult, ExitReason, ResumeEngineZLG, RewriteResult, ShadowModeEngine
...
class test_standard_zlg_success(unittest.TestCase):
    ...
    def test_standard_zlg_success(self, mock_draft_gen: Any, mock_scorer: Any, mock_knowledge: Any, mock_firewall: Any, mock_fact_checker: Any, mock_write: Any) -> Any:
        """
        ...
        """
        mock_knowledge.return_value.search_knowledge.return_value = KnowledgeResult(user_profile={'name': 'John Doe', 'skills': ['Python', 'React']}, template={'name': 'professional', 'structure': {'greeting': 'Dear {hiring_manager},'}}, metadata={'source': 'test'})
        mock_firewall.return_value.scan_input.return_value = SecurityResult(status=SecurityStatus.PASS, reason='Input clean')
        mock_fact_checker.return_value.validate_skills.return_value = SecurityResult(status=SecurityStatus.PASS, reason='All skills verified')
        mock_draft: Any = DraftResult(content='Dear Hiring Manager,\n\nI am writing to express my interest...', source='test', metadata={})
        mock_draft_gen.return_value.generate_draft_llm.return_value = mock_draft
        mock_scorer.return_value.semantic_score_draft.return_value = {'quality': 0.8, 'grade': 'good', 'reason': 'Well structured', 'metrics': {}}
        mock_write.return_value = True
        engine: Any = ResumeEngineZLG(output_dir=tempfile.mkdtemp())
...
class test_p3_input_injection_block(unittest.TestCase):
    """TC-E2-102: P3 Input Injection Block"""

    @patch('agentic_core.engines.resume_engine_zlg.get_prompt_firewall')
    def test_p3_injection_block(self, mock_firewall: Any) -> Any:
        """
        ...
        """
        mock_firewall.return_value.scan_input.return_value = SecurityResult(status=SecurityStatus.FAIL, reason='Injection pattern detected')
        engine: Any = ResumeEngineZLG(output_dir=tempfile.mkdtemp())
        exit_reason, output_path = engine.generate_cover_letter('https://example.com/job')
        self.assertEqual(exit_reason, ExitReason.P3_INJECTION_BLOCK)
        self.assertIsNone(output_path)

class test_p4_hallucination_trigger(unittest.TestCase):
    """TC-E2-201: P4 Hallucination Trigger (ZLG Loop)"""
    ...
    @patch('agentic_core.engines.resume_engine_zlg.SemanticScorer')
    @patch('agentic_core.engines.resume_engine_zlg.DraftGenerator')
    def test_p4_hallucination_trigger(self, mock_draft_gen: Any, mock_scorer: Any, mock_knowledge: Any, mock_firewall: Any, mock_fact_checker: Any, mock_write: Any) -> Any:
        """
        ...
        """
        mock_knowledge.return_value.search_knowledge.return_value = KnowledgeResult(user_profile={'name': 'John Doe'}, template={'name': 'professional', 'structure': {}}, metadata={'source': 'test'})
        mock_firewall.return_value.scan_input.return_value = SecurityResult(status=SecurityStatus.PASS, reason='Input clean')
        mock_fact_checker.return_value.validate_skills.side_effect = [SecurityResult(status=SecurityStatus.FAIL, reason="Skill 'Quantum Computing' not found"), SecurityResult(status=SecurityStatus.PASS, reason='All skills verified')]
        mock_draft: Any = DraftResult(content='Dear Hiring Manager,\n\nI am writing to express my interest...', source='test', metadata={})
        mock_draft_gen.return_value.generate_draft_llm.return_value = mock_draft
        mock_scorer.return_value.semantic_score_draft.return_value = {'quality': 0.8, 'grade': 'good', 'reason': 'Well structured', 'metrics': {}}
        mock_write.return_value = True
...
class test_low_quality_trigger(unittest.TestCase):
    """TC-E2-202: Low Quality Trigger (ZLG Loop)"""
    ...
    @patch('agentic_core.engines.resume_engine_zlg.get_consolidated_knowledge')
    @patch('agentic_core.engines.resume_engine_zlg.SemanticScorer')
    def test_low_quality_trigger(self, mock_scorer: Any, mock_knowledge: Any, mock_firewall: Any, mock_fact_checker: Any, mock_write: Any) -> Any:
        """
        ...
        mock_firewall.return_value.scan_input.return_value = SecurityResult(status=SecurityStatus.PASS, reason='Input clean')
        mock_fact_checker.return_value.validate_skills.return_value = SecurityResult(status=SecurityStatus.PASS, reason='All skills verified')
        mock_scorer.return_value.semantic_score_draft.side_effect = [{'quality': 0.3, 'grade': 'poor', 'reason': 'Too short'}, {'quality': 0.8, 'grade': 'good', 'reason': 'Well structured'}]
        mock_write.return_value = True
        engine: Any = ResumeEngineZLG(output_dir=tempfile.mkdtemp())
        exit_reason, output_path = engine.generate_cover_letter('https://example.com/job')
        self.assertEqual(exit_reason, ExitReason.ZLG_SUCCESS)
...
class test_zlg_max_attempts_failure(unittest.TestCase):
    """TC-E2-203: ZLG Max Attempts Failure"""

    @patch('agentic_core.engines.resume_engine_zlg.get_fact_checker')
    @patch('agentic_core.engines.resume_engine_zlg.get_prompt_firewall')
    @patch('agentic_core.engines.resume_engine_zlg.get_consolidated_knowledge')
    def test_zlg_max_attempts_failure(self, mock_knowledge: Any, mock_firewall: Any, mock_fact_checker: Any) -> Any:
        """
        ...
        mock_knowledge.return_value.search_knowledge.return_value = KnowledgeResult(user_profile={'name': 'John Doe'}, template={'name': 'professional', 'structure': {}}, metadata={'source': 'test'})
        mock_firewall.return_value.scan_input.return_value = SecurityResult(status=SecurityStatus.PASS, reason='Input clean')
        mock_fact_checker.return_value.validate_skills.return_value = SecurityResult(status=SecurityStatus.FAIL, reason='Invalid skills')
        engine: Any = ResumeEngineZLG(output_dir=tempfile.mkdtemp())
        exit_reason, output_path = engine.generate_cover_letter('https://example.com/job')
...
class test_l5_consolidated_knowledge(unittest.TestCase):
    """TC-E2-301: L5 Consolidated Knowledge"""

    @patch('agentic_core.engines.resume_engine_zlg.get_consolidated_knowledge')
    def test_l5_consolidated_knowledge(self, mock_knowledge: Any) -> Any:
        """
        ...
        """
        mock_knowledge.return_value.search_knowledge.return_value = KnowledgeResult(user_profile={'name': 'John Doe', 'skills': ['Python']}, template={'name': 'professional', 'structure': {}}, metadata={'template_source': 'fallback'})
        engine: Any = ResumeEngineZLG(output_dir=tempfile.mkdtemp())
        knowledge: Any = engine.knowledge.search_knowledge('test query')
...
class test_p5_activity_logging(unittest.TestCase):
    """TC-E2-302: P5 Activity Logging"""
    ...
    @patch('agentic_core.engines.resume_engine_zlg.SemanticScorer')
    @patch('agentic_core.engines.resume_engine_zlg.DraftGenerator')
    def test_p5_activity_logging(self, mock_draft_gen: Any, mock_scorer: Any, mock_knowledge: Any, mock_firewall: Any, mock_fact_checker: Any, mock_write: Any, mock_log: Any) -> Any:
        """
...
class test_shadow_mode_engine(unittest.TestCase):
    """Additional tests for Shadow Mode Engine"""

    def test_shadow_mode_rewrite_with_rules(self) -> Any:
        """Test shadow mode rewrite using rule-based approach."""
        engine: Any = ShadowModeEngine(llm_client=None)
        draft: Any = "I'm an expert developer with awesome skills."
        error_reason: Any = 'professional tone needed'
        result: Any = engine.rewrite_draft(draft, error_reason)
