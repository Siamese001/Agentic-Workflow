...
async def _setup_hunter_test_environment(ctx: ValidationContext) -> HallucinationHunter:
    ...
    # Using a generator expression for efficiency when only a count is needed.
    pipeline_output_signals_count = sum(1 for s in ctx.signals if s.startswith('PIPELINE_OUTPUT:'))
    logger.info(f"   Added {pipeline_output_signals_count} PIPELINE_OUTPUT signals")
...
def _prepare_hallucination_test_data(ctx: ValidationContext):
    """Prepares source and generated data for the hallucination test.""" # Fixed typo: "Prepar es" -> "Prepares"
    ...
    # Calculate character counts dynamically for accuracy
    logger.info(f"   Source data: {len(source_data.strip())} characters")
    logger.info(f"   Generated resume: {len(generated_resume.strip())} characters")
    ...
    if hasattr(ctx, 'integrity_reports'):
        for stage_name, report in ctx.integrity_reports.items():
            ...
            if report.hallucination_percentage > hunter.HALLUCINATION_THRESHOLD:
                logger.error("   🚨 HALLUCINATION THRESHOLD EXCEEDED")
...
def _log_hallucination_test_summary():
    ...
    logger.info("\nAudit Trail:")
    logger.info("  - Sidecar JSON files: {output_file}_audit.json (illustrative placeholder)")
...
async def test_claim_extraction():
    ...
    # Note: Calling a private method (_extract_claims) directly for testing purposes.
    # In production code, this would typically be accessed via a public interface.
