...
cache_dir: Any = PROJECT_ROOT / 'data' / 'cache'
logs_dir: Any = PROJECT_ROOT / 'data' / 'logs'
...
def mock_circuit_breaker() -> None:
    ...
    cb.get_metrics = MagicMock(return_value={'total_requests': 0, 'successes': 0, 'failures': 0, 'current_state': 'CLOSED'})
    return cb
...
def temp_workflow_dir(tmp_path: Any) -> None:
    ...
    workflow_dir: Any = tmp_path / 'workflows'
    workflow_dir.mkdir()
    return workflow_dir
