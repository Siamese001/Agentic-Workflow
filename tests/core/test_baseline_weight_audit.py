...
class TestBaselineWeightAudit:
    ...
    def test_l0_maintenance_baseline_weights(self):
        ...
        l0_patterns = [path for path in AST_PLACEMENT_SIGNALS.keys() 
    ...
    def test_weight_hierarchy_strict_ordering(self):
        ...
        min_constitutional = min(tier_weights["constitutional"]) if tier_weights["constitutional"] else float('inf')
        
        # Strict ordering: utilities < state < domain < safety < constitutional
        assert max_utility < min_state, f"TIER OVERLAP: Utilities (max {max_utility}) overlap with State (min {min_state})"
        assert max_state < min_domain, f"TIER OVERLAP: State (max {max_state}) overlap with Domain (min {min_domain})"
        assert max_domain < min_safety, f"TIER OVERLAP: Domain (max {max_domain}) overlap with Safety (min {min_safety})"
        assert max_safety < min_constitutional, f"TIER OVERLAP: Safety (max {max_safety}) overlap with Constitutional (min {min_constitutional})"
