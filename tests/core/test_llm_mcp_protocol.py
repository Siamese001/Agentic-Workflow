...
'''
        ...
        time_called = any(
            call["tool"] == "get_current_time" for call in tool_calls)
        ...
        # Check parameters
        write_call = next(
            call for call in tool_calls if call["tool"] == "write_file")
        time_call = next(
            call for call in tool_calls if call["tool"] == "get_current_time")
...
def test_real_gemini_api_integration():
    ...
    if not api_key:
        ...
        logger.info(
            "   To run this test: set GEMINI_API_KEY environment variable")
    ...
    try:
        ...
        if len(actual_tool_calls) >= 1:
            logger.info(
                f"✅ Tools executed: {[call['tool'] for call in actual_tool_calls]}")
...
def test_tool_response_format_validation():
    ...
    try:
        ...
        if passed == len(test_cases):
            ...
            logger.error(
                f"❌ {len(test_cases) - passed} response formats failed validation")
            return False
...
def test_orchestrator_tool_routing():
    ...
    try:
        ...
        import core_utils
        core_utils_tools = [attr for attr in dir(
            core_utils) if not attr.startswith('_')]
...
def main():
    ...
    # Run all tests
    results.append(("Mock LLM Tool Invocation",
                   test_mock_llm_tool_invocation()))
    results.append(("Tool Response Format Validation",
                   test_tool_response_format_validation()))
    results.append(("Orchestrator Tool Routing",
                   test_orchestrator_tool_routing()))
    results.append(("Real Gemini API Integration",
                   test_real_gemini_api_integration()))
