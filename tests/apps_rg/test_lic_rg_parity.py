...
class TestCyclicRetryLogic:
    ...
    async def test_retry_on_validation_failure(self):
        ...
        with patch('apps_rg.engines.quality.content_quality_engine.ContentQualityEngine.run') as mock_quality:
            mock_quality.return_value = None
            
            # Mock buffer to return failing quality report
            ctx.buffer.write = Mock()
            ctx.buffer.read = Mock(return_value={"status": "failed", "score": 50, "issues": ["weak verbs"]})
    ...
    async def test_max_retry_limit_enforcement(self):
        ...
        with patch('apps_rg.engines.quality.content_quality_engine.ContentQualityEngine.run') as mock_quality:
            mock_quality.return_value = None
            ctx.buffer.write = Mock()
            ctx.buffer.read = Mock(return_value={"status": "failed", "score": 30, "issues": ["many issues"]})
...
class TestFullArchitectureParity:
    ...
    async def test_complete_workflow_with_all_features(self):
        ...
        with patch('apps_rg.engines.hops.hop1_clerk_engine.ClerkExtractionEngine.run') as mock_hop1, \
             patch('apps_rg.engines.hops.hop2_enrichment_engine.DataEnrichmentEngine.run') as mock_hop2, \
             patch('apps_rg.engines.generation.k9_gap_closure_engine.GapClosureEngine.run') as mock_gap, \
             patch('apps_rg.engines.refinement.content_optimizer_engine.ContentOptimizerEngine.run') as mock_opt, \
             patch('apps_rg.engines.refinement.section_ranker_engine.SectionRankerEngine.run') as mock_rank, \
             patch('apps_rg.engines.safety.ats_compatibility_engine.ATSCompatibilityEngine.run') as mock_ats, \
             patch('apps_rg.engines.quality.content_quality_engine.ContentQualityEngine.run') as mock_quality:
            
            # Mock buffer operations
            ctx.buffer.write = Mock()
            ctx.buffer.read = Mock(return_value={
                "status": "passed",
                "score": 85,
                "valid": True,
                "issues": []
            ...
            if orch.toggles.use_persistent_tracing:
                trace_file = Path(f"logs/missions/full_parity_test/trace.jsonl")
...
class TestGapClosureValidation:
    ...
    def test_gap_4_subatomic_testing_closed(self):
        ...
        assert callable(getattr(orch, 'run_subatomic_test'))
