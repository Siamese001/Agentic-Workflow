...
def test_confidence_score_numeric_and_in_range():
    out = run_workflow({"resume": "ConfidenceTest", "jd": "AI Exec"})
    qa = out["resume"]["qa"]
...
def test_conflicting_summary_and_rag_claims_flagged():
    ...
    """
    out = run_workflow({"resume": "CONFLICT_TRIGGER", "jd": "AI Exec"})
    issues = _issues(out)
...
def test_empty_summary_triggers_qa_issue(text):
    out = run_workflow({"resume": text, "jd": "Anthropic"})
    issues = _issues(out)
...
def test_conflicting_claims_not_allowed(resume):
    ...
    """
    out = run_workflow({"resume": resume, "jd": "AI Director"})
    issues = _issues(out)
...
def test_role_semantics_invalid_trigger(role):
    out = run_workflow({"resume": "RoleTest", "jd": role})
    issues = _issues(out)
...
def test_semantic_validator_updates_metrics():
    out = run_workflow({"resume": "MetricsCase", "jd": "AWS"})
...
def test_draft_to_qa_alignment():
    out = run_workflow({"resume": "DQATest", "jd": "AI Exec"})
    resume = out["resume"]
...
def test_low_confidence_generates_remediation(resume_text):
    out = run_workflow({"resume": resume_text, "jd": "Analyst"})
    qa = out["resume"]["qa"]
