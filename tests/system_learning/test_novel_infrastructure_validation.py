"""Enhanced behavioral tests for agentic_core.embeddings.embedding_input_guard."""
...
def test_embeddinginputguard_is_instantiable():
    """EmbeddingInputGuard can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
...
def test_embeddinginputviolation_is_instantiable():
    """EmbeddingInputViolation can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
...
def test_guardedtext_is_instantiable():
    """GuardedText can be instantiated (if it's a class)."""
    try:
        mod = importlib.import_module(MODULE_PATH)
        cls = getattr(mod, class_name)
    except (ImportError, AttributeError) as e:
