...
class TestCVI002:
    ...
    def test_version_timestamp_validation(self, validator):
        ...
        test_cases = [
            ...
            ((current_time.replace(hour=current_time.hour - 1)).isoformat(), True),  # 1 hour ago
            ((current_time.replace(day=current_time.day - 1)).isoformat(), False),  # 1 day ago (stale)
            ((current_time.replace(hour=current_time.hour + 1)).isoformat(), True),  # Future (clock skew)
        ]
        
        for timestamp, should_be_valid in test_cases:
            version_time = datetime.fromisoformat(timestamp.replace("Z", "+00:00"))
    ...
    def test_cache_invalidation_on_version_update(self, validator):
        ...
        # Set initial cache
        mock_cache_with_invalidation("design:audit_result", {"status": "stale"})
        mock_cache_with_invalidation("design:fix_cache", {"fix": "old_fix"})
        
        # Update version (should invalidate related cache)
        mock_cache_with_invalidation("figma:latest_version", {"version": "v2.0.0"})
