...
from l1_protocol_handler import L1ProtocolHandler, ToolResult, ToolExecutionError, BlackoutProtocolError
...
class TestCVAdvancedAdversarial:
    ...
    def test_cv_a_005_path_traversal_defense(self, handler):
        ...
        for malicious_path in traversal_attempts:
            result = handler.execute_tool('read_file', {'path': malicious_path})
    ...
    def test_cv_a_006_ebp_trigger_prevention(self, handler):
        ...
        result_read = handler.execute_tool('read_file', {'path': 'src/test.py'})
    ...
    def test_cv_a_008_llm_jailbreak_via_tool_output(self, handler):
        ...
        with patch.object(handler, '_mock_tavily_search') as mock_search:
            mock_search.return_value = ToolResult(
                ...
                source_data=["https://docs.python.org", "https://example.com/admin", "https://security.example.com"]
            )
            
            # Execute the search
            result = handler.execute_tool('tavily_search', {'query': 'Python security'})
            ...
            assert "system(" not in result.content.lower(), "Should remove system() calls"
    ...
    def test_cv_a_008_complex_jailbreak_patterns(self, handler):
        ...
        with patch.object(handler, '_mock_tavily_search') as mock_search:
            ...
            result = handler.execute_tool('tavily_search', {'query': 'API security'})
