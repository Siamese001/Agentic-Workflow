...
class TestCVA004:
        ...
        def mock_protocol_verification(response):
            ...
            try:
                ...
                if not isinstance(parsed, dict):
                    protocol_errors.append("L1_PROTOCOL_ERROR: Response not an object")
                    return None
                
                if "status" not in parsed:
                    protocol_errors.append("L1_PROTOCOL_ERROR: Missing status field")
                    return None
                
                if parsed["status"] is None:
                    protocol_errors.append("L1_PROTOCOL_ERROR: Null status value")
                    return None
                
                return parsed
                
            except json.JSONDecodeError as e:
                protocol_errors.append(f"L1_PROTOCOL_ERROR: JSON decode failed - {str(e)}")
                return None
            except Exception as e:
                protocol_errors.append(f"L1_PROTOCOL_ERROR: Validation failed - {str(e)}")
                return None
        ...
        def mock_binary_detector(response):
            ...
            if isinstance(response, bytes):
                ...
                if response.startswith(b'\x89PNG'):
                    protocol_violations.append("L1_PROTOCOL_ERROR: Binary PNG response")
                elif response.startswith(b'%PDF'):
                    protocol_violations.append("L1_PROTOCOL_ERROR: Binary PDF response")
                elif response.startswith(b'\xCA\xFE\xBA\xBE'):
                    protocol_violations.append("L1_PROTOCOL_ERROR: Binary Java class")
                elif response.startswith(b'PK\x03\x04'):
                    protocol_violations.append("L1_PROTOCOL_ERROR: Binary ZIP response")
                else:
                    protocol_violations.append("L1_PROTOCOL_ERROR: Unknown binary response")
                return False
    ...
    def test_response_size_limits(self, validator):
        ...
        def mock_size_enforcer(response):
            ...
            if size > max_size:
                size_violations.append(f"L1_PROTOCOL_ERROR: Response too large - {size} bytes")
                return None
