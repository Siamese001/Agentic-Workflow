...
class TestCVU002:
    ...
    def test_redis_timeout_translation(self, validator):
        ...
        mock_cache = Mock()
        mock_cache.check.side_effect = redis.exceptions.TimeoutError("Redis timeout")
        ...
        assert "timeout" in result.get("message", "").lower() or result["status"] == "valid"
    
    def test_redis_connection_error(self, validator):
        """Test that Redis connection errors are handled"""
        mock_cache = Mock()
        mock_cache.check.side_effect = redis.exceptions.ConnectionError("Connection failed")
        validator.cache = mock_cache
    ...
    def test_cache_fallback_on_timeout(self, validator):
        ...
        mock_cache = Mock()
        mock_cache.check.side_effect = redis.exceptions.TimeoutError("Cache timeout")
