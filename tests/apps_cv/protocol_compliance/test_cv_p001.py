...
from l1_protocol_handler import L1ProtocolHandler, ToolResult, ToolExecutionError, GitConflictError
...
class TestCVProtocolCompliance:
    ...
    def test_cv_p_001_output_normalization(self, handler):
        ...
        with patch.object(handler, '_mock_tavily_search') as mock_search:
            mock_search.return_value = ToolResult(
                ...
                source_data=["https://docs.python.org/3/good-practices.html", "https://example.com/advanced-python.html"]
            )
            
            # Execute the tool
            result = handler.execute_tool('tavily_search', {'query': 'Python best practices'})
            
            # L1 Assertion: Result must be properly normalized
            assert isinstance(result, ToolResult), "Result must be ToolResult object"
            assert result.content == "Found 2 search results", "Content should be clean summary, not raw JSON"
            assert len(result.source_data) == 2, "Source data should preserve sanitized URLs"
    ...
    def test_cv_p_002_strict_input_schema_check(self, handler):
        ...
        test_cases = [
            ...
            ('read_file', {'path': 123}, 'Parameter path must be string, got int'),
            ('write_file', {'path': 'test.txt'}, 'Missing required parameter: content'),
            ('write_file', {'path': 456, 'content': 'test'}, 'Parameter path must be string, got int'),
            ('tavily_search', {'query': []}, 'Parameter query must be string, got list'),
            ('git_commit', {'message': None}, 'Parameter message must be string, got NoneType'),
        ]
    ...
    def test_cv_p_003_error_signal_integrity(self, handler):
        ...
        with patch.object(handler, '_mock_git_commit') as mock_commit:
            mock_commit.side_effect = GitConflictError("Merge conflict in feature branch")
            
            # Execute the tool
            result = handler.execute_tool('git_commit', {'message': 'conflict test'})
    ...
    def test_cv_p_004_no_side_effect_call_tracing(self, handler):
        ...
        with patch('builtins.__import__') as mock_import:
            ...
            mock_import.side_effect = lambda name, *args, **kwargs: mock_l5 if 'observations' in name else Mock()
