...
def test_post_init_logic():
    ...
    if len(init_bypasses) == 0:
        ...
        print(f"❌ FAIL: __post_init__ agent incorrectly flagged: {[v.message for v in init_bypasses]}")
        return False
...
def test_scope_exclusion():
    ...
    violations_from_excluded = [
        ...
        if any(excl in v.file_path for excl in excluded_dirs)
    ]
    
    if len(violations_from_excluded) == 0:
        print(f"✅ PASS: No violations from excluded directories. Total violations: {len(violations)}")
        return True
    else:
...
def test_violation_count_reduction():
    ...
    if len(init_bypasses) < 100:
        ...
        print(f"⚠️ WARNING: INIT_BYPASS count is still high ({len(init_bypasses)}). May need further calibration.")
