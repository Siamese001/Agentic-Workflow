...
class TestGovernanceResilience:
    ...
    def test_gr001_cost_overrun_l3(self, mock_pinecone, mock_brave):
        ...
        assert any(r.get("source") == "Pinecone_HighCost" for r in results)  # Fallback used
        assert any("quota" in str(r).lower() for r in results)  # Quota hit logged
    ...
    def test_gr002_redis_atomicity_failure_l4(self, mock_validator):
        ...
        mock_redis.exec = Mock(side_effect=Exception("Redis connection lost before EXEC"))
        ...
        with patch('canon_validator.redis_client', mock_redis):
            ...
            try:
                ...
                assert "Atomic State Integrity Error" in str(e) or "connection lost" in str(e).lower()
    ...
    def test_cost_tracking_enforcement(self, mock_validator):
        ...
        def track_cost(operation, cost):
            ...
            if cost_tracker["total"] > 100:  # $100 daily limit
                raise Exception(f"Daily cost limit exceeded: ${cost_tracker['total']}")
