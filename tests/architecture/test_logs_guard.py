#!/usr/bin/env python3
"""
Test for Logs & Outputs Governance Guard

CI enforcement test that ensures logs guard passes
...
def test_logs_guard_execution():
    """Test that logs guard executes and produces valid report."""
    root_path = Path(__file__).parent.parent.parent
    report_path = root_path / "artifacts" / "governance" / "logs_guard_report.json"

    # Execute logs guard
    result = subprocess.run(
        [sys.executable, str(root_path / "tools" / "governance" / "logs_guard.py")],
        capture_output=True,
        text=True,
        cwd=str(root_path),
    )
    ...
    assert len(report["violations"]) == 0, f"Logs governance violations detected. See {report_path}"

    # Assert files were actually scanned (optional - may be zero if no logs exist)
    ...
    for item in report["inventory"]:
        ...
        assert "kind" in item, "Inventory item missing 'kind' field"

    print("✓ Logs guard executed successfully")
...
def test_no_files_modified():
    """Test that logs guard does not modify any tracked files."""
    root_path = Path(__file__).parent.parent.parent
    ...
    assert changes_before == changes_after, f"Tracked files were modified by logs guard: {changes_after}"

    print("✓ No tracked files modified by logs guard")


if __name__ == "__main__":
    print("Running logs governance guard enforcement test...")

    try:
        test_logs_guard_execution()
        test_no_files_modified()
        print("\n✓ All logs guard tests passed")
    except AssertionError as e:
        print(f"\n✗ Test failed: {e}")
        print("See artifacts/governance/logs_guard_report.json for details")
        sys.exit(1)
