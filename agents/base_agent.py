...
class BaseAgent(ABC):
    ...
    def __init__(self, agent_id: str, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"BaseAgent {agent_id} initialized and connected to SwarmNetwork")
    ...
    def _consult_canon(
        ...
        context_vector = self.network.gatekeeper.embed_action(action_description)
    ...
    def _record_outcome(
        ...
        try:
            ...
            logger.debug(f"Recorded outcome for {self.agent_id}: {'SUCCESS' if success else 'FAILURE'}")
    ...
    def _update_latency(self, latency_ms: int):
        ...
        self.metrics["avg_latency_ms"] = ((current * count) + latency_ms) / (count + 1)
    ...
    def execute(self, task: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            self.current_token = self._consult_canon(
                action_description=task.get("action", f"Execute task for {self.agent_id}"),
                code=task.get("code"),
            ...
            # Record violation
            self._record_outcome(success=False, error_trace=str(CanonViolationError))
            raise
        except Exception as e:
            # Record failure
            latency_ms = int((time.perf_counter() - start_time) * 1000)
            self._record_outcome(success=False, latency_ms=latency_ms, error_trace=str(e))
