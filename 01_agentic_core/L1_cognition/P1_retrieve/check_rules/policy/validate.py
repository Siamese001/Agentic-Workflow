...
def sha256_of(path: Path) -> str:
    ...
    Returns:
        Hexadecimal SHA-256 hash string, or empty hash if file cannot be read.
    """
    h = hashlib.sha256()
    try:
...
class ValidationStep:
    """
    Represents a single validation step with tracking metadata.

    Attributes:
        step_id: Unique identifier for the step.
        step_name: Human-readable name.
        status: Current status (PENDING, RUNNING, COMPLETED, FAILED).
        start: ISO timestamp when step started.
        end: ISO timestamp when step ended.
...
class Phase05Validator:
    ...
    def _validate_ssot(self, step: ValidationStep) -> bool:
        ...
        try:
            ...
            self.fail("K1c", f"META unreadable: {e}")

        try:
            import yaml
            yaml.safe_load(ssot.read_text(encoding="utf-8"))
            self.ok("K1d")
        except (yaml.YAMLError, OSError, UnicodeDecodeError) as e:
            self.fail("K1d", f"SSoT YAML parse error: {e}")
