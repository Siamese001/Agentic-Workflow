...
cur.execute('SELECT COUNT(*), AVG(fan_in), MAX(fan_in), AVG(fan_out), MAX(fan_out) FROM mv_hotspot_centrality')
...
if max_fi and max_fi > 0:
    ...
    cur.execute('SELECT resolved_path, layer, fan_in, fan_out FROM mv_hotspot_centrality ORDER BY fan_in DESC LIMIT 10')
    print('\nTop 10 by fan_in:')
...
if max_score and max_score > 0:
    ...
    cur.execute('SELECT resolved_path, layer, direct_fan_in, hop2_fan_in, cone_risk_score FROM mv_dependency_cone_risk ORDER BY cone_risk_score DESC LIMIT 10')
    print('\nTop 10 by cone risk:')
    for row in cur.fetchall():
        layer = row[1] or 'N/A'
