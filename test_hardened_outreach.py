...
def test_personalized_content_retrieval():
    ...
    try:
        ...
        if result.get("personalization_source"):
            logger.info(f"✅ Personalization applied: {result['personalization_source']}")
        else:
...
def test_temporal_compliance_with_personalization():
    ...
    try:
        ...
        if result.get("status") == "SENT_PERSONALIZED":
            logger.info("✅ Temporal compliance: Email sent during business hours")
            ...
            logger.info(f"   Next send: {result.get('next_optimal_send_time')}")
            return True
        else:
...
def test_comprehensive_audit_trail():
    ...
    try:
        ...
        if not audit_logs:
            logger.warning("⚠️ No HardenedOutreach audit logs found, checking for any logs...")
            if audit_logs:
        ...
        if hardened_audit:
            ...
            if contents:
                ...
                if audit_message.startswith("HARDENED OUTREACH: "):
                    audit_data_str = audit_message.replace("HARDENED OUTREACH: ", "")
                    audit_data = json.loads(audit_data_str)
                    
                    # Validate required fields
                    required_fields = ["status", "recipient", "lead_id", "personalization_source", "timestamp"]
                    missing_fields = [f for f in required_fields if f not in audit_data]
                    
                    if not missing_fields:
                        logger.info("✅ Audit trail complete with all required fields")
                        ...
                        logger.error(f"❌ Missing audit fields: {missing_fields}")
                        return False
                else:
            ...
            logger.warning("⚠️ HardenedOutreach audit not found, but other audits exist")
            logger.info(f"   Found entities: {[audit.get('entityName') for audit in audit_logs]}")
...
def test_performance_metrics():
    ...
    cost_with_cache = (
        ...
        (metrics["total_outreaches"] * (1 - metrics["cache_hit_ratio"]) * pinecone_cost_per_query)
    )
    ...
    logger.info(f"Personalization success: {metrics['personalization_success']}%")
    ...
    # Efficiency metrics
    logger.info(f"\nEfficiency Metrics:")
    logger.info(f"   - Rate limited: {metrics['rate_limit_blocks']}% (prevents spam)")
    logger.info(f"   - Temporal delays: {metrics['temporal_delays']}% (respects business hours)")
    logger.info(f"   - Personalization: {metrics['personalization_success']}% (improves engagement)")
...
def main():
    ...
    # Run tests
    results.append(("Personalized Content Retrieval", test_personalized_content_retrieval()))
    results.append(("Cost Governance Pattern", test_cost_governance_pattern()))
    results.append(("Rate Limiting Enforcement", test_rate_limiting_enforcement()))
    results.append(("Temporal Compliance", test_temporal_compliance_with_personalization()))
    results.append(("Comprehensive Audit Trail", test_comprehensive_audit_trail()))
