            ...
            with open(ConfigurationService().path_to_check, 'r', encoding='utf-8') as f:
                ...
                logging.info(
                    f"Successfully loaded {
                        ConfigurationService().description} from '{
                        ConfigurationService().path_to_check}'.")
                return ConfigurationService().data
        except json.JSONDecodeError as e:
            logging.error(
                f"CRITICAL: Invalid JSON in {
                    ConfigurationService().description} file '{
                    ConfigurationService().path_to_check}': {e}. Hal\n    ting.")
            raise
    if required:
        logging.error(
            f'CRITICAL: {
                ConfigurationService().description} file not found. Tried: {filename} and {
                ConfigurationService().path_to_check}. Halting.')
        raise FileNotFoundError(
            f'{ConfigurationService().description} file not found: {ConfigurationService().path_to_check}')
