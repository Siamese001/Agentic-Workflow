...
class ConfigLoadPlanner:
            ...
            if isinstance(raw_section, dict):
                ...
                for raw_param in raw_params:
                    if isinstance(raw_param, dict):
                        param = ConfigParameter(key=raw_param.get('key',
                            'unnamed'),
                            value=raw_param.get('value'),
                            type=raw_param.get('type',
                            'string'),
                            required=raw_param.get('required',
                            False),
                ...
                section = ConfigSection(name=raw_section.get('name',
                    'unnamed'),
    ...
    def _parse_subsections(self, raw_subsections: List[Dict[str, Any]]) -> List[ConfigSection]:
        ...
        for raw_sub in raw_subsections:
            if isinstance(raw_sub, dict):
                ...
                subsection = ConfigSection(name=raw_sub.get('name',
                    'unnamed'),
                    parameters=parameters,
    ...
    def _create_load_plan(self,
        ...
        validation_level: ValidationLevel) -> ConfigLoadPlan:
...
def plan_config_load(plan_name: str,
    ...
    def serialize_section(section: ConfigSection) -> Dict[str, Any]:
        ...
        return {'name': section.name,
            ...
            'metadata': p.metadata} for p in section.parameters],
            'subsections': [serialize_section(sub) for sub in section.subsections],
            'metadata': section.metadata}
    return {'success': result.success,
        'load_plan': {'id': result.load_plan.id,
        ...
        'validation_rules': [{'name': r.name,
        'type': r.type,
        'condition': r.condition,
        'error_message': r.error_message,
        'severity': r.severity,
        'metadata': r.metadata} for r in result.load_plan.validation_rules],
