...
with ADGGraphWatchlistBuilder(sqlite_path) as builder:
    ...
    print(f'Thresholds: rev={rev_thresh:.2f}, bridge={bridge_thresh:.2f}, scc={scc_thresh:.2f}, blast={blast_thresh:.2f}')
    print()
    ...
    for item in watchlist[:5]:
        ...
        signals = []
        if high_rev: signals.append('rev')
        if high_bridge: signals.append('bridge')
        if high_scc: signals.append('scc')
        if high_blast: signals.append('blast')
        ...
        print(f'     Raw: rev={item.reverse_dep_score:.2f}, bridge={item.bridge_score:.2f}, scc={item.scc_cluster_size}, blast={item.blast_radius:.2f}')
...
if watchlist:
    ...
    for item in watchlist[:3]:
        ...
        layer_mult = 1.25 if item.layer in {'L0', 'L1', 'L2', 'L3', 'L4', 'L5', 'L6', 'L_APP', 'L_SHARED', 'L_RUNTIME'} else 1.0
        computed = base_total * layer_mult
        
        print(f'{item.rank}. {item.file[:40]}')
        print(f'   Weights: rev={rev_w:.2f} + bridge={bridge_w:.2f} + scc={scc_w:.2f} + blast={blast_w:.2f} = {base_total:.2f}')
