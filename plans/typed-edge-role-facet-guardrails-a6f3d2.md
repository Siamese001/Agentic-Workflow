...
> **ROOT CAUSE (refined — supersedes the "evidence sufficiency / proof-eligible facts" framing, which is DISPROVEN):** the block is **not** per-lane fact count — **competencies with all 26 eligible facts is ALSO rated `support_status='WEAK'`**, same as ibm_bullets with 3. The graph proof-pool evidence is rated plain **`WEAK`** (bottom tier) by agentic_core's evidence-metrics extractor for **every** graph-lane section, and core's W2 invariant (`final_evidence_contract.SUPPORT_STATUS_PASSING_VALUES = {PASS}`) passes **only `PASS`** (`WEAK`/`WEAK_WITH_CAVEATS`/`EMPTY` are all non-passing by design). So **no graph-lane section can satisfy `grounding_required` as currently produced** — a structural 0/11, independent of fact enrichment.
> **Next RCA (the real W2 unblock):** find *why* graph proof-pool evidence rates `WEAK` rather than `PASS` — likely the apps_rg graph→`EvidenceItem` construction under-populates fields the core extractor needs for `PASS` (e.g. `citation_anchor`, `confidence_score`, corroboration/source-diversity), OR the support-target is mis-calibrated for graph-lane proof pools. Locus: `apps_rg/runtime/spine/c0_fec_compose.py` + the graph→EvidenceItem path + `agentic_core` `evidence_metrics_extractor`. **Consult plan `apps-rg-e2e-gap-remediation-7e2d9c` first** (referenced by the fact-vectors bootstrap) for existing support-WEAK analysis before re-deriving. (Advisory non-blocker: L2_CACHE semantic-cache tried a HF-hosted SentenceTransformer offline → default-EF fallback; product block is FEC, not this.)
