...
| W2 | C0.2 retrieval truth + instrumentation | 🔄 In progress | PR 2 done (G8 per-lane traces). PR 3: ✅ G6 PASS-but-empty guard (status=PASS ⟹ selected_count>0) + ✅ G9 embedding-dimension parity (loud on 384≠1024 stored vs query) — 13 tests. ⏳ pending: G7 filter-discards-all + bounded fallback, G11/G14 base-resume identity-only |
