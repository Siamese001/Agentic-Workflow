...
| W2 | C0.2 retrieval truth + instrumentation | 🔄 In progress | PR 2 done (G8 per-lane traces). PR 3: ✅ G6 PASS-but-empty guard, ✅ G9 embedding-dimension parity, ✅ G7 filter-discards-all → bounded broad fallback + named reason (`filter_removed_all`). ⏳ pending: G11/G14 base-resume identity-only enforcement |
