...
| W2 | C0.2 retrieval truth + instrumentation | 🔄 In progress | PR 2 done (G8): per-lane C0.2 trace fields — raw dense hits / applied `where` / post-filter survivors / similarity threshold / embedding id+dim — additive, no behavior change, 40 tests green. PR 3 pending: G6 PASS-not-empty, G7 filter-discards-all, G9 embedding parity, G11/G14 base-resume identity-only |
