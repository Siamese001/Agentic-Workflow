...
dod_exempt: false
supersedes: [apps-rg-lane-reasoning-optimization-7c4e9b]
---
...
> **Relationship (not superseded):** `apps-rg-aig-e2e-remediation-e4b7c1` (parent, IN_PROGRESS) and `apps-rg-aig-remaining-lanes-closeout-d4e1f7` (W4 closeout in flight by a parallel session). Their *residuals* — (a) ibm per-slot fact↔content grounding, (b) headline governance-signal stochastic miss, (c) ibm_narrative cascade — are absorbed here as W4 scope and activate when d4e1f7 closes. `apps-rg-c02-bootstrap-gate-correctness-c02f1a` continues independently (evidence durability).
...
| G14 | No bullet judge-remediation: decisive judge fail = dead run; pool holds unused alternates | unify/ibm bullets | stochastic | W4 |
| G15 | Per-slot fact↔content semantic alignment uninstrumented before judge ("$10M ARR" drift) | ibm/unify bullets | stochastic | W4 |
...
| W4 | W4.1–W4.4 | **Stochastic-content repair coverage** — bullet judge-feedback reselection, headline signal rung, selection-time fact-entailment, pre-X2 short-circuit | ~55K | Bullet pool retains alternates; entailment scoped to selection-time check (not the unbounded content dig) | 🔲 TODO | G14: decisive judge fail triggers ONE reselection from existing pool (no new generation) before final verdict; G13: content-signal miss gets a targeted rung or prompt emphasis (flap rate →0 over 5 canaries); G15/G17: selector verifies bullet claims entailed by cited facts (catches "$10M ARR" drift pre-judge); G16: empty selection fails once pre-X2 with true reason |
...
| W4.1 | Bullet judge-feedback reselection (pool alternates, no new generation) | 🔲 TODO |
| W4.2 | Headline content-signal repair rung / prompt emphasis | 🔲 TODO |
| W4.3 | Selection-time fact-entailment check (per-slot claim ⊆ cited fact) | 🔲 TODO |
| W4.4 | Pre-X2 empty-selection short-circuit | 🔲 TODO |
...
|---|---|---|
| All 42 inventory gaps (G1–G42) | Yes — W1–W9 | — |
