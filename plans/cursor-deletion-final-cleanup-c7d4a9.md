...
| Repo path inventory `.cursor/` | 955 files / 8196 hits | Fixed-string scan excluding `.git`, `.venv`, `node_modules`, `__pycache__`, `.claude/worktrees` |
...
|---|---|---|
| `.github/workflows/author-gate-gates.yml` | Path filters and run command target `.cursor/schemas` / `.cursor/scripts` | Retarget to `.claude/schemas` and `.claude/governance/scripts` |
| `.github/workflows/apps-e2e-harness-nightly.yml` | Path filters target `.cursor/schemas/apps_e2e_*.schema.json` | Retarget to `.claude/schemas` if files exist; otherwise remove stale filter |
...
| `ops_scripts/ci/governance_w3_hook_audit_matrix.py` | Active constants point to `.cursor/hooks.json` and `.cursor/hooks/...` | Retarget to `.claude/settings.json` / `.claude/hooks/...` or archive |
| `ops_scripts/ci/check_mcp_config_sovereignty.py` | `.cursor/plans`, `.cursor/scripts/filesystem_mcp_launcher.js` | Retarget to `.claude/plans` and current launcher |
| `ops_scripts/ci/check_windsurf_config_schema.py` | Validates `.cursor/hooks.json` and `.cursor/mcp.json` | Retire or rewrite for `.claude/settings.json` and root `.mcp.json` |
...
| `tests/unit/agentic_core/L2_execution/healers/test_l2_cascade_router_ledger.py` | Reads `.cursor/schemas/router_l2_cascade_ledger.schema.sql` | Retarget to `.claude/schemas/router_l2_cascade_ledger.schema.sql` |
...
| `tests/unit/ops_scripts/hooks/windsurf/test_enforcement_gaps.py` | Expects removed legacy `hooks.json`; patches old module shape | Archive or rewrite against `.claude/settings.json`/current hook module |
...
- Retarget live registry/config defaults to root `.mcp.json` and `.claude/**`.
...
|---|---|---|
| `.github/workflows/author-gate-gates.yml` | Changed SSOT comment, pull request path filters, push path filters, and binder dry-run command from `.cursor/schemas` / `.cursor/scripts` to `.claude/schemas` / `.claude/governance/scripts`. | Successor schema/script files exist under `.claude/**`; YAML parse passed. |
| `.github/workflows/apps-e2e-harness-nightly.yml` | Changed active apps E2E schema path filters from `.cursor/schemas/apps_e2e_*.schema.json` to `.claude/schemas/apps_e2e_*.schema.json`; changed stale plan comment to historical `.claude/plans/_archive/...`. | Matching apps E2E schemas exist under `.claude/schemas`; YAML parse passed. |
| `.github/workflows/graph-skills-authority-ratchet.yml` | Changed stale plan comment from `.cursor/plans/...` to historical `.claude/plans/_archive/...`. | Comment-only; YAML parse passed. |
...
1. Update `.github/workflows/author-gate-gates.yml`.
   - Path filters:
     - `.cursor/schemas/decision_ledger.schema.sql` to `.claude/schemas/decision_ledger.schema.sql`
     - `.cursor/schemas/decision_record.schema.json` to `.claude/schemas/decision_record.schema.json` if present, or remove if obsolete
     - `.cursor/scripts/apply_ledger_schema.py` to `.claude/governance/scripts/apply_ledger_schema.py`
     - `.cursor/scripts/post_agent_author_gate_capture.py` to current `.claude/governance/scripts/...` successor if present
     - `.cursor/scripts/post_commit_outcome_binder.py` to `.claude/governance/scripts/post_commit_outcome_binder.py`
   - Run command:
     - `python .cursor/scripts/post_commit_outcome_binder.py ...` to current `.claude/governance/scripts/...`
2. Update `.github/workflows/apps-e2e-harness-nightly.yml`.
   - Retarget `.cursor/schemas/apps_e2e_*.schema.json` to `.claude/schemas/apps_e2e_*.schema.json` if matching files exist.
...
3. Review `.github/workflows/graph-skills-authority-ratchet.yml` and `.github/workflows/notion-plan-file-drift-nightly.yml`.
   ...
   - Otherwise update prose to `.claude/plans`.
...
| `agentic_core/L0_routing/config/path_constants.py` | Project-root markers and protected files use `.mcp.json` / `.claude/rules`; deprecated `CURSOR_*` aliases now resolve to current `plans/` or `.claude/**` locations. | Python compile passed. |
| `.claude/hooks/lib/claude_hook_common.py` | Removed `.cursor/mcp.json` fallback; MCP server key loading is root `.mcp.json` only; legacy execution token points to `.claude/governance/scripts/_legacy_windsurf`. | Importlib load returned 8 MCP server keys. |
...
| `tools/cursor/governance_dedup_e2e_verify.py` | Retargeted required hook path to `.claude/hooks/after_agent_governance_dispatch.py`, settings check to `.claude/settings.json`, and active plan count to `plans/`. | Python compile passed. |
| `.claude/governance/scripts/check_cursor_optimized_config.py` | Repaired moved-script repo-root detection; current checker reads `.claude/settings.json`, root `.mcp.json`, and `plans/`; current `.md` rule files are normalized to historical Option A identifiers. | `python .claude/governance/scripts/check_cursor_optimized_config.py --strict` passed with warnings only. |
| `tools/cursor/emit_governance_dispatch_shadow_baseline.py` | Dispatch hook source now reads `.claude/hooks/after_agent_governance_dispatch.py`. | Python compile passed. |
...
| `tools/generate/entrypoint_scanner.py` | Entrypoint scanner now reads `.claude/settings.json` and root `.mcp.json`; hook-command extraction handles nested Claude settings. | `scan_all_entrypoints()` returned 186 entrypoints. |
...
2. Review `.cursor` constants in `agentic_core/L0_routing/config/path_constants.py`.
   - Replace live constants with `.claude/plans`, `.claude/schemas`, `.claude/templates`, `.claude/governance/scripts`.
   - Keep old names only as deprecated aliases if imports require them, and make values point to `.claude/**`.
...
4. Review `.claude/hooks/lib/claude_hook_common.py`.
...
| Surface | Change | Verification |
|---|---|---|
| `tests/unit/agentic_core/L2_execution/healers/test_l2_cascade_router_ledger.py` | Schema fixture now reads `.claude/schemas/router_l2_cascade_ledger.schema.sql`. | Targeted pytest passed. |
...
| `tests/unit/ops_scripts/hooks/windsurf/test_enforcement_gaps.py` | Removed retired hooks.json assertions; now checks current `.claude/settings.json` `UserPromptSubmit` wiring. ADG health tests now cover SQLite snapshot semantics instead of the old subprocess MCP probe. | Targeted pytest passed. |
...
| File | Change | Verification |
|---|---|---|
| `ops_scripts/ci/check_no_cursor_refs.py` | Expanded active scanning to `.github`, `ops_scripts`, `tools`, `agentic_core`, `.claude/governance`, `.claude/hooks`, and `apps_*` for `.py`, `.ps1`, `.yml`, and `.yaml`; added detection for Python `Path`/`open`/glob/`endswith`, PowerShell `Join-Path`, and YAML/list path literals. | Unit coverage added; gate passes on current tree. |
...
| `ops_scripts/ci/check_mcp_config_sovereignty.py` and tests | Retargeted diagnostic launcher references from deleted `.cursor/scripts` to `.claude/governance/scripts`; tests now document that the standalone diagnostic reports `MISSING_FILESYSTEM` for current root `.mcp.json`, and the contract runner no longer invokes it. | Focused unit tests pass. |
...
| `python scripts/governance/verify_codex_backup.py` | Pass | Codex backup adapter verification passed |
...
| Tests keep passing for wrong historical behavior | Rewrite expectations to root `.mcp.json` and `.claude/**` |
...
- Live code defaults use `.claude/**` and root `.mcp.json`.
- Targeted tests pass.
