...
| W2 | P2 | Map 6 modules → 06.x chapters; reconcile RCA + gauntlet dupes | ~80k | Overlap is reconcilable, not divergent | TODO | Per-module target + merged API documented; no behavior dropped |
...
| P2 | Overlap reconciliation | RCA + gauntlet de-dup design | TODO |
...
## Wave W2 — Map modules → 06.x chapters; reconcile duplicates

| L6_learning module | Target in L6_system_learning | Reconcile against |
|---|---|---|
| `types.py` | `types/` (new `future_run_promotion_types.py`) | existing `types/rca_types.py` |
| `completed_run_evaluator.py` | 06.3/06.4 `engines/` | outcome-eval engines |
| `rca_synthesizer.py` | 06.5 `engines/` | **`engines/incident_rca_engine.py` (dup)** |
| `future_run_proposal_builder.py` | 06.6 `pipelines/`/`engines/` | proposal-authoring engines |
| `promotion_gauntlet.py` | 06.7 `engines/` | **`engines/gauntlet_gate.py` + `pipelines/approval_gates.py` (dup)** |
| `package_driven_l6_binding.py` | 06.7 top-level binding | UWG promotion entrypoint |
- Decide per duplicate: keep-one-merge vs keep-both-with-distinct-roles. Document the merged public
  API (what `from agentic_core.L6_learning import …` consumers must resolve to).
- ⚠️ Hard constraint: the `L6GauntletResult.gate_id` / G29 semantics and observer-law receipt
  fields must survive verbatim (G29 firewall + `test_l6_observer_law_prohibitions.py` depend on them).

## Wave W3 — Physical move + compat shim + receipt

- `git mv` modules into the mapped L6_system_learning locations; build `__init__` chain.
- Compat shim at `agentic_core/L6_learning/__init__.py` (+ per-module shims): `sys.modules`
  forward-alias to new paths + `DeprecationWarning` (W5 `l6-repo-reorganization` precedent).
