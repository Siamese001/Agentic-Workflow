...
| W2 | W2.1, W2.2 | **S1** Author-Gate machinery → archive (highest value, lowest risk) | ~40K | AG chain already unwired (verified 3×); §6 invariant lives in CLAUDE.md | 🔄 IN PROGRESS | Audit + classifier only; **no archival landed** — attempt REVERTED (skills imported by live AG tooling/tests; root `archives/` dir forbidden by `check_structure_policy`). Full S1 retirement (via `git rm`, history-preserved) DEFERRED to a lockstep pass |
