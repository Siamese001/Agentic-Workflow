...
created_in_session: cursor_agent
priority: medium
...
7. Delete this reminder file or move it to `.cursor/reminders/archived/`.
...
A tiny `.cursor/scripts/pre_user_prompt_reminder_check.py` could scan `.cursor/reminders/*.md`
at session start, parse the YAML frontmatter `due_date`, and if `today >= due_date` print to stderr:
```
[REMINDER] 2026-04-28: ADR-023 formal acceptance due — see .cursor/reminders/2026-04-28-adr-023-acceptance.md
```
