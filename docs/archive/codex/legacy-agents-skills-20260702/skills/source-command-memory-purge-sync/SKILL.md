---
name: "source-command-memory-purge-sync"
description: "Memory graph purge sync \u2014 purge stale entities and re-import fresh ADG context"
---

# source-command-memory-purge-sync

Use this skill when the user asks to run the migrated source command `memory-purge-sync`.
...
Migrated from source command `memory-purge-sync` into a Codex skill. Invoke it as `$source-command-memory-purge-sync` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
