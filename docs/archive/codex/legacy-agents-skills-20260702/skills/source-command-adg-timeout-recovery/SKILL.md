---
name: "source-command-adg-timeout-recovery"
description: "Automatic timeout recovery using AST dependency graph analysis"
---

# source-command-adg-timeout-recovery

Use this skill when the user asks to run the migrated source command `adg-timeout-recovery`.
...
Migrated from source command `adg-timeout-recovery` into a Codex skill. Invoke it as `$source-command-adg-timeout-recovery` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
