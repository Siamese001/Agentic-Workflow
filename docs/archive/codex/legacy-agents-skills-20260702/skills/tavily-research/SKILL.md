...
**PREREQUISITE:** `TAVILY_API_KEY` must be set as a Windows OS environment variable (`setx TAVILY_API_KEY tvly-...` then restart Claude Code). `pre_mcp_gate.py` blocks every Tavily call with an actionable message until the key is present — the skill itself does not need to verify the key.

This skill is the Claude Code-native adaptation of [Tavily's Agent Skills](https://docs.tavily.com/documentation/agent-skills). The upstream skills are designed for Claude Code's `npx skills add` framework; this skill maps the same five capabilities onto the Tavily MCP tool surface that Claude Code actually invokes.
...
Review unsupported Claude skill fields manually: `deprecated`, `enforcement_layer`, `enforcement_timing`, `enforcement_type`, `metadata`, `redirect_to`.
