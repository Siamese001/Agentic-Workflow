...
are logged to artifacts/windsurf/author_gate_capture_violations.jsonl so the
coverage gate can alarm on capture rot.

CALLED BY
    - post_cascade_author_gate_capture.py (advisory — logs then proceeds)
    - audit_ledger_coverage.py (retrospective audit of existing rows)
    - CLI: python .windsurf/scripts/author_gate_marker_validator.py --marker "<text>"
