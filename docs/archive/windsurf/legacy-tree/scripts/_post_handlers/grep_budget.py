...
In-process equivalent of `.windsurf/scripts/post_cascade_grep_budget_audit.py`.
Logs a row to `artifacts/windsurf/grep_budget_violations.jsonl` when combined
...
def run(parsed: ParsedResponse, repo_root: Path) -> None:
    ...
    log_path = repo_root / "artifacts" / "windsurf" / "grep_budget_violations.jsonl"
