...
Behavior (FAIL-CLOSED on explicit violations, FAIL-OPEN on parse errors):
  - Block reads of paths outside the repo root (with narrow exceptions for
    docs/windsurf mirror, Windsurf home config, user tmp screenshots).
  ...
  - Append every block decision to artifacts/windsurf/secret_scan.jsonl
...
AUDIT_DIR = REPO_ROOT / "artifacts" / "windsurf"
AUDIT_LOG = AUDIT_DIR / "secret_scan.jsonl"

# Paths outside the repo that are explicitly allowed for read
# (documented Windsurf conventions — see docs/windsurf/*.md and agent screenshots)
_ALLOWED_OUTSIDE_REPO = (
    ...
    "\\Temp\\TemporaryItems\\",
    "/var/folders/",
    # Windsurf user-home config (read-only probes)
    ".codeium/windsurf/",
    "\\.codeium\\windsurf\\",
)
