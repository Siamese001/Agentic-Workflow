...
| Plan-file work | ❌ No | `.windsurf/plans/<name>-<6hex>.md` |
...
5. **Don't replicate plan-file content.** Plans are SSOT in `.windsurf/plans/`. Tasks are queryable handles, not narrative.
...
- For tracking refactor waves → use a `.windsurf/plans/<name>-<6hex>.md` file with wave/phase tables.
