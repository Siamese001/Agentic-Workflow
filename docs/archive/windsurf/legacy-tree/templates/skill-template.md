...
<One-paragraph plain-English summary: what this skill does, what problem it solves, and what would go wrong without it. End with the upstream reference if any: "Adapts the upstream <foo> docs (<url>) to the Windsurf MCP architecture.">
...
- MCP server config: `.windsurf/mcp_config.json` → `<server-id>`
- Auth gate (if any): `.windsurf/scripts/<gate>.py`
- Intent detection: `.windsurf/scripts/pre_prompt_classifier.py` (`_<SKILL>_SIGNALS`)
- Authority registry: `docs/guides/MCP_Registry.md` → `<server-id>`
- Sibling skill: `<sibling-name>`
- Constitutional rules: `.windsurf/rules/<rule>.md` §<N>
