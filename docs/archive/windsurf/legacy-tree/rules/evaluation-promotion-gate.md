...
description: Apply when promoting any prompt, policy, rubric, or config change through UWG to L4 via v33 §6D. Enforces a mandatory regression-suite pass before commit.
---
...
`artifacts/windsurf/eval_promotion_bypass.jsonl` and permits a one-off commit.
