...
In-process equivalent of `.windsurf/scripts/post_cascade_read_budget_audit.py`.
Logs a row to `artifacts/windsurf/read_budget_violations.jsonl` when combined
...
def run(parsed: ParsedResponse, repo_root: Path) -> None:
    ...
    log_path = repo_root / "artifacts" / "windsurf" / "read_budget_violations.jsonl"
