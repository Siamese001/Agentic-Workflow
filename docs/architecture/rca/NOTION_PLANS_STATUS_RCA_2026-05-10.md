...
**Author**: RCA conducted by Codex during recovery session  
...
| **B** | `register_ondisk_plans_batch.py` and Codex-direct `mcp7_API-post-page` calls do not coordinate dedup, leaving 11 slugs with duplicate Notion rows | Phantom rows accumulated. When (A) hit, the Status flipped on multiple copies of the same slug | The "still seeing Not Started after I patched it" experience |
...
| Codex-direct `mcp7_API-post-page` (e.g. when I registered the recovery plan today) | ❌ No | Codex fires the MCP tool with no dedupe step |
...
**The gap**: when Codex emits an `<invoke name="mcp7_API-post-page">` for plan registration via §36, there's no pre-write check that "a row with this slug already exists". The §36 pre-prompt hook surfaces unregistered plans but doesn't dedup if the cache says missing-but-actually-present.
...
- Trigger likely: §36 cache miss → Codex or hook re-registered the slug at default Status
...
2. **Hook-level**: `pre_mcp_tool_use` hook for `API-post-page` targeting Plans DB performs the same dedupe query and **blocks** the MCP call if a row already exists. Codex's slug-from-payload extraction is the same regex as the post-cursor-agent audit hook.
...
- **Per-response, not bulk**: hook fires once per Codex response and only patches rows mentioned in that specific response's `<invoke>` blocks
