...
**Plan:** `.claude/plans/token-burn-followup-f8c2d1.md` W3 / F4
**Predecessor data:** `.claude/plans/windsurf-token-burn-augmentation-b7a3f1.md` W2 / P6 (MCP schema cost audit)
...
If all three answers are "no" → **RETIRE**: write retirement ADR; remove the entry from `mcp_config.json`; AGENTS.md regenerates via `python .claude/governance/scripts/sync_mcp_config.py`.
...
- AGENTS.md MCP Quick Reference (auto-regenerated by `.claude/governance/scripts/sync_mcp_config.py`)
- Author-Gate enforcement: `.claude/rules/author-gate-enforcement.md`
