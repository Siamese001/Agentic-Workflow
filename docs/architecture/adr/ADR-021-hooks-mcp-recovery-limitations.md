...
**Date:** 2026-04-08
**Status:** Accepted
**Context:** Investigation into why Cursor Agent hooks were not auto-diagnosing and restoring red ADG MCP server
...
- **Impact:** Cursor Agent cannot probe or restore ADG health because the probe itself is blocked
...
Windsurf provides 12 hook events, all triggered by **Cursor Agent actions** (read, write, command, MCP call, prompt, response). There is no hook for:
...
2. Hope that Cursor Agent reads the stderr message and takes corrective action
...
3. **Constitutional rule §13:** Cursor Agent rules instruct the model to run `mcp1_adg_health` at session start
4. **Manual recovery:** User clicks refresh button in Windsurf MCP panel, or Cursor Agent runs `/mcp-failure-rca` workflow
...
- [Windsurf Cursor Agent Hooks docs](https://docs.windsurf.com/windsurf/cascade/hooks)
