...
Every enabled server in root `.mcp.json` is part of the Codex MCP registry for this repo, but startup blocking is controlled per server. Each server entry owns its projected `required`, `startup_timeout_sec`, and `tool_timeout_sec` policy; optional and task-specific MCPs must not block a basic first turn. `adg_sqlite` and `memory` use persistent Streamable HTTP URL routes (`http://127.0.0.1:8765/mcp` and `http://127.0.0.1:8766/mcp`) instead of primary Python stdio commands; their stdio launchers are fallback/probe tools only. ADG and Memory availability is enforced by the relevant skills and Codex readiness/preflight gates when the task requires those capabilities, not by globally requiring every MCP during every prompt submission.
...
On Windows, both task actions and the supported desktop shortcut enter through the repo-owned synchronous `run_hidden_wait.vbs` adapter. It runs the requested executable with no visible console, waits for completion, and propagates the exact exit code. Task health requires the complete managed fingerprint (action, arguments, working directory, principal, enabled state, both triggers, restart policy, battery policy, execution limit, and `IgnoreNew`), a verified managed listener, initialize, tools/list, and the configured health tool. ADG treats Redis timeout as `continue_degraded`; Memory treats it as `block`. Each dependency timeout is declared in `codex_mcp_http_services.psd1`.

SessionStart occurs after MCP initialization, so it cannot repair a service that was unavailable while a task was being created. Its Windows lifecycle step is status-only and advisory. For any server deliberately marked `required = true`, reverting to primary stdio, inflating timeouts, or accepting a listener/process as readiness would hide the pre-initialization failure instead of assigning it a durable owner.

The required-only per-turn MCP protocol gate is advisory by default for ordinary analysis, design, and refactoring prompts so transport churn does not block low-risk innovation work. Selected required-server failures remain blocking for proof, eval, PR/publication, `main` closeout, and explicit strict prompts; malformed MCP config and missing required secret passthrough also always block. Use `REQUIRED_MCP_GATE_SERVERS` to select task-specific servers explicitly, `REQUIRED_MCP_GATE_ENFORCE=block` to make selected failures blocking, or `REQUIRED_MCP_GATE_ENFORCE=warn` for explicit warn-only transport checks.
...
Deferred calls nested under `functions.exec` are accepted only when the executed source contains exactly one approved `tools.mcp__<server>__<tool>` call and the response contains one fresh `codex-deferred-mcp-result/v1` object whose server, tool, configured endpoint, timestamp, and structured successful MCP result all agree. Shell output, HTTP probes, missing responses, fabricated `mcp__` text, transport failures, stale receipts, endpoint mismatches, non-proof tools, and non-managed servers never write proof.
