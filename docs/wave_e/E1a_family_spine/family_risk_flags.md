...
**Recommended E1c action:** For each of the six, locate the specific constitutional-tier source (`.codex/rules/constitutional.md`, AGENTS.md always-on) and record it as a `SRC-RULE-*` or `SRC-INT-*`. If no constitutional-tier source can be cited, downgrade to GOVERNANCE and log the downgrade.
