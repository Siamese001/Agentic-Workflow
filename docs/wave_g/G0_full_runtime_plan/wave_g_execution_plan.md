...
- Map `.codex/rules/`, `.codex/skills/`, `AGENTS.md` to enforcers where they exist (and mark doctrine-only where they don't).
...
Exit of G4b: every rule in `.codex/rules/` is mapped; every config knob is catalogued; every env key has consumer + plane; every prompt file is located.
...
- Map `.codex/hooks.json` to hook scripts.
