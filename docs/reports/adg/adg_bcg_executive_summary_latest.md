...
| What blocks merge? | 2 P0 blocker row(s); 3 candidate-blocker row(s): L2_lpg_drift_ratchet, G_REACH_l0_reachability, agentic_core/L1_cognition/bridges/u0_to_l1_planning.py, +2 more. Live blocker drivers=2; top red FIX gate=G_REACH_l0_reachability; rows=1,451. |
...
| Merge decision | 2 P0 blocker row(s); 3 candidate-blocker row(s): L2_lpg_drift_ratchet, G_REACH_l0_reachability, agentic_core/L1_cognition/bridges/u0_to_l1_planning.py, +2 more. Live blocker drivers=2; top red FIX gate=G_REACH_l0_reachability; rows=1,451. | What blocks merge? | Resolve before lower-severity ranking. |
...
|---:|---|---|---|
| 1 | Clear P0 blocker rows. Clear open blockers first; otherwise attach candidate rows to a live blocker gate or move them to tracked debt. | 2 P0 blocker row(s); 3 candidate-blocker row(s): L2_lpg_drift_ratchet, G_REACH_l0_reachability, agentic_core/L1_cognition/bridges/u0_to_l1_planning.py, +2 more | Rerun ADG and confirm open blocker rows are zero or explicitly waived. |
...
| 5 | Address G_REACH_l0_reachability. Review the Production-layer modules with no import path from any L0 node (orphans). evidence. Investigate evidence before changing code; the current choices are Investigate evidence, Fix, Adapter/interface, Guardian exemption. | ADG `07112026_2208`: `G_REACH_l0_reachability` found 1,451 G_REACH_l0_reachability, +1 above baseline 1450. Breakout unavailable. | Rerun ADG and confirm the gate returns to green or is explicitly waived. |
| 6 | Restore L_PG boundary discipline. Review the imports touching the L_PG boundary evidence. Investigate evidence before changing code; the current choices are Investigate evidence, Fix, Adapter/interface, Guardian exemption. | ADG `07112026_2208`: `L2_lpg_drift_ratchet` found 1 L2_lpg_drift_ratchet, +1 above baseline 0. Breakout unavailable. | Rerun ADG and confirm the gate returns to green or is explicitly waived. |
| 7 | Fund mapped tests for agentic_core/L5_safety/reasoning/FileClassificationAgent.py. Add mapped tests before touching this surface again. | Add mapped tests/regression coverage for agentic_core. | Rerun ADG and confirm the relevant gate/test/report status is green or explicitly waived. |
...
| Metric | Value |
|---|---|
| Run ID | 07112026_2208 |
| Snapshot | 2026-07-12T02:23:56.275619+00:00 |
| SQLite snapshot | artifacts/adg/adg_indexed_07112026_2208.sqlite |
| Audit caveat | REPORT_INCONSISTENT; report consistency=FAIL |
| FIX gates (all bands) | 2 |
| Open critical gate drivers | 2 |
| Open blocker queue | 2 P0 blocker row(s); 3 candidate-blocker row(s): L2_lpg_drift_ratchet, G_REACH_l0_reachability, agentic_core/L1_cognition/bridges/u0_to_l1_planning.py, +2 more |
| Top FIX gate | G_REACH_l0_reachability; rows=1,451 |
| Action rows | 6 |
