...
| 2 | Clear infra wiring P0 block | Small P0 infra-wiring hard stops are usually the fastest path to remove a red ADG gate without broad refactor. | ADG `06282026_1945`: `10_infra_wiring` found 3 10_infra_wiring. Breakout unavailable. | Inspect the infra-wiring rows and remove or route the invalid pipeline/spine wiring. Do not re-baseline a P0 block. |
| 3 | Stop core importing apps | Core importing apps breaks the core/app boundary and directly weakens provider-agnostic core. | ADG `06282026_1945`: `13_core_imports_apps` found 35 13_core_imports_apps. Breakout unavailable. | Move app-specific bindings behind an adapter or app-owned wiring surface; core should keep only generic contracts. |
| 4 | Close UWG bypass regression | New UWG bypass paths weaken write-governance correctness and can materially affect app run safety. | ADG `06282026_1945`: `S2_uwg_bypass_ratchet` found 1,601 S2_uwg_bypass_ratchet, +30 above baseline 1571. Breakout unavailable. | Investigate the new bypass delta and route writes through UWG or an approved adapter; re-baseline only with explicit sign-off. |
...
| Priority | Move | Why it matters | Evidence | Next step |
|---|---|---|---|---|
| 1 | Clear infra wiring P0 block | Small P0 infra-wiring hard stops are usually the fastest path to remove a red ADG gate without broad refactor. | ADG `06282026_1945`: `10_infra_wiring` found 3 10_infra_wiring. Breakout unavailable. | Inspect the infra-wiring rows and remove or route the invalid pipeline/spine wiring. Do not re-baseline a P0 block. |
| 2 | Stop core importing apps | Core importing apps breaks the core/app boundary and directly weakens provider-agnostic core. | ADG `06282026_1945`: `13_core_imports_apps` found 35 13_core_imports_apps. Breakout unavailable. | Move app-specific bindings behind an adapter or app-owned wiring surface; core should keep only generic contracts. |
| 3 | Close UWG bypass regression | New UWG bypass paths weaken write-governance correctness and can materially affect app run safety. | ADG `06282026_1945`: `S2_uwg_bypass_ratchet` found 1,601 S2_uwg_bypass_ratchet, +30 above baseline 1571. Breakout unavailable. | Investigate the new bypass delta and route writes through UWG or an approved adapter; re-baseline only with explicit sign-off. |
