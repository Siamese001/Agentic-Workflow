...
- **Generated:** 2026-07-06T03:26:44+00:00
- **Gate-results source:** `artifacts\adg\adg_gate_results_20260706_032316.json`
- **Burndown source:** `artifacts\adg\adg_burndown_table_07052026_2301.json`
- **Snapshot timestamp:** 2026-07-06T03:23:16.794189+00:00
- **Total gates:** 49
...
- **Technical evidence:**
  ...
  - CLEAR gates: 28
  - block_fail=3; ratchet_regressed=1
...
|---------:|------|----------------|----------|-----------|
| 1 | Fix 3_write_sovereignty | This gate is marked FIX, so the ADG run is not decision-grade green until it clears. | P0 block gate; rows=766; sub=block. | BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block. |
| 2 | Fix C1_uwg_bypass_pview | This gate is marked FIX, so the ADG run is not decision-grade green until it clears. | P0 block gate; rows=7; sub=block. | BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block. |
...
| 4 | Fix 10_infra_wiring | This gate is marked FIX, so the ADG run is not decision-grade green until it clears. | P0 block gate; rows=2; sub=block. | BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block. |

Next step: BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block.
- **Action:** **FIX**=4 (address for green ADG) · **BURN**=12 (owned backlog) · **KPI**=5 (watchlist only) · **CLEAR**=28
...
|------|:------:|--------:|-------------------|-----------------|------------|-----------|
| P0 | BLOCKED | 3 | 3 gates / 2,210 rows | 0 gates / 0 rows | red gates present | fix red gates first |
| P1 | BLOCKED | 1 | 7 gates / 3,644 rows | 0 gates / 0 rows | red gates present | fix red gates first |
| P2 | PASS | 0 | 2 gates / 1,979 rows | 1 gate / 105 rows | green; tracked backlog | work ranked queue; do not treat as new failures |
| P3 | PASS | 0 | 0 gates / 0 rows | 4 gates / 12,470 rows | green; KPI/watchlist only | watch trend; no burn-down action |
...
|---------|:-------:|-------------|:-------:|:---:|---------:|---------------|--------|------------------|
| `10_infra_wiring` | P0 | block | FIX | block | 2 | 0 | Counts: Infra wiring defects (canonical pipeline / spine reachability). Sub: Run blocked (exit 1). | BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block. |
| `3_write_sovereignty` | P0 | block | FIX | block | 766 | 0 | Counts: Non-UWG durable write paths in mv_write_sovereignty_paths (inventory). Sub: Run blocked (exit 1). | BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block. |
| `C1_uwg_bypass_pview` | P0 | block | FIX | block | 7 | 0 | Counts: Any row in UWG-bypass materialized view (zero tolerance block). Sub: Run blocked (exit 1). | BLOCKER (P0): clear the zero-tolerance condition before merge. Do NOT re-baseline a P0 block. |
...
| `S2_uwg_bypass_ratchet` | P0 | ratchet | BURN | floor | 756 | 756 | Counts: Write paths that bypass UWG (overlay on write_sovereignty edges). Sub: 756 at floor (baseline 756); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `C3_silent_writes_ratchet` | P1 | ratchet | BURN | floor | 125 | 125 | Counts: writes_to edges without sibling emits_side_effect on same source. Sub: 125 at floor (baseline 125); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `M_taint_actionable_ratchet` | P1 | ratchet | BURN | floor | 686 | 686 | Counts: Actionable taint surfaces without output schema binding. Sub: 686 at floor (baseline 686); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `I2_replay_surface_gaps_ratchet` | P2 | ratchet | BURN | floor | 997 | 997 | Counts: State mutations not consumed by replay surface (gap_flag=1). Sub: 997 at floor (baseline 997); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `M1_module_loc_ratchet` | P3 | ratchet | KPI | floor | 472 | 472 | Counts: Production modules exceeding LOC ceiling (disk scan). Sub: 472 at floor (baseline 472); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| `Q2_cyclomatic_complexity_ratchet` | P3 | ratchet | KPI | floor | 1199 | 1199 | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: 1199 at floor (baseline 1199); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| `S4_unused_imports_ratchet` | P3 | ratchet | KPI | floor | 10796 | 10796 | Counts: Unused import edges in production modules. Sub: 10796 at floor (baseline 10796); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| Gate ID | Sub | Rows |
|---------|:---:|---------:|
| `3_write_sovereignty` | block | 766 |
| `C1_uwg_bypass_pview` | block | 7 |
| `H1_new_orphans_delta_ratchet` | regr | 3 |
| `10_infra_wiring` | block | 2 |
...
|---------|:----:|-----:|--------------------|-----------|
| `S4_unused_imports_ratchet` | P3 | 10796 | KPI/watchlist signal, not an owned burn-down item. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| `Q2_cyclomatic_complexity_ratchet` | P3 | 1199 | KPI/watchlist signal, not an owned burn-down item. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| `M1_module_loc_ratchet` | P3 | 472 | KPI/watchlist signal, not an owned burn-down item. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| P2 | anti_patterns_medium | 722 | 698 | 24 | +698 |
| P3 | style_warnings | 19321 | 87 | 19234 | +87 |
...
|---------|------:|---------|
| block_pass | 12 | Block-class gates that did not halt the run (exit 0). Rows may be non-zero. |
| block_fail | 3 | Block-class gates that halted the run — clear the gate blocking condition. |
...
|---------|------:|---------|
| FIX | 4 | Address before treating ADG as green (gate blocked or ratchet regressed). |
...
|------|:----:|:---:|:---:|---------:|--------|
| `3_write_sovereignty` | P0 | block | block | 766 | Counts: Non-UWG durable write paths in mv_write_sovereignty_paths (inventory). Sub: Run blocked (exit 1). |
| `C1_uwg_bypass_pview` | P0 | block | block | 7 | Counts: Any row in UWG-bypass materialized view (zero tolerance block). Sub: Run blocked (exit 1). |
...
| `10_infra_wiring` | P0 | block | block | 2 | Counts: Infra wiring defects (canonical pipeline / spine reachability). Sub: Run blocked (exit 1). |

---
## Next action

- **Queue:** `artifacts\adg\adg_action_queue_07052026_2301.json`
- **emit_status:** `ok`
- **degraded:** `False`
- **summary:** FIX=4 · TRACK=17 · actions_emitted=10
...
| 2 | FIX | fix_gate | `C1_uwg_bypass_pview` | fix_block_p0_violations_asc | Counts: Any row in UWG-bypass materialized view (zero tolerance block). Sub: Run blocked (exit 1). |
| 3 | FIX | fix_gate | `3_write_sovereignty` | fix_block_p0_violations_asc | Counts: Non-UWG durable write paths in mv_write_sovereignty_paths (inventory). Sub: Run blocked (exit 1). |
| 4 | FIX | fix_gate | `H1_new_orphans_delta_ratchet` | fix_regr_p1_delta_asc | Counts: Modules newly fan_in=0 vs prior snapshot (new orphans). Sub: +3 vs baseline 0. |
| 5 | GRAPHDB | test_hotspot_gap | `apps_rg/runtime/sections/executive_summary_lane.py` | mv_hotspot_coverage_risk_priority | Test hotspot gap from mv_hotspot_coverage_risk; priority=P1_URGENT; risk=CRITICAL; coverage=ABSENT (absent); critical... |
