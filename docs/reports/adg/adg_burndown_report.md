...
|--:|--------|------|--------:|-----|-----------|
| 1 | Fix now: P0 CI blockers | P0 | 2,850 | CI blocker; ADG is not green until this clears. | Fix the blocking gate condition and rerun ADG before burn-down work. |
| 2 | Fix now: P1 CI blockers | P1 | 216 | CI blocker; ADG is not green until this clears. | Fix the blocking gate condition and rerun ADG before burn-down work. |
| 3 | Fix now: P3 CI blockers | P3 | 1,550 | CI blocker; ADG is not green until this clears. | Fix the blocking gate condition and rerun ADG before burn-down work. |
| 4 | Burn down ratchets: `S2_UWG` 1,540; `L2_LPG` 1 | P0 | 1,541 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Use the P0 Action Plan below. |
| 5 | Burn down ratchets: `C3_silent_writes` 1,986; `E1_trace_stub` 1,030; `B2_layer_skip` 951; +5 more = 5,496 total | P1 | 5,496 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Open a burn-down slice for the listed ratchets, rerun ADG, then absorb the lower baseline. |
...
| 7 | Burn down ratchets: `S4_unused_imports` 10,832 | P3 | 10,832 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Open a burn-down slice for the listed ratchets, rerun ADG, then absorb the lower baseline. |
...
| 13 | Severity audit: 23 non-exempt from 743 gross | P2 | 23 | Review-only audit signal; not Fix now unless it maps to a failing gate. | Audit or map to a gate; do not treat as a CI blocker by itself. |
| 14 | Severity audit: 18,883 non-exempt from 18,975 gross | P3 | 18,883 | Review-only audit signal; not Fix now unless it maps to a failing gate. | Audit or map to a gate; do not treat as a CI blocker by itself. |
...
| 2 | Burn down ratchet | `S2_UWG` | 1,540 | Largest P0 ratchet floor; reduces the biggest accepted baseline first. | Open a burn-down slice for this gate, reduce records, rerun ADG, then absorb the lower baseline. |
...
|------|:------:|----------:|------------:|--------------:|------------------:|------------|-----------|
| P0 | BLOCKED | 1 | 2,850 | 1,541 | 934 | red gates present | fix red gates first |
| P1 | BLOCKED | 2 | 216 | 5,496 | 0 | red gates present | fix red gates first |
| P2 | PASS | 0 | 0 | 2,068 | 102 | green; ratchet burn-down/open work remains | work ranked queue; ratchets first |
| P3 | BLOCKED | 2 | 1,550 | 10,832 | 3 | red gates present | fix red gates first |
...
| `S2_uwg_bypass_ratchet` | P0 | ratchet | TRACK | floor | 1540 | 1540 | Counts: Write paths that bypass UWG (overlay on write_sovereignty edges). Sub: 1540 at floor (baseline 1540); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `O_tool_call_parity_ratchet` | P1 | ratchet | FIX | regr | 211 | 210 | Counts: Tool invocations missing observability receipt (Anthropic parity pattern). Sub: +1 vs baseline 210. | Regression +1 over baseline 210: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
...
| `C3_silent_writes_ratchet` | P1 | ratchet | TRACK | floor | 1986 | 1986 | Counts: writes_to edges without sibling emits_side_effect on same source. Sub: 1986 at floor (baseline 1986); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `M_taint_actionable_ratchet` | P1 | ratchet | TRACK | floor | 673 | 673 | Counts: Actionable taint surfaces without output schema binding. Sub: 673 at floor (baseline 673); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `Q2_cyclomatic_complexity_ratchet` | P3 | ratchet | FIX | regr | 1088 | 1083 | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: +5 vs baseline 1083. | Regression +5 over baseline 1083: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
| `S4_unused_imports_ratchet` | P3 | ratchet | TRACK | floor | 10832 | 10832 | Counts: Unused import edges in production modules. Sub: 10832 at floor (baseline 10832); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `Q2_cyclomatic_complexity_ratchet` | regr | 1088 |
| `M1_module_loc_ratchet` | regr | 462 |
| `O_tool_call_parity_ratchet` | regr | 211 |
...
| Gate ID | Sub | Records |
|---------|:---:|---------:|
| `S4_unused_imports_ratchet` | floor | 10832 |
| `C3_silent_writes_ratchet` | floor | 1986 |
| `S2_uwg_bypass_ratchet` | floor | 1540 |
...
| `M_taint_actionable_ratchet` | floor | 673 |
...
| P2 | anti_patterns_medium | 743 | 720 | 23 | +720 |
| P3 | style_warnings | 18975 | 92 | 18883 | +92 |
...
| `Q2_cyclomatic_complexity_ratchet` | P3 | ratchet | regr | 1088 | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: +5 vs baseline 1083. |
...
| `O_tool_call_parity_ratchet` | P1 | ratchet | regr | 211 | Counts: Tool invocations missing observability receipt (Anthropic parity pattern). Sub: +1 vs baseline 210. |
...
- **Queue:** `artifacts\adg\adg_action_queue_06152026_1043.json`
- **emit_status:** `ok`
- **degraded:** `False`
...
|-----:|------|------|--------|-----------------|--------|
| 1 | FIX | fix_gate | `G_REACH_l0_reachability` | fix_regr_p0_delta_asc | Counts: Production-layer modules with no import path from any L0 node (orphans). Sub: +7 vs baseline 2843. |
| 2 | FIX | fix_gate | `O_tool_call_parity_ratchet` | fix_regr_p1_delta_asc | Counts: Tool invocations missing observability receipt (Anthropic parity pattern). Sub: +1 vs baseline 210. |
...
| 4 | FIX | fix_gate | `M1_module_loc_ratchet` | fix_regr_p3_delta_asc | Counts: Production modules exceeding LOC ceiling (disk scan). Sub: +3 vs baseline 459. |
| 5 | FIX | fix_gate | `Q2_cyclomatic_complexity_ratchet` | fix_regr_p3_delta_asc | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: +5 vs baseline 1083. |
