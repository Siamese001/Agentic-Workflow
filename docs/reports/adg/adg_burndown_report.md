...
- **Generated:** 2026-06-14T21:33:28+00:00
- **Gate-results source:** `artifacts\adg\adg_gate_results_20260614_213159.json`
- **Burndown source:** `artifacts\adg\adg_burndown_table.json`
- **Snapshot timestamp:** 2026-06-14T21:31:59.912843+00:00
- **Total gates:** 48
- **Overall verdict:** **BLOCKED** (run halt — exit code)
- **Action:** **FIX**=3 (address for green ADG) · **TRACK**=18 (CI OK, backlog) · **CLEAR**=27
...
|--:|--------|------|--------:|-----|-----------|
| 1 | Fix now: P0 CI blockers | P0 | 2,845 | CI blocker; ADG is not green until this clears. | Fix the blocking gate condition and rerun ADG before burn-down work. |
| 2 | Fix now: P3 CI blockers | P3 | 1,544 | CI blocker; ADG is not green until this clears. | Fix the blocking gate condition and rerun ADG before burn-down work. |
| 3 | Burn down ratchets: `S2_UWG` 1,562; `L2_LPG` 1 | P0 | 1,563 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Use the P0 Action Plan below. |
| 4 | Burn down ratchets: `C3_silent_writes` 2,060; `E1_trace_stub` 1,030; `B2_layer_skip` 951; +6 more = 5,937 total | P1 | 5,937 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Open a burn-down slice for the listed ratchets, rerun ADG, then absorb the lower baseline. |
| 5 | Burn down ratchets: `F1_untyped_seam` 1,075; `I2_replay_gaps` 1,011 | P2 | 2,086 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Open a burn-down slice for the listed ratchets, rerun ADG, then absorb the lower baseline. |
| 6 | Burn down ratchets: `S4_unused_imports` 10,897 | P3 | 10,897 | Ratchet floor work lowers accepted baseline debt; P-band outranks raw size. | Open a burn-down slice for the listed ratchets, rerun ADG, then absorb the lower baseline. |
| 7 | Open non-ratchet work: `write_sovereignty` 917; `pipeline_wiring` 1 | P0 | 918 | Real open work, but it does not lower a ratchet floor or block CI. | Schedule after ratchets unless the item is tiny, already in hand, or high-leverage. |
| 8 | Open non-ratchet work: `D2_role_duplication` 102 | P2 | 102 | Real open work, but it does not lower a ratchet floor or block CI. | Schedule after ratchets unless the item is tiny, already in hand, or high-leverage. |
| 9 | Open non-ratchet work: `D1_layer_doc` 3 | P3 | 3 | Real open work, but it does not lower a ratchet floor or block CI. | Schedule after ratchets unless the item is tiny, already in hand, or high-leverage. |
| 10 | Severity audit: 5 non-exempt from 46 gross | P0 | 5 | Review-only audit signal; not Fix now unless it maps to a failing gate. | Audit or map to a gate; do not treat as a CI blocker by itself. |
| 11 | Severity audit: 4 non-exempt from 1,196 gross | P1 | 4 | Review-only audit signal; not Fix now unless it maps to a failing gate. | Audit or map to a gate; do not treat as a CI blocker by itself. |
| 12 | Severity audit: 21 non-exempt from 753 gross | P2 | 21 | Review-only audit signal; not Fix now unless it maps to a failing gate. | Audit or map to a gate; do not treat as a CI blocker by itself. |
| 13 | Severity audit: 20,089 non-exempt from 20,181 gross | P3 | 20,089 | Review-only audit signal; not Fix now unless it maps to a failing gate. | Audit or map to a gate; do not treat as a CI blocker by itself. |
...
|--:|------|------|--------:|-------------------|-----------|
| 1 | Fix now | `G_REACH` | 2,845 | Blocks green ADG; fix before burn-down work. | Fix the gate condition and rerun ADG before treating the run as green. |
| 2 | Burn down ratchet | `S2_UWG` | 1,562 | Largest P0 ratchet floor; reduces the biggest accepted baseline first. | Open a burn-down slice for this gate, reduce records, rerun ADG, then absorb the lower baseline. |
...
|------|:------:|----------:|------------:|--------------:|------------------:|------------|-----------|
| P0 | BLOCKED | 1 | 2,845 | 1,563 | 918 | red gates present | fix red gates first |
| P1 | PASS | 0 | 0 | 5,937 | 0 | green; ratchet burn-down/open work remains | work ranked queue; ratchets first |
| P2 | PASS | 0 | 0 | 2,086 | 102 | green; ratchet burn-down/open work remains | work ranked queue; ratchets first |
| P3 | BLOCKED | 2 | 1,544 | 10,897 | 3 | red gates present | fix red gates first |
...
|---------|:-------:|-------------|:------:|:---:|---------:|---------------|--------|------------------|
| `G_REACH_l0_reachability` | P0 | ratchet | FIX | regr | 2845 | 2843 | Counts: Production-layer modules with no import path from any L0 node (orphans). Sub: +2 vs baseline 2843. | Regression +2 over baseline 2843 (P0): investigate each NEW path; fix the regression — re-baseline only with explicit sign-off. |
...
| `S2_uwg_bypass_ratchet` | P0 | ratchet | TRACK | floor | 1562 | 1562 | Counts: Write paths that bypass UWG (overlay on write_sovereignty edges). Sub: 1562 at floor (baseline 1562); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `C3_silent_writes_ratchet` | P1 | ratchet | TRACK | floor | 2060 | 2060 | Counts: writes_to edges without sibling emits_side_effect on same source. Sub: 2060 at floor (baseline 2060); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `M_taint_actionable_ratchet` | P1 | ratchet | TRACK | floor | 698 | 698 | Counts: Actionable taint surfaces without output schema binding. Sub: 698 at floor (baseline 698); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `O_tool_call_parity_ratchet` | P1 | ratchet | TRACK | floor | 342 | 342 | Counts: Tool invocations missing observability receipt (Anthropic parity pattern). Sub: 342 at floor (baseline 342); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `I2_replay_surface_gaps_ratchet` | P2 | ratchet | TRACK | floor | 1011 | 1011 | Counts: State mutations not consumed by replay surface (gap_flag=1). Sub: 1011 at floor (baseline 1011); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `Q2_cyclomatic_complexity_ratchet` | P3 | ratchet | FIX | regr | 1084 | 1083 | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: +1 vs baseline 1083. | Regression +1 over baseline 1083: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
| `S4_unused_imports_ratchet` | P3 | ratchet | TRACK | floor | 10897 | 10897 | Counts: Unused import edges in production modules. Sub: 10897 at floor (baseline 10897); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| Gate ID | Sub | Records |
|---------|:---:|---------:|
| `G_REACH_l0_reachability` | regr | 2845 |
| `Q2_cyclomatic_complexity_ratchet` | regr | 1084 |
...
| Gate ID | Sub | Records |
|---------|:---:|---------:|
| `S4_unused_imports_ratchet` | floor | 10897 |
| `C3_silent_writes_ratchet` | floor | 2060 |
| `S2_uwg_bypass_ratchet` | floor | 1562 |
| `F1_untyped_seam_ratchet` | floor | 1075 |
| `E1_trace_stub_module` | floor | 1030 |
| `I2_replay_surface_gaps_ratchet` | floor | 1011 |
| `B2_layer_skip_ratchet` | floor | 951 |
| `3_write_sovereignty` | inventory | 917 |
| `I1_exit_disposition_ratchet` | floor | 742 |
| `M_taint_actionable_ratchet` | floor | 698 |
| `O_tool_call_parity_ratchet` | floor | 342 |
...
| P1 | anti_patterns_high | 1196 | 1192 | 4 | +1192 |
| P2 | anti_patterns_medium | 753 | 732 | 21 | +732 |
| P3 | style_warnings | 20181 | 92 | 20089 | +92 |
...
| ratchet_regressed | 3 | Ratchet-class gates with NEW records beyond baseline. |
...
|------|:----:|:---:|:---:|---------:|--------|
| `G_REACH_l0_reachability` | P0 | ratchet | regr | 2845 | Counts: Production-layer modules with no import path from any L0 node (orphans). Sub: +2 vs baseline 2843. |
| `Q2_cyclomatic_complexity_ratchet` | P3 | ratchet | regr | 1084 | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: +1 vs baseline 1083. |
...
- **Queue:** `artifacts\adg\adg_action_queue_06142026_1721.json`
- **emit_status:** `ok`
- **degraded:** `False`
- **summary:** FIX=3 · TRACK=18 · actions_emitted=10
...
|-----:|--------|-----------------------------|
| 4 | `apps_rg/runtime/sections/executive_summary_lane.py` | Add or repair tests here if the current slice touches this path or its callers. |
| 5 | `agentic_core/L5_safety/reasoning/FileClassificationAgent.py` | Add or repair tests here if the current slice touches this path or its callers. |
| 6 | `agentic_core/L5_safety/reasoning/hierarchy_healer.py` | Add or repair tests here if the current slice touches this path or its callers. |

| Rank | Lane | Kind | Target | ordering_reason | Signal |
|-----:|------|------|--------|-----------------|--------|
| 1 | FIX | fix_gate | `G_REACH_l0_reachability` | fix_regr_p0_delta_asc | Counts: Production-layer modules with no import path from any L0 node (orphans). Sub: +2 vs baseline 2843. |
| 2 | FIX | fix_gate | `M1_module_loc_ratchet` | fix_regr_p3_delta_asc | Counts: Production modules exceeding LOC ceiling (disk scan). Sub: +1 vs baseline 459. |
| 3 | FIX | fix_gate | `Q2_cyclomatic_complexity_ratchet` | fix_regr_p3_delta_asc | Counts: Functions with McCabe cyclomatic complexity above ceiling. Sub: +1 vs baseline 1083. |
