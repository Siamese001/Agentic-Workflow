...
- **Generated:** 2026-06-19T16:18:40+00:00
- **Gate-results source:** `artifacts\adg\adg_gate_results_20260619_155952.json`
- **Burndown source:** `artifacts\adg\adg_burndown_table.json`
- **Snapshot timestamp:** 2026-06-19T15:59:52.212297+00:00
- **Total gates:** 48
- **Overall verdict:** **BLOCKED** (run halt — exit code)
- **Action:** **FIX**=7 (address for green ADG) · **TRACK**=17 (CI OK, backlog) · **CLEAR**=24
...
- **Technical evidence:**
  ...
  - TRACK gates: 17
  - CLEAR gates: 24
  - Aggregate verdict rows: block_pass=13; block_fail=1; ratchet_pass=21; ratchet_regressed=6; warn=6.
...
|---------:|------|-------|----------------|-----------------|----------------|----------|
| 1 | Fix P0 red gates | P0 | P0 failures block the run and need immediate attention. | FIX=2; TRACK gates=5; TRACK rows=5,271; CLEAR=8. | P0 is the blocking band. | fix red gates first |
| 2 | Burn down P1 ratchets | P1 | P1 debt is the next highest maintenance drag. | FIX=2; TRACK gates=8; TRACK rows=4,654; CLEAR=8. | P1 follows the blocker band. | fix red gates first |
| 3 | Burn down P2 ratchets | P2 | P2 debt is accepted backlog and should follow higher bands. | FIX=1; TRACK gates=2; TRACK rows=1,083; CLEAR=4. | P2 remains below P1. | fix red gates first |
| 4 | Close P3 advisory backlog | P3 | P3 items are lowest urgency and should trail higher bands. | FIX=2; TRACK gates=2; TRACK rows=449; CLEAR=4. | P3 is the least urgent band. | fix red gates first |
...
|------|:------:|--------:|-----------------|------------|-----------|
| P0 | BLOCKED | 2 | 5 gates / 5,271 rows | red gates present | fix red gates first |
| P1 | BLOCKED | 2 | 8 gates / 4,654 rows | red gates present | fix red gates first |
| P2 | BLOCKED | 1 | 2 gates / 1,083 rows | red gates present | fix red gates first |
| P3 | BLOCKED | 2 | 2 gates / 449 rows | red gates present | fix red gates first |
...
| `8_trace_replay_eval` | P1 | ratchet | FIX | regr | 458 | unseeded | Counts: Eval/replay coverage regressions vs baseline by layer (mv_eval_coverage_by_path). Sub: Above ratchet baseline. | Regression +N over baseline None: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
| `B2_layer_skip_ratchet` | P1 | ratchet | FIX | regr | 882 | 878 | Counts: Import edges that skip more than one layer ordinal (layer-hop). Sub: +4 vs baseline 878. | Regression +4 over baseline 878: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
| `C3_silent_writes_ratchet` | P1 | ratchet | TRACK | floor | 2010 | 2010 | Counts: writes_to edges without sibling emits_side_effect on same source. Sub: 2010 at floor (baseline 2010); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
| `F1_untyped_seam_ratchet` | P2 | ratchet | FIX | regr | 1013 | 1006 | Counts: Cross-layer imports where target has empty type_surface. Sub: +7 vs baseline 1006. | Regression +7 over baseline 1006: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
...
| `S4_unused_imports_ratchet` | P3 | ratchet | FIX | regr | 10756 | 10740 | Counts: Unused import edges in production modules. Sub: +16 vs baseline 10740. | Regression +16 over baseline 10740: fix the new findings, or re-baseline via the gate's `--regenerate-baseline` if intentional/approved debt. |
...
| Gate ID | Sub | Rows |
|---------|:---:|---------:|
| `S4_unused_imports_ratchet` | regr | 10756 |
| `Q2_cyclomatic_complexity_ratchet` | regr | 1095 |
| `F1_untyped_seam_ratchet` | regr | 1013 |
| `B2_layer_skip_ratchet` | regr | 882 |
| `8_trace_replay_eval` | regr | 458 |
...
| `C3_silent_writes_ratchet` | floor | 2010 |
...
| ratchet_regressed | 6 | Ratchet-class gates with NEW rows beyond baseline. |
...
|---------|------:|---------|
| FIX | 7 | Address before treating ADG as green (gate blocked or ratchet regressed). |
| TRACK | 17 | CI passed this gate; rows are backlog — plan hygiene, not a halt. |
| CLEAR | 24 | Zero rows; nothing to do on this gate. |

## 5. Fix now (detail)

| Gate | Band | Enf | Sub | Rows | Signal |
|------|:----:|:---:|:---:|---------:|--------|
| `S4_unused_imports_ratchet` | P3 | ratchet | regr | 10756 | Counts: Unused import edges in production modules. Sub: +16 vs baseline 10740. |
...
| `F1_untyped_seam_ratchet` | P2 | ratchet | regr | 1013 | Counts: Cross-layer imports where target has empty type_surface. Sub: +7 vs baseline 1006. |
| `B2_layer_skip_ratchet` | P1 | ratchet | regr | 882 | Counts: Import edges that skip more than one layer ordinal (layer-hop). Sub: +4 vs baseline 878. |
| `8_trace_replay_eval` | P1 | ratchet | regr | 458 | Counts: Eval/replay coverage regressions vs baseline by layer (mv_eval_coverage_by_path). Sub: Above ratchet baseline. |
...
- **Queue:** `artifacts\adg\adg_action_queue_06192026_1144.json`
- **emit_status:** `ok`
- **degraded:** `False`
- **summary:** FIX=7 · TRACK=17 · actions_emitted=10
...
| 3 | FIX | fix_gate | `B2_layer_skip_ratchet` | fix_regr_p1_delta_asc | Counts: Import edges that skip more than one layer ordinal (layer-hop). Sub: +4 vs baseline 878. |
...
| 5 | FIX | fix_gate | `F1_untyped_seam_ratchet` | fix_regr_p2_delta_asc | Counts: Cross-layer imports where target has empty type_surface. Sub: +7 vs baseline 1006. |
