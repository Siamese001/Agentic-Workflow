...
- **Generated:** 2026-07-06T04:56:43+00:00
- **Gate-results source:** `artifacts\adg\adg_gate_results_20260706_045318.json`
- **Burndown source:** `artifacts\adg\adg_burndown_table_07062026_0048.json`
- **Snapshot timestamp:** 2026-07-06T04:53:18.136776+00:00
- **Total gates:** 49
...
|---------:|------|----------------|----------|-----------|
| 1 | Burn down G_REACH_l0_reachability | This is accepted or advisory debt; reduce it after FIX rows are clear. | Burn-down gate; rows=1453; sub=floor. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| 2 | Burn down I2_replay_surface_gaps_ratchet | This is accepted or advisory debt; reduce it after FIX rows are clear. | Burn-down gate; rows=997; sub=floor. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| 3 | Burn down E1_trace_stub_module | This is accepted or advisory debt; reduce it after FIX rows are clear. | Burn-down gate; rows=982; sub=floor. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| 4 | Burn down F1_untyped_seam_ratchet | This is accepted or advisory debt; reduce it after FIX rows are clear. | Burn-down gate; rows=982; sub=floor. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
...
|------|:------:|--------:|-------------------|-----------------|------------|-----------|
| P0 | PASS | 0 | 4 gates / 2,962 rows | 0 gates / 0 rows | green; tracked backlog | work ranked queue; do not treat as new failures |
| P1 | PASS | 0 | 7 gates / 3,644 rows | 0 gates / 0 rows | green; tracked backlog | work ranked queue; do not treat as new failures |
...
| `S2_uwg_bypass_ratchet` | P0 | ratchet | BURN | floor | 749 | 749 | Counts: Write paths that bypass UWG (overlay on write_sovereignty edges). Sub: 749 at floor (baseline 749); no new debt. | Backlog (within ratchet ceiling): shrink over time, defer to a plan wave. No action needed to pass CI. |
| `3_write_sovereignty` | P0 | block | BURN | inventory | 759 | warn inventory | Counts: Non-UWG durable write paths in mv_write_sovereignty_paths (inventory). Sub: 759 warn inventory; not halt-tier. | Advisory inventory: monitor; reduce opportunistically. Does not block CI. |
...
| `3_write_sovereignty` | inventory | 759 |
| `S2_uwg_bypass_ratchet` | floor | 749 |
...
| P2 | anti_patterns_medium | 723 | 698 | 25 | +698 |
| P3 | style_warnings | 19309 | 87 | 19222 | +87 |
...
- **Queue:** `artifacts\adg\adg_action_queue_07062026_0048.json`
- **emit_status:** `ok`
- **degraded:** `False`
...
| 5 | REFACTOR | refactor_candidate | `agentic_core/L0_routing/config/path_constants.py` | refactor_accelerator_candidates_desc | Refactor candidate; score=0.0646 |
