# RCA — `post_cascade_response` hook dispatcher silently dark; NEXT_STEP markers not auto-captured

**Date:** 2026-04-24 19:40 UTC-04:00
**Status:** **RESOLVED** — bypass executed, both NEXT_STEP markers captured + posted to Notion
**Scope:** Cascade response hook chain, NEXT_STEP / DEFERRED_SCOPE / Author-Gate auto-capture
...
The 2 `NEXT_STEP:` markers Cascade emitted in the prior response did **not** result in:
...
…even though the markers were syntactically correct and the corresponding hook (`post_cascade_next_step_capture.py`) is registered in `.windsurf/hooks.json`.

## Root Cause

**Windsurf 2.0.67 hook dispatcher bug** (documented in [`MEMORY:876ef21d`](../../README.md#known-issues)).

The `post_cascade_response` hook chain stops firing silently after some period within a session. `pre_user_prompt` hooks continue to work, but the post-response chain goes dark. No error is surfaced to the user; the IDE simply skips the dispatch.

### Evidence

```
artifacts/windsurf/post_cascade_heartbeat.jsonl last entry:
...
|---|---|
| Hook script missing | All 12 `post_cascade_response` scripts exist in `.windsurf/scripts/` |
| Hook script broken | Manual replay (`manual_post_cascade_replay.py`) ran all 12 hooks rc=0 in this RCA — they all work when invoked directly |
...
1. **Manual replay** of the full 12-hook chain on the prior response text:
   ```
   python .windsurf/scripts/manual_post_cascade_replay.py --file artifacts/windsurf/replay_response.txt
   ```
   Result: `hooks=12 failed=0`. `post_cascade_next_step_capture.py` reported `markers=2 auto_posted=2`.
...
| Trigger | Bypass |
|---|---|
| Cascade emits `DEFERRED_SCOPE:` marker | Cascade **must** also `python .windsurf/scripts/defer.py "<full marker>"` in the same response |
| Cascade emits `NEXT_STEP:` marker | User runs `python .windsurf/scripts/manual_post_cascade_replay.py --file <response.txt>` (or `--clipboard`) |
...
Cascade SHOULD invoke `defer.py` inline for DEFERRED_SCOPE markers (since it has shell access). For NEXT_STEP markers, the inline equivalent is the same `manual_post_cascade_replay.py` invocation, which Cascade can now also call directly when the heartbeat is stale.
...
2. Verify by running `python .windsurf/scripts/post_cascade_heartbeat.py` then sending a single Cascade message and confirming a new heartbeat row appears.
3. Retire the `defer.py` and `manual_post_cascade_replay.py` inline invocations.
...
- Heartbeat log: `@c:/Git/Agentic-Workflow/artifacts/windsurf/post_cascade_heartbeat.jsonl`
...
- Bypass scripts: `@c:/Git/Agentic-Workflow/.windsurf/scripts/defer.py`, `@c:/Git/Agentic-Workflow/.windsurf/scripts/manual_post_cascade_replay.py`
