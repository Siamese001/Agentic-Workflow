# Agentic Process v14 (PNG) Design Manifesto: Expanded Technical Specification
...
## PART 0: Global Design Principles (Non-Negotiable Invariants)

These principles are the *machine-checkable* contract for all layers and arrows; any violation is a **hard failure** and must be treated as a security/operational incident.

## 0.1 Fail-Closed Defaults

* **Default action is BLOCK.** Absence of a required header, token, schema field, signature, or health check must halt progression.

* **Timeout == reject.** Timeouts are never interpreted as partial approval.

* **Degraded mode is a freeze, not a bypass.** If validation services are unavailable, state mutation is forbidden.

## 0.2 Determinism & Replayability

* **Every request is replayable.** Given the same inputs (payload + policy hash + retrieved context set), the system must reproduce the same plan and same allowed side-effects.

* **Non-determinism is quarantined.** Any stochastic component must be bounded, logged, and excluded from commit paths unless proven deterministic in sandbox double-run.

## 0.3 No Silent State Mutation

* **Only L2 may mutate external state.** L1, L4, L6 must be physically incapable of write operations to runtime targets.

* **No implicit writes.** Any side-effect not registered in the Side-Effect Registry is an automatic abort.

## 0.4 Immutable Traceability

* **TraceID is mandatory and immutable.** Loss of TraceID is a fatal error (`zombie_process_kill`).

* **All artifacts are TraceID-addressable.** Every log, metric, diff, snapshot, token, and decision must embed TraceID.

## 0.5 Authority Is Tokenized

* **Write authority requires signed artifacts.** Conversational approval is non-authoritative.

* **Tokens are scoped and expiring.** Tokens bind (TraceID, action-set, target-set, policy-hash, timestamp, nonce).

## 0.6 Explicit Boundaries (Zero Trust Between Layers)

* **Layer APIs are typed and versioned.** Calls across layers must conform to schemas.

* **Health is binary.** Unknown health status is treated as unhealthy.
...
### L0 Invariants (Gateway Contract)

* **Verify policy hash before processing.** Deserialization, routing, and logging must not occur until `X-Policy-Signature` is verified.

* **Typed lane assignment is mandatory.** READ vs WRITE must be explicit in schema and enforced.

* **TraceID must be minted and attached before any downstream call.**

### L0 Illegal States

* Processing a request with missing/invalid policy signature.

* Emitting side-effects (including logs) without TraceID.
...
### L1 Invariants (Pure Planner)

* **No external mutation capability.** L1 must not have filesystem/network write access.

* **Prompt MRO is deterministic and hash-checked.** `System_Base -> Security_Overlays -> Retrieved_Context -> User_Intent` is immutable.

* **Negative exemplars are mandatory.** Top-K failures must be included by default in pre-plan retrieval.

### L1 Illegal States

* Any direct or indirect write to runtime targets.

* Dynamic prompt assembly order without a validated MRO hash.
...
### L2 Invariants (Atomic State Mutation)

* **All writes are transactional.** Snapshot -> validate -> transform -> check -> commit, or full rollback.

* **String patching is forbidden.** Transformations must be AST-based for code artifacts.

* **Side-effect registry is complete pre-flight.** All intended touches are enumerated and conflict-checked before execution.

### L2 Illegal States

* Writing outside the pipeline.

* Partial commits or mixed-version writes.
...
### L3 Invariants (Authoritative Approval)

* **Approval is cryptographic.** Requires signed artifact binding diff hash + policy hash + TraceID.

* **Diff-first UX is mandatory.** Approval cannot occur without rendering the diff.

* **Timeouts auto-reject.**

### L3 Illegal States

* Accepting conversational "yes" as approval.

* Approving without diff hash binding.
...
### L4 Invariants (Typed Memory)

* **Semantic memory is schema-gated:** RDF triplets or equivalent typed ontology objects only.

* **Embedding versioning is mandatory:** stale embeddings are quarantined and never served to L1.

* **Failure replay is repulsive:** negative exemplars must reduce similarity score or be hard-excluded.

### L4 Illegal States

* Unstructured blobs in semantic stores.

* Serving `dirty` embeddings to planning or validation.
...
### L5 Invariants (Authority & Health)

* **Binary health:** if Guardian is not `ONLINE_AND_HEALTHY`, all WRITE paths are blocked.

* **Authority is tokenized:** permissions are issued as signed artifacts and are scoped and time-bounded.

* **Dependency safety is enforced:** cycles, adapter coercions, and schema violations are reject-only.

### L5 Illegal States

* Allowing any state mutation while Guardian health is unknown/degraded.

* Accepting unsigned or replayed tokens.
...
### Budget Invariants (Cost as a Safety Property)

* **Pre-flight only:** projected spend must be computed before any API call.

* **Hard caps:** exceeding caps is a local reject with zero external cost.

### Budget Illegal States

* Post-hoc budget enforcement.

* Any retry loop that does not re-check budget and backoff state.
...
### L6 Invariants (Observer-Only)

* **No state mutation:** L6 may trigger rollback signals but must not modify artifacts directly.

* **Correlation required:** anomalies require multi-source triangulation as defined.

* **Deterministic dedupe:** dedupe windows and hashing must be stable and versioned.

### L6 Illegal States

* Single-signal anomalies escalating to rollback without corroboration (except explicitly policy-allowlisted emergencies).
...
## 3.0 Arrow Contract (Transport Semantics)

* **Every arrow is a contract:** it defines required preconditions, emitted artifacts, and allowed side-effects.

* **Red dashed (error/rejection) paths are bounded:** max retries + exponential backoff + escalation are mandatory.

* **Blue dashed (conditional) paths are test-gated:** simulation must be isolated and deterministic (double-run equality).

* **Feedback loops must be monotonic-tightening by default:** automated policy updates may tighten constraints, never relax them without explicit human-signed authorization.
