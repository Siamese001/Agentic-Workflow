...
HITL ADG OVERLAY (v2) — L5 SAFETY ENFORCEMENT EXPANSION
======================================================================================================================================================================

ADG-backed HITL relation set (03162026_0931):
- `escalates_to_human=1182`
- `requires_human_review=5`
- `routes_path=183`
- `reenters_safety=11`
- `gated_by_confidence=37`
- `enters_sandbox=39`
- `freezes_context=5`
- `unfreezes_context=2`
- `builds_dpo_batch=43`
- `produces_preference_pair=13`
- `proposal_commits_routing=3029`
- `updates_routing_strategy=3011`

L5 hardening implications:
1. L5 must treat confidence-gated escalation as a first-class review trigger, independent from privileged-tool review.
2. Path D lifecycle must be enforced as an invariant protocol: freeze context before human review, re-clear after decision, then unfreeze for execution reentry.
3. `reenters_safety` semantics apply to both policy violations and unresolved human-review outcomes (fail-closed behavior).
4. Human-reviewed outcomes remain auditable learning inputs through DPO pair generation and routing-strategy update edges.

Runtime anchor modules:
