# UNIFIED ARCHITECTURAL SPEC: AGENTIC RETRIEVAL & RAG PIPELINES (v16 - RAG HUB ALIGNMENT)
...
| ROLE:   L1 performs ONLY deterministic hash lookup (no vector operations). L1 = advisory only. Runs BEFORE embedding.                                                                   |
| OUTPUT: Proposes match signals (e.g., "Exact Hash Found").                                                                                                                              |
+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
                                                                | (Handoff)
                                                                v
+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
| INFRA:  Redis (RAM-first cache)  | INFRA:  GPTCache backed by Redis | INFRA:  Vector DB (FAISS/Chroma) | INFRA:  Local Process Heap       | INFRA:  Token Matrix Weights     |
...
|                ▼                 |                ▼                 | └──────────────┬───────────────┘ |                ▼                 |                ▼                 |
| ┌──────────────────────────────┐ | ┌──────────────────────────────┐ |                ▼                 | ┌──────────────────────────────┐ | ┌──────────────────────────────┐ |
| │ 2. RAG Priming Phase:        │ | │ 2. GPTCache (via Redis)      │ | ┌──────────────────────────────┐ | │ 2. Tool Auth / Sandbox Bind  │ | │ 2. Token Matrix Eval         │ |
...
| - L3 → RAG (🔵 intent vs 🟠 fact). Only L3 performs authoritative knowledge retrieval, but final truth and auditability reside in L4 (SQLite).                                          |
|                                                                                                                                                                                         |
| 🧩 VECTOR SEPARATION:                                                                                                                                                                   |
| - 🔵 intent_vec (queries)   → Redis only (cache, ephemeral)                                                                                                                             |
| - 🟠 fact_vec (documents)   → Vector DB only (FAISS/Chroma, durable)                                                                                                                    |
|   INVARIANT: L2 semantic cache NEVER reads from or writes to Vector DB. It operates ONLY on cached 🔵 intent_vec entries in Redis.                                                      |
+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
