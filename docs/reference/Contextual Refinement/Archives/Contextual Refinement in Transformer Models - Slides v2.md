...
We will track a single rider, [_bank], but we must remember: ALL riders are being updated together at the same time.
...
This toy sequence is reused ONLY to show how contextual refinement works around the token [_bank].
It does NOT mean embedding ingestion and LLM generation usually consume the same amount of text.
====================================================================================================================================
[ EMBEDDING / RETRIEVAL SIDE ]                                  │ [ GENERATION / LLM SIDE ]
Offline ingestion works chunk-by-chunk, not corpus-all-at-once: │ Live generation works on the CURRENT PROMPT / PREFIX:
                                                                │ 
   corpus                                                       │    current visible prompt / prefix (up to context window)
...
- The embedding encoder sees the FULL visible text of the       │ - The LLM can read the FULL CURRENT PREFIX, which may be
  CURRENT CHUNK / WINDOW.                                       │   much larger than one sequence.
- It does NOT attend across the entire corpus in one pass.      │ - But generation remains CAUSAL: each token can only use
- Corpus-wide behavior appears later when a query vector is     │   left-context / self at its position.
  compared against MANY stored chunk vectors.                   │ 
────────────────────────────────────────────────────────────────┴───────────────────────────────────────────────────────────────────
====================================================================================================================================
...
[ LOCAL MASK DEMO ONLY ] We now reuse the SAME local token sequence ONLY to isolate: "At [_bank], what positions are visible?"
====================================================================================================================================
...
[09:30 AM] LAYER 6: COMPOUNDING ENTANGLEMENT (TOKENS LEARNING FROM ALREADY-SMARTER TOKENS)
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
...
====================================================================================================================================
                                   ARCHITECTURAL MAPPING: ENCODERS & DECODERS IN AGENTIC SYSTEMS
====================================================================================================================================
How these twin slides map to the standard Agentic Process runtime:

[ SLIDE 1 (LEFT): ENCODERS ] - THE RETRIEVAL ENGINE
• C0 CONTEXT ENGINEERING: Encoders power the core of Retrieval-Augmented Generation (RAG). Before runtime, they process the "L4 Archive" to create stored 🟧 Fact Vectors. During runtime, they translate the user query into a 🟦 Intent Vector, comparing mathematical distances to retrieve grounded context.

[ SLIDE 2 (RIGHT): DECODERS ] - THE REASONING ENGINE
