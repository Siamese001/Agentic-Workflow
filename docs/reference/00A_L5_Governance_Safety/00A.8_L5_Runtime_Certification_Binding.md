...
------------------------------------------------------------------------------------------------------------------------
- ADR-100 (2026-05-09) — L5 Certification Reference Threading Through the Full Emit Chain.
  Extends the binding surface defined in this file: every inter-layer emit contract
  (ValidatedRequest → L1PlanContract → RouteContract → FinalEvidenceContract →
  CompiledPromptArtifact → L3StepContract → SealedL2Artifact → X3 packets →
  CommitRequest → UWGCommitReceipt → RuntimeExhaustBundle) now carries
  ``l5_certification_ref: str`` (singular, opaque cert ID).
  Verify helper: ``agentic_core.L5_safety.contracts.verify.verify_certification_ref``.
  CI gate: ``ops_scripts/ci/check_l5_cert_ref_on_emit_contracts.py`` (L5CR1).
  See ``docs/architecture/adr/ADR-100-l5-cert-ref-emit-chain-threading.md``.
