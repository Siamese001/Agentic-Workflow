# UNIFIED ARCHITECTURAL SPEC: AGENTIC RETRIEVAL & RAG PIPELINES (v16.18 - PERFECT JUSTIFICATION)
...
|  Durable Math Store (FAISS/Chroma)   Relational Truth (SQLite/Postgres: Canonical Data, ADG Edges, & Telemetry)                                                                         |
...
|          | │  ├─ Route direct to output   │ | │  ├─ Fetch text from Redis    │ | │  ├─ Trim context to budget   │ | │  ├─ Write L4 SQLite records  │ | │  ├─ Init parametric context  │ |
|          | │  └─ Close execution thread   │ | │  └─ Route direct to output   │ | │  └─ Yield C0 to Reading Room │ | │  └─ Yield data to Reading Rm │ | │  └─ Yield to Reading Room    │ |
|          | └──────────────┬───────────────┘ | └──────────────┬───────────────┘ | └──────────────┬───────────────┘ | └──────────────┬───────────────┘ | └──────────────┬───────────────┘ |
|          |                ▼                 |                ▼                 |                ▼                 |                ▼                 |                ▼                 |
|          | ┌──────────────────────────────┐ | ┌──────────────────────────────┐ | ┌──────────────────────────────┐ | ┌──────────────────────────────┐ | ┌──────────────────────────────┐ |
|          | │ 5. [⚡ Fast] Terminal Exit   │ | │ 5. [⚡ Fast] Terminal Exit   │ | │ 5. [🧠 Matrix] Read C0 Data  │ | │ 5. [🧠 Matrix] Read L4 State │ | │ 5. [🧠 Matrix] Read L5 Input │ |
|          | │  ├─ Bypass downstream LLM    │ | │  ├─ Bypass downstream LLM    │ | │  ├─ CoT, ToT, SC, Reflexion  │ | │  ├─ CoT, ToT, SC, Reflexion  │ | │  ├─ CoT, ToT, SC, Reflexion  │ |
