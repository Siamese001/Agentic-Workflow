...
| 7 | `SealedArtifact` | **EM** (via Tier-1 signal) | Tier-1 `L2.step.seal` category in `agentic_core/L6_system_learning/runtime_adg/span_contracts.py` — matches `l2.step.seal`, `step.seal`, `execution.seal`, `.seal` via multi-signal scoring. Emitter: the canonical L2 registry + L2 resolution spans. | `app.<app_name>.l2.sealed_artifact` | Add `contract_name=SealedArtifact`, `contract_id=sealed_artifact_id`, `artifact_hash`, `grounded`, `gate_disposition`. | No | standard + `sealed_artifact_id` + `artifact_hash` + `grounded` + `gate_disposition` | Parent: span #6 | Missing, or `artifact_hash` empty, or `grounded=False` with `gate_disposition=allow` |
...
`agentic_core/L6_system_learning/runtime_adg/span_contracts.py`). The attribute
...
`agentic_core/L6_system_learning/runtime_adg/span_contracts.py`. v2 therefore
...
|:---:|---|---|
| **1** | Tier-1 signal category (`agentic_core/L6_system_learning/runtime_adg/span_contracts.py`) — multi-signal match on name + kind + layer + attributes. | `L0.route.select` matches `heal_router.v1.route` and `route.select` |
...
file (`.claude/plans/<slug>-<6hex>.md`), and wave structure.
...
| **C.8** | Phase C closeout report | ✅ **DONE** (2026-04-30) | `tools/runtime_cert/reports/phase_c_closeout.py` defines `AppCloseoutSummary` + `PhaseCCloseoutReport` (both frozen; `__post_init__` enforces `NOT_CERTIFIED`, `total_apps == len(app_summaries) == not_certified_count`, `blocker_count ∈ [0, total_apps]`, valid `evidence_kind`) + `build_phase_c_closeout_report(app_contracts, rows_by_app, *, generated_at=None, cc_shared_env=None)` + `write_phase_c_closeout_markdown(report, output_path)` + best-effort `load_contracts_from_manifests(repo_root)`. Dispatches per-app by `route_shape`: R3_grounded_read → `extract_r3_evidence` + C.7 gap report; build_time_compiler → `extract_btc_evidence` + C.7 gap report; evaluator_only/core_adjacent_utility → `extract_formal_exception_evidence` (gap report explicitly skipped with note — formal-exception apps carry no required R3/BTC contracts). Core builder is pure — no filesystem scan (verified by `test_core_builder_performs_no_filesystem_scan`). Summaries sorted by `app_name`; blocker count computed from missing_contracts / forbidden_violations / attribute_hardening_required / unknown_needs_runtime_run / failed formal controls / gap_count; top_recommendations deduplicated in first-seen order with 25-entry cap. **Markdown emitter** writes a complete readiness dashboard: title + generated_at + disclaimer + summary table + per-app table + blockers section + top-recommendations + next-phase note pointing to Phase D planning. Every app row in the Markdown carries `NOT_CERTIFIED` by design. **This is the Phase C handoff artifact — it documents readiness and blockers but does NOT certify any app and does NOT promote any `runtime_certification_status` value.** `passed_trace_observed` / `passed_formal_exception_observed` are readiness flags only. Tests: `tests/unit/tools/runtime_cert/reports/test_phase_c_closeout.py` (27 passing). No emitter/scanner/CI changes. | Phase C.7 |
| **C.9+** | Phase D planning | ✅ **ADR published** (2026-04-30) | ADR-080 (`docs/architecture/adr/ADR-080-runtime-cert-phase-d-planning.md`) captures the full Phase D decision-record design, gate math, ledger layout, and implementation plan. D.1/D.2/D.3/D.4/D.5 all implemented per §11 (see rows below). **Phase D complete as of 2026-05-01.** | Phase C.8 |
| **D.1** | Cert-decision record schema | ✅ **DONE** (2026-05-01) | `tools/runtime_cert/decisions/cert_decision_record.py` — frozen `CertificationDecisionRecord` (19 fields) + deterministic `compute_decision_id(app_name, manifest_hash, closeout_report_hash)` + `make_certification_decision_record` helper + `to_dict` / `to_json` serializers. `__post_init__` enforces: `runtime_certification_status_before == _after == NOT_CERTIFIED`, `trace_observed_success_n <= trace_observed_n`, `0 ≤ evidence_rate,wilson_lower ≤ 1`, `z_score,uplift` finite, `decision_id` is 64-hex lowercase and equal to `compute_decision_id(...)`, closed `verdict` set (`certify`/`hold`/`reject`), `failure_reasons` tuple with entries from a closed ontology, ISO-8601 `generated_at_utc` / `next_review_utc`. Tests: `tests/unit/tools/runtime_cert/decisions/test_cert_decision_record.py` (54 passing). No emitter/scanner/CI/ledger changes. | Phase C.8 |
| **D.2** | Closeout-to-decision evaluator | ✅ **DONE** (2026-05-01) | `tools/runtime_cert/decisions/cert_decision_evaluator.py` — pure function `evaluate_phase_c_closeout(report, history=(), *, closeout_report_id=None, closeout_report_hash=None) -> tuple[CertificationDecisionRecord, ...]` per approved plan `.claude/plans/runtime-cert-d2-decision-evaluator-227b38.md`. Local `wilson_lower_bound(successes, n, z=1.96)` stdlib helper (declined reuse of `agentic_core.L2_execution.healers.cascade_calibrator.wilson_lower_bound` — different contract + layer-gravity). Closed 12-reason failure ontology: `CLOSEOUT_MISSING`, `SAMPLE_SIZE_TOO_SMALL`, `WILSON_BELOW_THRESHOLD`, `Z_SCORE_BELOW_THRESHOLD`, `UPLIFT_NOT_POSITIVE`, `CRITICAL_BLOCKERS_PRESENT`, `FORBIDDEN_SPAN_VIOLATION`, `FORMAL_CONTROL_MISSING_OR_FAILED`, `MANIFEST_HASH_DRIFT`, `AMBIGUOUS_EVIDENCE`, `NOT_TRACE_OBSERVED_READY`, `NOT_FORMAL_EXCEPTION_OBSERVED_READY`. Global thresholds (ADR-080 §7): `MIN_N=30`, `MIN_WILSON_LOWER=0.60`, `MIN_Z_SCORE=1.96`, `MIN_UPLIFT=0.0` (strict inequality). Review cadences: 30d certify, 7d hold, 7d reject. Accumulates `(successes, n)` only when `prior.manifest_hash == summary.manifest_hash`; uplift baseline = most-recent prior for same app regardless of manifest. `derive_closeout_report_hash(report)` = deterministic SHA-256 over canonical-JSON `report.to_dict()` (no filesystem read). **Pure** — no I/O, no sqlite, no subprocess, verified by `test_no_filesystem_or_sqlite_access` that patches `open` and `sqlite3.connect` to raise. Output preserves `report.app_summaries` order. Tests: `tests/unit/tools/runtime_cert/decisions/test_cert_decision_evaluator.py` (50 passing). Every produced record carries `runtime_certification_status_after == NOT_CERTIFIED` even when `verdict == "certify"` — structurally enforced by D.1 `__post_init__`. **No app certified.** No emitter/scanner/CI/ledger changes. | Phase D.1 |
| **D.3** | Cert-decision ledger writer | ✅ **DONE** (2026-05-01) | `tools/runtime_cert/decisions/cert_decision_ledger.py` — local stdlib-only SQLite writer per approved plan `.claude/plans/runtime-cert-d3-cert-decision-ledger-85989c.md`. Per-app files at `artifacts/ledgers/cert_decision_<app_name>.sqlite`; `INSERT OR IGNORE` idempotency on `decision_id` (`INSERT OR IGNORE` + `total_changes==0` pattern); `CertDecisionLedgerWriteResult` fail-soft dataclass with exactly-one-of-(`written`/`already_exists`/`skipped`) invariant; `fail_soft=True` default absorbs `sqlite3.Error`; `TypeError`/`ValueError` always raise. DDL at `.windsurf/schemas/cert_decision_ledger.schema.sql` with SQL `CHECK` constraints enforcing `NOT_CERTIFIED` on both status columns (belt-and-suspenders with D.1 `__post_init__`). Read-back hydrates via `CertificationDecisionRecord(**d)` + verifies stored `decision_id` matches `compute_decision_id` — so any SQL-layer tamper (direct UPDATE to status, record_json mutation, decision_id mutation) surfaces as `ValueError` on read. `CERT_DECISION_LEDGER_BYPASS=1` env var; in-process lock keyed on `db_path` for thread-safety. **NOT registered in `tools/ledgers/schema_registry.py` LEDGER_REGISTRY** (verified via `test_ddl_not_registered_in_ledger_registry` and pre-flight review of `tools/ledgers/apply_schema.py` which iterates the registry only). 35 tests in `test_cert_decision_ledger.py`. **This writer persists decisions; it does NOT certify apps.** Phase D writes `NOT_CERTIFIED` verbatim regardless of `verdict`; scanner promotion to `RUNTIME_CERTIFIED` / `FORMAL_EXCEPTION_VERIFIED` remains Phase F (out of scope). No emitter/scanner/CI/pre-commit changes. | Phase D.2 |
| **D.4** | Cert-decision smoke harness | ✅ **DONE** (2026-05-01) | `tools/runtime_cert/smoke/cert_decision_smoke.py` per approved plan `.claude/plans/runtime-cert-d4-cert-decision-smoke-7acad5.md`. Public API: `run_cert_decision_smoke(report, *, repo_root, history=(), fail_soft=True) -> CertDecisionSmokeReport` + `write_cert_decision_smoke_report(report, output_path) -> Path` + frozen `CertDecisionSmokeReport` dataclass. Pipeline: `evaluate_phase_c_closeout` (D.2) → `write_cert_decision_record` (D.3) per record → `read_cert_decision_records` (D.3) per distinct app in first-seen order → `CertDecisionSmokeReport`. `repo_root` is a required keyword arg; tests pass `tmp_path` so no real `artifacts/ledgers/` writes occur in CI. Closed 6-reason failure ontology (`WRITE_COUNT_MISMATCH`, `LEDGER_WRITE_SKIPPED`, `MISSING_READBACK`, `STATUS_NOT_NOT_CERTIFIED`, `DECISION_COUNT_DOES_NOT_MATCH_INPUT`, `READBACK_DECISION_ID_MISMATCH`) — diagnostics, never raises. JSON writer emits `schema_version="d4-smoke-v1"` + `disclaimer` + `runtime_certification_status="NOT_CERTIFIED"` as top-level keys; `Path` instances converted to `str`; `write_results` and `read_back_records` emitted as nested dicts for full auditability. Dataclass `__post_init__` enforces status pin + schema-version pin + disclaimer phrase + tuple-length parallelism + count balance (`written+already_exists+skipped == decision_count`) + read-back ceiling + every `ledger_path` non-empty + every read-back row carries `NOT_CERTIFIED` on both status columns + `failure_reasons ⊆ SMOKE_FAILURE_REASONS`. `to_dict`/`to_json` JSON-safe. Non-promotion triple-checked at five layers (C.8 input + D.1 `__post_init__` + D.3 SQL `CHECK` + D.3 read-back re-validation + smoke `__post_init__`). Tests: `tests/unit/tools/runtime_cert/smoke/test_cert_decision_smoke.py` (27 passing) — covers hold/certify/reject round trip, idempotent second run, fail-soft injection, missing-readback injection, JSON writer + disclaimer, dataclass invariant rejections, multi-app single call, order preservation, `repo_root` required, no-forbidden-imports audit, tmp_path-only writes. **A `verdict == "certify"` row is NOT a certification.** No scanner/CI/emitter/app-behavior changes. | Phase D.3 |
| **D.5** | Phase D closeout report | ✅ **DONE** (2026-05-01) | `docs/reports/runtime_cert/phase_d_closeout/2026-W18.md` per approved plan `.claude/plans/runtime-cert-d5-phase-d-closeout-5e9d2a.md`. Markdown-only documentation artifact; no Python code, no ledger writes, no scanner/CI/emitter/app-behavior changes. Summarizes D.1/D.2/D.3/D.4 with test counts (D.1 54 + D.2 50 + D.3 35 + D.4 27 = 166 Phase D subtotal; 191 combined sweep; 139 decisions subtotal). Includes 6-layer status invariant table (C.8 input + D.1 `__post_init__` + D.2 inheritance + D.3 SQL `CHECK` + D.3 read-back re-validation + D.4 smoke `__post_init__`), negative-evidence table (scanner/CI/emitter/app/LEDGER_REGISTRY/real-repo-ledgers/forbidden-imports/runtime_mode/promotion-workflow all unchanged), and explicit Phase E / Phase F boundary statements. Primary ADR anchor: ADR-080; cross-reference: ADR-050 (intelligence-ledger family). Every reference to `runtime_certification_status` in the report is `NOT_CERTIFIED`; a `verdict == "certify"` row is explicitly documented as a non-promoting decision record, not a certification. **Phase D is now closed.** No app was certified. No scanner `runtime_mode` was changed. No CI gate was added. Phase E (fail-closed CI gate) and Phase F (scanner `runtime_mode` bucket extension + promotion workflow) remain independently gated on their own Author-Gates. | Phase D.4 |
| **D** | Certification report generator | Pending | Offline tool running the fail-closed gate (§9) against a sample of traces; produces `docs/reports/runtime_cert/<app>/<YYYY-Ww>.md`. | Phase C |
...
| **Q6** | How is `manifest_hash` computed — whole-file SHA-256 of raw bytes, or YAML-canonicalized? | \u2705 **RESOLVED BY PHASE B.3**. Answer: `sha256-raw-bytes` \u2014 lowercase hex SHA-256 of the raw file bytes with no YAML parse, no line-ending normalization, no comment stripping, no canonicalization. Reified in `agentic_core/L6_system_learning/runtime_adg/manifest_hash.py` with `MANIFEST_HASH_ALGORITHM = "sha256-raw-bytes"`. Rationale: comment prose (e.g. CC-SHARED-05) carries compensating-control semantics, so any byte change \u2014 including comments \u2014 must invalidate cached cert evidence. |
