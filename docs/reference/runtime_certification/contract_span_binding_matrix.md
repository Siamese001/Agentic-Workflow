...
| **C.9+** | Phase D planning | ADR published (ADR-080) | Phase D certification-decision logic, `CERT_DECISION:` intelligence-ledger binding (§29), per-app cert ledger at `artifacts/ledgers/cert_decision_<app_name>.sqlite`, Wilson-CI promotion gate mirroring the `L6/promo` router family. Planning input contract is C.8 closeout Markdown. See ADR-080 §11 sub-phase status below. | Phase C.8 |
| **D.1** | Decision-record schema | ✅ **DONE** (2026-05-01) | `tools/runtime_cert/decisions/cert_decision_record.py` — `CertificationDecisionRecord` frozen dataclass + `compute_decision_id(app_name, manifest_hash, closeout_report_hash)` + `make_certification_decision_record(**kwargs)` + `to_dict`/`to_json`. `__post_init__` enforces `runtime_certification_status_before == runtime_certification_status_after == NOT_CERTIFIED` unconditionally — record cannot be constructed with any other status. 54 tests in `test_cert_decision_record.py`. No scanner change, no CI gate, no emitter change, no certification. | Phase C.9+ |
| **D.2** | Closeout-to-decision evaluator | ✅ **DONE** (2026-05-01) | `tools/runtime_cert/decisions/cert_decision_evaluator.py` — pure function `evaluate_phase_c_closeout(report, history, *, closeout_report_id=None, closeout_report_hash=None) -> tuple[CertificationDecisionRecord, ...]` + local stdlib `wilson_lower_bound(successes, n, z=1.96)` + `derive_closeout_report_hash(report)` + 12-constant closed `FAILURE_REASONS` ontology. Verdict rules (plan §5): reject-pre-check collects all firing reasons (`CRITICAL_BLOCKERS_PRESENT`, `FORBIDDEN_SPAN_VIOLATION`, `FORMAL_CONTROL_MISSING_OR_FAILED`, `MANIFEST_HASH_DRIFT` guarded by first-ever-run exception, `AMBIGUOUS_EVIDENCE`); certify requires `n≥30` + `wilson≥0.60` + `z≥1.96` + `uplift>0`; otherwise hold with specific threshold/not-observed-ready reasons. Per-closeout-summary counting adapts to D.1's boolean readiness flags: each summary contributes `(1,1)` if pass / `(0,1)` otherwise, so `n≥30` requires 30 weekly closeouts over stable manifest (ADR-080 §13). 50 tests in `test_cert_decision_evaluator.py`. Pure; no I/O; no ledger write; no scanner promotion. | Phase D.1 |
