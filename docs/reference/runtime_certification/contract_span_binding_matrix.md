...
| **C.4** | `build_time_compiler` evidence extractor | Pending | Analogous to C.3 but for the 3-contract BTC shape (`apps_qna`). | Phase C.3 |
| **C.5–C.n** | Remaining Phase C sub-phases | Pending | Formal-exception evidence extractor, pipeline assembly, `FinalEvidenceContract` UNR resolution. | Phase C.3+ |
