...
| **C.9+** | Phase D planning | Pending | Phase D certification-decision logic, `CERT_DECISION:` intelligence-ledger binding (§29), per-app cert ledger at `artifacts/ledgers/cert_decision_<app_name>.sqlite`, Wilson-CI promotion gate mirroring the `L6/promo` router family. Planning input contract is C.8 closeout Markdown. | Phase C.8 |
