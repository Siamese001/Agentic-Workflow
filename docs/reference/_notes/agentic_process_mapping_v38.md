...
----------------------------------------------------------------------------------------------------------------------
 ...
 🧱 CONTROL / NON-MODEL STEP = contract, gate, policy, state, or write-control logic
   ...
   🔐 cert         = 00A/L5 certification evidence
   🗄️ state        = 00B/L4 durable state/read surface
   ✒️ commit       = 00B/UWG durable write admission only
...
- Ask: "Is this packet certified under valid authority, policy, registry, origin-trust, capability, sandbox, egress,
  HITL, replay, and audit evidence?"
- Emits: certification statuses, receipts, manifests, hashes, gap reports, evidence refs.
- Owns: governance certification evidence only.
- Does not emit live GateVerdict.
- Does not emit final X3.
- Does not commit durable state.
- Does not retrieve, assemble prompts, execute, route, or learn from completed runs.

00C RUNTIME GATES
- 🚦 live current-run gate mesh.
- Ask: "Can this current live step, packet, tool call, output, escalation, or write proposal proceed right now?"
- Emits: GateVerdict.
- Owns: G01-G29 live gate law and bounded proceed/stop checks.
- UNKNOWN is never PASS.
- Does not emit final X3.
- Does not certify 00A evidence as its own output.
- Does not write L4.
...
- Emits exactly one X3 disposition.
- May emit CommitRequest to UWG when durable mutation is requested and cleared.
- Does not execute.
- Does not retrieve.
- Does not write L4.

00B L4 STATE ARCHIVE + UWG
- 🗄️ L4 durable system-of-record state.
- ✒️ UWG durable write admission only.
- Ask: "What durable state exists, and may this cleared mutation be admitted into L4?"
- L4 owns durable records and read surfaces: policies, registries, memory, cache, retrieval surfaces, audit, replay.
- UWG owns CommitRequest validation, write lock, atomic commit, rollback, audit append, and read-surface refresh receipts.
- No layer writes around UWG.
- Exit may request durable mutation, but UWG is the only path that commits it.
- L6 may propose future-run learning, but approved promotion still passes through UWG.
...
==============================================================================================================================
[ LEGEND ] LAYER / CROSS-CUTTING DEFINITIONS
==============================================================================================================================
...
───────┼──────────────────────────┼───────────────────────────────────────────────────────────────────────────────────────────
 ...
 L4    │ Archivist                │ 🗄️ authoritative durable state; broad governed read, UWG-only write.
 L5    │ Safety Officer           │ 🔐 governance certification evidence; no live disposition, no durable write.
 L6    │ Observer                 │ 🧾 completed-run evaluation and ✍️ future-run proposals only.
 00A   │ L5 Governance Pack       │ 🔐 cross-cutting policy/auth/origin/egress/HITL/replay certification evidence.
 00B   │ L4/UWG State Pack        │ 🗄️ durable system-of-record state + ✒️ only durable write admission path.
