# UNIFIED ARCHITECTURAL SPEC: AGENTIC RETRIEVAL & RAG PIPELINES (v20 - RAG HUB ALIGNMENT)
...
+-------------------------------------+-----------------+-----------------+---------------------------------------------+-------+-------+------+-------+----------------------------------+
| A: MODEL TRAINING                   | Offline         | N/A             | Public Corpora -> Train -> Model Weights    |  NO   |  NO   |  NO  |  NO   | Defines base reasoning.          |
...
| (Writing new books based on exper.) | (Post-runtime)  | (Adds memory)   | -> Embed -> Store in Vector DB              |       |       |      |       | Output: Evolved index/patterns.  |
+=========================================================================================================================================================================================+
...
|       |                |                       │                                                                                                            |                           |
|       |                |                       ▼                                                                                                            |                           |
|       |                | 5. OUTPUT:      [ ENRICHED KNOWLEDGE VECTOR (k_vector) ] <--- (Permanent concept vector; stored in Vector DB)                      |                           |
|       |                +------------------------------------------------------------------------------------------------------------------------------------+                           |
|       +------------------------------------+------------------------------------+                                                                                                       |
|       |                                    |                                    |                                                                                                       |
|       v                                    v                                    v                                                                                                       |
|  [ VECTOR DB ]                       [ METADATA DB ]                      [ CANONICAL STORE ]                                                                                           |
|  Stores vectors + Payload            Stores entity relations / ADG        Stores original files & telemetry (Blob/S3/Postgres)                                                          |
...
|                       ▼                                                                                                                                                                 |
| 5. OUTPUT:      [ EPHEMERAL INTENT VECTOR (q_vector) ] <--- (Bundled with raw_text into Dual-Rail Payload)                                                                              |
+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
                                                                |
...
[q_vector] ──────┼──────────────────────────────────┬──────────────────────────────────┐                                  │                                  │
                 │                                  │                                  │                                  │                                  │
                 ▼ (Text)                           ▼ (Vector Required)                ▼ (Vector Required)                ▼ (Text)                           ▼ (Text)
+----------------------------------+----------------------------------+----------------------------------+----------------------------------+----------------------------------+
| 1. LAYER 1: EXACT CACHE          | 2. LAYER 2: SEMANTIC CACHE       | 3. LAYER 3: AGENTIC RAG (C0)     | 4. LAYER 4: AGENTIC ACTION       | 5. LAYER 5: LLM FALLBACK         |
+----------------------------------+----------------------------------+----------------------------------+----------------------------------+----------------------------------+
| SIGNAL: Hash(raw_text)           | SIGNAL: [q_vector]               | SIGNAL: [q_vector + intent]      | SIGNAL: [raw_text + schemas]     | SIGNAL: raw_text (No q_vector)   |
...
| EVAL: Exact Call Number?         | EVAL: Familiar Request?          | EVAL: Topic in Archival Stacks?  | EVAL: External Action?           | EVAL: No External Matches?       |
...
+----------------------------------+----------------------------------+---------------------------------------------------------------------+----------------------------------+
|                                  |                                  | RAG INTEGRITY HUB: SOVEREIGNTY INVARIANTS                           |                                  |
...
DATA PAYLOAD              | Strings, hashes, JSON     | q_vector vs q_vector      | q_vector vs k_vector      | API requests, JSON, code  | System Prompt vs Weights
