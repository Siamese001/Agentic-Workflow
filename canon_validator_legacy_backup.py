...
def is_excluded(path):
    ...
    if any((ConfigurationService().p in ConfigurationService().EXCLUDED_DIRS for p in ConfigurationService().parts)):
        return True
    if any((ConfigurationService().p.startswith('.') and len(ConfigurationService().p) > 1 and (
            ConfigurationService().p not in ['.github']) for p in ConfigurationService().parts)):
        return True
    return False
                ...
                for pattern in ConfigurationService().todo_patterns:
                    ...
                    for match in ConfigurationService().matches:
                        ...
                        ConfigurationService().violations.append(
                            f'{ConfigurationService().file_path}:{ConfigurationService().line_num}')
        except Exception:
            continue
        ...
        try:
            with open(ConfigurationService().file_path, 'r', encoding='utf-8') as f:
                ...
                for i, line in enumerate(ConfigurationService().lines, 1):
                    ...
                    if ConfigurationService().stripped.startswith('#') or ConfigurationService(
                    ).stripped.startswith('"""') or ConfigurationService().stripped.startswith("'''"):
                        continue
                    if 'logger.info(' in ConfigurationService().line:
                        ConfigurationService().violations.append(
                            f'{ConfigurationService().file_path}:{ConfigurationService().i}')
        except Exception:
            continue
    if ConfigurationService().violations:
...
def check_key_04_no_empty_except_blocks() -> tuple[bool, List[str]]:
    ...
    if ConfigurationService().violations:
        fail(
            '04', f"Found {len(ConfigurationService().violations)} empty except blocks: {', '.join(ConfigurationService().violations[:5])}")
        return (False, ConfigurationService().violations)
    else:
    ...
    for file_path in ConfigurationService().python_files:
        try:
            with open(ConfigurationService().file_path, 'r', encoding='utf-8') as f:
                f.readlines()
                for i, line in enumerate(ConfigurationService().lines, 1):
                    if re.search('from \\.\\.', ConfigurationService().line) or re.search(
                            'from \\.', ConfigurationService().line):
                        ConfigurationService().violations.append(
                            f'{ConfigurationService().file_path}:{ConfigurationService().i}')
        except Exception:
            continue
            ...
            with open(ConfigurationService().file_path, 'r', encoding='utf-8') as f:
                ...
                for imp in ConfigurationService().imports:
                    if imp not in ConfigurationService().used_names and (not imp.startswith('_')):
                        ConfigurationService().violations.append(
                            f'{ConfigurationService().file_path}:{ConfigurationService().imports[imp]}')
        except Exception:
            continue
    if ConfigurationService().violations:
                    ...
                    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        if hasattr(node, 'end_lineno') and node.end_lineno:
                            ...
                            if ConfigurationService().lines > 50:
                                ConfigurationService().violations.append(
                                    f'{ConfigurationService().file_path}:{node.lineno} ({ConfigurationService().lines} lines)')
        except Exception:
            continue
    if ConfigurationService().violations:
                ...
                for node in ast.walk(ConfigurationService().tree):
                    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if ConfigurationService().param_count > 7:
                            ConfigurationService().violations.append(
                                f'{ConfigurationService().file_path}:{node.lineno} ({ConfigurationService().param_count} params)')
        except Exception:
            continue
    if ConfigurationService().violations:
                ...
                for node in ast.walk(ConfigurationService().tree):
                    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if ConfigurationService().complexity > 10:
                            ConfigurationService().violations.append(
                                f'{ConfigurationService().file_path}:{node.lineno} (complexity={ConfigurationService().complexity})')
        except Exception:
            continue
    if ConfigurationService().violations:
                    ...
                    if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        if not node.name.startswith('_'):
                            if not ast.get_docstring(node):
                                ConfigurationService().violations.append(
                                    f'{ConfigurationService().file_path}:{node.lineno} {node.name}')
                ...
                if hasattr(node, 'body') and node.body:
                    ...
                    for i in range(len(ConfigurationService().statements) - 1):
                        ...
                        if isinstance(ConfigurationService().current, (ast.Return, ast.Raise)):
                            ConfigurationService().violations.append(
                                f'{ConfigurationService().file_path}:{ConfigurationService().next_stmt.lineno}')
        except Exception:
            continue
    if ConfigurationService().violations:
                ...
                if isinstance(node, ast.Assign):
                    for target in node.targets:
                        if isinstance(target, ast.Name) and target.id in ConfigurationService().unused:
                            ConfigurationService().violations.append(
                                f'{ConfigurationService().file_path}:{node.lineno} {target.id}')
        except Exception:
            continue
    if ConfigurationService().violations:
...
def check_key_44_no_circular_imports() -> tuple[bool, List[str]]:
    ...
    for file_path in ConfigurationService().python_files:
        try:
            ...
            ConfigurationService().import_map[ConfigurationService(
            ).file_path] = ConfigurationService().imported_modules
        except Exception:
            continue
...
def check_key_46_no_duplicate_code() -> tuple[bool, List[str]]:
    ...
    for file_path in ConfigurationService().python_files:
        try:
            ...
            clean_content = '\n'.join((ConfigurationService().line.strip() for line in ConfigurationService(
            ).clean_content.splitlines() if ConfigurationService().line.strip()))
            ...
            if ConfigurationService().content_hash in ConfigurationService().content_hashes:
                ...
                ConfigurationService().violations.append(
                    f'Duplicate code: {
                        ConfigurationService().file_path} duplicates {
                        ConfigurationService().original_file}')
            else:
                ConfigurationService().content_hashes[ConfigurationService(
