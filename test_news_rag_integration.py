    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            if result.get("contextual_intro"):
                logger.info(f"   Contextual intro: {result.get('contextual_intro')[:100]}...")
...
def test_news_rag_outreach_integration():
    ...
    try:
        ...
        if "NewsRAG" in personalization_source:
            ...
            logger.warning(f"⚠️ News RAG not used. Source: {personalization_source}")
            logger.info("   This might be due to missing API key or no news found")
            return False
...
def test_news_rag_error_handling():
    ...
    try:
        ...
        if result.get("status") in ["SENT_PERSONALIZED", "SENT_TEMPLATE"]:
            ...
            logger.info(f"   Personalization source: {result.get('personalization_source')}")
            return True
        else:
            logger.error(f"❌ System failed to handle News RAG error: {result.get('status')}")
            return False
...
def test_news_rag_personalization_quality():
    ...
    try:
        ...
        if avg_score >= 5:
            logger.info(f"✅ Good personalization quality: {avg_score:.1f}/10 average")
            return True
        else:
            logger.warning(f"⚠️ Low personalization quality: {avg_score:.1f}/10 average")
            return False
...
def main():
    ...
    results.append(("News RAG Outreach Integration", test_news_rag_outreach_integration()))
    results.append(("News RAG Error Handling", test_news_rag_error_handling()))
    results.append(("News RAG Personalization Quality", test_news_rag_personalization_quality()))
