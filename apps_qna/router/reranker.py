...
NOTE (bge-m3-gap-closure-c8f3a2 W2.1): TWO reranker files exist in apps_qna:
  - THIS file (apps_qna/router/reranker.py): router-layer entry point.
    Exports `rerank_candidate_scores` called by `semantic_router.py`.
  - apps_qna/engines/router/reranker.py: engines-layer entry point,
    used by DAG-enhancement and engine-router flows.
Both are LIVE. Both delegate to `agentic_core.knowledge.retrieval.bge_reranker_adapter`
(BAAI/bge-reranker-v2-m3 cross-encoder — distinct from the BGE-M3 dense embedder used
for vector index retrieval). BGE-M3 surface map: see ADR-055 §Surface Map.
