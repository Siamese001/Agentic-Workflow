...
""",
    ...
    for i, test_case in enumerate(test_cases[:iterations], 1):
        ...
        if result["matched_pattern"]:
            ...
            patterns = validator.search_similar_patterns(test_case["code"], max_results=5)
            
            if patterns:
                top_pattern = patterns[0]
                logger.info(f"Top similar pattern:")
                logger.info(f"  Success Rate: {top_pattern['success_rate']:.2%}")
...
def demonstrate_meta_learning(validator: CanonValidator):
    ...
    logger.info("\nLearning Statistics:")
    logger.info(f"  Promotion Threshold: {stats['promotion_threshold']} successes")
    ...
    for pattern in golden_patterns[:3]:
        logger.info(f"  - {pattern['id']}: {pattern['success_rate']:.2%} success rate")
...
def main():
    ...
    try:
        ...
        if "redis_stats" in final_stats:
            ...
            logger.info(f"  Total patterns: {redis_stats.get('total_patterns', 0)}")
            logger.info(f"  Safe patterns: {redis_stats.get('safe_patterns', 0)}")
            logger.info(f"  Blocked patterns: {redis_stats.get('blocked_patterns', 0)}")
        ...
        if "qdrant_stats" in final_stats:
            ...
            logger.info(f"  Successes ingested: {qdrant_stats.get('successes_ingested', 0)}")
            logger.info(f"  Failures ingested: {qdrant_stats.get('failures_ingested', 0)}")
