...
class SovereignRAGManager(SovereignBaseAgent):
    ...
    def index_document(self, doc_id: str, chunks: list[str]):
        ...
        if self.bm25_store:
            try:
                self.bm25_store.add_documents([{'id': f'{doc_id}_chunk_{i}', 'text': chunk, 'metadata': {'doc_id': doc_id}} for i, chunk in enumerate(chunks)])
            # guardian: allow-silent-swallow
            except Exception as e:
        ...
        if self.embedder and self.vector_store:
            try:
                ...
                vectors = [(f'{doc_id}_chunk_{i}', emb, {'text': chunk, 'doc_id': doc_id}) for i, (emb, chunk) in enumerate(zip(embeddings, chunks, strict=False))]
    ...
    def retrieve(self, query: str, top_k: int=5) -> list[dict[str, Any]]:
        ...
        if self.embedder and self.vector_store:
            try:
                ...
                if query_emb:
                    ...
                    vector_results = [{'source': 'vector', 'id': r.get('id'), 'score': r.get('score', 0.0), 'text': (r.get('metadata') or {}).get('text', ''), 'metadata': r.get('metadata') or {}} for r in raw or []]
            # guardian: allow-silent-swallow
            except Exception as e:
        ...
        if self.bm25_store:
            try:
                ...
                bm25_results = [{'source': 'bm25', 'id': r.get('id'), 'score': r.get('score', 0.0), 'text': r.get('content', ''), 'metadata': r.get('metadata') or {}} for r in bm25_results or []]
            # guardian: allow-silent-swallow
            except Exception as e:
    ...
    def _fuse_results(self, vector: list[dict[str, Any]], bm25: list[dict[str, Any]], k: int=60) -> list[dict[str, Any]]:
