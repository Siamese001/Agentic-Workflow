...
class LegacyAgentAdapter:
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            if hasattr(self.agent, 'fix'):
                ...
                return {'status': 'failed', 'errors': [f'Agent {self.name} has no recognized healing method (fix/run/resolve)']}
    ...
    def _wrap_legacy_result(self, result: Any) -> dict[str, Any]:
        ...
        if isinstance(result, bool):
            return {'status': 'success' if result else 'failed', 'details': 'Legacy boolean return', 'artifacts': [], 'errors': []}
        if isinstance(result, str):
            return {'status': 'success', 'details': result, 'artifacts': [], 'errors': []}
        if isinstance(result, list):
            return {'status': 'success', 'details': f'Modified {len(result)} files', 'artifacts': result, 'errors': []}
