...
class ExecutionTrace:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'trace_id': self.trace_id, 'plan_hash': self.plan_hash, 'actor': self.actor, 'target': self.target, 'diff': self.diff, 'policy_hash': self.policy_hash, 'timestamp': self.timestamp, 'prev_hash': self.prev_hash, 'replay_key': self.replay_key, 'governed_payload_hash': self.governed_payload_hash}

def create_execution_trace_skeleton(trace_id: str, plan_hash: str, governed_payload: GovernedPayload, actor: str='L3_Orchestrator', target: str | None=None) -> ExecutionTrace:
    """
    ...
    payload_dict = {'s0_system': governed_payload.s0_system, 'i0_instructional': governed_payload.i0_instructional, 'c0_context': governed_payload.c0_context, 'u0_user_prompt': governed_payload.u0_user_prompt, 'manifest_hash': governed_payload.manifest_hash, 'routing_hash': governed_payload.routing_hash}
    governed_payload_hash = hashlib.sha256(canonical_json(payload_dict).encode('utf-8')).hexdigest()
    trace = ExecutionTrace(trace_id=trace_id, plan_hash=plan_hash, actor=actor, target=target, governed_payload_hash=governed_payload_hash, timestamp=datetime.now(timezone.utc).isoformat())
    return trace
