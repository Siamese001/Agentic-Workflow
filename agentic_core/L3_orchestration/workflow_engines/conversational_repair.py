...
class ConversationalRepair:
    ...
    async def debate_failure(self, failure_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for specialist_id, config in self.specialists.items():
            response: Any = await self._query_specialist(specialist_id, failure_context, previous_responses=[])
            specialist_responses[specialist_id] = response
            debate_log.append({'round': 1, 'specialist': config['name'], 'analysis': response['analysis'], 'proposal': response['proposal']})
            LOGGER.info(f"  {config['name']}: {response['analysis'][:100]}...")
        LOGGER.info('Round 2: Reactive analysis and refinement')
        for specialist_id, config in self.specialists.items():
            others: Any = [f"{self.specialists[oid]['name']}: {specialist_responses[oid]['proposal']}" for oid in self.specialists if oid != specialist_id]
            response: Any = await self._query_specialist(specialist_id, failure_context, previous_responses=others)
            specialist_responses[specialist_id] = response
            debate_log.append({'round': 2, 'specialist': config['name'], 'analysis': response['analysis'], 'proposal': response['proposal']})
        ...
        result: Any = {'success': consensus_code is not None, 'consensus_code': consensus_code, 'debate_log': debate_log, 'specialist_responses': specialist_responses, 'consensus_reasoning': consensus_response}
        if result['success']:
    ...
    async def _query_specialist(self, specialist_id: str, failure_context: Dict[str, Any], previous_responses: List[str]) -> Dict[str, str]:
        ...
        prompt = config['prompt_template'].format(failure_info=json.dumps(failure_context, indent=2), previous_responses='\n\n'.join(previous_responses) if previous_responses else 'None yet')
    ...
    async def _query_llm(self, prompt: str) -> str:
        ...
        try:
            import openai
            response = await openai.ChatCompletion.acreate(model='gpt-4', messages=[{'role': 'user', 'content': prompt}], temperature=0.7, max_tokens=1000)
