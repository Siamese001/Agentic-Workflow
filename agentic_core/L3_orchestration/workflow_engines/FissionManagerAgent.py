...
class FissionManagerAgent(MCPHardenedMixin, SubatomicTestingMixin, HealerMixin):
    ...
    def should_trigger_fission(
            ...
            try:
                with open(file_path, encoding="utf-8") as f:
                    ...
                    if line_count > self.line_limit:
                        return (
                            True,
                            f"Static Metric: File exceeds L4 line limit (Key 42): {line_count} > {self.line_limit}",
                        )
        ...
        if lines_deleted > self.deletion_guardrail:
            return (
                True,
                f"L5 Safety: Detected destructive mass deletion ({lines_deleted}L > {self.deletion_guardrail}L)",
            )
        if current_round >= self.max_rounds and last_error and ("SyntaxError" in str(last_error)):
            return (
                True,
                f"L1 Cognition: Reasoning exhausted after {current_round} rounds (SyntaxError loop)",
            )
        return (False, None)
    ...
    async def execute_fission(self, file_path: str, content: str, reason: str) -> FissionResult:
        ...
        try:
            ...
            config: Any = types.GenerateContentConfig(
                temperature=0.2,
                thinking_config=types.ThinkingConfig(thinking_budget=50000),
                tools=[],
            )
            response: Any = self._client.models.generate_content(
                model=os.getenv("GEMINI_MODEL", "gemini-2.5-flash"), contents=prompt, config=config
            )
    ...
    def handle_architect_output(
        self, output_text: str, mission_type: str, current_target: str | None = None
    ) -> bool:
        """
        ...
        for file_path in python_files:
            try:
                ...
                trigger, reason = self.should_trigger_fission(
                    file_path=str(file_path), current_round=1, last_error=None, lines_deleted=0
                )
