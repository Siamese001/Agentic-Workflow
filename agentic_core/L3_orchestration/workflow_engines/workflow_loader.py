...
class WorkflowLoader:
    ...
    def _load_workflow(self) -> None:
        ...
        try:
            ...
            LOGGER.warning(
                f"Workflow file not found at {self.workflow_path}, using fallback defaults"
            )
            ...
            LOGGER.error(
                f"Failed to load workflow from {self.workflow_path}: {e}, using fallback defaults"
            )
            self._workflow_data = self._get_fallback_workflow()
    ...
    def get_pre_flight_tests(self) -> list[dict[str, Any]]:
        ...
        if self._cached_pre_flight_tests is None:
            self._cached_pre_flight_tests = self._workflow_data.get(
                "pre_flight_engine_validation", {}
            ).get(TESTS_DIR, [])
    ...
    def get_file_complexity_thresholds(self) -> dict[str, int]:
        ...
        if self._cached_file_complexity_thresholds is None:
            ...
            self._cached_file_complexity_thresholds = CONTEXT.get(
                "pre_flight_file_complexity_gate", {}
            ).get("thresholds", {})
