...
class ForwardRollingFacade:
    ...
    def __init__(
        ...
        Logger.info(
            f"[ForwardRollingFacade] Initialized with stage={initial_stage.value}, "
    ...
    def _execute_forward_rolling(
        self, agent_name: str, context: ExecutionContext
    ) -> ForwardRollingResult:
    ...
    def _update_metrics(self, result: ForwardRollingResult, duration_ms: float) -> None:
        ...
        if self._execution_times:
            self._metrics.avg_execution_time_ms = sum(self._execution_times) / len(
        ...
        if self._result_cache:
            ...
            cache_hits = sum(
                1 for r in self._result_cache.values() if r.metadata.get("cache_hit", False)
            )
