...
class SubAtomicEngineImpl:
    ...
    def __init__(
        ...
        if gemini_client:
            ...
            if not api_key:
                ...
                if api_key:
                    Logger.warning(
                        "[L5] Using legacy GEMINI_API_KEY. Please migrate to GOOGLE_API_KEY."
                    )
            if not api_key:
    ...
    def get_safe_config(is_fission: bool = False) -> Any:
        ...
        return types.GenerateContentConfig(
            temperature=0.1, thinking_config=types.ThinkingConfig(thinking_budget=safe_budget)
        )
    ...
    async def get_embedding(self, text: str) -> list[float]:
        ...
        try:
            result: Any = await asyncio.to_thread(
                self._client.models.embed_content, model="text-embedding-004", contents=text
            )
    ...
    async def resilient_mutation(self, *args, system_prompt: str | None = None, **kwargs) -> str:
        ...
        scrubbed_kwargs: Any = {
            ...
            if k not in ["stop_sequences", "top_p", "response_format"]
        }
        ...
        **kwargs,
        ...
        if self.redis_client:
            ...
            if current_fails and int(current_fails) >= 2:
                Logger.warning(
                    f"   [ADAPTIVE] Repeat failure ({current_fails}) detected for {file_path}. Bumping temperature."
                )
                temp_override = 0.8
                ...
                if self.pinecone and hasattr(self.pinecone, "semantic_search"):
                    try:
                        context_chunks = await self.pinecone.semantic_search(
                            query=Task, file_path=Path(file_path), top_k=3
                        )
                        if context_chunks:
                            context_str = "\n\n".join(context_chunks)
                    except Exception as e:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                response = await asyncio.to_thread(
                    self.chat_sessions[chat_key].send_message, prompt
                )
                break
                ...
                Logger.warning(
                    f"   [!] Gemini Transient Error ({attempt}/{max_retries}): {e}. Retrying in {wait:.1f}s"
                )
        ...
        if response and response.candidates and response.candidates[0].content.parts:
            ...
            if not fission_active:
                try:
                    ...
                    ValidationResult = validator.validate_healed_code(
                        code, healed_code, Path(file_path)
                    )

                    if not ValidationResult["valid"]:
                        Logger.error(
                            f"   [X] HEAL REJECTED ({ValidationResult['stage']}): {ValidationResult['reason']}"
                        )
                        if self.redis_client:
                            self.redis_client.incr(f"fail_count:{file_path}")
                        return code
