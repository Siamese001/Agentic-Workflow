...
def tool_spec_to_json(spec: ToolSpec) -> str:
    ...
    data = {'tool_id': spec.tool_id, 'description': spec.description, 'side_effect_class': spec.side_effect_class, 'args': [{'name': arg.name, 'kind': arg.kind, 'required': arg.required, 'default': arg.default} for arg in spec.args], 'output_kind': spec.output_kind, 'version': spec.version}
...
def tool_call_result_to_json(result: ToolCallResult) -> str:
    ...
    data = {'exit_code': result.exit_code, 'stdout': result.stdout, 'stderr': result.stderr, 'truncated': result.truncated, 'hashes': result.hashes}
    return canonical_json(data)
__all__ = ['ToolArg', 'ToolSpec', 'ToolCall', 'ToolCallResult', 'canonical_json', 'sha256_hex', 'generate_call_id', 'hash_result_data', 'tool_spec_to_json', 'tool_call_to_json', 'tool_call_result_to_json']
