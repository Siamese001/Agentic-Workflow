...
def _hash_command_result(command: ReplayCommand, result: ReplayResult) -> str:
    ...
    data = {'argv': command.argv, 'cwd': command.cwd, 'env_allowlist': command.env_allowlist, 'exit_code': result.exit_code, 'stdout': result.stdout, 'stderr': result.stderr}
...
def run_and_record(commands: list[ReplayCommand]) -> ReplayRecord:
    ...
    for command in commands:
        ...
        result = subprocess.run(command.argv, shell=False, text=True, capture_output=True, cwd=command.cwd, env=env, timeout=command.timeout_s)
        ...
        replay_result = ReplayResult(exit_code=result.returncode, stdout=truncated_stdout, stderr=truncated_stderr)
    ...
    metrics = ReplayMetrics(per_command_bytes_out=per_command_bytes_out, per_command_bytes_err=per_command_bytes_err, total_bytes_out=sum(per_command_bytes_out), total_bytes_err=sum(per_command_bytes_err))
...
def record_to_json(record: ReplayRecord) -> str:
    ...
    """
    data = {'version': record.version, 'created_utc': record.created_utc, 'commands': [{'argv': cmd.argv, 'cwd': cmd.cwd, 'env_allowlist': cmd.env_allowlist, 'timeout_s': cmd.timeout_s} for cmd in record.commands], 'results': [{'exit_code': res.exit_code, 'stdout': res.stdout, 'stderr': res.stderr} for res in record.results], 'hashes': record.hashes}
    if record.metrics is not None:
        data['metrics'] = {'per_command_bytes_out': record.metrics.per_command_bytes_out, 'per_command_bytes_err': record.metrics.per_command_bytes_err, 'total_bytes_out': record.metrics.total_bytes_out, 'total_bytes_err': record.metrics.total_bytes_err}
    return json.dumps(data, sort_keys=True, indent=2)

def record_from_json(json_str: str) -> ReplayRecord:
    """Deserialize JSON string to ReplayRecord."""
    data = json.loads(json_str)
    commands = [ReplayCommand(argv=cmd['argv'], cwd=cmd['cwd'], env_allowlist=cmd['env_allowlist'], timeout_s=cmd.get('timeout_s', 300)) for cmd in data['commands']]
    results = [ReplayResult(exit_code=res['exit_code'], stdout=res['stdout'], stderr=res['stderr']) for res in data['results']]
    metrics = None
    if 'metrics' in data:
        metrics_data = data['metrics']
        metrics = ReplayMetrics(per_command_bytes_out=metrics_data['per_command_bytes_out'], per_command_bytes_err=metrics_data['per_command_bytes_err'], total_bytes_out=metrics_data['total_bytes_out'], total_bytes_err=metrics_data['total_bytes_err'])
    return ReplayRecord(version=data['version'], created_utc=data['created_utc'], commands=commands, results=results, hashes=data['hashes'], metrics=metrics)
...
def replay_and_compare(record: ReplayRecord) -> ComparisonResult:
    ...
    for i, (command, original_result) in enumerate(zip(record.commands, record.results)):
        try:
            ...
            env.update(command.env_allowlist)
            result = subprocess.run(command.argv, shell=False, text=True, capture_output=True, cwd=command.cwd, env=env, timeout=command.timeout_s)
            current_result = ReplayResult(exit_code=result.returncode, stdout=result.stdout, stderr=result.stderr)
            if current_result.exit_code != original_result.exit_code:
                mismatches.append(f'Command {i + 1}: Exit code mismatch (original={original_result.exit_code}, current={current_result.exit_code})')
            ...
            if orig_stdout_norm != curr_stdout_norm:
                ...
                if not first_diff_lines:
                    ...
                    for j, (orig, curr) in enumerate(zip(orig_lines, curr_lines)):
                        if orig != curr:
                            first_diff_lines.extend([f'First difference at line {j + 1}:', f'Original: {orig}', f'Current:  {curr}'])
                            break
    ...
    return ComparisonResult(is_match=len(mismatches) == 0, mismatches=mismatches, first_diff_summary='\n'.join(first_diff_lines[:200]))
__all__ = ['ReplayCommand', 'ReplayResult', 'ReplayRecord', 'ComparisonResult', 'run_and_record', 'record_to_json', 'record_from_json', 'replay_and_compare']
