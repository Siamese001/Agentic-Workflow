...
def _run_guardians(repo_root: Path, timestamp: str, correlation_id: str | None=None, write_artifacts_dir: str | None=None) -> dict:
    ...
    result = run_all_guardians(repo_root=repo_root, write_artifacts_dir=write_artifacts_dir, timestamp=timestamp, correlation_id=correlation_id)
    return json.loads(result.to_json())

def _run_dispatcher(guardian_aggregate: dict, write_artifacts_dir: Path, created_utc: str, *, apply: bool=False, repo_root: Path | None=None, allow_repo_mutation: bool=False) -> dict:
    ...
    try:
        result = run_dispatcher(guardian_result_path=agg_path, write_artifacts_dir=write_artifacts_dir, created_utc=created_utc, apply=apply, repo_root=repo_root, allow_repo_mutation=allow_repo_mutation)
        return result.to_dict()
    finally:
        _wg.remove_file(agg_path)

def run_pipeline(mode: str='scan', repo_root: Path | None=None, write_artifacts_dir: str | None=None, timestamp: str | None=None, correlation_id: str | None=None, allow_repo_mutation: bool=False) -> dict:
    ...
    guardian_aggregate = _run_guardians(repo_root=repo_root, timestamp=timestamp, correlation_id=correlation_id, write_artifacts_dir=guardian_artifacts_dir)
    pipeline_result: dict = {'tool_id': TOOL_ID, 'mode': mode, 'timestamp': timestamp, 'guardian_result': guardian_aggregate}
    if mode == 'scan':
        return pipeline_result
    heal_dir = Path(write_artifacts_dir) if write_artifacts_dir else repo_root / 'docs' / REPORTS_DIR / 'plans'
    heal_result = _run_dispatcher(guardian_aggregate=guardian_aggregate, write_artifacts_dir=heal_dir, created_utc=timestamp, apply=mode == 'apply', repo_root=repo_root if mode == 'apply' else None, allow_repo_mutation=allow_repo_mutation)
    pipeline_result['heal_result'] = heal_result
    return pipeline_result

def main() -> int:
    parser = argparse.ArgumentParser(description='L0 Thin Router — Guardian→Dispatcher→Healer pipeline', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='Modes:\n  --scan       Run guardians only, emit aggregate JSON (default)\n  --dry-run    Run guardians + dispatcher + healers in dry-run mode\n  --apply      Run full pipeline with apply-mode healers (sandbox-gated)\n')
    ...
    if args.apply:
        ...
        mode = 'scan'
    try:
        result = run_pipeline(mode=mode, repo_root=Path(args.repo_root) if args.repo_root else None, write_artifacts_dir=args.write_artifacts, timestamp=args.timestamp, correlation_id=args.correlation_id, allow_repo_mutation=args.allow_repo_mutation)
