...
def _get_imports():
    ...
    return {'ValidationContext': None, 'CanonSwarmScheduler': CanonSwarmScheduler, 'start_intervention_server': start_intervention_server, 'approval_event': approval_event, 'FASTAPI_AVAILABLE': FASTAPI_AVAILABLE, 'ArchitectureGovernor': ArchitectureGovernor, 'CodeStyleGuardian': None, 'ConcurrencyGuardianAgent': None, 'DependencySentinelAgent': None, 'GitAgent': None, 'Historian': None, 'HygieneGuardian': None, 'NamingEnforcer': None, 'TypeEnforcer': None, 'PatternEnforcerAgent': None, 'StructuralEngineer': None, 'SafetyInspectorAgent': None, 'TestPilot': None, 'ReflectionAgent': None, 'StrategicPlannerAgent': None, 'WatchmanHandler': WatchmanHandler}
...
def _v15_build_mission_manifest(mode_name: str, target_layer: str='L3'):
    ...
    manifest = SurgicalManifest(schema_version='1.0.0', correlation_id=trace_id, node_id='MissionRunner', target_layer=target_layer, ast_snippet=ast_snippet, serialization_canon='mission_runner', fix_constraint=FixConstraint.RELAXED, manifest_hash=_hl.sha256(ast_snippet.encode()).hexdigest(), change_history=(), provenance_chain=(trace_id,))
...
def _v15_gateway_audit(manifest, trace_id: str) -> None:
    ...
    try:
        ...
        gw.execute(manifest, lambda m: {'status': 'audit', 'errors': 0}, lambda: (_hl.sha256(b'fs_mission').hexdigest(), _hl.sha256(b'git_mission').hexdigest(), _hl.sha256(b'mem_mission').hexdigest()), trace_id=trace_id, agent_id='mission_runner')
...
def run_standard_mode():
    ...
    # guardian: allow-silent-swallow
        ...
        sys.exit(1)
    agents = [ArchitectureGovernor(ctx), HygieneGuardian(ctx), CodeStyleGuardian(ctx), DependencySentinelAgent(ctx), SafetyInspectorAgent(ctx), PatternEnforcerAgent(ctx), NamingEnforcer(ctx), TypeEnforcer(ctx), ConcurrencyGuardianAgent(ctx), StructuralEngineer(ctx), Historian(ctx), TestPilot(ctx)]
...
async def _check_intervention(cycle: int, ctx, FASTAPI_AVAILABLE: bool, start_intervention_server, approval_event) -> bool:
    """Check if human intervention is required and handle it."""
    high_risk = cycle >= 3 and len(ctx.modified_files) > 8 or ('TEST_FAILURE' in ctx.signals and cycle > 2) or len(ctx.signals) > 5
