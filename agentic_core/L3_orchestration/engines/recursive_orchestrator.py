...
class RetryContext:
    ...
    def to_parameters(self) -> dict[str, Any]:
        ...
        return {'retry_context': {'original_node_id': self.original_node_id, 'attempt_number': self.attempt_number, 'max_attempts': self.max_attempts, 'failure_reasons': self.failure_reasons, 'accumulated_context': self.accumulated_context}}
...
class RecursiveOrchestrator(SovereignBaseAgent):
    ...
    # guardian: allow-type-erasure
    def handle_task_status(self, node_id: str, status: TaskStatus, failure_reason: str | None=None, retry_function: str | None=None, additional_context: dict[str, Any] | None=None) -> dict[str, Any]:
        """
        ...
        if status in (TaskStatus.FAILED, TaskStatus.NEEDS_REVISION):
            ...
            return self.handle_task_failure(failed_node_id=node_id, failure_reason=failure_reason, retry_function=retry_function, additional_context=additional_context)
        return {'action': 'none', 'status': status.value, 'node_id': node_id}

    # guardian: allow-type-erasure
    def handle_task_failure(self, failed_node_id: str, failure_reason: str, retry_function: str | None=None, additional_context: dict[str, Any] | None=None) -> dict[str, Any]:
        """
        ...
        if not retry_ctx.can_retry:
            logger.warning(f'Max retries exceeded for chain starting at {retry_ctx.original_node_id}. Attempts: {retry_ctx.attempt_number}/{retry_ctx.max_attempts}')
            if self.on_max_retries_exceeded:
                self.on_max_retries_exceeded(failed_node_id, retry_ctx)
            return {'action': 'max_retries_exceeded', 'original_node_id': retry_ctx.original_node_id, 'attempts': retry_ctx.attempt_number, 'failure_reasons': retry_ctx.failure_reasons}
        if retry_function is None:
            retry_function = self._get_node_function(failed_node_id)
        if retry_function is None:
            raise ValueError(f'Cannot determine retry function for {failed_node_id}. Provide retry_function parameter.')
        result = self._spawn_retry_successor(failed_node_id=failed_node_id, retry_function=retry_function, retry_context=retry_ctx)
        if result.get('success'):
            ...
            logger.info(f'Spawned retry node {new_node_id} for {failed_node_id} (attempt {retry_ctx.attempt_number}/{retry_ctx.max_attempts})')
        return result

    # guardian: allow-type-erasure
    def _spawn_retry_successor(self, failed_node_id: str, retry_function: str, retry_context: RetryContext) -> dict[str, Any]:
        """
        ...
        """
        from agentic_core.L3_orchestration.reasoning.dag_mutator_agent_config import DAGMutation, HopSpec, MutationAction
        ...
        hop_spec = HopSpec(hop_function=retry_function, parameters=final_params, priority=1, retry_policy={'max_attempts': 0})
        mutation = DAGMutation(action=MutationAction.SPAWN_SUCCESSOR, target_hop_id=failed_node_id, new_hop_spec=hop_spec, reason=f'Retry attempt {retry_context.attempt_number} after failure: {retry_context.failure_reasons[-1][:100]}', requester_hop_id='recursive_orchestrator')
        mutation_result = self.dag_manager.request_mutation(mutation)
        return {'success': mutation_result.success, 'message': mutation_result.message, 'new_node_id': hop_spec.hop_id if mutation_result.success else None, 'mutation_id': mutation_result.mutation_id, 'affected_nodes': mutation_result.affected_nodes, 'attempt_number': retry_context.attempt_number}
    ...
    def _cleanup_retry_context(self, node_id: str) -> None:
        ...
        if node_id in self.retry_contexts:
            ...
            logger.info(f'Retry chain completed successfully for {ctx.original_node_id} after {ctx.attempt_number} attempts')
    ...
    def get_retry_status(self, node_id: str) -> dict[str, Any] | None:
        ...
        if node_id not in self.retry_contexts:
            return None
        ctx = self.retry_contexts[node_id]
        return {'original_node_id': ctx.original_node_id, 'current_attempt': ctx.attempt_number, 'max_attempts': ctx.max_attempts, 'can_retry': ctx.can_retry, 'failure_count': len(ctx.failure_reasons), 'failure_reasons': ctx.failure_reasons}
