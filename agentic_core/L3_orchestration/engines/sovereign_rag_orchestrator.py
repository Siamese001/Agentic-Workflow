...
from agentic_core.L3_orchestration.types.rag_provider_types import IRagProvider, RagDocument, RagQuery, RagResult
...
class SovereignRagOrchestrator(SovereignBaseAgent, IRagProvider):
    ...
    def __init__(self, retriever: Any | None=None, query_planner: Any | None=None, guardrail: Any | None=None, engine: Any | None=None) -> None:
        """
    ...
    def _init_titanium_pipeline(self) -> None:
        ...
        try:
            ...
            self.titanium_pipeline = TitaniumRAGPipeline(enable_compression=True, enable_decomposition=True, enable_reranking=True, enable_caching=True)
    ...
    def _save_sovereign_config(self) -> None:
        ...
        _wg.write_text(self.config_path, json.dumps({'faithfulness_threshold': self.faithfulness_threshold, 'max_hops': self.max_hops, 'base_top_k': self.base_top_k}))
    ...
    async def retrieve(self, query: RagQuery) -> RagResult:
        ...
        if self.titanium_pipeline:
            ...
            result = await self.titanium_pipeline.query(query.query, retrieval_function=retrieval_func, top_k_final=query.top_k)
            documents = [RagDocument(id=doc.doc_id, text=doc.metadata.get('text', ''), score=doc.final_score, metadata=doc.metadata, source='titanium_pipeline') for doc in result['documents']]
            return RagResult(query=query.query, documents=documents, latency_ms=(time.time() - start_time) * 1000, cached=result['metadata'].get('cached', False), reranked=result['metadata'].get('reranked', False), metadata=result['metadata'])
        else:
            legacy_result = await self.sovereign_retrieve(query.query, top_k=query.top_k, filters=query.filters, mission_context=query.mission_context)
            documents = [RagDocument(id=f'doc_{i}', text=doc.text if hasattr(doc, 'text') else str(doc), score=doc.score if hasattr(doc, 'score') else 0.0, metadata={}, source='legacy_retriever') for i, doc in enumerate(legacy_result.get('documents', []))]
            return RagResult(query=query.query, documents=documents, latency_ms=(time.time() - start_time) * 1000, faithfulness_score=legacy_result.get('faithfulness', 0.0), metadata=legacy_result)
    ...
    def get_health(self) -> dict[str, Any]:
        ...
        return {'retriever_available': self.retriever is not None, 'guardrail_available': self.guardrail is not None, 'engine_available': self.engine is not None, 'titanium_pipeline_available': self.titanium_pipeline is not None, 'config': self.get_config()}
    ...
    async def sovereign_retrieve(self, query: str, top_k: int | None=None, filters: dict | None=None, mission_context: dict | None=None) -> dict[str, Any]:
        ...
        _anchors = [AnchoredResult(content=doc.content if hasattr(doc, 'content') else str(doc), anchor=RetrievalAnchor(source_doc_id=getattr(doc, 'doc_id', getattr(doc, 'id', f'doc-{i}')), chunk_id=getattr(doc, 'chunk_id', f'chunk-{i}'), char_start=0, char_end=len(doc.content if hasattr(doc, 'content') else str(doc)), retrieved_at_utc=RetrievalAnchor.now_utc(), version_hash=getattr(doc, 'content_hash', getattr(doc, 'hash', 'unknown')))) for i, doc in enumerate(final_docs)]
        result: Any = {'query': query, 'documents': final_docs, 'anchors': _anchors, 'faithfulness': 0.85, 'top_k': top_k, 'hops': hop + 1}
    ...
    async def agentic_retrieve_with_reflection(self, query: str, top_k: int | None=None) -> dict[str, Any]:
        ...
        for _round in range(self._MAX_REFLECTION_ROUNDS):
            ...
            if sufficiency >= self._SUFFICIENCY_THRESHOLD:
                break
            logger.info('agentic_retrieve_with_reflection: sufficiency=%.3f < %.3f — round %d refinement', sufficiency, self._SUFFICIENCY_THRESHOLD, _round + 1)
            try:
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {'status': 'skipped', 'details': f'SovereignRagOrchestrator heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        # guardian: allow-silent-swallow
        except Exception as e:
            return {'status': 'failed', 'details': f'SovereignRagOrchestrator heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
