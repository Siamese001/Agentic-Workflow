...
class AgentGym(SovereignBaseAgent):
    ...
    def _load_default_scenarios(self) -> None:
        ...
        if self.golden_evaluator.golden_cases:
            TrainingScenario(id='golden_dataset_core', NAME='Core Golden Dataset', ScenarioType=ScenarioType.GOLDEN_DATASET, DESCRIPTION='Core test cases from golden dataset', test_cases=self.golden_evaluator.golden_cases, success_threshold=THRESHOLD)
            self._scenarios[scenario.id] = scenario
