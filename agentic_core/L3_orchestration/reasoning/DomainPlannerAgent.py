...
class DomainPlannerAgent(L3OrchestrationBase):
    ...
    async def run_async(self, plan: StrategyPlan, job_context: dict[str, Any], workflow_id: str) -> PlannerAssessment:
        ...
        if not focus_matches:
            recommended_actions.append('Introduce a focus area that explicitly references the job title or company priorities.')
        rationale = 'Focus areas reference role/company context.' if focus_matches else 'No explicit domain alignment detected.'
        assessment = PlannerAssessment(planner_name='DomainPlanner', vote=vote, rationale=rationale, confidence=round(confidence, 2), recommended_actions=recommended_actions)
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {'status': 'skipped', 'details': f'DomainPlannerAgent heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        except Exception as e:
            return {'status': 'failed', 'details': f'DomainPlannerAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
...
class RiskAssessorAgent(SovereignBaseAgent):
    ...
    async def run_async(self, plan: StrategyPlan, job_context: dict[str, Any], workflow_id: str) -> PlannerAssessment:
        ...
        rationale = 'Low strategic risk detected.' if not risk_signals else 'Detected ' + ', '.join(risk_signals)
        confidence = 0.7 if vote == 'approve' else 0.6
        assessment = PlannerAssessment(planner_name='RiskAssessor', vote=vote, rationale=rationale, confidence=confidence, recommended_actions=recommended_actions)
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        violation_type = violation.get('type', 'unknown')
        try:
            return {'status': 'skipped', 'details': f'RiskAssessorAgent heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        except Exception as e:
            return {'status': 'failed', 'details': f'RiskAssessorAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
...
class FeasibilityAnalystAgent(SovereignBaseAgent):
    ...
    async def run_async(self, plan: StrategyPlan, job_context: dict[str, Any], workflow_id: str) -> PlannerAssessment:
        ...
        assessment = PlannerAssessment(planner_name='FeasibilityAnalyst', vote=vote, rationale=rationale, confidence=confidence, recommended_actions=recommended_actions)
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        violation_type = violation.get('type', 'unknown')
        try:
            return {'status': 'skipped', 'details': f'FeasibilityAnalystAgent heal() not implemented for {violation_type}', 'artifacts': [], 'errors': []}
        except Exception as e:
            return {'status': 'failed', 'details': f'FeasibilityAnalystAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}

class StrategyScenarioSimulatorAgent(SovereignBaseAgent):
    """Runs lightweight scenario stress tests on a strategy plan."""

    async def run_async(self, plan: StrategyPlan, job_context: dict[str, Any], workflow_id: str) -> list[ScenarioSimulationResult]:
        ...
        scenarios.append(ScenarioSimulationResult(scenario_name='Hiring Manager Adoption', risk_level=adoption_risk, impact_score=adoption_impact, summary='Metrics-driven achievements improve adoption.' if quantified else 'Lack of metrics may slow stakeholder buy-in.', mitigation_actions=adoption_mitigations))
        ...
        scenarios.append(ScenarioSimulationResult(scenario_name='Technical Deep Dive', risk_level=technical_risk, impact_score=technical_impact, summary='Technical focus prepares for deep dive discussions.' if technical_focus else 'Potential technical grilling may expose gaps in focus areas.', mitigation_actions=technical_mitigations))
        cross_functional_risk = 'low' if leadership_focus else 'medium'
        cross_functional_impact = 0.3 if leadership_focus else 0.6
        cross_functional_mitigations = [] if leadership_focus else ['Introduce leadership/collaboration narratives into focus areas.']
        scenarios.append(ScenarioSimulationResult(scenario_name='Cross-Functional Alignment', risk_level=cross_functional_risk, impact_score=cross_functional_impact, summary='Leadership emphasis supports cross-functional narratives.' if leadership_focus else 'Missing leadership signal may reduce collaboration confidence.', mitigation_actions=cross_functional_mitigations))
        self.log_feedback(workflow_id, 'strategy_simulation', 'success', {'scenarios': [scenario.model_dump() for scenario in scenarios]})
        return scenarios
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        violation_type = violation.get('type', 'unknown')
        try:
            return {'status': 'skipped', 'details': f'StrategyScenarioSimulatorAgent heal() not implemented for {violation_type}', 'artifacts': [], 'errors': []}
        except Exception as e:
            return {'status': 'failed', 'details': f'StrategyScenarioSimulatorAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
...
class StrategyCoordinatorAgent(SovereignBaseAgent):
    ...
    async def run_async(self, job_context: dict[str, Any], base_plan: StrategyPlan, workflow_id: str, downstream_feedback: dict[str, Any] | None=None) -> StrategyPlan:
        ...
        assessments = await asyncio.gather(self.domain_planner.run_async(plan, job_context, workflow_id), self.risk_assessor.run_async(plan, job_context, workflow_id), self.feasibility_analyst.run_async(plan, job_context, workflow_id))
        ...
        for assessment in assessments:
            ...
            rationale_parts.append(f'{assessment.planner_name}: {assessment.vote} ({_truncate(assessment.rationale, 80)})')
        ...
        scenario_summary = ', '.join((f'{result.scenario_name}:{result.risk_level}' for result in scenario_results))
        ...
        self.log_feedback(workflow_id, 'strategy_coordinator', 'success', {'aggregated_decision': aggregated_decision, 'aggregated_confidence': aggregated_confidence, 'feedback_signals': feedback_signals})
        return plan
        ...
        if isinstance(qa_feedback, dict):
            ...
            if isinstance(jd_skills, dict):
                ...
                if isinstance(missing_keywords, list) and missing_keywords:
                    ...
                    signals.append('Augmented focus areas with QA missing keywords: ' + ', '.join(missing_keywords[:5]))
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        violation_type = violation.get('type', 'unknown')
        try:
            return {'status': 'skipped', 'details': f'StrategyCoordinatorAgent heal() not implemented for {violation_type}', 'artifacts': [], 'errors': []}
        except Exception as e:
            return {'status': 'failed', 'details': f'StrategyCoordinatorAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
