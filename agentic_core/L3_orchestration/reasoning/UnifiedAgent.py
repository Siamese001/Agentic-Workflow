...
class ValidationResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'passed': self.passed, 'issues': self.issues, 'suggestions': self.suggestions, 'score': self.score, 'metadata': self.metadata}
...
class OrchestrationResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary for serialization."""
        return {'completed': self.completed, 'stage': self.stage, 'signals': self.signals, 'artifacts': self.artifacts, 'next_actions': self.next_actions, 'errors': self.errors, 'metadata': self.metadata}
...
class HealingResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary for serialization."""
        return {'violations_found': self.violations_found, 'violations_fixed': self.violations_fixed, 'errors': self.errors, 'skipped': self.skipped, 'artifacts': self.artifacts}
...
class BaseStrategy(ABC):
    ...
    @abstractmethod
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> ValidationResult | OrchestrationResult | HealingResult | dict[str, Any]:
        """Execute strategy logic."""
        pass

    def heal_repository(self, agent: UnifiedAgent, dry_run: bool, execute: bool, **kwargs: Any) -> dict[str, int]:
    ...
    def heal(self, agent: UnifiedAgent, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        return {'status': 'skipped', 'details': f'{self.__class__.__name__} heal() not yet implemented', 'artifacts': [], 'errors': []}
...
class ValidatorStrategy(BaseStrategy):
    ...
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> ValidationResult:
        ...
        if not target_data:
            return ValidationResult(passed=False, issues=['No target data available for validation'], suggestions=[])
        issues: list[str] = []
        suggestions: list[str] = []
        score: float | None = None
        ...
        passed = len(issues) == 0
        return ValidationResult(passed=passed, issues=issues, suggestions=suggestions, score=score, metadata={'rules_applied': list(self.validation_rules.keys())})
...
class OrchestrationStrategy(BaseStrategy):
    ...
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> OrchestrationResult:
        ...
        return OrchestrationResult(completed=len(errors) == 0, stage=current_stage, signals=signals, artifacts=artifacts, next_actions=next_actions, errors=errors)

    async def _execute_workflow_step(self, agent: UnifiedAgent, step: dict[str, Any], **kwargs: Any) -> dict[str, Any]:
...
class HealingStrategy(BaseStrategy):
    ...
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> HealingResult:
        ...
        return HealingResult(violations_found=violations_found, violations_fixed=violations_fixed, errors=errors, skipped=skipped, artifacts=artifacts)
    ...
    def _scan_violations(self, agent: UnifiedAgent) -> list[dict[str, Any]]:
        ...
        for rule_name, rule_config in self.healing_rules.items():
            ...
            if rule_type == 'pattern_match':
                pattern = rule_config.get('pattern', '')
                if pattern:
                    violations.append({'type': rule_name, 'pattern': pattern, 'severity': rule_config.get('severity', 'medium')})
        return violations
...
class GenericStrategy(BaseStrategy):
    ...
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> dict[str, Any]:
        ...
        result = {'status': 'completed', 'message': 'Generic execution completed', 'category': agent._category.value}
        return result
...
class LocationHealingStrategy(HealingStrategy):
    ...
    def heal_repository(self, agent: UnifiedAgent, dry_run: bool, execute: bool, **kwargs: Any) -> dict[str, Any]:
        ...
        return {'violations_found': 0, 'violations_fixed': 0, 'files_moved': 0, 'files_deleted': 0, 'backups_created': 0, 'status': 'NO_VIOLATIONS'}
...
class StructuralValidatorStrategy(ValidatorStrategy):
    ...
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> ValidationResult:
        ...
        return ValidationResult(passed=len(issues) == 0, issues=issues, suggestions=suggestions, metadata={'validator': 'StructuralValidatorAgent'})
...
class CodeValidatorStrategy(ValidatorStrategy):
    ...
    async def execute(self, agent: UnifiedAgent, **kwargs: Any) -> ValidationResult:
        ...
        if file_path and hasattr(agent, 'validate_file'):
            ...
            suggestions = [v.suggested_fix for v in violations if v.suggested_fix]
        return ValidationResult(passed=len(issues) == 0, issues=issues, suggestions=suggestions, metadata={'validator': 'CodeValidatorAgent'})
...
class StructureHealingStrategy(HealingStrategy):
    ...
    def heal_repository(self, agent: UnifiedAgent, dry_run: bool, execute: bool, **kwargs: Any) -> dict[str, Any]:
        ...
        return {'violations_found': violations_found, 'violations_fixed': violations_fixed, 'errors': errors, 'actions': actions}
STRATEGY_MAP: dict[AgentCategory, type] = {AgentCategory.VALIDATOR: ValidatorStrategy, AgentCategory.ORCHESTRATOR: OrchestrationStrategy, AgentCategory.HEALER: HealingStrategy, AgentCategory.GENERIC: GenericStrategy, AgentCategory.EXECUTOR: GenericStrategy, AgentCategory.MONITOR: GenericStrategy, AgentCategory.ANALYZER: ValidatorStrategy, AgentCategory.GOVERNOR: ValidatorStrategy}
...
class UnifiedAgent(SovereignBaseAgent):
    ...
    def __post_init__(self) -> None:
        ...
        self.log_info(f'UnifiedAgent initialized: category={self._category.value}, config={self._config_name}')
    ...
    async def execute(self, **kwargs: Any) -> ValidationResult | OrchestrationResult | HealingResult | dict[str, Any]:
        """Unified execute method delegating to category strategy."""
        if self._strategy is None:
            self._strategy = self._create_strategy()
        return await self._strategy.execute(self, **kwargs)

    def execute_sync(self, **kwargs: Any) -> ValidationResult | OrchestrationResult | HealingResult | dict[str, Any]:
...
__all__ = ['UnifiedAgent', 'AgentCategory', 'ValidationResult', 'OrchestrationResult', 'HealingResult', 'BaseStrategy', 'ValidatorStrategy', 'StructuralValidatorStrategy', 'CodeValidatorStrategy', 'OrchestrationStrategy', 'HealingStrategy', 'LocationHealingStrategy', 'StructureHealingStrategy', 'GenericStrategy', 'STRATEGY_MAP']
