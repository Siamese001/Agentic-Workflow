...
class AutonomousWorkflowEngine:
    ...
    async def run(
        ...
        for iteration in tqdm(range(1, self.max_iterations + 1), desc="Processing", unit="item"):
            ...
            except (ValueError, TypeError) as exc:  # guardian: allow-log-and-swallow -- policy execution: non-fatal, error recorded and loop terminates
            ...
            try:
                ...
                consecutive_failures = 0
            except (ValueError, TypeError) as exc:  # guardian: allow-log-and-swallow -- action execution: non-fatal, step error recorded and failure count incremented
                step.error = str(exc)
