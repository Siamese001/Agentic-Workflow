...
class RLCoordinator(WorkflowCoordinator):
    ...
    def _lazy_load_strategy(self, name: str):
        """Lazy load RL strategy implementation."""
        # Extracted logic from old agents (implement per strategy)
        # Example placeholder:
        if name == 'ppo':
            # from agentic_core.L3_orchestration.strategies.ppo_strategy import PPOStrategy
            # return PPOStrategy()
