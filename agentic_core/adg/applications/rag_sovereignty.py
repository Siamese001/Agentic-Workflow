...
_DECISION_NODES: frozenset[str] = frozenset({canonical_name('Decision', 'RoutingDecision'), canonical_name('Decision', 'SafetyThreshold'), canonical_name('Decision', 'TierSelection')})
_C0_CONTEXT_NODE = canonical_name('Retrieval', 'C0Context')
_RAG_MODULE_PATTERNS: frozenset[str] = frozenset({'SovereignRAGManager', 'SovereignRAGManagerAgent', 'knowledge/reasoning', 'EmbeddingSovereignAgent', 'bmg_embedding_similarity', 'healing_contexts', 'retrieval'})
_DECISION_MODULE_PATTERNS: frozenset[str] = frozenset({'shadow_router_classifier', 'path_router', 'reasoning_policy_engine', 'escalation_router', 'timeshift_router', 'assembly_stage'})
...
def check_rag_sovereignty(result: ScanResult, client: ADGMCPClient | None=None, extra_edges: list[dict] | None=None) -> RAGSovereigntyReport:
    ...
    for edge in decision_edges:
        violations.append(RAGSovereigntyViolation(violation_type='RAG_INFLUENCES_DECISION', from_module=_module_rel(edge.from_name), to_module=_module_rel(edge.to_name), source_file=edge.source_file, line_no=edge.line_no))
    if extra_edges:
        for ee in extra_edges:
            ...
            if relation == 'influences' and to_adg in _DECISION_NODES:
                violations.append(RAGSovereigntyViolation(violation_type='C0_CONTEXT_INFLUENCES_DECISION', from_module=from_adg, to_module=_module_rel(to_adg), source_file='<synthetic>', line_no=0))
    proof_digest = _compute_proof_digest(result, violations)
    report = RAGSovereigntyReport(violations=violations, rag_edges_count=len(rag_edges), decision_edges_count=len(decision_edges), snapshot_digest=proof_digest)
...
def _persist_proof(result: ScanResult, report: RAGSovereigntyReport, client: ADGMCPClient) -> None:
    ...
    client.upsert_entity(proof_node, 'snapshot', [f'commit:{result.commit_sha}', f'snapshot_digest:{report.snapshot_digest}', f'rag_edges:{report.rag_edges_count}', f'decision_edges:{report.decision_edges_count}', f'violation_count:{len(report.violations)}', f'policy_id:{_POLICY_ID}'])

def _ensure_graph_nodes(client: ADGMCPClient) -> None:
    client.upsert_entity(_C0_CONTEXT_NODE, 'retrieval_component', ['component:C0Context', f'policy_id:{_POLICY_ID}'])
