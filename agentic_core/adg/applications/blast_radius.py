...
_LAYER_WEIGHTS: dict[str, int] = {'L0': 100, 'L1': 80, 'L2': 90, 'L3': 70, 'L4': 60, 'L5': 85, 'L6': 50, 'L_APP': 40, 'L_SL': 45, 'L_TOOLS': 30, 'L_OPS': 20, 'L_UNKNOWN': 10}
...
class BlastRadiusResult:
    ...
    def print_summary(self) -> None:
        print(f"ADG-BLAST-RADIUS: commit={self.commit_sha or 'none'} risk_score={self.risk_score} route_mode={self.route_mode} impact_digest={self.impact_digest} changed={len(self.changed_modules)} impacted={len(self.impacted_modules)}")

def _module_rel(adg_name: str) -> str:
    prefix = 'ADG::Module::'
    return adg_name[len(prefix):] if adg_name.startswith(prefix) else adg_name

def compute_blast_radius(changed_files: list[str], result: ScanResult, commit_sha: str='', client: ADGMCPClient | None=None, run_id: str='') -> BlastRadiusResult:
    ...
    if total_weight >= _HUMAN_REVIEW_THRESHOLD:
        ...
        route_mode = 'NORMAL'
    br_result = BlastRadiusResult(changed_modules=sorted(changed_files), impacted_modules=[_module_rel(n) for n in impacted_sorted], risk_score=total_weight, route_mode=route_mode, impact_digest=impact_digest, commit_sha=commit_sha)
...
def _persist_blast_radius(br: BlastRadiusResult, client: ADGMCPClient, run_id: str, commit_sha: str) -> None:
    if run_id:
        ...
        client.upsert_entity(run_node, 'scan_run', [f'commit:{commit_sha}', f'risk_score:{br.risk_score}', f'route_mode:{br.route_mode}', f'impact_digest:{br.impact_digest}', f'impacted_count:{len(br.impacted_modules)}'])
    if commit_sha:
        snap_node = canonical_name('Snapshot', commit_sha, 'blast_radius')
        client.upsert_entity(snap_node, 'snapshot', [f'commit:{commit_sha}', f'risk_score:{br.risk_score}', f'route_mode:{br.route_mode}', f'impact_digest:{br.impact_digest}', f'impacted_count:{len(br.impacted_modules)}'])
