...
class _InMemoryStore:
    ...
    def search_nodes(self, query: str) -> list[dict]:
        q = query.lower()
        return [e for e in self._entities.values() if q in e['name'].lower() or q in e['entityType'].lower() or any((q in obs.lower() for obs in e['observations']))]
    ...
    def open_nodes(self, names: list[str]) -> list[dict]:
        ...
        for n in names:
            if n in self._entities:
                ...
                e['relations'] = [{'from': f, 'relationType': r, 'to': t} for f, r, t in sorted(self._relations) if f == n or t == n]
                result.append(e)
        return result

    def to_json(self) -> str:
        return json.dumps({'entities': self.get_entities(), 'relations': self.get_relations()}, indent=2, sort_keys=True)
