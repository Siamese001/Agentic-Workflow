...
def _get_commit_sha() -> str:
    ...
    try:
        import subprocess
        r = subprocess.run(['git', 'rev-parse', '--short', 'HEAD'], capture_output=True, text=True, timeout=DEFAULT_TIMEOUT)
...
def load_or_scan(repo_root: str | None=None, cache_path: Path | None=None) -> ScanResult:
    ...
    Cache key: commit_sha + scanner_version + schema_version + python_ast_version.
    """
    from agentic_core.adg.extraction.static_scanner import _SCANNER_VERSION, _SCHEMA_VERSION, ADGStaticScanner, ScanResult
