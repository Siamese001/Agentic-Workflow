...
class SSOTCognitiveRecoveryMixin:
    ...
    def suggest_recovery(self, failure_type: str, failure_context: dict[str, Any] | None=None) -> dict[str, Any] | None:
        ...
        suggestion = {'failure_type': failure_type, 'strategy': self._derive_strategy(failure_type), 'confidence': self._estimate_recovery_confidence(failure_type), 'policy_hash': policy_hash, 'timestamp': time.time(), 'context': failure_context or {}}
        self._ssot_recovery_hints.append(suggestion)
        _logger.debug('[SSOTRecovery] Suggestion: %s -> %s (confidence=%.2f)', failure_type, suggestion['strategy'], suggestion['confidence'])
        return suggestion
    ...
    def _derive_strategy(failure_type: str) -> str:
        ...
        strategies = {'import_error': 'fix_imports', 'naming_violation': 'rename_file', 'hierarchy_violation': 'relocate_file', 'syntax_error': 'ast_repair', 'test_failure': 'regenerate_test'}
