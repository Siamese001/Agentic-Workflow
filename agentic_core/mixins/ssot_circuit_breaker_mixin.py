...
# All safety exceptions that must propagate immediately
    ...
    def breaker_call(self, bucket: str, fn: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        *args, **kwargs
        ...
        if state["status"] == "open":
            ...
            if elapsed < self.BREAKER_RECOVERY_TIMEOUT:
                raise CircuitOpenError(
                    f"Circuit breaker open for {scoped_bucket} "
                    f"({elapsed:.1f}s / {self.BREAKER_RECOVERY_TIMEOUT}s)"
                )
        ...
        try:
            ...
            # SAFETY NON-INTERCEPTION: always propagate L5 exceptions
            raise

        except Exception as exc:
            # Record failure (unless replay mode)
            if not getattr(self, "is_replay_mode", False):
