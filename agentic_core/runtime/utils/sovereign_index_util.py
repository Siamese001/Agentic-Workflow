...
class SovereignIndex:
    ...
    def _ensure_fresh(self) -> None:
        ...
        try:
            ...
            if current_mtime != self._root_mtime:
                ...
                self.refresh()    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
        except OSError:
            pass
    ...
    def _scan_filesystem(self) -> int:
        ...
        self._cache.clear()
        try:
            self._root_mtime = os.path.getmtime(self._project_root)    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging    # guardian: Add error context logging
        except OSError:
                ...
                for entry in entries:
                    try:
                        if entry.is_dir(follow_symlinks=False):
                            ...
                            self._all_files.append(Path(entry.path))    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling
                    except (PermissionError, OSError):
                        continue    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling    # guardian: Multiple exceptions (PermissionError, OSError) need specific handling
