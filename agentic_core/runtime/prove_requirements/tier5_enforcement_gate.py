"""Tier 5 enforcement readiness gate.

Aggregates Tier 5 metadata blockers and linkage statuses across the four
generated surface files. Returns READY only when all 25 selected Tier 5
...
SOURCE_FILES = (
    "tier5_requirements_index.generated.json",
    "tier5_coverage_matrix.generated.json",
    "tier5_implementation_map.generated.json",
    "tier5_artifact_linkage.generated.json",
)

OUT_RESULT = "tier5_enforcement_gate_result.json"
OUT_REPORT = "tier5_enforcement_gate_report.md"

GATE_NAME = "tier5_enforcement_readiness"
EXPECTED_TIER5_TOTAL = 25
...
def evaluate() -> Dict[str, Any]:
    ...
    if missing_sources:
        return {
            "gate_name": GATE_NAME,
            "result": "BLOCKED",
            "tier5_total": EXPECTED_TIER5_TOTAL,
            "tier5_seen": 0,
            "blocked_count": EXPECTED_TIER5_TOTAL,
            "blocker_counts": {},
    ...
    if len(seen_req_ids) != EXPECTED_TIER5_TOTAL:
        for rid in seen_req_ids - blocking_req_ids:
            blocking_req_ids.add(rid)

    result_status = (
        "READY"
        if (len(seen_req_ids) == EXPECTED_TIER5_TOTAL and not blocking_req_ids)
        else "BLOCKED"
    )
...
def _build_blocking_reasons(
    ...
    reasons: List[str] = []
    if seen_count != EXPECTED_TIER5_TOTAL:
        reasons.append(
            f"Tier 5 row count mismatch: seen={seen_count} expected={EXPECTED_TIER5_TOTAL}"
        )
...
def write_report(result: Mapping[str, Any]) -> Path:
    lines: List[str] = []
    lines.append("# Tier 5 Enforcement Readiness Gate Report")
    lines.append("")
    lines.append(f"- Gate: `{result['gate_name']}`")
    lines.append(f"- Result: **{result['result']}**")
    lines.append(f"- Evaluated at: {result['evaluated_at_utc']}")
    lines.append(f"- Tier 5 seen / expected: {result['tier5_seen']} / {result['tier5_total']}")
