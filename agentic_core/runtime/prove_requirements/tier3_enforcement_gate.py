"""Tier 3 enforcement readiness gate.

Aggregates Tier 3 metadata blockers and linkage statuses across the four
generated surface files. Returns READY only when all 25 selected Tier 3
...
OUT_RESULT = "tier3_enforcement_gate_result.json"
OUT_REPORT = "tier3_enforcement_gate_report.md"

GATE_NAME = "tier3_enforcement_readiness"
EXPECTED_TIER3_TOTAL = 25
...
def evaluate() -> Dict[str, Any]:
    ...
    if missing_sources:
        return {
            "gate_name": GATE_NAME,
            "result": "BLOCKED",
            "tier3_total": EXPECTED_TIER3_TOTAL,
            "tier3_seen": 0,
            "blocked_count": EXPECTED_TIER3_TOTAL,
            "blocker_counts": {},
    ...
    if len(seen_req_ids) != EXPECTED_TIER3_TOTAL:
        for rid in seen_req_ids - blocking_req_ids:
            blocking_req_ids.add(rid)

    result_status = (
        "READY"
        if (len(seen_req_ids) == EXPECTED_TIER3_TOTAL and not blocking_req_ids)
        else "BLOCKED"
    )
...
def _build_blocking_reasons(
    ...
    reasons: List[str] = []
    if seen_count != EXPECTED_TIER3_TOTAL:
        reasons.append(
            f"Tier 3 row count mismatch: seen={seen_count} expected={EXPECTED_TIER3_TOTAL}"
        )
...
def write_report(result: Mapping[str, Any]) -> Path:
    lines: List[str] = []
    lines.append("# Tier 3 Enforcement Readiness Gate Report")
    lines.append("")
    lines.append(f"- Gate: `{result['gate_name']}`")
    lines.append(f"- Result: **{result['result']}**")
    lines.append(f"- Evaluated at: {result['evaluated_at_utc']}")
    lines.append(f"- Tier 3 seen / expected: {result['tier3_seen']} / {result['tier3_total']}")
