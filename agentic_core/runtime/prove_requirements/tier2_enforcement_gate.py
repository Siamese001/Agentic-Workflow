"""Tier 2 enforcement readiness gate.

Aggregates Tier 2 metadata blockers and linkage statuses across the four
generated surface files. Returns READY only when all 22 selected Tier 2
...
OUT_RESULT = "tier2_enforcement_gate_result.json"
OUT_REPORT = "tier2_enforcement_gate_report.md"

GATE_NAME = "tier2_enforcement_readiness"
EXPECTED_TIER2_TOTAL = 22
...
def evaluate() -> Dict[str, Any]:
    ...
    if missing_sources:
        return {
            "gate_name": GATE_NAME,
            "result": "BLOCKED",
            "tier2_total": EXPECTED_TIER2_TOTAL,
            "tier2_seen": 0,
            "blocked_count": EXPECTED_TIER2_TOTAL,
            "blocker_counts": {},
    ...
    if len(seen_req_ids) != EXPECTED_TIER2_TOTAL:
        for rid in seen_req_ids - blocking_req_ids:
            blocking_req_ids.add(rid)

    result_status = (
        "READY"
        if (len(seen_req_ids) == EXPECTED_TIER2_TOTAL and not blocking_req_ids)
        else "BLOCKED"
    )
...
def _build_blocking_reasons(
    ...
    reasons: List[str] = []
    if seen_count != EXPECTED_TIER2_TOTAL:
        reasons.append(
            f"Tier 2 row count mismatch: seen={seen_count} expected={EXPECTED_TIER2_TOTAL}"
        )
...
def write_report(result: Mapping[str, Any]) -> Path:
    lines: List[str] = []
    lines.append("# Tier 2 Enforcement Readiness Gate Report")
    lines.append("")
    lines.append(f"- Gate: `{result['gate_name']}`")
    lines.append(f"- Result: **{result['result']}**")
    lines.append(f"- Evaluated at: {result['evaluated_at_utc']}")
    lines.append(f"- Tier 2 seen / expected: {result['tier2_seen']} / {result['tier2_total']}")
