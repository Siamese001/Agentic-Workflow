"""Tier 5 runtime-proof gate.

Validates targeted static evidence consistency for the 25 Tier 5 REQ_IDs
...
OUT_RESULT = "tier5_runtime_proof_gate_result.json"
OUT_REPORT = "tier5_runtime_proof_gate_report.md"

GATE_NAME = "tier5_runtime_proof_gate"

# 18 check classes per REQ_ID (per spec): step1 exists, tier=TIER5,
...
def _row_iter(payload: Any) -> Sequence[Mapping[str, Any]]:
    if isinstance(payload, dict):
        for key in ("rows", "tier5_rows", "items", "entries"):
...
def _load_tier5_req_ids() -> Tuple[str, ...]:
...
def _check_row(
    ...
    """Run the 18-class checks for one Tier 5 REQ_ID."""
    failures: List[str] = []
    ...
    if not req_id or req_id not in canonical:
        failures.append(f"{req_id}: not in canonical Tier 5 REQ_IDs")
    ...
    for surface_name, row in rows_per_surface.items():
        ...
        if tier_val and tier_val != "TIER5":
            failures.append(f"{req_id} [{surface_name}]: tier={tier_val!r} (expected TIER5)")
...
def evaluate(
    ...
    """Validate static-evidence consistency for the 25 Tier 5 REQ_IDs."""
    tier5_req_ids = _load_tier5_req_ids()
    canonical = set(tier5_req_ids)
    ...
    for fname in SOURCE_FILES:
        ...
        if not path.is_file():
            return {
                "gate_name": GATE_NAME,
                "result": "BLOCKED",
                "tier5_total": len(tier5_req_ids),
                "failed_count": len(tier5_req_ids),
                "failed_req_ids": list(tier5_req_ids),
                "checks_performed": 0,
                "artifacts_checked": [],
