"""Tier 4 runtime-proof gate.

Validates targeted static evidence consistency for the 25 Tier 4 REQ_IDs
across the four generated metadata surfaces, the on-disk
code/validator/test/artifact/replay/negative-control references they
declare, and the JSON content of any artifact/replay payloads that carry
``step1_req_id`` or ``expected_fail_reason``.
...
SELECTION_PATH = (
    REPO_ROOT / "docs" / "reference" / "contracts" / "tier4" / "TIER4_SELECTION.json"
)
...
OUT_RESULT = "tier4_runtime_proof_gate_result.json"
OUT_REPORT = "tier4_runtime_proof_gate_report.md"

GATE_NAME = "tier4_runtime_proof_gate"

# 18 check classes per REQ_ID (per spec): step1 exists, tier=TIER4,
# linkage_status=LINKED_LITERAL, blockers empty, expected_fail_reason
# present, ref paths exist on disk (code/validator/otel/test/artifact/
# replay/negative_control), otel_span_refs non-empty, artifact step1
# match, artifact efr match, replay step1 match, replay efr match,
# replay invariant_digest pair stable, negative-control efr match.
_CHECKS_PER_ROW = 18
...
def _row_iter(payload: Any) -> Sequence[Mapping[str, Any]]:
    if isinstance(payload, dict):
        for key in ("rows", "tier4_rows", "items", "entries"):
...
def _load_tier4_req_ids() -> Tuple[str, ...]:
...
def _check_row(
    ...
    """Run the 18-class checks for one Tier 4 REQ_ID."""
    failures: List[str] = []
    ...
    if not req_id or req_id not in canonical:
        failures.append(f"{req_id}: not in canonical Tier 4 REQ_IDs")
    ...
    for surface_name, row in rows_per_surface.items():
        ...
        if tier_val and tier_val != "TIER4":
            failures.append(
                f"{req_id} [{surface_name}]: tier={tier_val!r} (expected TIER4)"
            )
        ...
        if row.get("blockers"):
            failures.append(
                f"{req_id} [{surface_name}]: non-empty blockers={row.get('blockers')}"
            )
    ...
    for stem, members in pair_groups.items():
        ...
        if len(set(digests)) != 1:
            failures.append(
                f"{req_id}: replay pair {stem} invariant_digest mismatch: {digests}"
            )

    # Negative-control artifact refs (when JSON) must carry matching efr.
    ...
    return (
        ...
        len(replay_payloads),
        pairs_checked,
        len(primary.get("negative_control_refs") or []),
    )
...
def evaluate(
    ...
    """Validate static-evidence consistency for the 25 Tier 4 REQ_IDs."""
    tier4_req_ids = _load_tier4_req_ids()
    canonical = set(tier4_req_ids)
    ...
    for fname in SOURCE_FILES:
        ...
        if not path.is_file():
            return {
                "gate_name": GATE_NAME,
                "result": "BLOCKED",
                "tier4_total": len(tier4_req_ids),
                "failed_count": len(tier4_req_ids),
                "failed_req_ids": list(tier4_req_ids),
                "checks_performed": 0,
                "artifacts_checked": [],
    ...
    for req_id in tier4_req_ids:
        per_surface = rows_by_req.get(req_id, {})
        failures, _na, _np, _nn = _check_row(
            req_id,
            per_surface,
            canonical,
            artifacts_checked,
        ...
        checks_performed += _CHECKS_PER_ROW
        if failures:
