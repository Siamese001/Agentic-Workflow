"""Tier 2 runtime-proof gate.

Validates targeted static evidence consistency for the 22 Tier 2 REQ_IDs
...
SELECTION_PATH = (
    REPO_ROOT / "docs" / "reference" / "contracts" / "tier2" / "TIER2_SELECTION.json"
)
...
OUT_RESULT = "tier2_runtime_proof_gate_result.json"
OUT_REPORT = "tier2_runtime_proof_gate_report.md"

GATE_NAME = "tier2_runtime_proof_gate"

# 18 check classes per REQ_ID (spec): 1 step1 exists, 2 tier=TIER2,
# 3 linkage_status=LINKED_LITERAL, 4 blockers empty, 5 expected_fail_reason
# present, 6-12 ref paths exist on disk (code/validator/otel/test/artifact/
# replay/negative_control), 13 otel_span_refs non-empty, 14 artifact step1
# match, 15 artifact efr match, 16 replay step1 match, 17 replay efr match,
# 18 replay invariant_digest pair stable + negative-control efr (combined).
_CHECKS_PER_ROW = 18
...
def _row_iter(payload: Any) -> Sequence[Mapping[str, Any]]:
    if isinstance(payload, dict):
        for key in ("rows", "tier2_rows", "items", "entries"):
...
def _load_tier2_req_ids() -> Tuple[str, ...]:
...
def _check_row(
    ...
    """Run the 18-class checks for one Tier 2 REQ_ID."""
    failures: List[str] = []
    ...
    if not req_id or req_id not in canonical:
        failures.append(f"{req_id}: not in canonical Tier 2 REQ_IDs")
    ...
    for surface_name, row in rows_per_surface.items():
        ...
        if tier_val and tier_val != "TIER2":
            failures.append(
                f"{req_id} [{surface_name}]: tier={tier_val!r} (expected TIER2)"
            )
...
def evaluate(
    ...
    """Validate static-evidence consistency for the 22 Tier 2 REQ_IDs."""
    tier2_req_ids = _load_tier2_req_ids()
    canonical = set(tier2_req_ids)
    ...
    for fname in SOURCE_FILES:
        ...
        if not path.is_file():
            return {
                "gate_name": GATE_NAME,
                "result": "BLOCKED",
                "tier2_total": len(tier2_req_ids),
                "failed_count": len(tier2_req_ids),
                "failed_req_ids": list(tier2_req_ids),
                "checks_performed": 0,
                "artifacts_checked": [],
