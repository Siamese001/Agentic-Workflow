"""Tier 4 enforcement readiness gate.

Aggregates Tier 4 metadata blockers and linkage statuses across the four
generated surface files. Returns READY only when all 25 selected Tier 4
...
SOURCE_FILES = (
    "tier4_requirements_index.generated.json",
    "tier4_coverage_matrix.generated.json",
    "tier4_implementation_map.generated.json",
    "tier4_artifact_linkage.generated.json",
)

OUT_RESULT = "tier4_enforcement_gate_result.json"
OUT_REPORT = "tier4_enforcement_gate_report.md"

GATE_NAME = "tier4_enforcement_readiness"
EXPECTED_TIER4_TOTAL = 25
...
def evaluate() -> Dict[str, Any]:
    ...
    if missing_sources:
        return {
            "gate_name": GATE_NAME,
            "result": "BLOCKED",
            "tier4_total": EXPECTED_TIER4_TOTAL,
            "tier4_seen": 0,
            "blocked_count": EXPECTED_TIER4_TOTAL,
            "blocker_counts": {},
            "blocking_status_counts": {},
            "linkage_status_counts": {},
            "blocking_reasons": [
                f"Missing required source file: {fname}" for fname in missing_sources
            ],
    ...
    for rid in seen_req_ids:
        ...
        is_blocked = bool(rid_blockers) or any(
            s not in ACCEPTABLE_FOR_READY for s in rid_statuses
        )
        if is_blocked:
    ...
    if len(seen_req_ids) != EXPECTED_TIER4_TOTAL:
        for rid in seen_req_ids - blocking_req_ids:
            blocking_req_ids.add(rid)

    result_status = (
        "READY"
        if (len(seen_req_ids) == EXPECTED_TIER4_TOTAL and not blocking_req_ids)
        else "BLOCKED"
    )
...
def _build_blocking_reasons(
    ...
    reasons: List[str] = []
    if seen_count != EXPECTED_TIER4_TOTAL:
        reasons.append(
            f"Tier 4 row count mismatch: seen={seen_count} expected={EXPECTED_TIER4_TOTAL}"
        )
...
def write_report(result: Mapping[str, Any]) -> Path:
    lines: List[str] = []
    lines.append("# Tier 4 Enforcement Readiness Gate Report")
    lines.append("")
    lines.append(f"- Gate: `{result['gate_name']}`")
    lines.append(f"- Result: **{result['result']}**")
    lines.append(f"- Evaluated at: {result['evaluated_at_utc']}")
    lines.append(f"- Tier 4 seen / expected: {result['tier4_seen']} / {result['tier4_total']}")
