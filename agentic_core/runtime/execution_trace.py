...
class ExecutionTraceManager:
    ...
    def start_trace(self, plan_hash: str, policy_hash: str, hierarchy_hash: str, metadata: dict[str, Any] | None=None) -> str:
        """Start a new execution trace and return its trace_id."""
        with self._lock:
            trace_id = str(uuid.uuid4())
            self._active_trace = ExecutionTrace(trace_id=trace_id, plan_hash=plan_hash, policy_hash=policy_hash, determinism_digest='', hierarchy_hash=hierarchy_hash, metadata=metadata or {})
            return trace_id
    ...
    def bind_determinism_digest(self, determinism_digest: str) -> None:
        ...
        with self._lock:
            ...
            self._active_trace = ExecutionTrace(trace_id=self._active_trace.trace_id, plan_hash=self._active_trace.plan_hash, policy_hash=self._active_trace.policy_hash, determinism_digest=determinism_digest, hierarchy_hash=self._active_trace.hierarchy_hash, metadata=self._active_trace.metadata)
...
def start_execution_trace(plan_hash: str, policy_hash: str, hierarchy_hash: str, metadata: dict[str, Any] | None=None) -> str:
    """Start a new execution trace and return its trace_id."""
