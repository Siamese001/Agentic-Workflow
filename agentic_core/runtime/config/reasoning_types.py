...
class ModelConfig(BaseModel):
    ...
    model_name: str = Field(default_factory=lambda: os.getenv('OPENAI_MODEL', 'gpt-4o'), description='LLM model name')
    temperature: float = Field(default_factory=lambda: float(os.getenv('OPENAI_TEMPERATURE', '0.7')), ge=0.0, le=2.0)
    max_tokens: int = Field(default_factory=lambda: int(os.getenv('OPENAI_MAX_TOKENS', '2000')), ge=1, le=32000)
...
class RAGConfig(BaseModel):
    ...
    similarity_threshold: float = Field(default_factory=lambda: float(os.getenv('RAG_SIMILARITY_THRESHOLD', '0.8')), ge=0.0, le=1.0)
...
class GovernorConfig(BaseModel):
    ...
    safety_threshold: float = Field(default_factory=lambda: float(os.getenv('GOVERNOR_SAFETY_THRESHOLD', '0.95')), ge=0.0, le=1.0)
    content_filter_enabled: bool = True
    pii_detection_enabled: bool = True
    bias_detection_enabled: bool = True
    audit_logging_enabled: bool = True
    max_requests_per_minute: int = Field(default=100, ge=1, le=10000)
    allowed_models: list[str] = Field(default_factory=lambda: [os.getenv('OPENAI_MODEL', 'gpt-4o'), 'gpt-4o-mini', os.getenv('ANTHROPIC_MODEL', 'claude-3-5-sonnet-20241022')])
