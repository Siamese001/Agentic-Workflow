...
async def run(self: Any, context: Dict) -> Dict[str, Any]:
    ...
    try:
        ...
        hardened_system, wrapped_input, canary = await self._setup_injection_defense(
            context.get("system_prompt", ""), str(redacted_context)
        )

        # === STAGE 3: THINK (L1 Cognition) ===
        plan, think_cost = await self._execute_think_stage(hardened_system, wrapped_input, trace_id)
        ...
        validated_results = await self._validate_constitutionally(
            results, context, canary, trace_id
        )
...
async def _protect_privacy(self: Any, context: Dict) -> Tuple[Dict, Dict]:
    ...
    # Redact PII
    redacted_text, self.pii_session_id = self.pii_vault.redact(context_text, self.session_id)
...
async def _setup_injection_defense(
    self: Any, system_prompt: str, user_input: str
    ...
    hardened_system, _ = self.canary_defense.inject_canary(system_prompt, self.active_canary)
...
async def _execute_think_stage(
...
async def _validate_constitutionally(
    self: Any, results: List[Any], context: Dict, canary: CanaryToken, trace_id: str
) -> str:
    ...
    is_valid, validation_result = await self.overseer.verify_with_canary(
        output_text, canary.token, context=str(context), agent_type=self.agent_type
    )
