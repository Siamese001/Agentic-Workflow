...
class MathematicalDeterminismEngine:
    ...
    def add_artifact(self, name: str, hash_value: str, metadata: dict[str, Any] | None=None) -> None:
        ...
        with self._lock:
            ...
            self._artifacts[name] = DeterministicArtifact(name=name, hash_value=hash_value, metadata=metadata or {})
    ...
    def seal(self) -> DeterminismProof:
        ...
        with self._lock:
            ...
            core_payload: dict[str, Any] = {'artifacts': {name: {'hash': artifact.hash_value, 'metadata': artifact.metadata} for name, artifact in sorted_artifacts.items()}, 'policy_hash': self._policy_hash, 'hierarchy_hash': self._hierarchy_hash, 'authority_hash': self._authority_hash}
            core_json = json.dumps(core_payload, sort_keys=True, separators=(',', ':'))
            core_digest = hashlib.sha256(core_json.encode('utf-8')).hexdigest()
            import time
            self._proof = DeterminismProof(core_digest=core_digest, run_id=self._run_id, creation_timestamp=time.time(), artifact_count=len(self._artifacts), policy_hash=self._policy_hash, hierarchy_hash=self._hierarchy_hash, authority_hash=self._authority_hash)
            self._sealed = True
            return self._proof
    ...
    def verify_replay(self, expected_proof: DeterminismProof) -> bool:
        ...
        assert self._proof is not None
        return self._proof.core_digest == expected_proof.core_digest and self._proof.policy_hash == expected_proof.policy_hash and (self._proof.hierarchy_hash == expected_proof.hierarchy_hash) and (self._proof.authority_hash == expected_proof.authority_hash) and (self._proof.artifact_count == expected_proof.artifact_count)
