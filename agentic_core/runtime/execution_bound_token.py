...
class SecureCapabilityAuthority:
    ...
    def _load_authority_secret() -> str:
        ...
        if not secret:
            raise RuntimeError(
                "AGENTIC_AUTHORITY_SECRET environment variable is required "
                "but not set. Cannot initialize SecureCapabilityAuthority."
            )
        return secret
    ...
    def issue_token(
        ...
        raw_signature_payload = (
            f"{token_id}:{capability_type.value}:{caller_context}:"
            f"{target_context}:{execution_trace_id}:{policy_hash}:"
            f"{determinism_digest}:{hierarchy_hash}"
        )
    ...
    def verify_token(self, token: ExecutionBoundToken) -> bool:
        ...
        raw_signature_payload = (
            f"{token.token_id}:{token.capability_type.value}:{token.caller_context}:"
            f"{token.target_context}:{token.execution_trace_id}:{token.policy_hash}:"
            f"{token.determinism_digest}:{token.hierarchy_hash}"
        )
