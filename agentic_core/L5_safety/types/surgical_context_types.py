...
class SurgicalContext:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'file_path': str(self.file_path), 'violation_id': self.violation_id, 'violations': [v.__dict__ for v in self.violations], 'target_coordinates': [c.__dict__ for c in self.target_coordinates], 'detector_agent': self.detector_agent, 'detection_method': self.detection_method, 'detection_timestamp': self.detection_timestamp, 'surrounding_context': self.surrounding_context, 'related_violations': self.related_violations, 'suggested_fixes': self.suggested_fixes, 'preservation_rules': self.preservation_rules}
    ...
    def from_dict(cls, data: dict[str, Any]) -> SurgicalContext:
        ...
        return cls(file_path=Path(data['file_path']), file_content=data['file_content'], ast_tree=tree, violation_id=data['violation_id'], violations=violations, target_coordinates=coordinates, detector_agent=data['detector_agent'], detection_method=data['detection_method'], detection_timestamp=data['detection_timestamp'], surrounding_context=data.get('surrounding_context', {}), related_violations=data.get('related_violations', []), suggested_fixes=data.get('suggested_fixes', []), preservation_rules=data.get('preservation_rules', []))
...
class SurgicalContextBuilder:
    ...
    def build_context(self, violation_id: str, violations: list[dict[str, Any]], target_nodes: list[ast.AST], **kwargs) -> SurgicalContext:
        ...
        for i, node in enumerate(target_nodes):
            coord = ASTCoordinate(node_id=f'{self.detection_method}_{i}_{node.lineno}_{node.col_offset}', node_type=type(node).__name__, line=node.lineno, column=node.col_offset, end_line=getattr(node, 'end_lineno', None), end_column=getattr(node, 'end_col_offset', None))
            coordinates.append(coord)
        return SurgicalContext(file_path=self.file_path, file_content=self.file_content, ast_tree=self.ast_tree, violation_id=violation_id, violations=violation_constraints, target_coordinates=coordinates, detector_agent=self.detector_agent, detection_method=self.detection_method, detection_timestamp=datetime.now().isoformat(), **kwargs)
