...
class ConstitutionalGovernanceGuardrail:
    ...
    def __init__(self):
        ...
        self.principle_checks = {ConstitutionalPrinciple.HARMLESSNESS: {'negative_patterns': ['harm', 'hurt', 'damage', 'destroy', 'kill', 'attack', 'exploit', 'abuse'], 'weight': 1.0}, ConstitutionalPrinciple.HONESTY: {'negative_patterns': ['lie', 'deceive', 'mislead', 'fake', 'pretend to be'], 'weight': 0.9}, ConstitutionalPrinciple.PRIVACY: {'negative_patterns': ['personal data', 'private information', 'ssn', 'password'], 'weight': 0.8}, ConstitutionalPrinciple.FAIRNESS: {'negative_patterns': ['discriminate', 'bias against', 'unfair'], 'weight': 0.8}}
    ...
    async def review(self, content: str, context: dict[str, Any] | None=None) -> GovernanceResult:
        ...
        return GovernanceResult(compliant=len(violations) == 0, violations=violations, audit_id=audit_id, review_notes=self._generate_notes(violations))
        ...
        for principle, config in self.principle_checks.items():
            for pattern in config['negative_patterns']:
                if pattern in content_lower:
                    violations.append(PrincipleViolation(principle=principle, severity='moderate', description=f"Potential violation of {principle.value}: contains '{pattern}'", suggested_revision=f"Consider removing or rephrasing content containing '{pattern}'"))
    ...
    def _check_governance(self, content: str, context: dict[str, Any] | None) -> list[PrincipleViolation]:
        ...
        if len(content) > 10000:
            violations.append(PrincipleViolation(principle=ConstitutionalPrinciple.TRANSPARENCY, severity='minor', description='Content exceeds governance length limit'))
        return violations
    ...
    def _create_audit(self, content: str, violations: list[PrincipleViolation]) -> str:
        ...
        self.audit_log.append({'audit_id': audit_id, 'timestamp': time.time(), 'content_length': len(content), 'violation_count': len(violations), 'violations': [{'principle': v.principle.value, 'severity': v.severity, 'description': v.description} for v in violations]})
        return audit_id
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'reviews_performed': self.reviews_performed, 'violations_found': self.violations_found, 'revisions_suggested': self.revisions_suggested, 'audit_log_size': len(self.audit_log), 'compliance_rate': (self.reviews_performed - self.violations_found) / self.reviews_performed * 100 if self.reviews_performed > 0 else 100}
