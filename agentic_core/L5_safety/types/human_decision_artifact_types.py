...
class HumanDecisionArtifact:
    ...
    def _signable_dict(self) -> dict:
        return {'action': self.action, 'original_plan_hash': self.original_plan_hash, 'policy_hash': self.policy_hash, 'reviewer_id': self.reviewer_id, 'trace_id': self.trace_id}

    def sign(self, secret: bytes) -> HumanDecisionArtifact:
        mac = hmac.new(secret, json.dumps(self._signable_dict(), sort_keys=True, separators=(',', ':')).encode('ascii'), hashlib.sha256)
        return HumanDecisionArtifact(trace_id=self.trace_id, policy_hash=self.policy_hash, reviewer_id=self.reviewer_id, action=self.action, original_plan_hash=self.original_plan_hash, structured_patch_schema=self.structured_patch_schema, reviewer_sig=mac.hexdigest().lower())
    ...
    def assert_plan_hash_matches(self, submitted_plan_hash: str) -> None:
        ...
        if self.original_plan_hash != submitted_plan_hash:
            raise HumanDecisionViolation(f'original_plan_hash mismatch: artifact={self.original_plan_hash[:12]} submitted={submitted_plan_hash[:12]}')
