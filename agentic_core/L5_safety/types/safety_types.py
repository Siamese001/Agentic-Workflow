...
class SelfUpdatingSafetyEngine:
    ...
    async def detect_threats(self, text: str, context: dict[str, Any] | None=None) -> ThreatDetection:
        ...
        confidence = 0.0
        if matched_rules:
            confidence = sum((1.0 if not rule.auto_generated else 0.8 for rule in matched_rules)) / len(matched_rules)
        recommendations = self._generate_recommendations(matched_rules)
        detection = ThreatDetection(detected=len(matched_rules) > 0, ThreatLevel=max_threat_level, matched_rules=matched_rules, confidence=confidence, recommendations=recommendations)
