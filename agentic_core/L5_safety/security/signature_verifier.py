...
class SignatureVerifier:
    ...
    def __init__(self):
        self._trusted_signers: dict[str, str] = {'system': 'system_signer_hash', 'agent': 'agent_signer_hash', 'gateway': 'gateway_signer_hash'}
    ...
    def verify_instruction_packet(self, packet: InstructionPacket) -> VerificationContext:
        ...
        if packet.signer_id not in self._trusted_signers:
            raise SignatureVerificationError(f'INSTRUCTION_PACKET_UNTRUSTED_SIGNER: signer_id={packet.signer_id}')
        ...
        if packet.signature != expected_signature:
            raise SignatureVerificationError(f'INSTRUCTION_PACKET_INVALID_SIGNATURE: expected={expected_signature[:16]}..., provided={packet.signature[:16]}...')
        return VerificationContext(is_verified=True, signature_hash=packet.signature, signer_id=packet.signer_id, packet_hash=packet_hash)
    ...
    def verify_sandbox_envelope(self, envelope: SandboxEnvelope) -> VerificationContext:
        ...
        if envelope.envelope_signature != expected_envelope_sig:
            raise SignatureVerificationError(f'SANDBOX_ENVELOPE_INVALID_SIGNATURE: expected={expected_envelope_sig[:16]}..., provided={envelope.envelope_signature[:16]}...')
        return VerificationContext(is_verified=True, signature_hash=envelope.envelope_signature, signer_id='system', packet_hash=envelope_hash)
