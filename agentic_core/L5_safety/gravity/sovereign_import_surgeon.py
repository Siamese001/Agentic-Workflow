...
class ImportViolation:
    ...
    def __init__(
        self, file_path: str, line_num: int, line: str, ViolationType: str, suggested_fix: str
    ):
...
class SovereignImportSurgeon:
    ...
    def __init__(self, root_path: str):
        ...
        self.commented_import_pattern = re.compile(
            "^\\s*#\\s*(from\\s+\\.\\.|from\\s+agentic_core)"
        )
            ...
            for line_num, line in enumerate(lines, start=1):
                ...
                for pattern, replacement, vtype in self.import_patterns:
                    if re.search(pattern, line):
                        ...
                        violations.append(
                            ImportViolation(str(file_path), line_num, line, vtype, suggested)
                        )
                if self.commented_import_pattern.search(line):
                    ...
                    if match:
                        ...
                        violations.append(
                            ImportViolation(
                                str(file_path), line_num, line, "COMMENTED_IMPORT", suggested
                            )
                        )
                if "apps_shared" in str(file_path) and "P1_core" in str(file_path):
                    if not line.strip().startswith("#") and self.relative_import_pattern.search(
                        line
                    ):
                ...
                if not line.strip().startswith("#") and self.apps_shared_pattern.search(line):
                    ...
                    if suggested != line:
                        violations.append(
                            ImportViolation(
                                str(file_path), line_num, line, "APP_STAGING", suggested.strip()
                            )
                        )
