...
class PytestEnforcementGuard:
                ...
                if self._is_getoption_call(child):
                    if child.args and isinstance(child.args[0], ast.Constant):
                        if child.args[0].value == '-m':
                            self.errors.append(f'{conftest}: Brittle marker access detected: config.getoption("-m") should be replaced with getattr(config.option, "markexpr", "")')
    ...
    def _get_used_test_markers(self) -> set[str]:
        ...
        markers = set()
        builtin_markers = {'skipif', 'filterwarnings', 'usefixtures', 'skip', 'parametrize', 'xfail', 'fixture', 'yield_fixture', 'tryfirst', 'trylast'}
