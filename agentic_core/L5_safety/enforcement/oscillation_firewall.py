...
class OscillationFirewall:
    ...
    def assert_no_oscillation(self, tier: str, cycle: int) -> None:
        ...
        try:
            ...
            raise OscillationFirewallTripped(
                f"OscillationFirewall: tier {tier!r} is oscillating at cycle "
                f"{cycle}. Routing frozen.\nDetector: {exc}"
            ) from exc
    ...
    def get_frozen_tiers(self, cycle: int) -> set[str]:
        ...
        return {
            tier
            for tier in self._tier_histories
            if self._detector.is_frozen(tier, cycle)
        }
...
# ---------------------------------------------------------------------------
# Standalone threshold validator (stateless, for invariant tests)
# ---------------------------------------------------------------------------
...
def validate_threshold(
    ...
    # Check for A-B-A pattern in the window.
