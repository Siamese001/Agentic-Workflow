...
from agentic_core.L0_routing.config.path_constants import AGENT_DISCOVERY_JSON, AGENTIC_CORE_DIR, APPS_LIC_DIR, APPS_RG_DIR, APPS_SHARED_DIR, TESTS_DIR
...
EXCLUDED_DIRS: Final[frozenset[str]] = GLOBAL_EXCLUDED_DIRS | SOVEREIGN_EXCLUDED_FOLDERS | DISCOVERY_EXCLUDED_TERRITORIES
    ...
    def _parse_agent_file(self, file_path: Path) -> AgentInfo | None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef) and node.name.endswith('Agent'):
                ...
                return AgentInfo(class_name=node.name, file_path=file_path, relative_path=relative_path, layer=self._extract_layer(relative_path), has_agent_class=True, inheritance=bases, key_methods=methods[:10])
        return None
    ...
    def verify_registry(self) -> VerificationResult:
        ...
        for reg_agent in registry_agents:
            ...
            if class_name not in fs_by_class:
                result.orphan_agents.append({'class_name': class_name, 'registry_path': reg_path, 'reason': 'Class not found in filesystem'})
            elif reg_path not in fs_by_path:
                actual_agent = fs_by_class[class_name]
                result.path_mismatches.append({'class_name': class_name, 'registry_path': reg_path, 'actual_path': actual_agent.relative_path.replace('\\', '/'), 'reason': 'Path mismatch between registry and filesystem'})
        ...
        result.is_complete = len(result.orphan_agents) == 0 and len(result.missing_agents) == 0 and (len(result.path_mismatches) == 0)
        return result

    def generate_report(self, result: VerificationResult) -> str:
        """Generate markdown report from verification result."""
        lines = ['# Phase 1: Registry Verification Report', '', '## Summary', '', f'- **Total Filesystem Agents:** {result.total_filesystem_agents}', f'- **Total Registry Agents:** {result.total_registry_agents}', f'- **Valid Agents:** {len(result.valid_agents)}', f'- **Missing from Registry:** {len(result.missing_agents)}', f'- **Orphan Agents:** {len(result.orphan_agents)}', f'- **Path Mismatches:** {len(result.path_mismatches)}', f'- **Coverage:** {result.coverage_percentage:.1f}%', f"- **Status:** {('PASS' if result.is_complete else 'FAIL')}", '']
        if result.orphan_agents:
            lines.extend(['## Orphan Agents (In Registry, Not in Filesystem)', '', '| Class Name | Registry Path | Reason |', '|------------|---------------|--------|'])
        ...
        if result.path_mismatches:
            lines.extend(['## Path Mismatches', '', '| Class Name | Registry Path | Actual Path |', '|------------|---------------|-------------|'])
        ...
        if result.missing_agents:
            lines.extend(['## Missing from Registry (In Filesystem, Not in Registry)', '', '| Class Name | File Path | Layer |', '|------------|-----------|-------|'])
