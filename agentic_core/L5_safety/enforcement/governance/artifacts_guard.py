...
def scan_sensitive_content(file_path: Path) -> list[str]:
    ...
    sensitive_patterns = ['(?i)api[_-]?key\\s*[:=]', '(?i)secret\\s*[:=]', 'sk-[A-Za-z0-9]{20,}', 'xox[baprs]-[A-Za-z0-9-]{10,}']
    violations = []
    try:
...
def scan_artifacts_directory(artifacts_path: Path) -> dict[str, Any]:
    ...
    for file_path in all_files:
        ...
        if is_forbidden_artifact_name(relative_path):
            violations.append({'file': str(relative_path), 'type': 'forbidden_artifact_name', 'detail': f'Forbidden artifact name: {relative_path}'})
