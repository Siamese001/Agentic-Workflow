...
class FastDashboardE2EPipeline:
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if stripped and (not stripped.startswith('#')) and (not stripped.startswith('"""')) and (not stripped.startswith("'''")):
                        insert_index = i
                        break
                lines.insert(insert_index, '        super(, **kwargs).heal_repository(, **kwargs)\n', **kwargs)
                new_method_body = '\n'.join(lines)
            ...
            try:
                ...
                if 'from agentic_core' in content and 'import' in content and (agent['class_name'] in content):
                    if content.count(f"class {agent['class_name']}") == 0:
                        continue
                class_pattern = f"class\\s+{re.escape(agent['class_name'])}\\s*\\((.*?)\\)\\s*:"
                ...
                if 'from agentic_core.mixins.mcp_hardened_mixin import MCPHardenedMixin' not in content:
                    ...
                    for i, line in enumerate(lines):
                        if line.strip().startswith('import ') or line.strip().startswith('from '):
                            insert_idx = i + 1
                    lines.insert(insert_idx, 'from agentic_core.mixins.mcp_hardened_mixin import MCPHardenedMixin')
                    content = '\n'.join(lines)
    ...
    def step0_validate_data(self) -> bool:
        ...
        if not validation_script.exists() or not base_agent_script.exists():
            ...
            return True
        try:
            result = safe_execute([sys.executable, str(base_agent_script)], cwd=str(self.project_root), capture_output=True, check=False, text=True, timeout=DEFAULT_TIMEOUT)
            if result.returncode != 0:
                ...
                return False
            result = safe_execute([sys.executable, str(validation_script)], cwd=str(self.project_root), capture_output=True, check=False, text=True, timeout=DEFAULT_TIMEOUT)
            if result.returncode != 0:
