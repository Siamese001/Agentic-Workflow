...
class HealingStrategy:
    ...
    def __init__(self, project_root: Path | None=None, target_tier: int | None=None) -> None:
        ...
        self._tiers: dict[str, list[str]] = {'Tier 0: Pre-Flight': CORE_HYGIENE_AGENTS['tier_0_preflight'], 'Tier 1: Structural': ['TwoPhaseDeduplicationAgent_PhaseA'] + CORE_HYGIENE_AGENTS['tier_1_structural'], 'Tier 2: Architectural': ['StructuralHealerAgent'] + CORE_HYGIENE_AGENTS['tier_2_architectural'] + ['TwoPhaseDeduplicationAgent_PhaseB'], 'Tier 3: Dynamic': ['CodeEnforcerAgent'] + CORE_HYGIENE_AGENTS['tier_3_autonomy'], 'Tier 4: Final Gate': []}
    ...
    def _get_dedup_agent(self) -> Any | None:
        ...
        if self._dedup_agent is None:
            try:
                from agentic_core.L5_safety.enforcement.TwoPhaseDeduplicationAgent import TwoPhaseDeduplicationAgent
    ...
    def execute_agent(self, agent: Any, agent_name: str, dry_run: bool=True, execute: bool=False, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            if agent_name == 'TwoPhaseDeduplicationAgent_PhaseA':
                ...
                return {'status': 'ERROR', 'violations_found': 0, 'violations_fixed': 0, 'error_message': f'Agent {agent_name} missing heal_repository()'}
    ...
    def _normalize_result(self, result: dict[str, Any], execution_time_ms: float) -> dict[str, Any]:
        ...
        """
        violations_found = result.get('violations_found') or result.get('violations') or result.get('errors') or 0
        ...
        if not status:
            if result.get('error_message'):
                ...
                status = 'FAIL'
        return {'status': status, 'violations_found': violations_found, 'violations_fixed': violations_fixed, 'execution_time_ms': execution_time_ms, 'error_message': result.get('error_message'), 'raw_result': result}
