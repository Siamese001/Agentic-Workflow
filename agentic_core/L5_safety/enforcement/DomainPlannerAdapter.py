...
class DomainPlannerAdapter:
    ...
    def _validate_input(self, context: AdapterContext, *args, **kwargs) -> bool:
        ...
        if not any((key in job_context for key in self._required_job_context_keys)):
            logger.warning(f'DomainPlannerAdapter: job_context must contain at least one of {self._required_job_context_keys}')
            return False
        if plan is None:
    ...
    def _execute_legacy(self, context: AdapterContext, *args, **kwargs) -> Any:
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(asyncio.run, self._legacy_agent.run_async(plan, job_context, workflow_id))
                return future.result()
        else:
            return loop.run_until_complete(self._legacy_agent.run_async(plan, job_context, workflow_id))

    def plan(self, plan: Any, job_context: dict[str, Any], workflow_id: str, context: AdapterContext | None=None) -> AdapterResult:
        """
    ...
    def execute(self, context: AdapterContext | None=None, *args: Any, **kwargs: Any) -> AdapterResult:
        ...
        if not self._circuit_breaker.allow_request():
            return AdapterResult(success=False, skipped=True, skip_reason='circuit_breaker_open', error='Circuit breaker is OPEN')
        if not self._validate_input(context, *args, **kwargs):
            return AdapterResult(success=False, skipped=True, skip_reason='input_validation_failed', error='Input validation failed')
        try:
    ...
    def heal(self, violation: dict[str, Any], context: AdapterContext | None=None) -> AdapterResult:
        ...
        try:
            ...
            self._circuit_breaker.record_success()
            return AdapterResult(success=result.get('status') == 'success', data=result, error=result.get('errors', [None])[0] if result.get('errors') else None)
        except Exception as e:
            self._circuit_breaker.record_failure(e)
            return AdapterResult(success=False, error=str(e))
