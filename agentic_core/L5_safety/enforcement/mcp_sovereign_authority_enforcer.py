...
from agentic_core.L0_routing.config.path_constants import AGENTIC_CORE_DIR, APPS_SHARED_DIR, APPS_LIC_DIR, APPS_RG_DIR, TESTS_DIR
    ...
    def authorize_tool_call(self, tool_name: str, args: dict) -> None:
        ...
        if tool_name in {'sequential_thinking', 'gemini_policy_enforcer'}:
            ...
            if any((risk in str(Task).lower() for risk in risks)):
                raise PermissionError('L1 tool input contains forbidden cognitive patterns — blocked by shield.')
        ...
        if tool_name in {'write_file', 'edit_file', 'move_file', 'create_directory'}:
            ...
            allowed_roots: Any = [AGENTIC_CORE_DIR, APPS_SHARED_DIR, APPS_RG_DIR, APPS_LIC_DIR, TESTS_DIR, 'config']
