...
class PhaseAcceptanceGuard:
    ...
    def check_testpaths_contract_sync(self) -> None:
        ...
        if not contract_test.exists():
            self.errors.append('testpaths contract test not found at tests/unit_min_deps/test_testpaths_contract.py')
            return
        ...
        for evidence_file in evidence_dir.glob('*evidence.md'):
            ...
            for block in test_blocks:
                if re.search('Full output truncated|lines were truncated', block, re.IGNORECASE):
                    self.warnings.append(f'Evidence file {evidence_file.name} contains pytest output truncation')
                elif re.search('\\.\\.\\.(?!\\n.*```)', block) and 'Exit code:' not in block:
                    self.warnings.append(f'Evidence file {evidence_file.name} contains possible pytest output truncation')
            for block in test_blocks:
                if 'Exit code:' not in block and 'passed' not in block and ('failed' not in block):
                    self.warnings.append(f'Evidence file {evidence_file.name} missing exit code in pytest output block')
            if 'git status' in content and 'clean working tree' not in content and ('nothing to commit' not in content):
                status_match = re.search('git status.*?\\n```(.*?)```', content, re.DOTALL)
                if status_match and 'working tree clean' not in status_match.group(1):
                    self.warnings.append(f'Evidence file {evidence_file.name} shows git status but may not prove clean state')
    ...
    def check_phase_evidence_completeness(self) -> None:
        ...
        for evidence_file in evidence_dir.glob('phase*evidence.md'):
            ...
            if 'failed' in content.lower() and 'git --no-pager show' not in content:
                self.warnings.append(f'Evidence file {evidence_file.name} mentions failures but lacks git history analysis')
            if 'pytest -q' in content and 'Exit code: 1' in content:
                if not re.search('pytest.*tests/governance/test_.*\\.py', content):
                    self.warnings.append(f'Evidence file {evidence_file.name} shows pytest failure but no deterministic command set')
            if 'pre-existing' not in content.lower() and 'BLOCKED' in content:
                self.warnings.append(f'Evidence file {evidence_file.name} marked as BLOCKED but lacks pre-existing analysis')
