...
class SystemValidator:
    ...
    def check_external_touch(self, code: str) -> bool:
        ...
        external_markers = ['pinecone', 'Pinecone', 'redis', 'Redis', 'git', 'subprocess.run', 'requests.', 'httpx.', 'aiohttp.', 'fetch', 'http://', 'https://']
    ...
    def print_report(self):
        ...
        for layer in sorted(layer_stats.keys()):
            ...
            Logger.info(f"  {layer}: {stats['total']} agents | Testing: {stats['testing']} | Healing: {stats['healing']} | MCP: {stats['mcp']}")
        Logger.info('')
        Logger.info('=' * 60)
        ...
        report_data = {'total_core': r.total_core, 'testing_pass': r.testing_pass, 'healing_pass': r.healing_pass, 'external_agents': r.external_agents, 'mcp_hardened': r.mcp_hardened, 'regressions': r.regressions, 'testing_pct': testing_pct, 'healing_pct': healing_pct, 'mcp_pct': mcp_pct, 'pass': testing_pct >= 80 and healing_pct >= 70 and (mcp_pct >= 80) and (len(r.regressions) == 0)}
