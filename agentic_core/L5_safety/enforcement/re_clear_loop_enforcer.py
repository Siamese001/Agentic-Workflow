...
class ReClearStatus(str, Enum):
    PENDING = "PENDING"       # Violation detected, awaiting remediation
    CLEARED = "CLEARED"       # Re-evaluation passed — constraint satisfied
    BLOCKED = "BLOCKED"       # Re-evaluation failed — escalation required
    ESCALATED = "ESCALATED"   # Explicitly escalated by authorized party
...
class ReClearTicket:
    ...
    def _assert_mutable(self) -> None:
        if self.status in (ReClearStatus.CLEARED, ReClearStatus.BLOCKED):
            raise ReClearViolation(
                f"ReClearTicket '{self.ticket_id}' is terminal (status={self.status.value}). "
                f"Cleared/Blocked tickets are immutable. Create a new ticket for re-remediation."
            )
    ...
    def escalate(self, note: str) -> "ReClearTicket":
        ...
        if self.status != ReClearStatus.BLOCKED:
            raise ReClearViolation(
                f"Only BLOCKED tickets may be escalated. "
                f"Ticket '{self.ticket_id}' is {self.status.value}."
            )
