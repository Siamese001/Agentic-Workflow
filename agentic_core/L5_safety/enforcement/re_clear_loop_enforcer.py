...
class ReClearTicket:
    ...
    def _assert_mutable(self) -> None:
        if self.status in (ReClearStatus.CLEARED, ReClearStatus.BLOCKED):
            raise ReClearViolation(f"ReClearTicket '{self.ticket_id}' is terminal (status={self.status.value}). Cleared/Blocked tickets are immutable. Create a new ticket for re-remediation.")

    def re_evaluate(self, constraint_fn: Callable[[], bool], evidence: dict[str, Any] | None=None) -> 'ReClearTicket':
        ...
        return ReClearTicket(ticket_id=self.ticket_id, constraint_id=self.constraint_id, violation_summary=self.violation_summary, status=new_status, remediation_evidence=new_evidence, escalation_note=self.escalation_note)
    ...
    def escalate(self, note: str) -> 'ReClearTicket':
        ...
        if self.status != ReClearStatus.BLOCKED:
            raise ReClearViolation(f"Only BLOCKED tickets may be escalated. Ticket '{self.ticket_id}' is {self.status.value}.")
        if not note or not note.strip():
            raise ReClearViolation('escalate() requires a non-empty note explaining the escalation rationale.')
        return ReClearTicket(ticket_id=self.ticket_id, constraint_id=self.constraint_id, violation_summary=self.violation_summary, status=ReClearStatus.ESCALATED, remediation_evidence=self.remediation_evidence, escalation_note=note)
...
def open_ticket(ticket_id: str, constraint_id: str, violation_summary: str) -> ReClearTicket:
    ...
    return ReClearTicket(ticket_id=ticket_id, constraint_id=constraint_id, violation_summary=violation_summary)
