...
ALLOWED_SHIM_PAIRS = {'agentic_core/base_agents/decorators.py': 'agentic_core/utils/decorators.py', 'system_learning/types/meta_learning_types.py': 'agentic_core/L5_safety/types/meta_learning_types.py', 'agentic_core/L5_safety/enforcement/sealed_interface_check_enforcer.py': 'agentic_core/enforcement/sealed_interface_check_enforcer.py'}
ALLOWED_SHIM_PAIRS_NORMALIZED = {canonical.replace('\\', '/'): shim.replace('\\', '/') for canonical, shim in ALLOWED_SHIM_PAIRS.items()}
...
def detect_collisions(scans: dict[str, dict[str, list[Path]]]) -> dict[str, list[tuple[str, list[Path]]]]:
    ...
    violations = {'duplicate_filenames': [], 'duplicate_logical_paths': [], 'case_insensitive_collisions': [], 'module_package_dual_definitions': []}
    all_files = []
...
def main():
    ...
    if os.environ.get('MODULE_COLLISION_UPDATE_BASELINE') == '1':
        ...
        print(f'[OK] Baseline updated with {sum((len(items) for items in collisions.values()))} collision groups')
        sys.exit(0)
    missing_roots = [name for name, path in roots_to_scan.items() if not path.exists()]
    if missing_roots:
        print(f'ERROR: Missing roots: {missing_roots}')
