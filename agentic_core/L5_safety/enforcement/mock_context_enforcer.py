    ...
    for file_path in get_data_files(project_root, extensions=target_exts):
        ...
        if rel.parts[0] == AGENTIC_CORE_DIR:
            ...
            if depth != agentic_core_exact_depth:
                violations.append({'file': str(rel), 'actual_depth': depth, 'expected_depth': agentic_core_exact_depth, 'type': file_path.suffix})
    return violations
...
def main():
    ...
    print('=' * 80)
    total_issues = len(l2_l3_result['missing_dirs']) + len(apps_violations) + len(tests_violations) + len(universal_violations)
