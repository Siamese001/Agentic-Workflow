...
class ErrorRecoveryGuardrail:
    ...
    def _classify_error(self, error: Exception, context: dict[str, Any]) -> ErrorContext:
        ...
        return ErrorContext(error=error, error_type=error_type, message=str(error), stack_trace=traceback.format_exc(), timestamp=time.time(), category=category, severity=severity, recoverable=recoverable, metadata=context)
