...
class HealingInvocationAudit:
    ...
    def __init__(self, project_root: Path=None):
        ...
        self.results = {'total_methods': 0, 'with_super': 0, 'without_super': [], 'confirmed_agents': [], 'missed_agents': [], 'chain_depth_estimates': {}}
        ...
        try:
            ...
            for match in matches:
                ...
                if has_super:
                    ...
                    self.results['confirmed_agents'].append({'file': str(file_path.relative_to(self.project_root)), 'agent': agent_name, 'status': 'confirmed', 'notes': 'super() present and chain active'})
                else:
                    self.results['without_super'].append(file_path)
                    self.results['missed_agents'].append({'file': str(file_path.relative_to(self.project_root)), 'agent': agent_name, 'reason': 'Missing super().heal_repository() call', 'priority': 'HIGH'})
    ...
    def _check_super_presence(self, file_path: Path) -> bool:
        ...
        try:
            with open(file_path) as f:
                content = f.read()
                method_match = re.search('def heal_repository\\(.*?\\).*?:.*?(?=\\n    def |\\nclass |\\Z)', content, re.DOTALL)
                if method_match:
