...
class HOP1ProfileDeterministic:
    ...
    def classify_profile_heuristic(self, profile: dict[str, Any]) -> HOPValidationResult:
        ...
        score = (completeness + (1.0 if industry != 'unknown' else 0.0) + (1.0 if seniority != 'unknown' else 0.0)) / 3.0
        classification = f'{seniority}_{industry}'
        return HOPValidationResult(passed=len(issues) == 0, issues=issues, score=score, classification=classification, metadata={'hop': 'HOP1', 'validation_type': 'deterministic'})
...
class HOP3DataExtractionDeterministic:
    ...
    def extract_grounded_entities(self, json_data: dict[str, Any]) -> HOPValidationResult:
        ...
        if not isinstance(json_data, dict):
            ...
            return HOPValidationResult(passed=False, issues=issues, metadata={'hop': 'HOP3', 'validation_type': 'deterministic'})
        ...
        return HOPValidationResult(passed=len(issues) == 0, issues=issues, score=score, metadata={'hop': 'HOP3', 'extracted_entities': extracted_entities})
...
class HOP4ConditionDeterministic:
    ...
    def check_conditions(self, context: dict[str, Any]) -> HOPValidationResult:
        ...
        return HOPValidationResult(passed=len(issues) == 0, issues=issues, score=score, metadata={'hop': 'HOP4', 'passed_conditions': passed_conditions})
...
class HOP6PlaceholderDeterministic:
    ...
    def validate_placeholders(self, content: str) -> HOPValidationResult:
        ...
        return HOPValidationResult(passed=len(found_placeholders) == 0, issues=issues, score=score, metadata={'hop': 'HOP6', 'placeholders': found_placeholders})
...
class HOP7GateDecisionDeterministic:
    ...
    def classify_gate_decision(self, violations: list[dict[str, Any]]) -> HOPValidationResult:
        ...
        return HOPValidationResult(passed=decision == 'proceed', issues=issues, score=score, classification=decision, metadata={'hop': 'HOP7', 'violation_counts': violation_counts})
