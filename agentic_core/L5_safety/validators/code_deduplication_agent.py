...
class CodeDeduplicationAgent(SovereignBaseAgent):
    ...
    def scan_for_duplicates(self, python_files: list[str]) -> Any:
        ...
        candidates: list[
            tuple[Path, str, int, str, str, int]
        ] = []  # (path, name, line, code, norm_str, len_norm)
    ...
    async def auto_extract_duplicates(self, project_root: Path, ctx: Any) -> Any:
        ...
        for _block_hash, occurrences in self.duplicate_groups.items():
            ...
            import_stmt: Any = (
                f"from agentic_core.utils.deduplicated.{module_name} import {func_name}"
            )
    ...
    def scan_filename_duplicates(self, python_files: list[Path], project_root: Path) -> None:
        ...
        for path in python_files:
            ...
            if (
                not path.exists()
                or ARCHIVES_DIR in str(path)
                or path.name in {"__init__.py", "setup.py"}
            ):
                pbar.update(1)
                continue
        ...
        for basename, entries in basename_to_entries.items():
            if len(entries) > 1:
                ...
                status = (
                    "IDENTICAL CONTENT" if len(hashes) == 1 else "DIVERGENT CONTENT (RENAME ONLY)"
                )
    ...
    CONTENT_DIR_MAPPING = [
        ...
        (
            ["thought", "cognition", "reasoning", "score"],
            f"{AGENTIC_CORE_DIR}/L1_cognition/thought_engine",
        ),
    ...
    def resolve_duplicates_safely(self, project_root: Path, dry_run: bool = True) -> None:
        ...
        for _file_hash, paths in self.file_duplicate_groups.items():
            if len(paths) > 1:
                ...
                primary = min(
                    paths, key=lambda p: (ARCHIVES_DIR in str(p), "old" in str(p), str(p))
                )
                for p in paths:
                    if p != primary:
                        if not dry_run:
                ...
                for p in paths:
                    if p != primary:
                        if not dry_run:
                            ...
                            p.unlink()
                            print(
                                f"      [✓] AGGRESSIVE PURGE: Deleted {p} (Backup: {backup.name})"
                            )
                            self.consolidated_count += 1
                        else:
                            print(f"      [DRY-RUN] Would delete: {p}")
    ...
    def detect_dead_code(self, file_path: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "unused_imports": [
                {"name": n, "line": import_lines.get(n)} for n in imported if n not in used
            ],
            "unused_functions": [
