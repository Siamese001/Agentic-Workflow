...
class DashboardGenerator:
    ...
    def load_agent_discovery(self) -> bool:
        ...
        try:
            ...
            if not isinstance(self.agents, list):
                print(
                    f"❌ ERROR: Invalid agent discovery data (expected list, got {type(self.agents)})"
                )
                return False
    ...
    def compute_territory_metrics(self, agents_list: list[dict]) -> dict[str, Any]:
        ...
        if unhardened_l5:
            ...
            health = base_health
        risk = (
            "HIGH" if avg_cc > 12 or health < 60 else "MED" if avg_cc > 8 or health < 80 else "LOW"
        )
    ...
    def build_total_row(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        complexity_health = round(
            sum(r["Complexity Health"] * r["Total"] for r in rows) / total_agents, 1
        )
        code_quality = round(
            sum(r["Code Quality Score"] * r["Total"] for r in rows) / total_agents, 1
        )
    ...
    def generate_dashboard_data(self) -> list[dict[str, Any]]:
        ...
        for territory_name in TERRITORY_ORDER:
            ...
            if territory_name in territories and len(territories[territory_name]) > 0:
                ...
                if metrics:
                    row = self.build_territory_row(
                        territory_name, metrics, priority, is_infrastructure
                    )
                    rows.append(row)
                    priority += 1
    ...
    def validate_html_before_write(self, html: str) -> tuple[bool, list[str]]:
        ...
        if size_kb > 900:
            errors.append(
                f"WARNING: HTML size is {size_kb:.1f}KB (expected <900KB) - possible duplication"
            )
        elif size_kb < 250:
            errors.append(
                f"WARNING: HTML size is {size_kb:.1f}KB (expected >250KB) - possible data missing"
            )
    ...
    def update_dashboard_html(
        self, data: list[dict[str, Any]], per_agent_data: dict[str, dict]
    ) -> bool:
        ...
        try:
            ...
            new_html = (
                html[:data_start_idx] + new_data_block + real_agent_block + html[data_end_idx:]
            )
