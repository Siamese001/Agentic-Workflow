...
from agentic_core.L5_safety.config.structure_blueprint import CORE_SUBFOLDER_MAP, DEPTH_RULES, PROJECT_ROOT_WHITELIST, SOVEREIGN_EXCLUDED_FOLDERS
...
class SovereignHealthReport:
    ...
    def total_violations(self) -> int:
        ...
        return len(self.gravity_violations) + len(self.import_violations) + len(self.hierarchy_violations) + len(self.drift_violations)
...
class UnifiedSSOTValidator:
    ...
    def _check_gravity_violations(self) -> list[GravityViolation]:
        ...
        for agent in agents:
            violations.append(GravityViolation(file_path=agent.relative_path, actual_layer=agent.layer, assigned_layer=agent.assigned_layer, agent_name=agent.class_name))
        return violations
                    ...
                    if isinstance(node, ast.Import | ast.ImportFrom):
                        ...
                        if target_layer and target_layer in self.layer_hierarchy:
                            if self.layer_hierarchy[source_layer] < self.layer_hierarchy[target_layer]:
                                violations.append(ImportViolation(file_path=str(py_file.relative_to(self.project_root)), source_layer=source_layer, target_layer=target_layer, import_line=import_line, line_number=node.lineno, severity=8))
                ...
                for dir_name in dirs:
                    ...
                    try:
                        ...
                        if actual_depth > max_depth:
                            violations.append(HierarchyViolation(folder_path=str(folder.relative_to(self.project_root)), actual_depth=actual_depth, max_depth=max_depth, root_folder=root_name))
                    except ValueError:
                        continue
        return violations
    ...
    def _check_drift_violations(self) -> list[DriftViolation]:
        ...
        for folder in agentic_core.iterdir():
            ...
            if folder_name not in authorized_l1:
                violations.append(DriftViolation(folder_path=str(folder.relative_to(self.project_root)), parent_folder=AGENTIC_CORE_DIR, violation_type='orphaned'))
            else:
                ...
                for subfolder in folder.iterdir():
                    ...
                    if authorized_l2 and subfolder_name not in authorized_l2:
                        violations.append(DriftViolation(folder_path=str(subfolder.relative_to(self.project_root)), parent_folder=folder_name, violation_type='orphaned'))
        return violations
