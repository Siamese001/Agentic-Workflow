...
def load_dashboard_data():
    ...
    content = "\n".join(lines)
    content = (
        content.replace("const dashboardData = ", "").replace("window.dashboardData = ", "").strip()
    )
    if content.endswith(";"):
        content = content[:-1]
...
def calculate_metrics_from_source(source_data):
    ...
    for territory, agents in by_territory.items():
        ...
        cc_values = [
            ...
            for a in agents
            if a.get("cyclomatic_complexity") is not None
        ]
                ...
                mismatches.append(
                    {
                        ...
                        if isinstance(dash_value, int | float)
...
def check_complexity_health_distribution():
    ...
    if low_health:
        for territory, metrics in low_health[:10]:
            print(
                f"      {territory}: {metrics['complexity_health']:.1f}% (Avg CC: {metrics['avg_cc']:.1f})"
            )
    else:
