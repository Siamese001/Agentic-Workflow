...
class MigrationHelper:
    ...
    def check_agent_compliance(cls, agent_class: type, strict: bool = False) -> ComplianceResult:
        ...
        """
        _emit_validated_by_safety_plane(str(uuid.uuid4()), "MigrationHelper.check_agent_compliance", "L5_POLICY")
        import uuid as _uuid  # noqa: PLC0415
        _trace_id = str(_uuid.uuid4())
        _emit_records_execution_trace(_trace_id, LayerSegment.L5_POLICY, "MigrationHelper.check_agent_compliance")
        import hashlib as _hashlib  # noqa: PLC0415
        _seg_hash = _hashlib.sha256(f"{_trace_id}:MigrationHelper.check_agent_compliance".encode()).hexdigest()[:24]
