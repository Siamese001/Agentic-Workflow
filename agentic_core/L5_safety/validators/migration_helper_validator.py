...
class ComplianceResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'agent_name': self.agent_name, 'compliant': self.compliant, 'has_feature_flag_mixin': self.has_feature_flag_mixin, 'has_verification_gate': self.has_verification_gate, 'has_human_review': self.has_human_review, 'has_meta_learning': self.has_meta_learning, 'has_audit_trail': self.has_audit_trail, 'missing_components': self.missing_components, 'recommendations': self.recommendations}
...
class MigrationStatus:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary."""
        return {'total_agents': self.total_agents, 'compliant_agents': self.compliant_agents, 'non_compliant_agents': self.non_compliant_agents, 'compliance_percentage': self.compliance_percentage, 'agents_by_status': self.agents_by_status, 'feature_flag_status': self.feature_flag_status}

class MigrationHelper:
    """Helper for tracking migration compliance."""
    REQUIRED_COMPONENTS = ['FeatureFlaggedAgentMixin', 'verification_gate', 'human_review', 'meta_learning', 'audit_trail']
    ...
    def check_agent_compliance(cls, agent_class: type, strict: bool=False) -> ComplianceResult:
        ...
        if strict:
            ...
            compliant = has_mixin
        return ComplianceResult(agent_name=agent_name, compliant=compliant, has_feature_flag_mixin=has_mixin, has_verification_gate=has_verification, has_human_review=has_review, has_meta_learning=has_ml, has_audit_trail=has_audit, missing_components=missing, recommendations=recommendations)
    ...
    def get_migration_status(cls, agent_classes: list[type], strict: bool=False) -> MigrationStatus:
        ...
        return MigrationStatus(total_agents=total, compliant_agents=len(compliant), non_compliant_agents=len(non_compliant), compliance_percentage=compliance_pct, agents_by_status={'compliant': compliant, 'non_compliant': non_compliant}, feature_flag_status=FeatureFlagManager.get_all_flags())
    ...
    def generate_migration_report(cls, agent_classes: list[type], strict: bool=False) -> str:
        ...
        lines = ['=' * 60, 'AGENT MIGRATION STATUS REPORT', '=' * 60, '', f'Total Agents: {status.total_agents}', f'Compliant: {status.compliant_agents}', f'Non-Compliant: {status.non_compliant_agents}', f'Compliance: {status.compliance_percentage:.1f}%', '', 'Feature Flag Status:']
