...
def check_meta_learning_trigger():
    ...
    print('=' * 80)
    test_cases = [(False, 5, True, 'dry_run=False, fixed=5'), (True, 5, False, 'dry_run=True, fixed=5'), (False, 0, False, 'dry_run=False, fixed=0')]
    all_passed = True
...
def main():
    ...
    results = {'gemini_embedder': check_gemini_embedder(guardian), 'redis_cache': check_redis_cache(guardian), 'pinecone_vector': check_pinecone_vector(guardian), 'trigger_logic': check_meta_learning_trigger(), 'end_to_end': simulate_healing_with_meta_learning(guardian)}
    print('\n' + '=' * 80)
