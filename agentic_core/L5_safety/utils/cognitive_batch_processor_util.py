...
class CognitiveBatchProcessor:
    ...
    def __init__(self, agent: Any, checkpoint_file: str | Path='cognitive_checkpoint.json', rate_limit_delay: float=1.0, checkpoint_interval: int=10, max_retries: int=3):
        """
    ...
    def _process_single_violation(self, violation: Any, file_path_str: str) -> bool:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                self.results[file_path_str] = {'action': decision.action, 'target_path': decision.target_path, 'reason': decision.reason, 'confidence': decision.confidence, 'violation_type': v_type}
            ...
            # guardian: allow-silent-swallow
                ...
                if attempt < self.max_retries:
                    ...
                    self.results[file_path_str] = {'action': 'ERROR', 'target_path': None, 'reason': f'Processing failed after {self.max_retries} attempts: {e}', 'confidence': 0.0, 'violation_type': v_type}
                    return False
        return False
