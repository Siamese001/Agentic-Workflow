...
def create_airlock_interface():
    ...
    if args.action == "list":
        ...
        if pending:
            print("Pending Airlock Requests:")
            for req in pending:
                print(f"  {req['ticket_id']}: {req['tool_name']} (risk={req['risk_score']})")
        else:
            print("No pending requests")
    
    elif args.action == "approve":
        if not args.ticket_id:
            print("Error: --ticket-id required for approval")
            return
        airlock.approve_request(args.ticket_id)
        print(f"Approved ticket {args.ticket_id}")
    
    elif args.action == "reject":
        if not args.ticket_id:
            print("Error: --ticket-id required for rejection")
            return
        reason = args.reason or "Rejected via CLI"
        airlock.reject_request(args.ticket_id, reason)
        print(f"Rejected ticket {args.ticket_id}: {reason}")
