...
def _get_enclosing_function(tree: ast.AST, target_lineno: int) -> ast.FunctionDef | ast.AsyncFunctionDef | None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            if hasattr(node, 'lineno') and hasattr(node, 'end_lineno'):
                if node.end_lineno is not None:
                    if node.lineno <= target_lineno <= node.end_lineno:
                        if best is None or (best.end_lineno is not None and node.end_lineno - node.lineno < best.end_lineno - best.lineno):
                            best = node
    return best
            ...
            for node in ast.walk(tree):
                ...
                for import_str, line_no in targets:
                    ...
                    if fn is not None:
                        ...
                        context = '__unknown__'
                    results.append(LazyUpwardImport(source_file=py_file, source_layer=src_layer, target_layer=tgt_layer, import_statement=import_str, line_number=line_no, context=context))
    return results
...
class LazySeamScanner:
    ...
    def scan_codebase(self) -> list[dict[str, Any]]:
        ...
        for item in metric['items']:
            seam_entry = {'file_path': str(item.source_file.relative_to(self.root_path)), 'function_name': item.context, 'imported_modules': [item.import_statement] if item.import_statement else [], 'imported_symbols': [], 'reason_code': 'TBD', 'justification': 'TBD'}
    ...
    def export_allowlist(self, output_path: Path) -> None:
        ...
        allowlist = {'description': 'Lazy Seam Allowlist - Phase 4 Option A (Phase 3B universe)', 'seams': self.seams}
...
def main():
    ...
    assert len(seams) == phase3b_total, f'Phase 4 scanner total ({len(seams)}) != Phase 3B total ({phase3b_total}). Scanner must be aligned to Phase 3B universe.'
