...
class LazySeamClassifier:
    ...
    REASON_TAXONOMY = {'D1_EXTERNAL_OPTIONAL_DEP': 'Optional external dependencies (pinecone/redis/etc.)', 'D2_ENTRYPOINT_SCRIPT': 'CLI/scripts that orchestrate', 'D3_PLUGIN_REGISTRY_DISPATCH': 'Registry/dynamic dispatch boundaries', 'D4_OBSERVABILITY_INTEGRATION': 'Telemetry/probes integration', 'D5_SECURITY_SAFETY_ADAPTER': 'Policy adapters (boundary-only)'}
    ...
    def _classify_seam(self, seam: dict[str, Any]) -> tuple[str, str]:
        ...
        external_deps = {'pinecone', 'redis', 'torch', 'transformers', 'openai', 'anthropic', 'numpy', 'pandas', 'matplotlib', 'plotly'}
        ...
        if 'scripts' in file_path or OPS_SCRIPTS_DIR in file_path or function_name.endswith('_orchestrator') or function_name.endswith('_runner'):
            return ('D2_ENTRYPOINT_SCRIPT', 'Script/orchestration entrypoint with lazy loading')
        registry_keywords = {'registry', 'dispatch', 'factory', 'router', 'broker', 'agent', 'sovereign', 'mcp', 'workflow'}
        if any((keyword in function_name.lower() for keyword in registry_keywords)) or any((keyword in file_path.lower() for keyword in registry_keywords)):
            return ('D3_PLUGIN_REGISTRY_DISPATCH', 'Plugin registry or dynamic dispatch boundary')
        obs_keywords = {'telemetry', 'tracing', 'metrics', 'observability', 'monitoring', 'logging', 'reporting'}
        if any((keyword in function_name.lower() for keyword in obs_keywords)) or 'L6_observability' in file_path:
            return ('D4_OBSERVABILITY_INTEGRATION', 'Observability/telemetry integration point')
        safety_keywords = {'safety', 'security', 'validator', 'enforcement', 'guard', 'policy', 'archival', 'healing', 'adapter'}
        if any((keyword in function_name.lower() for keyword in safety_keywords)) or 'L5_safety' in file_path or 'enforcement' in file_path:
