...
class SovereignLLMRouterMCPClient:
    ...
    def __init__(self, role: str = "safety_validation"):
        ...
        # Lazy import to prevent L5 <-> L3 circular dependency
    ...
    async def validate_content(self, content: str, validation_type: str = "safety") -> Dict[str, Any]:
        ...
        payload = {
            ...
            "model": config.LLM_ROUTER_SAFETY_MODEL,
            "temperature": config.LLM_ROUTER_VALIDATION_TEMPERATURE,
            "max_tokens": config.LLM_ROUTER_MAX_TOKENS
        }
        
        try:
            result = await self.router.manager.call_tool("llm_router_validate", payload)
            ...
            # Fail Closed - Assume unsafe if we cannot validate
            return {"is_safe": False, "reason": f"VALIDATION_SYSTEM_FAILURE: {str(e)}"}
    
    async def classify_intent(self, query: str) -> Dict[str, Any]:
        """Classify user intent via MCP."""
        try:
            return await self.router.manager.call_tool(
                "llm_router_classify",
                {
                    "query": query,
                    "model": config.LLM_ROUTER_DEFAULT_PROVIDER,
                    "temperature": 0.3
                }
            )
        except Exception as e:
