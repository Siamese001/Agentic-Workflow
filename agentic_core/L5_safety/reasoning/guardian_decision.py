...
class GuardianDecision:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {'allow': self.allow, 'escalate': self.escalate, 'violations': self.violations, 'budget_remaining': self.budget_remaining, 'policy_version': self.policy_version}
...
class L5Guardian:
    ...
    def __init__(self, policy_version: str='1.0') -> None:
        ...
        self.tool_allowlist = {'file_read', 'file_write', 'ast_parse', 'llm_call', 'redis_get', 'redis_set', 'pinecone_query', 'pinecone_upsert'}
        self.file_scope_whitelist = {'/tmp', '/workspace', AGENTIC_CORE_DIR}
        # guardian: allow-magic-config
        self.token_budget = 1000000
        self.agent_permissions = {'L1_cognition': ['read', 'transform'], 'L2_execution': ['read', 'write', 'validate'], 'L3_orchestration': ['read', 'write', 'orchestrate'], 'L5_safety': ['read', 'enforce', 'block']}
    ...
    def validate(self, manifest: Any, state: Any, policy_version: str | None=None) -> GuardianDecision:
        ...
        return GuardianDecision(allow=allow, escalate=escalate, violations=violations, budget_remaining=budget_remaining, policy_version=policy_version or self.policy_version)
