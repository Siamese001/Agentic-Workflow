...
class RootHygieneAgent(SovereignBaseAgent):
    ...
    def scan_root_violations(self, target_territory: str = None) -> dict[str, Any]:
        ...
        # Approved root-level names: all SSOT territory keys plus standard
        # config files / dotfiles that legitimately live at project root.
        approved_dirs = set(SOVEREIGN_TERRITORIES.keys()) | {
            # Version control / IDE / CI tooling dirs
            ".git",
            ".github",
            ".vscode",
            ".idea",
            ".nox",
            ".tox",
            # Windsurf / workspace dirs
            ".windsurf",
            # Healing/test infrastructure dirs
            ".healing_backups",
            ".pytest_tmp",
            # Project-level dirs that live at root by convention
            "tools",
            "system_learning",
            "logs",
        }
        ...
        delete_patterns = {
            ...
            ".venv", "venv", "coverage_html", ".mypy_cache",
            ".ruff_cache", "htmlcov", "dist", "build", ".eggs",
        }
