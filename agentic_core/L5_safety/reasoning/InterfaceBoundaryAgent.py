...
class InterfaceBoundaryAgent(SovereignBaseAgent):
    ...
    def audit_boundaries(self) -> list[dict]:
        ...
        for py_file in [f for f in all_py if str(f).startswith(str(l0_path))]:
            ...
            if metrics['method_count'] > self.threshold:
                self.violations.append({'file': str(py_file), 'complexity': metrics, 'action': 'EXTRACT_INTERFACE'})
    ...
    def _analyze_file_complexity(self, file_path: Path) -> dict:
        ...
        try:
            ...
            return {'method_count': len(methods), 'class_count': len(classes), 'loc': len(file_path.read_text().splitlines())}
    ...
    def generate_interface_stub(self, violation: dict) -> str:
        ...
        content = ['from abc import ABC, abstractmethod', '', f'class {interface_name}(ABC):', f'    """Automatically extracted interface for {source_path.name}"""']
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {'status': 'skipped', 'details': f'InterfaceBoundaryAgent heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        # guardian: allow-silent-swallow
        except Exception as e:
            return {'status': 'failed', 'details': f'InterfaceBoundaryAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
