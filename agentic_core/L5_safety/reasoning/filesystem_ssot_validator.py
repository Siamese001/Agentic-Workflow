...
class FilesystemSSOTValidatorAgent:
    ...
    def to_check_dict(self) -> dict[str, Any]:
        ...
        drift = self.scan()
        violations_count = len(drift.get('forbidden_folders', [])) + len(drift.get('archived_files_at_root', [])) + len(drift.get('duplicate_folders', []))
        return {'check_id': CHECK_ID, 'evidence': drift, 'violations_count': violations_count, 'repo_root': str(self.project_root)}
