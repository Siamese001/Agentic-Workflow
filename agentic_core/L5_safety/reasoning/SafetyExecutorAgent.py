...
class SafetyExecutorAgent(SovereignBaseAgent):
    ...
    def _init_default_gates(self) -> None:
        ...
        self._gates.append(SafetyGate(name='context_integrity', check_fn=lambda ctx: ctx is not None, severity='HIGH', blocking=True))

    def add_gate(self, name: str, check_fn: Callable[..., bool], severity: str='HIGH', blocking: bool=True) -> None:
    ...
    def execute(self, fn: Callable[..., T], *args, context: dict[str, Any] | None=None, **kwargs) -> ExecutionResult:
        """
        ...
        with self._lock:
            ...
            try:
                ...
                exec_result = ExecutionResult(status=ExecutionStatus.ALLOWED, message='Execution completed successfully', result=result, execution_time_ms=execution_time)
            ...
            # guardian: allow-silent-swallow
                ...
                end_time = datetime.utcnow()
                execution_time = (end_time - start_time).total_seconds() * 1000
                exec_result = ExecutionResult(status=ExecutionStatus.FAILED, message=f'Execution failed: {str(e)}', execution_time_ms=execution_time)
                self._results.append(exec_result)
                return exec_result
        ...
        try:
            ...
            if hasattr(self._detector, 'detect_injection'):
                ...
                if injection_threats and self._agent_config.block_on_high_severity:
                    return ExecutionResult(status=ExecutionStatus.BLOCKED, block_reason=BlockReason.DETECTOR_FLAG, message=f'Blocked by injection detector: {len(injection_threats)} threat(s)')
            if hasattr(self._detector, 'detect_all'):
                ...
                if high_severity and self._agent_config.block_on_high_severity:
                    return ExecutionResult(status=ExecutionStatus.BLOCKED, block_reason=BlockReason.DETECTOR_FLAG, message=f'Blocked by safety detector: {len(high_severity)} high-severity threat(s)')
            if hasattr(self._detector, 'get_safety_score'):
                ...
                if score < self._agent_config.safety_score_threshold:
                    return ExecutionResult(status=ExecutionStatus.BLOCKED, block_reason=BlockReason.THRESHOLD_EXCEEDED, message=f'Safety score {score:.2f} below threshold {self._agent_config.safety_score_threshold}')
        # guardian: allow-silent-swallow
        except Exception as e:
    ...
    def _run_gates(self, context: dict[str, Any]) -> ExecutionResult:
        ...
        for gate in self._gates:
            try:
                ...
                if not passed:
                    if gate.blocking:
                        return ExecutionResult(status=ExecutionStatus.BLOCKED, block_reason=BlockReason.INTEGRITY_FAILURE, message=f'Integrity gate failed: {gate.name}')
                    else:
            ...
            # guardian: allow-silent-swallow
                ...
                if gate.blocking:
                    return ExecutionResult(status=ExecutionStatus.BLOCKED, block_reason=BlockReason.INTEGRITY_FAILURE, message=f'Integrity gate error: {gate.name} - {e}')
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'allowed': self._allowed_count, 'blocked': self._blocked_count, 'total': self._allowed_count + self._blocked_count, 'block_rate': self._blocked_count / max(1, self._allowed_count + self._blocked_count), 'gates_count': len(self._gates)}
    ...
    def heal(self, violation: dict) -> dict:
        ...
        return {'violations_fixed': 0, 'violations_found': 1, 'errors': 0, 'skipped': 1, 'reason': 'Execution violations are runtime-managed, not code-healable'}
