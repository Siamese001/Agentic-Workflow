...
class RegressionOracleAgent(SovereignBaseAgent):
            ...
            if isinstance(node, _ast.Call):
                ...
                for dangerous in DANGEROUS_CALLS:
                    if dangerous in name:
                        violations.append(f"Line {node.lineno}: dangerous call '{name}' detected in generated code")
        return violations
        ...
        for change in changes:
            ...
            if test_code and test_file:
                ...
                if safety_violations:
                    Logger.error(f'[GAP-05] Rejecting generated test for {change.method_name} — {len(safety_violations)} dangerous node(s): {safety_violations[:3]}')
                    self.generated_tests.append(GeneratedTest(test_file=str(test_file), test_name=f'test_{change.method_name}', test_code=test_code, target_method=change.method_name, edge_cases=edge_cases, passed=False, error_message=f'AST safety rejection: {safety_violations[0]}'))
                    continue
                passed, error_msg = await self.test_runner.run_and_correct_test(change, test_file, test_code, max_iterations=self.MAX_CORRECTION_ITERATIONS)
    ...
    def post_heal_validation(self, generated_tests: list[GeneratedTest], dry_run: bool=True) -> dict[str, Any]:
        ...
        """
        report = {'post_heal_status': 'SKIPPED', 'tests_generated': len(generated_tests), 'tests_passed': 0, 'tests_failed': 0, 'message': ''}
        if dry_run:
    ...
    # guardian: allow-magic-config
    def cleanup_violations(self, violations: list[RegressionViolation], dry_run: bool=True, max_actions: int=50) -> list[dict[str, Any]]:
        """
        ...
        for i, violation in enumerate(violations):
            if i >= max_actions:
                ...
                break
            action = {'type': 'REGRESSION_TEST_HEALING', 'file_path': violation.file_path, 'method_name': violation.method_name, 'violation': violation.message, 'applied': False, 'action_taken': ''}
            try:
        ...
        batch_report = {'batch_post_heal_status': 'PREVIEW' if dry_run else 'APPLIED', 'batch_healed_count': sum((1 for a in actions if a.get('applied'))), 'batch_message': f'Processed {len(actions)} regression violations'}
    ...
    def run_with_cleanup(self, dry_run: bool=True) -> dict[str, Any]:
        ...
        for test in self.generated_tests:
            if not test.passed:
                all_violations.append(RegressionViolation(is_valid=False, message=f'TEST_FAILED: {test.error_message}', file_path=test.test_file, method_name=test.target_method, severity=4))
        ...
        return {'tests_generated': len(self.generated_tests), 'tests_passed': sum((1 for t in self.generated_tests if t.passed)), 'tests_failed': sum((1 for t in self.generated_tests if not t.passed)), 'violations_detected': len(all_violations), 'actions_applied': sum((1 for a in cleanup_results if a.get('applied'))), 'detailed_actions': cleanup_results, 'batch_post_heal_summary': batch_summary, 'post_heal_validation': post_heal_report, 'dry_run': dry_run}
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {'status': 'skipped', 'details': f'RegressionOracleAgent heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        # guardian: allow-silent-swallow
        except Exception as e:
            return {'status': 'failed', 'details': f'RegressionOracleAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
