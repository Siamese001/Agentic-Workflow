...
from agentic_core.L0_routing.config.path_constants import APPS_SHARED_DIR, OPS_SCRIPTS_DIR, ARCHIVES_DIR, APPS_LIC_DIR, APPS_RG_DIR
from agentic_core.L5_safety.config.structure_blueprint import DEPTH_RULES, LAYER_PREFIX_EXEMPT_TERRITORIES, PROJECT_ROOT_WHITELIST
...
class LocationValidatorAgent(SovereignBaseAgent):
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        return {'status': 'skipped', 'details': 'LocationValidatorAgent is validation-only. Use LocationHealerAgent for healing.', 'artifacts': [], 'errors': []}
    ...
    def validate_file_location(self, file_path: Path) -> tuple[bool, str]:
        ...
        if 'BaseAgent' in file_path.name or file_path.name == 'SovereignBaseAgent.py':
            if file_path.parent.name != 'base_agents':
                return (False, f"CRITICAL: Base Agents must reside in 'agentic_core/base_agents/', not '{file_path.parent.name}'")
        try:
    ...
    def _validate_forbidden_patterns(self, parts: tuple, root_folder: str) -> tuple[bool, str]:
        ...
        from agentic_core.L5_safety.config.structure_blueprint import FORBIDDEN_FOLDER_PATTERN, FORBIDDEN_ROOT_FOLDERS
        for part in parts:
                    ...
                    for alias in node.names:
                        for prefix in forbidden_prefixes:
                            if alias.name.startswith(prefix):
                                return (False, f"SEMANTIC VIOLATION: Root script imports '{alias.name}'. Files importing '{prefix}' belong in agentic_core/L0_routing/scripts/")
                elif isinstance(node, ast.ImportFrom):
                    if node.module:
                        for prefix in forbidden_prefixes:
                            if node.module.startswith(prefix):
                                return (False, f"SEMANTIC VIOLATION: Root script imports from '{node.module}'. Files importing '{prefix}' belong in agentic_core/L0_routing/scripts/")
        except SyntaxError:
            pass
    ...
    def _validate_depth_requirements(self, parts: tuple, root_folder: str, rel_path: Path) -> tuple[bool, str]:
        ...
        """
        from agentic_core.L5_safety.config.structure_blueprint import VARIABLE_DEPTH_SUBFOLDERS, validate_flat_directory
    ...
    def _validate_app_specific_files(self, root_folder: str, file_path: Path) -> tuple[bool, str]:
        ...
        from agentic_core.L5_safety.config.structure_blueprint import get_correct_app_path, is_app_specific_file
        if root_folder == AGENTIC_CORE_DIR and is_app_specific_file(file_path.name):
            ...
            return (False, f"APP-SPECIFIC IN CORE VIOLATION: '{file_path.name}' is application-specific and must not live in agentic_core. Move to '{correct_path}/'.")
        return (True, 'OK')
    ...
    def _validate_filename_patterns(self, file_path: Path) -> tuple[bool, str]:
        ...
        from agentic_core.L5_safety.config.structure_blueprint import check_forbidden_signals, has_forbidden_layer_prefix
        try:
    ...
    def _check_forbidden_imports(self, tree: Any, current_l1: str, rel_path: Path) -> tuple[bool, str]:
        ...
        if forbidden_app_import:
            return (False, f'GRAVITY VIOLATION (AST-resolved): Imports from apps_* modules forbidden in agentic_core. Move file to correct apps_*/engines/ folder. File: {rel_path}')
        if forbidden_layer_import:
            return (False, f'INTERNAL GRAVITY VIOLATION: {forbidden_layer_import} import direction forbidden. Refactor to respect layer gravity or move file. File: {rel_path}')
        return (True, 'OK')
            ...
            if file_path.exists():
                ...
                if correct_folder and current_subfolder != correct_folder:
                    if file_type != 'UTILITY':
                        return (False, f"TERRITORY ALIGNMENT (FCA): File classified as {file_type}, belongs in '{correct_folder}/' not '{current_subfolder}/'. File: {rel_path}")
    ...
    def _calculate_semantic_scores(self, tree: Any) -> tuple[float, float, dict[str, float]]:
        ...
        import ast
        from agentic_core.L5_safety.config.structure_blueprint import APP_LIC_AST_TERMS, APP_RG_AST_TERMS, CORE_TERRITORY_KEYWORDS
        app_rg_score = 0.0
        app_lic_score = 0.0
    ...
    def _check_territory_alignment(self, current_territory: str, territory_scores: dict[str, float], rel_path: Path) -> tuple[bool, str]:
        ...
        from agentic_core.L5_safety.config.structure_blueprint import MIN_ALIGNMENT_SCORE, TERRITORY_MISMATCH_THRESHOLD
        ...
        if current_score < MIN_ALIGNMENT_SCORE and max_other >= MIN_ALIGNMENT_SCORE:
            return (False, f"TERRITORY ALIGNMENT WEAK: Current '{current_territory}' score {current_score:.2f} < {MIN_ALIGNMENT_SCORE}. Lacks semantic signals — refactor or move to '{best_territory}'. File: {rel_path}")
        if max_other > current_score + TERRITORY_MISMATCH_THRESHOLD:
            return (False, f"TERRITORY MISMATCH VIOLATION: Stronger signals for '{best_territory}' ({max_other:.2f}) vs current ({current_score:.2f}). Move to agentic_core/{best_territory}. File: {rel_path}")
        return (True, 'OK')
    ...
    def run(self, target_territory: str | None=None) -> dict[str, Any]:
        ...
        roots_scanned = []
        if target_territory:
            target_roots = [target_territory] if target_territory in PROJECT_ROOT_WHITELIST else [AGENTIC_CORE_DIR]
        else:
        ...
        for root_name in target_roots:
            ...
            for py_file in root_path.rglob('*.py'):
                if any((skip in py_file.parts for skip in ['__pycache__', '.git', ARCHIVES_DIR, '.venv', '.sovereign_healing_backup', 'node_modules', OPS_SCRIPTS_DIR, 'artifacts'])):
                    continue
                total_files += 1
        ...
        return {'violations': violations, 'total_files_scanned': total_files, 'compliant_files': compliant_files, 'roots_scanned': roots_scanned, 'status': 'COMPLETE'}
