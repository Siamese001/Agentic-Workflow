...
class LocationValidatorAgent(SovereignBaseAgent):
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    if node.module:    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime
                        for prefix in forbidden_prefixes:
                            if node.module.startswith(prefix):    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling
                                return (
                                    False,
    ...
    def _validate_filename_patterns(self, file_path: Path) -> tuple[bool, str]:
        ...
        if file_path.name.endswith((".bak", ".backup", ".old", ".tmp")):
            return (False, "BROKEN BACKUP FILE: Remove stale backup file")    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling
        try:
            content = None
