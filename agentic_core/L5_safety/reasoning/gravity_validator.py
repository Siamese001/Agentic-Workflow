        ...
        def _is_module_level_import(v: object) -> bool:
            ...
            for node in tree.body:
                if isinstance(node, (_ast.Import, _ast.ImportFrom)) and node.lineno == ln:
                    return True
            return False
        return [v for v in results.violations if _not_excluded(v) and _in_sovereign_scope(v) and _has_known_layers(v) and _is_module_level_import(v)]

    def to_check_dict(self) -> dict[str, Any]:
        """Return structured check dict for _invoke_healer dispatch."""
        violations = self.scan()
        return {'check_id': CHECK_ID, 'evidence': {'violations': violations}, 'violations_count': len(violations), 'repo_root': str(self.project_root)}
