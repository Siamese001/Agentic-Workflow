...
class ConstitutionalReviewerAgent(SovereignBaseAgent, L5SafetyBase):
    ...
    async def run_async(self, final_draft: str, workflow_id: str) -> ConstitutionalReviewResult:
        ...
        prompt = await _format_prompt_with_defaults(prompt_template, {'final_draft': final_draft, 'constitution': constitution_text}, self.BudgetManager, client.goal_state, client.top_failures)
        response = await client.chat_completion_async(messages=[{'role': 'user', 'content': prompt}], temperature=self.config.ModelConfig.constitutional_review_model.temperature, response_format='json_object')
    ...
    def heal(self, violation: dict) -> dict:
        ...
        """
        return {'violations_fixed': 0, 'violations_found': 1, 'errors': 0, 'skipped': 1, 'reason': 'Constitutional violations require content revision'}
