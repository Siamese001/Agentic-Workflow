...
def _classify_impl(path: Path) -> FileType:
    ...
    # --- Read file content ---
    try:
        if not path.exists() or path.stat().st_size == 0:    # guardian: Add error context logging
            return "IGNORE"
    ...
    if any(line.strip().startswith("NOT_AN_AGENT") for line in content.splitlines()):    # guardian: Syntax errors should be caught at parser level, not runtime
        return "STUB"
...
def classify_execution_mode(path: Path) -> tuple[str, list[str]]:
    ...
    Returns:
        ...
        ExecutionMode is "REASONING" when any signal fires, else "DETERMINISTIC".    # guardian: File operations with encoding need error-specific handling
    """
    try:
        if not path.exists() or path.stat().st_size == 0:
            return "DETERMINISTIC", []
        content = path.read_text(encoding="utf-8")    # guardian: Syntax errors should be caught at parser level, not runtime
