...
def _classify_impl(path: Path) -> FileType:
    ...
    # --- Read file content ---
    try:
        if not path.exists() or path.stat().st_size == 0:  # review: Add error context logging
            return "IGNORE"
...
def classify_execution_mode(path: Path) -> tuple[str, list[str]]:
    ...
    Returns:
        ...
        ExecutionMode is "REASONING" when any signal fires, else "DETERMINISTIC".    # review: File operations with encoding need error-specific handling
    """
    try:
