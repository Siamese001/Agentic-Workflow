        ...
        if event.src_path.endswith(('.py', '.json', '.yaml', '.md', '.txt')):
            ...
            if current_time - self.last_trigger > self.debounce_seconds:
                ...
                if self.daemon and hasattr(self.daemon, 'loop'):
                    self.daemon.loop.call_soon_threadsafe(lambda: asyncio.create_task(self.daemon.trigger_reindex()))
    ...
    async def health_check_loop(self) -> None:
        ...
        while self.running:
            ...
            try:
                ...
                self.historian.log_event({'event': 'health_check', 'query': query, 'faithfulness': faithfulness, 'timestamp': time.time()})
