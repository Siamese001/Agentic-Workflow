...
class SystemArchitectAgent(SovereignBaseAgent):
    ...
    def heal(self, violation: dict) -> dict:
        ...
        """
        return {'status': 'manual_required', 'reason': 'Architectural restructuring requires human approval.', 'suggested_action': f"Review {violation.get('file')} dependencies."}
    ...
    def validate_core_architecture(self, target_path: str) -> dict[str, Any]:
        ...
        scan_roots = [self.project_root / territory for territory in sorted(CODE_TERRITORIES) if (self.project_root / territory).exists()]
        cache_key = tuple(sorted((str(r) for r in scan_roots)))
        Logger.info(f'SystemArchitect: Building dependency graph for {target_path} across {len(scan_roots)} territories')
        ...
        for mod in list(dependency_graph.keys()):
            if mod not in visited:
                dfs(mod)
        return {'valid': len(circular_dependencies) == 0, 'imports_valid': True, 'circular_dependencies': circular_dependencies, 'files_scanned': len(python_files)}
