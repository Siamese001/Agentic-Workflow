...
import hashlib
import logging
from agentic_core.L5_safety.config.structure_blueprint import GLOBAL_EXCLUDED_DIRS, L0_MAINTENANCE_DIR, L1_COGNITION_DIR, L2_EXECUTION_DIR, L3_ORCHESTRATION_DIR, L4_STATE_DIR, L5_SAFETY_DIR
...
class DuplicateCodeDetectorAgent(AtomicExecutionMixin, SubatomicTestingMixin, HealerMixin, MCPHardenedMixin):
    ...
    def __post_init__(self):
        ...
        super().__init__()
    SUPPORTED_EXTENSIONS = {'.py', '.html', '.htm', '.css', '.js', '.json', '.yaml', '.yml', '.md', '.txt', '.xml', '.svg', '.toml', '.ini', '.cfg', '.conf'}
    WHOLE_FILE_TYPES = {'.json', '.yaml', '.yml', '.toml', '.ini', '.cfg', '.conf'}
    CANONICAL_PREFIXES = [L5_SAFETY_DIR, L4_STATE_DIR, L3_ORCHESTRATION_DIR, L2_EXECUTION_DIR, L1_COGNITION_DIR, L0_MAINTENANCE_DIR, 'agentic_core/observability', UTILS_DIR]
    ...
    def _generate_deletion_plan(self, duplicates: list[DuplicateFile]) -> list[dict]:
        ...
        for dup in duplicates:
            ...
            recommendations.append({'keep': str(keep_path.relative_to(self.project_root)), 'delete': [str(p.relative_to(self.project_root)) for p in delete_paths], 'rationale': rationale, 'size': dup.size, 'file_type': dup.file_type, 'hash': dup.hash[:16]})
    ...
    def archive_duplicates(self, recommendations: list[dict], dry_run: bool=True) -> dict:
        ...
        return {'archived_count': len(archived), 'archived_files': archived, 'archive_location': str(archive_dir.relative_to(self.project_root)) if not dry_run else f'archives/duplicates_{timestamp}', 'errors': errors, 'dry_run': dry_run}
