...
def heal(violation: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if 'DEPTH' in violation_type:
            return {'status': 'manual_required', 'details': 'Depth violations require StructuralHealerAgent.heal_file_moves()', 'artifacts': [], 'errors': []}
        elif 'ATOMICITY' in violation_type or 'SIZE' in violation_type:
            return {'status': 'manual_required', 'details': 'Atomicity violations require StructuralHealerAgent.heal_fission()', 'artifacts': [], 'errors': []}
        else:
            return {'status': 'skipped', 'details': f'No healing strategy for violation type: {violation_type}', 'artifacts': [], 'errors': []}
...
class GovernanceAgent(SovereignBaseAgent):
    ...
    def __init__(self, root_dir: str=None) -> None:
        ...
        try:
            from agentic_core.L5_safety.config.structure_blueprint import ROOT_PROTECTED_FILES, SOVEREIGN_REGISTRY
        except ImportError:
        ...
        self.sovereign_dirs = {AGENTIC_CORE_DIR, 'schemas', 'scripts', 'docs', TESTS_DIR, 'config', 'data', 'cache', 'observability', '.git', '__pycache__', '.pytest_cache', '.tox', 'venv', '.venv', 'node_modules', '.idea', '.vscode', 'dist', 'build', 'coverage', '.github', 'htmlcov', '.mypy_cache', '.coverage', 'eggs', '.eggs', '*.egg-info'}
        ...
        if is_noise:
            ...
            try:
                ...
                while target.exists():
                    ...
                    counter += 1
                result = self.gatekeeper.safe_move(file_path, target, 'GovernanceAgent', 'Move root script to scripts/')
                if result.success:
    ...
    def enforce_depth_law(self, file_path: str) -> str | None:
        ...
        """
        import warnings
        warnings.warn('GovernanceAgent.enforce_depth_law() is deprecated. Use HealerAgent.heal_file_moves() for actual moves.', DeprecationWarning, stacklevel=2)
        path: Any = Path(file_path)
    ...
    def _create_empty_report(self) -> dict[str, Any]:
        ...
        return {'root_violations': [], 'depth_violations': [], 'atomicity_violations': [], 'complexity_violations': [], 'enforced_actions': [], 'BlastRadius': None, 'overall_status': 'PASS'}
    ...
    def _has_violations(self, report: dict[str, Any]) -> bool:
        ...
        return bool(report['root_violations'] or report['depth_violations'] or report['atomicity_violations'] or report['complexity_violations'])
    ...
    def cleanup_violations(self, file_paths: list[str]=None, dry_run: bool=True) -> list[dict[str, Any]]:
        ...
        for v in root_violations:
            actions.append({'violation': v, 'type': 'ROOT_HYGIENE', 'applied': not dry_run, 'action_taken': 'SANITIZED' if not dry_run else 'PREVIEW'})
        if file_paths:
            for fp in file_paths:
                depth_v = self.check_depth_law(fp)
                if depth_v:
                    actions.append({'violation': depth_v, 'path': fp, 'type': 'DEPTH', 'applied': False, 'action_taken': 'SUGGEST: Use HealerAgent.heal_file_moves()'})
                    affected_paths.append(fp)
                atom_v = self.check_atomicity_law(fp)
                if atom_v:
                    actions.append({'violation': atom_v, 'path': fp, 'type': 'ATOMICITY', 'applied': False, 'action_taken': 'SUGGEST: Use HealerAgent.heal_fission()'})
                    affected_paths.append(fp)
        ...
        if dry_run:
            ...
            if hierarchy_report['hierarchy_status'] == 'FULL_SUCCESS' and import_report['import_status'] == 'FULL_SUCCESS':
    ...
    def run_with_cleanup(self, file_paths: list[str]=None, dry_run: bool=True) -> dict[str, Any]:
        ...
        return {'violations_detected': len(cleanup_results), 'actions_applied': sum((1 for a in cleanup_results if a.get('applied'))), 'detailed_actions': cleanup_results, 'architecture_report': arch_report, 'batch_post_heal_summary': batch_summary, 'hierarchy_validation_summary': batch_summary.get('hierarchy_validation', {}), 'import_validation_summary': batch_summary.get('import_validation', {}), 'blast_radius': batch_summary.get('blast_radius'), 'dry_run': dry_run}
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {'status': 'skipped', 'details': f'GovernanceAgent heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        # guardian: allow-silent-swallow
        except Exception as e:
            return {'status': 'failed', 'details': f'GovernanceAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
