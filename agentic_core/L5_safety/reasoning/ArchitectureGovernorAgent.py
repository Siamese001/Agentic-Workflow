...
from agentic_core.L5_safety.reasoning.FileClassificationAgent import FileClassificationAgent, get_python_files_fast
from agentic_core.utils.decorators_compat_util import standard_heal
from agentic_core.utils.timeout_decorator_util import timeout
from agentic_core.L0_routing.config.path_constants import TESTS_DIR, TESTS_DIR
...
class ArchitectureGovernorAgent(SovereignBaseAgent):
    ...
    def _get_structure_validator(self):
        ...
        if self._structure_validator is None:
            ...
            config = StructureConfig(check_gravity=True, check_duplicates=True, check_orphans=True, check_registry=False, check_contracts=False, check_hierarchy=True, project_root=self.project_root)
    ...
    def _do_heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            if violation_type == 'GRAVITY':
                ...
                return {'status': 'success' if result else 'failed', 'details': f"Gravity violation healing {('succeeded' if result else 'failed')}", 'artifacts': [file_path] if result else [], 'errors': [] if result else ['Gravity healing failed']}
            elif violation_type == 'NAMING':
                result = self._heal_naming_violation(violation, auto_approve=True)
                return {'status': 'success' if result else 'failed', 'details': f"Naming violation healing {('succeeded' if result else 'failed')}", 'artifacts': [file_path] if result else [], 'errors': [] if result else ['Naming healing failed']}
            elif violation_type == 'DUPLICATE':
                result = self._resolve_collision(violation)
                return {'status': 'success' if result else 'failed', 'details': f"Duplicate resolution {('succeeded' if result else 'failed')}", 'artifacts': [file_path] if result else [], 'errors': [] if result else ['Duplicate resolution failed']}
            elif violation_type == 'ORPHAN':
                file_path_obj = Path(file_path)
                result = self._process_cognitive_disposition(file_path_obj, 'ORPHAN')
                return {'status': 'success' if result else 'failed', 'details': f"Orphan file processing {('succeeded' if result else 'failed')}", 'artifacts': [file_path] if result else [], 'errors': [] if result else ['Orphan processing failed']}
            else:
                return {'status': 'skipped', 'details': f'No healer available for violation type: {violation_type}', 'artifacts': [], 'errors': []}
    ...
    def heal_repository(self, dry_run: bool=True, execute: bool=False, depth: int=0, max_depth: int=3, _call_path: set | None=None, auto_approve: bool | None=None, target_territory: str | None=None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            for root_name in target_roots:
                ...
                for violation in report.violations:
                    ...
                    violation_dict = {'type': v_type_name, 'file': str(violation.file_path) if violation.file_path else None, 'message': violation.message, 'severity': getattr(violation, 'severity', None), 'suggestion': getattr(violation, 'suggestion', None), 'source_layer': getattr(violation, 'source_layer', None), 'target_layer': getattr(violation, 'target_layer', None)}
            ...
            if dry_run:
                Logger.info(f'[DRY-RUN] Found {violations_found} violations across {len(roots_scanned)} territories')
            else:
                Logger.info(f'Found {violations_found} violations, fixed {violations_fixed}')
            if violations_found > 0 and (not execute):
                Logger.warning(f'[{agent_name}] SHIELD ALERT: {violations_found} violations blocking baseline purity.')
            ...
            if not dry_run:
                try:
                    ...
                    if cleanup_stats.get('errors', 0) > 0:
                        Logger.warning(f"[{agent_name}] SSOT Cleanup reported errors: {cleanup_stats['errors']}")
                # guardian: allow-silent-swallow
                except Exception as e:
                    Logger.error(f'[{agent_name}] SSOT Cleanup Sub-routine failed: {e}')
            return {'violations_found': violations_found, 'violations_fixed': violations_fixed, 'roots_scanned': roots_scanned, 'status': 'PASS' if violations_found == 0 else 'FAIL', 'deduplication_audit': dedup_results, 'ssot_moves': ssot_moves, 'ssot_imports_updated': ssot_imports_updated}
        finally:
    ...
    def run_audit(self, target_territories: list[str] | None=None) -> dict[str, Any]:
        ...
        self.stats.update({'violations_found': total_violations, 'drift_detected': len(drift_violations), 'errors': structural_results.get('total_errors', 0)})
        ...
        return {'success': success, 'stats': self.stats, 'violations': structural_results.get('violation_details', []), 'drift_violations': drift_violations}
    ...
    def _orchestrate_guardian_scan(self, target_territories: list[str] | None=None) -> dict[str, Any]:
            ...
            for root_path in scan_targets:
                ...
                try:
                    ...
                    for h_violation in h_report.get('violations', []):
                        total_violations += 1
                        violation_details.append({'type': 'STRUCTURE', 'file': h_violation.get('file'), 'message': 'File sitting in territory root. Should be in SSOT subfolder.', 'severity': 'ERROR', 'suggestion': 'Relocate to approved subfolder (reasoning/, enforcement/, validators/, etc.)'})
                # guardian: allow-silent-swallow
                except Exception as e:
                    raise
        ...
        # guardian: allow-silent-swallow
            ...
            total_errors += 1
        return {'total_violations': total_violations, 'total_errors': total_errors, 'violation_details': violation_details, 'roots_scanned': roots_scanned}
    ...
    def validate_layer_boundaries(self, file_path: Path) -> tuple[bool, str]:
        ...
        """
        import subprocess
        result = subprocess.run(['python', 'tests/guardian/test_architecture_governance.py', str(file_path)], capture_output=True, text=True, cwd=self.project_root)
    ...
    def _heal_gravity_violation(self, violation: dict[str, Any], auto_approve: bool) -> bool:
        ...
        try:
            ...
            fix = repair_agent.analyze_violation(file_path=Path(file_path), import_statement=violation.get('message', ''), file_layer=source_layer or '', import_layer=target_layer or '')
        ...
        if not file_path.name.endswith('Agent.py') and 'Agent' in violation.get('message', ''):
            ...
            if auto_approve:
                try:
                    ...
                    result = gatekeeper.safe_move(source=file_path, destination=new_path, requester_agent='ArchitectureGovernorAgent', reason='Naming convention fix: add Agent suffix')
                    if result.success:
    ...
    def _trigger_deduplication_audit(self, roots: list[str], execute: bool=False) -> dict[str, Any]:
        ...
        for root_name in roots:
            ...
            for dup in duplicates:
                collisions.append({'root': root_name, 'type': 'DUPLICATE_AGENT', 'message': dup.message, 'file': str(dup.file_path) if dup.file_path else None, 'violation': dup})
        collisions_fixed = 0
        ...
        if collisions:
            Logger.warning(f'  [DEDUP] Found {len(collisions)} potential collisions, fixed {collisions_fixed}')
        else:
            Logger.info(f'  [DEDUP] No collisions found across {len(roots)} roots')
        return {'roots_audited': roots, 'collisions_found': len(collisions), 'collisions_fixed': collisions_fixed, 'collisions': collisions[:10] if collisions else []}
    ...
    def _resolve_collision(self, violation: Any) -> int:
        ...
        """
        priority = {'agentic_core': 0, 'apps_shared': 1, 'apps_rg': 2, 'apps_lic': 3, 'tests': 4, 'scripts': 5}
        ...
        for file_path in to_archive:
            try:
                result = gatekeeper.safe_move(file_path, destination_category='deduplication_cleanup', reason=f'Duplicate of {master.name}')
                if result.success:
    ...
    def _cleanup_empty_dirs(self, path: Path) -> None:
        ...
        remaining = [p for p in path.iterdir() if p.name not in {'__pycache__', '__init__.py', '.gitkeep'} and (not p.name.startswith('.'))]
        if not remaining:
            try:
    ...
    def capture_golden_baseline(self) -> Path:
        ...
        manifest = {'version': '1.0', 'timestamp': datetime.utcnow().isoformat(), 'audit_id': str(uuid.uuid4()), 'files': {}}
        ...
        try:
            ...
            for rel_path, expected_hash in baseline['files'].items():
                ...
                if current_hash != expected_hash:
                    violations.append({'type': 'CONTENT_DRIFT', 'path': rel_path, 'expected': expected_hash, 'actual': current_hash, 'severity': 'CRITICAL'})
        # guardian: allow-silent-swallow
        except Exception as e:
    ...
    def _persist_audit_report(self, structural_results: dict[str, Any], drift_violations: list[dict[str, Any]]) -> None:
        ...
        {'timestamp': datetime.utcnow().isoformat(), 'audit_id': str(uuid.uuid4()), 'structural_summary': structural_results, 'drift_violations': drift_violations, 'stats': self.stats}
    ...
    def execute_cognitive_purge(self, checkpoint_file: str='cognitive_checkpoint.json', rate_limit_delay: float=1.0) -> dict[str, Any]:
        """
        ...
        if not violations:
            ...
            return {'violations_found': 0, 'batch_stats': {'PROCESSED': 0, 'SKIPPED': 0, 'ERRORS': 0, 'TOTAL': 0}}
        ...
        cognitive = self._get_cognitive_agent()
        processor = CognitiveBatchProcessor(agent=cognitive, checkpoint_file=checkpoint_file, rate_limit_delay=rate_limit_delay)
        ...
        Logger.info('=' * 60)
        return {'violations_found': len(violations), 'batch_stats': batch_stats, 'results_stats': results_stats, 'checkpoint_file': checkpoint_file}
        ...
        try:
            ...
            for territory in target_territories:
                ...
                for v in h_report.get('violations', []):
                    audit_results['violations'].append({'type': 'STRUCTURE', 'file': v.get('file'), 'message': 'File sitting in territory root; must be in SSOT subfolder.', 'severity': 'ERROR'})
        # guardian: allow-silent-swallow
        except Exception as e:
            raise
            ...
            for territory in target_territories:
                ...
                if not arch_report.get('imports_valid', True):
                    for circ in arch_report.get('circular_dependencies', []):
                        audit_results['violations'].append({'type': 'GRAVITY', 'file': territory, 'message': f'Circular dependency detected: {circ}', 'severity': 'CRITICAL'})
        # guardian: allow-silent-swallow
        except Exception as e:
            raise
        ...
        for py_file in territory_path.rglob('*.py'):
            try:
                ...
                if line_count > max_lines:
                    violations.append({'type': 'FILE_SIZE', 'file': str(py_file.relative_to(self.project_root)), 'message': f'{py_file.name}: {line_count} lines exceeds max {max_lines}', 'severity': 'medium', 'recommended_action': 'Split or refactor file to reduce line count'})
    ...
    def generate_healing_plan(self, gov_report: dict[str, Any]) -> dict[str, Any]:
        ...
        plan = {'actions': [], 'requires_healing': len(violations) > 0, 'violations_count': gov_report.get('stats', {}).get('violations_found', 0), 'drift_count': gov_report.get('stats', {}).get('drift_detected', 0), 'errors_count': gov_report.get('stats', {}).get('errors', 0), 'target_territories': gov_report.get('target_territories', []), 'naming_fixes': [v for v in violations if v.get('type') == 'NAMING'], 'structure_fixes': [v for v in violations if v.get('type') == 'STRUCTURE']}
        if has_naming:
    ...
    def _process_cognitive_disposition(self, file_path: Path, violation_type: str) -> bool:
        ...
        try:
            ...
            if decision.action == 'MOVE' and decision.target_path:
                ...
                gatekeeper = self._get_archival_gatekeeper()
                result = gatekeeper.safe_move(file_path, destination_category=decision.target_path, reason=f'Cognitive disposition: {decision.reason}')
                ...
                gatekeeper = self._get_archival_gatekeeper()
                result = gatekeeper.safe_move(file_path, destination_category=archive_path, reason=f'Cognitive archive: {decision.reason}')
                if hasattr(result, 'success') and result.success:
