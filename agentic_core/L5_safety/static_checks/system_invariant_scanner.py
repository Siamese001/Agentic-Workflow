...
class BypassViolation:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'file_path': self.file_path, 'line': self.line, 'rule_id': self.rule_id, 'snippet': self.snippet, 'description': self.description}

class SystemInvariantScanner(ast.NodeVisitor):
    """AST visitor to detect sovereignty bypass violations."""
    ALLOWLISTED_MODULES = {'agentic_core.L2_execution.UniversalWriteGateway', 'agentic_core.L2_execution.engines.execution_gateway', 'agentic_core.L2_execution.healers.healing_provider_adapters', 'system_invariant_scanner', 'system_learning.engines.embedding_service_factory', TESTS_DIR, 'test_'}
    RESTRICTED_PROVIDERS = {'openai', 'anthropic', 'google.generativeai', 'vertexai', 'litellm', 'transformers', 'torch', 'tensorflow'}
    RESTRICTED_FILE_OPS = {'open', 'Path.write_text', 'Path.write_bytes', 'Path.unlink', 'os.remove', 'os.rename', 'os.replace', 'os.mkdir', 'os.makedirs'}
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name):
            ...
            if func_name in self.RESTRICTED_FILE_OPS:
                self._add_violation(node.lineno, 'GATEWAY_BYPASS', f'Direct call to {func_name}', f"Direct file operation '{func_name}' detected - use UniversalWriteGateway")
        elif isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Name):
                module_name = node.func.value.id
                if module_name in self.RESTRICTED_PROVIDERS:
                    self._add_violation(node.lineno, 'PROVIDER_BYPASS', f'Direct call to {module_name}.{node.func.attr}', 'Direct provider SDK call detected - use HealingProviderInvoker')
        self.generic_visit(node)
    ...
    def visit_Import(self, node: ast.Import) -> None:
        ...
        for alias in node.names:
            if alias.name in self.RESTRICTED_PROVIDERS:
                self._add_violation(node.lineno, 'PROVIDER_BYPASS', f'Import of restricted provider {alias.name}', 'Direct provider import detected - use HealingProviderInvoker seam')
            elif any((restricted in alias.name for restricted in self.RESTRICTED_EMBEDDING)):
                self._add_violation(node.lineno, 'EMBEDDING_BYPASS', f'Import of restricted embedding {alias.name}', 'Direct embedding import detected - use EmbeddingServiceFactory')
        self.generic_visit(node)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if node.module:
            if node.module in self.RESTRICTED_PROVIDERS:
                self._add_violation(node.lineno, 'PROVIDER_BYPASS', f'From-import of restricted provider {node.module}', 'Direct provider import detected - use HealingProviderInvoker seam')
            elif any((restricted in node.module for restricted in self.RESTRICTED_EMBEDDING)):
                self._add_violation(node.lineno, 'EMBEDDING_BYPASS', f'From-import of restricted embedding {node.module}', 'Direct embedding import detected - use EmbeddingServiceFactory')
        self.generic_visit(node)
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        if node.name in self.RESTRICTED_EMBEDDING:
            self._add_violation(node.lineno, 'EMBEDDING_BYPASS', f'Class definition of {node.name}', 'Direct embedding class definition detected - use EmbeddingServiceFactory')
        self.generic_visit(node)

    def _add_violation(self, line: int, rule_id: str, snippet: str, description: str) -> None:
        """Add a violation to the list."""
        violation = BypassViolation(file_path=str(self.file_path), line=line, rule_id=rule_id, snippet=snippet, description=description)
    ...
    for pattern in patterns:
        for file_path in repo_root.glob(pattern):
            ...
            try:
                ...
                violations.append(BypassViolation(file_path=str(file_path), line=e.lineno or 0, rule_id='SYNTAX_ERROR', snippet='Syntax error', description=f'Syntax error: {e}'))
            except Exception as e:
                raise
                violations.append(BypassViolation(file_path=str(file_path), line=0, rule_id='SCAN_ERROR', snippet='Scan error', description=f'Scan error: {e}'))
...
def get_bypass_scan_summary(violations: list[BypassViolation]) -> dict[str, Any]:
    ...
    summary = {'total_violations': len(violations), 'by_rule': {}, 'files_affected': len(files_affected), 'files_affected_list': sorted(files_affected)}
    for violation in violations:
