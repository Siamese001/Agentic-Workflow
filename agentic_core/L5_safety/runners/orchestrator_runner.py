...
def run_orchestrator_mission(project_root: Path, targets: list[str], execute: bool=False) -> dict:
    ...
    try:
        ...
        active_roster = [('LocationAgent', LocationHealerAgent(project_root)), ('HierarchyAgent', HierarchyAgent(project_root)), ('ArchitectureGovernorAgent', ArchitectureGovernorAgent(project_root)), ('GravityLeakRepairAgent', GravityLeakRepairAgent(project_root))]
        mission_context = {'dry_run': not execute, 'execute': execute, 'domains': targets, 'scan_mode': 'leveraged'}
