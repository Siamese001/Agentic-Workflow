...
def run_ci_verification(project_root: Path, auto_approve: bool=True) -> dict:
    ...
    return {'success': is_compliant, 'violations_found': results.get('violations_found', 0), 'roots_scanned': results.get('roots_scanned', []), 'raw_result': results}
...
def main() -> int:
    ...
    parser.add_argument('--action', choices=['verify', 'capture_baseline', 'audit'], required=True, help='Action to perform')
    parser.add_argument('--project-root', type=str, default=None, help='Project root path (defaults to auto-detect)')
    parser.add_argument('--targets', type=str, default=None, help='Comma-separated target territories for audit')
    parser.add_argument('--auto-approve', action='store_true', default=True, help='Auto-approve mode (default: True)')
