...
"""
CodeValidatorAgent - Single-Pass AST Code Validation
...
class RuleSet:
    ...
    def strict(cls) -> RuleSet:
        ...
        return cls(check_type_hints=True, check_docstrings=True, require_mcp_mixin=True)
...
class UnifiedASTVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if self.rules.check_async:
            self._check_time_sleep_usage(node)
        self.generic_visit(node)

    def visit_Try(self, node: ast.Try) -> None:
        """Check for excessive try-except blocks."""
        if self.rules.check_async:
...
def create_legacy_print_validator(**kwargs: Any) -> CodeValidatorAgent:
    ...
    rules = RuleSet(check_syntax=True, check_canon=False, check_async=False, check_print=True)
    return CodeValidatorAgent(default_rules=rules, **kwargs)
