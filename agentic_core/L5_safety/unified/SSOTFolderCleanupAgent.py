...
class SSOTFolderCleanupAgent:
    ...
    def triage_file(self, file_path: Path) -> dict[str, Any]:
        """
        Use CognitiveDispositionAgent to determine where a file should go.
    ...
    def update_imports_for_moved_file(
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            try:
                ...
                # Update imports
                new_content = self._update_imports_in_content(
        ...
        for i, line in enumerate(lines, 1):
            if i in import_lines:
                ...
                line = from_pattern.sub(
                    lambda m: f"from {new_module}{m.group(1) or ''} import",
                    line
                )
                # 2. Handle "import X"
                ...
                if not line.strip().startswith("from"):
                    line = import_pattern.sub(
                        lambda m: f"import {new_module}{m.group(1) or ''}",
                        line
                    )
    ...
    def delete_empty_folders(self, start_path: Optional[Path] = None) -> int:
        """
    ...
    def cleanup_repository(self) -> dict[str, Any]:
        ...
        for file_path in non_approved_files:
            ...
            if triage["action"] == "MOVE" and triage["target_path"]:
                ...
                Logger.info(
                    f"Skipping {file_path}: {triage['action']} - {triage['reason']}"
                )
