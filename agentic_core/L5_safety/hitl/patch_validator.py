...
def validate_patch(patch: dict[str, Any]) -> ValidatedPatch:
    ...
    if missing:
        raise HumanPatchValidationError(f'HITL patch missing required field(s): {missing}. All MODIFY_DIFF patches must include: original_plan_hash, structured_patch_schema, reviewer_signature.')
    patch_hash = hashlib.sha256(json.dumps(patch, sort_keys=True, ensure_ascii=True, default=str).encode()).hexdigest()
    logger.info('HITL patch validated: reviewer=%s patch_hash=%s', patch.get('reviewer_signature', '')[:16], patch_hash[:16])
    return ValidatedPatch(original_plan_hash=patch['original_plan_hash'], structured_patch_schema=patch['structured_patch_schema'], reviewer_signature=patch['reviewer_signature'], patch_hash=patch_hash, raw=patch)
