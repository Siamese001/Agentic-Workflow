...
ALLOWED_MODULES: frozenset[str] = frozenset({'__future__', 'typing', 'types', 'collections', 'functools', 'itertools', 'dataclasses'})
SCAN_ROOTS: tuple[str, ...] = ('agentic_core', 'apps_lic', 'apps_rg', 'apps_shared', 'artifacts', 'ops_scripts', 'tests')
...
def main() -> int:
    ...
    if '--acknowledge-import-change' in sys.argv:
        ...
        try:
            ...
            for i, entry in enumerate(bl):
                if not (isinstance(entry, list) and len(entry) == 2 and all((isinstance(s, str) for s in entry))):
    ...
    if not isinstance(c_st, MappingProxyType):
        imm_violations.append(f'SOVEREIGN_TERRITORIES: top-level type={type(c_st).__name__}, expected MappingProxyType')
    ...
    print('-' * 40)
    excluded_names = ['SubfolderDefinition', 'TerritoryDefinition', 'build_sovereign_territories', 'LAYER_OVERRIDES', 'get_sovereign_territories', 'get_core_subfolder_map', 'get_subfolder_metadata', 'get_apps_lic_subfolder_map', 'get_apps_rg_subfolder_map', 'get_apps_shared_subfolder_map', 'agentic_core_registry', 'verify_derived_registries', 'L4_SUBFOLDER_MAP', 'L4_APPROVED_FOLDERS', 'SCRIPTS_PLACEMENT_RULES', 'get_app_specific_patterns_compiled', 'get_classification_suffix_patterns_compiled', 'get_compound_suffix_patterns_compiled']
    ...
    for fpath in scan_files:
        ...
        file_counted = False
        for node in ast.walk(tree):
            if not (isinstance(node, ast.ImportFrom) and node.module and any((t in node.module for t in targets))):
                continue
            if not file_counted:
                file_counted = True
                checked += 1
            if node.module in ('structure_blueprint', 'structure_blueprint_config'):
                for a in node.names or []:
                    policy_errors.append(f'{os.path.relpath(fpath, root)}:{node.lineno}:{a.name} (short-path: {node.module})')
                continue
    ...
    if os.path.isfile(baseline_path):
        try:
            ...
            if bad_paths:
                raise ValueError(f'{len(bad_paths)} baseline path(s) not repo-relative-normalized (no backslashes, no absolute, no ..); first: {bad_paths[0]}')
            out_of_scope = [e[0] for e in saved_baseline if not _path_under_scan_roots(e[0])]
            if out_of_scope:
                raise ValueError(f'{len(out_of_scope)} baseline path(s) not under SCAN_ROOTS; first: {out_of_scope[0]}. Valid roots: {SCAN_ROOTS}')
    ...
    forbidden_calls = {'os.getenv', 'os.environ', 'os.getcwd', 'open', 'Path.read_text', 'Path.read_bytes', 'Path.cwd', 'time.time', 'time.monotonic', 'datetime.now', 'datetime.utcnow', 'random.random', 'random.randint', 'random.choice', '__import__', 'importlib.import_module'}
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if top not in ALLOWED_MODULES:
                    allowlist_violations.append(f"line {node.lineno}: 'import {alias.name}' ('{top}' not in allowlist)")
            ...
            if node.level and node.level > 0:
                ...
                if top not in ALLOWED_MODULES:
                    allowlist_violations.append(f"line {node.lineno}: 'from {node.module} import ...' ('{top}' not in allowlist)")
    for node in ast.walk(constants_tree):
    ...
    assert len(debt_rows) == len(phantom_set), f'debt_rows ({len(debt_rows)}) != phantom_set ({len(phantom_set)})'
    debt_lines: list[str] = ['# Phantom Import Debt Register', '', f'Phantom count: {len(debt_rows)}', '', '| Path | Missing Name | Import Line | Suggested Fix |', '| --- | --- | --- | --- |']
    ...
    from agentic_core.L5_safety.config.structure_blueprint.enforcement.types import emit_report_json, make_report
    enforcement_root = _Path(root)
    print('  Building import graph...')
    import_graph = ImportGraph(enforcement_root, SCAN_ROOTS)
    print(f'  Import graph: {import_graph.files_parsed} files parsed, {len(import_graph.parse_errors)} errors')
    ...
    from agentic_core.L5_safety.config.structure_blueprint.enforcement import blueprint_hash, cross_layer, leaf_node, mixin_ast, territory_diff, volatile_rules
    enforcement_results = []
    ...
    print(f"  territory_diff: {len(td_result['violations'])} violation(s)  [{td_stats['territories_checked']} territories checked]")
    ln_result = leaf_node.check(enforcement_root, c_st)
    enforcement_results.append(ln_result)
    ln_stats = ln_result['stats']
    print(f"  leaf_node: {len(ln_result['violations'])} violation(s)  [{ln_stats['territories_checked']} dirs with allow_root_py=False]")
    ...
    print(f"    edges: {cl_stats.get('total_edges', 0)} total, {cl_stats.get('internal_edges', 0)} internal, {cl_stats.get('cross_layer_edges_analyzed', 0)} cross-layer analyzed")
