...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='ssot_sim_') as tmpdir:
        ...
        try:
            _wg.open_write(syntax_err_path, 'from agentic_core.L5_safety.config.structure_blueprint import FAKE\ndef broken(\n')
            rc, out = _run_verify()
        ...
        try:
            ...
            FORBIDDEN = ['--init-phantom-baseline', '--update-phantom-baseline', '--repair-phantom-baseline', '--acknowledge-import-change']
    ...
    artifacts = [('phantom_baseline.json', baseline_path, snap_baseline), ('allowlist_hash.txt', hash_path, snap_hash), ('phantom_debt.md', debt_path, snap_debt)]
    ...
    try:
        git_result = subprocess.run(['git', 'diff', '--exit-code', '--', 'docs/reports/plans/phantom_baseline.json', 'docs/reports/plans/allowlist_hash.txt'], cwd=root, capture_output=True, text=True, timeout=DEFAULT_TIMEOUT)
