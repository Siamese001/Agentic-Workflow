...
def calc_code_quality_score(
    typed_pct: float, documented_pct: float, schema_pct: float, canonical_pct: float
) -> float:
    """
...
def get_canonical_health_score(metrics_dict: dict[str, Any], is_l0: bool = False) -> float:
    ...
    if is_l0:
        ...
        return round(
            ...
            + safe_numeric(metrics_dict.get(FIELD_CYCLOMATIC_COMPLEXITY, 0))
            * WEIGHT_HEALTH_L0_COMPLEXITY,
            2,
        )
...
def get_canonical_code_quality_score(metrics_dict: dict[str, Any]) -> float:
    ...
    return round(
        ...
        + safe_numeric(metrics_dict.get(FIELD_SCHEMA_STRICTNESS, 0))
        * WEIGHT_CODE_QUALITY_SCHEMA_STRICTNESS
        + safe_numeric(metrics_dict.get(FIELD_PROPER_BASE_CLASS, 0))
        * WEIGHT_CODE_QUALITY_CANONICAL_INHERITANCE,
        2,
    )
