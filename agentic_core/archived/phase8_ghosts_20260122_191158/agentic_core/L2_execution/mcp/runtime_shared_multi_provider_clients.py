...
def get_api_key(Provider: Provider) -> str:
    ...
    if not api_key:
        raise ValueError(
            f"API key for {Provider.value} not set. Please set {env_var} environment variable."
        )
    return api_key
...
def get_client(
    Provider: Provider, config: ProviderConfig | None = None, force_new: bool = False
) -> Any:
