...
class YamlInjectionLoader:
    ...
    # Layer mapping from YAML directory names to InjectionLayer enum
    LAYER_MAPPING = {
    ...
    def enumerate_yaml_files(self) -> list[Path]:
        ...
        # Find all YAML files and sort for deterministic ordering
    ...
    def load_all_patterns(self) -> dict[str, list[InstructionalPattern]]:
        ...
        # Initialize all layers as empty lists
        ...
        # Sort patterns within each layer by ID for deterministic ordering
    ...
    def _load_yaml_file(self, yaml_file: Path) -> list[InstructionalPattern]:
        ...
        # Handle both v5_* files and generic structure
    ...
    def _extract_patterns_from_dict(
        ...
        for pattern_name in sorted_pattern_names:
            ...
            if not isinstance(pattern_data, dict):
                continue  # Skip non-dict entries like "enabled", "injection_count"

            # Check for required keys - be more tolerant of different structures
            has_description = isinstance(pattern_data.get("description"), str)
            has_template = isinstance(pattern_data.get("prompt_template"), str)

            # Skip patterns that don't have the basic required structure
    ...
    def _determine_layer_from_path(self, yaml_file: Path) -> str:
        ...
        """
        # Extract layer from path (e.g., .../modular/framing/... -> FRAMING)
        ...
        # Try to determine from filename for test files
        ...
        # Default to FRAMING if layer cannot be determined
        logger.warning(f"Could not determine layer for {yaml_file}, defaulting to FRAMING")
        return InjectionLayer.FRAMING.value


# Global instance for deterministic behavior
