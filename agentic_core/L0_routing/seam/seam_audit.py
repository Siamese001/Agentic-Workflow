...
class SeamAuditLogger:
    ...
    def get_records(self, seam_id: Optional[str] = None) -> List[SeamAuditRecord]:
        """Get audit records, optionally filtered by seam_id."""
        if seam_id is None:
            return self._records.copy()
        return [r for r in self._records if r.seam_id == seam_id]
    
    def get_digest(self, seam_id: Optional[str] = None) -> str:
...
def get_seam_audit_digest(seam_id: Optional[str] = None) -> str:
