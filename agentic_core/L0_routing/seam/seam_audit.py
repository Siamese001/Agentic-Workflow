...
class SeamAuditLogger:
    ...
    def log_seam_operation(self, seam_id: str, operation: str, inputs: Any, outputs: Any, layer_source: str, layer_target: str, caller_id: str | None=None, metadata: dict[str, Any] | None=None) -> SeamAuditRecord:
        ...
        invocation_data = {'seam_id': seam_id, 'operation': operation, 'inputs_hash': inputs_hash, 'outputs_hash': outputs_hash, 'layer_source': layer_source, 'layer_target': layer_target, 'caller_id': caller_id, 'metadata': metadata or {}}
        invocation_hash = self._compute_hash(invocation_data)
        record = SeamAuditRecord(seam_id=seam_id, operation=operation, inputs_hash=inputs_hash, outputs_hash=outputs_hash, invocation_hash=invocation_hash, timestamp=time.time(), layer_source=layer_source, layer_target=layer_target, caller_id=caller_id, metadata=metadata or {})
...
def seam_audit_hook(seam_id: str, operation: str, layer_source: str, layer_target: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            logger.log_seam_operation(seam_id=seam_id, operation=operation, inputs=inputs, outputs=outputs, layer_source=layer_source, layer_target=layer_target, caller_id=caller_id)
            return outputs
        return wrapper
    return decorator

def log_seam_operation(seam_id: str, operation: str, inputs: Any, outputs: Any, layer_source: str, layer_target: str, caller_id: str | None=None, metadata: dict[str, Any] | None=None) -> SeamAuditRecord | None:
    """Log a seam operation manually."""
    logger = get_seam_audit_logger()
    return logger.log_seam_operation(seam_id=seam_id, operation=operation, inputs=inputs, outputs=outputs, layer_source=layer_source, layer_target=layer_target, caller_id=caller_id, metadata=metadata)
