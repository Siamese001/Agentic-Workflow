        ...
        try:
            ...
            if self.compliance_violations:
                if self.strict_mode:
                    ...
                    boot_result["status"] = (
                        "failed"  # review: SystemExit should be handled with specific context
                    )
                    ...
                    logger.warning(
                        f"⚠️  Continuing with {len(self.compliance_violations)} compliance violations.",  # review: SystemExit should be handled with specific context
                    )
            else:
