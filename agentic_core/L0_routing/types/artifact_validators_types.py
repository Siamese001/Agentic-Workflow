...
def _to_raw_dict(obj: object) -> dict[str, Any]:
    ...
    raise TypeError(f'VALIDATE_ARTIFACT:UNSUPPORTED_TYPE|type={type(obj).__name__}|expected=dict_or_dataclass')

def _require_str(d: dict[str, Any], key: str, artifact_name: str) -> None:
    """Require a non-empty string field."""
    val = d.get(key)
    if not isinstance(val, str) or not val:
        raise ValueError(f'VALIDATE_{artifact_name}:MISSING_OR_INVALID_FIELD|field={key}|value={val!r}|expected=non_empty_str')
...
def _require_int(d: dict[str, Any], key: str, artifact_name: str, *, min_val: int | None=None) -> None:
    ...
    if not isinstance(val, int):
        raise ValueError(f'VALIDATE_{artifact_name}:MISSING_OR_INVALID_FIELD|field={key}|value={val!r}|expected=int')
...
def _require_sequence_of_str(d: dict[str, Any], key: str, artifact_name: str) -> None:
    ...
    if not isinstance(val, (list, tuple)):
        raise ValueError(f'VALIDATE_{artifact_name}:MISSING_OR_INVALID_FIELD|field={key}|value={val!r}|expected=sequence_of_str')
    for i, item in enumerate(val):
        if not isinstance(item, str):
            raise ValueError(f'VALIDATE_{artifact_name}:INVALID_ELEMENT|field={key}[{i}]|value={item!r}|expected=str')
...
def validate_result_artifact(obj: object) -> dict[str, Any]:
    ...
    return {'trace_id': d['trace_id'], 'execution_outcome': d['execution_outcome'], 'final_state_hash': d['final_state_hash'], 'artifact_class': d['artifact_class'], 'emitting_layer': d['emitting_layer']}
...
def validate_healing_plan(obj: object) -> dict[str, Any]:
    ...
    return {'trace_id': d['trace_id'], 'plan_id': d['plan_id'], 'manifests': d['manifests'], 'semantic_clock_tick': d['semantic_clock_tick'], 'policy_liaison_node': d['policy_liaison_node'], 'emitting_layer': d['emitting_layer']}
...
def validate_incident_artifact(obj: object) -> dict[str, Any]:
    ...
    return {'trace_id': d['trace_id'], 'incident_id': d['incident_id'], 'correlation_hash': d['correlation_hash'], 'severity_enum': d['severity_enum'], 'telemetry_events': d['telemetry_events']}
...
def validate_stale_write_incident(obj: object) -> dict[str, Any]:
    ...
    return {'trace_id': d['trace_id'], 'target_path': d['target_path'], 'expected_hash': d['expected_hash'], 'actual_hash': d['actual_hash'], 'semantic_clock_tick': d['semantic_clock_tick']}
...
__all__ = ['make_healing_plan_from_dataclass', 'make_result_artifact_from_dataclass', 'to_healing_plan_dict', 'to_incident_artifact_dict', 'to_result_artifact_dict', 'to_stale_write_incident_dict', 'validate_healing_plan', 'validate_incident_artifact', 'validate_result_artifact', 'validate_stale_write_incident']
