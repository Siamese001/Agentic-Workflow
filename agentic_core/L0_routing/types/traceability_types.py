...
def validate_trace_id(trace_id: str) -> str:
    ...
    if not TRACE_ID_PATTERN.match(trace_id):
        raise ValueError(f"FAIL (P4): Trace ID '{trace_id}' does not match required pattern ^CC3AL1-[0-9A-F]{{8}}$")
    return trace_id
...
class ErrorSignature:
    ...
    def __post_init__(self) -> None:
        ...
        if self.signature_hash != expected:
            raise ValueError(f'ErrorSignature: signature_hash mismatch. Expected {expected}, got {self.signature_hash}')
...
class PolicyConfigPin:
    ...
    def __post_init__(self) -> None:
        ...
        if self.semantic_clock_tick < 0:
            raise ValueError(f'PolicyConfigPin: semantic_clock_tick must be >= 0, got {self.semantic_clock_tick}')
...
class PlanProvenance:
    ...
    def __post_init__(self) -> None:
        ...
        if self.semantic_clock_tick < 0:
            raise ValueError(f'PlanProvenance: semantic_clock_tick must be >= 0, got {self.semantic_clock_tick}')
...
class RetrievalQuery:
    ...
    def __post_init__(self) -> None:
        ...
        if self.query_hash != expected:
            raise ValueError(f'RetrievalQuery: query_hash mismatch. Expected {expected}, got {self.query_hash}')
        if not self.source_agent:
            raise ValueError('RetrievalQuery: source_agent must be non-empty')
        if self.semantic_clock_tick < 0:
            raise ValueError(f'RetrievalQuery: semantic_clock_tick must be >= 0, got {self.semantic_clock_tick}')
...
class RetrievedChunk:
    ...
    def __post_init__(self) -> None:
        ...
        if self.content_hash != expected:
            raise ValueError(f'RetrievedChunk: content_hash mismatch. Expected {expected}, got {self.content_hash}')
...
class CognitiveDiffBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if self.semantic_clock_tick < 0:
            raise ValueError(f'CognitiveDiffBundle: semantic_clock_tick must be >= 0, got {self.semantic_clock_tick}')
...
class KnowledgeAdvisoryConstraint:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.directive_type, KnowledgeDirective):
            raise TypeError(f'KnowledgeAdvisoryConstraint: directive_type must be KnowledgeDirective, got {type(self.directive_type).__name__}')
        if not self.content:
...
__all__ = ['TRACE_ID_PATTERN', 'CitationBundle', 'CitationEntry', 'CognitiveDiffBundle', 'ErrorSignature', 'KnowledgeAdvisoryConstraint', 'KnowledgeDirective', 'PlanProvenance', 'PolicyConfigPin', 'RerankScore', 'RetrievalQuery', 'RetrievedChunk', 'compute_error_signature_hash', 'validate_trace_id']
