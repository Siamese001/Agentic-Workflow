...
class ReasoningIntensityProfile:
    ...
    def __post_init__(self) -> None:
        ...
        expected = _compute_profile_hash(version=self.reasoning_profile_version, policy_hash=self.reasoning_policy_hash, tier=self.tier.value, max_branches=self.max_branches, max_depth=self.max_depth, enable_reflection=self.enable_reflection, token_budget_per_stage=[{'stage_id': b.stage_id, 'max_tokens': b.max_tokens} for b in self.token_budget_per_stage], allowed_modes=sorted(self.allowed_modes))
        if self.profile_hash != expected:
            raise ValueError(f'ReasoningIntensityProfile: profile_hash mismatch. Expected {expected[:16]}..., got {self.profile_hash[:16]}...')
...
class SignedExecutionEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        expected = _compute_envelope_hash(route_decision_trace_id=self.route_decision.trace_id, profile_hash=self.reasoning_profile.profile_hash, policy_hash=self.policy_hash)
        if self.envelope_hash != expected:
            raise ValueError(f'SignedExecutionEnvelope: envelope_hash mismatch. Expected {expected[:16]}..., got {self.envelope_hash[:16]}...')
...
def _compute_profile_hash(version: str, policy_hash: str, tier: str, max_branches: int, max_depth: int, enable_reflection: bool, token_budget_per_stage: list[dict[str, int]], allowed_modes: list[str]) -> str:
    """Compute SHA256 over deterministic canonical serialization of profile parameters."""
    canonical = json.dumps({'version': version, 'policy_hash': policy_hash, 'tier': tier, 'max_branches': max_branches, 'max_depth': max_depth, 'enable_reflection': enable_reflection, 'token_budget_per_stage': sorted(token_budget_per_stage, key=lambda x: x['stage_id']), 'allowed_modes': sorted(allowed_modes)}, sort_keys=True, separators=(',', ':'))
...
def _compute_envelope_hash(route_decision_trace_id: str, profile_hash: str, policy_hash: str) -> str:
    ...
    canonical = json.dumps({'route_decision_trace_id': route_decision_trace_id, 'profile_hash': profile_hash, 'policy_hash': policy_hash}, sort_keys=True, separators=(',', ':'))
    return hashlib.sha256(canonical.encode('utf-8')).hexdigest()

def build_profile_hash(version: str, policy_hash: str, tier: ReasoningTier, max_branches: int, max_depth: int, enable_reflection: bool, token_budget_per_stage: list[StageTokenBudget], allowed_modes: list[str]) -> str:
    """Compute the profile_hash for use before constructing ReasoningIntensityProfile."""
    return _compute_profile_hash(version=version, policy_hash=policy_hash, tier=tier.value, max_branches=max_branches, max_depth=max_depth, enable_reflection=enable_reflection, token_budget_per_stage=[{'stage_id': b.stage_id, 'max_tokens': b.max_tokens} for b in token_budget_per_stage], allowed_modes=sorted(allowed_modes))
...
def build_envelope_hash(route_decision_trace_id: str, profile_hash: str, policy_hash: str) -> str:
    ...
    return _compute_envelope_hash(route_decision_trace_id=route_decision_trace_id, profile_hash=profile_hash, policy_hash=policy_hash)
TIER_PARAMETER_TABLE: dict[ReasoningTier, dict[str, Any]] = {ReasoningTier.LOW: {'max_branches': 1, 'max_depth': 1, 'enable_reflection': False, 'allowed_modes': ['cot'], 'token_budget_multiplier': 0.5}, ReasoningTier.MEDIUM: {'max_branches': 2, 'max_depth': 2, 'enable_reflection': False, 'allowed_modes': ['cot', 'hybrid_cot_tot'], 'token_budget_multiplier': 1.0}, ReasoningTier.HIGH: {'max_branches': 3, 'max_depth': 3, 'enable_reflection': True, 'allowed_modes': ['cot', 'hybrid_cot_tot', 'tot'], 'token_budget_multiplier': 1.5}, ReasoningTier.CRITICAL: {'max_branches': 5, 'max_depth': 5, 'enable_reflection': True, 'allowed_modes': ['cot', 'hybrid_cot_tot', 'tot', 'reflexion'], 'token_budget_multiplier': 2.0}}
__all__ = ['TIER_PARAMETER_TABLE', 'ReasoningConstraintViolation', 'ReasoningEnforcementTelemetry', 'ReasoningIntensityProfile', 'ReasoningTier', 'SignedExecutionEnvelope', 'StageTokenBudget', 'build_envelope_hash', 'build_profile_hash']
