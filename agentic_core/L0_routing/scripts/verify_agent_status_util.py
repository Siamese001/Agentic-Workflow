...
SUSPECT_FILES = ['agentic_core/L0_routing/scripts/full_agent_discovery.py', 'agentic_core/L0_routing/scripts/auto_remediate_signatures.py', 'agentic_core/L2_execution/pinecone_mcp_client.py', 'agentic_core/L2_execution/caching_redis_mcp_client.py', 'agentic_core/L5_safety/ArchivalGatekeeper.py', 'agentic_core/L5_safety/validators/context.py', 'agentic_core/L5_safety/validators/constants.py', 'agentic_core/L6_observability/reasoning_utils.py', 'agentic_core/utils/core_extensions/infrastructure_mixin.py', 'agentic_core/utils/core_extensions/healer_mixin.py']
SOVEREIGN_BASES = {'SovereignBaseAgent', 'L0RoutingBaseAgent', 'L1CognitionBase', 'L2ExecutionBase', 'L3OrchestrationBase', 'L4StateBase', 'L5SafetyBase', 'L6ObservabilityBase'}
LAYER_BASES = SOVEREIGN_BASES | {'HealerMixin', 'MCPHardenedMixin', 'CanonBaseAgent', 'CognitionCanonBaseAgent', 'ExecutionCanonBaseAgent'}
...
def analyze_file(file_path: Path) -> dict[str, Any]:
    ...
    result = {'file': str(file_path.relative_to(PROJECT_ROOT)), 'exists': file_path.exists(), 'is_script': False, 'classes': [], 'verdict': 'UNKNOWN', 'reason': ''}
    ...
    for cls in classes:
        ...
        class_info = {'name': cls.name, 'ends_with_agent': cls.name.endswith('Agent'), 'bases': list(bases), 'inherits_sovereign': bool(bases & SOVEREIGN_BASES), 'inherits_layer_base': bool(bases & LAYER_BASES), 'has_heal_repository': has_method(cls, 'heal_repository'), 'is_mixin': 'Mixin' in cls.name, 'is_dataclass': any((isinstance(d, ast.Name) and d.id == 'dataclass' or (isinstance(d, ast.Call) and isinstance(d.func, ast.Name) and (d.func.id == 'dataclass')) for d in cls.decorator_list))}
        is_sovereign = class_info['ends_with_agent'] and (class_info['inherits_sovereign'] or class_info['inherits_layer_base']) and (not class_info['is_mixin'])
    ...
    if sovereign_classes:
        ...
        result['reason'] = f"Has Agent suffix but no sovereign inheritance: {[c['name'] for c in agent_classes]}"
    else:
        result['verdict'] = 'NOT_AGENT'
