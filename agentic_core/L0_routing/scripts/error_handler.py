...
from .execution import STRATEGY_REGISTRY, ExecutionStatus, ExecutionStrategy, WorkflowContext, WorkflowResult, WorkflowStep, get_strategy
...
class ErrorHandler:
    ...
    def __init__(self):
        ...
        self.recovery_strategies = {'retry': self._retry_strategy, 'fallback': self._fallback_strategy, 'skip': self._skip_strategy, 'abort': self._abort_strategy}
        # guardian: allow-magic-config
        self.max_retries = 3

    async def handle_error(self, error: Exception, context: WorkflowContext, recovery_type: str='retry') -> WorkflowResult:
    ...
    async def _retry_strategy(self, error: Exception, context: WorkflowContext) -> WorkflowResult:
        ...
        if retries < self.max_retries:
            ...
            return WorkflowResult(workflow_id=context.workflow_id, status=ExecutionStatus.PENDING, error=f'Retry {retries + 1}/{self.max_retries}: {str(error)}')
    ...
    async def _fallback_strategy(self, error: Exception, context: WorkflowContext) -> WorkflowResult:
        """Use fallback logic."""
        return WorkflowResult(workflow_id=context.workflow_id, status=ExecutionStatus.COMPLETED, output={'fallback': True, 'original_error': str(error)}, error=f'Fallback used: {str(error)}')

    async def _skip_strategy(self, error: Exception, context: WorkflowContext) -> WorkflowResult:
        """Skip failed step."""
        return WorkflowResult(workflow_id=context.workflow_id, status=ExecutionStatus.COMPLETED, output={'skipped': True}, error=f'Skipped: {str(error)}')

    async def _abort_strategy(self, error: Exception, context: WorkflowContext) -> WorkflowResult:
        """Abort workflow."""
        return WorkflowResult(workflow_id=context.workflow_id, status=ExecutionStatus.FAILED, error=str(error))
...
class UnifiedWorkflowEngine:
    ...
    async def execute(self, workflow_type: str, input_data: dict[str, Any], steps: list[WorkflowStep] | None=None, metadata: dict[str, Any] | None=None) -> WorkflowResult:
        """
        ...
        context = WorkflowContext(workflow_id=workflow_id, workflow_type=workflow_type, input_data=input_data, metadata=metadata or {})
        ...
        try:
            ...
            if coordinator:
                ...
                if steps:
                    ...
                    result = WorkflowResult(workflow_id=workflow_id, status=ExecutionStatus.COMPLETED, output={'message': 'No steps provided'})
    ...
    async def execute_with_coordinator(self, coordinator_name: str, input_data: dict[str, Any], metadata: dict[str, Any] | None=None) -> WorkflowResult:
        """
        ...
        if not coordinator:
            return WorkflowResult(workflow_id=str(uuid.uuid4()), status=ExecutionStatus.FAILED, error=f'Coordinator not found: {coordinator_name}')
        context = WorkflowContext(workflow_id=str(uuid.uuid4()), workflow_type=coordinator_name, input_data=input_data, metadata=metadata or {})
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'metrics': {'total_workflows': self.metrics.total_workflows, 'completed_workflows': self.metrics.completed_workflows, 'failed_workflows': self.metrics.failed_workflows, 'success_rate': self.metrics.completed_workflows / self.metrics.total_workflows * 100 if self.metrics.total_workflows > 0 else 0, 'total_time': self.metrics.total_time, 'avg_latency': self.metrics.avg_latency}, 'active_workflows': len(self.active_workflows), 'strategies': list(self.strategies.keys()), 'coordinators': self.coordinator_registry.get_statistics()}
