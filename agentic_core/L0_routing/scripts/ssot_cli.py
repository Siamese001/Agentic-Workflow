...
def cmd_validate(args) -> int:
    ...
    if args.json:
        json_data = {'compliance_score': report.compliance_score, 'total_violations': report.total_violations, 'is_compliant': report.is_compliant, 'scan_duration': report.scan_duration, 'violations': {'gravity': len(report.gravity_violations), 'imports': len(report.import_violations), 'hierarchy': len(report.hierarchy_violations), 'drift': len(report.drift_violations)}}
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='ssot', description='SSOT - Sovereign Single Source of Truth CLI', epilog='For more information on a command, use: python scripts/ssot_util.py <command> --help')
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    scan_parser = subparsers.add_parser('scan', help='Scan and list all discovered agents')
    scan_parser.add_argument('--summary', action='store_true', help='Show summary by layer instead of full listing')
    scan_parser.add_argument('--violations-only', action='store_true', help='Show only agents with gravity violations')
    ...
    validate_parser.add_argument('--summary', action='store_true', help='Show brief summary instead of full report')
    ...
    enforce_parser.add_argument('--execute', action='store_true', help='Execute enforcement (default is dry-run)')
    ...
    enforce_parser.add_argument('--all', action='store_true', default=True, help='Fix all violations (default)')
