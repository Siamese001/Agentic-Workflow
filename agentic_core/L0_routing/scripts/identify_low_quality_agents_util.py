...
def main():
    ...
    for agent in agents:
        ...
        agent_scores.append({'name': agent.get('class_name'), 'path': agent.get('path'), 'typed_pct': agent.get('typed_pct', 0), 'documented_pct': agent.get('documented_pct', 0), 'schema_strictness': agent.get('schema_strictness', 0), 'quality_score': score})
    ...
    for batch_num, i in enumerate(range(0, min(30, len(needs_work)), batch_size), 1):
        ...
        for agent in batch:
            ...
            print(f"  Typed: {agent['typed_pct']:.0f}% | Doc: {agent['documented_pct']:.0f}% | schema: {agent['schema_strictness']:.0f}%")
