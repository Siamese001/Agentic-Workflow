...
try:
    from agentic_core.utils.terminal_colors import Colors, agent_status, heartbeat, log_status, mission_header, mission_summary, phase_header, progress_bar, tier_summary
...
_runtime_state = {'status': 'idle', 'start_time': None, 'end_time': None, 'current_agent': None, 'current_layer': None, 'agents_order': [], 'total_agents': 0, 'completed_agents': [], 'events': [], 'meta_learning': {'enabled': False, 'total_experiences': 0, 'patterns_extracted': 0, 'strategy_weights': {'cot': 1.0, 'tot': 1.0, 'react': 1.0, 'reflection': 1.0}, 'recent_experiences': [], 'pattern_history': []}, 'redis': {'connected': False, 'operations': {'get': 0, 'set': 0, 'delete': 0, 'total': 0}, 'cache_hits': 0, 'cache_misses': 0, 'hit_rate': 0.0, 'recent_operations': []}, 'pinecone': {'connected': False, 'operations': {'upsert': 0, 'query': 0, 'delete': 0, 'total': 0}, 'vectors_stored': 0, 'avg_similarity': 0.0, 'recent_queries': []}, 'execution_timeline': []}
...
def _add_event(event_type: str, message: str):
    ...
    _runtime_state['events'].append({'time': datetime.now().isoformat(), 'type': event_type, 'message': message})
...
def _update_meta_learning_state(experience_data: dict):
    ...
    if 'pattern' in experience_data:
        ml['pattern_history'].append({'pattern': experience_data['pattern'], 'timestamp': datetime.now().isoformat()})
...
def _update_redis_state(operation: str, key: str, hit: bool=None):
    ...
    redis['recent_operations'].insert(0, {'operation': operation, 'key': key, 'hit': hit, 'timestamp': datetime.now().isoformat()})
...
def _update_pinecone_state(operation: str, metadata: dict=None):
    ...
    if metadata:
        ...
        if 'similarity' in metadata:
            ...
            if total_queries > 0:
                pc['avg_similarity'] = (pc['avg_similarity'] * (total_queries - 1) + metadata['similarity']) / total_queries
        if operation == 'query':
            pc['recent_queries'].insert(0, {'results': metadata.get('results', []), 'top_k': metadata.get('top_k', 0), 'avg_score': metadata.get('similarity', 0), 'timestamp': datetime.now().isoformat()})
...
def _update_agent_execution(agent_name: str, layer: str, start_time: float, end_time: float, success: bool):
    ...
    _runtime_state['execution_timeline'].append({'agent': agent_name, 'layer': layer, 'start': start_time, 'end': end_time, 'duration': end_time - start_time, 'success': success})
AGENT_LAYERS = {'NamingAgent': 'L5 – Safety & Governance', 'AutonomyGuardian': 'L5 – Safety & Governance', 'LocationAgent': 'L5 – Safety & Governance', 'HierarchyAgent': 'L5 – Safety & Governance', 'StructuralHealerAgent': 'L5 – Safety & Governance', 'ComplianceOrchestratorAgent': 'L5 – Safety & Governance', 'AutonomyGuardianAgent': 'L5 – Safety & Governance', 'ImportAgent': 'L5 – Safety & Governance', 'CodeDeduplicationAgent': 'L5 – Safety & Governance', 'CodeValidatorAgent': 'L5 – Safety & Governance', 'StructureEnforcerAgent': 'L5 – Safety & Governance', 'GitHygieneAgent': 'L5 – Safety & Governance', 'FileCleanupAgent': 'L5 – Safety & Governance', 'CodeJanitorAgent': 'L5 – Safety & Governance', 'CanonDependencySentinelAgent': 'L5 – Safety & Governance', 'HygieneGuardianAgent': 'L5 – Safety & Governance', 'StructuralEngineerAgent': 'L2 – Execution & Tools', 'GovernanceAgent': 'L1 – Cognition & Intelligence', 'DocumentationAgent': 'L1 – Cognition & Intelligence', 'CheckpointManagerAgent': 'L4 – State & Memory', 'PerformanceAnalystAgent': 'L6 – observability & Metrics', 'BootstrapAgent': 'L0 – Maintenance & Infrastructure', 'FilesystemSSOTReconcilerAgent': 'L0 – Maintenance & Infrastructure'}
...
sovereign_paths = [project_root / AGENTIC_CORE_DIR / 'runtime' / 'shared_runtime', project_root / APPS_SHARED_DIR / 'utils']
...
def main():
    ...
    parser.add_argument('--target', type=str, default='.', help='Target folder for validation (default: entire repo)')
    parser.add_argument('--reset', action='store_true', help='Reset sovereign state before validation')
    parser.add_argument('--heal', action='store_true', help='Run autonomous domain healing (dry-run by default)')
    parser.add_argument('--execute-heal', action='store_true', help='Execute heal changes (use with --heal)')
    parser.add_argument('--agent', type=str, help="Run a specific agent directly (e.g., 'naming', 'location', 'hierarchy', 'filesystem', 'governance', 'guardian')")
    parser.add_argument('--execute', action='store_true', help='Execute changes (use with --agent, same as --execute-heal)')
    parser.add_argument('--list-agents', action='store_true', help='List all discoverable agents')
    parser.add_argument('--yes', '-y', action='store_true', help='Automatic yes to all prompts (non-interactive mode)')
    parser.add_argument('--report', action='store_true', help='Run autonomy compliance report')
    parser.add_argument('--method', type=str, default='heal_repository', help='Agent method to invoke (default: heal_repository)')
    parser.add_argument('--tier', type=int, choices=[0, 1, 2, 3, 4], default=None, help='Run specific healing tier only (0=Pre-Flight, 1=Structural, 2=Architectural, 3=Dynamic, 4=Final Gate)')
    ...
    parser.add_argument('--preflight-only', action='store_true', help='Run mandatory preflight checks only (syntax, imports, location)')
    ...
    if args.preflight_only or args.hygiene or args.full_hygiene:
        ...
        if args.preflight_only:
            ...
            print(f'   [MODE] {mode_str}')
            strategy = HealingStrategy(project_root=project_root)
            strategy._tiers = {'Tier 0: Pre-Flight': CORE_HYGIENE_AGENTS['tier_0_preflight'], 'Tier 1: Structural': CORE_HYGIENE_AGENTS['tier_1_structural']}
        elif args.full_hygiene:
            print('\n[*] FULL HYGIENE MODE - Running Tier 0-3 agents')
            print(f'   [MODE] {mode_str}')
            strategy = HealingStrategy(project_root=project_root)
            strategy._tiers = {'Tier 0: Pre-Flight': CORE_HYGIENE_AGENTS['tier_0_preflight'], 'Tier 1: Structural': CORE_HYGIENE_AGENTS['tier_1_structural'], 'Tier 2: Architectural': CORE_HYGIENE_AGENTS['tier_2_architectural'], 'Tier 3: Autonomy': CORE_HYGIENE_AGENTS['tier_3_autonomy']}
    ...
    if args.heal:
        ...
        print(f'   [MODE] {mode_str}')
        try:
            from agentic_core.L0_routing.seams.safety_validators_seam import load_healing_strategy as _load_hs2
            Orchestrator = _get_orchestrator_class()
            ...
            def get_performance_analyst_safe(root):
                try:
                    ...
                    spec = importlib.util.find_spec('agentic_core.L6_observability.engines.PerformanceAnalystAgentSimple')
                    if spec:
            ...
            orchestrator = Orchestrator(strategy=strategy, project_root=project_root, name='SovereignHealOrchestrator')
            checkpoint_manager = get_checkpoint_manager(project_root)
            performance_analyst = get_performance_analyst_safe(project_root)
            mission_context = {'dry_run': not execute_heal, 'execute': execute_heal, 'checkpoint_manager': checkpoint_manager, 'performance_analyst': performance_analyst, 'scan_mode': 'unified_sovereign_sweep'}
            ...
            gemini_active = False
            try:
                from agentic_core.L0_routing.seams.safety_validators_seam import load_autonomy_guardian as _load_ag2
            ...
            # guardian: allow-silent-swallow
                ...
                pass
            _runtime_state.update({'status': 'healing', 'start_time': datetime.now().isoformat(), 'agents_order': all_agent_names, 'total_agents': len(all_agent_names), 'completed_agents': [], 'events': [], 'execution_timeline': []})
            ...
            for i, agent_result in enumerate(results.get('agent_results', [])):
                _runtime_state['execution_timeline'].append({'agent': agent_result.get('agent_name', f'agent_{i}'), 'status': agent_result.get('status', 'UNKNOWN'), 'fixes': agent_result.get('violations_fixed', 0), 'violations': agent_result.get('violations_found', 0), 'duration_ms': agent_result.get('execution_time_ms', 0)})
            _save_runtime_state(project_root)
            ...
            consolidated_results = [{'domain': 'Sovereign Repository', 'agents_run': agents_run, 'total_fixed': total_fixes, 'total_violations': total_violations, 'compliance_score': 100 if total_fixes + total_violations == 0 else int((1 - total_violations / max(total_fixes + total_violations, 1)) * 100)}]
...
def report_consolidated_summary(results, gemini_active):
    ...
    print('=' * 60)
    total_summary = {'agents_run': 0, 'total_renamed': 0, 'total_errors': 0, 'total_fixed': 0, 'total_violations': 0}
