...
def extract_components(html_content, name):
    ...
    components = {'tabs': [], 'kpi_boxes': [], 'chart_cards': [], 'tables': [], 'filters': [], 'modals': [], 'footnotes': [], 'functions': [], 'data_files': []}
    ...
    kpi_matches = re.findall('class="kpi-box[^"]*"[^>]*>.*?<div class="kpi-label">([^<]+)</div>', html_content, re.DOTALL)
    ...
    filter_matches = re.findall('checkbox[^>]*>([^<]+)</label>|checkbox[^>]*>\\s*<[^>]*>([^<]+)<', html_content, re.DOTALL)
...
def compare_components():
    ...
    with open(mod_path, encoding='utf-8') as f:
        mod_html = f.read()
    js_content = ''
    js_files = ['agentic_core/L6_observability/dashboards/js/renderers/table-renderer.js', 'agentic_core/L6_observability/dashboards/js/main.js']
    ...
    categories = [('tabs', 'Navigation Tabs'), ('kpi_boxes', 'KPI Boxes'), ('chart_cards', 'Chart Cards'), ('filters', 'Filter Controls'), ('modals', 'Modals'), ('footnotes', 'Footnotes & Legends'), ('data_files', 'Data Files'), ('functions', 'JavaScript Functions')]
    all_issues = []
    ...
    print(f"{'=' * 50}")
    critical_features = [('renderTerritorySummaryTable', 'Territory Summary Table Renderer'), ('renderCodeQualityTable', 'Code Quality Table Renderer'), ('openDrillModal', 'Drill-down Modal'), ('toggleFilter', 'Filter Toggle Function'), ('toggleToxicityFilter', 'Toxicity Filter'), ('toggleZombieFilter', 'Zombie Filter'), ('toggleOutlierFilter', 'Outlier Filter'), ('loadData', 'Data Loading Function'), ('openTab', 'Tab Navigation'), ('manualRefresh', 'Manual Refresh'), ('Factory analogy', 'Factory Analogies in Footnotes'), ('Icon Legend', 'Icon Legend'), ('Heal Capability %:', 'Heal Capability Definition'), ('Health Score:', 'Health Score Formula'), ('drillModal', 'Drill-down Modal Element')]
