...
def generate_markdown_report(live_cap_counter: Counter, dead_cap_detail: dict, unique_to_dead: set, underrepresented: dict, recommendations: list) -> str:
    ...
    if unique_to_dead:
        ...
        for cap in sorted(unique_to_dead):
            donors = [name for name, detail in dead_cap_detail.items() if cap in detail['caps']['semantic_tags'] or cap in detail['caps']['patterns']]
...
def analyze_supplementation():
    ...
    for agent_meta in report['agents']:
        ...
        try:
            ...
            class_node = next((n for n in ast.walk(tree) if isinstance(n, ast.ClassDef) and n.name == agent_name))
        # guardian: allow-silent-swallow
    ...
    md_report = generate_markdown_report(live_cap_counter, dead_cap_detail, unique_to_dead, underrepresented, recommendations)
