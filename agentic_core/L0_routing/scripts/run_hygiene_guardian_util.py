    ...
    for root_folder in ALLOWED_ROOT_FOLDERS:
        ...
        for dirpath, _dirnames, _filenames in os.walk(root_path, topdown=False):
            ...
            try:
                ...
                meaningful_children = [x for x in children if x.name not in IGNORE_FILES and (not x.name.startswith('.'))]
...
def main():
    ...
    return {'before': {'artifacts': len(artifacts), 'empty_folders': len(empty_folders), 'init_only': len(init_only)}, 'fixed': {'artifacts_removed': artifacts_removed, 'folders_removed': folders_removed}, 'after': {'artifacts': len(remaining_artifacts), 'empty_folders': len(remaining_empty), 'init_only': len(remaining_init_only)}, 'artifacts_list': [str(a.relative_to(project_root)) for a in artifacts], 'empty_folders_list': [str(f.relative_to(project_root)) for f in empty_folders], 'init_only_list': [str(f.relative_to(project_root)) for f in init_only], 'remaining_empty': [str(f.relative_to(project_root)) for f in remaining_empty]}
