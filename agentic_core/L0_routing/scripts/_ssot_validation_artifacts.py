...
def _write_artifact_integrity_json(trace_id: str, output_dir: Path) -> None:
    ...
    for artifact_path in output_dir.glob("*.json"):
        ...
        try:
            ...
            artifacts[artifact_path.name] = {"sha256": sha256_hash, "size_bytes": len(content)}    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling    # guardian: File operations with encoding need error-specific handling
