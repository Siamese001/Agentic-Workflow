...
try:
    from agentic_core.L0_routing.config import AGENTIC_CORE_DIR, APPS_LIC_DIR, APPS_RG_DIR, APPS_SHARED_DIR, FORENSIC_DISCOVERY_INTEGRITY_HASH, FORENSIC_DISCOVERY_SCRIPT
...
def build_class_bases_map(project_root: Path) -> dict[str, list[str]]:
    ...
    scan_roots = [project_root / AGENTIC_CORE_DIR, project_root / APPS_LIC_DIR, project_root / APPS_RG_DIR, project_root / APPS_SHARED_DIR]
...
def resolve_full_mro(direct_bases: list[str], class_map: dict[str, list[str]], _seen: set[str] | None=None) -> list[str]:
...
def stub_sentinel_detected(content: str) -> bool:
    ...
    return 'STATUS: STUB' in head or ('__AGENT_STATUS__' in head and 'STUB' in head) or any((line.strip() == 'NOT_AN_AGENT' for line in head.splitlines()))

def forensic_inspect(name: str, layer: str, file_path: Path) -> ForensicAgentRecord:
    """
    Analyzes a file to build the Forensic Record.
    """
    record = ForensicAgentRecord(agent_name=name, layer=layer, file_path=str(file_path), class_name='Unknown', mro_signature=[], status='INVALID', methods_detected=[])
...
def get_git_commit(root: Path) -> str:
    try:
        out = _get_safe_subprocess_check_output()(['git', '-C', str(root), 'rev-parse', 'HEAD'], allow_protected_root_mutation=True)
...
def _compute_ssot_validation(project_root: Path) -> dict[str, str]:
    ...
    if script_path.exists():
        ...
        computed = ''
    return {'blueprint_hash': FORENSIC_DISCOVERY_INTEGRITY_HASH, 'status': 'MATCH' if computed == FORENSIC_DISCOVERY_INTEGRITY_HASH else 'MISMATCH'}
...
def _to_v54_schema(legacy: dict, project_root: Path) -> dict:
    ...
    v54: dict = {'meta': {'timestamp': meta.get('generated_at', ''), 'root_path': meta.get('root', ''), 'git_hash': meta.get('git_commit', ''), 'schema_version': V54_SCHEMA_VERSION, 'python_version': meta.get('python_version', ''), 'platform': meta.get('platform', ''), 'total_candidates': meta.get('total_candidates', 0)}, 'ssot_validation': _compute_ssot_validation(project_root), 'agents': []}
    for agent in legacy.get('environment_under_test', []):
        mro_chain = agent.get('mro_signature', [])
        v54['agents'].append({'identity': agent.get('agent_name', ''), 'layer': agent.get('layer', ''), 'status': agent.get('status', ''), 'file_path': agent.get('file_path', ''), 'class_name': agent.get('class_name', ''), 'mro_chain': mro_chain, 'mixins': _derive_mixins(mro_chain), 'detected_methods': agent.get('methods_detected', []), 'integrity_hash': agent.get('file_sha256', ''), 'is_sovereign': agent.get('is_sovereign', False)})
    return v54
...
def run_forensic_discovery(out_path: Path | None=None, *, legacy_schema: bool=False) -> int:
    ...
    raw_candidates = sorted(raw_candidates, key=lambda c: (c.get('layer', ''), c.get('class_name', ''), c.get('file', '')))
    manifest = {'audit_meta': {'root': str(project_root), 'total_candidates': len(raw_candidates), 'generated_at': datetime.now(timezone.utc).isoformat(), 'output_schema_version': OUTPUT_SCHEMA_VERSION, 'python_version': sys.version.split()[0], 'platform': platform.platform(), 'git_commit': get_git_commit(project_root)}, 'environment_under_test': [], 'ignored_artifacts': [], 'counts': {}}
    for candidate in raw_candidates:
        ...
        if not rel_path:
            record = ForensicAgentRecord(agent_name=name, layer=layer, file_path='', class_name='Unknown', mro_signature=[], status='INVALID', methods_detected=[], selection_reason='Missing path in SSOT candidate')
        ...
        # guardian: allow-silent-swallow
            ...
            raise
            record = ForensicAgentRecord(agent_name=name, layer=layer, file_path=str(full_path), class_name='Unknown', mro_signature=[], status='INVALID', methods_detected=[], selection_reason='Path fails validate_path_within_project')
            manifest['ignored_artifacts'].append(asdict(record))
            continue
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('--legacy-schema', action='store_true', default=False, help='Emit legacy schema (audit_meta/environment_under_test) instead of v5.4')
