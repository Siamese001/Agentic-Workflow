                ...
                if 'from agentic_core' in content:
                    ...
                    for i, line in enumerate(lines):
                        if line.startswith('import ') or line.startswith('from '):
                            last_import_idx = i
                    lines.insert(last_import_idx + 1, 'from agentic_core.mixins.subatomic_testing_mixin import SubatomicTestingMixin')
                    content = '\n'.join(lines)
                elif content.startswith('"""'):
                    end_docstring = content.find('"""', 3) + 3
                    content = content[:end_docstring] + '\nfrom agentic_core.mixins.subatomic_testing_mixin import SubatomicTestingMixin\n' + content[end_docstring:]
                else:
                    content = 'from agentic_core.mixins.subatomic_testing_mixin import SubatomicTestingMixin\n' + content
