...
LAYER_BEST_PRACTICES = {'L1_cognition': {'default': 'Pure reasoning and thought generation. No side effects. Immutable inputs → deterministic outputs.', 'thought_engine': 'Chain-of-thought, tree-of-thought, ReAct pattern implementations. Reference: Yao et al. (2022) ReAct paper.', 'planning': "Hierarchical Task decomposition. HTN, BDI patterns. Reference: Ghallab et al. 'PDDL'.", 'knowledge': 'Static, curated eternal truth. No dynamic retrieval here — use semantic_memory for runtime.', 'static_index': 'Permanent store of vetted research papers, prompt constitutions, tool schemas. Indexed at embed time.'}, 'L2_execution': {'default': 'Safe, sandboxed tool interaction. All tools must be registered and validated.', 'tool_registry': 'Single source of truth for all available tools. Each tool: schema + implementation + safety policy.', 'sandbox': 'Isolated execution environment. No direct system access outside approved tools.'}, 'L3_orchestration': {'default': 'Workflow composition and agent handoff. Memory-aware routing.', 'workflow_engines': 'State machine, DAG, and reactive workflow implementations. Reference: Temporal.io patterns.'}, 'L4_state': {'default': 'Persistent, auditable state management. Redis-backed ledger.', 'ValidationContext': 'Checkpointing, session persistence, drift detection.', 'persistence_layer': 'Single interface to Redis/Pinecone/filesystem — abstraction only.'}, 'L5_safety': {'default': 'Red-team guards, policy enforcement, auto-immune response.', 'policy': 'Formal policy definitions. All actions routed through L5 before execution.', 'audit_logs': 'Immutable forensic ledger. Every decision recorded.'}, 'semantic_memory': {'vector_stores': 'Abstract interface to Pinecone/Chroma/etc. No direct imports — use registry.', 'embedding_logic': 'Gemini-only embedding pipeline. No fallback to other providers.'}, 'prompt_governance': {'meta_prompts': 'Sovereign prompt constitution and system prompts. No raw strings outside this folder.'}}
