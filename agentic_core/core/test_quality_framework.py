...
class TestQualityAnalyzer:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                for pattern in self.weak_assertion_patterns:
                    if pattern in line and 'def test_' in line[:100]:  # Within a test method
                ...
                if 'def test_' in line and ('try:' in line or 'with' in line):
                    if not any('assert' in l for l in lines[i:i+10]):
                        issues.append(TestQualityIssue(
                            test_name=self._extract_test_name(lines[:i]),
                            file_path=file_path,
                            issue_type="missing_exception_assertion",
                            description="Test uses try/with but lacks exception assertions",
                            line_number=i,
                            current_strength=AssertionStrength.WEAK,
                            suggested_improvement="Add pytest.raises() or explicit exception testing"
                        ))
    ...
    def _suggest_improvement(self, pattern: str) -> str:
        """Suggest improvement for a weak assertion pattern."""
        suggestions = {
            "assert obj is not None": "Add behavioral validation: assert obj.method() returns expected",
            "assert obj is None": "Add state validation: assert obj.state == 'empty' or similar",
            "assert len(obj) == 0": "Add content validation: assert obj.is_empty() or obj.validate()",
            "assert len(obj) > 0": "Add behavioral validation: assert obj.contains(expected_item)",
            "assert isinstance(obj,": "Add interface validation: assert obj.interface_method() works",
            "assert hasattr(obj,": "Add functional validation: assert obj.attribute_method() works",
            "assert obj.attr == value": "Add behavioral context: assert obj.get_attr() == value after operation",
        }
        return suggestions.get(pattern, "Add stronger behavioral or state validation")
