...
class Hypothesis(BaseModel):
    ...
    confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Confidence level (0.0 to 1.0)"
    )
...
class MetacognitionReport(BaseModel):
    ...
    hypotheses: list[Hypothesis] = Field(
        default_factory=list, description="List of system hypotheses"
    )
    global_confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Global confidence score"
    )
    uncertainty_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="System uncertainty level"
    )
