...
class RetryPolicy(BaseModel):
    ...
    backoff_max: float = Field(
        default=30.0, ge=1.0, le=300.0, description="Maximum backoff delay in seconds"
    )
...
class HopSpec(BaseModel):
    ...
    timeout_seconds: int = Field(
        default=30, ge=1, le=300, description="Timeout in seconds for this HOP"
    )
...
class AgentContract(BaseModel):
    ...
    retry_policy: RetryPolicy = Field(
        default_factory=RetryPolicy, description="Agent's retry policy"
    )
