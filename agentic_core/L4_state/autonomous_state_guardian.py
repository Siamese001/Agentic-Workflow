...
class AutonomousStateGuardian:
    ...
    async def verify_checkpoint_integrity(self, checkpoint_id: str) -> bool:
        ...
        try:
            is_valid, errors = await self.checkpoint_manager.verify_checkpoint(checkpoint_id)
            return is_valid
        except Exception as e:
    ...
    async def detect_state_corruption(self) -> List[str]:
        ...
        try:
            history = self.checkpoint_manager.get_checkpoint_history()
        ...
        try:
            ...
            for cp_id in reversed(history):
                if cp_id not in corrupt_ids and await self.verify_checkpoint_integrity(cp_id):
                    ...
                    await self.checkpoint_manager.rollback_to_checkpoint(cp_id)
