...
class ExperienceBuffer:
    ...
    # guardian: allow-magic-config
    def find_similar(self, action: str | None=None, target: str | None=None, context_hash: str | None=None, limit: int=20, **extra_filters) -> list[dict[str, Any]]:
        """
    ...
    def predict_success_probability(self, action: str, target: str | None=None, context_hash: str | None=None, **extra_context) -> float:
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'total_entries': len(entries), 'success_rate': successes / len(entries), 'most_common_action': max((e.get('action') for e in entries if e.get('action')), key=lambda a: sum((1 for e in entries if e.get('action') == a)), default=None)}
