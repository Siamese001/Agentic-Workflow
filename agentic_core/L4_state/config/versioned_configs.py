...
class PolicyConfig:
    ...
    version: str = '1.0.0'
    tool_allowlist: tuple[str, ...] = ('file_read', 'file_write', 'ast_parse', 'llm_call', 'redis_get', 'redis_set', 'pinecone_query', 'pinecone_upsert')
    file_scope_whitelist: tuple[str, ...] = ('/tmp', '/workspace', AGENTIC_CORE_DIR)
    token_budget: int = 1000000

    def canonical_bytes(self) -> bytes:
        doc = {'version': self.version, 'tool_allowlist': sorted(self.tool_allowlist), 'file_scope_whitelist': sorted(self.file_scope_whitelist), 'token_budget': self.token_budget}
...
class RoutingConfig:
    ...
    def canonical_bytes(self) -> bytes:
        doc = {'version': self.version, 'anomaly_routing_threshold': self.anomaly_routing_threshold, 'depth_breaker': self.depth_breaker, 'escalation_mode': self.escalation_mode, 'escalation_severity_threshold': self.escalation_severity_threshold, 'escalation_threshold': self.escalation_threshold, 'escalation_violation_code_denylist': sorted(self.escalation_violation_code_denylist), 'escalation_window_ticks': self.escalation_window_ticks, 'fallback_mode': self.fallback_mode}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()
...
class ModelConfig:
    ...
    def canonical_bytes(self) -> bytes:
        doc = {'version': self.version, 'cognition_model': self.cognition_model, 'embedding_model': self.embedding_model, 'embedding_dimensions': self.embedding_dimensions}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()
...
class BudgetConfig:
    ...
    def canonical_bytes(self) -> bytes:
        doc = {'version': self.version, 'token_budget': self.token_budget, 'max_k': self.max_k, 'max_retries': self.max_retries, 'backoff_base_seconds': self.backoff_base_seconds}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()
...
class L4ActiveConfigs:
    ...
    def hashes(self) -> dict[str, str]:
        return {'policy_hash': self.policy.config_hash, 'routing_hash': self.routing.config_hash, 'model_hash': self.model.config_hash, 'budget_hash': self.budget.config_hash}
...
class MLCacheConfig:
    ...
    def canonical_bytes(self) -> bytes:
        doc = {'version': self.version, 'default_ttl_seconds': self.default_ttl_seconds, 'eviction_mode': self.eviction_mode, 'max_entries': self.max_entries}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()
