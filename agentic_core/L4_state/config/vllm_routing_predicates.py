...
def default_routing(ctx: Mapping[str, Any]) -> bool:
    ...
    return True
ROUTING_PREDICATES: tuple[RoutingPredicate, ...] = (RoutingPredicate(name='requires_policy_read', predicate=requires_policy_read, provider=Provider.OPUS), RoutingPredicate(name='iteration_count_exceeded', predicate=iteration_count_exceeded, provider=Provider.OPUS), RoutingPredicate(name='invalid_ast_detected', predicate=invalid_ast_detected, provider=Provider.OPUS), RoutingPredicate(name='default_routing', predicate=default_routing, provider=Provider.LOCAL_VLLM))
...
def evaluate(context: Mapping[str, Any]) -> RoutingDecision:
    ...
    for entry in ROUTING_PREDICATES:
        if entry.predicate(context):
            ...
            break
    decision = RoutingDecision(provider=matched_provider, predicate_evaluation_hash=predicate_hash, routing_version=str(context.get('routing_version', 'unknown')))
