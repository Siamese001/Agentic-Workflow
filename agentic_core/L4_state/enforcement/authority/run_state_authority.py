...
Usage — facade (zero behaviour change)::
    ...
    from agentic_core.L4_state.enforcement.authority.run_state_authority import get_run_state_authority
    ...
    def _backend_read(self, key: str, default: Any) -> Any:
        ...
        if self._backend is not None and hasattr(self._backend, "get"):
            try:
                ...
                TypeError,
                ValueError,
            ) as exc:  # guardian: allow-log-and-swallow -- multi-backend read best-effort; failure logged and next backend tried
