...
class PromotionAuthority:
    ...
    def update_pointer_via_gateway(self, new_pointer: str, capability_token) -> PromotionPointerUpdate:
        ...
        update = PromotionPointerUpdate(old_pointer=old_pointer, new_pointer=new_pointer, timestamp=time.time(), capability_token_hash=hashlib.sha256(str(capability_token).encode()).hexdigest(), guardian_signature='guardian_signature_placeholder', semantic_clock_tick=capability_token.semantic_clock_tick)
        self._write_gateway.update_pointer(namespace=capability_token.target_namespace, old_pointer=old_pointer, new_pointer=new_pointer, capability_token=capability_token)
        self._active_updates[capability_token.target_namespace] = update
        Logger.info(f'Pointer updated in namespace {capability_token.target_namespace}: {old_pointer} -> {new_pointer}')
        return update
    ...
    def validate_pointer_update_integrity(self, namespace: str, expected_hash: str) -> bool:
        ...
        if not update:
            return False
        computed_hash = hashlib.sha256(f'{update.old_pointer}{update.new_pointer}{update.timestamp}'.encode()).hexdigest()
