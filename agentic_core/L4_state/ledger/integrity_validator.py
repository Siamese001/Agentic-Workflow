...
def validate_ledger_chain(entries: list[dict[str, Any]]) -> None:
    ...
    for idx, entry in enumerate(entries):
        ...
        if stored_hash is None:
            raise LedgerIntegrityViolation(f"Ledger entry {idx} missing '_hash' field — integrity cannot be verified")
        ...
        if expected_hash != stored_hash:
            raise LedgerIntegrityViolation(f'Ledger hash mismatch at entry {idx}: expected={expected_hash[:16]}... stored={stored_hash[:16]}...')
