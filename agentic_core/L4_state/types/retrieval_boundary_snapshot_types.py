...
class RetrievalBoundarySnapshot:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != _SCHEMA_VERSION:
            raise ValueError(f'RetrievalBoundarySnapshot: schema_version must be {_SCHEMA_VERSION}, got {self.schema_version!r}')
    ...
    def canonical_bytes(self) -> bytes:
        ...
        """
        doc: dict[str, Any] = {'active_config_hashes': {k: self.active_config_hashes[k] for k in sorted(self.active_config_hashes)}, 'anchors': [a.to_dict() for a in sorted(self.anchors, key=lambda a: a.sort_key())], 'created_at_utc': self.created_at_utc, 'mission_id': self.mission_id, 'request_hash': self.request_hash, 'schema_version': self.schema_version}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()

    def to_dict(self) -> dict[str, Any]:
        return {'schema_version': self.schema_version, 'mission_id': self.mission_id, 'request_hash': self.request_hash, 'active_config_hashes': dict(self.active_config_hashes), 'anchors': [a.to_dict() for a in self.anchors], 'created_at_utc': self.created_at_utc, 'snapshot_hash': self.snapshot_hash}
...
def create_retrieval_boundary_snapshot(mission_id: str, query: str, top_k: int, domain: str, active_config_hashes: dict[str, str], anchors: list[AnchorEntry], created_at_utc: str) -> RetrievalBoundarySnapshot:
    """
    ...
    return RetrievalBoundarySnapshot(schema_version=_SCHEMA_VERSION, mission_id=mission_id, request_hash=request_hash, active_config_hashes=active_config_hashes, anchors=anchors, created_at_utc=created_at_utc)
