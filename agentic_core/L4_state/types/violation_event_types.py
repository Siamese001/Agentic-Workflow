...
class ViolationEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if self.guardian_decision not in _VALID_DECISIONS:
            raise ValueError(
                f"ViolationEvent: guardian_decision must be one of "
                f"{sorted(_VALID_DECISIONS)}, got {self.guardian_decision!r}"
            )
        ...
        # Normalise: sort in place for determinism
        self.violation_codes = sorted(self.violation_codes)
        # Compute hash after all validation
