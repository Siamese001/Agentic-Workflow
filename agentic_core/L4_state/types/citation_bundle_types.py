...
class CitationBundle:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != _SCHEMA_VERSION:
            raise ValueError(f'CitationBundle: schema_version must be {_SCHEMA_VERSION}, got {self.schema_version!r}')
    ...
    def canonical_bytes(self) -> bytes:
        ...
        """
        doc: dict[str, Any] = {'anchors': [{'char_end': a.char_end, 'char_start': a.char_start, 'chunk_id': a.chunk_id, 'source_doc_id': a.source_doc_id, 'version_hash': a.version_hash} for a in sorted(self.anchors, key=_anchor_sort_key)], 'request_hash': self.request_hash, 'schema_version': self.schema_version}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()

    def to_dict(self) -> dict[str, Any]:
        return {'schema_version': self.schema_version, 'request_hash': self.request_hash, 'anchors': [a.to_dict() for a in self.anchors], 'citation_hash': self.citation_hash}
