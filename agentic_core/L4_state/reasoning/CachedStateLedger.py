...
class CachedStateLedger(SovereignBaseAgent):
    ...
    def __init__(self, project_root: Path, session_id: str):
        ...
        try:
            ...
            connection_kwargs = {'host': parsed.hostname or 'localhost', 'port': parsed.port or 6379, 'password': parsed.password, 'username': parsed.username, 'decode_responses': True, 'socket_timeout': 5, 'socket_connect_timeout': 5, 'retry_on_timeout': True}
        ...
        # guardian: allow-silent-swallow
            ...
            raise InfrastructureDependencyError(f'[CachedStateLedger] Redis is a mandatory dependency and is unavailable: {e}') from e
    ...
    def cache_validation_context(self, key: str, context: dict):
        ...
        self._record_successful_trace({'operation': 'cache_validation_context', 'key': key, 'timestamp': time.time()})
    ...
    def get_cached_validation_context(self, key: str) -> dict | None:
        ...
        try:
            if self.redis:
                ...
                if data:
                    self._record_successful_trace({'operation': 'get_cached_validation_context', 'key': key, 'hit': True, 'timestamp': time.time()})
