...
class ReasoningMemory:
    ...
    def store(self, thought: dict[str, Any]) -> str:
        ...
        thought_obj = Thought(thought_id=thought_id, content=thought.get('content', thought.get('text', str(thought))), thought_type=thought.get('type', 'reasoning'), context=thought.get('context', {}), confidence=thought.get('confidence', 0.8), metadata=thought.get('metadata', {}))
        ...
        while len(self.thoughts) > self.capacity:
            ...
            if self.semantic_offload and self.semantic_memory:
                self.semantic_memory.add_thought({'id': evicted.thought_id, 'text': evicted.content, 'type': evicted.thought_type, 'context': evicted.context, 'confidence': evicted.confidence})
        if self.persist:
    ...
    def retrieve_relevant(self, query: str, top_k: int=5) -> list[dict[str, Any]]:
        ...
        if self.semantic_memory:
            ...
            combined = in_memory_results + [r.get('content', r) for r in semantic_results if not any((self._is_duplicate(r, im) for im in in_memory_results))]
    ...
    def _thought_to_dict(self, thought: Thought) -> dict[str, Any]:
        ...
        return {'id': thought.thought_id, 'content': thought.content, 'type': thought.thought_type, 'context': thought.context, 'confidence': thought.confidence, 'timestamp': thought.timestamp, 'metadata': thought.metadata}
    ...
    def _load_persistent(self) -> None:
        ...
        try:
            ...
            for entry in entries:
                ...
                if thought_dict:
                    self.thoughts.append(Thought(thought_id=thought_dict.get('id', ''), content=thought_dict.get('content', ''), thought_type=thought_dict.get('type', 'reasoning'), context=thought_dict.get('context', {}), confidence=thought_dict.get('confidence', 0.8), timestamp=thought_dict.get('timestamp', time.time()), metadata=thought_dict.get('metadata', {})))
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'capacity': self.capacity, 'current_size': len(self.thoughts), 'total_stored': self.total_stored, 'total_evicted': self.total_evicted, 'total_retrieved': self.total_retrieved, 'persist_enabled': self.persist, 'semantic_offload_enabled': self.semantic_offload}
