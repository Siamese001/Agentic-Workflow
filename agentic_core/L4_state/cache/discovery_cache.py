...
trace_contract._emit_dispatches_healing_run("p1", "discovery_cache", "L4")
trace_contract._emit_routes_through("p1", "discovery_cache", "L4")
...
trace_contract._emit_escalates_to_human("p1", "discovery_cache", "L4")
trace_contract._emit_reads_policy_state("p1", "discovery_cache", "L4")
...
class AgentDiscoveryCache:
    ...
    def get_or_fetch(
        ...
        *fetch_from_disk* is a zero-argument callable that reads and parses the
        ...
        trace_contract._emit_records_execution_trace(_trace_id, trace_contract.LayerSegment.L4_STATE, "AgentDiscoveryCache.get_or_fetch")
        ...
        if not replay_mode:
            try:
                ...
                raise
            except (  # guardian: allow-log-and-swallow -- hash compute best-effort: non-fatal, fetch proceeds from disk without cache key
                OSError,
                ValueError,
            ) as e:
                ...
                try:
                    ...
                    OSError,
                    ConnectionError,
                ) as e:  # guardian: allow-log-and-swallow -- cache read failure: non-fatal, falls through to disk fetch
