...
_emit_dispatches_healing_run("p1", "discovery_cache", "L4")
_emit_routes_through("p1", "discovery_cache", "L4")
...
_emit_escalates_to_human("p1", "discovery_cache", "L4")
_emit_reads_policy_state("p1", "discovery_cache", "L4")
...
class AgentDiscoveryCache:
    ...
    def get_or_fetch(
        ...
        *fetch_from_disk* is a zero-argument callable that reads and parses the
        ...
        _emit_records_execution_trace(_trace_id, LayerSegment.L4_STATE, "AgentDiscoveryCache.get_or_fetch")
            ...
            try:
                ...
                try:
                    ...
                    if cached is not None:
                        ...
                        return cached
                except (OSError, ConnectionError) as e:  # guardian: allow-log-and-swallow -- cache read failure: non-fatal, falls through to disk fetch
        ...
        if not replay_mode:
            try:
                ...
                pass
            except (OSError, ConnectionError) as e:  # guardian: allow-log-and-swallow -- cache write failure: non-fatal, disk fetch already served caller
