...
trace_contract._emit_dispatches_healing_run("p1", "config_file_cache", "L4")
trace_contract._emit_routes_through("p1", "config_file_cache", "L4")
...
trace_contract._emit_escalates_to_human("p1", "config_file_cache", "L4")
trace_contract._emit_reads_policy_state("p1", "config_file_cache", "L4")
...
class ConfigFileCache:
    ...
    def get_or_fetch(
        ...
        *fetch_from_disk* is a zero-argument callable that reads and parses the
        ...
        trace_contract._emit_records_execution_trace(_trace_id, trace_contract.LayerSegment.L4_STATE, "ConfigFileCache.get_or_fetch")
        ...
        if not replay_mode:
            try:
                ...
                OSError,
                ConnectionError,
            ) as e:  # guardian: allow-log-and-swallow -- cache read failure: non-fatal, falls through to disk parse
        ...
        if not replay_mode:
            try:
                ...
                OSError,
                ConnectionError,
            ) as e:  # guardian: allow-log-and-swallow -- cache write failure: non-fatal, parsed result already served caller
