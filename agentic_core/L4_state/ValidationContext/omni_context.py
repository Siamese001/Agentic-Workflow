...
class TheOmniContext:
    ...
    async def build_context(self, file_summaries: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.file_index = {}
        self.file_contents = {}
        stats: Any = {'files_processed': 0, 'total_characters': 0, 'repositories': set(), 'skipped': 0}
        buffer_parts: Any = []
        ...
        for file_key, data in file_summaries.items():
            ...
            try:
                ...
                self.file_index[file_key] = {'start': start_pos, 'end': current_position - 1, 'repository': repository, 'path': relative_path, 'summary': data['summary']}
    ...
    def query_context(self, query: str, ContextWindow: int=10000) -> Dict[str, Any]:
        ...
        for file_key, index_data in self.file_index.items():
            if query_lower in index_data['path'].lower() or (index_data['summary'] and query_lower in index_data['summary'].lower()):
                content: Any = self.get_file_content(file_key)
                if content:
                    matches.append({'file': file_key, 'repository': index_data['repository'], 'path': index_data['path'], 'summary': index_data['summary'], 'content': content[:1000] + '...' if len(content) > 1000 else content})
        if len(matches) < 5:
