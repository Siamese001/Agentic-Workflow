...
class VerifiableCheckpointManager:
    ...
    async def load_checkpoint(
        ...
        if verify:
            ...
            if stored_checksum != calculated_checksum:
                ...
                raise ValueError(
                    f"Corrupted Checkpoint: {stored_checksum} != {calculated_checksum}"
                )
            else:
    ...
    async def save_snapshot(
        self, session_id: str, snapshot_name: str, state: dict[str, Any]
    ) -> str:
        """
...
def create_checkpoint_manager(
    storage_type: str = "local", **storage_kwargs
