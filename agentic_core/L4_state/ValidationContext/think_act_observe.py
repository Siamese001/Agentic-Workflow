...
class ThinkActObserveEngine:
    ...
    def __init__(self, config: CycleConfig | None = None, enable_logging: bool = True):
        ...
        if self.enable_logging:
            LOGGER.info(
                "think_act_observe_engine_initialized", extra={"config": self.config.to_dict()}
            )
    ...
    async def execute_cycle(
        ...
        if self.enable_logging:
            LOGGER.info(
                "cycle_started", extra={"mission": mission, "scene_keys": list(scene.keys())}
            )
            ...
            try:
                {
                    ...
                    "previous_observations": self.state.observations[-3:]
                ...
                for step in trace.steps:
                    if step.action and step.action != "FINISH":
                        actions.append(
                            {"type": "action", "action": step.action, "thought": step.thought}
                        )
                return {
                    "success": True,
                    "actions": actions,
