...
class TestGlobalKeyPurge:
    ...
    def test_hierarchy_import_integrity(self):
        ...
        if hierarchy_path.exists():
            with open(hierarchy_path, "r") as f:
                content = f.read()
                assert "CANON_KEY_TO_FOLDER_MAP" not in content, "LEAK: Deprecated map still referenced in HierarchyAgent."
    ...
    def test_structure_blueprint_constants_removed(self):
        ...
        for constant in deprecated_constants:
            assert constant not in content, f"CRITICAL: Deprecated constant {constant} still found in structure_blueprint.py"
    ...
    def test_no_canon_key_imports_remain(self):
        ...
        for py_file in project_root.rglob("*.py"):
            ...
            if any(part.startswith('.') for part in py_file.parts):
                continue
            if any(part in ['__pycache__', '.git', 'node_modules', '.venv', 'venv'] for part in py_file.parts):
                continue
            # Skip archive folders (legacy/backups)
            if any(part in ['archives', 'archive', 'legacy', 'void_violations', 'location_violations', 
                           'healing_backups', 'gatekeeper', 'deprecated'] for part in py_file.parts):
                continue
        ...
        assert len(violations) == 0, f"CRITICAL: Found {len(violations)} files with deprecated imports:\n" + "\n".join(violations)
