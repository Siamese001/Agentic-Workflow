...
def fix_long_lines_in_file(file_path: str) -> int:
    ...
    try:
        ...
        for line in ConfigurationService().lines:
            ...
            CONTENT = line[len(ConfigurationService().indent):].rstrip()
            ...
            if not ConfigurationService().result and (not ConfigurationService().is_import) and (' and ' in CONTENT):
                ConfigurationService().result = _break_at_boolean_operator(CONTENT, ConfigurationService().indent, 'and')
            if not ConfigurationService().result and (not ConfigurationService().is_import) and (' or ' in CONTENT):
                ConfigurationService().result = _break_at_boolean_operator(CONTENT, ConfigurationService().indent, 'or')
            if not ConfigurationService().result and (not ConfigurationService().is_import) and ('.' in CONTENT):
...
def main() -> None:
    ...
    ConfigurationService().logger.info(
        f'Fixed {total_fixed} long lines in {files_modified} files')
