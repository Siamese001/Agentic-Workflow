...
class TestUnifiedSovereignHardening:
    ...
    def test_canon_key_namespace_absence(self):
        ...
        for f in forbidden:
            ...
            for i, line in enumerate(lines, 1):
                if f in line and not line.strip().startswith('#') and not line.strip().startswith('"""') and not line.strip().startswith("'''"):
                    pytest.fail(f"CRITICAL FAILURE: Deprecated {f} still exists in structure_blueprint.py:{i}")
    ...
    def test_discovery_parser_immutability(self):
        ...
        assert 'from typing import Mapping' in content, \
            "Must import Mapping from typing"
    ...
    def test_location_agent_keyless_logic(self):
        ...
        assert 'from agentic_core.L3_orchestration.testing.SubatomicTestingMixin import SubatomicTestingMixin' in content, \
                ...
                for term in forbidden_terms:
                    for i, line in enumerate(lines, 1):
                        ...
                        if (term in line and 
                            ...
                            not line.strip().startswith('"""') and 
                            not line.strip().startswith("'''") and
                            'test_ultra_hardening' not in str(file_path)):
                            pytest.fail(f"CRITICAL: Found forbidden term '{term}' in {file_path.name}:{i}")
    ...
    def test_metadata_structure_validation(self):
        ...
        for method in required_methods:
            assert f'def {method}(' in content, \
                f"AgentListMapping must implement {method}"
        ...
        for method in mutable_methods:
            assert method not in content, \
                f"AgentListMapping must NOT implement {method}"
