#!/usr/bin/env python3
"""
Sprint 3 - Phase 3: L4 → L5 Interface Extraction

Batch refactor L4 state layer files to use MCPHardenedMixin
...
def main():
    """Refactor all L4 files with MCPHardenedMixin imports."""
    
    print("=" * 80)
    print("  Sprint 3 - Phase 3: L4 → L5 Interface Extraction")
    print("=" * 80)
    print()
    ...
    # Find all Python files in L4_state
    l4_dir = REPO / AGENTIC_CORE_DIR / "L4_state"
    
    if not l4_dir.exists():
        print(f"❌ Directory not found: {l4_dir}")
        return 1
    ...
    for py_file in get_python_files(l4_dir):
    ...
    if files_modified > 0:
        ...
        print("  • ~20 L4→L5 violations eliminated")
