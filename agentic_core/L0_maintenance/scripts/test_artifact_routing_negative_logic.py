...
def test_forbidden_extensions():
    ...
    test_cases = [
        ...
        ("audit_results.md", "# Assessment Report", False, "Markdown file should be accepted for docs/reports"),
        ("mission_trace.jsonl", '{"mission_id": "123"}', False, "JSONL should be accepted for logs"),
    ]
...
def test_forbidden_keywords():
    ...
    test_cases = [
        ...
        ("util_script.py", "def main():\n    pass", False, "Python script should be accepted for scripts/"),
    ]
    ...
    for filename, content, should_be_forbidden, description in test_cases:
        ...
        if should_be_forbidden:
            if rejection:
                ...
                print(f"❌ FAIL: {description}")
                print(f"   Expected rejection but got None")
                failed += 1
        else:
            if not rejection:
                print(f"✅ PASS: {description}")
