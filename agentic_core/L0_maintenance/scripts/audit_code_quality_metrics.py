...
def main():
    ...
    print(
        f"  Agents below 100%: {len(typed_below_100)} ({len(typed_below_100) / total * 100:.1f}%)"
    )
    ...
    print(
        f"  Agents below 100%: {len(schema_below_100)} ({len(schema_below_100) / total * 100:.1f}%)"
    )
    ...
    for a in agents:
        if (
            a.get("typed_pct", 0) < 100.0
            or a.get("documented_pct", 0) < 100.0
            or a.get("schema_strictness", 0) < 100.0
        ):
