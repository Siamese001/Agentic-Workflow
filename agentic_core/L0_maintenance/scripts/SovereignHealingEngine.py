...
class SovereignHealingEngine:
        ...
        try:
            ...
            for issue in target_issues:
                ...
                if fix_successful:
                    ...
                    Logger.error(
                        f"[L0 HEALING] Failed to apply fix for {issue.get('file', 'unknown')}"
                    )
                    if not config.HEALING_AUTO_APPLY:
                        raise Exception(
                            "Strict healing mode: failure on single fix triggers rollback"
                        )
    ...
    async def _generate_fix(self, content: str, ViolationType: str, message: str) -> str | None:
        ...
        if "HTTP" in message or "requests" in message.lower():
            ...
            new_content = new_content.replace(
                "requests.post(", "# await get_fetch_client().fetch_url("
            )
        if "Redis" in message:
            new_content = new_content.replace(
                "import redis",
        ...
        if "PATH_BREACH" in ViolationType or "tools/" in message:
            ...
            new_content = new_content.replace(
                "from agentic_core.tools.", "from agentic_core.utils."
            )
