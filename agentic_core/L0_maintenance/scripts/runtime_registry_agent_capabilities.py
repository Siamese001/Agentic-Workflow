...
def get_registry_stats(self: Any) -> Dict[str, Any]:
    ...
    Returns:
        Registry statistics
    """
    return {'total_roles': len(self.list_roles()), 'registered_specs': len(self._specs), 'legacy_mappings': len(LEGACY_MAPPING), 'categories': {'research': 3, 'strategy': 3, 'content': 3, 'quality': 3, 'specialized': 3}}
...
def create_functional_agent(role: AgentRole, hop_function: Callable, **kwargs) -> SubatomicHop:
    ...
    """
    SPEC: Any = AgentSpec(role=role, hop_function=hop_function, **kwargs)
    return spec.create_hop()
...
class legacy_code_error(Exception):
...
def validate_no_legacy_code(text: str, context: str='Unknown') -> None:
    ...
    """
    get_agent_registry()
    legacy_refs: Any = registry.validate_no_legacy_references(text)
    if legacy_refs:
        raise LegacyCodeError(f'Legacy K-node references found in {context}: {legacy_refs}. Use functional roles instead.')
