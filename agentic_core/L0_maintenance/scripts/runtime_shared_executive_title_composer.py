...
def __init__(
    self: Any, config: ReasoningConfig, word_count_min: int, word_count_max: int, char_limit: int
) -> None:
...
async def execute(self: Any, context: Dict[str, Any]) -> HeadlineOutput:
    ...
    logger.info(
        f"ExecutiveTitleComposer complete: {word_count} words, {char_count} chars, "
        f"Industry-First={industry_first_compliant}"
    )

    if not industry_first_compliant:
        logger.error(
            f"INDUSTRY-FIRST VIOLATION: Technology keywords in Segment 1: "
            f"{tech_keywords_in_seg1}"
        )
