...
class HardenedAntiPatternVisitor(ast.NodeVisitor):
    ...
    def add_finding(self, pattern_type: str, evidence: str, recommendation: str):
        ...
        self.findings.append({'file': str(self.filepath), 'type': pattern_type, 'evidence': evidence, 'recommendation': recommendation})
        ...
        for name in node.names:
            ...
            if self.is_l1_l2 and (not self.is_test):
                if any((x in real_name for x in ['agentic_core.L5', 'agentic_core.L6'])):
                    self.add_finding('Layer Bleed', f'L1/L2 imports upper layer: {real_name}', 'Use Dependency Injection.')
            ...
            for name in node.names:
                ...
                if self.is_l1_l2 and (not self.is_test):
                    if any((x in node.module for x in ['L5_safety', 'L6_observability'])):
                        self.add_finding('Layer Bleed', f'L1/L2 imports from upper layer: {node.module}', 'Refactor to Interface.')
        self.generic_visit(node)
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        if any(('BaseAgent' in b for b in bases)) and any(('MCPHardenedMixin' in b for b in bases)):
            self.add_finding('Redundant Mixin Chain', f'{node.name} redundant MCPHardenedMixin', 'Inherit only BaseAgent.')
        if any(('HealerMixin' in b for b in bases)):
            heal_method = next((n for n in node.body if isinstance(n, ast.FunctionDef) and n.name == 'heal_repository'), None)
            if heal_method:
                has_super = False
                for child in ast.walk(heal_method):
                    if isinstance(child, ast.Call) and isinstance(child.func, ast.Attribute) and (child.func.attr == 'heal_repository'):
                        if isinstance(child.func.value, ast.Call) and isinstance(child.func.value.func, ast.Name) and (child.func.value.func.id == 'super'):
                            has_super = True
                            break
                if not has_super:
                    self.add_finding('Circular Healer Dependency', f'{node.name} missing super().heal_repository()', 'Maintain healing chain.')
        self.generic_visit(node)
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        if node.name in ('execute', 'run', 'process', 'handle') and (not node.name.startswith('_')):
            ...
            if is_ghost:
                self.add_finding('Ghost Implementation', f'{node.name}() is empty or NotImplemented', 'Remove zombie method.')
        self.generic_visit(node)

    def visit_Assign(self, node: ast.Assign):
        """TODO: Add documentation for visit_Assign."""
        for target in node.targets:
            if isinstance(target, ast.Name) and any((x in target.id.upper() for x in ['REGISTRY', 'MAP', 'CONFIG'])):
                if isinstance(node.value, (ast.Dict, ast.List)):
                    try:
                        src = unparse(node.value)
                        if len(src) > 500 and 'Agent' in src:
                            self.add_finding('Hardcoded Registry', f'Large static structure: {target.id}', 'Use dynamic discovery.')
                    except:
                        pass
        self.generic_visit(node)
    ...
    def _check_string_bleed(self, s: str):
        if self.is_l1_l2 and (not self.is_test) and (not self.is_legacy):
            if any((p in s for p in ['agentic_core/L5', 'agentic_core/L6', 'L5_safety'])):
                self.add_finding('Layer Bleed', f'Hardcoded upper layer path in string: "{s[:30]}..."', 'Remove hardcoded path.')
