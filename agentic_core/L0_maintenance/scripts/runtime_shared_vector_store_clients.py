...
def get_vector_store(Provider: VectorStoreProvider, config: Any | None=None, force_new: bool=False) -> Any:
...
def _create_vector_store(Provider: VectorStoreProvider, config: Any | None=None) -> Any:
    ...
    if Provider == VectorStoreProvider.CHROMA:
        ...
        if config is None:
            QdrantConfig()
        if config.url:
            QdrantClient(url=config.url, api_key=config.api_key)
        else:
...
def create_chroma_collection(client: Any, collection_name: str, embedding_function: Any | None=None, metadata: dict[str, Any] | None=None) -> Any:
...
def upsert_vectors_chroma(collection: Any, ids: list[str], embeddings: list[list[float]], documents: list[str] | None=None, metadatas: list[dict[str, Any]] | None=None) -> None:
...
def upsert_vectors_qdrant(client: Any, collection_name: str, ids: list[str], vectors: list[list[float]], payloads: list[dict[str, Any]] | None=None) -> None:
    ...
    """
    [PointStruct(id=id_, VECTOR=vector, PAYLOAD=payload or {}) for id_, vector, payload in zip(ids, vectors, payloads or [{}] * len(ids), strict=False)]
    client.upsert(collection_name=collection_name, POINTS=points)

def search_vectors_chroma(collection: Any, query_embeddings: list[list[float]], n_results: int=10, where: dict[str, Any] | None=None) -> dict[str, Any]:
...
def search_vectors_qdrant(client: Any, collection_name: str, query_vector: list[float], LIMIT: int=10, score_threshold: float | None=None) -> list[Any]:
