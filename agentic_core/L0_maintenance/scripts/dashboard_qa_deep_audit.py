...
def get_territory_for_agent(agent: Dict) -> str:
    ...
    if 'L6_observability' in path or layer == 'L6':
        if '/metrics' in path: return 'L6_Observability/Metrics'
        if '/telemetry' in path or '/agents' in path: return 'L6_Observability/Telemetry'
        if '/tracing' in path: return 'L6_Observability/Tracing'
        if '/compliance' in path: return 'L6_Observability/Compliance'
        return 'L6_Observability/Telemetry'  # Default L6

    # L5 Safety territories
    if 'L5_safety' in path or layer == 'L5':
        if '/guardrails' in path: return 'L5 Safety/Guardrails'
        if '/validators' in path: return 'L5 Safety/Validators'
        if '/red_teaming' in path: return 'L5 Safety/Red Teaming'
        if '/gravity' in path: return 'L5 Safety/Gravity'
        if '/bases' in path or 'Base' in agent.get('class_name', ''): return 'L5 Safety/Base Class'
        return 'L5 Safety/Guardrails'  # Default

    # L4 State territories
    if 'L4_state' in path or layer == 'L4':
        if '/ValidationContext' in path or '/validation_context' in path: return 'L4 State/Core'
        if '/ledger' in path: return 'L4 State/Core'
        if '/memory' in path: return 'L4 State/Core'
        if 'Base' in agent.get('class_name', ''): return 'L4 State/Base Class'
        if '/infrastructure' in path: return 'L4 State/Infrastructure'
        return 'L4 State/Core'

    # L3 Orchestration territories
    if 'L3_orchestration' in path or layer == 'L3':
        if '/workflow' in path or '/fission' in path or '/mcp' in path: return 'L3 Orchestration/Core'
        if 'Base' in agent.get('class_name', ''): return 'L3 Orchestration/Base Class'
        return 'L3 Orchestration/Core'

    # L2 Execution territories
    if 'L2_execution' in path or layer == 'L2':
        if '/ToolRegistry' in path or '/tool_registry' in path or '/mcp' in path: return 'L2 Execution/Core'
        if 'Base' in agent.get('class_name', ''): return 'L2 Execution/Base Class'
        return 'L2 Execution/Core'

    # L1 Cognition territories
    if 'L1_cognition' in path or layer == 'L1':
        if 'Base' in agent.get('class_name', ''): return 'L1 Cognition/Base Class'
    ...
    # Apps
    if 'apps_lic' in path or layer == 'apps_lic': return 'Apps Lic'
    if 'apps_rg' in path or layer == 'apps_rg': return 'Apps Rg'
    if 'apps_shared' in path or layer == 'apps_shared': return 'Apps Shared'
...
def calculate_expected_metrics(agents: List[Dict], territory: str) -> Dict[str, Any]:
    ...
    observable_count = sum(1 for a in territory_agents if a.get('observability'))  # dict is truthy if has any keys
...
def run_deep_audit():
    ...
    for territory in sorted(all_territories):
        ...
        if actual.get('Total', 0) != expected['Total']:
            issues.append(f"Total: Dashboard={actual.get('Total', 'MISSING')} vs Expected={expected['Total']}")

        # Check percentages match counts
        if expected['Total'] > 0:
            # Heal Cap %
            if abs(actual.get('Heal Cap %', 0) - expected['Heal Cap %']) > 1:
                issues.append(f"Heal Cap %: Dashboard={actual.get('Heal Cap %', 0)}% but {expected['Heal Cap Count']}/{expected['Total']} = {expected['Heal Cap %']}%")

            # Hardened %
            if abs(actual.get('Hardened %', 0) - expected['Hardened %']) > 1:
                issues.append(f"Hardened %: Dashboard={actual.get('Hardened %', 0)}% but {expected['Hardened Count']}/{expected['Total']} = {expected['Hardened %']}%")

            # MCP Capable %
            if abs(actual.get('MCP Capable %', 0) - expected['MCP Capable %']) > 1:
                issues.append(f"MCP Capable %: Dashboard={actual.get('MCP Capable %', 0)}% but {expected['MCP Capable Count']}/{expected['Total']} = {expected['MCP Capable %']}%")

            # Test %
            if abs(actual.get('Test %', 0) - expected['Test %']) > 1:
                issues.append(f"Test %: Dashboard={actual.get('Test %', 0)}% but {expected['Test Count']}/{expected['Total']} = {expected['Test %']}%")

            # Observable %
            if abs(actual.get('Observable %', 0) - expected['Observable %']) > 1:
                issues.append(f"Observable %: Dashboard={actual.get('Observable %', 0)}% but {expected['Observable Count']}/{expected['Total']} = {expected['Observable %']}%")

            # Typed %
            if abs(actual.get('Typed %', 0) - expected['Typed %']) > 1:
                issues.append(f"Typed %: Dashboard={actual.get('Typed %', 0)}% but {expected['Typed Count']}/{expected['Total']} = {expected['Typed %']}%")

            # Documented %
            if abs(actual.get('Documented %', 0) - expected['Documented %']) > 1:
                issues.append(f"Documented %: Dashboard={actual.get('Documented %', 0)}% but {expected['Documented Count']}/{expected['Total']} = {expected['Documented %']}%")
