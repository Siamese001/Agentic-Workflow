...
class SovereignGitKrakenMCPClient:
    ...
    def __init__(self, role: str = "governance_git"):
        ...
        # Lazy import to avoid circular dependency with L3
    ...
    async def create_healing_commit(self, files: List[str], message: str) -> Dict[str, Any]:
        ...
        """
        full_message = f"{config.GITKRAKEN_PR_TITLE_PREFIX} {message}"
    ...
    async def create_pr(self, title: str, description: str, source_branch: Optional[str] = None, target_branch: str = "main") -> Dict[str, Any]:
        ...
        source = source_branch or config.GITKRAKEN_HEALING_BRANCH
        
        try:
            result = await self.router.manager.call_tool(
                "mcp0_pull_request_create",
                {
                    "provider": "github",
                    "title": full_title,
                    "body": description,
                    ...
                    "repository_name": config.GITKRAKEN_DEFAULT_REPO.split("/")[1],
                    "repository_organization": config.GITKRAKEN_DEFAULT_REPO.split("/")[0]
                }
            )
