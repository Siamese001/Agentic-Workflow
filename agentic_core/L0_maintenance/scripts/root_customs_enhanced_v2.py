...
class RootCustomsAgent:
    ...
    def __init__(self, project_root: Optional[Path] = None, dry_run: bool = True):
        ...
        print(f"🛃 Enhanced Root Customs Agent v2.0 Initialized")
        print(f"📁 Project Root: {self.project_root}")
        print(f"🔍 Mode: {'DRY RUN' if dry_run else 'EXECUTE'}")
        print(f"🧠 AST Analysis: Enabled (Test Taxonomy + Zombie Code Detection)")
        print()
    ...
    def determine_routing(self, file_path: Path, content_matches: Dict[str, Any], ast_matches: Dict[str, Any]) -> RoutingDecision:
    ...
    def _determine_test_routing(self, file_path: Path, ast_matches: Dict[str, Any]) -> Optional[RoutingDecision]:
    ...
    def _determine_legacy_routing(self, file_path: Path, ast_matches: Dict[str, Any]) -> Optional[RoutingDecision]:
    ...
    def _determine_ast_placement_routing(self, file_path: Path, ast_matches: Dict[str, Any]) -> Optional[RoutingDecision]:
        ...
        for file_path in root_files:
            ...
            if decision.ast_matches:
                for match_type, matches in decision.ast_matches.items():
                    if matches and match_type != 'error':
                        print(f"   🧠 AST {match_type.title()}: {matches[:3]}{'...' if len(matches) > 3 else ''}")
