            ...
            try:
                ...
                for old_name, new_name in RENAMES.items():
                    ...
                    content = content.replace(
                        f"from agentic_core.L5_safety.policy_engine.{old_module}",
                        f"from agentic_core.L5_safety.policy_engine.{new_module}",
                    )
                    content = content.replace(
                        f"import agentic_core.L5_safety.policy_engine.{old_module}",
                        f"import agentic_core.L5_safety.policy_engine.{new_module}",
                    )
                for old_class, new_class in CLASS_RENAMES.items():
                    content = content.replace(old_class, new_class)
                if content != original:
