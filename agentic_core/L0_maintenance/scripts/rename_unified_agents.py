...
RENAMES = {'CodeDetectorAgent.py': 'CodeDetectorAgent.py', 'CodeEnforcerAgent.py': 'CodeEnforcerAgent.py', 'CodeHealerAgent.py': 'CodeHealerAgent.py', 'CodeValidatorAgent.py': 'CodeValidatorAgent.py', 'ResourceManagerAgent.py': 'ResourceManagerAgent.py', 'SafetyDetectorAgent.py': 'SafetyDetectorAgent.py', 'SafetyExecutorAgent.py': 'SafetyExecutorAgent.py', 'SecurityManagerAgent.py': 'SecurityManagerAgent.py', 'StructureEnforcerAgent.py': 'StructureEnforcerAgent.py', 'StructureHealerAgent.py': 'StructureHealerAgent.py'}
CLASS_RENAMES = {'CodeDetectorAgent': 'CodeDetectorAgent', 'CodeEnforcerAgent': 'CodeEnforcerAgent', 'CodeHealerAgent': 'CodeHealerAgent', 'CodeValidatorAgent': 'CodeValidatorAgent', 'ResourceManagerAgent': 'ResourceManagerAgent', 'SafetyDetectorAgent': 'SafetyDetectorAgent', 'SafetyExecutorAgent': 'SafetyExecutorAgent', 'SecurityManagerAgent': 'SecurityManagerAgent', 'StructureEnforcerAgent': 'StructureEnforcerAgent', 'StructureHealerAgent': 'StructureHealerAgent'}
...
def update_imports_codebase():
    ...
    scan_dirs = [PROJECT_ROOT / 'agentic_core', PROJECT_ROOT / 'tests', PROJECT_ROOT / 'scripts', PROJECT_ROOT / 'apps_rg', PROJECT_ROOT / 'apps_shared']
    files_updated = 0
        ...
        for py_file in scan_dir.rglob('*.py'):
            ...
            try:
                ...
                for old_name, new_name in RENAMES.items():
                    ...
                    content = content.replace(f'from agentic_core.L5_safety.unified.{old_module}', f'from agentic_core.L5_safety.unified.{new_module}')
                    content = content.replace(f'import agentic_core.L5_safety.unified.{old_module}', f'import agentic_core.L5_safety.unified.{new_module}')
                for old_class, new_class in CLASS_RENAMES.items():
                    content = content.replace(old_class, new_class)
                if content != original:
                    py_file.write_text(content, encoding='utf-8')
