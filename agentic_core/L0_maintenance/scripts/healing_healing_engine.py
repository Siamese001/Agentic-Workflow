...
class SovereignHealingEngine:
    ...
    async def execute_autonomous_cycle(self, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        target_issues = issues[:config.HEALING_MAX_FIXES_PER_CYCLE]
        affected_files = []
        
        try:
            # 1. Start Transaction (Pre-fix backup)
            logger.info("[L0 HEALING] Starting transaction with backups")
            
            for issue in target_issues:
                # New Granular Action Mapping
    ...
    async def _exec_replace_llm(self, fix: Dict) -> bool:
        ...
        try:
            ...
            # Add client import if missing
            if fix["new_client"] not in content:
                content = f"{fix['import_path']}\n{content}"
            
            # Replace Constructor
            content = re.sub(rf"{fix['sdk']}\(.*?\)", f"{fix['new_client']}", content)
            
            # Note: Method calls (e.g., .chat.completions) still require L1-specific refactoring
    ...
    async def _exec_replace_io(self, fix: Dict) -> bool:
        ...
        try:
            ...
            # Replace common file I/O patterns
            # Note: This is a simplified version - full implementation would need more sophisticated parsing
    ...
    async def _generate_fix(self, content: str, violation_type: str, message: str) -> Optional[str]:
        ...
        if "HTTP" in message or "requests" in message.lower():
            ...
            new_content = new_content.replace(
                "requests.post(",
                "# await get_fetch_client().fetch_url("
            )
        
        # Fix Redis violations
        if "Redis" in message:
            new_content = new_content.replace(
                "import redis",
        ...
        if "PATH_BREACH" in violation_type or "tools/" in message:
            ...
            new_content = new_content.replace(
                "from agentic_core.tools.",
                "from agentic_core.utils."
            )
    ...
    async def _create_healing_commit(self, affected_files: List[str]):
        ...
        try:
            ...
            for file in affected_files[:10]:  # Limit to first 10 in message
                commit_message += f"- {file}\n"
            
            if len(affected_files) > 10:
                commit_message += f"... and {len(affected_files) - 10} more files\n"
            
            await self.git_client.add_and_commit(
