...
class CoreSynthesisAnalyzer:
    ...
    def generate_report(self) -> str:
        ...
        archive_count = sum(
            1 for r in self.analysis_results if r.disposition == Disposition.ARCHIVE
        )
        synthesize_count = sum(
            1 for r in self.analysis_results if r.disposition == Disposition.SYNTHESIZE
        )
        ...
        report.append(
            f"- **KEEP Disposition:** {keep_count} files ({keep_count / total_files * 100:.1f}%)"
        )
        report.append(
        ...
        # Synthesis plan
        synthesize_results = [
            r for r in self.analysis_results if r.disposition == Disposition.SYNTHESIZE
        ]
