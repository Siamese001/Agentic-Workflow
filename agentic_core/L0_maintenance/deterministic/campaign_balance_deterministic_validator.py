...
class CampaignBalanceDeterministic:
    """
    ...
    def suggest_improvements(
        ...
        if ratio is not None:
            ...
            if ratio > max_ratio:
                ...
                suggestions.append(
                    f"Create {needed_messages - len(messages)} more message templates"
                )

            elif ratio < min_ratio:
                needed_leads = len(messages) * min_ratio
                suggestions.append(
                    f"Add {needed_leads - len(leads)} more leads or remove templates"
                )
