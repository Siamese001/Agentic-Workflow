"""
Deliverability Deterministic Layer

Extracted deterministic logic from DeliverabilityAgent.
This module contains pure deterministic deliverability validation.
...
class DeliverabilityDeterministic:
    """
    ...
    def validate_deliverability(self, messages: list[dict[str, Any]]) -> DeliverabilityResult:
        ...
        for i, message in enumerate(messages):
            ...
            # Check spam triggers (deterministic keyword matching)
            spam_issues = self._check_spam_triggers(content, i)
            issues.extend(spam_issues)

            # Check link count (deterministic counting)
            link_issues = self._check_link_count(content, i)
            issues.extend(link_issues)

            # Check image count (deterministic counting)
