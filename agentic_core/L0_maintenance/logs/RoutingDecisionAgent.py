...
class RootCustomsAgent:
    ...
    def __init__(self, project_root: Optional[Path] = None, dry_run: bool = True):
        ...
        print(f"🛃 Root Customs Agent Initialized")
    ...
    def determine_routing(self, file_path: Path, content_matches: Dict[str, Any]) -> RoutingDecision:
    ...
    def _calculate_routing_score(self, file_path: Path, content_matches: Dict[str, Any], config: Dict[str, Any]) -> float:
    ...
    def run_inspection(self) -> Dict[str, Any]:
        ...
        for file_path in root_files:
            ...
            # Display decision
            status_icon = "🛡️" if decision.is_protected else "✅" if decision.is_allowed_pattern else "📦" if decision.destination else "❓"
        ...
        return {
            ...
            'unmatched_files': sum(1 for d in self.routing_decisions if not d.destination and not d.is_protected and not d.is_allowed_pattern)
        }
    ...
    def _print_summary(self):
        ...
        unmatched = sum(1 for d in self.routing_decisions if not d.destination and not d.is_protected and not d.is_allowed_pattern)
