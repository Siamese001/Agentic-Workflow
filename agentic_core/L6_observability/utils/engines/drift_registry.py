    ...
    def _persist(self, entry: DriftRegistryEntry) -> None:
        try:
            ...
            with self._timeline_path.open("a", encoding="utf-8") as fh:
                ...
                fh.flush()
                os.fsync(fh.fileno())
        except (AttributeError, OSError, RuntimeError, TypeError, ValueError) as exc:  # guardian: allow-log-and-swallow -- drift timeline persist failed: non-fatal; in-memory record still maintained
