        ...
        <div style="margin-top:16px; padding:16px; background:#f8fafc; border-radius:8px; border-left:4px solid var(--primary);">
            ...
            <div style="font-size:0.85em; color:#475569; line-height:1.7;">
                <div style="display:grid; gap:10px;">
                    <div><strong>Heal Capability %:</strong> Percentage of agents that have healing capability through: (1) Direct implementation: Agent defines <code>heal()</code>, <code>apply_fix()</code>, <code>heal_violation()</code>, or <code>heal_repository()</code> method, or (2) Inheritance: Agent inherits from a class that has healing capability (MRO-aware detection via <code>has_healing_in_chain()</code>). <em>Factory analogy: Workers with personal repair toolkits—each has their own customized fix-it list for their station.</em></div>
