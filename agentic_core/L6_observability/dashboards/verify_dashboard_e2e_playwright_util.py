...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Automated Dashboard E2E Verification with Playwright', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--headless', action='store_true', help='Run browser in headless mode (no visible window)')
    parser.add_argument('--keep-server', action='store_true', help='Keep server running after verification (requires Ctrl+C to stop)')
...
async def mcp_verify_dashboard(url: str=DASHBOARD_URL, expected_min_rows: int=EXPECTED_MIN_ROWS) -> dict:
    ...
    try:
        ...
        return {'success': success, 'text_preview': page_text[:500], 'screenshot_name': screenshot_name, 'errors': errors, 'rows_found': rows_found}
    # guardian: allow-silent-swallow
    except Exception as e:
        return {'success': False, 'text_preview': '', 'screenshot_name': '', 'errors': [str(e)], 'rows_found': 0}
