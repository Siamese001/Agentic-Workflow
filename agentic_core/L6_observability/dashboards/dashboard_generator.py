...
from agentic_core.L5_safety.config.structure_blueprint import AGENT_DISCOVERY_JSON, AGENTIC_CORE_DIR, APPS_LIC_DIR, APPS_RG_DIR, APPS_SHARED_DIR, get_validated_project_root
from agentic_core.L0_routing.config.path_constants import BATCH_SIZE, BUFFER_SIZE, DEFAULT_SLEEP, DEFAULT_TIMEOUT, MAX_DEPTH, MAX_FILES, MAX_RETRIES, THRESHOLD
if sys.platform == 'win32':
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
TERRITORY_ORDER = ['Base/Root', 'Base/Mixins', 'L6 observability/Base Agent', 'L6 observability/Metrics', 'L6 observability/Tracing', 'L6 observability/Compliance', 'L6 observability/Infrastructure', 'L5 Safety/Base Agent', 'L5 Safety/Validators', 'L5 Safety/Guardrails', 'L5 Safety/Gravity', 'L5 Safety/Red Teaming', 'L4 State/Base Agent', 'L4 State/Core', 'L4 State/Infrastructure', 'L4 State/Specialized', 'L3 Orchestration/Base Agent', 'L3 Orchestration/Core', 'L3 Orchestration/Infrastructure', 'L3 Orchestration/Specialized', 'L2 Execution/Base Agent', 'L2 Execution/Core', 'L2 Execution/Specialized', 'L1 Cognition/Base Agent', 'L1 Cognition/Core', 'L1 Cognition/Specialized', 'L0 Maintenance/Base Agent', 'L0 Maintenance/Core', 'L0 Maintenance/Infrastructure', 'Apps Lic', 'Apps Rg', 'Apps Shared']
REQUIRED_FIELDS = ['Territory', 'Total', 'Compliant', 'Heal Cap %', 'Heal Invocation %', 'Invocation %', 'Hardened %', 'MCP Capable %', 'Test %', 'Observable %', 'Avg CC', 'Avg LOC', 'Typed %', 'Documented %', 'Metadata %', 'Canonical Inheritance %', 'schema Strictness %', 'Complexity Health', 'Code Quality Score', 'Criticality', 'Health', 'Health Breakdown', 'Risk', 'Used %', 'Priority', 'IsInfrastructure']
...
class DashboardGenerator:
    ...
    def group_agents_by_territory(self) -> dict[str, list[dict]]:
        ...
        for agent in self.agents:
            ...
            if APPS_LIC_DIR in sub_dir:
                ...
                if 'BaseAgent' in class_name or class_name == 'L1CognitionBase' or 'base_agent' in path.lower() or ('base_class' in path.lower()):
                ...
                if 'BaseAgent' in class_name or class_name == 'L0RoutingBaseAgent' or 'base_agent' in path.lower() or ('base_class' in path.lower()):
    ...
    def compute_territory_metrics(self, agents_list: list[dict]) -> dict[str, Any]:
        ...
        base_health = round(heal_cap_pct * 0.3 + heal_inv_pct * 0.1 + test_pct * 0.25 + obs_pct * 0.2 + complexity_health * 0.15, 1)
        ...
        return {'total': total, 'compliant': heal_cap, 'heal_cap_pct': heal_cap_pct, 'heal_inv_pct': heal_inv_pct, 'test_pct': test_pct, 'obs_pct': obs_pct, 'avg_cc': avg_cc, 'avg_loc': avg_loc, 'typed_pct': typed_pct, 'doc_pct': doc_pct, 'schema_pct': schema_pct, 'metadata_pct': metadata_pct, 'used_pct': used_pct, 'complexity_health': complexity_health, 'code_quality': code_quality, 'health': health, 'risk': risk, 'hardened_pct': hardened_pct, 'mcp_pct': mcp_pct, 'proper_base_pct': proper_base_pct}
    ...
    def calculate_layer_criticality(self, territory_name: str) -> int:
        ...
        """
        LAYER_WEIGHTS = {'L5': 100, 'Base': 95, 'L4': 85, 'L3': 75, 'Apps': 70, 'L2': 60, 'L1': 50, 'L0': 40, 'L6': 30}
    ...
    def build_territory_row(self, territory_name: str, metrics: dict[str, Any], priority: int, is_infrastructure: bool=False) -> dict[str, Any]:
        ...
        return {'Territory': territory_name, 'Total': metrics['total'], 'Compliant': metrics['compliant'], 'Heal Cap %': metrics['heal_cap_pct'], 'Heal Invocation %': metrics['heal_inv_pct'], 'Invocation %': metrics['heal_inv_pct'], 'Hardened %': metrics['hardened_pct'], 'MCP Capable %': metrics['mcp_pct'], 'Test %': metrics['test_pct'], 'Observable %': metrics['obs_pct'], 'Avg CC': metrics['avg_cc'], 'Avg LOC': metrics['avg_loc'], 'Typed %': metrics['typed_pct'], 'Documented %': metrics['doc_pct'], 'Metadata %': metrics['metadata_pct'], 'Canonical Inheritance %': metrics['proper_base_pct'], 'schema Strictness %': metrics['schema_pct'], 'Complexity Health': metrics['complexity_health'], 'Code Quality Score': metrics['code_quality'], 'Criticality': self.calculate_layer_criticality(territory_name), 'Health': metrics['health'], 'Health Breakdown': f"Heal:{metrics['heal_cap_pct']:.0f}+Inv:{metrics['heal_inv_pct']:.0f}+Test:{metrics['test_pct']:.0f}+Obs:{metrics['obs_pct']:.0f}+CC:{metrics['complexity_health']:.0f}", 'Risk': metrics['risk'], 'Used %': metrics['used_pct'], 'Priority': priority, 'IsInfrastructure': is_infrastructure}
    ...
    def build_total_row(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        health = round(heal_cap_pct * 0.3 + heal_inv_pct * 0.1 + test_pct * 0.25 + obs_pct * 0.2 + complexity_health * 0.15, 1)
        risk = 'HIGH' if avg_cc > 12 or health < 60 else 'MED' if avg_cc > 8 or health < 80 else 'LOW'
        return {'Territory': 'TOTAL', 'Total': total_agents, 'Compliant': sum((r['Compliant'] for r in rows)), 'Heal Cap %': heal_cap_pct, 'Heal Invocation %': heal_inv_pct, 'Invocation %': heal_inv_pct, 'Hardened %': hardened_pct, 'MCP Capable %': mcp_pct, 'Test %': test_pct, 'Observable %': obs_pct, 'Avg CC': avg_cc, 'Avg LOC': avg_loc, 'Typed %': typed_pct, 'Documented %': doc_pct, 'Metadata %': metadata_pct, 'Canonical Inheritance %': proper_base_pct, 'schema Strictness %': schema_pct, 'Complexity Health': complexity_health, 'Code Quality Score': code_quality, 'Criticality': avg_criticality, 'Health': health, 'Health Breakdown': f'Heal:{heal_cap_pct:.0f}+Inv:{heal_inv_pct:.0f}+Test:{test_pct:.0f}+Obs:{obs_pct:.0f}+CC:{complexity_health:.0f}', 'Risk': risk, 'Used %': used_pct, 'Priority': 'ALL', 'IsInfrastructure': False}
    ...
    def build_per_agent_data(self, territories: dict[str, list[dict]]) -> dict[str, dict]:
        ...
        for territory_name in TERRITORY_ORDER:
            ...
            if not agents_list:
                per_agent_data[territory_name] = {'healCap': [], 'invocation': [], 'hardened': [], 'test': [], 'complexityHealth': [], 'health': [], 'typed': [], 'documented': [], 'schemaStrictness': [], 'properBase': [], 'codeQuality': [], 'agents': []}
                continue
            heal_cap_values = []
            invocation_values = []
            hardened_values = []
            ...
            for agent in agents_list:
                ...
                agent_objects.append({'name': agent.get('class_name', 'Unknown'), 'path': agent.get('path', ''), 'rel': rel_path, 'abs_file': abs_path, 'abs_class': abs_path, 'class_line': agent.get('line_number', 1), 'has_mixin': agent.get('has_healing', False), 'invocation': agent.get('invocation', 'No'), 'has_tests': agent.get('has_tests', False), 'obs_summary': obs_summary, 'mcp_summary': mcp_summary, 'typing_summary': typing_summary, 'typed_pct': typed, 'overall_typed_pct': typed, 'complexity': cc, 'health': health, 'healCap': heal_cap, 'test': test, 'complexityHealth': complexity_health, 'hardened': hardened, 'documented': documented, 'schema': schema, 'base': base, 'quality': quality, 'loc': agent.get('loc', 0)})
            per_agent_data[territory_name] = {'healCap': heal_cap_values, 'invocation': invocation_values, 'hardened': hardened_values, 'test': test_values, 'complexityHealth': complexity_health_values, 'health': health_values, 'typed': typed_values, 'documented': documented_values, 'schemaStrictness': schema_values, 'properBase': base_values, 'codeQuality': quality_values, 'agents': agent_objects}
    ...
    def validate_dashboard_data(self, data: list[dict[str, Any]]) -> bool:
        ...
        for i, row in enumerate(data[1:], 1):
            if row.get('Total', 0) > 0:
                ...
                expected_health = round(heal_cap * 0.3 + heal_inv * 0.1 + test * 0.25 + obs * 0.2 + complexity * 0.15, 1)
    ...
    def validate_html_before_write(self, html: str) -> tuple[bool, list[str]]:
        ...
        errors = []
        const_patterns = {'dashboardData': 'const\\s+dashboardData\\s*=', 'realAgentData': 'const\\s+realAgentData\\s*='}
    ...
    def update_dashboard_html(self, data: list[dict[str, Any]], per_agent_data: dict[str, dict]) -> bool:
        ...
        try:
            ...
            real_agent_pattern3 = 'const realAgentData = \\{.*?\\n\\s*(?=const |function |let |var |\\s*</script>)'
