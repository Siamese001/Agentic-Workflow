...
class DashboardDataGenerator:
    ...
    def compute_territory_metrics(self, agents: list[Path], used_stems: set, registry: dict[str, Any]) -> dict[str, Any]:
        ...
        m = {'total': len(agents), 'compliant': 0, 'heal_cap': 0, 'heal_inv': 0, 'test': 0, 'cc_sum': 0, 'typed': 0, 'doc': 0, 'obs': 0, 'used': 0, 'schema_strictness': 0.0, 'proper_base': 0.0}
        for agent in agents:
    ...
    def build_territory_row(self, territory_name: str, metrics: dict[str, Any], priority: int, is_infrastructure: bool) -> dict[str, Any]:
        ...
        return {'Territory': territory_name, 'Total': t, 'Heal Cap %': round(metrics['heal_cap'] / t * 100, 1), 'Heal Invocation %': round(metrics['heal_inv'] / t * 100, 1), 'Test %': round(metrics['test'] / t * 100, 1), 'Avg CC': avg_cc, 'Typed %': round(metrics['typed'] / t, 1), 'Documented %': round(metrics['doc'] / t, 1), 'Health': health, 'Risk': 'HIGH' if avg_cc > 12 or health < 60 else 'MED' if avg_cc > 8 or health < 80 else 'LOW', 'Priority': priority, 'schema Strictness %': round(metrics['schema_strictness'] / t, 1), 'Proper Base %': round(metrics['proper_base'] / t, 1)}
    ...
    def build_total_row(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {'Territory': 'TOTAL', 'Total': total_agents, 'Heal Cap %': weighted_avg('Heal Cap %'), 'Heal Invocation %': weighted_avg('Heal Invocation %'), 'Test %': weighted_avg('Test %'), 'Avg CC': weighted_avg('Avg CC'), 'Typed %': weighted_avg('Typed %'), 'Documented %': weighted_avg('Documented %'), 'Health': health, 'Risk': 'HIGH' if health < 70 else 'MED' if health < 85 else 'LOW', 'schema Strictness %': weighted_avg('schema Strictness %'), 'Proper Base %': weighted_avg('Proper Base %')}
