...
def analyze_color_bug():
    ...
    gradient_match = re.search(
        "const getGradientBg = \\(value, target = \\d+\\) => \\{(.*?)\\};", html, re.DOTALL
    )
    if gradient_match:
    ...
    outlier_match = re.search(
        "function formatOutlierBadge\\(countAtZero, countBelowThreshold, threshold = \\d+\\)", html
    )
    if outlier_match:
    ...
    badge_match = re.search(
        "\\$\\{formatOutlierBadge\\(healCapOutliers\\.atZero, healCapOutliers\\.belowThreshold, 50\\)\\}",
        html,
    )
    if badge_match:
        print(
