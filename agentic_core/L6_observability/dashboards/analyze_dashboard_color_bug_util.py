...
def analyze_color_bug():
    ...
    gradient_match = re.search('const getGradientBg = \\(value, target = \\d+\\) => \\{(.*?)\\};', html, re.DOTALL)
    if gradient_match:
    ...
    heal_cap_bg_match = re.search('background:\\$\\{getGradientBg\\(healCapStats\\.min\\)\\}', html)
    ...
    outlier_match = re.search('function formatOutlierBadge\\(countAtZero, countBelowThreshold, threshold = \\d+\\)', html)
    if outlier_match:
    ...
    badge_match = re.search('\\$\\{formatOutlierBadge\\(healCapOutliers\\.atZero, healCapOutliers\\.belowThreshold, 50\\)\\}', html)
    if badge_match:
    ...
    print('   - Health')
    return {'total_heal_cap': total_row['Heal Cap %'], 'min_heal_cap': min_heal_cap, 'bug_confirmed': True, 'fix_location': 'Lines 2525-2545 (TOTAL row cell backgrounds)'}
