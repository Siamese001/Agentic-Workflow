...
_CANONICAL_PROVIDERS: dict[str, str] = {'anthropic': 'claude-3-5-sonnet', 'deterministic': 'LOCAL_AGENT', 'gemini': 'gemini-2.5-pro', 'openai': 'gpt-4o', 'qwen': 'Qwen2.5-14B-Instruct-AWQ'}
...
class ProviderBindingFingerprint:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.fingerprint, str) or len(self.fingerprint) != 64:
            raise ValueError(f'ProviderBindingFingerprint: fingerprint must be 64-char hex, got {self.fingerprint!r}')
...
def capture_provider_bindings(overrides: dict[str, str] | None=None) -> ProviderBindingFingerprint:
    ...
    if overrides:
        for pid, mid in sorted(overrides.items()):
            registry[pid] = mid
    tier_map = {'deterministic': 'DETERMINISTIC', 'qwen': 'QWEN', 'gemini': 'GEMINI', 'anthropic': 'LLM_API', 'openai': 'LLM_API'}
    bindings = tuple((ProviderBinding(provider_id=pid, model_id=mid, tier=tier_map.get(pid, 'UNKNOWN')) for pid, mid in sorted(registry.items())))
    material = {'bindings': [{'model_id': b.model_id, 'provider_id': b.provider_id, 'tier': b.tier} for b in bindings]}
...
__all__ = ['ProviderBinding', 'ProviderBindingFingerprint', 'capture_provider_bindings', 'fingerprint_matches']
