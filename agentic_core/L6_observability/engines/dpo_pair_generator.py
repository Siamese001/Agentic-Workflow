...
def create_bounded_dpo_pairs(pairs: list[DPOPair], policy: DPOBoundingPolicy) -> list[BoundedDPOPair]:
    ...
    """
    # 1. Deterministic Sort: by (control_hash, candidate_hash).
    #    This is critical to prevent replay mismatches.
    sorted_pairs = sorted(pairs, key=lambda p: (p.control_hash, p.candidate_hash))

    bounded_pairs: list[BoundedDPOPair] = []
    for pair in sorted_pairs:
        # 2. Clamp the raw score to the absolute min/max range.
        clamped_score = max(policy.min_clamp, min(pair.raw_score, policy.max_clamp))

        # 3. Enforce the maximum delta from a neutral score of 1.0.
    ...
    # The digest contribution would include the policy values and a hash of the results.
    # digest_material = {
    #     "policy": asdict(policy),
    #     "result_hashes": [p.pair.candidate_hash for p in bounded_pairs],
    # }
