...
class EntropyTelemetryEngine:
    ...
    def __init__(self, window_size: int = 1000) -> None:
        ...
        # Path D tracking
        self._path_d_events: deque[tuple[str, int, int]] = deque(maxlen=window_size)  # (reason, start, end)
    ...
    def _compute_metrics(self) -> None:
        ...
        total_decisions = len(self._tier_decisions)
        flip_rate = total_flips / max(1, total_decisions - 1)  # -1 because first decision can't be a flip
...
# Global instance for system-wide entropy telemetry
