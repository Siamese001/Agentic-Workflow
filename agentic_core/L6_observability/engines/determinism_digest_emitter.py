...
class DeterminismDigestEmitter:
    ...
    def compute(self, *, policy_hash: str, registry_hash: str, config_surface_hash: str, transcript_hash: str, dependency_lock_hash: str) -> str:
        ...
        """
        components = {'config_surface_hash': config_surface_hash, 'dependency_lock_hash': dependency_lock_hash, 'policy_hash': policy_hash, 'registry_hash': registry_hash, 'transcript_hash': transcript_hash}
        for name, value in components.items():
            if not isinstance(value, str) or len(value) != 64:
                raise ValueError(f'DeterminismDigestEmitter: {name} must be a 64-char hex string, got {value!r}')
    ...
    def emit_once(self, digest: str) -> str:
        ...
        if not isinstance(digest, str) or len(digest) != 64:
            raise ValueError(f'DeterminismDigestEmitter.emit_once: digest must be 64-char hex, got {digest!r}')
        with self._LOCK:
            if self._emitted:
                raise DuplicateEmissionError('DeterminismDigestEmitter: DETERMINISM-DIGEST already emitted for this instance.')
            self._emitted = True
...
def build_stable_config_surface() -> dict[str, Any]:
    ...
    """
    return {'blas_eps': 1e-12, 'cutoff': 0.5, 'decision_delta_limit': 0.1, 'embedding_batch': 500, 'embedding_enabled': True, 'embedding_retry': 8, 'max_k': 20, 'meta_learning_enabled': True, 'model_version': 'multilingual-e5-large', 'oscillation_detector_enabled': True, 'proposal_only': True, 'rlhf_delta_max': 2.0, 'rlhf_delta_min': 0.1, 'top_k': 20}
...
def _canonical_json_bytes(data: Any) -> bytes:
    ...
    return json.dumps(data, sort_keys=True, separators=(',', ':'), ensure_ascii=True, cls=_Encoder).encode('utf-8')
__all__ = ['DeterminismDigestEmitter', 'DuplicateEmissionError', 'build_stable_config_surface', 'hash_config_surface']
