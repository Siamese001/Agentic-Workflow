...
class DeterminismDigestEmitter:
        ...
        *,
        ...
        for name, value in components.items():
            if not isinstance(value, str) or len(value) != 64:
                raise ValueError(
                    f"DeterminismDigestEmitter: {name} must be a 64-char hex string"
                    f", got {value!r}"
                )
    ...
    def emit_once(self, digest: str) -> str:
        ...
        if not isinstance(digest, str) or len(digest) != 64:
            raise ValueError(
                f"DeterminismDigestEmitter.emit_once: digest must be 64-char hex"
                f", got {digest!r}"
            )
        with self._LOCK:
            if self._emitted:
                raise DuplicateEmissionError(
                    "DeterminismDigestEmitter: DETERMINISM-DIGEST already emitted"
                    " for this instance."
                )
            self._emitted = True
...
# ---------------------------------------------------------------------------
# Stable config surface builder — no wall-clock, no env-vars
# ---------------------------------------------------------------------------
