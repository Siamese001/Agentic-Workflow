...
class DPOExampleId:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        """
        return json.dumps({'control_hash': self.control_hash, 'candidate_hash': self.candidate_hash}, separators=(',', ':'), sort_keys=True).encode('ascii')
...
class DPOPair:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        Returns:
            Bytes with deterministic ordering and formatting.
        """
        return json.dumps({'example_id': {'control_hash': self.example_id.control_hash, 'candidate_hash': self.example_id.candidate_hash}, 'control_output_hash': self.control_output_hash, 'candidate_output_hash': self.candidate_output_hash, 'human_decision': self.human_decision, 'reasons': list(self.reasons)}, separators=(',', ':'), sort_keys=True).encode('ascii')
...
class DPOBatch:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        for pair in self.pairs:
            pairs_data.append({'example_id': {'control_hash': pair.example_id.control_hash, 'candidate_hash': pair.example_id.candidate_hash}, 'control_output_hash': pair.control_output_hash, 'candidate_output_hash': pair.candidate_output_hash, 'human_decision': pair.human_decision, 'reasons': list(pair.reasons)})
