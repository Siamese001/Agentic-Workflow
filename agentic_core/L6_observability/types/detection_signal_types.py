...
class DetectionSignal:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        doc = {'anomaly_score': self.anomaly_score, 'created_at_utc': self.created_at_utc, 'escalation_rate': self.escalation_rate, 'mission_id': self.mission_id, 'retry_rate': self.retry_rate, 'schema_version': self.schema_version, 'violation_density': self.violation_density}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()

    @staticmethod
    def compute_hash(schema_version: int, mission_id: str, created_at_utc: int, anomaly_score: float, escalation_rate: float, retry_rate: float, violation_density: float) -> str:
        """Compute signal_hash from raw field values (before construction)."""
        doc = {'anomaly_score': anomaly_score, 'created_at_utc': created_at_utc, 'escalation_rate': escalation_rate, 'mission_id': mission_id, 'retry_rate': retry_rate, 'schema_version': schema_version, 'violation_density': violation_density}
        raw = json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()
        return _sha256(raw)

    @classmethod
    def build(cls, mission_id: str, created_at_utc: int, anomaly_score: float, escalation_rate: float, retry_rate: float, violation_density: float, schema_version: int=1) -> DetectionSignal:
        """Factory: compute signal_hash automatically."""
        h = cls.compute_hash(schema_version=schema_version, mission_id=mission_id, created_at_utc=created_at_utc, anomaly_score=anomaly_score, escalation_rate=escalation_rate, retry_rate=retry_rate, violation_density=violation_density)
        return cls(schema_version=schema_version, mission_id=mission_id, created_at_utc=created_at_utc, anomaly_score=anomaly_score, escalation_rate=escalation_rate, retry_rate=retry_rate, violation_density=violation_density, signal_hash=h)
