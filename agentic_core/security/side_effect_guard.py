...
class SideEffectGuard:
    ...
    def __init__(self):
        self._active_context: Optional[VerificationContext] = None
    ...
    def set_context(self, context: VerificationContext) -> None:
        ...
        if not context.is_verified:
            raise UnverifiedSideEffectError(
                "ATTEMPT_TO_SET_UNVERIFIED_CONTEXT: Context must be verified"
            )
    ...
    def require_verified(self, operation: str = "side-effect") -> VerificationContext:
        ...
        if not self._guard_enabled:
            ...
            # Return a dummy context when disabled
            from .signature_verifier import VerificationContext
...
_global_guard: Optional[SideEffectGuard] = None
...
# Decorator for functions requiring verification
def requires_verification(operation_name: Optional[str] = None):
