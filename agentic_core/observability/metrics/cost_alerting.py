...
class CostTracker:
    ...
    def __init__(self, default_budget_per_agent: Optional[float]=None, alert_threshold_percent: float=0.8, enable_logging: bool=True):
        ...
        if self.enable_logging:
            LOGGER.info('cost_tracker_initialized', extra={'default_budget': default_budget_per_agent, 'alert_threshold': alert_threshold_percent})
    ...
    def record_cost(self, agent_id: str, spiffe_id: str, model_id: str, tokens: int, cost: float) -> None:
        ...
        record: Any = {'spiffe_id': spiffe_id, 'model_id': model_id, 'tokens': tokens, 'cost': cost, 'timestamp': time.time()}
    ...
    def get_metrics(self, agent_id: str, period_hours: int=24) -> Optional[CostMetrics]:
        ...
        Returns:
            CostMetrics or None
        """
        records: Any = self._agent_costs.get(agent_id, [])
        if not records:
            return None
        period_start: Any = time.time() - period_hours * 3600
        period_records: Any = [r for r in records if r['timestamp'] >= period_start]
        if not period_records:
            return None
        total_cost: Any = sum((r['cost'] for r in period_records))
        token_count: Any = sum((r['tokens'] for r in period_records))
        request_count: Any = len(period_records)
        avg_cost: Any = total_cost / request_count if request_count > 0 else 0.0
        model_breakdown: Dict[str, float] = {}
        for record in period_records:
            model_id: Any = record['model_id']
            model_breakdown[model_id] = model_breakdown.get(model_id, 0.0) + record['cost']
        spiffe_id: Any = period_records[-1]['spiffe_id']
        metrics: Any = CostMetrics(agent_id=agent_id, spiffe_id=spiffe_id, total_cost=total_cost, token_count=token_count, request_count=request_count, avg_cost_per_request=avg_cost, period_start=period_start, period_end=time.time(), model_breakdown=model_breakdown)
        return metrics
    ...
    def get_all_metrics(self, period_hours: int=24) -> List[CostMetrics]:
        ...
        for agent_id in self._agent_costs.keys():
            metrics: Any = self.get_metrics(agent_id, period_hours)
            if metrics:
                all_metrics.append(metrics)
        return all_metrics

    def get_alerts(self, agent_id: Optional[str]=None, level: Optional[CostAlertLevel]=None) -> List[CostAlert]:
        """Get cost alerts.
        ...
        if level:
            alerts: Any = [a for a in alerts if a.level == level]
        return alerts
    ...
    def _check_budget(self, agent_id: str, spiffe_id: str) -> None:
        ...
        if usage_percent >= 1.0:
            self._create_alert(agent_id=agent_id, spiffe_id=spiffe_id, level=CostAlertLevel.CRITICAL, message=f'Budget exceeded: ${current_cost:.2f} / ${budget:.2f}', current_cost=current_cost, budget_limit=budget)
        elif usage_percent >= self.alert_threshold_percent:
            self._create_alert(agent_id=agent_id, spiffe_id=spiffe_id, level=CostAlertLevel.WARNING, message=f'Budget at {usage_percent:.1%}: ${current_cost:.2f} / ${budget:.2f}', current_cost=current_cost, budget_limit=budget)

    def _create_alert(self, agent_id: str, spiffe_id: str, level: CostAlertLevel, message: str, current_cost: float, budget_limit: float) -> None:
        """Create cost alert.
        ...
        """
        alert = CostAlert(alert_id=f'cost_alert_{agent_id}_{int(time.time())}', agent_id=agent_id, spiffe_id=spiffe_id, level=level, message=message, current_cost=current_cost, budget_limit=budget_limit)
        self._alerts.append(alert)
        if len(self._alerts) > 100:
            self._alerts = self._alerts[-100:]
        if self.enable_logging:
            LOGGER.warning('cost_alert_triggered', extra={'agent_id': agent_id, 'level': level.value, 'current_cost': current_cost, 'budget': budget_limit})

def create_cost_tracker(default_budget_per_agent: Optional[float]=None) -> CostTracker:
