...
# ---------------------------------------------------------------------------
# EvaluationSignals — new input class for Meta-Learning
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Completeness-Aware RAGProposer Extension
# ---------------------------------------------------------------------------

_MIN_OBSERVATIONS = 5
_LOW_COMPLETENESS_THRESHOLD = 0.60
_HIGH_FRAGMENTATION_THRESHOLD = 0.30
_LOW_SUPPORT_THRESHOLD = 0.60
_HIGH_SIM_WRONG_ANSWER_THRESHOLD = 0.20
_LOW_PARENT_EXPANSION_RATE = 0.30
...
class CompletenessRAGProposer:
    ...
    def propose(
        ...
        if (
            ...
            proposals.append(
                CompletenessChangePackage(
                    ...
                    new_value=2,
                    justification=(
                        f"mean_completeness={signals.mean_completeness_score:.3f} < "
                        f"{_LOW_COMPLETENESS_THRESHOLD}; "
                        f"parent_expansion_rate={signals.parent_reconstruction_applied_rate:.3f} < "
                        f"{_LOW_PARENT_EXPANSION_RATE}; increase expansion depth"
                    ),
        ...
        if signals.chunk_fragmentation_error_rate > _HIGH_FRAGMENTATION_THRESHOLD:
            proposals.append(
                CompletenessChangePackage(
                    ...
                    justification=(
                        f"chunk_fragmentation_error_rate="
                        f"{signals.chunk_fragmentation_error_rate:.3f} > "
                        f"{_HIGH_FRAGMENTATION_THRESHOLD}; switch to section-aware chunking"
                    ),
                    snapshot_id=signals.snapshot_id,
                    proposal_only=True,
                )
        ...
        if (
            ...
            proposals.append(
                CompletenessChangePackage(
                    ...
                    new_value="hybrid",
                    justification=(
                        f"fully_supported_rate={signals.fully_supported_rate:.3f} < "
                        f"{_LOW_SUPPORT_THRESHOLD}; "
                        f"high_similarity_wrong_answer_rate="
                        f"{signals.high_similarity_wrong_answer_rate:.3f} > "
                        f"{_HIGH_SIM_WRONG_ANSWER_THRESHOLD}; enable hybrid retrieval"
                    ),
                    snapshot_id=signals.snapshot_id,
                    proposal_only=True,
                )
        ...
        if signals.missing_condition_rate > 0.30 or signals.missing_scope_rate > 0.30:
            proposals.append(
                CompletenessChangePackage(
                    ...
                    new_value=1.5,
                    justification=(
                        f"missing_condition_rate={signals.missing_condition_rate:.3f}, "
                        f"missing_scope_rate={signals.missing_scope_rate:.3f}; "
                        f"raise lexical exact-match boost for codes/conditions/versions"
                    ),
                    snapshot_id=signals.snapshot_id,
                    proposal_only=True,
                )
            ...
            proposals.append(
                CompletenessChangePackage(
                    ...
                    justification=(
                        f"mean_completeness={signals.mean_completeness_score:.3f} < "
                        f"{_LOW_COMPLETENESS_THRESHOLD}; "
                        f"increase reranker completeness weight over similarity"
                    ),
                    snapshot_id=signals.snapshot_id,
                    proposal_only=True,
                )
        ...
        if signals.parent_reconstruction_applied_rate < _LOW_PARENT_EXPANSION_RATE:
            proposals.append(
                CompletenessChangePackage(
                    ...
                    justification=(
                        f"parent_expansion_rate={signals.parent_reconstruction_applied_rate:.3f} < "
                        f"{_LOW_PARENT_EXPANSION_RATE}; increase neighbor window size"
                    ),
                    snapshot_id=signals.snapshot_id,
                    proposal_only=True,
                )
