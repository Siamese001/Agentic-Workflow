...
class JudgeScore:
    ...
    @classmethod
    def create(cls, faithfulness: float, answer_relevancy: float, context_precision: float, groundedness: float, reasoning: str, judge_model: str) -> JudgeScore:
        canonical = json.dumps({'faithfulness': faithfulness, 'answer_relevancy': answer_relevancy, 'context_precision': context_precision, 'groundedness': groundedness, 'judge_model': judge_model}, sort_keys=True, separators=(',', ':'))
        digest = hashlib.sha256(canonical.encode('utf-8')).hexdigest()
        return cls(faithfulness=faithfulness, answer_relevancy=answer_relevancy, context_precision=context_precision, groundedness=groundedness, reasoning=reasoning, judge_model=judge_model, deterministic_digest=digest)
...
class NullJudge:
    ...
    def score(self, query: str, context: str, answer: str) -> JudgeScore:
        return JudgeScore.create(faithfulness=self.FIXED_SCORE, answer_relevancy=self.FIXED_SCORE, context_precision=self.FIXED_SCORE, groundedness=self.FIXED_SCORE, reasoning='NullJudge: deterministic stub', judge_model='null')
...
class GeminiJudge:
    ...
    def score(self, query: str, context: str, answer: str) -> JudgeScore:
        ...
        return JudgeScore.create(faithfulness=float(data.get('faithfulness', 1)), answer_relevancy=float(data.get('answer_relevancy', 1)), context_precision=float(data.get('context_precision', 1)), groundedness=float(data.get('groundedness', 1)), reasoning=str(data.get('reasoning', '')), judge_model=self.MODEL_ID)
