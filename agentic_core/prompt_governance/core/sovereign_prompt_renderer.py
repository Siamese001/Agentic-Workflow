...
class SovereignPromptRenderer:
    ...
    def _parse_template_schema(self, template_name: str) -> TemplateSchema:
        ...
        schema_match = re.search('\\{#\\s*SCHEMA:\\s*required_vars=\\[([^\\]]*)\\](?:,\\s*optional_vars=\\[([^\\]]*)\\])?\\s*#\\}', content)
        if schema_match:
        ...
        schema = TemplateSchema(required_vars=required_vars, optional_vars=optional_vars, description=description)
    ...
    def validate_context(self, template_name: str, context: dict[str, Any]) -> None:
        ...
        if missing_vars:
            raise TemplateValidationError(f"Template '{template_name}' requires variables {missing_vars} but only {provided_vars} were provided. Description: {schema.description}")

    def render(self, template_name: str, context: dict[str, Any] | None=None, metadata: dict[str, Any] | None=None, validate: bool=True) -> str:
        ...
        if validate:
            try:
                ...
                if 'No description provided' not in str(e):
                    raise
        full_context = {**context, '_sovereign_metadata': {'renderer': self.__class__.__name__, 'template': template_name, 'root': str(self.template_root), **metadata}}
        try:
    ...
    def render_tagentic(self, base_template: str, fragments: list[str], context: dict[str, Any] | None=None, validate: bool=True) -> str:
