...
from agentic_core.prompt_governance.contracts.slot_contracts import SLOT_ORDER, SlotC0, SlotD0, SlotI0, SlotS0, SlotU0, validate_slot_order
from agentic_core.prompt_governance.security.validators.output_schema_validator import validate_against_schema, validate_context_contract, validate_healer_reentry
from agentic_core.L0_routing.config.path_constants import BATCH_SIZE, BUFFER_SIZE, DEFAULT_SLEEP, DEFAULT_TIMEOUT, MAX_DEPTH, MAX_FILES, MAX_RETRIES, THRESHOLD
__all__ = ['AssembledPrompt', 'PromptAssembler', 'PromptComponents', 'PromptTemplate', 'SecurityIntegrityError']
...
class PromptAssembler:
    ...
    def _load_templates(self) -> None:
        ...
        for file_path in xml_files:
            try:
                ...
                self.templates[template_name] = PromptTemplate(name=template_name, template=template_content, description='Custom template')
                Logger.debug(f'Loaded template: {template_name}')
            except Exception as e:
                raise
                Logger.error(f'Failed to load template {file_path}: {e}')

    def assemble(self, role: str, objective: str, context_data: dict[str, Any] | str, injections: list[InjectionMatch], negative_constraints: list[str] | None=None, examples: str | None=None, output_schema: dict[str, Any] | None=None, template_name: str | None=None, metadata: dict[str, Any] | None=None, enforce_contract: bool=False, contract_id: str | None=None) -> str:
        ...
        _slot_map: dict[str, object] = {'S0': SlotS0(content=f'{role}: {objective}'), 'D0': SlotD0(content=_healer_directive or 'directives', authority='BINDING'), 'I0': SlotI0(content='instructional'), 'C0': SlotC0(content=_normalized), 'U0': SlotU0(content=str(context_data))}
        ...
        try:
            ...
            for injection in injections:
                if hasattr(injection, 'content'):
                    ...
                    sanitized_injection = type(injection)(pattern=injection.pattern, content=sanitized_content, **{k: v for k, v in injection.__dict__.items() if k not in ['pattern', 'content']})
        ...
        prompt = template.format(role=sanitized_role, objective=sanitized_objective, context_data=context_str, directives=directives, negative_constraints=negative_str, examples=sanitized_examples if sanitized_examples else '', output_format=output_format)
    ...
    def assemble_with_schema(self, role: str, objective: str, context_data: dict[str, Any] | str, injections: list[InjectionMatch], **kwargs) -> AssembledPrompt:
        ...
        """
        prompt_text = self.assemble(role=role, objective=objective, context_data=context_data, injections=injections, **kwargs)
        return AssembledPrompt(text=prompt_text, manifest_hash=getattr(self, '_last_manifest_hash', ''), response_schema=self._last_response_schema)
    ...
    def _format_directives(self, injections: list[InjectionMatch]) -> str:
        ...
        lines = []
        sorted_injections = sorted(injections, key=lambda x: (x.injection.priority, x.relevance_score), reverse=True)
    ...
    def parse_response(self, response: str, *, schema: Any | None=None) -> dict[str, Any]:
        ...
        if schema is not None:
            ...
            if not ok:
                return {'type': 'schema_validation_failed', 'content': result.get('content'), 'schema_error_code': code, 'schema_error': details}
        return result
...
def assemble_prompt(role: str, objective: str, context_data: dict[str, Any] | str, injections: list[InjectionMatch], **kwargs) -> str:
    ...
    return assembler.assemble(role=role, objective=objective, context_data=context_data, injections=injections, **kwargs)

def assemble_prompt_with_schema(role: str, objective: str, context_data: dict[str, Any] | str, injections: list[InjectionMatch], **kwargs) -> AssembledPrompt:
    ...
    """
    assembler = get_prompt_assembler()
    return assembler.assemble_with_schema(role=role, objective=objective, context_data=context_data, injections=injections, **kwargs)
...
def enhance_prompt_with_fencing(base_prompt: str, injections: list[InjectionMatch], role: str='Assistant', objective: str='Follow the instructions', context: dict[str, Any] | None=None) -> str:
    ...
    return assemble_prompt(role=role, objective=objective, context_data=context, injections=injections, legacy_mode=True)
