...
class DashboardTestSuite:
    ...
    def test_data_integrity(self):
        ...
        self.test(
            "Has TOTAL row",
            any(r.get("Territory") == "TOTAL" for r in self.dashboard_data),
            "Missing TOTAL row",
        )
        self.test(
            "Has multiple territories",
            len(self.dashboard_data) >= 10,
            f"Only {len(self.dashboard_data)} rows",
        )
        ...
        for row in self.dashboard_data[:5]:  # Check first 5 rows
            if row.get("Territory") != "TOTAL":
                for field in required_fields:
                    self.test(
                        f"Field '{field}' in {row.get('Territory', 'unknown')[:20]}",
                        field in row,
    ...
    def test_target_configuration(self):
        ...
        if non_total_rows:
            ...
            self.test(
                "Target Invocation present",
                "Target Invocation" in sample,
                f"Missing in {sample.get('Territory')}",
            )
            self.test(
                "Target MCP present",
                "Target MCP" in sample,
                f"Missing in {sample.get('Territory')}",
            )
            self.test(
                "Target Tests present",
                "Target Tests" in sample,
                f"Missing in {sample.get('Territory')}",
            )
            self.test(
            ...
            self.test(
                "Target Complexity present",
                "Target Complexity" in sample,
                f"Missing in {sample.get('Territory')}",
            )
            ...
            self.test(
                "Invocation targets match autonomy_targets.py config",
                valid_targets,
    ...
    def test_sparkline_infrastructure(self):
        # Check CSS classes
        self.test(
            "Sparkline container CSS",
            ".sparkline-container" in self.template_html,
            "Missing .sparkline-container CSS",
        )
        self.test(
            "Sparkline SVG CSS",
            ".sparkline-svg" in self.template_html or "sparkline" in self.template_html.lower(),
        ...
        self.test(
            "Sparkline generation function exists",
            found_any,
    ...
    def test_drilldown_data(self):
        ...
        self.test(
            "Territories have agents arrays",
        ...
        for row in self.dashboard_data:
            if row.get("Territory") != "TOTAL" and "agents" in row and row["agents"]:
                ...
                self.test(
                    "Agent has 'compliant' field",
                    "compliant" in agent,
                    "Missing 'compliant' in agent",
                )
                break
    ...
    def test_strategic_recommendations(self):
        ...
        self.test(
            "Recommendations render function exists",
    ...
    def test_gauge_kpis(self):
        ...
        if gauge_match:
            try:
                ...
                self.test(
                    "Healing cap gauge exists",
                    "healing_cap" in gauge_data,
                    "Missing healing_cap gauge",
                )
                self.test(
                    "Compliance gauge exists",
                    "compliance" in gauge_data,
                    "Missing compliance gauge",
                )
                self.test("Health gauge exists", "health" in gauge_data, "Missing health gauge")

                # Check values are reasonable
                self.test(
                    "Healing cap value valid",
                    0 <= gauge_data.get("healing_cap", -1) <= 100,
                    f"Invalid: {gauge_data.get('healing_cap')}",
                )
                self.test(
                    "Compliance value valid",
                    0 <= gauge_data.get("compliance", -1) <= 100,
    ...
    def test_autonomy_targets_integration(self):
        ...
        self.test(
            "All territory rows have target fields",
        ...
        self.test(
            "Target values are valid types (numeric or N/A)",
            valid_target_values,
            f"Invalid: {invalid_examples[:3]}",
        )
        ...
        self.test(
            "Target Invocation values vary by territory",
            len(target_values) > 1,
    ...
    def test_context_target_resolver(self):
        ...
        if guardian_path.exists():
            ...
            self.test(
                "Context stored in generate_compliance_report",
            ...
            self.test(
                "Target resolver accessed from context",
    ...
    def test_total_vs_territory_rows(self):
        ...
        self.test(
            "Exactly one TOTAL row exists",
            len(total_rows) == 1,
            f"Found {len(total_rows)} TOTAL rows",
        )

        self.test(
            "Multiple territory rows exist",
            len(non_total_rows) >= 10,
        ...
        if total_rows:
            ...
            self.test(
                "TOTAL row target handling documented",
                True,
        ...
        if non_total_rows:
            ...
            self.test(
                "All territory rows have consistent structure",
        ...
        self.test("Territory names are unique", unique_names, "Duplicate territories found")
    ...
    def test_template_features(self):
        ...
        self.test(
            "formatPctCell displays 'N/A' text",
        ...
        self.test(
            "Target values used in table cells (tgtInv)",
    ...
    def test_na_handling(self):
        ...
        self.test(
            "Template can handle N/A targets",
        ...
        self.test(
            "Neutral background for N/A metrics",
        ...
        self.test(
            "Exception tooltip for N/A targets",
        ...
        self.test(
            "Color logic respects N/A targets",
    ...
    def test_dashboard_refresh(self):
        ...
        self.test(
            "Dashboard data not empty",
            len(self.dashboard_data) > 0,
    ...
    def test_invocation_update_detection(self):
        ...
        self.test(
            "All territories have Invocation %",
        ...
        if l0_rows:
            ...
            self.test(
                "L0 Maintenance has expected low invocation",
            ...
            for row in l0_rows:
                ...
                if "Infrastructure" in terr or "Infrast" in terr:
                    ...
                    if target != 20:
                        l0_targets_correct = False
            self.test(
                "L0 Maintenance targets correct (20 or 70 for Infra)",
                l0_targets_correct,
