...
def _validate_dict_schema(obj: Any, schema: dict) -> tuple[bool, str | None, dict]:
    ...
    # Only attempt JSON parse when schema expects a structured type (object/array)
    # or when the string looks like JSON. For primitive schema types, validate directly.
...
def _check_node(value: Any, schema: dict, path: str) -> list[str]:
    ...
    if schema_type is None:
        # No type constraint — pass through (e.g. anyOf not supported → skip)
        return errors
