...
_PRIMITIVE_TYPES = {'string': str, 'integer': int, 'number': (int, float), 'boolean': bool, 'null': type(None)}
...
def validate_context_contract(payload: dict) -> tuple[bool, str | None, dict]:
    ...
    if 'telemetry_envelope' in payload:
        ...
        if not isinstance(te.get('empty_result_signal'), bool):
            return (False, INVALID_TELEMETRY_ENVELOPE, {})
        normalized['telemetry_envelope'] = {'hit_rate': te['hit_rate'], 'recall_estimate': te['recall_estimate'], 'empty_result_signal': te['empty_result_signal']}
