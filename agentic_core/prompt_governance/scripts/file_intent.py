...
class HardenedNamingAuditor:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    class_info = {'name': node.name, 'line': node.lineno, 'is_agent': self._is_agent_class(node, content), 'has_methods': len([n for n in node.body if isinstance(n, ast.FunctionDef)]) > 0, 'inherits_from_agent': self._inherits_from_agent(node), 'docstring': ast.get_docstring(node) or ''}
                    classes.append(class_info)
                elif isinstance(node, ast.FunctionDef):
                    func_info = {'name': node.name, 'line': node.lineno, 'is_private': node.name.startswith('_'), 'is_dunder': node.name.startswith('__') and node.name.endswith('__')}
            ...
            return {'classes': classes, 'functions': functions, 'imports': imports, 'constants': constants, 'line_count': len(content.splitlines()), 'has_main': '__main__' in content, 'content_preview': content[:200] + '...' if len(content) > 200 else content}
    ...
    def validate_naming_compliance(self, file_path: Path) -> ViolationReport | None:
        ...
        if intent == FileIntent.CLASS_EXPORT and naming != NamingConvention.PASCAL_CASE:
            ...
            violation = ViolationReport(file_path=str(file_path), current_name=filename, detected_intent=intent, current_naming=naming, proposed_name=f'{primary_class}.py', rationale=f"Primary class export '{primary_class}' found in snake_case file. Violates: 'PascalCase files should contain primary class/agent exports'", confidence=confidence, requires_manual_review=confidence < 0.8, ast_analysis=analysis)
        elif intent in [FileIntent.UTILITY_MODULE, FileIntent.SCRIPT_MODULE] and naming != NamingConvention.SNAKE_CASE:
            violation = ViolationReport(file_path=str(file_path), current_name=filename, detected_intent=intent, current_naming=naming, proposed_name=self._to_snake_case(filename), rationale=f"Utility module with {len(analysis['functions'])} functions found in PascalCase file. Violates: 'snake_case files should contain utilities/scripts'", confidence=confidence, requires_manual_review=confidence < 0.8, ast_analysis=analysis)
        elif intent == FileIntent.MIXED_CONTENT:
            violation = ViolationReport(file_path=str(file_path), current_name=filename, detected_intent=intent, current_naming=naming, proposed_name='MANUAL_REVIEW_REQUIRED', rationale=f"Mixed content: {len(analysis['classes'])} classes and {len(analysis['functions'])} functions. Requires architectural decision", confidence=0.5, requires_manual_review=True, ast_analysis=analysis)
        return violation
    ...
    def generate_disposition_table(self) -> str:
        ...
        for violation in self.violations:
            ...
            table.append(f'| {violation.current_name} | {violation.detected_intent.value} | {violation.proposed_name} | {violation.rationale} | {confidence_icon}{violation.confidence:.2f} {manual_icon} |')
        return '\n'.join(table)
