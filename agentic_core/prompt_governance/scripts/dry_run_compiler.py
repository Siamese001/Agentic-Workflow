...
def compile_template(env: Environment, template_path: Path, relative_to: Path) -> dict:
    ...
    try:
        ...
        return {'status': 'PASS', 'template_path': relative_path, 'full_path': str(template_path), 'error': None}
    except TemplateSyntaxError as e:
        return {'status': 'FAIL', 'template_path': relative_path, 'full_path': str(template_path), 'error': f'Syntax error at line {e.lineno}: {e.message}', 'line': e.lineno, 'error_type': 'SYNTAX_ERROR'}
    except TemplateError as e:
        return {'status': 'FAIL', 'template_path': relative_path, 'full_path': str(template_path), 'error': f'Template error: {str(e)}', 'error_type': 'TEMPLATE_ERROR'}
    # guardian: allow-silent-swallow
    except Exception as e:
        return {'status': 'FAIL', 'template_path': relative_path, 'full_path': str(template_path), 'error': f'Unexpected error: {str(e)}', 'error_type': 'UNEXPECTED_ERROR'}
