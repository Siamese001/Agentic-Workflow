    ...
    for template_name, prompt_versions in prompts.items():
        for prompt_data in prompt_versions:
            ...
            if not template_path.exists():
                failed.append({'prompt_id': template_name, 'template_path': str(template_path.relative_to(base_dir)), 'error': 'Template file not found', 'status': 'FAIL'})
                continue
            schema = extract_schema_from_template(template_path)
            if not schema['has_header']:
                failed.append({'prompt_id': template_name, 'template_path': str(template_path.relative_to(base_dir)), 'error': 'Missing Phase 4 schema header', 'status': 'FAIL'})
                continue
            passed.append({'prompt_id': template_name, 'template_path': str(template_path.relative_to(base_dir)), 'required_vars': schema['required_vars'], 'optional_vars': schema['optional_vars'], 'status': 'PASS'})
