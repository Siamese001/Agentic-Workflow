        ...
        for line in content.split('\n')[:20]:
            if '{# SCHEMA:' in line:
                ...
                if 'required_vars=[' in schema_match:
                    ...
                    required_vars = [v.strip() for v in req_part.split(',') if v.strip() and v.strip() != '[]']
...
class TemplateRenderVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Attribute) and node.func.attr == 'render' and isinstance(node.func.value, ast.Name):
            template_name = None
            context_dict = None
            if node.args:
                arg = node.args[0]
    ...
    def _validate_render_call(self, node, template_name: str, context_dict: ast.Dict):
        ...
        if missing_vars:
            self.violations.append({'file': self.current_file, 'line': node.lineno, 'class': self.current_class, 'function': self.current_function, 'template': template_name, 'required_vars': schema['required_vars'], 'provided_vars': sorted(context_keys), 'missing_vars': sorted(missing_vars)})
