...
FORBIDDEN_IMPORTS = {'agentic_core.L1_cognition', 'agentic_core.L2_resources', 'agentic_core.L3_orchestration', 'agentic_core.L4_coordination', 'agentic_core.L5_safety', 'agentic_core.L6_observability'}
    ...
    def _check_import(self, import_path: str, line: int):
        ...
        for forbidden in FORBIDDEN_IMPORTS:
            if import_path.startswith(forbidden):
                self.violations.append({'file': str(self.file_path), 'line': line, 'import_statement': import_path, 'violated_layer': forbidden, 'violation_type': 'UPWARD_IMPORT'})
...
def analyze_file(file_path: Path) -> list[dict]:
    ...
    try:
        ...
        return [{'file': str(file_path), 'line': e.lineno or 0, 'import_statement': 'SYNTAX_ERROR', 'violated_layer': 'N/A', 'violation_type': 'SYNTAX_ERROR', 'error': str(e)}]
    # guardian: allow-silent-swallow
    except Exception as e:
        return [{'file': str(file_path), 'line': 0, 'import_statement': 'PARSE_ERROR', 'violated_layer': 'N/A', 'violation_type': 'PARSE_ERROR', 'error': str(e)}]
