...
def canonical_json_bytes(obj: Any) -> bytes:
    ...
    try:
        ...
        raise TypeError(f'Object contains non-JSON-serializable type: {exc}. Only dict, list, str, int, float, bool, None are allowed.') from exc
...
class DeterministicRedisCache:
    ...
    def __init__(self, db: CacheDB=CacheDB.HOT, redis_url: str | None=None, fallback_maxsize: int=_FALLBACK_MAX_ENTRIES) -> None:
        self._db = db
    ...
    def _connect(self) -> Any:
        ...
        try:
            ...
            if not self._tcp_reachable(host, port):
                raise OSError(f'TCP pre-check failed: {host}:{port} unreachable within {_REDIS_SOCKET_TIMEOUT_S}s')
            params: dict[str, Any] = {'host': host, 'port': port, 'db': int(self._db), 'decode_responses': False, 'socket_timeout': _REDIS_SOCKET_TIMEOUT_S, 'socket_connect_timeout': _REDIS_SOCKET_TIMEOUT_S}
            if parsed.password:
        ...
        # guardian: allow-silent-swallow
            ...
            logger.warning('[cache] Redis unavailable (db=%s): %s — switching to in-process fallback', int(self._db), exc)
    ...
    def acquire_lease(self, key: str, holder_id: str, nonce: str, semantic_clock_tick: int, ttl_seconds: int=30) -> bool:
        ...
        payload = canonical_json_bytes({'holder_id': holder_id, 'nonce': nonce, 'semantic_clock_tick': semantic_clock_tick})
        conn = self._connect()
        if conn is not None:
            try:
    ...
    def get_stats(self) -> dict[str, Any]:
        return {'db': int(self._db), 'using_fallback': self._use_fallback, 'fallback_entries': len(self._fallback), 'hits': self.stats.hits, 'misses': self.stats.misses, 'fallback_hits': self.stats.fallback_hits, 'fallback_misses': self.stats.fallback_misses, 'errors': self.stats.errors, 'bypassed_replay': self.stats.bypassed_replay}
...
def check_redis_health(redis_url: str | None=None) -> dict[str, object]:
    ...
    result: dict[str, object] = {'healthy': False, 'url': url, 'using_fallback': True, 'error': None, 'fix': None}
    try:
        import redis as _redis
        parsed = urllib.parse.urlparse(url)
        conn = _redis.Redis(host=parsed.hostname or 'localhost', port=int(parsed.port or 6379), db=0, socket_timeout=_REDIS_SOCKET_TIMEOUT_S, socket_connect_timeout=_REDIS_SOCKET_TIMEOUT_S)
        conn.ping()
