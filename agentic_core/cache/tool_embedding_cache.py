...
class ToolEmbeddingCache:
    ...
    def __init__(self, cache: DeterministicRedisCache | None=None, ttl_seconds: int=_DEFAULT_EMBEDDING_TTL):
        self._cache = cache or get_hot_cache()
        self._ttl = ttl_seconds

    def get_or_fetch(self, tool_definitions: list[dict[str, Any]], fetch_embeddings: Any, *, replay_mode: bool=False) -> tuple[list[list[float]], list[str]]:
        ...
        *fetch_embeddings* is a zero-argument callable that computes and returns
        ...
        if not replay_mode:
            try:
                fingerprint = self._compute_tool_fingerprint(tool_definitions)
                cache_key = f'tool_embeddings:{fingerprint}'
                self._cache.set_json(cache_key, {'embeddings': embeddings, 'tool_names': tool_names}, ttl_seconds=self._ttl)
            except ValueError:
                pass
            # guardian: allow-silent-swallow
            except Exception as e:
    ...
    def _compute_tool_fingerprint(self, tool_definitions: list[dict[str, Any]]) -> str:
        ...
        fingerprint_data = json.dumps([{'name': t.get('name', ''), 'description': t.get('description', ''), 'tags': sorted(t.get('tags', []))} for t in sorted_tools], sort_keys=True)
