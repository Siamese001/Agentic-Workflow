...
# THE TARGET DESTINATIONS (Where they SHOULD be)
ANNEXATION_PLAN = {
...
def force_annexation():
    ...
    for old_name, destination in ANNEXATION_PLAN.items():
        ...
        # Pull everything out of the old folder
        for item in list(old_path.iterdir()):
            # Don't move the new core into itself if there's a name collision
            if item.name == "agentic_core": continue 
            ...
            if target_item.exists():
                ...
                target_item = destination / f"{item.stem}_{timestamp}{item.suffix}"
        ...
        # Final Cleanup: Kill the old root folder shell
        try:
    ...
    for key in ANNEXATION_PLAN.keys():
        exists_in_root = (ROOT / key).exists()
