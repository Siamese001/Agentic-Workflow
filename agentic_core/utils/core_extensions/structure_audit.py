...
hierarchy: Any = {
    "LEVEL_0_SOVEREIGN_CORE": {
        "folders": [AGENTIC_CORE_DIR],
        "description": "The Sovereign Core - L1-L5 layers",
        "can_import_from": [],
    },
    "LEVEL_1_ARCHITECTURAL": {
        "folders": ["prompt_governance", "schemas", "config", SCRIPTS_DIR],
    ...
    "LEVEL_4_DOWNSTREAM": {
        "folders": [APPS_RG_DIR, APPS_LIC_DIR],
            ...
            if imported_module:
                ...
                if imported_level != -1:
                    if imported_module not in allowed_imports and imported_module != own_folder:
                        violations.append(
                            f"GRAVITY VIOLATION: {rel_path} imports from '{imported_module}' (Level {imported_level}), but '{own_folder}' (Level {own_level}) can only import from: {allowed_imports}"
                        )
    except Exception:
        pass
    return violations
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        if results["total_issues"] == 0:
            ...
            if results["unauthorized_folders"]:
                f.write(
                    f"### Unauthorized Root Folders ({len(results['unauthorized_folders'])})\n\n"
                )
