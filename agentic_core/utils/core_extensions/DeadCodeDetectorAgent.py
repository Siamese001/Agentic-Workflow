...
class ast_dead_code_visitor(ast.NodeVisitor):
    """
    ...
    def visit_Import(self, node: ast.Import) -> Any:
        ...
        for alias in node.names:
            name: Any = alias.asname if alias.asname else alias.name
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if isinstance(node.func, ast.Name):
            ...
            if isinstance(node.func.value, ast.Name):
                class_hint: Any = node.func.value.id
...
class dead_code_detector_agent:
    ...
    def __init__(self, project_root: Path):
        ...
        self.results = {'unused_imports': [], 'unused_functions': [], 'unused_classes': [], 'unused_methods': [], 'dead_files': []}
    ...
    def analyze_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            add_parents(tree)
            visitor: Any = ASTDeadCodeVisitor(file_path)
            visitor.visit(tree)
        except SyntaxError as e:
            return {'error': f'Syntax error in {file_path}: {e}'}
        findings: Any = {'file_path': str(file_path.relative_to(self.project_root)), 'unused_imports': [], 'unused_functions': [], 'unused_classes': [], 'unused_methods': []}
        for import_name in visitor.imported_names:
            if import_name not in visitor.used_names:
                line_no: Any = visitor.import_line_numbers.get(import_name, 'unknown')
                findings['unused_imports'].append({'name': import_name, 'line': line_no})
        for func_name in visitor.defined_functions:
            if func_name not in visitor.used_functions and func_name not in visitor.used_names and (not func_name.startswith('_')):
                line_no: Any = visitor.definition_line_numbers.get(func_name, 'unknown')
                findings['unused_functions'].append({'name': func_name, 'line': line_no})
        for class_name in visitor.defined_classes:
            if class_name not in visitor.used_classes and class_name not in visitor.used_names and (not class_name.startswith('_')):
                line_no: Any = visitor.definition_line_numbers.get(class_name, 'unknown')
                findings['unused_classes'].append({'name': class_name, 'line': line_no})
        for class_name, methods in visitor.class_methods.items():
            used_methods: Any = visitor.used_methods.get(class_name, set())
            for method_name in methods:
                ...
                if method_name not in used_methods and method_name not in visitor.used_names:
                    full_name: Any = f'{class_name}.{method_name}'
                    line_no: Any = visitor.definition_line_numbers.get(full_name, 'unknown')
                    findings['unused_methods'].append({'class': class_name, 'name': method_name, 'line': line_no})
        return findings
    ...
    def scan_directory(self, directory: Path, recursive: bool=True) -> Dict:
        ...
        if recursive:
            py_files: Any = list(directory.rglob('*.py'))
        else:
            py_files: Any = list(directory.glob('*.py'))
        py_files: Any = [f for f in py_files if '__pycache__' not in str(f) and '.pytest_cache' not in str(f)]
        results: Any = {'scanned_files': len(py_files), 'findings': [], 'summary': {'total_unused_imports': 0, 'total_unused_functions': 0, 'total_unused_classes': 0, 'total_unused_methods': 0}}
        for file_path in py_files:
            finding: Any = self.analyze_file(file_path)
            if 'error' in finding:
    ...
    def generate_report(self, results: Dict) -> str:
        ...
        report.append('')
        summary: Any = results.get('summary', {})
        ...
        for finding in results.get('findings', []):
            if 'error' in finding:
                ...
                continue
            file_path: Any = finding.get('file_path', 'unknown')
...
def main() -> Any:
    ...
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    target_dir: Any = Path(sys.argv[1])
    if not target_dir.is_absolute():
        target_dir: Any = Path.cwd() / target_dir
    detector: Any = DeadCodeDetectorAgent(target_dir)
    results: Any = detector.scan_directory(target_dir)
