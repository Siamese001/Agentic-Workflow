...
class ShadowModeEngine:
    ...
    def refine_pitch(self, pitch: PitchResult, error_reason: str) -> ShadowModeResult:
        ...
        result: Any = ShadowModeResult(
            pitch=refined_pitch, improvements=improvements, confidence=confidence
        )
        Logger.info(
            f"P10_SHADOW_COMPLETE: {len(improvements)} improvements, confidence={confidence}"
        )
        return result
    ...
    def _load_refinement_rules(self) -> dict[str, Any]:
        ...
        return {
            "brand_compliance": {
                "keywords_to_remove": ["amazing", "incredible", "revolutionary", "game-changing"],
                "tone_adjustments": {
                    "salesy": "professional",
                    "hype": "informative",
                    "urgent": "considerate",
            ...
            "spam_detection": {
                "remove_patterns": ["!!", "FREE", "ACT NOW", "LIMITED TIME"],
                "replace_words": {
                    "guarantee": "confident",
                    "promise": "commitment",
                    "win": "succeed",
            ...
            "length_issues": {
                "max_words": 200,
                "min_words": 100,
                "truncate_sections": ["intro", "closing"],
            },
            "personalization": {
                "required_elements": ["recipient_name", "company_reference", "mutual_connection"],
                "boost_signals": ["recent_news", "shared_interest", "referral"],
            },
        }

    def _analyze_improvements(
        self, original: PitchResult, refined: PitchResult, error_reason: str
    ) -> list:
        ...
        if abs(original_word_count - refined_word_count) > 10:
            improvements.append(
                f"Adjusted length from {original_word_count} to {refined_word_count} words"
            )
    ...
    def apply_refinement(
        self, original: PitchResult, shadow_result: ShadowModeResult
    ) -> PitchResult:
        """
        ...
        if shadow_result.confidence >= 0.7:
            ...
            Logger.info(
                f"P10_SHADOW_APPLY: Applied refinement with confidence {shadow_result.confidence}"
            )
            return shadow_result.pitch
        else:
            Logger.warning(
                f"P10_SHADOW_REJECT: Low confidence {shadow_result.confidence}, keeping original"
            )
            return original
