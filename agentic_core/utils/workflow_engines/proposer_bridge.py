...
class ImprovementSignal:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'signal_type': self.signal_type, 'metric_name': self.metric_name, 'current_value': self.current_value, 'target_value': self.target_value, 'delta': self.delta, 'priority': self.priority, 'source': self.source, 'message': self.message}
...
class ImprovementProposal:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'proposal_id': self.proposal_id, 'timestamp': self.timestamp, 'signals': [s.to_dict() for s in self.signals], 'dpo_pair_count': self.dpo_pair_count, 'recommended_actions': self.recommended_actions, 'overall_health_score': self.overall_health_score, 'requires_intervention': self.requires_intervention, 'metadata': self.metadata}

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> ImprovementProposal:
        return cls(proposal_id=data['proposal_id'], timestamp=data['timestamp'], signals=[ImprovementSignal(**s) for s in data.get('signals', [])], dpo_pair_count=data['dpo_pair_count'], recommended_actions=data['recommended_actions'], overall_health_score=data['overall_health_score'], requires_intervention=data['requires_intervention'], metadata=data.get('metadata', {}))
...
class EvaluatorProposerBridge:
    ...
    def propose(self, eval_report: EvaluationReport | None=None, retrieval_snapshot: RetrievalDriftSnapshot | None=None, answer_snapshot: AnswerQualitySnapshot | None=None, dpo_batch: DPOBatch | None=None) -> ImprovementProposal:
        ...
        proposal = ImprovementProposal(proposal_id=str(uuid.uuid4()), timestamp=_utcnow(), signals=signals, dpo_pair_count=dpo_count, recommended_actions=actions, overall_health_score=health, requires_intervention=requires_intervention)
    ...
    def _signals_from_eval(self, report: EvaluationReport) -> list[ImprovementSignal]:
        ...
        for metric, target in {**self.RETRIEVAL_TARGETS, **self.QUALITY_TARGETS}.items():
            ...
            signals.append(ImprovementSignal(signal_type='eval_metric', metric_name=metric, current_value=current, target_value=target, delta=delta, priority=priority, source=f'eval_report:{report.run_id[:8]}', message=f'{metric} = {current:.3f} (target {target:.3f}, delta {delta:+.3f})'))
        return signals
    ...
    def _signals_from_retrieval(self, snapshot: RetrievalDriftSnapshot) -> list[ImprovementSignal]:
        ...
        signals.append(ImprovementSignal(signal_type='retrieval_drift', metric_name='retrieval_hit_rate', current_value=snapshot.retrieval_hit_rate, target_value=hit_target, delta=delta, priority=priority, source=f'retrieval_snapshot:{snapshot.timestamp[:10]}', message=f'Hit rate {snapshot.retrieval_hit_rate:.3f} vs target {hit_target:.3f}'))
        return signals
    ...
    def _signals_from_answer(self, snapshot: AnswerQualitySnapshot) -> list[ImprovementSignal]:
        ...
        signals.append(ImprovementSignal(signal_type='answer_quality_drift', metric_name='groundedness_rate', current_value=snapshot.groundedness_rate, target_value=groundedness_target, delta=g_delta, priority=g_priority, source=f'answer_snapshot:{snapshot.timestamp[:10]}', message=f'Groundedness rate {snapshot.groundedness_rate:.3f}'))
        hall_target = 0.1
        h_delta = hall_target - snapshot.hallucination_rate
        h_priority = 'ok' if h_delta >= 0 else 'critical' if h_delta < -0.15 else 'warning'
        signals.append(ImprovementSignal(signal_type='answer_quality_drift', metric_name='hallucination_rate', current_value=snapshot.hallucination_rate, target_value=hall_target, delta=h_delta, priority=h_priority, source=f'answer_snapshot:{snapshot.timestamp[:10]}', message=f'Hallucination rate {snapshot.hallucination_rate:.3f} (target < {hall_target:.3f})'))
        return signals
    ...
    def _persist(self, proposal: ImprovementProposal) -> None:
        try:
            ...
            artifact = create_artifact(kind='improvement_proposal', logical_id=f'proposal_{proposal.proposal_id[:8]}', payload=proposal.to_dict())
