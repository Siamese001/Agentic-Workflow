...
class Chunk:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'chunk_id': self.chunk_id, 'doc_id': self.doc_id, 'content': self.content, 'token_count': self.token_count, 'start_char': self.start_char, 'end_char': self.end_char, 'parent_section': self.parent_section, 'metadata': self.metadata}

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> Chunk:
        return cls(chunk_id=data['chunk_id'], doc_id=data['doc_id'], content=data['content'], token_count=data['token_count'], start_char=data['start_char'], end_char=data['end_char'], parent_section=data.get('parent_section', ''), metadata=data.get('metadata', {}))
...
class ChunkManifest:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'doc_id': self.doc_id, 'policy_name': self.policy_name, 'chunks': [c.to_dict() for c in self.chunks], 'metadata': self.metadata}
...
class FixedTokenChunkPolicy(ChunkPolicy):
    ...
    def chunk(self, document: str, doc_id: str='doc') -> list[Chunk]:
        ...
        for idx, start in enumerate(range(0, len(words), self.chunk_size)):
            ...
            chunks.append(Chunk(chunk_id=_make_chunk_id(doc_id, idx), doc_id=doc_id, content=content, token_count=len(word_slice), start_char=len(' '.join(words[:start])), end_char=len(' '.join(words[:start + len(word_slice)])), metadata={'policy': self.name, 'chunk_size': self.chunk_size}))
        return chunks
...
class OverlapWindowChunkPolicy(ChunkPolicy):
    ...
    def chunk(self, document: str, doc_id: str='doc') -> list[Chunk]:
        ...
        while start < len(words):
            word_slice = words[start:start + self.chunk_size]
            content = ' '.join(word_slice)
            chunks.append(Chunk(chunk_id=_make_chunk_id(doc_id, idx), doc_id=doc_id, content=content, token_count=len(word_slice), start_char=start, end_char=start + len(word_slice), metadata={'policy': self.name, 'chunk_size': self.chunk_size, 'overlap': self.overlap}))
            start += step
            idx += 1
        return chunks
...
class SectionAwareChunkPolicy(ChunkPolicy):
    ...
    def chunk(self, document: str, doc_id: str='doc') -> list[Chunk]:
        ...
        for idx, section in enumerate(sections):
            ...
            chunks.append(Chunk(chunk_id=_make_chunk_id(doc_id, idx), doc_id=doc_id, content=section, token_count=token_count, start_char=char_offset, end_char=char_offset + len(section), parent_section=section[:80].replace('\n', ' '), metadata={'policy': self.name, 'max_section_tokens': self.max_section_tokens}))
...
class SemanticChunkPolicy(ChunkPolicy):
    ...
    def chunk(self, document: str, doc_id: str='doc') -> list[Chunk]:
        ...
        for idx, group in enumerate(groups):
            ...
            chunks.append(Chunk(chunk_id=_make_chunk_id(doc_id, idx), doc_id=doc_id, content=content, token_count=token_count, start_char=char_offset, end_char=char_offset + len(content), metadata={'policy': self.name, 'target_size': self.target_size}))
            char_offset += len(content) + 1
        return chunks
__all__ = ['Chunk', 'ChunkManifest', 'ChunkPolicy', 'FixedTokenChunkPolicy', 'OverlapWindowChunkPolicy', 'SectionAwareChunkPolicy', 'SemanticChunkPolicy', '_approx_token_count']
