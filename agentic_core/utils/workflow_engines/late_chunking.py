...
VALID_POOLING_STRATEGIES: frozenset[str] = frozenset(
    {
        "mean",
        "max",
        "cls",
...
class LateChunkingProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if self.pooling_strategy not in VALID_POOLING_STRATEGIES:
            raise ValueError(
                f"Invalid pooling_strategy: {self.pooling_strategy!r}. "
                f"Must be one of {sorted(VALID_POOLING_STRATEGIES)}"
            )
...
# ---------------------------------------------------------------------------
# Late Chunking Pipeline (deterministic, no randomness, no wall-clock)
# ---------------------------------------------------------------------------
