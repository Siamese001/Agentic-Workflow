...
def status_bar(
    ...
    Args:
        ...
        status: One of 'running', 'success', 'error', 'warning', 'pending'
...
def agent_status(
    ...
    Args:
        ...
        fixes: Number of fixes applied
        violations: Number of violations found
        duration_ms: Execution duration in milliseconds
        
    Returns:
        Formatted agent status line
    """
    status_styles = {
...
def tier_summary(
    ...
    Args:
        ...
        total_violations: Total violations remaining
        duration_sec: Execution duration in seconds
        passed: Whether tier passed
        
    Returns:
        Formatted tier summary
    """
    if passed:
...
def discovery_status(
    ...
    """
    Generate discovery progress status.
    
    Args:
        files_scanned: Number of files scanned
        agents_found: Number of agents discovered
        current_file: Current file being scanned
        status: Discovery status
        
    Returns:
        Formatted discovery status
    """
    status_colors = {
        'scanning': Colors.BRIGHT_CYAN,
    ...
    return (
        f"{color}{SYMBOLS['running'] if status == 'scanning' else SYMBOLS['success']} "
        f"Discovery: {files_scanned} files | {agents_found} agents{Colors.RESET}"
        f"{f' | {Colors.DIM}{current_file}{Colors.RESET}' if current_file else ''}"
    )
...
def mission_header(mode: str, execute: bool = False) -> str:
    ...
    Args:
        mode: Mission mode (heal, validate, etc.)
        execute: Whether in execute mode
        
    Returns:
        Formatted mission header
    """
...
def heartbeat(iteration: int) -> str:
    ...
    Args:
        iteration: Current iteration (for spinner animation)
