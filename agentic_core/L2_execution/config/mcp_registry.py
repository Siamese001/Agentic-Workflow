...
def get_mcp_registry() -> dict[str, McpServerConfig]:
    ...
    if config.REDIS_MCP_ENABLED:
        registry['redis'] = McpServerConfig(name='redis', target_layer='L4', mode=McpServerMode.LOCAL, capabilities=['caching', 'state_management', 'session_storage'], command='npx', args=['-y', '@modelcontextprotocol/server-redis'], env={'REDIS_URL': config.redis_url}, description='Redis caching and state management via MCP', sovereignty_impact='HIGH - Provides sovereign caching with MCP integration')
    return registry
