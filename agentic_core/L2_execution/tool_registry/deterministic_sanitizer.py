...
class deterministic_cleaner:
    ...
    def _apply_isort(self, code: str, file_path: Optional[str]=None) -> str:
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
                ...
                temp_file = f.name
            try:
                result = subprocess.run(['isort', '--profile', 'black', temp_file], capture_output=True, text=True, check=True)
    ...
    def _apply_autopep8(self, code: str, file_path: Optional[str]=None) -> str:
        ...
        try:
            result = subprocess.run(['autopep8', '--', '-'], input=code, capture_output=True, text=True, check=True)
