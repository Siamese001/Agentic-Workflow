...
def execute_with_timeout(command: List[str], timeout: int=30, cwd: Optional[str]=None, capture_output: bool=True, check: bool=False) -> subprocess.CompletedProcess:
    ...
    try:
        result: Any = subprocess.run(command, cwd=str(work_dir), capture_output=capture_output, text=True, timeout=timeout, check=check)
        return result
...
def check_tool_installed(tool_name: str) -> bool:
    ...
    for command in ALLOWED_COMMANDS[tool_name]:
        try:
            result: Any = subprocess.run([command, '--version'], capture_output=True, timeout=5, check=False)
