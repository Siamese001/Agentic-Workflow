...
class SovereignActionPlaneAgent(IActionPlane):
    ...
    async def _log_to_signal_ledger(self, request: ActionRequest, result: ActionResult) -> None:
        ...
        if self._signal_ledger:
            execution_result = {'request': {'action_type': request.action_type, 'parameters': request.parameters, 'timestamp': time.time()}, 'result': {'success': result.success, 'output': result.output, 'error': result.error, 'execution_time': result.execution_time, 'metadata': getattr(result, 'metadata', {})}}
...
def create_sovereign_action_plane(safety_layer: Any=None, signal_ledger: Any=None) -> IActionPlane:
