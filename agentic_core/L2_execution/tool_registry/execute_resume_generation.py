...
class execute_resume_generation:
    ...
    def __init__(self, config: Optional[Dict[str, object]]=None, workflow_loader: Optional[LocalWorkflowLoader]=None):
        ...
        self.job_analyzer = JobAnalyzer(llm_client=self.config.get('llm_client'), PROVIDER=self.config.get('provider'), workflow_config=self.workflow.get_knode_config('K.0'))
        self.resume_generator = ResumeGenerator(llm_client=self.config.get('llm_client'), PROVIDER=self.config.get('provider'), creative_brief=creative_brief, validation_rules=self.workflow.get_validation_rules())
