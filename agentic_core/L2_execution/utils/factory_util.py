...
def parse_mcp_client_specs(raw_specs: list[dict[str, Any]]) -> list[MCPClientSpec]:
    ...
    for raw in raw_specs:
        ...
        spec = MCPClientSpec(name=name, provider=provider, module=module, class_name=class_name, parameters=parameters, optional=bool(raw.get('optional', False)))
        spec.validate()
        specs.append(spec)
    return specs
...
def instantiate_mcp_client(spec: MCPClientSpec) -> object:
    ...
    if not module_name:
        raise MCPClientInitializationError(f"Cannot create MCP client '{spec.name}': no module specified and no provider mapping found.", client_name=spec.name, provider=spec.provider)
    if not class_name:
        raise MCPClientInitializationError(f"Cannot create MCP client '{spec.name}': no class_name specified and no provider mapping found.", client_name=spec.name, provider=spec.provider)
    try:
        module = importlib.import_module(module_name)
    except Exception as exc:
        if spec.optional:
            logger.warning(f"Optional MCP client '{spec.name}' module '{module_name}' not available, using stub: {exc}")
            return MCPClientStub(spec.name, {'error': f'Module not available: {exc}'})
        raise MCPClientInitializationError(f"Failed to import MCP module '{module_name}' for client '{spec.name}': {exc}", client_name=spec.name, provider=spec.provider) from exc
    try:
        client_cls = getattr(module, class_name)
    except AttributeError as exc:
        if spec.optional:
            logger.warning(f"Optional MCP client '{spec.name}' class '{class_name}' not found in '{module_name}', using stub")
            return MCPClientStub(spec.name, {'error': f'Class not found: {class_name}'})
        raise MCPClientInitializationError(f"Module '{module_name}' missing class '{class_name}' for MCP client '{spec.name}'.", client_name=spec.name, provider=spec.provider) from exc
    try:
        ...
        raise MCPClientInitializationError(f"Failed to instantiate MCP client '{spec.name}': {exc}", client_name=spec.name, provider=spec.provider) from exc
