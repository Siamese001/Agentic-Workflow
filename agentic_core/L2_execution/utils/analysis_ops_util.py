...
def validate_python_syntax(file_path: str) -> tuple[bool, str | None]:
    ...
    _trace_id = str(_uuid.uuid4())    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime    # guardian: Syntax errors should be caught at parser level, not runtime
...
def run_black_format(file_path: str, check_only: bool = False) -> tuple[int, str, str]:
    ...
    if check_only:    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access
        cmd.append("--check")
    try:
        result: Any = safe_execute(cmd, capture_output=True, text=True, timeout=DEFAULT_TIMEOUT, check=False)
        return (result.returncode, result.stdout, result.stderr)
