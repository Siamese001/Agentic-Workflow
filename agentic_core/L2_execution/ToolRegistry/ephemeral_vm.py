...
class EphemeralVm:
    ...
    def __init__(
        ...
        if self.enable_logging:
            LOGGER.info(
                "ephemeral_vm_initialized", extra={"isolation": self.IsolationConfig.to_dict()}
            )
    ...
    def _handle_execution_error(
        self, vm_id: str, error: Exception, start_time: float
    ) -> ExecutionResult:
        """Handle execution error."""
        if self.enable_logging:
            LOGGER.error(
                "execution_failed", extra={"vm_id": vm_id, "error": str(error)}, exc_info=True
            )
        return ExecutionResult(
            success=False,
            output="",
