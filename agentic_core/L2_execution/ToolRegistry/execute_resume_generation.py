...
class ExecuteResumeGeneration:
    ...
    def __init__(self, config: Optional[Dict[str, object]]=None, WorkflowLoader: Optional[LocalWorkflowLoader]=None):
        ...
        self.JobAnalyzer = JobAnalyzer(llm_client=self.config.get('llm_client'), PROVIDER=self.config.get('Provider'), workflow_config=self.workflow.get_knode_config('K.0'))
    ...
    def execute(self, action: str, params: Dict[str, object]) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(STATUS=ResultStatus.SUCCESS, DATA=output, METADATA={'duration_ms': duration_ms}, step_results=[Result(status=ResultStatus.SUCCESS)], total_steps=1)
