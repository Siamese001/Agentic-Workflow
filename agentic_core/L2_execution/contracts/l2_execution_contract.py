...
class L2ExecutionAgent(ABC):
    ...
    def __init__(self, agent_id: str | None = None):
        ...
        self._heal_tier_router = None  # Lazy-loaded
    ...
    def l2_evaluate_and_heal(self, context: L2ExecutionContext) -> L2PhaseResult:
        """
        Default healing implementation using tier-based router.

        Subclasses can override for agent-specific healing logic,
        but must respect the heal_enabled flag and retry budget.
        """
        # Default implementation delegates to healing infrastructure
        ...
        healing_input = last_result.failure_signal.to_healing_input()

        # Route to appropriate healing tier
        from agentic_core.L2_execution.healers.healing_tier_router import route_by_confidence

        heal_decision = route_by_confidence(
            confidence=getattr(healing_input, "blast_radius_estimate", 0.5),
        ...
        if heal_decision.tier == HealingTier.LOCAL_AGENT:
            ...
            return L2PhaseResult(
                ...
                metadata={
                    "heal_escalated": True,
                    "target_tier": heal_decision.tier.value,
                    "reason_codes": heal_decision.reason_codes,
                },
            )
