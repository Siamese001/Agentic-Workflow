...
class RollbackRefinementRequest:
    ...
    def canonical_bytes(self) -> bytes:
        """Canonical byte representation for hashing."""
        data = {'failure_signature': self.failure_signature.canonical_bytes().decode('ascii'), 'candidates': tuple(sorted((c.name for c in self.candidates))), 'history_bytes': self.history_bytes.decode('ascii') if self.history_bytes else None}
        return json.dumps(data, separators=(',', ':'), sort_keys=True).encode('ascii')
...
class RollbackRefinementDecision:
    ...
    def canonical_bytes(self) -> bytes:
        """Canonical byte representation for hashing."""
        data = {'chosen': self.chosen.name, 'ranked': tuple((s.name for s in self.ranked)), 'reasons': tuple(sorted(self.reasons))}
        return json.dumps(data, separators=(',', ':'), sort_keys=True).encode('ascii')
