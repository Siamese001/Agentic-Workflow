...
class MLPatternRecord:
    ...
    def canonical_bytes(self) -> bytes:
        ...
        doc = {'domain_hash': self.domain_hash, 'domain_id': self.domain_id, 'model_hash': self.model_hash, 'pattern_id': self.pattern_id, 'payload': self.payload, 'policy_hash': self.policy_hash, 'schema_version': self.schema_version}
    ...
    @staticmethod
    def compute_record_hash(schema_version: int, domain_id: str, domain_hash: str, policy_hash: str, model_hash: str, pattern_id: str, payload: dict[str, Any]) -> str:
        doc = {'domain_hash': domain_hash, 'domain_id': domain_id, 'model_hash': model_hash, 'pattern_id': pattern_id, 'payload': payload, 'policy_hash': policy_hash, 'schema_version': schema_version}
        raw = json.dumps(doc, sort_keys=True, separators=(',', ':'), default=str).encode()
        return _sha256(raw)

    @classmethod
    def build(cls, domain_id: str, policy_hash: str, model_hash: str, pattern_id: str, payload: dict[str, Any], schema_version: int=1) -> MLPatternRecord:
        """Factory: compute domain_hash and record_hash automatically."""
        domain_hash = cls.compute_domain_hash(domain_id)
        record_hash = cls.compute_record_hash(schema_version=schema_version, domain_id=domain_id, domain_hash=domain_hash, policy_hash=policy_hash, model_hash=model_hash, pattern_id=pattern_id, payload=payload)
        return cls(schema_version=schema_version, domain_id=domain_id, domain_hash=domain_hash, policy_hash=policy_hash, model_hash=model_hash, pattern_id=pattern_id, payload=payload, record_hash=record_hash)

def enforce_pattern_compatibility(record: MLPatternRecord, query_domain_id: str, active_policy_hash: str, active_model_hash: str) -> None:
    """
    ...
    if record.domain_hash != expected_domain_hash:
        raise PatternCompatibilityError(PatternCompatibilityError.DOMAIN_MISMATCH, f"Pattern domain_hash {record.domain_hash[:8]}... does not match query domain '{query_domain_id}' (expected {expected_domain_hash[:8]}...)")
    if record.policy_hash != active_policy_hash:
        raise PatternCompatibilityError(PatternCompatibilityError.POLICY_MISMATCH, f'Pattern policy_hash {record.policy_hash[:8]}... != active policy_hash {active_policy_hash[:8]}...')
    if record.model_hash != active_model_hash:
        raise PatternCompatibilityError(PatternCompatibilityError.MODEL_MISMATCH, f'Pattern model_hash {record.model_hash[:8]}... != active model_hash {active_model_hash[:8]}...')
