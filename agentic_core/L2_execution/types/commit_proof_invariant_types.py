...
class CommitProofInvariant:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.digest) != 64 or not all((c in '0123456789abcdef' for c in self.digest)):
            raise DeterminismProofFailure(f"CommitProofInvariant.digest must be a 64-char lowercase hex string, got '{self.digest[:16]}...' (len={len(self.digest)})")
    ...
    def verify_stable(self, recompute_fn: Callable[[], str]) -> None:
        ...
        if actual != self.digest:
            raise DeterminismProofFailure(f'[Phase {self.phase_id}] Determinism proof FAILED: expected={self.digest[:16]}..., actual={actual[:16]}... Inputs changed without updating the committed proof.')
    ...
    def verify_unstable(self, recompute_fn: Callable[[], str]) -> None:
        ...
        actual = recompute_fn()
        if actual == self.digest:
            raise DeterminismProofFailure(f'[Phase {self.phase_id}] Negative control FAILED: tampered inputs produced the same digest={self.digest[:16]}... The determinism function is insensitive to this mutation.')
