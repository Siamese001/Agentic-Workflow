...
_MUTATING_CAPABILITIES: frozenset[ToolCapability] = frozenset({ToolCapability.MUTATING_EXTERNAL, ToolCapability.MUTATING_FS, ToolCapability.MUTATING_STATEBUS})
...
class ToolIntent:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != _SCHEMA_VERSION:
            raise ValueError(f'ToolIntent: schema_version must be {_SCHEMA_VERSION}, got {self.schema_version!r}')
        if not self.tool_name:
            raise ValueError('ToolIntent: tool_name must be non-empty')
        if not isinstance(self.capability, ToolCapability):
            raise TypeError(f'ToolIntent: capability must be ToolCapability, got {type(self.capability).__name__}')
        if not isinstance(self.args, dict):
            raise TypeError('ToolIntent: args must be a dict')
        if is_mutating(self.capability) and (not self.requires_commit):
            raise ValueError(f'ToolIntent: requires_commit must be True for capability {self.capability.value}')
    ...
    def canonical_bytes(self) -> bytes:
        ...
        doc: dict[str, Any] = {'args_hash': self.args_hash, 'budget_hash': self.budget_hash, 'capability': self.capability.value, 'model_hash': self.model_hash, 'policy_hash': self.policy_hash, 'requires_commit': self.requires_commit, 'routing_hash': self.routing_hash, 'schema_version': self.schema_version, 'tool_name': self.tool_name}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()

    def to_dict(self) -> dict[str, Any]:
        return {'schema_version': self.schema_version, 'tool_name': self.tool_name, 'capability': self.capability.value, 'args': self.args, 'args_hash': self.args_hash, 'requires_commit': self.requires_commit, 'policy_hash': self.policy_hash, 'model_hash': self.model_hash, 'budget_hash': self.budget_hash, 'routing_hash': self.routing_hash, 'intent_hash': self.intent_hash}

def build_tool_intent(tool_name: str, capability: ToolCapability, args: dict[str, Any], *, policy_hash: str='', model_hash: str='', budget_hash: str='', routing_hash: str='') -> ToolIntent:
    """
    Factory: build a ToolIntent from tool parameters.

    requires_commit is automatically set to True for MUTATING_* capabilities.
    """
    return ToolIntent(schema_version=_SCHEMA_VERSION, tool_name=tool_name, capability=capability, args=args, requires_commit=is_mutating(capability), policy_hash=policy_hash, model_hash=model_hash, budget_hash=budget_hash, routing_hash=routing_hash)
