...
class VLLMGatewayAdapter:
    ...
    def evaluate(self, prompt: str, task_class: str, severity: str, oldest_wait_seconds: float=0.0, fingerprint: VLLMInfrastructureFingerprint | None=None) -> VLLMGatewayCallResult:
        ...
        """
        from agentic_core.L2_execution.types.vllm_gateway_integration_types import VLLMGatewayCallResult, evaluate_gateway_call
        ...
        result = evaluate_gateway_call(prompt=prompt, task_class=task_class, severity=severity, queue_controller=q, breaker_registry=r, oldest_wait_seconds=oldest_wait_seconds, fingerprint=fingerprint)
        telemetry_dict = result.telemetry.as_dict()
        violations = verify_gateway_invariants(provider_selected=telemetry_dict['provider_selected'], local_request=result.local_request, telemetry_dict=telemetry_dict, fingerprint=fingerprint, replay_hash_enabled=False, gpu_import_policy_ok=True)
        ...
        if fail_violations:
            ...
            telemetry_with_failure = VLLMGatewayTelemetry(provider_selected=result.telemetry.provider_selected, model_tier=result.telemetry.model_tier, prompt_tokens_estimated=result.telemetry.prompt_tokens_estimated, max_output_tokens_requested=result.telemetry.max_output_tokens_requested, max_model_len_configured=result.telemetry.max_model_len_configured, token_budget_ok=result.telemetry.token_budget_ok, budget_margin_tokens=result.telemetry.budget_margin_tokens, queue_depth=result.telemetry.queue_depth, queue_full=result.telemetry.queue_full, queue_wait_seconds=result.telemetry.queue_wait_seconds, breaker_state=result.telemetry.breaker_state, breaker_failure_count=result.telemetry.breaker_failure_count, failure_type='INVARIANT_VIOLATION', model_name=result.telemetry.model_name, model_revision_sha=result.telemetry.model_revision_sha, vllm_version=result.telemetry.vllm_version, transformers_version=result.telemetry.transformers_version, torch_version=result.telemetry.torch_version, cuda_version=result.telemetry.cuda_version, driver_version=result.telemetry.driver_version, fingerprint_hash=result.telemetry.fingerprint_hash)
            result = VLLMGatewayCallResult(route_to_gemini=True, local_request=None, telemetry=telemetry_with_failure, preflight=result.preflight, backpressure=result.backpressure, invariant_violations=violations)
        elif violations:
            result = VLLMGatewayCallResult(route_to_gemini=result.route_to_gemini, local_request=result.local_request, telemetry=result.telemetry, preflight=result.preflight, backpressure=result.backpressure, invariant_violations=violations)
        return result
