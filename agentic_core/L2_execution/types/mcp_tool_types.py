...
class MCPToolServer:
    ...
    def __init__(self, name: str='agentic-workflow-tools', *, allow_legacy_capability_enforcer: bool=False):
    ...
    def set_capability_enforcer(self, enforcer: Any) -> None:
        ...
        if not self._allow_legacy_capability_enforcer:
            raise ValueError('legacy capability enforcer is disabled. Use allow_legacy_capability_enforcer=True at construction or pass capability_token= per call.')
    ...
    def register_function(self, name: str, description: str, parameters: dict[str, Any], handler: Callable, requires_approval: bool=False) -> None:
    ...
    def execute_tool(self, name: str, arguments: dict[str, Any], *, capability_token: Any | None=None) -> MCPToolResult:
        """Execute a tool.
        ...
        from agentic_core.L2_execution.types.capability_token_types import PERMISSION_CODES, CapabilityEnforcer, build_capability_decision
        ...
        if capability_token is not None:
            ...
            enforcer_local.check(tool_name=name, action='execute', requested_resource=resource_path, required_permission=required_perm, semantic_clock=capability_token.semantic_clock)
            ...
            enforcer_legacy.check(tool_name=name, action='execute', requested_resource=resource_path, required_permission=required_perm, semantic_clock=enforcer_legacy.token.semantic_clock)
        else:
            from agentic_core.L0_routing.types.determinism_types import SemanticClockSnapshot
            deny_clock = SemanticClockSnapshot(tick=0, vector_clock={})
            build_capability_decision(semantic_clock=deny_clock, tool_name=name, action='execute', requested_resource=resource_path, decision='DENY', deny_reason='NO_TOKEN_PROVIDED', capability_trace_id='NONE')
            raise PermissionError('CAPABILITY_DENIED:NO_TOKEN_PROVIDED')
        from agentic_core.L2_execution.enforcement.tool_policy_enforcer import _stable_args_hash, get_tool_policy_enforcer
        ...
        artifact = enforcer.build_artifact(tool_name=name, outcome=outcome, applied_slots=applied_slots, rationale=rationale, original_args_hash=original_hash, modified_args_hash=modified_hash)
        try:
...
def register_default_tools(server: MCPToolServer) -> None:
    ...
    def calculator(operation: str, a: float, b: float) -> float:
        ...
        operations = {'add': lambda x, y: x + y, 'subtract': lambda x, y: x - y, 'multiply': lambda x, y: x * y, 'divide': lambda x, y: x / y if y != 0 else float('inf')}
...
def execute_tool_with_capability(server: MCPToolServer, name: str, arguments: dict[str, Any], *, semantic_clock: Any, subject_kind: str, subject_id: str, issued_by: str, permissions: list[str], allowed_paths: list[str], max_tool_calls: int, policy_config_hash: str | None=None) -> MCPToolResult:
    ...
    token = issue_capability_token(semantic_clock=semantic_clock, subject_kind=subject_kind, subject_id=subject_id, issued_by=issued_by, permissions=permissions, allowed_paths=allowed_paths, max_tool_calls=max_tool_calls, policy_config_hash=policy_config_hash)
    return server.execute_tool(name, arguments, capability_token=token)

def execute_tool_calls(server: MCPToolServer, tool_calls: list[dict[str, Any]], *, capability_token: Any | None=None) -> list[MCPToolResult]:
