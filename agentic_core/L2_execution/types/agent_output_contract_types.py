...
class AgentOutputContract:
    ...
    def _signable_dict(self) -> dict:
        return {'agent_id': self.agent_id, 'output_contract_hash': self.output_contract_hash, 'schema_tag': self.schema_tag, 'trace_id': self.trace_id}
    ...
    def sign(self, secret: bytes) -> AgentOutputContract:
        ...
        return AgentOutputContract(agent_id=self.agent_id, trace_id=self.trace_id, schema_tag=self.schema_tag, output_contract_hash=self.output_contract_hash, payload=self.payload, signature=mac.hexdigest().lower())
...
def wrap_output(agent_id: str, trace_id: str, payload_model: Any, secret: bytes) -> AgentOutputContract:
    ...
    contract = AgentOutputContract(agent_id=agent_id, trace_id=trace_id, schema_tag=schema_tag, output_contract_hash=contract_hash, payload=payload_model.model_dump())
