...
def enforce_healer_pipe_order(expected_steps: tuple[str, ...], observed_steps: Sequence[str], trace_id: str | None=None) -> None:
    ...
    """
    assert len(expected_steps) == _REQUIRED_STEP_COUNT, f'enforce_healer_pipe_order: expected_steps must have exactly {_REQUIRED_STEP_COUNT} entries, got {len(expected_steps)}'
