...
def validate_provider_request(
    ...
    """
    # Check provider substitution
    if actual_provider != original_request.provider:
        violation_msg = (
            f"Provider substitution detected: agent '{original_request.agent_id}' "
            f"requested provider '{original_request.provider}' but got '{actual_provider}'. "
            f"Provider substitution is prohibited (REQ-415)."
        )
        Logger.error(violation_msg)
        raise ProviderSubstitutionViolation(violation_msg)

    # Check model substitution
    if actual_model != original_request.model:
        violation_msg = (
            f"Model substitution detected: agent '{original_request.agent_id}' "
            f"requested model '{original_request.model}' but got '{actual_model}'. "
            f"Model substitution is prohibited (REQ-415)."
        )
        Logger.error(violation_msg)
        raise ProviderSubstitutionViolation(violation_msg)
    ...
    Logger.debug(
        f"Provider request validated: agent '{original_request.agent_id}' "
        f"using provider '{actual_provider}' with model '{actual_model}'"
    )
...
def enforce_fail_closed_on_failure(
    ...
    """
    violation_msg = (
        f"Provider request failed for agent '{original_request.agent_id}' "
        f"with provider '{original_request.provider}' and model '{original_request.model}'. "
        f"Error: {error}. "
        f"Fail-closed enforced - no substitution allowed (REQ-415)."
    )

    if attempted_substitution:
        violation_msg += (
            f" Attempted substitution to provider '{attempted_substitution.get('provider', 'unknown')}' "
            f"with model '{attempted_substitution.get('model', 'unknown')}' was blocked."
        )
...
def test_provider_substitution_prohibition() -> bool:
    ...
    try:
        ...
        # Test validation with wrong provider
        try:
        ...
        # Test validation with wrong model
        try:
        ...
        try:
            ...
            return False  # Should not raise for valid case
