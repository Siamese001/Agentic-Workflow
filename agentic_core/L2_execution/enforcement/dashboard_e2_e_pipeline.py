...
from agentic_core.L0_routing.config import AGENTIC_CORE_DIR, DASHBOARD_DIR, get_validated_project_root
...
class DashboardE2EPipeline:
    ...
    def __init__(self):
        ...
        self.stats = {'heal_fixes': 0, 'agents_discovered': 0, 'dashboard_rows': 0, 'heal_invocation_before': 0, 'heal_invocation_after': 0}
    ...
    def step3_regenerate_discovery(self) -> bool:
        ...
        try:
            ...
            result = safe_execute([sys.executable, str(discovery_script)], cwd=str(self.project_root), capture_output=True, check=False, text=True, timeout=DEFAULT_TIMEOUT)
    ...
    def step4_regenerate_dashboard(self) -> bool:
        ...
        dashboard_script = self.project_root / AGENTIC_CORE_DIR / 'L6_observability' / 'dashboards' / 'generate_dashboard.py'
        if not dashboard_script.exists():
            print('   ❌ Dashboard generator not found')
            return False
        try:
            result = safe_execute([sys.executable, str(dashboard_script)], cwd=str(self.project_root), capture_output=True, check=False, text=True, timeout=DEFAULT_TIMEOUT)
            if result.returncode != 0:
    ...
    def step5_run_e2e_tests(self) -> bool:
        ...
        if not test_script.exists():
            ...
            return False
        try:
            result = safe_execute([sys.executable, str(test_script)], cwd=str(self.project_root), capture_output=True, check=False, text=True, timeout=DEFAULT_TIMEOUT, env={'PYTHONPATH': str(self.project_root)})
            print(result.stdout)
            if result.returncode != 0:
                print('   ❌ Tests failed')
                if result.stderr:
