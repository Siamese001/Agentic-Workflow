...
def _get_UnifiedAgent_mapping() -> dict[str, type]:
    ...
    return {'BareExceptValidator': ASTValidatorAgent, 'BareExceptValidatorAgent': ASTValidatorAgent, 'EmptyExceptValidator': ASTValidatorAgent, 'EmptyExceptValidatorAgent': ASTValidatorAgent, 'EvalExecValidator': ASTValidatorAgent, 'EvalExecValidatorAgent': ASTValidatorAgent, 'DangerousBuiltinsValidator': ASTValidatorAgent, 'DangerousBuiltinsValidatorAgent': ASTValidatorAgent, 'DebuggerValidator': ASTValidatorAgent, 'DebuggerValidatorAgent': ASTValidatorAgent, 'HygieneGuardian': StructureValidatorAgent, 'HygieneGuardianAgent': StructureValidatorAgent, 'HygieneValidator': StructureValidatorAgent, 'HygieneValidatorAgent': StructureValidatorAgent, 'CheckpointManager': CheckpointManagerAgent, 'CheckpointManagerAgent': CheckpointManagerAgent, 'AutonomousCheckpointManager': CheckpointManagerAgent, 'AutonomousCheckpointManagerAgent': CheckpointManagerAgent, 'BaseClassEnforcer': CodeEnforcerAgent, 'BaseClassEnforcerAgent': CodeEnforcerAgent, 'PatternEnforcer': CodeEnforcerAgent, 'PatternEnforcerAgent': CodeEnforcerAgent, 'TypeHintEnforcement': CodeEnforcerAgent, 'TypeHintEnforcementAgent': CodeEnforcerAgent, 'ManifestManager': StateManagementAgent, 'ManifestManagerAgent': StateManagementAgent, 'MemoryManager': StateManagementAgent, 'MemoryManagerAgent': StateManagementAgent, 'AutonomousStateGuardian': StateManagementAgent, 'AutonomousStateGuardianAgent': StateManagementAgent}
...
def _get_phase3_manager_enforcer_mapping() -> dict[str, type]:
    ...
    return {'BudgetManagerAgent': ResourceManagerAgent, 'ProactiveResourceManagerAgent': ResourceManagerAgent, 'FallbackManagerAgent': ResourceManagerAgent, 'AgentPermissionManagerAgent': SecurityManagerAgent, 'SecureCheckpointManagerAgent': SecurityManagerAgent, 'SecureConfigManagerAgent': SecurityManagerAgent, 'CodeSSOTEnforcerAgent': CodeEnforcerAgent, 'CodeEnforcerAgent': CodeEnforcerAgent, 'PatternEnforcerAgent': CodeEnforcerAgent, 'TypeEnforcerAgent': CodeEnforcerAgent, 'PythonFileSovereigntyEnforcerAgent': CodeEnforcerAgent, 'GravityEnforcerAgent': StructureEnforcerAgent, 'HierarchyEnforcerAgent': StructureEnforcerAgent, 'NamingEnforcerAgent': StructureEnforcerAgent, 'DocEnforcerAgent': StructureEnforcerAgent, 'ASCIIEnforcerAgent': StructureEnforcerAgent, 'StrictDocEnforcerAgent': StructureEnforcerAgent, 'FileClassificationEnforcerAgent': StructureEnforcerAgent}
...
def _get_phase4_detector_healer_router_executor_mapping() -> dict[str, type]:
    ...
    return {'DeadCodeDetectorAgent': CodeDetectorAgent, 'DeadlockDetectorAgent': CodeDetectorAgent, 'DriftDetectorAgent': CodeDetectorAgent, 'MethodChangeDetectorAgent': CodeDetectorAgent, 'MemoryLeakDetectorAgent': CodeDetectorAgent, 'BiasDetectorAgent': SafetyDetectorAgent, 'HallucinationDetectorAgent': SafetyDetectorAgent, 'PromptInjectionDetectorAgent': SafetyDetectorAgent, 'CanonHealerAgent': CodeHealerAgent, 'ImportHealerAgent': CodeHealerAgent, 'StructuralHealerAgent': CodeHealerAgent, 'GravityHealerAgent': StructureHealerAgent, 'HierarchyHealerAgent': StructureHealerAgent, 'NamingLawHealerAgent': StructureHealerAgent, 'TerritoryHealerAgent': StructureHealerAgent, 'BlueprintHierarchyHealerAgent': StructureHealerAgent, 'ModelRouterAgent': ModelRouterAgent, 'DynamicModelRouterAgent': ModelRouterAgent, 'MultiProviderRouterAgent': ModelRouterAgent, 'ReasoningRouterAgent': ModelRouterAgent, 'McpRouterAgent': ModelRouterAgent, 'IntegrityGateExecutorAgent': SafetyExecutorAgent, 'L5IntegrityGateExecutorAgent': SafetyExecutorAgent, 'SafetyExecutorAgent': SafetyExecutorAgent}
...
def _get_phase2_validator_mapping() -> dict[str, type]:
    ...
    return {'SyntaxValidatorAgent': CodeValidatorAgentWrapper, 'CanonAstValidatorAgent': CodeValidatorAgentWrapper, 'CanonValidatorAgent': CodeValidatorAgentWrapper, 'AsyncBlockingValidatorAgent': CodeValidatorAgentWrapper, 'PrintStatementValidatorAgent': CodeValidatorAgentWrapper, 'GravityValidatorAgent': StructureValidatorAgent, 'HygieneValidatorAgent': StructureValidatorAgent, 'StructureValidatorAgent': StructureValidatorAgent, 'AgentRegistryValidatorAgent': StructureValidatorAgent, 'CognitiveContractValidatorAgent': StructureValidatorAgent, 'ContactValidatorAgent': AppContentValidatorAgent, 'ContentCleanlinessValidatorAgent': AppContentValidatorAgent, 'MessageDiversityValidator': AppContentValidatorAgent}
...
class SubAtomicRegistryAgent(SovereignBaseAgent):
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef | ast.AsyncFunctionDef):
                        ...
                        methods.append({'id': f'{py_file.stem}_{node.name}', 'path': str(py_file), 'method': node.name, 'docstring': doc, 'source_snippet': f'Method: {node.name}\nimport logging\n\nLogger = logging.getLogger(__name__)\nDoc: {doc}\nSource: {source_lines[:200]}...', 'line_number': node.lineno, 'is_async': isinstance(node, ast.AsyncFunctionDef)})
    ...
    def find_method(self, Task: str, top_k: int=3) -> list[dict]:
        ...
        results = [m for m in self._local_method_index if any((kw in m.get('docstring', '').lower() or kw in m.get('method', '').lower() for kw in task_lower.split()))][:top_k]
        try:
            if results:
    ...
    def heal(self, violation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {'status': 'skipped', 'details': f'SubAtomicRegistryAgent heal() not yet implemented for {violation_type}', 'artifacts': [], 'errors': []}
        except (ImportError, AttributeError, TypeError, ValueError) as e:
            return {'status': 'failed', 'details': f'SubAtomicRegistryAgent heal() failed: {str(e)}', 'artifacts': [], 'errors': [str(e)]}
