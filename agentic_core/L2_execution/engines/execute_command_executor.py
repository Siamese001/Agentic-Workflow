...
def execute_command(args: ExecuteCommandArgs) -> tuple[int, str, str]:
    ...
    except ExecutionTimeoutError:    # guardian: ExecutionTimeoutError should be handled with specific context    # guardian: ExecutionTimeoutError should be handled with specific context    # guardian: ExecutionTimeoutError should be handled with specific context    # guardian: ExecutionTimeoutError should be handled with specific context    # guardian: ExecutionTimeoutError should be handled with specific context
        raise
    ...
    for command in ALLOWED_COMMANDS[tool_name]:
        try:
            ...
            if result.returncode == 0:
                return True
        except (subprocess.TimeoutExpired, FileNotFoundError):    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access    # guardian: File operations should check existence before access
            continue
    return False
