...
class ExecutionGateway:
    ...
    async def execute_with_trace(self, envelope: SandboxEnvelope, tool_fn: Any, policy_hash: str='', prev_hash: str='', transcript_hash: str='') -> tuple[ExecutionTrace, Any]:
        ...
        builder = ExecutionTraceBuilder(trace_id=envelope.envelope_id, instruction_packet_id=envelope.instruction_packet_id)
        ...
        try:
            ...
            tool_result = ToolResult.from_budget_enforcer(exit_code=exit_code, stdout_bytes=stdout_bytes, stdout_bytes_cap=envelope.budget.stdout_bytes)
            ...
            builder.extra = {'stdout_bytes': len(stdout_bytes), 'stdout_hash': hashlib.sha256(stdout_bytes).hexdigest(), 'exit_code': exit_code, 'budget_compute_ms': envelope.budget.compute_ms, 'budget_memory_mb': envelope.budget.memory_mb, 'budget_stdout_bytes': envelope.budget.stdout_bytes}
            builder.hash_chain_root = hashlib.sha256(f'{builder.trace_id}{builder.timing_ms}{builder.validation_decision}'.encode()).hexdigest()
        trace = builder.seal()
        return (trace, stdout_bytes)

    def create_envelope(self, tool_name: str, tool_args: dict[str, Any], instruction_packet_id: str, agent_id: str, **metadata: Any) -> SandboxEnvelope:
        ...
        envelope = SandboxEnvelope(envelope_id=f'{instruction_packet_id}_{tool_name}', tool_name=tool_name, tool_args=tool_args, instruction_packet_id=instruction_packet_id, invocation_metadata=env_metadata, budget=ToolBudget())
        return envelope
