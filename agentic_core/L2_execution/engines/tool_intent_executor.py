...
class ToolResult:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != _SCHEMA_VERSION:
            raise ValueError(f'ToolResult: schema_version must be {_SCHEMA_VERSION}, got {self.schema_version!r}')
    ...
    def canonical_bytes(self) -> bytes:
        ...
        doc: dict[str, Any] = {'anchor_ids': sorted(self.anchor_ids), 'args_hash': self.args_hash, 'output_summary': self.output_summary, 'schema_version': self.schema_version, 'success': self.success, 'tool_name': self.tool_name}
        return json.dumps(doc, sort_keys=True, separators=(',', ':')).encode()

    def to_dict(self) -> dict[str, Any]:
        return {'schema_version': self.schema_version, 'tool_name': self.tool_name, 'args_hash': self.args_hash, 'success': self.success, 'output_summary': self.output_summary, 'anchor_ids': list(self.anchor_ids), 'result_hash': self.result_hash}
...
class ToolIntentExecutor:
    ...
    def execute(self, intent: ToolIntent, fn: Callable[[dict[str, Any]], dict[str, Any]]) -> ToolResult:
        ...
        if intent.requires_commit and (not is_commit_sandbox_active()):
            raise ToolViolation(code='TOOL_WRITE_OUTSIDE_SANDBOX', detail=f"tool '{intent.tool_name}' requires commit sandbox (capability={intent.capability.value})")
        raw = fn(intent.args)
        output_summary = str(raw.get('output_summary', ''))
        anchor_ids: list[str] = list(raw.get('anchor_ids', []))
        return ToolResult(schema_version=_SCHEMA_VERSION, tool_name=intent.tool_name, args_hash=intent.args_hash, success=raw.get('success', True), output_summary=output_summary, anchor_ids=anchor_ids)
