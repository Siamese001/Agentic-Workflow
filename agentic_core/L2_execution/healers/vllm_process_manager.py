...
class VLLMProcessManager:
    ...
    def start_server(self, model_config: dict) -> int:
        ...
        cmd = ['python', '-m', 'vllm.entrypoints.openai.api_server', '--model', model_id, '--host', '0.0.0.0', '--port', '8000', '--trust-remote-code', '--max-model-len', '8192', '--gpu-memory-utilization', str(QWEN_GPU_MEM_UTIL)]
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            with _urllib_request.urlopen(f'{self.base_url}/health', timeout=DEFAULT_TIMEOUT) as _resp:
    ...
    def get_memory_usage(self) -> dict:
        ...
        try:
            import subprocess
            result = subprocess.run(['nvidia-smi', '--query-gpu=memory.used,memory.total', '--format=csv,noheader,nounits'], capture_output=True, text=True, timeout=DEFAULT_TIMEOUT)
...
def get_model_config(model_size: str='7B') -> dict:
    ...
    configs = {'7B': {'model_id': 'Qwen/Qwen2.5-7B-Instruct', 'max_model_len': 8192, 'gpu_memory_utilization': QWEN_GPU_MEM_UTIL}, '14B': {'model_id': 'Qwen/Qwen2.5-14B-Instruct', 'max_model_len': 4096, 'gpu_memory_utilization': QWEN_GPU_MEM_UTIL}}
