...
def heal_file_classification(check: dict, *, repo_root: Path | None=None, apply: bool=False) -> HealCheckResult:
    ...
    if not violations_count:
        return HealCheckResult(check_id=CHECK_ID, status=HealStatus.HEALED, changes_made=(), notes='no file classification violations detected')
    if not apply:
        return HealCheckResult(check_id=CHECK_ID, status=HealStatus.SKIPPED, changes_made=(f'would_fix:{violations_count}_classification_violations',), notes='dry-run: no mutations applied')
    ...
    try:
        ...
        res = classifier.heal_repository(target_territory=territory, dry_run=False, auto_approve=True, cached_scan=cached_scan)
    ...
    return HealCheckResult(check_id=CHECK_ID, status=status, changes_made=tuple(sorted(changes)), notes=f'files_healed={files_healed} status={res_status}')
