...
def heal_filesystem_ssot_drift(check: dict, *, repo_root: Path | None=None, apply: bool=False) -> HealCheckResult:
    ...
    all_items = sorted([f'forbidden_root_folder:{f}' for f in forbidden] + [f'archived_file_at_root:{f}' for f in archived_at_root] + ['duplicate_folder:{}'.format(d.get('name', d) if isinstance(d, dict) else str(d)) for d in duplicate_folders])
    if not all_items:
        return HealCheckResult(check_id=CHECK_ID, status=HealStatus.HEALED, changes_made=(), notes='no filesystem ssot drift detected')
    if not apply:
        return HealCheckResult(check_id=CHECK_ID, status=HealStatus.SKIPPED, changes_made=tuple((f'would_fix:{item}' for item in all_items)), notes='dry-run: no mutations applied')
    if repo_root is None:
        return HealCheckResult(check_id=CHECK_ID, status=HealStatus.FAILED, changes_made=(), notes='apply mode requires repo_root', needs_llm_escalation=False)
    ...
    return HealCheckResult(check_id=CHECK_ID, status=status, changes_made=tuple(sorted(performed)), notes=f'applied={len(performed)} failed={len(failed)}')
