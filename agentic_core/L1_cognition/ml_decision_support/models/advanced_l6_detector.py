...
class AdvancedL6Detector(BaseMLModel):
    ...
    def _calculate_overall_health(self, system_context: dict[str, Any]) -> dict[str, Any]:
        ...
        overall_score = (
            metric_health['score'] * 0.3 +
            performance_health['score'] * 0.4 +
            resource_health['score'] * 0.3
        )

        return {
            'score': overall_score,
            'status': 'healthy' if overall_score > 0.7 else 'degraded' if overall_score > 0.4 else 'unhealthy',
            'components': {
    ...
    def get_feature_importance(self, model_input: ModelInput) -> list[dict[str, Any]]:
        ...
        try:
            ...
            importances = np.ones(len(self.feature_names)) / len(self.feature_names)  # Isolation Forest doesn't have feature_importances
