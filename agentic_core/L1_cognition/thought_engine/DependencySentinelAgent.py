...
class ImportAnalyzer(ast.NodeVisitor):
    ...
    def visit_Import(self, node: ast.Import) -> Any:
        ...
        for alias in node.names:
            self.imports.append({'module': alias.name, 'alias': alias.asname, 'line': node.lineno, 'type': 'import'})
...
class DependencySentinelAgent(SubatomicTestingMixin, HealerMixin, MCPHardenedMixin):
    ...
    def _check_cross_layer_violation(self, analyzer: ImportAnalyzer, module: str, line: int) -> Optional[DependencyViolation]:
        ...
        if analyzer.layer == imported_layer:
            return None
        return DependencyViolation('cross_layer_violation', analyzer.file_path, line, f'Illegal cross-layer import: {analyzer.layer} -> {imported_layer}', {'from_layer': analyzer.layer, 'to_layer': imported_layer, 'module': module})
