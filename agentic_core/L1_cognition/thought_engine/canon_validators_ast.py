...
class PrintStatementValidator(CanonASTValidator):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == "print":
            # Skip if we're inside a TYPE_CHECKING block
...
class EmptyExceptValidator(CanonASTValidator):
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler):
        ...
        # Empty if no body or only contains 'pass'
        is_empty = (not node.body or 
...
# ==============================================================================
# KEY 23: No External HTTP Imports
# ==============================================================================
...
class ExternalHTTPValidator(CanonASTValidator):
    ...
    FORBIDDEN_MODULES = {'requests', 'urllib', 'urllib3', 'httpx', 'aiohttp'}
        ...
        if not self.in_type_checking:
            for alias in node.names:
                ...
                if module_root in self.FORBIDDEN_MODULES:
                    self.report(
                        f"Forbidden HTTP library import: {alias.name} "
                        f"(use MCP fetch_client_sovereign instead)",
                        node
                    )
        self.generic_visit(node)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        if not self.in_type_checking and node.module:
            ...
            if module_root in self.FORBIDDEN_MODULES:
                self.report(
                    f"Forbidden HTTP library import: from {node.module} "
                    f"(use MCP fetch_client_sovereign instead)",
                    node
                )
        self.generic_visit(node)
...
# ==============================================================================
# KEY 31: No Blocking Calls in Async
# ==============================================================================
        ...
        if self.in_async_function and not self.in_type_checking:
            ...
            if isinstance(node.func, ast.Attribute):
                if (isinstance(node.func.value, ast.Name) and 
                    ...
                    self.report(
                        "Blocking time.sleep() in async function (use asyncio.sleep())",
                        node
                    )
                ...
                # requests.get/post/etc
                    ...
                    self.report(
                        f"Blocking requests.{node.func.attr}() in async function "
                        f"(use httpx.AsyncClient or asyncio.to_thread())",
                        node
                    )
...
class DangerousBuiltinsValidator(CanonASTValidator):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Name) and node.func.id in self.DANGEROUS_BUILTINS:
            if not self.in_type_checking:
                self.report(
                    f"Dangerous builtin {node.func.id}() detected "
