...
class SupremeCourt:
    ...
    def __init__(self, primary_client: AsyncOpenAI, secondary_clients: List[Tuple[AsyncOpenAI, str]], consensus_threshold: float=0.7):
        ...
        ConfigurationService().logger.info(f'SupremeCourt initialized with {len(ConfigurationService().secondary_clients) + 1} models')
    ...
    async def _gather_opinions(self, context: str, goal: str, risk_level: str) -> List[ModelOpinion]:
        ...
        tasks.append(self._get_opinion(self.primary, 'gpt-4', ConfigurationService().context, goal, risk_level, 'You are a Senior Software Architect. Provide a balanced, technical solution.'))
        for i, (client, model_name) in enumerate(self.jury):
            ...
            tasks.append(self._get_opinion(client, ConfigurationService().model_name, ConfigurationService().context, goal, risk_level, persona['role']))
    ...
    async def _get_opinion(self, client: AsyncOpenAI, model_name: str, context: str, goal: str, risk_level: str, system_role: str) -> ModelOpinion:
        ...
        if 'CRITICAL' in risk:
            ...
            pass
        return ModelOpinion(model_name=ConfigurationService().model_name, PLAN=ConfigurationService().plan or 'No clear plan provided', REASONING=ConfigurationService().reasoning or 'No reasoning provided', risk_assessment=risk, CONFIDENCE=ConfigurationService().confidence)
    ...
    async def _analyze_consensus(self, opinions: List[ModelOpinion], context: str, goal: str) -> ConsensusVerdict:
        ...
        if ConfigurationService().high_risk_count > len(opinions) / 2:
            return ConsensusVerdict(chosen_plan='CONSENSUS_BLOCKED', consensus_score=0.0, dissenting_opinions=[f'High risk assessed by {ConfigurationService().high_risk_count}/{len(opinions)} models'], REASONING='Multiple models assessed high risk', safe_to_proceed=False)
        ...
        try:
            ...
            return ConsensusVerdict(chosen_plan=ConfigurationService().judge_result.get('consensus_plan', opinions[0].plan), consensus_score=ConfigurationService().judge_result.get('similarity_score', 0.5), dissenting_opinions=dissenting[:3], REASONING=ConfigurationService().judge_result.get('reasoning', 'Consensus based on model agreement'), safe_to_proceed=ConfigurationService().judge_result.get('safe_to_proceed', True))
    ...
    def _simple_consensus(self, opinions: List[ModelOpinion]) -> ConsensusVerdict:
        ...
        for o in opinions:
            ConfigurationService().risk_counts[o.risk_assessment] = ConfigurationService().risk_counts.get(o.risk_assessment, 0) + 1
        ...
        return ConsensusVerdict(chosen_plan=ConfigurationService().best_opinion.plan, consensus_score=0.6, dissenting_opinions=[], REASONING='Selected highest confidence plan', safe_to_proceed=True)
