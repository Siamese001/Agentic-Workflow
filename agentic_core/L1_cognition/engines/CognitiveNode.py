...
class PerceptionNode:
    ...
    async def process_async(self, raw_input: dict, context: dict) -> dict:
        ...
        return {'query': raw_input.get('user_query', ''), 'context': context, 'timestamp': time.time(), 'input_type': self._classify_input(raw_input)}
...
class ReasoningNode:
    ...
    async def reason_async(self, perceived: dict) -> dict:
        ...
        return {'goal': reasoning.get('goal', perceived['query']), 'domain': reasoning.get('domain', 'general'), 'thought_type': thought_type, 'reasoning': reasoning, 'confidence': reasoning.get('confidence', 0.7)}
    ...
    def _generate_reasoning(self, query: str, thought_type: str, patterns: list[Any]) -> dict[str, Any]:
        ...
        reasoning = {'goal': query, 'domain': 'general', 'thought_type': thought_type, 'steps': [], 'confidence': 0.8}
        if thought_type == 'cot':
...
class PlanningCoordinator:
    ...
    def plan(self, goal: str, domain: str, context: dict) -> dict[str, Any]:
        ...
        plan = {'goal': goal, 'domain': domain, 'steps': self._generate_steps(goal, domain), 'score': 0.8, 'patterns_applied': 0}
        if memory_patterns:
...
class CognitiveNode:
    ...
    async def process_async(self, raw_input: dict, context: dict) -> CognitiveResult:
        ...
        try:
            ...
            if self.meta_learner:
                ...
                self.meta_learner.store_experience(state=perceived, thought_type=reasoned['thought_type'], outcome={'output': output, 'success': True}, reward=reward)
                if self.missions_processed % 10 == 0:
                    await self._async_replay_and_learn()
            latency_ms = (time.time() - start_time) * 1000
            self.total_latency_ms += latency_ms
            return CognitiveResult(output=output, thought_type=reasoned['thought_type'], plan=plan, memory_used=memory_used, governance={'status': 'compliant'}, latency_ms=latency_ms, success=True)
        # guardian: allow-silent-swallow
        except Exception as e:
            latency_ms = (time.time() - start_time) * 1000
            return CognitiveResult(output=f'Error: {str(e)}', thought_type='error', latency_ms=latency_ms, success=False)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'missions_processed': self.missions_processed, 'average_latency_ms': avg_latency, 'semantic_memory_enabled': self.semantic_memory is not None, 'meta_learning_enabled': self.meta_learner is not None, 'meta_learner_stats': self.meta_learner.get_statistics() if self.meta_learner else {}}
