...
FILE_GRAPH_RELS = {"imports", "belongs_to_layer", "covers", "in_cycle", "dead_imports", "exports", "influences"}
SYMBOL_GRAPH_RELS = {"calls", "instantiates", "implements", "reads_from", "writes_to", "writes_through", "invokes_provider", "routes_through", "type_annotation", "decorated_by"}
TEST_GRAPH_RELS = {"covers", "covers_module", "covers_symbol"}
GOVERNANCE_GRAPH_RELS = {"generates_prompt", "consumes_prompt", "assembles_into", "injects_into", "overrides_prompt", "executed_with_prompt", "triggered_telemetry", "proposed_improvement", "updated_prompt", "executes_action", "invokes_tool", "crosses_layer", "bypasses_uwg", "routes_through_uwg", "layer_authority_violation", "policy_hash_mismatch", "lineage_of", "violates", "dynamic_exec", "in_cycle", "antipattern"}
...
sqlite_rels = {row[0]: row[1] for row in conn.execute('SELECT relation_type, COUNT(*) FROM edges GROUP BY relation_type')}
conn.close()
...
for r, n in sorted(sqlite_rels.items(), key=lambda x: -x[1]):
    ...
    planes = "+".join(p for p, flag in [("FILE",in_file),("SYM",in_sym),("TEST",in_test),("GOV",in_gov)] if flag)
