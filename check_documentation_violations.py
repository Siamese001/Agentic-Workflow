#!/usr/bin/env python3
"""Quick script to check violations in DocumentationAgent.py"""
import asyncio
import sys
...
async def main():
    ...
    file_path = project_root / "agentic_core" / "L1_cognition" / "thought_engine" / "DocumentationAgent.py"
    
    violations = await validator.detect_violations(file_path)
    
    print(f"\n📊 Violations in DocumentationAgent.py: {len(violations)}")
    for v in violations:
