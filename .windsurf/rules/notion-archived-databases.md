...
description: Reference when deciding whether to write to a Notion database. Five databases were archived 2026-05-02; filesystem is now the SSOT for their content. Prevents accidental writes to dead Notion endpoints.
---
...
| **MCP Registry** | DB `59693bbc-...` / DS `e7b149b4-...` | `.windsurf/mcp_config.json` is the SSOT; Notion mirror caused drift | `.windsurf/mcp_config.json` |
...
- `post_cascade_adr_registry_capture.py` — now logs `kind: adr_filesystem_only` to JSONL; no Notion write
...
Constitutional §25 (MCP serialization — remote MCPs only). AGENTS.md Notion Workspace Map (archived rows marked with ~~strikethrough~~). Plan `notion-integration-consistency-audit-b2c4d8`.
