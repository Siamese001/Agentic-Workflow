...
description: Apply for any task mentioning runtime certification, signoff, RTC-REQ-*, attestation, evidence assertions, mutation rejection, compile_requirement_signoff.py, or claims of SIGNED_OFF / certified. Demoted from always_on 2026-05-01 per Anthropic two-tier compliance.
---
...
## Forbidden prose claims (post_cascade audit logs to violations file)
...
| Concern | Location |
|---|---|
| Full assertion contract + forbidden-pattern table + producer allowlist | `.windsurf/skills/fortknox-evidence/SKILL.md` (canonical procedural SSOT) |
...
| Pre-write hook (exit 2) | `.windsurf/scripts/pre_write_fortknox_guard.py` |
| Post-response audit | `.windsurf/scripts/post_cascade_fortknox_integrity_audit.py` → `artifacts/windsurf/fortknox_integrity_violations.jsonl` |
