...
31. **SSOT folder routing for new files.** Every NEW Python file MUST land in its canonical SSOT folder. Forbidden roots for NEW files: repo-root `scripts/` (except the legacy tier-verify allowlist: `verify_tier\d+_(enforcement|runtime_proof)_gate.py`, `verify_all_requirements_*.py`, `verify_tier_gate_hardening.py`, `c0_evidence_harness.py`, `scripts/proof/**`); repo-root `*.py` (except `conftest.py`); `tools/_oneoff/`, `tools/_oneshot/`; hook-prefix files (`pre_*_*.py`, `post_*_*.py`) outside `.windsurf/scripts/`. Routing rules — CI gates / `check_*.py` / `*_gate.py` → `ops_scripts/ci/`; calibration / `*_binder.py` / `*_poller.py` → `ops_scripts/calibration/`; `purge_*.py` / `cleanup_*.py` → `ops_scripts/maintenance/`; Windsurf hooks → `.windsurf/scripts/`; everything else → `tools/<domain>/`. SSOT logic: `.windsurf/scripts/_ssot_folder_check.py`. Hook layer: `pre_write_gate.py` blocks NEW-file violations at write time (exit 2). CI layer: `ops_scripts/ci/check_ssot_folder_routing.py` (pre-commit `T7q`) blocks NEW-file violations at commit time. Pre-existing files NEVER trigger the gate — only new files. Bypass: `SSOT_FOLDER_BYPASS=1`. Failure precedent: 2026-04-30 — Cascade authored `scripts/check_*.py` instead of `ops_scripts/ci/check_*.py`. See `ssot-folder-enforcement.md`.
