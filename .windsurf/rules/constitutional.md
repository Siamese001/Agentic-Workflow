...
30. **Author-Gate capture health mandatory.** Every refactor-class response MUST emit `DECISION_CAPTURED:` (post-hook OR inline `tools/capture/append_marker.py`). Ledger + queue at `.windsurf/state/refactor_decisions/` and `artifacts/capture/markers.jsonl` MUST receive writes within rolling 24h. Pre-session staleness check + CI gate `check_capture_queue_freshness.py`. Detail: `author-gate-enforcement.md`. Bypass: `AUTHOR_GATE_STALE_BYPASS=1`.
...
32. **Fort Knox certification integrity.** Runtime certification claims emerge ONLY from `scripts/compile_requirement_signoff.py` consuming atomic assertions at `certification/evidence_assertions.jsonl`. Producers restricted to `tools/cert/*.py`, `scripts/verify_*_gate.py`, `scripts/verify_rtc_*.py`. `RTC-REQ-001` is positive-control canary. Compiler outputs (`*.json/sha256/merkle.json/signature.json`, `*.xlsx`) MUST NOT be hand-edited. Author-Gate `certification_claim` fires before SIGNED_OFF prose. Detail: `fortknox-certification-discipline.md` + skill `fortknox-evidence`. Bypass: `FORTKNOX_DISCIPLINE_BYPASS=1`.
...
PowerShell §0 · shell=True §0 · scope growth §18 · grep-vs-ADG §5/§22/§28 · completion-without-verify §19 · anti-pattern §6/§8 · progress bar §16 · `DEFERRED_SCOPE:` §24 · remote-MCP batch §25 · config schema §27 · interactive pagers §26 · `DECISION_CAPTURED:` §30 · refactor-class capture §6/§30.
