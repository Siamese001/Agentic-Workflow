---
description: Apply when extending any post-hook, calibration script, or consulting skill participating in the ten intelligence ledgers family (ADR-050). Enforces writer contract, fail-soft discipline, and consulting-skill discoverability.
...
- All DDL changes go through `.windsurf/schemas/ledger_base.schema.sql` or
  `.windsurf/schemas/<name>_ledger.schema.sql`.
...
`mcp-serialization.md` TTL pattern:

1. Operator writes `.windsurf/config/ledger_ttl_<name>.json` with
...
- **Base schema**: `.windsurf/schemas/ledger_base.schema.sql`
- **Registry**: `tools/ledgers/schema_registry.py`
- **Writer / Consulter / Helpers**: `tools/ledgers/{writer,consulter,hook_helpers}.py`
- **Base consulting skill**: `.windsurf/skills/ledger-consulter/SKILL.md`
