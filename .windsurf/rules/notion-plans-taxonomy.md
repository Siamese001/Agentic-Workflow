...
**Audit (write-time)**: `post_cascade_notion_plans_status_audit.py` — fires when a `Status=Waiting` write is detected in the Cascade response without a corresponding `Waiting For` value in the same invoke body.  

**Enforcement layers**:
1. **Write-time** — `post_cascade_notion_plans_status_audit.py` logs `WAITING_EMPTY_WAITING_FOR` to `artifacts/windsurf/notion_plans_status_violations.jsonl` when Cascade writes `Status=Waiting` without `Waiting For` in the same API call.
