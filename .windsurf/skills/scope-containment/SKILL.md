...
|-----|-------|------------|--------|
| `grep_search` + `code_search` | 3/response | `post_cascade_grep_budget_audit.py` | `GREP_BUDGET_BYPASS=1` |
| File reads (native + MCP) | 10/response | `post_cascade_read_budget_audit.py` | `READ_BUDGET_BYPASS=1` |
...
|--------|----------|-------------|
| `NEXT_STEP:` | Optional follow-up ideas, out-of-scope improvements | `post_cascade_next_step_capture.py` |
| `SCOPE_RESET:` | Cross-turn topic transitions | This rule |
| `DEFERRED_SCOPE:` | Wave/phase descoping decisions | `post_cascade_deferred_scope_capture.py` |
...
| Token telemetry | `post_cascade_token_telemetry.py` → `artifacts/windsurf/turn_budget.jsonl` |
| Out-of-scope ideas | `NEXT_STEP:` marker (`next-step-capture.md`) |
| Topic transitions | `SCOPE_RESET:` marker |

## References

- Rule: `.windsurf/rules/scope-containment.md` (invariants)
