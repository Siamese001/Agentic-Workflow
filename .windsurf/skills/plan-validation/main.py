...
def validate_wave_table(content: str) -> Tuple[bool, List[str]]:
    ...
    # Look for wave table
    wave_table_pattern = r'\| Wave \| Phase IDs \| Focus \| Est\. Tokens \| Assumptions \| Status \| Success Criteria \|'
...
def validate_token_estimates(content: str) -> Tuple[bool, List[str]]:
    ...
    # Check if any estimate exceeds 175K (RED status)
    for match in matches:
        tokens = int(match.replace(',', ''))
        if tokens > 175000:
            issues.append(f"Token estimate {tokens:,} exceeds RED threshold (175K)")
...
def validate_plan_location(file_path: str) -> Tuple[bool, List[str]]:
    ...
    # Must be in docs/reports/plans/
    if not str(path).startswith('docs/reports/plans/'):
        issues.append(f"Plan path {file_path} not in SSOT location docs/reports/plans/")
        return False, issues
    
    # Check for .windsurf/plans/ violation
    if '.windsurf' in str(path):
        issues.append(f"Plan path {file_path} violates SSOT (contains .windsurf)")
        return False, issues
