...
Status: **W0 + W1 EXECUTED — W1b..W4 deferred** · Tier: **T3** (plan is T2, execution waves are T3)
Date: 2026-04-23 (audit + W0 + W1 execution)
...
| W1b | W1b.P1 | Enrich `L0RouteContract` reason_codes to closed enum; surface per-claim coverage from C0 | ~15k 🟢 | L0RouteContract is still lightly consumed | 🔴 blocked-on-user-approval | Enum exists, C0 contract carries `coverage_score` |
| W2 | W2.P1, W2.P2 | Derive calibrated thresholds per namespace; retire hardcoded 0.98 / 0.50 literals | ~22k 🟡 | W0 harness output is accepted by calibrators | 🔴 blocked-on-W1 | Thresholds in `config/` YAML, not code literals |
| W3 | W3.P1, W3.P2, W3.P3 | Wire features → router: (a) R3 gate on grounding-need score, (b) R5 on multi-signal, (c) R1B per-namespace threshold | ~48k 🔴 | PathRouter public API can evolve | 🔴 blocked-on-W1,W2 | `PathRouter.route_with_features()` returns `L0RouteContract` driven by features |
| W4 | W4.P1, W4.P2 | OTEL metrics + feedback loop (`cached_tokens`, `hit_ratio`, `false_r5_rate`); weekly calibration cadence | ~20k 🟢 | OTEL MCP runtime ingest works | 🔴 blocked-on-W3 | Calibration refresh job lives under `ops_scripts/calibration/` |
...
| W1b.P1 | Close `reason_codes` enum; add `coverage_score` to `c0_evidence_contract_types` | `agentic_core/L0_routing/types/routing_artifact_types.py`, `agentic_core/L3_orchestration/types/c0_evidence_contract_types.py` | Types-only change, 0 consumers today | 15k | 🔴 blocked |
| W2.P1 | Move thresholds (`similarity_threshold`, `DEFAULT_ABSTAIN_THRESHOLD`) to `config/routing_thresholds.yaml` | `agentic_core/runtime/contracts/abstain_contract.py`, `agentic_core/L4_state/utils/memory/semantic_cache_manager.py`, new YAML | Per-namespace override precedence | 12k | 🔴 blocked |
| W2.P2 | Calibrate initial thresholds from W0 harness output | `config/routing_thresholds.yaml` + calibration report in `docs/reports/` | Requires W0 harness run on real data | 10k | 🔴 blocked |
| W3.P1 | Wire `grounding_need_score` into R3 gate | `PathRouter.route_with_confidence`, new helper in `route_gates.py` | Back-compat fallback when score absent | 18k | 🔴 blocked |
| W3.P2 | Wire multi-signal R5 (5+ triggers with labeled reason codes) | `PathRouter.route_with_confidence`, `plan_abstain` extension | Must not double-fire | 18k | 🔴 blocked |
| W3.P3 | Wire per-namespace R1B threshold lookup into `check_d2_semantic_cache` | `route_gates.py`, `semantic_cache_manager.py` | Cache key namespace already plumbed | 12k | 🔴 blocked |
| W4.P1 | OTEL metrics: `r1_exact_hits`, `r1_semantic_hits`, `r5_fires_by_reason`, `r3_coverage_below_threshold` | `agentic_core/L6_observability/…` | OTEL MCP schema | 10k | 🔴 blocked |
| W4.P2 | Weekly calibration refresh job + drift alert | `ops_scripts/calibration/weekly_refresh.py` (new) | Cadence + reviewer assignment needs HITL | 10k | 🔴 blocked |
