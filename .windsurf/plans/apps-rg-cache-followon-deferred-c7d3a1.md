...
| W2 | W2.P1 | GitHub Actions workflow for scheduled R1B warm-up | ~6k | GitHub Actions available; warm-up script functional | Not Started | `.github/workflows/r1b-cache-warmup.yml` runs `warm_r1b_cache.py` on schedule + manual trigger; skips if `SEMANTIC_CACHE_D2_ENABLED != 1` |
| W3 | W3.P1 | Pair discovery from `route_registry.yaml` | ~8k | `route_registry.yaml` contains or can reference known company/role pairs | Not Started | `warm_r1b_cache.py --from-registry` reads pairs from app route registry rather than hardcoded TOP_PAIRS |
| W4 | W4.P1 | `cache.similarity_score` on R1B hit span | ~5k | R1B recall returns similarity score alongside payload | Not Started | `apps_rg.cache.r1b.check` span carries `cache.similarity_score` float attribute on hit path; miss path omits attribute |
...
| W2.P1 | GitHub Actions warm-up workflow | `.github/workflows/r1b-cache-warmup.yml` (new) | No scheduled pre-warm step exists; cold-start misses for common pairs on fresh deployments | ~6k | Not Started |
| W3.P1 | Route-registry pair discovery | `tools/apps_rg/warm_r1b_cache.py` | W3.P1 spec stated "reads route_registry.yaml for known pairs"; current impl uses hardcoded TOP_PAIRS; registry-driven pairs would auto-update as routes evolve | ~8k | Not Started |
| W4.P1 | Similarity score on R1B hit span | `apps_rg/__main__.py`, `apps_rg/cache/r1b_adapter.py` | `check_r1b_for_apps_rg` currently returns the payload dict or None; the raw similarity score from `SemanticCacheManager.recall` is not propagated back to the caller so `__main__.py` cannot set `cache.similarity_score` on the span | ~5k | Not Started |
...
### DS-2: No GitHub Actions workflow for scheduled R1B cache warm-up

**Source:** W3.P2 original spec — `.github/workflows/` mentioned but deferred  
**Detail:** The warm-up script (`tools/apps_rg/warm_r1b_cache.py`) is functional but has no scheduled trigger. Without automation, warm-up only runs when manually invoked. Common company/role pairs will still cold-start after deployments or cache resets.  
**Target wave:** W2.P1  
**Priority:** P3

---

### DS-3: Warm-up script uses hardcoded TOP_PAIRS instead of route_registry.yaml
...
### DS-4: R1B hit OTEL span missing `cache.similarity_score` attribute
...
- W2: Workflow triggers on schedule and `workflow_dispatch`; skips gracefully when disabled
- W3: `--from-registry` flag reads pairs from app config; TOP_PAIRS retained as fallback
- W4: `cache.similarity_score` visible on R1B hit spans in `otel_mcp`; miss spans unaffected
