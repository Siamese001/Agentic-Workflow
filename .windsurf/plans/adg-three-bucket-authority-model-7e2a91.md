...
| **W2 — Runtime hardening** | P2.1 runtime ingest, P2.2 trace evidence_refs, P2.3 runtime tests | Wire `tools/generate/generate_runtime_adg.py` outputs as bucket=`runtime` with proper edge_kinds (RUNTIME_OBSERVED, TOOL_CALL_RUNTIME, etc.) and evidence_refs (run_id, trace_id, span_id) | ~15,000 | Runtime ADG SQLite already exists; need to lift its edges into the unified authority schema | **Deferred** | Every runtime edge has run_id + trace_id; missing-trace edges classify as RISK_SIGNAL_ONLY; static edge cannot claim runtime |
| **W3 — Registry resolver** | P3.1 agent registry, P3.2 MCP/tool registry, P3.3 prompt slot registry, P3.4 registry tests | Implement registry resolvers that emit bucket=`registry` edges with registry_digest evidence | ~20,000 | apps_*/config/agent_specs.json + .windsurf/mcp_config.json + apps_*/config/*.yaml are the source registries | **Deferred** | Every registry edge has registry_digest; stale/missing → RISK_SIGNAL_ONLY; registry permission cannot imply runtime |
| **W4 — Consumer audit** | P4.1 inventory, P4.2 mode declarations, P4.3 mode-mismatch CI gate | Audit every ADG consumer (hotspots, chokepoints, blast radius, layer-boundary, forbidden-imports, bypass detection, coverage prioritization, refactor impact, governance proof, MV definitions, MCP ADG tools, CLI commands, docs generators) and add explicit proof_mode / risk_mode / inventory_mode declarations | ~30,000 | Consumer count: ~40-80 across `agentic_core/adg`, `tools/adg`, `tools/analysis`, `ops_scripts/ci`, `apps_*/integrations` | **Deferred** | Every consumer declares mode; CI gate fails on missing-mode; no consumer silently uses inventory_view as proof |
