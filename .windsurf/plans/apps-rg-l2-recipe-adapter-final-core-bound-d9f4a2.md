...
|------|--------|-------|------------|--------|
| W1 | L2 step adapters created | `apps_rg/l2_recipe/` | A | ~12K 🟢 |
| W2 | R4 runner accepts app_name, resolves recipe | `agentic_core/runtime/entrypoints/` | B | ~15K 🟢 |
| W3 | __main__.py is pure shim | `apps_rg/__main__.py` | C | ~5K 🟢 |
| W4 | Tests prove sovereignty | `tests/_apps_contract/` | D | ~15K 🟢 |
...
|----------|-------|---------------|-------------|-------------|--------|
| 1.1 | Create L2 step adapter classes | `apps_rg/l2_recipe/__init__.py`, `apps_rg/l2_recipe/steps.py` | No adapter pattern exists yet | ~8K | 🔲 TODO |
| 1.2 | Create recipe registration module | `apps_rg/l2_recipe/registry.py` | Registry metadata only, not resolution | ~4K | 🔲 TODO |
| 2.1 | Extend R4 runner API with app_name param | `agentic_core/runtime/entrypoints/integrated_r4_deterministic_pipeline_run.py` | Must preserve backward compat for tests | ~8K | 🔲 TODO |
| 2.2 | Add recipe resolver inside R4 runner | `agentic_core/runtime/entrypoints/integrated_r4_deterministic_pipeline_run.py` | Must map app_name→DAG→callable | ~7K | 🔲 TODO |
| 3.1 | Strip __main__.py to pure shim | `apps_rg/__main__.py` | Remove 140 lines of domain code | ~5K | 🔲 TODO |
| 4.1 | Register apps_rg DAG in core registry | `agentic_core/L3_orchestration/registry/static_dag_registry.py` | Must expose step callables | ~4K | 🔲 TODO |
| 5.1 | PA guard on LLM-backed steps | `apps_rg/l2_recipe/steps.py` | Fail-closed stub | ~3K | 🔲 TODO |
| 6.1 | Test: __main__ is thin shim | `tests/_apps_contract/test_apps_rg_main_is_thin_core_shim.py` | — | ~3K | 🔲 TODO |
| 6.2 | Test: core resolves recipe | `tests/_apps_contract/test_apps_rg_core_resolves_l2_recipe.py` | — | ~3K | 🔲 TODO |
| 6.3 | Test: cannot inject l2_callable | `tests/_apps_contract/test_apps_rg_cannot_inject_l2_callable.py` | — | ~3K | 🔲 TODO |
| 6.4 | Test: missing recipe fails closed | `tests/_apps_contract/test_apps_rg_missing_recipe_fails_closed.py` | — | ~3K | 🔲 TODO |
| 6.5 | Test: steps only via core recipe | `tests/_apps_contract/test_apps_rg_l2_steps_only_via_core_recipe.py` | — | ~3K | 🔲 TODO |
| 6.6 | Test: LLM step requires PA | `tests/_apps_contract/test_apps_rg_llm_step_requires_pa_artifact.py` | — | ~3K | 🔲 TODO |
...
- [ ] `apps_rg/__main__.py` zero hits for forbidden keywords
- [ ] R4 runner resolves recipe by `app_name="apps_rg"` without caller-supplied callable
- [ ] Missing recipe → fail closed (exit 1, no artifacts)
- [ ] Injecting l2_callable without _test_mode → fail closed
- [ ] L2 steps registered in `apps_rg/l2_recipe/steps.py`
- [ ] LLM-backed steps fail closed without PA artifact
- [ ] 35+ existing tests pass
- [ ] 6+ new tests pass
- [ ] `apps_rg` cannot produce résumé artifacts when recipe resolution is disabled
