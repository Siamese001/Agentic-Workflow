...
| W2 — Write gateway threading | P2.1, P2.2, P2.3 | Add `principal_chain` parameter to write_gateway interface + concrete L2/L4 implementations | 7,500 | Todo 🟡 | All write sites accept chain; default fallback to env chain for internal writes; tests green |
| W3 — LLM Gateway + MCP wrappers | P3.1, P3.2 | SovereignLLMGateway audit carries chain; MCP client_wrappers propagate chain header | 8,000 | Todo 🟡 | All external egress records chain in audit_log; MCP envelopes include chain field |
| W4 — L5 exit-control verification | P4.1, P4.2 | HITL exit-control reads `delegation_depth`, `permission_ladder_entry`; policy enforces band caps | 6,500 | Todo 🟡 | ADR-023 exit flow verifies chain per `capability_token.schema.md §7` contract |
| W5 — Audit emission + forensic reconstruction | P5.1 | safety_audit_emitter binds `invoking_user` to every emitted record; replay_envelope carries chain | 5,000 | Todo 🟡 | Every audit record reconstructable back to `invoking_user` per SAIF requirement |
| W6 — Regenerate ADG + verify no new P0 | P6.1 | Full ADG regeneration, confirm `v_p0_write_bypass_uwg` still empty, no new SC/AP defects | 2,500 | Todo 🟡 | Clean ADG run; no new P0 violations; burndown stable or improved |
