...
| **W5 — Registry W3-tail** | P5.1 route-contract resolver, P5.2 prompt-slot resolver or defer-doc, P5.3 lift end-to-end | Add `resolve_route_contracts` (reads `agentic_core/L0_routing/c0_retrieval/route_contract.py` registry); for prompt-slot, document deferral if no registry exists yet; run lift end-to-end to populate `bucket=registry` rows | ~6,000 | route_contract module exposes a discoverable registry; prompt-slot may not exist yet | **Done** | `resolve_route_contracts` reads `v15_policy_pack.json`; integrated into `resolve_all_registries`; 7/7 tests pass; prompt-slot deferred (no canonical manifest) |
...
- **Prompt-slot registry resolver** if no registry exists yet (DEFERRED_SCOPE marker emitted instead)
- **Full GenAI semconv coverage** (target: ≥80%, not 100% — long-tail spans are out of scope)
