...
| `.windsurf/rules/author-gate-enforcement.md` | Compact continuous-execution and Author-Gate core |
...
| `anti-pattern-author-gate.md` | A change may require a guardian exception |
| `approval-exception-policy.md` | Evaluating whether an exception is even eligible |
| `author-gate-decision-points.md` | Need to build a proper Author-Gate packet |
| `author-gate-svp-calibration.md` | Need to rank options and recommend one |
...
| `antipattern-author-gate.md` | Guardian exception or anti-pattern gate triggered |
| `author-gate-decision-gate.md` | Need a Author-Gate packet with options |
...
| "Should we stop for Author-Gate?" | `rules/author-gate-enforcement.md` then `rules/author-gate-decision-points.md` |
...
| "Can we add this exception?" | `rules/approval-exception-policy.md` + `rules/anti-pattern-author-gate.md` |
...
| `anti-pattern-author-gate.md` | `model_decision` | Use before introducing any new anti-pattern instance |
...
| `author-gate-decision-points.md` | `model_decision` | Use when a Author-Gate decision point is reached — full trigger patterns, option shape, scoring guidance |
...
- 2026-04-19: **REFACTOR DECISION MEMORY** — New rule `refactor-decision-memory.md` (model_decision). New skill `refactor-decision-memory/` with `lookup_refactor_decisions.py`. New hook `post_cascade_author_gate_capture.py` wired into `post_cascade_response`. SQLite+FTS5 ledger at `.windsurf/state/refactor_decisions/`. `author-gate-enforcement.md` unchanged — memory system sits under the policy layer. 30 unit tests added.
- 2026-04-15: **RULE SIZE REDUCTION** — `constitutional.md` 29K→3.3K (always_on core, 16 constraints + tier table). `author-gate-enforcement.md` 33K→2.7K (always_on pipeline + bypass + thresholds). New `author-gate-decision-points.md` (model_decision, 10 decision triggers + HITL-10 shape + telemetry). `sequential-thinking-enforcement.md` converted from always_on (6.9K) to model_decision (1.8K). `adg-test-accelerator-enforcement.md` description frontmatter added. `skills/graph-analysis/fail_closed_discipline.md` created. All 7 skill entry files confirmed as `SKILL.md`. Always_on rules: constitutional (3.3K), global_rules (3.0K), hitl-enforcement (2.7K), plan-location (1.9K).
