#!/usr/bin/env python3
"""post_cascade_mcp_preflight_audit.py — MCP destructive-call health preflight audit.

Companion to the preflight check inside ``pre_mcp_gate.py``. Runs on every
``post_cascade_response`` payload. Two responsibilities:
...
2. **Audit logger** — when the response contains a destructive MCP call
   ...
   ``artifacts/windsurf/mcp_preflight_violations.jsonl``.
...
``.windsurf/plans/mcp-destructive-gate-preflight-e9a14b.md`` W1 Phase P2.
"""
...
ttl_config = repo_root / ".windsurf" / "config" / "mcp_preflight_ttl.json"
...
# ---------------------------------------------------------------------------
# Regex primitives (reused shape from post_cascade_mcp_serialization_audit.py)
# ---------------------------------------------------------------------------
...
def main() -> int:
    ...
    if violations:
        ...
        print(
            ...
            f"See: artifacts/windsurf/mcp_preflight_violations.jsonl",
            file=sys.stderr,
        )
