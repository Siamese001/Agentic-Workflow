...
Merges:
- post_cascade_notion_plans_status_audit.py (post-flight status drift detection)
...
- Logs violations to artifacts/windsurf/notion_plans_status_violations.jsonl
...
violations_log = repo_root / "artifacts" / "windsurf" / "notion_plans_status_violations.jsonl"
...
def _detect_violations(response_text: str) -> list[dict[str, Any]]:
    """Detect all status violations in Cascade response."""
    violations = []
...
def main() -> int:
    ...
    Reads Cascade response from stdin, detects violations, logs them.
