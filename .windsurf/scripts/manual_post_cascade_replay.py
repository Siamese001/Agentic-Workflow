"""Manual replay of the post_cascade_response hook chain.

Workaround for Windsurf 2.0.67 bug where post_cascade_response hooks
...
-----
    ...
    python .windsurf/scripts/manual_post_cascade_replay.py --clipboard

    # From a file:
    python .windsurf/scripts/manual_post_cascade_replay.py --file response.txt

    # Dry-run (show what would run without invoking hooks):
    python .windsurf/scripts/manual_post_cascade_replay.py --file response.txt --dry-run

What it does
------------
Reads the response text, wraps it in the Windsurf payload shape
``{"tool_info": {"response": "..."}}``, and pipes that JSON through
every entry in ``hooks.post_cascade_response`` from ``.windsurf/hooks.json``.
Reports each hook's exit code + stderr summary so silent failures surface.

Why this exists
---------------
Windsurf's post_cascade_response hook dispatcher stops invoking hooks
...
Evidence: ``artifacts/windsurf/post_cascade_heartbeat.jsonl`` gap.
"""
...
def main() -> int:
    ...
    print(f"[manual_replay] replaying {len(hooks)} post_cascade_response hooks")
