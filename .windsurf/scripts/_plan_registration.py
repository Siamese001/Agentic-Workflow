...
Shared by:
  - post_cascade_plan_registration_capture.py (PLAN_CREATED marker capture)
  - pre_user_prompt_plan_registration_surface.py (proactive surface)
  - tools/windsurf/wave_execution_state.py (wave-start block)
...
State files:
  ...
  - .windsurf/state/plan_registration_cache.json
...
STATE_DIR = REPO_ROOT / ".windsurf" / "state"
...
def list_on_disk_plans() -> list[str]:
    """Return sorted list of plan slugs present under .windsurf/plans/*.md."""
...
def plan_file_path(slug: str) -> str:
    ...
    return f".windsurf/plans/{slug}.md"
