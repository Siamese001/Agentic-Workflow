...
def compute_metrics(window: Window) -> Metrics:
    ...
    for d in decisions:
        ...
        # precedent: context_fingerprint_json is a blob; we check for precedent fields
        # on the row if your schema extends it. For now infer from recommended_option_id.
        if d.get("recommended_option_id"):
            m.dec_with_precedent += 1  # proxy
