#!/usr/bin/env python3
"""
post_cascade_author_gate_schema_audit.py — AG-10 packet shape conformance audit.

Sibling of ``post_cascade_author_gate_ui_audit.py``. The UI audit checks that
...
parser ``post_cascade_author_gate_capture.py`` keys on (``decision_id``,
...
``artifacts/windsurf/author_gate_schema_violations.jsonl``.
...
# ---------------------------------------------------------------------------
# IO helpers (mirrored from post_cascade_author_gate_ui_audit.py for behavioral
        ...
        findings.append(
            {
                ...
                "observed": policy,
                "remediation": (
                    "Pipe spec to .windsurf/skills/author-gate-packet-builder/"
