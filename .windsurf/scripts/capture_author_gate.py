...
This script is an exact analog of ``.windsurf/scripts/defer.py`` for the
DEFERRED_SCOPE capture path: it wraps each marker in the Windsurf payload
shape and invokes ``post_cascade_author_gate_capture.py`` directly.

USAGE
-----
    # Single marker as CLI arg:
    python .windsurf/scripts/capture_author_gate.py "DECISION_CAPTURED: type=refactor_scope, repo_area=.windsurf, selected=W1, outcome=executed, confidence=0.92, gap=0.18"

    # From a file (one or more markers, one per line):
    python .windsurf/scripts/capture_author_gate.py --file markers.txt

    # From stdin:
    echo "DECISION_CAPTURED: ..." | python .windsurf/scripts/capture_author_gate.py --stdin
...
HOOK_SCRIPT = REPO_ROOT / ".windsurf" / "scripts" / "post_cascade_author_gate_capture.py"
...
def _invoke_hook(marker: str) -> tuple[int, str]:
    """Invoke the capture hook with ``marker`` wrapped in the Windsurf payload shape."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inline DECISION_CAPTURED bypass for Windsurf 2.0.67 hook-dark window."
    )
