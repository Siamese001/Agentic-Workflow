...
from schemas import UnifiedSemanticElement, CanonEntry, generate_ast_structure
...
def backfill_qdrant(
    ...
    if not code_files:
        logger.warning(f"No files found matching {file_pattern} in {source_dir}")
        return 0
...
def hydrate_redis(
    ...
    logger.info(f"Fetching top {max_patterns} golden patterns (success > {success_threshold})")
...
def run_etl_pipeline(
    ...
    # Backfill Qdrant
    if backfill:
        results["entries_backfilled"] = backfill_qdrant(source_dir, db_manager=db_manager)
