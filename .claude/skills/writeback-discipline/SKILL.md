...
| Modified `.mcp.json` or a gate | Notion MCP Registry | `templates/notion_row_shapes.md` §2 |
...
- Memory is for **Claude Code's next-5-minutes recall** — observations must be recall-actionable ("run `python X` with flag `-n 0`"), not diary prose ("fixed the bug today").
