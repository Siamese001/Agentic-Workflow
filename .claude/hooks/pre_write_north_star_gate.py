...
_OFF_STAR_PREFIXES = (
    ".claude/rules", ".claude/hooks", ".claude/governance", ".claude/skills",
    ".claude/agents", ".claude/settings", ".claude/plans", ".claude/commands",
...
def evaluate(data: dict, state: dict | None = None, env: dict | None = None) -> tuple[int, str]:
    ...
    if not file_path:
        return 0, ""
    # harness scratch under ~/.claude is never a repo edit
    try:
