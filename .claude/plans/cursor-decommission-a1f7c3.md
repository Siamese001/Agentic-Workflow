...
- **Complication.** `.cursor/` is *not* inert. The governance engine under
  `.cursor/scripts/**` (364 files) is invoked by `.claude/hooks/` via `subprocess` +
...
|---|---|---|---|
| `scripts/**` | 364 | **Live engine** (hooks call it) | `.claude/governance/scripts/` |
| `plans/**` | 2148 | **Live data** (plan SSOT) | `.claude/plans/` |
| `state/**` | 32 | **Live data** (ledgers, AG queue, caches) | `.claude/state/` |
| `schemas/**` | 56 | **Live data** (AG packet, ledger DDL) | `.claude/schemas/` |
| `templates/**` | 2 | **Live data** (plan template) | `.claude/templates/` |
| `rules/**` | 65 | **Dead mirror** of `.claude/rules/` (63) | delete |
| `skills/**` | 92 | **Dead mirror** of `.claude/skills/` (87) | delete |
...
| W1 | P1.1–P1.3 | Delete dead mirrors; retarget the few gates pointing at them | ~30k | `.cursor/{rules,skills,workflows,agents,mcp.json,hooks}` have zero runtime readers | Not Started | Dead mirrors gone; pre-commit globs repointed to `.claude/`; CI green |
| W2 | P2.1–P2.4 | Relocate `plans/**` → `.claude/plans/`; rewrite rule + refs + globs | ~70k | 2148 plan files move via `git mv` | Not Started | Plans resolve under `.claude/plans/`; plan gates pass; no `.cursor/plans` refs remain |
| W3 | P3.1–P3.3 | Relocate `state/**` (ledgers, AG queue, caches) | ~45k | `ledger_paths.py` is the path chokepoint | Not Started | Ledger writes/reads hit `.claude/state/`; AG capture + queue drain work; SQLite intact |
...
| W5 | P5.1–P5.4 | Relocate governance engine `scripts/**` → `.claude/governance/scripts/` | ~90k | Engine is self-contained + importable from new path | Not Started | `.claude/hooks` invoke new path; all post/pre hooks fire; governance tests pass |
| W6 | P6.1–P6.3 | Reference sweep; CLAUDE.md + rules rewrite; add anti-regression gate | ~50k | Comment-only refs acceptable to bulk-edit | Not Started | Zero `.cursor` refs in live trees; `check_no_cursor_refs.py` gate added + green |
...
| P0.2 | Create new homes (`.claude/{plans,state,schemas,templates,governance}/`) with `.gitkeep` | 5 new | — | ~4k | Not Started |
...
| P2.4 | Rewrite `.claude/hooks` plan-path refs | ~3 | — | ~10k | Not Started |
| P3.1 | `git mv .cursor/state .claude/state`; update `ledger_paths.py` chokepoint | ~32 + 1 | SQLite path; drift-mirror logic | ~16k | Not Started |
...
| P4.1 | `git mv .cursor/schemas .claude/schemas`; update AG packet builder + schema refs | ~10 | `author_gate_packet.schema.json` | ~14k | Not Started |
| P4.2 | `git mv .cursor/templates .claude/templates`; update template readers | ~3 | `execution-plan-template.md` | ~11k | Not Started |
| P5.1 | `git mv .cursor/scripts .claude/governance/scripts` (keep `_legacy_*` or archive) | 364 | Internal sibling imports (`_post_handlers`, `_author_gate_queue`) | ~30k | Not Started |
| P5.2 | Update `.claude/hooks/*.py` `SCRIPTS` const + `sys.path` inserts + `.claude/settings.json` direct call | ~7 | importlib spec loading | ~20k | Not Started |
...
| P6.2 | Rewrite `CLAUDE.md` + every `.claude/rules/*.md` mention of `.cursor` | ~30 | Rules cite `.cursor` paths | ~16k | Not Started |
...
- **P0.1** Add `tools/paths/cursor_compat.py` exporting `PLANS_DIR`, `STATE_DIR`,
  ...
  **new** `.claude/...` home with a **read-fallback** to the old `.cursor/...` path
...
`.claude/` SSOT (counts: 65/63, 92/87, 25/24, 4/4) with **no runtime readers** (the
...
Move `.cursor/scripts/**` → `.claude/governance/scripts/`. The engine has internal
...
`.claude/hooks/*.py` `SCRIPTS` constant + `sys.path` inserts + the `importlib` spec
path; the **direct** `.claude/settings.json` call to
...
Bulk-rewrite cosmetic `.cursor` mentions, rewrite `CLAUDE.md` and all `.claude/rules/*.md`
...
| 2 | `.claude/hooks` governance chain fires (submit/edit/stop) with engine at new path | Live hook smoke run; check `artifacts/cursor/*` (or new) audit JSONL updates |
| 3 | Refactor-decision ledger + AG queue read/write under `.claude/state/` | `python -m tools.capture.append_marker` round-trip; ledger freshness gate green |
| 4 | Plan governance resolves `.claude/plans/`; new plan registers correctly | `check_plan_*` gates green on a test plan |
...
| 8 | Smoke: governance engine entrypoint runs from new home | `python .claude/governance/scripts/pre_prompt_classifier.py --help` exits 0 |
...
- **Two-tier token budget gate** counts `.claude/rules` after W1 retarget — confirm
