...
| W2 | P2.1–P2.3 | Re-baseline + dead-legacy audit | ~12k | See re-baseline banner | ✅ Done (2026-06-07) | Re-scoped to reality: `_legacy_*` retained (live importers); a1f7c3 retired. No destructive action taken. ⚠️ **CORRECTED by P5.1 (2026-06-07):** the "`.cursor/` = dead pyc" claim was WRONG — `.cursor/state/` holds the LIVE Author-Gate ledger (`path_constants.CURSOR_STATE_DIR`, GH-workflow upload). W6 delete must become migrate-then-delete. |
...
1. **W2's "`.cursor/` = untracked dead `__pycache__/*.pyc` only" is WRONG.** `.cursor/` has **71 files** and `agentic_core/.../path_constants.py:189` defines `CURSOR_STATE_DIR=".cursor/state"`; the GitHub Actions Author-Gate workflow uploads `.cursor/state/refactor_decisions/refactor_decision_ledger.sqlite`. **`.cursor/state/` holds the live Author-Gate decision ledger + `plan_registration_cache.json` + `author_gate_queue/`.** The W6/P6.2 "delete dead `.cursor/`" step would **destroy live state** — it must become a *migrate-then-delete* (`.cursor/state/` → `.claude/state/`) with the core path edit + GH-workflow edit done atomically.
