...
**Unchanged obligations**: Author-Gate and `ask_user_question` when rules require; `WAVE_START` / `WAVE_COMPLETE` (and related lifecycle) when executing multi-wave plans; native plan mode before edits on T2/T3 per `plan-first-enforcement.md`, with `structured-reasoning` available as decomposition / retrieval guidance.
