...
36. **Plan–Notion registration mandatory.** Every new `plans/<slug>-<6hex>.md` (legacy: `.claude/plans/<slug>-<6hex>.md`) MUST emit `PLAN_CREATED:` marker AND post Plans DB row (Slug, Status, Exists On Disk, Plan File Path, Summary, AI Summary) before wave execution. `wave_execution_state.py start` blocks unregistered plans. Claude Code MUST NOT claim registration without a live `API-query-data-source` call same-response. Helper: `_plan_registration.py`. Pre-commit T7u: `check_plan_registration_freshness.py`. Detail: `plan-governance` skill + `plan-location.md`. Bypass: `PLAN_REGISTRATION_BYPASS=1`.
...
> Repointed W2 (plan `governance-rule-residue-cleanup-7e3a91`), then **deleted W3** (plan `enforcement-surface-consolidation-d8b3f6`): the deprecated ADG stubs (`adg-repair-discipline`, `adg-hotspot-enforcement`, `adg-graph-layer-enforcement`, `adg-test-accelerator-enforcement`, `adg-p7-analyst-artifacts`) plus the Author-Gate / deferred-scope / next-step / notion-plan / mcp-serialization stubs were removed. Signal lives at the canonical targets (`adg-analysis-procedures.md`, CLAUDE.md §Author-Gate, constitutional §6/§24/§36, `plan-governance` skill); full map in `docs/reports/governance/retired-rules-index.md`.
