...
30. **Author-Gate capture health — RETIRED (W1 claude-native-supersession-9d3f7a, ADR-093).** The `DECISION_CAPTURED:` marker → SQLite-ledger capture pipeline emulated a structured-choice tool; native `AskUserQuestion` supersedes it. Slot retained for stable numbering; no marker required.
...
32. **Fort Knox certification integrity — RETIRED (2026-06-14).** The bottom-up requirements-certification arm (`RTC-REQ-*` evidence assertions → `compile_requirement_signoff.py` → Merkle/signature signoff, hostile verifier, mutation-rejection, positive-control canary) is decommissioned: its `certification/evidence_assertions.jsonl` input no longer exists, the methodology was abandoned, and the canonical contract-gate runner never invoked it. The integrity *principle* it encoded — certification claims emerge only from a compiler, never hand-edited prose; no green theater — lives in §37 and `002-pass-blocked-proof-contract`. Slot retained for stable numbering; no gate, hook, or bypass required. (Runtime machinery under `agentic_core/L7_auditability/**` + `tools/cert/**` is a separate, deferred teardown — see `fortknox-certification-discipline.md`.)
...
35. **Author-Gate queue drain — RETIRED (W1 claude-native-supersession-9d3f7a, ADR-093).** The `AUTHOR_GATE_PACKET:` queue/seed/drain machinery is superseded by asking with native `AskUserQuestion` at the point of decision. Slot retained for stable numbering.
36. **Plan–Notion registration — RETIRED (notion-wave-enforcement-removal).** The windsurf/cursor-era plan→Notion registration pipeline (`PLAN_CREATED:` marker, `wave_execution_state.py start` chokepoint, `_plan_registration.py`, pre-commit T7u `check_plan_registration_freshness.py`, the NP-series Notion status gates) never functioned (advisory + token-gated, archived DBs) and is removed. **Plans are disk-only** — no Notion registration. The only plan gates are the disk-side format lints (`check_plan_format_compliance`, `check_plan_wave_summary_top`, `check_plan_definition_of_done`) + `plan-location.md` (SSOT location). Slot retained for stable numbering.
...
> Repointed W2 (plan `governance-rule-residue-cleanup-7e3a91`), then **deleted W3** (plan `enforcement-surface-consolidation-d8b3f6`): the deprecated ADG stubs (`adg-repair-discipline`, `adg-hotspot-enforcement`, `adg-graph-layer-enforcement`, `adg-test-accelerator-enforcement`, `adg-p7-analyst-artifacts`) plus the Author-Gate / deferred-scope / next-step / notion-plan / mcp-serialization stubs were removed. The Notion plan-status / wave-deferral / plan-registration / plan-lifecycle rules + the `plan-governance` and `notion` skills were further **removed (notion-wave-enforcement-removal)** — that enforcement never functioned; plans are disk-only. Signal lives at the canonical targets (`adg-analysis-procedures.md`, CLAUDE.md §Author-Gate, constitutional §6/§24, §36 retired, `plan-location.md`); full map in `docs/reports/governance/retired-rules-index.md`.
