...
<!-- Converted from `.claude/rules/author-gate-queue-drain.md`. Original Cursor trigger: `always_on`. -->
...
Capture hook `post_cursor_agent_ag_queue_seed_capture.py` writes markers to queue JSONL. Pre-commit gate `check_ag_queue_seed_markers.py` blocks plan commits where prose count > marker count (quoted examples excluded).
...
This rule + `_author_gate_queue.py` (SSOT helper) + `pre_user_prompt_ag_queue_surface.py` (emits `AG_QUEUE_PENDING:`) + `post_cursor_agent_ag_queue_drain_audit.py` (violations) + `post_cursor_agent_ag_queue_seed_capture.py` (marker → queue) + `check_ag_queue_seed_markers.py` (T7t pre-commit) + `check_ag_queue_drain_freshness.py` (weekly drift). §6, §24, §30, §35.
