#!/usr/bin/env python3
"""
post_agent_author_gate_miss_detector.py — Retroactive Author-Gate miss audit.

Hooks post_agent_response. Scans the response for signals that a genuine
...
Signals (any one is a candidate; count must exceed threshold to flag):
    ...
    - plan files created/modified under plans/ or legacy .claude/plans/
...
def _compute_miss_score(text: str) -> tuple[int, dict[str, Any]]:
    ...
    plan_hits = re.findall(r"(?:^|[\s\"'`(])(?:\.claude[/\\])?plans[/\\][^\s\"']+\.md", text)
    if plan_hits:
        score += 1
    ...
    # post_agent_ask_user_question_packet_audit hook (plan W4.P4.1) which
