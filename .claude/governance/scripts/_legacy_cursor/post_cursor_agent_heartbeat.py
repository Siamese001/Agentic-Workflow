"""Post-cursor-agent heartbeat writer (W7.1 observability gap).

Writes a single heartbeat record per Cursor Agent response to
``artifacts/cursor/post_cursor_agent_heartbeat.jsonl``.

The companion session-start script
``.claude/governance/scripts/pre_user_prompt_hook_health_check.py`` inspects the
latest heartbeat and warns the user when the prior session's
post_cursor_agent_response chain did not fire (indicating Windsurf skipped
...
_HEARTBEAT_PATH = _ARTIFACTS_DIR / "post_cursor_agent_heartbeat.jsonl"
...
def _previous_timestamp() -> float | None:
    ...
    the preceding Cursor Agent turn (including the full post-cursor-agent hook
...
def _record() -> dict[str, object]:
    ...
    return {
        ...
        "pid": os.getpid(),
        "hook": "post_cursor_agent_heartbeat",
...
def main() -> int:
    if os.getenv("POST_CURSOR_AGENT_HEARTBEAT_DISABLE") == "1":
        return 0
    try:
        ...
        print(
            f"[post_cursor_agent_heartbeat] WARN: could not write heartbeat: {exc}",
            file=sys.stderr,
        )
    return 0
