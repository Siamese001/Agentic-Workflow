"""Shared payload extractor for post_cascade_response hooks.

Windsurf delivers the cascade response text to hook stdin in this shape:
...
This helper is the SSOT extractor. All post_cascade_*.py hooks MUST call
``extract_response_text(sys.stdin.read())`` rather than rolling their own
parser. A CI gate (`ops_scripts/ci/check_post_cascade_payload.py`) blocks
any post_cascade_*.py that reimplements the logic.
"""
