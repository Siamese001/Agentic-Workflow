"""Manual replay of the post_cursor_agent_response hook chain.

Workaround for Windsurf 2.0.67 bug where post_cursor_agent_response hooks
silently stop firing mid-session (pre_user_prompt hooks continue to work).

Usage
-----
    # From stdin (paste Cursor Agent response text):
    python .claude/governance/scripts/manual_post_cursor_agent_replay.py < response.txt

    # From clipboard (Windows / pyperclip):
    python .claude/governance/scripts/manual_post_cursor_agent_replay.py --clipboard

    # From a file:
    python .claude/governance/scripts/manual_post_cursor_agent_replay.py --file response.txt

    # Dry-run (show what would run without invoking hooks):
    python .claude/governance/scripts/manual_post_cursor_agent_replay.py --file response.txt --dry-run

What it does
------------
Reads the response text, wraps it in the Windsurf payload shape
``{"tool_info": {"response": "..."}}``, and pipes that JSON through
every entry in ``hooks.post_cursor_agent_response`` from ``.cursor/hooks.json``.
Reports each hook's exit code + stderr summary so silent failures surface.

Why this exists
---------------
Windsurf's post_cursor_agent_response hook dispatcher stops invoking hooks
...
Evidence: ``artifacts/cursor/post_cursor_agent_heartbeat.jsonl`` gap.
"""
...
def main() -> int:
    ...
    print(f"[manual_replay] replaying {len(hooks)} post_cursor_agent_response hooks")
