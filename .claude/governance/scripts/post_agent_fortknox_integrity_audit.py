...
def _log_violations(rows: list[dict], repo: Path) -> None:
    ...
    try:
        ...
        print(f"[post_agent_fortknox_integrity_audit] WARN: log write failed: {exc}", file=sys.stderr)
...
def main() -> int:
    ...
    try:
        ...
        print(f"[post_agent_fortknox_integrity_audit] WARN: {exc!r} — fail-open", file=sys.stderr)
    return 0
