...
Bypass for Cursor 2.0.67 bug where `post_cursor_agent_response` hooks silently
...
-----
    ...
    python .claude/governance/scripts/defer.py "DEFERRED_SCOPE: plan=foo-abc123 wave=W1 phase=W1.A layer=L0 fan_in=5 surface=None coverage_gap_pct=10.0 est_tokens=100 reason=test"

    # From stdin (one or more markers, one per line):
    echo "DEFERRED_SCOPE: ..." | python .claude/governance/scripts/defer.py --stdin

    # From a file:
    python .claude/governance/scripts/defer.py --file markers.txt

Under the hood
--------------
Wraps each marker in the Cursor payload shape
`{"tool_info": {"response": "<marker>"}}` and invokes
`post_cursor_agent_deferred_scope_capture.py` — same code path the hook chain
...
HOOK_SCRIPT = REPO_ROOT / ".claude" / "governance/scripts" / "post_cursor_agent_deferred_scope_capture.py"
