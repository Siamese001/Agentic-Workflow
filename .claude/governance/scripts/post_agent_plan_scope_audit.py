"""
post_agent_plan_scope_audit.py — Post-cursor-agent hook for plan scope authorization audit.
...
W2_MODULE_PATH = REPO_ROOT / ".claude" / "governance/scripts" / "_legacy_windsurf" / "_plan_scope_expansion_check.py"
...
def detect_active_plan(response_text: str, file_paths: list[str]) -> str | None:
    ...
    2. Check file paths for .claude/plans/ references
...
def main() -> int:
    """Post-cursor-agent hook entry point.
    ...
    # Read response from stdin or env (Windsurf passes cursor agent response)
