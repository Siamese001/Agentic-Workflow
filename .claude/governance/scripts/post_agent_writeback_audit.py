#!/usr/bin/env python3
"""
post_agent_writeback_audit.py — post_agent_response MEMORY writeback discipline audit.

Reads the agent response payload from stdin. Detects signals indicating a **file-memory**
writeback was due, confirms whether a `WRITEBACK:` receipt line or a recent memory update
corroborates it, and (W4.3) emits one summarizing row to the `memory_recall` intelligence
ledger. Missed writebacks are appended to the advisory violations log.

> [notion-wave-enforcement-removal] The **Notion** writeback half (ADR / MCP / Author-Gate /
> Wave-Phase / SC-AP row detection) was stripped — that Notion plan-status writeback enforcement
> never functioned and is retired. Only the memory-side 15/3 advisory (constitutional §17) + the
> `memory_recall` ledger writer remain. Policy SSOT: .claude/rules/memory-management.md.

Detection signals (any one fires → memory writeback due):
    1. Response mentions creating/editing a `plans/*-<6hex>.md` plan file
    2. Response contains recurring-bug language (fix recipes, RCA, root cause, procedural pattern)
...
# [notion-wave-enforcement-removal] Notion-target signals stripped (ADR / MCP / Author-Gate /
# SC-AP / Wave-Phase). Only the memory-side signals remain.
SIGNALS: list[tuple[str, re.Pattern[str], str]] = [
    (
        "plan_created_or_modified",
        re.compile(r"(?:^|[/\\])plans[/\\][\w\-]+-[0-9a-f]{6}\.md", re.IGNORECASE),
        "memory:Project:*",
    ),
    (
...
def main() -> int:
    ...
    try:
        ...
        emit_ledger_event(
            ...
            repo_area=".claude/governance/scripts/post_agent_writeback_audit.py",
        )
