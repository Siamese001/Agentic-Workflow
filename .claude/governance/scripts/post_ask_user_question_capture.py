...
``.claude/hooks/after_ask_user_question.py`` (registered under ``PostToolUse`` matcher
``AskUserQuestion``).
...
``settings.json`` had no ``PostToolUse`` matcher for ``AskUserQuestion`` — so the user's actual
