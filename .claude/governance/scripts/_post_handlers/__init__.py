"""Post-Codex dispatcher handlers.
...
Wired in by `.claude/governance/scripts/post_agent_dispatch.py` when env
`POST_AGENT_DISPATCHER=1`. When unset, the existing standalone scripts
in `.claude/governance/scripts/post_agent_*.py` run unchanged.
"""
...
class ParsedResponse:
    """Single-pass parse of the Codex post-response payload.
    ...
    """Parsed JSON if legacy editor provided structured payload; None otherwise."""

    response_text: str = ""
    """The Codex response body (markers, prose, code blocks)."""
