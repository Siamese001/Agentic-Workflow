#!/usr/bin/env python3
"""
post_write_audit.py — Cursor post_write_code advisory audit hook (Phase 1.5).
...
def lint_mcp_config(file_path: str, edits: list[dict]) -> list[str]:
    """Run JSON-native lint checks on mcp.json writes. Returns list of findings."""
    findings: list[str] = []
    ...
    try:
        ...
        findings.append(f"mcp.json could not be parsed: {exc}")
        return findings
        ...
        for var_name, var_val in env.items():
            if isinstance(var_val, str) and _SHELL_ENV_VAR_RE.search(var_val):
                findings.append(
                    ...
                    f"'${{VAR:-default}}' — migrate to '${{env:VAR_NAME}}' (Cursor native).",
                )
    ...
    for edit in edits:
        ...
        if old and not new:
            findings.append("Risky edit: server block removed from mcp.json.")
        if new and "mcpServers" in new and old == "":
            findings.append("Risky edit: new server added to mcp.json.")
