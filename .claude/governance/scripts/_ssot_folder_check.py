...
def _check_scripts_root(rel: str) -> Violation | None:
    ...
    return Violation(
        ...
        message=(
            ...
            f"scripts/proof/. See .claude/rules/ssot-folder-enforcement.md."
        ),
    )
        ...
        if rel.endswith(".py"):
            ...
            return Violation(
                ...
                message=(
                    ...
                    f"{suggested}. See .claude/rules/ssot-folder-enforcement.md."
                ),
            )
    return None
...
def _check_hook_script_misroute(rel: str) -> Violation | None:
    """Hook-prefix file landing outside .claude/governance/scripts/."""
    if not rel.endswith(".py"):
        return None
    if rel.startswith(".claude/governance/scripts/"):
        return None
    filename = PurePosixPath(rel).name
