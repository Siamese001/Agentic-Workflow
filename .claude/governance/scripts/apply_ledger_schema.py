...
Reads canonical DDL from .claude/schemas/decision_ledger.schema.sql and applies it.
For existing tables, adds any missing columns via ALTER TABLE ADD COLUMN
(SQLite's only supported non-destructive evolution).

Usage:
    python .claude/governance/scripts/apply_ledger_schema.py           # apply
    python .claude/governance/scripts/apply_ledger_schema.py --check   # dry-run + diff report
    python .claude/governance/scripts/apply_ledger_schema.py --force   # recreate FTS index
