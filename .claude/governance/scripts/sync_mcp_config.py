...
def generate_agents_quick_reference() -> str:
    ...
    lines.append("")
    lines.append(
        "Per-server `SKILL.md` files under `.claude/skills/<name>/` are **redirect stubs**; "
        "procedural SSOT is [`mcp-integration`](.claude/skills/mcp-integration/SKILL.md) sections §1–§13."
    )
    lines.append("")
    return "\n".join(lines)
