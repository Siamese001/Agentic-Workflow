...
Fires on every Cursor `pre_write_code` event. Blocks (exit 2) when
...
Input: Cursor passes the candidate write target on stdin as JSON
...
def _load_payload() -> tuple[str | None, str | None]:
    """Return (path, content) from Cursor stdin JSON or argv fallback."""
