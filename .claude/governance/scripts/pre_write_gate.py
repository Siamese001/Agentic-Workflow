...
Blocks (exit 2) on:
  ...
  - Deletion of mcp.json (file_path ends with mcp.json, edits empty → DENY)

Warns (stderr, exit 0) on:
  - Risky mcp.json edits (server removal, transport change, env var change)
...
def check_task_exists(file_path: str) -> str | None:
    ...
    if ".claude/governance/scripts/" in file_path.replace("\\", "/"):
        return None
...
def check_mcp_config(file_path: str, edits: list[dict]) -> tuple[bool, list[str]]:
    """
    Returns (should_block, warning_messages) for mcp.json edits.
    ...
    if not edits:
        return True, ["mcp.json deletion is DENIED — MCP config is critical infrastructure."]
    ...
    for edit in edits:
        ...
        if old and not new:
            warnings.append(
                "mcp.json: server block removed — verify this is intentional.",
            )
        for pat in _RISKY_MCP_PATTERNS:
            if pat.search(new):
                warnings.append(
                    "mcp.json: risky edit detected (server/transport/env change) — review carefully.",
                )
                break
