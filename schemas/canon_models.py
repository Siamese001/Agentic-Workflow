...
class CanonEntry(BaseModel):
    ...
    # Primary identifier
    id: UUID = Field(default_factory=uuid4, description="Unique identifier for the canon entry")
    
    # Vector representation (768 dimensions for sentence-transformers)
    vector: List[float] = Field(..., min_items=768, max_items=768, description="768-dimensional vector embedding")
    
    # Content tracking
    ast_json: Dict = Field(..., description="Abstract Syntax Tree as JSON for structural validation")
    ast_hash: str = Field(..., min_length=32, max_length=64, description="SHA-256 hash of AST structure")
    
    # Meta-Learning metadata
    policy_key: str = Field(..., description="The specific Canon rule key triggered (e.g., 'canon:rule:12')")
    failure_count: int = Field(default=0, ge=0, description="Number of times this pattern caused validation failure")
    success_count: int = Field(default=0, ge=0, description="Number of successful validations")
    latency_ms: int = Field(default=0, ge=0, description="Time taken for L5 Agent to resolve the issue")
    last_validated: datetime = Field(default_factory=datetime.utcnow, description="Timestamp of last validation")
    project_tag: str = Field(default="default", description="Project identifier for cross-project knowledge transfer")
    ...
    def calculate_risk_score(cls, v, values):
        ...
        if total_runs > 0:
            ...
            risk = int(risk * (1 - success_rate))  # Reduce risk based on success rate
...
class CanonQuery(BaseModel):
    ...
    filter_safe_only: bool = Field(default=True, description="Only return safe patterns")
