...
def test_autonomous_confidence_scoring():
    ...
    try:
        ...
        for unknown, existing in medium_confidence_cases:
            confidence = healer._calculate_subfolder_confidence(unknown, existing)
            print(f"  '{unknown}' vs {existing}: confidence = {confidence:.2f}")
            assert 0.5 <= confidence < 0.8, f"Expected medium confidence for '{unknown}', got {confidence}"
...
def test_autonomous_decision_logic():
    ...
    try:
        ...
        result = healer._autonomous_void_violation_resolution(
            test_file, "test_zone", "utils", "Test violation",
            ["scripts", "tests"], True, [], []
        )
        ...
        result = healer._autonomous_void_violation_resolution(
            test_file, "test_zone", "test_helpers", "Test violation",
            ["test_data", "test_scripts"], True, [], []
        )
        ...
        result = healer._autonomous_void_violation_resolution(
            test_file, "test_zone", "test_data", "Test violation",
            ["test_scripts", "test_helpers"], True, [], []
        )
...
def test_no_user_prompts_in_autonomous_mode():
    ...
    try:
        ...
        result = healer._heal_void_violation(
            test_file, "VOID VIOLATION: Path not in sovereign territory", 
            True, [], []
        )
        ...
        assert "AUTONOMOUS" in action or "PREVIEW" in action, f"Expected autonomous decision, got: {action}"
        assert "SKIPPED" not in action or "PREVIEW" in action, f"Should not skip in autonomous mode: {action}"
