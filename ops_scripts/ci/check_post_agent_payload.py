...
This gate enforces:
  ...
  1. Every ``.codex/governance/scripts/post_agent_*.py`` hook (excluding helpers
...
_SCRIPTS = _ROOT / ".codex" / "governance" / "scripts"
...
def check_file(path: Path) -> list[str]:
    ...
    if has_broken and not has_tool_info:
        problems.append(
            ...
            f".codex/governance/scripts/_post_agent_payload.py)."
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if all_problems:
        ...
        print(
            ...
            f"    {_REQUIRED_IMPORT}\n"
            "See .codex/governance/scripts/_post_agent_payload.py for the SSOT."
        )
        return 1
