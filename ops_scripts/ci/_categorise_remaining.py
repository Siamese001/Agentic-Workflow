...
from agentic_core.L5_safety.config.structure_blueprint.ssot import (
    ENFORCED_TERRITORIES, SOVEREIGN_EXCLUDED_FOLDERS,
)
        ...
        for fname in files:
            ...
            for const, lit in CONSTS:
                ...
                for lineno, line in enumerate(lines, 1):
                    ...
                    # Categorise
                    if re.search(r"""\.(startswith|endswith)\s*\(""", line):
                        categories["B_startswith"].append(entry)
                    elif re.search(r"""['"]""" + re.escape(lit) + r"""['"]\s*:""", line):
                        categories["C_dict_key"].append(entry)
                    elif re.search(r""":\s*['"]""" + re.escape(lit) + r"""['"]""", line):
                    ...
                    elif re.search(r"""\[.*['"]""" + re.escape(lit) + r"""['"].*,""", line):
                        # mixed list — some consts already replaced, siblings not yet
