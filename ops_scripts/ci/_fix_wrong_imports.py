...
REPLACEMENTS: list[tuple[re.Pattern, str]] = [
    (
        re.compile(r"\bfrom\s+agentic_core\.L5_safety\.config\.structure_blueprint_config\s+import\b"),
        "from agentic_core.L5_safety.config.structure_blueprint import",
    ),
    (
        re.compile(r"\bimport\s+agentic_core\.L5_safety\.config\.structure_blueprint_config\b"),
        "import agentic_core.L5_safety.config.structure_blueprint",
    ),
    (
        re.compile(r"\bfrom\s+structure_blueprint_config\s+import\b"),
        "from agentic_core.L5_safety.config.structure_blueprint import",
    ),
    (
        re.compile(r"\bimport\s+structure_blueprint_config\b"),
        "import agentic_core.L5_safety.config.structure_blueprint",
    ),
]
