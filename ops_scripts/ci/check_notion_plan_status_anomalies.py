...
def _load_audit_logs() -> list[dict]:
    ...
    log_file = Path("artifacts/governance/plan_identity_verifications.jsonl")
    if log_file.exists():
    ...
    violation_file = Path("artifacts/governance/notion_plans_status_violations.jsonl")
...
def detect_anomalies(plans: list[dict], audit_logs: list[dict]) -> list[Anomaly]:
    ...
    Anomaly types:
    - COMPLETED_TO_LOWER_PRIORITY: Completed plan marked Lower Priority within 24h
