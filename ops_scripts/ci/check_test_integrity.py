        ...
        if isinstance(child, ast.Call):
            ...
            if isinstance(func, ast.Attribute) and func.attr == "raises":
                if isinstance(func.value, ast.Attribute):
                    if (
                        func.value.attr == "mark" or func.value.id
    ...
    for node in ast.walk(func):
        ...
        for handler in getattr(node, "handlers", []):
            ...
            if not has_raise and not has_pytest_fail and not _has_guardian_allow(handler, source_lines):
                violations.append(
                    (handler.lineno, "except block swallows exception silently (no raise/pytest.fail)")
                )
    return violations
...
def _xfail_violations(
    ...
    for dec in func.decorator_list:
        ...
        if not isinstance(dec, ast.Call):
            # Bare @pytest.mark.xfail — no strict possible
            if not _has_guardian_allow(dec, source_lines):
                violations.append(
                    (
                        dec.lineno,
                        "@pytest.mark.xfail without strict=True (Section 7.2 requires strict=True, reason='linked_issue: #N')",
                    )
                )
            continue
...
def _infra_skip_violations(
    ...
    for node in ast.walk(func):
        ...
        ) or (isinstance(fn, ast.Name) and fn.id == "pytest.skip")
        if not is_skip:
            continue
        reason = ""
        ...
        if any(kw in reason for kw in _INFRA_SKIP_KEYWORDS) and not _has_guardian_allow(node, source_lines):
            violations.append(
                (
                    node.lineno,
                    f"pytest.skip() with infrastructure reason '{reason[:60]}' — use degraded-path test instead",
                )
            )
    return violations
...
def scan_file(filepath: Path) -> list[tuple[int, str]]:
    ...
    for node in ast.walk(tree):
        ...
        # Check 1: assertion-less test
        if not _contains_assert_or_raises(node):
            violations.append(
                (node.lineno, f"test function '{node.name}' has zero assert/pytest.raises statements")
            )

        # Check 2: silent except blocks
        violations.extend(_silent_except_in_test(node, source_lines))

        # Check 3: xfail without strict
        violations.extend(_xfail_violations(node, source_lines))

        # Check 4: infrastructure skips
