...
class GenerationRoutingScanner(ast.NodeVisitor):
            ...
            if any(module.startswith(forbidden) for forbidden in FORBIDDEN_IMPORTS):
                self.violations.append(
                    RoutingViolation(
                        ...
                        node.lineno,
                        "FORBIDDEN_IMPORT",
                        f"Import of '{module}' not allowed outside allowlist modules"
                    )
                )
            ...
            if any(node.module.startswith(forbidden) for forbidden in FORBIDDEN_IMPORTS):
                self.violations.append(
                    RoutingViolation(
                        ...
                        node.lineno,
                        "FORBIDDEN_IMPORT",
                        f"Import from '{node.module}' not allowed outside allowlist modules"
                    )
                )
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        # Check for direct client instantiation
            ...
            if func_name in {"OpenAI", "Anthropic", "VertexAI"}:
                self.violations.append(
                    RoutingViolation(
                        ...
                        f"Direct instantiation of '{func_name}' not allowed"
                    )
                )
            ...
            if node.func.attr in {"ChatCompletion", "Messages", "GenerativeModel"}:
                self.violations.append(
                    RoutingViolation(
                        ...
                        node.lineno,
                        "DIRECT_API_USAGE",
                        f"Direct usage of '{node.func.attr}' API not allowed"
                    )
                )
        
        self.generic_visit(node)
    
    def visit_Str(self, node: ast.Str) -> Any:  # Python < 3.8
    ...
    def _check_model_literal(self, value: str, line: int):
        ...
        model_patterns = {
            ...
            "gemini", "gemini-pro", "gemini-3",
            "text-embedding-3", "text-davinci", "text-curie",
        }
        
        value_lower = value.lower()
        if any(pattern in value_lower for pattern in model_patterns):
            # Check if this is in an allowed config file
            if not any(allowed in self.file_path for allowed in ALLOWED_MODEL_CONFIGS):
                self.violations.append(
                    RoutingViolation(
                        ...
                        line,
                        "MODEL_LITERAL",
                        f"Model string literal '{value}' not allowed outside config files"
                    )
                )
