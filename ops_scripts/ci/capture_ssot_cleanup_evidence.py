...
def main():
    ...
    # 3. git show --name-only --stat HEAD
    ...
    # 4. git show HEAD -- docs/evidence/phase_SSOT_cleanup_remediation_dispatcher_docstrings.md
    rc, stdout, stderr = run_git_command(
        ["show", "HEAD", "--", "docs/evidence/phase_SSOT_cleanup_remediation_dispatcher_docstrings.md"]
    )
    check_for_shell_injection(stdout + stderr)
    ...
    # 2. pre-commit run --all-files (capture but don't fail)
    ...
    # 3. git status --porcelain (after pre-commit)
    rc, stdout, stderr = run_git_command(["status", "--porcelain"])
    check_for_shell_injection(stdout + stderr)
    ...
    # If pre-commit modified files, restore them
    if stdout.strip():
