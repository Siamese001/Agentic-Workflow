        ...
        for md_file in category_dir.rglob("*.md"):
            ...
            for header in headers:
                ...
                if header in rule_headers and rule_headers[header] != md_file:
                    issues.append(
                        f"Duplicate header '{header}' in {md_file} and {rule_headers[header]}",
                    )
                else:
...
def main():
    ...
    if not WINDSURF_DIR.exists():
        print("[X] .windsurf directory not found")
        sys.exit(1)
    ...
    if issues:
        ...
        print("  [OK] No broken references to archived files")
