...
BAD_PATTERNS = [re.compile('\\w+\\["' + '(?:tests|tools|reports|archives|agentic_core|apps_\\w+|system_learning|ops_scripts)' + '"\\]'), re.compile('\\.get\\s*\\(\\s*["\\\'](?:tests|tools|reports|archives|agentic_core|apps_\\w+)\\s*["\\\']'), re.compile('in\\s+(?:kwargs|kw|options|config|params|settings)\\b'), re.compile('def\\s+\\w+.*=\\s*["\\\']'), re.compile('(?:e\\.g\\.|i\\.e\\.|#\\s|""".*|\\\'\\\'\\\'.*|Example:)'), re.compile('"(?:territory|domain|root|name|type|key|group|label|tag)":\\s*"'), re.compile('\\.(startswith|endswith)\\s*\\(\\s*["\\\']'), re.compile('(?:print|log(?:ger)?\\.(?:info|debug|warning|error|critical))\\s*\\(')]
good: list[dict] = []
bad: list[dict] = []
