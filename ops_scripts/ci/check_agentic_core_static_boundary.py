...
Wrapper for tests/governance/test_agentic_core_static_boundary.py
...
BYPASS_VAR = "GOV_BOUNDARY_BYPASS"
...
def run_governance_test() -> tuple[int, str, str]:
    ...
    test_path = repo_root / "tests" / "governance" / "test_agentic_core_static_boundary.py"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="GOV-2: Check agentic_core static boundary"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
    ...
    print(f"[GOV-2] Agentic core static boundary")
    print(f"[GOV-2] Mode: {mode_reason}")
    ...
    # Determine outcome
    if test_rc == 0:
        print(f"[GOV-2] ✅ PASS: Agentic core static boundary respected")
        return 0
    
    # Test found violations or had errors
    if test_rc == 2 or "AssertionError" in stderr or "Error" in stderr:
        if is_strict:
            print(f"[GOV-2] ❌ FAIL: Violations or errors found in strict mode")
            if stdout:
                print(f"[GOV-2] STDOUT:\n{stdout}")
            if stderr:
                print(f"[GOV-2] STDERR:\n{stderr}", file=sys.stderr)
            return 2
        else:
            print(f"[GOV-2] ⚠️  WARN: Violations or errors found ({mode_reason})")
            print(f"[GOV-2] Reason: Boundary violations or test execution failed")
            if stdout:
                print(f"[GOV-2] Details:\n{stdout}")
            return 0  # Advisory = non-blocking
    
    # Test failures
    if is_strict:
        print(f"[GOV-2] ❌ FAIL: Test failures in strict mode")
        if stdout:
            print(f"[GOV-2] STDOUT:\n{stdout}")
        if stderr:
            print(f"[GOV-2] STDERR:\n{stderr}", file=sys.stderr)
        ...
        print(f"[GOV-2] ⚠️  WARN: Test failures ({mode_reason})")
        if stdout:
            print(f"[GOV-2] Details:\n{stdout}")
        return 0  # Advisory = non-blocking
