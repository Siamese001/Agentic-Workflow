...
def normalize_import_block(src: str, used_constants: set[str]) -> str:
    ...
    if not m:
        if not used_constants:
            return src
        new_block = 'from agentic_core.L0_routing.config.path_constants import (\n    ' + ',\n    '.join(sorted(used_constants)) + ',\n)'
...
def audit(fp: Path) -> list[str]:
    ...
    m = re.search('from agentic_core\\.L0_routing\\.config\\.path_constants import \\(([^)]*)\\)', src, re.DOTALL)
