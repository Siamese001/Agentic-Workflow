...
# Load targeting manifest v3 (SSOT)
...
def check_shim(rel_path: str) -> None:
    ...
    for node in ast.iter_child_nodes(tree):
        ...
        # 3. No FunctionDef / AsyncFunctionDef
        ...
        # 4b. No conditionals (except `if __name__`)
        if isinstance(node, ast.If):
            # Allow `if __name__ == "__main__"` guard
            test = node.test
            is_name_guard = (
        ...
        # 4c. No bare expression calls (import-time side effects)
        if isinstance(node, ast.Expr) and isinstance(node.value, ast.Call):
            fail(f"{rel_path}: contains import-time function call (shim must not have side effects)")

    # 3. Import scope: only from canonical executor module
    imports_from = []
    ...
    # Shim should import from exactly one canonical executor module
    # Allow __future__ and typing imports
