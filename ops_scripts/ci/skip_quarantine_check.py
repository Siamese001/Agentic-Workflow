...
def main() -> int:
    ...
    # Check critical test files have 0 skips/xfails
    ...
    if total_skips > SKIP_CEILING:
        violations.append(
            f"Total skip count {total_skips} exceeds ceiling {SKIP_CEILING}",
        )
    ...
    for rel, count in files_with_skips:
        if rel not in known_files:
            violations.append(
                f"{rel} has {count} skip(s) but is not in KNOWN_FAILING_TESTS.md or QUARANTINE_MANIFEST.json",
            )

    # Quarantine manifest ceiling check
    manifest_path = project_root / QUARANTINE_MANIFEST
    if manifest_path.is_file():
        ...
        if manifest_total > manifest_ceiling:
            violations.append(
                f"Quarantine manifest has {manifest_total} entries, "
                f"exceeds manifest ceiling {manifest_ceiling}",
            )
        if manifest_total > QUARANTINE_CEILING:
            violations.append(
                f"Quarantine entries {manifest_total} exceeds gate ceiling {QUARANTINE_CEILING} "
                f"(requires QUARANTINE_CEILING_BUMP:<reason> commit tag)",
            )
    else:
        manifest_total = 0
        manifest_ceiling = 0
