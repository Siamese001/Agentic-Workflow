...
def _collect_violations(source: str) -> list[tuple[int, int, str, list[str]]]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Assign, ast.AugAssign)):
            ...
            # Find end line by brace depth tracking
            brace_depth = 0
            end_line = start_line
...
def _replacement_line(line: str, needed: list[str]) -> str:
    ...
    # Detect if original had type annotation e.g. `VAR: frozenset[str] =`
    ann_match = re.match(r"^(\s*[A-Za-z_][A-Za-z0-9_]*\s*:[^=]+=)", line)
    if ann_match:
...
def fix_file(path: pathlib.Path) -> tuple[bool, list[str]]:
    ...
    # Sort violations in reverse order so line number replacements stay valid
    # Collect all needed SSOT names
    ...
    # Re-index after import insertion (added one block = multiple lines)
    # We need to recompute violations on the modified source
