...
SINGLE_IMPORT_RE = re.compile(
    r"from agentic_core\.L0_routing\.config\.path_constants import ([A-Z_]+(?:,\s*[A-Z_]+)*)\n"
)
...
def fix_syntax_error(src: str) -> str:
    ...
    """
    # Pattern: injected block sits between opening paren and first symbol of another import
    injected = re.compile(
        r"(from \S+ import \()\s*\n"
    ...
    # Remove the injected block from inside the other import
    src = src[: match.start(2)] + "\n" + src[match.end(2) :]

    # Now add the injected block after the last import statement
    # Find end of the now-repaired original import block
...
def get_used_constants(src: str) -> set[str]:
    ...
    for const in ALL_CONSTANTS:
        if re.search(r"\b" + re.escape(const) + r"\b", src):
            used.add(const)
    return used
...
def get_imported_constants(src: str) -> set[str]:
    ...
    if m:
        imported.update(re.findall(r"\b([A-Z_]{3,})\b", m.group(1)))
...
def add_missing_to_import(src: str, missing: set[str]) -> str:
    ...
    if m:
        body = m.group(1)
        existing = set(re.findall(r"\b([A-Z_]{3,})\b", body))
        truly_missing = missing - existing
        if not truly_missing:
            return src
        # Append to block
        sorted_missing = sorted(truly_missing)
        # Strip trailing whitespace/comma from body
    ...
    # No existing block — insert a new one after the last import
...
def repair_file(filepath: Path) -> tuple[bool, str]:
    ...
    # Step 1: fix syntax errors from malformed insertion
    try:
        ast.parse(src)
    except SyntaxError:
    ...
    # Step 2: ensure all used constants are in the import
...
def main():
    problem_files = [
        ...
        "tests/governance/test_ssot_structure_validation_enforcer.py",
        "tests/guardian/test_activation_gate.py",
        "tests/guardian/test_deterministic_loop_detector.py",
        "tests/guardian/test_execute_ssot_v15_contract.py",
        "tests/guardian/test_v15_p10_2_policy_pack.py",
        "tests/guardian/test_v15_p8_cat_c.py",
        "tests/guardian/test_v15_p8_cat_d.py",
        "tests/guardian/test_v15_p8_cat_e.py",
        "tests/integration_full_deps/test_seed_pack_full_build_b5.py",
        "tests/unit/agentic_core/L0_routing/scripts/test_execute_ssot_contracts.py",
        "tests/unit/agentic_core/L0_routing/scripts/test_healer_naming_convention.py",
        "tests/unit/agentic_core/L2_execution/healers/test_healing_provider_adapters.py",
        "tests/unit/agentic_core/L3_orchestration/workflow_engines/test_dag_runtime_inspector_agent.py",
        "tests/unit/agentic_core/L5_safety/reasoning/test_depth_pipeline_execute_ssot.py",
        "tests/unit/test_depth_violation_no_archive_invariant.py",
        "tests/unit/test_integration_config.py",
        "tests/unit/test_l1_purity_enforcement.py",
        "tests/unit/test_l3_orchestration_agent_inventory_contract.py",
        "tests/unit/test_l4_state_agent_inventory_contract.py",
        "tests/unit/test_l6_agent_inventory_contract.py",
        "tests/unit/test_meta_learning_bridge.py",
        "tests/unit/test_nested_lcd_detection_hook.py",
        "tests/unit/test_phase4_ml_end_to_end_envelope.py",
        "tests/unit/test_phase6_retrieval_snapshot.py",
        "tests/unit/test_wave2_gravity_exclusion.py",
        "tests/unit/test_wave5_longpaths_guard.py",
        "tests/unit/test_wave6_hitl_gates.py",
        ...
        "tests/unit_min_deps/test_formal_verification.py",
        "tests/unit_min_deps/test_import_graph_contract.py",
        "tests/unit_min_deps/test_proposal_capture.py",
        "tests/unit_min_deps/test_ssot_mutation_fence.py",
        "tests/unit_min_deps/test_static_checks.py",
