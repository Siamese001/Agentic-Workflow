"""CI gate: every post_cursor_agent_*.py hook must use the shared payload extractor.

The 2026-04-23 RCA found two hooks (`post_cursor_agent_deferred_scope_capture.py`
and `post_cursor_agent_writeback_audit.py`) silently dropping DEFERRED_SCOPE
...
_NO_STDIN_HOOKS: frozenset[str] = frozenset(
    {
        "post_cursor_agent_heartbeat.py",
        "post_cursor_agent_cleanup.py",
        "post_cursor_agent_hitl_capture.py",  # thin shim, no payload reading
    }
)

_REQUIRED_IMPORT = "from _post_cursor_agent_payload import extract_response_text"
...
def _should_check(path: Path) -> bool:
    name = path.name
    if not name.startswith("post_cursor_agent_"):
        return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    targets = sorted(p for p in _SCRIPTS.glob("post_cursor_agent_*.py") if _should_check(p))
    if args.verbose:
        print(f"[check_post_cursor_agent_payload] scanning {len(targets)} hook(s)")
        for t in targets:
            print(f"  - {t.name}")
    ...
    if all_problems:
        print("\n[check_post_cursor_agent_payload] VIOLATIONS:")
    ...
    if args.verbose:
        print("[check_post_cursor_agent_payload] OK")
    return 0
