...
def collect_evidence(repo_root: Path, synthetic_fail: bool = False) -> dict:
    ...
    for ep in inventory["entrypoints"]:
            ...
            entries.append(
                {
                    ...
                    "evidence": {
                        "path": ep["path"],
                        "note": "Pre-existing V15 enforcement (Phase 1 artifact)",
                    },
                },
            )
            continue
        ...
        # Synthetic fail mode: force first non-enforced entry to appear unwired
    ...
    return {
        ...
        "entrypoints_total": len(inventory["entrypoints"]),
        "wired_count": wired,
        "unwired_count": unwired,
        "already_enforced_count": already_enforced,
        "unwired_ids": unwired_ids,
...
def main() -> int:
    ...
    # Print summary
    print(
        f"P2 Evidence: {evidence['wired_count']} WIRED, "
        f"{evidence['already_enforced_count']} ALREADY_ENFORCED, "
        f"{evidence['unwired_count']} UNWIRED "
        f"(total={evidence['entrypoints_total']})",
    )
