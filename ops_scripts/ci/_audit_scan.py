...
# ── SSOT exclusion sets ─────────────────────────────────────────────────────
# Import from the canonical SSOT — never hardcode directory names here.
try:
    ...
    raise RuntimeError(
        "Cannot import SSOT exclusion constants. "
        "Ensure agentic_core is importable before running this script."
    ) from _e

# Union of all SSOT exclusion sets — used by _excluded() to prune rglob walks
EXCLUDE_DIRS: frozenset[str] = (
    GLOBAL_EXCLUDED_DIRS | SOVEREIGN_EXCLUDED_FOLDERS | DISCOVERY_EXCLUDED_TERRITORIES
)

# ── Approved-skip allowlist ─────────────────────────────────────────────────
# Format: relative path prefix (using /) -> human-readable rationale
APPROVED_SKIP_PATTERNS: dict[str, str] = {
    ...
    "tests/_config/import_strict_mode.py": (
        "controlled import-strict ramp: skip is documented degraded-mode fallback"
    ),
    "tests/performance/test_adg_runtime_acceleration.py": (
        "performance gate: env-gated skip for slow/GPU-only environments"
    ),
    "tests/integration_full_deps/": (
        "integration_full_deps: requires live external packages not present in unit CI"
    ),
    "tests/sovereign_hardening/test_ssot_pipeline_protocol.py": (
        "sovereign_hardening: environment-gated skip for missing optional infra"
    ),
    "tests/unit/test_ai_checking_ai_hardenings.py": (
        "ai_checking hardenings: skips when AI service is unavailable in unit CI"
    ),
    "tests/hardening/test_all_gap_fixes.py": (
        "gap-fix hardening: skipif guards for missing optional dependencies"
    ),
    "tests/unit/test_sovereign_seal_state.py": (
        "sovereign seal: env-gated skipif for missing crypto dependency"
    ),
    "tests/integration_full_deps/test_seed_pack_full_build_b5.py": (
        "full-build integration: skipif for missing full-stack dependencies"
    ),
}
...
def scan_xfail(out: list[str]) -> int:
    ...
    pat = re.compile(r"@pytest\.mark\.xfail")
    count = 0
    for f in sorted(TESTS_DIR.rglob("*.py")):
        ...
        for i, line in enumerate(lines):
            if not pat.search(line):
                continue
            # Collect multi-line decorator block (up to 8 lines)
            block = ""
...
def scan_skips(out: list[str]) -> tuple[int, int]:
    ...
    skip_call = re.compile(r"\bpytest\.skip\s*\(")
    skipif_mark = re.compile(r"@pytest\.mark\.skipif\b")
    importorskip = re.compile(r"\bpytest\.importorskip\b")
    violations = 0
    approved = 0
    for f in sorted(TESTS_DIR.rglob("*.py")):
        if _excluded(f):
...
def scan_conftest(out: list[str]) -> int:
    ...
    skip_pat = re.compile(r"(pytest\.skip|skipif|importorskip)")
    violations = 0
    # Scope to TESTS_DIR only — prevents traversal into archives/ and repo root
...
APPROVED_CI_SUPPRESSIONS: frozenset[str] = frozenset(
    {
        # Redis integration test — requires live Redis, excluded from unit CI
        "--ignore=tests/integration/test_redis_integration.py",
        # MRO import test — excluded while import-strict rollout is in progress
        "--ignore=tests/integration/test_imports_no_mro_error.py",
        # Evaluation suite — pending infra; not yet part of CI gate
...
def scan_ci_config(out: list[str]) -> int:
    ...
    danger = re.compile(
        r"(-m\s+[\"'`]?not\s+|--ignore=tests|--deselect|addopts.*-k\s|norecursedirs.*tests)",
        re.IGNORECASE,
    )
    count = 0
    ...
    for cf in config_files:
        ...
        for i, line in enumerate(lines):
            if not danger.search(line):
                continue
            stripped = line.strip()
            # Check if this line matches an approved suppression
