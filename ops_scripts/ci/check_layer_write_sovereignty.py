...
# Files that legitimately perform writes in sovereign layers:
# - enforcement wrappers that implement controlled write prohibition (detect & block, not execute)
# - one-shot migration/utility scripts under L0_routing/scripts (not runtime paths)
# - meta_apply is the one controlled write gate (intentional)
_ALLOWED_WRITE_PATH_SEGMENTS = frozenset(
    {
        "scripts",
        "meta_control",
        "reasoning",
...
# Only block filesystem persistence methods — subprocess.run is NOT a write operation
BLOCKED_ATTRS = {
    "write_text",
    ...
    "remove",
    "rename",
    "unlink",  # os filesystem mutations
}
