...
# Patterns that indicate silent state mutation
FORBIDDEN_GLOBAL_PATTERNS = [
    "os.environ[",  # direct environ dict mutation
    "os.putenv(",  # C-level env mutation
    "globals()[",  # global dict mutation
]
...
class P3EvidenceCollector:
    ...
    def _check_forbidden_mutation_patterns(self):
        ...
        for rel_path in ENFORCEMENT_FILES:
            ...
            for i, line in enumerate(lines, 1):
                stripped = line.strip()
                # Skip comments and guardian-allowed lines
                ...
                for pattern in FORBIDDEN_GLOBAL_PATTERNS:
                    if pattern in stripped:
                        self.violations.append(
                            {
                                "check": "forbidden_mutation_pattern",
                                "file": rel_path,
                                "line": i,
                                "pattern": pattern,
                ...
                self.checks_passed.append(
                    {
                        "check": "forbidden_mutation_pattern",
                        "file": rel_path,
                        "detail": "No forbidden mutation patterns found",
                    },
                )

    def _check_semantic_clock_wiring(self):
        """Verify SemanticClock is imported and tick() is called in gateway."""
        gw_path = self.repo_root / "agentic_core/L0_maintenance/enforcement/v15_execution_gateway.py"
    ...
    def _check_state_hash_fn_present(self):
        ...
        if has_state_hash and "state_hash_fn=state_hash_fn" in content:
            self.checks_passed.append(
                {
                    "check": "state_hash_fn_present",
                    "file": "SovereignBaseAgent.py",
                    "detail": "state_hash_fn passed to gateway.execute()",
                },
            )
        else:
            self.violations.append(
                {
                    "check": "state_hash_fn_present",
                    "file": "SovereignBaseAgent.py",
                    "detail": "state_hash_fn not properly wired",
                },
            )
