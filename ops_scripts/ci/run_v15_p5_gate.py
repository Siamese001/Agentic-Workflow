...
# Required fields for PolicyExceptionArtifact (§3.7)
POLICY_EXCEPTION_REQUIRED_FIELDS = [
    "trace_id",
    "nonce",
    "exception_scope",
]
...
class P5EvidenceCollector:
    ...
    def _check_signed_modify_type(self):
        ...
        if not p5_path.exists():
            self.violations.append(
                {
                    "check": "signed_modify_type",
                    "detail": "crypto_trust_types.py not found",
                },
            )
            return
        ...
        if has_class and not missing_fields:
            self.checks_passed.append(
                {
                    "check": "signed_modify_type",
                    "detail": f"SignedModify found with all {len(SIGNED_MODIFY_REQUIRED_FIELDS)} required fields",
                },
            )
        else:
            self.violations.append(
                {
                    "check": "signed_modify_type",
                    "detail": f"class={has_class}, missing_fields={missing_fields}",
                },
            )
    ...
    def _check_ensure_v15_signed(self):
        ...
        if not gc_path.exists():
            self.violations.append(
                {
                    "check": "ensure_v15_signed",
                    "detail": "guardian_contract.py not found",
                },
            )
            return
        ...
        if "ensure_v15_signed" in content:
            self.checks_passed.append(
                {
                    "check": "ensure_v15_signed",
                    "detail": "ensure_v15_signed found in guardian_contract",
                },
            )
        else:
            self.violations.append(
                {
                    "check": "ensure_v15_signed",
                    "detail": "ensure_v15_signed not found in guardian_contract",
                },
            )
    ...
    def _check_policy_exception_artifact(self):
        ...
        if not p3_path.exists():
            self.violations.append(
                {
                    "check": "policy_exception_artifact",
                    "detail": "governance_types.py not found",
                },
            )
            return
        ...
        if has_class and not missing_fields:
            self.checks_passed.append(
                {
                    "check": "policy_exception_artifact",
                    "detail": "PolicyExceptionArtifact found with required fields",
                },
            )
        else:
            self.violations.append(
    ...
    def _check_authority_immutability(self):
        ...
        content = p5_path.read_text(encoding="utf-8")
        # Count frozen dataclass decorators vs total dataclass decorators
        ...
        if total_dc > 0 and frozen_count == total_dc:
            self.checks_passed.append(
                {
                    "check": "authority_immutability",
                    "detail": f"All {total_dc} dataclasses in crypto_trust_types are frozen=True",
                },
            )
        elif total_dc > 0:
            self.violations.append(
    ...
    def _check_evidence_pack_type(self):
        ...
        if "class EvidencePack" in content and "trace_id:" in content:
            self.checks_passed.append(
                {
                    "check": "evidence_pack_type",
                    "detail": "EvidencePack found with trace_id for human escalation",
                },
            )
        else:
            self.violations.append(
                {
                    "check": "evidence_pack_type",
                    "detail": "EvidencePack missing or incomplete",
                },
            )
