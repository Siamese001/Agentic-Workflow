...
Wrapper for tests/governance/test_no_app_specific_literals_in_core.py
...
BYPASS_VAR = "GOV_LITERALS_BYPASS"
...
def run_governance_test() -> tuple[int, str, str]:
    ...
    test_path = repo_root / "tests" / "governance" / "test_no_app_specific_literals_in_core.py"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="GOV-1: Check for app-specific literals in agentic_core"
    )
    parser.add_argument(
        "--strict",
        action="store_true",
    ...
    print(f"[GOV-1] No app-specific literals in core")
    print(f"[GOV-1] Mode: {mode_reason}")
    ...
    # Determine outcome
    if test_rc == 0:
        print(f"[GOV-1] ✅ PASS: No app-specific literals found in agentic_core")
        return 0
    
    # Test found violations or had errors
    if test_rc == 2 or "AssertionError" in stderr or "Error" in stderr:
        # Hard failure (test error or strict violation)
        if is_strict:
            print(f"[GOV-1] ❌ FAIL: Violations or errors found in strict mode")
            if stdout:
                print(f"[GOV-1] STDOUT:\n{stdout}")
            if stderr:
                print(f"[GOV-1] STDERR:\n{stderr}", file=sys.stderr)
            return 2
        else:
            # Advisory mode - warn but exit 0 (non-blocking)
            print(f"[GOV-1] ⚠️  WARN: Violations or errors found ({mode_reason})")
            print(f"[GOV-1] Reason: Test found app-specific literals or failed to execute")
            if stdout:
                print(f"[GOV-1] Details:\n{stdout}")
            return 0  # Advisory = non-blocking
    
    # Test failures (rc != 0, != 2) - treat as violations
    if is_strict:
        print(f"[GOV-1] ❌ FAIL: Test failures in strict mode")
        if stdout:
            print(f"[GOV-1] STDOUT:\n{stdout}")
        if stderr:
            print(f"[GOV-1] STDERR:\n{stderr}", file=sys.stderr)
        ...
        print(f"[GOV-1] ⚠️  WARN: Test failures ({mode_reason})")
        if stdout:
            print(f"[GOV-1] Details:\n{stdout}")
        return 0  # Advisory = non-blocking
