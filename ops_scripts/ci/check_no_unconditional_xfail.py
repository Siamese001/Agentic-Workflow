...
def _is_unconditional_xfail(decorator: ast.expr) -> bool:
    ...
    # Must be a call: pytest.mark.xfail(...)
    if not isinstance(decorator, ast.Call):
        return False
    func = decorator.func
    # Check it's pytest.mark.xfail
    ...
    # If no arguments at all — unconditional
    if not decorator.args and not decorator.keywords:
        return True

    # Check for 'condition' keyword — if present and non-trivial, it's conditional
    for kw in decorator.keywords:
        if kw.arg == "condition":
            # Any condition is acceptable
            return False

    # Check if reason keyword contains env-gate keywords
    for kw in decorator.keywords:
        if kw.arg == "reason":
    ...
    # No condition, no env-gated reason → unconditional
    return True
