...
class DynamicImportScanner(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        # Check for attribute access like importlib.import_module
            ...
            if node.func.attr in {"import_module"}:
                self.violations.append(
                    DynamicImportViolation(
                        ...
                        node.lineno,
                        "DYNAMIC_IMPORT",
                        f"Direct call to {node.func.attr} detected",
                    )
                )
    ...
    def _check_dangerous_call(self, node: ast.Call, func_name: str):
        ...
        # Check if any argument contains provider module names
                ...
                if any(provider in value for provider in PROVIDER_MODULES):
                    self.violations.append(
                        DynamicImportViolation(
                            ...
                            node.lineno,
                            "DYNAMIC_IMPORT",
                            f"{func_name} with provider module '{value}'",
                        )
                    )

        # Special handling for eval/exec
        if func_name in {"eval", "exec"}:
            self.violations.append(
                DynamicImportViolation(
                    ...
                    node.lineno,
                    "CODE_EXECUTION",
                    f"Use of {func_name} - potential bypass vector",
                )
            )
    ...
    def visit_Attribute(self, node: ast.Attribute) -> Any:
        ...
        # Check for getattr on provider modules
        if isinstance(node.value, ast.Name):
            var_name = node.value.id
            if var_name in PROVIDER_MODULES and node.attr in {
                "ChatCompletion",
                "Messages",
                "GenerativeModel",
            }:
                self.violations.append(
                    DynamicImportViolation(
                        ...
                        node.lineno,
                        "PROVIDER_ACCESS",
                        f"Direct access to {var_name}.{node.attr}",
                    )
                )
            ...
            if alias.name.startswith("importlib"):
                self.violations.append(
                    DynamicImportViolation(
                        ...
                        node.lineno,
                        "DYNAMIC_IMPORT",
                        f"Import of '{alias.name}' - potential dynamic import",
                    )
                )
        ...
        if node.module and node.module in {"importlib"}:
            self.violations.append(
                DynamicImportViolation(
                    ...
                    node.lineno,
                    "DYNAMIC_IMPORT",
                    f"From import of '{node.module}' - potential dynamic import",
                )
            )
...
def scan_directory(root_dir: Path) -> list[DynamicImportViolation]:
    ...
    for py_file in root_dir.rglob("*.py"):
        ...
        if not any(
            py_file.is_relative_to(Path(p)) if Path(p).exists() else file_str.startswith(p)
            for p in first_party_prefixes
        ):
