...
class EmbeddingUsageScanner(ast.NodeVisitor):
        ...
        for alias in node.names:
            if any(primitive in alias.name for primitive in EMBEDDING_PRIMITIVES):
                self.violations.append(
                    EmbeddingUsageViolation(
                        self.file_path,
                        node.lineno,
                        "EMBEDDING_IMPORT",
                        f"Import of '{alias.name}' outside factory"
                    )
                )
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> Any:
        ...
        if node.module:
            # Check for transformers imports
                    ...
                    if any(primitive in alias.name for primitive in EMBEDDING_PRIMITIVES):
                        self.violations.append(
                            EmbeddingUsageViolation(
                                ...
                                node.lineno,
                                "EMBEDDING_IMPORT",
                                f"From import '{alias.name}' from transformers outside factory"
                            )
                        )
            
            # Check for sentence_transformers imports
            if "sentence_transformers" in node.module:
                self.violations.append(
                    EmbeddingUsageViolation(
                        ...
                        node.lineno,
                        "EMBEDDING_IMPORT",
                        f"From import from sentence_transformers outside factory"
                    )
                )
            ...
            if func_name in EMBEDDING_PRIMITIVES:
                self.violations.append(
                    EmbeddingUsageViolation(
                        ...
                        node.lineno,
                        "EMBEDDING_USAGE",
                        f"Direct usage of '{func_name}' outside factory"
                    )
                )
        
        # Check for attribute calls like OpenAI()
            ...
            if node.func.attr in {"ChatCompletion", "Embedding"}:
                self.violations.append(
                    EmbeddingUsageViolation(
                        ...
                        node.lineno,
                        "EMBEDDING_USAGE",
                        f"Direct usage of '{node.func.attr}' API outside factory"
                    )
                )
    ...
    def visit_Attribute(self, node: ast.Attribute) -> Any:
        ...
        # Check for access to embedding primitives
            ...
            if var_name in EMBEDDING_PRIMITIVES:
                self.violations.append(
                    EmbeddingUsageViolation(
                        ...
                        node.lineno,
                        "EMBEDDING_ACCESS",
                        f"Access to {var_name}.{node.attr} outside factory"
                    )
                )
...
def main():
    ...
    # First verify factory implementation
