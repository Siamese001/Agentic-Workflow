...
def get_deleted_directories(repo_root: Path, base_ref: str) -> list[str]:
    ...
    # Extract unique parent directories of deleted files
    deleted_dirs: set[str] = set()
    for f in deleted_files:
        f_posix = f.replace("\\", "/")
        # Only care about protected roots
    ...
    # Filter to directories that no longer exist on disk
...
def find_imports_referencing_directory(
    repo_root: Path,
    deleted_dir: str,
) -> list[dict]:
    ...
    """
    # Convert path to module prefix: agentic_core/L0_routing/P1_core -> agentic_core.L0_routing.P1_core
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="CI Gate: Directory Deletion Import Sweep",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
