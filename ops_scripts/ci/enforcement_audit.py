...
REPORT_PATH = Path(__file__).resolve().parents[2] / 'docs' / REPORTS_DIR / 'plans' / 'EnforcementAuditReport.json'
TAGGED_HEADER_RE = re.compile('\\|\\s*Req ID\\s*\\|\\s*Domain\\s*\\|\\s*Requirement\\s*\\|\\s*Enforcement\\s*\\|\\s*Severity\\s*\\|\\s*ENFORCEMENT_LAYERS\\s*\\|\\s*ENFORCEMENT_CLASS\\s*\\|')
    ...
    for line in text.splitlines():
        ...
        if in_table and stripped.startswith('| REQ-'):
            ...
            if len(parts) >= 7:
                ...
                requirements.append({'req_id': parts[0], 'domain': parts[1], 'requirement': parts[2], 'enforcement': parts[3], 'severity': parts[4], 'enforcement_layers': layers, 'enforcement_class': parts[6]})
...
def audit(requirements: list[dict[str, str]]) -> dict:
    ...
    for req in requirements:
        ...
        if eclass == 'EXECUTION_PATH':
            if not has_2:
                failures.append({'req_id': req['req_id'], 'reason': f'EXECUTION_PATH with <2 layers: {layers}'})
            if not has_runtime:
                failures.append({'req_id': req['req_id'], 'reason': f'EXECUTION_PATH without Runtime: {layers}'})
    ...
    return {'phase': 'W-FINAL Phase 2', 'title': 'EnforcementAuditReport', 'total_critical': total_critical, 'with_runtime': with_runtime, 'with_runtime_pct': round(with_runtime / total_critical * 100, 1) if total_critical else 0, 'with_2_layers': with_2_layers, 'with_2_layers_pct': round(with_2_layers / total_critical * 100, 1) if total_critical else 0, 'failures_by_req_id': failures, 'failure_count': len(failures), 'status': 'PASS' if len(failures) == 0 else 'FAIL'}
