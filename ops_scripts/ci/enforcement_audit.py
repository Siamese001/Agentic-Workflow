...
TAGGED_HEADER_RE = re.compile(
    r"\|\s*Req ID\s*\|\s*Domain\s*\|\s*Requirement\s*\|\s*Enforcement\s*\|\s*Severity"
    r"\s*\|\s*ENFORCEMENT_LAYERS\s*\|\s*ENFORCEMENT_CLASS\s*\|"
)
        ...
        if eclass == "EXECUTION_PATH":
            if not has_2:
                failures.append(
                    {
                        "req_id": req["req_id"],
                        "reason": f"EXECUTION_PATH with <2 layers: {layers}",
                    }
                )
            if not has_runtime:
                failures.append(
                    {
                        "req_id": req["req_id"],
                        "reason": f"EXECUTION_PATH without Runtime: {layers}",
                    }
                )
            ...
            if not has_ast_ci:
                failures.append(
                    {
                        "req_id": req["req_id"],
                        "reason": f"STRUCTURAL without AST/CI: {layers}",
                    }
                )
        else:
            failures.append(
                {
                    "req_id": req["req_id"],
                    "reason": f"Missing ENFORCEMENT_CLASS: '{eclass}'",
                }
            )
...
def main() -> int:
    ...
    # Check that corpus has enforcement metadata columns
