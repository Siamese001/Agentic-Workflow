...
Two responsibilities:
    1. Fetch the Notion Plans DB snapshot and cache it at
       ``.windsurf/state/plan_registration_cache.json`` (TTL 1h).
    2. On pre-commit, fail when a newly-staged ``.windsurf/plans/<slug>-<6hex>.md``
...
def _staged_new_plans() -> list[str]:
    """Return slugs of newly-added `.windsurf/plans/*.md` files in the index."""
    try:
    ...
    for line in result.stdout.splitlines():
        p = line.strip()
        if not p.startswith(".windsurf/plans/") or not p.endswith(".md"):
            continue
        name = Path(p).name
        import re as _re
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        "  Summary, AI Summary. See .windsurf/rules/plan-registration-enforcement.md.\n"
        "Bypass: PLAN_REGISTRATION_BYPASS=1",
        file=sys.stderr,
    )
