...
class P6EvidenceCollector:
    ...
    def _check_ssot_binding_type(self):
        ...
        p6_path = self.repo_root / "agentic_core/L0_maintenance/types/v15_p6_types.py"
            ...
            self.violations.append(
                {
                    "check": "ssot_binding_type",
                    "detail": (
                        f"class={has_class}, node_id={has_node_id}, "
                        f"blueprint_entry={has_blueprint}, resolved={has_resolved}"
                    ),
                },
            )
    ...
    def _check_layer_boundary_imports(self):
        ...
        layer_re = re.compile(r"from\s+agentic_core\.(L\d)_")
        ...
        for rel_path in V15_TYPE_FILES:
            ...
            # All V15 type files live in L0; they must not import from L1+
            try:
