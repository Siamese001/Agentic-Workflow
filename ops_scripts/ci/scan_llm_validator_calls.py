            ...
            if desc:
                ...
                self.hits.append(
                    LLMValidatorHit(
                        ...
                        func_name=fn.name,
                        call_expr=desc,
                        gap_hint=_infer_gap(rel, fn.name, desc),
                    )
                )
        self.generic_visit(node)
                ...
                if name in ML_IMPORT_MODULES:
                    hits.append(
                        LLMValidatorHit(
                            ...
                            call_expr=f"import {name}",
                            gap_hint="GAP-09/GAP-10",
                        )
                    )
    return hits
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Path to llm_validator_allowlist.json",
    )
    parser.add_argument(
        "--fail-on-new",
        action="store_true",
        default=True,
        help="Exit 1 if any un-allowlisted hits are found",
    )
    parser.add_argument(
        "--report-only",
        action="store_true",
        default=False,
        help="Print findings but exit 0 (audit mode)",
    )
    ...
    print(
        f"LLM validator scan: {len(all_hits)} total hit(s), "
        f"{len(all_hits) - len(new_hits)} allowlisted, "
        f"{len(new_hits)} new un-allowlisted"
    )
    ...
    if ml_hits:
        print(
            f"\nWARN: {len(ml_hits)} ML library import(s) in scan roots "
            f"(verify deterministic fallback exists):"
        )
        for h in ml_hits:
