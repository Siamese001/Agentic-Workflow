#!/usr/bin/env python3
"""
check_post_cursor_agent_alive.py — W5.6 post_cascade hook heartbeat gate.

Windsurf 2.0.67 has a documented bug where post_cursor_agent_response hooks
silently stop firing mid-session. The post-hook chain writes a heartbeat
row to `artifacts/cursor/post_cursor_agent_heartbeat.jsonl` on every fire.
...
Bypass: POST_CURSOR_AGENT_ALIVE_BYPASS=1
"""
...
HEARTBEAT = REPO_ROOT / "artifacts" / "windsurf" / "post_cursor_agent_heartbeat.jsonl"
VIOLATIONS_LOG = REPO_ROOT / "artifacts" / "windsurf" / "post_cursor_agent_alive_violations.jsonl"
BYPASS_LOG = REPO_ROOT / "artifacts" / "windsurf" / "post_cursor_agent_alive_bypass.jsonl"
...
def main() -> int:
    ...
    if os.environ.get("POST_CURSOR_AGENT_ALIVE_BYPASS") == "1":
        _log(BYPASS_LOG, {"reason": "env"})
        print("[check_post_cursor_agent_alive] BYPASS (env). Logged.", file=sys.stderr)
        return 0
    ...
    if not active:
        print(
            "[check_post_cursor_agent_alive] OK — repo idle (no commits in "
    ...
    if last is None:
        print(
            "[check_post_cursor_agent_alive] FAIL — repo active but no heartbeat "
            "found. Windsurf post_cursor_agent_response hook may be dark; "
    ...
    print(f"[check_post_cursor_agent_alive] PASS — heartbeat age {age_h:.1f}h", file=sys.stderr)
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"[check_post_cursor_agent_alive] script error: {exc}", file=sys.stderr)
        sys.exit(2)
