...
def reapply_replacements(fpath: Path, rel: str) -> list[str]:
    ...
    for const, literal, module in CONST_DEFS:
        ...
        for i, line in enumerate(lines):
            ...
            col = m.start()
            is_safe = any(
                pl == lineno and abs(pc - (col + 1)) <= 1
                for pl, pc in safe_pos
            )
            if not is_safe:
                continue
...
# --- minimal safe-position collector (same logic as fixer) ---
_PATH_CALLS = {"Path", "PurePath", "walk", "makedirs", "mkdir", "listdir",
               "scandir", "isdir", "isfile", "exists", "join", "abspath",
               "realpath", "relpath", "expanduser", "glob", "rglob",
               "add", "append", "discard", "remove"}
...
def main() -> None:
    ...
    # Step 2: restore each from HEAD, then re-apply replacements
    restored = 0
    for fpath in broken:
    ...
    # Step 3: verify all restored files are syntax-clean
