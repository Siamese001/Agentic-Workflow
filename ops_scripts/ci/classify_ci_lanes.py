...
def _write_github_output(path: Path, payload: dict[str, object], base_ref: str) -> None:
    selected = [str(item) for item in payload["selected_lanes"]]
    lane_hits = payload["lane_hits"]
    changed = [str(item) for item in payload["changed_files"]]
    if not isinstance(lane_hits, dict):
        raise TypeError("lane_hits must be a mapping")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        if payload["selected_lanes"]:
            print("selected:", ", ".join(str(item) for item in payload["selected_lanes"]))
    return 0
