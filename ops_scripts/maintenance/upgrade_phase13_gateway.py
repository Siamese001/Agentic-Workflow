"""
Upgrade Script - Phase 13 Gateway Enhancement

[PHASE 13]
Upgrades SovereignLLMGateway to support advanced provider configurations
(e.g., Gemini Thinking Config) passed via kwargs.
"""

from pathlib import Path

PROJECT_ROOT = Path(__file__).parent.parent.parent
GATEWAY_PATH = PROJECT_ROOT / "agentic_core" / "L2_execution" / "mcp" / "SovereignLLMGateway.py"

FULL_GATEWAY_CONTENT = '''"""
...
class SovereignLLMGateway:
    ...
    async def _call_provider(self, provider: Provider, prompt: str, model: str, temperature: float, max_tokens: int, **kwargs) -> dict:
        if provider == "openai":
    ...
    async def _call_google(self, prompt: str, model: str, temperature: float, max_tokens: int, **kwargs) -> dict:
        """Call Google Gemini API with Phase 13 generation_config support."""
...
def upgrade_gateway():
    print("--- STARTING PHASE 13 GATEWAY UPGRADE ---")

    if not GATEWAY_PATH.exists():
        print(f"[ERROR] Gateway not found at {GATEWAY_PATH}")
        return
