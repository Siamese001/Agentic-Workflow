...
class ReportMigrator:
    ...
    def __init__(
        self, project_root: Path | None = None, dry_run: bool = False, pilot_count: int | None = None
    ):
    ...
    def migrate_file(self, source: Path) -> MigrationEntry:
        ...
        entry = MigrationEntry(
            ...
            timestamp=timestamp,
            git_tracked=self.is_git_tracked(source),
        )
        if self.dry_run:
    ...
    def run_migration(self) -> MigrationManifest:
        ...
        for result in misplaced:
            ...
            status_icon = {"migrated": "✅", "dry_run": "🔍", "skipped": "⏭️", "failed": "❌"}.get(
                entry.status, "❓"
            )
    ...
    def save_manifest(self) -> Path:
        ...
        manifest_data = {
            ...
            "migrated_files": self.manifest.migrated_files,
            "failed_files": self.manifest.failed_files,
            "skipped_files": self.manifest.skipped_files,
            "rollback_available": self.manifest.rollback_available,
            ...
            json.dump(
                {
                    ...
                    "migrated_files": manifest.migrated_files,
...
def main() -> int:
    ...
    parser.add_argument(
        "--pilot", type=int, metavar="N", help="Migrate only the first N reports (for testing)"
    )
