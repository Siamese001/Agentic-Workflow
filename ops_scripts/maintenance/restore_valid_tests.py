...
def get_deleted_test_files() -> list:
    ...
    for commit in commits:
        result = subprocess.run(['git', 'diff', '--name-only', '--diff-filter=D', f'{commit}~1', commit], capture_output=True, text=True, cwd=PROJECT_ROOT)
...
def extract_tested_module(content: str) -> list:
    ...
    modules = []
    patterns = ['from (agentic_core\\.[^\\s]+) import', 'from (apps_lic\\.[^\\s]+) import', 'from (apps_rg\\.[^\\s]+) import', 'from (apps_shared\\.[^\\s]+) import', 'import (agentic_core\\.[^\\s]+)']
...
def get_file_content(commit: str, file_path: str) -> str:
    ...
    result = subprocess.run(['git', 'show', f'{commit}~1:{file_path}'], capture_output=True, text=True, cwd=PROJECT_ROOT)
...
def analyze_deleted_files():
    ...
    for file_path, commit in deleted_files:
        ...
        if existing_modules and (not missing_modules):
            ...
            unclear.append((file_path, commit, f'Mixed: exists={existing_modules}, missing={missing_modules}'))
        else:
