...
# Extra imports needed by each module (only what's missing)
EXTRA_IMPORTS: dict[str, list[str]] = {
    ...
    "artifacts": [],  # Already has re, Mapping, etc.
    "semantics": [],  # Already has Mapping, Any, Final
}
...
def add_imports_if_missing(content: str, imports: list[str]) -> str:
    ...
    # Insert after last stdlib import
    for imp in missing:
...
def append_fragment(module_name: str) -> None:
    ...
    # Check if fragment is already appended (idempotency)
    # Use first non-empty line of fragment as marker
    first_line = ""
