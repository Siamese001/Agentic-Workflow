...
COMPONENT_PATTERNS = {'DetectionSignal': ['DetectionSignal\\s*\\(', 'from.*detection_signal.*import'], 'VerificationGate': ['VerificationGate\\s*\\(', 'verification_gate'], 'HumanReviewQueue': ['HumanReviewQueue\\s*\\(', 'review_queue', 'submit_for_review'], 'recall_or_execute': ['recall_or_execute\\s*\\(', 'self\\.recall_or_execute'], 'log_audit_event': ['log_audit_event\\s*\\(', 'self\\.log_audit_event']}

def analyze_file_with_ast(file_path: Path) -> dict:
    """Analyze a Python file using AST to extract class info."""
    result = {'classes': [], 'imports': [], 'uses_detection_signal': False, 'uses_verification_gate': False, 'uses_human_review': False, 'uses_meta_learning': False, 'uses_audit_trail': False}
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                result['classes'].append({'name': node.name, 'bases': bases, 'methods': [n.name for n in node.body if isinstance(n, ast.FunctionDef)]})
...
def analyze_all_agents():
    ...
    with open(discovery_path, encoding='utf-8') as f:
        agents = json.load(f)
    results = {'total_agents': len(agents), 'by_layer': defaultdict(list), 'by_priority': defaultdict(list), 'mixin_usage': defaultdict(int), 'component_usage': {'detection_signal': 0, 'verification_gate': 0, 'human_review': 0, 'meta_learning': 0, 'audit_trail': 0}, 'agents_with_healing': [], 'agents_without_healing': [], 'detailed_analysis': []}
    for agent in agents:
        ...
        ast_result = {'uses_detection_signal': False, 'uses_verification_gate': False, 'uses_human_review': False, 'uses_meta_learning': False, 'uses_audit_trail': False}
        if full_path.exists():
        ...
        if p0_missing >= 4:
            ...
            priority = 'P3'
        analysis = {'name': name, 'path': path, 'layer': layer, 'priority': priority, 'p0_missing': p0_missing, 'p1_missing': p1_missing, 'missing_mixins': [m for m, p in missing_mixins if p == 'P0'][:5], 'missing_components': [m for m, p in missing_components][:5], 'has_healing': has_healing, 'has_memory': has_memory, 'mcp_hardened': mcp_hardened, 'uses_detection_signal': ast_result.get('uses_detection_signal', False), 'uses_verification_gate': ast_result.get('uses_verification_gate', False), 'uses_meta_learning': ast_result.get('uses_meta_learning', False)}
...
def generate_migration_json(results):
    ...
    output = {'summary': {'total_agents': results['total_agents'], 'p0_count': len(results['by_priority']['P0']), 'p1_count': len(results['by_priority']['P1']), 'p2_count': len(results['by_priority']['P2']), 'p3_count': len(results['by_priority']['P3']), 'component_gaps': results['component_usage']}, 'by_layer': {layer: list(agents) for layer, agents in results['by_layer'].items()}, 'by_priority': {p: list(agents) for p, agents in results['by_priority'].items()}}
