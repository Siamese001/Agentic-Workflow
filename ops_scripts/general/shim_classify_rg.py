...
def classify_file(py_file):
    ...
    # Classify shim quality
    if all_assigns and not non_trivial:
        shim_type = "PURE_SHIM"  # clean re-export shim
    elif not all_assigns and not non_trivial:
        shim_type = "SHIM_MISSING_ALL"  # re-exports but no __all__
    elif non_trivial:
        shim_type = "IMPURE_SHIM"  # has extra logic (For, If, With, Try, etc.)
    else:
        shim_type = "UNKNOWN"
