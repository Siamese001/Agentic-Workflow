...
TESTS_VALID_TYPES: frozenset[str] = frozenset({'unit', 'integration', 'e2e', 'fixtures', 'guardian', 'autogen'})
BASE_AGENT_PATTERN = re.compile('.*BaseAgent\\.py$')
BASE_AGENT_CANONICAL_DIR = 'agentic_core/base_agents'
FORBIDDEN_PATTERNS: list[tuple[str, str]] = [('apps_shared/test_', 'Test files must be in tests/unit/apps_shared/'), ('apps_rg/test_', 'Test files must be in tests/unit/apps_rg/'), ('apps_lic/test_', 'Test files must be in tests/unit/apps_lic/'), ('agentic_core/common/', 'Use agentic_core/utils/ instead'), ('agentic_core/utils/core_extensions/', 'Evicted per CANON_VALIDATION_REGISTRY'), ('apps_shared/[A-Z]*Agent.py', 'Agents must be in apps_shared/agents/')]
ROOT_FORBIDDEN_PATTERNS: list[str] = ['^[A-Z].*Agent\\.py$']
ROOT_ALLOWED_FILES: frozenset[str] = frozenset({'conftest.py', 'setup.py', 'AgentTechnicalStatus.py', 'NuclearAuditAgent.py'})
...
def validate_base_agent_location(file_path: str) -> tuple[bool, str]:
    ...
    if path.name in core_base_agents:
        ...
        if posix_path.startswith(BASE_AGENT_CANONICAL_DIR + '/'):
            ...
            return (False, f"[CONSTITUTIONAL VIOLATION] Core base agent '{path.name}' must reside in {BASE_AGENT_CANONICAL_DIR}/, found in: {file_path}")
    ...
    for prefix in app_prefixes:
        if path.name.startswith(prefix) and path.name.endswith('BaseAgent.py'):
            ...
            if posix_path.startswith(expected_app_dir + '/'):
                ...
                return (False, f"[CONSTITUTIONAL VIOLATION] App-specific base agent '{path.name}' must reside in {expected_app_dir}/, found in: {file_path}")
    # guardian: allow-path-string
    if posix_path.startswith(BASE_AGENT_CANONICAL_DIR + '/'):
        return (True, '')
    return (False, f"[CONSTITUTIONAL VIOLATION] Base agent '{path.name}' must reside in {BASE_AGENT_CANONICAL_DIR}/ or appropriate apps_* directory, found in: {file_path}")
    ...
    if territory.startswith('apps_'):
        ...
        if not subfolder.endswith('.py'):
            if subfolder not in APPS_VALID_SUBFOLDERS:
                return (False, f"Invalid subfolder '{subfolder}' in {territory}: {file_path}. Valid subfolders: {sorted(APPS_VALID_SUBFOLDERS)}")
    if territory == AGENTIC_CORE_DIR:
        subfolder = parts[1]
        if not subfolder.endswith('.py'):
            if subfolder not in AGENTIC_CORE_VALID_SUBFOLDERS:
                return (False, f"Invalid subfolder '{subfolder}' in agentic_core: {file_path}. Valid subfolders: {sorted(AGENTIC_CORE_VALID_SUBFOLDERS)}")
    if territory == TESTS_DIR:
        if len(parts) >= 2:
            test_type = parts[1]
            if not test_type.endswith('.py') and test_type not in TESTS_VALID_TYPES:
                if not test_type.startswith('__'):
                    return (False, f"Invalid test type '{test_type}' in tests/: {file_path}. Valid types: {sorted(TESTS_VALID_TYPES)}")
    return (True, '')
...
def validate_path(file_path: str) -> list[str]:
    ...
    """
    errors: list[str] = []
    validators = [validate_base_agent_location, validate_territory, validate_subfolder_structure, validate_forbidden_patterns]
