...
# 1. Build full import graph for SSOT dirs
import_graph = defaultdict(set)  # module -> set of modules it imports
module_to_file = {}
syntax_errors = []
...
def find_cycles(graph):
    ...
    def dfs(node, path):
        ...
        for neighbor in graph.get(node, set()):
            # Only follow intra-repo edges we know about
...
# 4. Fan-in / fan-out analysis (hotspot detection)
...
# 6. What does a dep graph tool need to provide?
# Check if any dep graph tooling already exists
existing_tools = []
