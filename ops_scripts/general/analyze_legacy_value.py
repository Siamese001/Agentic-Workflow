...
def analyze_file(file_path: Path) -> dict:
    ...
    return {'file': file_path.name, 'classes': [n.name for n in ast.walk(tree) if isinstance(n, ast.ClassDef)], 'prompts': extract_strings(tree), 'regex_patterns': extract_regex(content), 'methods': [n.name for n in ast.walk(tree) if isinstance(n, ast.FunctionDef)]}
