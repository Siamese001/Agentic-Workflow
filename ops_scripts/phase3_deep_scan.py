...
QUARANTINED_DIRS = {
    ...
    ".venv", "venv", "node_modules", "dist", "build", "scripts", "tests"
}
...
def main():
    ...
    for root, dirs, files in os.walk(ROOT_DIR):
        ...
        for f in files:
            if not is_pascal_case(f): continue
            if f.endswith(IGNORED_SUFFIXES): continue
            if f == "Config.py": continue # Already handled, but check just in case
