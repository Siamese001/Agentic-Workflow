...
BUNDLE_FILES: dict[str, callable] = {'inputs/env_snapshot.txt': _env_snapshot, 'inputs/command_log.txt': _command_log, 'artifacts/guardian_report.json': _guardian_report_placeholder, 'artifacts/review_summary.md': _review_summary_placeholder, 'artifacts/policy_pack.json': _policy_pack_placeholder, 'analysis/triage.md': _triage, 'analysis/root_cause.md': _root_cause, 'analysis/remediation.md': _remediation}
...
def main() -> int:
    ...
    exit_code, messages = create_bundle(out_dir=Path(args.out_dir), incident_id=args.incident_id, force=args.force)
    for msg in messages:
        if exit_code == 0:
            print(msg)
        else:
