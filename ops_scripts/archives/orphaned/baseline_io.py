...
Provides safe JSON baseline file operations:
  - read_json(path) — load and parse JSON
  - write_json_atomic(path, data) — write via temp + atomic replace
  - CI GUARD: refuse writes if CI=true or GITHUB_ACTIONS=true
    unless ALLOW_BASELINE_WRITES_IN_CI=1 is explicitly set
