...
W(f'1. **SovereignLockError** ({sovereign_lock_count} occurrences) — Core integrity hash mismatch blocks LocationAgent, LocationValidatorAgent, SystemArchitectAgent, RootHygieneAgent. The sovereign core hash has drifted from the expected baseline.')
W(f'2. **AtomicExecutionMixin NameError** ({atomic_mixin_count} occurrences) — HierarchyAgent fails to import because `AtomicExecutionMixin` is not defined. This blocks the standard `execute_ssot_entrypoint.py` from running at all (mandatory import).')
W(f"3. **StructuralValidatorAgent_types** ({struct_validator_count} occurrences) — ArchitectureGovernorAgent's guardian scan fails due to missing module `StructuralValidatorAgent_types`.")
W('')
    ...
    for agent_name, result in t_results.items():
        if result.get('success'):
            ...
            if isinstance(r, dict):
                ...
                if ac:
                    detail += f", renames={ac.get('renames', 0)}, territory_moves={ac.get('territory_moves', 0)}"
            else:
                detail = str(r)[:150]
...
W('')
W('These are all `[DETECT]` findings from FileClassificationAgent across all territories, showing proposed file renames:')
W('')
...
W('')
other_tags = ['TERRITORY', 'FOLDER_PURITY', 'FOLDER_SUFFIX', 'COMPOUND_SUFFIX', 'FORBIDDEN', 'PASSIVE_AGENT_NAMING', 'DUAL-TAG', 'CROSS_LAYER', 'CROSS_DOMAIN', 'EPHEMERAL', 'FAKE_CONFIG', 'MISPLACED-TEST']
...
for vtype in sorted(layer_by_type.keys()):
    ...
    if vtype == 'AGENT_LAYER_MISPLACEMENT':
        ...
        for v in items:
            ...
            W(f"| `{v['file']}` | {v.get('current_layer', '')} | {v.get('suggested_layer', '')} | {v.get('confidence', '')} | {ev} |")
...
W('')
W('1. **SovereignLock hash baseline update** — The sovereign core hash has drifted. Either update the expected hash or investigate what changed.')
W('2. **AtomicExecutionMixin resolution** — Fix the missing `AtomicExecutionMixin` in `HierarchyAgent.py` (blocks entire SSOT pipeline).')
W("3. **StructuralValidatorAgent_types module** — Stale import reference in ArchitectureGovernorAgent's guardian scan.")
W('4. **FilesystemSSOTReconcilerAgent.heal_repository** — Missing `target_territory` parameter support (TypeError on invocation).')
W('')
