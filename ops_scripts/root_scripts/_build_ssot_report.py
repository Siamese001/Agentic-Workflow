...
W(f"- **Agents failed import**: {len(agents_fail)} (`{'`, `'.join(agents_fail) if agents_fail else 'none'}`)")
...
# Extract unique error patterns from stderr
...
W(
    f"1. **SovereignLockError** ({sovereign_lock_count} occurrences) — Core integrity hash mismatch blocks "
    f"LocationAgent, LocationValidatorAgent, SystemArchitectAgent, RootHygieneAgent. "
    f"The sovereign core hash has drifted from the expected baseline.",
)
W(
    f"2. **AtomicExecutionMixin NameError** ({atomic_mixin_count} occurrences) — HierarchyAgent fails to "
    f"import because `AtomicExecutionMixin` is not defined. This blocks the standard "
    f"`execute_ssot_entrypoint.py` from running at all (mandatory import).",
)
W(
    f"3. **StructuralValidatorAgent_types** ({struct_validator_count} occurrences) — ArchitectureGovernorAgent's "
    f"guardian scan fails due to missing module `StructuralValidatorAgent_types`.",
)
W("")
    ...
    for agent_name, result in t_results.items():
        if result.get("success"):
            ...
            if isinstance(r, dict):
                ...
                # Add action counters if present
...
# ── FCA Proposed Renames (from stderr) ──
W("## FCA Proposed Renames (File Diffs)")
W("")
W(
    "These are all `[DETECT]` findings from FileClassificationAgent across all territories, "
    "showing proposed file renames:",
)
W("")

# Parse [DETECT] lines from stderr
detect_lines = re.findall(r"\[DETECT\] (.+?) -> (.+?)$", stderr, re.MULTILINE)
rename_by_territory = defaultdict(list)

# Group by territory context from stderr
current_territory = None
for line in stderr.split("\n"):
    if "heal_repository(" in line and "FileClassificationAgent" in line:
        m = re.search(r"heal_repository\((\w+)\)", line)
        if m:
            current_territory = m.group(1)
    if "[DETECT]" in line and current_territory:
        m = re.search(r"\[DETECT\] (.+?) \((\w+)\) -> (.+)", line)
        if m:
...
for tag in other_tags:
    tag_lines = re.findall(rf"\[{re.escape(tag)}\] (.+?)$", stderr, re.MULTILINE)
    if tag_lines:
...
for vtype in sorted(layer_by_type.keys()):
    ...
    if vtype == "AGENT_LAYER_MISPLACEMENT":
        ...
        for v in items:
            ...
            W(
                f"| `{v['file']}` | {v.get('current_layer', '')} | {v.get('suggested_layer', '')} | "
                f"{v.get('confidence', '')} | {ev} |",
            )
...
# ── ArchitectureGovernorAgent Results ──
...
W("")
W(
    "1. **SovereignLock hash baseline update** — The sovereign core hash has drifted. "
    "Either update the expected hash or investigate what changed.",
)
W(
    "2. **AtomicExecutionMixin resolution** — Fix the missing `AtomicExecutionMixin` in "
    "`HierarchyAgent.py` (blocks entire SSOT pipeline).",
)
W(
    "3. **StructuralValidatorAgent_types module** — Stale import reference in "
    "ArchitectureGovernorAgent's guardian scan.",
)
W(
    "4. **FilesystemSSOTReconcilerAgent.heal_repository** — Missing `target_territory` "
    "parameter support (TypeError on invocation).",
)
W("")
