...
def main():
    ...
    server_process = safe_popen([sys.executable, '-m', 'http.server', '8765'], cwd=str(dashboard_dir), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(DEFAULT_SLEEP)
    print('   ✅ Server started')
    try:
        with sync_playwright() as p:
            print('\n2. Launching browser with cache disabled...')
            browser = p.chromium.launch(headless=False, args=['--disable-cache', '--disable-application-cache', '--disable-offline-load-stale-cache'])
