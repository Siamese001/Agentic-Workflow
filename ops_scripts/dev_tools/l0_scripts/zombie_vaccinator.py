...
class ZombieVaccinator:
    ...
    """
    VACCINE_PREFIXES = ['_validate', '_reconcile', '_cleanup', '_heal', '_fix', '_repair', 'validate_', 'heal_', 'cleanup_', 'reconcile_', 'fix_', 'repair_', '_scan', '_check', '_audit', '_enforce']
    EXCLUDE_METHODS = {'_call_path', '__init__', '__post_init__', '_initialize', 'heal_repository', '_merge_results', '_log_results'}
    ...
    def vaccinate_agent(self, agent: dict[str, Any], dry_run: bool) -> dict[str, Any] | None:
        ...
        result = {'class_name': agent['class_name'], 'path': agent['path'], 'orphans': orphans, 'vaccinated': False}
        if not orphans:
    ...
    def _apply_vaccine(self, path: Path, source: str, orphans: list[str], class_name: str) -> bool:
        ...
        try:
            ...
            if insertion_idx is None:
                Logger.warning(f'    Could not find super(, **kwargs).heal_repository(, **kwargs) call in {class_name}', **kwargs)
                return False
            indented_injection = []
...
def main():
    ...
    parser.add_argument('--pilot', type=str, default=None, help='Target a single agent by class name for pilot vaccination')
    parser.add_argument('--discovery', type=str, default='agent_discovery_full.json', help='Path to discovery JSON file')
