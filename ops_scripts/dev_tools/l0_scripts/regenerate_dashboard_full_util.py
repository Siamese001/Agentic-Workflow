...
from agentic_core.L5_safety.validators.dashboard_ssot_definitions_config import FIELD_CYCLOMATIC_COMPLEXITY, FIELD_DOCUMENTED_PCT, FIELD_HAS_HEALING, FIELD_HAS_TESTS, FIELD_INVOCATION, FIELD_MCP_HARDENED, FIELD_PROPER_BASE_CLASS, FIELD_SCHEMA_STRICTNESS, FIELD_TYPED_PCT, calc_avg_cc, calc_canonical_inheritance_pct, calc_code_quality_score, calc_complexity_health, calc_documented_pct, calc_hardened_pct, calc_heal_cap_pct, calc_health_score, calc_invocation_pct, calc_schema_strictness_pct, calc_test_pct, calc_typed_pct, get_heal_cap_display, get_invocation_display, get_territory_sort_key, is_l0_territory, sort_dashboard_data
TERRITORY_MAPPING = {'Base/Base Class': 'Base/Root', 'L0 Maintenance/Base Class': 'L0 Maintenance/Base Agent', 'L1 Cognition/Base Class': 'L1 Cognition/Base Agent', 'L2 Execution/Base Class': 'L2 Execution/Base Agent', 'L3 Orchestration/Base Class': 'L3 Orchestration/Base Agent', 'L4 State/Base Class': 'L4 State/Base Agent', 'L5 Safety/Base Class': 'L5 Safety/Base Agent', 'L6_Observability/Base Class': 'L6 observability/Base Agent', 'L6_Observability/Metrics': 'L6 observability/Metrics', 'L6_Observability/Telemetry': 'L6 observability/Infrastructure', 'L1/Prompt_Governance': 'L1 Cognition/Core', 'Utils': 'Apps Shared'}
...
def build_real_agent_data(agents: list[dict], territory_mapping: dict[str, str]) -> dict[str, Any]:
    ...
    for territory, agent_list in territory_agents.items():
        ...
        for agent in agent_list:
            ...
            agent_health = calc_health_score(has_healing, has_invocation, has_tests, observable, comp_health, is_l0=is_l0_territory(territory))
            ...
            agents_data.append({'name': agent.get('class_name', 'Unknown'), 'path': agent.get('path', ''), 'rel': agent.get('path', ''), 'abs_file': str(PROJECT_ROOT / agent.get('path', '')), 'abs_class': str(PROJECT_ROOT / agent.get('path', '')), 'class_line': 1, 'has_mixin': agent.get(FIELD_HAS_HEALING, False), 'invocation': agent.get(FIELD_INVOCATION, 'No'), 'has_tests': agent.get(FIELD_HAS_TESTS, False), 'obs_summary': f"Logging: {('✓' if obs.get('logging') else '✗')} | Metrics: {('✓' if obs.get('metrics') else '✗')} | Tracing: {('✓' if obs.get('tracing') else '✗')}", 'mcp_summary': f"Shield: {('✓' if agent.get(FIELD_MCP_HARDENED) else '✗')} | @hardened: ✗ | Safe: ✓", 'typing_summary': f'Typed: {typed_pct:.0f}%', 'typed_pct': typed_pct, 'overall_typed_pct': typed_pct, 'complexity': cc, 'health': agent_health, 'healCap': has_healing, 'test': has_tests, 'complexityHealth': comp_health, 'hardened': is_hardened, 'documented': doc_pct, 'schema': schema_pct, 'base': base_pct, 'proper_base_class': has_proper_base, 'base_class_name': base_class_name, 'has_base_violation': not has_proper_base, 'quality': quality, 'loc': agent.get('loc', 50)})
        real_agent_data[territory] = {'healCap': heal_cap, 'invocation': invocation, 'hardened': hardened, 'test': test, 'complexityHealth': complexity_health, 'health': health, 'typed': typed, 'documented': documented, 'schemaStrictness': schema_strictness, 'properBase': proper_base, 'codeQuality': code_quality, 'agents': agents_data}
...
def build_dashboard_data(agents: list[dict], territory_mapping: dict[str, str]) -> list[dict]:
    ...
    total_row = {'Territory': 'TOTAL', 'Total': total_agents, 'Compliant': total_agents, 'Heal Cap %': heal_cap_pct, 'Heal Invocation %': invocation_pct, 'Invocation %': invocation_pct, 'Test %': test_pct, 'Observable %': 50.0, 'Avg CC': round(avg_cc, 1), 'Typed %': round(avg_typed, 1), 'Documented %': round(avg_documented, 1), 'Metadata %': 100.0, 'Canonical Inheritance %': proper_base_pct, 'schema Strictness %': round(avg_schema, 1), 'Complexity Health': complexity_health, 'Code Quality Score': code_quality, 'Health': health, 'Risk': 'Low' if health >= 75 else 'Medium' if health >= 50 else 'High', 'Hardened %': hardened_pct, 'Criticality': 75}
    ...
    for territory, agent_list in sorted(territory_agents.items(), key=lambda x: get_territory_sort_key(x[0])):
        ...
        t_health = calc_health_score(t_heal_cap_pct_val, t_invocation_pct_val, t_test_pct, 50.0, t_complexity_health, is_l0=is_l0)
        t_code_quality = calculate_code_quality(t_typed, t_documented, t_schema, t_proper_base_pct)
        territory_row = {'Territory': territory, 'Total': count, 'Compliant': count, 'Heal Cap %': t_heal_cap_pct, 'Heal Invocation %': t_invocation_pct, 'Invocation %': t_invocation_pct, 'Test %': t_test_pct, 'Observable %': 50.0, 'Avg CC': round(t_cc, 1), 'Typed %': round(t_typed, 1), 'Documented %': round(t_documented, 1), 'Metadata %': 100.0, 'Canonical Inheritance %': t_proper_base_pct, 'schema Strictness %': round(t_schema, 1), 'Complexity Health': t_complexity_health, 'Code Quality Score': t_code_quality, 'Health': t_health, 'Risk': 'Low' if t_health >= 75 else 'Medium' if t_health >= 50 else 'High', 'Hardened %': t_hardened_pct, 'Criticality': 75}
...
def generate_strategic_recommendations(dashboard_data: list[dict]) -> dict[str, Any]:
    ...
    """
    try:
        from agentic_core.L1_cognition.reasoning.StrategicRecommendationAgent import StrategicRecommendationAgent
        ...
        return {'review': 'Strategic analysis unavailable - module not found.', 'recommendations': []}
    # guardian: allow-silent-swallow
    except Exception as e:
        print(f'  ⚠️  StrategicRecommendationAgent failed: {e}')
        return {'review': 'Strategic analysis unavailable - agent initialization failed.', 'recommendations': []}
...
def inject_strategic_observations(content: str, recommendations: dict[str, Any]) -> str:
    ...
    for i, rec in enumerate(recommendations.get('recommendations', []), 1):
        if '<br>' in rec:
            ...
            description = ''
        recs_data.append({'priority': i, 'title': title, 'description': description, 'impact': 'HIGH' if i <= 3 else 'MEDIUM' if i <= 7 else 'LOW', 'effort': 'MEDIUM'})
    obs_data = {'macro_observations': recommendations.get('macro_observations', []), 'metric_observations': recommendations.get('metric_observations', [])}
...
def validate_html_integrity(content: str) -> tuple[bool, list[str]]:
    ...
    js_declarations = [('dashboardData', 'const\\s+dashboardData\\s*='), ('realAgentData', 'const\\s+realAgentData\\s*='), ('recommendationsData', 'const\\s+recommendationsData\\s*='), ('strategicObservationsData', 'const\\s+strategicObservationsData\\s*=')]
