...
Usage:
    python scripts/regenerate_dashboard_util.py --full        # Full regeneration (HTML + data)
    python scripts/regenerate_dashboard_util.py --data-only   # Regenerate data files only
...
def regenerate_full():
    ...
    result = subprocess.run([sys.executable, str(script_path)], cwd=str(PROJECT_ROOT), capture_output=False)
