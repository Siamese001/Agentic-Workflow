...
def main():
                ...
                for i, line in enumerate(lines):
                    ...
                    for old_stem, pattern in import_patterns.items():
                        if pattern.search(line) and 'common_utils' in line:
                            errors.append(f"[BROKEN IMPORT] {path.relative_to(ROOT_DIR)}:{i + 1} references '{old_stem}' -> '{line.strip()}'")
    ...
    for folder in SEARCH_PATHS:
        ...
        for path in root_path.rglob('*.py'):
            stem = path.stem
            if stem[0].isupper() and '_' not in stem and (not stem.isupper()) and (path.name not in ['__init__.py']):
                try:
                    content = path.read_text(encoding='utf-8')
                ...
                # guardian: allow-silent-swallow
                    ...
                    pass
    print(f'  Found {len(pascal_violations)} potential PascalCase violations (Files without matching Classes).')
