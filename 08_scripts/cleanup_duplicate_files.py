...
class DuplicateFileScanner:
    ...
    def scan(self) -> ScanReport:
        ...
        """
        print(f"🔍 Scanning {self.root_path} for duplicate files...")
...
class DuplicateFileCleaner:
    ...
    def delete_identical_duplicates(self, dry_run: bool = True) -> List[str]:
        ...
        for duplicate in self.scanner.duplicates:
            ...
            if dry_run:
                print(f"  [DRY RUN] Would delete: {duplicate.duplicate_path}")
                ...
                try:
                    ...
                    print(f"  ✗ Error deleting {duplicate.duplicate_path}: {e}")
    ...
    def delete_all_duplicates(self, dry_run: bool = True) -> List[str]:
        ...
        for duplicate in self.scanner.duplicates:
            ...
            if dry_run:
                ...
                print(f"  [DRY RUN] Would delete ({status}): {duplicate.duplicate_path}")
                deleted.append(duplicate.duplicate_path)
            else:
                try:
                    # Backup before deletion
                    ...
                    duplicate_path.unlink()
                    status = "IDENTICAL" if duplicate.identical_content else "DIFFERENT"
                    print(f"  ✓ Deleted ({status}): {duplicate.duplicate_path}")
                    deleted.append(duplicate.duplicate_path)
                except Exception as e:
                    print(f"  ✗ Error deleting {duplicate.duplicate_path}: {e}")
...
def print_report(report: ScanReport):
    ...
    print("\n" + "="*80)
    print("📊 DUPLICATE FILE SCAN REPORT")
    print("="*80)
    
    print(f"\n📈 Summary:")
    print(f"  Total duplicates found: {report.total_duplicates}")
    print(f"  Identical to original:  {report.identical_duplicates}")
    print(f"  Different from original: {report.different_duplicates}")
    print(f"  Wasted space (identical): {report.total_wasted_bytes:,} bytes ({report.total_wasted_bytes / 1024:.2f} KB)")
    
    print(f"\n🏷️  Duplicates by suffix type:")
    for suffix, count in sorted(report.duplicates_by_suffix.items(), key=lambda x: -x[1]):
        print(f"  {suffix:15s}: {count:3d} files")
    ...
    for dir_path, count in sorted_dirs:
        print(f"  {dir_path:60s}: {count:3d} files")
  ...
  python cleanup_duplicate_files.py --mode delete --confirm-delete-all
    ...
    if args.mode == 'delete':
        ...
        if args.identical_only:
            print("  Mode: Delete identical duplicates only")
            deleted = cleaner.delete_identical_duplicates(dry_run=args.dry_run)
        elif args.confirm_delete_all:
            print("  Mode: Delete ALL duplicates (including non-identical)")
            if not args.dry_run:
