...
def test_similarity_search():
    ...
    for i, query in enumerate(test_queries):
        ...
        for j, idx in enumerate(top_indices):
            ...
            meta = metadata[idx]
            print(
                f"  {j + 1}. Score: {similarity_score:.4f} | Trace: {meta['trace_id']} | Hash: {meta['content_hash'][:16]}..."
            )
...
def performance_benchmark():
    ...
    test_texts = [
        f"Test healing context {i}: system recovery and restoration" for i in range(max(batch_sizes))
    ]

    print("\n=== Performance Benchmark ===")
    print(f"Dataset size: {len(embeddings):,} embeddings")
    print("Model: BGE-M3 (CPU)")
    print("Hardware: AMD Ryzen 9 + 64GB DDR5")
    ...
    for batch_size in batch_sizes:
        ...
        print(
            f"Batch {batch_size:2d}: {batch_time:.3f}s total, {per_embedding:.3f}s each, {embeddings_per_second:.1f} embeddings/sec"
        )
