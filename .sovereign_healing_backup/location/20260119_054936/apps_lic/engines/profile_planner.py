...
class ProfilePlanner:
    ...
    def __init__(self, telemetry_bus: Optional[Any]=None) -> None:
        ...
        self.executive_keywords = {'ceo', 'cto', 'cfo', 'ciso', 'chief', 'president', 'founder', 'co-founder', 'vp', 'vice president', 'director', 'head', 'leader', 'executive'}
        self.senior_ta_keywords = {'principal', 'staff', 'senior', 'lead', 'architect', 'engineer', 'developer', 'technical', 'software', 'data', 'infrastructure', 'devops', 'security'}
        self.recruiter_keywords = {'recruiter', 'talent', 'sourcer', 'hiring', 'hr', 'human resources', 'staffing'}
        ...
        self.company_size_patterns = {'startup': ['startup', 'early stage', 'seed', 'series a', 'series b'], 'small': ['small business', 'smb', '1-50', '50-100'], 'medium': ['mid-size', 'medium', '100-500', '500-1000'], 'large': ['large', 'enterprise', '1000-5000', '5000-10000'], 'enterprise': ['fortune', 'global', 'multinational', '10000+']}
        self.industry_keywords = {'technology': ['software', 'tech', 'saas', 'fintech', 'biotech', 'ai', 'ml'], 'finance': ['bank', 'financial', 'investment', 'insurance', 'fintech'], 'healthcare': ['health', 'medical', 'pharma', 'biotech', 'hospital'], 'consulting': ['consulting', 'advisory', 'services', 'solutions'], 'manufacturing': ['manufacturing', 'production', 'industrial', 'automotive'], 'retail': ['retail', 'ecommerce', 'consumer', 'sales'], 'education': ['education', 'university', 'academic', 'learning'], 'government': ['government', 'public', 'federal', 'state', 'municipal']}

    def plan(self, *, recipient_profile: Dict[str, object], outreach_context: Dict[str, object]=None) -> ProfilePlan:
        ...
        inferred_archetype: Any = self._infer_archetype(signals)
        seniority_level: Any = self._determine_seniority(recipient_profile, signals)
        company_size: Any = self._analyze_company_size(recipient_profile, signals)
        ...
        plan: Any = ProfilePlan(inferred_archetype=final_archetype, seniority_level=final_seniority, confidence_score=confidence_score, overrides=overrides, signals=signals, company_size=company_size, industry_focus=industry_focus, decision_authority=decision_authority, metadata=metadata)
    ...
    def _extract_profile_signals(self, profile: Dict[str, object]) -> List[ProfileSignal]:
        ...
        if title:
            ...
            signals.append(ProfileSignal(signal_type='title_keywords', value=', '.join(title_keywords), confidence=0.9, metadata={'original_title': profile.get('title', '')}))
        ...
        if industry:
            ...
            if industry_classification:
                signals.append(ProfileSignal(signal_type='industry', value=industry_classification, confidence=0.8, metadata={'original_industry': profile.get('industry', '')}))
        ...
        for size, patterns in self.company_size_patterns.items():
            for pattern in patterns:
                if pattern in company:
                    signals.append(ProfileSignal(signal_type='company_size', value=size, confidence=0.7, metadata={'pattern_matched': pattern}))
                    break
        return signals
    ...
    def _analyze_experience_level(self, experience: List[Any]) -> ProfileSignal:
        ...
        if job_count > 10:
            ...
            level = 'entry'
            confidence = 0.3
        return ProfileSignal(signal_type='experience_level', value=level, confidence=confidence, metadata={'job_count': job_count})
    ...
    def _analyze_skills_profile(self, skills: List[str]) -> ProfileSignal:
        ...
        if not skills:
            return ProfileSignal(signal_type='skills_orientation', value='unknown', confidence=0.0)
        skills_text = ' '.join(skills).lower()
        technical_count = sum((1 for skill in skills if any((kw in skill.lower() for kw in ['software', 'programming', 'data', 'engineering', 'technical']))))
        business_count = sum((1 for skill in skills if any((kw in skill.lower() for kw in ['management', 'business', 'strategy', 'sales', 'marketing']))))
        ...
        return ProfileSignal(signal_type='skills_orientation', value=orientation, confidence=confidence, metadata={'technical_count': technical_count, 'business_count': business_count})
