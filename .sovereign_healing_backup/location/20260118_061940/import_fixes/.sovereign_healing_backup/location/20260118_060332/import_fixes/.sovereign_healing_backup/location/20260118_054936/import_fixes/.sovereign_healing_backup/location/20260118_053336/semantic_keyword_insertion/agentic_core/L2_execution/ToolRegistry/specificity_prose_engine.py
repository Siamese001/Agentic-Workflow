...
class SpecificityProseEngine:
    ...
    def generate_cover_letter(self, company_research: Dict[str, Any], resume_highlights: List[str], context: Dict[str, Any]) -> SpecificityProseResult:
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            cover_letter: Any = self._generate_content(company_research=company_research, resume_highlights=resume_highlights, CONTEXT=context, TEMPERATURE=self.recovery_loop.current_temperature, ATTEMPT=attempt)
    ...
    def _validate_paragraph_structure(self, paragraphs: List[str]) -> ValidationResult:
        ...
        if len(paragraphs) != self.config.paragraph_count:
            return ValidationResult(gate_id='VG_PARAGRAPH_STRUCTURE', PASSED=False, SEVERITY='BLOCK', MESSAGE=f'BLOCKED: Expected {self.config.paragraph_count} paragraphs, got {len(paragraphs)}', DETAILS={'expected': self.config.paragraph_count, 'actual': len(paragraphs)})
        VIOLATIONS = []
