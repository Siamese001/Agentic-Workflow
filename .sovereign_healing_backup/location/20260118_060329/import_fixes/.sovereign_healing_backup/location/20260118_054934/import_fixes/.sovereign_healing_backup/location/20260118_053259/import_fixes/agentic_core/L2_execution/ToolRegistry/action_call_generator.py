...
class ActionCallGenerator:
    ...
    """
    ROUTE_CHAR_LIMITS: Any = {RouteType.CONNECTION_REQ: 300, RouteType.SHORT_NEW: (360, 380), RouteType.INMAIL: 1900, RouteType.FOLLOW_UP: 1000}
    ...
    def generate_cta(self, RouteType: RouteType, message_body: str, context: Dict[str, Any]) -> CTAResult:
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            ...
            if not clarity_result.passed:
                recovery: Any = self.recovery_loop.record_failure(gate_id=clarity_result.gate_id, message=clarity_result.message, details=clarity_result.details)
                if not recovery.should_retry:
                    break
                continue
