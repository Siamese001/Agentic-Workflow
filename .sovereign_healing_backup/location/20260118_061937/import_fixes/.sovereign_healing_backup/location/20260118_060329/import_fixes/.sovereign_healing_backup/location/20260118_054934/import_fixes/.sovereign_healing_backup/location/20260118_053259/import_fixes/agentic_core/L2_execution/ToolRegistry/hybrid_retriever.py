...
class HybridRetriever:
    ...
    async def dense_search(self, query: str, top_k: int=15) -> List[RetrievalResult]:
        ...
        try:
            ...
            return [RetrievalResult(text=r.page_content, score=r.score if hasattr(r, 'score') else 0.0, source=r.metadata.get('source', 'unknown'), metadata=r.metadata) for r in results]
        except Exception as e:
