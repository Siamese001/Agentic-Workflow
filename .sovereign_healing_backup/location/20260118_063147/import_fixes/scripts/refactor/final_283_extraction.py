...
def remove_subatomic_stubs():
    ...
    for i, line in enumerate(lines):
        if line.strip().startswith('from agentic_core.utils.core_extensions.healer_mixin'):
            lines.insert(i + 1, "from agentic_core.L1_cognition.thought_engine.SubAtomicAgent import SubAtomicAgent")
            lines.insert(i + 2, "")
            import_added = True
            break
    ...
    if not import_added:
        ...
        for i, line in enumerate(lines):
            if line.strip() and not line.strip().startswith(('from', 'import', '#', '"""', "'''")):
                lines.insert(i, "from agentic_core.L1_cognition.thought_engine.SubAtomicAgent import SubAtomicAgent")
                lines.insert(i + 1, "")
                break
