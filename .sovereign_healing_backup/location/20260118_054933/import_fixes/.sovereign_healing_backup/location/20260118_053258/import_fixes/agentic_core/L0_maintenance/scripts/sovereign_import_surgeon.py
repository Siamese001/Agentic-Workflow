                ...
                if 'apps_shared' in str(file_path) and 'P1_core' in str(file_path):
                    if not line.strip().startswith('#') and self.relative_import_pattern.search(line):
                        ...
                        if suggested != line.strip():
                            violations.append(ImportViolation(str(file_path), line_num, line, 'RELATIVE_TO_ABSOLUTE', suggested))
                if not line.strip().startswith('#') and self.apps_shared_pattern.search(line):
                    suggested: Any = line.replace('from apps_shared import', 'from apps_shared.P1_core import')
                    if suggested != line:
                        violations.append(ImportViolation(str(file_path), line_num, line, 'APP_STAGING', suggested.strip()))
